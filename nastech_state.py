...
# transitive dependency via nastech_state_common). run_agent keeps its own
# predating copy — nastech_state cannot import run_agent (circular) — guarded
...
from nastech_startup_watchdog import report_startup_progress
...
from nastech_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``nastech -c``/``--resume`` to continue the most recent session in
...
#
# The ceilings above bound Nastech's SQLite descriptors, which is only ever part
...
# ~/.nastech/state.db accumulated pytest fixture rows — sessions with
...
_STATE_DB_GUARD_BYPASS_ENV = "NASTECH_STATE_DB_GUARD_BYPASS"
...
def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Nastech root for the guard.

    Deliberately avoids ``Path.home()`` / ``nastech_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``nastech_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "nastech"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "nastech"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".nastech"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``NASTECH_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Nastech test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``NASTECH_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "NASTECH_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``NASTECH_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``nastech`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Nastech home *root*.
    ...
    under ``~/.nastech/nastech-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``NASTECH_HOME`` together,
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``nastech sessions optimize`` (FTS merge + VACUUM)
    ...
    ``nastech_cli/kanban_db.py`` already bounds its WAL growth with
...
def _wal_reset_repair_hint() -> str:
    ...
    ``nastech update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Nastech-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Nastech"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Nastech-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `nastech doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``nastech_cli/managed_uv._default_live_venv``, which treats a database
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with nastech_cli.config.
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``nastech doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Nastech host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``nastech serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Nastech's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``nastech_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        st = db_path.stat()
        try:
            from nastech_cli.sqlite_safe_read import (
                LiveConnectionError,
                offline_file_access,
            )
        except ImportError:
            # Scaffold/embed installs ship nastech_state without nastech_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``nastech_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Nastech home tree (``get_nastech_home()``) — the safe repair scope:
      Nastech owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``nastech_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_nastech_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Nastech needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (nastech_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_nastech_state.py:600-632)
        ...
        probe_session_id = f"_nastech_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Nastech deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (nastech gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.nastech/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``nastech sessions optimize-storage`` on a capable host).
#
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages as JSON lines under NASTECH_HOME/sessions.
...
# (see nastech_cli/sqlite_safe_read.py for the module built around this rule).
...
#
# The registry itself lives in nastech_state_registry.py — a bounded
# module owning acquisition, generation identity, refcounting,
# retirement, and teardown.  These re-exports keep the historical
# import path (``from nastech_state import get_shared_session_db``)
# working for every call site and test that imports from here.
#
# Routing rules (see nastech_state_registry for the full lifecycle):
...
from nastech_state_registry import (  # noqa: F401  (re-export)
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship nastech_state without nastech_cli). A
    ...
    try:
        ...
        logger.debug(
            "nastech_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch nastech_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``nastech_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    if size < 0:
        return False
    from nastech_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `nastech snapshot list` / "
                "`nastech snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (nastech doctor, dashboards) ──────────
...
_NASTECH_CMDLINE_MARKERS = ("nastech_cli.main", "nastech_cli/main", "nastech serve",
                           "nastech-agent", "nastech gateway", "nastech chat")


def _looks_like_nastech(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Nastech process?
    ...
    We only flag processes that look like Nastech, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _NASTECH_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # legitimately held for multi-second stretches by sibling Nastech
    ...
    # `nastech update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `nastech snapshot list` / "
                        f"`nastech snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`nastech db optimize`),
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `nastech sessions optimize-storage` on a host "
    ...
    def _db_file_was_replaced(self) -> bool:
        ...
        if recorded_app:
            ...
            # replace. A copied Nastech DB that minted its own id is nonzero.
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Nastech process —
                        # only flag uninspectable holders that look like
                        # another Nastech instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_nastech(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from nastech_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    #
    # `optimize_fts_storage()` (the `nastech sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`nastech sessions optimize-storage`) ──
    #
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_nastech_root().resolve()
    ...
    # offline repair path behind ``nastech sessions repair-routing``.
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``nastech --resume`` into a fresh
    ...
    def sweep_orphaned_sessions(
        ...
        *,
        ...
        ``nastech serve``, desktop WS sidecar), each backend registers a row
    ...
    # of truth: nastech_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, Nastech) receive
        ...
        if repair_alternation and messages:
            # Lazy import: nastech_state already depends on agent.* (see
    ...
    def get_conversation_root(self, session_id: str) -> str:
        ...
        identifier — used for Nastech Portal ``conversation=`` usage tagging.
    ...
    def search_sessions(
        ...
        ``nastech -c``/``--resume`` so the "last" session is the last one in
    ...
    def latest_conversation_boundary(
        ...
        uses — never the key alone.  ``X-Nastech-Session-Key`` accepts any
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `nastech sessions vacuum` runs in a transient
