...
def _persistent_repair_exhausted_error(db_path: Path) -> str:
    ...
    return (
        ...
        "a backup, or salvage with `nastech sessions recover --source "
        f"{db_path} --inspect-only`, then (if it reports recoverable) "
        f"`nastech sessions recover --source {db_path} --output "
        ...
        try:
            ...
            if usage.free - need < headroom:
                reason = (
                    ...
                    f"`nastech sessions recover --source {db_path} "
                    "--inspect-only` first)."
                )
            ...
            reason = (
                ...
                f"with `nastech sessions recover --source {db_path} "
                "--inspect-only` first)."
            )
            logger.error("Refusing forensic backup of %s: %s", db_path, reason)
            return None, reason
...
_STATE_DB_CORRUPT_MSG = (
    ...
    "Stop the gateway, then run `nastech sessions recover --source <state.db> "
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `nastech sessions recover --source %s "
            "--inspect-only`.",
            self.db_path,
            exc,
            self.db_path,
            ...
            if self._conn:
                if self._db_corrupt:
                    ...
                    logger.warning(
                        ...
                        "then run `nastech sessions recover --source %s "
                        "--inspect-only`.",
                        self.db_path,
                        self._db_corrupt_reason,
                        self.db_path,
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        Schema versions:
          ...
          v3 — ``profile_name`` dimension on both tables so multiplexed
               ...
               per Nastech profile (issue #76423).
        """
