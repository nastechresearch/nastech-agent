...
export const zhHant: Translations = {
  ...
  pluginsPage: {
    ...
    headline:
      "探索、安裝、啟用並更新 Nastech 外掛（對齊 `nastech plugins` CLI）。",
    ...
    removeConfirm: "從 ~/.nastech/plugins/ 移除此外掛？",
    removeHint: "僅可移除位於 ~/.nastech/plugins 下使用者安裝的外掛。",
  ...
  skills: {
    ...
    categories: "分類",
    filters: "篩選",
    noSkills: "找不到技能。技能由 ~/.nastech/skills/ 載入",
  ...
  env: {
    ...
    hideValue: "隱藏值",
    customTitle: "自訂密鑰",
    customHint: "儲存在 .env 中、Nastech 無法識別的任意環境變數。可用於為技能、MCP 伺服器或你自己的工具注入環境變數。",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "從真實工作階段歷史中獲得的 Nastech 可收集徽章。已知尚未達成的成就會顯示為「已發現」；秘密成就在首次出現相符行為之前保持隱藏。",
      scan_subtitle:
        "正在掃描 Nastech 工作階段歷史。在歷史紀錄較多時，首次掃描可能需要 5–10 秒。",
    },
    actions: {
      rescan: "重新掃描",
    },
    stats: {
      ...
      latest: "最新",
      latest_hint_empty: "多多執行 Nastech",
      none_yet: "尚無",
    },
    state: {
      unlocked: "已解鎖",
    ...
    guide: {
      ...
      secret_header: "秘密成就",
      secret_body:
        "秘密成就會隱藏其確切觸發條件。一旦 Nastech 偵測到相關訊號，卡片便會變為「已發現」並顯示其需求。",
      scan_status_header: "掃描狀態",
      scan_status_body:
        "Nastech 正在對本機歷史進行一次掃描，之後卡片會自動出現。即使需要幾秒鐘，也並未卡住。",
