  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        ws.onclose = (ev) => {
          ...
          const message =
            ev.code === 1006 && !hasReadyFrameRef.current
              ? "Console could not connect to the dashboard server. Check that `nastech dashboard` is running, then click Reconnect."
              : ev.code === 1000
                ? "Console closed."
        ...
        writeLine(
          term,
          "\x1b[31mConsole could not connect to the dashboard server. Check that `nastech dashboard` is running, then click Reconnect.\x1b[0m",
        );
      }
    })();
