...
export const API_UNREACHABLE_MESSAGE =
  "Nastech dashboard cannot reach the Nastech service. Is `nastech dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Nastech service hit an internal error.",
  502: "The dashboard proxy could not reach the Nastech service.",
  503: "The Nastech service is not ready yet. Try again in a moment.",
  504: "The Nastech service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Nastech service answered with an unexpected error (${status}).`;
}
