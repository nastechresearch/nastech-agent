...
an unlinked SQLite DB/WAL/SHM generation.  ``nastech_state`` supplies only the
...
_NASTECH_EXECUTABLES = frozenset({"nastech", "nastech-agent", "nastech-acp"})
_NASTECH_PYTHON_MODULES = frozenset({"acp_adapter", "nastech_cli.main"})
_NASTECH_PYTHON_SCRIPTS = frozenset({"nastech_cli/main.py", "run_agent.py"})
...
def _looks_like_nastech(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Nastech execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _NASTECH_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _NASTECH_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _NASTECH_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the NASTECH_HOME root, so an absolute-path token
    containing a ``/.nastech`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Nastech home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.nastech deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.nastech" in normalized or normalized.endswith("/.nastech"):
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # NASTECH_HOME would otherwise make every holder invisible and let maintenance proceed.
