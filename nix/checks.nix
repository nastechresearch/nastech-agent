      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "nastech-check";
                homeDirectory = "/home/nastech-check";
                stateVersion = "24.11";
              };
            }
            { services.nastech-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.nastech-agent = programs;
              services.nastech-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.nastech-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.nastech-agent);
      ...
      checks = {
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.NASTECH_TEST = "1";
                environmentFiles = [ "/run/secrets/nastech-env" ];
                nastechHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "nastech" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "nastech" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.nastechAgentSetup.data;

            failures =
              lib.optional (names != [
                "nastech-agent"
                "nastech-backend"
              ]) "expected nastech-agent + nastech-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/nastech gateway" (argvOf "nastech-agent")
              ) "gateway process does not run `nastech gateway`: ${argvOf "nastech-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/nastech serve" (argvOf "nastech-backend")
              ) "backend process does not run `nastech serve`: ${argvOf "nastech-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "nastech-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/nastech-check/.nastech" (envOf n)) names
              ) "gateway and backend must share one NASTECH_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.NASTECH_HOME or null != "/home/nastech-check/.nastech"
              ) "programs.nastech-agent.enable must export NASTECH_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "nastech-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/nastech-check/.nastech/SOUL.md" activation
              ) "nastechHomeFiles must install into NASTECH_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads NASTECH_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "NASTECH_MANAGED=home-manager" (envOf n)) names
              ) "processes must report NASTECH_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "nastech-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "nastech-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/nastech-check";
            ...
            cases = [
              ...
              {
                name = "nastechHomeFiles needs no directory";
                ok = accepts { nastechHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "nastech-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one NASTECH_HOME ───────────────
        # `programs.enable` exports NASTECH_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.nastech
        # while the services use the NASTECH_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/nastech-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                nastechHome = "/home/nastech-check/.nastech-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "nastech-desktop") cfg.home.packages;
            ...
            # second Nastech runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "nastech-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.nastech-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.nastech-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one nastech-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "NASTECH_HOME" != "/home/nastech-check/.nastech-work"
              ) "the launcher must carry NASTECH_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "NASTECH_HOME")})"
              ++ lib.optional (
                setValue "NASTECH_MANAGED" != "home-manager"
              ) "the launcher must report NASTECH_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "NASTECH_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "NASTECH_DESKTOP_NASTECH" != "${lib.head agentPackages}/bin/nastech"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "NASTECH_DESKTOP_NASTECH")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "NASTECH_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "NASTECH_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "NASTECH_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "NASTECH_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "NASTECH_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "nastech-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares NASTECH_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set NASTECH_HOME" wrapper
              ) "the launcher must carry NASTECH_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "NASTECH_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "NASTECH_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? nastech-backend || cfg.launchd.agents ? nastech-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "nastech-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `nastech`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.nastech-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.nastech-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "nastech-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "nastech-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "nastech" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."nastech-agent-setup".text;

            failures =
              lib.optional (names != [
                "nastech-agent"
                "nastech-backend"
              ]) "expected nastech-agent + nastech-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/nastech gateway" (execOf "nastech-agent")
              ) "gateway unit does not run `nastech gateway`: ${execOf "nastech-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/nastech dashboard" (execOf "nastech-backend")
              ) "backend unit does not run `nastech dashboard`: ${execOf "nastech-backend"}"
              ++ lib.optional (
                units.nastech-agent.environment.NASTECH_HOME != units.nastech-backend.environment.NASTECH_HOME
              ) "gateway and backend must share one NASTECH_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/nastech/.nastech/SOUL.md" activation
              ) "nastechHomeFiles must install into NASTECH_HOME";
          ...
          in
          pkgs.runCommand "nastech-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs nastech, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.nastech-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/nastech serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec nastech directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "nastech-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "nastech-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec nastech, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/nastech dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "nastech-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        cron-worker-user-scope =
          let
            ...
            managed = configOf { };
            gateway = managed.systemd.services.nastech-agent;
            ...
            unmanaged = (configOf { createUser = false; }).systemd.services.nastech-agent;
          ...
          in
          pkgs.runCommand "nastech-cron-worker-user-scope" { } (
            if failures != [ ] then
        ...
        # $NASTECH_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                NASTECH_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "nastech-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'NASTECH_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--nastech-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "nastech-service-argv" { } ''
            set -e
