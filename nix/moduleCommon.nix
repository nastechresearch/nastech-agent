...
#
# `services.nastech-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  nastechHome, programs.nastech-agent (the CLI and
...
let
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Nastech Agent";
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Nastech configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `nastech config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $NASTECH_HOME/.env. Nastech reads that file at each start, with
          load_nastech_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."nastech/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Nastech refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Nastech reads SOUL.md and memories/ from NASTECH_HOME, so put those
          files in `nastechHomeFiles`.
      ...
      nastechHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into NASTECH_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Nastech reads SOUL.md and the memory files from NASTECH_HOME and not
          from the working directory. Declare those files here, or Nastech
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      # `nastech serve` and `nastech dashboard` are the same entry point,
      # nastech_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Nastech Desktop
      ...
      # controls an external gateway, with `nastech gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Nastech
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            NASTECH_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Nastech Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."nastech/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `nastech` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.nastech-agent.installPackage was removed. Nastech now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.nastech-agent.enable = ${lib.boolToString (value != false)};  # the nastech CLI, and NASTECH_HOME for your shells
        programs.nastech-agent.desktop.enable = true;  # the desktop application

      `services.nastech-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "nastech-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "nastech-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "nastech-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "nastech-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "nastech-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # NASTECH_MANAGED variable of the service. The value tells the shell
    ...
    let
      ...
      homeDocumentTree = mkDocumentTree {
        inherit pkgs;
        documents = cfg.nastechHomeFiles;
      };
    ...
    in
    ''
      # Directories. The service units and Nastech make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${nastechHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Nastech
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${nastechHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${nastechHome}/config.yaml
            ${run}chmod ${modes.config} ${nastechHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "nastech-managed" managedSystem} ${nastechHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${nastechHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${nastechHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${nastechHome}/auth.json"
        else
          ''
            if [ ! -e ${nastechHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${nastechHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree nastechHome cfg.nastechHomeFiles}
      ...
      ${run}find ${nastechHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "nastech-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${nastechHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/nastech"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps nastech as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "nastech-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "nastech-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "nastech-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          NASTECH_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export NASTECH_DASHBOARD_SESSION_TOKEN

          if [ -z "$NASTECH_DASHBOARD_SESSION_TOKEN" ]; then
            echo "nastech-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "nastech-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "nastech-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "nastech-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "nastech-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "nastech-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "nastech-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Nastech Agent web dashboard and desktop backend"
    else
      "Nastech Agent backend for Nastech Desktop";

  # The environment that each Nastech process needs, from either module.
  #
  # managedSystem gives the value of NASTECH_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Nastech an identity and a memory, use
          ${optionPath}.nastechHomeFiles instead. Those files go to
          NASTECH_HOME. Nastech reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of NASTECH_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
