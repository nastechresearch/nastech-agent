...
Config surface is deliberately env-only (``NASTECH_STARTUP_WATCHDOG=0`` to
disable, ``NASTECH_STARTUP_WATCHDOG_TIMEOUT_S`` to tune): the watchdog must be
...
ENV_STARTUP_WATCHDOG = "NASTECH_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "NASTECH_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module-level singleton: the arm sites (nastech_cli.main / nastech_cli.gateway
# / gateway.run.main / cli.py --gateway / scripts/nastech-gateway) and the
...
def _process_nastech_home() -> Path:
    """NASTECH_HOME for process-level diagnostic files.

    Stdlib-only replica of ``nastech_constants``' platform default — this
    module must not import application code (see module docstring). Hosted
    images always set ``NASTECH_HOME`` explicitly.
    """
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<NASTECH_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_nastech_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``NASTECH_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``NASTECH_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
