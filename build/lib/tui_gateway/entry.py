...
# from shadowing Nastech's own top-level modules.  ``nastech_bootstrap`` lives at
...
# shared owner in nastech_cli.mcp_startup — the wait/in-flight/join helpers
...
# nastech_cli.mcp_startup.start_background_mcp_discovery can actually fire —
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `NASTECH_TUI_SIDECAR_URL`, set by the dashboard's
...
# tested; override via ``NASTECH_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("NASTECH_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``NASTECH_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``nastech --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``nastech_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is not None and thread.is_alive():
        try:
            from nastech_cli.mcp_startup import _resolve_discovery_timeout
    ...
    # → nastech_cli.mcp_startup); wait on it so the first agent build still
    ...
    # session profile's NASTECH_HOME first), so a launch profile with no
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``nastech --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``nastech dashboard`` spawn theirs via
    ``nastech_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``nastech_cli.mcp_startup`` thread used by the
    ...
    try:
        from nastech_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's NASTECH_HOME override — the shared owner in
    ``nastech_cli.mcp_startup`` captures the caller's context-local override
