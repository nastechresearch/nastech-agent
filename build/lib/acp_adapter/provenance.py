...
This is an additive Nastech extension surfaced under ACP ``_meta.nastech`` so
...
compression rotates the internal Nastech head, ``build_session_provenance`` lets
...
def build_session_provenance(
    ...
    *,
    ...
    Args:
        ...
        current_nastech_session_id: The live internal Nastech DB session id
            (``state.agent.session_id``).
        previous_nastech_session_id: The internal id from before the most recent
            turn, when known. Supplied by ``prompt()`` to flag a rotation.

    Returns:
        A dict suitable for ``{"nastech": {"sessionProvenance": <dict>}}`` under
        ACP ``_meta``, or ``None`` if the session can't be read.
    """
    try:
        row = db.get_session(current_nastech_session_id)
    except Exception:
        return None
    if not row:
        return None
    ...
    root_id = current_nastech_session_id
    compression_depth = 0
    ...
    rotated = bool(
        ...
        and previous_nastech_session_id != current_nastech_session_id
    )
...
def session_provenance_meta(
    ...
    *,
    ...
    if prov is None:
        return None
    return {"nastech": {"sessionProvenance": prov}}
