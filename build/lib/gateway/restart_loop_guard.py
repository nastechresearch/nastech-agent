...
Defenses 1 and 2 (the ``_NASTECH_GATEWAY`` guard on ``nastech gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.nastech.gateway")``,
...
State lives in ``<NASTECH_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_nastech_home() / "gateway" / "restart_loop.json"
