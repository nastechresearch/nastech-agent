...
the CLI through OS package managers and signed installers, so Nastech expects
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Takes effect on the next Nastech invocation."
    )
    if not op_cfg.get("enabled"):
        console.print(
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "`nastech secrets onepassword setup` first.[/yellow]"
        )
        return 1
