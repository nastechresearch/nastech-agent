"""``nastech sync`` subcommand parser — Skill Sync.

Cloned from ``nastech_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    nastech sync status                 show gate/opt-in/head state
    nastech sync pull                   pull and materialize opted-in skills
    nastech sync push                   push opted-in skills
    nastech sync now                    reconcile: pull then push
    nastech sync enable <skill>         opt a skill into sync
    nastech sync disable <skill>        opt a skill out of sync
    nastech sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    nastech sync propose <skill>        share a skill with your organisation

Sync is INERT unless the resolved Nastech token carries the access-gate claim
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  nastech sync status            what is synced, and from where\n"
            "  nastech sync enable my-skill   include a skill in your sync\n"
            "  nastech sync now               pull, then push\n"
            "  nastech sync propose my-skill  share a skill with your team\n"
        ),
