...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``nastech login`` was removed in favor of ``nastech auth`` / ``nastech model``
    (the runtime handler in ``nastech_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``nastech login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``nastech --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``nastech login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``nastech model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `nastech auth` to manage credentials, "
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `nastech model`",
    )
    login_parser.add_argument(
