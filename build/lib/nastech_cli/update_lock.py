"""Cross-process mutual exclusion for in-flight Nastech updates.

Three different surfaces can start an update of the same install tree:

* ``nastech update`` from a terminal,
* the dashboard's Update button (``POST /api/nastech/update`` →
  ``_spawn_nastech_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``nastech-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``nastech update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <NASTECH_HOME>/.nastech-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``nastech update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Nastech is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``nastech-setup`` binary under ``~/.nastech`` is only refreshed by a full
  installer run (``copy_self_to_nastech_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Nastech is still running") on every
  GUI update forever, with no Nastech process actually running.
"""
...
MARKER_NAME = ".nastech-update-in-progress"

# Set by an orchestrating updater (the Tauri `nastech-setup --update` flow) to
# its own pid before spawning `nastech update` as a child stage. The parent
...
HANDOFF_PID_ENV = "NASTECH_UPDATE_HANDOFF_PID"
...
# to show "Nastech is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Nastech home (never the context-local profile override):
    the Rust updater resolves ``$NASTECH_HOME`` or the platform default, and the
    ...
    return get_process_nastech_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``nastech update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``nastech-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Nastech update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `nastech update` as a stage): we run under ITS claim
