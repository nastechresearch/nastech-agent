...
Nastech is exactly the topology this hits: gateway, dispatcher, dashboard,
...
class _TrackingMixin:
    ...
    _nastech_tracked_path: str | None = None

    def close(self) -> None:  # type: ignore[misc]
        with _live_lock:
            path = getattr(self, "_nastech_tracked_path", None)
            ...
            if path is not None:
                self._nastech_tracked_path = None
    ...
    **kwargs,
    ...
    with _live_lock:
        ...
        try:
            ...
            conn._nastech_tracked_path = resolved
