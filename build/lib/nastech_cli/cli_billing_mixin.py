...
This module hosts the Nastech billing/subscription methods lifted out of
``cli.py``'s ``NastechCLI`` class. ``NastechCLI`` inherits
...
Import discipline mirrors ``nastech_cli.cli_commands_mixin``:
...
class CLIBillingMixin:
    ...
    def _print_nastech_credits_block(self) -> bool:
        """Print the Nastech dollar balance block (two-bar view) when a Nastech account
        ...
        truth). Falls back to the legacy ``nastech_credits_lines`` text only when the
        model is unavailable. Agent-independent (a portal fetch gated on "a Nastech
        ...
        wall-clock-bounded; honors NASTECH_DEV_CREDITS_FIXTURE for offline testing.
        """
    ...
    def _print_usage_cta(self) -> None:
        ...
        usage read with the same nudge. Only called when a Nastech account is logged
        in (the balance block printed), since both commands are Nastech-account only.
        """
    ...
    def _show_subscription(self):
        """`/subscription` (alias `/upgrade`) — view the Nastech plan + browser hand-off.
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nastech Portal.')}")
                print("  Run `nastech portal` to log in, then /subscription.")
            return

        # Team context: no personal plan — teams run on a shared balance.
        if state.context == "team":
            print()
            _cprint(f"  𓄃 {_b('Team subscription')}")
            print(f"  {'─' * 41}")
            if state.org_name:
    ...
    def _open_url_in_browser(self, url: str) -> bool:
        ...
        use (``nastech_cli.auth``): ``webbrowser.open()`` returns ``True`` even when
    ...
    def _subscription_preview_and_confirm(self, state, tier_id, *, allow_stepup=True):
        ...
        from nastech_cli.nastech_billing import BillingError, BillingScopeRequired, post_subscription_preview
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        step_up_nastech_billing_scope directly (it opens the browser + blocks), then
        ...
        if not getattr(self, "_app", None):
            print("  Run `nastech portal` and allow Remote Spending, then re-run /subscription.")
            return
        confirm_choices = [
        ...
        try:
            from nastech_cli.auth import step_up_nastech_billing_scope

            granted = step_up_nastech_billing_scope(open_browser=True)
        ...
        try:
            from nastech_cli import nastech_billing as _nb
    ...
    def _show_billing(self, command: str = "/topup"):
        """`/topup` — Remote Spending for Nastech (one interactive modal).
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nastech Portal.')}")
                print("  Run `nastech portal` to log in, then /topup.")
            return
    ...
    def _billing_overview(self, state):
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Nastech Agent page to add funds here.",
            )
            return
    ...
    def _billing_require_admin(self, state) -> bool:
        ...
        if not state.cli_billing_enabled:
            ...
            self._billing_portal_hint(
                state,
                reason="A billing admin can turn it on from the portal's Nastech Agent page before adding funds.",
            )
            return False
        return True
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Nastech Research to charge your card."
        )
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        if isinstance(exc, BillingRemoteSpendingRevoked) or code == "remote_spending_revoked":
            ...
            print(f"  🔴 {who} Reconnect to restore — run `nastech portal` to re-authorize.")
        elif isinstance(exc, BillingSessionRevoked) or code == "session_revoked":
            print("  🔴 Your session was logged out. Run `nastech portal` to log in again.")
            ...
            print("  Remote spending is off for this account — a billing admin can turn it on from the portal's Nastech Agent page.")
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        if not getattr(self, "_app", None):
            print("  Run `nastech portal` and allow Remote Spending, then retry.")
            return
        confirm_choices = [
            ("yes", "Allow Remote Spending", "open your browser to authorize"),
            ("no", "Not now", "cancel"),
        ...
        if not (fresh.logged_in and fresh.cli_billing_enabled):
            print("  Remote Spending is allowed for this terminal, but it's still off for this org. A billing admin can turn it on from the portal's Nastech Agent page, then run /topup again.")
        ...
        from nastech_cli.nastech_billing import BillingError, post_charge
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Nastech Research to charge {card.masked} "
