...
Automates what a user otherwise does by hand: open the Nastech Portal
...
This command:
  1. Resolves a fresh Nastech Portal access token from the existing login
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      ...
      2. The ``portal_base_url`` stored on the Nastech login — this is the
...
def _register_self_hosted_client(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Nastech Portal rejected the access token (401). "
        ...
        raise RuntimeError(
            f"Could not reach Nastech Portal at {portal_base_url}: {exc.reason}"
        ) from exc
    ...
    *,
    ...
    if custom_redirect_uri:
        ...
        try:
            ...
            host = "your-host"
        print("  To require Nastech login on your registered host, run the dashboard")
        print(f"  bound publicly (it must be reachable at https://{host}) and log in")
        print("  at its /login page.")
    else:
        print("  To require Nastech login (e.g. exposing on your LAN or a public host):")
...
def cmd_dashboard_register(args) -> None:
    ...
    # 1. Resolve a fresh Nastech access token (refreshes if near expiry). Fail fast
    ...
    default_portal = "https://portal.nastechresearch.com"
