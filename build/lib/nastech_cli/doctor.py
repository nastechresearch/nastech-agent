...
def run_doctor(args):
    ...
    if _safe_which("node"):
        ...
        # `nastech acp --setup-browser` installs agent-browser into the
        # Nastech-managed node prefix, which isn't necessarily on PATH. Mirror
        # dep_ensure._has_nastech_agent_browser() so doctor and dep_ensure agree
        ...
        _managed_ab = (
            _which_in(NASTECH_HOME / "node" / "bin")
            or _which_in(NASTECH_HOME / "node")
        )
        _legacy_ab = _which_in(NASTECH_HOME / "node_modules" / ".bin")
