...
def reconcile_profile_gateways(
    *,
    ...
    actions.append(ReconcileAction(
        ...
        prior_exit=_read_prior_exit_label(nastech_home),
    ))
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    ...
    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/nastech/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/nastech/docker/main-wrapper.sh
    ...
    ``nastech`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``nastech <subcommand>``, so an
    explicit leading ``nastech`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `nastech <subcommand>`; peel an explicit nastech.
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``nastech dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted NASTECH_HOME,
...
def main() -> int:
    ...
    # bind-mounted NASTECH_HOME, both race to flock() the same s6-log lock
