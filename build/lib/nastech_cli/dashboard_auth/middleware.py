...
from nastech_cli.dashboard_auth import list_session_providers
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from nastech_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["nastech.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``nastech_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``nastech_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # a self-hosted-OIDC session hits the `nastech` provider first, which
        # tries to reach Nastech Portal's JWKS; if that's unreachable it raises,
