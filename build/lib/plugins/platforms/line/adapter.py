"""
LINE Messaging API platform adapter for Nastech Agent.
...
to Nastech Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``NASTECH_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            nastech_home = Path.home().joinpath(".nastech").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``nastech status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``nastech status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nastech setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.nastech/.env`` via ``nastech_cli.config``.
    """
    print()
    ...
    try:
        from nastech_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("nastech_cli.config not available; set LINE_* vars manually in ~/.nastech/.env")
        return
