...
Optional environment variables:
    ...
    NASTECH_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``nastech setup`` can describe it even when websockets is
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("NASTECH_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``nastech setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.nastech/.env`` via
    ``nastech_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "nastech_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.nastech/.env"
        )
        return
