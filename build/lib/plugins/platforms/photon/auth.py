...
Credential storage mirrors every other Nastech channel:

    * runtime SDK creds  -> ``~/.nastech/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.nastech/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a nastech dependency
...
# registers Nastech as its own client_id.
...
# Default name of the project Nastech provisions for the operator.
DEFAULT_PROJECT_NAME = "Nastech Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of nastech-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.nastech/auth.json`` honouring the active Nastech profile."""
    try:
        from nastech_constants import get_nastech_home
        return Path(get_nastech_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.nastech")) / "auth.json"
...
def _save_auth(data: Dict[str, Any]) -> None:
    ...
    # nastech_cli/auth.py:_save_auth_store (#19673, #21148).
    fd = os.open(
        str(tmp),
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.nastech/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.nastech/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.nastech/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: nastech_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `nastech photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `nastech photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `nastech photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `nastech photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `nastech photon setup`)"
