...
# ``model_tools`` import time, so ``nastech status``, ``nastech chat``, etc.
...
# ``nastech_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Nastech is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a nastech MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "nastech-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${NASTECH_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _nastech_home = _get_nastech_home()
        except (ModuleNotFoundError, ImportError):
            _nastech_home = _Path.home() / ".nastech"
        self._thread_count_store = _ThreadCountStore(
            _nastech_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Nastech is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("NASTECH_HOME", str(_Path.home() / ".nastech"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${NASTECH_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a nastech MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Nastech is thinking…' marker message.
        ...
        "Nastech is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Nastech is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Nastech is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Nastech is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``nastech setup``.

    The setup wizard at ``nastech_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.nastech/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. nastech-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.nastech/.env")
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nastech setup` to install Google Chat support.",
        ...
        # the "Nastech is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Nastech is thinking…' marker message "
