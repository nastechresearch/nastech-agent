"""nastech-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $NASTECH_HOME/config.yaml (profile-scoped):
  plugins:
    nastech-memory-store:
      db_path: $NASTECH_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "nastech-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, nastech_home):
        ...
        config_path = Path(nastech_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["nastech-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_nastech_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _default_db = _nastech_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $NASTECH_HOME in user-supplied paths so config values like
        # "$NASTECH_HOME/memory_store.db" or "~/.nastech/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$NASTECH_HOME", _nastech_home)
            db_path = db_path.replace("${NASTECH_HOME}", _nastech_home)
