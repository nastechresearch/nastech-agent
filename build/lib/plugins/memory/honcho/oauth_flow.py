...
``nastech://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Nastech OAuth client. IP-literal so
...
def begin_authorization(
    ...
    *,
    ...
    (``nastech-desktop`` / ``nastech-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Nastech.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local NASTECH_HOME override can't reach.
