...
def _write_env(env_path: Path, env_writes: dict[str, str]) -> None:
    ...
    if env_path.exists():
        ...
        # nastech_cli/config.py. read_text() with no encoding falls back to the
...
def _save_mem0_json(nastech_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(nastech_home) / "mem0.json"
    existing = {}
...
def _setup_platform(nastech_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "nastech-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "nastech"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.nastech/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.nastech/.env (or unset it) or Nastech will keep "
...
def _setup_selfhosted(nastech_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "nastech-user"
    )
...
def _setup_oss(nastech_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "nastech-user")
    ...
    if env_writes:
        _write_env(Path(nastech_home) / ".env", env_writes)
    _save_mem0_json(nastech_home, {"mode": "oss", "user_id": user_id, "agent_id": "nastech", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, nastech_home: str) -> str:
    ...
    if not existing:
        ...
        if env_path.exists():
            ...
            # nastech_cli/config.py; a Notepad BOM on the first line would
...
_PGVECTOR_CONTAINER = "nastech-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "nastech"
...
def _setup_oss_interactive(nastech_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], nastech_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], nastech_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'nastech-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "nastech-user")

    agent_id = input("  Agent ID [nastech]: ").strip()
    agent_id = agent_id or "nastech"
    ...
    if env_writes:
        _write_env(Path(nastech_home) / ".env", env_writes)
    _save_mem0_json(nastech_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(nastech_home: str, config: dict) -> None:
    """Entry point called by nastech memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(nastech_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(nastech_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(nastech_home, config, flags)
        return
