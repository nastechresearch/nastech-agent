...
from agent.portal_tags import get_conversation_context, nastech_portal_tags
...
class NastechProfile(ProviderProfile):
    ...
    def resolve_aux_model(self, *, vision: bool = False) -> str:
        ...
        ``/api/nastech/recommended-models`` is the authoritative, tier-aware
        source (free vs paid), so the auxiliary fast tier tracks the live
        catalog instead of a hardcoded id that 404s the day Nastech retires it.
    ...
    def build_extra_body(
        ...
        #
        # Resolve it exactly like ``nastech_portal_tags`` resolves the
    ...
    def build_api_kwargs_extras(
        ...
        **context,
        ...
        """Nastech: passes full reasoning_config, but OMITS when disabled."""
        extra_body = {}
        if supports_reasoning:
            if reasoning_config is not None:
                rc = dict(reasoning_config)
                if rc.get("enabled") is False:
                    pass  # Nastech omits reasoning when disabled
                else:
...
nastech = NastechProfile(
    name="nastech",
    aliases=("nastech-portal", "nastechresearch"),
