"""`nastech meet node ...` subcommand tree.

Wired into the existing ``nastech meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``nastech meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="nastech-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``nastech meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             nastech meet node approve <name> ws://<host>:{args.port} {token}")
        try:
