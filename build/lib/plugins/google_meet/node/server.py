...
Launched by ``nastech meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$NASTECH_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``nastech meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_nastech_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "nastech-meet-node",
    ) -> None:
        self.host = host
        self.port = port
