...
Each 429 from Nastech triggers up to 9 API calls per conversation turn
...
_STATE_FILENAME = "nastech.json"


def _state_path() -> str:
    """Return the path to the Nastech rate limit state file."""
    try:
...
def record_nastech_rate_limit(
    *,
    ...
    try:
        ...
        logger.info(
            "Nastech rate limit recorded: resets in %.0fs (at %.0f)",
...
def nastech_rate_limit_remaining() -> Optional[float]:
...
def clear_nastech_rate_limit() -> None:
...
def is_genuine_nastech_rate_limit(
    *,
    ...
    MiMo, Nastech, ...) behind one endpoint.  A 429 can mean either:
      ...
      (a) The caller's own RPM / RPH / TPM / TPH bucket on Nastech is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Nastech.

    Tripping the cross-session breaker on (b) blocks ALL Nastech requests
    (and all models, since Nastech is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Nastech emits
