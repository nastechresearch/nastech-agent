...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Nastech writes
    ...
    ``nastech_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Nastech parser: save_env_value
    ...
    from nastech_cli.config import _parse_env_value
