...
row taxonomy is inferred from the atlas shape so Nastech can render both legacy
...
# rows (1536x1872), while older Nastech/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Nastech' activity state names. They are not always identical to the
...
# Legacy Nastech/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Nastech activity names -> accepted row-name aliases in descending
