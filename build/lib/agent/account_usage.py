...
def build_nastech_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        ...
        details.append(f"Top up: {nastech_portal_topup_url(account_info)}")
...
def nastech_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered Nastech-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a Nastech account is logged in"
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_nastech_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_nastech_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_nastech_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /topup)") that build_nastech_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=nastech_portal_topup_url(account),
...
def _resolve_codex_usage_credentials(
    ...
    state, then the credential pool.  Nastech's native OAuth setup now stores
    ...
    *,
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No Codex credentials available. Run `nastech auth` to sign in with your ChatGPT account.",
        )
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                ...
                message=(
                    ...
                    "run `nastech auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
    ...
    if code == "reset":
        ...
        # persisted pool cooldowns so Nastech doesn't keep the credential
