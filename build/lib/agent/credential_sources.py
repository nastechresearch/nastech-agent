...
Nastech seeds its credential pool from many places:
    ...
    device_code   — auth.json providers.<provider> (nastech, openai-codex, ...)
...
qwen-cli, nastech device_code (partial), nastech_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"nastech"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    try:
        ...
        if env_path.exists():
            ...
            # canonical reader in nastech_cli/config.py. read_text() with no
...
def _remove_nastech_device_code(provider: str, removed) -> RemovalResult:
    """Nastech OAuth lives in auth.json providers.nastech — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `nastech auth add nastech` (or any other path that writes providers.nastech)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `nastech auth add nastech` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Nastech: single-source OAuth state with refresh tokens.
