"""Upload a Nastech session transcript to Hugging Face as an agent trace.

Nastech stores sessions in its own SQLite store (``nastech_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``nastech trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Nastech' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "nastech-traces"
_NASTECH_VERSION = "nastech-agent"
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Nastech' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Nastech conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.nastech/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `nastech trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/nastech-traces`` dataset. Returns a
