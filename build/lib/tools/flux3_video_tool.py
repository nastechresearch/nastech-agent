"""Native BFL FLUX 3 video generation tools, backed by the Nastech tool gateway.
...
there is no Nastech sign-in to call it with — never on entitlement, which is the
...
(sandbox confinement, credential guard) and delivered via the Nastech upload
protocol (presign, direct PUT to storage, ``nastech-upload:<token>`` reference).
...
from tools.managed_tool_gateway import (
    ...
    peek_nastech_access_token,
    read_nastech_access_token,
)
...
_SIGN_IN_MESSAGE = (
    "BFL video generation needs a Nastech Portal sign-in. "
    "Ask the user to run `nastech model` and sign in to Nastech, then retry."
)
...
def _warm_nastech_token() -> None:
    """Refresh the Nastech token once, before any parallel upload needs it.

    ``read_nastech_access_token`` takes no lock and, when a refresh fails, falls
    ...
    try:
        ...
        logger.debug("Nastech token warm-up failed before parallel uploads: %s", exc)
...
async def _deliver_media(value, permitted: tuple, task_id: Optional[str]):
    """Replace a local path with a ``nastech-upload:`` reference; pass URLs through.
...
def _default_directory():
    ...
    is not: an operator running NASTECH_MEDIA_DELIVERY_STRICT=1 delivers only
    ...
    if _delivers_as_an_attachment():
        try:
            ...
            return get_nastech_dir("cache/videos", "video_cache")
        except Exception:
...
def _has_nastech_credential() -> bool:
    """True when a Nastech bearer is on hand, without spending a refresh to learn it.

    Two lookups, because the transport itself has two.
    ``peek_nastech_access_token`` covers the env override and the active store's
    ...
    fallback ``resolve_nastech_access_token`` takes when the transport refreshes.
...
def check_bfl_requirements() -> bool:
    """Visible to anyone signed in to Nastech; the gateway rules on the rest.
    ...
    A sign-in is still required, because the gateway takes a Nastech bearer and
    ...
    try:
        if _endpoints() is None:
            return False
        return _has_nastech_credential()
    except Exception:
        return False
...
_MEDIA_SENTENCE = (
    "Media fields accept a local file path (uploaded automatically to Nastech-managed temporary "
