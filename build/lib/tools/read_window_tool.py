#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Nastech desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Nastech window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Nastech desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        ...
        "Nastech desktop window — what the user is working in behind this app. "
        ...
        "permission — never prompted for, noted in `note`). Other Nastech "
        "windows are skipped: the nearest non-Nastech window is reported. "
