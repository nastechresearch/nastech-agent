...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "nastechresearch.com"
...
def _read_nastech_provider_state() -> Optional[dict]:
    try:
        ...
        if not isinstance(providers, dict):
            return None
        nastech_provider = providers.get("nastech", {})
...
def peek_nastech_access_token() -> Optional[str]:
    """Cheap probe for a Nastech gateway token without triggering refresh.
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and a likely-usable Nastech token are present.
...
# managed vendor's gateway URL is pinned: adding one is a Nastech release, and
...
#
# The gateway exposes a Nastech-owned REST contract per vendor; it names the
...
def is_managed_nastech_gateway_url(
    url: object,
    gateway_builder: Optional[Callable[[str], str]] = None,
) -> bool:
    """True when ``url`` is on the Nastech tool-gateway origin this client builds.
...
def managed_gateway_auth_headers(
    ...
    Read fresh on every call rather than cached: a Nastech access token expires
    ...
    """
    if not is_managed_nastech_gateway_url(url, gateway_builder):
        return {}
...
# the tool argument carries an opaque `nastech-upload:<token>` reference instead.
#
# The protocol lives HERE rather than in a vendor tool module: the presign
# request shape, the response contract, and the `nastech-upload:` scheme are Nastech
...
def build_managed_media_uploader(
    ...
    Nastech URL, or no ``upload_path``); callers then refuse local paths with a
    ...
    3. Return ``nastech-upload:<token>`` for the tool argument. The token is
       bound to this Nastech principal and is redeemable only through the
       gateway, so it is inert anywhere else it might end up.
    """
    if not is_managed_nastech_gateway_url(server_url, gateway_builder):
        return None
    ...
    async def upload(data: bytes, mime: str) -> str:
        ...
        if not headers:
            raise RuntimeError("no Nastech credential is available for the upload")
        ...
        # is_managed_nastech_gateway_url) plus the pinned upload_path — the same
