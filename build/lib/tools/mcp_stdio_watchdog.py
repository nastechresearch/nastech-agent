...
<url>``) is spawned as a direct child of the Nastech process. Nastech's own
...
Nastech process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Nastech startup's opt-in
