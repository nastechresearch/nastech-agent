...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Nastech
...
def _snapshot_store_path() -> Path:
    return get_nastech_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.nastech"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_nastech = (
            "/.nastech"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.nastech"
        )
        archive_member = remote_nastech.lstrip("/")
