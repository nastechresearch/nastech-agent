...
Design contract (matches the Nastech "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    nastech_meta = meta.get("nastech") if isinstance(meta, dict) else None
    if not isinstance(nastech_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.nastech.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in nastech_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.nastech.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("nastech", "agent", "nastech agent") and (
        ...
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Nastech Agent' (proper caps) "
