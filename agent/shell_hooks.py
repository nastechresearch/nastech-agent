...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    # Modify tool input for pre_tool_call (Nastech-canonical):
...
from nastech_cli._subprocess_compat import IS_WINDOWS, kill_process_tree, windows_hide_flags
...
# nastech_cli.plugins.get_pre_tool_call_block_message / _get_pre_tool_call_
...
# their own plugin manager (see nastech_cli.plugins.get_plugin_manager) — can
...
def re_register_config_hooks() -> None:
    ...
    Only the idempotence keys for the *current* Nastech home are cleared —
...
def run_once(
    ...
    holding the canonical Nastech-wire-shape response — including exit-code-2
    blocking and ``fail_closed`` semantics, so what ``nastech hooks test``
