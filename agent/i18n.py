"""Lightweight i18n for Nastech' static user-facing strings (approval prompts, a few gateway replies).
...
Language resolution: explicit ``lang=`` > ``NASTECH_LANGUAGE`` > ``display.language`` > ``en``.
"""
...
def _locales_dir() -> Path:
    """Locale dir: ``NASTECH_BUNDLED_LOCALES`` (sealed packaging, e.g. Nix) if it exists, else ``<repo-root>/locales``.
...
@lru_cache(maxsize=8)
def _config_language_cached(nastech_home: str) -> str | None:
...
def _config_language() -> str | None:
    ...
    return _config_language_cached(str(get_nastech_home()))
...
def get_language() -> str:
    """Resolve the active language using env > config > default order. ``NASTECH_LANGUAGE`` is a
    ...
    try:
        env_lang = get_secret("NASTECH_LANGUAGE")
    except UnscopedSecretError:
        env_lang = os.environ.get("NASTECH_LANGUAGE")  # unscoped default-profile path: environ IS its own value
