...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'nastech memory status'. Note: "
        "systemd/gateway services do not inherit ~/.nastech/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from nastech_cli.providers import NASTECH_OVERLAYS, get_provider
        overlay = NASTECH_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from nastech_cli.providers import normalize_provider as normalize_registry_provider
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$NASTECH_HOME`` is profile-scoped; not a config key)."""
    return get_nastech_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
    ...
    elif agent.provider in {"nastech", "nastech-portal", "nastechresearch"}:
        ...
        agent.api_mode = nastech_api_mode(agent.model)
    else:
        ...
        try:
            ...
            from nastech_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    # The Nastech agent key lives ~1 h. Without the proactive refresher every agent in the process
    ...
    # Nastech model policy follows the ROUTE (the welcome host serves one model); a credential-pool
    # swap can change the route later, so ``_swap_credential`` applies the same helper again.
    from nastech_cli.anon_auth import pin_model_for_route
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, NASTECH_DEV_CREDITS) from x-nastech-credits-* headers; session
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from nastech_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    # would starve the root file handlers. Noise reduction belongs in nastech_logging.
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Optional[Dict[str, Any]]:
    ...
    if _routed_client is not None:
        from nastech_cli.providers import is_actual_route, normalize_provider
        effective_provider = getattr(_routed_client, "_nastech_aux_effective_provider", "")
    ...
    raise RuntimeError(
        f"No LLM provider configured. Run `nastech {_sel}model` to "
        f"select a provider, or run `nastech {_sel}setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("nastech_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "nastech_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched NASTECH_HOME since model_tools was imported;
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.nastech/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_nastech_home() / "sessions"
    ...
    # Process-scoped --yolo is persisted so `nastech --resume` restores the bypass
...
def _memory_provider_init_kwargs(agent, platform) -> Dict[str, Any]:
    ...
    with suppress(Exception):
        ...
        kwargs["agent_workspace"] = "nastech"
    return kwargs
        ...
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if _mp is None:
                    ...
                    from nastech_cli.memory_provider_migration import recover_at_startup
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "nastech", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, nastech, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from nastech_cli.auth import resolve_provider as resolve_auth_provider
...
def _warn_nonagentic_nastech_model(agent):
    # Nastech Nastech 3/4 are chat models, not tool-call-tuned. cli.py show_banner() already
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from nastech_cli.model_switch import _check_nastech_model_warning
        _nastech_warn = _check_nastech_model_warning(agent.model or "")
        if _nastech_warn:
            _user_msg = (
                "⚠ Nastech Research Nastech 3 & 4 models are NOT agentic — they "
            ...
            _ra().logger.warning(_nastech_warn)
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, nastech_home=str(get_nastech_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.nastech.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.nastech/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from nastech_cli.config import load_config_readonly as _load_agent_config
    ...
    _warn_nonagentic_nastech_model(agent)
