...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    not inheriting ``~/.nastech/.env``. See NastechResearch/nastech-agent#2765.
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$NASTECH_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$NASTECH_HOME`` just means the notice
...
def _refuse_checkpoint_required_on_codex_app_server(
    ...
    the default — Nastech never even initiates the compaction), so no
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .nastech.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / NASTECH_HOME
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        try:
            from nastech_cli.config import load_config_readonly as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if base_url_host_matches(effective_base, "openrouter.ai"):
                ...
                client_kwargs["default_headers"] = nastech_xai_default_headers()
    ...
    # A multiplexed gateway may enter a different NASTECH_HOME after
    ...
    try:
        from nastech_cli.config import load_config_readonly as _load_sess_cfg
    ...
    # `nastech --resume <id>` can restore the bypass (CLI resume paths read
    ...
    # management API (the historical Nastech behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Nastech' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "nastech", "off"}:
        _ra().logger.warning(
    ...
    # Nastech Nastech 3/4 are chat models, not tool-call-tuned. The interactive
