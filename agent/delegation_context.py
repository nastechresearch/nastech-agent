...
A Nastech process may itself be a Kanban dispatcher worker with NASTECH_KANBAN_* in
...
_DELEGATED_CHILD_CONTEXT: ContextVar[bool] = ContextVar("nastech_delegated_child_context", default=False)
...
_NON_DISPATCHER_OWNED_CONTEXT: ContextVar[bool] = ContextVar("nastech_non_dispatcher_owned_context", default=False)

DELEGATED_CHILD_ENV_MARKER = "NASTECH_DELEGATED_CHILD_CONTEXT"

KANBAN_ENV_KEYS: tuple[str, ...] = (
    "NASTECH_KANBAN_TASK", "NASTECH_KANBAN_RUN_ID", "NASTECH_KANBAN_CLAIM_LOCK",
    "NASTECH_KANBAN_GOAL_MODE", "NASTECH_KANBAN_GOAL_MAX_TURNS",
)
...
def is_dispatcher_owned_worker_context() -> bool:
    """The single predicate every ``NASTECH_KANBAN_*`` identity gate should use."""
    return not (is_delegated_child_process_context() or _NON_DISPATCHER_OWNED_CONTEXT.get())


def owned_kanban_task() -> str:
    """The board task this execution OWNS: ``NASTECH_KANBAN_TASK`` for the dispatcher-owned
    ...
    if not is_dispatcher_owned_worker_context():
        return ""
    return (os.environ.get("NASTECH_KANBAN_TASK") or "").strip()
...
def _fenced_kanban_root() -> str:
    ...
    try:
        from nastech_cli.kanban_db import kanban_home
...
def scrub_kanban_env(env: Mapping[str, str] | MutableMapping[str, str]) -> dict[str, str]:
    ...
    against a temp ``NASTECH_HOME`` got a silently read-only board there. An inherited
    path-valued marker is kept (a grandchild that moved NASTECH_HOME must not re-fence
...
def kanban_path_is_fenced(path: "os.PathLike[str] | str") -> bool:
    ...
    descendant only when *path* is the dispatcher-pinned ``NASTECH_KANBAN_DB`` or lies under the
    ...
    pinned = os.environ.get("NASTECH_KANBAN_DB", "").strip()
...
def delegated_child_subprocess_env(
    ...
    """
    if not (is_delegated_child_process_context() or os.environ.get("NASTECH_KANBAN_TASK")
            or (env and (env.get("NASTECH_KANBAN_TASK") or env.get(DELEGATED_CHILD_ENV_MARKER)))):
