...
_GITHUB_MODELS_HINT = (
    ...
    "      request at ~8K tokens. Nastech' system prompt + tool schemas baseline",
    "      exceeds that floor, so this endpoint cannot run an agentic loop.",
    "      Use the `copilot` provider with a Copilot subscription token (`nastech",
...
def _recover_context_length(st: _Recovery, _retry: TurnRetryState, error_msg: str) -> OverflowVerdict:
    ...
    # Nastech knows for this model (after adopting any limit the server reported), so compressing
