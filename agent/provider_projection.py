"""Fold an agent-as-provider's own activity back into Nastech' turn state.

Most providers are models: they ask Nastech to run a tool and Nastech runs it, so
...
time Nastech sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Nastech would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Nastech tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``nastech_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``nastech_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "nastech_projected_messages", None)
    ...
    raw_iters = getattr(response, "nastech_provider_tool_iterations", 0)
    try:
