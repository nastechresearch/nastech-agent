...
work must never come back as pending ``tool_calls`` (Nastech would re-run it) — but
...
``reasoning``. The client hands back ``nastech_projected_messages`` (completed
assistant/tool rows) and ``nastech_provider_tool_iterations`` on the completion
...
def splice_provider_projection(agent: Any, response: Any, messages: list[dict[str, Any]]) -> int:
    ...
    try:
        iterations = int(getattr(response, "nastech_provider_tool_iterations", 0) or 0)
