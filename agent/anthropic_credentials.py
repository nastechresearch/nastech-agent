...
``ANTHROPIC_API_KEY``, Nastech-owned OAuth grants in the ``auth.json`` credential
pool, then ``~/.claude/.credentials.json`` / macOS Keychain as a borrowed fallback.
``~/.nastech/.anthropic_oauth.json`` (Nastech PKCE) and
...
# Claude Code's macOS Keychain entry (generic password). Nastech reads it
...
_SPENT_ROTATION_SIDECAR_COMMENT = (
    ...
    "consumed server-side but never durably committed. Written by Nastech so sibling "
...
def _spent_rotation_sidecar_path(source_path: Path) -> Path:
    return source_path.with_name(source_path.name + ".nastech-spent-rotations.json")
...
_SINGLETON_SOURCE_PATHS = {
    "claude_code": lambda: claude_code_credentials_path(), "nastech_pkce": lambda: _get_nastech_oauth_file()
}
...
def claude_code_credentials_path() -> Path:
    ...
    like the Claude CLI itself (blank = unset, as in ``nastech_cli.foreign_sessions``). The supported opt-out of
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        from nastech_cli.auth import AUTH_LOCK_TIMEOUT_SECONDS, _auth_store_lock, env_float
        refresh_timeout_seconds = env_float("NASTECH_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20)
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout_seconds, target_path=cred_path):
            ...
            if is_rotation_consumed_uncommitted(refresh_token, source_path=cred_path):
                logger.debug("Refresh token was already consumed by an uncommitted rotation "
                             "- refusing to replay it; run 'nastech auth add anthropic'")
                return None
            ...
            try:
                ...
                if is_terminal_anthropic_refresh_error(e):
                    ...
                    logger.warning(
                        "Claude Code OAuth refresh token is terminally invalid (%s); Nastech cannot use this "
                        "login. Run 'nastech auth add anthropic' to give Nastech its own login.", e)
                else:
            ...
            try:
                ...
                logger.error(
                    ...
                    "run 'nastech auth add anthropic' to give Nastech its own login",
                    cred_path, e,
                )
...
def _mirror_claude_code_credentials_to_keychain(
    ...
    """After a Nastech refresh, write the rotated pair into the Claude Code Keychain item too (#98334).
...
def _resolve_claude_code_token_from_credentials(creds: Optional[Dict[str, Any]] = None) -> Optional[str]:
    ...
    if not refreshed:
        logger.debug("Token refresh failed — run 'nastech auth add anthropic' to give Nastech its own login")
    return refreshed or None


def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    """Prefer refreshable Claude Code creds over a static env OAuth token: Nastech historically persisted setup tokens
...
def _resolve_anthropic_pool_token(*, skip_borrowed: bool = False) -> Optional[str]:
    ...
    refresh=False`` (never ``select()``) so diagnostic call sites (account_usage, ``nastech models``) never mutate
...
# ── Nastech-native PKCE OAuth flow (~/.nastech/.anthropic_oauth.json); mirrors Claude Code / pi-ai / OpenCode ──


def _get_nastech_oauth_file() -> Path:
    return get_nastech_home() / ".anthropic_oauth.json"
...
def run_nastech_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    print("\n".join([
        "", "Authorize Nastech with your Claude Pro/Max subscription.", "",
...
def read_nastech_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    data = _load_json_if_exists(_get_nastech_oauth_file(), "Nastech OAuth credentials")
    return data if data is not None and data.get("accessToken") else None


def _write_nastech_oauth_credentials(
    access_token: str, refresh_token: Optional[str], expires_at_ms: Optional[int],
) -> None:
    """Commit refreshed nastech_pkce tokens to ``<NASTECH_HOME>/.anthropic_oauth.json`` (``CredentialPersistError``
