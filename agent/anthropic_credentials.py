...
3. ``~/.nastech/.anthropic_oauth.json`` (Nastech PKCE login)
...
def _getenv(name: str, default: str = "") -> str:
    ...
    read) when on. Mirrors the same wrapper in nastech_cli/runtime_provider.py.
    """
...
class CredentialPersistError(RuntimeError):
    ...
    ``~/.nastech/.anthropic_oauth.json`` for ``nastech_pkce``).
...
#     authority boundary of ``claude_code``/``nastech_pkce`` is the shared
#     singleton file, which other Nastech processes/profiles read with fresh
...
def spent_rotation_source_path(source: Any) -> Optional[Path]:
    ...
    if source == "nastech_pkce":
        return _get_nastech_oauth_file()
    return None
        ...
        payload = json.dumps(
            {
                ...
                "comment": (
                    ...
                    "never durably committed. Written by Nastech so sibling "
...
def claude_code_credentials_path() -> Path:
    ...
    This file is not profile-owned: every Nastech profile's credential pool
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Nastech notices an expired token, Claude Code may have already rotated it.
    ...
    try:
        ...
        refresh_timeout_seconds = env_float(
            "NASTECH_ANTHROPIC_REFRESH_TIMEOUT_SECONDS", 20
        )
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``nastech models``) never mutates ``~/.nastech/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      ...
      5. Anthropic credential_pool OAuth entry (~/.nastech/auth.json)
    ...
    # 5. Nastech credential_pool OAuth entry.
...
def _root_nastech_oauth_file() -> Optional[Path]:
    ...
    try:
        ...
        if root.resolve(strict=False) == get_nastech_home().resolve(strict=False):
            return None
...
def _write_nastech_oauth_credentials(
    ...
    *,
    ...
    """Write refreshed nastech_pkce tokens back to ~/.nastech/.anthropic_oauth.json.
    ...
    its own NASTECH_HOME (#100339).

    Without this, a successful pool-level refresh of a ``nastech_pkce``-sourced
    ...
    """
    oauth_file = target if target is not None else _get_nastech_oauth_file()
    try:
        oauth_data = {
            "accessToken": access_token,
            "refreshToken": refresh_token,
        ...
        logger.error(
            "Failed to write refreshed Nastech OAuth credentials to %s: %s", oauth_file, e
        )
