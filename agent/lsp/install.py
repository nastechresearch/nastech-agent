...
Installs go to a Nastech-owned staging dir, ``<NASTECH_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``nastech lsp status`` reports it).
...
# ``bin`` in ``<NASTECH_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `nastech lsp status` reports its presence.
...
def _link_into_bin(target: Path) -> str:
    ...
    link = nastech_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    # Managed npm first: $NASTECH_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Nastech installed.
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``nastech lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "nastech_lsp_bin_dir"]
