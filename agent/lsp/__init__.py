...
# Routed multiplex profiles (NASTECH_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``nastech chat`` exit leaks pyright processes for a few seconds
