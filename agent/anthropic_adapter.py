...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry's Anthropic-style endpoint, Palantir Foundry's LLM proxy, and Nastech
    Portal's Messages route follow this pattern.
    """
    if _is_nastech_portal_endpoint(base_url):
        return True
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to nastech's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to nastech's outer conversation
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://nastech-agent.nastechresearch.com")
    ...
    # from ``ANTHROPIC_API_KEY`` (Nastech loads that into the process env from
    # ``~/.nastech/.env``). The result is dual auth —
...
def _manage_thinking_signatures(
    ...
    Nastech Portal's ``/v1/messages`` route is the exception among third-party
    ...
    _is_third_party = (
        ...
        and not _is_nastech_portal_endpoint(base_url)
    )
...
def build_anthropic_kwargs(
    ...
    # Nastech Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``);
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                ...
                text = text.replace("Nastech agent", "Claude Code")
                text = text.replace("nastech-agent", "claude-code")
                text = text.replace("Nastech Research", "Anthropic")
                block["text"] = text
        ...
        #      a) bare Nastech-native tools (``read_file``)  -> ``mcp__read_file``
...
def create_anthropic_message(
    ...
    *,
    ...
    parsed ``Message`` drops — Nastech Portal's ``x-nastech-credits-*`` balance family
