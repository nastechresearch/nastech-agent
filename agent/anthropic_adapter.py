"""Anthropic Messages API adapter: client construction + the Messages call for Nastech's
...
from agent.anthropic_endpoints import (
    ...
    _is_minimax_anthropic_endpoint, _is_nastech_portal_endpoint, _is_opencode_endpoint,
...
# Nastech effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Nastech tool schemas/prose as a
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    nastech's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str]):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.nastech/.env) and then sends dual
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to nastech's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Nastech tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SYSTEM_REPLACEMENTS = (
    ("Nastech Agent", "Claude Code"), ("Nastech agent", "Claude Code"),
    ("nastech-agent", "claude-code"), ("Nastech Research", "Anthropic"),
)
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Nastech shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # Nastech Portal routes on its own catalog ids (``anthropic/claude-opus-4.8``); normalizing would
    # make the model unresolvable there (prefix AND dots kept).
    if not _is_nastech_portal_endpoint(base_url):
    ...
    # which silently hides reasoning text that Nastech surfaces in its CLI. We request "summarized" so the
...
def create_anthropic_message(
    ...
    ``on_response(httpx_response)`` exposes headers the parsed Message drops (Nastech Portal's
    ``x-nastech-credits-*`` balance family)."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nastech_home': ('nastech_constants', 'get_nastech_home'),
    ...
    'read_nastech_oauth_credentials': ('agent.anthropic_credentials', 'read_nastech_oauth_credentials'),
    ...
    'run_nastech_oauth_login_pure': ('agent.anthropic_credentials', 'run_nastech_oauth_login_pure'),
