...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``nastech-tools`` server already overflows 64 chars,
...
# output items.  Nastech injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Nastech-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Nastech'
...
def _normalize_codex_response(
    ...
    *,
    ...
    if tool_calls:
        ...
        # See https://github.com/NastechResearch/nastech-agent/issues/64434
