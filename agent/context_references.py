...
from nastech_cli._subprocess_compat import IS_WINDOWS, harden_git_argv, noninteractive_git_env, windows_hide_flags
...
def preprocess_context_references(
    ...
    # NASTECH_HOME override is lost and the credential-path guard checks the launch profile's .env.
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    home, nastech_home = Path(os.path.expanduser("~")).resolve(), get_nastech_home().resolve()
    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES} | {nastech_home / ".env"}
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS] + [nastech_home / rel for rel in _SENSITIVE_NASTECH_DIRS]
