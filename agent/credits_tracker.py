"""Nastech credits: parse ``x-nastech-credits-*`` / ``x-nastech-tool-pool-*`` response
...
@dataclass
class CreditsState:
    """Credits state parsed from x-nastech-credits-* response headers."""
...
def _is_nastech_welcome_route(base_url: str) -> bool:
    """True when *base_url* is the Nastech welcome host, which serves only the free tier. Local data only;
    False wherever the free tier is not built in. The host is the evidence, not the model name: the paid
    inference host can serve ``nastech/welcome`` to a named account, and that account's depletion is real."""
    try:
        from nastech_cli.anon_auth import route_is_welcome_host
    except ImportError:
        return False
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """True when *model* is a Nastech free-tier model, using ONLY local data: (1) ``:free`` suffix — canonical
    Nastech free SKU marker; (2) ``stealth/`` prefix — stealth-preview SKUs are free without the suffix
    (naming-convention trust: a PAID ``stealth/`` model would wrongly suppress the banner); (3) a PEEK into
    ``nastech_cli.models``' pricing cache (filled by the model picker; a miss never fetches). Fail-open to
    ...
    if not base_url:
        return False
    # (4) the Nastech free tier: the welcome host serves only the free tier. A free-tier identity carries $0
    ...
    if _is_nastech_welcome_route(base_url):
        return True
    try:
        from nastech_cli.models import _is_model_free
        from nastech_cli.models_pricing import peek_cached_pricing
...
def evaluate_credits_notices(state: CreditsState, latch: dict, *, model_is_free: bool = False) -> tuple[list[AgentNotice], list[str]]:
    ...
    if target_band != latch.get("usage_band"):
        ...
        if target_band is not None:
            # Absolute dollars used (a bare "N%" is only meaningful against a Nastech cap): cap − remaining,
...
# Header name = ``x-nastech-credits-<field>`` (``x-nastech-<field>`` for tool_pool_*), underscores → dashes.
...
def _header_name(field: str) -> str:
    return "x-nastech-" + ("" if field.startswith("tool_pool_") else "credits-") + field.replace("_", "-")
...
def parse_credits_headers(headers: Mapping[str, str], provider: str = "") -> Optional[CreditsState]:
    ...
    try:
        ...
        # version header is absent — the hot path for non-Nastech providers.
        if not any(k.lower() == "x-nastech-credits-version" for k in headers):
            return None
        lowered = {k.lower(): v for k, v in headers.items()}
        version_val = _safe_int(lowered.get("x-nastech-credits-version"))
        if version_val is _SENTINEL:
            return None
        if version_val != 1:
            if version_val > 1 and not _version_warning_emitted:
                _version_warning_emitted = True
                logger.warning("credits header version %d unsupported, ignoring — update Nastech", version_val)
            return None
        fields: dict[str, Any] = {
            name: _parse_field(kind, lowered.get(_header_name(name)), *default) for name, kind, *default in _HEADER_FIELDS
        }
        lim_micros_raw = lowered.get("x-nastech-credits-subscription-limit-micros")
        lim_usd_raw = lowered.get("x-nastech-credits-subscription-limit-usd")
        ...
        if _SENTINEL in fields.values() or denominator_kind not in _VALID_DENOMINATOR_KINDS:
            return None
        disabled_reason = lowered.get("x-nastech-credits-disabled-reason")  # None if absent (omitted when null)
...
# ── Dev fixtures (NASTECH_DEV_CREDITS_FIXTURE): throwaway scaffolding to trigger any notice state
...
def dev_fixture_credits_state() -> Optional[CreditsState]:
    """Fixture CreditsState for NASTECH_DEV_CREDITS_FIXTURE, or None (unknown name / unset). Prod-leak guard:
    applies ONLY when NASTECH_DEV_CREDITS is also on, so a stray fixture env var never surfaces fabricated balances."""
    name = os.environ.get("NASTECH_DEV_CREDITS_FIXTURE", "").strip()
    if not name or not is_truthy_value(os.environ.get("NASTECH_DEV_CREDITS")):
        return None
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a NastechPortalAccountInfo into a header-shaped CreditsState for the seed. Float account dollars →
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    try:
        if getattr(agent, "provider", "") != "nastech" or getattr(agent, "_credits_state", None) is not None:
            return False
        try:
