"""Credits tracking for Nastech inference API responses.
...
def is_free_tier_model(model: str, base_url: str = "") -> bool:
    """Return True when *model* is a Nastech free-tier model, using ONLY local data.

    Two signals, both zero-network:

    1. The ``:free`` suffix — the canonical Nastech free SKU marker (e.g.
       ``nvidia/nemotron-3-ultra:free``). Free by construction on the API side
       (spend is forced to 0 for ``:free`` ids).
    2. The ``stealth/`` prefix — Nastech stealth-preview SKUs (e.g.
    ...
    3. A peek into the in-process pricing cache in ``nastech_cli.models``
    ...
    if not base_url:
        return False
    try:
        from nastech_cli.models import _is_model_free, peek_cached_pricing
...
def evaluate_credits_notices(
    ...
    *,
    ...
    ``model_is_free``: True when the session's active model is a Nastech free-tier
    ...
    if target_band != shown_band:
        ...
        if target_band is not None:
            ...
            # only meaningful against a Nastech subscription cap (no cap → never fires),
...
def parse_credits_headers(
    ...
    try:
        ...
        # sentinel header is absent (the common case for non-Nastech providers, on
...
def _credits_state_from_account(info) -> Optional[CreditsState]:
    """Map a NastechPortalAccountInfo into a header-shaped CreditsState for the seed.
...
def seed_credits_at_session_start(agent) -> bool:
    ...
    Returns True if it seeded this call, False otherwise (not nastech / already seeded /
    fail-open error). Never raises — credits must never block session startup.
    """
    try:
        if getattr(agent, "provider", "") != "nastech":
            return False
