...
# `export NASTECH_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in nastech_cli/main.py, gateway/run.py, and cli.py) or `NASTECH_REDACT_SECRETS=false` in
# ~/.nastech/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
...
# served under a NASTECH_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in nastech_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    if get_nastech_home_override() is None:
    ...
    try:
        ...
        raw = scope.get("NASTECH_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``nastech config`` / ``status`` / ``dump``):
...
# a shell rc/profile, Nastech' own ``config.yaml`` where ``nastech mcp add --env`` writes
...
_NASTECH_HOME_PREFIXES = ("$NASTECH_HOME/", "${NASTECH_HOME}/")
# ``$HOME/.nastech/config.yaml`` keeps the ``.nastech`` segment, so stripping the prefix is
...
def _is_under_nastech_home(path: str) -> bool:
    """True when an absolute ``config.yaml`` path sits under the active Nastech home or root.

    The default home's basename is an installation detail — ``.nastech`` on POSIX, ``nastech``
    under ``AppData/Local`` on Windows — and a resolved path never spells ``$NASTECH_HOME``,
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.nastech`` directory, ``$NASTECH_HOME``, or the resolved Nastech home (never arbitrary
    YAML). The resolved-home arm is what covers native Windows, where the home directory
    is ``%LOCALAPPDATA%\\nastech`` and carries no ``.nastech`` segment."""
    path = arg.strip("\"'").replace("\\", "/")
    nastech_home = False
    for prefix in _NASTECH_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    # ``config.yaml`` plus the ``config.yaml.good.<stamp>`` / ``.corrupt.<stamp>`` copies Nastech
    ...
    if parts[-1] != "config.yaml" and not parts[-1].startswith(("config.yaml.good.", "config.yaml.corrupt.")):
        return False
    return nastech_home or ".nastech" in parts[:-1] or _is_under_nastech_home(path)
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, Nastech ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
