...
class RateLimitCreditsMixin:
    ...
    def _capture_nastech_model_switch(self, http_response: Any) -> None:
        """Record the Nastech gateway's ``x-nastech-model-switch`` header (a named account asked for the
        free tier's model; the gateway served its backing model and named it). Applied between
        calls by ``nastech_cli.anon_auth.apply_model_switch``. Fail-open."""
        headers = _response_headers(http_response)
        if not headers:
            return
        try:
            from nastech_cli.anon_auth import note_model_switch
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        depletion-notice bug cannot vanish silently. NASTECH_DEV_CREDITS_FIXTURE injects a chosen state instead.
        """
        try:
        ...
        if not headers:
            return
        dev = is_truthy_value(os.environ.get("NASTECH_DEV_CREDITS"))
        ...
        if state is None:
            if dev:
                logger.info("credits ▸ response had no valid x-nastech-credits-* headers "
                            "(miss — producer off / non-Nastech path / >TTL stale)")
            return

        _adopt_credits_state(self, state)
        if dev:
            # NASTECH_DEV_CREDITS streams each capture to agent.log (`nastech logs -f`, grep 'credits ▸').
