...
# who installed plain `nastech-agent` and only later selected a Gemini model.
try:
...
def has_explicit_vertex_config() -> bool:
    """True only when the user deliberately pointed Nastech at Vertex.
    ...
    Nastech up for Vertex would suddenly see it (and could spend against those
    credentials). So this checks only Nastech-scoped signals:
      ...
      * a resolvable ``VERTEX_CREDENTIALS_PATH`` service-account file
        (the Nastech-specific path var — NOT ``GOOGLE_APPLICATION_CREDENTIALS``).
    """
    if _resolve_project_override():
        return True
