...
from nastech_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def _nastech_snapshot(windows: list, details: list, tail: list, *, source: str, plan: Optional[str] = None) -> Optional[AccountUsageSnapshot]:
    """Nastech snapshot with *tail* lines appended, or None when there is nothing to show."""
    if not windows and not details:
        return None
    return _snapshot("nastech", source, windows, details + tail, title="Nastech credits", plan=plan)


def build_nastech_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """NastechPortalAccountInfo → /usage snapshot: dollar magnitudes + renewal date + portal CTA, plus a ``% used``
    gauge when the portal supplies ``monthly_credits``. Fail-open → None."""
    try:
        from nastech_cli.nastech_account import nastech_portal_topup_url
        ...
        return _nastech_snapshot(windows, details, [f"Top up: {nastech_portal_topup_url(account_info)}", "(or run /topup)"],
                              source="portal-account", plan=getattr(sub, "plan", None) if sub is not None else None)
    except (AttributeError, TypeError):
        return None


def _nastech_logged_in() -> bool:
    """Cheap local auth-state check: a Nastech access token is present. Fail-open False."""
    try:
        from nastech_cli.auth import get_provider_auth_state
        tok = (get_provider_auth_state("nastech") or {}).get("access_token")
...
def _fetch_portal_account(timeout: float):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        return pool.submit(context.run, get_nastech_portal_account_info, force_fresh=True).result(timeout=timeout)


def nastech_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Rendered Nastech-credits /usage lines, or [] when there's nothing to show. Independent of any live agent
    (logged-in gate, then a bounded portal fetch); shared by CLI ``_show_usage`` and the TUI ``session.usage`` RPC.
    Fail-open: any hiccup or timeout → []. NASTECH_DEV_CREDITS_FIXTURE renders from the fixture instead of the portal."""
    try:
    ...
    if not _nastech_logged_in():
        return []
    try:
        snapshot = build_nastech_credits_snapshot(_fetch_portal_account(timeout))
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    try:
        ...
        if getattr(state, "paid_access", True) is False:
            details.append(_DEPLETED_LINE)
        return _nastech_snapshot(windows, details, ["(dev fixture — NASTECH_DEV_CREDITS_FIXTURE)"], source="dev-fixture")
    except (AttributeError, TypeError):
        return None
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    balance_lines = [
        line
        for line in render_account_usage_lines(build_nastech_credits_snapshot(account), markdown=markdown)
    ...
    return CreditsView(
        ...
        identity_line=("Topping up as " + " / ".join(who)) if who else None, topup_url=nastech_portal_topup_url(account),
...
def _codex_reset_outcome(body: dict, available: int) -> CodexResetRedeemResult:
    ...
    if code == "reset":
        ...
        try:
            from nastech_cli.auth import clear_codex_pool_quota_cooldowns
...
def redeem_codex_reset_credit(
    *, base_url: Optional[str] = None, api_key: Optional[str] = None, force: bool = False,
    ...
    try:
        ...
        return _unavailable("No Codex credentials available. Run `nastech auth` to sign in with your ChatGPT account.")
    ...
    try:
        ...
        if code in (401, 403):
            return _unavailable(f"Codex backend rejected the request (HTTP {code}). Reset credits require ChatGPT-account "
                                "(OAuth) auth — run `nastech auth` and sign in with your ChatGPT account.")
