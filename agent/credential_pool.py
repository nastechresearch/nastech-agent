...
# un-throttled log fires on *every* selection. On Windows several Nastech
...
# readiness handshake ("Timed out connecting to Nastech backend after
...
def custom_provider_pool_key_candidates(
    ...
    ``nastech auth add <key>`` stores new-style ``providers.<key>`` credentials
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): nastech, openai-codex, and xai-oauth rotate the
...
def _singleton_target_for_entry(pool: "CredentialPool", entry: "PooledCredential") -> Optional[Path]:
    """Root ``.anthropic_oauth.json`` when *entry* is a borrowed nastech_pkce row, else None."""
    if entry.source != "nastech_pkce" or entry.id not in getattr(pool, "_borrowed_root_ids", ()):
        return None
    try:
        from agent.anthropic_credentials import _root_nastech_oauth_file
        return _root_nastech_oauth_file()
    except Exception:
        return None
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.nastech/auth.json`` / profile equivalent), so it works for
        every *pool-owned* Anthropic source - ``nastech_pkce`` and
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Mirrors the Nastech/Anthropic resync paths above.  Only applies to
    ...
    def _sync_nastech_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Nastech pool entry from auth.json if tokens differ.

        Nastech OAuth refresh tokens are single-use.  When another process
        ...
        avoiding a "refresh token reuse" revocation on the Nastech Portal.
        """
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Nastech state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Nastech, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Nastech/Codex/xAI refresh in a multi-provider
        ...
        # device-code sources (nastech, openai-codex, xAI) use ``device_code``.
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Nastech
        ...
        # Previously "anthropic" was excluded from this tuple: two Nastech
        ...
        # "claude_code" (nastech_pkce, dashboard-issued manual entries) —
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Nastech profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Nastech process) that might
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        ``nastech_pkce`` sources the singleton file — not ``auth.json`` — is the
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``NASTECH_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            ...
            if self.provider == "xai-oauth"
            else "NASTECH_ANTHROPIC_REFRESH_TIMEOUT_SECONDS"
        )
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # Same rationale for the singleton source nastech_pkce:
                # _seed_from_singletons() reads ~/.nastech/.anthropic_oauth.json
                ...
                # Do not use endswith here: manual:nastech_pkce is already
                ...
                # a second authority for the same refresh-token family.
                    ...
                    try:
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.nastech/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
            ...
            if self.provider == "anthropic" and entry.source == "claude_code":
                ...
                # Backstop for non-claude_code sources (nastech_pkce,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    self._persist()
                    return updated
            # For xai-oauth: same race as nastech — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the Nastech quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nastech — another Nastech process
            ...
            if self.provider == "openai-codex":
                ...
                # in-memory pool.  Mirrors the xAI and Nastech quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    ...
                    with self._lock:
                        ...
                        self._persist(removed_ids=removed_ids)
                    return None
            # For nastech: another process may have consumed the refresh token
            ...
            if self.provider == "nastech":
                ...
                if isinstance(exc, TimeoutError):
                    ...
                    # ("matched no nastech entry ... pool size 0"). Leave the
                ...
                if auth_mod._is_terminal_nastech_refresh_error(exc):
                    logger.debug("Nastech refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "nastech":
            # Nastech refresh can require network access and should happen when
        ...
        for entry in self._entries:
            ...
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For nastech entries, sync from auth.json before status checks.
    ...
    def add_entry(self, entry: PooledCredential) -> PooledCredential:
        with self._lock:
            ...
            if borrowed_ids:
                # ``nastech -p <profile> auth add <single-use provider>``: the
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `nastech auth add nastech --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (nastech auth remove copilot gh_cli) must not pay either on
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with nastech/codex).
            source = "device_code"
...
def get_env_prefer_dotenv(key: str) -> str:
    ...
    # load_nastech_dotenv() -> apply_onepassword_secrets()).  The raw
