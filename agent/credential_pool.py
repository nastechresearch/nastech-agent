...
def _is_source_suppressed_fn() -> Callable[[str, str], bool]:
    """``nastech_cli.auth.is_source_suppressed`` (late-bound), or an always-False stub."""
    try:
        from nastech_cli.auth import is_source_suppressed
...
# singleton to re-seed from and the user can re-add via ``nastech auth add``.
...
# selection. On Windows several Nastech processes share one rotating log guarded by concurrent-log-handler's
...
# readiness handshake ("Timed out connecting to Nastech backend after 15000ms"). Logging the condition at
...
_EXTRA_KEYS = frozenset({
    ...
    # Nastech guest identity (``auth_method: anonymous``): the anon_ credential is the refresh material.
...
# Nastech singleton metadata mirrored between auth.json state and ``entry.extra``.
_NASTECH_EXTRA_STATE_KEYS = (
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "nastech":
            # Nastech stores the runtime inference credential in agent_key for
...
def _codex_principal_identity(access_token: Any) -> Optional[Tuple[str, str]]:
    ...
    Decoded without signature verification: this only decides whether two credentials Nastech
...
def _codex_entry_tracks_singleton(entry: PooledCredential, singleton_tokens: Dict[str, Any]) -> bool:
    ...
    ``nastech auth add openai-codex`` (must never be overwritten — adopting turned two logins into
...
def custom_provider_pool_key_candidates(
    ...
    ``nastech auth add <key>`` stores ``providers.<key>`` credentials under the
...
# ``nastech_cli.auth`` refresh function and terminal-error predicate (looked
...
REFRESHABLE_OAUTH_PROVIDERS = frozenset({"anthropic", "nastech", *_TOKENS_SINGLETON_PROVIDERS})
...
_REFRESH_TIMEOUT_ENV_VARS = {
    "openai-codex": "NASTECH_CODEX_REFRESH_TIMEOUT_SECONDS",
    "xai-oauth": "NASTECH_XAI_REFRESH_TIMEOUT_SECONDS",
}
...
class CredentialPool(CredentialPoolAdminMixin, CredentialPoolModelCooldownMixin):
    ...
    def _sync_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        A fresh ``nastech model`` / ``nastech auth`` login writes new tokens
    ...
    def _sync_nastech_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Nastech device_code entry from auth.json ``providers.nastech`` if state differs.

        Another process refreshing via ``resolve_nastech_runtime_credentials``
        writes fresh tokens under ``_auth_store_lock``; adopting them avoids a
        "refresh token reuse" revocation on the Nastech Portal.
        """
        if self.provider != "nastech" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                state = _load_provider_state(_load_auth_store(), "nastech")
            if not state:
                return entry
            comparable = {
                key: state.get(key)
            ...
            if not any(v not in (None, "") and getattr(entry, k, None) != v for k, v in comparable.items()):
                return entry
            logger.debug("Pool entry %s: syncing Nastech state from auth.json", entry.id)
            ...
            extra_updates.update(
                {k: state[k] for k in _NASTECH_EXTRA_STATE_KEYS if state.get(k) is not None}
            )
            return self._adopt(entry, extra=extra_updates, **field_updates)
        except Exception as exc:
            logger.debug("Failed to sync Nastech entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        token. Applies to Nastech, OpenAI Codex and xAI OAuth singletons.
        ...
        if entry.source != "device_code" or self.provider not in ("nastech", *_TOKENS_SINGLETON_PROVIDERS):
            return
        try:
            with _auth_store_lock():
    ...
    def _apply_entry_to_singleton_state(self, entry: PooledCredential, state: Dict[str, Any]) -> bool:
        ...
        if self.provider == "nastech":
            ...
            for extra_key in _NASTECH_EXTRA_STATE_KEYS:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # across Nastech processes, or two processes adopt the same on-disk
    ...
    def _fail_closed_unpersisted_rotation(
        ...
        *,
        ...
        For ``claude_code`` / ``nastech_pkce`` the singleton file — not
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        env_var = _REFRESH_TIMEOUT_ENV_VARS.get(self.provider, "NASTECH_ANTHROPIC_REFRESH_TIMEOUT_SECONDS")
    ...
    def _commit_anthropic_rotation(
        ...
        and other profiles see it). nastech_pkce -> <NASTECH_HOME>/.anthropic_oauth.json
        (``_seed_from_singletons`` re-seeds it every load). Not ``endswith``:
        manual:nastech_pkce is pool-owned and a singleton for it would be a second
        authority for the same refresh-token family.
        """
        if entry.source == "claude_code":
            store = "~/.claude/.credentials.json"
        elif entry.source == "nastech_pkce":
            store = "~/.nastech/.anthropic_oauth.json"
        else:
            return
        try:
    ...
    def _refresh_anthropic(self, entry: PooledCredential) -> PooledCredential:
        ...
        refreshed = refresh_anthropic_oauth_pure(entry.refresh_token, use_json=entry.source.endswith("nastech_pkce"))
        updated = replace(
            entry,
            ...
            if self.provider == "anthropic":
                ...
                if synced is not entry:
                    ...
                    if force and entry.runtime_api_key and entry.runtime_api_key != stale_key:
                        logger.debug("Nastech entry %s: adopting peer-rotated token, skipping refresh", entry.id)
                        return entry
                auth_mod.resolve_nastech_runtime_credentials(force_refresh=force, stale_access_token=stale_key or None)
    ...
    def _recover_failed_refresh(self, entry: PooledCredential, exc: Exception) -> Optional[PooledCredential]:
        ...
        if self.provider == "anthropic":
            if entry.source == "claude_code":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            synced.refresh_token, use_json=synced.source.endswith("nastech_pkce"),
                        )
                ...
                # Backstop for pool-owned sources (nastech_pkce, manual:dashboard_pkce):
            ...
            if is_terminal_anthropic_refresh_error(exc):
                ...
                # CLI's credentials file here — only Nastech' own row goes DEAD.
                logger.warning(
                    "Anthropic OAuth refresh token for %s is terminally invalid (%s); the credential "
                    "leaves rotation. Re-run 'nastech auth add anthropic' to sign in again.",
            ...
            # entries from the pool (mirrors the Nastech quarantine path).
            if getattr(auth_mod, terminal_fn_name)(exc):
                # WARNING, not debug: this is the moment a login is lost. At the default log level a
                # silent quarantine looked like "I logged in once and Nastech keeps failing" (#113023).
                logger.warning(
                    "%s OAuth refresh token is terminally invalid (%s); clearing local token state. "
                    "Re-run 'nastech auth add %s' to sign in again.", display, exc, self.provider)
            ...
            synced = self._sync_nastech_entry_from_auth_store(entry)
            if synced.refresh_token != entry.refresh_token:
                logger.debug("Nastech refresh failed but auth.json has newer tokens — adopting")
            ...
            if isinstance(exc, TimeoutError):
                ...
                # pool for ~120 sessions ("matched no nastech entry ... pool size
                # 0"). The caller's retry re-syncs once the winner persisted.
                logger.debug("Nastech refresh skipped: auth store lock busy; not benching entry")
                return entry
            if auth_mod._is_terminal_nastech_refresh_error(exc):
                logger.warning(
                    "Nastech refresh token is terminally invalid (%s); clearing local token state. "
                    "Re-run 'nastech auth add nastech' to sign in again.", exc)
                self._clear_terminal_nastech_state(entry, exc)
                self._quarantine_sources(
                    entry,
                    {auth_mod.NASTECH_DEVICE_CODE_SOURCE, f"manual:{auth_mod.NASTECH_DEVICE_CODE_SOURCE}"},
                )
                self._mark_dead_refresh_grant(entry, exc)
                return None
        self._mark_exhausted(entry, None)
    ...
    def _mark_dead_refresh_grant(self, entry: PooledCredential, exc: Exception) -> None:
        ...
        (``nastech auth add``) survives, and an unmarked survivor re-enters rotation and re-fires the
    ...
    def _clear_terminal_nastech_state(self, entry: PooledCredential, exc: Exception) -> None:
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
                state = _load_provider_state(auth_store, "nastech") or {
                ...
                if not store_refresh or store_refresh == str(entry.refresh_token or "").strip():
                    auth_mod._quarantine_nastech_oauth_state(state, exc, reason="credential_pool_refresh_failure")
                    auth_mod._quarantine_nastech_pool_entries(auth_store, exc, reason="credential_pool_refresh_failure")
                    _save_provider_state(auth_store, "nastech", state)
                    _save_auth_store(auth_store)
        except Exception as clear_exc:
            logger.debug("Failed to clear terminal Nastech OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        # Nastech refresh can require network access and happens when runtime
    ...
    def _resync_stale_entry(self, entry: PooledCredential) -> PooledCredential:
        ...
        The user may have re-authed (``nastech model`` / ``nastech auth``, the
        ...
        if self.provider == "nastech":
            return self._sync_nastech_entry_from_auth_store(entry)
...
class _Seeder:
    ...
    def upsert(self, source: str, payload: Dict[str, Any]) -> bool:
        """Upsert unless suppressed (``nastech auth remove`` must stay stable across loads)."""
...
def _seed_anthropic_singletons(seed: _Seeder) -> None:
    # Only auto-discover external credentials (Claude Code, Nastech PKCE) when
    ...
    # API-key vs OAuth is a user-visible choice at `nastech setup`. The API-key
    ...
    # ~/.nastech/.env over os.environ, as `_seed_from_env` does.
    _env_file = load_env()
    ...
    if _env_val("ANTHROPIC_API_KEY") and not anthropic_oauth_env:
        ...
        seed.changed |= _retain_sources_not_in(seed.entries, {"nastech_pkce", "claude_code"})
        return
    ...
    sources = [("nastech_pkce", read_nastech_oauth_credentials())]
    if adopt_external_logins_enabled():
        ...
        # earlier (adopting) process persisted, or it keeps rotating a login Nastech no longer reads.
...
def _seed_nastech_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    if not (state and has_runtime_material):
        return
    # Prefer a user-supplied label embedded in the singleton state (``nastech
    # auth add nastech --label <name>``) over the token-derived fingerprint.
    ...
    seed.upsert("device_code", {
        ...
        # ones (#15099).
        **{key: state.get(key) for key in _NASTECH_EXTRA_STATE_KEYS},
...
def _seed_tokens_singleton(seed: _Seeder, auth_store: Dict[str, Any]) -> None:
    ...
    Nastech owns its own Codex auth state and does NOT auto-import
    ...
    explicit one-time prompt via `nastech auth openai-codex`.
    """
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    if provider == "anthropic":
        ...
        _seed_nastech_singleton(seed, auth_store)
        ...
        # `nastech auth remove openai-codex` suppresses device_code; without
...
def get_env_prefer_dotenv(key: str) -> str:
    """Resolve a credential env var, preferring ~/.nastech/.env over os.environ.
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "nastech auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # requested (an `nastech auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons and Nastech PKCE disappear when their backing file is gone.
        return is_borrowed_credential_source(entry.source, entry.provider) or entry.source == "nastech_pkce"
