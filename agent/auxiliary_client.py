...
Text auto chain: main provider+model → OpenRouter → Nastech Portal → custom endpoint →
native Anthropic → direct API-key providers → None. Vision auto chain: main
provider (if a supported vision backend) → OpenRouter → Nastech → Anthropic → custom.
...
from agent.auxiliary_unavailable import (
    AuxiliaryClientUnavailable, clear_nastech_credential_failure, missing_provider_credentials_message,
    nastech_credential_failure_detail, record_nastech_credential_failure)
from nastech_constants import OPENROUTER_BASE_URL, nastech_home_key
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_verify``, ``NASTECH_CA_BUNDLE`` / ``SSL_CERT_FILE``); any failure → httpx default (``True``)."""
    try:
        from agent.ssl_verify import resolve_httpx_verify
        from nastech_cli.config import get_custom_provider_tls_settings, load_config_readonly
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Nastech owns aux retry/fallback policy; the SDK default (max_retries=2) would triple
    # wall time on a hung endpoint before Nastech sees one failure.
    # Nastech owns auxiliary retry + provider/model fallback policy (the same-provider transient retry in
    ...
    # so a 120s timeout can stall ~360s before Nastech sees a single failure (issue #54465). Disable
    # SDK-internal retries by default and let Nastech control the budget; explicit callers can still override
    # via kwargs.
...
def _run_protected_sync_provider_call(callback: Callable[[dict[str, Any]], Any], kwargs: dict[str, Any]) -> Any:
    ...
    threading.Thread(
        target=provider_context.run, args=(_provider_worker,), name="nastech-protected-aux-provider",
        daemon=True).start()
    while True:
...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    not isinstance, so out-of-tree clients can opt out of wrappers unimported (cf. SUPPORTS_NASTECH_TOOL_CALLS)."""
    try:
...
# provider — mirrors nastech_cli.auth._PROVIDER_ALIASES. Without this group an explicit
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    """
    is_nastech = provider_id.strip().lower() == "nastech"
    try:
        from nastech_cli.auth import resolve_api_key_provider_credentials
        from nastech_cli.models_pricing import fetch_models_with_pricing
        ...
        if not api_key and is_nastech:
            # Nastech is OAuth (resolver raises); anonymous reads return the full catalog.
            try:
                from nastech_cli.models_pricing import _resolve_nastech_pricing_credentials
                api_key, base_url = _resolve_nastech_pricing_credentials()
            except Exception:
                logger.debug("No Nastech credentials for catalog", exc_info=True)
        if not base_url:
        ...
        if base_url.endswith("/v1"):  # fetch_models_with_pricing appends /v1/models
            base_url = base_url[:-3]
        # Nastech-only args must match the pickers' or the seeded cache loses sale chrome and
        # policy-catalog expiry.
        _nastech_kwargs = {}
        if is_nastech:
            from nastech_cli.models_pricing import _NASTECH_CATALOG_TTL_SECONDS
            _nastech_kwargs = {"include_sale_original": True, "cache_ttl_seconds": _NASTECH_CATALOG_TTL_SECONDS}
        catalog = fetch_models_with_pricing(
            api_key=api_key or None, base_url=base_url, timeout=3.0, **_nastech_kwargs) or {}
    except Exception:
    ...
    if is_nastech:
        ...
        try:
            from nastech_cli.models_pricing import nastech_policy_allowed_ids, restrict_to_nastech_policy
            ids = restrict_to_nastech_policy(ids, nastech_policy_allowed_ids())
        except Exception:
            logger.debug("Nastech policy filter unavailable", exc_info=True)
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    if picked and provider_id.strip().lower() == "nastech":
        try:
            from nastech_cli.models_pricing import nastech_policy_allowed_ids, restrict_to_nastech_policy
            allowed = nastech_policy_allowed_ids()
            if allowed and not restrict_to_nastech_policy([picked], allowed):
                return ""
        except Exception:
            logger.debug("Nastech policy check unavailable", exc_info=True)
    return picked
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Precedence env > config > default: ``NASTECH_OPENROUTER_CACHE`` overrides
    ``openrouter.response_cache``; ``NASTECH_OPENROUTER_CACHE_TTL`` (1-86400 s) overrides
...
_NVIDIA_NIM_CLOUD_HEADERS = {"X-BILLING-INVOKE-ORIGIN": "NastechAgent"}
...
# Nastech Portal attribution extra_body. Tags come from agent.portal_tags so the client= marker
# tracks nastech_cli.__version__ — never inline a literal here.
from agent.portal_tags import nastech_portal_tags as _nastech_portal_tags


def _nastech_extra_body() -> dict:
    """Fresh Nastech Portal ``extra_body`` (per call, so a hot-reloaded version is reflected)."""
...
def _auth_json_path():
    ...
    would otherwise authenticate to Nastech with the default profile's token."""
...
def _to_openai_base_url(base_url: str) -> str:
    ...
    if base_url_hostname(url) == "api.actual.inc":
        from nastech_cli.auth import normalize_actual_base_url
...
def _pool_runtime_api_key(entry: Any) -> str:
    # runtime_api_key handles provider-specific fallback (e.g. agent_key for nastech); None entry → "".
...
class _CodexCompletionsAdapter:
    ...
    def _build_responses_kwargs(self, kwargs: Dict[str, Any]) -> Tuple[Dict[str, Any], str, Any]:
        ...
        wire_tools, wire_aliases = _alias_wire_tools(
            ...
            {"provider": getattr(self._client, "_nastech_aux_effective_provider", "") or None, "base_url": host},
            is_xai,
        )
        ...
        resp_kwargs: Dict[str, Any] = {
            # Codex only knows the base slug; strip the Nastech ``-900k`` picker suffix.
    ...
    def create(self, **kwargs) -> Any:
        ...
        if is_actual_route(
            getattr(self._client, "_nastech_aux_effective_provider", ""),
        ...
        try:
            ...
            # Undo only the aliases THIS request emitted, before the call reaches Nastech dispatch.
...
class _AnthropicCompletionsAdapter:
    ...
    def __init__(self, real_client: Any, model: str, is_oauth: bool = False, base_url: str | None = None):
        ...
        # Caller URL first; fall back to the SDK client's host only for Nastech Portal — a blanket
        ...
        if not self._base_url:
            ...
            if candidate:
                with contextlib.suppress(Exception):
                    from agent.anthropic_endpoints import _is_nastech_portal_endpoint
    ...
    def create(self, **kwargs) -> Any:
        ...
        # (already TRANSLATED to native fields — raw would 400 on strict gateways) and ``_`` Nastech plumbing.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``nastech_cli.runtime_provider._detect_api_mode_for_url`` so aux and main agree: any
...
def _maybe_wrap_anthropic(
    ...
    """
    # Anthropic/Bedrock/Codex wrappers, plus any client declaring NASTECH_SKIP_TRANSPORT_WRAP
    ...
    if (
        ...
        or _client_declares(client_obj, "NASTECH_SKIP_TRANSPORT_WRAP")
    ):
        return client_obj
...
def _read_nastech_auth() -> Optional[dict]:
    """Nastech provider state dict from the credential pool or ~/.nastech/auth.json; None when not active with tokens."""
...
def _nastech_api_key(provider: dict) -> str:
    ...
    for token_key, expiry_key in (("agent_key", "agent_key_expires_at"), ("access_token", "expires_at")):
        ...
        if not isinstance(token, str) or not token.strip():
            continue
        if _nastech_invoke_jwt_is_usable(token, scope=provider.get("scope"), expires_at=provider.get(expiry_key)):
            return token
    return ""
...
def _resolve_nastech_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    if force_refresh or not _agent_key_is_usable(_entry_state(entry), _nastech_min_key_ttl_seconds()):
        try:
        ...
        if refreshed is None:
            return None
        entry = refreshed
    api_key = _nastech_api_key(_entry_state(entry))
...
def _resolve_nastech_runtime_api(
    *, force_refresh: bool = False, stale_access_token: Optional[str] = None
    ...
    """Fresh Nastech runtime credentials (pool first, then auth store + JWT refresh) — mirrors the main
    ...
    try:
        ...
        record_nastech_credential_failure(exc)
        return None
    clear_nastech_credential_failure()
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    """
    try:
        from nastech_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL, _xai_validate_inference_base_url
        ...
        if pool and pool.has_credentials():
            ...
            if entry is not None:
                ...
                base_url = _xai_validate_inference_base_url(
                    _url(_scoped_key_env("NASTECH_XAI_BASE_URL"))
...
def _endpoint_default_headers(
    ...
    if base_url_host_matches(base_url, "api.kimi.com"):
        ...
        headers = dict(nastech_xai_default_headers())
    else:
...
def _try_nastech(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if runtime is None and not nastech:
        logger.warning("Auxiliary Nastech client unavailable: no Nastech authentication found (run: nastech auth).")
        _mark_provider_unhealthy("nastech", ttl=60, reason="no Nastech authentication found", level=logging.DEBUG)
        return None, None
    if runtime is None and nastech:
        logger.debug("Auxiliary Nastech: runtime JWT refresh failed; checking stored auth.json token.")
    if runtime is not None:
        ...
        if not api_key:
            ...
            _mark_provider_unhealthy("nastech", ttl=60, reason="no usable Nastech inference JWT", level=logging.DEBUG)
            return None, None
        base_url = str(
            (nastech or {}).get("inference_base_url") or _scoped_key_env("NASTECH_INFERENCE_BASE_URL") or _NASTECH_DEFAULT_BASE_URL
        ).rstrip("/")
    with contextlib.suppress(Exception):
        from agent.nastech_rate_guard import nastech_rate_limit_remaining
        from nastech_cli.anon_auth import is_anonymous_request
        anonymous = is_anonymous_request("nastech", api_key)
        remaining = nastech_rate_limit_remaining(anonymous=anonymous)
        if remaining is not None and remaining > 0:
            logger.debug("Auxiliary: skipping Nastech Portal (rate-limited, resets in %.0fs)", remaining)
            ...
            _mark_provider_unhealthy(
                "nastech", ttl=min(remaining, 60.0) if anonymous else remaining,
                reason="Nastech Portal rate-limited", level=logging.INFO)
            return None, None
    ...
    from nastech_cli.anon_auth import GUEST_MODEL, route_is_welcome_host
    global auxiliary_is_nastech
    if route_is_welcome_host(base_url):
        auxiliary_is_nastech = True
        logger.debug("Auxiliary/%s: Nastech free tier; using %s", lane, GUEST_MODEL)
        return _create_openai_client(api_key=api_key, base_url=base_url), GUEST_MODEL
    auxiliary_is_nastech = True
    logger.debug("Auxiliary client: Nastech Portal")
    # Portal recommended-models is authoritative (tier-aware); _NASTECH_MODEL when unreachable/null.
...
def _refresh_nastech_recommended_model(*, vision: bool, stale_model: Optional[str]) -> Optional[str]:
    """Fresh Portal recommended model after a stale-model 404 (long-lived processes pin dropped models).

    Returns the fresh recommendation, else ``_NASTECH_MODEL``, whichever differs from ``stale_model``; None if neither.
    """
    ...
    try:
        ...
        logger.debug("Nastech recommended-model refresh failed (%s); using default %s", exc, _NASTECH_MODEL)
    if fresh and fresh.strip().lower() != stale:
        return fresh
    return _NASTECH_MODEL if _NASTECH_MODEL.strip().lower() != stale else None
...
def _read_main_field(field: str, *, readonly: bool, lower: bool = False) -> str:
    ...
    with contextlib.suppress(Exception):
        from nastech_cli import config as _cfg_mod
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = _create_openai_client(
        api_key=api_key, base_url=base_url, default_headers=nastech_xai_default_headers()
    )
    return CodexAuxiliaryClient(real_client, model), model


def _codex_base_url_override() -> str:
    """Profile-scoped ``NASTECH_CODEX_BASE_URL`` (same read as the API-key env vars: under a
    multiplexer the routed profile's .env decides the endpoint, never a sibling's process env)."""
    return _scoped_key_env("NASTECH_CODEX_BASE_URL").rstrip("/")
...
def _get_provider_chain() -> List[tuple]:
    ...
    """
    return [
        ("openrouter", _try_openrouter), ("nastech", _try_nastech),
...
_AUX_UNHEALTHY_LABEL_ALIASES = {
    "openrouter": "openrouter", "nastech": "nastech", "custom": "local/custom",
...
def _nastech_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        ...
        return get_nastech_portal_account_info(force_fresh=True).paid_service_access is True
    except Exception as exc:
...
def _evict_cached_clients(provider: str) -> None:
    ...
    Scoped to the calling profile (``nastech_home_key()`` is the first key slot): a rotation in
...
def _refresh_copilot_credentials() -> bool:
    from nastech_cli.copilot_auth import _jwt_cache, _token_fingerprint, exchange_copilot_token, resolve_copilot_token
...
def _refresh_nastech_credentials() -> bool:
    from nastech_cli.auth import resolve_nastech_runtime_credentials
    return _creds_have_api_key(resolve_nastech_runtime_credentials(
        timeout_seconds=env_float("NASTECH_NASTECH_TIMEOUT_SECONDS", 15), force_refresh=True
    ))
...
_CREDENTIAL_REFRESHERS: Dict[str, Callable[..., bool]] = {
    ...
    "nastech": _refresh_nastech_credentials, "anthropic": _refresh_anthropic_credentials,
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        with contextlib.suppress(Exception):
            client._nastech_fallback_destination = _fallback_destination_from_entry(entry, client, resolved_model)
...
def _warn_stale_openai_base_url(runtime_provider: str) -> None:
    ...
    ~/.nastech/.env value after `nastech model` poisons routing)."""
...
def _discovery_chain_allowed(main_provider: str, task: Optional[str] = None) -> bool:
    ...
    compression silently charged to a Nastech Portal balance)."""
    ...
    logger.warning(
        ...
        "configured — refusing to guess another logged-in provider. Re-authenticate (`nastech model`) "
...
def _resolve_auto_route(
    ...
    top-level chain; (3) OpenRouter → Nastech → custom → Codex → API-key providers, only with no policy
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if _client_declares(sync_client, "NASTECH_SKIP_ASYNC_WRAP"):
        return sync_client, model
    ...
    # Nastech owns the auxiliary retry/timeout budget; disable SDK-internal retries.
    # See #54465.
...
def _wrap_transport(req: _ResolveRequest, client_obj: Any, final_model_str: str,
    ...
    if _is_actual_auxiliary_route(req, base_url_str):
        ...
        client._nastech_aux_effective_provider = "actual"
        return client
    needs_codex = not (
...
def _resolve_auto_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if routed_client is not None and effective_provider:
        try:
            setattr(routed_client, "_nastech_aux_effective_provider", effective_provider)
...
def _resolve_nastech_branch(req: _ResolveRequest) -> _ResolveResult:
    """Nastech Portal (OAuth)."""
    model = req.model
    # Vision: caller flag, _PROVIDER_VISION_MODELS override, or a known vision id.
    client, default = _try_nastech(vision=(req.is_vision or model in _PROVIDER_VISION_MODELS.values()
                                        or (model or "").strip().lower() == "mimo-v2-omni"))
    if client is None:
        logger.warning("resolve_provider_client: nastech requested but Nastech Portal not configured (run: nastech auth)")
        return None, None
    ...
    client = _maybe_wrap_anthropic(
        ...
        str(getattr(client, "base_url", "") or ""), nastech_api_mode(final_model),
    )
...
def _resolve_openai_codex_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if not model:
        ...
        return None, None
    no_token_msg = "resolve_provider_client: openai-codex requested but no Codex OAuth token found (run: nastech model)"
    if req.raw_codex:
...
def _resolve_named_custom_branch(req: _ResolveRequest) -> Optional[_ResolveResult]:
    ...
    # canonical name (e.g. ``nastech``) still defer to the built-in.
    custom_entry = None
    ...
    if not custom_base:
        ...
        return None, None
    from nastech_cli.config import normalize_extra_headers
...
def _resolve_azure_foundry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    return _route_or_warn(req, client, default_model,
                          ...
                          "runtime resolution failed (run: nastech doctor for diagnostics)")
...
def _resolve_registry_branch(req: _ResolveRequest) -> _ResolveResult:
    ...
    if auth_type == "vertex":
        ...
        # nastech / openai-codex / xai-oauth already returned from their explicit branches.
...
_EXPLICIT_PROVIDER_BRANCHES: Dict[str, Callable[[_ResolveRequest], _ResolveResult]] = {
    ...
    "nastech": _resolve_nastech_branch,
...
def resolve_provider_client(
    ...
    # Excluded: ``auto`` (a stale main slug could pair with any picked provider) and Nastech + vision (the
    # Portal's tier-aware vision recommendation must win over a text-only model).
    if not model and provider != "auto" and not (provider == "nastech" and is_vision):
        ...
        # not explicitly request one. (# compression-current-model) Nastech + vision is the one carve-out: the
        # branch below resolves its model from the Portal's tier-aware vision recommendation
        # (``_try_nastech(vision= True)``), and ``final_model = model or default`` means anything pre-filled
...
_VISION_AUTO_PROVIDER_ORDER = ("openrouter", "nastech", "deepinfra")
...
# Strict (explicitly requested) vision backends by normalized provider name. nastech MUST go
# through resolve_provider_client so anthropic/* picks wrap onto /v1/messages (a bare _try_nastech
...
_STRICT_VISION_BACKENDS: Dict[str, Callable[[Optional[str]], Tuple[Optional[Any], Optional[str]]]] = {
    ...
    "nastech": lambda model: resolve_provider_client("nastech", model, is_vision=True),
...
def get_available_vision_backends() -> List[str]:
    """Available vision backends in auto-selection order (active provider → OpenRouter → Nastech → DeepInfra).
...
def _vision_main_provider_client(
    ...
    if main_provider == "nastech":
        # Nastech picks its vision model from Portal tier-aware slots inside _try_nastech(vision=True);
...
def _vision_auto_route(
    ...
    """Auto-detect order: 1. main provider + model, 2. OpenRouter, 3. Nastech Portal, 4. DeepInfra, 5. stop."""
...
def get_auxiliary_extra_body() -> dict:
    """Return extra_body kwargs (Nastech Portal product tags when Nastech-backed, else {})."""
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    direct_openai_family = (
        not _scoped_key_env("OPENROUTER_API_KEY") and _read_nastech_auth() is None
...
def _client_cache_key(
    ...
    # Profile home leads the key: callers that omit api_key (pool / Nastech auth.json paths) would
    # otherwise share one client across multiplex profiles holding different credentials.
    return (nastech_home_key(), provider, async_mode, base_url or "", api_key_key, api_mode or "", runtime_key, is_vision, task_key, pool_hint, model_key)
...
def _refresh_nastech_auxiliary_client(
    *, cache_provider: str, model: Optional[str], async_mode: bool, base_url: Optional[str] = None,
    ...
    """
    runtime = _resolve_nastech_runtime_api(force_refresh=True, stale_access_token=api_key)
    if runtime is None:
        return None, model
...
def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
    ...
    try:
        ...
        return normalized in {
            "anthropic", "copilot", "copilot-acp", "minimax-oauth", "nastech", "openai-codex", "qwen-oauth", "xai-oauth",
        }
...
def _acquire_sync_aux_semaphore(task: Optional[str]) -> Optional[threading.BoundedSemaphore]:
    ...
    return _cached_semaphore(_aux_sync_semaphores, (nastech_home_key(), task), limit, threading.BoundedSemaphore)
...
def _acquire_async_aux_semaphore(task: Optional[str]):
    ...
    try:
        ...
        return None
    from nastech_constants import nastech_home_key
    return _cached_semaphore(_aux_async_semaphores, (nastech_home_key(), task, id(loop)), limit, asyncio.Semaphore)
...
_NASTECH_PROVIDER_NAMES = frozenset({"nastech", "nastech-portal", "nastechresearch"})


def _nastech_on_messages_wire(provider_norm: str, model: str) -> bool:
    """True when a Nastech Portal route serves ``model`` over /v1/messages (dual-wire catalog)."""
    if provider_norm not in _NASTECH_PROVIDER_NAMES:
        return False
    from nastech_cli.providers import nastech_api_mode
    return nastech_api_mode(model) == "anthropic_messages"
...
def _forwards_max_tokens(provider: str, provider_norm: str, model: str, effective_base: str, task: Optional[str]) -> bool:
    ...
    return (
        ...
        or _nastech_on_messages_wire(provider_norm, model)
...
def _merge_aux_extra_body(
    ...
    """Caller extra_body + profile body/reasoning + generic reasoning fallback + Nastech tags."""
    ...
    # _get_task_extra_body) takes the same wire clamp: Nastech-only ``ultra`` never reaches the
    ...
    if provider_norm in _NASTECH_PROVIDER_NAMES:
...
def _build_call_kwargs(
    ...
    # ``extra_body.reasoning`` fallback. Clamp Nastech-internal levels (``ultra``) to the
    ...
    if reasoning_config and isinstance(reasoning_config, dict):
        ...
        if (
            provider_norm == "anthropic" or projection.messages_wire or _nastech_on_messages_wire(provider_norm, model)
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Nastech manages."""
    if not base_url:
        return False
...
def _resolve_call_client(
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is None:
                    nastech_detail = nastech_credential_failure_detail() if _explicit == "nastech" else None
                    raise AuxiliaryClientUnavailable(
                        nastech_detail or missing_provider_credentials_message(_explicit))
    ...
    if client is None:
        raise AuxiliaryClientUnavailable(f"No LLM provider configured for task={task} "
                                         f"provider={resolved_provider}. Run: nastech setup")
...
def _refreshed_nastech_step(route: _LadderRoute, kwargs: Dict[str, Any], message: str) -> Optional[_LadderStep]:
    """Rebuild the Nastech client after a credential event; None when nothing refreshed."""
    refreshed_client, refreshed_model = _refresh_nastech_auxiliary_client(
        cache_provider=route.resolved_provider or "nastech", model=route.final_model,
...
def _ladder_nastech_rungs(
    first_err: Exception, route: _LadderRoute, kwargs: Dict[str, Any], client_is_nastech: bool,
):
    """Nastech-only rungs: stale-model self-heal, paid-account refresh, 401 refresh.
    ...
    if _is_model_not_found_error(first_err) and client_is_nastech:
        ...
        if healed_model and healed_model != kwargs.get("model"):
            logger.warning("Auxiliary %s%s: model %r no longer in Nastech catalog; "
    ...
    if _is_payment_error(first_err) and client_is_nastech and _nastech_portal_account_has_fresh_paid_access():
        step = _refreshed_nastech_step(
            route, kwargs,
            "Auxiliary %s%s: refreshed Nastech runtime credentials after paid account check, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
    ...
    if _is_auth_error(first_err) and client_is_nastech:
        step = _refreshed_nastech_step(
            route, kwargs, "Auxiliary %s%s: refreshed Nastech runtime credentials after 401, retrying")
        if step is not None:
            resp, first_err = yield from _rung(
                step, lambda exc: _credential_rung_accepts(exc) or _is_connection_error(exc))
            if first_err is None:
...
def _aux_recovery_ladder(
    ...
    strips → Nastech heal/refresh → credential refresh/pool rotation → provider fallback.
    ...
    if first_err is None:
        return resp
    resp, first_err = yield from _ladder_credential_rungs(first_err, route, kwargs, client_is_nastech)
    if first_err is None:
        return resp
