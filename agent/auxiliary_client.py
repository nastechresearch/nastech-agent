...
def _client_declares(client_obj: Any, flag: str) -> bool:
    ...
    without this module importing it. Mirrors ``SUPPORTS_NASTECH_TOOL_CALLS`` in
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        ...
        if not api_key and is_nastech:
            # Nastech is OAuth, so the resolver above raises for it. An anonymous
            # read returns the full catalog, and a model picked from it is
            # refused at request time by the org's policy.
            try:
                from nastech_cli.models import _resolve_nastech_pricing_credentials
        ...
        if base_url.endswith("/v1"):
            base_url = base_url[:-3]
        # Same entry the pickers use, so the Nastech-only arguments must match
        # theirs: seeding it here without them costs the picker its sale chrome
        # and leaves the policy catalog with no expiry.
        _nastech_kwargs = {}
        if is_nastech:
            from nastech_cli.models import _NASTECH_CATALOG_TTL_SECONDS
        ...
        catalog = fetch_models_with_pricing(
            api_key=api_key or None, base_url=base_url, timeout=3.0, **_nastech_kwargs
        ) or {}
    except Exception:
...
def _nastech_policy_blocks(model_id: str) -> bool:
    """True when the org's model policy does not admit *model_id*."""
    try:
        from nastech_cli.models import nastech_policy_allowed_ids, restrict_to_nastech_policy

        allowed = nastech_policy_allowed_ids()
        return bool(allowed) and not restrict_to_nastech_policy([model_id], allowed)
    except Exception:
...
def _get_aux_model_for_provider(provider_id: str, *, prefer_fast: bool = False) -> str:
    ...
    if picked and provider_id.strip().lower() == "nastech" and _nastech_policy_blocks(picked):
        return ""
    return picked
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Nastech-side ``-900k`` large-context picker suffix —
...
def _maybe_wrap_anthropic(
    ...
    - It's already a complete client — an Anthropic/Codex wrapper, or any
      client declaring ``NASTECH_SKIP_TRANSPORT_WRAP`` (the native and ACP
    ...
    if _client_declares(client_obj, "NASTECH_SKIP_TRANSPORT_WRAP"):
        return client_obj
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.nastechresearch.github.io"):
        return "nastech"
...
def _auth_refresh_provider_for_route(
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference-api.nastechresearch.github.io"):
        return "nastech"
    return normalized
...
def resolve_provider_client(
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from nastech_cli.models import opencode_zen_free_runtime as _oc_free_rt
...
def _refresh_nastech_auxiliary_client(
    *,
    ...
    default ``"Nastech-4-405B"``); it is stored as the entry's usable model and
    ...
    on the default Nastech config, where ``call_llm`` looks up with
    ...
    element ``"Nastech-4-405B"`` vs the lookup's ``""``), leaving the expired
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Nastech will call. A requested model covers a compressor
    ...
    *,
    ...
    try:
        ...
        _heal_is_nastech = (
            ...
            or base_url_host_matches(_base_info, "inference-api.nastechresearch.github.io")
        )
...
async def _async_call_llm_impl(
    ...
    *,
    ...
    try:
        ...
        _heal_is_nastech = (
            resolved_provider == "nastech"
            or base_url_host_matches(_client_base, "inference-api.nastechresearch.github.io")
        )
        if _is_model_not_found_error(first_err) and _heal_is_nastech:
            healed_model = _refresh_nastech_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
