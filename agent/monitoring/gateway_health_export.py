...
_OBSERVABLE_METRIC_NAMES = (
    "nastech.gateway.up", "nastech.gateway.state", "nastech.gateway.active_agents", "nastech.gateway.busy",
    "nastech.gateway.drainable", "nastech.gateway.restart_requested", "nastech.gateway.background_work",
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="nastech-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("nastech.gateway.background_work", _read_background_work_count),
            ("nastech.gateway.background_delegations", _read_background_delegations_count),
        ):
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("nastech.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'nastech-agent[otlp]'", exc_info=True)
