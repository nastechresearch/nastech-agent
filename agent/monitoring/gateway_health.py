...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "nastech.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("nastech.gateway.up", int(bool(gateway_running))),
        metric("nastech.gateway.active_agents", active_agents),
        metric("nastech.gateway.busy", int(bool(busy))),
        metric("nastech.gateway.drainable", int(bool(drainable))),
        metric("nastech.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("nastech.gateway.state", 1, **{"nastech.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"nastech.platform": str(platform), "nastech.platform.state": state}
        metrics.append(metric("nastech.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("nastech.platform.degraded", int(is_degraded), **pattrs, **{"nastech.error_code": error_code}))
        if is_degraded:
