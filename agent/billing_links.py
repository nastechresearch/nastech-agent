...
@dataclass
class BillingBlock:
    """Structured billing-wall descriptor shared across every surface. ``is_nastech`` is the routing bit:
    Nastech has an in-app billing surface (Settings → Billing, ``/topup``) preferred over ``billing_url``;
...
def is_nastech_inference_route(provider: str, base_url: str) -> bool:
    """True when the failing route is the Nastech-managed inference gateway."""
    return (provider or "").strip().lower() == "nastech" or base_url_host_matches(str(base_url or ""), "inference-api.nastechresearch.github.io")


def _nastech_billing_url() -> Optional[str]:
    """Best-effort Nastech portal billing URL (text-surface fallback; Nastech prefers the in-app flow)."""
    try:
        from nastech_cli.nastech_account import nastech_portal_billing_url
...
def build_billing_block(*, provider: str, base_url: str, model: str, message: str = "") -> BillingBlock:
    ...
    if is_nastech_inference_route(slug, base_url):
        return BillingBlock(slug or "nastech", "Nastech Portal", model, _nastech_billing_url(), True, message or "")
