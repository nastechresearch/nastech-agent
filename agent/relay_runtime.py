...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when Nastech finalization forwards runtime metadata. Filter to
...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "Nastech Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "Nastech could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside Nastech native ownership; leaving it unchanged and "
                    "disabling Nastech-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "Nastech Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # Nastech only enters Relay's initialization path after an
                ...
                logger.warning(
                    "Nastech Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
            ...
            logger.info(
                ...
                "hosted by this Nastech process."
            )
            return state
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Nastech Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "Nastech Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
        ...
        with session.lock:
            ...
            try:
                self.run_in_session(
                    ...
                    old_handle,
                    output={"nastech.session.segment_reason": reason},
                    metadata={
                        RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                        RUNTIME_INSTANCE_KEY: self.runtime_id,
                    },
                ...
                logger.warning(
                    "Nastech Relay segment close failed (session=%s segment=%d); "
            ...
            scope_metadata = {
                RUNTIME_SCHEMA_KEY: RUNTIME_SCHEMA_VERSION,
                RUNTIME_INSTANCE_KEY: self.runtime_id,
                "nastech.session.segment": session.segment,
                "nastech.session.segment_reason": reason,
            }
            parent_handle = None
            if session.parent_session_id:
                with self._sessions_lock:
            ...
            try:
                ...
                logger.warning(
                    "Nastech Relay segment open failed (session=%s segment=%d); "
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "Nastech Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "Nastech Relay drained %d orphaned scope(s) before closing %s",
    ...
    def shutdown(self) -> None:
        ...
        if has_active_operations:
            thread = threading.Thread(
                ...
                name=f"nastech-nemo-relay-shutdown-{self.runtime_id[:8]}",
                daemon=True,
            )
            try:
                thread.start()
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "Nastech Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
...
class RelaySessionCoordinator:
        ...
        *,
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Nastech Relay segment rotation failed", exc_info=True
                )
            try:
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "Nastech Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "Nastech Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Nastech Relay compaction notification failed", exc_info=True
            )
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "Nastech [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "Nastech Relay plugin configuration could not be loaded from "
...
def _resolve_plugin_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nastech-nemo-relay-plugin-lifecycle",
        daemon=True,
    )
    thread.start()
    thread.join()
