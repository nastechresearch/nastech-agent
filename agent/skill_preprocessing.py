"""Shared SKILL.md preprocessing helpers: ``${NASTECH_*}`` template tokens and
...
from nastech_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger(__name__)

# ${NASTECH_SKILL_DIR} / ${NASTECH_SESSION_ID} tokens. Unresolvable ones (e.g. no
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${NASTECH_SKILL_DIR} / ${NASTECH_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "NASTECH_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "NASTECH_SESSION_ID": str(session_id) if session_id else None,
    }
