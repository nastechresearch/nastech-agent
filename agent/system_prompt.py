...
from agent.prompt_builder import (
    ...
    NASTECH_AGENT_HELP_GUIDANCE, NASTECH_AGENT_HELP_GUIDANCE_NO_SKILLS, KANBAN_GUIDANCE,
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    r"^## Plugin Context: (?P<id>[a-z0-9][a-z0-9._-]{0,127})\n<!-- nastech-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    """Append the desktop embedded-terminal clarifier when ``NASTECH_DESKTOP_TERMINAL``
    is set (only the desktop's TUI PTY, never the chat backend). Idempotent."""
    if not hint or _TUI_EMBEDDED_PANE_CLARIFIER in hint or not is_truthy_value(os.getenv("NASTECH_DESKTOP_TERMINAL")):
        return hint
...
def _active_profile_name(agent: Any, ambient) -> str:
    ...
    failure. Ambient resolution misreports on threads that lost the NASTECH_HOME
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        try:
            from nastech_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from nastech_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    A bound NASTECH_HOME ContextVar override wins (the gateway multiplexes
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses ``get_default_nastech_root()`` (NOT ``get_nastech_home()``): on a bound
    ...
    try:
        ...
        rel = home.resolve().relative_to((get_default_nastech_root() / "profiles").resolve())
...
def _auto_load_parts(agent: Any) -> List[str]:
    ...
    NASTECH_IGNORE_RULES are read on the first build only) so the prompt stays byte-stable
    ...
    if not getattr(agent, "_auto_load_skills_resolved", False):
        ...
        try:
            if not is_truthy_value(os.environ.get("NASTECH_IGNORE_RULES")):
...
def _active_profile_line(agent: Any) -> str:
    """Name the running profile so the agent doesn't conflate ``~/.nastech/skills``
    (default) with ``~/.nastech/profiles/<active>/skills``.  Resolved from the
    ...
    if active_profile == "default":
        ...
        # ROOT (get_nastech_home() on a bound profile session is the PROFILE dir).
        # Without one, keep the ambient (patchable) resolution byte-identical.
        _root_str = str(get_default_nastech_root() if _agent_home_path is not None else get_nastech_home())
        return (
    ...
    profile_home = str(_agent_home_path) if _agent_home_path is not None else str(get_nastech_home())
    ...
    # default profile's data sits at the ROOT (get_default_nastech_root()), which in ambient profile mode is
    # NOT get_nastech_home().
...
def _cron_delivery_hint(agent: Any) -> str:
    ...
    deliver_key = get_session_env("NASTECH_CRON_AUTO_DELIVER_PLATFORM", "").lower().strip()
...
def _timestamp_line(agent: Any) -> str:
    ...
    from nastech_time import get_timezone as _nastech_tz, now as _nastech_now
    now = _nastech_now()
    _bits = _zone_bits(now, _nastech_tz())
...
def _context_files_part(agent: Any, ctx_len: Optional[int], soul_loaded: bool) -> List[str]:
    ...
    fallback they really are so the guard can reject Nastech's bundled AGENTS.md."""
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    # nastech-agent skill installed, so the variant is chosen after the skills
    # index is built; this slot holds its position.
    _help_guidance_slot = len(stable_parts)
    stable_parts.append(NASTECH_AGENT_HELP_GUIDANCE_NO_SKILLS)
    stable_parts.extend(_guidance_parts(agent))
    skills_prompt = _skills_prompt(agent)
    # Skill-pointer variant requires BOTH skill_view AND the nastech-agent skill
    # in the rendered index (pure string check — inherits the index's stability).
    if "skill_view" in (agent.valid_tool_names or set()) and "- nastech-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = NASTECH_AGENT_HELP_GUIDANCE
