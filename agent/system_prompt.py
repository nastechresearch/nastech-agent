...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, nastech subscription block, tool-use
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- nastech-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the NASTECH_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound NASTECH_HOME ContextVar override wins. Surfaces that multiplex
...
def _profile_name_for_home(home: Path) -> str:
    ...
    Uses :func:`get_default_nastech_root` (NOT ``get_nastech_home()``): on a
    correctly bound profile session the ambient home IS the profile dir, so
    ``get_nastech_home()/profiles`` would never contain ``home`` and every
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if agent.load_soul_identity or not agent.skip_context_files:
        ...
        # ambient resolution on a thread that lost the NASTECH_HOME ContextVar
    ...
    # Pointer to the docs (and, when it exists, the nastech-agent skill) for
    # user questions about Nastech itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the nastech-agent skill not installed — so the
    ...
    # the nastech-agent skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- nastech-agent:" in skills_prompt:
    ...
    # `nastech -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['nastech-bots']).
    ...
    # ambient NASTECH_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_nastech_root(): get_nastech_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_nastech_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_nastech_root())
    else:
        _home_str = _root_str = str(get_nastech_home())
    if active_profile == "default":
        ...
        # data sits at the ROOT (get_default_nastech_root()), which in ambient
        # profile mode is NOT get_nastech_home().
    ...
    if not agent.skip_context_files:
        ...
        if getattr(agent, "_context_cwd_is_launch_artifact", False):
            ...
            # then reject Nastech's bundled contributor AGENTS.md (#97448).
            context_cwd = None
