...
def _fm_field(fm: dict[str, Any], key: str) -> Any:
    """Top-level ``key`` or ``metadata.nastech.<key>``; tolerant of the string-valued
    ...
    return nastech.get(key) if isinstance(nastech, dict) else None
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        try:
            return json.loads((get_nastech_home() / "skills" / ".usage.json").read_text(encoding="utf-8"))
        except Exception:
            return {}
...
def _memory_cards() -> list[dict[str, Any]]:
    ...
    base = get_nastech_home() / "memories"
...
def build_learning_graph() -> dict[str, Any]:
    ...
    roots = [("base", Path(__file__).resolve().parent.parent / "skills"), ("profile", get_nastech_home() / "skills")]
    learned_skills = {
