...
def serves_routed_profile() -> bool:
    ...
    multiplexing, else when a NASTECH_HOME override names another home (dashboard/desktop backend,
    ...
    if is_multiplex_active():
        return True
    from nastech_constants import get_nastech_home_override, get_process_nastech_home, nastech_home_key
    override = get_nastech_home_override()
    return override is not None and nastech_home_key(override) != nastech_home_key(get_process_nastech_home())
...
class UnscopedSecretError(RuntimeError):
    ...
    def __init__(self, secret_name: str = "", developer_detail: str = ""):
        ...
        super().__init__(
            f"Nastech could not read {what} (an internal profile-scoping bug on the multiplexed "
            "gateway, not your configuration). Run `nastech gateway restart`; if it keeps happening, "
            "report it with `nastech debug share`."
        )
...
_GLOBAL_ENV_EXACT = frozenset({
    ...
    "NASTECH_MAX_ITERATIONS", "NASTECH_API_TIMEOUT",
    "NASTECH_REDACT_SECRETS", "NASTECH_NASTECH_TIMEOUT_SECONDS",
    "_NASTECH_GATEWAY",
    # OS / interpreter
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Nastech writes itself (bare, 'single', or "double" with
    ``\\"`` / ``\\\\`` escapes)."""
...
# ``invalidate_env_file_cache()`` is the explicit knob; ``nastech_cli.config.invalidate_env_cache()``
# calls it for Nastech's own .env writers.
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    """THE ``.env`` tokenizer: every reader (profile scope, ``nastech_cli.config.load_env``, the dashboard
...
def build_profile_secret_scope(nastech_home: Path) -> Dict[str, str]:
    ...
    secrets = load_env_file(Path(nastech_home) / ".env")
    try:
        from nastech_cli.env_loader import get_secret_source_values
        external_secrets = get_secret_source_values(Path(nastech_home))
    except Exception:
        external_secrets = {}
    ...
    if bridged is not None and _is_process_home(nastech_home):
        secrets.setdefault("GATEWAY_ALLOW_ALL_USERS", bridged)
    return secrets


def _is_process_home(nastech_home: Path) -> bool:
    from nastech_constants import get_process_nastech_home
    try:
        return Path(nastech_home).resolve() == get_process_nastech_home().resolve()
    except OSError:
        return False
