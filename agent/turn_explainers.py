...
_PERSISTENCE_CAUSE_EXPLANATIONS: Dict[str, str] = {
    ...
    "turn_lease": (
        "the turn was stopped because another Nastech process "
    ...
    # logger.error at nastech_state.py::_raise_if_db_replaced — never in the chat reply.
    "replaced": (
        "the session database file was replaced while Nastech was running, so this "
        "message was not saved (a copy is kept in {home}/sessions/). Stop Nastech "
        "(`nastech {profile_arg}gateway stop`), run `nastech {profile_arg}doctor` — not "
        "`nastech {profile_arg}doctor --fix`, which would repair the wrong file in place — "
        "then start it again and send your message once more. Advanced recovery steps are "
        "in the log."
    ),
    "deleted_wal": (
        "the session database was changed or replaced while Nastech was running, so this "
        "message was not saved (a copy is kept in {home}/sessions/). Stop Nastech "
        "(`nastech {profile_arg}gateway stop`), run `nastech {profile_arg}doctor`, then start "
    ...
    "corrupt": (
        ...
        "1. Run `nastech {profile_arg}doctor --fix`\n"
        "2. Stop the gateway, then recover with:\n"
        "   nastech {profile_arg}sessions recover --source {db_path} --inspect-only\n"
        "   (if it reports recoverable) nastech {profile_arg}sessions recover "
    ...
    "fts_index": (
        ...
        "recovery tools or restore a backup. Run `nastech {profile_arg}doctor --fix` "
        "(or restart Nastech, which repairs the index on open), then "
        "send your message again."
    ),
    "disk": (
        "Nastech couldn't save this conversation to disk, so it stopped rather than lose "
...
_PERSISTENCE_DEFAULT_EXPLANATION = (
    "Nastech couldn't save this conversation, so it stopped rather than lose your messages. "
    "Possible causes: the drive is out of room, or another Nastech process is holding the "
    "database. Close other Nastech windows, run `nastech {profile_arg}doctor` to check "
...
def _display_flag_enabled(agent, *, env_var: str, config_key: str, cache_attr: str) -> bool:
    ...
    and tests patch it at ``nastech_cli.config``). Any failure → True (safe default: on)."""
    try:
...
class TurnExplainersMixin:
    ...
    def _record_file_mutation_result(
        ...
        *, task_id: Optional[str] = None,
        ...
        if landed:
            ...
            # Nastech-authored content from later user hand-edits.
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        """``display.file_mutation_verifier`` / ``NASTECH_FILE_MUTATION_VERIFIER`` (a patchable seam)."""
        return _display_flag_enabled(
            self, env_var="NASTECH_FILE_MUTATION_VERIFIER", config_key="file_mutation_verifier",
            cache_attr="_file_mutation_verifier_enabled_cache",
        )

    def _turn_completion_explainer_enabled(self) -> bool:
        """``display.turn_completion_explainer`` / ``NASTECH_TURN_COMPLETION_EXPLAINER``."""
        return _display_flag_enabled(
            self, env_var="NASTECH_TURN_COMPLETION_EXPLAINER", config_key="turn_completion_explainer",
    ...
    def _format_turn_completion_explanation(
        ...
        if body is None and reason == "session_persistence_failed":
            from nastech_constants import display_nastech_home, profile_cli_selector

            # Copy-pasteable, so pin every `nastech` command to the profile whose store failed:
            # a multi-profile backend (Desktop serve) hosts sessions whose state.db is NOT the
            # process default, and a bare `nastech` follows active_profile (#105887).
            body = (
                _PERSISTENCE_CAUSE_EXPLANATIONS.get(
                    persistence_cause or "unknown", _PERSISTENCE_DEFAULT_EXPLANATION
                )
                .replace("{home}", display_nastech_home())
            ...
            if persistence_cause in ("corrupt", "fts_index"):
                ...
                body = body.replace(
                    "{backups_dir}", str(get_default_nastech_root() / "backups")
                )
