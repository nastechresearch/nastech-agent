...
def prepare_iteration(
    ...
    # nastech.anthropic_wire=auto: a wire switch decided from the previous response lands here,
    # before this iteration's request is built and with nothing in flight.
    if getattr(agent, "_nastech_wire_pending", None):
        from agent.nastech_wire import apply_pending_wire_switch
    ...
    # Nastech agent keys live ~1 h and a single turn can run for hours: adopt the keepalive's fresh
    ...
    try:
        agent._adopt_nastech_key_before_expiry()
    except Exception:
        logger.debug("Nastech key pre-expiry adoption failed", exc_info=True)
    ...
    if repaired_seq > 0:
        ...
        # (nastech-webui) would write the current turn to the FRONT of the context. Re-anchor as
