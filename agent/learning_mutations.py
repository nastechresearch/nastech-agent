...
Shared by CLI ``nastech journey``, the TUI ``/journey`` overlay and the desktop.
Deleting a skill *archives* it (``nastech curator restore`` recovers it);
...
def _locate_memory(node_id: str) -> tuple[Path, list[str], int]:
    ...
    path = get_nastech_home() / "memories" / _MEMORY_FILES[source]
    if not path.exists():
