...
# ``nastech_portal_tags()``. A ContextVar so concurrent agents in one process never
# see each other's id; ``propagate_context_to_thread`` workers inherit it.
_conversation_id: ContextVar[Optional[str]] = ContextVar("nastech_portal_conversation_id", default=None)

# Ambient affinity scope (ROUTING value): OpenRouter's sticky ``session_id``, Nastech
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("nastech_affinity_scope", default=None)
