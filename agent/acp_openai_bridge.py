...
ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Nastech' tool schemas travel INTO the
...
read/edit/execute tools forwards only Nastech' agent-level tools, since re-offering overlapping ones
makes Nastech redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``nastech_projected_messages``)
    Nastech reads off the ``create`` result; a plain list would drop them on the stream path."""
