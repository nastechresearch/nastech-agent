"""OpenAI-shape bridge shared by Nastech' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Nastech as
...
the agent's *own* tool notifications. Nastech' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Nastech' perspective).
...
of its own (Copilot) forwards everything Nastech offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Nastech'
agent-level tools, because re-offering the overlapping ones makes Nastech re-run
...
class StreamChunks(list):
    ...
    Nastech reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``nastech_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Nastech'
