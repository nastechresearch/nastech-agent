...
headers, and request quirks (MiniMax, Kimi/Moonshot, DeepSeek, OpenCode, Azure AI Foundry, Nastech
...
def _is_deepseek_anthropic_endpoint(base_url: str | None) -> bool:
    ...
    keep-unsigned policy used for Kimi's ``/coding`` endpoint. See nastech-agent#16748.
    """
...
def _requires_bearer_auth(base_url: str | None) -> bool:
    ...
    Foundry, Palantir Foundry's LLM proxy, CommandCode, Nastech Portal. Palantir/CommandCode use
    hostname matching (not substring) so ``evil.com/palantirfoundry`` paths don't trigger it."""
    normalized = _normalized_lower(base_url)
    return (
        _is_nastech_portal_endpoint(base_url)
