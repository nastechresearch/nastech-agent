"""Cross-session rate limit guard for Nastech Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway, cron,
auxiliary) can check whether Nastech Portal is currently rate-limited before making
requests. Without it each 429 fans out into up to 9 calls per turn (3 SDK
retries x 3 Nastech retries), all counted against RPH.
"""
...
def _state_path(*, anonymous: bool = False) -> str:
    """Path to the Nastech rate limit state file."""
    try:
    ...
    return os.path.join(base, "rate_limits", "nastech-anonymous.json" if anonymous else "nastech.json")
...
def record_nastech_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
    anonymous: bool = False,
) -> None:
    """Record that Nastech Portal is rate-limited in the shared state file.
    ...
    try:
        ...
        logger.info("Nastech rate limit recorded: resets in %.0fs (at %.0f)", reset_at - now, reset_at)
    except Exception as exc:
        logger.debug("Failed to write Nastech rate limit state: %s", exc)


def nastech_rate_limit_remaining(*, anonymous: bool = False) -> Optional[float]:
...
def clear_nastech_rate_limit(*, anonymous: bool = False) -> None:
    """Clear the rate limit state (e.g., after a successful Nastech request)."""
    try:
        os.unlink(_state_path(anonymous=anonymous))
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Nastech rate limit state: %s", exc)
...
def is_genuine_nastech_rate_limit(
    *, headers: Optional[Mapping[str, str]] = None, last_known_state: Optional[Any] = None,
    ...
    Nastech multiplexes upstream providers behind one key, so a 429 may be one
    upstream model out of capacity (clears in seconds) rather than our quota;
    tripping the breaker on that would block every Nastech model for minutes.
...
def is_long_welcome_rate_limit(error_context: Any) -> bool:
    """True for a Nastech welcome-tier ``rate_limited`` refusal whose reset is long enough to be an
    exhausted allowance (``WELCOME_LONG_WAIT_SECONDS``), as parsed into ``error_context``
    (``welcome_refusal`` from ``nastech_cli.anon_auth.parse_welcome_refusal``). Capacity refusals
