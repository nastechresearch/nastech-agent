...
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # nastech_cli/models.py), and models.dev's "opencode" catalog is the one
...
def _models_dev_to_nastech_ids(mdev_id: str) -> List[str]:
    ...
    if _MODELS_DEV_TO_PROVIDER is None:
        ...
        for nastech_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            reverse.setdefault(mapped, []).append(nastech_id)
...
def _get_etag_path() -> Path:
    ...
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "models_dev_cache.etag"
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Nastech provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Nastech id.
    for nastech_id in _models_dev_to_nastech_ids(provider_key):
        if nastech_id != provider_key:
            candidates.append(nastech_id)
...
def list_provider_models(
    ...
    picker (``nastech model``), which is an interactive user-facing flow
