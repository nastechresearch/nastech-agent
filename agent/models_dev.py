...
thread refreshes) → disk cache (~/.nastech/models_dev_cache.json, any age) →
...
from nastech_constants import openrouter_variant_base
...
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # Nastech provider is "meta-ai"; both aliases are needed or muse-spark-*
...
# Reverse mapping: models.dev id → Nastech ids (built lazily; many-to-one).
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, List[str]]] = None


def _models_dev_to_nastech_ids(mdev_id: str) -> List[str]:
    """Return the Nastech provider ids that map to *mdev_id* (may be [])."""
    global _MODELS_DEV_TO_PROVIDER
    if _MODELS_DEV_TO_PROVIDER is None:
        _MODELS_DEV_TO_PROVIDER = {}
        for nastech_id, mapped in PROVIDER_TO_MODELS_DEV.items():
            _MODELS_DEV_TO_PROVIDER.setdefault(mapped, []).append(nastech_id)
...
def _models_dev_id(provider: str) -> Optional[str]:
    """models.dev provider id for a Nastech provider id, or None. A custom provider reaches the
    ...
    if mdev_id is None:
        ...
        if mdev_id is not None and mdev_id not in PROVIDER_TO_MODELS_DEV.values() \
            ...
            if (key, alias) not in _UNKNOWN_CATALOG_PROVIDER_WARNED:
                ...
                logger.warning("providers.%s: catalog_provider %r is neither a Nastech provider id nor a "
...
def _nastech_path(name: str) -> Path:
    from nastech_constants import get_nastech_home
    return get_nastech_home() / name


def _get_cache_path() -> Path:
    return _nastech_path("models_dev_cache.json")


def _get_etag_path() -> Path:
    return _nastech_path("models_dev_cache.etag")
...
def _save_etag(etag: str) -> None:
    def write() -> None:
        ...
        mkdir_under_nastech_home(etag_path.parent)
...
def _get_provider_models(provider: str, *, allow_network: bool = False) -> Optional[Dict[str, Any]]:
    """Resolve a Nastech provider ID to its models dict, or None if unknown.
...
# models the catalog does not know and never displace catalog data. Provider keys accept the Nastech
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    """Override section for *provider* (keyed by Nastech OR models.dev id), or None."""
    overrides = _load_model_overrides()
    provider_key = (provider or "").strip()
    if not overrides or not provider_key:
        return None
    # Forward (Nastech → models.dev id) and reverse (caller passed a models.dev id, config keyed by Nastech id) aliases.
    candidates = [provider_key, PROVIDER_TO_MODELS_DEV.get(provider_key), *_models_dev_to_nastech_ids(provider_key)]
...
def _builtin_model_metadata(provider: str, model: str) -> Optional[Dict[str, Any]]:
    """Built-in metadata for a provider/model pair, if Nastech has a vendor-specific entry."""
...
def get_provider_info(provider_id: str, *, allow_network: bool = True) -> Optional[ProviderInfo]:
    """Provider metadata by Nastech or models.dev ID, or None if not cataloged. ``allow_network`` defaults to True (interactive setup)."""
...
def get_model_info(provider_id: str, model_id: str, *, allow_network: bool = False) -> Optional[ModelInfo]:
    """Full model metadata by Nastech or models.dev provider ID (exact match, then case-insensitive), or
