...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    NASTECH_CODEX_TTFB_MAX_SECONDS, NASTECH_CODEX_HARD_TIMEOUT_SECONDS.
    """
    ...
    if ttfb_timeout <= 0:
        ...
        ttfb_cap = env_float("NASTECH_CODEX_TTFB_MAX_SECONDS", 120.0)
        if ttfb_cap > 0 and ttfb_timeout > ttfb_cap:
            logger.info("Capping openai-codex no-event TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set NASTECH_CODEX_TTFB_MAX_SECONDS to tune.", ttfb_timeout, ttfb_cap,
                f"{est_tokens:,}")
