...
def _provider_preferences_for_agent(agent) -> Dict[str, Any]:
    ...
    with contextlib.suppress(Exception):
        ...
        from nastech_constants import resolve_per_model_provider_routing
...
def _merge_nastech_portal_messages_extra_body(agent, anthropic_kwargs: dict) -> dict:
    ...
    The Nastech profile is only consulted by the OpenAI-wire transport; ``session_id``
    ...
    try:
        ...
        if nastech_profile is not None:
            anthropic_kwargs.setdefault("extra_body", {}).update(
                nastech_profile.build_extra_body(session_id=getattr(agent, "session_id", None)))
...
def _configured_stale_base(agent) -> float:
    """Per-provider ``stale_timeout_seconds`` config, else NASTECH_STREAM_STALE_TIMEOUT (180s)."""
    cfg = get_provider_stale_timeout(agent.provider, agent.model)
    return cfg if cfg is not None else env_float("NASTECH_STREAM_STALE_TIMEOUT", 180.0)
...
def _bedrock_converse_call(api_kwargs: dict, *, stream: bool, on_stream_denied=None):
    """Pop the Nastech routing keys and call ``converse`` / ``converse_stream`` (boto3
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from nastech_cli.local_runtime.load_progress import get_loading_progress, get_prefill_progress
...
def _resolve_nonstream_watchdogs(agent, api_kwargs: dict) -> _NonStreamWatchdogs:
    ...
    NASTECH_CODEX_TTFB_TIMEOUT_SECONDS,
    NASTECH_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each),
    NASTECH_CODEX_TTFB_DISABLE_ABOVE_TOKENS / NASTECH_CODEX_TTFB_STRICT,
    NASTECH_CODEX_TTFB_MAX_SECONDS (opt-in ceiling, default 0 = none), NASTECH_CODEX_HARD_TIMEOUT_SECONDS.
    """
    ...
    if codex and openai_codex_backend:
        ...
        hard_timeout = env_float("NASTECH_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
        if hard_timeout > 0:
    ...
    ttfb_enabled = codex
    ttfb_explicit = env_float("NASTECH_CODEX_TTFB_TIMEOUT_SECONDS", -1.0) != -1.0
    ttfb_timeout = env_float("NASTECH_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if ttfb_timeout <= 0:
        ttfb_enabled = False
        ...
        disable_above = env_float("NASTECH_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        strict = os.environ.get("NASTECH_CODEX_TTFB_STRICT", "").strip().lower() in {"1", "true", "yes", "on"}
        if not strict and disable_above > 0 and est_tokens >= disable_above and ttfb_timeout < idle_default:
            logger.info("Scaling openai-codex no-event TTFB watchdog from %.0fs to %.0fs "
                ...
                "Set NASTECH_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.", ttfb_timeout, idle_default,
        ...
        ttfb_cap = env_float("NASTECH_CODEX_TTFB_MAX_SECONDS", 0.0)
        if ttfb_cap > 0 and ttfb_timeout > ttfb_cap:
            logger.info("Capping openai-codex no-event TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens) per NASTECH_CODEX_TTFB_MAX_SECONDS.", ttfb_timeout, ttfb_cap,
                f"{est_tokens:,}")
    ...
    idle_explicit = env_float("NASTECH_CODEX_EVENT_STALE_TIMEOUT_SECONDS", -1.0) != -1.0
    idle_timeout = env_float("NASTECH_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_default)
...
def _build_chat_completions_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    ...
    _qwen_meta = {"sessionId": agent.session_id or "nastech", "promptId": str(uuid.uuid4())} if _is_qwen else None
    _profile = None
    with contextlib.suppress(Exception):
        from providers import get_provider_profile
...
def _assistant_content_for_storage(agent, assistant_message):
    ...
    # history / state.db / gateway delivery (no-op with NASTECH_REDACT_SECRETS off).
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    if (fb.get("provider") or "").strip().lower() != "nastech":
        return None
    try:
    ...
    return None if has_token else "nastech_token_missing"
...
def _fallback_api_mode_resolved(agent, fb_provider: str, fb_model: str, fb_base_url: str) -> str:
    ...
    from nastech_cli.runtime_provider_custom import _opencode_family_for_custom
    ...
    if fb_provider in {"nastech", "nastech-portal", "nastechresearch"}:
        ...
        return nastech_api_mode(fb_model)
...
def _iteration_summary_api_messages(agent, messages: list) -> list:
    ...
    for msg in messages:
        ...
        # every Nastech-internal underscore-prefixed scaffolding key.
...
def _anthropic_summary_attempt(agent, api_messages: list, api_request_id: str):
    def _attempt(retry_count: int) -> str:
        ...
        ant_kw = _merge_nastech_portal_messages_extra_body(agent, ant_kw)
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        ...
        # Strict machine-readable mode (nastech chat -Q, oneshot, background review): keep diagnostics out of
...
class _BedrockStream:
    ...
    def _on_stale(self, stale_elapsed: float) -> None:
        ...
        self.last_event = time.time()
        # Raises RuntimeError past NASTECH_STREAM_STALE_GIVEUP; otherwise end
...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _stream_timeouts(self) -> tuple[float, float, float]:
        ...
        ``request_timeout_seconds`` wins over NASTECH_API_TIMEOUT (1800s) and
        NASTECH_STREAM_READ_TIMEOUT (120s); connect/pool cover the handshake, not
        inference: 30s, or capped at 60s when configured."""
        cfg = get_provider_request_timeout(self.agent.provider, self.agent.model)
        base = cfg if cfg is not None else env_float("NASTECH_API_TIMEOUT", 1800.0)
        if cfg is not None:
            return base, cfg, min(base, 60.0)
        read = env_float("NASTECH_STREAM_READ_TIMEOUT", 120.0)
    ...
    def _choiceless_chunk(chunk, finish_reason):
        ...
        # Nastech Portal usage frames (choices=[] + lastOne=true, no [DONE]) are a
    ...
    def _chat_stream_created(self, raw_stream: Any) -> None:
        ...
        self.agent._capture_nastech_model_switch(response)
    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        if self.agent.provider == "moa":
            # Nastech interrupts the managed stream; Relay alone closes the provider stream.
    ...
    def _call(self):
        ...
        # consume the transient budget: on the last attempt (or NASTECH_STREAM_RETRIES=0) the
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        NASTECH_LOCAL_STREAM_STALE_TIMEOUT — an infinite one stalled sessions on a
        ...
        if base == 180.0 and self.agent.base_url and is_local_endpoint(self.agent.base_url):
            ...
            self._stream_stale_timeout = env_float("NASTECH_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
