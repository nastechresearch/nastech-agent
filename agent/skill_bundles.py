...
YAML files in ``<NASTECH_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``NASTECH_BUNDLES_DIR`` override (tests) or ``<NASTECH_HOME>/skill-bundles``."""
    override = os.environ.get("NASTECH_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_nastech_home() / "skill-bundles"
...
# File-level CRUD — used by `nastech bundles`.
