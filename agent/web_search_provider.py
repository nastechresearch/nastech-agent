...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`nastech_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.nastech/.env``) so credentials set
    through Nastech' config layer are visible even when they were never
