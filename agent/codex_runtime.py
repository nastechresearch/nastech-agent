...
def _record_codex_app_server_usage(agent, turn, messages=None) -> dict[str, Any]:
    """Translate Codex app-server token usage into Nastech accounting. Prompt bucket = uncached + cached
    ...
    estimate grows monotonically and nastech-mode fires thread compaction on tiny threads (#100381)."""
...
def _record_codex_app_server_compaction(agent, turn, *, approx_tokens: int | None = None, force: bool = False) -> bool:
    ...
    if compressor is not None:
        ...
        # Codex owns this summary: a prior Nastech deterministic-fallback flag must not leak into it.
...
# --- Codex app-server → Nastech UI bridge -------------------------------------
# The app-server bypasses the Nastech tool loop, so the bridge translates JSON-RPC notifications
# into the callbacks the standard runtime fires (tool_progress_callback, _fire_stream_delta, ...).

# Item types that project to a Nastech tool_call (keep in sync with agent/transports/codex_event_projector.py
...
# Internal MCP server wrapping Nastech' native tools: its inner dispatch has no tool_progress_callback, so the
# codex-level mcpToolCall IS the display event and the mcp.nastech-tools.* prefix is stripped (users see Nastech tools).
...
def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Nastech tool name for a codex item (mirrors CodexEventProjector)."""
    item_type = item.get("type") or ""
    if item_type == "mcpToolCall":
        server, tool = item.get("server") or "mcp", item.get("tool") or "unknown"
        return tool if server == NASTECH_TOOLS_MCP_SERVER_NAME else f"mcp.{server}.{tool}"
...
def _ensure_codex_session(agent) -> None:
    ...
    # Approval callback: Nastech' standard prompt flow when a CLI thread installed one.
    ...
    # explicit approval bypass (approvals.mode: off, /yolo, --yolo, NASTECH_YOLO_MODE) hands policy to codex's sandbox.
    ...
    # Nastech' gateway UI callbacks (tool_progress_callback, _fire_stream_delta,
