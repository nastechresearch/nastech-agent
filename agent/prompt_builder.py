...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "explicit Nastech YOLO uses an unrestricted runtime after the user's "
        "launch/session risk acceptance. Permission mode and grants are fixed "
        "when Nastech launches that runtime.\n\n"
        ...
        "run `nastech computer-use doctor` and share the output. That command "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Nastech terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        ...
        "Widgets can talk back: window.nastech.send(\"prompt\") — or a "
        "data-nastech-send=\"prompt\" attribute on any clickable element — sends "
