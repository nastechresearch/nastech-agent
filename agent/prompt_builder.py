...
def _find_nastech_md(cwd: Path) -> Optional[Path]:
    ...
    # could pick up a .nastech.md planted in /tmp, /home, etc.
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Nastech Agent, built by Nastech Research. Be direct: match the "
...
NASTECH_AGENT_HELP_GUIDANCE = (
    ...
    # precedence. Injected only when skill_view exists AND the nastech-agent
    # skill is actually installed (see system_prompt.py slot resolution).
    "You run on Nastech Agent (by Nastech Research). When the user needs help with "
    "Nastech itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://nastechresearch.github.io/nastech-agent/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. The `nastech-agent` skill has the actual commands and proven "
    "workflows — load it with skill_view(name='nastech-agent') before configuring, "
    "modifying, or troubleshooting Nastech so you don't guess or invent workarounds."
)
...
NASTECH_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    ...
    "Nastech itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://nastechresearch.github.io/nastech-agent/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "- **Workspace.** `cd $NASTECH_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${NASTECH_KANBAN_BRANCH:-wt/$NASTECH_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$NASTECH_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`nastech profile list`, or ask the user), "
...
#
# The nastech-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to nastech-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Nastech appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Nastech floating over another application, so the
    user is rarely asking about Nastech — they are asking about the thing behind
    ...
    sentences = [
        ...
        "rather than anything inside Nastech. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Nastech terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the Nastech desktop app, a graphical chat "
        ...
        "data-nastech-send=\"prompt\" on any clickable element (or "
        "window.nastech.send(\"prompt\")) sends that prompt as a hidden user "
...
def build_skills_system_prompt(
    ...
    # NASTECH_HOME would otherwise fall back to the launch (default) home and
    ...
    try:
        ...
        # Trusted project-local dirs (./.nastech/skills, ./.agents/skills at
...
def load_soul_md(
    ...
    ambient — which on a thread that lost the NASTECH_HOME ContextVar falls
    ...
    _home = Path(home_override) if home_override is not None else get_nastech_home()
...
def _load_nastech_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    ...
    if not nastech_md_path:
        return ""
    try:
        content = (_read_text_with_timeout(nastech_md_path) or "").strip()
        if not content:
            return ""
        ...
        return _truncate_content(
            result, ".nastech.md", context_length=context_length,
            read_path=str(nastech_md_path),
        )
    except Exception as e:
...
def build_context_files_prompt(
    ...
    # Never let a FALLBACK-picked directory inside the Nastech install/source
    ...
    # explicitly configured cwd is honored verbatim — the Nastech tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Nastech in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Nastech install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_nastech_md(cwd_path, context_length)
