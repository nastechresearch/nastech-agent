"""The provider call for the conversation turn's retry loop: ``nastech_rate_limit_guard`` (skip
the attempt while another session's Nastech Portal rate limit is active), ``perform_api_call``
...
def nastech_rate_limit_guard(
...
) -> NastechRateGuardVerdict:
    """Skip the call if another session recorded a Nastech Portal rate limit: every attempt (incl.
    ...
    def _verdict(action: str, result: Optional[Dict[str, Any]] = None) -> NastechRateGuardVerdict:
        return NastechRateGuardVerdict(
    ...
    if agent.provider == "nastech":
        # A gateway ``x-nastech-model-switch`` recorded on the previous response moves this session
        # (and the config default, when it still names the free tier's model) before the next call.
        try:
            from nastech_cli.anon_auth import apply_model_switch
            apply_model_switch(agent)
        except Exception:
            pass
        try:
            from agent.nastech_rate_guard import (
                nastech_rate_limit_remaining, format_remaining as _fmt_nastech_remaining
            )
            from nastech_cli import anon_auth
            _anonymous = anon_auth.is_anonymous_agent(agent)
            _nastech_remaining = nastech_rate_limit_remaining(anonymous=_anonymous)
            if _nastech_remaining is not None and _nastech_remaining > 0:
                reset = _fmt_nastech_remaining(_nastech_remaining)
                if _anonymous:
                    _nastech_msg = anon_auth.FREE_TIER_RATE_LIMIT_CHAT.format(
                        reset=anon_auth.friendly_wait(_nastech_remaining))
                else:
                    _nastech_msg = f"Your Nastech account has hit its rate limit; it resets in {reset}."
                agent._buffer_vprint(f"⏳ {_nastech_msg} Trying fallback...")
                agent._buffer_diagnostic_status(f"⏳ {_nastech_msg}")
                ...
                return _verdict("return", stamp_failure({
                    "final_response": (f"⏳ {_nastech_msg}" if _anonymous
                                       else f"⏳ {_nastech_msg}\n\n{site_copy('nastech_rate_limit')}"),
                    "messages": messages,
                    ...
                    **({"free_tier": {"kind": "rate_limited", "message": anon_auth.FREE_TIER_RATE_LIMIT_CARD.format(
                        reset=anon_auth.friendly_wait(_nastech_remaining))}} if _anonymous else {}),
