...
* The ``iron-proxy`` binary is auto-installed into ``<nastech_home>/bin/iron-proxy``
  on first use.  Nastech pins one upstream version (``_IRON_PROXY_VERSION``)
...
* A long-lived CA at ``<nastech_home>/proxy/ca.{crt,key}`` is generated on
  first ``nastech egress setup``.  Sandboxes trust this CA so iron-proxy can
  terminate TLS and rewrite headers.

* The proxy config lives at ``<nastech_home>/proxy/proxy.yaml``.  It enumerates
...
* The proxy runs as a managed subprocess (``nastech egress start``), pidfile
  at ``<nastech_home>/proxy/iron-proxy.pid``.  Daemon output (including
  per-request records on v0.39) goes to ``<nastech_home>/proxy/iron-proxy.log``;
...
# request needs the bearer key below.  ``nastech egress reload`` is the
# client.
#
# The key is minted at setup time, stored at
# ``<nastech_home>/proxy/management.token`` (0600), and injected into the
...
_MGMT_API_KEY_ENV = "NASTECH_IRON_PROXY_MGMT_KEY"
...
_DEFAULT_ALLOWED_HOSTS: Tuple[str, ...] = (
    ...
    "api.together.xyz",
    "api.deepseek.com",
    "inference.nastechresearch.github.io",
)
...
_BEARER_PROVIDERS: Dict[str, Tuple[str, ...]] = {
    ...
    "NASTECH_API_KEY": ("inference.nastechresearch.github.io",),
}
...
# credential (Nastech' auth.py keys Google on both GEMINI_API_KEY and
...
def _proxy_state_dir_ro() -> Path:
    ...
    this — there's no reason to materialize ``~/.nastech/proxy/`` just to
    check whether a pidfile exists.
    """
    from nastech_constants import get_nastech_home

    return get_nastech_home() / "proxy"
...
def find_iron_proxy(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<nastech_home>/bin/iron-proxy``  (our managed copy — preferred)
...
def install_iron_proxy(*, force: bool = False) -> Path:
    ...
    these; the user-facing ``nastech proxy install`` surface lets them
    ...
    with tempfile.TemporaryDirectory(prefix="nastech-iron-proxy-") as tmpdir:
        tmp = Path(tmpdir)
...
def iron_proxy_version(binary: Path) -> str:
    ...
    # Don't cache empty output — that would poison ``nastech egress
...
def ensure_ca_cert(*, force: bool = False) -> Tuple[Path, Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="nastech-proxy-ca-") as tmpdir:
        tmp = Path(tmpdir)
        tmp_key = tmp / "ca.key"
        tmp_crt = tmp / "ca.crt"
...
def mint_proxy_token(prefix: str = "nastech-proxy") -> str:
...
def ensure_management_token(*, force: bool = False) -> str:
    ...
    Stored at ``<nastech_home>/proxy/management.token`` with 0600 perms.
    The daemon receives it via the ``NASTECH_IRON_PROXY_MGMT_KEY`` env var
    (named in the generated config's ``management.api_key_env``);
    ``nastech egress reload`` reads the same file to authenticate.
    """
    ...
    if not force and p.exists():
        try:
            ...
            pass
    token = mint_proxy_token(prefix="nastech-mgmt")
    fd = os.open(
        str(p),
...
def reload_proxy() -> bool:
    ...
    if not pid or not _pid_alive(pid):
        raise RuntimeError(
            ...
            "Run `nastech egress start`."
        )
    mgmt = _read_management_listen_from_config()
    if mgmt is None:
        raise RuntimeError(
            "The generated proxy.yaml has no management listener (written "
            "before reload support).  Re-run `nastech egress setup` and use "
            "`nastech egress restart` this one time."
        )
    token = _read_management_token()
    if not token:
        raise RuntimeError(
            "management.token is missing — re-run `nastech egress setup`, "
            "then `nastech egress restart`."
        )
...
def build_proxy_config(
    ...
    * ``secrets`` transform takes ``secrets:`` (plural), each with a
    ...
    return {
        ...
        # in ``ProxyStatus`` / ``nastech egress status``.
        "metrics": {
            "listen": "127.0.0.1:0",
        },
        ...
        # pipeline — `nastech egress reload` applies allowlist/token/mapping
...
def write_proxy_config(config: Dict) -> Path:
    """Serialize the config dict to ``<nastech_home>/proxy/proxy.yaml``.

    Uses ``yaml.safe_dump`` so we never emit Python tags.
    """

    try:
        import yaml  # PyYAML is already a Nastech dep
...
def discover_uncovered_providers(
    *,
    ...
    The wizard and ``nastech egress status`` use this to print a warning.
...
def merge_mappings(
    *,
    ...
    re-running ``nastech egress setup`` should not invalidate the tokens
...
# arbitrary env value).
_NASTECH_IRON_PROXY_NONCE_ENV = "NASTECH_IRON_PROXY_NONCE"
    ...
    if nonce_candidates:
        try:
            ...
            for nonce in nonce_candidates:
                needle = f"{_NASTECH_IRON_PROXY_NONCE_ENV}={nonce}".encode()
...
def start_proxy(
    *,
    ...
    if bin_path is None:
        raise RuntimeError(
            "iron-proxy binary not available — run `nastech egress install`."
        )
    ...
    if not cfg.exists():
        raise RuntimeError(
            ...
            "Run `nastech egress setup` first."
        )
    ...
    env[_NASTECH_IRON_PROXY_NONCE_ENV] = _proxy_nonce
    ...
    # close-on-success path, every restart leaked one fd in the Nastech
    # process.
    #
    ...
    # ``nastech egress stop`` can clean up the orphan.  Failure paths
    ...
    # hits Ctrl-C while waiting for ``nastech egress start`` doesn't leak
    ...
    try:
        ...
        if existing_pid and _pid_alive(existing_pid):
            raise RuntimeError(
                ...
                f"Run `nastech egress stop` if that proxy is stuck."
            )
            ...
            if access_token and project_id:
                ...
                if missing:
                    ...
                    if not (bitwarden_config or {}).get("allow_env_fallback"):
                        raise RuntimeError(
                            ...
                            f"`nastech egress setup --no-bitwarden`, or set "
                ...
                if warnings:
                    logger.warning(
                        ...
                        "run `nastech secrets bitwarden status` for detail.",
                        len(warnings),
                    )
            else:
