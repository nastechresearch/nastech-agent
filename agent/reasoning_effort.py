...
Nastech' internal effort ladder (``nastech_constants.VALID_REASONING_EFFORTS``
...
# ``ultra`` is Nastech-internal ladder vocabulary (the Codex product tier); no
# provider wire accepts it verbatim anywhere. Every declared wire set below
# therefore stops at ``max`` — ``ultra`` always clamps down.

#: The widest OpenAI-compatible wire vocabulary (OpenRouter, Nastech Portal):
