...
# OpenRouter/Nastech run a quality-first model chain and may fall back depending on
# account access, so fidelity can vary by configured backend.
_REF_CAPABLE = ("nastech", "openai", "openai-codex", "openrouter", "krea")
# Friendly display label per reference-capable provider (desktop pet-gen picker).
_PROVIDER_LABELS = {"nastech": "Nastech Portal", "openrouter": "OpenRouter", "openai": "OpenAI", "openai-codex": "OpenAI (Codex)", "krea": "Krea"}
...
def resolve_provider(*, require_references: bool = True, prefer: str | None = None) -> SpriteProvider:
    ...
    Preference: ``NASTECH_PET_IMAGE_PROVIDER`` (QA override, unknown values ignored),
    ...
    raise GenerationError(
        ...
        "Open `nastech tools` → Image Generation and configure Nastech Portal, "
...
def generate(
    ...
    *aspect_ratio* ``"landscape"`` (row strips) gives every frame horizontal room so
    ...
    # Providers disagree on the ref kwarg name: our OpenRouter/Nastech backends read
