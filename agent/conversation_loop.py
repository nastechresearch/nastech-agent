...
from agent.turn_api_call import handle_api_interrupt, nastech_rate_limit_guard, perform_api_call
...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in nastech_state.py; kept local so importing
# nastech_state (module-level DEFAULT_DB_PATH) is not forced at load time.
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Nastech tool use: model=%s provider=%s base_url=%s "
    ...
    return (
        f"Ollama loaded `{model}` with only {runtime_ctx:,} tokens of runtime context, but Nastech "
        ...
        "again. A known-good starting point is 65,536 tokens. In Nastech config, set "
...
def _nastech_entitlement_message(capability: str) -> str:
    try:
        ...
        return format_nastech_portal_entitlement_message(
...
def _print_nastech_entitlement_guidance(agent, capability: str) -> bool:
    return _print_guidance(agent, _nastech_entitlement_message(capability))
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            return "\n".join([
                ...
                "Retry with a fresh credential state: `nastech auth reset anthropic`. Until that "
...
def _run_api_retry_loop(agent, s: _LoopState) -> Optional[Dict[str, Any]]:
    ...
    while s.retry_count < s.max_retries:
        _ng = _run_phase(nastech_rate_limit_guard, agent, s)
...
def _run_conversation_turn(
    ...
    # Adopt ~/.nastech/.env credential/base-url edits made since the last turn — a
...
def _close_durable_failed_turn(agent, result: Any) -> None:
    """Append a Nastech-authored assistant boundary when a failed turn left ``user`` as the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'PARTIAL_STREAM_STUB_ID': ('nastech_constants', 'PARTIAL_STREAM_STUB_ID'),
