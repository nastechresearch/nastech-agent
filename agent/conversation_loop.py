...
# Must mirror _STALE_TOOL_CALL_MARKER_RE in nastech_state.py — kept local
# to avoid importing nastech_state at module load time (its module-level
# DEFAULT_DB_PATH = get_nastech_home() / "state.db" breaks tests that
# monkeypatch get_nastech_home to return a str).
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.nastech/prefill.json incident, 20/20 blocked with
...
def _maybe_grow_local_window(agent: Any, compressor: Any,
    ...
    if "127.0.0.1" not in base_url and "localhost" not in base_url:
        return None
    try:
        from nastech_cli.local_runtime.growth import maybe_grow_window
...
def _is_nastech_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference-api.nastechresearch.github.io")
    )
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `nastech auth reset anthropic`. Until "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Nastech' own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `nastech fallback add`."
)
...
def run_conversation(
    ...
    if moa_config is None:
        try:
            from nastech_cli.moa_config import decode_moa_turn
    ...
    # Adopt any ~/.nastech/.env credential/base-url edits made since the last
    ...
    # 'disk', or 'unknown' — see nastech_state.classify_persistence_error).
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        while retry_count < max_retries:
            # ── Nastech Portal rate limit guard ──────────────────────
            # If another session already recorded that Nastech is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "nastech":
                try:
                    ...
                    if _nastech_remaining is not None and _nastech_remaining > 0:
                        _nastech_msg = (
                            f"Nastech Portal rate limit active — "
            ...
            try:
                ...
                try:
                    from nastech_cli.lifecycle import (
                        has_hook,
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Nastech/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                # Clear Nastech rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Nastech.
                if agent.provider == "nastech":
                    try:
                ...
                if (
                    ...
                    if _try_refresh_nastech_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 Nastech paid access verified — "
                ...
                if (
                    ...
                    if agent._try_refresh_nastech_client_credentials(force=True):
                        agent._buffer_vprint(f"🔐 Nastech agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Nastech 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nastech_entitlement_guidance(agent, "Nastech model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: nastech auth add nastech")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.nastechresearch.github.io")
                ...
                # and omits the field, and Nastech' local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `nastech model`."
                        )
                ...
                # ── Nastech Portal: record rate limit & skip retries ─────
                # When Nastech returns a 429 that is a genuine account-
                ...
                if (
                    ...
                    if _genuine_nastech_rate_limit:
                        ...
                        # top-of-loop Nastech guard handles fallback or
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Nastech may add API-only content not present in persisted
                    ...
                    # recovery arms on the TRUE request that overflowed. See nastech-lcm engine
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            pass
                        elif _provider == "nastech" and _print_nastech_entitlement_guidance(
                            agent,
                            "Nastech model access",
                        ):
                            pass
                        elif _provider in {"openai-codex", "xai-oauth", "nastech"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}   💡 Nastech Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: nastech portal", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Check your portal account: https://portal.nastechresearch.github.io", force=True)
                                # ``:free`` is OpenRouter slug syntax; Nastech Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Nastech Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Nastech catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.nastech/config.yaml` to extend the per-call "
                            f"timeout. (Nastech's built-in floor is 600s for "
                    ...
                    return {
                        ...
                        # descriptor (provider, billing_url, is_nastech, message).
        ...
        try:
            ...
            # with the iterations Nastech never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Nastech mid-turn, resume logic still sees the
                ...
                try:
                    ...
                    from nastech_cli.plugins import get_pre_verify_continue_message
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: nastech --resume <session-id>"
                )
