...
(``gateway/platforms/webhook.py``): inbound wakes Nastech when the world
changes; outbound tells the world when Nastech does something.
...
* Payloads are signed with HMAC-SHA256 (GitHub-style
  ``X-Nastech-Signature-256: sha256=<hexdigest>`` over the raw body) when
...
* No consent prompt: unlike shell hooks, an outbound target executes no
  ...
  in config.  ``NASTECH_SAFE_MODE=1`` still skips registration, matching
  plugins / MCP / shell hooks.
* Registration is idempotent — safe to invoke from both the CLI entry
  point and the gateway entry point.

Config schema (``~/.nastech/config.yaml``)::
    ...
    hooks:
      outbound:
        - url: https://ci.example.com/nastech-events
          ...
          secret_env: NASTECH_OUTBOUND_WEBHOOK_SECRET
          ...
          name: ci-notify   # optional label for logs / `nastech hooks list`
...
# nastech_cli.plugins.get_plugin_manager) — can register identical webhook
...
def register_from_config(cfg: Optional[Dict[str, Any]]) -> List[WebhookTarget]:
    ...
    if env_var_enabled("NASTECH_SAFE_MODE"):
        logger.info("NASTECH_SAFE_MODE=1 — outbound webhook registration skipped")
        return []
...
def _serialize_payload(
    ...
    ``delivery_id`` is shared with the ``X-Nastech-Delivery`` header so
...
def _build_delivery(
    ...
    if target.secret:
        ...
        headers["X-Nastech-Signature-256"] = f"sha256={digest}"
    return {
        "url": target.url,
        "label": target.label,
        "event": event,
