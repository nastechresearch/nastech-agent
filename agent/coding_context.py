...
In a code workspace on an interactive surface Nastech adopts a **coding posture**: a
...
from nastech_cli._subprocess_compat import bounded_git_probe
...
# Operating brief. Tool names referenced here are in the coding toolset and _NASTECH_CORE_TOOLS.
...
def _agent_config_value(config: Optional[dict[str, Any]], key: str, default: Any, *, readonly: bool) -> Any:
    ...
    if config is None:
        try:
            from nastech_cli.config import load_config, load_config_readonly
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        apply it only when the user hasn't pinned a selection (``--toolsets``, ``NASTECH_TUI_TOOLSETS``)."""
