...
# The Nastech gateway's own words for "the free tier will not serve this" — a billing wall for a
...
# Parameters Nastech sends on SOME routes only → hosts where that is deliberate.
...
# A reasoning-mandatory route answering ``reasoning: {enabled: false}`` (Nastech Portal + OpenRouter wording).
...
# 404: Nastech API surfaces credit depletion as a paid model vanishing from the
...
def _plugin_verdict(c: _Ctx) -> Optional[Verdict]:
    ...
    try:
        from nastech_cli.plugins import get_plugin_error_classification
...
def _nastech_welcome_tier(c: _Ctx) -> Optional[Verdict]:
    """The Nastech inference gateway's welcome-tier (free tier) refusals, read from the structured body.
    ...
    """
    from nastech_cli.anon_auth import (
    ...
    if not c.anonymous:
        ...
        if c.provider == "nastech" and status == 400 and welcome_route_refusal(status, c.msg) == "named_on_welcome_host":
...
def _provider_special_cases(c: _Ctx) -> Optional[Verdict]:
    ...
    welcome = _nastech_welcome_tier(c)
...
def classify_api_error(
    ...
    ``base_url`` (optional) is the route the call went to; the Nastech welcome tier keys its
...
def _off_route_host(c: _Ctx) -> str:
    ...
    from nastech_cli.route_identity import provider_owns_route
...
def _classify_400(c: _Ctx) -> Verdict:
    ...
    # Route rejecting a reasoning disable: a reasoning-mandatory route (GLM-5.3 on Nastech Portal /
