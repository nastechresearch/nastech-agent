"""Nastech Portal ``anthropic/*`` wire selection when ``nastech.anthropic_wire`` is ``auto``.

Portal serves Claude two ways and Nastech cannot tell which from the request: an OpenRouter
...
NastechResearch/api#227), so the session must ride chat/completions there. On GMI that is untested,
...
def maybe_switch_wire_after_first_response(agent: Any, response: Any, api_call_count: int) -> bool:
    ...
    Only for provider=nastech, anthropic/* models, ``nastech.anthropic_wire: auto``, and only on the
    session's first API call.
    """
    if api_call_count != 1 or getattr(agent, "_nastech_wire_decided", False):
        return False
    if (getattr(agent, "provider", "") or "").lower() != "nastech":
        return False
    model = str(getattr(agent, "model", "") or "")
    if not model.lower().startswith("anthropic/"):
        return False
    try:
        from nastech_cli.providers import _nastech_anthropic_wire
        if _nastech_anthropic_wire() != "auto":
            return False
    except Exception:
        return False
    agent._nastech_wire_decided = True  # one decision per session, whatever it is
    upstream = classify_upstream(response)
    want = wire_for_upstream(upstream)
    if want == getattr(agent, "api_mode", None):
        logger.debug("nastech wire auto: upstream=%s, staying on %s", upstream, want)
        return False
    agent._nastech_wire_pending = (want, upstream)
    return True
...
def apply_pending_wire_switch(agent: Any) -> bool:
    ...
    pending = getattr(agent, "_nastech_wire_pending", None)
    if not pending:
        return False
    agent._nastech_wire_pending = None
    want, upstream = pending
    try:
        from agent.agent_runtime_helpers import switch_model
        switch_model(agent, agent.model, "nastech", api_key=getattr(agent, "api_key", "") or "",
                     base_url=getattr(agent, "base_url", "") or "", api_mode=want)
    except Exception as exc:  # never let wire selection break a turn
        logger.warning("nastech wire auto: switch to %s failed (%s); staying on %s", want, exc, agent.api_mode)
        return False
    logger.info("nastech wire auto: upstream=%s -> %s for the rest of session %s", upstream, want,
