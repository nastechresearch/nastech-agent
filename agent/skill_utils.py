...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no nastech_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at NASTECH_HOME; absolute paths pass through."""
    from nastech_constants import get_nastech_home
    return p if p.is_absolute() else get_nastech_home() / p


# Never disableable: `nastech-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"nastech-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``NASTECH_PLATFORM`` / ``NASTECH_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("NASTECH_PLATFORM") or get_session_env("NASTECH_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``nastech config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to NASTECH_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to NASTECH_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_nastech_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.nastech/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.nastech/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".nastech", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (NASTECH_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under NASTECH_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`nastech skills trust`)
...
# high-confidence findings only. The scan cache lives under NASTECH_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from nastech_constants import get_nastech_home
        cache_dir = get_nastech_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped NASTECH_HOME), so normalization
    ...
    # ~/.nastech/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _nastech_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.nastech`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    nastech = metadata.get("nastech") if isinstance(metadata, dict) else None
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    nastech = _nastech_metadata(frontmatter)
    return {key: nastech.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.nastech.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _nastech_metadata(frontmatter).get("config")
...
def _expand_skill_config_path(value: str) -> str:
    """Expand ``~`` / ``$HOME`` against the HOME Nastech injects into tool subprocesses.

    Skill config defaults describe paths the agent hands to tools, so in a container where the
    control process HOME (``/opt/data``) differs from the tool HOME (``{NASTECH_HOME}/home``) a
