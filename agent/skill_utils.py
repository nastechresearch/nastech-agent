...
# Org mirrors live under ~/.nastech/skills/_org/<org_id>/. Resolution is
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``nastech_cli.config`` on the
...
# `nastech-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Nastech itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Nastech, so disable requests for these names are ignored
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``nastech config set`` and JSON-mode editor saves store lists as quoted
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    this directory instead of the profile-local ``~/.nastech/skills/``, and
    ...
    against NASTECH_HOME.  A value that resolves to the local skills dir is
...
def get_all_skills_dirs() -> List[Path]:
    ...
    NOTE: trusted project-local dirs (``./.nastech/skills`` at the git root) are
...
#   <root>/.nastech/skills/   — Nastech-native location
...
# a one-line "run `nastech skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when NASTECH_HOME itself lives inside a git checkout:
    ``<root>/.nastech/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``nastech skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`nastech skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under NASTECH_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_nastech_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.nastech/skills/`` (including via symlinks) or configured
    ...
    # follows the live profile-scoped NASTECH_HOME (the import-time
    ...
    # ~/.nastech/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Nastech can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    return {
        ...
        "session_platforms": nastech.get("session_platforms", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Nastech metadata, VCS, virtualenv/dependency, cache, and skill
