...
def _try_refresh_nastech_paid_entitlement_credentials(agent) -> bool:
    """Refresh Nastech runtime credentials after a fresh paid-entitlement check."""
    try:
        from nastech_cli.nastech_account import get_nastech_portal_account_info

        if get_nastech_portal_account_info(force_fresh=True).paid_service_access is not True:
            return False
        return agent._try_refresh_nastech_client_credentials(force=True)
    except Exception:
        return False
...
def _print_nastech_401_diagnostics(agent: Any, api_error: Exception) -> None:
    """Nastech 401 that survived a credential refresh: likely Portal OAuth expired/revoked,
    no credits, or agent key blocked."""
    from agent.conversation_loop import _print_nastech_entitlement_guidance
    from nastech_constants import display_nastech_home
    _body_text = ""
    try:
        ...
        pass
    _plines(agent, "🔐 Nastech 401 — Portal authentication failed.")
    if _body_text:
        _plines(agent, f"   Response: {_body_text}")
    try:
        from nastech_cli.anon_auth import is_anonymous_agent
        if is_anonymous_agent(agent):
            ...
            _plines(agent, "   Your session ended and Nastech couldn't start a new one.",
                    "   Sign in with a Nastech account (it's free), or switch providers with /model.")
            return
    except Exception:
        pass
    if not _print_nastech_entitlement_guidance(agent, "Nastech model access"):
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `nastech doctor` for credential-chain diagnostics, or",
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Nastech-managed OAuth/setup tokens",
        ...
        "     • Nastech login (OAuth): run 'nastech auth add anthropic' to sign in again, then retry",
        "     • Inspect what Nastech holds: nastech auth list anthropic",
        "     • Legacy cleanup: nastech config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: nastech config set ANTHROPIC_API_KEY \"\"",
    )
...
def _refresh_credentials_after_401(
    ...
    """Per-provider one-shot credential refresh on 401 (codex/xai, vertex, nastech, copilot,
    anthropic), printing user-facing diagnostics when the nastech/anthropic refresh fails.
    ...
    if (
        ...
        if agent._try_refresh_nastech_client_credentials(force=True):
            agent._buffer_vprint("🔐 Nastech agent key refreshed after 401. Retrying request...")
            return True
        _print_nastech_401_diagnostics(agent, api_error)
...
_WELCOME_ROUTE_HEAL_COPY = {
    ...
    "named_on_welcome_host": "Reconnected to your Nastech account's own route.",
}


def _recover_welcome_tier(agent: Any, classified: Any, _retry: TurnRetryState) -> bool:
    """Two one-shot repairs for the Nastech free tier, both silent on the wire and named once in chat.
    ...
    ``NASTECH_INFERENCE_BASE_URL`` falls straight through to the terminal copy.

    Reads the CLASSIFIER's context (``classified.error_context``): that is where
    ``_nastech_welcome_tier`` parks ``welcome_refusal`` / ``welcome_route``. The turn's other context
    ...
    if isinstance(refusal, dict) and refusal.get("reason") == "model_not_free" and not _retry.welcome_model_switch_attempted:
        ...
        if target and target != requested:
            try:
                agent.model = target
                agent._nastech_model_switch = (requested, target)
            except Exception:
                return False
            _vlines(agent, f"↪️  {requested} isn't available without signing in; using {target} for now. Retrying...")
            logger.info("%sNastech free tier: moved %s -> %s after model_not_free", agent.log_prefix, requested, target)
            return True
    ...
    if route in _WELCOME_ROUTE_HEAL_COPY and not _retry.welcome_route_heal_attempted:
        ...
        try:
            healed = bool(agent._try_refresh_nastech_client_credentials(force=True))
        except Exception:
            healed = False
        if healed:
...
def recover_after_classification(
    ...
    Nastech paid-entitlement refresh → credential-pool rotation → image shrink →
    ...
    Returns ``(retry_now, recovered_with_pool)``; the latter feeds the Nastech rate-limit guard."""
    from agent.conversation_loop import _is_nastech_inference_route
    ...
    if (
        ...
        if _try_refresh_nastech_paid_entitlement_credentials(agent):
            _vlines(agent, "🔐 Nastech paid access verified — refreshed runtime credentials and retrying request...")
            return True, False
    ...
    # Route rejecting a reasoning disable: a reasoning-mandatory route (Nastech Portal / OpenRouter,
    ...
    if (
        ...
        try:
            from nastech_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    from agent.conversation_loop import _print_billing_or_entitlement_guidance, _print_nastech_entitlement_guidance
    ...
    if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
        ...
        return
    if provider == "nastech" and _print_nastech_entitlement_guidance(agent, "Nastech model access"):
        return
    if provider in {"openai-codex", "xai-oauth", "nastech"} and status_code == 401:
        if provider == "openai-codex":
            ...
            _vlines(
                ...
                "      refreshed by another client (Codex CLI, VS Code) or another Nastech profile.",
            ...
            _vlines(
                ...
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `nastech model`.",
            )
        else:  # nastech
            _vlines(
                agent,
    ...
    _vlines(
        ...
        "      • Is the key valid? Run: nastech setup",
...
def _welcome_tier_guidance(classified: Any, *, model: Any, in_chat: bool, door: bool = True) -> str:
    """Copy for a Nastech free-tier refusal the classifier parsed (``welcome_refusal`` /
    ...
    if not refusal and not route:
        return ""
    from nastech_cli.anon_auth import welcome_refusal_copy, welcome_route_refusal_copy
    if refusal:
...
def _welcome_outage_copy(base_url: Any, classified: Any, *, anonymous: bool = False) -> str:
    """On the Nastech free tier, a transport / server failure that outlived every retry reads as one
    ...
    try:
        from nastech_cli.anon_auth import FREE_TIER_OUTAGE_COPY, route_is_welcome_host
...
def _missing_vendor_prefix_suggestion(api_error: Exception, provider: Any, model: Any) -> Optional[str]:
    ...
    if getattr(api_error, "status_code", None) != 404:
        return None
    try:
        from nastech_cli.model_normalize import suggest_prefixed_model_id
...
def nonretryable_client_error_result(
    ...
    if _welcome_hint:
        ...
        _vlines(agent, f"   💡 Fix: pick another model (/model), or check `{display_nastech_home()}/logs/agent.log`.")
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "      To route future blocks to another provider automatically: nastech fallback add",
        )
    ...
    if classified.reason == FailoverReason.ssl_cert_verification:
        _vlines(
            agent,
            "   💡 Nastech couldn't verify the provider's security certificate. This fails the same",
            ...
            "      • Missing/stale system CA store? Refresh it (in Nastech's venv: `uv pip install",
...
def max_retries_exhausted_result(
    ...
    result.update({
        ...
        # Present only for billing walls: (provider, billing_url, is_nastech, message).
...
class ClassifiedErrorVerdict:
    ...
    ``"break"`` (restart armed on ``_retry``), ``"continue"`` (re-enter the retry loop; Nastech
...
def _is_genuine_nastech_rate_limit(agent: Any, api_error: Exception, error_context: Any, classified: Any = None) -> bool:
    """Record a genuine account-level Nastech 429 to the cross-session breaker; upstream
    ...
    *error_context* is the turn's (``extract_api_error_context``); *classified* brings the
    ...
    try:
        from agent.nastech_rate_guard import (
            is_genuine_nastech_rate_limit, is_long_welcome_rate_limit, record_nastech_rate_limit)
        ...
        _genuine = (
            ...
            or is_genuine_nastech_rate_limit(headers=_err_hdrs, last_known_state=agent._rate_limit_state))
        if _genuine:
            _merged = {**(error_context if isinstance(error_context, dict) else {}), **_classified_ctx}
            record_nastech_rate_limit(headers=_err_hdrs, error_context=_merged, anonymous=anonymous)
        else:
            logger.info(
                "Nastech 429 looks like upstream capacity "
...
def route_classified_error(
    ...
    chain once; genuine Nastech 429 → cross-session breaker + re-enter the loop exactly once."""
    ...
    # Nastech Portal: a genuine account-level 429 is recorded to a shared file so ALL
    # sessions back off; is_genuine_nastech_rate_limit excludes upstream 429s.
    if (
        is_rate_limited
        and agent.provider == "nastech"
        and classified.reason == FailoverReason.rate_limit
        and not recovered_with_pool
        and _is_genuine_nastech_rate_limit(agent, api_error, error_context, classified)
    ):
        # Re-enter the loop exactly once so the top-of-loop Nastech guard runs
