...
def record_response_usage(
    ...
    # nastech.anthropic_wire=auto: the session's wire is decided once, from this first response.
    if agent.session_api_calls == 1 and (agent.provider or "") == "nastech":
        with suppress(Exception):
            from agent.nastech_wire import maybe_switch_wire_after_first_response
