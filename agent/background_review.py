...
def _parent_can_emit_tool_calls(agent: Any) -> bool:
    ...
    calls. A provider that IS an autonomous agent reaches Nastech through a client
    shim, and a shim that cannot carry Nastech tool calls back turns the fork into
    ...
    exists so a shim that can't declares ``SUPPORTS_NASTECH_TOOL_CALLS = False``
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • PINNED skills (marked via 'nastech curator pin'). You are an "
    ...
    "'nastech curator adopt <name>' — do not try to patch it.\n"
...
_COMBINED_REVIEW_PROMPT = (
    ...
    "  • Hub-installed skills (installed via 'nastech skills install').\n"
    "  • Skills in skills.external_dirs (externally owned).\n"
    "  • PINNED skills (marked via 'nastech curator pin'). Pin blocks "
    ...
    "reply and recommend 'nastech curator adopt <name>' instead.\n"
    "If the only skills that need updating are protected, say\n"
    "'Nothing to save.' and stop.\n\n"
...
def _run_review_in_thread(
    ...
    # An agent-as-provider whose client can't carry Nastech tool calls back would
