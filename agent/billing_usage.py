...
unreadable at terminal widths). Source: ``NastechPortalAccountInfo.paid_service_access_info``
...
def nastech_logged_in() -> bool:
    """Cheap local auth-state check: a Nastech access token is present. Fail-closed."""
    try:
...
def fetch_nastech_account(timeout: float):
...
def usage_model_from_account(account_info: Any) -> UsageModel:
    """Build a :class:`UsageModel` from a ``NastechPortalAccountInfo``. Never raises."""
    try:
...
def build_usage_model(*, timeout: float = 10.0) -> UsageModel:
    """Fetch account-info and build the usage model; fail-open. ``NASTECH_DEV_CREDITS_FIXTURE`` short-circuits to a fixture."""
    fixture = _dev_fixture_usage_model()
    if fixture is not None:
        return fixture
    if not nastech_logged_in():
        return UsageModel(available=False)
    try:
        return usage_model_from_account(fetch_nastech_account(timeout))
    except Exception:
...
def _dev_fixture_usage_model() -> Optional[UsageModel]:
    """``NASTECH_DEV_CREDITS_FIXTURE`` -> fixture model (``free|healthy|low|topup|depleted``), else None."""
    name = (os.getenv("NASTECH_DEV_CREDITS_FIXTURE") or "").strip().lower()
