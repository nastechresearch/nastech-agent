...
def _moa_privacy_mode(moa_raw: Any) -> str:
    ...
    from nastech_cli.moa_config import coerce_privacy_filter
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    # nastech_home_key() in the key: the resolved api_key/base_url are per-profile, and under a
    ...
    cache_key = (nastech_home_key(), provider, model)
    now = time.monotonic()
...
def _run_references_parallel(
    ...
    *agent*, the wait polls every ``_REFERENCE_POLL_INTERVAL_S`` so a user interrupt
    ...
    # Propagate the turn's contextvars (approval callbacks, Nastech conversation tag).
