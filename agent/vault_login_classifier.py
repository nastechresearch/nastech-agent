...
INSPECTION_STAMP_ATTR = "data-nastech-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-nastech-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-nastech-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-nastech-vault-slot]").forEach((n) => n.removeAttribute("data-nastech-vault-slot"));
