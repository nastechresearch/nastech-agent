...
_INITIALIZE_PARAMS = {
    ...
    "clientInfo": {"name": "nastech-agent", "title": "Nastech Agent", "version": "0.0.0"},
}
_DEPRECATED_CLI_ERROR = (
    "Nastech ACP mode requires the NEW GitHub Copilot CLI (github.com/github/copilot-cli), but the binary it just "
...
def _resolve_command() -> str:
    return os.getenv("NASTECH_COPILOT_ACP_COMMAND", "").strip() or os.getenv("COPILOT_CLI_PATH", "").strip() or "copilot"


def _resolve_args() -> list[str]:
    return shlex.split(os.getenv("NASTECH_COPILOT_ACP_ARGS", "").strip()) or ["--acp", "--stdio"]
...
def _format_messages_as_prompt(
    ...
    # one. Copilot has no tools of its own that collide with Nastech', so forward the whole toolset.
...
class CopilotACPClient:
    ...
    NASTECH_SKIP_TRANSPORT_WRAP = True
    NASTECH_SKIP_ASYNC_WRAP = True
    ...
    def _spawn(self) -> subprocess.Popen[str]:
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                "NASTECH_COPILOT_ACP_COMMAND / NASTECH_COPILOT_ACP_ARGS to a working pair."
            )
        try:
            from nastech_cli._subprocess_compat import windows_hide_flags  # hide the Windows console flash (#56747); pipes intact for the ACP wire
            ...
            raise RuntimeError(f"Could not start Copilot ACP command '{self._acp_command}'. Install GitHub Copilot CLI or set "
                               "NASTECH_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.") from exc
    ...
    def _handle_server_message(
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(message_id, -32601, f"ACP client method '{method}' is not supported by Nastech yet.")
