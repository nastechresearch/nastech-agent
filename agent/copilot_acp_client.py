...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    NASTECH_COPILOT_ACP_ARGS transport is the operator's business.
    """
    if "--acp" not in args:
        return True
...
def _format_messages_as_prompt(
    ...
    # Copilot has no tools of its own that would collide with Nastech', so it
        ...
        *,
        ...
        if _acp_supported(self._acp_command, self._acp_args) is False:
            ...
            raise RuntimeError(
                ...
                f"NASTECH_COPILOT_ACP_COMMAND / NASTECH_COPILOT_ACP_ARGS "
                f"to a working pair."
            )

        # Note the model Nastech selected; it is applied after session/new via
        ...
        try:
            ...
            # Select the model Nastech asked for. Prefer the stable ACP v1
