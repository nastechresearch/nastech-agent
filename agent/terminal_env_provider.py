...
Providers live in ``~/.nastech/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.nastech/cache`` files land
  inside the backend (e.g. ``"~/.nastech"`` for home-synced backends,
  ``"/root/.nastech"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_nastech_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Nastech cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``nastech setup`` after this backend is selected.
