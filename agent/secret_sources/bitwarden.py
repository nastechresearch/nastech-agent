...
Pulls API keys from BSM at startup so they need not live in ``~/.nastech/.env``.
``bws`` is auto-installed into ``<nastech_home>/bin/bws`` (one pinned version,
...
# <nastech_home>/cache/bws_cache.json holds only secret VALUES (never the access
...
_ENCRYPTED_CACHE_INFO = b"nastech-bws-encrypted-cache-v1"
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    """Managed ``<nastech_home>/bin/bws`` first, then PATH, then optional auto-install."""
...
class BitwardenSource(SecretSource):
    ...
    _AUTH_HINT = (
        "Run `nastech secrets bitwarden token` to paste a fresh access "
        "token (create one in the Bitwarden web app: Secrets Manager → "
        "Machine accounts → Access tokens).  Wrong region?  Re-run "
        "`nastech secrets bitwarden setup` and pick EU/self-hosted."
    )
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        if not access_token:
            return result.fail(f"secrets.bitwarden.enabled is true but {access_token_env} is "
                               "not set.  Run `nastech secrets bitwarden setup`.", ErrorKind.NOT_CONFIGURED)
        project_id = str(cfg.get("project_id") or "")
        if not project_id:
            return result.fail("secrets.bitwarden.project_id is empty.  Run `nastech secrets bitwarden setup`.",
        ...
        if binary is None:
            return result.fail("bws binary not available and auto-install is disabled.  "
                               "Run `nastech secrets bitwarden setup` to install.", ErrorKind.BINARY_MISSING)
