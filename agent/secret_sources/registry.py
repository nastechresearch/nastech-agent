...
def get_source(name: str, *, scope: Optional[str] = None) -> Optional[SecretSource]:
    ...
    with _REGISTRY_LOCK:
        return _SCOPED_SOURCES.get(scope or nastech_home_key(), {}).get(
            name
...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``nastech`` invocation.
    """
...
def apply_all(secrets_cfg: dict, home_path: Path,
    ...
    enabled = _ordered_enabled_sources(
        secrets_cfg, scope=nastech_home_key(home_path)
    )
    if not enabled:
        return report
