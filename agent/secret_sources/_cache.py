...
a cache problem must never block Nastech startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_nastech_dotenv()``, else ``$NASTECH_HOME``/``~/.nastech``."""
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<nastech_home>/cache/<basename>``::
