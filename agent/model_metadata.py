...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``NASTECH_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
...
def _get_endpoint_metadata_cache_path() -> Path:
    ...
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "cache" / "endpoint_model_metadata.json"
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    (``nastech -q``, cron, every Bot Mode DM hop) start cold and re-probed the
    live ``/models`` endpoint on every launch — 0.3–0.6s of pure network per
    process on Nastech, whose persistent context cache is bypassed by design so
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "qwen3.8-flash": 1_000_000,   # 1M context (OpenRouter & Nastech portal, verified 2026-08-28)
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.nastechresearch.github.io": "nastech",
...
# who never asked for it. The variant suffix is a Nastech-side alias: it is
...
# Nastech-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a Nastech picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are Nastech picker aliases — the Codex catalog only
