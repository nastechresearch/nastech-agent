...
def _routermint_headers() -> dict:
    ...
    return {"User-Agent": f"NastechAgent/{_NASTECH_VERSION}"}
...
_ROUTE_DEFAULT_HEADERS = (
    ...
    ("githubcopilot.com", lambda self, url: _lazy_attr("nastech_cli.models", "copilot_default_headers")()),
    ...
    ("x.ai", lambda self, url: _lazy_attr("tools.xai_http", "nastech_xai_default_headers")()),
)
...
class ClientLifecycleMixin:
    ...
    def _try_refresh_nastech_client_credentials(self, *, force: bool = True, require_account: str | None = None) -> bool:
        # Portal serves anthropic/* on the native Messages route, so either client kind may hold the expiring JWT.
        if self.provider != "nastech" or self.api_mode not in ("chat_completions", "anthropic_messages"):
            return False
        try:
            from nastech_cli.auth import resolve_nastech_runtime_credentials
            timeout = env_float("NASTECH_NASTECH_TIMEOUT_SECONDS", 15)
        ...
        if require_account is not None:
            ...
            if str(new_account or "") != require_account:
                logger.info(
                    "Nastech pre-expiry adoption skipped: the store's key belongs to a different account "
        ...
        return self._adopt_openai_credentials(api_key, base_url, reason="nastech_credential_refresh")
    ...
    _NASTECH_KEY_ADOPT_SKEW_S = 180

    def _adopt_nastech_key_before_expiry(self) -> bool:
        """Swap in a fresh Nastech agent key BEFORE the one in hand expires, so the request never 401s.
        ...
        """
        if getattr(self, "provider", "") != "nastech" or not getattr(self, "api_key", None):
            return False
        try:
            from nastech_cli.auth_constants import _decode_jwt_claims
        ...
        if not account or not isinstance(exp, (int, float)) or exp - time.time() > self._NASTECH_KEY_ADOPT_SKEW_S:
            return False
        return self._try_refresh_nastech_client_credentials(force=False, require_account=str(account))
    ...
    def _resolve_env_credentials(self) -> Optional[tuple]:
        ...
        if pconfig and getattr(pconfig, "auth_type", "") == "api_key" and getattr(pconfig, "api_key_env_vars", ()):
            ...
            if self.provider == "actual":
                ...
                from nastech_cli.runtime_provider import _config_base_url_for_provider, _get_model_config
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ``~/.nastech/.env`` credential/base-url edits at the turn boundary (a Settings save updates ``.env``
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        if not self._is_copilot_provider():
            return False
        try:
            from nastech_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token, evict_cached_exchanged_token
    ...
    def _apply_client_headers_for_base_url(self, base_url: str, *, apply_user_headers: bool = True) -> None:
        ...
        if self.api_mode not in ("anthropic_messages", "bedrock_converse"):
            try:
                from nastech_cli.config import apply_custom_provider_extra_headers_to_client_kwargs
    ...
    def _swap_credential(self, entry) -> bool:
        ...
        from nastech_cli.anon_auth import route_can_serve_model
