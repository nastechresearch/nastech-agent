...
_API_CLIENT = f"nastech-agent/{_NASTECH_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Nastech typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.nastech/.env, "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.nastech/.env (get one at https://aistudio.google.com/app/apikey), or run `nastech setup` to "
...
class GeminiNativeClient:
    ...
    # (No NASTECH_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
