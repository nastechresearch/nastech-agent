...
_SUMMARY_ROUTE_PIN: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("nastech_summary_route_pin", default=None)
)
...
_SUMMARY_ROUTE_CONSUMED: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("nastech_summary_route_consumed", default=None)
)
...
# heuristics. See https://github.com/NastechResearch/nastech-agent/issues/38389
#
...
# re-accesses the compacted region. Nastech already persists every
...
def _build_recovery_footer(session_id: str, region_len: int) -> str:
    ...
    Nastech persists every pre-compaction message in state.db; session_search
...
class ContextCompressor(ContextEngine):
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates so Nastech compresses before a provider rejects the
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def compress(
        ...
        # ``nastech kanban`` worker seeded with a single short
