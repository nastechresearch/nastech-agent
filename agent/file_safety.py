...
def _constants_path(getter_name: str) -> Path:
    """Call ``nastech_constants.<getter_name>()`` (local import avoids cycles); ``~/.nastech`` on any failure."""
    try:
        import nastech_constants

        return getattr(nastech_constants, getter_name)()
    except Exception:
        return Path(os.path.expanduser("~/.nastech"))


def _nastech_home_path() -> Path:
    """Active NASTECH_HOME (profile-aware). Tests monkeypatch this name."""
    return _constants_path("get_nastech_home")


def _nastech_root_path() -> Path:
    """Nastech root dir (parent of any profile, never per-profile)."""
    return _constants_path("get_default_nastech_root")


def _nastech_dirs() -> list[Path]:
    """Resolved active NASTECH_HOME and global root, deduplicated.

    Both are checked so credential stores at <root>/... stay guarded when
    running under a profile (NASTECH_HOME = <root>/profiles/<name>).
    """
    return list(dict.fromkeys(_resolve_each((_nastech_home_path(), _nastech_root_path()))))
...
def _guard_homes(path: str = "") -> set[str]:
    ...
    containers, and spawned workers pin ``HOME`` to ``{NASTECH_HOME}/home``, which leaves
    ...
    with suppress(Exception):
        from nastech_constants import get_real_home, get_subprocess_home, _profile_home_path
...
# they are a routine local form (see nastech_cli/windows_ssh_runtime.py) and
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    # Secret material under NASTECH_HOME, on both the active profile and the global
    ...
    paths = [
        ...
        *(str(base / f) for f in nastech_files for base in (_nastech_home_path(), _nastech_root_path())),
...
def get_safe_write_roots() -> set[str]:
    """Resolved NASTECH_WRITE_SAFE_ROOT paths (``os.pathsep``-separated list)."""
    roots: set[str] = set()
    for path in filter(None, os.getenv("NASTECH_WRITE_SAFE_ROOT", "").split(os.pathsep)):
...
# NASTECH_HOME / root subpaths that the agent's generic file tools must not
...
_NASTECH_PROTECTED_SUBPATHS = ("state.db", "sessions", "mcp-tokens", "pairing", "vault", "browser-profile")
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    for base in _nastech_dirs():
        for sub in _NASTECH_PROTECTED_SUBPATHS:
            with suppress(Exception):
...
# Exact-file credential stores under NASTECH_HOME / <root>. The agent never
...
# Directory-prefix read denies under NASTECH_HOME / <root>: (subdir, message for
...
_READ_DENIED_DIRS = (
    ("mcp-tokens",
     "is the Nastech MCP token directory and cannot be read directly.",
     "is a Nastech MCP token file and cannot be read directly."),
    ("browser-profile",
     "is the Nastech real-profile browser snapshot directory (copied cookies/logins) and cannot be read directly.",
     "is inside the Nastech real-profile browser snapshot (copied cookies/logins) and cannot be read directly."),
    # vault.key + vault.json.enc sit side by side; key + ciphertext = plaintext, so the whole dir is one credential.
    ("vault",
     "is the Nastech credential vault directory and cannot be read directly (secrets are filled server-side by browser_vault_fill).",
     "is inside the Nastech credential vault (encrypted secrets + local key) and cannot be read directly (browser_vault_fill resolves them server-side)."),
)
...
def get_read_block_error(path: str) -> Optional[str]:
    ...
    stores under NASTECH_HOME and the global root (exact files, plus anything
    ...
    nastech_dirs = _nastech_dirs()
    reason = None
    if any(_is_under(resolved, hd / "skills" / ".hub") for hd in nastech_dirs):
        reason = (
            "is an internal Nastech cache file and cannot be read directly to prevent "
            "prompt injection. Use the skills_list or skill_view tools instead."
        )
    elif any(resolved in _resolve_each(hd / name for hd in nastech_dirs) for name in _CREDENTIAL_FILE_NAMES):
        reason = (
            "is a Nastech credential store and cannot be read directly. Provider tools "
            "consume these credentials through internal channels." + _DID_SUFFIX
        )
    else:
        for subdir, dir_msg, file_msg in _READ_DENIED_DIRS:
            for blocked_dir in _resolve_each(hd / subdir for hd in nastech_dirs):
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Nastech read (see ``get_read_block_error``).
...
def _resolve_active_profile_name() -> str:
    """Active profile name from NASTECH_HOME: ``~/.nastech`` -> ``"default"``,
    ``~/.nastech/profiles/X`` -> ``"X"``; ``"default"`` on any resolution failure."""
    try:
        parts = _nastech_home_path().resolve().relative_to(_nastech_root_path().resolve() / "profiles").parts
...
def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Nastech state: ``None``
    for non-mirror paths, else ``target_path`` (resolved), ``mirror_root`` (the
    ``…/home/.nastech`` prefix) and ``inner_path`` (what the agent meant on the host)."""
    target = _resolve_target(path)
    parts = target.parts if target is not None else ()
    # Need at least: sandboxes / <backend> / <task> / home / .nastech / <thing>; inner_idx = the .nastech part.
    inner_idx = next(
        (i + 4 for i, part in enumerate(parts)
         if part == "sandboxes" and i + 5 < len(parts) and parts[i + 3] == "home" and parts[i + 4] == ".nastech"),
        None,
    )
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "Writes here land on a copy that the host Nastech process never reads — the "
        "authoritative file is likely {inner_path!r} under the real NASTECH_HOME.",
...
def classify_container_mirror_target(path: str, mirror_prefix: str | None = None) -> Optional[dict]:
    ...
    the bind mount strips the ``sandboxes/`` prefix (the agent sees plain ``/root/.nastech/…``),
...
def get_container_mirror_warning(path: str, mirror_prefix: str | None = None) -> Optional[str]:
    ...
    return _mirror_warning(
        ...
        "the container's bind-mounted home — a per-task mirror that the host Nastech "
        "process never reads. The authoritative file is {inner_path!r} under "
        "the real NASTECH_HOME.",
