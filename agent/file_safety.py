...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    return {
        ...
        for p in [
            ...
            str(nastech_home / "cache" / "bws_cache.enc.json"),
            str(nastech_root / "cache" / "bws_cache.enc.json"),
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    for hd in nastech_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Nastech real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Nastech real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Nastech read (see
