...
A host that mints one physical ``session_id`` per RESPONSE (Nastech Studio's
...
Nastech sends — ``prompt_cache_key`` on both OpenAI-wire transports, plus the
OpenRouter/Nastech sticky ``session_id`` and xAI's ``x-grok-conv-id`` through
...
Nastech must not infer the logical conversation from the id's SYNTAX (that
...
``X-Nastech-Session-Key`` header, which branching deliberately does NOT key
...
def _agent_source(
    ...
    :meth:`~nastech_state.SessionDB.declared_scope_identity` still takes.
    ...
    diverge whenever ``NASTECH_SESSION_SOURCE`` overrides the platform, and the
...
def declared_conversation_scope(agent: Any) -> Optional[str]:
    ...
    Resolved from ``agent._gateway_session_key`` (the ``X-Nastech-Session-Key``
