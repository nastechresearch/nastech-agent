...
def list_providers(*, scope: Optional[str] = None) -> List[ImageGenProvider]:
    ...
    with _lock:
        ...
        merged.update(_scoped_providers.get(scope or nastech_home_key(), {}))
...
def get_provider(name: str, *, scope: Optional[str] = None) -> Optional[ImageGenProvider]:
    ...
    with _lock:
        key = name.strip()
        return _scoped_providers.get(scope or nastech_home_key(), {}).get(key) or _providers.get(key)
...
def get_active_provider() -> Optional[ImageGenProvider]:
    ...
    **Availability semantics** (mirrors :mod:`agent.web_search_registry`):
    ...
    # The managed "Nastech Subscription" selection is serviced by the FAL
    # plugin through the managed fal-queue gateway (the legacy FAL pipeline
    # routes managed when the stored selection is "nastech").
    if configured:
        try:
            from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER

            if configured.lower() == NASTECH_MANAGED_PROVIDER:
                configured = "fal"
    ...
    with _lock:
        ...
        snapshot.update(_scoped_providers.get(nastech_home_key(), {}))
