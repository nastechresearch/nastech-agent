...
``nastech auth remove <provider> <N>`` must make the entry stay gone across
...
EXTERNAL_LOGINS_NOT_ADOPTED_NOTICE = (
    ...
    "Nastech uses only its own logins; run `nastech auth add <provider>` to add one."
)
...
def adopt_external_logins_enabled() -> bool:
    ...
    Codex and Claude OAuth refresh tokens are single-use and rotate, so once Nastech borrows a CLI's
    token pair the two programs hold one token family and whichever refreshes first logs the other
    out. When the user opts out, Nastech never reads or refreshes those files and says so once per
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.nastech/.env; hint when the shell exports it."""
    ...
    # .env as utf-8-sig like nastech_cli/config.py: a BOM-sensitive read would
...
def _remove_auth_store_oauth(provider: str, removed) -> RemovalResult:
    """Clear auth.json ``providers.<provider>`` (nastech, minimax-oauth, xai-oauth, openai-codex).
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        provider="nastech", source_id="device_code",
        remove_fn=_remove_auth_store_oauth,
        description="auth.json providers.nastech",
    ),
    RemovalStep(
