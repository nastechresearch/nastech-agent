...
def try_recover_primary_transport(
    ...
    transport error. Skipped for aggregators (OpenRouter, Nastech) that manage retries server-side."""
    ...
    if (
        (agent.provider or "").strip().lower() in {"nastech", "nastech-portal", "nastechresearch"}
...
def dump_api_request_debug(
    ...
    try:
        ...
        # Sanitize the session ID (may come from an untrusted X-Nastech-Session-Id header) so a
...
def anthropic_prompt_cache_policy(
    ...
    if supports_cache_markers and (
        ...
        try:
            from nastech_cli.config import get_custom_provider_model_capability
    ...
    if (is_openrouter or is_nastech_portal) and (is_claude or is_kimi) and not is_anthropic_wire:
        return True, False
    # Nastech Portal Qwen takes the envelope path too; the alibaba-family check below only matches
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # before the built-in ladder so a profile registered from ~/.nastech/plugins/ or a pip entry
    ...
    # Delegate all rate-limit / 5xx retry to nastech's outer conversation loop, which honors Retry-After and
    ...
    # All primary construction and recovery paths must identify Nastech to the official Codex
...
def _resolve_switch_destination(agent, new_model, new_provider, base_url, api_mode, capabilities, old_norm, new_norm):
    ...
    from nastech_cli.providers import determine_api_mode, is_actual_route
    from agent.native_compaction import resolve_native_compaction_capabilities
    from nastech_cli.models import opencode_provider_family
    # Pass model so dual-wire providers (Nastech Portal anthropic/* -> Messages) resolve correctly.
    if not api_mode:
...
def _pre_tool_block_message(agent, function_name, function_args, effective_task_id, tool_call_id, middleware_trace):
    ...
    try:
        from nastech_cli.plugins import _dispatch_pre_tool_call_hooks
...
def _session_id_for_heal_log() -> str:
    try:
        from nastech_logging import _session_context
        ...
        if escalate:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `nastech "
...
def _realign_tool_result_names(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #   the OpenAI-compat serializer keeps it provider-agnostic: Gemini reaches Nastech under many model
...
def _iter_pool_sockets(client: Any):
    ...
    if not pools:
        return
    from agent.process_bootstrap import NASTECH_TRANSPORT_OWNER_EXT
    seen: set[int] = set()
    for pool, owner in pools:
        ...
        for pool_req in list(getattr(pool, "_requests", None) or []):
            if owner is not None:
                ...
                if exts.get(NASTECH_TRANSPORT_OWNER_EXT) != owner:
                    continue
