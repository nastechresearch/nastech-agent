...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    # that are canonicalized only after Nastech sends the request. Honor its
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # built-in ladder so a profile registered from ~/.nastech/plugins/ or a pip
    ...
    if agent.provider == "opencode-free":
        from nastech_cli.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify Nastech to the
...
def get_sanitizer_heal_stats() -> Dict[str, Dict[str, Any]]:
    ...
    Surfaced by diagnostics (``nastech doctor`` / debug share callers) so
...
def _log_empty_non_final_heal(healed: int) -> None:
    ...
    / ``nastech doctor`` — once per session, never re-armed by a new window.
    """
    key = _session_id_for_heal_log() or "-"
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # provider-agnostic: Gemini reaches Nastech under many model strings and
