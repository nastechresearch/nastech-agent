...
Every tool-provider ABC shares the same identity + ``nastech tools`` picker surface.
...
class ProviderBase(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Provider row for the ``nastech tools`` picker.
...
class CatalogProviderBase(ProviderBase):
    ...
    def is_available(self) -> bool:
        ...
        ``nastech setup`` call it on every paint.
        """
        return True
