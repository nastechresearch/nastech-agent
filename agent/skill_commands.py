...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_nastech_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_home() -> str:
    """Return the effective Nastech home the skill scan should be scoped to.

    A gateway session can switch between profiles that each carry their own
    ``skills.external_dirs`` (via ``set_nastech_home_override``), but the
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        # launch home, so a multiplexed profile scope (set_nastech_home_override)
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Nastech slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Nastech command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Nastech home changes (e.g. Desktop switching profiles
