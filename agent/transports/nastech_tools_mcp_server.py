...
Codex owns the loop and tool list there, so a curated subset of Nastech tools is
exposed over stdio MCP; codex registers it via ``~/.codex/config.toml
[mcp_servers.nastech-tools]``. Run: ``python -m agent.transports.nastech_tools_mcp_server``.
"""
...
NASTECH_TOOLS_MCP_SERVER_NAME = "nastech-tools"
...
# Each name MUST match a registered Nastech tool ``model_tools.handle_function_call()`` can dispatch.
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    # Kanban handoff tools: stateless (read NASTECH_KANBAN_TASK, write kanban.db).
    ...
    # Orchestrator-only (the kanban tool gates them on NASTECH_KANBAN_TASK unset).
    "kanban_create", "kanban_unblock", "kanban_link",
)


def _build_server() -> Any:
    """Create the MCP server with Nastech tools attached (lazy imports: importable without ``mcp``)."""
    try:
        # mcp 2.0 renamed `mcp.server.fastmcp` to `mcp.server.MCPServer` (same surface).
        from mcp.server import MCPServer
    except ImportError as exc:  # pragma: no cover - install hint
        raise ImportError(f"nastech-tools MCP server requires the 'mcp' package: {exc}") from exc

    from model_tools import get_tool_definitions, handle_function_call

    mcp = MCPServer(
        NASTECH_TOOLS_MCP_SERVER_NAME,
        instructions=(
    ...
    # Authoritative Nastech schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Nastech process", name)
            continue
    ...
    logger.info("nastech-tools MCP server registered %d/%d tools", exposed_count, len(EXPOSED_TOOLS))
    return mcp
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Keep Nastech' own banners off stdout (the MCP wire).
