...
# nastech_<name> and map them back before local dispatch.
...
# (nastech_<name>), map back in normalize_response so Nastech dispatch is unaffected. See #85589.
...
def _alias_wire_tools(response_tools: Any, params: dict[str, Any], is_xai_responses: bool) -> tuple[Any, dict[str, str]]:
    ...
    swaps it 1:1 for the built-in, client mode keeps Nastech dispatch under an alias.
    """
    ...
    # Keep Nastech's client-side functions available under wire aliases.
    ...
    # xAI server-side web search vs Nastech web providers. grok models on xAI's /v1/responses surface have a
    ...
    # or resolved): keep Nastech dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
    # the wire tool to ``nastech_web_search`` so Grok cannot hijack the name. The alias is mapped back to
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        kwargs = {
            # ``-900k`` picker variants are Nastech-side aliases; the backend knows only the base slug.
            "model": wire_model,
