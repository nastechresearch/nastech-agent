...
# Wire-name used when Nastech keeps client-side web_search on xAI Responses.
...
# avoids that while still dispatching through Nastech's configured provider
# (Firecrawl / Exa / …). Mapped back to ``web_search`` in normalize_response.
_XAI_CLIENT_WEB_SEARCH_ALIAS = "nastech_web_search"
...
# rename on the wire (nastech_<name>), map back in normalize_response so
# Nastech dispatch is unaffected.
...
# Nastech's progressive-disclosure bridge registers exactly that literal
...
_RESERVED_TOOL_ALIAS_PREFIX = "nastech_"
...
# named ``nastech_tool_search`` is never silently rewritten to ``tool_search``
...
def _alias_reserved_tools(
    ...
    a legitimate user/plugin/MCP tool already named ``nastech_<x>`` is
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # xAI server-side web search vs Nastech web providers.
        #
        ...
        #    keep Nastech dispatch so ``web.backend`` / ``web.search_backend``
        #    is honored, but rename the wire tool to
        #    ``nastech_web_search`` so Grok cannot hijack the name. The alias
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        ...
        if msg and msg.tool_calls:
            ...
            for tc in msg.tool_calls:
                ...
                # Undo THIS request's wire aliases before Nastech dispatch.
                ...
                # ``nastech_tool_search`` etc. is dispatched as itself when
