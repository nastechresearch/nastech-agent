...
# Nastech' tools.terminal.security_mode -> Codex permissions profile id.
...
class CodexAppServerSession:
    """One Codex thread per Nastech session, lifetime owned by AIAgent. Not thread-safe: one caller at a time."""
    ...
    def __init__(
        ...
        self._permission_profile = permission_profile or _NASTECH_TO_CODEX_PERMISSION_PROFILE.get(
            os.environ.get("NASTECH_TERMINAL_SECURITY_MODE", "auto"), "workspace-write"
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(client_name="nastech", client_title="Nastech Agent", client_version=_get_nastech_version())
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Answer a codex server request (approval / elicitation) via Nastech' approval flow.
    ...
    def _respond_elicitation(self, params: dict) -> dict:
        """MCP elicitation: auto-accept our own nastech-tools server (opted in by enabling the runtime;
        exposes nothing codex's shell can't do); decline others so the user opts in via codex's own flow."""
        action = "accept" if (params.get("serverName") or "") == NASTECH_TOOLS_MCP_SERVER_NAME else "decline"
...
# Nastech approval choice -> codex decision (app-server-protocol v2). "deny" and
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map a Nastech approval choice onto codex's approval decision wire value."""
