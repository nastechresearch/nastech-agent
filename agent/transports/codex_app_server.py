...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # codex needs LLM provider creds but must not receive Tier-1 Nastech secrets (gateway/GitHub/infra tokens).
        ...
        # `os.environ.copy()` also handed it every Tier-1 Nastech secret — gateway bot tokens, GitHub auth,
        ...
        # Native shell children remain unowned. Only Nastech' managed MCP tool
        ...
        owned_task = os.environ.get("NASTECH_KANBAN_TASK") and is_dispatcher_owned_worker_context()
        if owned_task:
            for key in (*KANBAN_ENV_KEYS, "NASTECH_KANBAN_DB", "NASTECH_KANBAN_BOARD"):
                if key in os.environ:
                    cmd += ["-c", f"mcp_servers.{NASTECH_TOOLS_MCP_SERVER_NAME}.env.{key}={json.dumps(os.environ[key])}"]
            cmd += ["-c", f'mcp_servers.{NASTECH_TOOLS_MCP_SERVER_NAME}.env.{DELEGATED_CHILD_ENV_MARKER}=""']
        ...
        if owned_task:
            ...
            default_root = os.path.join(spawn_env.get("NASTECH_HOME", os.path.expanduser("~/.nastech")), "kanban")
            kanban_root = os.path.dirname(kanban_db) if kanban_db else spawn_env.get("NASTECH_KANBAN_ROOT", default_root)
            cmd += [
    ...
    def initialize(
        self, client_name: str = "nastech", client_title: str = "Nastech Agent",
