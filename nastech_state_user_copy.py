...
_DOCTOR = "Run `nastech {profile_arg}doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another Nastech process",
        "Wait a moment and try again; if it persists, stop the other Nastech process "
        "(`nastech {profile_arg}gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "fts_index": (
        ...
        "Run `nastech {profile_arg}doctor --fix` (or `nastech {profile_arg}sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
    ...
    "turn_lease": (
        "storage_busy",
        "another Nastech process took over this session",
