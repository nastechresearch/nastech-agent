...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["NASTECH_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(NastechTokenStorage("positive", nastech_home=owner).get_tokens())
        ...
        override = set_nastech_home_override(other)
        try:
...
def main():
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="nastech-oauth-http-") as temp:
            home = Path(temp)
            (home / ".nastech").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "NASTECH_HOME": str(home / ".nastech"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
