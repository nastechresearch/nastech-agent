...
def main():
    ...
    if not args.child:
        with tempfile.TemporaryDirectory(prefix="kanban-identity-") as home:
            env = {"HOME": home, "NASTECH_HOME": home + "/nastech", "PATH": os.defpath,
    ...
    from nastech_cli import kanban_db as kb, kanban_db_connect as kbc
    from nastech_cli.kanban_db_dispatch import dispatch_once
    from tools.kanban_tools import _handle_create
    from nastech_cli.kanban_decompose import _apply_fanout, _Routing
    graph = importlib.import_module("nastech_cli.kanban_db_graph") if (args.repo / "nastech_cli/kanban_db_graph.py").exists() else kb
