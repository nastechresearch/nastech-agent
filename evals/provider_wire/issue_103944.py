...
os.environ.clear()
os.environ.update(
    HOME=home, NASTECH_HOME=home + "/.nastech", PATH="/usr/bin:/bin", NO_PROXY="*"
)
os.chdir(home)
...
assert Path(os.environ["NASTECH_HOME"]).is_relative_to(Path(os.environ["HOME"]))
...
Path(os.environ["NASTECH_HOME"]).mkdir(parents=True, exist_ok=True)
# JSON is valid YAML, allowing the real config loader to read the seeded file.
(Path(os.environ["NASTECH_HOME"]) / "config.yaml").write_text(json.dumps(config))
from run_agent import AIAgent
from providers import get_provider_profile
from nastech_cli.config import get_compatible_custom_providers, load_config
from nastech_cli.config_providers import get_custom_provider_context_length
    ...
    json.dumps(
        {
            ...
            "nastech_home": os.environ["NASTECH_HOME"],
            "captures": captures,
            "rows": rows,
