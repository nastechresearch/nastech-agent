...
Runs N independent Nastech ``AIAgent`` sessions concurrently, each the same ~8-step tool loop whose
...
Results that motivated nastech-agent #104284 / #104421 and NastechResearch/api#227 (2026-09-05/06,
Fable 5.1, 20 sessions x 6 calls unless noted):
  nastech, native /v1/messages         13.9% stuck (4 runs 14-20%)     -> chat is the Nastech default
  nastech, /v1/chat/completions         0 / 320 pairs
...
Usage:
  python -m evals.postmortem.live_ab.cache_concurrency_probe --repo . --provider nastech \
      --workers 20 --calls 6 --out /tmp/probe.jsonl [--wire chat|native] [--model ID] \
      [--pin anthropic] [--settle 2] [--ttl 5m]
  providers: nastech (Portal creds from NASTECH_HOME), openrouter (OPENROUTER_API_KEY or --api-key),
             anthropic (ANTHROPIC_API_KEY or --api-key)
Cost: ~$50 per 20x6 arm on Fable 5.1 at the 5m tier. Every run starts from the real Nastech request
...
def _parse():
    ...
    ap.add_argument("--repo", required=True, help="nastech-agent checkout to import from")
    ap.add_argument("--provider", required=True, choices=["nastech", "openrouter", "anthropic"])
    ...
    ap.add_argument("--wire", choices=["chat", "native"], default=None,
                    help="force the wire (default: what Nastech would pick for the provider/model)")
...
os.environ.setdefault("NASTECH_HOME", os.path.expanduser("~/.nastech"))
import anthropic
...
class _Ctx:
    ...
    def __enter__(self):
        ...
        try:
            ...
            self.rec["nastech_headers"] = {k: v for k, v in hdrs.items() if k.lower().startswith("x-nastech") or k.lower() in ("cf-ray", "x-request-id", "request-id")}
...
if ARGS.wire:
    ...
    API_MODE = nastech_api_mode(MODEL)
else:
...
json.dump(dict(provider=PROVIDER, wire=API_MODE, model=MODEL, workers=N, calls_per_session=CALLS, ttl=ARGS.ttl, pin=ARGS.pin, settle_s=SETTLE_S,
               ...
               bad_pairs=[dict(kind=k, session=f"cacheprobe-{PROVIDER}-{ARGS.wire or API_MODE}-{N}-{w}", ok_id=p.get("message_id"), ok_cf_ray=(p.get("nastech_headers") or {}).get("cf-ray"),
                              bad_id=c.get("message_id"), bad_cf_ray=(c.get("nastech_headers") or {}).get("cf-ray"), read_before=p["cache_read"], read_after=c["cache_read"],
