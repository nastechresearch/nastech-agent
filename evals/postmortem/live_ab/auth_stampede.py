"""Live A/B of the Nastech hourly-expiry stampede, no real network.

Local server: accepts bearer FRESH, returns 401 {"type":"authentication_error", "Your API key is invalid,
blocked or out of funds..."} for any other bearer. resolve_nastech_runtime_credentials is patched to return
...
os.environ["NASTECH_HOME"] = tempfile.mkdtemp(prefix="hh-")
os.environ["NASTECH_STREAM_RETRIES"] = "0"
...
auth_mod.resolve_nastech_runtime_credentials = lambda **kw: {"api_key": FRESH, "base_url": base}
import nastech_cli.nastech_auth_keepalive as ka
ka.start_nastech_auth_keepalive = lambda **kw: None  # thread itself is out of scope here; we test adoption

from run_agent import AIAgent

agents = []
for i in range(n):
    a = AIAgent(api_key=STALE, base_url=base, provider="nastech", model="test/model", quiet_mode=True,
