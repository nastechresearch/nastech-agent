...
real result. Uses glm-5.3 via Nastech for cost. Deadline shortened via config to keep the run short."""
...
# Temp NASTECH_HOME with the real auth + a config that shortens the generic sequential deadline to 40 s, so the
...
home = tempfile.mkdtemp(prefix="dl_home_"); os.environ["NASTECH_HOME"] = home
real_home = os.environ.get("NASTECH_HOME_SOURCE", os.path.expanduser("~/.nastech"))  # credentials are copied from here into a temp home
...
rt = resolve_runtime_provider(requested="nastech", target_model=MODEL)
sid = f"dl_{arm}_{int(time.time())}"
ag = AIAgent(model=MODEL, provider="nastech", base_url=rt.get("base_url"), api_key=rt.get("api_key"), api_mode=rt.get("api_mode"),
