...
message prefix between call N and N+1. If Nastech strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--nastech-home DIR]   (default NASTECH_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("NASTECH_HOME", os.path.expanduser("~/.nastech"))
if "--nastech-home" in sys.argv:
    os.environ["NASTECH_HOME"] = sys.argv[sys.argv.index("--nastech-home") + 1]
...
rt = resolve_runtime_provider(requested="nastech", target_model="anthropic/claude-fable-5.1")
sid = f"f0wire_{arm}_{int(time.time())}"
ag = AIAgent(model="anthropic/claude-fable-5.1", provider="nastech", base_url=rt.get("base_url"), api_key=rt.get("api_key"),
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.nastech/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
