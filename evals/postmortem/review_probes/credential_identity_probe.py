...
os.environ.update(HOME=str(home), NASTECH_HOME=str(home/'nastech'), XDG_CONFIG_HOME=str(home/'config'), CODEX_HOME=str(home/'codex'))
for k in list(os.environ):
    if any(x in k for x in ('API_KEY','TOKEN','NASTECH_','SECRET')): os.environ.pop(k, None)
(home/'nastech').mkdir()
(home/'nastech'/'config.yaml').write_text('nastech:\n  keepalive_interval_seconds: 0\nmemory:\n  memory_enabled: false\n', encoding='utf-8')
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        data={'id':'local-probe','object':'chat.completion','created':int(time.time()),'model':'nastech-test','choices':[{'index':0,'message':{'role':'assistant','content':'local-only'},'finish_reason':'stop'}],'usage':{'prompt_tokens':1,'completion_tokens':1,'total_tokens':2}}
...
os.environ['NASTECH_INFERENCE_BASE_URL']=url
os.environ['NASTECH_SHARED_AUTH_DIR']=str(home/'shared')
...
print(json.dumps({'mode':MODE,'module':lifecycle.__file__,'has_new':hasattr(AIAgent,'_adopt_nastech_key_before_expiry'),'home':str(home)}),flush=True)
if MODE=='main':
...
for case, own_sub, store_sub, ttl in [('same-account','account-A','account-A',30),('explicit-account','account-A','account-B',30),('far-from-expiry','account-A','account-B',3000)]:
    ...
    agent=AIAgent(api_key=own,base_url=url,provider='nastech',model='nastech-test',quiet_mode=True,skip_context_files=True,skip_memory=True,enabled_toolsets=[])
    ...
    reply=client.chat.completions.create(model='nastech-test',messages=messages)
...
def worker(_):
    a=AIAgent(api_key=expired,base_url=url,provider='nastech',model='nastech-test',quiet_mode=True,skip_context_files=True,skip_memory=True,enabled_toolsets=[])
    ...
    try:
        c.chat.completions.create(model='nastech-test',messages=messages)
        status=200
...
if MODE != 'main':
    ...
    state_file=home/'nastech'/'auth.json'
    state=json.loads(state_file.read_text(encoding='utf-8')); state['providers']['nastech']['portal_base_url']=url.removesuffix('/v1');state_file.write_text(json.dumps(state), encoding='utf-8')
