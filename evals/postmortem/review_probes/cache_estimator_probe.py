...
sys.path.insert(0,sys.argv[1] if len(sys.argv)>1 else os.getcwd());os.environ['NASTECH_HOME']=tempfile.mkdtemp(prefix='cache-boundary-')  # usage: <repo_root>
...
a=types.SimpleNamespace(api_mode='anthropic_messages',provider='nastech',model=model,base_url=url,tools=[],_usage_anchor=None)
