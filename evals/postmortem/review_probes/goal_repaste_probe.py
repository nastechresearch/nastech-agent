...
# Remove all inherited Nastech/config and credential env before real imports.
...
os.environ['NASTECH_HOME'] = home.name
os.environ['NO_PROXY'] = '*'
os.environ['TZ'] = 'UTC'
...
from nastech_cli import cli_commands_mixin, goals
...
server._nastech_home = Path(home.name)
...
with patch('nastech_cli.goals.draft_contract', return_value=None), contextlib.redirect_stdout(io.StringIO()):
