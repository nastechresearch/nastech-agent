...
- **`forensics/`** reads a *copy* of a Nastech `state.db` (plus rotated `agent.log*` and git) and
...
- A Nastech checkout with its venv (`.venv/bin/python`). NeMo Relay is not required for the forensics;
  it is what the run itself used for the wire captures in `live_ab/cache_prefix_wire.py`.
- For forensics: a **copy** of `~/.nastech/state.db` (never point at the live file; `sqlite3 state.db ".backup copy.db"`
  or `cp` while Nastech is idle) and, optionally, the rotated `~/.nastech/logs/agent.log*`.
- For `--live` probes: real credentials in `NASTECH_HOME` (they spend cents per run).
...
$P -m evals.postmortem.forensics.logcalls   --db state_copy.db --logs "$HOME/.nastech/logs/agent.log*"
...
Nastech recorded (an estimator, not an invoice).
...
```bash
# offline probes (fake providers, temp NASTECH_HOME), main vs a branch or integration checkout:
...
The instrument behind #104284 / #104421 and NastechResearch/api#227. N concurrent real `AIAgent`
...
python -m evals.postmortem.live_ab.cache_concurrency_probe --repo . --provider nastech --workers 20 --calls 6 --out /tmp/p.jsonl
python -m evals.postmortem.live_ab.cache_concurrency_probe --repo . --provider nastech --wire native --workers 20 --calls 6 --out /tmp/p.jsonl
...
Reference results (2026-09-05/06): Nastech native wire 13.9% stuck (14–20% over 4 runs, unchanged by the
portal's provider pin or a 2 s settle); Nastech chat wire 0/320; OpenRouter pinned 0/161; OpenRouter
unpinned 9.8% collapse. Use this to clear a new upstream before flipping
`agent/nastech_wire.py::GMI_NATIVE_WIRE_CLEARED`. `--ttl 1h` reproduces the 2× write price
...
Forensic lanes: five parallel Nastech subagents (2026-09-04), rewritten here to take `--db`/`--root`
