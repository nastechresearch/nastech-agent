...
Run with a fresh HOME and NASTECH_HOME using the repository virtualenv:

```sh
probe=$(mktemp -d)
HOME="$probe" NASTECH_HOME="$probe/state" .venv/bin/python evals/gateway_completion/delivery_lifecycle.py
```
...
and unavailable raw API-owner retention. `NASTECH_EVAL_REPO` selects another checkout for A/B.
