...
env = {k: v for k, v in os.environ.items() if not (k.startswith('NASTECH_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), NASTECH_HOME=str(home), NASTECH_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', NASTECH_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'nastech_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
