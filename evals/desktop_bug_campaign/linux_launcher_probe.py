...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "NASTECH_HOME": str(root / "home/.nastech"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "NASTECH_NONINTERACTIVE": "1"}
    Path(env["NASTECH_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from nastech_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'nastech')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,nastech_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,nastech_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Nastech\n", b"Name=Nastech custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["NASTECH_HOME"]) / "config.yaml"
