...
Run from the repo with a clean environment and a temporary NASTECH_HOME:
...
from nastech_cli import heartbeat  # noqa: E402
...
async def main(base_poller):
    assert os.environ.get("NASTECH_HOME"), "Use a temporary NASTECH_HOME"
    ...
    with patch.object(heartbeat, "time", SimpleNamespace(time=lambda: clock.now)):
        ...
        if not base_poller:
            ...
            runner.session_store = SessionStore(
                sessions_dir=Path(os.environ["NASTECH_HOME"]) / "sessions", config=runner.config)
