...
def main():
        ...
        def do_POST(self):
            ...
            requests.append({"grant_type": payload.get("grant_type"),
                             "target_grant": (payload.get("refresh_token") == ["fixture-refresh-1"]
                                              or self.headers.get("x-nastech-refresh-token") == "fixture-refresh-1")})
            body = ({"access_token": nastech_new_token if self.path == "/api/oauth/token" else "fixture-new-access",
    ...
    nastech_new_token = token("singleton-renewed")
    ...
    cases.extend((name, "nastech", ["refresh", "nastech", target], None) for name, target in
                 (("nastech-independent", "row0"), ("nastech-singleton", "row1")))
    results = []
    try:
        for name, provider, command, status in cases:
            ...
            with tempfile.TemporaryDirectory(prefix="auth-pool-pty-") as temp:
                ...
                env = {k: v for k, v in os.environ.items() if not any(t in k for t in
                       ("TOKEN", "API_KEY", "SECRET", "PASSWORD", "NASTECH", "PYTEST"))}
                env.update(HOME=temp, NASTECH_HOME=temp, NASTECH_SHARED_AUTH_DIR=str(home / "shared"), PYTHONPATH=str(Path(args.repo).absolute()), TERM="xterm")
                bootstrap = ("import sys, httpx; original_send=httpx.Client.send; "
                             ...
                             f"sys.argv=['nastech','auth',*{command!r}]; main()")
                ...
                results.append({"case": name, "exit": exit_code, "output": output,
                                ...
                                "target_rotated": any(e["id"] == "row1" and e.get("access_token") == (nastech_new_token if provider == "nastech" else "fixture-new-access") for e in disk),
    ...
    independent = next(r for r in results if r["case"] == "nastech-independent")
    ...
    singleton = next(r for r in results if r["case"] == "nastech-singleton")
