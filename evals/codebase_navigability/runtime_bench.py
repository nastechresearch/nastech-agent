...
Each probe runs in a FRESH subprocess with an isolated NASTECH_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "NASTECH_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "NASTECH_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("NASTECH_SESSION", "NASTECH_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "nastech_cli.main", "gateway.run", "tools.registry", "nastech_state", "tui_gateway.server", "nastech_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "nastech-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `nastech --version`, `nastech --help`, `nastech doctor --help`, `nastech config get model` (no network)
CLI = {"version": ["nastech_cli/main.py", "--version"], "help": ["nastech_cli/main.py", "--help"], "config_get": ["nastech_cli/main.py", "config", "get", "model"], "tools_list": ["nastech_cli/main.py", "tools", "--help"], "skills_help": ["nastech_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_nastech_default_ms", lambda: resolve_toolset("nastech-default"), 200)
...
try:
    import nastech_state, tempfile, uuid
    from pathlib import Path
    db=nastech_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(nastech_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
