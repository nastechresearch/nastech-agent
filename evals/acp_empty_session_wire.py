...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='nastech-acp-empty-'))
    nastech = home / '.nastech'
    nastech.mkdir()
    (nastech / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), NASTECH_HOME=str(nastech), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               NASTECH_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(nastech / 'state.db') as db:
