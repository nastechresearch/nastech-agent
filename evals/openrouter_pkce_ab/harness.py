#!/usr/bin/env python3
"""Local A/B harness for `nastech auth add openrouter --type oauth` against a FAKE OpenRouter.

Runs the REAL entry point (``nastech_cli.auth_commands.auth_add_command``) with a temp NASTECH_HOME.
...
Scenarios (each prints PASS/FAIL, exit code = number of failures):
  ...
  api_key_path   `nastech auth add openrouter --api-key` still works with no --type (regression)

Usage: NASTECH_PYTHON=<venv python> python3 evals/openrouter_pkce_ab/harness.py [--json OUT]
...
def run(scenario: str, fake: FakeOpenRouter, home: str) -> dict:
    ...
    for m in [m for m in sys.modules if m.startswith(("nastech_cli", "agent", "nastech_constants"))]:
        del sys.modules[m]
    ...
    try:  # BEFORE (origin/main) has no auth_openrouter sibling; the flow itself must then fail.
        ...
        import nastech_cli.auth_device_flow as dfl
