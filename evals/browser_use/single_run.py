...
The run gets a throwaway NASTECH_HOME so no local config leaks in, and the
...
os.environ["NASTECH_HOME"] = hh
# Strip web-fetch shortcuts: every arm must drive the browser.
os.environ.pop("BROWSER_USE_API_KEY", None)
for k in ("FIRECRAWL_API_KEY", "NASTECH_API_KEY", "TAVILY_API_KEY", "SERPER_API_KEY"):
...
# Nastech-subscription path on boxes without an OpenRouter key. Credentials are
...
if _or_key:
    ...
    # Resolved by the orchestrator BEFORE NASTECH_HOME is redirected to the
    # throwaway home (auth state lives in the real profile). Never printed.
    _tok = os.environ.get("BUBENCH_NASTECH_TOKEN", "").strip()
    if not _tok:
