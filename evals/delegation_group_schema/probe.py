...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, NASTECH_HOME=home, PATH="/usr/bin:/bin", NASTECH_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["nastech-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["nastech-cli"], quiet_mode=True)
