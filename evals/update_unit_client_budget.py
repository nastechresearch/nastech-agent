...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), NASTECH_HOME=str(home / "nastech"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("nastech-gateway*", "nastech-serve*")] + [unit + ".service"]
...
unit = f"nastech-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
