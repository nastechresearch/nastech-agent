...
sandbox = tempfile.TemporaryDirectory(prefix="nastech-104260-")
os.environ.clear()
os.environ.update(
    HOME=sandbox.name,
    NASTECH_HOME=sandbox.name + "/nastech",
    PATH="/usr/bin:/bin",
    PYTHONDONTWRITEBYTECODE="1",
    NASTECH_DISABLE_MODEL_METADATA_FETCH="1",
)
Path(os.environ["NASTECH_HOME"]).mkdir()
...
# JSON is valid YAML; only the temporary Nastech home is written.
Path(os.environ["NASTECH_HOME"], "config.yaml").write_text(
