...
Run with the Nastech venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Nastech children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def rows_worker():
    ...
    b = _PickerBuild(current_provider="", current_base_url="", current_model="",
                     max_models=None, for_picker=True, force_fresh_nastech_tier=False,
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "nastech_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="nastech-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".nastech"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "NASTECH_HOME": str(state),
