...
supports multiple backends — **Browser Use** (cloud, default for Nastech
...
from nastech_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Nastech secret
...
def _build_browser_env() -> dict:
    ...
    Strips Nastech-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``nastech_subprocess_env`` import is deferred to keep ``browser_tool``
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``nastech doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    """Return the provider cached for the active Nastech profile."""
...
def _resolve_cloud_provider_uncached() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Nastech gateway or
    ...
    try:
        ...
        if isinstance(browser_cfg, dict) and "cloud_provider" in browser_cfg:
            ...
            if provider_key in ("local", "camofox"):
                ...
                _cloud_provider_resolved = True
                return None
            if provider_key == "nastech":
                # Managed "Nastech Subscription" selection is serviced by the
    ...
    if resolved is None and provider_key is None:
        ...
        # was ever written: Browser Use first (managed Nastech gateway or
...
# comparison sites (four here, plus nastech_cli/tools_config.py and
# nastech_cli/doctor.py) from drifting if the sentinel's exact spelling ever
# changes.
...
def lightpanda_engine_status() -> Tuple[bool, str]:
    ...
    if bu_mode:
        return True, "Browser Use mode: Nastech spawns `lightpanda serve` per session"
...
def _run_chrome_fallback_command(
    ...
    # uses, not a bare shutil.which("npx") — Nastech-managed-Node-only setups
    ...
    # Claim the dir before using it: another nastech process's orphan reaper
...
_REAL_PROFILE_SESSION = "nastech-real-profile"
...
def _cdp_http_ready(http_cdp: str) -> bool:
    ...
    try:
        from nastech_cli.browser_connect import is_browser_debug_ready
...
def _real_profile_cdp() -> tuple:
    ...
    Snapshots the user's default-Chromium profile into a nastech-owned copy
    ...
    if not _use_real_profile():
        ...
        try:
            from nastech_cli.browser_connect import cleanup_real_profile_snapshots
    ...
    with _real_profile_cdp_lock:
        ...
        # from a previous nastech process; if it is driving OUR copy dir, hand it
        ...
        if err or not snap_dir:
            from nastech_cli.browser_connect import _PROFILE_LOCKED_PREFIX
            ...
            if err and err.startswith(_PROFILE_LOCKED_PREFIX):
                ...
                return None, (
                    ...
                    "`nastech browser close-profile`, then retry."
                )
        ...
        from nastech_cli.browser_connect import chromium_executable
...
# run exactly once, before the cleanup loop started, which meant a nastech
...
# Hard ceiling for a daemon whose owning nastech process is still alive but
...
# Owner Nastech home per session (#86402).  The inactivity janitor is one
...
def _session_owner_scope(task_id: str):
    """Run under the Nastech home + secret scope that owns ``task_id``'s
    ...
    home_token = set_nastech_home_override(owner_home)
    try:
...
def _socket_dir_idle_seconds(socket_dir: str) -> Optional[float]:
    ...
    nastech restarts and does not depend on in-memory bookkeeping surviving an
    exception path.
...
def _reap_orphaned_browser_sessions():
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            ...
            # live nastech process and must not be touched — but "owner alive"
        ...
        if not os.path.isfile(pid_file):
            ...
            # agent-browser writes its PID/owner files. Another Nastech process
...
def _update_session_activity(task_id: str):
    ...
    Also records the owning Nastech home on first sight so the process-global
    ...
    with _cleanup_lock:
        ...
        _session_owner_homes.setdefault(task_id, str(get_nastech_home()))
...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    # browser running on a nastech-owned SNAPSHOT of their real profile —
    ...
    # ``browser.engine: lightpanda`` that endpoint is a Nastech-spawned
...
def _resolve_npx_bin() -> Optional[str]:
    """Resolve a runnable npx binary, preferring the Nastech-managed/Homebrew
    extended search over a bare ambient PATH lookup.

    Checking bare PATH first would let a broken or unrelated system npx
    shadow a healthy Nastech-managed one with no recovery — every candidate
    is therefore validated with ``node_tool_runnable`` (the same check
    ``find_nastech_node_executable`` uses to self-heal a managed Node tree)
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    # node_modules binary, which disappears on the next ``nastech update`` and
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_nastech_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_nastech_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_nastech_home() / "node")),
            ]
...
def warm_agent_browser_npx_cache(timeout: float = 60.0) -> bool:
    ...
    ``nastech update`` (or ``nastech doctor --fix``) warms npx's own cache
    ...
    provider/gateway credential Nastech holds, while running registry-fetched
    npm code on every ``nastech update`` (the GHSA-m4m8-xjp4-5rmm class of
    ...
    cmd = [
        ...
        "--ignore-scripts",
        # --prefer-offline: once cached, repeat `nastech update`/`doctor
...
def _store_full_snapshot(snapshot_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nastech_dir("cache/web", "web_cache")
...
def _cleanup_single_browser_session(task_id: str) -> None:
    ...
    if session_info:
        ...
        # A Lightpanda session is a process Nastech spawned itself (Browser
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/nastechresearch/nastech-agent:latest")
                else:
