...
from nastech_constants import get_nastech_home, nastech_home_key
...
# dependency could read every Nastech secret from process.env.
...
# than a concrete path (also compared in nastech_cli/tools_config.py and doctor.py).
...
# The config-derived ones are keyed by profile home (``nastech_home_key()``): the multiplexed
...
def _socket_safe_tmpdir() -> str:
    """Short temp dir for Unix sockets: macOS ``TMPDIR`` + ``agent-browser-nastech_…``
...
# Owner Nastech home per session: the janitor is one process-global thread, so each
...
def _url_policy_error(url: str, *, auto_local: bool = False) -> Optional[dict]:
    ...
    URL protects nothing. Nastech' own secrets leaking into a URL are caught by ``_secret_url_error``."""
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nastech_home_override': ('nastech_constants', 'get_nastech_home_override'),
    'nastech_home_key': ('nastech_constants', 'nastech_home_key'),
    ...
    'node_tool_runnable': ('nastech_constants', 'node_tool_runnable'),
