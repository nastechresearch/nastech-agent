...
# uses 30s) — comfortably below the 1800s default NASTECH_AGENT_TIMEOUT.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0

# Hard ceiling on how long the heartbeat keeps the parent watchdog at bay.
# The child cron run has its own inactivity watchdog (NASTECH_CRON_TIMEOUT,
# default 600s) that bounds a wedged job, but with NASTECH_CRON_TIMEOUT=0
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    if origin_platform and origin_chat_id:
        ...
        if thread_id and origin_platform == "slack":
            message_id = get_session_env("NASTECH_SESSION_MESSAGE_ID") or None
        ...
        return {
            ...
            # send_message, which passes NASTECH_SESSION_USER_ID to
            ...
            "scope_id": get_session_env("NASTECH_SESSION_SCOPE_ID") or None,
        }
    return None
...
def _local_delivery_notice(job: Dict[str, Any], user_deliver: Optional[str]) -> Optional[str]:
    ...
    ``NASTECH_SESSION_PLATFORM``/``CHAT_ID`` is set for them), so a
    ...
    for part in str(deliver).split(","):
        ...
        if not profile_exists(canon):
            return (
                ...
                "profile that exists here (nastech profile list), or omit the "
...
def _resolve_cron_context_deliver(deliver: Optional[str]) -> Optional[str]:
    ...
    delivery target — the ``NASTECH_CRON_AUTO_DELIVER_*`` contextvars that
    ...
    * Every other element (``local``, ``all``, explicit ``platform:...``)
    ...
    if not is_truthy_value(get_session_env("NASTECH_CRON_SESSION", "")):
        return deliver

    def _creator_target() -> str:
        platform = get_session_env("NASTECH_CRON_AUTO_DELIVER_PLATFORM", "").strip()
        chat_id = get_session_env("NASTECH_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
        if not platform or not chat_id:
            return "local"
        thread_id = get_session_env("NASTECH_CRON_AUTO_DELIVER_THREAD_ID", "").strip()
        if thread_id:
...
def _latest_job_output_excerpt(job_id: str, max_chars: int = 2000) -> Optional[str]:
    ...
    ``~/.nastech/cron/output/``. Never raises.
    """
    try:
...
def _try_dispatch_background_run(
    ...
    None
        ...
        ``nastech -z``, stateless HTTP, Kanban worker, nested cron run).
    ...
    # 'running' by a dead owner process -- e.g. a PRIOR one-shot `nastech
    ...
    if not session_key:
        # Direct Python callers (`nastech cron run`, tests) have no agent
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("NASTECH_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def _gateway_liveness_notice(plural: bool = False) -> dict:
    ...
    Thin adapter over the shared CLI helper ``nastech_cli.cron._builtin_gateway_liveness``
    ...
    """
    try:
        from nastech_cli.cron import _builtin_gateway_liveness
    ...
    if _gw is False:
        return {
            ...
            "warning": (
                f"The Nastech gateway is not running — {subject} "
                "but will NOT fire until the gateway is started "
                "(nastech gateway install / nastech gateway start). "
        ...
        if normalized == "create":
            ...
            try:
                job = create_job_with_scheduler_registration(
                    ...
                    # (nastech cron create --reasoning-effort) ONLY — it is
...
Scheduling from cron-run sessions is disabled by default and enabled via cron.allow_agent_scheduling in config.yaml. When enabled, jobs created from a cron run are user-owned in the same flat job table as every other job, and their delivery resolves to the creating job's own persistent target — never to the ephemeral cron-run session. Prefer updating an existing job (list first, then update by job_id) over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "monitor_script": {
                "type": "string",
                "description": f"Optional monitor-mode source script (same rules as `script`: relative to {display_nastech_home()}/scripts/, .sh/.bash via bash, else Python). Each tick it runs FIRST and its output is hashed as exact bytes: UNCHANGED output suppresses the agent run entirely (no LLM, no delivery, recorded as a silent no_change tick); CHANGED output injects a MONITOR CHANGE DETECTED block (unified diff + new output) into the prompt before a normal agent run. The first tick always runs the agent (baseline). Scripts must emit STABLE output — no timestamps or random ordering — or every tick looks changed. Mutually exclusive with monitor_url; incompatible with no_agent=True. On update, pass empty string to clear."
            },
            "monitor_url": {
                "type": "string",
...
registry.register(
    ...
    handler=lambda args, **kw: cronjob(
        ...
        # `nastech cron create/edit --model`, or hand-edited jobs). The agent
