...
3.11-3.13 runtimes Nastech ships, a bare pool worker starts with an EMPTY
Context and silently drops contextvar-based state (profile secret scope,
NASTECH_HOME override) — under the multiplexed gateway a credential read in
...
class DaemonThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def submit(self, fn, /, *args, **kwargs):
        ...
        3.11-3.13 (the runtimes Nastech ships) do not, so a pool worker
        starts with an empty Context and loses the multiplexed profile
        secret scope / NASTECH_HOME override.  Do it here unconditionally so
