...
``metadata.nastech.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.nastech.blueprint`` is a mapping with a
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.nastech.blueprint`` block) ready for ``nastech skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"nastech": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
