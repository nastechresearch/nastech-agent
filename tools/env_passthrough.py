...
# Config-based allowlist, keyed by Nastech home: under gateway.multiplex_profiles one process serves
...
def _is_nastech_provider_credential(name: str) -> bool:
    """True if ``name`` is a Nastech-managed provider credential per
    ``_NASTECH_PROVIDER_ENV_BLOCKLIST`` or a dynamic Nastech-internal secret
    ...
    in the ``execute_code`` child); non-Nastech keys (TENOR_API_KEY, …) stay
    registerable. Fails closed when the blocklist cannot be imported."""
    try:
        from tools.environments.local_env_policy import (
            _NASTECH_PROVIDER_ENV_BLOCKLIST, _is_nastech_internal_secret)
        ...
        return True
    return _is_nastech_internal_secret(name) or name in _NASTECH_PROVIDER_ENV_BLOCKLIST


def register_env_passthrough(var_names: Iterable[str]) -> None:
    """Register env var names as allowed in sandboxed environments (typically a
    skill's ``required_environment_variables``). Nastech-managed provider credentials
...
def _accepted(names, refusal_msg: str):
    """Yield non-empty *names* that are not Nastech provider credentials; refused
...
def scoped_passthrough_additions(present: Iterable[str]) -> dict[str, str]:
    ...
    ``os.environ`` (``load_nastech_dotenv`` skips the process-global load for a routed home), so a
