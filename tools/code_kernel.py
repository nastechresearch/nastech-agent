...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    (the client stub reconnects: NASTECH_RPC_PERSISTENT). The serving thread carries NO frozen
...
def _bind_rpc_socket(kernel: SessionKernel) -> str:
    ...
    if _IS_WINDOWS:
        ...
        rpc_endpoint = kernel.sock_path = os.path.join(sock_tmpdir, f"nastech_rpc_{uuid.uuid4().hex}.sock")
...
def _parent_process_handle(child_env: Dict[str, str]):
    ...
    try:
        ...
        if handle:
            child_env["NASTECH_KERNEL_PARENT_PROCESS_HANDLE"] = str(int(handle))
...
def _spawn(kernel: SessionKernel, *, child_python: str, child_cwd: str,
    ...
    for name, src in (("nastech_tools.py", generate_nastech_tools_module(list(sandbox_tools))),
                      ("nastech_kernel_runner.py", KERNEL_RUNNER_SOURCE)):
    ...
    try:
        kernel.proc = subprocess.Popen(
            [child_python, os.path.join(kernel.tmpdir, "nastech_kernel_runner.py")],
