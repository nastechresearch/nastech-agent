...
"""Auto-generated Nastech session-kernel runner. One exec cell per request."""
import contextlib
import io
import json
import os
...
_SENTINEL = os.environ["NASTECH_KERNEL_SENTINEL"]
_CAPTURE_LIMIT = {capture_limit}
_SPILL_DIR = os.environ.get("NASTECH_KERNEL_SPILL_DIR", "")
_SPILL_CAP = {spill_cap}
_PARENT_PROCESS_HANDLE = os.environ.pop("NASTECH_KERNEL_PARENT_PROCESS_HANDLE", "")
_PARENT_DEATH_FD = os.environ.pop("NASTECH_KERNEL_PARENT_DEATH_FD", "")
...
def _start_parent_death_pipe_watchdog():
    ...
    threading.Thread(target=_wait, name="nastech-parent-watchdog", daemon=True).start()
...
# nastech-agent#88637 by @z80dev (stable owner id, owner-teardown disposal,
...
def _resolve_owner(task_id: str) -> str:
    ...
    try:
        ...
        if is_delegated_child_context():
            ...
            child_id = get_session_env("NASTECH_SESSION_ID", "") or (task_id or "")
...
def shutdown_kernels_for_owner(owner: str) -> None:
    ...
    (the /new + session-close disposal shape from nastech-agent#88637).
    """
    if not owner:
        return
    with _KERNELS_LOCK:
...
def _rpc_forever(kernel: SessionKernel, max_tool_calls: int,
    ...
    client stub reconnects on its side (NASTECH_RPC_PERSISTENT).
...
def _spawn(kernel: SessionKernel, *, task_id: str, child_python: str,
    ...
    from tools.code_execution_tool import (
        _build_child_env,
        generate_nastech_tools_module,
    )

    kernel.tmpdir = tempfile.mkdtemp(prefix="nastech_kernel_")
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()

    kernel.rpc_token = secrets.token_urlsafe(32)
    kernel.sentinel = "@@NASTECH-KERNEL-" + secrets.token_urlsafe(16) + "@@"
    ...
    if _IS_WINDOWS:
        ...
        kernel.sock_path = os.path.join(_sock_tmpdir, f"nastech_rpc_{uuid.uuid4().hex}.sock")
    ...
    with open(os.path.join(kernel.tmpdir, "nastech_tools.py"), "w", encoding="utf-8") as f:
        f.write(tools_src)
    runner_path = os.path.join(kernel.tmpdir, "nastech_kernel_runner.py")
    ...
    child_env["NASTECH_KERNEL_SENTINEL"] = kernel.sentinel
    ...
    child_env["NASTECH_KERNEL_SPILL_DIR"] = kernel.tmpdir
    ...
    child_env["NASTECH_RPC_PERSISTENT"] = "1"
    ...
    if _IS_WINDOWS:
        try:
            ...
            if parent_process_handle:
                child_env["NASTECH_KERNEL_PARENT_PROCESS_HANDLE"] = str(
            ...
            child_env.pop("NASTECH_KERNEL_PARENT_PROCESS_HANDLE", None)
            parent_process_handle = None
            close_parent_process_handle = None
            startupinfo = None
    ...
    if not _IS_WINDOWS:
        ...
        child_env["NASTECH_KERNEL_PARENT_DEATH_FD"] = str(death_r)
        pass_fds = (death_r,)
