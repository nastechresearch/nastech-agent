...
def _emergency_cleanup_all_sessions():
    ...
    nastech processes — every clean exit reaps accumulated orphans, not only
    ...
    try:
        ...
        # Interpreter shutdown (or a half-updated tree mid-`nastech update` where the
    ...
    # owned by other live nastech processes.
    _best_effort("Orphan reap on exit", _reap_orphaned_browser_sessions)


@contextlib.contextmanager
def _session_owner_scope(task_id: str):
    """Run under the Nastech home + secret scope owning ``task_id``'s session (no-op if unrecorded).
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record this nastech PID in ``<socket_dir>/<session>.owner_pid`` so the orphan
...
def _reap_socket_dir(socket_dir: str, session_name: str, tracked_names: set) -> bool:
    ...
    A live ``owner_pid`` means another nastech process owns it — leave it UNLESS untracked
...
def _reap_orphaned_browser_sessions():
    """Kill agent-browser daemons whose owning nastech process is gone (an unclean exit loses
    ...
    for prefix in ("agent-browser-h_*", "agent-browser-cdp_*", "agent-browser-nastech_*",
...
def _update_session_activity(task_id: str):
    """Touch the activity timestamp and record the owning Nastech home on first sight (the
    ...
    with _bt._cleanup_lock:
        ...
        _bt._session_owner_homes.setdefault(task_id, str(get_nastech_home()))
...
def _cleanup_old_recordings(max_age_hours=72):
    ...
    try:
        recordings_dir = get_nastech_home() / "browser_recordings"
    except Exception as e:
