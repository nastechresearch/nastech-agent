...
# Sensitive write targets, matched via ~ / $HOME / $NASTECH_HOME spellings. The resolved absolute
# home is folded into these forms at detection time by _normalize_command_for_detection(), so no
# import-time path snapshot (stale once NASTECH_HOME is set after import) lives in the patterns.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_NASTECH_ENV_PATH = (
    r'(?:~\/\.nastech/|(?:\$home|\$\{home\})/\.nastech/|(?:\$nastech_home|\$\{nastech_home\})/)' r'\.env\b'
)
# ~/.nastech/config.yaml IS the security policy (approvals.mode, yolo, allowlist) and the config cache is mtime-keyed,
# so a write takes effect mid-session. Terminal-side coverage (sed -i, tee, >, cp) pairs the file_tools deny.
_NASTECH_CONFIG_PATH = (
    r'(?:~\/\.nastech/|(?:\$home|\$\{home\})/\.nastech/|(?:\$nastech_home|\$\{nastech_home\})/)' r'config\.yaml\b'
)
...
_SENSITIVE_WRITE_TARGET = (
    rf'(?:{_SYSTEM_CONFIG_PATH}|/dev/sd|{_SSH_SENSITIVE_PATH}|{_NASTECH_ENV_PATH}|{_NASTECH_CONFIG_PATH}|'
...
DANGEROUS_PATTERNS = [
    ...
    # between `nastech` and `gateway` (`nastech -p ade gateway restart`) are allowed so a profile flag can't slip past.
    ...
    # launchctl-driven gateway stop/restart on macOS (label `ai.nastech.gateway`). Two independent lookaheads, NOT a
    # sequential match: a for-loop building the label from a list defined EARLIER (`for item in 'ai.nastech...'; do
    # launchctl bootout "$label"`) never has "nastech" after the verb, and that slipped past and restarted 4 gateways
    ...
    (r'\A(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:nastech|ai\.nastech)\b)', "stop/restart nastech launchd service (kills running agents)"),
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a credential/SSH/shell-rc/Nastech file (key implant, login-time
    ...
    # ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files, ~/.nastech/config.yaml/.env), but cp/mv/install was
    ...
    # sed -i on Nastech config/.env bypasses the redirection/tee rules; pairs the file_tools
    # write_file/patch deny so the terminal side is not an open door.
    # In-place edit of a Nastech-managed security file (~/.nastech/config.yaml or .env). sed -i bypasses the
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    command = re.sub(r'\\\r?\n', '', command)
    # Fold absolute user/Nastech home prefixes to ~/ and ~/.nastech/ so the static patterns catch /home/alice/.bashrc
    # and C:\Users\alice\.bashrc. Resolved at detection time (not import time) so it tracks HOME/NASTECH_HOME set
    # later. MUST run before the backslash strip (which would dissolve C:\Users\alice to C:Usersalice). Nastech home
...
def _rewrite_resolved_nastech_home(command: str) -> str:
    """Resolved NASTECH_HOME (and its realpath) -> ``~/.nastech/`` so the _NASTECH_CONFIG_PATH /
    _NASTECH_ENV_PATH rules match Docker/gateway deployments that spell the absolute path."""
    try:
        ...
        return command
    return _fold_home_prefixes(command, paths, "~/.nastech")
...
_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = "stop/restart nastech gateway via shell-spliced verb (kills running agents)"
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    ...
    return (
        ...
        and re.fullmatch(r"nastech-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
    )
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    verb is an ARGUMENT, so ``launchctl kick"start" -k gui/501/ai.nastech.gateway`` auto-approved.
    Delegates to ``cron.lifecycle_guard`` (shlex-tokenized, anchored on a nastech-gateway
