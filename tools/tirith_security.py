...
GitHub releases to $NASTECH_HOME/bin/tirith in a background thread -- SHA-256 always verified,
...
from nastech_constants import get_nastech_home, get_nastech_home_override, nastech_home_key
...
def _cached_path() -> str | None:
    ...
    if get_nastech_home_override() is not None:
        return _resolved_path_by_home.get(nastech_home_key())
...
def _store_resolved(path: str) -> None:
    ...
    if get_nastech_home_override() is not None:
        _resolved_path_by_home[nastech_home_key()] = path
    else:
        _resolved_path = path
...
def _failure_marker_path() -> str:
    return os.path.join(str(get_nastech_home()), ".tirith-install-failed")
...
def _nastech_bin_dir() -> str:
    """$NASTECH_HOME/bin, created if needed."""
    os.makedirs(d := os.path.join(str(get_nastech_home()), "bin"), exist_ok=True)
    return d
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $NASTECH_HOME/bin/tirith -> ``(installed_path,
...
def _find_local_tirith() -> str | None:
    """Cheap local lookup for the default "tirith": PATH, then $NASTECH_HOME/bin."""
    nastech_bin = os.path.join(_nastech_bin_dir(), "tirith")
    return shutil.which("tirith") or (nastech_bin if _is_executable(nastech_bin) else None)
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    $NASTECH_HOME/bin/tirith → install; failures cached in-process and on disk for 24h). On a miss
