...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    if not isinstance(nastech_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.nastech.{tags, related_skills}.")
    elif "tags" not in nastech_meta:
        yield _warn("missing-metadata", "metadata.nastech.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("nastech", "agent", "nastech agent") and (
        author != "Nastech Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Nastech Agent' (proper caps) "
