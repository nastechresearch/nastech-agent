...
class NastechMCPOAuthProvider(NastechProviderMixin, *_SDK_BASES):
    ...
    fixes come from ``NastechProviderMixin``. Only usable when the SDK's OAuth module imported.

    Reference: Claude Code's ``invalidateOAuthCacheIfDiskChanged`` (``src/utils/auth.ts:1320``, CC-1096 /
    GH#24317).
    """

    _nastech_logger = logger
    ...
    def _nastech_storage(self):
        """The context storage when it is a ``NastechTokenStorage``, else None."""
        from tools.mcp_oauth import NastechTokenStorage
        return self.context.storage if isinstance(self.context.storage, NastechTokenStorage) else None

    def _log_nonfatal(self, what: str, exc: BaseException) -> None:
        logger.debug("MCP OAuth '%s': %s failed (non-fatal): %s", self._nastech_server_name, what, exc)
    ...
    async def _initialize(self) -> None:
        ...
        Bearer tokens as "valid"; seeding the expiry (``NastechTokenStorage`` persists absolute
        ...
        await super()._initialize()  # NastechProviderMixin: restores metadata from disk, enforces issuer binding
    ...
    async def _prefetch_oauth_metadata(self) -> None:
        ...
        # The SDK's httpx flavour, not Nastech': `create_oauth_metadata_request` returns *its* (httpx2) Request objects.
        ...
        async def _send(client, url: str, label: str):
            try:
                ...
                logger.debug("MCP OAuth '%s': %s discovery to %s failed: %s", self._nastech_server_name, label, url, exc)
                return None
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            for url in build_oauth_authorization_server_metadata_discovery_urls(self.context.auth_server_url, server_url):
                ...
                if asm:
                    ...
                    storage = self._nastech_storage()  # persist now so a later cold-load skips discovery
    ...
    async def _maybe_flag_poisoned_client(self, response: Any) -> None:
        ...
        is swallowed. The browser-side "Redirect URI Mismatch" case has no HTTP signal (``nastech mcp reauth``).

        See #36767.
        """
        try:
            if (self._nastech_preregistered or getattr(response, "status_code", None) not in (400, 401)
            ...
            # back into the same refusal (`nastech mcp login` clears the marker).
            ...
            if cimd_url and getattr(self.context.client_info, "client_id", None) == cimd_url:
                logger.warning("MCP OAuth '%s': authorization server rejected our Client ID Metadata Document (%s) "
                               ...
                               self._nastech_server_name, cimd_url)
    ...
    async def async_auth_flow(self, request):  # type: ignore[override]
        try:  # pre-flow hook: reload from disk if it changed (non-fatal on error)
            await get_manager().invalidate_if_disk_changed(self._nastech_server_name, nastech_home=self._nastech_home)
...
_NASTECH_PROVIDER_CLS: Optional[type] = NastechMCPOAuthProvider if _SDK_BASES else None
...
class MCPOAuthManager:
    ...
    @staticmethod
    def _key(server_name: str, nastech_home: str | Path | None = None) -> tuple[str, str]:
    ...
    def _build_provider(self, server_name: str, entry: _ProviderEntry) -> Optional[Any]:
        """Build a ``NastechMCPOAuthProvider``; None if the SDK's OAuth support is unavailable."""
        ...
        if get_dashboard_oauth_flow() is None and not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                ...
                f"Run `nastech mcp login {server_name}` interactively first to complete initial authorization.")
    ...
    def remove(self, server_name: str, *, nastech_home: str | Path | None = None) -> _ProviderEntry | None:
        """Evict the provider from cache AND delete tokens from disk (``nastech mcp remove`` / forced re-auth)."""
        entry = self.evict(server_name, nastech_home=nastech_home)
    ...
    def restore_entry(self, server_name: str, entry: _ProviderEntry | None, *, nastech_home: str | Path | None = None) -> None:
    ...
    def evict(self, server_name: str, *, nastech_home: str | Path | None = None) -> _ProviderEntry | None:
        """Drop only the in-process provider, preserving persisted OAuth state."""
        with self._entries_lock:
            return self._entries.pop(self._key(server_name, nastech_home), None)

    async def invalidate_if_disk_changed(self, server_name: str, *, nastech_home: str | Path | None = None) -> bool:
        ...
        async with entry.lock:
            try:
                mtime_ns = (_get_token_dir(nastech_home) / f"{_safe_filename(server_name)}.json").stat().st_mtime_ns
            except OSError:
                return False
