...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   ``$NASTECH_HOME/cache/spillover/{tool_use_id}.txt`` — alongside the other
   Nastech-owned caches (images, audio, documents, ...) instead of littering
...
def get_spillover_dir():
    """Return $NASTECH_HOME/cache/spillover as a Path (not created)."""
    from nastech_constants import get_nastech_home

    return get_nastech_home() / SPILLOVER_SUBDIR
...
def _write_to_spillover(content: str, filename: str):
    """Write content host-side to $NASTECH_HOME/cache/spillover.
...
def maybe_persist_tool_result(
    ...
    # Always persist host-side first: $NASTECH_HOME/cache/spillover is the
    # single canonical home for spilled results (with the other Nastech-owned
