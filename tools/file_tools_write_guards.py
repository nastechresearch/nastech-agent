...
# resolve per call in production (below) because ``get_nastech_home()`` /
...
_real_nastech_home_cached: str | None = None
_real_nastech_home_loaded = False
...
def _nastech_home_real() -> str:
    from nastech_constants import get_nastech_home
    return os.path.realpath(str(get_nastech_home()))


def _get_nastech_config_resolved() -> str | None:
    """Resolved absolute path of the Nastech config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``NASTECH_HOME`` scope (#107327);
    a test may pin it via ``_nastech_config_resolved`` + ``_nastech_config_resolved_loaded``."""
    ...
    try:
        ...
        # home rather than the active multiplex ``NASTECH_HOME`` — comparing
        ...
        # ``get_nastech_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from nastech_constants import get_nastech_home
            return str((Path(str(get_nastech_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_nastech_home() -> str | None:
    """Realpath of the authoritative Nastech home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``NASTECH_HOME`` scope (#107327);
    a test may pin it via ``_real_nastech_home_cached`` + ``_real_nastech_home_loaded``.
    Consumers exempting a whole TREE want ``_nastech_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_nastech_home_loaded:
        return _real_nastech_home_cached
    try:
        return _nastech_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_nastech_config_resolved``): re-derive from ``get_nastech_home()``
        # rather than ``_expand_tilde("~/.nastech")`` so the protected-instruction
        ...
        # the consumer: the ``~/.nastech`` exemption is skipped, so the gate runs.
        try:
            from nastech_constants import get_nastech_home
            return os.path.realpath(str(get_nastech_home()))
        except Exception:
...
def _nastech_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the Nastech home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the Nastech ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.nastech`` component rule, which
    gated them like a project-local ``<repo>/.nastech/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.nastech`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``NASTECH_HOME`` alone."""
    home = _get_real_nastech_home()
    if not home:
        return ()
    try:
        from nastech_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if nastech_config and nastech_config in candidates:
        return (
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .nastech tree).
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.nastech itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.nastech`` component rule, which would match the home.
    # ``_nastech_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.nastech/<file> cannot read as project-local ``.nastech`` config.
    for real_home in _nastech_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .nastech config dirs (<repo>/.nastech/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.nastech.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".nastech":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Nastech mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.nastech" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Nastech state (a write the host never reads). Not profile isolation — that guard was
