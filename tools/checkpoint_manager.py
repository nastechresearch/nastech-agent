...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Nastech wrote
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Nastech just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Nastech last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Nastech last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Nastech'
        last write or that Nastech never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Nastech has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Nastech never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Nastech wrote it: restoring it back is
                # safe — its last content was Nastech-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Nastech changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Nastech changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Nastech' last write — per the agent-write ledger —
        are left untouched, and only Nastech-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Nastech-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Nastech created it. Deleting it would not restore a prior
