...
def register_credential_file(
    ...
    *relative_path* is relative to ``NASTECH_HOME`` (e.g. ``google_token.json``).
    ...
    Containment alone is not sufficient, because NASTECH_HOME is exactly where
    ...
    # inside NASTECH_HOME and therefore pass the containment check above.
...
def get_cache_directory_mounts(
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if not host_dir.is_dir():
            ...
            # mount costs the feature. get_nastech_dir() already resolved
...
def to_agent_visible_cache_path(
    ...
    backend's Nastech cache lands):

    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at
      ``/root/.nastech`` (the *container_base* default).
    * ssh / daytona / vercel_sandbox — file-synced under the remote user's
      home; ``~/.nastech`` is shell-expanded by the remote shell, so tool
