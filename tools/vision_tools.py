...
Nastech, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
# ``agent.auxiliary_client`` pulls credential_pool → nastech_cli.auth → httpx
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: NASTECH_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("NASTECH_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _supported_media_types() -> frozenset:
    ...
    try:
        ...
        from nastech_cli.local_runtime.capabilities import (
...
def _normalize_to_supported_image(
    ...
    out_dir = get_nastech_dir("cache/vision", "temp_vision_images")
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * Anthropic Messages API (``anthropic`` provider, plus aggregators that
        proxy Claude — ``openrouter``, ``nastech``, ``vertex``, ``bedrock``):
    ...
    # them.
    _AGGREGATORS = {
        "openrouter", "nastech", "vertex", "bedrock", "anthropic-vertex",
        "google-vertex",
    }
...
def check_vision_requirements() -> bool:
    ...
    and if that fails, the auto chain (main provider → openrouter → nastech).
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Nastech, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
