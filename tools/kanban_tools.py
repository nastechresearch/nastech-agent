...
Registered only under the dispatcher (``NASTECH_KANBAN_TASK`` set) or when the profile
enables the ``kanban`` toolset. Tools rather than ``nastech kanban`` shell-outs: they run
...
def _is_dispatcher_owned_worker() -> bool:
    ...
    a worker — i.e. whenever NASTECH_KANBAN_* is present but not ours."""
...
def _visible(*, to_env_worker: bool) -> bool:
    ...
    (NASTECH_KANBAN_TASK) per flag; else the profile toolset decides."""
    if _is_delegated_child_context():
        return False
    if os.environ.get("NASTECH_KANBAN_TASK") and _is_dispatcher_owned_worker():
...
def _reject_delegated_child_mutation(tool_name: str) -> None:
    """A delegate_task child shares the parent's process, so inherited NASTECH_KANBAN_*
...
def _worker_run_id(task_id: str) -> Optional[int]:
    ...
    raw = _own_task_env(task_id, "NASTECH_KANBAN_RUN_ID")
    try:
...
def _stamp_worker_session_metadata(task_id: str, metadata: Optional[dict]) -> Optional[dict]:
    ...
    session_id = _own_task_env(task_id, "NASTECH_SESSION_ID")
...
def _enforce_worker_task_ownership(tid: str) -> None:
    """A dispatcher-spawned worker may only mutate its own NASTECH_KANBAN_TASK; a
...
# worker context (no ``NASTECH_KANBAN_TASK``). - No durable note on these auto-heartbeats; that's reserved
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    heartbeated; ``NASTECH_KANBAN_CLAIM_LOCK`` absent -> default claimer (local workers)."""
    ...
    try:
        ...
        with _board(None, quiet_close=True) as (kb, conn):
            ops = ((kb.heartbeat_claim, {"claimer": os.environ.get("NASTECH_KANBAN_CLAIM_LOCK")}),
            ...
            for fn, kwargs in ops:
                ...
                try:
                    ...
                    # inherited NASTECH_DELEGATED_CHILD_CONTEXT next to NASTECH_KANBAN_TASK, so it is
                    ...
                    if not _auto_heartbeat_fence_warned:
                        ...
                        logger.warning(
                            ...
                            "NASTECH_DELEGATED_CHILD_CONTEXT together with NASTECH_KANBAN_TASK, so the board "
...
def inject_new_comments_from_env(agent: Any) -> bool:
    ...
    tid = os.environ.get("NASTECH_KANBAN_TASK") if _is_dispatcher_owned_worker() else None
    now = time.monotonic()
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        try:
            ...
            return tool_error(
                ...
                f"to finish, or an operator can run `nastech kanban complete --force {tid}`.")
...
def _handle_request_review(args: dict, **kw) -> str:
    ...
    if reviewer:
        from nastech_cli.profiles import list_profile_names, profile_exists
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    from nastech_cli import kanban_db_dispatch as kbd
    with _board(args.get("board")) as (kb, conn):
        # The dispatcher pins NASTECH_KANBAN_CLAIM_LOCK at spawn; the default
        # claimer covers locally-driven workers that bypassed the dispatcher.
        kb.heartbeat_claim(conn, tid, claimer=os.environ.get("NASTECH_KANBAN_CLAIM_LOCK"))
...
def _download_url_with_cap(url: str, max_bytes: int) -> tuple[bytes, Optional[str]]:
    ...
    for _ in range(_MAX_ATTACH_URL_REDIRECTS + 1):
        ...
        chunks: list[bytes] = []
        total = 0
        with httpx.stream("GET", current_url, headers={"User-Agent": "nastech-kanban/attach"},
...
def _persisted_session_id(session_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        state = SessionDB(db_path=get_nastech_home() / "state.db", read_only=True)
...
def _handle_create(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        self_tid = (os.environ.get("NASTECH_KANBAN_TASK")
        ...
        session_id = (_persisted_session_id(args.get("session_id"))
                      ...
                      or _persisted_session_id(get_session_env("NASTECH_SESSION_ID", "")))
        ...
        new_tid = kb.create_task(
            ...
            parents=tuple(parents), tenant=args.get("tenant") or os.environ.get("NASTECH_TENANT"),
...
def _resolve_notify_target() -> Optional[dict[str, Any]]:
    ...
    Gateway sessions: ``NASTECH_SESSION_PLATFORM``/``CHAT_ID`` ContextVars. TUI/desktop:
    those are cleared but the subprocess inherits ``NASTECH_SESSION_KEY`` -> ``platform="tui"``
    for the TUI poller. ``NASTECH_SESSION_ID`` is deliberately NOT a fallback: it is set for
    ...
    platform, chat_id = env("NASTECH_SESSION_PLATFORM", ""), env("NASTECH_SESSION_CHAT_ID", "")
    if not platform or not chat_id:
        session_key = env("NASTECH_SESSION_KEY", "") or os.environ.get("NASTECH_SESSION_KEY", "")
        if not session_key:
            return None
    ...
    delivery_metadata: dict[str, Any] = {
        k: v for k, v in (
            ...
            ("scope_id", env("NASTECH_SESSION_SCOPE_ID", "")),
            ("parent_chat_id", env("NASTECH_SESSION_PARENT_CHAT_ID", "")),
        ) if v}
    ...
    return dict(
        ...
        user_id=env("NASTECH_SESSION_USER_ID", "") or None,
        user_id_alt=env("NASTECH_SESSION_USER_ID_ALT", "") or None,
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    try:
        ...
        from nastech_cli import kanban_db_notify as _kbn
