"""Managed ("Nastech Subscription") image generation: one picker row, one model catalog.

Three gateways sit behind the single stored selection ``image_gen.provider: nastech``; the stored
...
* ``portal`` — Nastech Portal chat-completions image models (``plugins/image_gen/openrouter``'s
  ``nastech`` provider), anything else.

Before this module the second and third gateways each had their own picker row that also
wrote ``provider: nastech`` — every managed row read "active" at once and picking the Portal row
...
def managed_image_catalog(
    *, include_krea: bool = True, include_portal: bool = True,
    ...
    if include_portal:
        for row in _plugin_rows("nastech"):
            mid = row["id"]
