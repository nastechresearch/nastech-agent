...
several Nastech profiles from one process. Terminal settings were historically
...
_terminal_scope_var: ContextVar = ContextVar("nastech_terminal_scope", default=None)
...
def build_profile_terminal_scope(nastech_home: "Any") -> Dict[str, str]:
    ...
    defaults.setdefault("sandbox_dir", "")    # tool derives NASTECH_HOME path
    ...
    #    NASTECH_HOME override so the profile's own file is consulted; a
    ...
    override_token = None
    if get_nastech_home_override() != str(home):
        override_token = set_nastech_home_override(home)
    try:
        ...
        if config_path.exists():
            ...
            from nastech_cli.config import fast_safe_load
...
def install_profile_terminal_scope(nastech_home: "Any") -> Token:
    ...
    """
    try:
        return set_terminal_scope(build_profile_terminal_scope(nastech_home))
...
def install_and_reset_profile_terminal_scope(
    ...
    """
    token = install_profile_terminal_scope(nastech_home)
    try:
        yield
    finally:
        reset_terminal_scope(token)
