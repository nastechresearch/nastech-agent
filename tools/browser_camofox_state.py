...
With managed persistence enabled, Nastech sends a deterministic userId derived from the
...
def get_camofox_state_dir() -> Path:
    ...
    return get_nastech_home() / "browser_auth" / "camofox"


def get_camofox_identity(task_id: Optional[str] = None) -> Dict[str, str]:
    """Stable Nastech-managed Camofox identity: userId is profile-scoped, session key is
    ...
    return {"user_id": f"nastech_{user_digest}", "session_key": f"task_{session_digest}"}
