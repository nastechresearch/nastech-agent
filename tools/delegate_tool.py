...
def _blocked_toolsets_for_role(role: str) -> List[str]:
    ...
    mixed platform bundles such as ``nastech-cli`` because those also contain
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Nastech Portal).
    """
    ...
    # Blocked tools also live inside mixed platform bundles (nastech-cli,
    # nastech-telegram, etc.) that _strip_blocked_tools must keep because they
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Nastech Portal).
    ...
    #
    # Nastech Portal is dual-wire within a single provider: anthropic/* → Messages,
    ...
    # reverse). agent_init honors an explicit api_mode above its nastech branch, so
    # re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    _effective_provider_norm = (effective_provider or "").strip().lower()
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif _effective_provider_norm in {"nastech", "nastech-portal", "nastechresearch"}:
        from nastech_cli.providers import nastech_api_mode

        effective_api_mode = nastech_api_mode(effective_model)
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nastech_dir("cache/delegation", "delegation_cache")
...
def _finalize_child_results(
    ...
    with _parent_finalization_lock(parent_agent):
        ...
        parent_session_id = getattr(parent_agent, "session_id", None)
        try:
            from nastech_cli.plugins import invoke_hook as invoke_hook
        except Exception:
            invoke_hook = None
...
def delegate_task(
    ...
    if background:
        ...
        if not _async_ok:
            ...
            # with that id in X-Nastech-Session-Id once the batch completes.
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "one-shot runner such as `nastech -z`, a cron job, a Kanban "
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, nastech, zai, kimi-coding, minimax."
        ) from exc
