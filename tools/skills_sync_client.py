...
tick ``maybe_pull_skills`` and ``nastech sync``. Lives under tools/ so it never imports the CLI at
module load; ``skills_sync_client_wire`` / ``skills_sync_client_org`` are re-exported here.
ACCESS GATE (pre-launch): INERT unless the user is a Nastech admin per the ``tool_gateway_admin``
...
cross-device state is the ``sync-manifest`` blob in the plane. Only ~/.nastech/skills/ skills qualify."""
...
# Gate claim (NAS's wire name; means "Nastech admin" / Permissions.ADMIN_ACCESS). The bearer comes
# from resolve_nastech_runtime_credentials(); its payload is decoded unverified to read this.
NASTECH_ADMIN_CLAIM = "tool_gateway_admin"


class SyncInertError(RuntimeError):
    """Sync must no-op: not logged in, no bearer, or not a Nastech admin. Caught by the gate-and-swallow hooks."""


def resolve_identity() -> Dict[str, Any]:
    """``{api_key, base_url, owner, nastech_admin, claims}``; SyncInertError if not logged in / no bearer.
    ...
    try:
        ...
        creds = resolve_nastech_runtime_credentials() or {}
    ...
    return {"api_key": api_key, "base_url": creds.get("base_url"), "owner": str(owner),
            "nastech_admin": claims.get(NASTECH_ADMIN_CLAIM) is True, "claims": claims}


# Configuration -- env-first so Nastech Cloud can enable sync via environment alone. Every knob:
# NASTECH_SYNC_<KEY> env -> config.yaml ``sync.<key>`` -> default (base_url = the sync plane, NOT
...
def resolve_sync_base_url() -> Optional[str]:
    """NASTECH_SYNC_BASE_URL -> ``sync.base_url`` -> production plane, without trailing slash
...
def sync_feature_enabled() -> bool:
    """Master switch; the gate-and-swallow entrypoints ALSO require the Nastech-admin gate and a base URL."""
    return _sync_config_bool("NASTECH_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """False (default): edits to an org skill stay LOCAL until ``nastech sync propose``. True: every
    edit is proposed right away (an admin still approves unless the editor is one)."""
    return _sync_config_bool("NASTECH_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False)


def sync_default_opt_in() -> bool:
    """False (default): opt-IN -- a skill syncs only after ``nastech sync enable`` or a plane manifest
    opting it in. True: opt-OUT -- every eligible skill syncs unless disabled (Nastech Cloud default)."""
...
def _rel_to_skills_dir(skill_dir: Path) -> Optional[Path]:
    """*skill_dir* relative to ~/.nastech/skills/, or None if outside/unresolvable."""
    try:
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """The skill's path relative to ~/.nastech/skills/ (posix), or None."""
...
def _all_local_skill_names() -> List[str]:
    """Every local skill name (dir under ~/.nastech/skills/ with SKILL.md; frontmatter ``name`` or dir name)."""
    names: List[str] = []
    root = _skills_dir()
    try:
...
def stable_device_id() -> str:
    """Per-device label at ~/.nastech/skills/.sync_device_id. An existing file always wins; else seeded
    from NASTECH_SYNC_DEVICE_NAME (first use only, for Nastech Cloud) or a friendly default, then persisted."""
    with suppress(OSError):
        val = _device_id_path().read_text(encoding="utf-8").strip()
        if val:
            return val
    val = (os.environ.get("NASTECH_SYNC_DEVICE_NAME") or "").strip() or _default_device_label()
    try:
...
# the bundled manifest (skills_sync.py) and the plane's `sync-manifest`. ~/.nastech/skills/.sync_state.
...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
                identity: Optional[Dict[str, Any]] = None, message: str = "nastech skill sync") -> Dict[str, Any]:
...
def _gate_and_swallow(op: str, run: Callable[[Dict[str, Any]], Optional[Dict[str, Any]]]):
    """Run *run(identity)* only if all gates hold (Nastech admin, feature on, base URL); None if inert/error."""
    try:
        identity = resolve_identity()
        if not identity.get("nastech_admin") or not sync_feature_enabled() or not resolve_sync_base_url():
            return None
        return run(identity)
    except Exception as e:
...
def sync_status() -> Dict[str, Any]:
    """Snapshot for ``nastech sync status``; never raises. ``org_available`` False = not in a shared org."""
    status: Dict[str, Any] = {"nastech_admin": False, "logged_in": False, "feature_enabled": sync_feature_enabled(),
    ...
    try:
        identity = resolve_identity()
        status.update(logged_in=True, owner=identity.get("owner"), nastech_admin=bool(identity.get("nastech_admin")))
