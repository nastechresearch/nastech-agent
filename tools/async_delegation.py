...
def _connect() -> sqlite3.Connection:
    from nastech_cli.sqlite_util import open_db
    # Same state.db as nastech_state.SessionDB -- reuse its owner-only (0600)
    # hardening so this writer doesn't create/leave the file (and its WAL
    # sidecars) at the process umask. See nastech_state._secure_state_db_files.
    from nastech_state import _secure_state_db_files
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from nastech_state_repair import apply_durability_barriers
    from nastech_state_schema import reconcile_state_schema
...
def _transaction():
    from nastech_cli.sqlite_util import transaction
...
def _capture_routing_origin() -> Dict[str, Any]:
    ...
    try:
        ...
        return {k: v for k in _ROUTING_KEYS if (v := get_session_env(f"NASTECH_SESSION_{k.upper()}", ""))}
...
def _current_origin_session_id() -> str:
    """Raw session id of the ORIGINATING api_server request, or ``""``. ``NASTECH_SESSION_ID``
    ...
    request-scoped ``NASTECH_SESSION_CHAT_ID`` (raw X-Nastech-Session-Id on api_server) survives
    child construction; on push platforms chat_id is a chat, not a session => ``""``."""
    try:
        from gateway.session_context import get_session_env
        is_api = get_session_env("NASTECH_SESSION_PLATFORM", "") == "api_server"
        return (get_session_env("NASTECH_SESSION_CHAT_ID", "") or "") if is_api else ""
    except Exception:
        return ""
