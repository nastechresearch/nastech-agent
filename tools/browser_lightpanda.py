...
def find_lightpanda_binary() -> Optional[str]:
    ...
    fallbacks), then installer/agent-browser locations, then ``$NASTECH_HOME/bin``. No Windows build."""
    if os.name == "nt":
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Nastech is gone; return the count. A live owner is
