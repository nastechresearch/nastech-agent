...
left behind by a crashed Nastech.
"""
...
def _home_candidates() -> list:
    ...
    try:
        ...
        candidates.append(Path(get_nastech_home()) / "bin" / "lightpanda")
    except Exception as e:  # pragma: no cover - defensive
        logger.debug("nastech home unavailable for lightpanda lookup: %s", e)
    return candidates
...
def find_lightpanda_binary() -> Optional[str]:
    ...
    ``$NASTECH_HOME/bin/lightpanda``. Lightpanda has no Windows build.
    """
    if os.name == "nt":
...
def _state_dir() -> Path:
    ...
    path = Path(get_nastech_home()) / "cache" / "browser-use" / "lightpanda"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _http_cache_dir() -> Path:
    """Filesystem HTTP cache shared by every Lightpanda this Nastech spawns.
    ...
    bounds it without Nastech managing eviction.
    """
...
def launch_lightpanda(
    ...
    ``$NASTECH_HOME/cache/browser-use/lightpanda/<session>.log`` so a chatty
    ...
    if not binary:
        if os.name == "nt":
            return None, (
                ...
                "build. Set browser.engine to auto (or run Nastech under WSL2)."
            )
        return None, (
...
def reap_orphaned_lightpanda() -> int:
    """Kill ``lightpanda serve`` processes whose owning Nastech is gone.

    Records are written by :func:`launch_lightpanda`; a live owner (another
    Nastech process, or this one still tracking the session) is never
