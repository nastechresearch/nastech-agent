...
def managed_nastech_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Return True when the user is entitled to the Nastech Tool Gateway.

    Entitlement is paid Nastech Portal service access OR a live free tool pool
...
def nastech_tool_gateway_unavailable_message(
    capability: str = "the Nastech Tool Gateway",
    *,
    force_fresh: bool = False,
) -> str:
    """Return account-aware guidance for an unavailable Nastech Tool Gateway path."""
    try:
    ...
    return (
        ...
        "Nastech Portal login and billing status."
    )
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    if managed_enabled is None:
        managed_enabled = managed_nastech_tools_enabled()
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``nastech auth add
    ...
    2. The environment / ``~/.nastech/.env``. Under a multiplexed gateway turn
       ...
       ``nastech_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``nastech auth
    ...
    when omitted, ``nastech_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `nastech auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``nastech auth add openai-api``) via
...
# The provider value the managed "Nastech Subscription" picker rows write for
# every category (image_gen.provider: nastech, web.backend: nastech,
# browser.cloud_provider: nastech, ...). Runtime dispatch is a plain switch on
# the stored string: "nastech" → managed gateway client; any vendor name → that
# vendor direct with the user's own credentials; no key ever written →
# legacy credential autodetect.
NASTECH_MANAGED_PROVIDER = "nastech"
...
def read_selection(section: str) -> str | None:
    """Return the stored `nastech tools` provider string for a config section.

    THE single runtime read of the persisted selection. Returns:
    - ``"nastech"`` — the managed Nastech Tool Gateway row was selected,
    ...
    key. ``use_gateway: true`` was only ever written by the managed "Nastech
    Subscription" row, so it maps to ``"nastech"`` regardless of the name key;
    ...
    if "use_gateway" in raw and is_truthy_value(raw.get("use_gateway"), default=False):
        return NASTECH_MANAGED_PROVIDER
...
# config check (nastech_cli.config.validate_config_structure) and
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    if note:
        failure = note
    return (
        f"{section} is configured to use {selection_name} (set via nastech "
        f"tools), but {failure}. Run 'nastech tools' to change it."
    )
