...
def managed_nastech_tools_enabled(*, force_fresh: bool = False) -> bool:
    """Coarse gate: entitled to the Nastech Tool Gateway (paid Portal access OR a live free
    ...
    try:
        ...
        account_info = (get_nastech_portal_account_info(force_fresh=True) if force_fresh
                        else get_nastech_portal_account_info())
        return bool(account_info.logged_in) and account_info.tool_gateway_entitled
    except Exception:
        return False


def nastech_tool_gateway_unavailable_message(capability: str = "the Nastech Tool Gateway", *,
    ...
    try:
        ...
        message = format_nastech_portal_entitlement_message(
            get_nastech_portal_account_info(force_fresh=force_fresh), capability=capability,
            in_chat=True)
        if message:
            return message
    except Exception:
        pass
    return (f"{capability} is unavailable. Run `nastech model` to refresh your "
...
def _dotenv_value(env_var: str) -> str:
    """``.env`` value via ``nastech_cli.config.get_env_value`` (``""`` when unavailable)."""
    try:
...
def resolve_provider_secret(env_var: str, provider_id: str, config_value: str = "",
    ...
    ``nastech_cli.config.get_env_value``) -> credential pool for ``provider_id``. Under an
    ...
    Resolution order (fixes #68003 — keys added via ``nastech auth add <provider>`` were invisible to the
...
def resolve_openai_audio_api_key() -> str:
    ...
    Outside a multiplexed turn, ``OPENAI_API_KEY`` additionally falls back to the credential pool (``nastech
...
# Provider value the managed "Nastech Subscription" picker rows write for every category;
...
def read_selection(section: str) -> str | None:
    """THE single runtime read of the persisted `nastech tools` selection: ``"nastech"`` (managed
    ...
    to ``"nastech"`` regardless of the name key. Never raises."""
...
# (nastech_cli.config.validate_config_structure) and selection_error() consult this map so the user learns
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    return (f"{section} is configured to use {selection_name} (set via nastech "
