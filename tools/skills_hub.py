...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _nastech_home() / "skills"
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Nastech Research) but not activated
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Nastech-native bundle
        # under pbakaus/impeccable:.nastech/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``nastech update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.nastech.upstream`` in its SKILL.md
        frontmatter:
            ...
            metadata:
              nastech:
                upstream:
                  ...
                  path: .nastech/skills/impeccable
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.nastech.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(nastech_meta, dict):
            return None
        upstream = nastech_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``nastech_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
NASTECH_INDEX_URL = "https://nastechresearch.github.io/nastech-agent/docs/api/skills-index.json"
NASTECH_INDEX_TTL = 6 * 3600  # 6 hours


def _nastech_index_cache_file() -> Path:
    return _index_cache_dir() / "nastech-index.json"
...
def _load_nastech_index() -> Optional[dict]:
    ...
    """
    # Check local cache
    nastech_index_cache_file = _nastech_index_cache_file()
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            logger.debug(
                "Nastech index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
    ...
    try:
        ...
        nastech_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
