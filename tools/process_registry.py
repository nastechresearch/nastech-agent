...
# ---------------------------------------------------------------------------
# When Nastech runs as a systemd gateway with MemoryHigh/MemoryMax limits,
...
#
# Wrapping the spawn in ``systemd-run --user --scope --unit=nastech-worker-<pid>``
...
def _is_supervised_gateway_process() -> bool:
    """Return whether this process is in a supervised Nastech gateway runtime.

    Both supervisor markers and ``_NASTECH_GATEWAY`` are inherited by every
...
def _build_systemd_scope_argv(
    ...
    if binary is None:
        ...
        return shell_argv
    unit_name = f"nastech-worker-{unit_suffix}"
...
class ProcessRegistry:
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from nastech_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
        ...
        if use_pty:
            ...
            try:
                ...
                if pty_use_systemd_scope:
                    ...
                    session.systemd_unit = f"nastech-worker-{session.id}.scope"
    ...
    def is_session_waiting(self, session_id: str) -> bool:
        ...
        Used by the goal-loop wait barrier (``nastech_cli.goals``) to support
    ...
    def wait_for_pending_completions(
        ...
        *,
        ...
        One-shot CLI runs (``nastech -q/-Q/-z``) exit as soon as their single
        ...
        (#90879): a recipient invoked as ``nastech -p <bot> chat -Q
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Nastech desktop app.",
            }
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            ...
                            # ~/.nastech/processes.json with the raw command
...
def _delegation_model_not_found_notice(results) -> "list[str] | None":
    ...
    lines = [
        ...
        "nastech config get delegation.model).",
    ]
    try:
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Nastech'}"
    elif _reason == "lost":
