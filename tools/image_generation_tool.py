#!/usr/bin/env python3
"""Image generation via FAL.ai (model picked in ``nastech tools``, persisted to ``image_gen.model``).
...
from tools.tool_backend_helpers import (
    NASTECH_MANAGED_PROVIDER, fal_key_is_configured, managed_nastech_tools_enabled,
    nastech_tool_gateway_unavailable_message, read_selection, selection_error)
...
# --- Managed FAL gateway (Nastech Subscription) ---
def _resolve_managed_fal_gateway():
    """Managed gateway config for the stored `nastech tools` selection, or ``None`` for direct FAL.

    ``"nastech"`` (or legacy ``use_gateway: true``) → managed ONLY (unreachable = selection-naming
    ...
    if selected == NASTECH_MANAGED_PROVIDER:
        gateway = resolve_managed_tool_gateway("fal-queue")
        if gateway is None:
            raise ValueError(selection_error(
                "image_gen", NASTECH_MANAGED_PROVIDER,
                "the Nastech Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
...
def _get_managed_fal_client(managed_gateway):
    ...
    client_config = (managed_gateway.gateway_origin.rstrip("/"), managed_gateway.nastech_user_token)
    with _managed_fal_client_lock:
        if _managed_fal_client is None or _managed_fal_client_config != client_config:
            ...
            _managed_fal_client = _ManagedFalSyncClient(
                _load_fal_client(), key=managed_gateway.nastech_user_token,
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            if billing is not None:
                raise ValueError(
                    f"Nastech Subscription gateway rejected model '{model}' (HTTP {status}): {billing}") from exc
            gateway_message = ""
            if status in {401, 402, 403}:
                gateway_message = "\n\n" + nastech_tool_gateway_unavailable_message(
                    "managed FAL image generation", force_fresh=True)
            raise ValueError(
                f"Nastech Subscription gateway rejected model '{model}' (HTTP {status}). This model "
                f"may not yet be enabled on the Nastech Portal's FAL proxy. Either:\n"
...
def _plugin_provider_name() -> Optional[str]:
    """Configured provider that must go through the plugin registry; None for unset/fal/nastech."""
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", NASTECH_MANAGED_PROVIDER):
        return None
    return configured
...
_CACHE_BASE_BY_BACKEND = {"docker": "/root/.nastech", "singularity": "/root/.nastech",
                          "modal": "/root/.nastech", "ssh": "~/.nastech"}
...
def _build_no_backend_setup_message() -> str:
    ...
    managed = managed_nastech_tools_enabled()
    ...
    if managed:
        ...
        if gateway_message := nastech_tool_gateway_unavailable_message("managed FAL image generation"):
    ...
    if managed:
        lines.append("  2. Sign in to a Nastech account that has the managed FAL gateway enabled "
                     "(`nastech setup`)")
    lines.append("  3. Configure a different image_gen provider via `nastech tools` → Image Generation "
                 "(run `nastech plugins list` to see installed backends)")
    return "\n".join(lines)
...
def _dispatch_to_plugin_provider(
    ...
    (provider unset / ``"fal"`` / ``"nastech"``). Providers without ``upscale`` ignore it via ``**kwargs``."""
    configured = _plugin_provider_name()
    if configured is None:
        return None
    try:
    ...
    if provider is None:
        return _provider_error(
            ...
            f"Run `nastech plugins list` to see available image gen backends.", "provider_not_registered")
    ...
    try:
        ...
        if is_type_error and ("image_url" in kwargs or "reference_image_urls" in kwargs):
            ...
            return _provider_error(
                ...
                f"text-to-image, or pick a backend that supports editing via `nastech tools` → "
...
def _managed_model_plugin() -> Optional[tuple]:
    ...
    The managed row writes ``provider: nastech`` for three gateways; the model id says which. FAL
    models (and an unset model) return ``None`` so the in-tree FAL path handles them. Only the
    ``nastech``/unset selection qualifies — a direct/BYO provider pick dispatches normally.
    """
    from tools.image_generation_managed import KREA, PORTAL, managed_backend_for_model

    configured_provider = _read_configured_image_provider()
    if configured_provider is not None and configured_provider != NASTECH_MANAGED_PROVIDER:
        return None
    model_id = _read_configured_image_model()
    backend = managed_backend_for_model(model_id)
    if backend == KREA:
        return "krea", model_id
    if backend == PORTAL and configured_provider == NASTECH_MANAGED_PROVIDER:
        return "nastech", model_id
    return None
...
def _maybe_route_managed_model(
    ...
    if provider is None:
        if plugin_name == "krea":
            return None
        return _provider_error(
            f"image_gen.model='{model_id}' is a Nastech Portal model but the Portal image backend is not "
            f"available. Pick another model via `nastech tools` → Image Generation.", "provider_not_registered")
...
def _handle_image_generate(args, **kw):
    ...
    # Portal — only under the "nastech"/unset selection, so BYO/direct FAL stays untouched), then FAL.
