...
FAL_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "fal-ai/nano-banana-pro": {
        ...
        "defaults": {
            ...
            # Users on Nastech Subscription should stay at 1K for predictable billing.
            "resolution": "1K",
        },
        "supports": {
    ...
    "fal-ai/gpt-image-2": {
        ...
        "defaults": {
            # Same quality pinning as gpt-image-1.5: medium keeps Nastech
...
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `nastech tools` selection.

    Dispatch is a plain switch on the stored ``image_gen`` provider string:
    - ``"nastech"`` (or legacy ``use_gateway: true``) → managed fal-queue
...
def _submit_fal_request(model: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                f"Nastech Subscription gateway rejected model '{model}' "
                f"(HTTP {status}). This model may not yet be enabled on "
                f"the Nastech Portal's FAL proxy. Either:\n"
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `nastech tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    Used by the in-tree FAL path. Mentions:
      - FAL_KEY signup link
      - managed-gateway status (if Nastech tools are enabled)
    ...
    if managed_nastech_tools_enabled():
        lines.append(
            "  2. Sign in to a Nastech account that has the managed FAL "
...
def _dispatch_to_plugin_provider(
    ...
    configured = _read_configured_image_provider()
    if not configured or configured in ("fal", NASTECH_MANAGED_PROVIDER):
        # Unset/explicit FAL keeps the legacy FAL path; "nastech" (managed
        # Nastech Subscription selection) also runs the legacy pipeline, which
...
def _maybe_route_managed_krea(
    ...
    # (other than the managed "nastech" selection, which IS a managed-mode
