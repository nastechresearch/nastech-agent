...
(also serves the managed ``nastech`` selection), mistral, xai, elevenlabs, deepinfra; plus
...
def _resolve_stt_language(
    ...
    (plus *extra_keys* aliases, e.g. ``language_code``) > ``stt.language`` > ``NASTECH_LOCAL_STT_LANGUAGE``
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Whether *provider* is exempt from Nastech's remote upload cap."""
...
def _get_provider(stt_config: dict) -> str:
    ...
    # The managed "Nastech Subscription" selection is the OpenAI backend routed via the managed gateway.
...
def _start_idle_unload_watcher(timeout_seconds: int) -> None:
    ...
    with _idle_unload_mgmt_lock:
        ...
        _idle_unload_thread = threading.Thread(target=_watch, name="nastech-stt-idle-unload", daemon=True)
...
def _no_provider_error(provider: str, stt_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Surface it — with its `nastech tools` remediation — instead of the all-provider setup hint (#93045).
