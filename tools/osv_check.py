...
# ladders, parked-server self-probes and repeated `nastech mcp test` runs re-run the preflight
...
# The cache is also persisted under the Nastech home so separate processes and gateway
...
def _disk_cache_path() -> Optional[Path]:
    ...
    Uses ``nastech_constants.get_nastech_home()`` so the cache follows the
    active profile and is isolated across Nastech homes. The cache directory
    is created on demand. Returns ``None`` when Nastech home cannot be
...
def _query_osv(package: str, ecosystem: str, version: Optional[str] = None) -> list:
    ...
    req = urllib.request.Request(
        ...
        headers={"Content-Type": "application/json", "User-Agent": "nastech-agent-osv-check/1.0"},
        method="POST")
