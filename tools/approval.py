...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart nastech gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "nastech update (restarts gateway, kills running agents)"),
