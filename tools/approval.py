...
# Nastech session id (observability identity, distinct from the gateway
...
# os.environ["NASTECH_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export NASTECH_INTERACTIVE.
_nastech_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "nastech_interactive",
    default=None,
)


def set_nastech_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["NASTECH_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``NASTECH_INTERACTIVE`` env var for legacy callers.
    """
    return _nastech_interactive_ctx.set("1" if interactive else "")


def reset_nastech_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_nastech_interactive_context`."""
    _nastech_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``NASTECH_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _nastech_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("NASTECH_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Nastech session id so observer plugins parent approval
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("NASTECH_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("NASTECH_CRON_SESSION")
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    ``NASTECH_SESSION_PLATFORM`` like chat gateways do, but there is no human
...
def _is_single_query_approval_context() -> bool:
    ...
    ``nastech chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``NASTECH_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("NASTECH_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("NASTECH_SINGLE_QUERY_SESSION")
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``NASTECH_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_nastech_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Nastech: this command exceeded the inline command\n"
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Nastech
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]nastech[^\n]*\.env\b', "access to Nastech secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `nastech` and `gateway` (e.g. `nastech -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bnastech\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart nastech gateway (kills running agents)"),
    ...
    # a lifecycle command past the guard — same treatment as the `nastech ...
    ...
    # the `nastech gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.nastech.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "nastech"/"ai.nastech" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.nastech.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "nastech" appear
    ...
    # an unrelated "nastech" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:nastech|ai\.nastech)\b)', "stop/restart nastech launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Nastech file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.nastech/config.yaml/.env), but cp/mv/install was only paired for /etc and
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.nastech/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / NASTECH_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Nastech home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\nastech), so folding the user home
    # first would eat the prefix the Nastech-home fold needs.
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / NASTECH_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.nastech``); the matched
...
def _rewrite_resolved_nastech_home(command: str) -> str:
    ...
    /home/nastech/.nastech/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$NASTECH_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.nastech")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``nastech send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Nastech ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"nastech-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart nastech gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.nastech.gateway`` auto-approved
    ...
    anchored on a nastech-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Nastech YOLO.
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Nastech approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``NASTECH_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            NASTECH_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    # Single-query (-q) sessions export NASTECH_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set NASTECH_INTERACTIVE or "
                "NASTECH_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set NASTECH_INTERACTIVE or "
            "NASTECH_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}
    ...
    if is_gateway or env_var_enabled("NASTECH_EXEC_ASK"):
        ...
        # can see (NASTECH_EXEC_ASK / platform-marker leaks into CLI).
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``NASTECH_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from nastech_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from nastech_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # NASTECH_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from nastech_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # Ask-mode (NASTECH_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # NASTECH_EXEC_ASK (and sometimes a session platform marker) can leak
