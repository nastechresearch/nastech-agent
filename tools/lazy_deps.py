...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Nastech, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # HF Agent Trace Viewer upload (nastech trace upload / /upload-trace).
    #
    ...
    # presence, the `nastech update` lazy-refresh pass re-asserts THIS pin on
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``nastech update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Nastech under WSL to use Matrix on Windows."
        )
    return None
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Nastech
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Nastech cannot install "
