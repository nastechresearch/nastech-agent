...
_SYNC_BACK_TEMP_PREFIX = "nastech-sync-back-"
...
def iter_sync_files(container_base: str = "/root/.nastech") -> list[tuple[str, str]]:
    ...
    remapped from the hardcoded /root/.nastech to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.nastech", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``NASTECH_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def _sync_transaction(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="nastech-sync-push-") as staging:
                staged_files = []
                pushed_hashes = {}
    ...
    def sync_back(self, nastech_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.nastech/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(nastech_home=nastech_home)

    def _sync_back_transaction(self, nastech_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .nastech/.
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.nastech/skills/b.md`` -> ``~/.nastech/skills/b.md``)."""
