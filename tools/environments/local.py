...
#
# get_temp_dir() now defaults to NASTECH_HOME/cache/terminal (real storage)
...
#
# Background-process artifacts come in triplets (nastech_bg_<id>.log/.pid/
...
_BG_GROUP_RE = re.compile(r"^(nastech_bg_[A-Za-z0-9_-]+)\.(log|pid|exit)$")


def _default_terminal_temp_dir() -> "Path | None":
    """Return NASTECH_HOME/cache/terminal, or None if unresolvable."""
    try:
        from nastech_constants import get_nastech_home
        return get_nastech_home() / "cache" / "terminal"
    except Exception:
        return None
...
def cleanup_terminal_temp_cache(
    ...
    Only prunes the managed default dir (``NASTECH_HOME/cache/terminal``).
    ...
    # Newest mtime per nastech_bg_<id> group, so a live server's fresh .log
...
def _resolve_local_initial_cwd(cwd: str) -> str:
    ...
    directory Nastech was already launched from (for example ``nastech-agent``
    while the process cwd is ``~/.nastech/nastech-agent``), passing it through
    unchanged makes the wrapper run ``cd nastech-agent`` *inside* the project
    ...
    # Common recovery for config values like ``nastech-agent`` when Nastech was
    # launched from that directory already.  ``os.path.abspath`` would point at
    # a nonexistent nested ``./nastech-agent``; use the current directory instead.
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Nastech-managed inference credential — Claude subscription auth is not a
    # working Nastech provider path.  Stripping it broke agent-spawned
    ...
    # the :func:`nastech_subprocess_env` Tier-2 strip (browser / TUI host /
...
# ``buzz`` (the gateway's ``NASTECH_SESSION_PLATFORM`` ContextVar; concurrency
...
# :func:`nastech_subprocess_env` (browser / TUI host / copilot-executor
...
# from the blocklist entirely because it is NOT a Nastech credential; these ARE
# Nastech-managed first-party platform credentials, so they stay IN the
...
def _buzz_terminal_context_active() -> bool:
    ...
    * ``BUZZ_MANAGED_AGENT`` in the process env — set exclusively by Buzz
      Desktop's buzz-acp harness when it spawns ``nastech acp`` (#76243).
      Gateway / CLI / cron / kanban processes never carry it.
    * The live session's platform is ``buzz`` — the gateway's
      ``NASTECH_SESSION_PLATFORM`` ContextVar via
    ...
    try:
        ...
        return get_session_env("NASTECH_SESSION_PLATFORM", "").strip().lower() == "buzz"
    except Exception:
        return False
...
# project's dependencies into the Nastech venv path instead of the project's own
# ``.venv`` — silently clobbering the Nastech environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Nastech venv stays reachable via PATH (its bin dir is first), so stripping
...
# system/venv Pythons — to the Nastech venv's stdlib, which crashes with
# version-mismatch errors before a child script even imports a package
# (#75018). Nastech itself treats PYTHONHOME as contamination in its own
...
# _strip_nastech_owned_pythonpath() which removes only Nastech-owned entries,
...
def _is_nastech_internal_secret(key: str) -> bool:
    ...
    This is the single source of truth for "Nastech-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`nastech_subprocess_env` helper all call it, so the
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    # the separate Nastech runtime venv.  The filter validates that relationship
    # against the repo layout before trusting it.
    _strip_nastech_owned_pythonpath_and_runtime_markers(sanitized)

    # Keep bare ``nastech`` invocations available to child jobs even when the
    ...
    if path_key is not None:
        sanitized[path_key] = _prepend_nastech_bin_dir(sanitized.get(path_key, ""))
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_nastech_internal_secret, but _ID has no secret suffix, so it must be
...
def nastech_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    * **Tier 2 (conditional):** the rest of ``_NASTECH_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _plugin_terminal_env_strip_keys():
        env.pop(key, None)
    # Internal routing hints and Nastech-internal dynamic secrets
    ...
    _inject_context_nastech_home(env)
    from nastech_constants import apply_subprocess_home_env
    apply_subprocess_home_env(env)

    _strip_nastech_owned_pythonpath_and_runtime_markers(env)
...
def _git_bash_aslr_help(bash: str, details: str = "") -> str:
    ...
    return (
        ...
        "Then restart Nastech. If the override is blocked or later re-applied, "
...
def _resolve_nastech_bin_dir() -> str | None:
    """Return the directory holding the ``nastech`` console-script, or None.
    ...
    nastech install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``nastech``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``nastech`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``nastech`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("nastech")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``nastech`` executable (covers nix-store / venv wrappers).
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "nastech" or base.startswith("nastech."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "nastech.exe" if _IS_WINDOWS else "nastech"
...
def _prepend_nastech_bin_dir(existing_path: str) -> str:
    """Prepend the nastech install dir to ``existing_path`` if it's missing.
...
def _managed_runtime_path_entries() -> list[str]:
    ...
    ``get_nastech_home()`` is profile-scoped and a managed tree can appear
    mid-process (``heal_nastech_managed_node``, a first browser install).
    """
    try:
        from nastech_constants import get_nastech_home, iter_nastech_node_dirs

        candidates = [*iter_nastech_node_dirs(), get_nastech_home() / "bin"]
...
def _append_missing_sane_path_entries(existing_path: str) -> str:
    ...
    Nastech-managed runtime dirs are appended alongside the sane entries, not
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Nastech
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_NASTECH_PROVIDER_ENV_FORCE_PREFIX):
            ...
            if k in _NASTECH_PROVIDER_ENV_BLOCKLIST and not (passthrough or first_party):
                continue
    ...
    if path_key is not None:
        ...
        # Ensure the nastech install dir is reachable so plugins can shell out
        # to bare ``nastech`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_nastech_bin_dir(new_path)
    ...
    _strip_nastech_owned_pythonpath_and_runtime_markers(run_env)
...
def _build_nastech_repo_root_aliases(
    resolved_root: Path,
    lexical_root: Path,
    configured_home: Path,
) -> tuple[Path, ...]:
    """Return exact repo-root spellings emitted by Nastech launchers.

    ``gateway_windows._preserve_nastech_home_path`` maps a physical path under
    the resolved NASTECH_HOME back onto the configured NASTECH_HOME spelling.
    Mirror that producer contract here so a junction-backed install is matched
    without treating arbitrary descendants of NASTECH_HOME as Nastech-owned.
    ...
    # lexically the same way get_default_nastech_root() does (parent of a
    ...
    # while still never matching arbitrary descendants of NASTECH_HOME.
    ...
    # junction/symlink under the configured root (e.g. D:\nastech\nastech-agent
    # -> C:\...\nastech-agent) while the import spelling (editable install)
    ...
    # strict resolve before accepting it as Nastech-owned.  Fail-closed: a
...
#: The Nastech repository root - three levels up from this file
#: (``tools/environments/local.py`` -> ``tools/environments`` -> ``tools``
#: -> repo root).  This is the directory the Electron app prepends to
#: PYTHONPATH so the backend can do ``import tools``, ``import nastech_cli``,
#: etc.  Subprocesses that are NOT the Nastech backend don't need it and it
#: can shadow local packages.
_nastech_repo_root: Path = Path(__file__).resolve().parents[2]

#: Alternate spellings of the repo root that Nastech launchers may emit.
#: ``Path(__file__).resolve()`` canonicalizes symlinks/junctions, but the
#: Windows gateway launcher deliberately renders Nastech-owned paths under
#: the configured NASTECH_HOME spelling (which may be a junction to another
#: drive — see ``nastech_cli/gateway_windows.py::_preserve_nastech_home_path``).
...
_nastech_repo_root_aliases: tuple[Path, ...] = _build_nastech_repo_root_aliases(
    _nastech_repo_root,
...
#: subprocess spawn.
_nastech_site_packages: list[Path] | None = None
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    ``<Nastech repo>/venv`` layout and a real venv marker; require both before
    ...
    if not any(_same_path(candidate, repo_root / "venv") for repo_root in _nastech_repo_root_aliases):
        return None
...
def _get_nastech_site_packages(env: dict) -> list[Path]:
    """Return exact site-packages dirs owned by the Nastech runtime.
    ...
    """
    global _nastech_site_packages
    if _nastech_site_packages is not None:
        result = list(_nastech_site_packages)
    else:
        result = []
        if _in_venv:
            try:
        ...
        _nastech_site_packages = list(result)
...
def _strip_nastech_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Nastech-owned PYTHONPATH entries, then the runtime marker vars.
    ...
    """
    _strip_nastech_owned_pythonpath(env)
    for _marker in _ACTIVE_VENV_MARKER_VARS:
        env.pop(_marker, None)


def _strip_nastech_owned_pythonpath(env: dict) -> None:
    """Remove Nastech-owned PYTHONPATH entries from subprocess environments.

    Launchers prepend the Nastech repo root and the Nastech venv's
    ...
    user entries, so only entries proven Nastech-owned are removed:
    ...
    nastech_site_packages = _get_nastech_site_packages(env)
    ...
    for entry in pp.split(os.pathsep):
        ...
        # Preserve raw spelling unless the exact component is Nastech-owned.
        if entry == "":
            kept.append(entry)
            continue
        ...
        # --- Check 1: Nastech venv site-packages ---
        ...
        for sp in nastech_site_packages:
        ...
        # resolved and unresolved (NASTECH_HOME/junction) spellings count as
        # Nastech-owned.
        if not should_strip:
            should_strip = any(
                _same_path(entry_path, repo_root)
                for repo_root in _nastech_repo_root_aliases
            )
    ...
    if stripped:
        logger.debug(
            "Stripped Nastech-owned entries from PYTHONPATH: %s",
            stripped,
        )
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Nastech host itself — controller-side platform
    ...
    def _additional_profile_scoped_passthrough_names(self) -> tuple[str, ...]:
        ...
        ``nastech-snap-<id>.sh`` and a later command from profile B sharing
    ...
    def get_temp_dir(self) -> str:
        ...
        **Default (no override set):** a dedicated cache dir under
        ``NASTECH_HOME`` (``~/.nastech/cache/terminal``) rather than ``/tmp``.
        On several distros (Arch and friends) ``/tmp`` is a small RAM-backed
        tmpfs, and Nastech session artifacts — background-process logs,
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        # Honored ahead of the generic TMPDIR so users can redirect Nastech' temp
        ...
        # Default: NASTECH_HOME/cache/terminal — real storage, mirroring the
        # Windows branch above. /tmp is only a last-resort fallback now
        # because RAM-backed tmpfs /tmp fills up under Nastech load.
        try:
            from nastech_constants import get_nastech_home
            cache_dir = get_nastech_home() / "cache" / "terminal"
            cache_dir.mkdir(parents=True, exist_ok=True)
