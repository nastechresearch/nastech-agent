...
def _apply_profile_home(env: dict) -> None:
    """Bridge the context-local NASTECH_HOME override, then the subprocess HOME contract."""
    from nastech_constants import apply_subprocess_home_env, get_nastech_home_override
    try:
        if value := get_nastech_home_override():
...
def _inject_session_context_env(env: dict) -> None:
    """Bridge gateway session ContextVars (NASTECH_SESSION_*) into a child env.
...
def _filter_secret_env(
    ...
    """Copy *items* into *out*, dropping Nastech-managed secrets. ``_NASTECH_FORCE_<NAME>``
    ...
    for key, value in items.items():
        if key.startswith(_NASTECH_PROVIDER_ENV_FORCE_PREFIX):
            if not unwrap_force:
                continue
            key = key[len(_NASTECH_PROVIDER_ENV_FORCE_PREFIX):]
...
def _finalize_child_env(env: dict) -> dict:
    ...
    bridging, Nastech-owned PYTHONPATH + venv-marker strip, MSYS defaults, delegate_task
...
def _scrubbed_env(parts, plugin_strip: frozenset, fix_path) -> dict:
    ...
    *fix_path* (always prepending the nastech install dir so bare ``nastech`` resolves
    ...
    # Keep bare ``nastech`` invocations available to child jobs even when the gateway was launched by a
    ...
    if path_key is not None:
        out[path_key] = _prepend_nastech_bin_dir(fix_path(out.get(path_key, "")))
    return _finalize_child_env(out)


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Nastech-managed secrets from a subprocess environment (background/PTY
...
def served_profile_child_env(
    ...
    """Child env for a process that acts FOR the active (possibly served) profile: ``nastech -p X``
    ...
    (its ``.env`` + hydrated sources, what a standalone ``nastech -p X`` loads itself) are overlaid — never
...
def strip_launch_profile_env(env: dict, target_home: "str | Path | None" = None) -> dict:
    ...
    the secret scrub removes credentials but not settings (``NASTECH_MODEL``, ``TERMINAL_ENV``,
    ``NASTECH_LANGUAGE``...), so a standalone ``nastech -p X`` worker and a served one saw different
...
def _resolve_nastech_bin_dir() -> str | None:
    ...
    elif (os.path.isabs(argv0) and (base == "nastech" or base.startswith("nastech."))
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    and ``wmic``. Nastech runs terminal commands through bash on Windows, so set the standard MSYS opt-out by
    default. Refs #56700.
...
# spellings — the Windows gateway launcher renders Nastech-owned paths under the
# configured NASTECH_HOME spelling (possibly a junction to another drive).
...
_nastech_repo_root_aliases: tuple[Path, ...] = _build_nastech_repo_root_aliases(
    _nastech_repo_root, Path(__file__).absolute().parents[2], get_process_nastech_home())
_in_venv: bool = (getattr(sys, "base_prefix", sys.prefix) != sys.prefix
                  or hasattr(sys, "real_prefix"))  # real_prefix: virtualenv<20
_nastech_site_packages: list[Path] | None = None  # lazily cached by local_pythonpath
...
class LocalEnvironment(BaseEnvironment):
    ...
    # Commands run on the Nastech host itself — controller-side platform behavior
