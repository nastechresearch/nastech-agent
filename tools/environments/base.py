...
def _pipe_stdin(proc: subprocess.Popen, data: str) -> None:
    ...
    recorded on ``proc._nastech_stdin_errors`` while stdin is still closed
    ...
    proc._nastech_stdin_errors = errors
    ...
    proc._nastech_stdin_thread = thread
    thread.start()
...
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    ...
    "NASTECH_CRON_SESSION|NASTECH_BROWSER_CONTROL_)"
)
...
def _export_dump_excluding_session_vars(
    ...
    return (
        ...
        "${!NASTECH_BROWSER_CONTROL_*} "
        # AI_AGENT / NASTECH_AGENT are per-command attribution markers
...
class BaseEnvironment(ABC):
    ...
    # True only when commands execute on the SAME host as the Nastech process
    ...
    def init_session(self):
        ...
        _snap_tmp = '"$__nastech_snap_tmp"'
        snapshot_excluded = self._snapshot_excluded_passthrough_names()
        bootstrap = (
            f"umask 077\n"
            f"__nastech_snap_tmp=$(mktemp {_snap_tmp_template}) || exit 1\n"
            ...
            f"__nastech_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__nastech_fns\" ] && declare -f $__nastech_fns "
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # under Nastech via the cross-agent ``AI_AGENT`` standard (read by e.g.
        # huggingface_hub's agent detection) plus the Nastech-specific
        # ``NASTECH_AGENT`` marker.  The value MUST equal our id in the public
        # agent-harness registry (``nastech-agent`` — see huggingface.js
        ...
        # inherited from the Nastech process.  ``${VAR:-default}`` semantics:
        # never clobber an outer harness value that arrived via the inherited
        # process env (Nastech running inside another agent's terminal).
        parts.append(
            'export AI_AGENT="${AI_AGENT:-nastech-agent}" '
            'NASTECH_AGENT="${NASTECH_AGENT:-true}"'
        )
        ...
        # Restrict Nastech metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__nastech_snap_tmp=$(mktemp {_snap_tmp_template}) && "
    ...
    def _wait_for_process(
        ...
        *,
        ...
        if bounded_capture:
            ...
            try:
                spill_dir = get_nastech_home() / "cache" / "terminal-output"
        ...
        stdin_thread = getattr(proc, "_nastech_stdin_thread", None)
        if stdin_thread is not None:
            stdin_thread.join(timeout=5)
        rendered = output.render()
        result = self._finalize_wait_result(output, rendered, proc.returncode)
        stdin_errors = getattr(proc, "_nastech_stdin_errors", None)
        if stdin_errors:
