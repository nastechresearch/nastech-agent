...
from tools.environments.local_env_policy import _NASTECH_PROVIDER_ENV_BLOCKLIST, _is_nastech_internal_secret


def load_nastech_env_vars() -> dict[str, str]:
    """``~/.nastech/.env`` values, or ``{}`` — a broken .env must not fail command execution."""
    try:
...
def resolve_passthrough_env(explicit_forward: Iterable[str] = (),
                            nastech_env_loader: Callable[[], dict[str, str]] = load_nastech_env_vars,
    ...
    is filtered through the Nastech provider-credential blocklist and the dynamic internal-secret
    ...
    try:
        ...
        pass
    implicit_forward = {k for k in passthrough_keys if not _is_nastech_internal_secret(k)}
    forward_keys = set(explicit_forward) | (implicit_forward - _NASTECH_PROVIDER_ENV_BLOCKLIST)
    nastech_env = nastech_env_loader() if forward_keys else {}
