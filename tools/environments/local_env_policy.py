"""Secret-scrub policy for Nastech child processes: pure data + predicates for which env
names are Nastech-managed credentials. The env *builders* applying it (``_make_run_env``,
``_sanitize_subprocess_env``, ``nastech_subprocess_env``) live in ``tools.environments.local``."""
...
# Nastech-managed AWS *inference* credentials for ``auth_type="aws_sdk"`` (Bedrock):
...
_STATIC_PROVIDER_ENV_BLOCKLIST = frozenset({
    ...
    "EMAIL_HOME_ADDRESS", "EMAIL_HOME_ADDRESS_NAME", "NASTECH_DASHBOARD_SESSION_TOKEN",
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    # Claude Code install, not Nastech: stripping it made agent-spawned ``claude`` CLIs
    ...
    # It is set and owned by the user's Claude Code install (subscription OAuth), not a Nastech-managed
    # inference credential — Claude subscription auth is not a working Nastech provider path. It arrives via
...
# ``_sanitize_subprocess_env``); execute_code, nastech_subprocess_env, docker and
...
# ``buzz`` (the gateway's ``NASTECH_SESSION_PLATFORM`` ContextVar; concurrency safe under a multi-session
...
# scope. Every other surface stays sealed — execute_code scrubbing, :func:`nastech_subprocess_env` (browser /
...
# the blocklist entirely because it is NOT a Nastech credential; these ARE Nastech-managed first-party
...
# ANOTHER project's deps into the Nastech venv (still reachable via PATH, so stripping
# is safe); PYTHONHOME redirects a child interpreter's stdlib to the Nastech venv
# (version-mismatch crashes). PYTHONPATH is handled separately (Nastech-owned entries only).
...
# dependencies into the Nastech venv path instead of the project's own ``.venv`` — silently clobbering the
# Nastech environment (e.g. a project pinned to a different Python version overwrites it and breaks the
# gateway). PYTHONHOME is included because a gateway-inherited value redirects the standard-library search
# of ANY child interpreter — including unrelated system/venv Pythons — to the Nastech venv's stdlib, which
# crashes with version-mismatch errors before a child script even imports a package (#75018). Nastech itself
...
# _strip_nastech_owned_pythonpath() which removes only Nastech-owned entries, preserving user-set paths.
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX", "PYTHONHOME")


def _is_nastech_internal_secret(key: str) -> bool:
    """True for Nastech-internal secrets injected under *dynamic* names the static
...
# adapter is covered without a second edit; ``NASTECH_*`` never counts (``NASTECH_MEDIA_ALLOW_DIRS``,
# ``NASTECH_ALLOW_PRIVATE_URLS`` are process settings, not adapter gates).
...
def is_profile_gate_env(name: str) -> bool:
    ...
    upper = name.upper()
    if upper.startswith("NASTECH_") or upper.startswith("_"):
        return False
...
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    "HASS_TOKEN", "EMAIL_PASSWORD", "NASTECH_DASHBOARD_SESSION_TOKEN",
