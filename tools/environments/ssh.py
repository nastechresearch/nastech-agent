...
# Module-level binding: tests patch ``ssh._load_nastech_env_vars`` to fake the .env file.
...
class SSHEnvironment(BaseEnvironment):
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        ...
        # (home/user/.nastech/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.nastech".lstrip("/")
        # Live sockets inside .nastech (gateway.sock and friends) cannot be archived: tar prints
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False, timeout: int = 120,
        ...
        values, unset_names = resolve_passthrough_env(nastech_env_loader=_load_nastech_env_vars)
