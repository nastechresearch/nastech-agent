...
class ManagedModalEnvironment(BaseEnvironment):
    ...
    def __init__(self, image: str, cwd: str = "/root", timeout: int = 60,
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and Nastech user token")
    ...
    def _request(self, method: str, path: str, *, json: Dict[str, Any] | None = None, timeout: int = 30,
        ...
        headers = {"Authorization": f"Bearer {self._nastech_user_token}", "Content-Type": "application/json",
                   **(extra_headers or {})}
