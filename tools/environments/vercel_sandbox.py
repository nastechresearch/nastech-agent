...
under ``NASTECH_HOME`` and new sandboxes are restored from them on task reuse.
"""
...
def _ensure_vercel_sdk() -> None:
    ...
    # The SDK (>=0.7) ships default-on telemetry; Nastech policy is opt-in only, so disable it
...
def _snapshot_store() -> Path:
    return get_nastech_home() / "vercel_sandbox_snapshots.json"
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _remote_nastech_dir(self) -> str:
        return f"{self._remote_home.rstrip('/')}/.nastech"
    ...
    def _attach_fresh_sandbox(self, requested_cwd: str) -> None:
        ...
        container_base = self._remote_nastech_dir()
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        archive_member = self._remote_nastech_dir().lstrip("/")
