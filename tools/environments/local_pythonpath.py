"""Nastech-owned PYTHONPATH stripping for child processes. Launchers prepend the repo
root and the Nastech venv's site-packages so the backend can ``import tools``; leaked
into a child Python of a DIFFERENT version they load the backend's C extensions and
crash. Only entries proven Nastech-owned by *path provenance* are removed — never by a
cross-version heuristic. Module state (``_nastech_repo_root_aliases``, ``_in_venv``,
``_nastech_site_packages``) lives in ``tools.environments.local`` (via :func:`_state`)
...
def _build_nastech_repo_root_aliases(
    ...
    """Exact repo-root spellings emitted by Nastech launchers. Mirrors
    ``gateway_windows._preserve_nastech_home_path`` (physical path under the resolved
    NASTECH_HOME -> configured spelling) so a junction-backed install matches without
    treating arbitrary NASTECH_HOME descendants as Nastech-owned. A repo-level junction
    ...
    # and the repo lives beside the profiles dir (as get_default_nastech_root() does).
...
def _validated_runtime_venv(env: dict) -> Path | None:
    ...
    if not env.get("VIRTUAL_ENV") or not any(
            _same_path(candidate, root / "venv") for root in _state()._nastech_repo_root_aliases):
        return None
    try:
...
def _get_nastech_site_packages(env: dict) -> list[Path]:
    """Exact site-packages dirs owned by the Nastech runtime (cached):
    ...
    local = _state()
    if local._nastech_site_packages is None:
        result: list[Path] = []
        if local._in_venv:
            try:
                import site
        ...
        local._nastech_site_packages = list(result)
    result = list(local._nastech_site_packages)
...
def _strip_nastech_owned_pythonpath_and_runtime_markers(env: dict) -> None:
    """Strip Nastech-owned PYTHONPATH entries, then the runtime marker vars. Order is
...
def _strip_nastech_owned_pythonpath(env: dict) -> None:
    """Remove Nastech-owned PYTHONPATH entries: only exact matches of the repo root
    ...
    if not pp:
        return
    owned_paths = [*_get_nastech_site_packages(env), *_state()._nastech_repo_root_aliases]
    ...
    if stripped:
        logger.debug("Stripped Nastech-owned entries from PYTHONPATH: %s", stripped)
