...
def _snapshot_store() -> Path:
    ...
    return get_nastech_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .nastech/ as a tar archive (sandboxes run as root, so /root/.nastech)."""
        # --exclude: live sockets cannot be archived ("socket ignored") and must not fail the download.
        data = self._exec("tar cf - --exclude='*.sock' -C / root/.nastech", timeout=120, fail_label="bulk download", capture=True)
