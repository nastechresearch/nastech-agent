...
processes survive cleanup, idle reaping, or Nastech exit.
...
from tools.tool_backend_helpers import coerce_modal_mode, managed_nastech_tools_enabled
...
def _current_session_profile() -> str:
    """Active session's Nastech profile name, or "" (same lookup discipline as
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps Exited containers labeled ``nastech-agent=1`` for the current
    profile — leftovers of Nastech processes that died without firing
    ...
    Nastech processes sharing a profile) and the once-per-interpreter flag so
...
def _ensure_terminal_env_bridged() -> None:
    ...
    at startup; processes that skip those paths (``nastech serve``, Desktop
    ...
    env values (possibly stale from ``nastech setup``); env values for omitted keys
    ...
    context-local ``NASTECH_HOME`` override (multiplexed dashboard / gateway
...
def terminal_tool(
    ...
    ``_host_local`` forces the local backend for Nastech-owned control-plane
