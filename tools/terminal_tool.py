...
def _current_session_key() -> str:
    ...
    Single lookup point for the ``NASTECH_SESSION_KEY`` ContextVar with the
    ...
    return get_session_env("NASTECH_SESSION_KEY", "")
...
def _in_delegated_child_context() -> bool:
    ...
    inherit process-wide interactivity signals (``NASTECH_INTERACTIVE=1`` set
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (NASTECH_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # NASTECH_INTERACTIVE=1 (and, on a recycled worker thread, potentially a
    ...
    should_prompt_for_sudo = (
        env_var_enabled("NASTECH_INTERACTIVE") or has_sudo_prompt_callback
...
def _docker_persistent_profile_scoped() -> bool:
    ...
    ``container_persistent: true`` is ONE long-lived container per Nastech
...
def _current_session_profile() -> str:
    """Return the active session's Nastech profile name, or "" when unset.
    ...
    return get_session_env("NASTECH_SESSION_PROFILE", "")
...
def _ensure_terminal_env_bridged() -> None:
    ...
    vars at startup — but processes that skip all of those paths (``nastech
    ...
    be stale from ``nastech setup``). Environment values for omitted terminal
    ...
    try:
        from nastech_cli.config import apply_terminal_config_to_env, read_raw_config
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Nastech Tool Gateway access is not currently available and no direct "
        ...
        if provider is not None:
            ...
            try:
                env_obj._nastech_backend_name = provider.name.strip().lower()
            except AttributeError:
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Nastech tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/nastech
        # restart|stop|uninstall targeting nastech-gateway) must never run inside the
        ...
        # never restart. This mirrors the `nastech gateway restart` guard in
        # nastech_cli/gateway.py and the cron-path guard in nastech_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        # Gate on the SUPERVISED-gateway probe, not the raw _NASTECH_GATEWAY
        # marker: gateway.run sets it at import time, so it leaks into every
        # process that merely imports gateway.run (nastech serve --isolated,
        # CLI, web server) which are NOT the gateway and must be able to
        # restart it. A plain foreground `nastech gateway run` (env set, PID
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Nastech cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `nastech gateway restart` from a "
        ...
        if background:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        result_data["notify_unsupported"] = (
                            ...
                            "the turn ends (a one-shot runner such as `nastech -z`, a "
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                can_reprompt = (
                    has_sudo_prompt_callback or env_var_enabled("NASTECH_INTERACTIVE")
                ) and not _in_delegated_child_context()
                if can_reprompt:
                    output += (
