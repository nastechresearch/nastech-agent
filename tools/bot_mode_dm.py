...
def message_agent_tool(
    ...
    if peer_match or bare_peer:
        ...
        command = f"nastech peer dm {shlex.quote(dm_target)} < {shlex.quote(_write_dm_file(prefix + body))}"
    ...
    command = (
        f"nastech -p {shlex.quote(resolved)} chat --in ~ -c \"Bot Chat\" "
...
def _write_dm_file(content: str) -> str:
    ...
    fd, path = tempfile.mkstemp(prefix="nastech-dm-", suffix=".txt", text=True)
