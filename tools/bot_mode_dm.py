...
teammate agent (another Nastech profile on this install, or an agent on a
...
DM each other via a prompt-injected ``nastech -p <bot> chat ...`` shellout.
...
- Everything here is additive. The legacy protocol transports
  (``nastech -p`` / ``nastech peer dm``) keep working for older prompts.

The transports themselves are unchanged and proven:
- local teammate  → ``nastech -p <name> chat --in ~ -c "Bot Chat"
  --create-if-missing -Q --query-file <tmp>`` (one turn, reply on stdout)
- peer teammate   → ``nastech peer dm <peer>[/<name>] < <tmp>``
...
def _resolve_local_name(target: str, roster: list[str]) -> Optional[str]:
    """Map a target handle to a profile name ('nastech' → 'default')."""
    want = target.strip()
    if not want:
        return None
    if want.lower() == "nastech":
...
def message_agent_tool(
    ...
    if peer_match or bare_peer:
        ...
        # Pin the registry-owning profile (#93935): `nastech peer` resolves
...
def cleanup_bot_dm_cache(
    ...
    locations: list[tuple[Path, str]] = [
        (temp_root, "nastech-dm-*.txt"),
        (temp_root, "nastech-relay-dm-*.txt"),
    ]
    try:
...
def _delivery_lock(argv: list[str], *, stdin_file: bool):
    ...
    Local deliveries (``nastech -p <profile> chat …``) collide with relay
    ...
    # resolves the venv-relative nastech next to this gateway's interpreter
    ...
    if (
        stdin_file
        or len(argv) < 3
        or cli not in ("nastech", "nastech.exe")
        or argv[1] != "-p"
    ):
        return contextlib.nullcontext()
    from tools.bot_relay import acquire_turn_lock

    home = Path(os.getenv("NASTECH_HOME") or os.path.expanduser("~/.nastech"))
    return acquire_turn_lock(_nastech_root(home), argv[2])
...
def _agent_home(agent: Any) -> str:
    ...
    try:
        ...
        pass
    return os.getenv("NASTECH_HOME") or os.path.expanduser("~/.nastech")
