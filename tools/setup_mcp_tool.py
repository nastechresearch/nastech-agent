...
``nastech mcp install <name>`` in the terminal.
"""
...
def setup_mcp_tool(
    ...
    if callback is None:
        return tool_error(
            "setup_mcp is only available in the Nastech desktop app. Use the "
            "terminal instead: `nastech mcp install <name>` for catalog entries, "
            "`nastech mcp login <name>` for OAuth."
        )
...
SETUP_MCP_SCHEMA = {
    ...
    "description": (
        ...
        "Nastech desktop chat. The card lets them install a catalog entry, "
        ...
        "continue without the server. Catalog names: run `nastech mcp catalog` "
