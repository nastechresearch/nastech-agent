...
def gateway_lifecycle_block(
    *,
    ...
    ``systemctl``/``launchctl``/``nastech gateway restart|stop|uninstall``
    targeting nastech-gateway would SIGTERM the gateway — and this very
    ...
    SUPERVISED-gateway probe, not the raw ``_NASTECH_GATEWAY`` marker: that
    marker leaks into every process that merely imports gateway.run (nastech
    serve, CLI, web server), which must still be able to restart the gateway;
    an unsupervised foreground ``nastech gateway run`` has no KeepAlive to turn
    ...
    if lifecycle_scan_root_within_budget(command) and contains_launchctl_submit_command(command):
        return _blocked_json(
            ...
            "or determine whether it is independent of Nastech. Perform authorized "
