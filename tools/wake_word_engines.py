...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_nastech") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under NASTECH_HOME. Keywords are tokenized at RUNTIME.
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        phrase = str(ww._get(cfg, "phrase") or "hey nastech").strip()
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="nastech-kws-", delete=False,
                                         encoding="utf-8")
