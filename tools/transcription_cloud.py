...
local server > env > managed Nastech gateway). Facade-owned state and helpers
...
def _transcribe_xai(
    ...
    if not api_key:
        return _error_result("No xAI credentials found. Configure xAI OAuth in `nastech model` or set XAI_API_KEY")
    ...
    def _post() -> Any:
        ...
        def _post_transcription(bearer: str, endpoint_base_url: str):
            headers = {"Authorization": f"Bearer {bearer}", "User-Agent": nastech_xai_user_agent()}
...
def _transcribe_deepinfra(
    ...
    """Resolve DeepInfra credentials/model (shared ``nastech_cli.models`` helpers), then delegate to :func:`_transcribe_openai`."""
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    ``"nastech"`` -> managed gateway ONLY (a direct OPENAI_API_KEY must NOT override it); any other
    ...
    from tools.tool_backend_helpers import (
        NASTECH_MANAGED_PROVIDER, managed_nastech_tools_enabled, nastech_tool_gateway_unavailable_message,
    ...
    def _managed() -> Optional[tuple[str, str]]:
        ...
        if gateway is None:
            return None
        return gateway.nastech_user_token, urljoin(f"{gateway.gateway_origin.rstrip('/')}/", "v1")

    if selected == NASTECH_MANAGED_PROVIDER:
        managed = _managed()
        if managed is None:
            raise ValueError(selection_error("stt", NASTECH_MANAGED_PROVIDER,
    ...
    if managed is None:
        ...
        if managed_nastech_tools_enabled():
            message += ". " + nastech_tool_gateway_unavailable_message("managed OpenAI audio for transcription")
