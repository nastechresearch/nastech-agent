#!/usr/bin/env python3
"""Bridge desktop-only tools to Nastech-desktop renderer events.
...
``NASTECH_UI_SESSION_ID`` so the event lands on the window that owns the turn
...
def user_enabled(setting: str, default: bool) -> bool:
    ...
    Nastech should not be told about a surface it isn't allowed to use.
...
def emit(event: str, payload: dict) -> bool:
    ...
    if fn is None:
        return False
    fn(get_session_env("NASTECH_UI_SESSION_ID", ""), event, payload)
    return True
