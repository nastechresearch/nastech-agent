...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Nastech's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Nastech's venv site-packages, and a
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_nastech_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `nastech tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Nastech' own bin dir ($NASTECH_HOME/bin) — where install.sh puts uv/uvx
...
def _find_cli() -> Optional[List[str]]:
    ...
    MANAGED-FIRST resolution: Nastech' own ``$NASTECH_HOME/bin`` copy — the
    one every browser backend selection installs and updates via
    ``install_cli()`` — always wins, so all sessions drive one canonical,
    Nastech-controlled binary. PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Nastech' managed uv (bootstrapped on demand via
    ``nastech_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$NASTECH_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Nastech-managed copy, or resolution
    # stays pinned to a binary we don't control (version drift, no updates
    # through nastech tools).
...
def _resolve_lightpanda_cdp(
    ...
    """Point the harness at a Nastech-spawned ``lightpanda serve``.
    ...
    try:
        ...
        return (
            ...
            "to use local Chrome, or switch backends via `nastech tools` → "
            "Browser Automation."
        )
...
def _resolve_backend_cdp(
    ...
    3. A configured cloud browser provider (Browserbase, Firecrawl, Nastech
    ...
    4. ``browser.engine: lightpanda``: a Nastech-spawned ``lightpanda serve``
    ...
    # Nastech-gateway variant (use_gateway: true) DOES resolve through the
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Nastech launches on a
    SNAPSHOT of their real profile (see nastech_cli.browser_connect). Two ways
    in:
...
def browser_exec(
    ...
    # Nastech gateway, CDP override, local Chrome, …). Named sessions compose
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Nastech-managed copy of "
