...
_PRIVATE_BROWSER_SENTINEL = "_NASTECH_BU_PRIVATE_BROWSER"
...
# BU_NAME + daemon pid).
_OWN_TAB_PREAMBLE = """\
# nastech: pin this named session to its own tab (once per daemon process)
def _nastech_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "nastech-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_nastech_ensure_own_tab()
del _nastech_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # (Nastech's venv) wins over its site-packages → wrong-ABI C-extensions and a crash.
    # PYTHONPATH/PYTHONHOME inherited from the agent process point at Nastech's venv site-packages, and a
    ...
    # #84841, #86006, #86104). Strip both — the CLI manages its own environment and never needs Nastech's
    # import path.
...
def _use_gateway(browser_cfg: dict) -> bool:
    """True when the browser section selects the Nastech Tool Gateway — by the current ``nastech tools``
    picker row (``cloud_provider: nastech``) or the pre-picker ``use_gateway: true`` flag. Reading only
    ...
    if is_truthy_value(browser_cfg.get("use_gateway"), default=False):
        return True
    try:
        from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER
    except Exception:  # pragma: no cover — helper ships with the package
        return False
    return str(browser_cfg.get("cloud_provider") or "").strip().lower() == NASTECH_MANAGED_PROVIDER
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_nastech_home()) / "cache" / ".browser_use_default_notice"
        ...
        with contextlib.suppress(OSError):
            ...
            stamp.touch()
        return ("Browser Use CLI not found — using the built-in browser tools. Run `nastech tools` "
...
def _managed_bin_dir() -> str:
    """$NASTECH_HOME/bin — where install.sh puts uv/uvx and install_cli() links browser-use."""
    return str(Path(get_nastech_home()) / "bin")


def _find_cli() -> Optional[List[str]]:
    """Locate the browser-use CLI, or None when it can't be run. MANAGED-FIRST: Nastech' own ``$NASTECH_HOME/bin``
    copy always wins so every session drives one Nastech-controlled binary; PATH and the user-level tool dir
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    the binary into ``$NASTECH_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()`` resolves it for every profile.
...
def _resolve_lightpanda_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at a Nastech-spawned ``lightpanda serve`` (``browser.engine: lightpanda`` and
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Lightpanda could not be started: {e} Set browser.engine to auto "
                   "to use local Chrome, or switch backends via `nastech tools` → Browser Automation."),
...
def _resolve_managed_chromium_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    """Point the harness at Nastech' packaged Chromium, launched through agent-browser for this cache key —
    ...
    if not cdp:
        return (f"The local browser could not be started: {(res or {}).get('error') or 'agent-browser returned no CDP endpoint'} "
                "Run `nastech tools` → Browser Automation to (re)install Chromium, or switch backends.")
    _set_cdp_env(env, cdp)
...
def _resolve_backend_cdp(env: dict, task_id: Optional[str], session_name: str = "") -> Optional[str]:
    ...
    (4) the local engine — ``browser.engine: lightpanda`` or Nastech' packaged Chromium via agent-browser
    ...
    # legacy provider would create a second, redundant session. Nastech-gateway configs (cloud_provider: nastech
    ...
    err = _export_session_cdp(
        ...
        lambda e: (f"Cloud browser provider {provider_name} failed to provide a session: {e}. "
                   "Fix the provider configuration or switch backends via `nastech tools` → Browser Automation."),
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    profile, nastech_cli.browser_connect) when consented. Two ways in: the effective backend is already local
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            ...
            "description": ("Drive the user's own local browser (a Nastech-managed copy of their real "
