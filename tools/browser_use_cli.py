...
def _find_cli() -> Optional[List[str]]:
    ...
    Prefers an installed browser-use binary (PATH, then Nastech' managed
    $NASTECH_HOME/bin); falls back to running it through uvx (PATH, then
    managed). The managed probes matter because Nastech bootstraps its own
    uv into $NASTECH_HOME/bin, which is not on the user's PATH.
    """
