"""Shared ``OAuthClientProvider`` customizations for Nastech MCP OAuth.
...
def _default_auth_request_user_agent() -> str:
    """``Nastech-Agent/<version>`` for SDK-built OAuth requests that would otherwise carry no User-Agent at
...
class NastechProviderMixin:
    """Token-endpoint fixes layered over the SDK's ``OAuthClientProvider`` (must precede it in
    the MRO; subclasses set ``_nastech_logger`` to keep their own logger name).
    ...
    def _prepare_token_request(self, request):
        ...
        ua = getattr(self, "_nastech_token_user_agent", None)  # tests build via __new__
        if ua:
            request.headers["User-Agent"] = ua
        return request

    def _coerce_client_secret_post(self) -> None:
        """Same rule as ``NastechTokenStorage._coerce_secret_auth_method``, applied to the
        ...
        if NastechTokenStorage._coerce_secret_auth_method(data):
    ...
    # _nastech_release_refresh_fence. Never shared across instances.
    _nastech_fence: int | None = None
    ...
    async def async_auth_flow(self, request):
        ...
        while True:
            ...
            try:
                ...
                await self._nastech_release_refresh_fence()
    ...
    async def _refresh_token(self):
        ...
        """
        self._coerce_client_secret_post()
        await self._nastech_acquire_refresh_fence()
        try:
            ...
            candidate = await self._nastech_rotated_candidate()
            if candidate is not None:
                if not self._nastech_install_disk_pair(candidate):
                ...
                if self._nastech_live_ttl() and self.context.is_token_valid():
    ...
    async def _nastech_acquire_refresh_fence(self) -> None:
        ...
        if tokens_path is None:  # pragma: no cover - non-Nastech storage
            return
        self._nastech_fence = await acquire_refresh_fence(tokens_path())

    async def _nastech_release_refresh_fence(self) -> None:
        """Release the fence if held. Idempotent and never raises."""
        from tools.mcp_oauth import release_refresh_fence

        fd, self._nastech_fence = self._nastech_fence, None
        if fd is not None:
            release_refresh_fence(fd)

    async def _nastech_rotated_candidate(self):
    ...
    def _nastech_install_disk_pair(self, tokens) -> bool:
    ...
    async def _initialize(self) -> None:
        ...
        storage = self.context.storage
        from tools.mcp_oauth import NastechTokenStorage
        if isinstance(storage, NastechTokenStorage) and self.context.oauth_metadata is None:
    ...
    async def _handle_refresh_response(self, response) -> bool:
        ...
        """
        try:
            return await self._nastech_handle_refresh_response(response)
        finally:
            await self._nastech_release_refresh_fence()

    async def _nastech_handle_refresh_response(self, response) -> bool:
        if not (200 <= response.status_code < 300):
            self._nastech_logger.warning("Token refresh failed: %s", response.status_code)
            # A writer outside the fence (interactive `nastech mcp login`, or a
            # pre-fence Nastech sharing this NASTECH_HOME) may have rotated the
            ...
            if await self._nastech_reload_tokens_after_refresh_failure():
                self._nastech_logger.info(
        ...
        try:
            ...
            self._nastech_logger.warning("Invalid refresh response: %s", response.status_code)
            self.context.clear_tokens()
            return False
    ...
    async def _nastech_reload_tokens_after_refresh_failure(self) -> bool:
        ...
        a writer outside the fence (an interactive ``nastech mcp login`` or a
        pre-fence Nastech) having rotated the grant between our read and our
        ...
        if (
            self._nastech_install_disk_pair(candidate)
            and self._nastech_live_ttl()
...
def bind_issuer_from_context(context: Any) -> None:
    ...
    it. No-op when metadata is not discovered yet or storage is not Nastech'."""
    from tools.mcp_oauth import NastechTokenStorage
    storage = getattr(context, "storage", None)
    issuer = _metadata_issuer(context)
    if isinstance(storage, NastechTokenStorage) and issuer:
...
def enforce_refresh_token_issuer(context: Any) -> None:
    ...
    if not isinstance(storage, NastechTokenStorage) or tokens is None or not getattr(tokens, "refresh_token", None):
        return
...
def prepare_oauth_config(server_name: str, server_url: str, oauth_config: dict | None) -> tuple[dict, "NastechTokenStorage"]:
    ...
    return cfg, mo.NastechTokenStorage(server_name)


def build_provider_kwargs(cfg: dict, storage: "NastechTokenStorage", *, ssh_proxy_hint: bool) -> dict[str, Any]:
