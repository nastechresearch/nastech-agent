...
from tools.tool_backend_helpers import (has_direct_modal_credentials, managed_nastech_tools_enabled,
                                        nastech_tool_gateway_unavailable_message, resolve_modal_backend_state)

# Log-record parity with the origin module.
logger = logging.getLogger("tools.terminal_tool")

# Human reason for the most recent failed requirements check (None after a passing one). The CLI
# startup notice and `nastech doctor` read it through terminal_backend_unavailable_reason() so the user
...
def _modal_unavailable_reason(modal_state: Dict[str, Any]) -> tuple[str, str]:
    ...
    gateway = nastech_tool_gateway_unavailable_message("managed Modal execution")
    if modal_state["managed_mode_blocked"] or modal_state["mode"] == "managed":
        tail = (("Nastech Tool Gateway access is not currently available and no direct Modal credentials/config "
...
def _build_docker_env(*, image, cwd, timeout, cc, task_id, host_cwd, **_):
    ...
    # One-shot reaper for labeled containers orphaned by prior Nastech processes that died before
...
def _ssh_pre(config: Dict[str, Any]) -> bool:
    ...
    return _reject("the SSH host and user are not configured (TERMINAL_SSH_HOST / TERMINAL_SSH_USER); "
                   "run `nastech setup terminal` to enter them or pick the 'local' backend")
