...
The LLM writes a Python script that calls Nastech tools via RPC, collapsing
...
# Loopback TCP replaces AF_UNIX on Windows, so execute_code runs on every platform Nastech does.
...
def _spill_full_stdout(stdout_text: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nastech_dir("cache/exec", "exec_spill")
...
def _missing_nastech_tools_import_hint(m, enabled_tools) -> str:
    missing = m.group(1)
    if missing in {"json_parse", "shell_quote", "retry"}:
        return (f"Import helpers with `from nastech_tools import {missing}`. "
                "If that import failed, the generated module may be stale or another "
                "nastech_tools may be first on sys.path. Check nastech_tools.__file__ "
...
def generate_nastech_tools_module(enabled_tools: List[str],
    ...
    """Source of the nastech_tools.py stub module for SANDBOX_ALLOWED_TOOLS ∩ *enabled_tools*.
...
def _call(tool_name, args):
    ...
    # re-accepts (NASTECH_RPC_PERSISTENT=1); retry once on a fresh socket.
    _attempts = 2 if os.environ.get("NASTECH_RPC_PERSISTENT") == "1" else 1
    with _call_lock:
...
def _run_remote_per_call(env, env_type: str, code: str, effective_task_id: str,
    ...
    """Per-call script ship: stage nastech_tools.py + script.py in a fresh remote sandbox dir,
    serve file-RPC from a polling thread, run, clean up."""
    sandbox_dir = f"{_env_temp_dir(env)}/nastech_exec_{uuid.uuid4().hex[:12]}"
    ...
    try:
        ...
        _ship_file_to_remote(env, f"{sandbox_dir}/nastech_tools.py",
                             generate_nastech_tools_module(list(sandbox_tools), transport="file"))
        ...
        rpc_thread.start()
        env_prefix = (f"NASTECH_RPC_DIR={quoted_rpc_dir} NASTECH_RPC_TOKEN={shlex.quote(rpc_token)} "
...
def _execute_remote(code: str, task_id: Optional[str], enabled_tools: Optional[List[str]],
    ...
    try:
        ...
        try:
            # --- Session-kernel path (nastech-agent#96873) ------------------- Same always-on model as
...
def execute_code(
    ...
    with RPC access to a subset of Nastech tools; returns the JSON result string. "Sandbox" means
    ...
    # execute_code is a straight bypass — the terminal() path refuses `launchctl bootout ai.nastech.gateway`,
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        ...
        cwd_note = (
            ...
            "Nastech's own python (the common case — stdlib plus Nastech's "
    ...
    description = (
        "Run Python that calls Nastech tools programmatically. Use when you "
        ...
        f"{cwd_note}\n\n"
        "Helpers require imports: `from nastech_tools import json_parse, shell_quote, retry`. "
