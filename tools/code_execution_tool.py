...
def _sandbox_failure_hint(stderr_text: str, enabled_tools=None) -> Optional[str]:
    ...
    nastech_tools import misuse (importing tools that aren't in the sandbox,
    ...
    try:
        m = re.search(
            r"cannot import name '(\w+)' from 'nastech_tools'", window
        )
        if m:
            missing = m.group(1)
...
def _execute_remote(
    ...
    try:
        ...
        # --- Session-kernel path (nastech-agent#96873) -------------------
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # ``nastech_tools.py`` always lives in the staging directory, so that
    # directory must be importable even when project mode changes CWD.
    # Nastech's own package root is useful too, but only when the child
    # uses the same Python environment. Project mode can select an
    # external venv; exposing Nastech's site-packages to that interpreter
    # can mix incompatible compiled extensions (for example, Python 3.12
    # NumPy with a Python 3.9 project interpreter).
    #
    # Before re-injecting PYTHONPATH, strip Nastech-owned entries that
    # leaked through _scrub_child_env (PYTHONPATH is in _SAFE_ENV_PREFIXES
    # so it passes the scrub).  They are redundant for same-Nastech-
    ...
    if _uses_nastech_python_environment(child_python):
        _pp_parts.append(_nastech_root)
    elif child_python not in _external_env_logged:
        # Import behavior changes silently otherwise — surface it (once
        # per interpreter path) so "import nastech_constants suddenly
        # fails" reports are diagnosable without log spam.
        _external_env_logged.add(child_python)
        logger.info(
            "execute_code: child interpreter %s is outside the Nastech "
            "environment; nastech root omitted from PYTHONPATH",
            child_python,
        )
    if _existing_pp:
...
def execute_code(
    ...
    # terminal() path refuses `launchctl bootout ai.nastech.gateway`, but the
...
def _python_environment_prefix(python_path: str) -> str:
    ...
    sticky empty result would silently drop the nastech root from every
...
def _uses_nastech_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Nastech's active Python environment.
