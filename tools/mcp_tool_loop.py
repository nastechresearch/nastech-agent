...
def _try_acquire_mcp_discovery_lock() -> Any:
    ...
    try:
        ...
        if get_nastech_home_override() is not None:
            lock_path = str(get_nastech_home() / ".mcp-discovery.lock")
        else:
            if _origin._MCP_DISCOVERY_LOCK_PATH is None:
                _origin._MCP_DISCOVERY_LOCK_PATH = str(get_nastech_home() / ".mcp-discovery.lock")
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local NASTECH_HOME override into ``coro`` (task-local on the MCP
    loop, so concurrent scopes don't interfere)."""
    try:
        from nastech_constants import get_nastech_home_override, reset_nastech_home_override, set_nastech_home_override
