...
``ui_meta['nastech-bots']`` block in its profile.yaml), a bot's canonical
...
def _is_bot_managed(profile_dir: Path) -> bool:
    """True when profile.yaml carries a ui_meta['nastech-bots'] block.
    ...
    try:
        ...
        if "nastech-bots" not in raw:
            return False
        import yaml

        data = yaml.safe_load(raw)
        ui_meta = data.get("ui_meta") if isinstance(data, dict) else None
        return isinstance(ui_meta, dict) and isinstance(ui_meta.get("nastech-bots"), dict)
    except Exception:
        return False
...
def is_bot_mode_managed(home: str | os.PathLike | None = None) -> bool:
    ...
    try:
        resolved = Path(
            str(home) if home else (os.getenv("NASTECH_HOME") or os.path.expanduser("~/.nastech"))
        )
...
def _profile_role(profile_dir: Path) -> str:
    ...
    The ui_meta['nastech-bots'].title is the name the user gave the bot in
    ...
    try:
        ...
        if isinstance(ui_meta, dict) and isinstance(ui_meta.get("nastech-bots"), dict):
            title = str(ui_meta["nastech-bots"].get("title") or "").strip()
            if title:
                parts.append(title)
...
def _peers(root: Path) -> list[str]:
    """Registered peer gateway names (``nastech peer``), for the protocol text.
...
def _remote_paragraph(root: Path) -> str:
    ...
    connected to the user's Desktop (local, remote URL, SSH, Nastech Cloud,
...
def get_bot_mode_protocol_section(home: str | os.PathLike | None = None, *, force_refresh: bool = False) -> str:
    ...
    not the ambient NASTECH_HOME — build threads can lose the ContextVar
    override and the env var would then name the wrong profile.
    """
    resolved = str(home) if home else (os.getenv("NASTECH_HOME") or os.path.expanduser("~/.nastech"))
    with _lock:
...
def capability_fingerprint(home: str | os.PathLike | None = None) -> str:
    ...
    resolved = Path(str(home) if home else (os.getenv("NASTECH_HOME") or os.path.expanduser("~/.nastech")))
    surface: dict = {}
    try:
    ...
    try:
        ...
        surface["peers"] = _peers(_nastech_root(resolved))
    except Exception:
        surface["peers"] = []
    try:
        ...
        surface["remote_roster"] = sorted(
            ...
            for r in read_remote_roster(_nastech_root(resolved))
        )
    except Exception:
