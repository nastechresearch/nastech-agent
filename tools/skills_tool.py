...
# Cache validation (mirrors nastech_cli/profiles.py::_count_skills, d5eee133e):
...
def _skills_dir() -> Path:
    ...
    set NASTECH_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped NASTECH_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    ...
    # the canonical readers in nastech_cli/config.py).
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    ...
    # (_skills_dir() resolves the LIVE profile NASTECH_HOME; the module-level
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `nastech skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `nastech sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
