...
# cron, kanban workers) import this module once under the launch NASTECH_HOME
# and later scope requests to a different profile via
# set_nastech_home_override(). Frozen module constants would then resolve —
...
# re-resolve from the live profile-scoped NASTECH_HOME on every call.
_NASTECH_HOME_AT_IMPORT = NASTECH_HOME
_SKILLS_DIR_AT_IMPORT = SKILLS_DIR
_MANIFEST_FILE_AT_IMPORT = MANIFEST_FILE


def _nastech_home() -> Path:
    """Return the active profile's NASTECH_HOME at call time."""
    configured = Path(NASTECH_HOME)
    if configured != _NASTECH_HOME_AT_IMPORT:
        return configured
...
def _essential_names() -> frozenset:
    ...
    try:
        ...
        return frozenset({"nastech-agent"})
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``nastech skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `nastech skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    ...
    # ``nastech-agent`` skill is the agent's own operating manual and the
    ...
    essential_only = (_nastech_home() / NO_BUNDLED_SKILLS_MARKER).exists()
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``NASTECH_HOME/skills/`` to prevent the catastrophic wipe of
    # ``~/.nastech/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``NASTECH_HOME``. Require a strict-child relationship so both escape
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``nastech update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``nastech update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``nastech skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `nastech skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    Returns:
        dict with keys: ok (bool), changed (bool), marker (str path),
                        message (str).
    """
    marker = _nastech_home() / NO_BUNDLED_SKILLS_MARKER
    existed = marker.exists()
    try:
        if enabled:
            _nastech_home().mkdir(parents=True, exist_ok=True)
            marker.write_text(
...
def is_bundled_skills_opt_out() -> bool:
    ...
    return (_nastech_home() / NO_BUNDLED_SKILLS_MARKER).exists()
