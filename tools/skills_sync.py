...
~/.nastech/skills/, tracking each synced skill's origin hash in .bundled_manifest (v2 "name:hash"
...
# retarget NASTECH_HOME after import, and frozen constants would resolve (and for
...
# once under the launch NASTECH_HOME and later scope requests to a different profile via
# set_nastech_home_override(). See #65828.
...
def _nastech_home() -> Path:
    return _live(NASTECH_HOME, _NASTECH_HOME_AT_IMPORT, get_nastech_home)


def _skills_dir() -> Path:
    return _live(SKILLS_DIR, _SKILLS_DIR_AT_IMPORT, lambda: _nastech_home() / "skills")
...
# Written by `nastech profile create --no-skills` / installer `--no-skills`: sync seeds only
# essential skills. Mirrors nastech_cli.profiles.NO_BUNDLED_SKILLS_MARKER (no CLI import here).
NO_BUNDLED_SKILLS_MARKER = ".no-bundled-skills"


def _get_bundled_dir() -> Path:  # NASTECH_BUNDLED_SKILLS env first, then repo-relative
...
def _write_manifest(entries: Dict[str, str]):
    ...
    mkdir_under_nastech_home(_manifest_file().parent)
    try:
    ...
    for candidate in st.active_index.get(skill_name, []):
        ...
        if not _matches_origin_hash(candidate, origin_hash):  # moving a customized copy would edit user work
            st.say(
                ...
                f"Run `nastech skills reset {skill_name} --restore` to move to the new location.")
            continue
        try:
        ...
        if dest.exists():
            ...
            if _dir_hash(dest) == bundled_hash:
                ...
                st.say(
                    ...
                    f"by this name — yours was kept. Run `nastech skills reset {skill_name}` to "
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills into ~/.nastech/skills/ using the manifest; returns the per-category
    result dict. Opted-out profiles seed ONLY ESSENTIAL_SKILLS (the system prompt always
    points at ``nastech-agent``)."""
...
def _rmtree_writable(path: Path) -> None:
    ...
    child of the active skills root (bad join / missing NASTECH_HOME / malicious manifest entry).
