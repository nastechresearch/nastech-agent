...
from tools.bot_mode_probe import _default_home, _nastech_root
...
# Mirrors RELAY_DELIVER_TIMEOUT_MS in apps/desktop/src/plugins/nastech-bots/relay.ts; both test suites pin it.
...
# One turn in a profile's canonical Bot Chat: ``nastech -p <profile> *BOT_CHAT_TURN_ARGS``.
...
# (``nastech_cli.quiet_single_query.adopt_unanswered_turn``, which consumes the variable before the turn).
RESUME_UNANSWERED_TURN_ENV = "NASTECH_RESUME_UNANSWERED_TURN"
...
def _ensure_dirs(root: Path | str) -> Path:
    ...
    for sub in (OUTBOX_DIR, CLAIMED_DIR, REPLIES_DIR):
        ...
        mkdir_under_nastech_home(base / sub)
    return base
...
def _bot_mode_cfg(key: str, *, loader: str) -> Any:
    ...
    try:
        import nastech_cli.config as cfgmod
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    handle = str(row.get("handle") or "").strip().lstrip("@") or ("nastech" if profile == "default" else profile)
...
def cleanup_bot_relay_artifacts(max_age_hours: float | None = None) -> int:
    ...
    del max_age_hours
    try:
        base = relay_root(_nastech_root(Path(_default_home())))
...
def _nastech_cli() -> str:
    """nastech CLI beside this interpreter, then ``shutil.which``, then the bare name
    (service contexts lack PATH, so a bare "nastech" died with ENOENT).

    The deliver RPC runs on the target gateway, whose process is the venv python — its bin/Scripts directory
    holds the matching ``nastech`` entrypoint. A bare ``"nastech"`` relies on PATH, which is exactly what
    ...
    """
    sibling = Path(sys.executable or "").parent / ("nastech.exe" if sys.platform == "win32" else "nastech")
    return str(sibling) if sibling.is_file() else shutil.which("nastech") or "nastech"
...
def local_delivery_command(profile: str, query_file: str) -> list[str]:
    ...
    return [_nastech_cli(), "-p", profile, *BOT_CHAT_TURN_ARGS, "--query-file", query_file]
...
def _delivery_child_session_env_names() -> "tuple[str, ...]":
    ...
    ``NASTECH_SESSION_*`` prefix match, which would also strip non-identity knobs
    (e.g. ``NASTECH_SESSION_STALL_TIMEOUT``)."""
...
def delivery_env(author: Optional[dict], profile_home: "str | Path | None" = None) -> dict[str, str]:
    """Environment for one delivery turn's ``nastech -p <profile>`` child. The dispatcher's own
    NASTECH_TURN_AUTHOR is dropped first so a delivery without an author never inherits the author of the turn
    ...
    ``os.environ``; ``-p`` alone only pinned NASTECH_HOME. ``profile_home`` is the target's home when the
...
# Deliveries are separate ``nastech`` subprocesses, so the lock is a per-profile
...
# target profile must never run their Bot Chat turns concurrently: deliveries spawn separate ``nastech``
