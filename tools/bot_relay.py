...
Nastech Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own NASTECH root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in nastech_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "nastech" if profile == "default" else profile
    if (
...
def _nastech_cli() -> str:
    """Resolve the nastech CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``nastech``
    entrypoint. A bare ``"nastech"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("nastech.exe" if sys.platform == "win32" else "nastech")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("nastech")
    if found:
        return found
    return "nastech"
...
# turns concurrently: deliveries spawn separate ``nastech`` subprocesses, so
