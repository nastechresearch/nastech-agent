...
# prefix, the exact-name NASTECH_ allowlist, or (Windows) an OS-essential name.
# The broad "NASTECH_" prefix is deliberately NOT safe — it leaked config vars
# without a secret substring (NASTECH_BASE_URL, NASTECH_KANBAN_DB, *_WEBHOOK).
# NASTECH_RPC_SOCKET / NASTECH_RPC_DIR / TZ / HOME are injected after scrubbing.
...
# imports may read at import time. NASTECH_DELEGATED_CHILD_CONTEXT must ride
# along or a child that imports Nastech code loses the Kanban mutation guard
# while still inheriting NASTECH_HOME.
_NASTECH_CHILD_ALLOWED = frozenset({
    "NASTECH_HOME", "NASTECH_PROFILE", "NASTECH_CONFIG", "NASTECH_ENV", "NASTECH_DELEGATED_CHILD_CONTEXT",
})
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    (4) operational NASTECH_* pass by exact name; (5) on Windows the
    ...
    scrubbed = {}
    # Non-secret NASTECH_* vars no allowlist admits are dropped on purpose; a script importing a
    ...
    for k, v in source_env.items():
        ...
        if (any(k.startswith(p) for p in _SAFE_ENV_PREFIXES)
                or k in _NASTECH_CHILD_ALLOWED
                or (is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS)):
            scrubbed[k] = v
        elif k.startswith("NASTECH_"):
    ...
    if _dropped_nastech:
        logger.debug(
            ...
            len(_dropped_nastech), ", ".join(sorted(_dropped_nastech)),
        )
    ...
    # non-allowlisted NASTECH_* values in otherwise ordinary execution environments.
    if scoped.get(DELEGATED_CHILD_ENV_MARKER):
        for key in (DELEGATED_CHILD_ENV_MARKER, "NASTECH_KANBAN_DB", "NASTECH_KANBAN_BOARD"):
            if key in scoped:
...
def _build_child_env(*, rpc_endpoint: str, rpc_token: str, tmpdir: str,
    ...
    # Only TZ reaches the child; NASTECH_TIMEZONE is an internal setting (and under the multiplexed
    # gateway holds only the default profile's value — nastech_time resolves the routed profile's).
    from nastech_time import get_timezone_name
    ...
    # NASTECH_HOME, but this turn runs under a per-profile override (ContextVar bound per turn).
    ...
    if _uses_nastech_python_environment(child_python):
        ...
        # Surface once per interpreter so "import nastech_constants fails" is diagnosable.
        _external_env_logged.add(child_python)
        logger.info("execute_code: child interpreter %s is outside the Nastech "
                    "environment; nastech root omitted from PYTHONPATH", child_python)
    if _existing_pp:
...
# Interpreter paths already reported as outside the Nastech environment.
...
def _uses_nastech_python_environment(python_path: str) -> bool:
    """Whether *python_path* belongs to Nastech's active Python environment. Short-circuits when
    it IS the running interpreter (by path or realpath — covers ``uv run`` venvs) so no probe
    runs on the default strict path and a flaky probe can never drop the nastech root."""
