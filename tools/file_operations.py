...
NOT_REGULAR_SENTINEL = "__nastech_not_regular__"
...
MISSING_SENTINEL = "__nastech_missing__"

_READ_SENTINEL_PREFIX = "__NASTECH_RF_"
_WRITE_SENTINEL_PREFIX = "__NASTECH_WF_"
...
class ShellFileOperations(FileOperations):
    ...
    def _escape_native_tool_arg(self, arg: str) -> str:
        ...
        paths — and Nastech disables MSYS argument conversion for its bash
    ...
    def _try_read_utf16(self, path: str, offset: int, limit: int,
        ...
        snippet = (
            ...
            "    if size > MAX:\n"
            "        print('NASTECH_UTF16:NO'); sys.exit(0)\n"
            "    with open(p, 'rb') as f:\n"
            "        data = f.read()\n"
            "    sample = data[:SAMPLE]\n"
            "    enc = None\n"
        ...
        marker = stdout.find("NASTECH_UTF16:OK")
        if result.exit_code != 0 or marker < 0:
            return None
        payload = stdout[marker + len("NASTECH_UTF16:OK"):].strip()
        try:
    ...
    def _native_read_enabled(self) -> bool:
        ...
        ``NASTECH_NATIVE_FILE_READ=0`` turns the fast path off.
        """
        flag = os.environ.get("NASTECH_NATIVE_FILE_READ", "1").strip().lower()
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        # globals (`Window.nastechDesktop` → TS2339) that are defined by the
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        # nothing converts back — Nastech sets MSYS_NO_PATHCONV for its bash).
