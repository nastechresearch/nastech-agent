...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    ``nastech-agent``) are treated as permanently pinned: the system prompt
    ...
    try:
        ...
        if name in ESSENTIAL_SKILLS:
            return (
                f"Skill '{name}' is essential to Nastech (the agent's own "
...
def _background_review_write_guard(
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`nastech curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
    ...
    try:
        ...
        # shapes; `nastech curator adopt <name>` is the supported way in.
        ...
        if not skill_usage._is_curator_managed_record(usage_rec):
            ...
            return {
                ...
                "error": (
                    ...
                    f"Run `nastech curator adopt {name}` to opt it in."
                ),
            }
    except Exception:
    ...
    try:
        ...
        if not ssc.sync_org_auto_propose():
            return (
                ...
                f"Run `nastech sync propose {name}` to share it back."
            )
        ...
        return (
            ...
            f"right now — run `nastech sync propose {name}` to retry."
        )
...
def _org_mirror_write_guard(name: str, skill_path: Path, action: str) -> Optional[Dict[str, Any]]:
    ...
    reaches the organisation when the user runs `nastech sync propose`, or
        ...
        if is_org_mirror_path(skill_path, _skills_dir()):
            return {
                ...
                "error": (
                    ...
                    "and can be proposed back with `nastech sync propose "
                    f"{name}`.)"
                ),
            }
    except Exception:
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"profiles (`nastech -p {other_profile}`) or edit the file "
...
def _attach_lint_findings(result: Dict[str, Any], skill_md: Path) -> None:
    ...
    result["lint_hint"] = (
        ...
        "to match Nastech skill standards."
    )
...
def _patch_skill(
    ...
    if not old_string:
        ...
        # how to recover. Upstream: NastechResearch/nastech-agent#33064.
        return {
            "success": False,
            "error": (
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.nastech/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `nastech curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `nastech curator
        ...
        # Inert unless the access gate is open (the user is a Nastech admin on the
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"{display_nastech_home()}/skills/; must precede that skill's other "
