...
def load_config() -> ConnectorConfig:
    try:
        from nastech_cli.config import load_config_readonly as _load
...
def connectors_available(
    ...
    try:
        ...
        if entitlement_check is None:
            ...
            if is_guest_state(_read_nastech_provider_state()):
                return True
...
def operation_session_key(session_id: Optional[str]) -> str:
    ...
    (``NASTECH_SESSION_KEY``), falling back to the agent's session id where no gateway bound one.
    ...
    return get_session_env("NASTECH_SESSION_KEY", "") or str(session_id or "")
...
def session_platform() -> str:
    ...
    platform = get_session_env("NASTECH_SESSION_PLATFORM", "") or get_session_env("NASTECH_SESSION_SOURCE", "")
