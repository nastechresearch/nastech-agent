...
_NASTECH_CONFIG_FILES = r'\.nastech/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.nastech/\.env|\~/\.nastech/\.env',
     "nastech_env_access", "critical", "exfiltration", "directly references Nastech secrets file"),
    ...
    # invocation: any client plugin that relays Nastech' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for Nastech/other config (setup docs routinely say
    ...
    (_prose_modify_re(_NASTECH_CONFIG_FILES),
     "nastech_config_mod", "high", "persistence", "modification language aimed at Nastech configuration files (verify intent)"),
    (_shell_write_re(_NASTECH_CONFIG_FILES),
     "nastech_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Nastech configuration files"),
    (r'\.nastech/config\.yaml|\.nastech/SOUL\.md',
     "nastech_config_ref", "low", "persistence", "references Nastech configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Nastech-native; `.clawhubignore` is honored for skills published through ClawHub.
