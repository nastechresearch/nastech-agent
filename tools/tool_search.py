...
# Session-gated GUI toolsets. Off ``_NASTECH_CORE_TOOLS`` so non-GUI clients
...
def is_deferrable_tool_name(name: str, defer_tools: Optional[frozenset] = None) -> bool:
    ...
    * it is neither in ``_NASTECH_CORE_TOOLS`` nor a session-gated GUI
...
def _schema_for_local_validation(node: Any) -> Any:
    ...
    JSON Schema null union.  Nastech' normal coercion path accepts that shape;
