...
def sdk_httpx():
    ...
    side by side with Nastech' own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Nastech' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Nastech explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Nastech and
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    the same precedence ``nastech_constants._candidate_node_command_names``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    server — and it buys nothing here, because Nastech already supervises the
...
# ---------------------------------------------------------------------------
# If this Nastech process dies without running its cleanup path (kill -9, OOM,
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Nastech document cache; returns a marker with
...
def _unwrap_exception_group(exc: BaseException) -> BaseException:
    ...
    Adapted from :func:`nastech_cli.mcp_config._unwrap_exception_group` with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Nastech profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Nastech' existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Nastech' logging (agent.log via nastech_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `nastech setup` to install MCP support, "
                "then retry."
            )
        ...
        # Nastech already supervises the child (shared death supervisor), so
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    ...
                    for _pid in new_pids:
                        try:
                            from nastech_cli.process_identity import register_child
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Nastech' approval system.
            ...
            try:
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``nastech mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Nastech' pinned ``httpx``, and the two define unrelated
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Nastech processes (e.g.
    ...
    try:
        ...
        if _MCP_DISCOVERY_LOCK_PATH is None:
            _MCP_DISCOVERY_LOCK_PATH = str(
                get_nastech_home() / ".mcp-discovery.lock"
            )
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    ...
    try:
        ...
        home_override = get_nastech_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_nastech_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # vanish here: OAuth token stores and any other get_nastech_home()
...
# Native MCP tool-name prefix. Nastech uses the ``mcp__<server>__<tool>``
...
def discover_mcp_tools(allowed_mcp_names: Optional[List[str]] = None) -> List[str]:
    ...
    Args:
        allowed_mcp_names: If provided, only spawn MCP servers whose names
            ...
            This is used by ``nastech -z -t <toolsets>`` to skip cold-starting
