...
def _nastech_tags(fm: dict) -> Any:
    """``metadata.nastech.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    nastech_meta = metadata.get("nastech", {}) if isinstance(metadata, dict) else None
    return nastech_meta.get("tags", []) if isinstance(nastech_meta, dict) else []
