...
Backend is selected during ``nastech tools`` (``web.backend`` in config.yaml; per
...
from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER, read_selection, selection_exists
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via the config-aware env layer (``nastech config set`` values), then process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set through Nastech' config/.env layer
    (``nastech config set``, ``nastech tools``) are honored here too — not just raw process-env exports.
...
def _get_backend() -> str:
    ...
    if configured:
        # "nastech" (managed subscription) is serviced by firecrawl, routed through the managed Tool Gateway.
        return "firecrawl" if configured == NASTECH_MANAGED_PROVIDER else configured
    if read_selection("web") is not None:
        # Shared selection exists (use_gateway) but no shared name: firecrawl, no ladder.
        return "firecrawl"

    # Never-configured install. Explicit user credentials beat the managed-gateway probe (a Nastech OAuth
...
def _web_requires_env() -> list[str]:
    ...
    on ``managed_nastech_tools_enabled()`` cost a synchronous portal HTTP refresh at every CLI startup.
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            fallback = "No web search provider configured. Run `nastech tools` to set one up."
...
def _provider_is_ready(provider) -> bool:
    ...
    False (so dispatch can emit a precise error), so readiness gates (tool check_fn, ``nastech doctor``)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'nastech_tool_gateway_unavailable_message': ('tools.tool_backend_helpers', 'nastech_tool_gateway_unavailable_message'),
