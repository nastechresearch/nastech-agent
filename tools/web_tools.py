...
When available, Nastech can route Firecrawl calls through a Nastech-hosted tool-gateway
for Nastech Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for Nastech Subscribers)
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``nastech tools``). A
    ...
    if configured:
        ...
        # The managed "Nastech Subscription" selection ("nastech") is serviced by
        ...
        if configured == NASTECH_MANAGED_PROVIDER:
            return "firecrawl"
        return configured
    ...
    # pre-empted by a Nastech OAuth token whose subscription tier may not
...
def _web_requires_env() -> list[str]:
    ...
    refresh against the Nastech portal on every CLI startup (invoked at
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `nastech plugins enable {disabled_key}` "
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `nastech plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`nastech plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``nastech doctor`` paints a
...
if __name__ == "__main__":
    ...
    from nastech_cli.config import get_env_value as _gev
