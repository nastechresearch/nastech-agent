...
# The managed OpenAI audio gateway (Nastech portal proxy) only proxies these speech
...
# Base URL now resolved via nastech_cli.models.deepinfra_base_url (shared).
...
def _default_output_dir() -> str:
    ...
    NASTECH_HOME and later scope requests to a different profile via
    ``nastech_constants.set_nastech_home_override()`` — a frozen module
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``nastech tools``); otherwise the historical Edge backend remains active.

    The managed "Nastech Subscription" selection (``tts.provider: nastech``) is
    ...
    if provider == NASTECH_MANAGED_PROVIDER:
        return "openai"
    return provider
...
# file, so retired models disappear from nastech the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``nastech_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = _resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `nastech setup` to configure, "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `nastech setup` to install Mistral support."
                }, ensure_ascii=False)
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the Nastech managed audio
