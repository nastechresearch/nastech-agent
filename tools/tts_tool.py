...
def _default_output_dir() -> str:
    ...
    ``nastech_constants.set_nastech_home_override()`` — a frozen module constant keeps writing synthesized
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    speech); ``nastech`` is serviced by the OpenAI path through the managed openai-audio gateway."""
    provider = (tts_config.get("provider") or DEFAULT_PROVIDER).lower().strip()
    return "openai" if provider == NASTECH_MANAGED_PROVIDER else provider
...
def _tts_schema_overrides() -> dict:
    ...
    params["properties"]["output_path"]["description"] = _output_path_description(display_nastech_home())
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": _output_path_description("the profile NASTECH_HOME")
            },
            "speed": {
                "type": "number",
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'nastech_xai_user_agent': ('tools.xai_http', 'nastech_xai_user_agent'),
