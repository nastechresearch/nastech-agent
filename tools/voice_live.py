"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Nastech.
...
delegates every real request to Nastech as its *client-delegation* backend. Nastech stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Nastech' answer is what the voice speaks.
...
# backend (Nastech) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Nastech, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- Nastech agent: a full AI agent with tools — it can run commands, read and edit files, "
...
def _resolve_credentials(live: Dict[str, Any]) -> tuple[str, str]:
    ...
    The Nastech-managed audio proxy does not carry ``/live/sessions``; this mode is direct-key only.
    """
