...
def _managed_fal_billing_error(exc: BaseException, what: str) -> Optional[str]:
    """Human-readable tail for a Nastech managed-gateway ``BILLING_ERROR`` response, else None.
    ...
    detail = upstream.get("error") or "Nastech Portal rejected the charge authorization"
    return (
        f"{error.get('message') or 'Charge authorization failed'} (BILLING_ERROR; {code}: {detail}). "
        "This is a Nastech Portal billing configuration issue, not a missing local API key. "
        f"The managed route cannot run this {what} until Nastech enables its billing meter; "
...
class _ManagedFalSyncClient:
    """Drives a Nastech-managed fal-queue gateway via ``fal_client.SyncClient`` primitives; carries
    ...
    def submit(
        ...
        # The Nastech gateway currently records a keyed submission before billing
