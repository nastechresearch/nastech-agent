...
URL of Nastech' published Client ID Metadata Document (CIMD) as the
...
def _get_token_dir(nastech_home: str | Path | None = None) -> Path:
    ...
    base = Path(nastech_home) if nastech_home is not None else Path(get_nastech_home())
...
def _cached_redirect_port(storage: "NastechTokenStorage | None") -> int | None:
    ...
    redirect URI that was registered with it. If Nastech restarts and chooses a
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``nastech mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `nastech mcp login <server>` interactively to (re)authorize, "
...
class NastechTokenStorage:
    ...
    File layout::
        ...
        NASTECH_HOME/mcp-tokens/<server_name>.cimd-off      -- CIMD refused here
    """
    ...
    def _tokens_path(self) -> Path:
        return _get_token_dir(self._nastech_home) / f"{self._server_name}.json"

    def _client_info_path(self) -> Path:
        return _get_token_dir(self._nastech_home) / f"{self._server_name}.client.json"

    def _meta_path(self) -> Path:
        return _get_token_dir(self._nastech_home) / f"{self._server_name}.meta.json"

    def _cimd_rejected_path(self) -> Path:
        return _get_token_dir(self._nastech_home) / f"{self._server_name}.cimd-off"
    ...
    def mark_cimd_rejected(self) -> None:
        ...
        refused. Cleared by ``remove()``, i.e. by ``nastech mcp login`` /
        ``nastech mcp remove``, so a fixed document gets another chance.
        """
    ...
    async def _redirect_handler(authorization_url: str) -> None:
        ...
        if on_ssh and redirect_uri:
            ...
            print(
                ...
                f"\n"
                f"  See: https://nastechresearch.github.io/nastech-agent/docs/guides/oauth-over-ssh\n",
                file=sys.stderr,
            )
        ...
        if result["auth_code"] is None:
            ...
            if cimd_url:
                hint = (
                    ...
                    f"Nastech' Client ID Metadata Document ({cimd_url}); set "
...
def _get_nastech_oauth_provider_class() -> type | None:
    global NastechOAuthClientProvider
    if NastechOAuthClientProvider is not None:
        return NastechOAuthClientProvider
    if not _ensure_sdk_loaded():
        return None

    class _NastechOAuthClientProvider(OAuthClientProvider):
        ...
        def _stamp_token_user_agent(self, request):
            ua = getattr(self, "_nastech_token_user_agent", None)
            if ua:
    ...
    _NastechOAuthClientProvider.__name__ = "NastechOAuthClientProvider"
    _NastechOAuthClientProvider.__qualname__ = "NastechOAuthClientProvider"
    NastechOAuthClientProvider = _NastechOAuthClientProvider
    return NastechOAuthClientProvider
...
# 2026-07-28. The SDK does the protocol work; Nastech only decides whether a
...
# nastechresearch.github.io/nastech-agent/docs/* 301s here.
_CIMD_CLIENT_METADATA_URL = (
    "https://nastechresearch.github.io/nastech-agent/docs/oauth/client-metadata.json"
)
...
# (section 4.2), so a CIMD flow cannot use the ephemeral port Nastech picks
...
def _has_cached_client_info(storage: "NastechTokenStorage | None") -> bool:
...
def _server_declined_cimd(storage: "NastechTokenStorage | None") -> bool:
    ...
    Nastech has to fix the redirect URI. Cached authorization-server metadata
...
def _maybe_use_cimd(
    ...
    Every early return below is a case where the redirect URI Nastech would
...
#   "Claude Code" → 200
#   "Codex"       → 200
#   "Nastech Agent" / "Nastech" / "Cursor" / "VS Code" / … → 403
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    metadata_kwargs: dict[str, Any] = {
        ...
        # rejecting loopback redirect_uris. Nastech is a CLI/desktop app
...
def _invalidate_tokens_on_client_change(
    ...
    if removed:
        logger.warning(
            ...
            "Re-authorize with: nastech mcp login %s",
...
def humanize_oauth_registration_error(
    ...
    *,
    ...
    Figma's remote MCP gates DCR on exact ``client_name``. Nastech auto-sets
    ``Claude Code`` (known-good); this message fires when the user overrode
    that with something Figma still rejects, or an older Nastech is running.
    """
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`nastech mcp login {server_name}` interactively first to complete "
    ...
    provider_class = _get_nastech_oauth_provider_class()
