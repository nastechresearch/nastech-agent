...
refresh; this module supplies ``NastechTokenStorage`` (on-disk persistence), the localhost callback
listener and ``build_oauth_auth()`` (legacy entry point). client_id is Nastech' Client ID Metadata
...
async def acquire_refresh_fence(path: "Path", *, timeout: float = _REFRESH_FENCE_TIMEOUT_SECONDS) -> int:
    ...
    try:
        ...
        mkdir_under_nastech_home(lock_path.parent)
...
def _get_token_dir(nastech_home: str | Path | None = None) -> Path:
    """``NASTECH_HOME/mcp-tokens/`` — per-profile token directory."""
    from nastech_constants import get_nastech_home

    return Path(nastech_home if nastech_home is not None else get_nastech_home()) / "mcp-tokens"
...
def _cached_client_info(storage: "NastechTokenStorage | None") -> dict | None:
...
def _cached_redirect(storage: "NastechTokenStorage | None") -> "tuple[str | None, int | None]":
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    ``lead`` is the boundary-specific first sentence; this helper appends the shared, actionable ``nastech
    mcp login`` next-step so the guidance wording lives in one place across every non-interactive OAuth
    boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `nastech mcp login <server>` interactively to (re)authorize, then restart or reload the gateway."
        )
...
class NastechTokenStorage:
    """Persist OAuth state as ``NASTECH_HOME/mcp-tokens/<server_name>`` + ``.json`` (tokens),
    ...
    def __init__(self, server_name: str, *, nastech_home: str | Path | None = None):
        ...
        self._nastech_home = Path(nastech_home) if nastech_home is not None else None
    ...
    def _path(self, suffix: str) -> Path:
        return _get_token_dir(self._nastech_home) / f"{self._server_name}{suffix}"
    ...
    def _fixup_loaded_tokens(self, data: dict) -> None:
        # ``nastech_issuer`` is Nastech bookkeeping, not an SDK OAuthToken field: pop before validation.
        self.loaded_issuer = data.pop("nastech_issuer", None)
    ...
    async def set_tokens(self, tokens: "OAuthToken") -> None:
        ...
        if self._bound_issuer:  # which authorization server granted these tokens (never sent on the wire)
            payload["nastech_issuer"] = self._bound_issuer
    ...
    def stamp_issuer(self, issuer: str) -> None:
        """Backfill ``nastech_issuer`` onto a pre-binding token file: adopt the currently discovered
        issuer once instead of forcing a re-login, so the *next* read is protected."""
        data = _read_json(self._tokens_path())
        if data is None or data.get("nastech_issuer"):
            return
        data["nastech_issuer"] = str(issuer)
        try:
    ...
    def restore(self, snapshot: dict[str, bytes], *, only_if_absent: bool = False) -> None:
        ...
        if not snapshot:
            return
        token_dir = _get_token_dir(self._nastech_home)
        from nastech_constants import mkdir_under_nastech_home

        mkdir_under_nastech_home(token_dir)
...
def _make_callback_handler() -> tuple[type, dict]:
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = ("<h2>Authorization Successful</h2><p>You can close this tab and return to Nastech.</p>" if parsed["code"]
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `nastech mcp login <server>` later to authenticate, "
...
def _callback_outcome(result: dict, cimd_url: str | None):
    ...
    if result["auth_code"] is None:
        hint = (
            ...
            f"server rejected Nastech' Client ID Metadata Document ({cimd_url}); set ``cimd: false`` under that "
...
NastechOAuthClientProvider: Any = None


def remove_oauth_tokens(server_name: str, *, nastech_home: str | Path | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    NastechTokenStorage(server_name, nastech_home=nastech_home).remove()
...
def _server_declined_cimd(storage: "NastechTokenStorage | None") -> bool:
    """True when cached metadata shows this server doesn't advertise CIMD. The SDK decides CIMD vs DCR
    in its 401 branch — after Nastech must fix the redirect URI — so cached metadata closes the gap;
...
def _maybe_use_cimd(cfg: dict, storage: "NastechTokenStorage | None" = None) -> "tuple[str, int] | None":
...
def _configure_callback_port(cfg: dict, storage: "NastechTokenStorage | None" = None) -> int:
...
def _invalidate_tokens_on_client_change(
    storage: "NastechTokenStorage", new_client_id: str, new_client_secret: str | None) -> None:
    ...
    if removed:
        logger.warning(
            ...
            "the previous client. Re-authorize with: nastech mcp login %s",
            storage._server_name, old_client_id, new_client_id, storage._server_name)


def _maybe_preregister_client(storage: "NastechTokenStorage", cfg: dict, client_metadata: "OAuthClientMetadata") -> None:
...
def humanize_oauth_registration_error(
    ...
    when the user overrode it or an older Nastech is running."""
    msg = str(exc)
    lowered = msg.lower()
...
def build_oauth_auth(server_name: str, server_url: str, oauth_config: dict | None = None) -> "OAuthClientProvider | None":
    ...
    global NastechOAuthClientProvider
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"requires browser authorization. Run `nastech mcp login {server_name}` interactively first to complete "
            "initial authorization, then cached tokens will be reused.")
    kwargs = build_provider_kwargs(cfg, storage, ssh_proxy_hint=True)
    if NastechOAuthClientProvider is None:
        from tools.mcp_oauth_provider import NastechProviderMixin

        NastechOAuthClientProvider = type("NastechOAuthClientProvider", (NastechProviderMixin, _sdk_class("OAuthClientProvider")), {
            "__doc__": "SDK provider plus Nastech' token-endpoint fixes (see ``NastechProviderMixin``).",
            "__module__": __name__, "_nastech_logger": logger})
    return NastechOAuthClientProvider(server_url=server_url, **kwargs)
