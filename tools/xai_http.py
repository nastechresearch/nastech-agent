...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    ...
    try:
        from nastech_constants import get_nastech_home
        marker_dir = get_nastech_home() / "state"
...
def _xai_base_url_override() -> str:
    """``NASTECH_XAI_BASE_URL`` then ``XAI_BASE_URL``, stripped; '' when unset."""
    from nastech_cli.config import get_env_value
    return str(get_env_value("NASTECH_XAI_BASE_URL") or get_env_value("XAI_BASE_URL") or "").strip().rstrip("/")
...
def resolve_xai_http_credentials(
    *, force_refresh: bool = False, api_key_hint: Optional[str] = None, prefer_api_key: bool = False,
    ...
    Default order: Nastech-managed xAI OAuth, then ``XAI_API_KEY`` (via ``get_env_value`` so
    ``~/.nastech/.env`` keys count). ``prefer_api_key=True`` inverts that for API-metered
    endpoints where the subscription OAuth bearer authorizes but misbehaves (x_search answers
    without citations, TTS 403s). Both branches honor ``NASTECH_XAI_BASE_URL``/``XAI_BASE_URL``
    ...
    Prefers Nastech-managed xAI OAuth credentials when available, then falls back to ``XAI_API_KEY`` resolved
    via ``nastech_cli.config.get_env_value`` so keys stored in ``~/.nastech/.env`` (the standard Nastech
    ...
    scoping is identical to the fallback branch, and the base URL honors ``NASTECH_XAI_BASE_URL`` /
