...
def _resolve_explicit_xai_base_url(default: str = "https://api.x.ai/v1") -> str:
    ...
    Honors ``NASTECH_XAI_BASE_URL`` then ``XAI_BASE_URL`` (the same override
    pair the OAuth branch reads) and pins the origin with
    :func:`nastech_cli.auth._xai_validate_inference_base_url` so a tampered
...
def resolve_xai_http_credentials(
    *,
    ...
    honors ``NASTECH_XAI_BASE_URL`` / ``XAI_BASE_URL`` behind the same
