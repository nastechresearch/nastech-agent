...
# ``nastech send`` CLI, the kanban notifier and the opt-in MCP server import the helpers.
...
def _handle_send(args):
    ...
    # (`nastech send` MESSAGE) and crash the UTF-8 marshal inside platform SDK
    # request bodies (feishu/lark, #113799). Every send_message caller (model tool
    # call, `nastech send`, dashboard console) enters here, so scrub once before the
...
def _not_configured_error(platform_name, platform, entry):
    ...
    process actually read (a hardcoded ``~/.nastech`` does not exist on a Windows or profile home)."""
    ...
    # gateway started from the default root (the reporter's shell had NASTECH_HOME=<root>/profiles/<p>)
    # never reads this profile's .env at all.
    try:
        from gateway.status import read_runtime_status, runtime_status_pid_is_live
        from nastech_constants import get_default_nastech_root, nastech_home_key
        root = get_default_nastech_root()
        gateways = [(home, read_runtime_status())]
        if nastech_home_key(root) != nastech_home_key(home):
...
def _maybe_skip_cron_duplicate_send(platform_name: str, chat_id: str, thread_id: str | None):
    ...
    from gateway.session_context import get_session_env
    auto_platform = get_session_env("NASTECH_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    auto_chat_id = get_session_env("NASTECH_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not (auto_platform and auto_chat_id and auto_platform == platform_name and auto_chat_id == str(chat_id)
            and (get_session_env("NASTECH_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None) == thread_id):
        return None
