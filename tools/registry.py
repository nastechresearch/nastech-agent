"""Central registry for all nastech-agent tools: each tool file calls ``registry.register()``
...
# timescales, so results are cached ~30 s: env-var flips via ``nastech tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "NASTECH_SESSION_ID",
    "NASTECH_BROWSER_CONTROL_PRINCIPAL",
    "NASTECH_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a Nastech-home override
...
def _core_tools_gated_by(fn: Callable) -> Set[str]:
    """Names of ``_NASTECH_CORE_TOOLS`` members whose registered ``check_fn`` is *fn*."""
    try:
        from toolsets import _NASTECH_CORE_TOOLS
        core = frozenset(_NASTECH_CORE_TOOLS)
    except Exception:
        return set()
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``nastech tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved NASTECH_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("nastech_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``nastech_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``nastech_plugins.pkg.handlers`` is still owned by the
                # ``nastech_plugins.pkg`` package — exact string equality would wrongly block root-module
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``nastech tools enable`` lands quickly."""
        result = []
