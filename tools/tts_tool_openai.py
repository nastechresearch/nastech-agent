...
Also owns the managed-gateway (Nastech portal ``openai-audio`` proxy) route selection that
...
# DeepInfra base URL is resolved via nastech_cli.models.deepinfra_base_url (shared).
...
def _managed_openai_audio_route() -> Optional[tuple]:
    ...
    if gateway is None:
        return None
    return gateway.nastech_user_token, urljoin(f"{gateway.gateway_origin.rstrip('/')}/", "v1"), True
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    Nastech proxy, so callers coerce the request). Strict on the stored ``tts`` selection: ``"nastech"``
    ...
    if route is None:
        ...
        if managed_nastech_tools_enabled():
            message += ". " + nastech_tool_gateway_unavailable_message("managed OpenAI audio for TTS")
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    """Resolve DeepInfra credentials/model (live ``nastech_cli.models`` catalog, no hardcoded ids), then
    delegate to the OpenAI-compatible handler."""
    api_key = _origin()._resolve_provider_key("DEEPINFRA_API_KEY", "deepinfra")
    if not api_key:
        raise ValueError("DEEPINFRA_API_KEY not set. Run `nastech setup` to configure, or set the env var directly.")
