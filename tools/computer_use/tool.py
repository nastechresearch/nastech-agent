...
`health_report` (surfaced by `nastech computer-use doctor`) reports the
...
# Per-Nastech-session cached backends. Each backend owns its own cua-driver
...
# See NastechResearch/nastech-agent#67052 gap 4.
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Nastech's explicit approval bypass onto Cua's immutable mode.

    Nastech has TWO session-identity namespaces: the tool-dispatch path passes
        ...
        with _backend_lock:
            ...
            if cached is not None:
                ...
                backend_name = os.environ.get(
                    "NASTECH_COMPUTER_USE_BACKEND", "cua"
                ).lower()
                if backend_name in {"cua", "cua-driver", ""}:
                    ...
                    raise RuntimeError(
                        f"Unknown NASTECH_COMPUTER_USE_BACKEND={backend_name!r}"
                    )
...
def release_computer_use_session(session_id: str) -> bool:
    ...
    Nastech YOLO selected unrestricted mode. Approval state is cleared even
    ...
    try:
        ...
        # while waiting: unrelated Nastech sessions remain independent.
...
def _shutdown_backend_atexit() -> None:
    ...
    without this a driver can survive the Nastech process that spawned it
...
def _request_approval(action: str, args: Dict[str, Any],
    ...
    silently authorize it (NastechResearch/nastech-agent#67052).
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    # none of those capabilities can be supplied across Nastech sessions.
...
# "foreground" (it doesn't know Nastech has a typed page route), so the model
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of NastechResearch/nastech-agent#47072 — trycua/cua#1961
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Save a capture in Nastech' media cache and return its absolute path.
    ...
    try:
        ...
        cache_dir = get_nastech_dir("cache/images", "image_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
        try:
            captures = sorted(
...
def _spill_elements_to_file(cap: CaptureResult) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_nastech_dir("cache/computer_use", "computer_use_cache")
        cache_dir.mkdir(parents=True, exist_ok=True)
...
def check_computer_use_requirements() -> bool:
    ...
    `nastech computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
