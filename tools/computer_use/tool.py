...
# Hard-blocked regardless of approval level (e.g. logout kills the session Nastech runs in). Alt is
...
# Per-Nastech-session cached backends (own cua-driver session, native target, refs, grant namespace).
...
def _cua_permission_mode(session_id: str) -> str:
    """Map Nastech's approval bypass onto Cua's immutable mode; fails closed. Both identity namespaces are consulted
...
def _scoped_sid(session_id: str) -> str:
    """Cache key for one Nastech session's backend. Outside a served-profile scope it is the bare id
    ...
    return sid if get_nastech_home_override() is None else f"{sid}@{nastech_home_key()}"
...
def _get_backend(session_id: str = "") -> ComputerUseBackend:
    ...
    while True:
        with _backend_lock:
            ...
            permission_mode = _cua_permission_mode(bare_sid)  # approval state is keyed by the Nastech session id
...
def _shutdown_backend_atexit() -> None:
    ...
    the Nastech process that spawned it (#28152 item 3). #69903 kept the orphan from burning a core by
...
class _NoopBackend(ComputerUseBackend):  # pragma: no cover
    """Test/CI stub (NASTECH_COMPUTER_USE_BACKEND=noop). Records ``(name, kwargs)`` calls; returns trivial results."""
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({"error": f"computer_use backend unavailable: {e}",
                           "hint": "If the cua-driver binary is missing, run `nastech computer-use install`. "
...
def _cache_file(subdir: str, legacy: str, name: str, pattern: str = "", cap: int = 0):
    """Path for a new file under ``$NASTECH_HOME/<subdir>`` (dir created). With ``pattern``/``cap``, first unlinks the
    oldest matching files so at most ``cap - 1`` remain (best-effort)."""
    from nastech_constants import get_nastech_dir  # lazy so tests can patch get_nastech_dir
    cache_dir = get_nastech_dir(subdir, legacy)
...
def _persist_capture_image(cap: CaptureResult) -> Optional[str]:
    """Copy of the capture in Nastech' media cache so attachment surfaces can deliver it (None without an image)."""
...
def _should_route_through_aux_vision() -> bool:
    ...
    try:
        ...
        if (cached := _AUX_VISION_ROUTE_CACHE.get(key := (nastech_home_key(), str(provider), str(model)))) is not None:
            return cached
        stage = "decision"
...
def _route_capture_through_aux_vision(cap: CaptureResult, summary: str, *, visible_elements: Optional[List[UIElement]] = None,
    ...
    """Pre-analyse the capture via ``vision_analyze_tool`` (temp file under ``$NASTECH_HOME/cache/vision/``) and merge
...
def check_computer_use_requirements() -> bool:
    """macOS/Windows/Linux + cua-driver binary (or env override). `nastech computer-use doctor` names blocked checks."""
