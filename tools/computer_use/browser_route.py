...
* the driver session id is injected by the adapter, never accepted from the
  model;
* refs are usable only from the latest snapshot in this Nastech session;
...
ordinary Nastech tool approval never substitutes for protected consent.
"""
    ...
    def prepare(
        ...
        *,
        ...
        if profile_mode == "existing_profile":
            ...
            # bounded manifest; explicit Nastech YOLO owns a private
            # unrestricted daemon.
            self.state.clear()
            args: Dict[str, Any] = {
                "pid": exact_pid,
            ...
            if isinstance(approval_token, str) and approval_token:
                # Minted out-of-band by `nastech computer-use browser-approve`
