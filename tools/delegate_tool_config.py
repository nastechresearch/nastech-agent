...
# match nastech_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _load_config() -> dict:
    ...
    (follows NASTECH_HOME/profile; no deepcopy, since this runs on every get_definitions() rebuild) over the legacy
    ``cli.CLI_CONFIG``, which can hide user-set keys — except that ``NASTECH_IGNORE_USER_CONFIG=1`` is only honored
    by the legacy loader, so it stays authoritative when that flag is set."""
    if os.environ.get("NASTECH_IGNORE_USER_CONFIG") != "1":
        try:
...
_NASTECH_PROVIDERS = frozenset({"nastech", "nastech-portal", "nastechresearch"})
...
def _resolve_child_runtime(
    ...
    or is Nastech Portal (dual-wire); a pinned ``delegation.command`` must exist on PATH or the spawn fails loudly;
    ...
    # otherwise). Nastech Portal is dual-wire within one provider (anthropic/* → Messages, else chat_completions), so
    ...
    # would pin a child Nastech/Qwen subagent onto the parent's Claude Messages wire (or the reverse).
    # agent_init honors an explicit api_mode above its nastech branch, so re-derive here before construction.
    _parent_provider = getattr(parent_agent, "provider", None) or ""
    if override_api_mode is not None:
        effective_api_mode = override_api_mode
    elif (effective_provider or "").strip().lower() in _NASTECH_PROVIDERS:
