import type { GatewayWsUrlResult } from '@nastech/shared'
import type { TranslucencyState } from '@nastech/shared/translucency'

import type { ScreenshotApi } from '../electron/command-screenshot-types'
import type { NastechNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    nastechDesktop: {
      ...
      // over nastech:window-control; Electron/OS chrome owns them elsewhere.
      windowControls: {
        custom: boolean
        minimize: () => void
        toggleMaximize: () => void
      ...
      // Nastech Cloud: one portal login powers discovery + silent per-agent
      ...
      readPluginSource?: (filePath: string) => Promise<NastechReadFileTextResult>
      ...
      watchDirectory?: (dir: string) => Promise<NastechPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: NastechActiveWork) => void
      ...
      /** Launch flag: the Nastech free tier is on for this launch
       *  (NASTECH_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (NASTECH_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh NASTECH_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      // Local Desktop runtime-plugin root (<NASTECH_HOME>/desktop-plugins),
      ...
      /** LOCAL `<NASTECH_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<NastechGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<NastechGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<NastechGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<NastechRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: NastechReviewScope, baseRef?: null | string) => Promise<NastechReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: NastechReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<NastechReviewShipInfo>
...
export interface NastechActiveWork {
  count: number
  titles: string[]
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Nastech Cloud connection: it carries
...
// A discovered Nastech Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface DesktopBootProgress {
  error: string | null
  fakeMode: boolean
  /** True when the boot failure is a Nastech Cloud agent that is down (HTTP 502/503/504). */
...
export interface NastechGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface NastechGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface NastechGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface NastechRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface NastechRepoStatus {
  branch: null | string
...
// turn began.
export type NastechReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface NastechReviewFile {
  path: string
  added: number
  removed: number
...
export interface NastechReviewList {
  files: NastechReviewFile[]
...
export interface NastechReviewPr {
  url: string
  state: string
  number: number
}
...
export interface NastechBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface NastechRepoPullRequests {
  ghReady: boolean
