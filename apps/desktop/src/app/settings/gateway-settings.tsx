...
// Nastech Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.nastechDesktop.setSecretStorageEncryption(on)
  ...
  // --- Nastech Cloud (cloud mode) state ---
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.nastechDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteNastechPath: state.sshRemoteNastechPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.nastechDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.nastechDesktop.saveConnectionConfig(payload(allowPlainTextToken))
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.nastechDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.nastechDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.nastechDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.nastechDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.nastechDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.nastechDesktop.testConnectionConfig(payload())
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Nastech Cloud panel: one portal sign-in, then a discovered-agent picker
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteNastechPath: event.target.value }))}
                placeholder={g.sshNastechPathPlaceholder}
                value={state.sshRemoteNastechPath}
              />
            }
            description={g.sshNastechPathDesc}
            title={g.sshNastechPathTitle}
          />
        </div>
      ) : null}
