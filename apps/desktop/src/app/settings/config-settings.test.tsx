...
vi.mock('@/nastech', () => ({
  ...
  saveNastechConfig: (config: unknown, profile?: string) => saveNastechConfig(config, profile),
...
beforeEach(() => {
  ...
  getNastechConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('renders and saves the Codex compression auto-raise setting', async () => {
    ...
    getNastechConfigSchema.mockResolvedValue({
      fields: {
    ...
    try {
      ...
      await vi.waitFor(() =>
        expect(saveNastechConfig).toHaveBeenCalledWith({ compression: { codex_gpt55_autoraise: false } }, undefined)
      )
    } finally {
      vi.useRealTimers()
    }
  ...
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getNastechConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await vi.waitFor(() => expect(saveNastechConfig).toHaveBeenCalledTimes(1))
      expect(saveNastechConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await vi.waitFor(() => expect(saveNastechConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveNastechConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
