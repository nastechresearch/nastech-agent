...
function ConfigSettingsInner({
  ...
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useNastechConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => nastechConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['nastech-config-schema'] : ['nastech-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getNastechConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `nastech config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<NastechConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveNastechConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: NastechConfigRecord) => {
