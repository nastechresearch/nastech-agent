...
export function ConfigSettings({
  ...
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useNastechConfigRecord()
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          const result = await saveNastechConfig(config)
