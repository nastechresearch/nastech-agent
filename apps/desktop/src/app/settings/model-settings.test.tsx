...
beforeEach(() => {
  ...
  setModelAssignment.mockResolvedValue({ ok: true, provider: 'nastech', model: 'nastech-4', gateway_tools: [] })
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) as a sparse patch, never the cached snapshot', async () => {
    ...
    await waitFor(() => expect(saveNastechConfig).toHaveBeenCalledWith({ agent: { service_tier: 'fast' } }))
  })
  ...
  it('edits auxiliary reasoning effort below the selected model and applies it with the assignment', async () => {
    getAuxiliaryModels.mockResolvedValueOnce({
      main: { provider: 'nastech', model: 'nastech-4' },
      tasks: [{ task: 'vision', provider: 'nastech', model: 'nastech-4', base_url: '', reasoning_effort: null }]
    })
...
describe('ModelSettings code-skew 503', () => {
  const skewError = new Error(
    'Error invoking remote method \'nastech:api\': Error: 503: {"detail":"Restart required: This process is running code from 08b4875f4a but the checkout on disk is now 48d2528066. The model picker would risk a stale-module crash — restart the Desktop-owned backend to load the new code (use Restart backend in Nastech Desktop, or quit and reopen the app)"}'
  )
  ...
  it('unwraps the stale-backend 503 instead of dumping IPC JSON', async () => {
    ...
    expect(screen.queryByText(/nastech:api/)).toBeNull()
  ...
  it('recycles the Desktop-owned backend and reloads the catalog', async () => {
    ...
    ;(window as unknown as { nastechDesktop: { recycleBackend: typeof recycleBackend } }).nastechDesktop = {
      recycleBackend
    }
