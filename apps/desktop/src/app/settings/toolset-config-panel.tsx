 ...
 * server-computed `status` (keys ∧ Nastech entitlement ∧ post-setup install
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange, profile }: ToolsetConfigPanelProps) {
  ...
  // Guard the Nastech Portal sign-in poll loop against unmount/state updates.
  ...
  async function handleSelect(provider: ToolProvider) {
    ...
    try {
      ...
      if (result.needs_nastech_auth) {
        // Managed Nastech row selected without Portal entitlement: the config
        ...
        // sign-in action). Reuses the existing Nastech Portal device-code flow.
        notify({
          kind: 'warning',
  ...
  // Drive the existing Nastech Portal OAuth device-code flow (the same session
  ...
  async function signInToNastechPortal() {
    try {
      const start = await startOAuthLogin('nastech', profile)

      if (start.flow !== 'device_code') {
        notifyNastechAuthFailed(`unexpected flow: ${start.flow}`)
      ...
      for (let attempt = 0; attempt < 120 && mountedRef.current; attempt += 1) {
        ...
        const polled = await pollOAuthSession('nastech', start.session_id, profile)

        if (polled.status === 'approved') {
          notify({ kind: 'success', title: copy.nastechAuthDoneTitle, message: copy.nastechAuthDoneMessage })
          await refresh()
          onConfiguredChange?.()
        ...
        if (polled.status !== 'pending') {
          notifyNastechAuthFailed(polled.error_message || `Sign-in ${polled.status}`)
      ...
      if (mountedRef.current) {
        notifyNastechAuthFailed(err instanceof Error ? err.message : String(err))
      }
    }
  }
  ...
  function notifyNastechAuthFailed(detail: string) {
    notify({
      kind: 'error',
      title: copy.nastechAuthFailed,
      message: copy.nastechAuthFailedMessage,
      detail,
      action: { label: copy.nastechAuthTryAgain, onClick: () => void signInToNastechPortal() }
    })
  }
