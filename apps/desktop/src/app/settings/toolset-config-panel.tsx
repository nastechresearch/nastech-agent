  ...
  async function signInToNastechPortal() {
    try {
      ...
      if (start.flow !== 'device_code') {
        notifyError(new Error(`unexpected flow: ${start.flow}`), copy.nastechAuthFailed)
      ...
      for (let attempt = 0; attempt < 120 && mountedRef.current; attempt += 1) {
        ...
        if (polled.status !== 'pending') {
          notifyError(new Error(polled.error_message || `Sign-in ${polled.status}`), copy.nastechAuthFailed)
      ...
      if (mountedRef.current) {
        notifyError(err, copy.nastechAuthFailed)
      }
    }
  }
