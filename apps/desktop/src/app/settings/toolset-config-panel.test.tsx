...
const saveNastechConfigRecord = vi.fn()
...
vi.mock('@/nastech', () => ({
  ...
  saveNastechConfigRecord: (config: unknown, profile?: unknown) => saveNastechConfigRecord(config, profile),
...
beforeEach(() => {
  ...
  saveNastechConfigRecord.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveNastechConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveNastechConfigRecord.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
    ...
    expect(saveNastechConfigRecord.mock.calls.at(-1)?.[1]).toBeUndefined()
  })
  ...
  it('autosaves the inline voice fields into the profile the panel is scoped to', async () => {
    ...
    await waitFor(() => expect(saveNastechConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const [saved, forwarded] = saveNastechConfigRecord.mock.calls.at(-1) as [
