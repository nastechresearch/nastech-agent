...
import { disconnectOAuthProvider, listOAuthProviders } from '@/nastech'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.nastechDesktop?.terminal)
...
//      in nastech_cli/provider_catalog.py) — the SAME provider identity
//      `nastech model` uses. This is authoritative: a provider tagged by the
...
function OAuthPicker({
  ...
  // The free tier holds a token but no account: it is never "connected"; the featured Nastech row
  // names it (Nastech · free tier) and offers the sign-in that keeps its connectors.
...
function ConnectedProviderRow({
  ...
  // Nastech can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Nastech can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Nastech never deletes creds another tool owns behind a silent API call.
