 ...
 * agent sources (local runtime + any number of remote gateways / Nastech Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.nastechDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.nastechDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `nastech update` to every eligible source; per-connection results
