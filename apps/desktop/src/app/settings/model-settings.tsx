...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useNastechConfigRecord(scopeProfile)
  const setConfig = useMemo(() => nastechConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateNastechConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveNastechConfig(setNested({}, key, value), scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.nastechDesktop?.recycleBackend?.(scopeProfile)
