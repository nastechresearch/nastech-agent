...
// Nastech session ids: <YYYYMMDD>_<HHMMSS>_<6 hex>. Used to offer a direct
...
function CommandPaletteBody({ onExited }: { onExited: () => void }) {
        ...
        : []),
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Download,
            id: 'cc-update-nastech',
            keywords: ['update', 'upgrade', 'nastech', 'version', 'system', 'restart'],
            label: cc.updateNastech,
  ...
  const searchGroups = useMemo<PaletteGroup[]>(() => {
    ...
    // Apply a theme directly from the root search (e.g. "nastech" → Nastech). Live
