...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/nastech'
...
export function SkillsView({
  ...
  const registryBridge = window.nastechDesktop?.connections
  const rosterBridge = window.nastechDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Nastech (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Nastech ships but
