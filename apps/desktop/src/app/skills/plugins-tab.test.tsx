...
describe('PluginsTab', () => {
  ...
  it('marks a remote-backend desktop half unavailable instead of forever copying', () => {
    ...
    const detail = within(screen.getByRole('row', { name: /^nastech-prices/ }))
  ...
  it('renders a unified package as ONE row with a Desktop switch and an Agent switch', () => {
    $pluginRecords.set({
      media: { id: 'media', name: 'Media Studio', kind: 'disk', status: 'loaded', packageName: 'nastech-media-studio' }
    })
    $agentPlugins.set([
      {
        description: '',
        key: 'nastech-media-studio',
  ...
  it('offers "Install here" for a desktop half whose agent half is not in the selected profile', async () => {
    $pluginRecords.set({
      media: {
        ...
        kind: 'disk',
        status: 'loaded',
        packageName: 'nastech-media-studio',
        packageOrigin: { repo: 'https://github.com/NastechResearch/nastech-media-studio.git', sha: 'abc' }
      }
    })
    ...
    await waitFor(() => {
      expect($pluginInstallRequest.get()).toMatchObject({
        legacyHint: 'agent',
        profile: 'workbot',
        repo: 'https://github.com/NastechResearch/nastech-media-studio.git',
        sha: 'abc'
      })
    })
  })
