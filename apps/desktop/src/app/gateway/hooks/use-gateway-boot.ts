...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    let pendingWindowState: NastechWindowState | null = null
    ...
    // The other way a cold boot concludes. Main keeps startNastech() available
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Nastech…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Nastech backend'
        )
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshNastechConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startNastech re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new NastechGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Nastech Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen({ explicit: true }))) {
          throw new Error('Nastech gateway is not connected')
        }
    ...
    const offExit = desktop.onBackendExit(() => {
      ...
      notify({
        ...
        durationMs: 0,
        action: {
          label: translateNow('boot.errors.restartNastech'),
    ...
    async function boot() {
      ...
      try {
        ...
        // round-trip must not hang "Starting Nastech…" forever. Initial boot
        ...
        const conn = await withTimeout(
          ...
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Nastech backend'
        )
        ...
        // reaches the recovery affordance instead of hanging "Starting Nastech…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshNastechConfig(),
          ...
          // retry it), not brick boot behind the "Nastech couldn't start"
