...
vi.mock('@/nastech', async importOriginal => {
  const actual = await importOriginal<typeof NastechModule>()

  class FakeNastechGateway {
  ...
  return { ...actual, NastechGateway: FakeNastechGateway }
})
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as NastechGateway, 'default')
  $gateway.set(primary as unknown as NastechGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.nastechDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as NastechGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { nastechDesktop: unknown }).nastechDesktop = { getConnection }
    $gateway.set(dropped)
