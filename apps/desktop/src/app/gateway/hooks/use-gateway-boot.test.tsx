...
function Harness({
  ...
  refreshNastechConfig?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshNastechConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshNastechConfig={refreshNastechConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForNastech, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting Nastech…" forever; the renderer needs to own its own bound
  ...
  it('manual reconnect replaces only the active secondary route', async () => {
    ;(window as { nastechDesktop?: unknown }).nastechDesktop = {
      ...fakeDesktop(),
      getConnectionFor: vi.fn(async () => coderConn),
      getGatewayWsUrlFor: vi.fn(async () => coderConn.wsUrl)
    }
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote Nastech gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('a backend exit while the boot overlay is up fails the overlay and does not add a dead-button toast', async () => {
    ...
    // Nastech" toast here would do nothing when clicked; the overlay's own
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startNastech() rejects with a
  ...
  it('RETRY CONTRACT: a post-connect failure stays terminal even when its socket closes before boot catches it — a closed socket after a good dial is not a dial failure', async () => {
    ...
    const refreshNastechConfig = vi.fn(async () => {
  ...
  it('a failed cold boot keeps its recovery surface while main replays cold-boot progress behind it (#112899)', async () => {
    // Main keeps startNastech() available after the renderer's boot concluded
    ...
    const desktop = fakeDesktop()
    desktop.getConnection = vi.fn(async () => {
      throw new Error('Nastech backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example')
    })
    desktop.getBootProgress = vi.fn(async () => ({
      error: 'Nastech backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example',
      fakeMode: false,
      message: 'Desktop boot failed',
      phase: 'backend.error',
      progress: 24,
    ...
    for (let replay = 0; replay < 2; replay += 1) {
      act(() => {
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Resolving Nastech backend',
        ...
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Connecting to remote Nastech backend at https://gateway.tailnet.example:8443',
          phase: 'backend.remote',
          progress: 24,
          running: true,
          timestamp: Date.now()
