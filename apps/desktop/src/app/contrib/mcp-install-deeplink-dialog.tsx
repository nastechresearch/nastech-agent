...
import { setNastechConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `nastech://mcp/install` deep links. The payload is
 ...
 * confirming lets Nastech spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getNastechConfigRecord())
