...
import { pathFromNastechDeepLink, resolveNastechOpenPath } from '@/lib/nastech-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Nastech" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.nastechDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  // nastech://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future nastechDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveNastechOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onDeepLink?.(payload => {
      ...
      // link (nastech://index-network/intent/1, nastech://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromNastechDeepLink(payload.kind, payload.name || '', payload.params || {})
  ...
  useEffect(() => {
    const unsubscribe = window.nastechDesktop?.onPreviewNav?.(command => {
