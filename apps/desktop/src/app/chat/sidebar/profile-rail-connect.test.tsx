  ...
  useI18n: () => ({
    t: {
      ...
      profiles: {
        ...
        connectGateway: 'Connect another Nastech gateway…',
...
describe('ProfileRail multi-gateway entry point', () => {
  it('deep-links to the unified Settings → Gateways page from the rail', () => {
    ...
    const pill = screen.getByRole('button', { name: 'Connect another Nastech gateway…' })
  ...
  it('keeps the entry point visible for single-profile users', () => {
    ...
    expect(screen.getByRole('button', { name: 'Connect another Nastech gateway…' })).toBeTruthy()
