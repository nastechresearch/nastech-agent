...
export function ProfileRemoteOverrideDialog({ profileNames }: { profileNames: string[] }) {
  ...
  useEffect(() => {
    ...
    window.nastechDesktop?.connections
      ?.list()
      .then(registry => {
        if (cancelled) {
          return
  ...
  const performSave = async () => {
    ...
    try {
      await window.nastechDesktop.applyConnectionConfig({
        mode: 'remote',
        profile,
  ...
  const removeOverride = async () => {
    ...
    try {
      await window.nastechDesktop.applyConnectionConfig({ mode: 'local', profile })
