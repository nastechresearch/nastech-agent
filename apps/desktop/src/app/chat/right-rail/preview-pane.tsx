...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.nastechDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  useEffect(() => {
    ...
    // `nastech:openExternal` channel, never a popup side effect.
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.nastechDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.nastechDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.nastechDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.nastechDesktop?.openExternal(currentUrl)
                : undefined
            }
