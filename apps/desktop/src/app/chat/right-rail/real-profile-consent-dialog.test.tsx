...
vi.mock('@/nastech', () => ({
  saveNastechConfigRecord: (config: Record<string, unknown>, profile?: unknown) => mocks.save(config, profile)
}))

const promptCopy = {
  title: 'Stay signed in to your sites',
  body: 'Let Nastech browse with a snapshot of your default browser profile.',
...
vi.mock('../../hooks/use-config-record', () => ({
  nastechConfigCacheWriter: () => (config: Record<string, unknown>) => mocks.cache(config),
  useNastechConfigRecord: () => ({ data: mocks.loadedConfig })
}))

describe('RealProfileConsentDialog', () => {
  beforeEach(() => {
    mocks.loadedConfig = { browser: { allow_private_urls: false }, model: { provider: 'nastech' } }
