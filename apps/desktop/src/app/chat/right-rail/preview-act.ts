...
const preamble = () => `  var w = window;
  ...
  w.__nastechActHolder = w.__nastechActHolder || {};
  w.__nastechAct = ${actEngineSource()};
  w.__nastechWatch_fn = (${watchInPage.toString()});
  w.__nastechWatchTag = ${WATCH_TAG};
  var holder = w.__nastechActHolder;
  var act = function (a) { return w.__nastechAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__nastechWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__nastechHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
