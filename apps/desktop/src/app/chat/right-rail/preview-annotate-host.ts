...
export function overlayInstallScript(source: string): string {
  return '(function(){var api=' + source + ';window.__nastechAnnotate=api;api.install();})()'
}
...
export async function teardownAnnotateOverlay(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    (function () {
      if (window.__nastechAnnotate && window.__nastechAnnotate.teardown) {
        window.__nastechAnnotate.teardown();
      }
      window.__nastechAnnotate = null;
    })()
  `)
}
...
export async function waitAnnotateEvent(guest: PreviewAnnotateGuest): Promise<AnnotatePageEvent> {
  const event = await guest.executeJavaScript(`
    window.__nastechAnnotate ? window.__nastechAnnotate.wait() : Promise.resolve({ type: 'end' })
  `)
...
export async function syncAnnotatePins(guest: PreviewAnnotateGuest, pins: AnnotatePinChrome[]): Promise<void> {
  await guest.executeJavaScript(`
    window.__nastechAnnotate && window.__nastechAnnotate.showPins(${JSON.stringify(pins)})
  `)
}
...
export async function showAnnotateDraft(
  ...
  await guest.executeJavaScript(`
    window.__nastechAnnotate && window.__nastechAnnotate.showDraft(${JSON.stringify(rect)}, ${number})
  `)
}

export async function hideAnnotateDraft(guest: PreviewAnnotateGuest): Promise<void> {
  await guest.executeJavaScript(`
    window.__nastechAnnotate && window.__nastechAnnotate.hideDraft()
  `)
}
...
export async function captureAnnotateCrop(
  ...
  await tryGuest(guest, 'window.__nastechAnnotate ? window.__nastechAnnotate.beginCapture() : null')

  try {
    return await guest.capture(padRect(rect))
  } finally {
    await tryGuest(guest, 'window.__nastechAnnotate && window.__nastechAnnotate.endCapture()')
  }
}
