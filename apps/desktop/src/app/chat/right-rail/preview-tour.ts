...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__nastechTourEngine) {
    ${driverIife}
    w.__nastechTourHolder = {};
    w.__nastechTourCollect = (${collectTourTargets.toString()});
    w.__nastechTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__nastech-tour-style')) {
    var style = document.createElement('style');
    style.id = '__nastech-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__nastechTourEngine(
    w.driver.js.driver,
    w.__nastechTourHolder,
    ${JSON.stringify(action)},
    w.__nastechTourCollect,
    document
  ));
})()`
}
