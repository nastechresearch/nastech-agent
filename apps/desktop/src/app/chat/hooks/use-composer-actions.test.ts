...
function stubTransfer(
  ...
  return {
    getData: (mime: string) => (mime === NASTECH_PATHS_MIME ? internalRaw : mime === 'text/uri-list' ? uriList : ''),
    files: {
      length: files.length,
...
describe('useComposerActions native image drops', () => {
  ...
  it('does not attach a screenshot when its draft changes during native image saving', async () => {
    ...
    const add = vi.fn()
    Object.defineProperty(window, 'nastechDesktop', { configurable: true, value: { saveImageBuffer } })
  ...
  it('copies dropped screenshot bytes before trusting a transient macOS path', async () => {
    ...
    const durablePath = '/Users/test/Library/Application Support/Nastech/composer-images/composer_saved.png'
...
describe('useComposerActions generated paste title metadata', () => {
  ...
  it('marks only a Nastech-generated large paste with a bounded title preview', async () => {
    const savePastedText = vi.fn(async () => '/tmp/composer-pastes/pasted-content.txt')
    const add = vi.fn<(attachment: ComposerAttachment) => void>()
    Object.defineProperty(window, 'nastechDesktop', { configurable: true, value: { savePastedText } })
    const { result } = renderHook(() => useComposerActions({
      activeSessionId: null,
      currentCwd: '/test',
      requestGateway: vi.fn(),
...
describe('attachImagePath thumbnail separation', () => {
  ...
  it('does not resurrect an attachment removed while its queued resize is in flight', async () => {
    ...
    ;(
      window as unknown as {
        nastechDesktop: { readFileDataUrl: typeof readFileDataUrl }
      }
    ).nastechDesktop = { readFileDataUrl }
  ...
  it('merges a delayed thumbnail into the latest staged state of the same occurrence', async () => {
    ...
    updateComposerAttachment({
      ...
      label: 'photo.png',
      path: '/root/.nastech/attachments/photo.png',
