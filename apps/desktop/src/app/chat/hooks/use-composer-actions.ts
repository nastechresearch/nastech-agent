...
export function useComposerActions({
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob, isCurrent: () => boolean = () => true) => {
      ...
      try {
        ...
        const savedPath = await window.nastechDesktop?.saveImageBuffer(data, blobExtension(blob), name)
   ...
   * Nastech-managed composer-pastes file via the main process, then attached
  ...
  const attachPastedText = useCallback(
    async (text: string) => {
      const save = window.nastechDesktop?.savePastedText
