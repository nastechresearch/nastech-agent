...
// Nastech is rendering, and when the two are opposite schemes it takes over
...
function renderedMode(doc: Document): RenderedMode {
  const attr = doc.documentElement.dataset.nastechMode
 ...
 * the only carriers of Nastech' palette — are dropped so the paste target's
