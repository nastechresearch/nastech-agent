...
export const ERROR_CODE_KEYS = [
  ...
  'SESSION_NOT_OWNED',
  'disk_full',
  // The Nastech free tier refused or could not serve the turn (agent/error_surface.py
...
export interface ErrorSurface {
  ...
  /** Auth layer only: display name of the failing provider ("Nastech Portal"). */
...
/** True when the Nastech free tier refused or could not serve the turn: the way
...
export interface ErrorRecoveryPlan {
  ...
  editMessage: boolean
  /** Reveal the Nastech data folder so the user can free space (disk_full). */
  openNastechFolder: boolean
...
const CODE_PLANS: Partial<Record<ErrorCodeKey, Partial<ErrorRecoveryPlan>>> = {
  ...
  disk_full: { openNastechFolder: true, retry: true },
