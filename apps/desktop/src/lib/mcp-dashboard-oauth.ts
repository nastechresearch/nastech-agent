...
const UPDATE_BACKEND = 'Update the Nastech backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.nastechDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Nastech Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.nastechDesktop.openExternal(authUrl)
    let pollFailures = 0
