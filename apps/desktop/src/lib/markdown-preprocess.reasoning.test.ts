...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Nastech Desktop')).toBe('no Nastech Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Nastech')).toBe('no Nastech')
  })
