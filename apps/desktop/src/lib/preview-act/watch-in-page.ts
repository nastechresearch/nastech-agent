...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __nastechWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__nastechWatch || (win.__nastechWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('nastech-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('nastech-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __nastechWatchTag?: number }).__nastechWatchTag
