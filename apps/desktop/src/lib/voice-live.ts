...
/**
 * GPT-Live voice chat: the full-duplex voice frontend that DELEGATES to Nastech.
 *
 ...
 * that into an ordinary Nastech turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Nastech keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Nastech) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await nastechApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Nastech; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await nastechApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Nastech is running the tests…"). */
