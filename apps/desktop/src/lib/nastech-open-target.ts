 ...
 * `nastech://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `nastech://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `nastech://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type NastechOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }
...
export function normalizeNastechOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('nastech://') || trimmed.startsWith(`${NASTECH_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // nastech://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // nastech://index-network/intent/1 → /index-network/intent/1
...
export function resolveNastechOpenPath(target: NastechOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeNastechOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeNastechOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeNastechOpenString(target.path)
...
export function pathFromNastechDeepLink(
  ...
  if (kind === 'open') {
    return resolveNastechOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveNastechOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `nastech://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromNastechDeepLink('open', name, params)
}
