...
function dialProfile(
  desktop: NonNullable<typeof window.nastechDesktop>,
  profile: string,
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: NastechConnection) => void
  /**
...
interface GatewayRegistryState {
  ...
  primaryGateway: NastechGateway | null
   ...
   *  `nastech serve --profile <primary>` child and can never stand in for a
  ...
  $gateway: ReturnType<typeof atom<NastechGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('nastech.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<NastechGateway | null>(null),
...
export function setPrimaryGatewayConnection(connection: Pick<NastechConnection, 'connectionId' | 'mode'> | null): void {
...
async function isAttachedSharedRemote(
  ...
  // A local primary is one `nastech serve --profile <primary>` child; every other
  ...
  try {
    ...
    // (early return above): it is one `nastech serve --profile <primary>` child
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the nastech module (null
...
function publishActiveConnection(connection: NastechConnection): void {
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: NastechGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Nastech gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.nastechDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Nastech Desktop.')
  }
...
function publishTurnLease(scope: string, activeTurn: boolean): void {
  void window.nastechDesktop?.touchBackend?.(scope, { activeTurn }).catch(() => undefined)
}
...
export async function ensureActiveGatewayOpen({ explicit = false }: { explicit?: boolean } = {}): Promise<NastechGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Nastech gateway is not connected" on the Sessions `+` action (#88880).
