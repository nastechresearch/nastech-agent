...
// user explicitly clicks Upload → diagnostics.share_nastech runs backend-side
// (collect + force-redact + Nastech-S3 upload) → the modal shows the private
// view link plus the support handoff (GitHub Issues · Nastech Portal Support ·
// Discord).
//
// Consent is per-upload and explicit — no "always allow", mirroring the CLI's
// `nastech debug share --nastech` confirmation contract. On a remote connection
...
interface ShareNastechResponse {
  error?: string
  expires_at?: string
  ok: boolean
  upload_id?: string
...
async function collectLocalExtras(): Promise<Record<string, string>> {
  try {
    const logs = await window.nastechDesktop?.getRecentLogs?.()
...
export async function confirmSendDiagnostics(): Promise<void> {
  ...
  try {
    ...
    const response = await gateway.request<ShareNastechResponse>(
