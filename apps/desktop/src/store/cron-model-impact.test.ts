...
import type { ModelAssignmentResponse } from '@/types/nastech'
...
describe('setMainModelAssignment', () => {
  it('shows one consumer warning and routes via a read-only review action', async () => {
    ...
    await setMainModelAssignment({ provider: 'nastech', model: 'new/model' })
  ...
  it('keeps an existing warning for an older backend but clears it on explicit zero impact', async () => {
    ...
    await setMainModelAssignment({ provider: 'nastech', model: 'one' })
    expect($notifications.get()).toHaveLength(1)

    setModelAssignment.mockResolvedValueOnce(response(undefined))
    await setMainModelAssignment({ provider: 'nastech', model: 'two' })
    expect($notifications.get()).toHaveLength(1)
    ...
    await setMainModelAssignment({ provider: 'nastech', model: 'three' })
    expect($notifications.get()).toEqual([])
  })
  ...
  it('publishes only the latest same-profile assignment when responses reverse', async () => {
    ...
    const firstCall = setMainModelAssignment({ provider: 'nastech', model: 'first' })
    const secondCall = setMainModelAssignment({ provider: 'nastech', model: 'second' })
  ...
  it('invalidates pending responses and action closures on profile or connection changes', async () => {
    ...
    const call = setMainModelAssignment({ provider: 'nastech', model: 'pending' })
    ...
    await setMainModelAssignment({ provider: 'nastech', model: 'current' })
    const action = $notifications.get()[0].action
    const requestCount = $cronReviewRequest.get()
