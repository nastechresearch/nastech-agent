...
describe('composer model persistence scope', () => {
  ...
  it('keeps inferred local-primary connections on the historical bare keys', () => {
    ...
    window.localStorage.setItem('nastech.desktop.composer.model', 'legacy-model')
    window.localStorage.setItem('nastech.desktop.composer.provider', 'legacy-provider')
    ...
    expect(window.localStorage.getItem('nastech.desktop.composer.model')).toBe('next-model')
    expect(window.localStorage.getItem('nastech.desktop.composer.model.registry.local.default')).toBeNull()
  })
...
describe('session owner hints', () => {
  ...
  it('ignores malformed persisted entries and never throws on hydrate', () => {
    window.localStorage.setItem(
      'nastech.desktop.sessionOwnerHints.v1',
      JSON.stringify([
        'junk',
        ['no-route', null],
