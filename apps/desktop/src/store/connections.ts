...
const LAST_PROFILE_STORAGE_KEY = 'nastech.desktop.lastProfileByConnection'
...
async function rememberConnection(connectionId: string): Promise<void> {
  const setLastUsed = window.nastechDesktop?.connections?.setLastUsed
...
export async function selectConnection(connectionId: string, options: SelectConnectionOptions = {}): Promise<void> {
  ...
  // The primary local descriptor (startNastech) historically publishes without
