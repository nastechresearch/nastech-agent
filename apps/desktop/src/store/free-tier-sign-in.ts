...
import { cancelOAuthSession, listOAuthProviders, pollOAuthSession, startOAuthLogin } from '@/nastech'

import { type FreeTierRequester, NASTECH_PROVIDER_ID, refreshFreeTierStatus } from './free-tier'
...
// unreachable or refused mid sign-in (`nastech_cli/anon_sign_in.py`). Anything
...
/**
 * Drive one sign-in attempt end to end: resolve what identity this Nastech is
...
export async function beginFreeTierSignIn(requestGateway: FreeTierRequester) {
  ...
  // No free-tier identity AND a real Nastech account already connected: there is
  ...
  if (minting) {
    try {
      ...
      const nastech = providers.find(provider => provider.id === NASTECH_PROVIDER_ID)

      if (nastech?.status.logged_in && nastech.status.free_tier !== true) {
  ...
  try {
    const start = await startOAuthLogin(NASTECH_PROVIDER_ID)
...
async function pollOnce(sessionId: string, requestGateway: FreeTierRequester, mine: number) {
  ...
  try {
    const result = await pollOAuthSession(NASTECH_PROVIDER_ID, sessionId)
