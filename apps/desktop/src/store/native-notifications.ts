...
import { type NastechOpenTarget, resolveNastechOpenPath } from '@/lib/nastech-open-target'
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `nastech://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `nastech://index-network/intent/1`). */
  activate?: NastechOpenTarget
...
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveNastechOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveNastechOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
