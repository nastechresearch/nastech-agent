import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@nastech/shared'
...
function profileListSource(connection: NastechConnection | null): null | string {
  if (!connection) {
    return null
  }
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await nastechApi<ActiveProfileResponse>({
...
export function prewarmProfileBackend(name: string, connectionId: null | string = null): void {
  ...
  // tunnel and spawns `nastech -p <profile> serve --isolated` on the remote
...
async function resolveConnectionForProfile(profile: string): Promise<NastechConnection | null> {
  const getConnection = window.nastechDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<NastechConnection | null> {
