...
export function canOpenBrowserWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.nastechDesktop?.openBrowserWindow === 'function'
}
...
export function canOpenSessionInTerminal(): boolean {
  return typeof window !== 'undefined' && typeof window.nastechDesktop?.openSessionInTerminal === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await runWindowOpen(
    () => window.nastechDesktop.openSessionWindow(sessionId, { ...opts, profile }),
...
export async function openNewWindow(route?: { connectionId: null | string; profile: string }): Promise<void> {
  ...
  await runWindowOpen(() => window.nastechDesktop.openWindow(route), 'Could not open a new window')
}
...
export async function openBrowserInNewWindow(tabId: string): Promise<boolean> {
  ...
  return runWindowOpen(() => window.nastechDesktop.openBrowserWindow(tabId), 'Could not pop out browser')
}
...
export async function openSessionInTerminal(
  ...
  await runWindowOpen(
    () => window.nastechDesktop.openSessionInTerminal(sessionId, opts),
