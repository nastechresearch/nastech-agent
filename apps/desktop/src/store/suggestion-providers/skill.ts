 ...
 *  Working in `~/www/nastech-agent`, the draft says "nastech-agent" constantly
 *  — a "Use skill: nastech-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`nastech-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/nastech-agent-dev`, `plugin:skill`). */
