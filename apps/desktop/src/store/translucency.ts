 ...
 * `@nastech/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-nastech-glass]`
 * block in styles.css).
 */
...
} from '@nastech/shared/translucency'
 ...
 * sets `nastechDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.nastechDesktop?.glassSupported === 'boolean'
    ? window.nastechDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.nastechDesktop?.translucencySupported === 'boolean'
    ? window.nastechDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-nastech-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-nastech-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-nastech-glass-on', glassLive)
  root.toggleAttribute('data-nastech-glass', glassOn)
  root.toggleAttribute('data-nastech-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-nastech-glass-scope', scope)
  } else {
    root.removeAttribute('data-nastech-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.nastechDesktop?.setTranslucency?.(state)
  })
