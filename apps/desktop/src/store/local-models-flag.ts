 ...
 *
 * Local models ship on main behind `--local` (either `nastech desktop --local`
 * or the flag on Nastech.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.nastechDesktop?.localModelsEnabled === true
)
