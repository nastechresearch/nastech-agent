...
import type { OAuthProvider, OAuthStartResponse } from '@/types/nastech'
...
export interface DesktopOnboardingState {
   ...
   *  overlay show its "Nastech is ready" screen once even though the app is
...
async function completeWithModelConfirm(
  ...
  if (defaults) {
    ...
    try {
      ...
      onFail(error instanceof Error ? error.message : 'Nastech could not save the selected model.')
