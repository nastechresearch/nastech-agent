...
import type { ProjectInfo } from '@/types/nastech'
  ...
  it('names the stale-backend remedy when a remote gateway lacks the worktree route', async () => {
    ...
    desktopGit.mockReturnValue({
      worktreeAdd: vi.fn(async () => {
        throw new Error(
          'Expected JSON from https://vps/api/git/worktree/add but got HTML (status 404). The endpoint is likely missing on the Nastech backend.'
        )
      })
    } as never)
...
describe('createProject', () => {
  ...
  it.each(['default', 'coder'])('creates in the active %s profile without leaving All profiles', async profile => {
    const created = { folders: [], id: 'p_new', name: 'Nastech Agent', primary_path: '/srv/nastech' }
    ...
    vi.mocked(nastech.nastechApi).mockResolvedValue({ projects: [tree], active_id: created.id })
    $activeGatewayProfile.set(profile)
    setShowAllProfiles(true)

    await expect(createProject({ folders: ['/srv/nastech'], name: created.name, use: true })).resolves.toEqual(created)
    ...
    expect(nastech.nastechApi).toHaveBeenCalledWith(
  ...
  it('does not retarget a project create when the profile changes during reconnect', async () => {
    ...
    const pending = createProject({ folders: ['/srv/nastech'], name: 'Nastech Agent' })
    const rejection = expect(pending).rejects.toThrow('Active Nastech profile changed while connecting')
...
describe('repository discovery policy', () => {
  ...
  it('does not scan the local filesystem for remote connections but still refreshes the project tree', async () => {
    ...
    // `scan: true`) — repos with zero Nastech sessions must still surface —
