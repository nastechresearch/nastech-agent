...
import type { FreeTierStatus } from '@/types/nastech'

/** The model the free-tier route runs on. Used to recognise a session that is
 *  still homed on the free tier after a sign-in. */
export const FREE_TIER_MODEL = 'nastech/welcome'

/** The provider slug the free-tier route and a signed-in Nastech account share. */
export const NASTECH_PROVIDER_ID = 'nastech'
...
export interface FreeTierSetupFailure {
  /** One of the backend's `anon_*` codes (`nastech_cli/anon_auth.py`), or a newer one this build does not know. */
  code: string
  door: 'retry' | 'sign_in'
  message: string
  retryAfter: number
