 ...
 *
 * The archive is the CLI's own `nastech profile export` tar.gz (config, skills,
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Nastech profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.nastechDesktop?.selectSavePath
