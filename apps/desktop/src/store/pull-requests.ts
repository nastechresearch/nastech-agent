...
import { scanSessionPullRequests, type SessionInfo } from '@/nastech'
...
export const $pullRequestsByBranch = atom<Record<string, NastechBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'nastech.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('nastech.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: NastechBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
