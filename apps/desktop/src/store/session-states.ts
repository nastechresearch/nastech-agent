...
const BOT_CHAT_SCOPE_KEY = 'nastech.desktop.botChatSessions.v1'
 ...
 *  (nastech-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_nastech_home() re-creates
 * the profile directory the delete just removed (nastech-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (nastech-agent#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('nastech-bots:pane') is not a
