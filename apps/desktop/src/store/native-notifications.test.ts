...
describe('dispatchNativeNotification session context', () => {
  it('names the session on blocking-prompt titles only, falling back to the id tail without a row', () => {
    ...
    try {
      ...
      dispatchNativeNotification({ kind: 'turnDone', sessionId: 'named-chat', title: 'Nastech finished' })
      expect(notify).toHaveBeenCalledWith(expect.objectContaining({ title: 'Nastech finished' }))
    } finally {
      setSessions([])
    }
  })
