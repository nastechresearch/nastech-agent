 ...
 *
 * This store is the only writer of nastech-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'nastech-intro-reveal-seen-v1'
...
/** The film will never play for this identity (NASTECH_SKIP_INTRO). Record it
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.nastechDesktop?.skipIntro !== true
}

/** The guided flow is on but its intro film was turned off at launch
 * (NASTECH_SKIP_INTRO). Distinct from isIntroRevealEnabled(): false there also
...
export function startIntroReveal(): void {
  ...
  void window.nastechDesktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.nastechDesktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.nastechDesktop?.introReveal
