 ...
 *    desktop shell fires only while the user is AWAY from Nastech. This is the
...
import { host, type PluginOs, type PluginRestOptions, type PluginTranslate } from '@nastech/plugin-sdk'
...
function notifyOne(kind: string, spec: { titleKey: string; toast: ToastKind }, ev: CompletionEvent): void {
  ...
  // is away from Nastech (the toast above covers the foreground case). Isolated:
