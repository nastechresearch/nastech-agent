...
Bundled Nastech Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@nastech/plugin-sdk`, `react`, and
...
`$NASTECH_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Nastech theme
...
Stream availability, regional restrictions, and metadata vary. Nastech is not
