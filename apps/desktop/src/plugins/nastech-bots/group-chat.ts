...
import { atom, host } from '@nastech/plugin-sdk'
...
/** The v3 envelope stored under the default profile's `nastech-bots-groups`
 ...
 *  failing `default`s are never one anonymous "Nastech" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Nastech' : profile)
  ...
  // title, then the local row's display_name, then default → Nastech.
  ...
  return isDefault ? 'Nastech' : trimmed
}
