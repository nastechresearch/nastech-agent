 ...
 *  click path's tile-staleness probe (nastech-agent#90102), which must
...
export function notifyBotOpenFailure(error: unknown, bot: RosterRow, step: BotOpenStep, botName?: string) {
  ...
  if (botModeGatewayNeedsUpdate(error)) {
    const connectionLabel = bot.connectionLabel || bot.connectionId || 'Nastech'
    host.notify?.({
      kind: 'error',
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Nastech Desktop to chat with bots on other connections.'
    )
  }
