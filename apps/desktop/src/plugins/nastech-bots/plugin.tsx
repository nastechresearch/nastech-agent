...
import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@nastech/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@nastech/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // A bot's chat before it has spoken: core's splash is Nastech' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['nastech-bots']?.title || bot.title || ''
            ).trim()

            // message_agent only resolves canonical identities: the relay
            // matches a roster row's handle/profile (± @connection-id), the
            // local path a bare profile name or 'nastech'. botHandle() prefers
