...
// The @mention middleware appends a handoff note whose nastech command the
...
/** Run the note's first nastech command under a stub that echoes each argv
 *  element — proves the shell received the interpolations as LITERALS. */
function runHandoffCommand(noteText) {
  const command = noteText.match(/`nastech -p [^`]*`/)[0].slice(1, -1)
  const script = `nastech() { printf '%s\\037' "$@"; }\n${command}`
...
test('security: a poisoned bot title stays literal in the handoff command', async () => {
  const quoteSentinel = `/tmp/nastech-bot-mode-mention-quote-${process.pid}`
  const subSentinel = `/tmp/nastech-bot-mode-mention-sub-${process.pid}`
...
test('regression: the handoff command quotes the recipient argument', async () => {
  ...
  assert.match(result.text, /`nastech -p 'ops' chat --in ~/)
})
...
test('behavior: a renamed default profile routes from another focused Bot Chat', async () => {
  ...
  assert.match(result.text, /`nastech -p 'default' chat --in ~/)
  assert.match(result.text, /Message from 🤖 Renametest \(@renametest\)/)
})

test('behavior: @dixie on a Connections bot stays in this chat and does not nastech -p', async () => {
  const values = new Map()
  const atom = initial => {
    const slot = { get: () => values.get(slot), set: value => values.set(slot, value) }
    values.set(slot, initial)
  ...
  assert.doesNotMatch(result.text, /nastech -p 'dixie'/)
