...
function renderBotRow(input = 'alpha') {
  ...
  const context = {
    ...
    Codicon: 'Codicon',
    Tip: 'Tip',
    ROSTER_KEY: ['nastech-bots', 'roster'],
    $botMeta: atom({}),
    $botUnread: atom({}),
    $botAttention: atom({}),
    BOT_ATTENTION_HINTS: {},
    ...
    host: {
      ...
      requestProfile: async (_route, method) =>
        method === 'profiles.list'
          ? { profiles: [{ name, ui_meta: { 'nastech-bots': { chat: 'owner-chat' } } }] }
          : { sessions: [] },
    ...
    requestForBot: async (_bot, method) => method === 'profiles.list'
      ? {
          profiles: [{
            ...
            ui_meta: { 'nastech-bots': { pinned: true } }
          }]
        }
      : {},
...
test('behavior: remote default never opens the same-name local chat', async () => {
  ...
  const context = {
    ...
    host: {
      ...
      request: async () => ({ profiles: [{ name: 'default', ui_meta: { 'nastech-bots': { chat: 'this-device-chat' } } }] }),
      notify: () => undefined,
