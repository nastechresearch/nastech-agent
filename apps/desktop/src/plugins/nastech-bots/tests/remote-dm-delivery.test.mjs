  ...
  const ctx = runtime({
    requestProfile: async (route, method, params) => {
      ...
      if (method === 'profiles.list') {
        return { profiles: [{ name: 'dixie', ui_meta: { 'nastech-bots': { chat: 'stored-42' } } }] }
      }
...
test('remote DM carries sender attribution and relays the reply', async () => {
  ...
  await ctx.__dm.deliverRemoteRosterMentions(
    ...
    { name: 'Nastech', handle: 'nastech' }
  )

  assert.equal(submits.length, 1)
  assert.match(submits[0], /^Message from 🤖 Nastech \(@nastech\): what is the disk space\?$/u)
  assert.ok(
