...
function sdkNamedImports(source) {
  const match = source.match(/import\s+\{([\s\S]*?)\}\s+from '@nastech\/plugin-sdk'/)
...
function writeLegacySdk(root) {
  const packageRoot = join(root, 'node_modules', '@nastech', 'plugin-sdk')
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ name: '@nastech/plugin-sdk', type: 'module', exports: './index.js' }, null, 2)}\n`
  )
...
test('legacy SDK without optional capability exports still links Bot Mode', async t => {
  const root = mkdtempSync(join(tmpdir(), 'nastech-bot-mode-legacy-sdk-'))
  ...
  assert.equal(loaded.default.id, 'nastech-bots')
