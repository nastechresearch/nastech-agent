...
// Bot Mode title or `nastech profile rename` display_name — must update what
...
test('mentionNameForms: slugged + collapsed, reserved tokens dropped', () => {
  ...
  // A bot renamed "Nastech" or "everyone" cannot hijack reserved tags.
  assert.equal(mentionNameForms('Nastech').length, 0)
...
test('botMentionTag: renamed slug wins, profile handle is the fallback', () => {
  ...
  assert.equal(botMentionTag({ name: 'default' }), 'nastech')
...
test('parseGroupChatMentions: display_name and ui_meta title forms address a member', () => {
  ...
  const members = [
    ...
    { name: 'ops', title: '', ui_meta: { 'nastech-bots': { title: 'Night Watch' } } }
  ]
