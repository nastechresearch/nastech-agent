...
function runtime() {
  ...
  const code = source
    .replace(/^import \* as sdk from '@nastech\/plugin-sdk'\r?\n/m, '')
...
test('default rows use source identity without borrowing another source title', () => {
  ...
  // label. Titled: the title wins. Untitled: it stays "Nastech". Regression:
  ...
  assert.equal(name(active, undefined), 'Nastech')
})

test('botHandle: precomputed multi-source handle wins; default stays nastech', () => {
  ...
  assert.equal(botHandle('default'), 'nastech')
})
...
test('displayName: local default stays Nastech; remote default uses the device label', () => {
...
test('resolveRosterMentions: @nastech in this chat is not a handoff to yourself', () => {
  const { __resolveRosterMentions: resolve } = runtime()
  const roster = [
    { name: 'default', connectionId: 'local', handle: 'nastech' },
    {
      name: 'default',
      connectionId: 'mac-mini',
      connectionLabel: 'Mac Mini',
  ...
  const hits = resolve('ask @nastech and @default-mac-mini', roster, {
    name: 'default',
    connectionId: 'local'
  })
