...
function load({ cacheKeyConnection = 'local' } = {}) {
  ...
  queryClient.setQueryData(['nastech-bots', 'roster', cacheKeyConnection], ROSTER)
    ...
    host: {
      ...
      requestProfile: async (route, method, params) => {
        ...
        if (method === 'profiles.list') {
          return {
            profiles: [{ name: 'default', ui_meta: { 'nastech-bots': { chat: 'remote-pin' } } }]
          }
        }
...
test('the roster cache entry lives under the connection-suffixed key, invisible to the bare-key read', () => {
  ...
  assert.equal(queryClient.getQueryData(['nastech-bots', 'roster']), undefined)
  assert.equal(queryClient.getQueriesData({ queryKey: ['nastech-bots', 'roster'] }).length, 1)
})
...
test('the middleware resolves a remote @name-device mention and routes delivery over Connections', async () => {
  ...
  // "do not run nastech -p" instruction must mention the phrase.
  assert.doesNotMatch(result.text, /nastech -p '?default/)
