 ...
 *
 * Mechanism (traced on 6ce7ab8-era main, apps/desktop/src/plugins/nastech-bots/plugin.js):
 *
...
test('#94478: detects an unanswered member mention in the post-user tail', () => {
  // Room state from the issue: user asks, member radar replies citing @nastech.
  const log = [
    { id: 1, from: { kind: 'user' }, text: 'kick this off' },
    { id: 2, from: { kind: 'member', name: 'radar' }, text: 'done — @nastech please review' }
  ]

  const mentioned = mentionedSinceLastUser(log, text => (text.includes('@nastech') ? ['nastech'] : []))
  ...
  // continuation round can be driven for exactly [nastech].
  assert.ok(mentioned.has('nastech'), 'cited member must be detected as pending')
})
...
test('#94478: does not flag a handoff the cited member already answered', () => {
  const log = [
    { id: 1, from: { kind: 'user' }, text: 'kick this off' },
    { id: 2, from: { kind: 'member', name: 'radar' }, text: 'done — @nastech please review' },
    { id: 3, from: { kind: 'member', name: 'nastech' }, text: 'reviewed, all good' }
  ]

  // nastech has a member entry AFTER the mentioning one — handoff resolved.
  // The fix's detector must not re-drive an answered mention (that would loop).
  const mentionIdx = log.findIndex(e => e.id === 2)
  const answer = [...log].reverse().find(e => e.from.kind === 'member' && e.from.name === 'nastech')
