 ...
 * job for a different profile keeps the `nastech -p <bot> chat` delegation
...
/** Run the delegation command under a `nastech` stub that prints its argv, so
...
function argvOf(prompt: string): string[] {
  ...
  const result = spawnSync('sh', ['-c', `nastech() { printf '%s\\037' "$@"; }\n${command}`], { encoding: 'utf8' })
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/nastech -p 'research' chat/)
