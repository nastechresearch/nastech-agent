...
function messagingProtocolSection(name, roster) {
  ...
  return [
    ...
    '',
    '```',
    'nastech -p <agent-name> chat --in ~ -c "Bot Chat" -Q -q "Message from \uD83E\uDD16 ' + handle + ' (@' + handle + '): your message"',
    '',
    ...
    '`nastech -p <agent-name> sessions rename <session-id> "Bot Chat"`.)',
    '',
  ...
  const open = async () => {
    ...
    if (bot.remoteSource) {
      if (typeof host.ensureAgent !== 'function') {
        host.notifyError?.(
          new Error('Update Nastech Desktop to chat with agents on other connections.'),
...
function BotsPane() {
  ...
  const activityOf = bot => {
    const created = allMeta[bot.name]?.created || bot.ui_meta?.['nastech-bots']?.created || 0
      ...
      data: {
        handler: async draft => {
          ...
          const note =
            ...
            mentioned.map(n => '`nastech -p ' + shellQuote(n) + ' chat --in ~ -c "Bot Chat" -Q -q "Message from \uD83E\uDD16 ' + shellDoubleQuote(senderName) + ' (@' + shellDoubleQuote(botHandle(active)) + '): <your composed message>"`').join('\n') +
            '\nAfter dispatching, tell the user the message was sent and END YOUR TURN — do not wait or poll; when the background process completes, its notification carries the reply — relay it then, attributed to that agent. If it fails with "No session found matching \'Bot Chat\'", send once without the -c flag, then run `nastech -p <agent> sessions rename <session_id from the output> "Bot Chat"`. ' +
