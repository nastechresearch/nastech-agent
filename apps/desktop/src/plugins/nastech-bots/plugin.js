...
async function saveBotMeta(name, patch) {
  ...
  try {
    ...
    serverRequest = Promise.resolve(host.request('profiles.configure', { name, ui_meta: { 'nastech-bots': rest } }))
  } catch {
 ...
 *  click opens (nastech-agent#88200). Unknown params are ignored by older
...
function botFriendlyNames(bot) {
  ...
  return [bot?.ui_meta?.['nastech-bots']?.title, localTitle, bot?.title, bot?.display_name]
}
...
async function ensureRemoteCanonicalChat(route, profile) {
  ...
  try {
    ...
    pinned = owner?.ui_meta?.['nastech-bots']?.chat || null
  } catch {
 ...
 *
 *  Identity rules (nastech-agent#88200 — the row must open the session its
 *  preview describes):
...
async function prepareBotSource(bot, pinnedChat) {
  ...
  try {
    ...
    return owner?.ui_meta?.['nastech-bots']?.chat || null
  } catch {
 ...
 *  literally named "default" — render it as Nastech (matching displayName and
 *  the @nastech handle) so the main agent never loses its name in rooms. */
        ...
        children: [
          jsxs('div', {
            ...
            children: [
              ...
              activeNow
                ? jsx('span', {
                    className: 'nastech-bots-pulse size-1.5 shrink-0 rounded-full bg-(--ui-accent,#4f9cf9)',
        ...
        handler: async draft => {
          ...
          if (localMentions.length) {
            note +=
              ...
              localMentions.map(bot => '`nastech -p ' + shellQuote(bot.name) + ' chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from 🤖 ' + shellDoubleQuote(senderName) + ' (@' + shellDoubleQuote(botHandle(live.name)) + '): <your composed message>"`').join('\n') +
          ...
          if (remoteMentions.length) {
            ...
            note +=
              ...
              ' over Connections in the background. Do not run nastech -p for them and do not switch Gateway. Tell the user they were messaged here; when a reply lands, relay it attributed to that agent.]'
          }
