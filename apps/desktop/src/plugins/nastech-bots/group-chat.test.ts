...
vi.mock('@nastech/plugin-sdk', async () => {
...
function published(room: Room) {
  ...
  return (configure?.params.ui_meta as Record<string, Record<string, unknown>>)?.['nastech-bots-groups']
}
...
describe('speaker labels', () => {
  it('relabels Nastech control-frame openers only in member-authored transcript lines', async () => {
  ...
  it('the default profile speaks as Nastech in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Nastech: hello room')
  ...
  it('honor friendly identity: Bot Mode title, then display_name, never a stale Nastech', async () => {
    ...
    expect(chat.groupSpeakerLabel('default')).toBe('Nastech')
  ...
  it('qualifies twins per room and never renders a raw key when the roster row is missing', async () => {
    ...
    expect(chat.groupSpeakerLabel('local::reviewer')).toBe('reviewer')
    expect(chat.groupSpeakerLabel('spark::default')).toBe('Nastech')
...
describe('sync worker', () => {
  it('retries a gateway CAS conflict and publishes the merged room', async () => {
    ...
    await drain(() => (room.gateway.uiMetaRevisions['nastech-bots-groups'] || 0) < 2, 60)

    const stored = room.gateway.uiMeta['nastech-bots-groups'] as {
      rooms: Record<string, { log: GroupMessage[] }>
    }

    expect(room.gateway.uiMetaRevisions['nastech-bots-groups']).toBe(2)
