 ...
 * `{type:'nastech-skill-pick'}` back and the plugin installs via skills.manage.
 *
...
import type * as NastechSdk from '@nastech/plugin-sdk'
...
vi.mock('@nastech/plugin-sdk', async importOriginal => {
  const original = await importOriginal<typeof NastechSdk>()
...
describe('hub pick messages', () => {
  it('installs the picked skill when it comes from our own frame', () => {
    ...
    postPick(
      { identifier: 'nastech/web-research', name: 'Web Research', type: 'nastech-skill-pick' },
      {
        source: frame.contentWindow
      }
    )

    expect(installCalls()).toEqual([['skills.manage', { action: 'install', query: 'nastech/web-research' }]])
  })
  ...
  it('regression: refuses identifiers outside the slug charset', () => {
    ...
    for (const identifier of ['../../etc/passwd', 'skill; rm -rf /', '-flag', 'name with spaces', '']) {
      postPick({ identifier, name: 'Web Research', type: 'nastech-skill-pick' }, { source: frame.contentWindow })
    }
  ...
  it('ignores messages that are not a skill pick', () => {
    ...
    postPick({ identifier: 'nastech/web-research', type: 'oauth-callback' }, { source: frame.contentWindow })
    postPick({ identifier: 'nastech/web-research', type: 'nastech-skill-pick' }, { source: frame.contentWindow })
