...
describe('routing', () => {
  ...
  it('resolves @nastech to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@nastech take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @nastech', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @nastech,/)
    ...
    expect(peer).toMatch(/group chat with @nastech/)
  })

  // #89720: a renamed primary is @bobby to the roster, autocomplete and the
  // mention resolver; introducing it to itself as @nastech made it treat
