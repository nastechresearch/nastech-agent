import type { McpCatalogResponse, McpServerSummary } from '@/types/nastech'
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return nastechApi<{ ok: boolean; status: string }>({
...
export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return nastechApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return nastechApi<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
