...
import type { NastechApiRequest } from '@/global'
  ...
  constructor() {
    super({
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Nastech Desktop has no server-request registry for ${request.method} (${request.id})`),
...
export function nastechApi<T>(request: NastechApiRequest): Promise<T> {
  return window.nastechDesktop.api<T>({ ...connectionScoped(), ...request })
}
