...
describe('deleteSession profile scoping', () => {
  it('scopes the DELETE to the owning profile in the URL (object owner)', async () => {
    ...
    nastechApi.mockResolvedValue({ ok: true } as never)
    // Mirrors the real capabilityScoped for an object owner (remote-stamped row).
    vi.mocked(client.capabilityScoped).mockReturnValue({ profile: 'tommy', connectionId: 'nastech-pi' })

    await deleteSession('sess-1', { connectionId: 'nastech-pi', profile: 'tommy' })

    expect(nastechApi.mock.calls[0][0]).toMatchObject({
      method: 'DELETE',
      path: '/api/sessions/sess-1?profile=tommy',
      connectionId: 'nastech-pi',
      profile: 'tommy'
    })
  })
  ...
  it('omits the profile query when no owner is known', async () => {
    ...
    expect((nastechApi.mock.calls[0][0] as { path: string }).path).not.toContain('profile=')
  })
...
describe('getSession dial priority', () => {
  it('does not dial an explicitly scoped session read foreground', async () => {
    ...
    nastechApi.mockResolvedValue({ id: 'sess-5' } as never)
    ...
    expect(nastechApi.mock.calls[0][0]).toMatchObject({ profile: 'tommy', connectionId: 'local' })
    expect(nastechApi.mock.calls[0][0]).not.toHaveProperty('priority')
  })
})
...
describe('setSessionArchived profile scoping', () => {
  ...
  it('omits the profile from the body when none is given', async () => {
    ...
    const req = nastechApi.mock.calls[0][0] as { body: Record<string, unknown> }
