import type { ConnectionRequestPayload } from '@nastech/shared'
...
export interface OAuthProviderStatus {
  /** Nastech only: the tier name the token resolves to, when the backend knows
   *  one. Null for a free-tier identity and for older backends. */
  account_tier?: null | string
  error?: string
  expires_at?: null | string
  /** Nastech only: true when the stored token belongs to a free-tier identity
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Nastech doesn't know how to remove it. */
...
export interface FreeTierStatus {
  ...
  enabled: boolean
  has_guest: boolean
  /** Display name for the route, e.g. "Nastech · free tier". */
  label: string
  model: string
   ...
   *  (`nastech_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in nastech_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `nastech model` uses — instead of
...
// The Nastech pairing service mints a bot on the user's behalf: the desktop shows
...
export interface NastechConfig {
  agent?: {
...
export type NastechConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['nastech-bots'].title) — the name the
...
// non-desktop) Nastech simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (nastech.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors nastech_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Nastech, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Nastech but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `nastech tools` active-provider detection). */
  is_active: boolean
  /** Honest readiness computed server-side (keys ∧ Nastech entitlement ∧
 ...
 *  not Nastech); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** nastech-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** nastech-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/nastech/update/check` — the backend's own update state.
...
/** One Nastech-approved MCP catalog entry from `GET /api/mcp/catalog`. */
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Nastech Tool Gateway as a result of
   *  switching the main provider to Nastech. Empty unless provider === 'nastech'
