...
import type { LocalRuntimeJob } from '@/types/nastech'
...
describe('ModelPickerDialog download rows', () => {
  ...
  it('shows a first-ever download under its own Local group when no local provider exists yet', async () => {
    ...
    expect(await screen.findByText('Nastech-4.5')).toBeTruthy()
...
describe('ModelPickerDialog search ranking', () => {
  ...
  it('orders model rows exactly as the shared fuzzyRank does', async () => {
    vi.mocked(requestModelOptions).mockResolvedValue({
      providers: [{ slug: 'nastech', name: 'Nastech', models: MODELS, authenticated: true }]
    })
    renderPicker({ currentModel: 'gpt-4o', currentProvider: 'nastech' })
  ...
  it.each([
    ...
    vi.mocked(requestModelOptions).mockResolvedValue({
      providers: [{ slug: 'nastech', name: 'Nastech', models: catalog, authenticated: true }]
    })
    renderPicker({ currentModel: 'gpt-4o', currentProvider: 'nastech' })
    await screen.findByText('gpt-4o')
