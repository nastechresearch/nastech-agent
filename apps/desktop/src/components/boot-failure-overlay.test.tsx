...
describe('BootFailureOverlay', () => {
  ...
  it('recovers a cloud connection through the portal cascade instead of native OAuth', async () => {
    ...
    const restore = stubDesktop(
      ...
      },
      {
        ...
        oauthLogoutConnectionConfig: logout,
        probeConnectionConfig: vi.fn().mockResolvedValue({ providers: [{ id: 'nastech', type: 'oauth' }] })
      }
    )
  ...
  it('shows the Nastech Cloud down recovery when the backend flags isCloudBackendDown', async () => {
    ...
    try {
      ...
      // remote-failure copy.
      expect(await screen.findByText(/Nastech Cloud agent is down/i)).toBeTruthy()
      ...
      expect(screen.getByText(/ares-3009\.agents\.nastechresearch\.github\.io/i)).toBeTruthy()
    } finally {
      restore()
    }
  })
