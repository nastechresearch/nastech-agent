 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.nastech.send(prompt)` (or declarative
 * `data-nastech-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'nastech-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'nastech-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `nastech.send(prompt)`.
 ...
 *  wires `data-nastech-send` so declarative HTML works with zero script:
 *  `<button data-nastech-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.nastech={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-nastech-send]"):null;' +
    'if(el)send(el.getAttribute("data-nastech-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `nastech serve` — renders live.)
