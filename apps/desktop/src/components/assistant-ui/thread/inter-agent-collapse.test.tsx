...
const DELIVERY = 'Message from 🤖 Nastech (@nastech): please check the build'
...
describe('inter-agent collapse gate', () => {
  it('keeps the report after a relayed answer to this bot\u2019s own dispatch expanded', async () => {
    // Bot Mode round trip seen from the DISPATCHING bot: it messaged @nastech,
    // the answer came back as an inbound "Message from" row, and the next
    // assistant message is its report to the human — not a reply to nastech.
    render(
      <Harness
        messages={[
          user('u1', 'ask nastech for the list'),
          dispatch('a0', '@Nastech'),
          user('u2', DELIVERY),
          assistant('a1', 'here is the list nastech sent', false)
        ]}
      />
    )

    await screen.findByText('here is the list nastech sent')
    ...
    render(
      <Harness
        messages={[
          ...
          user('u4', 'Message from 🤖 Nastech (@nastech): unsolicited: build broke'),
