...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's NASTECH_HOME/logs. On a
  ...
  // An expired/revoked OAuth grant (HTTP 401 on nastech / openai-codex / ...):
  ...
  // NASTECH_HOME/logs, and its parent is the Nastech data folder itself (what
  ...
  const openLocalDir = useCallback(async (resolve: (logsRoot: string) => string, failedMessage: string) => {
    try {
      const root = await window.nastechDesktop?.logsRoot?.()
      ...
      const result = await window.nastechDesktop?.openDir?.(resolve(root))
  ...
  const openNastechFolder = useCallback(
    () => openLocalDir(root => root.replace(/[\\/]+logs[\\/]*$/, ''), copy.errorOpenNastechFolderFailed),
    [copy.errorOpenNastechFolderFailed, openLocalDir]
  )
  ...
  const localFolders = Boolean(window.nastechDesktop?.logsRoot)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {plan.openNastechFolder && localFolders && (
        <button className="aui-error-action" onClick={() => void openNastechFolder()} type="button">
          {copy.errorOpenNastechFolder}
        </button>
      )}
      {plan.retry && (
