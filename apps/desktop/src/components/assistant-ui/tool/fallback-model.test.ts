...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Nastech Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Nastech docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Nastech Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Nastech docs', url: 'https://example.com/docs' }
    ])
  })
})
