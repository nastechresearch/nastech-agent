...
## The desktop is its own chat surface on a `nastech serve` backend
...
(`@nastech/shared`: `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard also consumes.
The desktop has **no build/runtime dependency on the dashboard frontend**: it spawns a headless
`nastech serve` (`headless_backend=True` → `cmd_dashboard` skips `_build_web_ui` and exports
`NASTECH_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists).
`dashboard` and `serve` share `cmd_dashboard`/`start_server` but neither launches the other. It does
NOT embed `nastech --tui` — own composer, transcript, slash pipeline.
...
and ONLY when it does not (older managed install / PATH `nastech` not yet updated) rewrites argv to
...
The backend the app spawns is a **pooled `nastech serve --port 0` per (connection, profile)**: its
launch home is that profile, `NASTECH_DESKTOP=1` is set, and its in-process cron ticker stands down
...
- `src/lib/desktop-slash-commands.ts` is the load-bearing file: `DESKTOP_COMMAND_SPECS` (built-ins
  ...
  `CommandDef` in `nastech_cli/commands.py`; the live `commands.catalog` carries it, and
  `src/lib/desktop-slash-registry.json` (regenerate with `scripts/dump_desktop_slash_registry.py`;
  `tests/nastech_cli/test_desktop_slash_registry.py` + the vitest file fail on drift) is the offline
...
## Bot Mode (`src/plugins/nastech-bots/`) — one bot = ONE canonical forever-chat, identified by NAME

Each bot is a Nastech **profile** with a persistent identity. This invariant regressed repeatedly,
...
**There is NO session-id pin.** The old design stored a pointer in `ui_meta['nastech-bots'].chat`;
...
in `src/plugins/nastech-bots/`: `canonical-chat-registry.test.ts` (tripwire: the open path never
...
"guest", "anonymous", "claim" or "Nastech Portal" in user-facing text.
