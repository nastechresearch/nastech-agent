...
describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend nastech_home', async () => {
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.nastech/plugins')
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.nastech/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.nastech/desktop-plugins/my-feature' }]
        }
      }

      if (dir === '/local/.nastech/desktop-plugins/my-feature') {
        return {
          entries: [
            { isDirectory: false, name: 'README.md', path: '/local/.nastech/desktop-plugins/my-feature/README.md' }
          ]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.nastech/desktop-plugins/my-feature')
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.nastech/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.nastech-package.json', path: `${root}/uni/.nastech-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.nastech-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend nastech_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.nastech/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  it('disposes runtime host event subscriptions before a hot reload (#112366)', async () => {
    ...
    try {
      const source = `
        import { host } from '@nastech/plugin-sdk'
        export default {
