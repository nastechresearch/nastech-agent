...
describe('scanDiskPlugins (#66899)', () => {
  ...
  it('treats a package without a Desktop half as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.nastech/plugins') {
        return { entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.nastech/plugins/my-feature' }] }
      }

      if (dir === '/local/.nastech/plugins/my-feature') {
        return {
          entries: [{ isDirectory: false, name: 'plugin.yaml', path: '/local/.nastech/plugins/my-feature/plugin.yaml' }]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.nastech/plugins/my-feature')
    expect(readDir).not.toHaveBeenCalledWith('/local/.nastech/plugins/my-feature/desktop')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    agentPluginsRoot.mockResolvedValue('')
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.nastech/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.nastech/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.nastech/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.nastech/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('loads a unified desktop half OPT-IN: inventoried but not activated by default', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.nastech/plugins') {
        return { entries: [{ isDirectory: true, name: 'uni', path: '/local/.nastech/plugins/uni' }] }
      }

      if (dir === '/local/.nastech/plugins/uni') {
        return {
          entries: [{ isDirectory: true, name: 'desktop', path: '/local/.nastech/plugins/uni/desktop' }]
        }
      }

      if (dir === '/local/.nastech/plugins/uni/desktop') {
        return {
            ...
            ? [
                {
                  isDirectory: false,
                  name: 'plugin.js',
                  path: '/local/.nastech/plugins/uni/desktop/plugin.js'
                }
              ]
            : []
        }
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.nastech/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.nastechDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.nastech/desktop-plugins')
    agentPluginsRoot.mockResolvedValue('')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.nastech/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.nastech/desktop-plugins/huge/plugin.js'
      })
