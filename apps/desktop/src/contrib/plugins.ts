 ...
 *    a `NastechPlugin` registers automatically (vite glob — drop a folder in).
 *    `nastech-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `nastech-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<nastech home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<nastech home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: NastechPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default NastechPlugin export — skipped`)
