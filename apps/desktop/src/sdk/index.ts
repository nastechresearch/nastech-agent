/**
 * @nastech/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__NASTECH_PLUGIN_SDK__` and maps the import to it, so a
...
import type { PaginatedSessions, UsageStats } from '@/types/nastech'
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.nastechDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and NastechGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (nastech-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.nastechDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Nastech Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.nastechDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Nastech Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Nastech Desktop to open another Bot chat.' })
  ...
  listPersistedSessions: async (
    ...
    return nastechApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return nastechApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
 ...
 *  "NASTECH AGENT" with it, a `chat.empty` contribution writes its own name. */
...
/** Nastech' reasoning levels, so a plugin surfacing a thinking depth uses the
