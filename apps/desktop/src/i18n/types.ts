...
export interface Translations {
  ...
  settings: {
    ...
    toolsets: {
      ...
      nastechAuthFailedMessage: string
      nastechAuthTryAgain: string
  ...
  freeTier: {
    /** Settings › Providers row title while the Nastech identity is the free tier. */
    ...
    // One sentence per backend code (`nastech_cli/anon_auth.py::ANON_*`); the copy never says
    // the free MODEL is off — what is unavailable is using Nastech without signing in.
    setupFailed: {
      gateClosed: string
      paused: string
      ...
      locked: string
      generic: string
      /** The sign-in door, when the account service is reachable: the Nastech row sits right below. */
      signInBelow: string
      tryAgain: string
      retrying: string
    }
  ...
  assistant: {
    thread: {
      ...
      errorOpenNastechFolder: string
      errorOpenNastechFolderFailed: string
