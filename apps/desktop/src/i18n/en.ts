  ...
  settings: {
    ...
    plugins: {
      ...
      agent: {
        title: 'Agent plugins',
        blurb:
          'Plugins you installed into the Nastech backend — tools, skills, MCP servers, hooks, and slash commands. Portable ones are Agent Plugins packages (skills + MCP bundles that work in other agents too). Toggles apply to new sessions.',
      ...
      installModal: {
        ...
        agentTargetLocal: profile => `Installs into the ${profile} backend (~/.nastech/plugins/)`,
    ...
    appearance: {
      ...
      tipsDesc:
        'A small bubble pointing at one part of the app, shown occasionally while idle and by Nastech when it helps. Closing one retires it for good.',
      tipsReset: (count: number) => `Bring back ${count} closed ${count === 1 ? 'tip' : 'tips'}`,
      toursTitle: 'Guided Tours',
      toursDesc: 'Let Nastech walk you through the app, dimming the screen and spotlighting each step.',
    ...
    localModels: {
      ...
      upToDateDetail: (tag, backend) => `Running llama.cpp ${tag} (${backend}) — the latest build Nastech ships.`,
