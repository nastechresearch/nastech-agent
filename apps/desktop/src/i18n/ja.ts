...
export const ja = defineLocale({
  catalog: {
    ...
    more: 'さらに表示',
    pinned: 'レビュー済みコミット',
    snapshotHint: 'Nastechカタログの情報です。閲覧時に提供元のリポジトリへ接続することはありません。',
  ...
  boot: {
    ready: 'Nastech Desktop の準備ができました',
    ...
    errors: {
      backgroundExited: 'Nastech バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Nastech バックグラウンドプロセスが終了しました。',
    ...
    failure: {
      ...
      cloudDownTitle: 'Nastech Cloud エージェントが停止しています',
      cloudDownDescription:
        'このゲートウェイが接続している Nastech 管理のクラウドエージェントがサーバーエラーを返しています。ここから再起動することはできません。ステータスを確認するか、ローカルゲートウェイに切り替えるか、サポートに連絡してください。',
      cloudDownHint:
        '下のボタンから Nastech Portal（インスタンスの状態と操作）を開くか、Discord でサポートを受けられます。',
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Nastech Desktop を再起動してください。',
    ...
    native: {
      ...
      inputBody: 'Nastech が応答を待っています。',
      turnDoneTitle: 'Nastech が完了しました',
      turnDoneBody: '',
  ...
  billingBlock: {
    titleNastech: 'Nastech クレジットが不足しています',
  ...
  sendDiagnostics: {
    title: 'Nastech に診断情報を送信',
    privacyNotice:
      'デバッグバンドルを Nastech 内部ストレージにアップロードします（公開ペーストではありません）。システム情報（OS、バージョン、プロバイダー、設定済み API キーの種類 — キー自体は含まれません）と、エージェント/ゲートウェイ/デスクトップの完全なログ（各最大 512 KB。会話内容、ツール出力、ファイルパスを含む可能性が高い）が含まれます。シークレットはアップロード前にマスクされます。閲覧できるのは Nastech スタッフと許可された Discord モデレーターのみで、14 日後に自動削除されます。',
    upload: 'アップロード',
    uploading: 'アップロード中…',
    cancel: 'キャンセル',
    close: '閉じる',
    ...
    failedHint:
      'ターミナルから `nastech debug share --nastech` を実行するか、`nastech debug share --local` でアップロードせずにレポートを表示することもできます。',
    handoffLead: '続きは次の場所で:',
    links: {
      github: 'GitHub Issues',
      portal: 'Nastech Portal サポート',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Nastech のデフォルトに戻しますか？',
    ...
    vault: {
      ...
      otpHint: '2FA を有効にするときにサイトが表示する「セットアップキー」。保存すると Nastech がコードを生成します。',
      ...
      sources: {
        ...
        notInstalled: name =>
          `未検出です。${name} のコマンドラインツールをインストールしてサインインすると、Nastech が自動的に検出します。`,
        disabledDesc: '検出済みですが、Nastech では無効になっています。',
        lockedDesc:
          '検出済み。エージェントがログイン情報を必要とするときにロック解除を求めます。今すぐ解除することもできます。',
        unlockedDesc: 'このセッションでロック解除済み。30分間操作がないか Nastech を閉じると自動的にロックされます。',
        statusLocked: 'ロック中',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Nastech がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Nastech が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Nastech がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
        ...
        plugin: {
          label: 'プラグイン通知',
          description: 'Nastech がバックグラウンドの間に、デスクトッププラグインが通知を送信しました。'
        }
      },
      test: 'テスト通知を送信',
      testTitle: 'Nastech',
    ...
    appearance: {
      ...
      tipsTitle: 'アプリ内ヒント',
      tipsDesc:
        'アプリや Nastech からのヒントをときどき表示します。各ヒントは一度だけ表示されます。利用開始から30日後に自動でオフになりますが、再びオンにできます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Nastech の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Nastech を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Nastech がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Nastech が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Nastech が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Nastech がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Nastech 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
    ...
    screenshot: {
      ...
      enabledDesc:
        'どのアプリからでも左右の Command キーを同時に押すと、最前面のウインドウを撮影し、Nastech の現在の下書きに添付します。自動送信はしません。初期設定はオフで、この Mac にのみ適用されます。機密情報が写る可能性があるため、送信前に添付画像を確認してください。',
      ...
      inputPermission:
        '入力監視の許可により、他のアプリがアクティブな間も両方の Command キーを検出できます。システム設定 → プライバシーとセキュリティ → 入力監視で Nastech を許可し、ここに戻って再試行してください。',
      screenPermission:
        '画面収録の許可により、このショートカットを使ったときに最前面のアプリのウインドウを撮影できます。システム設定 → プライバシーとセキュリティ → 画面収録で Nastech を許可し、ここに戻って再試行してください。macOS に求められた場合は Nastech を再起動してください。',
    ...
    quickEntry: {
      enabledTitle: 'クイック入力',
      enabledDesc:
        'グローバルショートカットで小さな入力欄をどこからでも呼び出し、Nastech を開かずにプロンプトを送信します。',
    ...
    gateway: {
      ...
      intro:
        'Nastech Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Nastech バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      envOverrideTitle: '環境変数がこのデスクトップセッションを制御しています。',
      envOverrideDesc:
        '保存された設定を使用するには NASTECH_DESKTOP_REMOTE_URL と NASTECH_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc:
        'ローカルホストでプライベートな Nastech バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの Nastech バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      remoteUrlTitle: 'リモート URL',
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/nastech などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'Nastech Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Nastech ${version}` : ''} に接続しました`,
      ...
      sshTitle: 'SSH で接続',
      sshDesc:
        'Nastech は SSH 経由でリモート上に起動され、このアプリにトンネルされます。リモート側で何かを起動・公開する必要はありません。ホストへの鍵ベースの SSH アクセスが前提です。',
      ...
      sshNastechPathTitle: 'Nastech パス（任意）',
      sshNastechPathDesc: 'リモートの nastech バイナリへのフルパス。空欄 = 自動検出。',
      sshNastechPathPlaceholder: '自動検出',
      sshTestConnection: 'SSH をテスト',
      sshConnect: '接続',
      sshButtonsHint: '「保存」は次回起動時に適用され、「接続」は今すぐ再接続します。',
      sshReachable: (host, platform) => `接続可能: ${host}（${platform}）— Nastech を検出`,
      sshIncompleteHost: '接続する前に SSH ホストを入力してください。',
      sshErrUnreachable: 'SSH でそのホストに到達できませんでした。ホスト、ポート、ネットワークを確認してください。',
      sshErrAuth:
        'SSH 認証に失敗しました。鍵を ssh-agent に読み込む（ssh-add）か、~/.ssh/config に IdentityFile を設定してください。Nastech は非対話的に ssh を実行します。',
      sshErrHostKey:
      ...
      sshErrUpdateRequired: 'Desktop SSH で接続する前に、リモートホストの Nastech を更新してください。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'リンクがこの MCP サーバーを Nastech に追加するよう要求しました。下の設定はリンク側から来たものです。内容を必ず確認してください。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) =>
        `ビルド ${tag}、${backend} バックエンド。サーバーは Nastech が起動・管理します。`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Nastech がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
    ...
    toolsets: {
      ...
      nastechAuthSignIn: 'サインイン',
      nastechAuthDoneTitle: 'Nastech Portal に接続しました',
      nastechAuthDoneMessage: 'サブスクリプションのバックエンドが有効になりました。',
      nastechAuthFailed: 'Nastech Portal のサインインが完了しませんでした',
      ...
      browserRealProfile: {
        ...
        prompt: {
          ...
          body: 'Nastech が既定ブラウザプロファイルのスナップショットでブラウジングできるようにすると、サイトはログイン済みの状態で開きます。',
  ...
  skills: {
    ...
    skillArchivedMessage: 'nastech curator restore で復元できます。',
  ...
  starmap: {
    ...
    emptyDesc: 'Nastech がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Nastech を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Nastech を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Nastech が通知します。',
    ...
    nastechActiveSessions: (version, count) => `Nastech ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: 'どちらの方法でも、あなたが管理するボットを接続し、資格情報はこの Nastech にのみ保存されます。',
      quickSetup: 'クイックセットアップ',
      recommended: '推奨',
      quickHelp:
        'QR コードをスキャンして Telegram で確認します。Nastech がボットを作成し、あなたの Telegram ユーザー ID を自動検出します。',
      createWithQr: 'QR で作成',
      starting: '開始中…',
      replaceWarning:
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: 'このプロファイルのセッションは、このパソコンではなく指定したリモートの Nastech で実行されます。',
      urlLabel: 'リモートアドレス',
    ...
    defaultDescription: 'Nastech の起動時と新しいチャットに使用します。既存のセッションのプロファイルは変わりません。',
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Nastech 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Nastech が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      deleteConfirm:
        'Nastech から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
      ...
      worktreeStaleBackend:
        'このリモート接続でワークツリーを作成するには Nastech バックエンドを更新してください — git ワークツリー API 以前のバージョンです。',
  ...
  composer: {
    ...
    placeholderStarting: 'Nastech を起動中...',
    placeholderReconnecting: 'Nastech に再接続中…',
    ...
    attachUrlDesc: 'Nastech がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Nastech にコミットと PR を任せる',
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Nastech はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Nastech をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Nastech バックエンドの新しいバージョンをインストールできます。',
    ...
    blockerTitle: 'Nastech を更新するためにローカルプレビューを閉じますか？',
    blockerBody:
      '更新する前に、これらのローカルプレビューを停止する必要があります。ファイルが変更または削除されることはありません。',
    foreignBlockerTitle: '他のプロセスを閉じて Nastech を更新',
    foreignBlockerBody:
      'Nastech はこれらのプロセスを安全に自動終了できません。各プロセスを所有するアプリ、ターミナル、またはサービスを閉じてから、もう一度更新してください。',
    mixedBlockerBody:
      'Nastech は以下のローカルプレビューを閉じることができます。更新を続けるには、他のプロセスを手動で閉じる必要があります。',
  ...
  guidedGreeting: {
    line: 'やあ、どうぞ。Nastech です。二分だけください、あなたに合わせて整えます。それから、本当にやりたいことに取りかかりましょう。\n\nまずは、何とお呼びすればいいですか。',
  ...
  install: {
    ...
    oneTimeTitle: 'Nastech には一度限りのインストールが必要です',
    ...
    setupChoiceTitle: 'Nastech Desktop をセットアップ',
    setupChoiceDesc:
      'すでに実行している Nastech ゲートウェイに接続するか、このコンピューターに Nastech をローカルインストールします。',
    connectExistingTitle: '既存の Nastech に接続',
    connectExistingShort: '既存環境に接続',
    connectExistingDesc:
      'セッショントークンまたはブラウザーサインインでリモートバックエンドを使用します。ローカルインストールは開始されません。',
    installLocalTitle: 'Nastech をローカルにインストール',
    installLocalDesc: 'Nastech をダウンロードし、Python 環境を作成して、このコンピューターでバックエンドを実行します。',
    localStartUnavailable:
      'ローカルインストールを開始できません。Nastech Desktop を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の Nastech に接続',
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。Nastech Desktop がトークンとブラウザーサインインのどちらが必要かを検出します。',
    remoteUrlTitle: 'ゲートウェイ URL',
    remoteUrlDesc: 'Nastech ゲートウェイのベース URL を使用します。リモートの場合は https:// を含めてください。',
    remoteUrlPlaceholder: 'https://gateway.example.com/nastech',
    probing: 'ゲートウェイ認証方式を検出中...',
    probeError: 'その Nastech ゲートウェイに到達できませんでした。',
    identityProvider: 'ID プロバイダー',
    authTitle: '認証',
    ...
    settingUpTitle: 'Nastech Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Nastech CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Nastech インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Nastech インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Nastech Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Nastech はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Nastech を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Nastech を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Nastech を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Nastech が自動接続します',
    ...
    authorizeThere: 'そこで Nastech を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Nastech をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Pro モデルには有料の Nastech サブスクリプションが必要です。',
    free: '無料',
    freeTier: '無料プラン',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Nastech は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Nastech を再起動中...',
      askRestart: 'Nastech にサーバーの再起動を依頼',
      lookingRestart: taskId => `Nastech は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Nastech はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      blankPageBody: '上のアドレス欄に入力するか、Nastech にページを開くよう頼んでください。',
      finishedRestarting: message =>
        `Nastech がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Nastech がサーバーを再起動できませんでした。',
      stillWorking:
        'Nastech はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Nastech が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'Nastech ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.nastech/config.yaml) に追加されます。Nastech はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    vaultSaveDesc: origin =>
      `Nastech は ${origin} のサインインページに到達しましたが、保存されたログイン情報がありません。ここで一度入力すると、このマシン上で暗号化して保存され、ページに直接入力されます。モデルはパスワードを一切見ません。`,
    ...
    vaultCodeDesc: site =>
      `${site} がワンタイムコード（SMS、メール、または認証アプリ）を求めています。ここに入力すると Nastech がページに入力します。モデルはコードを一切見ません。`,
    vaultCodeLabel: 'コード',
    vaultCodeFootnote:
      'ヒント：「設定 → パスワードとログイン」でこのログインに認証キーを保存すると、Nastech がコードを自動入力します。',
    vaultCodeSkip: 'スキップ',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Nastech Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Nastech Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`nastech gateway` は起動していますか？'
    }
  },
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '一度教えれば覚えます',
        text: 'スキルは手順書のフォルダで、必要な場面で Nastech が自分で読み込みます。'
      },
      messaging: {
        title: 'デスクを離れても Nastech',
        text: 'Telegram、Discord、Slack などに接続。同じエージェント、同じ記憶のままです。'
      },
      artifacts: {
        title: 'Nastech が作ったものすべて',
      ...
      profiles: {
        ...
        text: 'それぞれが独自のキー・メモリ・セッションを持つ、別の Nastech です。'
      },
      'composer-mentions': {
        title: 'ファイルとコマンド',
