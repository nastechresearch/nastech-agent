...
export const ru = defineLocale({
  catalog: {
    ...
    snapshotHint: 'Из каталога Nastech. При просмотре нет обращений к исходным репозиториям.',
  ...
  sessionImport: {
    ...
    subtitle: 'Перенесите разговор в Nastech и продолжите с того места, где остановились.',
    ...
    chooseHelp: 'Выберите сессию, чтобы прочитать историю перед импортом в Nastech.',
    ...
    you: 'Вы',
    snapshot: 'Этот разговор уже есть в Nastech. Откройте существующую копию, чтобы продолжить.',
    copyNotice:
      'Копируется текст разговора. Исходные файлы не меняются. Вывод инструментов и рассуждения не переносятся.',
    importing: 'Импорт…',
    open: 'Открыть в Nastech',
    continue: 'Продолжить в Nastech',
  ...
  boot: {
    ready: 'Nastech Desktop готов',
    desktopBootFailedWithMessage: message => `Не удалось запустить приложение: ${message}`,
    steps: {
      connectingGateway: 'Подключение к шлюзу',
      loadingSettings: 'Загрузка настроек Nastech',
      loadingSessions: 'Загрузка последних сеансов',
      retryingRemoteBackend: 'Переподключение к удалённому бэкенду Nastech…',
      startingDesktopConnection: 'Запуск подключения приложения',
      startingNastechDesktop: 'Запуск Nastech Desktop…'
    },
    errors: {
      backgroundExited: 'Фоновый процесс Nastech завершён.',
      backgroundExitedDuringStartup: 'Фоновый процесс Nastech завершился при запуске.',
    ...
    failure: {
      title: 'Nastech не удалось запустить',
      description:
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Ваш бэкенд Nastech старше этой сборки приложения и может работать некорректно. Обновите их, чтобы они совпали.',
    ...
    errors: {
      ...
      methodNotAllowed:
        'Бэкенд приложения отклонил запрос (405 Method Not Allowed). Попробуйте перезапустить Nastech Desktop.',
  ...
  settings: {
    ...
    resetConfirm: 'Сбросить все настройки к значениям Nastech по умолчанию?',
    ...
    notifications: {
      ...
      focusedHint: 'Оповещения о завершении срабатывают только пока Nastech в фоне.',
      kinds: {
        approval: {
        ...
        input: {
          label: 'Нужен ввод',
          description: 'Nastech задал вопрос или требует пароль/секрет.'
        },
        turnDone: {
          label: 'Ответ готов',
          description: 'Ход завершился, пока Nastech был в фоне.'
        },
        turnError: {
          label: 'Ход не удался',
        ...
        plugin: {
          ...
          description: 'Плагин приложения отправил уведомление, пока Nastech был в фоне.'
        }
      },
    ...
    appearance: {
      ...
      colorModeDesc: 'Выберите фиксированный режим или позвольте Nastech следовать настройкам системы.',
      ...
      reactionsDesc: 'Эмодзи-тапбеки в стиле iMessage — реагируйте на сообщения, и Nastech сможет реагировать на ваши.',
      ...
      pet: {
        title: 'Питомец',
        intro:
          'Заберите анимированного питомца из petdex, который парит над приложением и реагирует на действия Nastech — «бегает», пока выполняются инструменты, радуется успеху и хмурится при ошибках.',
        restartHint:
          'Питомцам нужен быстрый перезапуск — текущее приложение запустилось до появления этой функции. Выйдите из Nastech и откройте снова, затем вернитесь сюда.',
        on: 'Вкл',
        off: 'Выкл',
        scaleTitle: 'Размер',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      agent: {
        ...
        maxTurns: 'Верхний предел ходов с вызовами инструментов, после которого Nastech останавливает запуск.'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Максимальное число символов, которые Nastech может прочитать из одного запроса к файлу.',
      approvals: {
        mode: 'Как Nastech обрабатывает команды, требующие явного подтверждения.',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'Когда Nastech обновляет себя из приложения (без запроса в терминале), сохранять локальные правки исходников (stash) или выбрасывать (discard). Обновления из терминала всегда спрашивают.'
      }
    }),
    about: {
      ...
      bundleOutOfSyncDesc:
        'Рантайм Nastech обновлён, но само приложение — ещё старая сборка: новые функции интерфейса (например, Bot Mode) не появятся до обновления. Запустите обновление ниже, чтобы пересобрать приложение. Если предупреждение не исчезнет, переустановите с последнего установщика.',
      ...
      automaticUpdatesDesc: 'Nastech автоматически проверяет обновления в фоне и сообщает, когда они готовы.',
    ...
    config: {
      ...
      loading: 'Загрузка конфигурации Nastech…',
    ...
    screenshot: {
      ...
      enabledDesc:
        'Нажмите обе клавиши Command одновременно в любом приложении, чтобы снять его переднее окно и прикрепить снимок к текущему черновику Nastech. Автоматической отправки нет. По умолчанию выключено; действует только на этом Mac. Окно может содержать конфиденциальные данные — проверьте вложение перед отправкой.',
      ...
      inputPermission:
        'Разрешение на мониторинг ввода позволяет Nastech распознавать обе клавиши Command, когда активно другое приложение. Разрешите Nastech доступ в Системных настройках → Конфиденциальность и безопасность → Мониторинг ввода, затем вернитесь сюда и повторите попытку.',
      screenPermission:
        'Разрешение на запись экрана позволяет Nastech снимать переднее окно приложения при использовании этого сочетания. Разрешите Nastech доступ в Системных настройках → Конфиденциальность и безопасность → Запись экрана, затем вернитесь сюда и повторите попытку. Перезапустите Nastech, если macOS попросит.',
    ...
    quickEntry: {
      ...
      enabledDesc:
        'Глобальным горячим ключом вызывайте маленький композер откуда угодно и отправляйте запрос, не открывая Nastech.',
    ...
    connections: {
      ...
      intro:
        'Управляйте этим устройством и всеми шлюзами Nastech, до которых можно дотянуться через удалённые, SSH или Cloud-соединения.',
      stagedNote:
      ...
      updateSkippedCloud: 'Управляется Nastech Cloud',
      kindLocal: 'Локальный',
      kindRemote: 'Удалённый шлюз',
      kindCloud: 'Nastech Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: 'Рантайм Nastech, которым управляет это приложение.',
      kindRemoteDesc: 'Шлюз Nastech, доступный по HTTP(S) — LAN, Tailscale или интернет.',
      kindCloudDesc: 'Хостинговая инстанция, обнаруженная через ваш аккаунт Nastech Cloud.',
      kindSshDesc: 'Установка Nastech, доступная по SSH.',
      labelTitle: 'Имя',
      labelDesc:
      ...
      headersDesc:
        'Отправляются с каждым HTTP- и WebSocket-запросом к этому шлюзу — например, для прокси доступа вроде Cloudflare Access (CF-Access-Client-Id / CF-Access-Client-Secret). Значения хранятся зашифрованными. Заголовки, которыми управляет Nastech (Authorization, Cookie, Host…), игнорируются.',
      ...
      cloudAddHint:
        'Подсказка: вход в Nastech Cloud выше автоматически обнаруживает ваших агентов — эту форму используйте только для ручной регистрации известного URL инстанса.',
    ...
    gateway: {
      ...
      intro:
        'По умолчанию — локальный. Используйте удалённый, когда приложение должно управлять бэкендом Nastech в другом месте. Соединения шлюзов — на уровне машины; профили обнаруживаются из подключённых шлюзов.',
      envOverrideTitle: 'Переменные окружения управляют этой сессией приложения.',
      envOverrideDesc:
        'Сбросьте NASTECH_DESKTOP_REMOTE_URL и NASTECH_DESKTOP_REMOTE_TOKEN, чтобы использовать сохранённую настройку ниже.',
      modeTitle: 'Режим подключения',
      localTitle: 'Локальный шлюз',
      localDesc: 'Запускает приватный бэкенд Nastech на localhost. Это значение по умолчанию, работает офлайн.',
      remoteTitle: 'Удалённый шлюз',
      remoteDesc: 'Подключает это приложение к удалённому бэкенду Nastech.',
      remoteAuthHint:
      ...
      cloudDesc: 'Войдите в Nastech Cloud один раз и выбирайте агентов из своего аккаунта — без вставки URL.',
      cloudSignInTitle: 'Nastech Cloud',
      cloudSignIn: 'Войти в Nastech Cloud',
      cloudSignedIn: 'Вы вошли в Nastech Cloud',
      cloudNeedsSignIn: 'Войдите в Nastech Cloud, чтобы обнаружить агентов в вашем аккаунте.',
      ...
      cloudDiscoverFailed: 'Не удалось загрузить агентов Nastech Cloud',
      cloudConnectFailed: 'Не удалось подключиться к этому агенту',
      cloudSignInFailed: 'Не удалось войти в Nastech Cloud',
      cloudSignedOutTitle: 'Выход из Nastech Cloud',
      cloudSignedOutMessage: 'Сессия Nastech Cloud сброшена.',
      ...
      remoteUrlDesc: 'Базовый URL удалённого бэкенда дашборда. Поддерживаются префиксы пути, например /nastech.',
      ...
      restartingMessage: 'Nastech Desktop переподключится с сохранёнными настройками — оболочка останется открытой.',
      savedMessage: 'Сохранено для следующего перезапуска.',
      connectedTo: (baseUrl, version) => `Подключено к ${baseUrl}${version ? ` · Nastech ${version}` : ''}`,
      ...
      sshDesc:
        'Nastech запускается на удалённой машине по SSH и туннелируется в это приложение — ничего не нужно запускать или открывать самим. Требуется рабочая SSH-аутентификация по ключу на хост.',
      sshTrustHint:
      ...
      sshNastechPathTitle: 'Путь к Nastech (необязательно)',
      sshNastechPathDesc: 'Полный путь к бинарнику nastech на удалённой машине. Пусто = автоопределение.',
      sshNastechPathPlaceholder: 'автоопределение',
      ...
      sshReachable: (host, platform) => `Доступен: ${host} (${platform}) — Nastech найден`,
      ...
      sshErrAuth:
        'SSH-аутентификация не удалась. Загрузите ключ в ssh-agent (ssh-add) или задайте IdentityFile в ~/.ssh/config — Nastech запускает ssh в неинтерактивном режиме.',
      sshErrHostKey:
      ...
      sshErrUpdateRequired: 'Перед подключением через SSH обновите Nastech на удалённой машине.',
    ...
    mcp: {
      ...
      deepLinkDescription:
        'Ссылка запросила добавить этот MCP-сервер в Nastech. Проверьте конфигурацию ниже — она пришла из ссылки, а не из Nastech.',
    ...
    providers: {
      ...
      intro:
        'Войдите по подписке — копировать API-ключ не нужно. Nastech проведёт вход в браузере прямо здесь, в приложении.',
      ...
      localEndpoint: {
        ...
        description: 'Направьте Nastech на любой OpenAI-совместимый эндпоинт (Zyphra, vLLM, llama.cpp, Ollama и т. д.).'
      },
    ...
    toolsets: {
      ...
      nastechAuthFailed: 'Вход в Nastech Portal не завершён',
  ...
  skills: {
    ...
    skillArchivedMessage: 'Восстановить через nastech curator restore.',
    hub: {
  ...
  starmap: {
    ...
    emptyDesc: 'По мере того как Nastech создаёт навыки и память для вашей работы, они появятся здесь.',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'Перезапустите Nastech, чтобы использовать питомцев — бэкенд старше этой функции.',
    ...
    generatePet: {
      ...
      staleBackend: 'Обновите Nastech, чтобы генерировать питомцев.',
      backgroundHint: 'Можно закрыть — Nastech уведомит, когда будет готово.',
    ...
    nav: {
      ...
      settings: { title: 'Настройки', detail: 'Настройка Nastech desktop' },
    ...
    nastechActiveSessions: (version, count) => `Nastech ${version} · Активные сеансы: ${count}`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle:
        'Оба способа подключают бота под вашим контролем и сохраняют его данные только в этой установке Nastech.',
      quickSetup: 'Быстрая настройка',
      recommended: 'Рекомендуется',
      quickHelp:
        'Отсканируйте QR-код и подтвердите в Telegram. Nastech создаст бота и определит ваш Telegram ID автоматически.',
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ID пользователя бота', placeholder: '@nastech:example.org' },
  ...
  profiles: {
    ...
    defaultDescription: 'Используется при запуске Nastech и для новых чатов. Профили существующих сессий не меняются.',
    ...
    deleting: 'Удаление...',
    createDesc: 'Профили — это независимые среды Nastech: отдельные настройки, навыки и SOUL.md.',
    nameLabel: 'Имя',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      saveFailed: 'Nastech не сохранил это изменение модели.'
    },
    ...
    newCron: 'Новый cron',
    emptyDescNew:
      'Запланируйте промпт, который будет выполняться по cron-выражению. Nastech выполнит его и доставит результаты в выбранное вами место.',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'Обновите бэкенд Nastech, чтобы создавать проекты — ваш бэкенд старше этого desktop-приложения (Настройки → Обновления → Бэкенд).',
      deleteConfirm: 'Это удалит сохранённый проект из Nastech. Файлы, git-репозитории и worktrees не пострадают.',
      ...
      worktreeStaleBackend:
        'Обновите бэкенд Nastech, чтобы создавать worktrees по этому удалённому соединению — он старше git worktree API.',
  ...
  composer: {
    ...
    placeholderStarting: 'Запуск Nastech...',
    placeholderReconnecting: 'Переподключение к Nastech…',
    placeholderFollowUp: 'Отправить продолжение',
    newSessionPlaceholders: [
      'Что будем делать?',
      'Дайте Nastech задачу',
      'О чём вы думаете?',
      'Опишите, что нужно',
      'Что обсудим?',
    ...
    attachUrlDesc: 'Nastech загрузит страницу и добавит её как контекст для этого хода.',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Попросить Nastech открыть PR',
  ...
  updates: {
    stages: {
      ...
      pull: 'Почти готово…',
      pydeps: 'Завершаем…',
      update: 'Обновляем Nastech…',
      rebuild: 'Пересобираем desktop-приложение…',
      restart: 'Перезапускаем Nastech…',
    ...
    availableBody: 'Новая версия Nastech готова к установке.',
    availableTitleBackend: 'Доступно обновление бэкенда',
    availableBodyBackend: 'Новая версия подключённого бэкенда Nastech готова к установке.',
    ...
    manualBody:
      'Nastech установлен из командной строки, поэтому обновления тоже выполняются там. Вставьте это в терминал:',
    manualPickedUp: 'Nastech подхватит новую версию при следующем запуске.',
    guiSkewTitle: 'Обновите desktop-приложение',
    guiSkewBody:
      'Бэкенд обновлён, но пакет этого desktop-приложения не изменился. Обновите или переустановите desktop-приложение Nastech (ваш AppImage / .deb / .rpm), чтобы версии совпали.',
    copy: 'Копировать',
    copied: 'Скопировано',
    done: 'Готово',
    applyingBody:
      'Обновлятор Nastech возьмёт процесс на себя в своём окне и автоматически перезапустит Nastech, когда закончит. Пожалуйста, не открывайте Nastech вручную во время обновления.',
    applyingBodyBackend:
      'Удалённый бэкенд применяет обновление и перезапустится. Nastech переподключится автоматически, когда он снова заработает.',
    applyingClose: 'Это окно закроется во время обновления, затем Nastech откроется сам.',
    errorTitle: 'Обновление не завершилось',
    errorBody: 'Не переживайте — ничего не потеряно. Можно попробовать снова прямо сейчас.',
    blockerTitle: 'Закрыть локальные предпросмотры, чтобы обновить Nastech?',
    blockerBody:
      'Nastech нужно остановить эти локальные предпросмотры перед обновлением. Ваши файлы не будут изменены или удалены.',
    foreignBlockerTitle: 'Закрыть другие процессы, чтобы обновить Nastech',
    foreignBlockerBody:
      'Nastech не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или службу, которой принадлежит каждый из них, и повторите обновление.',
    mixedBlockerBody:
      'Nastech может закрыть локальные предпросмотры, перечисленные ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
  ...
  guidedGreeting: {
    line: 'Заходите. Я Nastech. Дайте мне пару минут — обустрою тут всё под вас, а потом займёмся тем, что вам правда нужно.\n\nДля начала: как к вам обращаться?',
  ...
  install: {
    ...
    oneTimeTitle: 'Nastech требует одноразовой установки',
    ...
    setupChoiceTitle: 'Настройка Nastech Desktop',
    setupChoiceDesc:
      'Подключите это приложение к уже работающему шлюзу Nastech или установите Nastech локально на этот компьютер.',
    connectExistingTitle: 'Подключиться к существующему Nastech',
    ...
    installLocalTitle: 'Установить Nastech локально',
    installLocalDesc: 'Скачайте Nastech, создайте его Python-окружение и запустите бэкенд на этом компьютере.',
    localStartUnavailable: 'Не удалось начать локальную установку. Перезапустите Nastech Desktop и попробуйте снова.',
    remoteSetupTitle: 'Подключиться к существующему Nastech',
    remoteSetupDesc: 'Введите URL вашего шлюза. Nastech Desktop определит, нужен токен или вход через браузер.',
    remoteUrlTitle: 'URL шлюза',
    remoteUrlDesc: 'Используйте базовый URL шлюза Nastech, включая https:// для удалённых.',
    remoteUrlPlaceholder: 'https://gateway.example.com/nastech',
    probing: 'Определяем аутентификацию шлюза...',
    probeError: 'Не удалось подключиться к этому шлюзу Nastech.',
    ...
    settingUpTitle: 'Настройка Nastech Agent',
    finishingTitle: 'Завершаем',
    failedDesc:
      'Один из шагов установки завершился ошибкой. На Windows это может произойти, если запущена другая инстанция Nastech CLI или desktop. Остановите все работающие инстанции Nastech и повторите. Подробности — ниже или в журнале desktop.',
    activeDesc:
      'Это одноразовая настройка. Установщик Nastech скачивает зависимости и настраивает вашу машину. При следующих запусках этот шаг будет пропущен.',
  ...
  onboarding: {
    headerTitle: 'Настроим для вас Nastech Agent',
    headerDesc: 'Подключите провайдера модели, чтобы начать общение. Большинство вариантов — в один клик.',
    preparingInstall: 'Nastech завершает установку. Обычно это занимает меньше минуты при первом запуске.',
    starting: 'Запускаем Nastech…',
    ...
    recommended: 'Рекомендуется',
    connected: 'Подключено',
    featuredPitch: 'Одна подписка, 300+ передовых моделей — рекомендуемый способ запускать Nastech',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'self-hosted',
        description:
          'Укажите Nastech локальный или self-hosted OpenAI-совместимый endpoint (vLLM, llama.cpp, Ollama и т.д.).'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'Откроет страницу подтверждения в браузере — Nastech подключится автоматически',
    ...
    authorizeThere: 'Авторизуйте Nastech там.',
    ...
    autoBrowser: provider =>
      `Мы открыли ${provider} в вашем браузере. Авторизуйте Nastech там, и подключение произойдёт автоматически — ничего копировать и вставлять не нужно.`,
  ...
  modelPicker: {
    ...
    pro: 'Pro',
    proNeedsSubscription: 'Модели Pro требуют платной подписки Nastech.',
    free: 'Free',
    freeTier: 'Бесплатный тариф',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} — ${size}. Nastech покажет только первые 512 КБ.`,
    ...
    web: {
      ...
      tryAgain: 'Попробовать снова',
      restarting: 'Nastech перезапускается...',
      askRestart: 'Попросить Nastech перезапустить сервер',
      lookingRestart: taskId => `Nastech ищет сервер предпросмотра для перезапуска (${taskId})`,
      restartingTitle: 'Перезапуск сервера предпросмотра',
      restartingMessage: 'Nastech работает в фоне. Следите за прогрессом в консоли предпросмотра.',
      ...
      restartFailedMessage: 'Nastech не смог перезапустить сервер.',
      stillWorking:
        'Nastech всё ещё работает, но результата перезапуска пока нет. Команда сервера может выполняться в foreground.',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Nastech загружает ответ',
    ...
    approval: {
      gatewayDisconnected: 'Шлюз Nastech не подключён',
      ...
      alwaysDescription: pattern =>
        `Это добавит паттерн «${pattern}» в ваш постоянный список разрешений (~/.nastech/config.yaml). Nastech больше не будет спрашивать о подобных командах — ни в этом сеансе, ни в будущем.`,
  ...
  prompts: {
    ...
    secretDesc: 'Nastech нужны учётные данные, чтобы продолжить.',
  ...
  desktop: {
    ...
    restartToUseSaveImage: 'Перезапустите Nastech Desktop, чтобы использовать «Сохранить изображение».',
    restartToSaveImages: 'Перезапустите Nastech Desktop, чтобы сохранять изображения',
    ...
    handoff: {
      ...
      timedOut: 'Превышено время ожидания шлюза. Выполняется ли `nastech gateway`?'
    }
  },
  errors: {
