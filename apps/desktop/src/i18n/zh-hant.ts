...
export const zhHant = defineLocale({
  catalog: {
    ...
    more: '顯示更多',
    pinned: '已審核的提交',
    snapshotHint: '內容來自 Nastech 目錄。瀏覽時不會連線至來源程式碼儲存庫。',
  ...
  sessionImport: {
    title: '從其他應用程式繼續',
    subtitle: '將對話匯入 Nastech，接著上次的進度繼續。',
    action: '匯入工作階段',
    readingFrom: '讀取自',
  ...
  settings: {
    ...
    vault: {
      ...
      otpHint: '啟用兩步驟驗證時網站顯示的「設定金鑰」。儲存後 Nastech 會自動產生驗證碼。',
      ...
      sources: {
        ...
        notInstalled: name => `未偵測到。安裝 ${name} 命令列工具並登入後，Nastech 會自動偵測。`,
        disabledDesc: '已偵測到，但已為 Nastech 關閉。',
        lockedDesc: '已偵測到。代理需要登入資訊時會請你解鎖，也可立即解鎖。',
        unlockedDesc: '本工作階段已解鎖。閒置 30 分鐘或關閉 Nastech 後會自動鎖定。',
        statusLocked: '已鎖定',
    ...
    appearance: {
      ...
      tipsTitle: '應用程式內提示',
      tipsDesc: '偶爾顯示來自應用程式和 Nastech 的提示，每則提示只出現一次。開始使用滿30天後自動關閉，你可以重新開啟。',
      tipsReset: (count: number) => `再次顯示 ${count} 則提示`,
      toursTitle: '導覽',
      toursDesc: '讓 Nastech 逐步標示每個位置，帶你認識應用程式。開始使用滿30天後自動關閉，你可以重新開啟。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      browser: {
        useRealProfile:
          '本機瀏覽會使用你的真實登入狀態。Nastech 會將預設瀏覽器的設定（Cookie、登入資訊與偏好）複製成受管理的快照，再以內建的 Chromium 驅動它——不會直接開啟你正在使用的設定檔，且每次執行都會從目前的設定檔重新整理副本。設定雲端瀏覽器後端時，也允許代理視需要開啟本機真實設定檔工作階段。僅支援 Chromium 系瀏覽器（Chrome、Edge、Brave、Brave Origin、Chromium）；若預設瀏覽器並非 Chromium 系，會顯示明確錯誤。預設關閉。'
      },
      voice: {
        autoTts: '自動朗讀助手回覆。',
        voiceChatMode:
          'chained：語音轉文字 → Nastech → 文字轉語音，使用下方的提供方。gpt-live：由全雙工 OpenAI 語音模型（gpt-live-1）負責聆聽與說話，並將每個實際請求交給 Nastech——由你選擇的任意模型使用完整工具集作答。需要 OpenAI API 金鑰；語音層每分鐘收費 $0.05。',
        gptLive: {
          voice: 'GPT-Live 模式使用的音色，可填入自訂音色 ID。',
          instructions: '附加至即時語音人設的句子（語氣、語速、語言）。Nastech 會保留自己的系統提示詞。'
        }
      },
      stt: {
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Nastech 執行環境已更新,但桌面應用程式本身仍是舊建置——在應用程式更新之前,新的介面功能(如 Bot Mode)不會顯示。請執行下方的更新以重新建置應用程式。如果此警告仍未消除,請從最新的桌面安裝程式重新安裝。',
      bundleOutOfSyncAction: '取得安裝程式',
      bundleSwapPending: '重新啟動以完成更新',
      bundleSwapPendingDesc:
        '更新後的應用程式已安裝完成，只需重新啟動 Nastech 即可載入新版本。聊天記錄和設定不會受到影響。',
      bundleSwapPendingAction: '重新啟動 Nastech',
      updates: '更新',
      checkNow: '立即檢查',
      checking: '檢查中…',
      seeWhatsNew: '查看新增內容',
    ...
    screenshot: {
      ...
      enabledDesc:
        '在任何應用程式中同時按下左右兩個 Command 鍵，即可擷取最前方的視窗並附加到目前的 Nastech 草稿。絕不會自動傳送。預設關閉，僅適用於這台 Mac。視窗可能包含敏感內容，請在傳送前檢查附件。',
      ...
      inputPermission:
        '輸入監控權限可讓 Nastech 在其他應用程式使用中時偵測兩個 Command 鍵。請在系統設定 → 隱私權與安全性 → 輸入監控中允許 Nastech，然後返回此處重試。',
      screenPermission:
        '螢幕錄製權限可讓 Nastech 在你使用此快速鍵時擷取最前方的應用程式視窗。請在系統設定 → 隱私權與安全性 → 螢幕錄製中允許 Nastech，然後返回此處重試。如果 macOS 提示，請重新啟動 Nastech。',
      openSettings: '開啟系統設定',
      retry: '重試',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        'Nastech Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 Nastech 後端，請使用遠端閘道。閘道連線屬於本機層級設定；設定檔是從已連線的閘道中探索出來的。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `組建 ${tag}，${backend} 後端。Nastech 會為您啟動並管理伺服器。`,
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '讓網站保持登入狀態',
          body: '讓 Nastech 使用預設瀏覽器設定檔的快照進行瀏覽，網站開啟時即已登入。',
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: '兩種方式都會連接由你控制的機器人，憑證僅儲存在此 Nastech 安裝中。',
      quickSetup: '快速設定',
      recommended: '推薦',
      quickHelp: '掃描 QR 碼並在 Telegram 中確認。Nastech 會自動建立機器人並偵測你的 Telegram 使用者 ID。',
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: '此設定檔中的工作階段將在你指定的遠端 Nastech 上執行，而不是這台電腦。',
      urlLabel: '遠端位址',
    ...
    defaultDescription: '用於 Nastech 啟動和新建聊天。現有工作階段仍保留在各自的設定檔中。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      worktreeStaleBackend: '請更新 Nastech 後端以在此遠端連線上建立工作樹 —— 該後端早於 git 工作樹 API。',
  ...
  updates: {
    ...
    gitUnusable: 'Nastech 無法在這台電腦上執行 Git，因此無法檢查更新。',
    ...
    blockerTitle: '關閉本機預覽以更新 Nastech？',
    blockerBody: 'Nastech 需要在更新前停止這些本機預覽。這不會修改或刪除您的檔案。',
    foreignBlockerTitle: '關閉其他處理程序以更新 Nastech',
    foreignBlockerBody:
      'Nastech 無法安全地自動關閉這些處理程序。請關閉擁有這些處理程序的應用程式、終端機或服務，然後重試更新。',
    mixedBlockerBody: 'Nastech 可以關閉下方列出的本機預覽。其他處理程序必須手動關閉，更新才能繼續。',
  ...
  guidedGreeting: {
    line: '來了，進來吧。我是 Nastech。給我兩分鐘，把這裡按你的習慣整理一下，然後我們找件你真正想做的事來做。\n\n先說，我該怎麼稱呼你？',
  ...
  preview: {
    ...
    web: {
      ...
      blankPageBody: '在上方輸入網址開始瀏覽，或請 Nastech 開啟頁面。',
  ...
  prompts: {
    ...
    vaultSaveDesc: origin =>
      `Nastech 到達了 ${origin} 的登入頁，但沒有為它儲存的登入資訊。在此輸入一次；它會在本機加密儲存並直接填入頁面，模型永遠看不到密碼。`,
    ...
    vaultCodeDesc: site =>
      `${site} 要求輸入一次性驗證碼（簡訊、電子郵件或驗證器應用程式）。在此輸入，Nastech 會將其填入頁面；模型永遠看不到它。`,
    vaultCodeLabel: '驗證碼',
    vaultCodeFootnote: '提示：在「設定 → 密碼與登入」中為此登入儲存驗證器金鑰後，Nastech 會自動填寫驗證碼。',
    vaultCodeSkip: '略過',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就夠',
        text: '技能是一組說明檔，Nastech 會在需要時自行載入。'
      },
      messaging: {
        title: '離開電腦也能用',
      ...
      profiles: {
        title: '設定檔彼此獨立',
        text: '每個都是獨立的 Nastech — 自己的金鑰、記憶與工作階段。'
      },
      'composer-mentions': {
        title: '附件與指令',
