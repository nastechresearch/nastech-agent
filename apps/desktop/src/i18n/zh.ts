...
export const zh = defineLocale({
  catalog: {
    ...
    more: '显示更多',
    pinned: '已审核的提交',
    snapshotHint: '内容来自 Nastech 目录。浏览时不会连接来源代码仓库。',
  ...
  connectors: {
    ...
    search: '查找应用',
    empty: '没有匹配的应用',
    disclaimer: '连接为可选操作。请仅授权你希望 Nastech 使用的应用。',
    execution: '连接器工具'
  },

  sessionImport: {
    title: '从其他应用继续',
    subtitle: '将对话导入 Nastech，接着上次的进度继续。',
    action: '导入会话',
    readingFrom: '读取自',
  ...
  boot: {
    ...
    steps: {
      ...
      retryingRemoteBackend: '正在重新连接远程 Nastech 后端…',
    ...
    failure: {
      ...
      cloudDownTitle: 'Nastech Cloud 代理已宕机',
      cloudDownDescription:
        '此网关连接的 Nastech 托管云代理正在返回服务器错误。无法在此处重启——请检查其状态、切换到本地网关或获取支持。',
      cloudDownHint: '使用下方按钮打开 Nastech Portal（查看实例状态与操作）或加入 Discord 获取支持。',
  ...
  notifications: {
    ...
    voice: {
      ...
      liveError: '实时语音',
      liveDelegationFailed: '无法将请求交给 Nastech',
  ...
  sendDiagnostics: {
    title: '向 Nastech 发送诊断信息',
    privacyNotice:
      '这会将调试包上传到 Nastech 内部存储（并非公开粘贴板）。内容包括系统信息（操作系统、版本、服务商、已配置的 API 密钥种类 — 绝不包含密钥本身）以及完整的 agent、gateway 和桌面端日志（每个最多 512 KB，很可能包含对话内容、工具输出与文件路径）。上传前会先脱敏。仅 Nastech 员工与获准的 Discord 版主可查看，14 天后自动删除。',
    upload: '上传',
    uploading: '上传中…',
    cancel: '取消',
    close: '关闭',
    ...
    failedTitle: '上传失败',
    failedHint:
      '您也可以在终端运行 `nastech debug share --nastech`，或运行 `nastech debug share --local` 在不上传的情况下查看报告。',
    handoffLead: '在以下位置继续讨论:',
    links: {
      github: 'GitHub Issues',
      portal: 'Nastech Portal 支持',
      discord: 'Discord'
    }
  },
  ...
  settings: {
    ...
    vault: {
      ...
      otpHint: '启用两步验证时网站显示的“设置密钥”。保存后 Nastech 会自动生成验证码。',
      ...
      sources: {
        ...
        notInstalled: name => `未检测到。安装 ${name} 命令行工具并登录后，Nastech 会自动识别。`,
        disabledDesc: '已检测到，但已为 Nastech 关闭。',
        lockedDesc: '已检测到。智能体需要登录信息时会请你解锁，也可立即解锁。',
        unlockedDesc: '本会话已解锁。闲置 30 分钟或关闭 Nastech 后会自动锁定。',
        statusLocked: '已锁定',
    ...
    plugins: {
      ...
      installModal: {
        ...
        catalogPinned: (name, sha) =>
          `Nastech 目录条目「${name}」— agent 部分将安装在经过审核的固定提交${sha ? ` ${sha}` : ''}，而不是分支最新代码。`,
    ...
    appearance: {
      ...
      tipsTitle: '应用内提示',
      tipsDesc: '偶尔显示来自应用和 Nastech 的提示，每条提示只出现一次。开始使用满30天后自动关闭，你可以重新开启。',
      tipsReset: (count: number) => `再次显示 ${count} 条提示`,
      toursTitle: '引导导览',
      toursDesc: '让 Nastech 逐步高亮每个位置，带你熟悉应用。开始使用满30天后自动关闭，你可以重新开启。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      browser: {
        useRealProfile:
          '本地浏览使用你的真实登录状态。Nastech 会把你默认浏览器的配置（Cookie、登录、偏好）复制为受管快照，并用自带的 Chromium 驱动它——不会直接打开你的实时配置，且每次运行都会从实时配置刷新副本。还允许智能体在配置了云端浏览器后端时，按需打开本地真实配置会话。仅支持 Chromium 系浏览器（Chrome、Edge、Brave、Brave Origin、Chromium）；默认浏览器不是 Chromium 系时会给出明确报错。默认关闭。'
      },
      voice: {
        autoTts: '自动朗读助手回复。',
        voiceChatMode:
          'chained：语音转文字 → Nastech → 文字转语音，使用下方的提供商。gpt-live：一个全双工的 OpenAI 语音模型（gpt-live-1）负责听和说，并把每个实际请求交给 Nastech——由你选择的任意模型带着完整工具集作答。需要 OpenAI API 密钥；语音层按每分钟 $0.05 计费。',
        gptLive: {
          voice: 'GPT-Live 模式使用的音色，可填写自定义音色 ID。',
          instructions: '附加到实时语音人设的句子（语气、语速、语言）。Nastech 保留自己的系统提示词。'
        }
      },
      stt: {
    ...
    about: {
      ...
      bundleOutOfSyncDesc:
        'Nastech 运行时已更新,但桌面应用本身仍是旧构建——在应用更新之前,新的界面功能(如 Bot Mode)不会显示。请运行下方的更新以重新构建应用。如果此警告仍未消除,请从最新的桌面安装程序重新安装。',
      bundleOutOfSyncAction: '获取安装程序',
      bundleSwapPending: '重启以完成更新',
      bundleSwapPendingDesc: '更新后的应用已安装完成，只需重启 Nastech 即可加载新版本。聊天记录和设置不会受到影响。',
      bundleSwapPendingAction: '重启 Nastech',
      updates: '更新',
      checkNow: '立即检查',
      checking: '检查中…',
      seeWhatsNew: '查看新增内容',
    ...
    screenshot: {
      enabledTitle: '截图快捷键',
      enabledDesc:
        '在任意应用中同时按下左右两个 Command 键，即可截取最前面的窗口并附加到当前 Nastech 草稿。绝不会自动发送。默认关闭，仅适用于这台 Mac。窗口可能包含敏感内容，请在发送前检查附件。',
      statusTitle: '截图快捷键状态',
      checking: '正在检查截图快捷键…',
      disabled: '截图快捷键已关闭。',
      ...
      inputPermission:
        '输入监控权限允许 Nastech 在其他应用处于活动状态时检测两个 Command 键。请在系统设置 → 隐私与安全性 → 输入监控中允许 Nastech，然后返回此处重试。',
      screenPermission:
        '屏幕录制权限允许 Nastech 在你使用此快捷键时截取最前面的应用窗口。请在系统设置 → 隐私与安全性 → 屏幕录制中允许 Nastech，然后返回此处重试。如果 macOS 提示，请重启 Nastech。',
      openSettings: '打开系统设置',
      retry: '重试',
    ...
    connections: {
      title: '已注册网关',
      intro: '管理本机以及通过远程、SSH 或 Nastech Cloud 连接可访问的每个 Nastech 网关。',
      stagedNote:
      ...
      updateSkippedCloud: '由 Nastech Cloud 托管',
      kindLocal: '本地',
      kindRemote: '远程网关',
      kindCloud: 'Nastech Cloud',
      kindSsh: 'SSH',
      kindLocalDesc: '由本应用管理的 Nastech 运行时。',
      kindRemoteDesc: '可通过 HTTP(S) 访问的 Nastech 网关——局域网、Tailscale 或互联网。',
      kindCloudDesc: '通过你的 Nastech Cloud 账户发现的托管实例。',
      kindSshDesc: '通过 SSH 访问的 Nastech 安装。',
      labelTitle: '名称',
      ...
      headersDesc:
        '随发往此网关的每个 HTTP 和 WebSocket 请求一起发送——用于 Cloudflare Access 等访问代理（CF-Access-Client-Id / CF-Access-Client-Secret）。值加密存储。由 Nastech 管理的请求头（Authorization、Cookie、Host 等）会被忽略。',
      ...
      cloudAddHint: '提示：在上方登录 Nastech Cloud 可自动发现你的智能体——此表单仅用于手动注册已知的实例 URL。',
      save: '保存连接',
      saving: '保存中…',
      cancel: '取消',
      empty: '尚未注册任何连接。'
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'Nastech Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Nastech 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      ...
      sshErrNotInstalled:
        '远程主机上未安装 Nastech。请在远程安装（curl -fsSL https://nastechresearch.github.io/nastech-agent/install.sh | sh）或设置 Nastech 路径。',
    ...
    mcp: {
      ...
      deepLinkDescription:
        '一个链接请求将此 MCP 服务器添加到 Nastech。请检查下方的完整配置——它来自该链接，而非 Nastech。',
    ...
    localModels: {
      ...
      runtimeInstalledDetail: (tag, backend) => `构建 ${tag}，${backend} 后端。Nastech 会为您启动并管理服务器。`,
    ...
    toolsets: {
      ...
      browserRealProfile: {
        ...
        prompt: {
          title: '让网站保持登录状态',
          body: '让 Nastech 使用默认浏览器配置文件的快照进行浏览，网站打开时即已登录。',
  ...
  skills: {
    ...
    plugins: {
      ...
      legacyBackend: '此后端版本较旧，不支持按键名切换插件 — 请更新 Nastech 后再在此管理。',
      portableBadge: '便携',
      catalogTitle: '插件目录',
      catalogBrowse: '浏览',
      catalogHide: '隐藏目录浏览器',
      ...
      catalogProvenance: (sha: string) => `从 Nastech 目录安装${sha ? `，固定提交 ${sha}` : ''}。`,
  ...
  messaging: {
    ...
    telegramQr: {
      ...
      subtitle: '两种方式都会连接一个由你控制的机器人，凭据仅保存在此 Nastech 安装中。',
      quickSetup: '快速设置',
      recommended: '推荐',
      quickHelp: '扫描二维码并在 Telegram 中确认。Nastech 会自动创建机器人并识别你的 Telegram 用户 ID。',
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      description: '此配置档案中的会话将在你指定的远程 Nastech 上运行，而不是这台电脑。',
      urlLabel: '远程地址',
    ...
    defaultDescription: '用于 Nastech 启动和新建聊天。现有会话仍保留在各自的配置档案中。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      worktreeStaleBackend: '请更新 Nastech 后端以在此远程连接上创建工作树 —— 该后端早于 git 工作树 API。',
  ...
  composer: {
    ...
    voiceControls: '语音',
    voiceEngine: '语音聊天引擎',
    voiceEngineChained: '语音转文字 + Nastech 语音',
    voiceEngineLive: 'GPT-Live（全双工，委托给 Nastech）',
  ...
  updates: {
    ...
    gitUnusable: 'Nastech 无法在这台电脑上运行 Git，因此无法检查更新。',
    ...
    blockerTitle: '关闭本地预览以更新 Nastech？',
    blockerBody: 'Nastech 需要在更新前停止这些本地预览。这不会修改或删除你的文件。',
    foreignBlockerTitle: '关闭其他进程以更新 Nastech',
    foreignBlockerBody: 'Nastech 无法安全地自动关闭这些进程。请关闭拥有这些进程的应用、终端或服务，然后重试更新。',
    mixedBlockerBody: 'Nastech 可以关闭下方列出的本地预览。其他进程必须手动关闭，更新才能继续。',
  ...
  guidedGreeting: {
    line: '来了，进来吧。我是 Nastech。给我两分钟，把这里按你的习惯收拾一下，然后我们找件你真正想做的事来做。\n\n先说，我该怎么称呼你？',
  ...
  freeTier: {
    providerRowTitle: 'Nastech · 免费层',
    providerRowPitch: '登录 Nastech 账户以解锁更多模型和工具。',
    readyTitle: 'Nastech 已就绪。',
    readyCaption: '免费 · 含连接器',
    begin: '开始',
    signInInstead: '改为登录 Nastech 账户',
    otherProviders: '其他提供方',
    stripTitle: '免费的 Nastech 推理和连接器现已可用。',
    stripBody: '打开模型选择器试用，或登录 Nastech 账户。',
    openModelPicker: '打开模型选择器',
    dismiss: '关闭',
    providerName: 'Nastech',
    statusLabel: model => `Nastech · ${model}`,
    signIn: '登录',
    signInHeading: '登录 Nastech 账户以解锁更多模型和工具。',
    settingUp: '正在设置免费推理…',
    ...
    alreadySignedInBody: '此 Nastech 已登录 Nastech 账户。'
  },
  ...
  preview: {
    ...
    web: {
      ...
      blankPageBody: '在上方输入地址开始浏览，或让 Nastech 打开一个页面。',
  ...
  prompts: {
    ...
    vaultSaveDesc: origin =>
      `Nastech 到达了 ${origin} 的登录页，但没有为它保存的登录信息。在此输入一次；它将在本机加密保存并直接填入页面，模型永远看不到密码。`,
    ...
    vaultCodeDesc: site =>
      `${site} 要求输入一次性验证码（短信、邮件或验证器应用）。在此输入，Nastech 会将其填入页面；模型永远看不到它。`,
    vaultCodeLabel: '验证码',
    vaultCodeFootnote: '提示：在“设置 → 密码与登录”中为该登录保存验证器密钥后，Nastech 会自动填写验证码。',
    vaultCodeSkip: '跳过',
  ...
  tips: {
    ...
    items: {
      ...
      skills: {
        title: '教一次就够',
        text: '技能是一组说明文件，Nastech 会在需要时自行加载。'
      },
      messaging: {
        title: '离开电脑也能用',
      ...
      profiles: {
        title: '配置文件彼此独立',
        text: '每个都是独立的 Nastech — 自己的密钥、记忆和会话。'
      },
      'composer-mentions': {
        title: '附件与命令',
