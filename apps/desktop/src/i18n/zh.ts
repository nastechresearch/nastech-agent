  ...
  settings: {
    ...
    plugins: {
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '你安装到 Nastech 后端的插件——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        appliesTo: '应用于：',
        empty: '尚未安装智能体插件。',
      ...
      installModal: {
        ...
        agentLabel: '智能体插件',
        desktopLabel: '桌面 UI',
        agentTargetLocal: profile => `安装到 ${profile} 后端（~/.nastech/plugins/）`,
    ...
    appearance: {
      ...
      tipsTitle: '应用内提示',
      tipsDesc: '指向应用某处的小气泡：空闲时偶尔出现，需要时 Nastech 也会给你一条。关掉一条就不再出现。',
      tipsReset: (count: number) => `恢复 ${count} 条已关闭的提示`,
      toursTitle: '引导导览',
      toursDesc: '让 Nastech 带你熟悉应用：调暗界面并逐步高亮每个位置。',
    ...
    localModels: {
      ...
      upToDateDetail: (tag, backend) => `正在运行 llama.cpp ${tag}（${backend}）——Nastech 提供的最新构建。`,
