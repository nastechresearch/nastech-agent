...
// "Timed out connecting to Nastech backend" that hangs the UI (#48504).
//
...
export class NastechGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Nastech gateway connection closed',
      connectErrorMessage: 'Could not connect to Nastech gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Nastech gateway is not connected',
...
// the call; each pooled backend already has its own NASTECH_HOME, so no backend
...
// process to route them to that host (nastech:api's registry branch). Null /
...
async function activeConnection(): Promise<NastechConnection> {
  const getConnectionFor = window.nastechDesktop.getConnectionFor

  if (_apiConnectionId && getConnectionFor) {
    return getConnectionFor({ connectionId: _apiConnectionId, profile: _apiProfile })
  }

  return window.nastechDesktop.getConnection(_apiProfile)
}

/** Options for a plugin REST call — mirrors the app's own `nastechDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see NastechApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.nastechDesktop?.api) {
    throw new Error('Nastech desktop bridge unavailable')
  }

  const suffix = pluginPathSuffix('pluginRest', path)

  return window.nastechDesktop.api<T>({
...
export async function listSessions(
  ...
  const result = await window.nastechDesktop.api<PaginatedSessions>({
    path:
...
// "Error invoking remote method 'nastech:api': Error: 404: ..." through the
...
export function scanSessionPullRequests(
  ...
  return window.nastechDesktop.api<{
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await window.nastechDesktop.api<SidebarSessionsResponse>({
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.nastechDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.nastechDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(
  ...
  return window.nastechDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.nastechDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.nastechDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.nastechDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.nastechDesktop.api<LogsResponse>({
    ...profileScoped(),
...
export function getNastechConfig(profile?: string): Promise<NastechConfig> {
  return window.nastechDesktop.api<NastechConfig>({
...
export function getNastechConfigRecord(profile?: null | string): Promise<NastechConfigRecord> {
...
export function getNastechConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.nastechDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveNastechConfig(config: NastechConfigRecord): Promise<{ ok: boolean }> {
  return window.nastechDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.nastechDesktop.api<MemoryProviderConfig>({
    ...profileScoped(),
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.nastechDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.nastechDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function getCustomEndpoints(): Promise<CustomEndpointsResponse> {
  return window.nastechDesktop.api<CustomEndpointsResponse>({
    ...profileScoped(),
...
export function validateCustomEndpoint(endpoint: CustomEndpointUpdate): Promise<CustomEndpointValidationResponse> {
  return window.nastechDesktop.api<CustomEndpointValidationResponse>({
...
export function activateCustomEndpoint(id: string): Promise<{ ok: boolean; provider: string; model: string }> {
  return window.nastechDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.nastechDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.nastechDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.nastechDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.nastechDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.nastechDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function testMcpServer(name: string, profile?: null | string): Promise<McpTestResult> {
  return window.nastechDesktop.api<McpTestResult>({
    ...profileScoped(profile),
 ...
 *  `saveNastechConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function authMcpServer(name: string, profile?: null | string): Promise<McpOAuthFlow> {
  return window.nastechDesktop.api<McpOAuthFlow>({
    ...profileScoped(profile),
...
export function cancelMcpOAuthFlow(flowId: string, profile?: null | string): Promise<{ ok: boolean; status: string }> {
  return window.nastechDesktop.api<{ ok: boolean; status: string }>({
    ...profileScoped(profile),
    path: `/api/mcp/oauth/flows/${encodeURIComponent(flowId)}`,
    method: 'DELETE'
  })
...
export function getToolsets(profile?: null | string): Promise<ToolsetInfo[]> {
  return window.nastechDesktop.api<ToolsetInfo[]>({
    ...profileScoped(profile),
...
export function getToolsetConfig(name: string, profile?: null | string): Promise<ToolsetConfig> {
  return window.nastechDesktop.api<ToolsetConfig>({
    ...profileScoped(profile),
...
export function getToolsetModels(
  ...
  return window.nastechDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(profile),
...
export function selectToolsetModel(
  ...
  return window.nastechDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(profile),
...
export interface SelectToolsetProviderResponse {
  ...
  capability?: string
  /** Present (true) when a managed Nastech row was selected but the Portal
   *  entitlement is missing — the row won't activate until the user signs
   *  in to Nastech Portal. */
  needs_nastech_auth?: boolean
  /** The managed feature key (e.g. "browser") when needs_nastech_auth is set. */
  feature?: string
}
...
export function selectToolsetProvider(
  ...
  return window.nastechDesktop.api<SelectToolsetProviderResponse>({
    ...profileScoped(profile),
...
export function runToolsetPostSetup(
  ...
  return window.nastechDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(profile),
...
export function getTerminalBackends(): Promise<TerminalBackendsResponse> {
  return window.nastechDesktop.api<TerminalBackendsResponse>({
    ...profileScoped(),
...
export function selectTerminalBackend(backend: string): Promise<{ ok: boolean; backend: string }> {
  return window.nastechDesktop.api<{ ok: boolean; backend: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.nastechDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.nastechDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.nastechDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.nastechDesktop.api<MessagingPlatformTestResponse>({
...
export function getPairing(): Promise<PairingResponse> {
  return window.nastechDesktop.api<PairingResponse>({
    ...profileScoped(),
    path: '/api/pairing'
  })
}
...
export function approvePairing(platform: string, requestId: string): Promise<{ ok: boolean; user: PairingUser }> {
  return window.nastechDesktop.api<{ ok: boolean; user: PairingUser }>({
    ...profileScoped(),
...
export function getWebhooks(): Promise<WebhooksResponse> {
  return window.nastechDesktop.api<WebhooksResponse>({
    ...profileScoped(),
    path: '/api/webhooks'
  })
}
...
export function enableWebhooks(): Promise<WebhookEnableResponse> {
  return window.nastechDesktop.api<WebhookEnableResponse>({
    ...profileScoped(),
...
export function createWebhook(body: WebhookCreatePayload): Promise<WebhookCreateResponse> {
  return window.nastechDesktop.api<WebhookCreateResponse>({
    ...profileScoped(),
    path: '/api/webhooks',
    method: 'POST',
    body
...
export function setWebhookEnabled(
  ...
  return window.nastechDesktop.api<{ enabled: boolean; name: string; ok: boolean }>({
    ...profileScoped(),
...
// Cron jobs are stored per-profile (<NASTECH_HOME>/cron/jobs.json), and the
...
export function getCronJobs(profile?: string): Promise<CronJob[]> {
  ...
  return window.nastechDesktop.api<CronJob[]>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.nastechDesktop.api<{ runs: SessionInfo[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export async function getCronDeliveryTargets(): Promise<CronDeliveryTarget[]> {
  const { targets } = await window.nastechDesktop.api<{ targets: CronDeliveryTarget[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getAutomationBlueprints(): Promise<{ blueprints: AutomationBlueprint[] }> {
  return window.nastechDesktop.api<{ blueprints: AutomationBlueprint[] }>({
    ...profileScoped(),
    ...connectionScoped(),
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.nastechDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.nastechDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.nastechDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.nastechDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.nastechDesktop.api<ProfileSetupCommand>({
...
export function exportProfileArchive(
  ...
  return window.nastechDesktop.api<{ archive: string; ok: boolean }>({
...
export function importProfileArchive(
  ...
  return window.nastechDesktop.api<{ desktop: null | ProfileDesktopOverlay; name: string; ok: boolean; path: string }>({
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.nastechDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.nastechDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.nastechDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.nastechDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function updateNastech(): Promise<ActionResponse> {
  return window.nastechDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/nastech/update',
    method: 'POST'
  })
}
...
export function checkNastechUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.nastechDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/nastech/update/check${force ? '?force=true' : ''}`
  })
}
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.nastechDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.nastechDesktop.api<AudioSpeakResponse>({
    ...profileScoped(),
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.nastechDesktop.api<ElevenLabsVoicesResponse>({
...
// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `nastech skills`
...
// `nastech mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveNastechConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.nastechDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(body: {
  ...
  return window.nastechDesktop.api<McpServerSummary>({
    ...profileScoped(),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getGhAuthStatus(refresh = false): Promise<{ available: boolean; authenticated: boolean }> {
  return window.nastechDesktop.api<{ available: boolean; authenticated: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `nastech memory` / `nastech curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.nastechDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.nastechDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.nastechDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.nastechDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `nastech doctor` / `nastech security
// audit` / `nastech backup` / `nastech debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.nastechDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.nastechDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.nastechDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.nastechDesktop.api<DebugShareResponse>({
