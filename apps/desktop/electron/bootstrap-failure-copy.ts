...
export const BOOTSTRAP_STAGE_LABELS: ReadonlyMap<string, string> = new Map([
  ...
  ['repository', 'Nastech source code'],
  ...
  ['path', 'Nastech command'],
  ['gateway', 'Nastech service'],
...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of Nastech running. ' +
  'Close other Nastech windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up Nastech stopped during the '${label}' step.`
    : 'Setting up Nastech stopped before it could finish.'
...
/**
 * Error.message for an installed Nastech with a piece missing (source tree,
 * Python environment). The renderer's install overlay offers the Repair install
 * button ('nastech:bootstrap:repair'), so the copy points there. `whatIsMissing`
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of Nastech' installation is missing (it may have been deleted or quarantined by antivirus). " +
