...
test('launches the unpacked helper without prompting and delivers only validated capture messages', () => {
  ...
  const monitor = new CommandScreenshotMonitor({
    platform: 'darwin', appPath: '/Applications/Nastech.app/Contents/Resources/app.asar', spawn,
  })
  monitor.start(value => captures.push(value), value => statuses.push(value))
  assert.equal(spawn.mock.calls.length, 1)
  assert.deepEqual(spawn.mock.calls[0], [
    '/Applications/Nastech.app/Contents/Resources/app.asar.unpacked/dist/native/command-screenshot-monitor',
...
test.skipIf(process.platform !== 'darwin')('native state machine requires distinct keys, a clean chord and full release', () => {
  const dir = mkdtempSync(resolve(tmpdir(), 'nastech-command-monitor-test-'))
  try {
...
test.skipIf(process.platform !== 'darwin')('builds a universal helper with a read-only permission check and real controller lifecycle', async () => {
  const dir = mkdtempSync(resolve(tmpdir(), 'nastech-command-monitor-build-'))
  try {
