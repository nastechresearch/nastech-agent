...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startNastech() boots the primary as the preference at that moment.
  ...
  // nastech:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same NASTECH_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
