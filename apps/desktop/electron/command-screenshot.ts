...
export function installCommandScreenshot({ rendererUrl }: { rendererUrl: string }): () => void {
  ...
  const publish = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('nastech:screenshot:status', status())
      }
    }
  }
  ...
  const start = (requestPermission = false) => {
    ...
    monitor.start(
      window => {
        ...
        // Retain the last Nastech chat window when another application takes focus.
        ...
        if (requestId) {
          recipient.webContents.send('nastech:screenshot:request', requestId)
        }
      },
      result => {
  ...
  const handle = (name: string, callback: (event: IpcMainInvokeEvent, value: unknown) => unknown) => {
    const channel = `nastech:screenshot:${name}`
  ...
  ipcMain.on('nastech:screenshot:subscribe', onSubscribe)
  ...
  const dispose = () => {
    ...
    ipcMain.removeListener('nastech:screenshot:subscribe', onSubscribe)
