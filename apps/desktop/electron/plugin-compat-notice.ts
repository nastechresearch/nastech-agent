 ...
 *
 * The Python side (nastech_cli/plugin_compat.py) statically scans the user's enabled external plugins on
 * every CLI/gateway/TUI start and writes NASTECH_HOME/.plugin-compat-report.json when any plugin imports a
...
export function readReport(nastechHome: string): PluginCompatReport | null {
  try {
    const raw = fs.readFileSync(path.join(nastechHome, REPORT_FILE), 'utf8')
...
export function pendingNotice(nastechHome: string, userData: string): PendingNotice | null {
  const report = readReport(nastechHome)
  ...
  const copy = report.in_effect
    ? {
        ...
        message: `These plugins were built for an older Nastech and were turned off: ${nameList}. Nastech works normally without them.`,
        detail: `Look for an updated version of each plugin or ask its author.\n\n${list}`
      }
    : {
        title: 'Some plugins need an update',
        message: `These plugins were built for an older Nastech and will stop working on ${report.removal_date}: ${nameList}.`,
