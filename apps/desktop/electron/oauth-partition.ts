 ...
 * (`persist:nastech-remote-oauth`) — the jar was keyed on the auth *mode*, not
 ...
 * setup: one box, two dashboards) fought over the same `nastech_session*`
 ...
 *     cascade deliberately shares one jar with the Nastech Portal session.
...
export const LEGACY_OAUTH_PARTITION = 'persist:nastech-remote-oauth'
...
// `Partitions/nastech-remote-oauth%3Aconn%3A<id>` folder is left on disk,
