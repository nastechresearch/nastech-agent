...
export function registerNativeNotifications({ getMainWindow, focusWindow }: NotificationHost): void {
  ...
  ipcMain.handle('nastech:notify', (event, payload: NastechNotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('nastech:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('nastech:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('nastech:notification-action', { sessionId: payload.sessionId, actionId: action.id })
