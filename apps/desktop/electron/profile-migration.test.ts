 ...
 *      top of startNastech() in main.ts, before primaryProfileKey() is read); the
...
function baseDeps(overrides: Record<string, unknown> = {}) {
  ...
  return {
    legacyActivePath: '/home/u/.nastech/active_profile',
    nastechHome: '/home/u/.nastech',
    profilesRoot: '/home/u/.nastech/profiles',
    existsSync: fs.existsSync,
    readFileSync: fs.readFileSync,
    statSync: fs.statSync,
    readdirSync: fs.readdirSync,
    isNastechProcess: () => true,
    now: () => NOW,
    writeJson: () => {},
    isValidProfileName,
    ...overrides
...
test('findRunningGatewayProfiles returns [] when no pid files exist', () => {
  const fs = makeFs({ '/home/u/.nastech/profiles/coder': { dir: true } })
  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.nastech/profiles', ['coder'], {
      ...fs,
      isNastechProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles drops stale (non-nastech) recycled PIDs', () => {
  // Two profiles have pid files, but only coder's pid is a live nastech process.
  // The recycled PID at 5678 belongs to an unrelated process (e.g. Chrome).
  const fs = makeFs({
    '/home/u/.nastech/profiles/coder/gateway.pid': { content: '{"pid":1234}' },
    '/home/u/.nastech/profiles/writer/gateway.pid': { content: '{"pid":5678}' }
  })

  const deps = {
    ...fs,
    isNastechProcess: (pid: number) => pid === 1234
  }

  assert.deepEqual(findRunningGatewayProfiles('/home/u/.nastech/profiles', ['coder', 'writer'], deps), ['coder'])
})

test('findRunningGatewayProfiles tolerates malformed pid files', () => {
  const fs = makeFs({
    '/home/u/.nastech/profiles/coder/gateway.pid': { content: '{not json' }
  })
...
test('findRunningGatewayProfiles drops non-integer and non-positive PIDs', () => {
  const fs = makeFs({
    '/home/u/.nastech/profiles/coder/gateway.pid': { content: '{"pid":-1}' },
    '/home/u/.nastech/profiles/writer/gateway.pid': { content: '{"pid":1.5}' },
    '/home/u/.nastech/profiles/extra/gateway.pid': { content: '{"pid":0}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.nastech/profiles', ['coder', 'writer', 'extra'], {
      ...fs,
      isNastechProcess: () => true
    }),
    []
...
test('findRunningGatewayProfiles preserves order of allProfiles', () => {
  const fs = makeFs({
    '/home/u/.nastech/profiles/coder/gateway.pid': { content: '{"pid":1}' },
    '/home/u/.nastech/profiles/writer/gateway.pid': { content: '{"pid":2}' }
  })

  const deps = { ...fs, isNastechProcess: () => true }
  assert.deepEqual(findRunningGatewayProfiles('/home/u/.nastech/profiles', ['coder', 'writer'], deps), [
    'coder',
    'writer'
  ])
})
...
test('listProfileDirs includes default and any name passing the regex', () => {
  const fs = makeFs({
    '/home/u/.nastech/profiles/default': { dir: true },
    '/home/u/.nastech/profiles/coder': { dir: true },
    '/home/u/.nastech/profiles/writer': { dir: true }
  })
...
test('listProfileDirs skips files (not directories) and invalid names', () => {
  const fs = makeFs({
    '/home/u/.nastech/profiles/default': { dir: true },
    '/home/u/.nastech/profiles/some-file.txt': { dir: false },
    '/home/u/.nastech/profiles/UPPERCASE': { dir: true } // regex rejects
  })
...
test('decideMigration suppresses write when best is default (single-profile fallback)', () => {
  ...
  // default-primary and we leave it alone. Default's DB is $NASTECH_HOME/state.db,
  // not profiles/default/state.db.
  const deps = baseDeps()

  const d = decideMigration(null, [], ['default', 'coder'], deps, p => (p.endsWith('/.nastech/state.db') ? 99 : 50))
...
test('migrateActiveProfileIfMissing is a no-op when a user-selected preference file exists', () => {
  ...
  const fs = makeFs({
    ...
    '/home/u/.nastech/profiles/coder': { dir: true },
    '/home/u/.nastech/profiles/writer': { dir: true },
    '/home/u/.nastech/profiles/writer/state.db': { size: 400 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing writes legacy choice with no _migrated flag', () => {
  ...
  const fs = makeFs({
    '/home/u/.nastech/active_profile': { content: 'coder' },
    '/home/u/.nastech/profiles/coder': { dir: true },
    '/home/u/.nastech/profiles/writer': { dir: true }
  })
...
test('migrateActiveProfileIfMissing writes heuristic choice with _migrated=true', () => {
  ...
  const fs = makeFs({
    '/home/u/.nastech/profiles/coder/state.db': { size: 50 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.nastech/profiles/writer/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing is a no-op for single-profile (default-only) installs', () => {
  ...
  // Production default DB is ~/.nastech/state.db, not profiles/default/state.db.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.nastech/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing prefers a single running gateway over heuristics', () => {
  ...
  const fs = makeFs({
    '/home/u/.nastech/profiles/coder/gateway.pid': { content: '{"pid":42}' },
    '/home/u/.nastech/profiles/writer/state.db': { size: 500 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isNastechProcess: (pid: number) => pid === 42,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
...
// ---------------------------------------------------------------------------
// Production layout: default is ~/.nastech, not ~/.nastech/profiles/default
// ---------------------------------------------------------------------------
...
test('profileGatewayPidPath puts default at nastechHome', () => {
  assert.equal(
    profileGatewayPidPath('default', '/home/u/.nastech', '/home/u/.nastech/profiles'),
    '/home/u/.nastech/gateway.pid'
  )
  assert.equal(
    profileGatewayPidPath('coder', '/home/u/.nastech', '/home/u/.nastech/profiles'),
    '/home/u/.nastech/profiles/coder/gateway.pid'
  )
})
...
test('findRunningGatewayProfiles sees default gateway.pid at nastechHome', () => {
  const fs = makeFs({
    '/home/u/.nastech/gateway.pid': { content: '{"pid":99}' },
    '/home/u/.nastech/profiles/coder/gateway.pid': { content: '{"pid":11}' }
  })

  assert.deepEqual(
    findRunningGatewayProfiles('/home/u/.nastech/profiles', ['default', 'coder'], {
      ...fs,
      nastechHome: '/home/u/.nastech',
      isNastechProcess: pid => pid === 99
    }),
    ['default']
  )
})
...
test('migrateActiveProfileIfMissing does not pin a tiny named profile over a large default DB', () => {
  ...
  // ~/.nastech/profiles/<name>, never scored ~/.nastech/state.db, and wrote
  // { profile: named, _migrated: true }.
  let written: unknown = null

  const fs = makeFs({
    '/home/u/.nastech/profiles/conduit': { dir: true },
    '/home/u/.nastech/state.db': { size: 409 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.nastech/profiles/conduit/state.db': { size: 2 * 1024 * 1024, mtime: NOW - 60_000 }
  })
...
test('migrateActiveProfileIfMissing still pins a named profile that actually beats default', () => {
  ...
  const fs = makeFs({
    '/home/u/.nastech/profiles/work': { dir: true },
    '/home/u/.nastech/state.db': { size: 5 * 1024 * 1024, mtime: NOW - 86_400_000 },
    '/home/u/.nastech/profiles/work/state.db': { size: 200 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })
...
test('migrateActiveProfileIfMissing does not pin default when only default gateway is running', () => {
  ...
  const fs = makeFs({
    '/home/u/.nastech/gateway.pid': { content: '{"pid":7}' },
    '/home/u/.nastech/state.db': { size: 10 * 1024 * 1024, mtime: NOW - 86_400_000 }
  })

  const deps = baseDeps({
    ...fs,
    isNastechProcess: (pid: number) => pid === 7,
    writeJson: (_p: string, payload: unknown) => {
      written = payload
    }
  })
