...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    nastechHome: 'C:\\Users\\andre\\.nastech',
    python: 'C:\\Users\\andre\\.nastech\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /\.nastech-update-in-progress/)
...
test('every emitted PowerShell script keeps try blocks attached to their catch/finally handlers', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(
    ...
    'C:\\Users\\alice\\.nastech'
  )
  scripts.push(
    decode(atomicWindowsSpawnCommand({ nastechHome: 'C:\\Users\\alice\\.nastech', python: 'C:\\py\\python.exe' })),
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command nastech.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          nastechHome: 'C:\\Users\\alice\\.nastech',
          nastechPath: 'C:\\Nastech\\nastech.exe',
          python: 'C:\\Nastech\\python.exe'
        })
      }

      if (script.includes('.nastech-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('nastech_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.nastech\\profiles\\research')
  assert.match(script, /\.nastech-update-in-progress/)
...
test('Windows probe validates Nastech and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  const fallbackJoin = script.indexOf('Join-Path $nastechHome')
