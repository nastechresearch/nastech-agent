 ...
 *   - the browser sees only a minimal "return to Nastech" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Nastech.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('nastech:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('nastech:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('nastech:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
