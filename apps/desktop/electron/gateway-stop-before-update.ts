 ...
 *
 * So we delegate to `nastech gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `nastech update`'s
 ...
 */
export function startGatewaysAfterUpdateAbort(nastechCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
...
function runGatewayLifecycleCommand(
  ...
  if (deps.spy) {
    deps.spy(nastechCliPath, args)
  }

  if (!existsSync(nastechCliPath)) {
    return false
  }

  try {
    exec(nastechCliPath, args, {
