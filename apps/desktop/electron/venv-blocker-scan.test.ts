        ...
        processes: [
          {
            pid: 47484,
            name: 'python.exe',
            cmdline: 'C:\\Nastech\\venv\\Scripts\\python.exe -m http.server 8766 --directory C',
            kind: 'local-preview',
            safeToStop: true,
...
describe('stopSafeVenvBlockers', () => {
  it('stops only blockers explicitly classified as safe local previews', async () => {
    ...
    assert.deepEqual(calls, [
      {
        command: 'C:\\Nastech\\venv\\Scripts\\python.exe',
        args: ['-m', 'nastech_cli._scan_venv_blockers', '--terminate-safe', '47484', '1722798000.25']
      }
    ])
