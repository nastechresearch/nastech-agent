...
test('terminalScriptEnv drops PATH in any casing and keeps the rest', () => {
  ...
  assert.deepEqual(env, { PYTHONPATH: '/repo', PYTHONUTF8: '1', NASTECH_HOME: '/home/b/.nastech' })
})
...
test('buildTerminalScript execs the resolved runtime with its env', () => {
  const script = buildTerminalScript({
    args: ['-m', 'nastech_cli.main', '--tui', '--resume', 'sess'],
    command: '/home/b/.nastech/nastech-agent/venv/bin/python',
    cwd: "/home/b/o'brien",
    env: { PYTHONPATH: '/home/b/.nastech/nastech-agent' },
    platform: 'darwin'
  })
  ...
  assert.equal(
    ...
    [
      ...
      `export PYTHONPATH='/home/b/.nastech/nastech-agent'`,
      `exec '/home/b/.nastech/nastech-agent/venv/bin/python' '-m' 'nastech_cli.main' '--tui' '--resume' 'sess'`,
      ''
    ].join('\n')
  )
})
