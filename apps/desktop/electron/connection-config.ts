...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startNastech latches
  ...
  // classification (isServerSideHttpError / makeNastechCloudBackendDownError) and
...
/** Which Nastech profile the remote SSH dashboard should actually run as.
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_nastech_action` registers the (often dynamic, e.g.
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process NASTECH_HOME can belong to a different
 ...
 * `nastech:api` handler — both routes reach a backend whose namespace is the
...
/**
 * Resolve the two decisions made by the `nastech:api` IPC handler from the same
