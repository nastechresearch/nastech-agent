...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'nastech-auth-recovery-'))
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.url === '/auth/native/refresh') {
      state.refreshes++
      expect(JSON.parse(body)).toMatchObject({ refresh_token: 'old-rt', provider: 'nastech' })
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForNastechReady(f.baseUrl, {
    ...
    const mediaRequest = { url: 'nastech-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
