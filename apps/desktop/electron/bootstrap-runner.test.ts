...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/nastech-runner-test',
    sourceRepoRoot: null,
    nastechHome: '/tmp/nastech-runner-test',
    logRoot: '/tmp/nastech-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'nastech-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'nastech-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/nastech-agent', '--nastech-home', '/tmp/nastech', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/nastech',
      nastechHome: '/tmp/home'
    }),
    ['--dir', '/tmp/nastech', '--nastech-home', '/tmp/home', '--branch', 'main']
  )
})
...
test('installer log lines reach the emitter without escape sequences; \\r redraws keep the last frame', () => {
  ...
  assert.equal(cleanInstallerLogLine('\u001b]0;nastech\u0007Installing Nastech'), 'Installing Nastech')
