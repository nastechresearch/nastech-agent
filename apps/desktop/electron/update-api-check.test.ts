...
test('compare payload maps to the behind count and a newest-first commit list; malformed = null', () => {
  ...
  assert.equal(githubRepoSlug('git@github.com:Someone/nastech-agent.git'), 'someone/nastech-agent')
  assert.equal(githubRepoSlug('https://gitlab.example/x/y.git'), null)
  assert.equal(
    branchTipApiUrl('nastechresearch/nastech-agent', 'bb/gui'),
    'https://api.github.com/repos/nastechresearch/nastech-agent/commits/bb%2Fgui'
  )
})
