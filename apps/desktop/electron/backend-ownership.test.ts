...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'nastech serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/nastech serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m nastech_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/nastech --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Nastech Runtime\\nastech.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('nastech chat --query serve'), false)
