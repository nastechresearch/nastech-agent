export type ParentWatchdogEnv = {
  NASTECH_PARENT_PID: string
  NASTECH_PARENT_START_MARKER?: string
  NASTECH_PARENT_NONCE?: string
  /** Spawn tag consumed by nastech_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { NASTECH_PARENT_PID: String(pid) }
  env.NASTECH_SPAWN = spawnTag(pid, startMarker)
  ...
  env.NASTECH_PARENT_START_MARKER = startMarker
  env.NASTECH_PARENT_NONCE = nonce

  return env
}

/** Build the `NASTECH_SPAWN` tag mirrored by nastech_cli/process_identity.py.
