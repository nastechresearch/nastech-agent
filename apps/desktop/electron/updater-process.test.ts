...
test('stagedUpdaterSupportsPrewrittenMarker rejects installers predating the self-adopt fix', () => {
  ...
  // refreshed because copy_self_to_nastech_home no-ops during --update.
  assert.equal(
    stagedUpdaterSupportsPrewrittenMarker('C:\\Nastech\\nastech-setup.exe', {
...
test('resolveStagedUpdaterBinary still returns a stale staged updater on Windows', () => {
  ...
  assert.equal(
    resolveStagedUpdaterBinary('C:\\Nastech', {
      fileExists: () => true,
      isWindows: true,
      stagedMtimeMs: () => MARKER_SELF_ADOPT_EPOCH_MS - 60 * DAY_MS
    }),
    path.join('C:\\Nastech', 'nastech-setup.exe')
  )
})
...
test('spawnUpdaterProcess hides the updater console and detaches the child on Windows', () => {
  ...
  const result = spawnUpdaterProcess(
    'nastech-setup.exe',
    ['--update', '--branch', 'main'],
    { cwd: 'C:\\Nastech', detached: true, stdio: 'ignore' },
    {
      isWindows: true,
  ...
  assert.deepEqual(calls, [
    {
      ...
      command: 'nastech-setup.exe',
      options: { cwd: 'C:\\Nastech', detached: true, stdio: 'ignore', windowsHide: true }
    }
  ])
})
...
test('resolveStagedUpdaterBinary hands Windows the staged installer it finds', () => {
  const home = 'C:\\Users\\nastech\\AppData\\Local\\nastech'
  const staged = path.join(home, 'nastech-setup.exe')
  const probed: string[] = []
...
test('resolveStagedUpdaterBinary returns null off Windows even when nastech-setup is staged (#74836)', () => {
  const home = '/Users/nastech/.nastech'
  let probes = 0

  const resolved = resolveStagedUpdaterBinary(home, {
    // The installer stages nastech-setup on macOS/Linux too, so "it exists" is
...
test('resolveStagedUpdaterBinary returns null on Windows when nothing is staged', () => {
  const resolved = resolveStagedUpdaterBinary('C:\\Users\\nastech\\AppData\\Local\\nastech', {
    fileExists: () => false,
    isWindows: true
  })
...
test('resolveUpdateScriptHandoff returns null when the checkout predates the script', () => {
  const handoff = resolveUpdateScriptHandoff(String.raw`C:\Users\nastech\AppData\Local\nastech\nastech-agent`, {
    isWindows: true,
    fileExists: () => false
  })
...
test('resolveUpdateScriptHandoff is Windows-only (POSIX updates in place)', () => {
  const handoff = resolveUpdateScriptHandoff('/home/nastech/.nastech/nastech-agent', {
    isWindows: false,
    fileExists: () => true
  })
...
test('resolvePosixScriptHandoff returns the bash recipe when the script exists', () => {
  const root = '/home/nastech/.nastech/nastech-agent'
...
test('resolvePosixScriptHandoff is null when the checkout predates the script', () => {
  const handoff = resolvePosixScriptHandoff('/home/nastech/.nastech/nastech-agent', {
    isWindows: false,
    fileExists: () => false
  })
...
test('resolvePosixScriptHandoff is null on Windows', () => {
  const handoff = resolvePosixScriptHandoff(String.raw`C:\Users\nastech\AppData\Local\nastech\nastech-agent`, {
    isWindows: true,
    fileExists: () => true
  })
...
test('collectRelaunchArgs drops Electron internals, keeps user/launcher args', () => {
  const argv = [
    ...
    '--no-sandbox',
    'nastech://open/session/abc',
    '--profile=work'
  ]

  assert.deepEqual(collectRelaunchArgs(argv), ['--no-sandbox', 'nastech://open/session/abc', '--profile=work'])
...
test('describeUpdaterHandoffFailure leads with plain copy and confines the raw outcome to Details', () => {
  for (const raw of ['updater exited 127 before the settle window elapsed', 'updater spawn failed: ENOENT']) {
    ...
    assert.match(lead, /Nastech keeps running/)
    assert.match(lead, /Try again/)
    assert.doesNotMatch(lead, /exited|spawn|ENOENT|settle window|nastech update|\d/)
