...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = shq(expandRemotePath(lock.nastechPath))
  const expectedHome = lock.nastechHome ? shq(expandRemotePath(lock.nastechHome)) : "''"
...
async function spawnRemoteDashboard(
  ssh,
  { nastechPath, profile, token, ownershipId, nastechHome = '~/.nastech', assertInstallClear = async () => {} }
) {
