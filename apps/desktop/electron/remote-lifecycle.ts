...
async function probeNastechVersion(ssh, nastechPath) {
  try {
    ...
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(nastechPath)} --version 2>&1`))).trim()
...
marker=home/'.nastech-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Nastech checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, nastechHome) {
  const home = assertSafeRemoteHome(nastechHome)
  let observation = ''
  ...
  const error: any = new Error(
    live
      ? `Remote Nastech update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Nastech update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteNastechProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteNastechHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Nastech profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Nastech home.')
...
async function pidIsOurDashboard(
  ...
  const script =
    ...
    // (#74411), but it may `exec python <install-dir>/nastech`, leaving neither
    // launcher nor NASTECH_HOME-derived entrypoint in argv. The ownership-scoped
    // token path + random nonce + exact profile below are the alternative proof.
    `nastech_home=os.path.expanduser(${shq(nastechHome)}) if ${shq(nastechHome)} else ""\n` +
    'expected_entries={expected}\n' +
    'if nastech_home:\n' +
    ' expected_entries.add(os.path.join(nastech_home,"nastech-agent","venv","bin","nastech"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.nastechPath)
  const expectedHome = lock.nastechHome ? expandRemotePath(lock.nastechHome) : "''"
  ...
  const executableMatch = lock.nastechHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.nastechPath === expected.nastechPath &&
    lock.nastechHome === expected.nastechHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.nastechPath)})
nastech_home=os.path.expanduser(${py(lock.nastechHome)})
expected_entries={expected_path,os.path.join(nastech_home,"nastech-agent","venv","bin","nastech")}
...
// execing Nastech. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"nastech-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(nastechPath, profile, opts: any = {}) {
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.nastechHome || '~/.nastech')}/.nastech-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.nastechHome || '~/.nastech')}/.nastech-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env NASTECH_DESKTOP=1${opts.guestOnboarding === true ? ' NASTECH_GUEST_ONBOARDING=1' : ''} ${nastech} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} nastech-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, nastechPath) {
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${nastech} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(nastechPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        nastechHome,
...
async function connect(deps) {
  ...
  await assertRemoteInstallUpdateClear(ssh, nastechHome)
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Nastech Desktop build (${lock.reason}). ` +
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    nastechHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, nastechHome)
  })
