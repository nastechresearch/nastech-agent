 ...
 *
 * The pre-update handoff kills Nastech-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `nastech`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Nastech-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Nastech and holds venv shims mapped).
 */
export function isNastechOwnedVenvDaemon(
