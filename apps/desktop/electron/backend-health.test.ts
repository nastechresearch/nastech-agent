...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  // The unsigned-in throw must set isReauthRequired so startNastech latches.
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Nastech gateway')), false)
})
...
test('waitForNastechReady surfaces actionable error for cloud agent 503', async () => {
  let attempts = 0
  ...
  try {
    ...
    assert.ok(error.message.includes('Nastech Cloud agent'), `unexpected message: ${error.message}`)
    assert.ok(error.message.includes('503'), `should mention status code: ${error.message}`)
    assert.ok(error.message.includes('portal.nastechresearch.github.io'), `should mention portal: ${error.message}`)
    assert.ok(error.message.includes('discord.gg/NastechResearch'), `should mention Discord: ${error.message}`)
...
test('waitForNastechReady does not cloud-wrap non-cloud 503 errors', async () => {
  const currentTime = { value: 0 }
...
test('makeNastechCloudBackendDownError produces the Cloud shape and preserves cause', () => {
  ...
  assert.ok(result?.message.includes('Nastech Cloud agent ares-3009.agents.nastechresearch.github.io is down'))
})
...
test('makeNastechCloudBackendDownError returns null for a non-Cloud 503 (generic remote failure)', () => {
  ...
  err.statusCode = 503
  assert.equal(makeNastechCloudBackendDownError('https://gateway.example.com', err), null)
  assert.equal(makeNastechCloudBackendDownError('http://127.0.0.1:9000', err), null)
})
