 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `nastech update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.nastechHome, 'Nastech home')
  const nastechPath = validateRemoteValue(target.nastechPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(nastechPath)

  const updateCommand =
    `env NASTECH_HOME=${homeWord} ` +
    `NASTECH_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'NASTECH_UPDATE_ORIGIN_PROFILE=default ' +
    `NASTECH_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `NASTECH_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:NASTECH_HOME=${psLiteral(home)}`,
    `$env:NASTECH_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:NASTECH_UPDATE_ORIGIN_PROFILE="default"',
    `$env:NASTECH_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:NASTECH_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:NASTECH_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:NASTECH_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:NASTECH_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(nastechPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.nastech-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Nastech updated and every managed SSH profile is ready.'
        : restoreOk
