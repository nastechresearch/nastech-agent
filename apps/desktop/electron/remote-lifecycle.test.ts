...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.nastech-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Nastech', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('NASTECH_HOME')) {
          return '/home/alice/.nastech\n'
        }

        if (command.includes('.nastech-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.nastech/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /\.nastech-update-in-progress/)
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  ...
  assert.ok(cmd.includes('.nastech-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"nastech-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /nastech-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'nastech-update-mutex-'))
  const nastechPath = path.join(directory, 'nastech')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *nastech-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(nastechPath, '', {
      nastechHome: path.join(directory, 'home'),
      logPath
    })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  ...
  assert.equal(READY_RE.exec('INFO  Started server process [4711]NASTECH_BACKEND_READY port=65238')?.[1], '65238')
})
...
test.skipIf(process.platform === 'win32')('capability probe survives a zsh login shell on the remote (#111949)', async t => {
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), 'nastech-zsh-probe-'))

  try {
    const nastech = path.join(dir, 'nastech')
    await writeFile(nastech, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

    const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

    assert.equal(await remoteSupportsSshOwnership(ssh, nastech), true)
  } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'Nastech Agent v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeNastechVersion(versionSsh, '/x/nastech'), 'Nastech Agent v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/nastech'), true)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'nastech-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.nastech/.nastech-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.nastech/desktop-ssh/${OWNERSHIP_ID}`
...
test('connect() post-spawn cleanup that cannot prove ownership keeps the original boot error', async () => {
  ...
  await assert.rejects(
    connect(connectDeps(ssh, { platform: { os: 'Linux', arch: 'x86_64' }, waitForNastech: async () => { throw boot } })),
