...
// Relative, not `@nastech/shared`: the electron bundle is built by esbuild with
...
function encryptDesktopSecret(value, safeStorageApi, options: { allowPlainText?: boolean } = {}) {
  ...
  if (!encryptionAvailable) {
    ...
    throw new Error(
      'Secure token storage is unavailable (no OS keyring service was found), so Nastech Desktop cannot save remote gateway tokens. ' +
        ...
        'or set NASTECH_DESKTOP_REMOTE_URL and NASTECH_DESKTOP_REMOTE_TOKEN in your environment.'
    )
  }
...
// NASTECH_DESKTOP_REMOTE_* env vars.
