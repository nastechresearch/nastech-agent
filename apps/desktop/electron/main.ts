...
async function claimBackendChild(child, command, profile, nonce) {
  try {
    ...
    throw new Error(`Could not persist ownership for the Nastech backend: ${error.message}`)
  }
}
...
const OAUTH_SESSION_PARTITION = 'persist:nastech-remote-oauth'
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, fingerprint, lease) {
  ...
  return { ...connection, remoteNastechVersion: result.nastechVersion || '' }
}
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  child.once('exit', (code, signal) => {
    ...
    if (!ready) {
      rejectStart?.(
        new Error(`Nastech backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
...
async function startNastech() {
  ...
  const connectionPromise = (async () => {
    ...
    await claimBackendChild(nastechProcess, `${backend.command} ${backend.args.join(' ')}`, profile, backendNonce)
    ...
    nastechProcess.once('exit', (code, signal) => {
      ...
      if (!backendReady) {
        const message = `Nastech backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(nastechProcess, { readyFile }),
      backendStartFailed
    ])
...
ipcMain.handle('nastech:connection', async (_event, profile) => ensureBackend(profile))
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: resetNastechConnection,
...
ipcMain.handle('nastech:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('nastech:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('nastech:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('nastech:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('nastech:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('nastech:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('nastech:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('nastech:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('nastech:pet-overlay:control', payload)
})

// --- HUD mode (chrome-free floating chat) -----------------------------------
ipcMain.handle('nastech:hud:open', async (_event, request) => {
  openHudWindow(
...
ipcMain.handle('nastech:hud:vibrancy', (_event, on) => {
...
// reaches the bar.
ipcMain.on('nastech:hud:ignore-mouse', (_event, ignore) => {
  if (hudWindow && !hudWindow.isDestroyed()) {
    hudWindow.setIgnoreMouseEvents(Boolean(ignore), { forward: true })
  }
})

ipcMain.on('nastech:hud:move-by', (event, delta) => {
...
ipcMain.on('nastech:hud:set-bounds', (event, bounds) => {
...
ipcMain.on('nastech:hud:session', (event, sessionId) => {
  if (hudWindow && !hudWindow.isDestroyed() && event.sender === hudWindow.webContents) {
    hudSessionId = typeof sessionId === 'string' && sessionId ? sessionId : null
  }
})

ipcMain.handle('nastech:hud:close', async () => {
  closeHudWindow()
...
// /api/nastech/update endpoint (the dashboard updater), which runs
// `nastech update` on THAT machine.
ipcMain.handle('nastech:connections:update-all', async () => {
  const registry = readDesktopConnectionsRegistry()
...
ipcMain.handle('nastech:preview:reach', async (_event, url) => reachablePreviewUrl(String(url || '')))
...
function terminalShellCommand() {
  // NASTECH_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.NASTECH_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Nastech' TUI into
  ...
  env.TERM_PROGRAM = 'Nastech'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a nastech/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from NASTECH_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.NASTECH_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `nastech:terminal:${id}:${suffix}`
}
...
ipcMain.handle('nastech:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('nastech:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('nastech:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('nastech:fs:openDir', async (_event, dirPath) => {
...
// The LOCAL Desktop runtime-plugin root: `<NASTECH_HOME>/desktop-plugins`,
// resolved from the main-process NASTECH_HOME (see resolveNastechHome) — NOT from
// the connected backend. A remote backend reports its own `nastech_home` over
...
ipcMain.handle('nastech:plugin:probe', async (_event, payload) => {
...
ipcMain.handle('nastech:plugin:installDesktop', async (_event, payload) => {
  const identifier = String(payload?.identifier || payload?.repo || '').trim()
...
ipcMain.handle('nastech:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('nastech:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('nastech:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('nastech:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('nastech:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('nastech:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('nastech:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('nastech:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))

ipcMain.handle('nastech:git:baseBranchList', async (_event, repoPath) => listBaseBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('nastech:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('nastech:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('nastech:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('nastech:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('nastech:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
...
ipcMain.handle('nastech:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('nastech:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('nastech:terminal:write', (_event, id, data) => {
...
ipcMain.handle('nastech:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nastech:terminal:cwd', async (_event, id) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('nastech:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
