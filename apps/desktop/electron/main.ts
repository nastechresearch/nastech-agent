...
const MEDIA_PROTOCOL = 'nastech-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(nastech-media://…)`.
...
async function discoverCloudAgents(org?: string) {
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Nastech Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
ipcMain.handle('nastech:find-in-page', (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
