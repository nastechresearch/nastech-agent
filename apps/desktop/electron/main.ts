...
import {
  ...
  makeNastechCloudBackendDownError,
...
async function waitForUpdateToFinish() {
  ...
  try {
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Nastech update did not finish',
        `${result.message}\n\nDetails: ${path.join(NASTECH_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
...
function unwrapWindowsVenvNastechCommand(command, backendArgs) {
...
// (covers a bare `nastech` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'nastech_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `nastech --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, NASTECH_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveNastechBackend step 4); execFileSync of a .cmd without
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the nastech-import
...
function venvNastechShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'nastech.exe')
    : path.join(updateRoot, 'venv', 'bin', 'nastech')
}
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Nastech will keep running — try again, or run \`nastech update\` from a terminal.`
...
function isActiveRuntimeUsable() {
  ...
  return (
    ...
    canImportNastechCli(venvPython, {
      env: {
...
function resolveNastechBackend(backendArgs) {
  ...
  if (!IS_PACKAGED && isNastechSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Nastech source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  if (process.env.NASTECH_DESKTOP_IGNORE_EXISTING !== '1') {
    ...
    if (nastechCommand) {
      const unwrapped = unwrapWindowsVenvNastechCommand(nastechCommand, backendArgs)
      ...
      if (shouldTrustNastechOverride(nastechOverride) || verifyNastechCli(nastechCommand, { shell: shellForProbe })) {
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Nastech bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(NASTECH_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any
    ...
    return ensureRuntime(resolveNastechBackend(backend.args))
  }
  ...
  if (!isNastechSourceRoot(ACTIVE_NASTECH_ROOT)) {
    throw new Error(
      `Nastech install at ${ACTIVE_NASTECH_ROOT} is missing or incomplete. ` +
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Nastech on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Nastech.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `Nastech venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Nastech backend.'
                  )
                )
...
// Whether the OAuth partition currently holds a live Nastech portal session — the
// credential that powers both discovery and the silent cascade. The portal
// authenticates via PRIVY, not the Nastech gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for nastech_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Nastech Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Nastech Cloud sign-in.'))
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Nastech Cloud',
        autoHideMenuBar: true,
        webPreferences: {
          contextIsolation: true,
          nodeIntegration: false,
...
// a valid stored value (pins the root NASTECH_HOME explicitly); null means "no
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (nastech #37247), and the gateway middleware transparently
    ...
    try {
      ...
      // For a Nastech-managed Cloud agent, a 502/503/504 from the WS-ticket mint
      ...
      // else (#85335).
      const cloudError = makeNastechCloudBackendDownError(baseUrl, error)
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Nastech gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
...
function resetNastechConnection({ soft = false } = {}) {
  ...
  const nastechProcess = backendConnectionState.invalidate()
...
async function ensureBackend(profile) {
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Nastech backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Nastech backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  const backend = await ensureRuntime(resolveNastechBackend(backendArgs))
  ...
  child.once('exit', (code, signal) => {
    rememberLog(`Nastech backend for profile "${profile}" exited (${signal || code})`)
    releaseLocalBackendSlot(entry)
    releaseBackendChild(child)
...
async function startNastech() {
  ...
  const connectionPromise = (async () => {
    ...
    nastechProcess.once('exit', (code, signal) => {
      ...
      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Nastech backend exit (${signal || code})`)
      ...
      rememberLog(`Nastech backend exited (${signal || code})`)
...
function createWindow() {
  ...
  startNastech().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.on('nastech:connection:active-route', (event, route) => {
...
ipcMain.handle('nastech:window:openInstance', async () => {
...
ipcMain.handle('nastech:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('nastech:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('nastech:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as nastech://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('nastech:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('nastech:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('nastech:updates:check', async () =>
