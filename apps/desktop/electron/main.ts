...
import { canImportNastechCli, PROBE_TIMEOUT_MS, shouldTrustNastechOverride, verifyNastechCli } from './backend-probes'
...
import { isNastechOwnedVenvDaemon } from './venv-holder-select'
...
const USER_DATA_OVERRIDE = process.env.NASTECH_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.NASTECH_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.NASTECH_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with NASTECH_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[nastech] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[nastech] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. NASTECH_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[nastech] ${why}`)
  }
}
...
// never go through `nastech desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Nastech sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[nastech] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[nastech] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[nastech] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[nastech] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[nastech] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('nastech:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Nastech burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[nastech] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[nastech] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[nastech] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[nastech] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// NASTECH_HOME — the user-facing root for everything Nastech-related. Mirrors
// scripts/install.ps1's $NastechHome and scripts/install.sh's $NASTECH_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\nastech (matches install.ps1)
//   macOS / Linux: ~/.nastech (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.nastech directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\nastech yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// NASTECH_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// NASTECH_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.nastech / %LOCALAPPDATA%\nastech.
function resolveNastechHome() {
  if (process.env.NASTECH_HOME) {
    return normalizeNastechHomeRoot(process.env.NASTECH_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'nastech-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a NASTECH_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\nastech and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('NASTECH_HOME')

    if (fromRegistry) {
      return normalizeNastechHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'nastech')
    const legacy = path.join(app.getPath('home'), '.nastech')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.nastech setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.nastech')
}

const NASTECH_HOME = resolveNastechHome()

function pathWithNastechManagedNode(...entries) {
  const managed = nastechManagedNodePathEntries(NASTECH_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_NASTECH_ROOT — the canonical mutable Nastech install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_NASTECH_ROOT = path.join(NASTECH_HOME, 'nastech-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_NASTECH_ROOT, 'venv')
...
// boots, going straight to `resolveNastechBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_NASTECH_ROOT (not alongside)
...
// active-profile.json records which Nastech profile the desktop launches its
// local backend as. When set, startNastech() passes `nastech --profile <name>
// dashboard …`, which deterministically pins NASTECH_HOME (see
// _apply_profile_override in nastech_cli/main.py) and bypasses the sticky
// ~/.nastech/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors nastech_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = DESKTOP_PROFILE_NAME_RE
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// nastechDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under NASTECH_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by nastech_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(NASTECH_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (nastech_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.NASTECH_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.NASTECH_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.NASTECH_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// Nastech free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (nastech:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()
const SKIP_INTRO = skipIntroEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.NASTECH_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'nastech:native-theme' ('dark' | 'light' | 'system'); we pin
...
// nastech:window-control IPC channel. See titleBarOverlayOptions.
...
// electron-builder derives from the build `appId` (com.nastechresearch.nastech) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nastechresearch.nastech')
}

// Seed the native About panel with the live Nastech version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `nastech update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveNastechVersion(),
  copyright: 'Copyright © 2026 Nastech Research'
})
...
// counts startNastech() calls that have not settled; `primaryRecoverySuppressed`
// is set by every intentional invalidate of the slot and cleared by the next
// startNastech(), so the dying child's stale exit never respawns behind a
...
// startNastech(); this pool only holds EXTRA profile
...
// The legacy NASTECH_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.NASTECH_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.NASTECH_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Nastech backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `nastech:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.NASTECH_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// respawning nastech serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Nastech backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  nastechLog.push(...lines)

  if (nastechLog.length > 300) {
    nastechLog.splice(0, nastechLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-nastech-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('nastech:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the nastech:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('nastech:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_NASTECH_ROOT
  })
}
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    ...
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Nastech will start automatically when it completes…',
        12
      )
    },
...
function findOnPath(command) {
  ...
  // shell-script shim named `nastech` — must not shadow `nastech.cmd`/`nastech.exe`.
...
async function unwrapWindowsVenvNastechCommand(command, backendArgs) {
  return resolveVenvNastechCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `nastech serve`; runtimes older than serve only have `dashboard`. We
...
const backendSupportsServe = createBackendServeSupportResolver(NASTECH_HOME, rememberLog)
...
function isNastechSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'nastech_cli', 'main.py'))
}

async function findPythonForRoot(root) {
  const override = process.env.NASTECH_DESKTOP_PYTHON
...
async function findSystemPython() {
  ...
  // CPython but several Nastech deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves NASTECH_GIT_BASH_PATH
...
// normal NASTECH_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'nastech', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'nastech', 'git', 'bin', 'git.exe'))
  }
...
function recentNastechLog() {
  return nastechLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's nastech root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_NASTECH_ROOT.
// NASTECH_DESKTOP_NASTECH_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.NASTECH_DESKTOP_NASTECH_ROOT && path.resolve(process.env.NASTECH_DESKTOP_NASTECH_ROOT),
    !IS_PACKAGED && isNastechSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isNastechSourceRoot(ACTIVE_NASTECH_ROOT) ? ACTIVE_NASTECH_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_NASTECH_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('nastech:updates:progress', merged)
  }
}
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Nastech can't update itself from inside the app. Download the latest version from the Nastech website, " +
        `or reinstall Nastech to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
      nastechRoot: updateRoot,
      branch
    }
  }
...
// Windows that binary owns ALL repo mutation — running `nastech update` +
...
// Kill only Nastech-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `nastech`,
...
function killNastechOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Nastech-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `nastech.exe` that
// spawned its own grandchildren (a `nastech` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Nastech backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Nastech backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.nastechBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Nastech backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.nastechBackendIdentity
...
// `nastechProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `nastech.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a nastech
// REPL, a pty terminal, the gateway) keep `nastech.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  // unlock — the shim probe only covers venv\Scripts\nastech.exe, but the
  // backend is `python.exe -m nastech_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (nastechProcess && Number.isInteger(nastechProcess.pid)) {
    initialPids.push(nastechProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(nastechProcess, {
  ...
  // `nastech gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvNastechShimPath(updateRoot), NASTECH_HOME)

  // Reap Nastech-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killNastechOwnedVenvDaemons(updateRoot)

  const shim = venvNastechShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentNastechProcess = backendConnectionState.getProcess()

        if (currentNastechProcess && Number.isInteger(currentNastechProcess.pid)) {
          stragglers.push(currentNastechProcess.pid)
        }
...
// `nastech update`). Instead we spawn the staged Nastech-Setup binary with
// --update and quit, so it can run `nastech update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater) {
      ...
      // `nastech desktop`, never the Tauri installer that self-copies
      // nastech-setup.exe into NASTECH_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `nastech` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `nastech update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `nastech update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, nastechRoot: updateRoot }
      }
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Nastech — this window will close and the updater will open. Don’t reopen Nastech yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // nastech.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running nastech, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Nastech install open ' +
        '(a second Nastech window or a terminal running nastech?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvNastechShimPath(updateRoot))
      }
    ...
    // Nastech processes running from this venv.  The updater normally refuses
    ...
    // `nastech update` will run (the venv shim is locked while we live).
    //
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          NASTECH_HOME,
          NASTECH_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithNastechManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `nastech update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(NASTECH_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Nastech update is already running (PID <itself>)" —
...
// Python-level pre-update snapshot inside `nastech update`.
function preflightStateDb(nastechHome, rememberLog) {
  const stateDbPath = path.join(nastechHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(nastechHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(nastechHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(nastechHome, old))
            } catch {
              void 0
            }
          }
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      NASTECH_HOME,
      NASTECH_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithNastechManagedNode(path.join(resolveVenvDir(updateRoot), 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
...
// Marker-independent: is the canonical install at ACTIVE_NASTECH_ROOT actually
...
async function isActiveRuntimeUsable() {
  ...
  return (
    isNastechSourceRoot(ACTIVE_NASTECH_ROOT) &&
    ...
    (await canImportNastechCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_NASTECH_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    }))
  )
}
...
async function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `nastech
...
function resolveWebDist() {
  const override = process.env.NASTECH_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set NASTECH_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndexWithMissing(): { index: string; missing: string[] } {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: nastech desktop --force-build`
  )
...
function resolveNastechCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Nastech.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.NASTECH_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveNastechCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_NASTECH_ROOT, the
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Nastech at ${ACTIVE_NASTECH_ROOT}`,
    command,
    args: ['-m', 'nastech_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      nastechHome: NASTECH_HOME,
      pythonPathEntries: [ACTIVE_NASTECH_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_NASTECH_ROOT,
    bootstrap: true,
    shell: false
  }
}
...
async function resolveNastechBackend(backendArgs) {
  ...
  //    installed `nastech` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isNastechSourceRoot.)
  if (!IS_PACKAGED && isNastechSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Nastech source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_NASTECH_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\nastech\\nastech-agent (Windows) or ~/.nastech/nastech-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Nastech runtime at ${ACTIVE_NASTECH_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `nastech` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    NASTECH_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.NASTECH_DESKTOP_IGNORE_EXISTING !== '1') {
    let nastechCommand = null
    const nastechOverride = process.env.NASTECH_DESKTOP_NASTECH

    if (nastechOverride) {
      const resolvedOverride = findOnPath(nastechOverride)

      if (resolvedOverride) {
        nastechCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(nastechOverride, { isWsl: IS_WSL })) {
        nastechCommand = nastechOverride
      } else {
        rememberLog(`Ignoring Windows Nastech override under WSL: ${nastechOverride}`)
      }
    } else {
      nastechCommand = findOnPath('nastech')
    }

    if (nastechCommand) {
      if (looksLikeDesktopAppBinary(nastechCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Nastech CLI: ${nastechCommand}`)
        nastechCommand = null
      }
    }

    if (nastechCommand) {
      const unwrapped = await unwrapWindowsVenvNastechCommand(nastechCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `nastech` shim
      ...
      const shellForProbe = isCommandScript(nastechCommand)

      // NASTECH_DESKTOP_NASTECH is an explicit deployment override (used by
      ...
      if (
        shouldTrustNastechOverride(nastechOverride) ||
        (await verifyNastechCli(nastechCommand, { shell: shellForProbe }))
      ) {
        ...
        // unwrapWindowsVenvNastechCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Nastech CLI at ${nastechCommand}`,
          command: nastechCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Nastech CLI at ${nastechCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed nastech_cli module via system Python.
  ...
  if (python) {
    ...
    // having nastech_cli installed -- common on dev boxes that have
    ...
    if (await canImportNastechCli(python)) {
      return {
        kind: 'python',
        label: `installed nastech_cli module via ${python}`,
        command: python,
        args: ['-m', 'nastech_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: nastech_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveNastechBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Nastech Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveNastechBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Nastech install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Nastech recovery was handed off to Nastech Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      nastechHome: NASTECH_HOME,
      logRoot: path.join(NASTECH_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Nastech install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startNastech() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // nastech:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(await resolveNastechBackend(backend.args), assertStillOwned)
  }
  ...
  // %LOCALAPPDATA%\nastech\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `nastech` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Nastech needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Nastech.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isNastechSourceRoot()
    // plus an importable nastech_cli before it hands back the active runtime.
  ...
  backend.label = `Nastech at ${ACTIVE_NASTECH_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Nastech runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----nastech-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Nastech backend URL protocol: ${parsed.protocol}`))
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Nastech backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Nastech-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Nastech-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('nastech:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveNastechCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('nastech:preview-file-changed', payload)
}
...
// waitForNastechReady how to read a 401 — rejected session vs gated route.
...
async function waitForNastech(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForNastechReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('nastech:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('nastech:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('nastech:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('nastech:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Nastech...".
function sendPowerResume() {
  ...
  webContents.send('nastech:power-resume')
}
...
// 'nastech:power-battery' push below.
ipcMain.handle('nastech:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nastech:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(NASTECH_HOME, app.getPath('userData'))
  } catch (err) {
  ...
  try {
    ...
    // 'Open Plugins' rides the existing deep-link channel (nastech://open/…),
    ...
    if (response === 0) {
      handleDeepLink(`${NASTECH_PROTOCOL}://open/skills?tab=plugins`)
    }
  } finally {
    try {
...
function sendOpenUpdatesRequested() {
  ...
  // before the window is up) queue the request; 'nastech:deep-link-ready' flushes it.
  ...
  webContents.send('nastech:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('nastech:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('nastech:context-menu-spellcheck', {
...
// Nastech Research) instead of a static session token. The auth model is
...
//   * Nastech Portal now issues a 24h ROTATING, reuse-detected refresh token
...
// "not signed in", which on the initial boot path (startNastech → the renderer's
// single-shot boot() with no retry) surfaced as the "Nastech couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Nastech
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Nastech Cloud agent…' : 'Sign in to Nastech gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Nastech backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in nastech_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Nastech Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
// The "cloud" connection mode lets a user sign in to the Nastech portal ONCE in
...
// portal session cookie living in `persist:nastech-remote-oauth`:
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Nastech Cloud. Open Settings → Gateway, choose Nastech Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Nastech Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Nastech Cloud session has expired. Sign in to Nastech Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Nastech Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every nastech:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteNastechPath`/`remoteProfile` —
  ...
  onDefaultChanged: route => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.webContents.isDestroyed()) {
        win.webContents.send('nastech:profile:default:changed', route)
      }
    }
  }
})
...
// contains "nastech", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isNastechProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('nastech')
  } catch {
...
//   1. Legacy ~/.nastech/active_profile (explicit CLI choice via nastech profile use)
//   2. Running gateway (gateway.pid with verified liveness + nastech identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(NASTECH_HOME, 'active_profile'),
    nastechHome: NASTECH_HOME,
    profilesRoot: path.join(NASTECH_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.NASTECH_DESKTOP_REMOTE_URL)
  ...
  return {
    ...
    remoteUrl,
    // The persisted Nastech Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteNastechPath: (ssh || savedSsh)?.remoteNastechPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by NASTECH_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Nastech Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Nastech Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteNastechPath: input.sshRemoteNastechPath ?? existingBlock.remoteNastechPath,
...
async function buildRemoteConnection(
  ...
  if (!token) {
    throw new Error(
      'Remote Nastech gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.NASTECH_DESKTOP_REMOTE_TOKEN,
      url: process.env.NASTECH_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      nastechPath: result.nastechPath,
      nastechHome: result.nastechHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { nastechPath: result.nastechPath, nastechHome: result.nastechHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteNastechPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteNastechPath: sshConfig.remoteNastechPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForNastech: (baseUrl, token) => waitForNastech(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds NASTECH_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        nastechVersion: result.nastechVersion || '',
        ...
        // per-profile pools also use bare keys. Only startNastech' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.nastechVersion || 'nastech (version unknown)'} at ${result.nastechPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteNastechVersion: result.nastechVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (NASTECH_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.NASTECH_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startNastech() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Nastech
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Nastech Research" instead of a generic
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteNastechPath: input.sshRemoteNastechPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteNastechPath || '')
          let nastechPath
          let nastechVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            nastechVersion = inspection.version
            supported = inspection.supported
          } else {
            nastechPath = await remoteLifecycle.locateNastech(ssh, sshConfig.remoteNastechPath || '')
            nastechVersion = await remoteLifecycle.probeNastechVersion(ssh, nastechPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, nastechPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Nastech on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteNastechPath: nastechPath,
            remoteNastechVersion: nastechVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startNastech())
  ...
  // connect to Nastech gateway". Mirror the renderer's connect here so the test
...
function resetNastechConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startNastech() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const nastechProcess = invalidatePrimaryConnection()
  stopBackendChild(nastechProcess)
...
// startNastech() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetNastechConnection() invalidates it.
  const nastechProcess = backendConnectionState.getProcess()
  const dying = nastechProcess && !nastechProcess.killed ? nastechProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('nastech:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nastech:connections:changed', payload)
    }
  }
}
...
// startNastech() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(profile, opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startNastech()
  ...
  // spawning, so two children never share one profile's NASTECH_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteNastechPath: source.remoteNastechPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. nastech:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: nastechLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForNastech(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.nastechPath || !state?.nastechHome) {
  ...
  return {
    ...
    nastechPath: state.nastechPath,
    nastechHome: state.nastechHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteNastechPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.nastechPath ? platform : await probeWindowsRemote(ssh, config.remoteNastechPath || '')
    ...
    const nastechPath = await remoteLifecycle.locateNastech(ssh, config.remoteNastechPath || '')
    const nastechHome = await remoteLifecycle.probeRemoteNastechHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, nastechPath, nastechHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { nastechPath: state.nastechPath, nastechHome: state.nastechHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForNastech(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startNastech() but without the boot-progress UI,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForNastech(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited NASTECH_HOME env (see _apply_profile_override
  // step 3 in nastech_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']

  const backend = await ensureRuntime(await resolveNastechBackend(backendArgs), () =>
    assertPoolEntryStillOwned(poolKey, entry)
  )
  ...
  assertPoolEntryStillOwned(poolKey, entry)
  const nastechCwd = resolveNastechCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(NASTECH_HOME, 'profiles', key))
  )
  rememberLog(`Starting Nastech backend for profile "${profile}" via ${backend.label}`)
  ...
  child.once('error', error => {
    rememberLog(`Nastech backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Nastech backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Nastech backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Nastech backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForNastech(baseUrl, token), startFailed])
  assertPoolEntryStillOwned(poolKey, entry, { releaseSlot: false })
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Nastech backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Nastech backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function broadcastPoolBackendRetiring(poolKey: string) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('nastech:pool:retiring', { poolKey })
    }
  }
}
...
// backend whose ensure_nastech_home() recreates the deleted profile directory.
//
...
function startNastech() {
  primaryRecoverySuppressed = false
  primaryStartsInFlight += 1

  const start = localBackendLifecycle.start(runNastechStart)
...
function scheduleUnexpectedPrimaryRecovery({ code = null, signal = null, error = null, ready = false } = {}) {
  ...
  startNastech().catch(respawnError => rememberLog(`[supervisor] backend respawn failed: ${respawnError.message}`))

  return true
}

async function runNastechStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Nastech Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startNastech() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Nastech backend', 8)
  ...
  // later nastech:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Nastech backend at ${remote.baseUrl}`, 24)
      await waitForNastech(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Nastech backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, nastechLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.nastech/active_profile file) and
    // resolves NASTECH_HOME the same way `nastech -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Nastech runtime', 28)

        return resolveNastechBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Nastech backend via ${backend.label}`, 84)
    rememberLog(`Starting Nastech backend via ${backend.label}`)
    ...
    const nastechProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: nastechCwd,
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
            // Explicitly pin NASTECH_HOME for the child so Python's get_nastech_home()
            // resolves to the SAME location our resolveNastechHome() picked. Without
            // this pin, Python falls back to ~/.nastech on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\nastech, which differs from C:\Users\<u>\.nastech.
            // Mismatch would split config / sessions / .env / logs across two
            // directories. install.ps1 sets NASTECH_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(nastechProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(nastechProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, nastechProcess)

    if (!processOwner) {
      stopBackendChild(nastechProcess)
      await waitForBackendExit(nastechProcess)
      releaseBackendChild(nastechProcess)
      throw new Error('Nastech backend start was superseded by a newer connection attempt.')
    }

    nastechProcess.stdout.on('data', rememberLog)
    nastechProcess.stderr.on('data', rememberLog)
    ...
    nastechProcess.once('error', error => {
      releaseBackendChild(nastechProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Nastech backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Nastech backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Nastech backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Nastech backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    nastechProcess.once('exit', (code, signal) => {
      releaseBackendChild(nastechProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Nastech backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Nastech backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Nastech backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Nastech backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentNastechLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Nastech backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Nastech backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    await Promise.race([waitForNastech(baseUrl, token), backendStartFailed])
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    backendReady = true
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => nastechProcess.exitCode === null && !nastechProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Nastech backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Nastech backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed NASTECH_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startNastech() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `nastech:openExternal`, not here. See window-open-policy.ts.
 ...
 * always carries the `persist:nastech-preview` partition, so the partition is
 ...
 * audited `nastech:openExternal` channel. Popup requests themselves stay
...
function installPreviewGuestPreload() {
  app.on('web-contents-created', (_event, contents) => {
    ...
    contents.on('will-attach-webview', (_attachEvent, webPreferences, params) => {
      if (params.partition !== 'persist:nastech-preview') {
        return
      }
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('nastech:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Nastech is
...
// pushes pet state over IPC (nastech:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via nastech:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Nastech app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Nastech icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see nastech:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Nastech in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('nastech:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Nastech can be driven
...
// left the user with NO Nastech window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('nastech:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('nastech:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[nastech] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('nastech:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  ...
  // grab, nastech:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('nastech:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('nastech:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('nastech:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'nastech desktop --force-build',
  ...
  const startup = defaultRoute ? connectDesktopProfileRoute(defaultRoute) : startNastech()
...
ipcMain.handle('nastech:connection', async (event, profile, extra) => {
...
ipcMain.handle('nastech:connection:for', async (_event, payload) => {
...
ipcMain.on('nastech:connection:active-route', (event, route) => recordWindowConnectionRoute(event.sender, route))
...
// "Starting Nastech…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('nastech:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetNastechConnection({ soft: true }),
...
ipcMain.handle('nastech:backend:touch', async (_event, profile, options) => {
...
ipcMain.handle('nastech:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('nastech:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('nastech:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('nastech:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('nastech:window:openInstance', async (event, options) => {
...
ipcMain.handle('nastech:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('nastech:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = await resolveNastechBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))
...
ipcMain.handle('nastech:zoom:get', event => {
...
ipcMain.on('nastech:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nastech:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('nastech:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startNastech() call restarts the
...
ipcMain.handle('nastech:bootstrap:repair', async () => {
  // Forceful repair: force the next startNastech() through the full installer
  ...
  // "bypass active runtime" path inside startNastech() and fall through
...
ipcMain.handle('nastech:bootstrap:cancel', async () => {
...
ipcMain.handle('nastech:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('nastech:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('nastech:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('nastech:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('nastech:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('nastech:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('nastech:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('nastech:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('nastech:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('nastech:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('nastech:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('nastech:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('nastech:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('nastech:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteNastechPath: entry.remoteNastechPath
    })
  ...
  if (entry.kind === 'local') {
    const local = await startNastech()
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteNastechPath: connection.remoteNastechPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteNastechProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root NASTECH_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('nastech:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// nastech:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('nastech:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('nastech:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `nastech update` to every eligible registered connection at once.
...
ipcMain.handle('nastech:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/nastech/update', {}, { timeoutMs: 15_000 })
...
// analogue of the nastech:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('nastech:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('nastech:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startNastech() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('nastech:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Nastech Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('nastech:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('nastech:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('nastech:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('nastech:profile:default:get', async () => desktopProfilePreferences.getDefault())
ipcMain.handle('nastech:profile:default:set', async (_event, route) => desktopProfilePreferences.setDefault(route))
ipcMain.handle('nastech:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of nastech:profile:set: records the profile the
...
ipcMain.handle('nastech:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('nastech:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new NASTECH_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('nastech:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('nastech:requestMicrophoneAccess', async () => {
...
ipcMain.handle('nastech:window:readBelow', async event => {
...
async function handleNastechApiRequest(request) {
  ...
  // backend calls ensure_nastech_home() which recreates the profile directory,
...
ipcMain.handle('nastech:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its NASTECH_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleNastechApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleNastechApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('nastech:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('nastech:data-url-read-max:get', () => ({
...
ipcMain.handle('nastech:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('nastech:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('nastech:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('nastech:readFileText', async (_event, filePath) => {
...
// `nastech:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('nastech:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('nastech:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('nastech:writeClipboard', (_event, text) => {
...
ipcMain.handle('nastech:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('nastech:readClipboard', () => clipboard.readText())

ipcMain.handle('nastech:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('nastech:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('nastech:context-menu:edit', (event, command) => {
...
ipcMain.handle('nastech:context-menu:copy-image', event => {
...
ipcMain.handle('nastech:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('nastech:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('nastech:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('nastech:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('nastech:savePastedText', async (_event, payload) => {
...
ipcMain.handle('nastech:saveClipboardImage', async () => {
...
ipcMain.handle('nastech:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('nastech:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('nastech:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('nastech:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('nastech:active-work', (event, payload) => {
...
ipcMain.on('nastech:titlebar-theme', (_event, payload) => {
...
ipcMain.on('nastech:native-theme', (_event, mode) => {
...
ipcMain.on('nastech:translucency:support', event => {
...
// it arrives from `nastech desktop --local` or directly on Nastech.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('nastech:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('nastech:translucency', (_event, payload) => {
...
ipcMain.on('nastech:keep-awake', (_event, on) => {
...
ipcMain.handle('nastech:quick-entry:settings:get', async () => {
...
ipcMain.handle('nastech:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('nastech:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('nastech:quick-entry:submit', {
...
ipcMain.on('nastech:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('nastech:quick-entry:state', payload)
  }
})

ipcMain.on('nastech:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('nastech:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('nastech:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `nastech:window:openSession`).
...
ipcMain.handle('nastech:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nastech:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('nastech:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('nastech:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveNastechCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('nastech:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveNastechCwd()
}))

ipcMain.handle('nastech:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('nastech:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('nastech:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('nastech:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('nastech:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('nastech:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('nastech:updates:check', async (_event, opts) =>
...
ipcMain.handle('nastech:updates:apply', async (_event, payload) =>
...
ipcMain.handle('nastech:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('nastech:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Nastech version (the one `release.py` bumps in
// nastech_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Nastech version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveNastechVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'nastech_cli', '__init__.py')
...
// Renderer-bundle skew: `nastech update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Nastech version and push it into the native About panel
// just before showing it, so an in-place `nastech update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveNastechVersion()} — app build out of date, update the desktop app`
        : resolveNastechVersion(),
      copyright: 'Copyright © 2026 Nastech Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('nastech:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveNastechVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    nastechRoot: resolveUpdateRoot(),
...
// The About page's "Restart Nastech" button (shown when bundleSwapPending):
...
ipcMain.handle('nastech:app:relaunch', async () => {
...
ipcMain.handle('nastech:machine:profile', async () => {
...
// the actual removal via `nastech uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (nastech_cli/uninstall.py + nastech_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_NASTECH_ROOT
  ...
  const fallback = () => ({
    ...
    agent_installed: isNastechSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'nastech_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, NASTECH_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  // Python with PYTHONPATH=<agentRoot> so `import nastech_cli` resolves from
  ...
  if (modeRemovesAgent(mode)) {
    ...
    if (sysPy) {
      py = sysPy
      pythonPath = ACTIVE_NASTECH_ROOT
    } else if (IS_WINDOWS) {
      rememberLog(
  ...
  try {
    await releaseBackendLock(ACTIVE_NASTECH_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `nastech-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `nastech-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('nastech:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('nastech:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('nastech:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('nastech:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// nastech:// deep links (e.g. nastech://blueprint/morning-brief?time=08:00,
// nastech://mcp/install?name=NAME&config=B64 — the vendor "Add to Nastech"
// button, or nastech://plugin/install?repo=owner/repo). Dev
// (`NASTECH_DESKTOP_DEV_SERVER`) registers nastech-dev:// instead — bare
// Electron or a stale OS handler often owns nastech:// on dev machines.
...
// ---------------------------------------------------------------------------
const NASTECH_PROTOCOL = DEV_SERVER ? 'nastech-dev' : 'nastech'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['nastech-dev', 'nastech'] : ['nastech']
...
function handleDeepLink(url) {
  ...
  // nastech://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('nastech:deep-link', payload)
...
ipcMain.handle('nastech:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${NASTECH_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(NASTECH_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(NASTECH_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${NASTECH_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `nastech://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startNastech), whose
...
app.whenReady().then(() => {
  ...
  // startNastech() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching nastech:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Nastech never keeps another app's chord hostage.
