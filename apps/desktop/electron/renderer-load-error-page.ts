 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `nastech update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `nastech desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Nastech couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>nastech desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>nastech desktop --force-build</code>, then restart the app.</p>
