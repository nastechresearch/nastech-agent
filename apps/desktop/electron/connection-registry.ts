 ...
 * runtime, remote gateways, Nastech Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@nastech/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteNastechPath || '').trim() !== String(activeSsh.remoteNastechPath || '').trim() ||
...
/** Turn `ls ~/.nastech/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Nastech Cloud
 * instances are platform-managed — we never run `nastech update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteNastechPath: input.remoteNastechPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Nastech-managed names; an empty result stores nothing.
 ...
 * `remoteNastechPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Nastech Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Nastech Cloud' : 'Remote gateway')
