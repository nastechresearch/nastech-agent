...
export function registerHudIpc({
  ...
  ipcMain.on('nastech:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('nastech:hud:windowing', event => {
  ...
  ipcMain.on('nastech:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('nastech:hud:frost', (_event, showing) => {
  ...
  ipcMain.on('nastech:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('nastech:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('nastech:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
