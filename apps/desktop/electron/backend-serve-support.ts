...
// spawns `nastech serve`; runtimes older than serve only have `dashboard`, so
...
export function createBackendServeSupportResolver(nastechHome: string, rememberLog: (message: string) => void) {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'nastech_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, NASTECH_HOME: nastechHome, ...(backend.env || {}) },
