...
async function probeWindowsRemote(ssh, explicitNastechPath = '') {
  ...
  const script = [
    ...
    'Assert-NoReparse $nastechHome $true',
    '$candidate=[IO.Path]::Combine($nastechHome, "nastech-agent\\venv\\Scripts\\nastech.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "nastech-agent\\.venv\\Scripts\\nastech.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $nastechHome "nastech-agent\\venv\\Scripts\\nastech.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "nastech-agent\\.venv\\Scripts\\nastech.exe"',
    '$candidates=@()',
...
function windowsUpdateMarkerProbeCommand(nastechHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class NastechMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$nastechHome=${psLiteral(nastechHome)}`,
    '$installRoot=$nastechHome',
    '$parent=Split-Path -Parent $nastechHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".nastech-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[NastechMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, nastechHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(nastechHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'nastech_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'nastech_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$nastechHome=${psLiteral(runtime.nastechHome)}`,
    '$installRoot=$nastechHome',
    '$parent=Split-Path -Parent $nastechHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".nastech-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};nastechPath=${psLiteral(reservation.nastechPath)};nastechHome=${psLiteral(reservation.nastechHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
async function connectWindowsRemote(deps) {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.nastechHome)
