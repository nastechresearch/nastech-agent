...
export function createIntroRevealWindowController({
  ...
  function spawnIntroRevealWindow() {
    ...
    win.on('closed', () => {
      ...
      if (main && !main.isDestroyed()) {
        main.webContents.send('nastech:intro-reveal:closed')
      }
    })
  ...
  ipcMain.handle('nastech:intro-reveal:open', (_event, payload?: IntroRevealOpenPayload) => openIntroReveal(payload))
  ipcMain.handle('nastech:intro-reveal:close', (_event, payload?: IntroRevealClosePayload) => closeIntroReveal(payload))
  ipcMain.on('nastech:intro-reveal:ready', event => {
    if (event.sender === introRevealWindow?.webContents) {
      introRevealShow?.reveal()
    }
  })
  ipcMain.on('nastech:intro-reveal:skip', event => {
    const main = mainWindow()

    if (event.sender === introRevealWindow?.webContents && main && !main.isDestroyed()) {
      main.webContents.send('nastech:intro-reveal:skip')
    }
  })
