...
async function setup() {
  electron.directory = await mkdtemp(path.join(os.tmpdir(), 'nastech-screenshot-'))
  cleanups.push(installCommandScreenshot({ rendererUrl: 'http://127.0.0.1:5174/' }))
}

const call = (channel: string, event: unknown, ...args: unknown[]) => electron.handlers.get(`nastech:screenshot:${channel}`)(event, ...args)
...
describe.skipIf(process.platform !== 'darwin')('Command screenshot native bridge', () => {
  it('persists opt-in, routes to the last focused subscribed window while backgrounded, and revokes on disable', async () => {
    ...
    ipcMain.emit('nastech:screenshot:subscribe', first.event, true)
    ipcMain.emit('nastech:screenshot:subscribe', second.event, true)
    ...
    const requests = second.wc.send.mock.calls.filter(([channel]) => channel === 'nastech:screenshot:request')
    expect(requests).toHaveLength(1)
    expect(first.wc.send.mock.calls.some(([channel]) => channel === 'nastech:screenshot:request')).toBe(false)
    ...
    for (let i = 0; i < 3; i += 1) {
      ipcMain.emit('nastech:screenshot:subscribe', second.event, false)
      ipcMain.emit('nastech:screenshot:subscribe', second.event, true)
    }
