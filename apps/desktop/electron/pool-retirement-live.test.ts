 ...
 * Opt in with NASTECH_TEST_REAL_SERVE=1; see pool-retirement-live-fixture/README.md.
 */
...
function isolatedEnv(root: string): NodeJS.ProcessEnv {
  ...
  return { ...env, HOME: root, USERPROFILE: root, NASTECH_HOME: join(root, '.nastech'),
    XDG_CONFIG_HOME: join(root, 'config'), XDG_CACHE_HOME: join(root, 'cache'),
    TMPDIR: root, TEMP: root, TMP: root, TZ: 'UTC', LANG: 'C.UTF-8',
    NASTECH_DESKTOP_CDP_PORT: 'off', NASTECH_DESKTOP_USER_DATA_DIR: join(root, 'user-data') }
}
...
test.skipIf(process.env.NASTECH_TEST_REAL_SERVE !== '1' || process.platform === 'win32')(
  'native retirement preserves backend-only cron and hands both waiters capacity only after real child exit',
  async () => {
    const python = process.env.NASTECH_TEST_PYTHON
    assert.ok(python && existsSync(python), 'Set NASTECH_TEST_PYTHON to an installed Nastech Python environment')
    const electron = process.env.NASTECH_TEST_ELECTRON || require('electron') as string
    assert.ok(existsSync(electron), 'NASTECH_TEST_ELECTRON must name a real native Electron executable')
    const root = mkdtempSync(join(tmpdir(), 'nastech-pool-retirement-live-'))
    const resultPath = join(root, 'result.json')
    let child: ChildProcess | undefined
    let output = ''

    try {
      mkdirSync(join(root, '.nastech'))
      ...
      await build({ entryPoints: [join(desktop, 'src/test/pool-retirement-renderer.ts')], outfile: join(root, 'renderer.js'),
        ...
        alias: { '@': join(desktop, 'src'), '@nastech/shared': join(repo, 'apps/shared/src') },
