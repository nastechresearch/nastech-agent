// The standalone desktop-plugin root (`<NASTECH_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.nastech-package.json` marker
...
export const PACKAGE_MARKER = '.nastech-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.nastech-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every nastech home the app knows about locally: the default plus each profile. */
export async function localHomes(nastechHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(nastechHome, 'profiles'))

  return [nastechHome, ...profiles.map(name => path.join(nastechHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(nastechHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(nastechHome, 'profiles'))) {
    const scopedRoot = path.join(nastechHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(nastechHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(nastechHome)) {
