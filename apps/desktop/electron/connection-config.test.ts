...
test('normalizeRemoteHeaders keeps safe proxy headers and drops transport/auth headers', () => {
  assert.deepEqual(
    normalizeRemoteHeaders({
      ...
      'X-Nastech-Session-Token': { encoding: 'plain', value: 'token' },
...
test('remoteRequestMatchesBaseUrl treats HTTPS and WSS as the same gateway origin', () => {
  assert.equal(
    remoteRequestMatchesBaseUrl(
      'wss://nastech.example.com/gateway/api/ws?ticket=abc',
      'https://nastech.example.com/gateway'
    ),
    true
  )
...
test('localProfileEntry preserves inactive SSH drafts but drops Cloud state', () => {
  const ssh = { mode: 'ssh', host: 'box', user: 'alice', remoteNastechPath: '/nastech' }
...
// --- translateSelfProfileQuery (registry SSH-scoped nastech:api contract) ---
...
test('resolveProfileApiRequest uses exact method and path eligibility for mixed families', () => {
  ...
  assert.deepEqual(
    resolveProfileApiRequest('iris', '/api/model/recommended-default?provider=nastech', {
      requestMethod: 'GET'
    }),
    {
      backendProfile: 'iris',
      requestPath: '/api/model/recommended-default?provider=nastech'
    }
  )
})
...
test('resolveProfileApiRequest routes action-status polls with the action-spawning routes', () => {
  ...
  // install): _spawn_nastech_action registers the dynamic action name only in
...
test('normalizeRemoteBaseUrl auto-prepends http:// for scheme-less host:port input', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('gw.example.com/nastech/'), 'http://gw.example.com/nastech')
})
...
// --- cookiesHavePrivySession (Nastech portal / Privy auth, NOT gateway cookies) ---
...
test('cookiesHavePrivySession does NOT treat nastech gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'nastech_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-nastech_session_rt', value: 'x' }]), false)
})
...
test('cookiesHavePrivyAccessToken is false for empty values, gateway cookies, and non-arrays', () => {
  ...
  assert.equal(cookiesHavePrivyAccessToken([{ name: 'nastech_session_at', value: 'x' }]), false)
...
test('gateway WS URL IPC result serializes success and the auth-vs-transport matrix', async () => {
  ...
  for (const error of [
    ...
    new Error('Timed out connecting to Nastech backend after 8000ms'),
