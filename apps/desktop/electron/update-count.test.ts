...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'nastech-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Nastech Test')
    git('config', 'user.email', 'nastech@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/NastechResearch/nastech-agent.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/NastechResearch/nastech-agent/compare/${SHA_A}...${SHA_B}`
  )
})
