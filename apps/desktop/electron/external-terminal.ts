// Launching the Nastech TUI in the user's OWN terminal emulator.
//
...
// `nastech --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Nastech runtime is often a venv Python invoked
//     as `python -m nastech_cli.main`, not a `nastech` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Nastech
