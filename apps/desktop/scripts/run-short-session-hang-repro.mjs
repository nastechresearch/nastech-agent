...
const UPSTREAM_URL = 'https://github.com/NastechResearch/nastech-agent.git'
...
const LEGACY_MIGRATION_KEY = 'nastech.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.nastechDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `nastech-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const nastechHome = join(sandbox, 'nastech-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(nastechHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(nastechHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        NASTECH_DESKTOP_APP_NAME: `NastechShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        NASTECH_DESKTOP_NASTECH_ROOT: target.targetRoot,
        NASTECH_DESKTOP_IGNORE_EXISTING: '1',
        NASTECH_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'nastech-short-session-ab-'))
  const prepared = []
