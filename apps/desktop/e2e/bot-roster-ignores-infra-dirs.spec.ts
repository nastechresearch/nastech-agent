...
async function seedBot(nastechHome: string, mockUrl: string, name: string): Promise<void> {
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.nastechHome, mock.url, 'alpha')
  ...
  for (const stray of ['sessions', 'logs']) {
    fs.mkdirSync(path.join(sandbox.nastechHome, 'profiles', stray, 'cron'), { recursive: true })
  }

  // A deleted profile: identity file present, tombstone in profiles/.deleted/<name>.
  const ghost = path.join(sandbox.nastechHome, 'profiles', 'ghost')
  fs.mkdirSync(ghost, { recursive: true })
  fs.writeFileSync(path.join(ghost, 'profile.yaml'), 'ui_meta:\n  nastech-bots: {}\n')
  fs.mkdirSync(path.join(sandbox.nastechHome, 'profiles', '.deleted'), { recursive: true })
  fs.writeFileSync(path.join(sandbox.nastechHome, 'profiles', '.deleted', 'ghost'), '')
