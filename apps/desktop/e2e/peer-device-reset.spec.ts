 ...
 * Requires the built desktop and a repo venv (or NASTECH_DESKTOP_PYTHON).
 */
...
function pythonBinary(): string {
  if (process.env.NASTECH_DESKTOP_PYTHON) {
    return process.env.NASTECH_DESKTOP_PYTHON
  }
  ...
  if (!candidate) {
    throw new Error('Create the repo Python venv or set NASTECH_DESKTOP_PYTHON before running this spec')
  }
...
function isolatedEnv(sandbox: Sandbox): Record<string, string> {
  ...
  for (const key of ['NASTECH_DESKTOP_USER_DATA_DIR', 'NASTECH_DESKTOP_IGNORE_EXISTING', 'NASTECH_DESKTOP_NASTECH_ROOT', 'NASTECH_DESKTOP_APP_NAME', 'NASTECH_DESKTOP_SKIP_QUIT_CONFIRM']) {
    env[key] = defaults[key]
  }
  ...
  return {
    ...
    NASTECH_DESKTOP_PYTHON: pythonBinary(),
...
const peerTest = test.extend<{ gateways: { app: ElectronApplication; source: Page } }>({
  ...
  gateways: async ({}, runTest, info) => {
    ...
    try {
      ...
      child = spawn(pythonBinary(), ['-m', 'nastech_cli.main', 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
        cwd: REPO_ROOT,
        detached: process.platform !== 'win32',
        env: { ...isolatedEnv(remote), NASTECH_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN },
