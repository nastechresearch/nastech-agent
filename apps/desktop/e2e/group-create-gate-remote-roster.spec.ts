 ...
 * only its `default` profile; "Homelab" is a REAL second `nastech serve`
...
/** The worktree's own backend: `python -m nastech_cli.main` from the repo root
 *  (the venv's `nastech` console script resolves the package it was installed
 *  from, which need not be this checkout). */
function nastechCommand(): { bin: string; args: string[] } {
  const venvPython = path.join(REPO_ROOT, '.venv', 'bin', 'python')

  if (fs.existsSync(venvPython)) {
    return { bin: venvPython, args: ['-m', 'nastech_cli.main'] }
  }

  const result = spawnSync('which', ['nastech'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return { bin: result.stdout.trim(), args: [] }
  }

  throw new Error('nastech backend not found: create the repo venv (uv sync) or put nastech on PATH')
}
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const nastech = nastechCommand()

  const child: ChildProcess = spawn(nastech.bin, [...nastech.args, 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
    cwd: REPO_ROOT,
    detached: true,
    env: { ...process.env, NASTECH_HOME: home, NASTECH_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN, PYTHONPATH: REPO_ROOT },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote nastech serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, { headers: { 'X-Nastech-Session-Token': REMOTE_TOKEN } })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote nastech serve never became ready:\n${log}`)
  }
...
test.describe('New Group Chat gate — one local bot plus remote-connection bots', () => {
  ...
  test('the menu entry enables and the dialog seats the remote bot', async () => {
    ...
    await dialog.getByText('Nastech', { exact: true }).first().locator('xpath=ancestor::label').getByRole('checkbox').click()
