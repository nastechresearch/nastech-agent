...
// group-turn prompt introduced it to itself as @nastech, so a member told
// "You are @nastech" read `@bobby …` as a message for someone else and passed.
...
test.beforeAll(async () => {
  ...
  // The primary profile carries a Bot Mode title — what `nastech profile rename`
  // / the Edit Profile dialog persist into profile.yaml's ui_meta.
  writeFileSync(
    join(sandbox.nastechHome, 'profile.yaml'),
    'ui_meta:\n  nastech-bots:\n    title: Bobby\n    shape: circle\n    color: "#4f8"\n',
    'utf8'
  )
...
test('a renamed primary bot is addressed by its own @tag in the group-turn prompt', async () => {
  ...
  await expect
    .poll(async () => (await roomLog(page, GROUP)).some(e => e.from?.name === 'default' && (e.text || '').trim() === 'B'), {
      timeout: 180_000,
      message: 'the renamed primary (default / Bobby) was introduced to itself as @nastech and never answered @bobby'
    })
    .toBe(true)
