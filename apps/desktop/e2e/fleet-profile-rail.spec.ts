 ...
 * second gateway, "Homelab", is a REAL second `nastech serve` this spec spawns
 * with its own NASTECH_HOME, profiles and session token, registered in the v2
...
function findNastechBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'nastech')
  ...
  throw new Error('nastech binary not found: create the repo venv (uv sync) or put nastech on PATH')
}
...
/**
 * Spawn a second, fully real `nastech serve` as the remote gateway. Its
 * session token is pinned through NASTECH_DASHBOARD_SESSION_TOKEN so the
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        NASTECH_HOME: home,
        NASTECH_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Nastech-Session-Token': REMOTE_TOKEN },
      })
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.nastechHome, ['research'])
