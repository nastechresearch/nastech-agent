...
const PYTHON = process.env.NASTECH_E2E_PYTHON ?? path.join(REPO_ROOT, '.venv', 'bin', 'python')
...
`
  ...
  return execFileSync(PYTHON, ['-c', script, profileHome, message], {
    cwd: REPO_ROOT,
    env: { ...process.env, PYTHONPATH: REPO_ROOT, NASTECH_HOME: profileHome },
    encoding: 'utf8'
  }).trim()
}
...
test('a teammate DM still reaches the open Bot Chat when an unreadable ticket sits in the mailbox', async () => {
  ...
  const profileHome = path.join(fixture!.sandbox.nastechHome, 'profiles', 'alpha')
