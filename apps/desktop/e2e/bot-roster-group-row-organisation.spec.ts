...
// record rather than any bot's profile meta (nastech-agent#89813, #105544).
...
async function storedRoom(page: Page): Promise<{ pinned?: boolean; sectionId?: null | string }> {
  return page.evaluate(
    room => JSON.parse(localStorage.getItem('nastech.plugin.nastech-bots.group-chats') || '{}')[room] || {},
    ROOM
  )
}
...
test.beforeAll(async () => {
  ...
  for (const name of ['alpha', 'beta']) {
    await seedBot(sandbox.nastechHome, mock.url, name)
  }
