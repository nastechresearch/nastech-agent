...
// Hosted Group Chat rooms live in the gateway, and the Desktop's own `nastech serve` backend runs
...
const PYTHON = process.env.NASTECH_DESKTOP_PYTHON ?? path.join(REPO_ROOT, 'venv', 'bin', 'python')

let fixture: MockBackendFixture | null = null
let rpc: Rpc | null = null

function seedProfile(nastechHome: string, mockUrl: string, name: string): void {
  const dir = path.join(nastechHome, 'profiles', name)
...
function seedLegacyRoom(nastechHome: string): void {
  const script = `
import sys
...
`
  const out = execFileSync(PYTHON, ['-c', script, nastechHome], {
    cwd: REPO_ROOT,
...
/** Locate the Electron-spawned `nastech serve` backend for this sandbox: its port and session token. */
function findBackend(nastechHome: string): { port: number; token: string; pid: number } {
  ...
  for (const pidText of pids) {
    ...
    if (vars.get('NASTECH_HOME') !== nastechHome) {
      continue
    }
    const token = vars.get('NASTECH_DASHBOARD_SESSION_TOKEN') ?? ''
    // The backend inherits Electron's remote-debugging socket fd too; the serve port is the listener
    // that only the nastech process holds.
  ...
  throw new Error(`no nastech serve backend found for ${nastechHome} (pids: ${pids.join(',')})`)
}
...
test.beforeAll(async () => {
  ...
  seedProfile(sandbox.nastechHome, mock.url, 'sentinel')
  seedProfile(sandbox.nastechHome, mock.url, 'friday')
  seedLegacyRoom(sandbox.nastechHome)
  expect(fs.existsSync(path.join(sandbox.nastechHome, 'shared-state.db'))).toBe(false)
  ...
  const backend = findBackend(sandbox.nastechHome)
...
test('rooms from before the shared-state.db split are still reachable from the Desktop backend (#109775)', async () => {
  ...
  expect(fs.existsSync(path.join(fixture!.sandbox.nastechHome, 'shared-state.db'))).toBe(true)
  expect(fs.existsSync(path.join(fixture!.sandbox.nastechHome, 'state.db'))).toBe(true)
})
