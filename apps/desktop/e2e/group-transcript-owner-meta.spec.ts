...
async function storedMeta(page: Page) {
  return page.evaluate(() => {
    const meta = JSON.parse(localStorage.getItem('nastech.plugin.nastech-bots.bot-meta-v2') || '{}')
...
test('a re-titled member is re-labelled in the room transcript and Activity feed', async () => {
  ...
  const logFrom = await page.evaluate(
    room => (JSON.parse(localStorage.getItem('nastech.plugin.nastech-bots.group-chats') || '{}')[room]?.log || []).map((entry: any) => entry.from.name),
    ROOM
  )
