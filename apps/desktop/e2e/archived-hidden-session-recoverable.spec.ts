 ...
 * host load; the desktop still reads them through the real `nastech serve`.
 */
function seedArchivedRows(nastechHome: string, hiddenId: string, controlId: string): void {
  execFileSync(
    RUNTIME_PYTHON,
    [
      '-c',
    ...
    { cwd: REPO_ROOT, env: { ...process.env, NASTECH_HOME: nastechHome }, stdio: 'pipe' },
  )
  expect(fs.existsSync(path.join(nastechHome, 'state.db'))).toBe(true)
}
...
async function setupSeededMockBackend(): Promise<MockBackendFixture & { controlId: string; hiddenId: string }> {
  ...
  seedArchivedRows(sandbox.nastechHome, hiddenId, controlId)
