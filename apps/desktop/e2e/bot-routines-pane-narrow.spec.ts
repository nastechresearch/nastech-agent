...
async function seedBot(nastechHome: string, mockUrl: string, name: string): Promise<string> {
...
function seedRoutine(profileHome: string, title: string): void {
  ...
  execFileSync(python, ['-c', script], {
    cwd: repoRoot,
    env: { ...process.env, NASTECH_HOME: profileHome },
    stdio: 'pipe'
  })
}
...
function routinesTab(page: Page) {
  return page.locator('[data-tree-tab="nastech-bots:routines"]').filter({ visible: true }).first()
}
...
test.beforeAll(async () => {
  ...
  alphaHome = await seedBot(sandbox.nastechHome, mock.url, 'alpha')
...
test('a long routine title never pushes the Switch, delete control or next-run label out of the 250px pane', async ({}, testInfo) => {
  ...
  await test.step('closing Scheduled jobs with ✕ is recoverable — leaving and re-entering Bot Mode brings the pane back (#102224)', async () => {
    const tab = page.locator('[data-tree-tab="nastech-bots:routines"]').first()
    await expect(tab).toBeVisible({ timeout: 30_000 })
    await tab.hover()
    ...
    await expect(page.locator('[data-tree-tab="nastech-bots:routines"]').first()).toBeVisible({ timeout: 30_000 })
  })
})
