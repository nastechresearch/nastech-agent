...
previews) and the preview pane's `<webview>` (`persist:nastech-preview`). Neither
...
- `main.ts` installs `electron/preview-guest-preload-entry.ts` via
  `will-attach-webview`, keyed on the `persist:nastech-preview` partition only.
...
- `PreviewPane` admits `http:`/`https:` only (`src/lib/preview-external.ts`)
  and hands the URL to the existing `nastech:openExternal` IPC, which applies
...
`env -u NODE_ENV NASTECH_GUEST_ONBOARDING=1 NASTECH_HOME=<tmp>/.nastech NASTECH_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`NASTECH_PORTAL_BASE_URL=http://127.0.0.1:8765 NASTECH_ANON_API_SECRET=test-secret NASTECH_SHARED_AUTH_DIR=<tmp>/.nastech/shared`
...
## Nastech free tier: state is pulled, never latched in the renderer

The free tier (a Nastech identity with no account, `nastech_cli/anon_auth.py`) reaches the renderer
...
`POST /api/providers/oauth/nastech/start` + poll route, which over a free-tier identity registers the
