...
/** One persisted reaction row (``nastech_state_messages.set_message_reaction``); ``seen`` is stamped once announced. */
...
export interface DiagnosticsShareNastechParams {
...
export interface DiagnosticsShareNastechResult {
  ok: boolean
...
/** One ``nastech_cli/inventory.py::build_models_payload`` provider row (the union of every field the builder sets; ``pricing_pending`` / ``free_tier_pending`` mark the cached-only path). */
...
/** ``nastech_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown, ``"free"``); the sale fields are Nastech Portal-only. */
...
/** ``nastech_cli/foreign_sessions_browser.py::list_foreign_sessions`` — ``id`` is an opaque handle, never a path. */
...
/** ``list`` → ``plugins`` + counts; ``toggle`` → ``ok``/``unchanged``/``name``/``plugin``; ``install`` → ``nastech_cli.plugins_cmd.dashboard_install_plugin``'s ok payload; ``update`` → ``ok``/``unchanged``/``sha``. */
...
/** ``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``NastechSkin``). ``{}`` when the skin engine failed to load. Colour maps are token → colour string. */
...
export interface RpcMethods {
  ...
  /** Read-only billing view (no scope); the Nastech free tier is answered locally without a portal call. */
  ...
  /** Run ``nastech <argv>`` non-interactively and capture its output; ``blocked`` explains a refusal. */
  ...
  /** Upload a force-redacted debug bundle to Nastech-internal diagnostics storage. */
  'diagnostics.share_nastech': { params: DiagnosticsShareNastechParams; result: DiagnosticsShareNastechResult }
  ...
  /** Re-read ~/.nastech/.env (CLI /reload parity); built agents keep their pool until /new. */
  ...
  /** Export the transcript to ~/.nastech/sessions/saved (classic /save). */
  ...
  /** Token / context / cost counters for the session (+ Nastech credit lines when available). */
