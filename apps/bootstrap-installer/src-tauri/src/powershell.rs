...
/// read side does not see EOF until the last of them closes it. `nastech update`
...
/// stranded with it.
///
/// Same bound `Invoke-NastechStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `nastech_home_override` propagates to the child as $NASTECH_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, nastech_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = nastech_home_override {
        cmd.env("NASTECH_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, nastech_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = nastech_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_nastech_home() {
    ...
    /// child. `nastech update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
