...
/// a dashboard-spawned `nastech update`) still blocks.
...
/// the marker name our PID — because a `nastech update` child that
...
async fn run_update(app: AppHandle) -> Result<()> {
    ...
    // checkout predating the NASTECH_UPDATE_HANDOFF_PID env fix (8c76fe19f)
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            ...
            // Nastech.exe, so an image-name kill would tear down the app itself.
            emit_log(
                app,
                Some(stage),
                LogStream::Stdout,
                &format!(
                    "[handoff] Nastech still holding install files ({}); locating backend shims…",
...
/// Find processes running the exact `venv\Scripts\nastech.exe` shim for this
/// installation. Windows image names are case-insensitive and the desktop is
/// also Nastech.exe, so matching by image name alone is unsafe.
#[cfg(windows)]
...
async fn run_streamed(
    ...
    // the same rule about pipe EOF: `nastech update` is precisely the shape that
...
mod tests {
    ...
    fn same_windows_path_accepts_case_only_difference() {
        assert!(same_windows_path(
            Path::new(r"C:\Users\tester\.nastech\nastech-agent\venv\scripts\NASTECH.EXE"),
    ...
    fn same_windows_path_rejects_desktop_binary() {
        assert!(!same_windows_path(
            Path::new(r"C:\Users\tester\.nastech\nastech-agent\apps\desktop\Nastech.exe"),
            Path::new(r"C:\Users\tester\.nastech\nastech-agent\venv\Scripts\nastech.exe"),
        ));
    }
    ...
    // checkout's `nastech update` reads it as a live foreign update and exits
    ...
    fn missing_or_garbage_marker_never_heals() {
        ...
        let garbage = dir.join(".nastech-update-in-progress");
    ...
    fn heal_end_to_end_marker_lifecycle() {
        ...
        // own exit 2, e.g. a genuinely still-running Nastech, stays terminal).
        assert!(!should_heal_self_marker_refusal(
            Some(UPDATE_EXIT_CONCURRENT),
            &marker
        ));
