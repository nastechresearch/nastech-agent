...
The underlying token machinery is the *same* one the CLI ``nastech mcp login``
uses — ``nastech_cli.mcp_config._probe_single_server`` under
...
def _worker(session_id: str, nastech_home: str, server_name: str, cfg: dict, reconnect_live: bool) -> None:
    ...
    NASTECH_HOME override + secret-scope + force_interactive_oauth +
...
def start_flow(
    ...
    *,
    ...
    OAuth-capable). ``nastech_home`` is the already-resolved profile home dir
    ...
    with _sessions_lock:
        ...
        if any(
            ...
            and r["nastech_home"] == nastech_home
    ...
    threading.Thread(
        target=_worker,
        args=(session_id, nastech_home, server_name, dict(cfg), reconnect_live),
        daemon=True,
