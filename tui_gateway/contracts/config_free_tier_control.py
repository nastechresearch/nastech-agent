...
Handlers: ``tui_gateway/methods_config.py`` (``config.get``, ``setup.*``, ``diagnostics.share_nastech``),
...
class ConfigProviderRef(OpenModel):
    """``nastech_cli/models.py::list_available_providers`` row."""
...
# ── diagnostics.share_nastech ────────────────────────────────────────────────────────────────────


class DiagnosticsShareNastechParams(Params):
    error_context: str | None = None
    extra_files: dict[str, str] | None = None
    log_lines: int | None = None


class DiagnosticsShareNastechResult(Result):
...
method("diagnostics.share_nastech", params=DiagnosticsShareNastechParams, result=DiagnosticsShareNastechResult,
       doc="Upload a force-redacted debug bundle to Nastech-internal diagnostics storage.")
...
class ModelPricing(Result):
    """``nastech_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
    ``"free"``); the sale fields are Nastech Portal-only."""
...
class ModelCapabilities(Result):
    """``nastech_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``nastech_cli/inventory.py::build_models_payload`` provider row (the union of every field
...
class GoalContractSnapshot(Result):
    """``nastech_cli/goals.py::GoalContract.to_dict``."""
