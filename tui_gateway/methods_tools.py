...
def _(rid, params: dict) -> dict:
    ...
    if name == "plan":
        ...
        # .nastech/plans/ via write_file. Works on any backend.
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # Re-bind NASTECH_HOME to the session's profile so get_skill_commands()
        ...
        _home_token = (
            set_nastech_home_override(_profile_home) if _profile_home else None
        )
        try:
...
def _(rid, params: dict) -> dict:
    ...
    # Optional profile scoping: cronjob() keys off NASTECH_HOME, so scoping the
    ...
    if profile:
        try:
            ...
            from nastech_constants import set_nastech_home_override
    ...
    try:
        ...
        if token is not None:
            try:
                from nastech_constants import reset_nastech_home_override
...
def _(rid, params: dict) -> dict:
    ...
    keys], transport}]}`` — the same catalog `nastech mcp` offers, so
...
# (nastech_cli/web_routers/mcp.py) so a desktop plugin can manage MCP servers for
# ANY profile, not just the launch profile. Each accepts an optional ``profile``
# param that scopes NASTECH_HOME via set_nastech_home_override (omitted/None = the
# launch profile) in a try/finally, exactly like ``skills.manage`` / ``mcp.catalog``.
# All persistence reuses nastech_cli/mcp_config.py helpers — no logic is duplicated.
...
def _(rid, params: dict) -> dict:
    ...
    (``window.nastechDesktop.openExternal``) and then polls
    ...
    the SAME interactive MCP OAuth machinery ``nastech mcp login`` uses
    ...
    try:
        ...
        nastech_home = str(get_nastech_home().expanduser().resolve(strict=False))
...
def _(rid, params: dict) -> dict:
    ...
    Actions:
      ...
      - ``install`` → git-clone into ``~/.nastech/plugins/`` (non-interactive).
    ...
    plugins live under each profile's NASTECH_HOME, so a client can list or
