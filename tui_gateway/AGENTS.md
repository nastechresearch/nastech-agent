...
activate with `nastech --tui` or `NASTECH_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `NASTECH_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.nastech`. The first non-launch home hosted flips
...
**A method that sets only `get_nastech_home_override()` is half-bound**: config paths resolve to the
...
(`nastech -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `nastech_cli/commands.py` (`nastech_cli/AGENTS.md`).
