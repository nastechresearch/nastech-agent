...
def _handle_admitted_request(req: dict) -> dict | None:
    ...
    if not (fn := _methods.get(method)):
        return _err(rid, -32601, f"unknown method: {method} — the client and the Nastech backend are out of sync "
                    "(different versions); run `nastech update` and restart both")
