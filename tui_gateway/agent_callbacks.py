...
def _validate_personality(value: str, cfg: dict | None = None) -> tuple[str, str]:
    ...
    from nastech_cli.personality import normalize_personality_name, render_personality_prompt
...
def _prompt_text(value) -> str:
    """Normalize config prompt values from YAML for AIAgent (nastech_cli.personality owns this)."""
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    if env_val := os.environ.get("NASTECH_TUI_MAX_TURNS"):
...
def _load_fallback_model():
    ...
    NastechCLI/gateway: ``fallback_providers`` first, legacy ``fallback_model`` merged after)."""
