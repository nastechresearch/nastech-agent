...
def _relay_root() -> Path:
    ...
    every NASTECH_HOME, including non-``profiles/`` subdirs of ``~/.nastech``."""
    from tools.bot_mode_probe import _default_home, _nastech_root
    return _nastech_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery) -> dict:
    ...
    GATEWAY via the one-turn ``nastech -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "nastech" else profile
        ...
        owner_home = live_home if live_home is not None else Path(_nastech_home)
        ...
        fd, tmp = tempfile.mkstemp(prefix="nastech-relay-dm-", suffix=".txt", text=True)
        try:
