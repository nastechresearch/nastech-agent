...
from nastech_constants import (
    get_nastech_home, get_nastech_home_override, get_process_nastech_home, profile_name_for_home,
...
_nastech_home = _NASTECH_HOME_AT_IMPORT = get_nastech_home()
load_nastech_dotenv(nastech_home=_nastech_home, project_env=Path(__file__).parent.parent / ".env")
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("NASTECH_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("NASTECH_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("NASTECH_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("NASTECH_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "nastech_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("nastech_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve Nastech-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
        managed_bin = str(Path(get_nastech_home()) / "bin")
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Nastech agent → needs provider credentials. Tier-1 secrets
        ...
        # Prepend the Nastech venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve Nastech-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("NASTECH_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("NASTECH_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("NASTECH_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
def _launch_state_db_path() -> Path:
    """Launch profile's ``state.db`` at call time: the patched ``_nastech_home`` when a test changed
    it, else the live process home — resolved through :func:`get_process_nastech_home`, which honours
    ``NASTECH_HOME`` but ignores the context-local override. The desktop multiplex cron ticker sets
    ...
    import time lets a harness that redirects ``NASTECH_HOME`` after import be honoured (#112692)."""
    home = _nastech_home if _nastech_home != _NASTECH_HOME_AT_IMPORT else get_process_nastech_home()
...
def _profile_db(params: dict | None = None, *, writer: bool = False):
    ...
    nastech_cli.web_routers.profiles._read_profile_db."""
    ...
    if (profile_home := _profile_home(profile)) is None:
        ...
        try:
            if writer:
                ...
                from nastech_cli.web_server_sessions import _open_session_db_at_path
...
def _canonical_profile_request(name: str) -> str:
    ...
    installation details — unless a real named profile of that name exists (``nastech`` is a legal
    id), in which case it wins; other unknown names keep failing closed in ``_profile_home``.
    """
    if name.casefold() in {".nastech", "nastech"}:
        from nastech_cli import profiles as profiles_mod
        # Check the profiles root directly: get_profile_dir rejects "nastech" as a
...
# backend, so calls carry ``profile`` → open that profile's db and bind its NASTECH_HOME (ContextVar
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s full runtime scope (NASTECH_HOME + secrets + terminal policy) around a
...
def _bind_build_profile_scopes(profile_home: "str | None") -> "_TurnScopes | None":
    """Bind a session profile's NASTECH_HOME / secret / terminal scopes for an agent build. ``None`` is the
    ...
    if profile_home:  # secret/terminal helper failed: keep at least the home + terminal refusal scope
        scopes.home = set_nastech_home_override(profile_home)
        with contextlib.suppress(Exception):
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's NASTECH_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _nastech_home) / "config.yaml"
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess NASTECH_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(NASTECH_GATEWAY_SESSION="1", NASTECH_EXEC_ASK="1", NASTECH_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Nastech Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Nastech Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``nastech --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("NASTECH_MODEL", "") or os.environ.get("NASTECH_INFERENCE_MODEL", "")).strip()
...
def _resolve_session_platform() -> str:
    """``NASTECH_DESKTOP=1`` without ``NASTECH_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``nastech --tui``)."""
    desktop = is_truthy_value(os.environ.get("NASTECH_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("NASTECH_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the NASTECH_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("NASTECH_TUI_PROVIDER", "").strip():
    ...
    with contextlib.suppress(Exception):
        ...
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("NASTECH_INFERENCE_PROVIDER", "").strip().lower() or "auto")
        # Same owner as NastechCLI/oneshot: ``custom:<name>:<model>`` selects that provider (#73943).
...
from nastech_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from nastech_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    room bots "out of Nastech credits" after a profile switch); signals: ``room_plumbing`` /
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # persist). The full scope, not NASTECH_HOME alone: the external memory provider's system_prompt_block()
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`nastech_constants.resolve_reasoning_config` (per-model override >
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_NASTECH_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``NASTECH_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a NASTECH_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] NASTECH_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown NASTECH_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in NASTECH_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit NASTECH_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("NASTECH_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        if resolved is not False:
            return resolved
        fallback_notice = "[tui] no valid NASTECH_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from nastech_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid NASTECH_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on NASTECH_DEV_CREDITS so the payload stays clean otherwise.
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if provider == "custom" and "provider" not in mirror and agent is not None:
        ...
        with _profile_build_scope(sess.get("profile_home") or _nastech_home):
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + NASTECH_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `nastech skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("nastech_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("NASTECH_IGNORE_RULES"))
    with _sessions_lock:
        session = _sessions.get(sid)
    agent = AIAgent(
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped NASTECH_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str, *, model_history_only: bool = False) -> tuple[list, list, list]:
    ...
    from nastech_state import SessionResumeTooLargeError
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("NASTECH_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`nastech setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`nastech gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`nastech sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`nastech config edit` needs $EDITOR in a real terminal",
}
