...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    ``NASTECH_TUI_WS_ORPHAN_REAP_GRACE_S`` env var is kept as an internal
    override for backward compatibility and wins when set.
    """
    raw = os.environ.get("NASTECH_TUI_WS_ORPHAN_REAP_GRACE_S")
...
def _resolve_ws_orphan_activity_stale() -> float:
    ...
    ``NASTECH_TUI_WS_ORPHAN_ACTIVITY_STALE_S`` env var is an internal
    ...
    """
    raw = os.environ.get("NASTECH_TUI_WS_ORPHAN_ACTIVITY_STALE_S")
    if raw is None or not str(raw).strip():
        try:
            from nastech_cli.config import load_config
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar(
    "nastech_gateway_rpc_method", default=""
)
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    """Prepend Nastech' managed bin, the venv bin dir, and the user-local
    bin dir to PATH so slash_worker child processes can resolve
    Nastech-managed CLIs (browser-use, uvx, uv) even when the parent
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # Prepend the Nastech venv bin dir and the user-local bin dir to PATH so
        # slash_worker child processes can resolve Nastech-managed CLIs
...
_SESSION_OWNERSHIP_UNAVAILABLE = (
    "Nastech could not safely reserve this session. Try again."
)
        ...
        with _session_resume_lock:
            with _sessions_lock:
                ...
                if current.get("close_on_disconnect"):
                    ...
                    # `nastech --tui` keeps real _stdio. UNLESS another window
...
# A `nastech serve` killed mid-update used to lose every un-flushed in-memory
...
try:
    _EXIT_FLUSH_BUDGET_S = float(
        os.environ.get("NASTECH_TUI_EXIT_FLUSH_BUDGET_S") or 5.0
    )
except (TypeError, ValueError):
...
try:
    _INCREMENTAL_FLUSH_INTERVAL_S = float(
        os.environ.get("NASTECH_TUI_SESSION_FLUSH_INTERVAL_S") or _REAPER_SCAN_S
    )
except (TypeError, ValueError):
...
def _flush_sessions_before_exit(budget_s: float | None = None) -> int:
    ...
    can never block exit longer than ``NASTECH_TUI_EXIT_FLUSH_BUDGET_S``
    ...
    worker = threading.Thread(target=_run, daemon=True, name="nastech-exit-flush")
    worker.start()
    worker.join(budget)
...
def install_exit_flush_signal_handlers() -> bool:
    ...
    Called by ``nastech serve`` / dashboard startup before uvicorn takes over
...
def _sweep_orphaned_session_rows() -> list[str]:
    ...
    ago (``NASTECH_TUI_SESSION_TTL_S``). A freshly created row that copied
...
_HEARTBEAT_REFRESH_S = float(
    os.environ.get("NASTECH_GATEWAY_HEARTBEAT_REFRESH_S") or 60.0
)
...
def _start_backend_heartbeat_refresher() -> None:
    ...
    thread = threading.Thread(
        ...
        args=(stop_event,),
        name="nastech-gateway-heartbeat",
        daemon=True,
    )
    thread.start()
...
def _schedule_startup_orphan_sweep() -> None:
    ...
    reads the DB. ``NASTECH_TUI_WS_ORPHAN_REAP_GRACE_S=0`` (park forever)
    and ``NASTECH_TUI_SESSION_TTL_S=0`` both suppress the sweep; so does
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s NASTECH_HOME around a handler.

    Pets (config + sprites) and projects (projects.db, discovery policy) both
    resolve via ``get_nastech_home``. The desktop sends ``profile`` so a single
...
def _save_cfg(cfg: dict):
    ...
    override = get_nastech_home_override()
    home = Path(override) if isinstance(override, str) and override else _nastech_home
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps(
    {
        "success": False,
        "error": (
            "No Nastech Desktop window answered the tour request. The tour is "
            ...
            "nothing listening. Update the Nastech Desktop app and start a new "
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    # Nastech (or any earlier provider) long after the profile was switched —
    # every room message then failed with "out of Nastech credits" while the
    ...
    # provider (e.g. "out of Nastech credits" after the profile was switched to
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # Re-bind NASTECH_HOME to the session's profile so load_soul_md() and
    ...
    profile_home = session.get("profile_home")
    home_token = (
        set_nastech_home_override(profile_home) if profile_home else None
    )
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    ...
    env_val = os.environ.get("NASTECH_TUI_MAX_TURNS")
    if env_val:
...
def _maybe_schedule_auto_continue(sid: str, session: dict, session_key: str) -> dict | None:
    ...
    def kickoff() -> None:
        ...
        # backends share one NASTECH_HOME; B resumes S while A runs it and
...
def _finalize_superseded_runtimes(stale: list[tuple[str, dict]]) -> None:
    ...
    nastech_state_common._RECOVERABLE_END_REASONS so canonical Bot Chat
...
def _maybe_fire_tui_loop_tick(sid: str, session: dict) -> None:
    ...
    """
    try:
        from nastech_cli.loops import LoopManager, goal_blocks_loop_tick
    except Exception:
        return
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    zero Nastech sessions still appear in the sidebar (#81723). Mirrors the
        ...
        try:
            for dirpath, dirnames, _filenames in os.walk(root):
                ...
                if ".git" in dirnames:
                    ...
                    # Not a repo: skip hidden dirs (e.g. .nastech) and node_modules.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.active_sessions import PER_SESSION_EXCLUSIVE_SUBMIT
