...
class _TurnScopes:
    ...
    home: Any = None  # per-turn NASTECH_HOME override for a resumed remote profile
    secret: Any = None
    terminal: Any = None
...
def _finish_turn(sid: str, session: dict, st: _TurnRun) -> None:
    ...
    if isinstance(run_kwargs, dict):
        run_kwargs.clear()
    try:  # while the profile NASTECH_HOME override is still active (session's own config)
    ...
    if scopes.home is not None:
        reset_nastech_home_override(scopes.home)
