...
# Stop a ``utils/`` (or ``proxy/``, ``ui/``) package in the launch directory from shadowing Nastech's own
...
# retry loop. ``nastech_bootstrap`` lives at the repo root (no collision risk), so importing it first is safe.
# ``nastech_bootstrap`` lives at the repo root, so importing it is safe before the guard runs (its name won't
...
_WATCHDOG_POLL_S = max(0.05, env_float("NASTECH_SLASH_WATCHDOG_POLL_S", 2.0))
_ORPHAN_GRACE_S = max(0.0, env_float("NASTECH_SLASH_WATCHDOG_GRACE_S", 5.0))
...
def _prepare_slash_worker_runtime() -> None:
    """Start bounded MCP discovery before NastechCLI snapshots tools: each slash_worker child is its
    own process — the parent ``nastech serve`` discovery thread does not populate this registry.

    See #61891.
    """
    from nastech_cli.mcp_startup import start_background_mcp_discovery, wait_for_mcp_discovery
...
def main():
    ...
    # Start before the (hundreds-of-ms) NastechCLI build — that window is itself an orphan risk if the
