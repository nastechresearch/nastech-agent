...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        _enable_gateway_prompts()
        home_token = (
            set_nastech_home_override(str(profile_home)) if profile_home is not None else None
        )
        secret_token = (
        ...
        with _session_resume_lock:
            ...
            try:
                init_home_token = (
                    set_nastech_home_override(str(profile_home))
                    if profile_home is not None
                    else None
                )
                init_secret_token = (
                ...
                try:
                    ...
                    if init_home_token is not None:
                        reset_nastech_home_override(init_home_token)
        ...
        # atexit.register(_drain_token_queue_at_exit) (nastech_state.py), which only
...
def _(rid, params: dict) -> dict:
    ...
    # ``nastech --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``nastech gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Nastech credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into Nastech
    # or on any portal hiccup.
    try:
        from agent.account_usage import nastech_credits_lines

        credits = nastech_credits_lines()
        if credits:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from nastech_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from nastech_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from nastech_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from nastech_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Nastech Portal / OpenRouter /
...
def _(rid, params: dict) -> dict:
    ...
    from nastech_cli.nastech_billing import BillingError, post_subscription_preview
...
def _(rid, params: dict) -> dict:
    ...
    billing:manage.
    """
    from nastech_cli.nastech_billing import BillingError, put_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.nastech_billing import BillingError, delete_subscription_pending_change
...
def _(rid, params: dict) -> dict:
    ...
    from nastech_cli.nastech_billing import BillingError, post_subscription_upgrade
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.nastech_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from nastech_cli.nastech_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from nastech_cli.nastech_billing import BillingError
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Nastech profile home
    # (~/.nastech/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if parent_home:
            ...
            branch_owns_db = True
        home_token = (
            set_nastech_home_override(parent_home) if parent_home else None
        )
