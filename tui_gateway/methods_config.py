...
from nastech_constants import display_nastech_home as _display_nastech_home
...
def _cfg_get_provider(params):
    from nastech_cli.models import list_available_providers, normalize_provider
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_nastech_home), "display": _display_nastech_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's NASTECH_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``NASTECH_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    added after boot — the Models page, a picker key, ``nastech setup`` from a shell — flips it
    ...
    try:
        ...
        from nastech_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Nastech provider is configured.", source)
            ...
            # free_tier is keyed on the SELECTED route (the welcome host serves only nastech/welcome), not
            # on profile state: a paid Nastech key beside a free-tier identity must not read as free.
            return {"ok": True, "provider": runtime.get("provider"), "model": model,
                    "source": runtime.get("source"),
                    "free_tier": provider == "nastech" and route_is_welcome_host(runtime.get("base_url")),
                    **scoped}
        return _readiness_check(rid, params, probe)
    except Exception as e:
...
@method("diagnostics.share_nastech")
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Nastech-internal diagnostics storage — same collection +
    force-redaction pipeline as ``nastech debug share --nastech``; redaction is NOT client-controllable
    ...
    try:
        from nastech_cli.debug import _redact_log_text, build_nastech_bundle, collect_share_bundle
        ...
        res = share_to_nastech(build_nastech_bundle(bundle, redact=True))
