    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Nastech sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_nastech_home), "display": display_nastech_home()})
    if key == "project":
...
def _readiness_profile_scope(params: dict):
    ...
    that profile's NASTECH_HOME and ``.env`` secret scope (ContextVars, so
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Nastech provider is configured.",
                    **scoped,
                },
            )
...
def _(rid, params: dict) -> dict:
    """Upload a redacted debug bundle to Nastech-internal diagnostics storage.

    Desktop's "Send Diagnostics" action (error card / diagnostics UI). Same
    collection + force-redaction pipeline as ``nastech debug share --nastech``
    (collect_share_bundle → build_nastech_bundle → share_to_nastech); redaction is
    ...
    try:
        ...
        blob = build_nastech_bundle(bundle, redact=True)
