"""Launch-profile policy for a process that hosts several profile homes (``nastech serve`` /
``nastech dashboard`` pooling, ``?profile=``, hosted rooms; the multiplexed gateway's own worker).
...
def launch_profile_runtime_scope(launch_home: "str | Path") -> Iterator[None]:
    ...
    terminal policy over the frozen launch ``TERMINAL_*`` overlay. No NASTECH_HOME override — the
