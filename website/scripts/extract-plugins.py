...
- ``plugin-catalog.json`` — ``{"entries": [raw YAML mappings], "removed": [...]}`` in the loader's own
  schema; installed Nastech clients fetch this for live catalog refresh
  (``nastech_cli.plugin_catalog.LIVE_CATALOG_URL``) so new entries and removals reach them without
...
def load_catalog_entries(catalog_dir: Path, stars: dict[str, int] | None = None) -> list[dict]:
    ...
    for path in sorted(catalog_dir.glob("*.yaml")):
        ...
        entries.append({
            ...
            "requiresNastech": str(raw.get("requires_nastech") or "").strip(),
            ...
            "installCommand": f"nastech plugins install {name}",
