...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NastechResearch/nastech-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    nastech_meta = (fm.get("metadata") or {}).get("nastech") or {}
    tags = nastech_meta.get("tags") or []
    related = nastech_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `nastech skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
