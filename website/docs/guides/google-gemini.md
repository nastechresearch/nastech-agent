...
doesn't end with an API version segment (`v1beta`, `v1alpha`, `v1`, ...), Nastech
...
`aiplatform.googleapis.com` (they get 403 on the AI Studio host). Nastech detects the
...
Nastech completes it to the `publishers/google` form; a base URL on any other host (a
...
Google publishes moving aliases for the Pro and Flash Gemini families. `gemini-pro-latest` and `gemini-flash-latest` are useful when you want Google to advance the model automatically without changing your Nastech config. Note that your usage charges may be affected if newer models introduce different rates.
