...
Top-level command for managing profiles. Running `nastech profile` without a subcommand shows help.
...
Sets `<name>` as the active profile. All subsequent `nastech` commands (without `-p`) will use this profile.
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `nastech profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `nastech update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.nastech`), use `nastech skills opt-out` / `nastech skills opt-in`. |
...
after the rename; `nastech profile migrate-identity` retries that step too if it was reported as
failed.

## `nastech profile migrate-identity`

```bash
nastech profile migrate-identity <old-name> <new-name>
```

Retries the identity migration of a rename that already completed. Run it if `nastech profile
...
#     nastech profile migrate-identity mybot assistant

nastech profile migrate-identity mybot assistant
# ✓ Session/routing identity migrated: mybot → assistant
```

## `nastech profile purge-identity`

```bash
nastech profile purge-identity <name>
```

Retries the identity purge of a delete that already completed. Run it if `nastech profile delete`
...
is decided by `nastech profile delete`, which removes the profile's own `profiles/<name>/`, its
...
#   Restart the gateway, then run:
#     nastech profile purge-identity mybot
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `nastech -p telemetry`). |
...
`nastech profile list` also shows the distribution name and version in a
`Distribution` column, and `nastech profile show <name>` / `delete <name>`
...
- `nastech profile <TAB>` — subcommands (list, use, create, etc.)
- `nastech profile use <TAB>` — profile names
- `nastech -p <TAB>` — profile names
