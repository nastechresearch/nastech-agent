...
# Set a valid model
nastech config set NASTECH_MODEL anthropic/claude-opus-4.7
...
There is no hard limit. Each profile is just a directory under `~/.nastech/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
