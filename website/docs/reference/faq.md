...
Yes. Run `nastech model`, select **Custom endpoint**, and enter your server's URL:
...
#### `nastech: command not found` after installation
...
Independently of the init files, every terminal command's `PATH` is completed with the standard system directories (`/usr/local/bin`, `/opt/homebrew/bin`, …), the Nastech-managed runtime dirs, and `~/.local/bin` when it exists (the `pip --user` / `pipx` / `uv tool` install target) — appended after your own entries, so precedence is unchanged. This covers backends started with a thin non-interactive PATH (systemd, GUI launchers, the Desktop SSH remote backend) without any configuration.
...
#### The agent says "Nastech policy" or "Nastech guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Nastech runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Nastech enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Nastech does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.

To isolate the source:

1. Run `/status` to confirm the active model and provider.
2. Check whether the refusal includes an actual Nastech tool error or approval prompt. If it is prose only, do not treat the model's attribution as runtime evidence.
...
After adding a new provider via `nastech model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `nastech model` (from terminal) |
| Enter/change API keys | `nastech model` (from terminal) |
...
# Or set directly
nastech config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.nastech/.env` for conflicting entries.
:::
...
# Set a valid model
nastech config set model.default anthropic/claude-opus-4.7

# Or specify per-session
nastech chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `nastech chat --provider <alternative>` to route to a different backend
...
- Or switch to the terminal interface for administrative tasks: `nastech chat`
...
cat ~/.nastech/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `nastech gateway setup`
- Check gateway logs: `cat ~/.nastech/logs/gateway.log | tail -50`
...
Configure in `~/.nastech/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
#### WSL: Gateway keeps disconnecting or `nastech gateway start` fails
...
tmux new -s nastech 'nastech gateway run'
# Reattach later: tmux attach -t nastech

# Option 3: Background via nohup
nohup nastech gateway run > ~/.nastech/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'nastech gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `nastech gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
nastech gateway install    # Re-snapshots your current PATH
nastech gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.nastech.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `nastech chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `nastech chat -t "terminal"`
...
There is no hard limit. Each profile is a directory under `~/.nastech/profiles/` that carries at least one identity file (`config.yaml`, `.env`, `SOUL.md`, `profile.yaml`, `auth.json` or `state.db`); a bare directory without one (a leftover from a log rotation or cron tick) is not a profile — it is not listed or served, `-p <name>` reports it as missing, and `nastech profile create <name>` refuses to overwrite it until you move or remove it. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `nastech skills config` settings don't seem to take effect.

**Solution:** Use `nastech skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`nastech gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
3. Copy the zip to the new machine and import it:
   ...
   scp ~/nastech-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   nastech import ~/nastech-backup-<timestamp>.zip
   ```

4. On the new machine, run `nastech setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
nastech profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
nastech profile import ./work-backup.tar.gz work
```
...
nastech config show | head -20

# Re-run model selection
nastech model

# Or test with a known-good model
nastech chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NastechResearch/nastech-agent/issues)
2. **Ask the community:** [Nastech Research Discord](https://discord.gg/nastechresearch)
