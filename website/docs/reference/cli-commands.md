...
| `nastech pause` / `nastech resume` | Global emergency stop: no new cron fires (built-in ticker, managed-cron webhook, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
...
nastech chat -q "Inspect this repository" --format stream-json
...
```bash
nastech chat -q "Summarize this repository" --format stream-json
```
...
(`NASTECH_KANBAN_TASK` set) whose turn failed only because the provider was
...
`nastech -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, the `completed` / `failed` / `partial` / `interrupted` flags and `turn_exit_reason` (why `completed` is false, e.g. `max_iterations_reached(3/3)`). Those top-level counters cover the **main agent loop** only. Auxiliary LLM calls made on the same run (title generation, vision, context compression, `web_extract`, background review, …) are reported separately under `auxiliary` — the same totals plus a per-task `by_task` map — and `total_including_auxiliary` (`estimated_cost_usd`, `total_tokens`, `api_calls`) is the grand total to bill on. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Nastech Relay](../user-guide/messaging/relay.md). |
...
`nastech gateway restart`, or service-restart update exits with status `75`
...
:::tip WSL users
Use `nastech gateway run` instead of `nastech gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s nastech 'nastech gateway run'`. See [WSL FAQ](./faq.md#wsl-gateway-keeps-disconnecting-or-nastech-gateway-start-fails) for details.
:::
...
nastech auth add openrouter --type oauth                  # Browser login (OpenRouter PKCE) mints a key for you
nastech auth add anthropic --type oauth                   # Add OAuth credential
nastech auth add openai-codex --type oauth --priority 0   # Add an account and try it first
nastech auth remove openrouter 2                          # Remove by index
nastech auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
nastech auth reset openrouter                             # Clear cooldowns
nastech auth reset openrouter 2                           # Clear the cooldown on one credential
nastech auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
...
Subscriptions persist to `~/.nastech/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart. Re-running `subscribe` for an existing name keeps its secret and profile binding unless you pass `--secret` / `--route-profile`.
...
| `-k`, `--keep <N>` | After a full backup, delete older `nastech-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |
...
- `models/`, `runtimes/`, `node/` at the root of `~/.nastech` (and of each `profiles/<name>/`) — regenerable runtime downloads, often tens of GB. Deeper directories with the same names (a skill's `models/`) are kept.
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Nastech plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance in a `.nastech-catalog.json` sidecar. Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist. |
| `search [term] [--json]` | Search the Nastech plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
nastech profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
