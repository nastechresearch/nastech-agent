...
Behavior:
- Nastech uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, client identification, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.nastech/mcp-tokens/<server>.json` (a named profile uses `~/.nastech/profiles/<name>/mcp-tokens/`) and reused across sessions
...
device authorization from a terminal on the machine running Nastech:

```bash
nastech mcp login protected_api --flow device
```

Open the printed verification URL on any device and enter the displayed user code.
Nastech polls for approval, respects `authorization_pending` and `slow_down`, and stops
...
Set `oauth.flow: device` on the server to make `nastech mcp login` and `nastech mcp reauth`
...
Nastech identifies itself to authorization servers with a **Client ID Metadata Document** (CIMD), the mechanism the MCP `2026-07-28` spec adopted in place of Dynamic Client Registration. The document is published at
`https://nastechresearch.github.io/nastech-agent/docs/oauth/client-metadata.json`, and that URL *is* the `client_id` — the authorization server fetches it to learn Nastech' name, logo, and permitted redirect URIs. Nothing is registered per install, and nothing is user-specific.
...
The document declares a fixed set of loopback redirect URIs, and the spec requires the redirect URI in an authorization request to be an *exact string match* against one of them — so a CIMD flow cannot use the random high port Nastech normally picks. Nastech therefore pins the callback to one of ports `27890`–`27894`.

That pin has to be chosen before the server's capabilities are known, because the redirect URI is fixed at the start of the flow while the server's metadata only arrives partway through. So Nastech pins the port for any flow that *could* end up using CIMD, and reverts to a random port for the rest:

- A server Nastech has connected to before, whose cached metadata does not advertise CIMD, keeps the random port it has always used.
- A server Nastech has never reached gets a pinned port on that first login, since guessing is the only way CIMD can ever be used.
...
If a server fetches the document and refuses it at the *token* endpoint (`invalid_client`), Nastech logs the rejection, records it under `~/.nastech/mcp-tokens/<server>.cimd-off`, and uses DCR for that server from then on.

A server that cannot fetch or validate the document at all aborts at the *authorization* endpoint instead, before any redirect happens. There is no signal Nastech can observe there, so the browser shows an invalid-client error and the login times out after five minutes. The timeout message names the document and points at `cimd: false`. Running `nastech mcp login <server>` clears the recorded rejection, so a corrected document gets another chance.
...
OAuth discovery and dynamic-client-registration requests (the `/.well-known/...` metadata documents and the `registration_endpoint` POST) always carry `User-Agent: Nastech-Agent/<version>`. The MCP SDK builds those requests without any client default headers, and WAF-fronted authorization servers answer a header-less request with `403` — the metadata document then looks unreadable, registration falls back to a guessed `/register` on the MCP host, and the login fails with `Registration failed: 404`. When every metadata fetch does fail, the error now leads with those statuses (`Could not read authorization-server metadata (403 from https://…/.well-known/oauth-authorization-server; …)`) before the registration fallback's own error.

## Add to Nastech link

MCP vendors and docs can offer a one-click **"Add to Nastech"** button that opens the Nastech desktop app with a pre-filled server config, mirroring Cursor's `cursor://anysphere.cursor-deeplink/mcp/install` scheme:

```text
nastech://mcp/install?name=NAME&config=BASE64
```
...
const link = `nastech://mcp/install?name=example&config=${btoa(JSON.stringify(config))
