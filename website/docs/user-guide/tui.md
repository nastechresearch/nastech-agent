...
- **Spawned gateway (default):** when the gateway process dies mid-session the TUI says *Nastech stopped unexpectedly — restarting and reopening your chat*, respawns it (bounded to a few attempts per minute) and reopens the same saved session. The reply that was in flight is lost with the process.
- **Attached gateway (dashboard chat):** when only the WebSocket drops the TUI says *Connection to Nastech lost — reconnecting and reopening your chat…*, reconnects with growing backoff and reattaches to the same session — including a reply that is still streaming on the backend. Nothing is resubmitted.
