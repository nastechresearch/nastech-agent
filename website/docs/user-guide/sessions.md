...
| `oneshot` | Finite non-interactive runs: `nastech chat --oneshot -q`, `-Q`, `nastech -z`, and `-q` on non-TTY stdio. Hidden from the TUI, Desktop and dashboard session pickers (like `kanban` and `tool`), even when launched from inside a TUI or Desktop session — the run inherits that transport's environment but is not that conversation. Still counts as CLI history: `nastech -c` / `--resume latest` continue the last one-shot, and `nastech sessions list` shows it. An explicit `--source <tag>` always wins (`nastech chat -q --source tui` is stored as `tui`). |
...
# ~/exports/nastech_session_20250305_091523_a1b2c3d4.jsonl
nastech sessions export ~/exports/ --session-id 20250305_091523_a1b2c3d4
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`nastech insights`](../reference/cli-commands.md#nastech-insights).
...
profile. Current versions put new state in the right place; `nastech sessions
repair-profiles` settles what is already crossed.

```bash
# Report only — every store is scanned, nothing is written
nastech sessions repair-profiles

# Perform the repairs (stop the gateway first; a snapshot of every store is taken)
nastech sessions repair-profiles --apply

# Machine-readable report
nastech sessions repair-profiles --json
```
...
`nastech profile migrate-identity <old> <new>`), and `agent:main:…` rows inside
...
Started a conversation in another agent CLI? You can pull it into Nastech and
continue it here. Nastech reads Claude Code's session logs
...
nastech sessions import

# Limit to one tool, or point at a specific file
nastech sessions import --from claude
nastech sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl

# Import-and-resume in one step
nastech --resume @claude
nastech --resume @codex
```

`nastech sessions import` creates a new Nastech session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `nastech --resume <id>` command.
`--resume @claude` / `--resume @codex` show the same picker and drop you
straight into the imported conversation.

**Nastech Desktop** has the same importer in the command palette (**Import
session**). It lists the logs on the machine the
connected backend runs on — not the computer running the app — shows a
read-only preview, and **Continue in Nastech** copies the conversation into the
...
Without pruning, `state.db` grows without bound — multi-GB files within weeks were reported on gateway + cron installs. If you would rather keep every ended session forever (the pre-#54189 behavior), turn it off in `~/.nastech/config.yaml`:
...
`nastech sessions recover` synthesizes for orphaned messages) whose last
...
:::tip
Auto-prune is **on by default**: ended sessions that have been inactive for `sessions.retention_days` (default 90) are removed at startup, and active sessions are never touched (see [Automatic Cleanup](#automatic-cleanup) above). Session history powers `session_search` recall across past conversations, so if you want to keep every ended session forever, set `sessions.auto_prune: false` in `config.yaml`, or raise `retention_days`. With auto-prune off, `nastech sessions prune` remains available for one-off cleanup (observed failure mode without any pruning: a 384 MB `state.db` with ~1000 sessions slowing down FTS5 inserts and `/resume` listing).
:::
