...
Under local WSLg, Nastech launches with `--ozone-platform=wayland` to avoid the XWayland maximized-window offset and shifted mouse hit-testing ([microsoft/wslg#1015](https://github.com/microsoft/wslg/issues/1015)). The platform must be selected at process launch, before Electron loads application JavaScript. Explicit `--ozone-platform=x11` and `desktop.ozone_platform_hint: x11` remain available. The app draws its own minimize, maximize and close controls on WSLg.

When `nastech gui` runs inside WSL2 with `/dev/dxg` present and Mesa's `d3d12_dri.so` installed, the launcher sets `GALLIUM_DRIVER=d3d12` for Electron so rendering uses the Windows GPU instead of the llvmpipe software rasterizer; an explicit `GALLIUM_DRIVER`, `MESA_LOADER_DRIVER_OVERRIDE`, `LIBGL_ALWAYS_SOFTWARE`, or `LIBGL_DRIVERS_PATH` in your environment is left untouched (for example `GALLIUM_DRIVER=llvmpipe nastech gui` keeps software rendering).
...
- **Messaging** — set up gateway channels. Telegram has a **Quick setup** card: click **Create with QR**, scan the code (or open the link) in Telegram, and Nastech creates the bot, detects your user ID for the allowlist, saves the credentials, and restarts the gateway for you. Any credential save, clear, or enable toggle keeps a **Restart now** banner on the page until the gateway has actually restarted; if a restart fails, the banner stays so you can retry or restart manually.
...
|---|---|---|
| Skills | [`/docs/api/skills.json`](https://nastechresearch.github.io/nastech-agent/docs/api/skills.json) | `https://nastechresearch.github.io/nastech-agent/docs/api/skills.json` |
| Plugins | [`/docs/api/plugins.json`](https://nastechresearch.github.io/nastech-agent/docs/api/plugins.json) | `https://nastechresearch.github.io/nastech-agent/docs/api/plugins.json` |
...
The public hubs' **Install in Nastech** buttons open `nastech://skill/install`
or `nastech://plugin/install` links and require confirmation in Desktop. Use
...
roster where every [Nastech profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Nastech cron). The roster lives in the left
...
each other's Bot Chats directly (`nastech -p <bot> chat`). The backend teaches
...
The desktop app and the Nastech backend it talks to update on separate clocks — the app package on your machine, the backend wherever it runs. When more than one update target exists (a remote gateway, or several registered gateways), the update affordances (**Update now** on the About panel, the ⌘K **Update Nastech** row, and the update-ready toast) update **everything**: the connected backend first, then every other eligible registered gateway (Nastech Cloud entries are platform-managed and skipped), and the desktop app itself last, since applying the client update relaunches the app. Single-machine installs keep the one-button experience.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every Nastech gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Nastech Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Nastech gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Nastech Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the Nastech Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `nastech update` to every eligible gateway at once — Nastech Cloud entries are skipped (the platform updates them), and each instance reports its own result.
...
You can also set the backend URL without the UI via the `NASTECH_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **Desktop column** — the half loaded into this app. It is app-level: the
  ...
  `~/.nastech/desktop-plugins/`; the desktop half of a unified agent+desktop
...
- A half the plugin does not ship shows a dash. A desktop half whose agent
  ...
  [Accent Picker](https://github.com/NastechResearch/nastech-desktop-accent-picker)
...
If the local Nastech backend process exits after it was ready, Desktop restarts it on its own and shows a **Nastech stopped working in the background** notice; the chat reconnects once the replacement is up. `NASTECH_HOME/logs/desktop.log` records the exit code together with the backend's last output lines (`Nastech backend exited (1)` followed by `Recent backend output:`), so the reason it died is in the log even when the app recovered by itself. A backend that keeps dying within seconds of every restart points at the backend itself — look at the traceback in that tail. After three such restarts within two minutes Desktop stops respawning and shows a **keeps crashing** notice instead of cycling; relaunch the app once the cause is fixed.
...
- **Open logs** — opens `NASTECH_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nastech-internal
  storage after an explicit consent prompt (same pipeline as
  `nastech debug share --nastech`; secrets are always redacted, the bundle is
  viewable by Nastech staff only and auto-deletes after 14 days). On success you
  get a private view link to paste into your support thread, plus quick links
  to GitHub Issues, Nastech Portal Support, and Discord. On a remote or Cloud
...
**Other native downloads (e.g. the `get-windows` prebuilt on Windows) that need a mirror:** put the npm keys in `$NASTECH_HOME/npmrc` (`%LOCALAPPDATA%\nastech\npmrc` on Windows, `~/.nastech/npmrc` elsewhere) — for example `node_get_windows_binary_host_mirror=https://<mirror>/sindresorhus/get-windows/releases/download/`. Every `npm ci`/`npm run` the updater spawns (desktop, web and TUI builds) points `NPM_CONFIG_USERCONFIG` at that file when it exists, so the config survives `nastech update`; the repo-root `.npmrc` is git-tracked and gets autostashed on every update, and `~/.npmrc` may be missed because the desktop hand-off inherits the GUI's environment. An `NPM_CONFIG_USERCONFIG` you set yourself is never overridden.
...
(Desktop, then Downloads, then Documents, ...) as Nastech touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Nastech' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Nastech.app** if you use Desktop.
3. Fully quit and relaunch them once.

`nastech doctor` reports whether the current terminal context already has the
grant, and `nastech setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.nastechresearch.nastech   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
Nastech. Grants are stable from then on.
...
certificate once and tell Nastech to use it. The one-shot command does
...
```bash
nastech desktop --setup-tcc-identity
```
...
4. `nastech config set desktop.macos_signing_identity "Nastech Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Nastech Local Signing`). The command is idempotent —
