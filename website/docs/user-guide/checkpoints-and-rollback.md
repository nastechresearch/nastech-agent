...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.nastech/checkpoints/ in the
  ...
  # cleanup is only ever done via the explicit `nastech checkpoints prune`
...
`/rollback <N>` restores only the files Nastech itself changed. Every successful
...
Nastech last wrote (you edited it afterwards, or Nastech never touched it) is
...
If the ledger is empty (a store created before this feature, or Nastech hasn't
