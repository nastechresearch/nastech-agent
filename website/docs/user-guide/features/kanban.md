...
- **Project** (in both modals) — binds the board to a Nastech Project
...
- **Download / remove** — the drawer lists each attachment with a download
  ...
  last reference is removed). From the CLI, `nastech kanban attach-rm
...
its own transient scope (`systemd-run --user --scope --unit nastech-worker-kanban-<task>-run-<run>`)
...
- **`nastech kanban dispatch` from your own unit** (a `Type=oneshot` timer, a
...
Mounting one `kanban.db` in several Nastech homes (containers, fleet hosts) shares
...
```bash
nastech -p planner tools enable kanban                      # CLI / TUI / Desktop chats
nastech -p planner tools enable kanban --platform telegram  # a gateway platform
```

Each platform has its own selection under `platform_toolsets.<platform>` in
`config.yaml`; the toolset is also a checkbox in `nastech tools` and the dashboard.
...
The auto-injected kanban guidance teaches the model which tool to call when and in what order. It is injected only for the dispatcher-spawned worker that owns the task: an interactive session that merely has the `kanban` toolset enabled keeps the tools but is not told it "has been assigned ONE task", and a `delegate_task` child or a cron run fired from inside a worker — which inherit the worker's `NASTECH_KANBAN_TASK` — neither receives the worker protocol nor records a terminal outcome against the worker's card when its own turn budget runs out.
...
revoked key parks the card exactly like an implementer. `nastech kanban show`
surfaces it as *Provider rejected this profile's credential or model — blocked
after one attempt*; fix the assignee profile's provider (`nastech -p <profile>
auth` / `setup`), then `nastech kanban unblock <id>`. The worker also writes its exit code as the
last line of its own log (`[kanban-worker-exit] rc=<code>`), so a per-tick
`nastech kanban dispatch` process — which never reaped the worker and cannot
...
guard is active only for the dispatcher-spawned worker itself (`NASTECH_KANBAN_TASK` is
...
and can be disabled with `NASTECH_KANBAN_STOP_NUDGE=0`.
...
breaker block) until `nastech kanban unblock <id>`, which also grants a fresh
...
**Worker session names.** A worker's session is titled after its card (`Fix the swap modal`, or `Kanban task <id>` when the board row can't be read) at spawn, so `nastech sessions` and session search show the card, not a model's guess. Workers never make the auxiliary `title_generation` model call that names interactive sessions; a manual `/title` in a worker session still wins.
...
A goal-mode worker's **Worker log** (dashboard drawer, `nastech kanban log <id>`) carries the same live tool feed as any other worker's, plus one `kanban goal loop: turn N/M verdict=…` line per judged turn, so you can follow what the loop is doing while the card is running.
...
nastech kanban complete <id>... [--result "..."] [--force]
...
Dispatcher workers creating tasks through `kanban_create` or `nastech kanban create`
...
[`agent-merge-conflict-arbiter` optional skill](https://github.com/NastechResearch/nastech-agent/blob/main/optional-skills/autonomous-ai-agents/agent-merge-conflict-arbiter/SKILL.md)
...
**Dependency refusal on complete names the parents.** `kanban_complete` / `nastech kanban complete` / the dashboard's "mark done" and "request review" actions (single and bulk) on a card whose direct parent is not `done`/`archived` (a parent reopened mid-run, or an edge that predates the running-child refusal) reports `unsatisfied parent dependencies: t_… (todo)` instead of the generic "unknown id, stale run, or already terminal" text; the card stays in-flight. `kanban_show` lists the same parents under `unsatisfied_parents`, and `nastech kanban show` / `nastech kanban diagnostics` / the dashboard raise a `running_with_open_parents` warning on a running card in that state. Finish the parent or `nastech kanban unlink <parent> <child>`; there is no force path through the dependency gate.

**Live-claim guard on complete.** A `running` task whose worker holds a live claim is only completed by that worker (`kanban_complete` from inside the run) or by an explicit operator override: `nastech kanban complete <id> --force` and the dashboard's "mark done" action. A claim-less `nastech kanban complete <id>` or an orchestrator session's `kanban_complete` is refused with a pointer to `--force` / `nastech kanban reclaim`, so a second session can no longer close a live worker's run underneath it. Completing `ready`, `blocked` or `review` cards without a claim is unchanged.
