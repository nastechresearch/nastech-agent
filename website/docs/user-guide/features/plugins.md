...
[Adding Tools](../../developer-guide/adding-tools.md) page is for built-in Nastech
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Nastech Plugin](../../developer-guide/plugins/index.md)** — step-by-step guide with a complete working example.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Nastech — see [Built-in Plugins](./built-in-plugins.md) |
...
| Nix | `services.nastech-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](../../getting-started/nix-setup.md#plugins) |
...
```bash
nastech plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Nastech checks out the commit detached, verifies that `HEAD` exactly matches the
...
The same pin is available in Nastech Desktop: **Skills → Plugins → Install from
...
can confirm everyone is running the same commit. `nastech plugins list` prints
...
its own. Every clone, pinned `--ref` fetch and `nastech plugins update` pull is
...
refuses anonymous access does Nastech look for a credential. For an `https://`
...
Nastech tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Nastech runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Nastech never
materializes the prompt on another surface.

Nastech still owns hardline blocks, sudo-stdin protection, user deny rules,
...
Plugins can register the 27 lifecycle events currently accepted by `nastech_cli.plugins.VALID_HOOKS`. The **[Event Hooks catalog](./hooks.md#shipped-plugin-hook-catalog)** is canonical for exact timing, return handling, payload fields, and privacy notes.
...
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Nastech Plugin](../../developer-guide/plugins/index.md) · [Adding Tools](../../developer-guide/adding-tools.md) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](./hooks.md) · [Build a Nastech Plugin](../../developer-guide/plugins/index.md) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Nastech Plugin](../../developer-guide/plugins/index.md) · [Extending the CLI](../../developer-guide/extending-the-cli.md) |
| A **subcommand** for `nastech <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](../../developer-guide/extending-the-cli.md) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Nastech auto-discovers the server's tools and registers them alongside built-ins. | [MCP](./mcp.md) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `nastech skills tap add <repo>` | [Skills Hub](./skills.md#skills-hub) · [Publishing a custom tap](./skills.md#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.nastech/hooks/<name>/` | [Event Hooks](./hooks.md#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `nastech plugins install` needed. See the **[Nix Setup guide](../../getting-started/nix-setup.md#plugins)** for full details.
...
```html
<a href="nastech://plugin/install?repo=owner/repo&enable=1">Install in Nastech</a>
```

MCP servers have the equivalent link form — see
[Add to Nastech link](../../reference/mcp-config-reference.md#add-to-nastech-link).
...
When a plugin declares capabilities, `nastech plugins install` (and
`nastech plugins enable`) shows the list with one-line risk descriptions and
...
`nastech plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
nastech plugins capabilities             # all plugins with declared/granted capabilities
nastech plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Nastech has not reviewed the plugin's code. Only install
...
A **plugin pack** is a declarative, shareable YAML file (`nastech-pack.yaml`)
...
config:                                  # optional, non-secret seeds only
  nastech-media-studio:
    default_model: flux-3
skills: []                               # declared list only (not auto-installed yet)
```

```bash
nastech plugins pack show ./nastech-pack.yaml     # dry-run review
nastech plugins pack install ./nastech-pack.yaml  # review → confirm → install
nastech plugins pack export > nastech-pack.yaml   # snapshot the current install
nastech plugins pack export --enabled-only       # only plugins.enabled
```
...
capability-consent prompt — identical to a single `nastech plugins install`.
...
provenance (installed via `nastech plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`nastech skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `nastech plugins install` and `nastech plugins update` runs a static
...
- Nastech reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Nastech rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Nastech can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
:::warning
Only grant gateway injection to plugins you trust. Nastech checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Nastech' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
