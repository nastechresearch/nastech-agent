...
description: "Browse and install reviewed, SHA-pinned Nastech plugins from the curated catalog"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Nastech plugins you
can install by name with a single command:

```bash
nastech plugins install <name>
```
...
**Install in Nastech** buttons and copyable CLI commands for every entry.
...
[`https://nastechresearch.github.io/nastech-agent/docs/api/plugins.json`](https://nastechresearch.github.io/nastech-agent/docs/api/plugins.json).
Desktop fetches it through
`https://nastechresearch.github.io/nastech-agent/docs/api/plugins.json`; the public
...
[`plugin-catalog/`](https://github.com/NastechResearch/nastech-agent/tree/main/plugin-catalog)
directory of the nastech-agent repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `nastech plugins install` |
...
| `tier` | `official` (maintained by NastechResearch) or `community` |
...
| `requires_nastech` | Minimum Nastech version, e.g. `>=0.19` (optional) |
...
- **Scanned at admission, trusted at install.** Admission CI runs the same
  security scanner the installer runs (`nastech plugins validate` includes a
...
On the website, **Install in Nastech** opens a protocol link of this form:
...
Skills Hub's new `nastech://skill/install?identifier=...` route). Older builds
...
The catalog name and the plugin's own manifest name can differ; `nastech
...
```bash
nastech plugins install touchdesigner
nastech plugins enable td
```
...
skill, so one install gives Nastech code, dependency, container and IaC scanning
plus the workflow for using it; the catalog name and manifest name match:

```bash
nastech plugins install snyk
nastech plugins enable snyk
```

### Updating a catalog install

`nastech plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `nastech plugins list` shows catalog
...
updating Nastech. Offline, the copy shipped with your checkout is used (a failed
...
`nastech plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/NastechResearch/nastech-agent/tree/main/plugin-catalog);
...
`nastech plugins list --json` reports `update_available`, the Desktop Plugins
tab shows an **Update to 1.4.0** button, and `nastech plugins update <name>`
...
- [Plugins](plugins.md) — the plugin system itself: manifest format, enabling,
  configuration
- [Built-in Plugins](built-in-plugins.md) — plugins that ship with Nastech
- [Build a Nastech Plugin](../../developer-guide/plugins/index.md) — write your own
