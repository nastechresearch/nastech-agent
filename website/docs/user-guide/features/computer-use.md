...
| Nastech session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Nastech approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Nastech risk acceptance; no runtime Cua prompts |
...
launched for bounded or unrestricted access. One Nastech
...
The command links the pack into `~/.nastech/skills/cua-driver` (Nastech is one
...
- **Windows: `nastech computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the Nastech venv
  ...
  `NASTECH_CUA_DRIVER_CMD` to a copy outside `WindowsApps`.
...
On Linux, native Wayland support remains an explicit opt-in. Nastech passes the
