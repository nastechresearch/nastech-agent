...
Nastech maps its existing approval UX onto cua-driver's immutable daemon
...
path. Nastech still exposes `nastech computer-use browser-approve` as a
...
for the format). Nastech launches a private per-session daemon with
...
The bounded and unrestricted daemons are private to that Nastech session.
...
machine-wide daemon's mode or grants another Nastech conversation the same
authority.
