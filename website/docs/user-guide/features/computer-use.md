...
The built-in `computer_use` toolset is the recommended Nastech integration. It
...
Already have cua-driver? Nastech reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `nastech update`, and the first
`computer_use` call of a session, Nastech checks the local version and
...
selected with `NASTECH_CUA_DRIVER_CMD` stays
under your control, so Nastech reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Nastech autodetection is a
planned cua-driver follow-up, so currently point Nastech at that directory or
...
interface. The built-in toolset provides Nastech actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `nastech computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Nastech host identity. |
...
Nastech maps its existing approval UX onto cua-driver's immutable runtime
...
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Nastech approvals; Cua stops at its protected boundary | Refuses unless `computer_use.grant_existing_profile: true` (one-time config opt-in) |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Nastech risk acceptance; no runtime Cua prompts | Refuses unless `computer_use.grant_existing_profile: true`; YOLO does not substitute for this grant |
...
Nastech then launches the cua-driver runtime with the trusted-launcher grant
...
computer_use:
  ...
  capability_manifest: ~/.nastech/cua-manifest.yaml
```
...
for the format). Nastech launches a private runtime with
...
identity instead of resetting with every Nastech build), and Nastech verifies
...
resetting or closing the Nastech session, cancellation cleanup, or process exit
closes that transport session. Nastech also stops private runtimes that it
launched for bounded, unrestricted, or existing-profile access. One Nastech
...
embedded service under the Nastech host identity.
...
Nastech run declares a public cua-driver **session name** (something like
`nastech-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Nastech disables it automatically where it is a known failure
...
Nastech keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Nastech-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Nastech
autodetection is a planned cua-driver follow-up, so currently point Nastech at
...
image capture also saves a bounded, shareable copy under Nastech' image cache
...
Pair with `approvals.mode: manual` in `~/.nastech/config.yaml` if you
...
```
NASTECH_CUA_DRIVER_CMD=/path/to/your/cua-driver
```
...
When it's on, `nastech computer-use doctor` reports `telemetry: enabled`;
...
```
# Windows (.env)
NASTECH_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
...
- `nastech computer-use status` prints the resolved binary path and
  version.
- `nastech computer-use doctor` confirms the binary is reachable and
...
**First action when anything's off: run `nastech computer-use doctor`.**
...
Run `nastech computer-use install` to fetch the cua-driver binary, or
run `nastech tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `nastech computer-use doctor` will
...
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Nastech autodetection is a
  planned follow-up; currently point Nastech at the installed pack directory
