...
Nastech Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — Nastech can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`nastech fallback` reuses the provider picker from `nastech model` — same provider list, same credential prompts, same validation. Use the subcommands `add`, `list` (alias `ls`), `remove` (alias `rm`), and `clear` to manage the chain. Changes persist under the top-level `fallback_providers:` list in `config.yaml`.

If you'd rather edit the YAML directly, add a top-level `fallback_providers` list to `~/.nastech/config.yaml`:
...
`fallback_providers` (plural, list) is the current config shape and supports multiple fallbacks tried in order. `fallback_model` (singular) is the legacy single-fallback key — Nastech still honors it for back-compat, but `nastech fallback` writes the current `fallback_providers` key and migrates legacy config on write. When both are set, `fallback_providers` takes priority.
:::
...
| xAI Grok OAuth (SuperGrok) | `xai-oauth` (alias `grok-oauth`) | `nastech model` → xAI Grok OAuth (browser login; SuperGrok subscription) |
| AWS Bedrock | `bedrock` | Standard boto3 auth (`AWS_REGION` + `AWS_PROFILE` or `AWS_ACCESS_KEY_ID`) |
| Qwen Portal (OAuth) | `qwen-oauth` | `nastech model` (Qwen Portal OAuth; optional: `NASTECH_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `nastech model` (MiniMax portal OAuth) |
...
The same re-resolution happens when the CLI falls back at **startup** because the primary provider's auth fails before the first request: the fallback model is sent its own configured effort, not the primary's. An explicit `nastech chat --reasoning <level>` is kept across that startup switch — it is your intent for the run.
...
**Nastech Portal as fallback for OpenRouter:**
```yaml
model:
  provider: openrouter
...
:::tip
There are no environment variables for the primary fallback chain — configure it exclusively through `config.yaml` or `nastech fallback`. This is intentional: fallback configuration is a deliberate choice, not something a stale shell export should override.
:::
...
Nastech uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
| Triage Specifier | `nastech kanban specify` / dashboard ✨ button — fleshes out a one-liner triage task into a real spec | `auxiliary.triage_specifier` |

### Auto-Detection Chain

When a task's provider is set to `"auto"` (the default), Nastech first tries the main provider + main model for that auxiliary task. If that route is unavailable or later fails with a capacity-style error, Nastech follows your configured fallback policy and then stops:
...
```text
OpenRouter → Nastech Portal → Custom endpoint → Codex OAuth →
...
auxiliary:
  vision:
    provider: "auto"              # auto | openrouter | nastech | codex | main | anthropic
...
`base_url` takes precedence over `provider`. Nastech uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.

---

## Auxiliary Capacity-Error Fallback

When you set an explicit auxiliary provider (e.g. `auxiliary.vision.provider: glm`), Nastech treats that as your preferred choice — but if the provider literally cannot serve the request because of a **capacity error** (HTTP 402 payment required, HTTP 429 daily-quota exhaustion, connection failure), Nastech falls back through a layered chain instead of failing silently:
...
4. **Warn + re-raise** — if every layer fails, Nastech logs `Auxiliary <task>: ... all fallbacks exhausted` at WARNING level and re-raises the original error
...
Nastech recognizes these as capacity-equivalent to 402 credit exhaustion (not transient rate limits):
...
If your provider returns a different phrase for daily-quota exhaustion and Nastech doesn't trigger fallback, that's a bug — open an issue with the exact error string.
...
auxiliary:
  compression:
    provider: "auto"                              # auto | openrouter | nastech | main
    model: "google/gemini-3-flash-preview"
```
...
If no provider is available for compression, Nastech drops middle conversation turns without generating a summary rather than failing the session.
