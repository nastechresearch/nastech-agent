...
:::tip
Hosted-mode auth uses Nastech Portal OAuth; if you also want the dashboard to talk to a real backend, `nastech setup --portal` wires up the model and tool gateway too. See [Nastech Portal](../../integrations/nastech-portal.md).
:::
...
- **Disk warnings** — the volume holding `~/.nastech` is nearly full
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.nastech`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
| `GET /api/nastech/update/check` | Report update availability (commits behind, install method) without applying. For git installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 24h cache (the check goes through the GitHub API, never `git fetch`) |
...
first; without one, Nastech fails closed at startup. This prevents the local SPA
...
Nastech always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
nastech dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
3. Server stashes PKCE state in a short-lived cookie, redirects user to `https://portal.nastechresearch.github.io/oauth/authorize?…`.
...
| `nastech_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip. SameSite=None + Secure over HTTPS (must survive the cross-site IDP redirect chain — Chromium drops SameSite=Lax cookies set on a 302 in a cross-site chain); SameSite=Lax on loopback HTTP |
