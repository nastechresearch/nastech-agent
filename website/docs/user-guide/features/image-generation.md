...
If the managed gateway returns `HTTP 4xx` for a specific model, that model isn't yet proxied on the portal side — the agent will tell you so, with remediation steps (switch to FAL.ai in `nastech tools` with your own `FAL_KEY` for direct access, or pick a different model).
:::
...
```yaml
image_gen:
  provider: fal                 # `nastech` if you picked Nastech Subscription
  model: fal-ai/flux-2/klein/9b
  max_parallel_requests: 4      # concurrent images in one tool-call batch
```

`image_gen.provider` is the single selection key: `nastech` routes through the managed Tool Gateway; a vendor name (`fal`, `openai`, `xai`, `krea`, ...) goes direct with your own key. The runtime always follows this stored selection — a `FAL_KEY` in `.env` is ignored while `provider: nastech`, and `provider: fal` without `FAL_KEY` errors with `image_gen is configured to use fal (set via nastech tools), but FAL_KEY is not set. Run 'nastech tools' to change it.` rather than silently rerouting. Change providers via `nastech tools`, not by adding/removing keys. (The old `use_gateway` boolean is legacy — still read as `nastech` when `true`, but never written anymore.)
...
new models appear in the picker as soon as OpenRouter serves them; no Nastech
...
Nastech Portal proxies the chat-completions protocol only, so its picker offers
...
only for now; responses are saved to `$NASTECH_HOME/cache/images/`.

## FAL: GPT Image 2.5

Select **GPT Image 2.5 Flare** or **GPT Image 2.5 Sunburst** under
`nastech tools` → Image Generation → FAL.ai. The model IDs are:
...
```bash
nastech config set image_gen.provider fal
nastech config set image_gen.model openai/gpt-image-2.5/flare/text-to-image
```
...
Both accept up to 16 source images. Nastech pins quality to `medium`, matching its
...
Select them through `nastech tools` → Image Generation → OpenAI, or set:

```bash
nastech config set image_gen.provider openai
nastech config set image_gen.openai.model gpt-image-2.5-flare
```
...
Nastech posts straight to the Codex backend's native
...
3. **Submission** — `_submit_fal_request()` routes via direct FAL credentials or the managed Nastech gateway, according to the stored `image_gen.provider` selection.
