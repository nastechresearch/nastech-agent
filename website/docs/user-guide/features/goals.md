...
`/goal draft <text>` both creates the goal and starts its first turn, including when drafting is unavailable and Nastech falls back to a free-form goal. `draft` is a whole-word subcommand: `/goal drafting docs` keeps `drafting docs` as the literal objective without calling the draft model.
...
| Command | What it does |
|---|---|
| `/goal wait <pid> [reason]` | Manually park the loop until the process with that PID exits. The PID must be a live process on the Nastech host; a remote or already-exited PID is rejected (and a judge `wait_on_pid` naming one continues instead of parking). |
