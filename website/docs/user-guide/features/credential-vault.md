...
fills the password through Nastech. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Nastech generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `nastech vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Nastech
...
in, Nastech picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Nastech hands the master password to the manager's
...
Prefer not to use a detected manager? `nastech vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `nastech vault add`), bound to the checkout site,
...
- **CLI**: `nastech vault list`, `nastech vault add`, `nastech vault rm <handle>`,
  `nastech vault sources`.

Items live encrypted under `~/.nastech/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `nastech chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Nastech: not in
