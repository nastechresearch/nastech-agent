...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Nastech CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Nastech CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.nastech/skins/mytheme.yaml
```

Or set the default skin in `~/.nastech/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Nastech — gold and kawaii | `Nastech Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Nastech Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Nastech Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Nastech Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Nastech Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Nastech Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Nastech Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! 𓄃` |
| `response_label` | Label on the response box header | ` 𓄃 Nastech ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default NASTECH_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.nastech/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.nastech/skins/mytheme.yaml
...
## Nastech Mod — Visual Skin Editor
...
![Nastech Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/nastech-mod/master/nastech.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Nastech skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.nastech/skins/`
- Activates a skin by updating `~/.nastech/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/nastech-mod.git
cd nastech-mod/app
npm install
npm start
```
...
- Built-in skins load from `nastech_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.nastech/skins/` take precedence over built-in skins with the same name.
