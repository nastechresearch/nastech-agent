...
Only **one** project context type is loaded per session (first match wins): `.nastech.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
If any threat pattern is detected in a project context file (`.nastech.md`, `AGENTS.md`, `CLAUDE.md`,
