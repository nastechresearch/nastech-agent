...
:::tip
Credential pools are mainly for API-key providers (OpenRouter, Anthropic). A single [Nastech Portal](../../integrations/nastech-portal.md) OAuth covers 300+ models, so most users don't need a pool when on Portal.
:::
...
Refresh-only lookups and peeks do not count. Status reads (`nastech doctor`, the `/model`
...
holding the same login rotated it first — or, for Nastech, the profile holds no Portal login or token
...
the remaining cooldown (or the `nastech auth add <provider>` re-login for a dead one) — the first-run
