...
snapshot: [`/docs/api/skills.json`](https://nastechresearch.github.io/nastech-agent/docs/api/skills.json).
The public docs alias serves the same snapshot as Desktop's fetch URL,
`https://nastechresearch.github.io/nastech-agent/docs/api/skills.json`. The docs
...
The Skills Hub has an **Install in Nastech** button on each installable card. It opens the
installed Nastech Desktop app with a URL-encoded, source-qualified skill target:
...
```text
nastech://skill/install?identifier=official%2Fsecurity%2F1password
```

Nastech shows **Install “skill-name”?** with separate **Source** and **Install to**
...
The public links use `nastech://`, not the development-only `nastech-dev://`
...
By default every profile is seeded with the bundled skill catalog, and each `nastech update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.nastech` profile):

```bash
curl -fsSL https://nastechresearch.github.io/nastech-agent/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
nastech profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
nastech skills opt-out            # stop future seeding — nothing on disk is touched
nastech skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
nastech skills opt-in --sync      # undo: remove the marker and re-seed now
```
...
(Plan mode works the same way but is a built-in command now: `/plan [request]` tells Nastech to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.nastech/plans/` relative to the active workspace/backend working directory.)
...
Settings are stored under `skills.config` in your config.yaml. `nastech config migrate` prompts for unconfigured settings, and `nastech config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
policy above), Nastech can run [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)
...
- **Create locally, update in place**: New agent-created skills are written to `~/.nastech/skills/` (or `skills.create_dir` when configured — see below). Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch` (targeted or full rewrite), `write_file`, `remove_file`, or `delete`.
...
By default the agent writes new skills to the profile-local `~/.nastech/skills/`. If you want agent-created skills to land somewhere else — a shared "brain" directory, a git-tracked repo, or a fleet-wide skills volume — set `create_dir` under the `skills` section:
...
Paths support `~` expansion and `${VAR}` substitution; relative paths resolve against your Nastech home. Setting `create_dir` to the local skills dir is the same as leaving it unset.


## Project-Local Skills

Repos can carry their own skills, active only for sessions started inside that project — the same pattern other agent harnesses use for repo-local configuration. When you launch Nastech inside a git checkout, it looks for skills in:

```text
<project-root>/.nastech/skills/    # Nastech-native location
...
Skills are procedure documents the agent follows, so Nastech does **not** auto-load them from arbitrary cloned repos. The first time you run Nastech in a repo with project skills, the banner shows a notice:

```text
◆ 3 project skill(s) found in /home/you/myproject but not loaded — run `nastech skills trust` to enable them.
```

Trust the repo once (from inside it, or by passing the path):

```bash
nastech skills trust             # trust the current repo
nastech skills trust ~/myproject # or explicitly
nastech skills untrust           # revoke
```

Trusted roots are stored in `skills.trusted_project_dirs` in `~/.nastech/config.yaml`. Set `skills.project_discovery: false` to turn the feature off entirely (no scanning, no notices).

### Precedence

Project skills are the **highest-precedence tier**: `project → local (~/.nastech/skills/) → external_dirs`. A project skill named `deploy` overrides a same-named profile or bundled skill for sessions inside that repo — that's the point: vendored repo skills win on their home turf, without touching your global profile. Project skills are tagged `[project]` in the agent's skill index so provenance stays visible.

Like external dirs, project skill directories are treated as repo-owned: autonomous skill maintenance (the curator) never modifies them, and new agent-created skills always go to `~/.nastech/skills/`.

### Scan-time quarantine

Trust is a repo-level decision, but a repo's skill content changes with every `git pull`. To close that gap, every project skill is scanned with the same security scanner used for Skills Hub installs before it enters the index. A skill whose scan verdict is **dangerous** (prompt-injection directives, credential-exfiltration commands, hidden-text tricks) is quarantined: it does not appear in the skill index, `skills_list`, slash commands, and refuses to load by name with an explanatory error. Scans are content-hash cached under `~/.nastech/cache/project_skill_scans/` (never inside your repo) and re-run automatically when the skill's content changes.
...
In the TUI and Desktop the project root follows each **session's workspace** (the directory shown in the sidebar / set with the workspace picker), so starting `nastech --tui` inside a trusted repo registers its project skills as slash commands even when `terminal.cwd` is left at the default placeholder `.`; two sessions open in two repos each see their own.
...
Bundles live in **`~/.nastech/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `nastech bundles list`.
...
# Re-scan ~/.nastech/skill-bundles/ and report changes
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.nastech/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.nastech/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.nastech/pending/skills/` and are
...
[Skills Hub](https://nastechresearch.github.io/nastech-agent/docs) page — instead of a
...
Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `nastech skills update` re-fetches from the same URL automatically when you want to refresh.
...
`nastech skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
nastech skills update react --force   # Overwrite a skill you've edited locally
```

This uses the stored source identifier plus the current upstream bundle content hash to detect drift.

Checks skip network requests for missing or non-directory installs (`orphaned`) and unsafe or unresolvable recorded paths (`invalid_install`). Missing-directory entries can be removed with `nastech skills uninstall <name>`; invalid paths require inspecting and repairing the active profile's `skills/.hub/lock.json` before retrying. No entries are removed automatically.
...
Skills you have edited locally (the on-disk content no longer matches the hash recorded at install time) are **skipped** by `nastech skills update` so your changes are never silently overwritten. Pass `--force` to replace them with the upstream version anyway.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.nastech.tags`, `version`, `author`, `platforms`, `metadata.nastech.config`).
...
The `nastech skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `nastech skills tap list` shows the effective path per tap.
...
Generated runtime caches inside a skill (`__pycache__/`, `.pytest_cache/`, `.mypy_cache/`, `.ruff_cache/`, and a `.pyc` sitting next to its `.py`) are not part of the hash, so running a skill's helper script never marks it user-modified or hides it from `nastech skills list-modified` / `diff`.

The protection is good, but it has one sharp edge. If you edit a bundled skill and then later want to abandon your changes and go back to the bundled version by just copy-pasting from `~/.nastech/nastech-agent/skills/`, the manifest still holds the *old* origin hash from whenever the last successful sync ran. Your fresh copy-paste contents (current bundled hash) won't match that stale origin hash, so sync keeps flagging it as user-modified.

`nastech skills reset` is the escape hatch:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `NASTECH_HOME`, so `nastech -p coder skills reset <name>` only affects that profile.
:::
