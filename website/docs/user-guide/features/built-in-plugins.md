...
The old `NASTECH_NEMO_RELAY_ATOF_*` and `NASTECH_NEMO_RELAY_ATIF_*` settings no longer activate exporters. `nastech doctor` reports these stale settings when no replacement `plugins.toml` is selected.
