...
See the [Plugins](./plugins.md) page for the general plugin system, and [Build a Nastech Plugin](../../developer-guide/plugins/index.md) to write your own.
...
type = "file"
output_directory = "/home/you/.nastech/telemetry/nemo-relay/atof"
filename = "nastech-atof.jsonl"
mode = "append"
...
model_name = "unknown"
output_directory = "/home/you/.nastech/telemetry/nemo-relay/atif"
...
Then add `NASTECH_NEMO_RELAY_PLUGINS_TOML=/home/you/.nastech/relay-plugins.toml` to `.env` and restart the gateway.
...
Both defaults preserve one session scope for the full session. Rotated spans retain the same `session_id` and add `nastech.session.segment` plus `nastech.session.segment_reason` (`compaction` or `max_turns`).
...
Bundled plugins are written exactly like any other Nastech plugin — see [Build a Nastech Plugin](../../developer-guide/plugins/index.md). The only differences are:
