...
description: "Plugins shipped with Nastech Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
2. **User** — `~/.nastech/plugins/<name>/`
3. **Project** — `./.nastech/plugins/<name>/` (requires `NASTECH_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `nastech_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `nastech memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `nastech plugins list` and the interactive `nastech plugins` UI), but none load until you explicitly enable them:

```bash
nastech plugins enable disk-cleanup
```

Or via `~/.nastech/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Nastech. You always opt in explicitly.
...
The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `nastech plugins enable <name>`.
...
| `nastech-achievements` | dashboard tab | Steam-style collectible badges generated from your real Nastech session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `nastech memory` and `nastech plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**State** — everything lives at `$NASTECH_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `NASTECH_HOME` or `/tmp/nastech-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `nastech plugins enable disk-cleanup` (or check the box in `nastech plugins`).

**Disabling again:** `nastech plugins disable disk-cleanup`.
...
**Enabling:** `nastech plugins enable security-guidance` (or check the box in `nastech plugins`).

**Disabling again:** `nastech plugins disable security-guidance`.
...
Traces Nastech turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Nastech' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `nastech logs`.
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Nastech turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `NASTECH_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Nastech session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `nastech chat` session lives under one Langfuse session.

**Verify:**

```bash
nastech plugins list                 # observability/langfuse should show "enabled"
nastech chat -q "hello"              # check the Langfuse UI for a "Nastech turn" trace
```
...
| `NASTECH_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `NASTECH_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `NASTECH_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Nastech-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Nastech-prefixed wins when both are set.
...
**Disabling:** `nastech plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.

### observability/nemo_relay

Relays Nastech execution boundaries — sessions, turns, LLM calls, and tool invocations — to an [NVIDIA NeMo Relay](https://docs.nvidia.com/nemo/relay/about-nemo-relay/overview) endpoint. Nastech core owns the Relay session/turn/LLM/tool scopes; the plugin configures exporters (ATOF JSONL, ATIF trajectories, OpenTelemetry) and adds observer marks for approvals and delegated subagents. Full exporter setup lives in the plugin's `README.md` under `plugins/observability/nemo_relay/`.
...
Rotated segments keep the same `session_id` attribute and add `nastech.session.segment` (0-based index) plus `nastech.session.segment_reason` (`compaction` or `max_turns`), so dashboards that group on `session_id` are unaffected. Rotation happens exclusively at turn boundaries and rides the same bounded scope-op executor as every other native Relay call — a wedged exporter costs one segment span, never the agent.
...
nastech meet setup   # preflight: playwright, chromium, auth file
nastech meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `nastech plugins disable google_meet`. Any saved transcripts stay in `~/.nastech/workspace/meetings/` until you remove them.

### nastech-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Nastech session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Nastech feature changes.

**How it works:**

- Scans your entire `~/.nastech/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$NASTECH_HOME/plugins/nastech-achievements/state.json`
...
| Secret | Hidden until Nastech detects the first related signal in your history |

**API** — routes mount under `/api/plugins/nastech-achievements/`:
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Nastech updates. |
...
**Enabling:** Nothing to enable — `nastech-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `nastech dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/nastech-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.nastech/plugins/nastech-achievements/` that ships no dashboard. The plugin's state files under `$NASTECH_HOME/plugins/nastech-achievements/` survive — reinstalling preserves your unlock history.
