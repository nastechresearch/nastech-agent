...
The curator is triggered by an inactivity check, not a cron job. On CLI session start, during gateway housekeeping, and on the Desktop/`nastech serve` maintenance timer, Nastech checks whether:
...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (14) become `stale`; skills unused for `archive_after_days` (30) are moved to `~/.nastech/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
nastech curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default: `archive_after_days`, 30)
nastech curator ledger           # list the per-mutation audit ledger (all actors)
nastech curator ledger --skill <name> --limit 50  # filter/paginate ledger entries
nastech curator rollback <entry-id>  # undo a single mutation from the ledger
nastech curator purge [--days N] [--dry-run]  # delete archived skills older than the TTL (explicit only)
```
...
Whole-run snapshots answer "undo everything the last curator pass did" — but sometimes you want to know *who changed what* and undo exactly one mutation. Every skill mutation — curator auto-transitions, agent `skill_manage` calls, and your own CLI archive/restore/purge — appends one entry to the append-only JSONL ledger at `~/.nastech/skills/.curator_ledger.jsonl`:
...
- **before/after** — per-file `{path, sha256}` manifests. File contents are stored content-addressed (deduped by hash) under `~/.nastech/.curator_backups/blobs/`, so a hundred entries touching the same unchanged file cost one blob.

```bash
nastech curator ledger                  # newest 20 entries
nastech curator ledger --skill my-skill --limit 50
nastech curator rollback <entry-id>     # restore that one mutation's before-state
```

Single-entry rollback restores exactly the files that mutation touched (and removes files it created) from the blob store — nothing else in the skills tree moves. Like whole-tree rollback, it takes a safety ledger entry of the current state first and **fails closed**: if the safety capture can't be written, nothing is changed. Because foreground deletes are ledgered too, `nastech curator rollback <entry-id>` can resurrect a hard-deleted skill.
...
Archived skills are kept forever by default. If you want `~/.nastech/skills/.archive/` bounded, set a TTL and purge explicitly — purging never runs automatically, and every purged skill is captured into the ledger (with blobs) first, so even a purge leaves an auditable, recoverable trail:
...
```bash
nastech curator purge --dry-run   # preview what would be deleted
nastech curator purge             # delete archives older than the TTL (with confirmation)
nastech curator purge --days 90   # one-off TTL override
```
