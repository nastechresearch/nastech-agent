...
Lists the agent as an available model. The advertised model name defaults to the [profile](../profiles.md) name (or `nastech-agent` for the default profile). Required by most frontends for model discovery.
...
Nastech can route browser tools through an authenticated extension that controls
the browser session associated with the current Nastech session. The feature is
...
register only for an existing server session. Nastech derives the controller
...
Nastech preserves the existing browser backend. Once the gateway binds a
...
Nastech never retries the same action through another backend.
...
2. Nastech returns a single-use ticket with a 30-second TTL and the filtered,
   server-bound controller scope.
3. Open `GET /v1/browser-control/ws` with both WebSocket subprotocols:
   `nastech-browser-control-v1` and
   `nastech-browser-control-ticket.<ticket>`.
...
Nastech sends `browser.controller.command` frames containing `command_id`,
...
When `session_id` identifies an existing Nastech session and no explicit
...
history: an explicit `X-Nastech-Session-Id` on Chat Completions, a native
...
To give multiple users their own isolated Nastech instance (separate config, memory, skills), use [profiles](../profiles.md):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Nastech instance. See the [Open WebUI guide](../messaging/open-webui.md#multi-user-setup-with-profiles) for details.
