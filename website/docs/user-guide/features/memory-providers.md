...
Peer identity is optional. By default, Nastech sends no peer ID and writes
...
Set `memory.openviking.agent: nastech` to restore the old peer-scoped writes.
...
Nastech sends `User-Agent: openviking-memory-nastech/<version>` on OpenViking
