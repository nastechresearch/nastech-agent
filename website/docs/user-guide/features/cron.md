...
nastech cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
`nastech cron doctor` is a read-only health check over every active job. It
...
- `nastech cron list` — a red `⚠ Missed scheduled fire:` line under the job
