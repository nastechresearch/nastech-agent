...
`nastech setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nastech Portal](../../integrations/nastech-portal.md).
:::
...
A cron job can load one or more skills before it runs the prompt. Each skill loads exactly as it does from `/skill-name` in a chat session, including the `[Skill config ...]` block with its resolved `metadata.nastech.config` values from `config.yaml`.
...
`nastech pause [--reason ...]` is the global emergency stop (`nastech resume` lifts it). While it is engaged no scheduled cron fire starts, whichever door it arrives through: the built-in ticker skips its dispatch, the managed-cron (hosted scheduler) fire webhook answers `503` with `Retry-After: 60` so the scheduler redelivers the fire after you resume, and the [misfire catch-up](#misfire-catch-up) sweep stays idle instead of force-firing everything that was held back. Runs already in flight are never killed, and nothing is lost: due work catches up on the first tick or sweep after `nastech resume`. Explicit manual runs (`nastech cron run`, the dashboard's Trigger button) are an operator override and still execute while paused.
...
```bash
nastech cron create "every 1h" "Post the digest" --paused --paused-reason "Awaiting review"
...
`nastech cron status` reports whether the scheduler is alive (gateway process, ticker heartbeat, last successful tick) and the soonest scheduled run across your active jobs, ordered by actual instant even when jobs store different UTC offsets. A `next_run_at` that is already more than 15 minutes in the past is never shown as an upcoming "Next run": `cron status` prints `⚠ Next run <time> is OVERDUE — passed 7h ago but the job has not fired`, `cron list` and the in-chat `/cron list` label the row `Overdue:`, the dashboard and the Desktop cron panel (including a Bot's Routines card) show `Overdue since`, and when the scheduler has stopped ticking `status` (with the gateway down) and the dashboard Cron page also say when it last ticked. That is the signature of a scheduler that stopped ticking — restart the gateway (`nastech gateway restart`) so the next tick picks the overdue job up, or run it right away with `nastech cron run <id>`.

For a named profile served by the default-profile multiplexer, `nastech cron status` names that scheduler host and reports the named profile's own heartbeat health. Missing or stale heartbeats point to `nastech --profile default gateway restart`. `cron list` and `cron create` also warn when that heartbeat is missing or stale; `cron status` additionally checks the last successful tick and reports tick errors.
...
The worker is the gateway's own interpreter running `python -m cron.scheduler`, with the gateway's checkout pinned on its `PYTHONPATH` (plus any entries the gateway itself was started with), so it imports the same Nastech tree the gateway runs — regardless of the venv's editable-install mapping, the unit's `WorkingDirectory`, or `PYTHONSAFEPATH` on the host. A worker that dies before acknowledging the handoff records its own stderr tail in the job's last error and in the execution ledger, so the failing import (or whatever killed it) is named instead of a bare exit code.
...
`nastech cron run` / `/cron run`, so a one-shot invocation with no scheduler
running heals the ledger too — Nastech marks an abandoned attempt `unknown` only
...
that the retained ledger records as completed, Nastech skips that replay and
...
in a row and suggesting you fix, pause (`nastech cron pause <job>`), or remove
it. Any successful run resets the streak, and `nastech cron list` shows the
...
```bash
nastech cron incidents                 # list incidents (newest activity first)
nastech cron incidents --state alerted # filter: detected | alerted | resolved | closed
nastech cron incidents ack <id>        # acknowledge — silence this signature for good
```
...
### Fleet health check: `nastech cron doctor`

`nastech cron doctor` is a read-only health check over every active job. It prints grouped, per-job issues and exits `1` while any finding stands, including historical late or catch-up dispatches (`0` when no findings remain).

A successful catch-up does not clear the lateness warning; the next on-time dispatch does. A watchdog such as `nastech cron doctor || alert` can therefore keep alerting for a full schedule interval after the host wakes, even if the catch-up succeeds.
...
- script missing, not a file, or resolving outside `NASTECH_HOME/scripts`,
...
`nastech cron incidents` (durable failure records) and `nastech cron runs`
...
`last_delivery_error`. `nastech cron list` shows it in yellow as
`delivery_failed: <reason>`, `nastech cron doctor` reports it as a delivery
...
in `nastech cron list`:

```
⚠ Delivery UNVERIFIED: adapter acked slack:C0123456 without message_id/raw_response
```

and in `nastech cron doctor` as `last delivery unverified (...)`. The marker is
...
The run is then recorded as failed (`last_status`, failure streak, `nastech cron runs` and `nastech cron incidents`
all reflect it) and the failure notice is delivered like any other failed run. The full response is still saved
under `~/.nastech/cron/output/` for triage. The marker is strict: mentioning or quoting `[CRON_FAILURE]` anywhere
...
Or set the `NASTECH_CRON_MEDIA_SEND_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 300s default. A timed-out attachment is recorded in the job's run status as a partial delivery failure (the text still delivers).
...
From the CLI: `nastech cron create "every 6h" "Scan for news" --continuity`, and `nastech cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
Nastech home. Its `## Error` section includes the chained traceback, with credential patterns
...
The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `nastech gateway restart`).
...
`nastech cron list` as `⚠ late` / `⚠ catch-up after missed fire`.
...
Or run `nastech config set cron.catch_up_missed false`. With this opt-out, a recurring
...
When `enabled_toolsets` is set on a job it wins; otherwise the `nastech tools` cron-platform config wins; otherwise Nastech falls back to the built-in defaults. If the cron-platform toolset config cannot be read at all (for example a malformed `platform_toolsets` block in `config.yaml`), the run fails with a recorded error instead of quietly running with every tool — check `nastech cron list` / `nastech cron doctor`. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.
