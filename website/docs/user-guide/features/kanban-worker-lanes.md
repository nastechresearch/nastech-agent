...
Nastech Kanban owns lifecycle truth — `ready` → `running` → `review` / `blocked` / `done` / `archived`. Worker lanes execute work but never own that truth; everything they do flows back through the kanban kernel via the `kanban_*` tools (or, for non-Nastech external workers, via the API). Reviewers gate the transition from "code change written" to "task done."
...
| Variable | Carries |
|---|---|
| `NASTECH_KANBAN_TASK` | the task id the worker is operating on |
| `NASTECH_KANBAN_DB` | absolute path to the per-board SQLite file |
| `NASTECH_KANBAN_BOARD` | board slug |
| `NASTECH_KANBAN_WORKSPACES_ROOT` | root of the board's workspace tree |
| `NASTECH_KANBAN_WORKSPACE` | absolute path to *this* task's workspace |
| `NASTECH_KANBAN_RUN_ID` | the current run's id (for the lifecycle gate) |
| `NASTECH_KANBAN_CLAIM_LOCK` | the claim lock string (`<host>:<pid>:<uuid>`) |
...
Nastech subprocess helpers carry a non-owner fence into shells, execution kernels,
...
dispatcher-pinned `NASTECH_KANBAN_DB`): a descendant that works against a different Kanban
home — a test or reproduction under a scratch `NASTECH_HOME` — gets a normal read-write board.

The dispatcher explicitly grants a newly assigned worker its own scope. The managed
Nastech-tools MCP endpoint can likewise act for its supervising worker, while the
...
The dashboard renders run history with summaries, metadata blocks, and exit-status badges. CLI users can run `nastech kanban tail <task_id>` to follow live, or `nastech kanban runs <task_id>` for the historical attempt list.
...
The historical issue for this is [#19931](https://github.com/NastechResearch/nastech-agent/issues/19931) and the closed-not-merged Codex-specific PR [#19924](https://github.com/NastechResearch/nastech-agent/pull/19924) — those describe the original architecture proposal but didn't land a runner.
...
- [`KANBAN_GUIDANCE`](https://github.com/NastechResearch/nastech-agent/blob/main/agent/prompt_builder.py) — the worker + orchestrator lifecycle injected into every kanban worker's system prompt.
