...
(`ai.nastech.gateway-<name>.plist`) or systemd user service
(`nastech-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
```bash
nastech config set gateway.multiplex_profiles true
...
With a multiplexer running, a named-profile `nastech gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.nastech/profiles/coder/.env`; the default listener key is rejected.
...
home). `nastech status` reports the multiplexer and the profiles it serves;
`nastech status -p <name>` slices to one profile. Each profile still writes its
