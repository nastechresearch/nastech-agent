...
If you only run one Nastech agent, you don't need this page — see
...
[Connecting Desktop to Many Nastech Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Nastech agents that should all
...
(`ai.nastech.gateway-<name>.plist`), a systemd user service
(`nastech-gateway-<name>.service`), a systemd **system** service when installed with
`sudo nastech gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `nastech serve` backend. This guide adds the patterns for managing them
collectively.
...
nastech profile create coder
nastech profile create personal-bot
nastech profile create research
...
[`nastech gateway migrate --multiplex`](#migrating-from-per-profile-gateways) and
...
`nastech gateway migrate --multiplex` one-liner. Nothing is changed on disk.
...
Other processes (`nastech -p <name> gateway start`, the dashboard, `nastech gateway
...
```bash
nastech config set gateway.multiplex_profiles true    # or false
nastech gateway restart
```

Equivalently, in the default profile's `~/.nastech/config.yaml`:
...
You do **not** run `nastech gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `nastech gateway run`, `start`,
...
loop. `nastech -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`nastech gateway stop` on the default profile takes down for every served profile.
...
The setup flows follow the same rule: `nastech -p coder setup gateway`, `nastech -p coder setup`,
`nastech -p coder gateway setup` and `nastech -p coder import` configure the profile's bots but
...
- **WhatsApp (bridge) and Relay are shared ingress owned by the default profile.**
  ...
  is not being served; `nastech gateway status --profile work` shows
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.nastech/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `nastech webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`nastech webhook ls` shows the
...
A standalone `nastech -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ nastech -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: nastech gateway status
...
`nastech gateway status` and `nastech status` on the default profile list the same
...
There is a single process-level PID and lock (the multiplexer, under the default home). `nastech status` on the default profile reports the multiplexer and lists the profiles it serves (`Serves: coder, research`). `nastech -p coder status` and `nastech -p coder gateway status` report "running via the default-profile multiplexer" instead of "stopped". The dashboard's `/api/status?profile=coder` / Channels page report the multiplexer as coder's running gateway, with coder's own adapters as its platforms. The single `gateway_state.json` lives under the default home: secondary adapters appear there as `<profile>:<platform>` entries beside `served_profiles`; no per-profile gateway status file is written.

`nastech -p coder cron status` prints `Scheduler host: default-profile multiplexer`, then checks coder's own ticker heartbeat and last successful tick. A missing or stale heartbeat produces a warning rather than an unconditional running verdict; the restart hint targets `nastech --profile default gateway restart`. `cron list` and `cron create` also warn when a served profile has no fresh heartbeat. `cron status` adds tick-failure details that those lightweight checks do not read.
...
`HINDSIGHT_BANK_ID`, `NASTECH_HONCHO_HOST`) are read from the routed profile's
...
`NASTECH_INFERENCE_BASE_URL`, `GATEWAY_PROXY_URL`, Firecrawl / Browserbase /
...
`NASTECH_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`trust_recent_files*`) and the Nastech `auth.json` used for auxiliary calls are all
...
`NASTECH_HOME`, without the default profile's secrets in their environment, and
...
| Provider logins and the credential pool (`auth.json`: OAuth tokens, `nastech auth add` keys) | The profile's own `auth.json` | "Not connected to any AI provider" with the `nastech -p <name> model` hint — never the default profile's login, and a refresh never writes to the root store |
...
| Dashboard actions (`nastech -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `NASTECH_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
...
archived or deleted instead — `nastech profile delete <name>`, or move the
...
multiplexer started as `nastech -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`nastech profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `nastech -p <name> gateway
...
adapters the same way, and `nastech profile rename` unroutes the old name before
...
`~/.local/bin/nastech-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: nastech-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    nastech gateway "$action"
  else
    nastech -p "$profile" gateway "$action"
  fi
}
...
```bash
nastech-gateways start      # start every configured profile
nastech-gateways stop       # stop every configured profile
nastech-gateways restart    # restart all
nastech-gateways status     # status across all
nastech-gateways list       # delegates to `nastech gateway list`
```

:::tip
The `default` profile is targeted with `nastech gateway <action>` (no `-p`),
not `nastech -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `nastech -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.nastech.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/nastech-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.nastech.gateway.plist` /
`nastech-gateway.service`.
...
tail -f ~/.nastech/logs/gateway.error.log

# Named profile
tail -f ~/.nastech/profiles/<name>/logs/gateway.log
tail -f ~/.nastech/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.nastech/logs/gateway.log ~/.nastech/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
nastech logs -f                  # follow default profile
nastech -p coder logs -f         # follow one profile
nastech logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
nastech profile list             # profiles + model + gateway state
nastech-gateways status          # full status across every profile
launchctl list | grep nastech    # macOS — PIDs and labels
systemctl --user list-units 'nastech-gateway-*'   # Linux — units
```
...
The default profile uses `~/.nastech/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
nastech config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.nastech/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=nastech --why="gateways running" \
  sleep infinity &
...
`nastech-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.nastech/.env ~/.nastech/profiles/*/.env
```
...
```bash
nastech gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
nastech gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
nastech gateway migrate --standalone            # roll back to per-profile gateways
```

### What `nastech update` does
...
service) and `gateway.multiplex_profiles` is off, `nastech update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `nastech gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `nastech update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `nastech update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `nastech profile create` produces.
...
| `NASTECH_HOME` outside `<default home>/profiles/` | a unit pinning `NASTECH_HOME=/opt/nastech/profiles/emma` |

In that case `nastech update` prints the boundary it found plus
`nastech gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `nastech gateway migrate --multiplex --dry-run`
...
```bash
nastech config set gateway.auto_multiplex_migration false
```

`nastech update` then leaves per-profile gateways exactly as they are, with no
...
other setting, so a value pinned in the managed scope (`/etc/nastech/config.yaml`)
wins over the profile's own file. It governs the **automatic** path only:
`nastech gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `nastech gateway migrate --multiplex` with
...
`nastech profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `nastech profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.nastech/gateway_migration.json` for rollback.
...
above). `nastech profile create` confirms this when the live multiplexer picked the
profile up; it prints the `nastech gateway restart` reminder only when it could not
reach the multiplexer (for example, a gateway started from an older build).

### Rollback

```bash
nastech gateway migrate --standalone
```
...
window, the next `nastech gateway migrate --multiplex` sees the flag on, the
...
with `nastech config set gateway.multiplex_profiles false && nastech gateway restart`
...
flag, stop the per-profile tasks, `nastech gateway restart`). The dashboard's
System page offers the same migration as a button when the preflight finds an
eligible install.

## Updating the code

`nastech update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `nastech gateway start` after a previous `nastech gateway stop`. The
...
```bash
ps -ef | grep "nastech_cli.*-p <profile>"
cat ~/.nastech/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.nastech.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.nastech.gateway-<profile>.plist

# Linux
systemctl --user restart nastech-gateway-<profile>.service
```

### Health check

```bash
nastech doctor                  # default profile
nastech -p <profile> doctor     # one profile
```
