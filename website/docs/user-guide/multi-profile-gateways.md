...
home). `nastech status` on the default profile reports the multiplexer and lists
the profiles it serves (`Serves: coder, research`); `nastech -p coder status`,
`nastech -p coder gateway status` and `nastech -p coder cron status` all report
