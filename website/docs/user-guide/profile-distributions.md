...
Nastech has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `nastech profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `nastech profile update` pulls new versions | Re-send the file |
...
| Made with | `nastech profile install` / `update` | `/export` and `/import`, or `nastech profile export` / `import` |
...
…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `nastech profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Nastech in 2026, that's already true.
...
:::caution
**Nastech does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `nastech profile install` or `nastech profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
Create `~/.nastech/profiles/research-bot/distribution.yaml`:
...
Create `~/.nastech/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `nastech profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.nastech/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
`nastech profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `nastech profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
When the lead ships v1.1 (better SOUL, new skill), engineers run `nastech profile update pr-reviewer` and everyone's on the new version within minutes.
...
You built Nastech-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `nastech profile info <name>` and hold off on updates until you're ready.
:::
...
# Iterate locally in ~/.nastech/profiles/forked-research-bot/
...
Without `-o`, the CLI and TUI place the archive in Nastech's managed
`profile-exports/` directory under the default Nastech home, not in the current
...
source file. If the Nastech home itself lives inside a Git checkout (some
Docker/custom deployments), the archive goes to `~/.nastech-profile-exports/`
...
```bash
nastech profile export research-bot
nastech profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
nastech profile import ./research-bot.tar.gz
nastech profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `nastech -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.nastech`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.nastech`) is exported through an allow-list — only known Nastech artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.nastech/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `nastech -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `nastech profile` — not a parallel command tree.
...
- Reserved profile names (`nastech`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`nastech profile export` / `import`](../reference/profile-commands.md#nastech-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Nastech](../guides/use-soul-with-nastech.md) — authoring personalities
