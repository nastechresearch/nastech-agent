---
sidebar_position: 2
title: "Nastech Agent Configuration"
description: "Configure Nastech Agent — config.yaml, providers, models, API keys, and more"
---

# Nastech Agent Configuration

All settings are stored in the `~/.nastech/` directory for easy access.

:::tip Easiest path to a working `config.yaml`
Run `nastech setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nastech Portal](../integrations/nastech-portal.md).
:::
...
├── auth.json       # OAuth provider credentials (Nastech Portal, etc.)
...
```bash
nastech config              # View current configuration
nastech config edit         # Open config.yaml in your editor
nastech config get KEY      # Print a resolved value
nastech config set KEY VAL  # Set a specific value
nastech config unset KEY    # Remove a user-set value
nastech config check        # Check for missing options (after updates)
nastech config migrate      # Interactively add missing options

# Examples:
nastech config get model
nastech config set model anthropic/claude-opus-4
nastech config set terminal.backend docker
nastech config unset terminal.backend
nastech config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `nastech config set` command automatically routes values to the right file — every `UPPER_SNAKE` name (`OPENROUTER_API_KEY`, `DISCORD_HOME_CHANNEL`, `TELEGRAM_GROUP_ALLOWED_USERS`, `NASTECH_TIMEZONE`, …) is an environment variable and is saved to `.env`, never to `config.yaml`; dotted settings go to `config.yaml`. Any other `UPPER_SNAKE` name is saved to `.env` as-is (it is exported to the process environment for plugins and skills); names on the env writer's denylist (`NASTECH_YOLO_MODE`, `PATH`, …) are refused. A known key written under the wrong prefix (`gateway.discord.foo`, where `discord.foo` is itself a known key) is refused with a did-you-mean before anything is written; pass `--force` to write it anyway. Any other unknown path under a known section (a typo such as `agent.max_turnz`, or a runtime-read key with no seeded default) is written together with a did-you-mean notice, since the schema alone cannot tell the two apart. `nastech config get` on such a path prints the value from your file together with a stderr notice that Nastech may not read it, so a leftover key cannot silently pass for a live setting.
:::
...
1. **CLI arguments** — e.g., `nastech chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.nastech/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.nastech/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Long-running Nastech server surfaces (including the gateway and
`nastech serve --isolated`) apply the configured `RLIMIT_NOFILE` soft limit
...
The default is `4096`. Nastech clamps the target to the operating system's hard
...
The `database:` section controls how Nastech opens its SQLite state database
...
database:
  ...
  # Nastech detects the mount and creates fresh databases in delete mode
  # automatically. Note: an existing on-disk WAL database is never
  # live-downgraded — Nastech keeps WAL and logs an error telling you the
...
Nastech also warns (once per process per database) when an existing
...
under open connections can corrupt it). `nastech doctor` warns
`<db> is in WAL mode despite database.journal_mode=delete` until you stop
every Nastech process for the profile and run a one-time offline
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `NASTECH_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `NASTECH_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`NASTECH_API_TIMEOUT=1800`s, `NASTECH_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/NastechResearch/nastech-agent/blob/main/cli-config.yaml.example).
...
retries after an hour). Applying an update (`nastech update`, or the desktop's
Update button) always fetches fresh and invalidates the cached answer. Explicit
checks — `nastech update --check`, the desktop's "Check for Updates…" menu item,
...
Nastech's isolated internal Git commands default to `ssh -o BatchMode=yes`:
...
`nastech update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: quick       # quick (state snapshot, default) | full (snapshot + NASTECH_HOME zip) | off
...
`pre_update_backup` is the single pre-update safety knob: `quick` (default) snapshots critical state files (pairing data, cron jobs, config, auth; files over 1 GiB are skipped) into `state-snapshots/`; `full` additionally zips all of `NASTECH_HOME` into `backups/` and can add minutes on large homes; `off` disables both. Legacy booleans are honored (`true` → `full`, `false` → `off`).

Point-in-time copies of `config.yaml` itself (taken before `nastech setup` rewrites it, before `nastech migrate` edits it, every time the file parses successfully, and when it fails to parse) go to `backups/config/config.yaml.<reason>.<timestamp>`. Identical repeats are skipped and only the newest five per reason are kept, so they never pile up beside `config.yaml`. If `config.yaml` is broken, Nastech serves the newest `good` copy instead of built-in defaults and warns on every start until the YAML is fixed; the broken file is never modified.

For git installs, Nastech auto-stashes dirty tracked files and untracked files before checking out the update branch or pulling. Interactive terminal updates prompt before restoring that stash. Non-interactive updates (desktop/chat app, gateway, or `--yes`) use `updates.non_interactive_local_changes`: `stash` restores local source edits after a successful pull, while `discard` drops the update-created stash after a successful pull. Use `discard` only on managed installs where local source edits are never meant to persist.

Before that stash step, Nastech also restores tracked `package-lock.json` diffs left by npm install/build churn. Commit or manually stash intentional lockfile edits before updating.

## Terminal Backend Configuration

Nastech supports seven terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox (direct or via the Nastech-managed gateway), a Daytona workspace, a Vercel Sandbox, or a Singularity/Apptainer container.
...
`desktop.font_family` sets the font for chat and the rest of the Nastech Desktop interface (the terminal pane has its own key above). Give it one installed family name (for example, `OpenDyslexic` or `Atkinson Hyperlegible`) or a CSS font stack; Nastech keeps the active theme's own stack behind it so CJK and emoji glyphs still resolve, and an empty value uses the theme's font. Edit it in **Settings → Appearance → Chat Font**.
...
For cloud sandboxes such as Modal, Daytona, and Vercel Sandbox, `container_persistent: true` means Nastech will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. Nastech
state is still profile-scoped through `NASTECH_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

Nastech does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that Nastech launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{NASTECH_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{NASTECH_HOME}/home` when it exists | Use `{NASTECH_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{NASTECH_HOME}/home` as `HOME`. Nastech also
sets `NASTECH_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{NASTECH_HOME}/home` in `auto` mode
because that directory lives on the persistent Nastech data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `NASTECH_HOME` for Nastech data and `NASTECH_REAL_HOME` for the account home:
...
real_home = Path(os.environ.get("NASTECH_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `nastech tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, shared across Nastech processes.** Nastech starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents. Working-directory changes, installed packages, files in `/workspace`, and **background processes** all carry over from one tool call to the next, and from one Nastech process to the next. When you close a TUI session, run `/quit`, or start a new `nastech` invocation, the container keeps running and the next Nastech process reuses it via a labeled lookup. See **Container lifecycle** below for the exact teardown rules.
...
terminal:
  ...
  docker_persist_across_processes: true   # Reuse container across Nastech restarts
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.nastech/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.

**`terminal.docker_extra_args`** (also overridable via `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'`) lets you pass arbitrary `docker run` flags that Nastech doesn't surface as first-class keys — `--gpus`, `--network`, `--add-host`, alternative `--security-opt` overrides, etc. Each entry must be a string; the list is appended last to the assembled `docker run` invocation so it can override Nastech' defaults if needed. Use sparingly — flags that conflict with the sandbox hardening (capability drops, `--user`, the workspace bind mount) will silently weaken isolation.

**`terminal.docker_network`** (default `true`; env: `TERMINAL_DOCKER_NETWORK`) — set to `false` to run the sandbox container with `--network=none`, cutting off all network egress from agent commands. This applies to the execution container used by `terminal`, `execute_code`, and the file tools. Because containers persist across Nastech processes, flipping this to `false` while an older networked container exists will remove that container and start a fresh air-gapped one (a warning is logged); background processes running inside it are lost. Prefer this key over passing `--network=none` through `docker_extra_args`.

**Requirements:** Docker Desktop or Docker Engine installed and running. Nastech probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `NASTECH_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

#### Container lifecycle

Every Nastech-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `nastech-agent=1` — marks it as Nastech-managed
- `nastech-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `nastech-profile=<sanitized profile name>` — scopes reuse and reaping to the active Nastech profile by default; when `docker_shared_container_key` is set, its sanitized value is used instead

On startup, Nastech runs `docker ps --filter label=nastech-task-id=<id> --filter label=nastech-profile=<identity>` and **attaches to the existing container** when it finds one. The identity is the active profile unless `docker_shared_container_key` explicitly opts trusted profiles into a common value. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.

When a Nastech process exits — `/quit`, closing a TUI session, gateway shutdown, even SIGKILL — the cleanup path is a **no-op for the container in default mode**. The container keeps running. The next Nastech process attaches to it in milliseconds via the label probe. This is the behavior the "one long-lived container shared across sessions" contract requires: it's the only way background processes (npm watchers, dev servers, long-running pytest) survive across sessions.
...
| Orphan reaper at next startup | Sweeps **Exited** nastech-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other — a container labeled `nastech-profile=work` is invisible to a Nastech process running under `nastech-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start Nastech again under their original profile.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.nastech/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
Without a matching `AcceptEnv`, the server silently drops the variables and the remote shell sees them unset. Nastech provider credentials (`OPENAI_API_KEY`, …) are never forwarded even if listed. See [Env Var Passthrough](security.md#environment-variable-passthrough).
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.nastech/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.nastech/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `nastech-{task_id}`.

**Disk limit:** Daytona enforces a 10 GiB maximum. Requests above this are capped with a warning.

### Vercel Sandbox Backend

Runs commands in a [Vercel Sandbox](https://vercel.com/docs/vercel-sandbox) cloud microVM. Nastech uses the normal terminal and file tool surfaces; there are no Vercel-specific model-facing tools.
...
**Required authentication:** Configure access-token auth with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This is the supported setup for deployments and normal long-running Nastech processes on Render, Railway, Docker, and similar hosts.
...
**Runtime:** `terminal.vercel_runtime` supports `node24`, `node22`, and `python3.13`. If unset, Nastech defaults to `node24`.

**Persistence:** When `container_persistent: true`, Nastech snapshots the sandbox filesystem during cleanup and restores a later sandbox for the same task from that snapshot. Snapshot contents can include Nastech-synced credentials, skills, and cache files that were copied into the sandbox. This preserves filesystem state only; it does not preserve live sandbox identity, PID space, shell state, or running background processes.

**Background commands:** `terminal(background=true)` uses Nastech' generic non-local background process flow. You can spawn, poll, wait, view logs, and kill processes through the normal process tool while the sandbox is alive. Nastech does not provide native Vercel detached-process recovery after cleanup or restart.

**Disk sizing:** Vercel Sandbox does not currently support Nastech' `container_disk` resource knob. Leave `container_disk` unset or at the shared default `51200`; non-default values fail diagnostics and backend creation instead of being silently ignored.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/nastech-agent` (HPC convention) → `~/.nastech/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `nastech config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Nastech logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `nastech doctor` to check.
...
For the **SSH**, **Modal**, and **Daytona** backends, Nastech pushes your `~/.nastech/` state (credential files, skills, cache) into the remote sandbox during the session, and on teardown **syncs changed state files back** to their original host locations. Files that differ from what was originally pushed (compared by content hash) are applied back in place; new remote files under a synced directory (e.g. a skill the agent created remotely) are mapped back to the corresponding host path. Upload-only credential files are never overwritten on the host.

- The sync-back retries up to 3 times with backoff and refuses to extract remote archives larger than 2 GiB; set `terminal.sync_back_max_bytes` (bytes) in `config.yaml` to raise the cap for a larger state tree. Live sockets under the remote `~/.nastech/` (e.g. `gateway.sock`) are skipped rather than failing the transfer.
- The downloaded archive is staged under the system temp directory (`nastech-sync-back-<pid>-*`); leftovers from a hard-killed process are reclaimed on the next sync-back.
- Docker and Singularity use bind mounts (live host filesystem view) and don't need this.
- This covers Nastech state (`~/.nastech/`), **not** arbitrary working-tree files inside the sandbox — have the agent copy important artifacts out explicitly (e.g. `scp`, `modal volume put`) before the sandbox is destroyed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.nastech/cache/documents:/output" # Gateway-visible exports
```
...
Nastech resolves each listed variable from your current shell first, then falls back to `~/.nastech/.env` if it was saved with `nastech config set`.
...
This is a snapd limitation ([LP#1908448](https://bugs.launchpad.net/snapd/+bug/1908448)), not something Nastech can probe around. Either install Docker from Docker's apt repository instead of the snap (preferred — all hardening stays on), or opt in:
...
Docker sandboxes stay isolated by default. Nastech does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Nastech from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Nastech from
...
- `nastech config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `nastech config show` displays all skill settings under "Skill Settings" with the skill they belong to
...
Controls how much content Nastech loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.nastech.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
Three related caps control how much raw output a tool can return before Nastech truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, Nastech keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
Nastech also flags **provider-side elision**: when an MCP or web tool result embeds its own truncation markers (`...N more items`, `"has_more": true`, "saved to sandbox" notes), a one-line notice is appended to the result warning that the visible data is incomplete and should be paged/fetched before treating any enumeration as complete.
...
`nastech tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as nastech -w)
...
Nastech automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
auxiliary:
  compression:
    ...
    provider: "auto"                                # Provider: "auto", "openrouter", "nastech", "codex", "main", etc.
...
`context_timeout_seconds` (default `120`) is the same **inactivity budget** for in-agent `compress_context` — the conversation loop, preflight compaction, and manual `/compress` — so a hung summary model cannot stall a session indefinitely. Streamed summary tokens extend the wait; only a silent worker is cut off. The budget is floored at the auxiliary compression request's own timeout (`auxiliary.compression.timeout`, minimum 300s), so the host never gives up on a silent summariser before the request itself would — a reasoning summariser thinking before its first token, or a route that cannot stream, gets the same budget the provider call has. On timeout Nastech retries the summary once against the first entry of `auxiliary.compression.fallback_chain` (using that entry's own `timeout` when it declares one) — a stalled route never raises, so the auxiliary client's own fallback handling cannot see it. If that attempt also fails, or no fallback chain is configured, what happens next depends on whether the request still fits the model's context window: a request that fits is sent uncompressed this turn (the summary-failure cooldown stops the retry from repeating every turn); a request above the window cannot be sent at all, so Nastech commits its deterministic fallback summary (old tool results pruned, a static handoff in place of the summarised middle) instead of ending the turn — ending the turn with the "compression timed out" recovery result (and, on the messaging gateway, the automatic session reset) is the last resort, reached only when even the deterministic pass cannot shrink the transcript. Set to `0` to disable. Gateway session hygiene keeps its own `hygiene_timeout_seconds` path and is not double-wrapped.
...
```
Works with any provider: `nastech`, `openrouter`, `codex`, `anthropic`, `main`, etc.
...
| `nastech` / `openrouter` / etc. | not set | Force that provider, use its auth |
...
If another turn still holds the session lease when this budget expires, Nastech
fails closed: it does not load the transcript or run the model for the waiting
message. The user receives a rejection notice and must resend. Nastech does not
...
- **Needs-attention escalation.** A platform continuously in the retry queue past `agent.reconnect_attention_after` (default `7200` seconds = 2 hours, `0` disables) gets `needs_attention: true` and a `retrying_since` timestamp in gateway runtime status (`nastech status`), plus a WARNING log. Retries continue unchanged — this is a signal, not a circuit breaker. The flag clears on successful reconnect.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `nastech plugins` → Provider Plugins → Context Engine.

See [Memory Providers](./features/memory-providers.md) for the analogous single-select system for memory plugins.

## Iteration Budget

When the agent is working on a complex task with many tool calls, it can burn through its iteration budget (default: 500 turns). Nastech does **not** inject mid-task pressure warnings — earlier builds warned the model at 70%/90% budget, which caused models to abandon complex tasks prematurely and was removed in April 2026.

Instead, when the budget is actually exhausted (500/500), Nastech injects one message asking the model to wrap up and allows a single **grace call** so it can deliver a final response. If that grace call still doesn't produce text, the agent is asked to summarise what it accomplished.
...
`verify_on_stop` accepts `true` (on everywhere), `false` (off — the default), or `"auto"` (legacy surface-aware behavior: on for interactive coding surfaces — CLI, TUI, desktop — and programmatic callers; off for messaging surfaces like Telegram/Discord where the verification narrative reads as chat noise). Off is the default everywhere: fresh installs ship `false` and the config migration turned it off on existing installs, so enabling it is an explicit opt-in. The `NASTECH_VERIFY_ON_STOP` env var overrides the config value when set.

The evidence that feeds this guard (which test/lint/build commands ran, which files were edited since) lives in `~/.nastech/verification_evidence.db`. That ledger is only written or created while the guard is enabled; with `verify_on_stop: false` nothing is recorded and an existing file can be deleted freely.
...
When a standing goal is active, Nastech judges whether each assistant response satisfies it. If not, it feeds a continuation prompt back into the same session and keeps working until the goal is done, the turn budget is exhausted, or the user pauses/clears it. The turn budget is the real backstop — judge failures fail **open** (continue) so a flaky judge never wedges progress.

```yaml
goals:
  max_turns: 20   # Max continuation turns before Nastech auto-pauses the goal (default: 20)
```

`max_turns` caps how many continuation turns a goal can drive before Nastech auto-pauses it and asks the user to `/goal resume`. It protects against judge false negatives (goal actually done but judge says continue) and unbounded model spend on fuzzy or unachievable goals. See [Goals](./features/goals.md) for the full feature.

### API Timeouts

Nastech has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `NASTECH_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Raised to a 900s ceiling (`agent.local_stream_stale_timeout`) | `NASTECH_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 90s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `NASTECH_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `NASTECH_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Nastech raises this to 30 minutes when it detects a local endpoint. If you explicitly set `NASTECH_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.

The **stale stream detection** kills connections that receive SSE keep-alive pings but no actual content. For local providers (which don't send keep-alive pings during prefill) the default is raised to a finite 900-second ceiling instead of the 180s base — configurable via `agent.local_stream_stale_timeout` or the `NASTECH_LOCAL_STREAM_STALE_TIMEOUT` env var.

The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Nastech disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `NASTECH_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Nastech turns on cross-session prompt caching automatically when the active provider supports it — no user config needed.

For Claude on **native Anthropic**, **OpenRouter**, and **Nastech Portal**, Nastech attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across `nastech` sessions and across forked subagents for the first hour.

The Qwen Cloud (Alibaba DashScope) upstream caps cache TTL at 5 minutes, so Nastech uses the 5-minute breakpoint TTL there instead. Other Claude-via-third-party paths (AWS Bedrock, Azure Foundry) fall back to the provider's own caching defaults. xAI Grok uses a separate session-pinned conversation-id mechanism — see [xAI prompt caching](../integrations/providers.md#xai-grok--responses-api--prompt-caching).
...
The one explicit knob is the cache TTL tier Nastech requests on Anthropic-style breakpoints:
...
If you do not want Nastech to auto-generate titles after the first exchange, set
`auxiliary.title_generation.enabled: false`. Manual titles still work through
`/title` and `nastech sessions rename`.
...
explicit repair command `nastech sessions retitle-skills` still calls the model. `enabled: false`
still disables both stages.

In Nastech Desktop, a plain-text paste over 3,000 characters becomes a generated `.txt`
...
Every model slot in Nastech — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Nastech ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Nastech uses that provider's built-in auth and base URL.

Available providers for auxiliary tasks: `auto`, `main`, plus any provider in the [provider registry](../reference/environment-variables.md) — `openrouter`, `nastech`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `gemini`, `qwen-oauth`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth`, `deepseek`, `nvidia`, `xai`, `xai-oauth`, `ollama-cloud`, `alibaba`, `bedrock`, `huggingface`, `arcee`, `xiaomi`, `kilocode`, `opencode-zen`, `opencode-go`, `commandcode`, `commandcode-anthropic`, `ai-gateway`, `azure-foundry` — or any named custom provider from your `providers:` dict (e.g. `provider: "beans"`).
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `nastech model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `nastech model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if Nastech is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  vision:
    provider: "auto"           # "auto", "openrouter", "nastech", "codex", "main", etc.
  ...
  # Kanban triage specifier — `nastech kanban specify <id>` (or the
...
Each auxiliary task can optionally define a `fallback_chain` — a list of provider/model entries that Nastech tries when the primary auxiliary provider fails due to rate limits, connectivity issues, or payment restrictions:
...
When the primary auxiliary provider (`openrouter` / `openai/gpt-4o-mini`) returns a rate-limit, connection timeout, or payment-required error, Nastech walks the `fallback_chain` in order. It skips entries whose provider matches the already-failed provider, and tries each remaining entry until one succeeds or the chain is exhausted. If all fallbacks fail, Nastech falls back to the main agent model as a final safety net.

Each entry supports the same three knobs as any auxiliary task config:

| Key | Description |
|-----|-------------|
| `provider` | Provider name (`nastech`, `openrouter`, `anthropic`, `gemini`, `main`, etc.) |
...
The shape mirrors what OpenRouter accepts in the chat completions request body. Nastech forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.nastech/.env`):
...
|----------|-------------|-------------|
| `"auto"` | Best available (default). Vision tries OpenRouter → Nastech → Codex. | — |
| `"openrouter"` | Force OpenRouter — routes to any model (Gemini, GPT-4o, Claude, etc.) | `OPENROUTER_API_KEY` |
| `"nastech"` | Force Nastech Portal | `nastech auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `nastech model` → ChatGPT or Codex Subscription |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `nastech model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `nastech model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `nastech model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Nastech uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
```
Run `nastech model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider Nastech uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `nastech config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
field — OpenRouter ignores it for them. Nastech transparently routes your
...
For other models routed through OpenRouter, Nastech reads the live model
...
automatically without waiting for a Nastech update; when the catalog is
unreachable or a model isn't listed, Nastech falls back to its built-in
...
display:
  ...
  # blocking prompts) so Warp's tab status and notification mailbox track Nastech. No extra keys needed.
  ...
  credits_notices: true   # Nastech credits status-bar notices (usage bands, grant-spent, depleted). false = silence them; /usage still works
...
- `+X -Y` line deltas appear only when the tool result already reports a diff (currently `patch`). Nastech never shells out to git to compute them, so a `write_file` edit is counted without a delta.
...
When `display.file_mutation_verifier` is `true` (default), Nastech appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and the target file was neither written successfully afterwards (under any spelling of its path) nor otherwise changed on disk before the turn ended. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
Set `file_mutation_verifier: false` (or `NASTECH_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end — a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.

**Trust the verifier over the model's summary.** The footer means the listed edit calls **failed** and Nastech saw no later change to those files, even if the assistant's closing message says the task is done. It only tracks `write_file`/`patch` receipts plus a modification-time check at turn end, so run `git status` or `read_file` to confirm what actually landed. Common causes:

- **Write denied** — path is on the credential denylist or outside `NASTECH_WRITE_SAFE_ROOT` (see [File write safety](./security.md#file-write-safety))
...
⚠️ File-mutation verifier: 2 file edit(s) FAILED this turn despite any wording above that may suggest otherwise. Run `git status` or `read_file` to confirm what actually landed.
  • ~/.nastech/cron/jobs.json — [patch] Write denied: '…' is outside NASTECH_WRITE_SAFE_ROOT (/path/to/project)
  • ~/.nastech/scripts/monitor.py — [write_file] Write denied: '…' is outside NASTECH_WRITE_SAFE_ROOT (/path/to/project)
```

If writes to Nastech state (cron jobs, skills, scripts under `~/.nastech/`) are failing, check whether `NASTECH_WRITE_SAFE_ROOT` is set in your environment. For cron changes, use the `cronjob` tool or `nastech cron edit` instead of patching `jobs.json` directly.
...
You can also set this per-session with the `NASTECH_LANGUAGE` env var, which overrides the config value.
...
`interim_assistant_messages` is gateway-only. When enabled, Nastech sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
send `originator: nastech-agent` and `User-Agent: NastechAgent/<version>`.
...
stt:
  ...
  # prompt: "Nastech, Teknium, Nastech Research, kanban"   # Static vocabulary hint (see below)
  local:
...
Language resolution is the same for **every** STT provider (local, groq, openai, mistral, xai, elevenlabs, deepinfra, command providers, and plugins): `stt.<provider>.language` → `stt.language` → `NASTECH_LOCAL_STT_LANGUAGE` env var → provider auto-detect. **The default is `stt.language: "en"`** — Whisper auto-detection frequently misidentifies short or accented clips, which shows up as voice notes transcribed in the wrong language. Non-English speakers should set `stt.language` to their language code once (e.g. `"es"`, `"zh"`, `"uk"`); set it to `""` to restore auto-detection for multilingual use.
...
- `groq` uses Groq's Whisper-compatible endpoint and reads `GROQ_API_KEY`. Pass `stt.groq.language` (or the global `NASTECH_LOCAL_STT_LANGUAGE` env var) to skip auto-detection and reduce latency.
...
An explicitly selected `stt.provider` is honored strictly — if it's unavailable, transcription errors with guidance to run `nastech tools` rather than switching providers. Only when no provider has ever been selected does Nastech auto-detect in this order: `local` → `groq` → `openai`.
...
stt:
  provider: "local"
  prompt: "Nastech, Teknium, Nastech Research, kanban, Ollama"
```
...
When the cap is reached, Nastech returns a direct limit message naming which
surfaces hold the slots. Existing active sessions keep their normal behavior.
Run `nastech status` to see the current slot usage and every holder.

The canonical key is top-level `max_concurrent_sessions`. Nastech also accepts
`gateway.max_concurrent_sessions` as a fallback, but the top-level key wins when
both are set.

The cap is enforced with a local runtime lease file and is best-effort: Nastech
fails open if the registry cannot be read or locked so users are not stranded.
It is intended for a single host/profile runtime, not a shared `$NASTECH_HOME`
...
container after `nastech update` restarts the backend) still counts toward the
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Nastech to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Nastech still keys DMs by chat/DM ID as usual.
...
Control what Nastech does when an unknown user sends a direct message:
...
- `pair` is the default for chat-style DM platforms. Nastech denies access, but replies with a one-time pairing code in DMs.
...
- `decline` sends one short, polite decline instead of a pairing code, then stays silent toward that sender for 24 hours. Override the default text:
  ...
  `nastech gateway setup` offers this as "Politely decline unknown senders" when you leave the allowlist empty; it writes `platforms.<platform>.unauthorized_dm_behavior: decline`.
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Nastech's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `nastech tools`:
...
web:
  ...
  # Pin Exa/Parallel to a tier (set by the nastech tools Free/Paid rows).
...
**Backend selection:** The runtime always uses the stored `web.backend` selection (set via `nastech tools`; `nastech` routes through the managed Tool Gateway). Only if no web backend has ever been selected is one auto-detected from available API keys: if only `SEARXNG_URL` is set, SearXNG is used; if only `EXA_API_KEY` is set, Exa; if only `TAVILY_API_KEY` is set, Tavily; if only `PERPLEXITY_API_KEY` is set, Perplexity; if only `PARALLEL_API_KEY` is set, Parallel; if only `KEENABLE_API_KEY` is set, Keenable. With **no selection and no credentials at all**, requests rotate round-robin across the keyless free-tier ring (Exa / Parallel / Firecrawl / Keenable) with automatic next-in-line failover on rate limits — see the [Web Search guide](./features/web-search.md) for details. Once a selection exists, adding a key to `.env` does not change the route. Selecting Tavily, Firecrawl, or Keenable in `nastech tools` also works without a key.
...
**Exa:** Set `EXA_API_KEY` in `~/.nastech/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.nastech/browser_recordings/
  # Optional CDP override — when set, Nastech attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when Nastech creates a tab
...
`nastech doctor` (and the startup config check) reports a value the runtime cannot load — a typo such as `Asia/Tokio` would otherwise silently put the agent clock and every cron schedule on server-local time. `NASTECH_TIMEZONE` overrides this key when set.
...
Control how Nastech handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `NASTECH_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: nastech chat --checkpoints). Default: false (opt-in).
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Nastech falls back to `OPENAI_API_KEY` only. When `delegation.provider` is set alongside `delegation.base_url`, the explicit endpoint and key still win, but that provider's request settings (`extra_body` overrides and max output tokens from your `custom_providers` entry) are carried into the subagent.
...
Configure how long Nastech waits for a response to a clarifying question. One value covers every surface — the classic CLI modal, the TUI/Desktop card, and the messaging gateway. The canonical key is `agent.clarify_timeout` (default `3600` seconds; `0` or less = unlimited); a legacy top-level `clarify.timeout` is still honored if explicitly set:
...
Nastech uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.nastech/SOUL.md` or `$NASTECH_HOME/SOUL.md` |
| `.nastech.md` / `NASTECH.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.nastech.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Nastech automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`nastech`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.nastech/config.yaml`; if unset, home directory `~` |
...
`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.nastech/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
...
`force_ipv4` — on servers with broken or unreachable IPv6, Python resolves AAAA records first and can hang for the full TCP timeout before falling back to IPv4. Nastech already races IPv6 and IPv4 for every outbound connection it makes (Happy Eyeballs, RFC 8305: the IPv4 attempt starts 250 ms after IPv6 and whichever connects first wins), so an advertised-but-blackholed IPv6 route costs about a quarter second per connection instead of the full timeout. This covers the gateway's WebSocket dials (relay connector, platform adapters) as well as HTTP. Set this to `true` only when you want to skip IPv6 entirely and connect over IPv4 directly. `nastech doctor` runs an `IPv6 route` check that detects a dead IPv6 path and points at this setting.
...
- `seen` — internal state. Nastech latches each shown hint here so it never fires again; the profile-build offer is also recorded here once shown. Don't hand-edit it — wipe the whole `onboarding` section if you want to re-see all hints.
...
dashboard:
  ...
  public_url: ""              # Full public authority for OAuth redirect_uri (env: NASTECH_DASHBOARD_PUBLIC_URL)
...
- `oauth` / `basic_auth` / `drain_auth` — auth provider config read by the bundled dashboard-auth plugins. The drain secret itself is **not** set here; it's provisioned via the `NASTECH_DASHBOARD_DRAIN_SECRET` env var. See [Web Dashboard](./features/web-dashboard.md) for full auth setup.
...
- `startup_orphan_sweep` (default `true`) — the WS-orphan reap timer above is in-process, so a gateway restart (update, crash, systemd) before it fires leaves the session row open forever — phantom "active" work in `/resume` and dashboards. On every gateway boot — both the stdio TUI (`entry.main`) and the desktop/dashboard WebSocket sidecar (`handle_ws`) — rows with source `tui` / `desktop` / `subagent` / `unknown` (a row the token-accounting guard had to materialize itself) whose start time **and** newest message are both older than the session TTL (`NASTECH_TUI_SESSION_TTL_S`, default 6 hours) are closed with `end_reason: startup_orphan_reap`. Messaging-platform sessions (Telegram, Discord, …) are never touched, live in-memory sessions (a client that already resumed) are excluded, and swept sessions remain resumable.
