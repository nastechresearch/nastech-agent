...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `NASTECH_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
`NASTECH_SUPERVISED_CHILD` marker they export.
...
the job uses KeepAlive or controls Nastech.
...
unattended ones: a cron job, `nastech chat -q` run or webhook session under
...
re-save it with `nastech config edit`. Other malformed values are ignored with
...
:::caution
The list is read when Nastech starts. A pattern you remove while a session is
already running stays approved in that session until it next writes the file
(the next time you answer `always` to a prompt) or you restart Nastech. If you
...
| Nastech secret stores | `.env`, `.anthropic_oauth.json`, `auth/google_oauth.json`, Bitwarden cache (`cache/bws_cache.json`, `cache/bws_cache.enc.json`), `vault/`, `browser-profile/`, `mcp-tokens/`, `pairing/` under NASTECH_HOME (active profile and global root). Control files (`auth.json`, `config.yaml`, `webhook_subscriptions.json`) are read-denied but stay writable. |
...
With the switch off Nastech never reads or refreshes those files: the `claude_code` credential-pool row disappears, `nastech auth list` prints one line saying so, and the log carries one INFO line per process. Only automatic adoption is affected — `nastech auth add openai-codex` still asks before importing an existing Codex CLI login. Add your own logins with `nastech auth add anthropic` / `nastech auth add openai-codex`.
...
Your own `SOUL.md` in `NASTECH_HOME` is treated differently: it is a file you wrote (file-tool writes to it
need your approval, and project checkouts never supply it), so a scanner hit there **does not block the
file**. Nastech logs a warning naming the matched pattern, loads the file as usual, and `/context` lists it as
...
the file. The exception does not extend to a `SOUL.md` shipped by a profile distribution: `nastech profile
install <git-url>` and `nastech profile update` copy a third party's `SOUL.md` into the profile home without
