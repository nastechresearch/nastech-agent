...
To run separate Nastech instances per user — each with their own config, memory, and skills — use [profiles](../profiles.md). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
