...
3. Run `nastech whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.nastech/platforms/whatsapp/session`. This means:
...
- Agent responses are prefixed with "𓄃 **Nastech Agent**" by default. You can customize or disable this in `config.yaml`:
...
- The `~/.nastech/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.nastech/platforms/whatsapp/session`
