...
transport in `~/.nastech/config.yaml`:
...
credentials in `~/.nastech/.env`. Unknown `*_security` values log a warning and
fall back to the secure default. Only disable `*_tls_verify` for loopback hosts —
Nastech logs a warning when verification is off for any other host.
