...
Discord behavior is controlled through two files: **`~/.nastech/.env`** for credentials and env-level toggles, and **`~/.nastech/config.yaml`** for structured settings. Environment variables always take precedence over config.yaml values when both are set.
...
| `NASTECH_DISCORD_TEXT_BATCH_DELAY_SECONDS` | No | `0.6` | Grace window the adapter waits before flushing a queued text chunk. Useful for smoothing streamed output. |
| `NASTECH_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS` | No | `2.0` | Longer quiet period for near-limit Discord splits and continuation chunks from a recently tagged bot in the same channel. |
...
The `discord` section in `~/.nastech/config.yaml` mirrors the env vars above. Config.yaml settings are applied as defaults — if the equivalent env var is already set, the env var wins.
...
No extra configuration is needed — any skill installed via `nastech skills install` is automatically registered as a Discord slash command on the next gateway restart.
...
Any file type a user uploads is accepted. Authorization to message the agent is the gate — not the file extension. Every upload is downloaded, cached under `~/.nastech/cache/documents/`, and surfaced to the agent as a `DOCUMENT`-typed message event so it can inspect the file with `terminal` (`ffprobe`, `unzip`, `file`, `strings`, etc.) or `read_file`.
...
**Cause**: Nastech requests privileged Gateway Intents that are not enabled for your bot in the Developer Portal. Discord then rejects the WebSocket connection. Nastech always requests **Message Content Intent**. It also requests **Server Members Intent** when your allowlist uses usernames (not numeric IDs) or when `DISCORD_ALLOWED_ROLES` is set. Presence Intent is not required.
...
3. Click **Save Changes**, then restart the gateway (`nastech gateway restart`).

The gateway log should name the exact intent(s) Nastech requested. Until they are enabled, Discord will keep rejecting the connection — this is a portal configuration error, not a flaky network issue.
...
**Fix**: Add your User ID to `DISCORD_ALLOWED_USERS` in `~/.nastech/.env` and restart the gateway.
...
**Fix**: Set this in `~/.nastech/config.yaml` and restart the gateway:
...
```bash
# ~/.nastech/.env — works alongside or instead of DISCORD_ALLOWED_USERS
...
```bash
# ~/.nastech/.env — env vars win over config.yaml
