...
devtunnel port create nastech-bot -p 3978 --protocol http  # replace 3978 with TEAMS_PORT if changed
...
The public tunnel URL uses HTTPS, but Nastech' local webhook listener uses plain HTTP. The tunnel terminates TLS and forwards HTTP to port `3978`; do not configure the local tunnel port as HTTPS.
...
For a permanent server, terminate TLS at a reverse proxy and forward requests to the plain HTTP Nastech listener, normally `http://127.0.0.1:3978`. Register the proxy's public HTTPS endpoint with Teams:
...
Make sure the public HTTPS endpoint is reachable from the internet and uses a valid TLS certificate. Teams rejects self-signed certificates. Keep the Nastech listener behind the proxy; port `3978` does not serve HTTPS itself.
...
| Logs show `"UNKNOWN / HTTP/1.0" 400` when Teams sends a message | The tunnel or reverse proxy is forwarding HTTPS to Nastech' plain HTTP listener. Terminate TLS at the proxy and forward HTTP to port `3978` |
