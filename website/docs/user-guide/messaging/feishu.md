...
:::warning
Without a published card callback, Feishu will still successfully *send* interactive cards (sending only requires `im:message:send` permission), but clicking any button returns error 200340. The card appears to work — the error only surfaces when a user interacts with it, and the click never reaches Nastech (nothing is logged), because Feishu rejects it before delivering the callback.
:::

Error codes 200672 / 200673 indicate the callback *did* reach Nastech and Feishu rejected the response; if you see them, please file an issue with the matching `gateway.log` lines.
