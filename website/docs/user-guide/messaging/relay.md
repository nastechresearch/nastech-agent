...
1. Resolves a fresh Nastech Portal access token from your existing login
   ...
   air-gapped / self-hosted-IdP path, no Nastech Portal involved): with
...
`nastech gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.nastech/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
| `gateway.idp.token_url` | `config.yaml` | When set, enrollment/provisioning authenticates against your own IdP instead of Nastech Portal: OAuth2 client-credentials when `gateway.idp.client_id`/`client_secret` are also set; otherwise an ambient token endpoint (plain GET returning the token, raw or `{"access_token": …}`). |
