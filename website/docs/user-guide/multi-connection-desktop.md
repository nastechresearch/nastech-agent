...
# Connecting Desktop to Many Nastech Instances

Register every Nastech backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Nastech Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Nastech gateway…"**) deep-links straight to
...
list of Nastech gateways. Its intro says it plainly: *"Manage this device and
every Nastech gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Nastech runtime managed by this app." | automatic |
| **Remote gateway** | "A Nastech gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Nastech install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Nastech Cloud** | "A hosted instance discovered through your Nastech Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Nastech Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Nastech Cloud** kind in the add-connection
...
The Nastech Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Nastech Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Nastech Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `nastech serve` backend,
     ...
     - **Authentication** — choose **Session token** or **OAuth**:
       - **Session token** — paste the dashboard session token from the
         ...
         token."*
       - **OAuth** — sign in through the Nastech Portal browser flow; no token
         to paste.
   - *SSH only:*
     ...
     - **Nastech path (optional)** — full path to the `nastech` executable on
       the remote (for example `/opt/nastech/bin/nastech`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `nastech` on its `PATH` and **Test** reports *"Nastech is not
...
Gateways are machines or hosted backends; profiles are isolated Nastech agents
...
connection is registered) dispatches `nastech update` to every eligible
...
- **Nastech Cloud** instances are skipped with a *"Managed by Nastech Cloud"*
...
panel, ⌘K **Update Nastech**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `nastech serve` is running on the remote host, the
...
- **"Update Nastech Desktop to chat with agents on other connections"** — the
