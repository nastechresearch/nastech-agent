...
These terms describe different parts of Nastech:
...
- **Agent** is the running Nastech assistant that uses that configuration and
  state. "Nastech Agent" also names the product.
...
- **Messaging bot** is an account on a platform such as Telegram, Discord, or
  Slack, connected to Nastech through the gateway. Its
...
```bash
nastech profile create coder       # creates profile + "coder" command alias
...
:::tip
Quickest setup: run `nastech setup --portal` inside the new profile to wire up models + tools at once. See [Nastech Portal](/integrations/nastech-portal).
:::
...
You can also set or auto-generate the description later with `nastech profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies **everything** — config, API keys, personality, all memories, skills, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. When cloning from the default profile, the local-model runtime trees (`models/`, `runtimes/`, `node/` — downloaded weights and managed binaries, re-fetched on demand) are skipped too, as `nastech backup` already does. **Cron jobs are not cloned** either: they are scheduled work bound to the source profile and its delivery channel, and a clone that inherited them would run every job twice (two gateways, same job ids). The new profile starts with an empty `cron/`. For a full backup including history and cron jobs, use `nastech profile export` or `nastech backup` instead.

:::note OAuth logins are never copied
Anthropic (Claude Pro/Max), OpenAI Codex, and xAI OAuth logins use **single-use refresh tokens** — a copy of one is not a second credential, it is the same credential with two owners, and the first profile to refresh it revokes it for every other copy. `--clone-all` (and the dashboard's credential mirroring) therefore drops those OAuth rows from the clone. Static API keys are copied as usual. Sign the new profile into the OAuth provider itself: `nastech -p <name> auth add <provider>` (or `nastech -p <name> model`).
:::

### Every profile owns its credentials

A named profile resolves providers from **its own** `auth.json` and `.env` only. It never inherits the root profile's logins or API keys, and a token refresh inside a profile never writes to the root store. A profile with no provider of its own is asked to set one up (`nastech -p <name> model`, or `nastech -p <name> auth add <provider>`) — it does not silently act as the owner. `nastech update` lists every profile that has no provider of its own so a bot never goes quiet unannounced.
...
parks the duplicate adapter (and `nastech gateway migrate --multiplex` refuses with one
duplicate-credential blocker per platform). Configure the new profile's own bots with
`nastech -p <name> setup` or the dashboard Messaging page.

```bash
nastech profile create twin --clone --clone-channels   # keep the source's bots anyway
```
...
without a clone flag. `nastech profile list` prints the same warning for any existing
...
The alias works with every nastech subcommand — it's just `nastech -p <name>` under the hood.
...
nastech chat -p coder -q "hello"    # works in any position
```

### Sticky default (`nastech profile use`)

```bash
nastech profile use coder
nastech chat                   # now targets coder
nastech tools                  # configures coder's tools
nastech profile use default    # switch back
```

Sets a default so plain `nastech` commands target that profile. Like `kubectl config use-context`.
...
- **`nastech profile`**: Shows current profile name, path, model, gateway status
...
```bash
coder gateway install         # creates nastech-gateway-coder systemd/launchd service
assistant gateway install     # creates nastech-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `nastech profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `nastech -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
default for **future CLI/gateway runs** (same as `nastech profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`nastech update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
nastech profile list           # show all profiles with status
nastech profile show coder     # detailed info for one profile
nastech profile rename coder dev-bot   # rename (updates alias + service)
nastech profile migrate-identity coder dev-bot   # retry a rename's identity migration
nastech profile purge-identity dev-bot   # retry a delete's identity purge
nastech profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
nastech profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.nastech` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
nastech profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `nastech profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.nastech/profile.yaml`; remove that line to
...
Use `--yes` to skip confirmation: `nastech profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.nastech`). To remove everything, use `nastech uninstall`.
:::
...
isolated by `NASTECH_HOME`, not by changing `HOME`. Container backends still use
`{NASTECH_HOME}/home` for persistent tool state, and host users who need strict
...
nastech profile export coder
nastech profile import ./coder.tar.gz --name coder
```
