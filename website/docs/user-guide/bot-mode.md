...
- **Direct messages** — a Bot reaches a teammate's Bot Chat through the standard CLI: `nastech -p <bot> chat --in ~ -c "Bot Chat" --create-if-missing -Q -q "Message from 🤖 <sender> (@<sender>): ..."`. The receiving Bot sees the message the next time it runs and knows how to reply, because the messaging protocol is part of its Bot Chat system prompt.
...
nastech peer list
nastech peer dm spark "Message from 🤖 dixie (@dixie): disk status?"
nastech peer dm spark/researcher "..."   # named profile on a multiplexed peer
```
