...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/autonomous-ai-agents/dynamic-workflow` |
...
| Related skills | [`nastech-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-nastech-agent), [`simplify-code`](/docs/user-guide/skills/bundled/software-development/software-development-simplify-code) |
...
for serial chains. For a refactor or fix campaign on nastech-agent itself, load
`nastech-agent` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `NASTECH_HOME`,
