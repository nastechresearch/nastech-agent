...
| | |
|---|---|
| Source | Optional — install with `nastech skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Nastech MCP catalog. The
...
Two halves, in this order: the editor side must be up before Nastech connects.
...
### One-time, Nastech side

    nastech mcp install unreal-engine
...
`~/.nastech/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Nastech — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Nastech connects
...
2. Start the Nastech session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Nastech
they appear as:

| Nastech tool | Purpose |
|---|---|
...
then happens at `nastech mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Nastech runs batched calls
...
- **Start order matters.** Editor + server up first, then the Nastech
...
- **Timeouts on long operations.** Nastech' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.nastech/config.yaml` for
