...
| List/filter by metadata (age, source, cost, tokens, workspace) | `nastech sessions list` / `stats` via terminal |
| Rename | `nastech sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `nastech sessions archive <filters>` |
| Delete (destructive) | `nastech sessions delete` / `nastech sessions prune <filters>` |
| Export before deleting anything valuable | `nastech sessions export --session-id <id> --format md` |
...
`nastech sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `nastech sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `nastech sessions list`
