...
This wrapper skill teaches the Nastech `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Nastech autodetection is a
planned cua-driver follow-up, so currently point Nastech at the resulting
...
1. **Config grant (standard and unrestricted modes).** When
   ...
   Nastech applies the same host-side floor in unrestricted mode. If it is not
...
Explicit Nastech YOLO (`--yolo`, `/yolo`, or `approvals.mode: off`) launches an
...
Nastech autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Nastech at that
