...
description: "How Nastech recovers state.db when the FTS index or the file itself is corrupt"
---
...
`nastech doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `nastech doctor --fix` while the gateway
is still up. Next steps:

```bash
nastech gateway stop
NASTECH_HOME="$HOME/.nastech" nastech sessions recover --source "$HOME/.nastech/state.db" --inspect-only
# if recoverable:
NASTECH_HOME="$HOME/.nastech" nastech sessions recover --source "$HOME/.nastech/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
nastech gateway stop
NASTECH_HOME="$HOME/.nastech" nastech sessions repair --check-only
NASTECH_HOME="$HOME/.nastech" nastech sessions repair
```
