...
renderer (`nastech cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
The same store fields drive every topology: a standalone `nastech -p X gateway
...
agent↔Nastech wire contract lives in [Chronos managed-cron contract](chronos-managed-cron-contract.md).
