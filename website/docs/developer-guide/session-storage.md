...
Source files: `nastech_state.py` (facade) plus the `nastech_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

## Nastech home and profile isolation

`get_nastech_home()` is the authoritative filesystem resolver for state and
configuration. It uses a context-local override first, then the `NASTECH_HOME`
environment variable, and finally the platform default (`~/.nastech` on macOS
and Linux; `%LOCALAPPDATA%/nastech` on Windows). Consequently, the default
database is always `get_nastech_home() / "state.db"`, not a path that callers
should hard-code as `~/.nastech/state.db`.

Named profiles are isolated directories: a profile named `coder`, for example,
uses `<default Nastech root>/profiles/coder/` and therefore has its own
...
profile must retain or pass that profile's `NASTECH_HOME`; falling back to the
...
of Nastech. An explicit `--profile`/`-p` resolves that profile and writes the
resolved directory to `NASTECH_HOME`. Without an explicit selector, a
profile-specific `NASTECH_HOME` is preserved; otherwise the bootstrap can use
the default root's active-profile selection. `HOME` only determines the
platform default used when no context override or `NASTECH_HOME` is available.
Changing `HOME` is not a safe way to select a named profile. In particular, a
subprocess that drops `NASTECH_HOME` can fall back to the default profile even
when another profile is active, so subprocess spawners should pass
`NASTECH_HOME` explicitly.

Use `display_nastech_home()` only for user-facing text. It formats the resolved
home relative to the user's home directory when possible (for example,
`~/.nastech/profiles/coder`); it does not provide a separate resolution rule.

### Test isolation guard

Tests must use a temporary `NASTECH_HOME` or an explicit temporary database
path. The live-system guard raises before a test-context process opens a
production `state.db` under the real default Nastech root or a real named
profile, preventing fixture data or SQLite side effects from reaching a live
installation.

`NASTECH_STATE_DB_GUARD_BYPASS=1` is a test-only escape hatch for a spawned
...
either bypass in normal Nastech commands, development shells, or application
...
Each named profile stores its transcript in its own `$NASTECH_HOME/state.db`,
including when one `nastech serve` process serves several profiles. In-session
...
splice boundary, Nastech excludes only that leading item when it exactly matches
...
`nastech sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `nastech_state_common.py` (applied by `nastech_state_schema.py`) for the full current column list
...
an authenticated `nastech serve` (OAuth or the basic username/password provider)
...
indexed columns — see `SCHEMA_SQL` in `nastech_state_common.py` for the exact SQL.
...
Default path: `get_nastech_home() / "state.db"` — `~/.nastech/state.db` for the
default profile, `~/.nastech/profiles/<name>/state.db` for a named profile, or
wherever `NASTECH_HOME` points (see [Nastech home and profile isolation](#nastech-home-and-profile-isolation)).
