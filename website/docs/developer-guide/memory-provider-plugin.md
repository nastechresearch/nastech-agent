...
description: "How to build a memory provider plugin for Nastech Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Nastech Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](./context-engine-plugin.md), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `nastech plugins`.
:::

## Installation Layouts

Nastech discovers memory providers from four sources, in this precedence order:
...
Nastech, in `$NASTECH_HOME/plugins/<name>/` when installed by a user, or in
`./.nastech/plugins/<name>/` for a project-local one:
...
`nastech_agent.memory_providers` group. The entry-point name is the provider
name users select in `memory.provider`; its value points to the provider's
`register(ctx)` function:

```toml title="pyproject.toml"
[project.entry-points."nastech_agent.memory_providers"]
...
package layout. No copy under `$NASTECH_HOME/plugins/` is required.

A package entry point gets everything a directory install does, including the
two files Nastech reads from disk rather than importing — `config_schema.py`
(the dashboard config panel) and `cli.py` (your `nastech <provider>`
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          nastech_home (str): Active NASTECH_HOME path. Use for storage.
        """
...
| Keyword | Meaning |
|---|---|
| `nastech_home` | Active profile's storage directory. |
...
| `session_title_source` | Stored title provenance, when available: `derived`, `llm`, or `user`. Automatic sources must not be mistaken for explicit identity overrides; missing provenance retains a provider's legacy behavior. Shared constants live in `nastech_state_common.py`. |
...
| `agent_workspace`, `agent_context` | Runtime agent scope. `agent_workspace` is `nastech`; `agent_context` is `cron` for scheduler runs, `subagent` for `delegate_task` children, else `primary` — skip automatic writes for the non-primary values. |
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `nastech memory setup` | **Yes** |
| `save_config(values, nastech_home)` | Write non-secret config to native location | **Yes** (unless env-var-only) |
...
None of the providers bundled with Nastech advertise checkpoint API v2 — the
...
The gate binds to every compaction authority, not just the Nastech
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Nastech
...
Every setup surface Nastech gives a bundled provider is driven by files in the provider's
...
| `nastech memory setup` wizard | `get_config_schema()` declares the fields the wizard prompts for, `save_config(config, nastech_home)` persists them, `post_setup(nastech_home, config)` runs afterwards for anything interactive (OAuth, first sync); `get_status_config()` feeds `nastech memory status` |
| `nastech <provider> …` subcommands | `cli.py` with `register_cli(subparser)` ([Adding CLI Commands](#adding-cli-commands)) |
| Python dependencies | `pyproject.toml` `[project] dependencies` (or `python_dependencies` in `plugin.yaml`); installed under Nastech' own pins at install time and re-applied across `nastech update` |

Your provider's name, `memory.<name>` config section, data directory and tool names are the
contract with existing users. A provider that moves out of core keeps all four; Nastech then
installs the catalog plugin automatically for anyone whose `memory.provider` still names it
(on `nastech update`, and once at agent start when `security.allow_lazy_installs` is on).

## Config Schema

`get_config_schema()` returns a list of field descriptors used by `nastech memory setup`:
...
**`sync_turn()` MUST be non-blocking.** If your backend has latency (API calls, LLM processing), run the work in a daemon thread — spawned with `agent.memory_provider.spawn_context_thread`, never a bare `threading.Thread`. Profile isolation (the active `NASTECH_HOME`, the per-turn secret scope) lives in `contextvars`, and a plain thread starts with an empty context: under multiplexed profiles it would silently write into the *default* profile's store, and `get_secret()` fails closed there.
...
The same applies to prefetch and writer threads. Small JSON config sidecars (`$NASTECH_HOME/<provider>.json`) are read with `utils.read_json_or_empty` and written with `utils.atomic_json_write`; anything under `config.yaml` goes through `nastech_cli.config.save_config(..., merge_existing=True)`.
...
the `messages` parameter; Nastech will continue calling them with the legacy
signature.
...
All storage paths **must** use the `nastech_home` kwarg from `initialize()`, not hardcoded `~/.nastech`:
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `nastech my-provider status`, `nastech my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `nastech <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `nastech --help`.
...
def register_cli(subparser) -> None:
    """Build the nastech my-provider argparse tree.
