...
Nastech runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.nastech/plugins/`, registering a backend the user selects
...
| `nastech setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `nastech status` / `nastech doctor` | `doctor_checks()` |
...
```python title="~/.nastech/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.nastech"    # where synced cache files land, or None
...
```yaml title="~/.nastech/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
nastech plugins enable acmebox
nastech config set terminal.backend acmebox
```
