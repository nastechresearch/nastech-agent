...
`load_nastech_dotenv()` often runs at import time **before** plugins register.
Nastech then re-pulls secrets after plugin discovery when any **enabled**
...
- `nastech update` never resolves external sources — not in the updater process
