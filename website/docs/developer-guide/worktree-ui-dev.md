...
hgui() (
  ...
  root="$(_nastech_root)" || { print -u2 'hgui: not in a Nastech checkout'; return 1; }
  deps="${NASTECH_GUI_DEPS_CHECKOUT:-$NASTECH_MAIN_CHECKOUT}"
  desktop="$root/apps/desktop"

  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _nastech_link_deps "$desktop" "$deps/apps/desktop" || return 1
    _nastech_link_deps "$root" "$deps" || return 1
  else
  ...
  export NASTECH_DESKTOP_NASTECH_ROOT="$root"
  export NASTECH_DESKTOP_PYTHON="$NASTECH_MAIN_CHECKOUT/.venv/bin/python"
  export NASTECH_DESKTOP_CWD="$root"
  export NASTECH_DESKTOP_DEV_SERVER="http://127.0.0.1:$vite_port"
  export NASTECH_DESKTOP_CDP_PORT="$cdp_port"
  export NASTECH_DESKTOP_USER_DATA_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/nastech-hgui/slot-$slot"
  # A userData override would otherwise also relocate the agent's home.
  export NASTECH_HOME="${NASTECH_HOME:-$HOME/.nastech}"
...
For example, after setting `NASTECH_MAIN_CHECKOUT` and sourcing the helpers:

```bash
# Terminal 1: main checkout
cd "$NASTECH_MAIN_CHECKOUT"
HGUI_SLOT=0 hgui

# Terminal 2: an existing worktree
cd /path/to/nastech-worktree
HGUI_SLOT=1 hgui
```
...
|----------|----------------|
| `HGUI_SLOT` | Helper-only slot number, `0`–`9`; not a Nastech setting. |
| `NASTECH_DESKTOP_NASTECH_ROOT` | Runs the backend from this worktree, not the packaged/PATH runtime. |
| `NASTECH_DESKTOP_PYTHON` | Reuses the main checkout's Python environment. Adjust for an installation that uses `venv` rather than `.venv`. |
| `NASTECH_DESKTOP_CWD` | Roots new desktop work in the worktree. |
| `NASTECH_DESKTOP_DEV_SERVER` | Points Electron at this instance's Vite server. |
| `NASTECH_DESKTOP_CDP_PORT` | Gives each instance its own renderer debugging port. |
| `NASTECH_DESKTOP_USER_DATA_DIR` | Separates Electron's single-instance lock, browser storage, and desktop preferences. |
| `NASTECH_HOME` | Explicitly preserves the agent home despite the Electron user-data override. |
...
The default `NASTECH_HOME` is shared: sessions, configuration, credentials, and profiles remain the same. Avoid editing the same conversation from both instances. For destructive tests or incompatible database migrations, pass a separate temporary `NASTECH_HOME` and configure that sandbox independently.
:::

Quit the app normally or press Ctrl-C in its launching terminal. `concurrently -k` manages its own child commands, and Electron owns its backend shutdown. Do not add a global `killport`, `pkill electron`, or a sweep of all `serve`/`dashboard --port 0` processes: those can terminate another instance. Remove the old `_nastech_gui_cleanup` trap if replacing an earlier version of this helper.
