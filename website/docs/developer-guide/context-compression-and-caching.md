...
- **Failed probes use the current table without persisting it.** Failures have a
  five-minute in-memory cooldown scoped to Nastech home, endpoint, model, and
  region. Expiry or explicit cache invalidation permits another attempt.

The cache remains at `context_length_cache.yaml` under the active Nastech home.
...
value is kept per `model@host` in `~/.nastech/cache/image_token_costs.json` and
...
- Keys may be **provider-scoped** as `"<provider>:<substring>"` (e.g.
  ...
  elsewhere (OpenRouter, Nastech, direct OpenAI) keeps the global `threshold`.
...
excluded because they already unlock the wider window), Nastech raises the
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Nastech keeps the
...
`gpt-5.4-900k`). These are Nastech-side aliases: the suffix is stripped before
