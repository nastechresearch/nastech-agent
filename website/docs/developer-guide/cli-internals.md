...
sidebar_position: 15
title: "CLI Internals"
description: "How nastech_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `nastech_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `nastech_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `nastech_cli.update_cmd._nastech_holder_subcommand` (top-level subcommand of any Nastech argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `nastech_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.nastech/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Nastech supports profiles — fully isolated instances, each with its own `NASTECH_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`nastech -p x <cmd>`),
`_apply_profile_override()` in `nastech_cli/main.py` sets `NASTECH_HOME` before any module imports, so
every `get_nastech_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["NASTECH_HOME"]` stays the launch
...
`Path.home() / ".nastech" / "profiles"`, not `get_nastech_home() / "profiles"`) so
`nastech -p coder profile list` sees all profiles regardless of which one is active — intentional.
