...
(`nastech-account-service`) and anyone debugging the managed-cron path.

Chronos lets a hosted Nastech gateway **scale to zero** while idle and still
...
ChronosCronScheduler.reconcile()        ── agent computes next_run_at
  │  POST {portal}/api/agent-cron/provision   (auth: agent's Nastech access token)
  ▼
...
|---|---|---|---|
| 1 | agent → NAS (`provision`/`cancel`/`list`) | the agent's existing **Nastech Portal access token** (Bearer) — for a hosted agent this is the **bootstrap-session token** NAS planted in `auth.json` (client `nastech-cli-vps`), NOT an `agent:*` client token | NAS (its normal agent-token path) |
...
> agent uses the **bootstrap-session access token** (`resolve_nastech_access_token`),
> minted under the bootstrap-only client `nastech-cli-vps` and seeded into the
...
- **Auth:** `Authorization: Bearer <agent Nastech access token>`. NAS validates via
...
1. **Dashboard app** (`nastech_cli/web_server.py`) — the agent's only public
...
- **Multi-replica agents:** the store CAS makes the fire at-most-once across N
  gateway replicas sharing one `NASTECH_HOME` — exactly one replica runs each
  fire.
...
If `callback_url` / `portal_url` is blank or the agent has no Nastech login,
...
config-only, so it cannot tell whether the stored Nastech token is the identity NAS
maps to a provisioned instance (hop 1 above). When `provision` answers 403
`invalid_client` — the token in `auth.json` is a plain `nastech-cli` user login
rather than the `nastech-cli-vps` bootstrap session or an `agent:*` client — the
rejection is deterministic for the life of that credential: every arm, re-arm
and `list` would fail the same way, and a `nastech auth` re-login makes it
