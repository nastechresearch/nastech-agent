...
title: "Build a Nastech Plugin"
description: "Step-by-step guide to building a complete Nastech plugin with tools, hooks, data files, and skills"
---

# Build a Nastech Plugin

This guide walks through building a complete Nastech plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Nastech has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](../../user-guide/features/tts.md#voice-message-transcription-stt) — set `NASTECH_LOCAL_STT_COMMAND` to an argv-tokenized template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](../../user-guide/features/hooks.md#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.nastech/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](../../user-guide/features/skills.md) — `nastech skills tap add <repo>` · [Publishing a tap](../../user-guide/features/skills.md#publishing-a-custom-skill-tap) |
...
components Nastech already owns. It does not replace native `plugin.yaml` plus
...
Nastech validates `plugin.json`, Agent Skills frontmatter, fixed component
...
profile-scoped writable directory managed by Nastech.
...
Portable `streamable-http` entries are routed through Nastech' existing native
...
same full-trust posture as other installed Nastech plugins.
...
records it as Published. Nastech keys behavior on the canonical v1.0.0 schema
...
not by one global plugin API number. Nastech does not expose a
...
continue to work after a normal Nastech upgrade.
...
- **Hook payloads are keyword payloads.** New hook data is added as keyword
  ...
  Nastech inspects callback signatures: a legacy callback receives the fields it
...
- **Manifests are open to additions.** Unknown `plugin.yaml` fields are ignored.
  Older Nastech releases can therefore load a plugin whose manifest contains
...
Nastech enforces this contract with frozen external-plugin fixtures discovered
from an isolated `NASTECH_HOME`. Those tests load and invoke the plugin through
...
`nastech plugins doctor [path-or-id]` runs the same directory discovery,
...
Doctor uses a temporary `NASTECH_HOME`, restores plugin registration state after
...
`llm.task_override` (see `nastech_cli/plugin_capabilities.py` for the
...
`nastech_agent.plugin_capabilities` entry-point group. Each declaration is
named `<plugin-id>.<capability-id>` and points at the same object as your
`nastech_agent.plugins` entry point:

```toml
[project.entry-points."nastech_agent.plugins"]
calculator = "my_pkg:register"

[project.entry-points."nastech_agent.plugin_capabilities"]
"calculator.tools.override" = "my_pkg:register"
```

Nastech reads these from installed metadata without importing your code, so
`nastech plugins capabilities` and the consent flow stay accurate for pip
installs.
...
this Nastech understands still loads with a warning.
...
4. **Accept `**kwargs`:** Nastech may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `nastech my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Nastech continues fine
...
Use plugin-relative config keys for user-visible behavior. Nastech resolves them
...
`<NASTECH_HOME>/plugin-data/`. Neither API exposes another plugin's namespace.
...
If your plugin doesn't show up — or shows up but isn't loading — set `NASTECH_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:
...
The same logs are always written to `~/.nastech/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:
...
- **Not enabled in config** — plugins are opt-in. Run `nastech plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout:** Native packages use `~/.nastech/plugins/<plugin-name>/plugin.yaml` (flat) or one category level. Portable packages use root `plugin.json` in the same locations. Anything deeper is ignored.
...
tree, and `nastech plugins update` / `remove` git-pull or delete it — your
...
# <nastech home>/plugin-data/<name>/ — created on first use
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.nastech/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.nastech/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `nastech plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Nastech into installing arbitrary packages — only specs Nastech itself ships are eligible |
...
auditable in `~/.nastech/logs/agent.log`. Plugins load after built-in
...
does not ship with Nastech core (user, project, or pip source),
...
plugins:
  entries:
    my-plugin:                    # the plugin's registry key from `nastech plugins list`
...
loader, the plugin is disabled and Nastech continues. The gate exists
...
You normally never edit this key by hand. `nastech plugins enable <name>`
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$NASTECH_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $NASTECH_HOME/hook_outputs
```
...
- **The system prompt is Nastech's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
The canonical list of kinds is `VALID_MIDDLEWARE` in `nastech_cli/middleware.py`:
...
| `llm_request` | `request`, `original_request`, context kwargs | Return `{"request": {...}}` to replace the effective provider kwargs before Nastech sends them. |
...
- Middleware payloads carry `middleware_schema_version` (`nastech.middleware.v1`) alongside the observer telemetry fields.
- Unknown kinds register with a warning instead of failing, so a plugin written against a newer Nastech still loads on an older one.

### Register CLI commands

Plugins can add their own `nastech <plugin>` subcommand tree:
...
After registration, users can run `nastech my-plugin status`, `nastech my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `nastech name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `nastech chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `NASTECH_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `nastech <subcommand>` (e.g. `nastech kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "nastech kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Nastech from a hook.
...
Nastech has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.nastech/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `nastech gateway status` without SDK instantiation.
...
Nastech also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](../../user-guide/features/plugins.md#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Nastech without any Python plugin. Declare them in `~/.nastech/config.yaml`:
...
Nastech connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](../../user-guide/features/mcp.md).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.nastech/hooks/<name>/`:
...
# Plugin auto-discovered on next nastech startup
```
...
```python
# Wrong — will break if Nastech passes extra context
def handler(args):
    ...
