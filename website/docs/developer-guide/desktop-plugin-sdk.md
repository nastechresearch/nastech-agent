...
title: "Desktop Plugin SDK (@nastech/plugin-sdk)"
description: "Extend the native Nastech Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Nastech Desktop](../user-guide/desktop.md) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `NastechPlugin`.
It imports one module — `@nastech/plugin-sdk` — and gets everything: the app's
...
`$NASTECH_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Nastech. This page is the **native
desktop app** (`nastech desktop`) SDK — the `@nastech/plugin-sdk` module and
`$NASTECH_HOME/desktop-plugins/`. The **web dashboard** (`nastech dashboard`) has
its own, unrelated plugin system on `window.__NASTECH_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](../user-guide/features/extending-the-dashboard.md). Python
CLI/gateway plugins are documented at [Build a Nastech Plugin](./plugins/index.md).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$NASTECH_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$NASTECH_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `NastechPlugin` contract, appear in **Capabilities → Plugins**,
...
Create `$NASTECH_HOME/desktop-plugins/hello/plugin.js` (that's `~/.nastech/...`
...
```javascript
// ~/.nastech/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@nastech/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Nastech' }),
      jsx('div', {
...
The only importable specifiers are `@nastech/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `NastechPlugin`:

```ts
interface NastechPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `nastech.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@nastech/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@nastech/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@nastech/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@nastech/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@nastech/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@nastech/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@nastech/plugin-sdk'
...
`window.nastech.send('get-price eth')` (or a declarative
`<button data-nastech-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, WORKSPACE_PAGE_HEADER_AREA } from '@nastech/plugin-sdk'
...
for requests; `targetProfile` is the backend Nastech profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Nastech
...
ctx.os.notify({
  ...
  // Body click → focus Nastech + navigate. Same vocabulary as OS deep links:
  activate: 'nastech://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'nastech://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `nastech://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `nastech://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@nastech/plugin-sdk'
...
```javascript
import { queryClient } from '@nastech/plugin-sdk'
...
copies that half into `$NASTECH_HOME/desktop-plugins/<id>/` beside a
`.nastech-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.nastech/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`nastech plugins update`, or
...
a package into `~/.nastech/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.nastech/plugins` is not reachable as a filesystem —
...
the `nastech://` scheme — a plain anchor on your website or README:
...
`nastech-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Nastech plugin and declare it in a
`manifest.json`:
...
nastech-agent codebase directly (`nastech_state`, `nastech_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.nastech/`)
...
(`nastech.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `NastechPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@nastech/plugin-sdk` alias.
2. It's still lint-fenced to `@nastech/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@nastech/plugin-sdk`, `react`,
...
| Plugin contract | `NastechPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `NastechOpenTarget`, `Contribution` |
...
### Agents: the `nastech-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`nastech-desktop-plugins`** skill — it carries the same contract as this page in
...
`$NASTECH_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `nastech logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@nastech/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.nastech/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.nastech/logs/errors.log` for
