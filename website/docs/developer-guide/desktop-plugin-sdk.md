...
All three take the same `NastechPlugin` contract, appear in **Settings → Plugins**,
...
by default, or `~/.nastech/profiles/<name>/...` under a named profile). The folder
...
desktop app also scans `$NASTECH_HOME/plugins/<id>/` — the regular agent-plugin
