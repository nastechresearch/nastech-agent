...
by `nastech_cli/gateway_multiplex_mode.py::resolve_multiplex_mode`, which runs
the `nastech gateway migrate` preflight and keeps the gateway standalone when a
...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_nastech_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`NASTECH_HOME`, `NASTECH_PROFILE`, proxy settings,
...
reload helper: `nastech_cli.env_loader.load_nastech_dotenv` skips the
...
## The NASTECH_HOME override

`nastech_constants.py` holds a context-local override consulted by
`get_nastech_home()` before the `NASTECH_HOME` env var. Everything that resolves
...
automatically. `get_process_nastech_home()` exists for the few machine-level
assets that must not follow the override. `nastech_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `nastech_cli/profiles.py`
...
DB handles are resolved at call time through the active NASTECH_HOME override —
...
NASTECH_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `nastech_home_key()`. |
...
- `agent/secret_scope.py`, `nastech_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `nastech_cli/profiles.py`
