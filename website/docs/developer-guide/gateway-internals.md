...
- **Explicit target delivery** — the send engine specifying `telegram:-1001234567890`, exposed via the [`nastech send` CLI](../guides/pipe-script-output.md) for shell scripts and via cron `deliver:` targets
...
| Child processes (`nastech -p X` workers, relay turns, browser drivers) | `tools/environments/local.py::served_profile_child_env` |
