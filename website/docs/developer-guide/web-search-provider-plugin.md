...
| `requires_env` | Interactive credential prompt during `nastech plugins install` (see [Build a Nastech Plugin](./plugins/index.md#gate-on-environment-variables) for the rich format) |
...
If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Nastech will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Nastech Plugin → Lazy-install](./plugins/index.md#lazy-install-optional-python-dependencies) for the security model.
...
- [Build a Nastech Plugin](./plugins/index.md) — general tools/hooks/slash commands guide
