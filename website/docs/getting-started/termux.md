...
This APT repository is **community-maintained by `@adybag14-cyber` and is not an official NastechResearch distribution**. NastechResearch does not build, sign, host, or audit these packages. Enabling the repository means trusting the contributor-operated repository and its signing key. Termux itself remains a Tier 2 / best-effort platform.
:::

For users who prefer a native package-manager install rather than building Python/Rust dependencies on the phone, a community-maintained APT repository is available. The repository bootstrap and packaging sources are published in [`adybag14-cyber/termux-python`](https://github.com/adybag14-cyber/termux-python), with the Nastech package build in [`adybag14-cyber/termux-nastech`](https://github.com/adybag14-cyber/termux-nastech).

Install the repository key/source and Nastech with:
...
APT-managed Nastech installs are marked with install method `apt`. Nastech therefore does not run its Git self-updater against package-owned files; use the package manager instead:
...
Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Nastech runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `nastech` into `$PREFIX/bin` so it stays on your Termux PATH
...
Nastech requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `nastech` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/nastech" "$PREFIX/bin/nastech"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `nastech` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.nastech/.env`.
...
### `nastech doctor` says ripgrep or Node is missing
