...
Nastech Agent ships a Nix flake, a NixOS module, and a Home Manager module.
...
:::note
This module needs NixOS. Nastech is an agent for one person. If you want an agent for one person and not a system service, use the [Home Manager module](#home-manager-module). That module runs on NixOS and on each other system that Home Manager supports.
:::
...
{ config, ... }: {
  services.nastech-agent = {
    ...
    # ── Documents ──────────────────────────────────────────────────────
    # USER.md is memory, so it goes to NASTECH_HOME. Workspace files use
...
| Give the agent an identity | `nastechHomeFiles."SOUL.md"` | `"You are a terse ops assistant."` |
...
Nastech reads files from two directories. Thus there are two options. Use the option for the directory that the file must go into.
...
```nix
{
  services.nastech-agent = {
    ...
    workingDirectory = "/var/lib/nastech/workspace";
    documents = {
...
`nastechHomeFiles` installs into **`NASTECH_HOME`**. Nastech reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file. Nastech does not load that file as the identity:

```nix
{
  services.nastech-agent.nastechHomeFiles = {
...
`nastechHomeFiles` needs no `workingDirectory`, because the module owns the `NASTECH_HOME` directory. Most users want `nastechHomeFiles`.
...
1. **The `NASTECH_MANAGED` environment variable.** The service sets it, and the gateway process reads it.
2. **The `.managed` marker file** in `NASTECH_HOME`. The activation script writes it, and an interactive shell reads it. Thus the CLI also blocks a command such as `docker exec -it nastech-agent nastech config set ...`.
...
The flake also exports `homeManagerModules.default`. Nastech is an agent for one person. The credentials, the memory, the sessions and the cron jobs all belong to that person. Thus a user service is the correct shape on a personal machine. It runs on each distribution that Home Manager supports, and not only on NixOS.

The option set is the same set that the NixOS module uses. It is `services.nastech-agent`, with the same `settings`, `environmentFiles`, `documents`, `mcpServers`, `extraPlugins` and `backend` options. Each example above works here without a change. Only the necessary parts are different:

| | NixOS module | Home Manager module |
|---|---|---|
| Runs as | a system user that you declare, with `user`, `group` and `createUser` | you |
| State directory | `stateDir` and `/.nastech` | `nastechHome`, set directly. The default is `~/.nastech`. |
| Service | `systemd.services` | `systemd.user.services` on Linux, `launchd.agents` on macOS |
| CLI on the PATH | `addToSystemPackages`, which exports `NASTECH_HOME` for the full system | `programs.nastech-agent.enable`, which exports it for your session only |
| Desktop application | not supported, because a system service cannot own a user session | `programs.nastech-agent.desktop.enable` |
...
`home-manager switch` makes `~/.nastech`, writes `config.yaml`, builds `.env` and starts the gateway as a user service.
...
`gateway.enable` runs the messaging gateway for Telegram, Discord, Slack and the other platforms. Nastech Desktop and the web dashboard connect to a *different* process, which is `nastech serve` or `nastech dashboard`. `backend.mode` runs that process with the gateway:
...
`serve` runs without a user interface. It gives the `/api/ws` and `/api/pty` sockets that Nastech Desktop connects to, and it does not build the web application. `dashboard` gives all of that, and also serves the browser admin panel. Both processes use one `NASTECH_HOME` with the gateway. Thus the sessions, the skills, the memory and the cron jobs are the same for all of them. `backend.mode` works in the same way on the NixOS module, but not in container mode.
...
```bash
# Linux
systemctl --user status nastech-agent
journalctl --user -u nastech-agent -f
...
| `nastechHomeFiles` | `attrsOf (either str path)` | `{}` | Files that go into `NASTECH_HOME`. `SOUL.md` and `memories/` must be here, or Nastech does not load them. |
...
### Backend (`nastech serve` / `nastech dashboard`)

This option runs the process that Nastech Desktop and the web dashboard connect to, with the gateway. You cannot use it with `container.enable`.
...
| Option | Type | Default | Description |
|---|---|---|---|
| `nastechHome` | `str` | `"${config.home.homeDirectory}/.nastech"` | `NASTECH_HOME` directly. The NixOS module builds it from `stateDir`. |
...
### `programs.nastech-agent` (Home Manager only)

Home Manager separates "install this application for me" from "run this
daemon". `services.nastech-agent` keeps the state, the configuration and the
daemons. `programs.nastech-agent` installs what you use, and reads
`nastechHome` and the backend address from the services.

| Option | Type | Default | Description |
|---|---|---|---|
| `enable` | `bool` | `false` | Add the `nastech` CLI to `home.packages`, and export `NASTECH_HOME` for your shells |
| `package` | `package` | `services.nastech-agent.package` | The package to install. The default applies `extraPythonPackages` and `extraDependencyGroups` from the services, so both are one build. |
| `desktop.enable` | `bool` | `false` | Add the Nastech Desktop application, with a launcher entry on Linux |
| `desktop.package` | `package` | `package.nastechDesktop` | The desktop package. The default follows `package`, so the application and the services run one Nastech runtime. |
...
services.nastech-agent = {
  enable = true;
  backend.mode = "serve";
  backend.sessionTokenFile = config.sops.secrets."nastech/desktop-token".path;
};
```

The launcher carries `NASTECH_HOME` itself. A desktop menu reads no shell
profile, so the value that `programs.nastech-agent.enable` exports with
`home.sessionVariables` reaches an interactive shell only. Without the
value in the launcher, the application opens `~/.nastech` while the
services use `nastechHome`, and you see no sessions and no keys.
...
`services.nastech-agent.installPackage` was removed by this split. A
...
├── .nastech/                         # NASTECH_HOME
│   ├── SOUL.md                      # from nastechHomeFiles: the agent identity
...
```
~/.nastech/                           # nastechHome (NASTECH_HOME), 0700
├── SOUL.md                          # from nastechHomeFiles
...
├── auth.json                        # OAuth credentials: seeded, then Nastech owns it
