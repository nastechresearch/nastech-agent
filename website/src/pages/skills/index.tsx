        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            <div className={styles.installHint}>
              <code>{skill.installCmd || `nastech skills install ${skillCatalogInstallIdentifier(skill) || skill.name}`}</code>
              <CopyButton
                text={skill.installCmd || `nastech skills install ${skillCatalogInstallIdentifier(skill) || skill.name}`}
              />
            </div>
            {onPick ? (
              <button
...
export default function SkillsDashboard() {
  ...
  // app (Nastech desktop's Bot Mode agent editor) as a skill PICKER. Site
  // chrome is hidden via a CSS class and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'nastech-skill-pick', name, identifier, installCmd, source }
  ...
  const pickSkill = useCallback(
    (skill: Skill) => {
      ...
      window.parent.postMessage(
        {
          type: "nastech-skill-pick",
          name: skill.name,
          identifier: skillCatalogInstallIdentifier(skill) || skill.name,
          installCmd: skill.installCmd || `nastech skills install ${skillCatalogInstallIdentifier(skill) || skill.name}`,
          source: skill.source,
        },
        "*"
      );
        ...
        <header className={styles.hero}>
          ...
          <div className={styles.heroContent}>
            ...
            <p className={styles.heroSub}>
              ...
              skills across {sources.length - 1} registries. Open in Nastech Desktop to review and install, or copy the CLI command.
              {loadError && (
