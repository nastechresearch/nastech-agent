...
这几个术语描述的是 Nastech 的不同部分：

- **Profile** 是一个助手的配置和数据的持久化主目录。它在多次对话和重启之间保持同一份状态。

- **Agent** 是使用该配置和状态运行中的 Nastech 助手。"Nastech Agent" 同时也是产品的名称。
...
- **消息平台 bot** 是 Telegram、Discord、Slack 等平台上的一个账号，通过 gateway 连接到 Nastech。它的 [bot token](#不同的-bot-token) 标识的是那个平台账号。
...
将当前 profile 的 `config.yaml`、`.env`、`SOUL.md`、skills 以及精选记忆文件 `memories/MEMORY.md` 和 `memories/USER.md` 复制到新 profile——记忆与 `SOUL.md` 一样被视为 agent 身份的一部分。会话、`state.db`、cron 任务及其他内容均从空白开始。若也想要空白记忆，请不加 `--clone` 创建 profile，或在创建后删除这两个文件；文件不存在时 agent 不会回退读取其他 profile 的记忆。编辑 `~/.nastech/profiles/work/.env` 可使用不同的 API 密钥，编辑 `~/.nastech/profiles/work/SOUL.md` 可设置不同的人格。
