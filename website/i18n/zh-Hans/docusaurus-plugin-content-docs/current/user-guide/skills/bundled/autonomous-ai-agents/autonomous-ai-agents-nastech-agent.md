...
Nastech Agent 是 Nastech Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Nastech 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。
...
**文档：** https://nastechresearch.github.io/nastech-agent/docs/
...
nastech auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nastech、openai-codex、qwen-oauth）
...
平台文档：https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://nastechresearch.github.io/nastech-agent/docs/reference/slash-commands)。命令注册表的权威来源是 `nastech_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
~/.nastech/sessions/         Session transcripts
...
Profiles 使用 `~/.nastech/profiles/<name>/`，布局相同。

### 配置节

使用 `nastech config edit` 或 `nastech config set section.key value` 编辑。
...
完整配置参考：https://nastechresearch.github.io/nastech-agent/docs/user-guide/configuration

### 提供商

支持 20+ 个提供商。通过 `nastech model` 或 `nastech setup` 设置。
...
完整提供商文档：https://nastechresearch.github.io/nastech-agent/docs/integrations/providers

### Toolset

通过 `nastech tools`（交互式）或 `nastech tools enable/disable NAME` 启用/禁用。
...
完整枚举位于 `toolsets.py` 的 `TOOLSETS` 字典中；`_NASTECH_CORE_TOOLS` 是大多数平台继承的默认工具包。
...
**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export NASTECH_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `nastech config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。
...
```bash
nastech config set privacy.redact_pii true    # 启用
nastech config set privacy.redact_pii false   # 禁用（默认）
```
...
```bash
nastech config set approvals.mode smart       # 推荐的折中方案
nastech config set approvals.mode off         # 绕过一切（不推荐）
```
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.nastech/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具

要完全阻止模型访问网络或媒体工具，打开 `nastech tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
- **fire-and-forget 使用 `nastech chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`nastech cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用户文档：https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/cron

### Curator（skill 生命周期）

agent 创建的 skill 的后台维护。跟踪使用情况，将闲置 skill 标记为过时，归档过时的 skill，保留运行前的 tar.gz 备份以防数据丢失。

- **CLI：** `nastech curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
- **遥测：** `~/.nastech/skills/.usage.json` 中的 sidecar 保存每个 skill 的 `use_count`、`view_count`、`patch_count`、`last_activity_at`、`state`、`pinned`。

配置：`curator.*`（`enabled`、`interval_hours`、`min_idle_hours`、`stale_after_days`、`archive_after_days`、`backup.*`）。
用户文档：https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `nastech kanban <verb>` 驱动；调度器生成的 worker 看到由 `NASTECH_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
- **隔离：** 看板是硬边界（worker 在环境中固定 `NASTECH_KANBAN_BOARD`）；租户是看板内用于工作区路径和记忆键隔离的软命名空间。

用户文档：https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/kanban
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`nastech config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。
...
### 工具不可用
1. `nastech tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `nastech doctor` — 检查配置和依赖
2. `nastech auth` — 重新认证 OAuth 提供商（或 `nastech auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `nastech model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
### Skill 未显示
1. `nastech skills list` — 验证已安装
2. `nastech skills config` — 检查平台启用状态
3. 显式加载：`/skill name` 或 `nastech -s name`
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed nastech-gateway`
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `nastech config edit` 或[配置文档](https://nastechresearch.github.io/nastech-agent/docs/user-guide/configuration) |
| 可用工具 | `nastech tools list` 或[工具参考](https://nastechresearch.github.io/nastech-agent/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://nastechresearch.github.io/nastech-agent/docs/reference/slash-commands) |
| Skill 目录 | `nastech skills browse` 或[Skill 目录](https://nastechresearch.github.io/nastech-agent/docs/reference/skills-catalog) |
| 提供商设置 | `nastech model` 或[提供商指南](https://nastechresearch.github.io/nastech-agent/docs/integrations/providers) |
| 平台设置 | `nastech gateway setup` 或[消息文档](https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/) |
| MCP 服务器 | `nastech mcp list` 或[MCP 指南](https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/mcp) |
| Profiles | `nastech profile list` 或[Profiles 文档](https://nastechresearch.github.io/nastech-agent/docs/user-guide/profiles) |
| Cron 任务 | `nastech cron list` 或[Cron 文档](https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/cron) |
| 记忆 | `nastech memory status` 或[记忆文档](https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/memory) |
| 环境变量 | `nastech config env-path` 或[环境变量参考](https://nastechresearch.github.io/nastech-agent/docs/reference/environment-variables) |
| CLI 命令 | `nastech --help` 或[CLI 参考](https://nastechresearch.github.io/nastech-agent/docs/reference/cli-commands) |
| Gateway 日志 | `~/.nastech/logs/gateway.log` |
| 会话文件 | `~/.nastech/sessions/` 或 `nastech sessions browse` |
| 源代码 | `~/.nastech/nastech-agent/` |

---

## 贡献者快速参考

面向偶尔贡献者和 PR 作者。完整开发者文档：https://nastechresearch.github.io/nastech-agent/docs/developer-guide/
...
配置：`~/.nastech/config.yaml`（设置）、`~/.nastech/.env`（API key）。
...
**2. 添加到 `toolsets.py`** → `_NASTECH_CORE_TOOLS` 列表。

自动发现：任何包含顶层 `registry.register()` 调用的 `tools/*.py` 文件都会自动导入——无需手动列出。

所有处理器必须返回 JSON 字符串。路径使用 `get_nastech_home()`，永远不要硬编码 `~/.nastech`。

### 添加斜杠命令

1. 在 `nastech_cli/commands.py` 的 `COMMAND_REGISTRY` 中添加 `CommandDef`
...
- 测试自动将 `NASTECH_HOME` 重定向到临时目录——永远不会触及真实的 `~/.nastech/`
- 推送任何变更前运行完整套件
...
- 所有路径使用 `nastech_constants` 中的 `get_nastech_home()`（profile 安全）
