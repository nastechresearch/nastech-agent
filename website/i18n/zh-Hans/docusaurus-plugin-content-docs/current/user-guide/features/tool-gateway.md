...
选择工具类别（Web、Browser、Image Generation、TTS），再将提供商选为 **Nastech Subscription**。这会把该类别的选择键写为 `nastech`（例如 `image_gen.provider: nastech`）。

### 手动编辑配置

每个工具类别只有一个选择键，选 **Nastech Subscription** 即写入 `nastech`：

```yaml
web:
  backend: nastech          # 网页搜索/抓取走 Tool Gateway
...
当某工具类别的选择键为 `nastech` 时，运行时会把 API 调用路由到 Nastech Tool Gateway，而不是使用直连 Key：
...
- **选择为 `nastech`** → 走网关，即使 `.env` 里仍有直连 Key（例如 `FAL_KEY` 会被忽略）
- **选择为具体厂商**（如 `fal`、`firecrawl`）→ 直连；若对应 Key 缺失则报错并提示运行 `nastech tools`，**不会**静默回退到网关
- **从未配置过的类别** → 按可用凭据自动检测（行为不变）；但一旦存在选择，仅往 `.env` 加 Key 不会改变路由

（旧版的 `use_gateway` 布尔键已废弃：不再写入，读取时 `use_gateway: true` 等同于 `nastech`。请改用 `nastech tools` 选择提供商。）
...
在 `nastech tools` 中选择非网关提供商时，选择键会被改写为该厂商名（旧的 `use_gateway` 键若存在会被一并移除），避免配置自相矛盾。
...
不需要。类别选择为 **Nastech Subscription**（`nastech`）时，运行时会忽略该类别的直连 Key；Key 仍保留在 `.env`。之后在 `nastech tools` 里改回直连提供商，Key 即恢复生效。

### 能否部分工具走网关、部分走直连？

可以。选择按工具类别独立配置。例如：网页与文生图选 Nastech Subscription，TTS 用 ElevenLabs，浏览器用 Browserbase。
