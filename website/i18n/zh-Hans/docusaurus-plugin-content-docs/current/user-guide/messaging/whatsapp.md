...
2. 用新号码注册 WhatsApp
3. 运行 `nastech whatsapp` 并从该 WhatsApp 账号扫描二维码
...
在 `~/.nastech/config.yaml` 中可选的行为设置：
...
Baileys 桥接将会话保存在 `~/.nastech/platforms/whatsapp/session` 目录下。这意味着：
...
- Agent 响应默认以"𓄃 **Nastech Agent**"为前缀。可在 `config.yaml` 中自定义或禁用：
...
- `~/.nastech/platforms/whatsapp/session` 目录包含完整会话凭证——请像保护密码一样保护它
- 设置文件权限：`chmod 700 ~/.nastech/platforms/whatsapp/session`
