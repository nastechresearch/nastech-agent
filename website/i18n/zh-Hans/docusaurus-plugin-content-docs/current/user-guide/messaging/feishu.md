...
2. **设置回调的接收方式：**
   在同一标签页中，Nastech 以 `websocket` 模式运行时选择 **长连接**（Lark SDK 通过现有连接接收回调）；webhook 模式则填写请求地址（与事件 webhook 相同的端点，例如 `https://your-server:8765/feishu/webhook`）。飞书必须能够访问并解析该地址，否则点击会返回 200342/200343。
...
:::warning
没有已发布的卡片回调时，飞书仍会成功*发送*交互式卡片（发送仅需 `im:message:send` 权限），但点击任意按钮将返回错误 200340。卡片看起来正常——错误仅在用户与其交互时才会出现，并且点击不会到达 Nastech（`gateway.log` 中没有任何记录），因为飞书在投递回调之前就拒绝了它。

错误码 200672 / 200673 表示回调*确实*到达了 Nastech 而飞书拒绝了响应；如果遇到，请附上对应的 `gateway.log` 日志提交 issue。
:::
