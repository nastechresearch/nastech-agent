...
原生 Windows 无需 WSL 即可运行 Nastech——CLI、gateway、TUI 和工具均可原生运行。（原生安装与 WSL2 安装可干净共存；唯一仅限 WSL2 的功能见下方功能说明。）遇到 bug 请[提交 issue](https://github.com/NastechResearch/nastech-agent/issues)。
...
- **CLI（`nastech chat`、`nastech setup`、`nastech gateway` 等）** — 原生，使用默认终端
...
如果遇到编码相关的 bug 并希望回退到旧版 cp1252 stdio 路径（用于问题定位），请在环境中设置 `NASTECH_DISABLE_WINDOWS_UTF8=1`。
:::

### 安装程序做了什么

安装程序自动处理一切——所有依赖（Python、Node.js、ripgrep、ffmpeg）、仓库克隆、虚拟环境、全局 `nastech` 命令配置以及 LLM 提供商配置。完成后即可开始聊天。
...
Root 模式的 **FHS 布局**（`/usr/local/lib/…`、`/usr/local/bin/nastech`）与其他系统级开发工具在 Linux 上的安装位置一致。适用于共享机器部署场景，一次系统安装可服务所有用户。每个用户的个人配置（认证、技能、会话）仍位于各自的 `~/.nastech/` 或显式指定的 `NASTECH_HOME` 下。
...
该命令一次性完成登录、设置 Nastech 为提供商并开启 Tool Gateway。
:::
...
3. **使 `nastech` 对服务用户的 shell 可用。** 安装程序将启动器写入 `~/.local/bin/nastech`。系统服务账户通常具有不包含 `~/.local/bin` 的最小 PATH。可以将其添加到用户环境，或将启动器符号链接到系统位置：
...
4. **验证：** `nastech doctor` 现在应能正常运行。如果出现 `ModuleNotFoundError: No module named 'dotenv'`，说明你在用系统 Python 调用仓库源码中的 `nastech` 文件（`~/.nastech/nastech-agent/nastech`），而非 venv 启动器（`~/.nastech/nastech-agent/venv/bin/nastech`）——请修正步骤 3。
...
| 问题                        | 解决方案                                                                           |
| --------------------------- | ---------------------------------------------------------------------------------- |
| `nastech: command not found` | 重新加载 shell（`source ~/.bashrc`）或检查 PATH                                    |
| `API key not set`           | 运行 `nastech model` 配置提供商，或 `nastech config set OPENROUTER_API_KEY your_key` |
| 更新后配置丢失              | 运行 `nastech config check`，然后运行 `nastech config migrate`                       |

如需更多诊断信息，运行 `nastech doctor`——它会告诉你确切缺少什么以及如何修复。
