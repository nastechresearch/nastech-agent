...
:::tip
网页搜索是 Nastech 支持的多种**后端插件**之一。其他插件（各有其 ABC）包括：[图像生成提供商插件](./image-gen-provider-plugin.md)、[视频生成提供商插件](./video-gen-provider-plugin.md)、[记忆提供商插件](./memory-provider-plugin.md)、[上下文引擎插件](./context-engine-plugin.md)和[模型提供商插件](./model-provider-plugin.md)。通用工具/hook/CLI 插件请参阅[构建 Nastech 插件](./plugins/index.md)。
:::
...
| `requires_env` | 在 `nastech plugins install` 期间进行交互式凭据提示（富格式说明参见[构建 Nastech 插件](./plugins/index.md#gate-on-environment-variables)） |
...
如果你的提供商封装了第三方 SDK（如 DDGS 封装了 `ddgs` 包），请勿在模块顶层 `import`。在 `is_available()` 或 `search()` 内部使用 `tools.lazy_deps.ensure(...)` ——Nastech 将在首次使用时安装该包，并受 `security.allow_lazy_installs` 控制。安全模型详见[构建 Nastech 插件 → 懒加载](./plugins/index.md#lazy-install-optional-python-dependencies)。
...
- [构建 Nastech 插件](./plugins/index.md) — 通用工具/hook/斜杠命令指南