...
:::tip
图像生成是 Nastech 支持的多种**后端插件**之一。其他插件（各有更专用的 ABC）包括：[Memory Provider 插件](./memory-provider-plugin.md)、[Context Engine 插件](./context-engine-plugin.md) 和 [Model Provider 插件](./model-provider-plugin.md)。通用工具/hook/CLI 插件请参阅 [构建 Nastech 插件](./plugins/index.md)。
:::
