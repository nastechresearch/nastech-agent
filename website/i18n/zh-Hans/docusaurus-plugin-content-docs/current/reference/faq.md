...
- **Nastech Portal** — Nastech Research 自有推理端点
...
使用 `nastech model` 设置提供商，或直接编辑 `~/.nastech/.env`。所有提供商 key 请参阅[环境变量](./environment-variables.md)参考文档。
...
可以。运行 `nastech model`，选择**自定义端点**，然后输入您服务器的 URL：
...
#### 安装后出现 `nastech: command not found`
...
通过 `nastech model` 添加新提供商后，启动新的聊天会话 — `/model` 将显示所有已配置的提供商。

:::tip 快速参考
| 目标 | 使用方式 |
|-----------|-----|
| 添加新提供商 | `nastech model`（从终端） |
| 输入/更改 API key | `nastech model`（从终端） |
...
:::warning
请确保 key 与提供商匹配。OpenAI 的 key 无法用于 OpenRouter，反之亦然。检查 `~/.nastech/.env` 中是否有冲突条目。
:::
...
- 升级您的提供商套餐
- 切换到其他模型或提供商
- 使用 `nastech chat --provider <alternative>` 路由到其他后端
...
**解决方案：**
- 使用 `nastech gateway setup` 验证您的 bot token 是否有效
- 检查网关日志：`cat ~/.nastech/logs/gateway.log | tail -50`
...
在 `~/.nastech/config.yaml` 中您的网关设置下进行配置。请参阅[消息文档](../user-guide/messaging/index.md)。
...
#### WSL：网关持续断开连接或 `nastech gateway start` 失败
...
1. 创建一个任务，运行 `wsl -d Ubuntu -- bash -lc 'nastech gateway run'`
2. 设置在用户登录时触发
:::
...
**解决方案：** 网关在您运行 `nastech gateway install` 时会捕获您的 shell PATH。如果您在设置网关后安装了新工具，请重新运行 install 以捕获更新后的 PATH：

```bash
nastech gateway install    # 重新快照当前 PATH
nastech gateway start      # 检测到更新的 plist 并重新加载
```
...
**解决方案：**
- 尝试更快/更小的模型：`nastech chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- 减少激活的工具集：`nastech chat -t "terminal"`
- 检查到提供商的网络延迟
...
验证您的 `~/.nastech/config.yaml` 中的 MCP 配置：
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
### Profiles 与直接设置 NASTECH_HOME 有何不同？

Profiles 是构建在 `NASTECH_HOME` 之上的托管层。您*可以*在每次命令前手动设置 `NASTECH_HOME=/some/path`，但 profiles 会为您处理所有底层工作：创建目录结构、生成 shell 别名（`nastech-work`）、在 `~/.nastech/active_profile` 中跟踪活动 profile，以及自动跨所有 profiles 同步技能更新。它们还与 tab 补全集成，让您无需记忆路径。
...
不共享。每个 profile 都有自己独立的记忆存储、会话数据库和技能目录，完全隔离。如果您想用现有的记忆和会话创建新 profile，请使用 `nastech profile create newname --clone-all` 从当前 profile 复制所有内容，或添加 `--clone-from <profile>` 从指定源 profile 复制。

### 运行 `nastech update` 时会发生什么？

`nastech update` 拉取最新代码并重新安装依赖项**一次**（不是每个 profile 各一次）。然后自动将更新的技能同步到所有 profiles。您只需运行一次 `nastech update` — 它覆盖机器上的每个 profile。

### 可以运行多少个 profiles？

没有硬性限制。每个 profile 只是 `~/.nastech/profiles/` 下的一个目录。实际限制取决于您的磁盘空间以及系统能处理多少个并发网关（每个网关是一个轻量级 Python 进程）。运行数十个 profiles 完全没问题；每个空闲的 profile 不占用任何资源。
...
**场景：** Telegram 有 100 个 slash 命令的限制，您的技能数量已超过此限制。您想禁用 Telegram 上不需要的技能，但 `nastech skills config` 设置似乎没有生效。

**解决方案：** 使用 `nastech skills config` 按平台禁用技能。这会写入 `config.yaml`：
...
更改后，**重启网关**（`nastech gateway restart` 或终止并重新启动）。Telegram bot 命令菜单在启动时重建。
...
2. 在**源机器**上创建完整备份：
   ...
   nastech backup
   ```
   这会将您整个 `~/.nastech/` 目录（配置、API key、记忆、技能、会话和 profiles）打包为 zip 文件，保存到主目录 `~/nastech-backup-<timestamp>.zip`。
...
4. 在新机器上运行 `nastech setup` 以验证 API key 和提供商配置是否正常工作。
...
### `nastech backup` 与 `nastech profile export` 的对比

| 功能 | `nastech backup` | `nastech profile export` |
| :--- | :--- | :--- |
| **使用场景** | **整机迁移** | **移植/共享特定 profile** |
| **范围** | 全局（整个 `~/.nastech` 目录） | 局部（单个 profile 目录） |
...
1. **搜索现有 issue：** [GitHub Issues](https://github.com/NastechResearch/nastech-agent/issues)
2. **向社区提问：** [Nastech Research Discord](https://discord.gg/nastechresearch)
