...
Usage (via cron with --no-agent):
    ...
    nastech cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$NASTECH_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Nastech-Watcher/1.0"})
