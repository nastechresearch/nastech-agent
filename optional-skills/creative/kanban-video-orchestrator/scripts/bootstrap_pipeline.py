...
adapted from alt-glitch's NastechResearch/kanban-video-pipeline.
...
def render_setup_sh(plan: dict, brief_md: str, team_md: str) -> str:
    ...
    for key in plan.get("api_keys_required", []):
        key_checks.append(f'check_key {key} nastech {key} || exit 1')
    ...
    for t in plan["team"]:
        profile_creates.append(
            f'nastech profile create {t["profile"]} --clone 2>/dev/null || true'
        )
    ...
    for t in plan["team"]:
        soul_writes.append(
            f'cat > "$HOME/.nastech/profiles/{t["profile"]}/SOUL.md" <<\'SOUL_EOF\'\n'
