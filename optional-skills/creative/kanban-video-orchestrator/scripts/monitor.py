#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `nastech kanban list` and
...
def nastech_available() -> bool:
    return shutil.which("nastech") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed nastech CLI."""
    try:
        out = subprocess.run(
            ["nastech", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `nastech kanban list`
    out = subprocess.run(
        ["nastech", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["nastech", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not nastech_available():
        print("ERROR: 'nastech' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
