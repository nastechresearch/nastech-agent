...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `nastech plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
key is present in `${NASTECH_HOME:-~/.nastech}/.env` (or macOS Keychain) before firing the kanban.
