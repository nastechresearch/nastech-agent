"""Meta Model API (Muse Spark) provider plugin for Nastech Agent.
...
Bundled from https://github.com/albertodepaola/nastech-meta-provider. Nastech'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to nastech' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Nastech' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in nastech_cli/providers.py and the retention hint in
