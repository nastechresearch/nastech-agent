...
after 14 days; empty dirs under NASTECH_HOME always. Prompt-only: research
(keep 10 newest, > 30 days), chrome-profile > 14 days, any file > 500 MB.
Scope: strictly NASTECH_HOME and /tmp/nastech-*; never ~/.nastech/logs/ or system dirs.
"""
...
def _state_file(name: str) -> Path:
    """``$NASTECH_HOME/disk-cleanup/<name>`` — deliberately outside ``$NASTECH_HOME/logs/``."""
    return get_nastech_home() / "disk-cleanup" / name


def is_safe_path(path: Path) -> bool:
    """Accept only paths under NASTECH_HOME or ``/tmp/nastech-*`` (rejects /mnt/c etc.)."""
    with contextlib.suppress(ValueError, OSError):
        path.resolve().relative_to(get_nastech_home())
        return True
    parts = path.parts
    return len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nastech-")
...
# Top-level NASTECH_HOME dirs whose empty subdirs are never swept (last row: user project trees).
...
# Top-level NASTECH_HOME entries guess_category() never auto-tracks: state, logs, memory,
...
def _is_protected_dir(p: Path) -> bool:
    """A tracked DIRECTORY that is NASTECH_HOME itself or lives under a protected top-level tree
    (``cache/terminal`` holds terminal snapshots) is never rmtree'd; only its files age out."""
    if not p.is_dir():
        return False
    with contextlib.suppress(ValueError, OSError):
        rel = p.resolve().relative_to(get_nastech_home())
...
# Paths under $NASTECH_HOME that must NEVER be deleted by quick(), regardless of what the stored category
...
def _is_protected_cron_path(p: Path) -> bool:
    return str(p.resolve()) in _protected_cron_paths(get_nastech_home())
...
def quick() -> Dict[str, Any]:
    ...
    for item, p, age in _live_items(load_tracked(), datetime.now(timezone.utc), log_stale=True):
        ...
        if err is None:
            ...
            errors.append(err)
            new_tracked.append(item)
    empty_removed = _sweep_empty_dirs(get_nastech_home())
...
def _sweep_empty_dirs(nastech_home: Path) -> int:
    """Remove empty dirs under NASTECH_HOME without recursing into durable/heavy trees (a full
    rglob over a checkout+venv under NASTECH_HOME can stall the gateway loop for minutes).
    Iterative post-order so parents emptied by child removal are caught."""
    removed = 0
    stack: List[Tuple[Path, bool]] = [
        (top, False) for top in _subdirs(nastech_home, _EMPTY_DIR_PROTECTED_TOP_LEVEL | _EMPTY_DIR_SWEEP_PRUNE_DIRS)]
    while stack:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    if not is_safe_path(path):
        return None
    with contextlib.suppress(ValueError):  # not under NASTECH_HOME (/tmp/nastech-*) — name rules only
        rel = path.resolve().relative_to(get_nastech_home())
