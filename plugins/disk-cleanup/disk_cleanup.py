...
def is_safe_path(path: Path) -> bool:
    ...
    try:
        path.resolve().relative_to(nastech_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/nastech-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("nastech-"):
        return True
    return False
...
def quick() -> Dict[str, Any]:
    ...
    try:
        for top in nastech_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    nastech_home = get_nastech_home()
    try:
        rel = path.resolve().relative_to(nastech_home)
