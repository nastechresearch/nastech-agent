...
Standalone subprocess spawned by ``process_manager.py``; configured via ``NASTECH_MEET_*`` env,
status + transcript written under ``$NASTECH_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``NASTECH_MEET_URL=... NASTECH_MEET_OUT_DIR=/tmp/x NASTECH_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__nastechMeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__nastechMeetInstalled) return;
  window.__nastechMeetInstalled = true;
  window.__nastechMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__nastechMeetQueue.push({
      ts: Date.now(),
  ...
  window.__nastechMeetDrain = () => {
    const out = window.__nastechMeetQueue.slice();
    window.__nastechMeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__nastechMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState") -> None:
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'nastech_meet_sink'}", str(pcm_path)]
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``NASTECH_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("NASTECH_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("NASTECH_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("NASTECH_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("NASTECH_MEET_AUTH_STATE", "").strip(),
        guest_name=env("NASTECH_MEET_GUEST_NAME", "Nastech Agent"),
        duration_s=_parse_duration(env("NASTECH_MEET_DURATION", "")),
        realtime=env("NASTECH_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # NASTECH_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("NASTECH_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("NASTECH_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("NASTECH_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("NASTECH_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("NASTECH_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__nastechMeetDrain && window.__nastechMeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — NASTECH_MEET_URL must be a "
