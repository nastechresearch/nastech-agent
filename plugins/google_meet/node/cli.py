"""`nastech meet node ...` subcommand tree, wired under the ``nastech meet`` parser."""
...
def _cmd_run(args: argparse.Namespace, reg: NodeRegistry) -> int:
    ...
    print(f"[meet-node] display_name={server.display_name}\n"
          ...
          f"             nastech meet node approve <name> ws://<host>:{args.port} {token}")
    try:
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``nastech meet node ...``; returns a process exit code."""
