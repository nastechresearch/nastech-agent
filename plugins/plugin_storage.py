...
``<nastech home>/plugins/<name>/``. That tree is the plugin *install* dir —
``nastech plugins remove`` deletes it and ``nastech plugins update`` git-pulls
...
``<nastech home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Nastech.
...
# Mirrors the plugin-name shape `nastech plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<nastech home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`nastech_constants.get_nastech_home` on every call. Don't cache the
    result across profile switches.
    """
    from nastech_constants import get_nastech_home

    root = get_nastech_home() / "plugin-data" / _validate_name(name)
