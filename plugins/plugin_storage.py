"""Per-plugin persistent storage: ``<nastech home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<nastech home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `nastech plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<nastech home>/plugin-data/<name>/``; resolves ``get_nastech_home()`` on
