...
Authentication via ``FAL_KEY`` or the managed Nastech gateway. Output is an
...
def _resolve_managed_fal_video_gateway():
    ...
    image FAL resolver: ``"nastech"`` (or legacy ``use_gateway: true``) →
...
def _submit_fal_video_request(endpoint: str, arguments: Dict[str, Any]):
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            raise ValueError(
                f"Nastech Subscription gateway rejected endpoint '{endpoint}' "
...
def _check_fal_video_available() -> bool:
    """True if the FAL video backend selected via `nastech tools` (or, on a
            ...
            if not endpoint:
                return error_response(
                    error=(
                        ...
                        f"via `nastech tools` → Video Generation."
                    ),
