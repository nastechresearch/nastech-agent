...
path containing one) → ``DEFAULT_MODEL``. Auth via ``FAL_KEY`` or the managed Nastech gateway; output is an HTTPS URL.
"""
...
def _resolve_managed_fal_video_gateway():
    """Resolve the FAL video route from the stored ``video_gen`` selection: ``"nastech"`` → managed only (unentitled ⇒
    selection-naming error); other stored provider → direct only (missing FAL_KEY ⇒ error); never-configured → autodetect."""
    from tools.managed_tool_gateway import resolve_managed_tool_gateway
    from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER, fal_key_is_configured, read_selection, selection_error
    selected = read_selection("video_gen")
    if selected == NASTECH_MANAGED_PROVIDER:
        gateway = resolve_managed_tool_gateway("fal-queue")
        if gateway is None:
            raise ValueError(selection_error("video_gen", NASTECH_MANAGED_PROVIDER, "the Nastech Tool Gateway is not available (not entitled or unreachable)"))
        return gateway
...
def _get_managed_fal_video_client(managed_gateway):
    ...
    with _managed_fal_video_client_lock:
        if _managed_fal_video_client is None or _managed_fal_video_client_config != client_config:
            _managed_fal_video_client = _ManagedFalSyncClient(_load_fal_client(), key=managed_gateway.nastech_user_token,
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            if billing is not None:
                raise ValueError(
                    f"Nastech Subscription gateway rejected endpoint '{endpoint}' (HTTP {status}): {billing}") from exc
            raise ValueError(f"Nastech Subscription gateway rejected endpoint '{endpoint}' (HTTP {status}). This model may not yet be enabled "
                             f"on the Nastech Portal's FAL proxy. Either:\n  • Set FAL_KEY in your environment to use FAL.ai directly, or\n"
                             f"  • Pick a different model via `nastech tools` → Video Generation.") from exc
        raise
...
_NO_BACKEND_MSG = ("No FAL backend available. Either set FAL_KEY (run `nastech tools` → Video Generation → FAL to configure) "
                   "or sign in to Nastech (`nastech setup`) for managed gateway access.")
_MODALITY_MISSING_MSG = {
    "image": "FAL family {fid} has no image-to-video endpoint. Pick a family with image-to-video support via `nastech tools` → Video Generation.",
