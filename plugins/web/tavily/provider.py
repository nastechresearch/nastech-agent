...
def _missing_key_error(action: str) -> str:
    return (
        ...
        f"or select Tavily in `nastech tools` for opt-in keyless {action}."
    )
...
class TavilyWebSearchProvider(WebSearchProvider):
    ...
    def is_keyless_available(self) -> bool:
        ...
        ``web.backend: tavily`` (or ``nastech tools`` pick) works without a
