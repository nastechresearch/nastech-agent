"""Firecrawl web search + extract provider (direct SDK, keyless cloud, or Nastech tool-gateway).
...
def _use_keyless_ring() -> bool:
    """Route via the keyless ring only with no direct credentials, when the managed Nastech
    ...
    if _env("FIRECRAWL_API_KEY") or _env("FIRECRAWL_API_URL"):
        return False
    from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER, read_selection
    from plugins.web.keyless_mcp import use_keyless
    # Both probes are optional layers: a failing probe never blocks the ring.
    for probe in (lambda: read_selection("web") == NASTECH_MANAGED_PROVIDER, lambda: _is_tool_gateway_ready() and not _is_explicit_firecrawl_selection()):
        try:
            if probe():
                return False
...
def _is_tool_gateway_ready() -> bool:
    """True when gateway URL + Nastech Subscriber token are available."""
    return _gateway.resolve_managed_tool_gateway("firecrawl", token_reader=_gateway.peek_nastech_access_token) is not None


def check_firecrawl_api_key() -> bool:
    """True when the route selected via ``nastech tools`` (or, on a never-configured
...
def _firecrawl_backend_help_suffix() -> str:
    return ", or use the Nastech Tool Gateway via your subscription (FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN)" if _backend_helpers.managed_nastech_tools_enabled() else ""
...
def _get_firecrawl_client() -> Any:
    ...
    ``"nastech"`` → managed Tool Gateway ONLY; any other stored backend → direct Firecrawl ONLY (never a silent
    managed fallback billed to Nastech); never-configured → direct when present, else managed. Raises ValueError
    when the resolved path is unusable."""
    wt = _wt()
    from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER, read_selection, selection_error
    selected = read_selection("web")
    direct_config = _get_direct_firecrawl_config()

    def _managed():
        gw = _gateway.resolve_managed_tool_gateway("firecrawl", token_reader=_gateway.read_nastech_access_token)
        if gw is None:
            return None
        return "sdk", {"api_key": gw.nastech_user_token, "api_url": gw.gateway_origin}, ("tool-gateway", gw.gateway_origin, gw.nastech_user_token)
    ...
    def _unconfigured_message() -> str:
        ...
        if _backend_helpers.managed_nastech_tools_enabled():
            return message + " With your Nastech subscription you can also use the Tool Gateway. run `nastech tools` and select Nastech Subscription as the web provider."
        return message + " " + _backend_helpers.nastech_tool_gateway_unavailable_message("managed Firecrawl web tools")

    # (resolved config, log detail, error message) per selection state; the message is built lazily.
    if selected == NASTECH_MANAGED_PROVIDER:
        resolved, log, message = _managed(), "the Nastech Subscription web selection is stored but the tool gateway is unavailable.", lambda: selection_error(
            "web", NASTECH_MANAGED_PROVIDER, "the Nastech Tool Gateway is not available (not entitled or unreachable)")
...
class FirecrawlWebSearchProvider(BaseWebSearchProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return setup_schema(
            ...
            "Full search + extract; supports keyless cloud, direct API, and Nastech tool-gateway routing.",
