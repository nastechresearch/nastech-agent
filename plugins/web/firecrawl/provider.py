...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # Nastech tool-gateway (subscribers)
...
def _use_keyless_ring() -> bool:
    ...
    managed Nastech gateway isn't the selected path, and the keyless tier
    ...
    try:
        if read_selection("web") == NASTECH_MANAGED_PROVIDER:
            return False
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + Nastech Subscriber token are available.

    Reads ``peek_nastech_access_token`` and ``resolve_managed_tool_gateway``
    via :mod:`tools.web_tools` rather than direct imports, so unit tests
    that ``patch("tools.web_tools._peek_nastech_access_token", ...)`` see
    ...
    return _wt.resolve_managed_tool_gateway(
        "firecrawl", token_reader=_wt._peek_nastech_access_token
    ) is not None
...
def check_firecrawl_api_key() -> bool:
    """Return True when the Firecrawl backend selected via `nastech tools`
    ...
    existing tests and the ``nastech tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_nastech_tools_enabled():
        return ""
    return (
        ", or use the Nastech Tool Gateway via your subscription "
...
def _raise_web_backend_configuration_error() -> "NoReturn":
    ...
    if _wt.managed_nastech_tools_enabled():
        message += (
            " With your Nastech subscription you can also use the Tool Gateway. "
            "run `nastech tools` and select Nastech Subscription as the web provider."
        )
    else:
        message += " " + _wt.nastech_tool_gateway_unavailable_message(
...
def _get_firecrawl_client() -> Any:
    ...
    - ``"nastech"`` (or legacy ``use_gateway: true``) → managed Tool Gateway
    ...
    functions (``resolve_managed_tool_gateway``, ``_read_nastech_access_token``,
    ...
    def _managed_kwargs():
        managed_gateway = _wt.resolve_managed_tool_gateway(
            "firecrawl", token_reader=_wt._read_nastech_access_token
        )
        if managed_gateway is None:
            return None
        kwargs = {
            "api_key": managed_gateway.nastech_user_token,
    ...
    if selected == NASTECH_MANAGED_PROVIDER:
        ...
        if managed is None:
            logger.error(
                "Firecrawl client initialization failed: the Nastech "
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "tag": (
                ...
                "and Nastech tool-gateway routing."
            ),
            "env_vars": [
                {
