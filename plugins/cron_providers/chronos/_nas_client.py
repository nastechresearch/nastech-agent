...
list, authenticated with the existing Nastech Portal token.
...
class NasCronClient:
    ...
    def _headers(self) -> Dict[str, str]:
        """Bearer auth with the agent's existing Nastech Portal access token (refresh-aware)."""
        from nastech_cli.auth import resolve_nastech_access_token
        return {"Authorization": f"Bearer {resolve_nastech_access_token()}",
