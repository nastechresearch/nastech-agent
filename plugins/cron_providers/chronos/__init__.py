...
NAS's ``agent-cron`` endpoints with the agent's existing Nastech token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def is_available(self) -> bool:
        ...
        callback URL (for NAS→agent fires), and a usable Nastech token (the agent
        ...
        if not (_cfg("cron", "chronos", "portal_url") and _cfg("cron", "chronos", "callback_url")):
            return False
        return self._have_nastech_token()

    def _have_nastech_token(self) -> bool:
        """True if the agent has a Nastech Portal login (no network call).

        Checks the stored auth state for a Nastech access token — does NOT refresh
