...
Holds no scheduler credentials — speaks only to NAS ``agent-cron`` endpoints with the Nastech token.
...
class ChronosCronScheduler(CronScheduler):
    ...
    def __init__(self) -> None:
        ...
        # Set when NAS answered 403 invalid_client: the Nastech token in auth.json is not this
    ...
    def is_available(self) -> bool:
        ...
        Nastech login; otherwise the resolver falls back to the built-in ticker."""
        ...
        try:
            ...
            return bool((get_provider_auth_state("nastech") or {}).get("access_token"))
        except Exception:
            return False
    ...
    def _note_identity_rejected(self) -> None:
        ...
        ``agent:*`` client or the hosted bootstrap session, and a plain ``nastech auth`` login is
        ...
        logger.warning(
            "Chronos: NAS rejected this agent's Nastech credential for agent-cron (403 invalid_client). "
            "The Nastech token in auth.json is not this instance's provisioned identity (an agent:* client "
            "or the hosted bootstrap session), so no job can be armed. A normal `nastech auth` re-login "
            "cannot fix this; the hosted credential has to be restored from the Nastech Portal. Falling back "
            "to the built-in cron ticker for this process so scheduled jobs keep firing on time.")
        if self._stop_event is None:
            return  # start() never ran (e.g. a CLI `nastech cron add`); nothing to tick here
