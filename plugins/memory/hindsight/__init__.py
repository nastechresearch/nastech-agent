...
Config: $NASTECH_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Nastech Agent and the User"
...
def _load_config() -> dict:
    """$NASTECH_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_nastech_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
    ...
    return {
        ...
        "banks": {"nastech": {"bankId": get_secret("HINDSIGHT_BANK_ID", "") or "nastech",
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Nastech event time with an explicit UTC offset."""
    event_time = _nastech_now()
    # nastech_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "nastech", "mid", ""
    ...
    def save_config(self, values, nastech_home):
        """Merge *values* into $NASTECH_HOME/hindsight/config.json."""
    ...
    def post_setup(self, nastech_home: str, config: dict) -> None:
        ...
        run_setup(self, nastech_home, config)
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "nastech"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "nastech"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "nastech", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "nastech"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            ...
            # the user would only see Nastech get sluggish (issue #13125). This is an automatic
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_nastech_home() / "logs" / "hindsight-embed.log"
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # See NastechResearch/nastech-agent#5820.
