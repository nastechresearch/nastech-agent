...
Daemon startup logs: `~/.nastech/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p nastech ui start
```
...
Config file: `~/.nastech/hindsight/config.json`
...
| `retain_source` | — | Opt-in `metadata.source` attached to retained memories (identifies the storing client, e.g. `nastech`). Empty by default — no attribution tag ships unless you set it. |
...
The LLM API key is stored in `~/.nastech/.env` as `HINDSIGHT_LLM_API_KEY`.
