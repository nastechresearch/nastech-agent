...
the ``nastech`` integration, and applies a chosen manifest to the user's bank
...
def fetch_nastech_templates(url: str | None = None) -> list[dict]:
    """Return catalog entries tagged for the ``nastech`` integration."""
    catalog = _get_json(url or catalog_url())
    entries = catalog.get("templates", []) if isinstance(catalog, dict) else []
    return [e for e in entries if "nastech" in (e.get("integrations") or [])]
...
def run_template_step(
    *,
    ...
    """
    try:
        entries = fetch_nastech_templates()
