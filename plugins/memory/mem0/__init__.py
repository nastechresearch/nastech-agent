...
self-hosted Mem0 server (MEM0_HOST, HTTP), or OSS Memory. Secrets live in $NASTECH_HOME/.env
(MEM0_API_KEY, MEM0_HOST); settings in $NASTECH_HOME/mem0.json via `nastech memory setup`:
...
def _load_config() -> dict:
    """Env vars provide defaults; $NASTECH_HOME/mem0.json overrides individual keys.
    ...
    config = {"mode": get_secret("MEM0_MODE", "") or "platform", "host": get_secret("MEM0_HOST", "") or "",
              "agent_id": get_secret("MEM0_AGENT_ID", "") or "nastech", "oss": {}}
    ...
    file_cfg = read_json_or_empty(get_nastech_home() / "mem0.json")
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._mode, self._api_key, self._host, self._user_id, self._agent_id = "platform", "", "", _DEFAULT_USER_ID, "nastech"
    ...
    def save_config(self, values, nastech_home):
        """Merge-write config to $NASTECH_HOME/mem0.json."""
    ...
    def post_setup(self, nastech_home: str, config: dict) -> None:
        ...
        post_setup(nastech_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._mode, self._api_key, self._host, self._agent_id = cfg.get("mode", "platform"), cfg.get("api_key", ""), cfg.get("host", ""), cfg.get("agent_id", "nastech")
