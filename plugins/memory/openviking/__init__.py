...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Nastech peer ID in OpenViking (default: nastech)
...
_DEFAULT_AGENT = "nastech"
_OPENVIKING_USER_AGENT = f"openviking-memory-nastech/{_NASTECH_VERSION}"
_AGENT_PROMPT_LABEL = "Nastech peer ID in OpenViking"
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Nastech to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Nastech refused the connection."
        ) from exc
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Nastech did not start "
    ...
    try:
        ...
        # The Nastech Desktop backend can include the Nastech venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Nastech venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Nastech venv,
        # aborting `nastech update` with access-denied on .pyd files.
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Nastech session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Nastech only", "write values only to Nastech .env"),
    ...
    _save_nastech_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Nastech .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See nastech-agent#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Nastech peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, nastech_home: str, config: dict) -> None:
        ...
        from nastech_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        nastech_home_path = Path(nastech_home)
        env_path = nastech_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        nastech_home = str(kwargs.get("nastech_home") or "").strip()
        if not nastech_home:
            try:
                from nastech_constants import get_nastech_home
                nastech_home = str(get_nastech_home())
            except Exception:
                nastech_home = str(Path.home() / ".nastech")
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Nastech will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.nastech/.env`` after startup never reach the live client and tools
        keep running against stale auth until the user restarts nastech (#21130).
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Nastech will retry on a "
    ...
    def _pending_session_dir(self) -> Optional[Path]:
        if not self._nastech_home:
            return None
        return Path(self._nastech_home) / _PENDING_SESSIONS_RELATIVE_DIR
    ...
    def _run_lock_dir(self) -> Optional[Path]:
        if not self._nastech_home:
            return None
        return Path(self._nastech_home) / _RUN_LOCKS_RELATIVE_DIR
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Nastech' full canonical transcript."""
        if not messages:
            return []
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See nastech-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/nastech/.overview.md).
