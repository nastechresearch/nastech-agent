...
does not ask for a peer ID. Without a configured peer, Nastech sends neither
...
order remains environment, linked OpenViking config, then Nastech YAML. To use
...
on the old implicit `nastech` peer now use user memory for new writes. Without
...
Set `agent: nastech` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `nastech-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Nastech
...
Use the same OpenViking profile and credentials as Nastech for manual recovery.
...
file URIs, such as
`viking://user/default/peers/nastech/memories/preferences/mem_abc123.md` (any
