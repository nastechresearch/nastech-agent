...
def _load_supermemory_config(nastech_home: str) -> dict:
...
def _resolve_container_tag_for_setup(nastech_home: str, *, identity: str = "default") -> str:
...
def _probe_supermemory_connection(api_key: str, nastech_home: str, *, identity: str = "default") -> dict:
    ...
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(nastech_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `nastech memory setup`.
        # All other options are documented for $NASTECH_HOME/supermemory.json
    ...
    def post_setup(self, nastech_home: str, config: dict) -> None:
        ...
        if env_writes:
            _write_env_vars(env_writes, nastech_home=nastech_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._nastech_home)
