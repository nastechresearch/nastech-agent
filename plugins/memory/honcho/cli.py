"""``nastech honcho`` subcommands: setup wizard, status, peers, sessions, identity, migrate."""
...
_RETRY_HINT = "  Re-run 'nastech honcho setup' to retry, or choose an API key instead.\n"
...
def _local_config_path() -> Path:
    ...
    return get_nastech_home() / "honcho.json"
...
def _write_config(cfg: dict, path: Path | None = None) -> None:
    ...
    with _refresh_lock, _config_refresh_lock(path):
        ...
        mkdir_under_nastech_home(path.parent)
...
def _label(host: str) -> str:
    return f"[{host}] " if host != "nastech" else ""
...
def _sync_profiles(verbose: bool) -> int:
    ...
    cfg = _read_config()
    if not cfg:
        return say("  No Honcho config found. Run 'nastech honcho setup' first.\n") or 0
    default_block, has_key = _default_block_and_key(cfg)
    if not default_block and not has_key:
        return say("  Honcho not configured on default profile. Run 'nastech honcho setup' first.\n") or 0
...
def sync_honcho_profiles_quiet() -> int:
    """Sync host blocks for all profiles from `nastech update`; no output, no exceptions."""
...
def cmd_enable(args) -> None:
    ...
    if not _resolve_api_key(cfg, block, env=False):
        ...
        setup = "nastech honcho setup" + (f" --target-profile {profile}" if profile != "default" else "")
...
def _resolve_effective_identity_mapping(cfg: dict, nastech_host: dict) -> tuple[bool, dict, str, bool, bool]:
    ...
    pin_sources = (nastech_host.get("pinUserPeer"), nastech_host.get("pinPeerName"),
                   cfg.get("pinUserPeer"), cfg.get("pinPeerName"))
    pin = bool(next((v for v in pin_sources if v is not None), False))

    def _inherit(key):
        if key in nastech_host:
            return nastech_host.get(key), False
        val = cfg.get(key)
...
def _apply_runtime_prefix(nastech_host: dict, current_prefix: str, prefix_from_root: bool, label: str) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        nastech_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(nastech_host: dict) -> None:
    print(f"  resolved →\n    pinUserPeer       = {bool(nastech_host.get('pinUserPeer'))}\n"
          f"    userPeerAliases   = {nastech_host.get('userPeerAliases') or '{}'}\n"
          f"    runtimePeerPrefix = {nastech_host.get('runtimePeerPrefix') or '(none)'}")


def _configure_raw_identity_mapping(nastech_host, current_pin, current_aliases, current_prefix,
    ...
    _scrub_identity_mapping(nastech_host)
    nastech_host["pinUserPeer"] = pin
    if pin:
        return
    ...
    while entry := _prompt("    alias", default="").strip():
        ...
        if rid and peer:
            aliases[rid] = peer
    if aliases:
        nastech_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(nastech_host, current_prefix, prefix_from_root,
                          "runtimePeerPrefix — namespace for unknown IDs (blank for none)")


def _setup_identity_mapping(cfg: dict, nastech_host: dict, current_peer: str, new_host: bool) -> None:
    ...
    current_pin, current_aliases, current_prefix, aliases_from_root, prefix_from_root = (
        _resolve_effective_identity_mapping(cfg, nastech_host))
    ...
    if gw_platforms:
        ...
        notice, question = (
            ("\n  Each gateway account (a Telegram user, a Discord user, ...)\n"
             ...
             "Running the Nastech gateway (Telegram/Discord/etc.)? (y/N)") if gw_platforms is None else
            ("\n  No gateway platforms connected — nothing to map.", "Configure anyway? (y/N)"))
        print(notice)
        if not _yes(_prompt(question, default="n")):
            return

    peer_target = nastech_host.get("peerName") or current_peer or "user"
    ai_peer_label = nastech_host.get("aiPeer") or cfg.get("aiPeer") or "nastech"
    ...
    print("\n  This step covers the HUMAN mapping only. Each account using the\n"
          ...
          "  Nastech profile brings its own AI peer to the gateway.\n"
    ...
    if shape == "raw":
        _configure_raw_identity_mapping(nastech_host, current_pin, current_aliases, current_prefix,
        ...
        if shape == "multi" and aliases_from_root:
            prior_aliases = {}
        _scrub_identity_mapping(nastech_host)  # each shape starts from a clean slate
        nastech_host["pinUserPeer"] = shape == "single"
        if shape == "single":
    ...
    _echo_identity_mapping(nastech_host)
...
def _apply_grant_to_host(cfg: dict, nastech_host: dict, cred) -> None:
...
def _setup_local_auth(cfg: dict, nastech_host: dict) -> None:
    ...
    if new_url := _prompt("Base URL", default=cfg.get("baseUrl") or "http://localhost:8000"):
        cfg["baseUrl"] = new_url
    current_host_key = nastech_host.get("apiKey", "")
    ...
    if new_local_key:
        nastech_host["apiKey"] = new_local_key
...
def _setup_device_login(cfg: dict, nastech_host: dict, write_path: Path, *, open_browser: bool) -> bool:
    ...
    try:
        cred = authorize_via_device_code(
            config_path=write_path, source="nastech-cli", apply_config=False, display=_show,
        ...
        print("\n  Device code expired before approval.\n  Re-run 'nastech honcho setup' to get a new code.\n")
    except AccessDenied:
        ...
        print(" approved")
        _apply_grant_to_host(cfg, nastech_host, cred)
        return True
    return False


def _setup_browser_login(cfg: dict, nastech_host: dict, write_path: Path) -> bool:
    ...
    try:
        cred = authorize_via_loopback(config_path=write_path, source="nastech-cli", apply_config=False, open_url=_open)
    except Exception as e:
...
def _setup_cloud_auth(cfg: dict, nastech_host: dict, write_path: Path) -> bool:
    ...
    if device_available and method in {"device", "d"}:
        return _setup_device_login(cfg, nastech_host, write_path, open_browser=can_browse and not is_remote)
    if method in {"oauth", "o"}:
        return _setup_browser_login(cfg, nastech_host, write_path)
    # A leftover grant on the host block would shadow the pasted key.
    stale_grant = existing_oauth is not None or is_oauth_access_token(nastech_host.get("apiKey"))
    current = ("" if stale_grant else nastech_host.get("apiKey", "")) or cfg.get("apiKey", "")
    ...
    if not key:
        print("\n  No API key configured. Get yours at https://app.honcho.dev\n"
              "  Run 'nastech honcho setup' again once you have a key.\n")
        return False
    nastech_host.pop("oauth", None)
    nastech_host["apiKey"] = key
    return True
...
def _choice_step(nastech_host, key, current, label, valid, fallback=None) -> None:
    """Prompt for one of ``valid``; an invalid answer writes ``fallback`` (None = keep current)."""
    new = _prompt(label, default=current)
    if new in valid:
        nastech_host[key] = new
    elif fallback is not None:
        nastech_host[key] = fallback


def _setup_tuning(cfg: dict, nastech_host: dict) -> None:
    ...
    _choice_step(nastech_host, "observationMode", _pref(nastech_host, cfg, "observationMode", "directional"),
    ...
    new_wf = _prompt("Write frequency", default=str(_pref(nastech_host, cfg, "writeFrequency", "async")))
    nastech_host["writeFrequency"] = _first_parsed([new_wf], int, new_wf if new_wf in {"async", "turn", "session"} else "async")

    _menu("Recall mode", *(f"{m:<7} -- {desc}" for m, desc in _MODES.items()))
    raw_recall = _pref(nastech_host, cfg, "recallMode", "hybrid")
    _choice_step(nastech_host, "recallMode", raw_recall if raw_recall in _MODES else "hybrid", "Recall mode", _MODES)

    nastech_host["recallSync"] = _yes(_prompt(
        "Wait for current-query recall (bounded by request timeout, default 5s)? (y/N)",
        default="y" if nastech_host.get("recallSync", cfg.get("recallSync", False)) else "n"))

    current_ctx_tokens = _pref(nastech_host, cfg, "contextTokens")
    ...
    _menu("Dialectic cadence",
          ...
          "Recommended: 1-5.")
    new_dialectic = _prompt("Dialectic cadence", default=str(_pref(nastech_host, cfg, "dialecticCadence") or "2"))
    if (val := _first_parsed([new_dialectic], int, None)) is None:
        nastech_host["dialecticCadence"] = 2
    elif val >= 1:
        nastech_host["dialecticCadence"] = val
    ...
    _choice_step(nastech_host, "dialecticReasoningLevel", _pref(nastech_host, cfg, "dialecticReasoningLevel") or "low",
                 "Reasoning level", REASONING_LEVELS, "low")

    _menu("Session strategy", *(f"{s:<13} -- {desc}" for s, desc in _STRATEGIES.items()))
    _choice_step(nastech_host, "sessionStrategy", _pref(nastech_host, cfg, "sessionStrategy", "per-session"),
...
def _setup_wizard(args) -> None:
    ...
    print(f"\nHoncho memory setup\n{RULE}\n  Honcho gives Nastech persistent cross-session memory.\n  Config: {write_path}")
    ...
    nastech_host = cfg.setdefault("hosts", {}).setdefault(_host_key(), {})
    _migrate_pin_key(cfg)  # canonicalize legacy pinPeerName before detection/writes
    _migrate_pin_key(nastech_host)
    # Taken before the prompts populate the block: an existing install must not default to pinning every account.
    new_host = not any(k in nastech_host or k in cfg for k in (*_IDENTITY_MAPPING_KEYS, "peerName", "workspace", "enabled"))
    ...
    if is_local:
        _setup_local_auth(cfg, nastech_host)
    elif not _setup_cloud_auth(cfg, nastech_host, write_path):
        return

    # --- 3. Identity ---
    current_peer = nastech_host.get("peerName") or cfg.get("peerName", "")
    for key, label, default in (
        ("peerName", "Your name (user peer)", current_peer or os.getenv("USER", "user")),
        ("aiPeer", "AI peer name", _pref(nastech_host, cfg, "aiPeer", "nastech")),
        ("workspace", "Workspace ID", _pref(nastech_host, cfg, "workspace", "nastech")),
    ):
        if new := _prompt(label, default=default):
            nastech_host[key] = new

    _setup_identity_mapping(cfg, nastech_host, current_peer, new_host)
    print("\n  For a gateway with many users and agents, run\n"
          "  'nastech honcho peers map' to map accounts interactively.")

    _setup_tuning(cfg, nastech_host)
    nastech_host["enabled"] = True
    nastech_host.setdefault("saveMessages", True)
    _write_config(cfg)
    ...
    try:  # auto-enable Honcho as memory provider in config.yaml
        ...
        nastech_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(nastech_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}\n  Run: nastech config set memory.provider honcho")
  ...
  Other commands:
    nastech honcho status     -- show full config
    nastech honcho mode       -- change recall/observation mode
    nastech honcho tokens     -- tune context and dialectic budgets
    nastech honcho peer       -- update peer names
    nastech honcho map <name> -- map this directory to a session name
""")
...
def _active_profile_name() -> str:
    """Active Nastech profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    cfg = _read_config()
    # _host_block (not hosts.get) keeps legacy dot-form keys ("nastech.work") readable.
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: nastech honcho setup\n")
        return
...
def _state_db_path() -> Path:
    ...
    if _profile_override and _profile_override not in {"default", "custom"}:
        try:
            ...
            pass
    return get_nastech_home() / "state.db"
...
def _classify_workspace_peers(
    ...
    nastech_hosts = {hostk for _, hostk, _ in profile_rows}
    ...
    # Host blocks that are not Nastech profiles: other apps sharing the config.
    for hostk, block in (cfg.get("hosts") or {}).items():
        if hostk in nastech_hosts or not isinstance(block, dict):
            continue
...
def cmd_peers_map(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    nastech_host = _host_block(cfg, host)
    pin, aliases, prefix, aliases_from_root, _ = _resolve_effective_identity_mapping(cfg, nastech_host)
    peer_name = nastech_host.get("peerName") or cfg.get("peerName") or ""

    if pin:
        print("\n  pinUserPeer is on: every gateway account resolves to peer")
        print(f"  '{peer_name or '(peerName not set)'}' and aliases have no effect.")
        print("  Turn the pin off with 'nastech honcho setup' to use per-account peers.")
    ...
    workspace = (
        ...
        or nastech_host.get("workspace") or cfg.get("workspace") or host
    )
...
def cmd_peer(args) -> None:
    ...
    def show(nastech, cfg):
        print(f"""
Honcho peers
{RULE}
  User peer:   {_pref(nastech, cfg, 'peerName') or '(not set)'}
    Your identity in Honcho. Messages you send build this peer's card.
  AI peer:     {_pref(nastech, cfg, 'aiPeer') or _host_key()}
    Nastech' identity in Honcho. Seed with 'nastech honcho identity <file>'.
    Dialectic calls ask this peer questions to warm session context.

  Dialectic reasoning:  {_pref(nastech, cfg, 'dialecticReasoningLevel') or 'low'}  ({', '.join(REASONING_LEVELS)})
  Dialectic cap:        {_pref(nastech, cfg, 'dialecticMaxChars') or 600} chars
""")
...
def _show_or_set_choice(args, *, attr: str, key: str, noun: str, title: str, choices: dict,
    ...
    if value is None:
        ...
        return print(f"\n  Set with: nastech honcho {attr} [{'|'.join(choices)}]\n")
...
{RULE}
  ...
  Context     {_pref(nastech, cfg, 'contextTokens') or '(Honcho default)'} tokens
  ...
  Dialectic   {_pref(nastech, cfg, 'dialecticMaxChars') or 600} chars, reasoning: {_pref(nastech, cfg, 'dialecticReasoningLevel') or 'low'}
    AI-to-AI inference. Nastech asks Honcho's AI peer a question
  ...
  Set with: nastech honcho tokens [--context N] [--dialectic N]
""")
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        return print("  No API key configured. Run 'nastech honcho setup' first.\n")
    ...
    if getattr(args, "show", False):
        ...
        print(ai_rep.get("representation") or ai_rep.get("card")
              or "  No representation built yet.\n  Run 'nastech honcho identity <file>' to seed one.")
        print()
        return
...
{RULE}
  ...
  AI peer:   {hcfg.ai_peer}
    ...
    nastech honcho identity --show        — show both peer representations
    nastech honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt
""")
        return
...
""")
  ...
  2. Run:  nastech honcho setup
     Paste the key when prompted.
""")
        if _yes(_prompt("  Run 'nastech honcho setup' now?", default="y")):
            cmd_setup(args)
            cfg = _read_config()
            has_key = bool(cfg.get("apiKey", ""))
        else:
            print("\n  Run 'nastech honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  No OpenClaw native memory files found in cwd or ~/.openclaw/.\n"
              ...
              "  or seed them manually:  nastech honcho identity <path/to/file>")
...
""")
    if user_files:
        ...
        print("""
  These are picked up automatically the first time you run 'nastech'
  with Honcho configured and no prior session history.
  (Nastech calls migrate_memory_files() on first session init.)

  If you want to migrate them now without starting a session:""")
        print("    nastech honcho migrate  — this step handles it interactively\n" * len(user_files), end="")
        if has_key:
            _offer("  Upload user memory files to Honcho now?", _migrate_upload, user_files)
        else:
            print("  Run 'nastech honcho setup' first, then re-run this step.")
    else:
...
Step 4  Seed AI identity files → Honcho AI peer
  ...
  In Nastech, they are seeded once into Honcho's AI peer through the
...
""")
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'nastech honcho setup' first, then seed manually:")
            print("\n".join(f"    nastech honcho identity {f}" for f in agent_files))
    else:
        print("  No agent identity files detected.\n  To seed manually:  nastech honcho identity <path/to/SOUL.md>")
...
Step 5  What changes vs. OpenClaw native memory
  ...
  Session naming
    ...
    Nastech:   per-session by default — each run gets its own session
              Map a custom name:  nastech honcho map <session-name>

Step 6  Next steps
""")
    if not has_key:
        print("  1. nastech honcho setup              — configure API key (required)\n"
              "  2. nastech honcho migrate            — re-run this walkthrough")
    else:
        print("""  1. nastech honcho status             — verify Honcho connection
  2. nastech                           — start a session
     (user memory files auto-uploaded on first turn if not done above)
  3. nastech honcho identity --show    — verify AI peer representation
  4. nastech honcho tokens             — tune context and dialectic budgets
  5. nastech honcho mode               — view or change memory mode""")
    print()
...
_SUBCOMMANDS = (
    ("setup", "Initial Honcho setup (redirects to nastech memory setup)", None, ()),
    ...
    ("migrate", "Step-by-step migration guide from openclaw-honcho to Nastech Honcho", cmd_migrate, ()),
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":  # honcho setup goes through the unified memory-provider path
        print("\n  Honcho is configured via the memory provider system.\n  Running 'nastech memory setup'...\n")
        from nastech_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``nastech honcho`` argparse subcommand tree on the ``nastech honcho`` parser."""
