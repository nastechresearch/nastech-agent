...
Handles: nastech honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "nastech" else ""
...
def cmd_sync(args) -> None:
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'nastech honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'nastech honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `nastech update` -- no output, no exceptions.
    """
    try:
        from nastech_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _local_config_path() -> Path:
    ...
    Always returns $NASTECH_HOME/honcho.json so each profile/instance gets
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        nastech_host.get("pinUserPeer"),
        nastech_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in nastech_host:
        aliases_src = nastech_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in nastech_host:
        prefix_src = nastech_host.get("runtimePeerPrefix")
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom nastech_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    nastech_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
...
def _echo_identity_mapping(nastech_host: dict) -> None:
    ...
    aliases = nastech_host.get("userPeerAliases")
    prefix = nastech_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(nastech_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _apply_runtime_prefix(
        nastech_host, current_prefix, prefix_from_root,
...
def cmd_setup(args) -> None:
    ...
    nastech_host = hosts.setdefault(_host_key(), {})
    ...
    if not is_local:
        ...
        if use_device:
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'nastech honcho setup' again once you have a key.\n")
                return
    ...
    if new_peer:
        nastech_host["peerName"] = new_peer

    current_ai = nastech_host.get("aiPeer") or cfg.get("aiPeer", "nastech")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        nastech_host["aiPeer"] = new_ai

    current_workspace = nastech_host.get("workspace") or cfg.get("workspace", "nastech")
    ...
    if run_mapping:
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(nastech_host)
            nastech_host["pinUserPeer"] = False
            if prior_aliases:
                nastech_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                nastech_host, current_prefix, prefix_from_root,
            ...
            if merged:
                nastech_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                nastech_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                nastech_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = nastech_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        nastech_host["observationMode"] = new_obs
    else:
        nastech_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(nastech_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        nastech_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        nastech_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = nastech_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        nastech_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = nastech_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    current_dialectic = str(nastech_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    current_reasoning = (
        nastech_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        nastech_host["dialecticReasoningLevel"] = new_reasoning
    else:
        nastech_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = nastech_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        nastech_host["sessionStrategy"] = new_strat
    ...
    try:
        ...
        print("  Run: nastech config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    nastech honcho status     -- show full config")
    print("    nastech honcho mode       -- change recall/observation mode")
    print("    nastech honcho tokens     -- tune context and dialectic budgets")
    print("    nastech honcho peer       -- update peer names")
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("nastech.work") stay readable per the README's back-compat
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'nastech honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'nastech honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: nastech honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        nastech = hosts.get(_host_key(), {})
        user = nastech.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = nastech.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = nastech.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = nastech.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = nastech.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = nastech.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = nastech.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'nastech honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'nastech honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    nastech honcho identity --show        — show both peer representations")
        print("    nastech honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    ...
    if has_key:
        ...
        print("  2. Run:  nastech honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'nastech honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'nastech honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  nastech honcho identity <path/to/file>")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            for f in agent_files:
                print(f"    nastech honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  nastech honcho identity <path/to/SOUL.md>")
    ...
    if not has_key:
        ...
        print("  2. nastech honcho migrate            — re-run this walkthrough")
    else:
        print("  1. nastech honcho status             — verify Honcho connection")
        print("  2. nastech                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. nastech honcho identity --show    — verify AI peer representation")
        print("  4. nastech honcho tokens             — tune context and dialectic budgets")
        print("  5. nastech honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'nastech memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``nastech honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``nastech honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to nastech memory setup)",
    )
