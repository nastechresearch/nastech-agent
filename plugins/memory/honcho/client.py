...
Config file resolution: $NASTECH_HOME/honcho.json -> ~/.honcho/config.json -> env vars
...
from nastech_state_common import TITLE_SOURCE_DERIVED, TITLE_SOURCE_LLM
...
def resolve_active_host() -> str:
    """Honcho host key: NASTECH_HONCHO_HOST (profile-scoped .env), else the active profile. The config's
    ...
    explicit = (get_secret("NASTECH_HONCHO_HOST", "") or "").strip()
    if explicit:
        return explicit
    try:
...
def resolve_config_path() -> Path:
    """Active Honcho config path: $NASTECH_HOME/honcho.json -> default profile's honcho.json
...
def _connection_fields(look: _HostLookup, host: str, path: Path) -> dict[str, Any]:
    ...
    # read it before the flat Nastech spellings.
...
class HonchoClientConfig:
    ...
    # re-resolving resolve_config_path()/get_nastech_home() later — those resolvers read a ContextVar that
    ...
    @classmethod
    def from_env(cls, workspace_id: str = "nastech", host: str | None = None) -> HonchoClientConfig:
        ...
        return cls(
            ...
            config_path=resolve_config_path(), nastech_home=get_nastech_home(),
        )
    ...
    def from_global_config(cls, host: str | None = None, config_path: Path | None = None) -> HonchoClientConfig:
        ...
        derives it from the active Nastech profile."""
        resolved_host = host or resolve_active_host()
        ...
        return cls(
            ...
            config_path=path, nastech_home=get_nastech_home(),
        )
