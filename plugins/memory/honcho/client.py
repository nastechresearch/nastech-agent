...
from nastech_cli.profiles import _get_default_nastech_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Nastech profile."""
...
def resolve_active_host() -> str:
    ...
    # but applying it before profile resolution makes every named Nastech
    # profile share that same host.  Keep named profiles isolated; only the
    # default Nastech profile may opt into the config's default host.
...
def resolve_config_path() -> Path:
    ...
    """
    local_path = get_nastech_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_nastech_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.nastech
    ...
    # resolve_config_path()/get_nastech_home() later — those resolvers read a
    ...
    def from_global_config(
        ...
        # A hosts.nastech block or explicit enabled flag means the user
        ...
        # baseUrl / base_url keys below are the Nastech-specific spelling.
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Nastech session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Nastech session title (from /title command; non-per-session)
...
def spawn_context_thread(
    ...
    *,
    ...
    (set_nastech_home_override); plain threading.Thread targets start with an
    EMPTY context, so any ambient resolution on the thread
    (resolve_config_path, resolve_active_host, get_nastech_home) silently
...
def _credential_fingerprint(config: HonchoClientConfig | None) -> str:
    ...
    makes 'nastech honcho setup' account switches produce a NEW cache identity
...
def _client_cache_key(config: HonchoClientConfig | None) -> tuple:
    ...
    if config is not None:
        return (
            ...
            str(config.nastech_home) if config.nastech_home is not None else "",
...
def _slot_for(key: tuple) -> SingletonSlot:
    ...
    When a (kind, host, config_path/nastech_home) identity reappears with a
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'nastech honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `nastech honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                nastech_cfg = load_config()
                honcho_cfg = nastech_cfg.get("honcho", {})
                if isinstance(honcho_cfg, dict):
