"""nastech-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $NASTECH_HOME/config.yaml under plugins.nastech-memory-store: db_path ($NASTECH_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from nastech_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "nastech-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, nastech_home):
        """Write config to config.yaml under plugins.nastech-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *nastech_home* is the active profile already.
        from nastech_cli.config import save_config
        save_config({"plugins": {"nastech-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from nastech_constants import display_nastech_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_nastech_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _nastech_home = str(get_nastech_home())
        db_path = self._config.get("db_path", _nastech_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $NASTECH_HOME so paths resolve to the active profile
            db_path = db_path.replace("$NASTECH_HOME", _nastech_home).replace("${NASTECH_HOME}", _nastech_home)
