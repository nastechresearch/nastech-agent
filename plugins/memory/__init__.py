...
``$NASTECH_HOME/plugins/<name>/``, project ``./.nastech/plugins/<name>/`` (opt-in via
NASTECH_ENABLE_PROJECT_PLUGINS), then ``nastech_agent.memory_providers`` entry points.
...
# Per Nastech home (plugin managers are per home too): pruning under one multiplexed profile must
...
def _registered_skills_for_active_home() -> dict[str, Path]:
    ...
    return _REGISTERED_MEMORY_PROVIDER_SKILLS.setdefault(nastech_home_key(), {})
...
def _get_project_plugins_dir() -> Optional[Path]:
    """``./.nastech/plugins/`` or None. Gated on NASTECH_ENABLE_PROJECT_PLUGINS like the
...
def find_provider_dir(name: str) -> Optional[Path]:
    ...
    provider silently loses its dashboard panel and ``nastech <provider>`` commands."""
    ...
    try:
        ...
        if cli_mod is None:
            if not _is_bundled(plugin_dir):
                # cli.py imports as _nastech_user_memory.<name>.cli, usually before the
