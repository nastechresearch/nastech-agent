"""nemo_relay — optional Nastech plugin for NeMo Relay observability."""
...
_SESSION_INITIALIZER_NAME = "nastech.nemo_relay.rich_observability"
...
class _Settings:
    ...
    atof_filename: str = "nastech-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "nastech-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Nastech Agent"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Nastech profile; keeping the existing process-global configuration "
...
class _Runtime:
    def __init__(
        ...
        self._atof_subscriber_name = f"nastech.nemo_relay.atof.{self.host.runtime_id}"
        self._execution_consumer_name = (
            f"nastech.nemo_relay.rich_observability.{self.host.runtime_id}"
        )
        ...
        with self._sessions_lock:
            ...
            if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
                ...
                state.atif_subscriber_name = (
                    f"nastech.nemo_relay.atif.{self.host.runtime_id}.{session_id}"
                )
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Nastech core Relay session is unavailable")
    ...
    def export_atif(self, state: _SessionState) -> None:
        ...
        thread = threading.Thread(
            target=_runner,
            name=f"nastech-nemo-relay-atif-export-{state.session_id}",
            daemon=True,
        )
        thread.start()
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("nastech.subagent.stop", kwargs)
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Nastech installs the managed execution
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("nastech.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nastech.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nastech.turn.end", kwargs))


def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nastech.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("nastech.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("NASTECH_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("NASTECH_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("NASTECH_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("NASTECH_NEMO_RELAY_ATOF_FILENAME") or "nastech-atof.jsonl",
        atof_mode=_env("NASTECH_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("NASTECH_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("NASTECH_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("NASTECH_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "nastech-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("NASTECH_NEMO_RELAY_ATIF_AGENT_NAME") or "Nastech Agent",
        atif_agent_version=_env("NASTECH_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("NASTECH_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
        atif_export_timeout_s=_env_float(
            "NASTECH_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S", 30.0
        ),
    )
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Nastech cannot activate Relay gateway [[plugins.dynamic]] records because "
                "the Python binding does not expose the CLI lifecycle resolver for "
                "enablement, trust policy, and worker environments. Use Nastech-owned "
...
def _atif_subagent_export_mode() -> str:
    mode = _env("NASTECH_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="nastech-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()
