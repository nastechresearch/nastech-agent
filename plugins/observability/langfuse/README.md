...
Generation observations include the Nastech system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `NASTECH_LANGFUSE_MAX_CHARS`).
...
`NASTECH_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
