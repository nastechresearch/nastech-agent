...
Optional env vars:
  ...
  NASTECH_LANGFUSE_CAPTURE     - content capture mode (default: "sanitized")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    with _LANGFUSE_CLIENT_LOCK:
        ...
        # short-lived-process gap (kanban workers / nastech chat -q / cron): exit
...
def _canonical_usage_and_cost(
    ...
    *,
    ...
    # Nastech estimate rather than summing components because it also includes
...
def _finalize_all_traces() -> None:
    ...
    kanban workers, ``nastech chat -q`` one-shots, cron jobs — can exit while
