"""langfuse — Nastech plugin tracing conversations, LLM calls and tool usage to Langfuse.

Activated via ``plugins.enabled``; hooks are inert without the ``langfuse`` SDK
and credentials. Env: NASTECH_LANGFUSE_PUBLIC_KEY / SECRET_KEY (required),
...
# Under a multiplexed profile override, one settled client (or _INIT_FAILED) per Nastech home: the
...
def _debug(message: str) -> None:
    if _env("NASTECH_LANGFUSE_DEBUG").lower() in {"1", "true", "yes", "on"}:
...
def _capture_mode() -> str:
    ...
    value = _env("NASTECH_LANGFUSE_CAPTURE").lower()
    ...
    if not _warned_invalid_capture:
        ...
        logger.warning(
            "Langfuse plugin: invalid NASTECH_LANGFUSE_CAPTURE=%r, falling back "
            "to %r (valid: %s)",
...
def _settled_client() -> Any:
    ...
    return _LANGFUSE_CLIENT_BY_HOME.get(nastech_home_key())
...
def _settle_client() -> Any:
    ...
    if get_nastech_home_override() is None:
        ...
        _LANGFUSE_CLIENT_BY_HOME[nastech_home_key()] = settled
...
def _build_client() -> Optional[Langfuse]:
    ...
    if Langfuse is None:
        logger.warning(
            ...
            "tracing is disabled. Run `nastech tools` and configure Langfuse "
            "Observability to reinstall it."
        )
        return None

    public_key, secret_key = (_secret(f"NASTECH_LANGFUSE_{n}") or _secret(f"LANGFUSE_{n}") for n in ("PUBLIC_KEY", "SECRET_KEY"))
    ...
    for key, name, default in (("base_url", "BASE_URL", "https://cloud.langfuse.com"), ("environment", "ENV", ""),
                               ("release", "RELEASE", "")):
        value = _secret(f"NASTECH_LANGFUSE_{name}") or _secret(f"LANGFUSE_{name}") or default
        if value:
            kwargs[key] = value
    sample_rate = _secret("NASTECH_LANGFUSE_SAMPLE_RATE")
    if sample_rate:
        try:
...
def _canonical_usage_and_cost(canonical: Any, *, provider: str, model: str,
    ...
    """Translate canonical Nastech usage into Langfuse usage and cost maps."""
    ...
    # need an explicit total (Nastech estimate also includes request pricing).
...
def _start_root_trace(task_key: str, *, task_id: str, session_id: str, platform: str, provider: str, model: str,
    ...
    metadata = {
        "source": "nastech", "task_id": task_id, "turn_id": turn_id, "api_request_id": api_request_id,
    ...
    def open_root():
        ctx = client.start_as_current_observation(trace_context=trace_ctx, name="Nastech turn", as_type="chain",
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(session_id=session_id or task_key, trace_name="Nastech turn",
...
def register(ctx) -> None:
    # Both hook-name variants so the plugin works across Nastech versions:
