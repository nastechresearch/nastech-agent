...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Nastech ``/command`` text."""
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    typed_command_prefix = "!"  # clients reserve typed "/" for local commands; "!command" always reaches Nastech
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        self._store_dir = _get_nastech_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        return (self._store_dir or _get_nastech_dir("platforms/matrix/store", "matrix/store")) / "crypto.db"
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("NASTECH_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("NASTECH_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Nastech Agent",
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        if self._client is None:
            return None
        result = await self.send(parent_chat_id, (name or "").strip() or "Nastech session")
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Nastech Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
def interactive_setup() -> None:
    ...
    from nastech_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success, print_warning
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        for line in ("📬 Home Room: where Nastech delivers cron job results and notifications.",
...
def _is_connected(config) -> bool:
    """Connected = homeserver + token (or password). Reads via nastech_cli.gateway.get_env_value so
