...
def _needs_server_members_intent(
    ...
    """Return True when Nastech must request Discord's Server Members intent.
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.nastech/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Nastech gateway
    ...
    def _classify_connect_exception(self, error: Exception) -> tuple:
        ...
        if _is("PrivilegedIntentsRequired"):
            # Name the exact intents Nastech requested (#79430): Message
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``NASTECH_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    async def _run_missed_message_backfill(self) -> None:
        ...
        that lack a substantive non-outage Nastech response. Emoji-only acks are
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Nastech session")
        ...
        @tree.command(name="status", description="Show Nastech session status")
        ...
        @tree.command(name="stop", description="Stop the running Nastech agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.nastech/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Nastech Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Nastech gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Nastech session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Nastech in the thread",
        ...
        # nastech_cli/commands.py automatically appear as Discord slash
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Nastech skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Nastech session in the thread
    ...
    def resolved_allowlist_user_ids(self) -> set:
        ...
        hot-reload (``load_nastech_dotenv(override=True)`` restores the raw
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Nastech has an LLM-generated session title and can safely rename
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Nastech needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            embed = discord.Embed(
                title="𓄃 Update Needs Your Input",
            ...
            # some clients (see send_exec_approval).
            content = self._self_contained_prompt_content(
                "𓄃 **Update Needs Your Input**", f"{prompt}{default_hint}"
            )
            msg = await channel.send(content=content, embed=embed, view=view)
            view._message = msg  # store for on_timeout expiration editing
...
def _component_check_auth(
    ...
    the pairing store (``nastech pairing approve``).
    ...
    # so users approved via ``nastech pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``nastech update`` prompts.
    ...
    class ModelPickerView(discord.ui.View):
        ...
        def _build_model_select(self, provider_slug: str):
            ...
            truncating at 25. This matters for providers like Nastech whose
        ...
        async def _on_provider_selected(self, interaction: discord.Interaction):
            ...
            # and silently dropped the tail (e.g. Nastech `:free` Portal picks).
...
# is not in this process (e.g. ``nastech cron`` running standalone) and no live
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``nastech_cli.gateway.get_env_value`` at call time (not via
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # nastech_cli/setup.py::_setup_discord function.  Same shape as Teams.
