...
``<nastech-venv>/bin/pip install microsoft-teams-apps aiohttp``) and credentials via env
...
async def _standalone_send(
    ...
    (``nastech cron``). ``TEAMS_SERVICE_URL`` is allowlisted and ``chat_id`` charset-checked
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    cwd-discovered ``.env`` at import, mutating process-global ``os.environ``. Nastech owns dotenv loading.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def _on_card_action(
        ...
        choice = _APPROVAL_CHOICES.get(nastech_action)
        if not choice:
            return self._invoke_message("Unknown action.")
    ...
    async def _send_exec_approval_prompt(self, prompt: ExecApprovalPrompt) -> SendResult:
        ...
        for label, choice, style in prompt.actions:
            ...
            actions.append(ExecuteAction(
                title=label, verb="nastech_approve",
                data={**btn_data_base, "nastech_action": self._EA_CARD_ACTIONS[choice]}, **kw))
...
_SETUP_INTRO = (  # "" → blank line
    ...
    '  teams app create --name "Nastech" --endpoint "https://<tunnel>/api/messages"', "",
...
def interactive_setup() -> None:
    ...
    from nastech_cli.cli_output import prompt, prompt_yes_no, print_info, print_success, print_warning
...
def _install_hint() -> str:
    ...
    ``venv_pip=True`` targets the real Nastech venv, sidestepping PEP 668 on Ubuntu 24.04."""
    try:
