...
_DEFAULT_DESCRIPTION = "Nastech Agent — a general-purpose agent reachable over A2A."
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed task to another local profile via ``nastech chat``. First contact creates a
        ...
        with self._forward_lock(key):
            ...
            cmd = ["nastech", "chat", "-q", framed_text, "-Q", "--source", "a2a"] + (["--resume", session_id] if session_id else [])
