"""Email platform adapter for the Nastech gateway: users talk to Nastech by sending email; IMAP (polled)
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        try:
            ...
            _nastech_version = "0"
        imap.xatom("ID", f'("name" "nastech-agent" "version" "{_nastech_version}" '
                         '"vendor" "NastechResearch" "support-email" "noreply@nastechresearch.github.io")')
...
class EmailAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing := [name for name, value in required if not value]:
            message = f"Not configured — missing {', '.join(missing)}. Set it via `nastech gateway setup` (env) or platforms.email in config.yaml."
    ...
    def _new_reply(self, to_addr: str, body: str, reply_to_msg_id: Optional[str] = None, *,
        ...
        msg_id = f"<nastech-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        for key, value in (("From", address), ("To", chat_id), ("Subject", "Nastech Agent"), ("Date", formatdate(localtime=True))):
            msg[key] = value
