...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            ...
            '"vendor" "NastechResearch" '
            '"support-email" "noreply@nastechresearch.github.io")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `nastech config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `nastech gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
...
# nastech_cli/gateway.py, and the _send_email dispatch in
