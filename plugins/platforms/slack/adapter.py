...
# User-Agent prefix (``NastechAgent/<version>``) for platform-partner attribution of API calls.
try:
...
_MODEL_PICKER_PROVIDER_ACTION = "nastech_model_provider"
_MODEL_PICKER_MODEL_ACTION = "nastech_model_model"
_MODEL_PICKER_BACK_ACTION = "nastech_model_back"
_MODEL_PICKER_CANCEL_ACTION = "nastech_model_cancel"
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # URL); labels after dates (dates carry a label); bot mention after labels (``<@U…|nastech>``).
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if team_key not in warned and "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "from `nastech slack` produces a manifest with these already included.",
    ...
    def _register_bolt_handlers(self) -> None:
        ...
        # Catch-all no-op ack for any other subscribed event type that Nastech has no listener for (e.g.
        ...
        async def handle_unhandled_event(event, body, logger):
            logger.debug(
                ...
                "events not handled by Nastech can be removed from the Slack app manifest via "
                "`nastech slack manifest`)",
        ...
        # ALSO be declared in the app manifest (`nastech slack manifest`): Socket Mode won't
        # deliver undeclared commands at all.
        from nastech_cli.commands_platforms import slack_native_slashes
        _slash_names = [name for name, _d, _h in slack_native_slashes()]
        if _slash_names:
            _slash_pattern = re.compile(
                r"^/(?:" + "|".join(re.escape(n) for n in _slash_names) + r")$")
        else:  # pragma: no cover - registry always non-empty
            _slash_pattern = re.compile(r"^/nastech$")
        ...
        self._app.action("nastech_feedback")(self._handle_feedback_action)
        # Clarify buttons (tools/clarify_gateway.py); indexed action IDs because
        # Block Kit requires unique IDs within an actions block.
        self._app.action(re.compile(r"^nastech_clarify_choice_\d+$"))(self._handle_clarify_action)
        self._app.action("nastech_clarify_other")(self._handle_clarify_action)
    ...
    def _new_web_client(token: str, proxy_url: Optional[str]) -> Any:
        client = AsyncWebClient(token=token, user_agent_prefix=_NASTECH_SLACK_USER_AGENT_PREFIX)
        _apply_slack_proxy(client, proxy_url)
        return client
    ...
    def _fatal_missing_env(self, env_name: str) -> None:
        ...
        logger.error(
            "[Slack] %s not set — this is a permanent config error; set %s via `nastech "
            "gateway setup` or in the active profile's ~/.nastech/.env file, then restart the "
            "gateway.", env_name, env_name)
        self._set_fatal_error(
            f"missing_{env_name.lower()}",
            f"{env_name} not configured. Use `nastech gateway setup` "
            "or add it to your active profile's ~/.nastech/.env file, then restart the gateway.",
            retryable=False)
    ...
    def _hint_allow_bots(self) -> None:
        ...
        if _allow_bots_cfg != "none":
            logger.info(
                ...
                "appropriate (run 'nastech slack manifest' if unsure), and (b) the other bot's "
    ...
    async def send_native_task_card_progress(
        self, chat_id: str, tasks: List[Dict[str, str]], *, title: str = "Nastech is working",
    ...
    def _synthetic_reaction_event(
        ...
        from conflating it with the reacted-to message; ``_nastech_force_process`` skips the mention
        requirement (user auth and allowed_channels still apply); ``_nastech_reaction`` is
        ...
        if target_channel:
            ...
            synthetic["_nastech_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_nastech_no_thread_response"] = True
        return synthetic
    ...
    def _session_thread_ts(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
            return thread_ts
        if event.get("_nastech_no_thread_response"):
    ...
    async def _handle_slack_message_impl(self, event: dict, payload: Optional[dict] = None) -> None:
        ...
        force_process = bool(event.get("_nastech_force_process"))
    ...
    _EA_ACTION_IDS = {"once": "nastech_approve_once", "session": "nastech_approve_session",
                      "always": "nastech_approve_always", "deny": "nastech_deny"}
        ...
        def _build() -> Tuple[str, list]:
            ...
            blocks = [
                {"type": "section", "text": {"type": "mrkdwn", "text": f"*{_title}*\n\n{body}"}},
                {
                    "type": "actions",
                    "elements": [
                        self._button("Approve Once", "nastech_confirm_once", value, style="primary"),
                        self._button("Always Approve", "nastech_confirm_always", value),
                        self._button("Cancel", "nastech_confirm_cancel", value, style="danger")]}]
    ...
    async def send_clarify(
        ...
        """Clarify prompt as Block Kit buttons: one ``nastech_clarify_choice_<idx>`` per option
        (value ``clarify_id|idx``) plus "✏️ Other…" (``nastech_clarify_other``), which flips the
        ...
        def _build() -> Tuple[str, list]:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append(
                    self._button(
                        label[:75], f"nastech_clarify_choice_{idx}",
                        f"{clarify_id}|{idx}", emoji=True))
            elements.append(
                self._button("✏️ Other…", "nastech_clarify_other", f"{clarify_id}|other", emoji=True)
            )
    ...
    _APPROVAL_CHOICES: ClassVar[Dict[str, str]] = {
        "nastech_approve_once": "once", "nastech_approve_session": "session",
        "nastech_approve_always": "always", "nastech_deny": "deny"}
    ...
    _CONFIRM_CHOICES: ClassVar[Dict[str, str]] = {
        "nastech_confirm_once": "once", "nastech_confirm_always": "always",
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "nastech_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        ``/nastech <subcommand> [args]``; other text after ``/nastech`` is a regular message."""
        ...
        # events only: free-form "/nastech <question>" replies must stay public.
    ...
    def _slash_command_text(command: dict) -> str:
        ...
        payload verbatim (internal/trailing spacing). ``/nastech`` (or a missing ``command``) maps
        ...
        if slash_name not in {"nastech", ""}:
            return f"/{slash_name}" if not raw_text else f"/{slash_name} {raw_text}"
        legacy_text = raw_text.strip()
        from nastech_cli.commands_platforms import slack_subcommand_map
...
# in ``nastech_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in ``tools/send_message_tool.py``).
# ──────────────────────────────────────────────────────────────────────────
...
def _write_slack_manifest_and_instruct() -> None:
    """Write the manifest under NASTECH_HOME and print paste instructions; non-fatal."""
    from nastech_cli.cli_output import print_info, print_success, print_warning
    try:
        from nastech_cli.slack_cli import _build_full_manifest
        from nastech_constants import get_nastech_home
        manifest = _build_full_manifest(
            bot_name="Nastech", bot_description="Your Nastech agent on Slack")
        target = _Path(get_nastech_home()) / "slack-manifest.json"
...
def interactive_setup() -> None:
    ...
    from nastech_cli.config import remove_env_value, save_env_value
    ...
    if declines_reconfigure("Slack", "Reconfigure Slack?", "SLACK_BOT_TOKEN"):
        ...
        if prompt_yes_no(
            ...
            "list? (recommended after `nastech update`)", True):
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nastech setup` to install Slack support.",
