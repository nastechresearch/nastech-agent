...
``$NASTECH_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `nastech photon install-sidecar` on npm failure so check_requirements() can
...
def dir_writable(path: Path) -> bool:
    ...
    probe = path / ".nastech-write-probe"
    try:
        probe.touch()
        probe.unlink()
    except OSError:
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_nastech_home() / "photon" / "sidecar"
    try:
