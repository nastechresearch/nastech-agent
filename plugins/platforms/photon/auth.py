...
``(id, projectSecret)``. Storage: runtime SDK creds -> ``~/.nastech/.env``; management
...
# published CLI client until Nastech gets its own client_id.
...
# -- auth.json helpers (shares the file with the rest of nastech-agent) ------------

def _auth_json_path() -> Path:
    """The active profile's ``auth.json`` (shared with the rest of nastech-agent)."""
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.nastech/.env`` (secret never bound to a printable local
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    rows = [
        ...
        "──────────────────────",
        "  device token        : " + (
            "✓ stored" if load_photon_token() else "✗ missing (run `nastech photon setup`)"),
        "  project id          : " + (sid if sid else "✗ missing"),
        "  project secret      : " + ("✓ stored" if sec else "✗ missing"),
        "  my number           : " + (phone if phone else "✗ missing (run `nastech photon setup --phone ...`)"),
        "  assigned number     : " + (assigned if assigned else "✗ missing (run `nastech photon setup`)")]
    emit("\n".join(rows))
