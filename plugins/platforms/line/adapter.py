...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, NASTECH_HOME).
        ...
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), nastech_home)):
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``nastech status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``nastech setup line`` wizard (writes ``~/.nastech/.env``); CLI helpers are lazy-imported."""
    from nastech_cli.config import get_env_value, save_env_value
    from nastech_cli.cli_output import print_header, print_info, prompt
