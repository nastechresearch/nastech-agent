...
# trigger its own recovery path. Refs: NastechResearch/nastech-agent#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: NastechResearch/nastech-agent#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        queued follow-ups after a gateway restart) prefer the Nastech topic's
        ``message_thread_id`` so they stay in the active topic lane (#87051);
        ``direct_messages_topic_id`` is only used when no topic thread
        resolves, since the native DM-topic id does not match the Nastech
        ...
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            ...
            if reply_to_message_id is None:
                ...
                # active topic lane: prefer the Nastech topic thread id when it
    ...
    def _prune_stale_dm_topic_binding(
        ...
        *,
        ...
        profile that wrote the binding, so the send's ``nastech_profile``
        ...
        try:
            profile_name = (
                (metadata or {}).get("nastech_profile")
                or getattr(self, "_nastech_profile_name", None)
                or "default"
            )
    ...
    # older PTB/clients. Streaming edits stay on Nastech' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Nastech must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                from nastech_cli.commands import telegram_menu_commands, telegram_menu_max_commands
                if not self._bot:
                    return
                # Telegram allows up to 100 commands but has an undocumented
                # payload size limit (~4KB total).  Nastech defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            disable_fallback = (
                os.getenv("NASTECH_TELEGRAM_DISABLE_FALLBACK_IPS", "")
                .strip()
                .lower()
                in {"1", "true", "yes", "on"}
            )
            ...
            if not fallback_ips and not disable_fallback:
                discovery_timeout = self._env_float_clamped(
                    "NASTECH_TELEGRAM_FALLBACK_DISCOVERY_TIMEOUT",
                    5.0,
                    min_value=0.0,
                )
                logger.warning(
            ...
            _max_connect = 8
            _init_timeout = _env_float("NASTECH_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set NASTECH_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
    ...
    async def _handle_inline_query(
        ...
        Nastech default), so most skill commands can never appear in the
    ...
    async def _handle_callback_query(
        ...
        try:
            await query.edit_message_text(
                text=self.format_message(f"𓄃 Update prompt answered: *{label}*"),
                parse_mode=ParseMode.MARKDOWN_V2,
                reply_markup=None,
            )
        except Exception:
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Nastech inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Nastech inbox (#52363).
        #
...
# nastech_cli/{setup,gateway}.py, and the _send_telegram dispatch in
...
def interactive_setup() -> None:
    ...
    _PLATFORMS["telegram"] static dict dispatch in nastech_cli/gateway.py.
    """
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `nastech setup` to install Telegram support.",
