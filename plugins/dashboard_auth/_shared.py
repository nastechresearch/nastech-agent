...
# JWKS Cache-Control max-age (nastech contract C7); self-hosted mirrors it.
...
def pkce_login_start(authorize_url: str, *, client_id: str, scope: str, redirect_uri: str) -> LoginStart:
    ...
    ``cookie_payload["nastech_session_pkce"]`` as a flat ``state=…;verifier=…`` string
    ...
    return LoginStart(
        ...
        cookie_payload={"nastech_session_pkce": f"state={state};verifier={code_verifier}"})
...
def make_jwks_client(jwks_url: str) -> Any:
    ...
    return PyJWKClient(
        ...
        headers={"Accept": "application/json", "User-Agent": "NastechAgent/1.0"})
...
class JwtOAuthProvider(DashboardAuthProvider):
    ...
    (nastech: Portal access token; self-hosted: OIDC ID token). Subclasses set ``_client_id`` and
