...
Implements ``nastech-account-service/docs/agent-dashboard-oauth-contract.md``
...
"state": …}`` and the route serializes those into the ``nastech_session_pkce``
cookie.
...
class NastechDashboardAuthProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        # The auth-route layer expects ``cookie_payload[\"nastech_session_pkce\"]``
        ...
        cookie_payload = {
            "nastech_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def complete_login(
        ...
        *,
        ...
        # we just receive it for symmetry with the protocol. Nastech Portal
    ...
    def refresh_session(self, *, refresh_token: str) -> Session:
        ...
        try:
            response = httpx.post(
                ...
                # ``x-nastech-refresh-token`` header. Portal's token endpoint
                ...
                # Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-nastech-refresh-
    ...
    def _check_contract_version(self, claims: Dict[str, Any]) -> None:
        ...
        if contract_version is None:
            logger.warning(
                "Nastech Portal token missing oauth_contract_version claim "
...
def _load_config_oauth_section() -> dict:
    ...
    try:
        ...
        logger.debug(
            "dashboard-auth-nastech: load_config() raised %s; "
...
def register(ctx) -> None:
    ...
    Registers ``NastechDashboardAuthProvider`` only when a client_id is
    ...
    engagement layer (``nastech_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "Nastech Portal provisions this env var (shape "
        ...
        logger.debug("dashboard-auth-nastech: %s", LAST_SKIP_REASON)
        return

    if not client_id.startswith("agent:"):
        LAST_SKIP_REASON = (
            f"NASTECH_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match "
            f"the contract shape 'agent:{{instance_id}}'. The Nastech Portal "
            f"provisions this value at deploy time; check your Fly app's "
            f"secrets or override with the value from the Portal admin UI."
        )
        logger.warning("dashboard-auth-nastech: %s", LAST_SKIP_REASON)
        return

    try:
        provider = NastechDashboardAuthProvider(
            client_id=client_id, portal_url=portal_url
        )
    except ValueError as exc:
        LAST_SKIP_REASON = f"NastechDashboardAuthProvider construction failed: {exc}"
        logger.warning("dashboard-auth-nastech: %s", LAST_SKIP_REASON)
        return

    ctx.register_dashboard_auth_provider(provider)
    logger.info(
        "dashboard-auth-nastech: registered provider (client_id=%s, portal=%s)",
        client_id,
        portal_url,
    )
