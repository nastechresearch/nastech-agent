"""NastechDashboardAuthProvider — Nastech Portal OAuth (authorization-code + PKCE).

Implements ``nastech-account-service/docs/agent-dashboard-oauth-contract.md``; registers only
when a client_id (``dashboard.oauth.client_id`` / ``NASTECH_DASHBOARD_OAUTH_CLIENT_ID``, shape
...
from nastech_cli.dashboard_auth import LoginStart, ProviderError, Session
...
class NastechDashboardAuthProvider(JwtOAuthProvider):
    """Nastech Portal OAuth via authorization-code + PKCE (S256)."""
    ...
    def _claims_for(self, access_token: str) -> Dict[str, Any]:
        ...
        if contract_version is None:
            logger.warning(
                "Nastech Portal token missing oauth_contract_version claim (contract says it should be %d); proceeding anyway.",
...
def _settings() -> dict:
    """Resolve NastechDashboardAuthProvider kwargs; the skip reason names BOTH configuration surfaces."""
    section = _load_config_oauth_section()
    client_id = resolve_env_or_cfg("NASTECH_DASHBOARD_OAUTH_CLIENT_ID", section.get("client_id", ""))
    portal_url = resolve_env_or_cfg("NASTECH_DASHBOARD_PORTAL_URL", section.get("portal_url", "")) or _DEFAULT_PORTAL_URL
    if not client_id:
        raise SkipRegistration(
            "NASTECH_DASHBOARD_OAUTH_CLIENT_ID is not set (and dashboard.oauth.client_id "
            "in config.yaml is empty). The Nastech Portal provisions this env var (shape "
            "'agent:{instance_id}') when it deploys a Nastech Agent instance — set it to "
    ...
    if not client_id.startswith("agent:"):
        raise SkipRegistration(
            f"NASTECH_DASHBOARD_OAUTH_CLIENT_ID={client_id!r} doesn't match the contract "
            f"shape 'agent:{{instance_id}}'. The Nastech Portal provisions this value at deploy "
...
def register(ctx) -> None:
    """Register ``NastechDashboardAuthProvider`` when a client_id is configured."""
    global LAST_SKIP_REASON
    LAST_SKIP_REASON = ""
    kwargs, LAST_SKIP_REASON = register_provider(ctx, logger, _TAG, NastechDashboardAuthProvider, _settings)
    if kwargs is not None:
        logger.info(
            "dashboard-auth-nastech: registered provider (client_id=%s, portal=%s)", kwargs["client_id"], kwargs["portal_url"])
...
_PLUGIN_COMPAT_LAZY = {
    'DashboardAuthProvider': ('nastech_cli.dashboard_auth', 'DashboardAuthProvider'),
    'InvalidCodeError': ('nastech_cli.dashboard_auth', 'InvalidCodeError'),
    'RefreshExpiredError': ('nastech_cli.dashboard_auth', 'RefreshExpiredError'),
    'classify_jwks_lookup_error': ('nastech_cli.dashboard_auth', 'classify_jwks_lookup_error'),
}
