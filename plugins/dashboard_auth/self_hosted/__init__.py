...
OAuth gate. Unlike the bundled ``nastech`` provider (which encodes Nastech Portal's
bespoke contract — ``agent:{instance_id}`` client ids, a custom access-token
JWT, the ``x-nastech-refresh-token`` header, an ``oauth_contract_version`` claim),
...
:class:`~nastech_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``nastech_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~nastech_cli.dashboard_auth.Session`.
...
only choice that is universally correct across self-hosted IDPs. (The ``nastech``
provider verifies its *access* token because Nastech Portal mints a custom JWT
...
same precedence convention as the ``nastech`` plugin)::
...
# JWKS cache (PyJWKClient handles its own caching; this mirrors the nastech
# provider's 5-minute lifespan so key rotation is picked up promptly).
_JWKS_CACHE_SECONDS = 300


# ---------------------------------------------------------------------------
# Skip-reason channel (mirrors the nastech plugin)
# ---------------------------------------------------------------------------
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the nastech provider.
        """
