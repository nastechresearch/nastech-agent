...
``BROWSER_USE_API_KEY`` (https://browser-use.com) or the managed Nastech tool gateway (bills to a
Nastech subscription). Direct first, managed second, unless ``tool_gateway.browser: gateway`` flips
...
class BrowserUseBrowserProvider(CloudBrowserProvider):
    ...
    # registered for the Nastech gateway path and legacy cloud_provider configs.
    setup_tag = None
    ...
    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Lazy: managed_tool_gateway pulls in the Nastech auth stack direct-key users never need.
        from tools.managed_tool_gateway import peek_nastech_access_token, resolve_managed_tool_gateway
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use", token_reader=None if refresh_token else peek_nastech_access_token)
            if managed is None:
                return None
            return {
        ...
        # Strict: "nastech" (or legacy use_gateway: true) → managed ONLY; any other stored selection →
    ...
    def _get_config(self) -> Dict[str, Any]:
        from tools.tool_backend_helpers import (
            NASTECH_MANAGED_PROVIDER, managed_nastech_tools_enabled, read_selection, selection_error)
        ...
        if selected == NASTECH_MANAGED_PROVIDER:
            raise ValueError(selection_error(
                "browser", NASTECH_MANAGED_PROVIDER,
