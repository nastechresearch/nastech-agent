...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Nastech tool
gateway (which Nastech uses to bill Browser Use sessions to a Nastech
...
Auth env vars (one of)::
    ...
    # OR a managed Nastech gateway entry (configured via 'nastech setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Nastech
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Nastech tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Nastech gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self, *, refresh_token: bool = True) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Nastech auth stack which can be
        ...
        def _managed_config() -> Optional[Dict[str, Any]]:
            ...
            managed = resolve_managed_tool_gateway(
                "browser-use",
                token_reader=None if refresh_token else peek_nastech_access_token,
            )
            if managed is None:
                return None
            return {
                "api_key": managed.nastech_user_token,
        ...
        # Strict selection: "nastech" (or legacy use_gateway: true) → managed
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"nastech_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
    ...
    def get_setup_schema(self) -> Optional[Dict[str, Any]]:
        # Hidden from the nastech tools picker: the "Browser Use" row now
        # activates the CLI-based backend (tools/browser_use_cli.py). This
        # provider stays registered for the Nastech gateway path and un-migrated
