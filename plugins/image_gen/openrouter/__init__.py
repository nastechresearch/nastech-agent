...
via :func:`nastech_cli.runtime_provider.resolve_runtime_provider`. OpenRouter alone also
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    """One instance per backend (OpenRouter, Nastech Portal); they differ only in the runtime
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        releases selectable); Nastech Portal = chat-completions only. Offline: default chain + snapshot."""
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        # No picker row: Portal models are offered inside the single managed "Nastech Subscription" row
        # (tools/image_generation_managed.py). A row of its own also wrote `provider: nastech` and so
        # read "active" alongside the managed FAL row while the runtime routed its pick to FAL.
        OpenRouterCompatImageProvider(
            provider_name="nastech", display_name="Nastech Portal", runtime_name="nastech", config_key="nastech",
            model_env_var="NASTECH_IMAGE_MODEL", setup_schema=None),
    ]
