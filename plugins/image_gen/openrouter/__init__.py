"""OpenRouter-compatible image generation backend (OpenRouter + Nastech Portal).

Both OpenRouter and the Nastech Portal inference endpoint speak the same
...
Nastech Portal proxies OpenRouter, so one implementation services both — we only
...
which already understands OpenRouter's key pool and the Nastech OAuth device-code
...
The Image API is OpenRouter-only: Nastech Portal proxies the chat-completions
protocol and has no ``/images/generations`` route, so the second surface is
enabled per provider (see ``supports_image_api``) and stays off for Nastech.
"""
...
# image_gen.model from ``nastech tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Nastech Portal). The two differ only
    ...
    def __init__(
        ...
        *,
        ...
        # OpenRouter only: Nastech Portal proxies the chat-completions protocol
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        ...
        after this code shipped — is selectable in ``nastech tools``. Nastech
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``nastech tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Nastech Portal).
            "HTTP-Referer": "https://github.com/NastechResearch/nastech-agent",
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            provider_name="nastech",
            display_name="Nastech Portal",
            runtime_name="nastech",
            config_key="nastech",
            model_env_var="NASTECH_IMAGE_MODEL",
            setup_schema={
                "name": "Nastech Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Nastech Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Nastech Portal image gen providers."""
