...
def _resolve_managed_krea_gateway():
    ...
    ``image_gen`` selection is ``nastech`` (or legacy ``use_gateway: true``), or
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def is_available(self) -> bool:
        # Available with a direct Krea key OR via the managed Nastech gateway
        # (Nastech Subscription), so portal users with no Krea key can still
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Route through the managed Nastech gateway (Nastech Subscription) when the
        ...
        # meters/bills per generation, so the caller token is the Nastech access
        ...
        if managed is not None:
            if isinstance(kwargs.get("styles"), list) and kwargs.get("styles"):
                return error_response(
                    error=(
                        "Managed Krea (Nastech Subscription) does not support "
        ...
        try:
            ...
            # the Nastech Portal, or the gateway's shared Krea key hit its
            # concurrency cap (429).
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "Nastech Portal's Krea gateway. Set KREA_API_KEY to use "
        ...
        # ``/jobs/{id}`` with the Nastech token (404 on cross-user/unknown jobs).
        job_url = f"{base_url}/jobs/{job_id}"
        poll_headers = {
            "Authorization": f"Bearer {auth_token}",
            "User-Agent": "Nastech-Agent/1.0 (krea-image-gen)",
