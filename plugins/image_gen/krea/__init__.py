...
# Nastech' 3 abstract ratios → Krea's enum (1:1, 4:3, 3:2, 16:9, 2.35:1, 4:5, 2:3, 9:16).
...
_USER_AGENT = "Nastech-Agent/1.0 (krea-image-gen)"
...
def _resolve_managed_krea_gateway():
    ...
    ``image_gen`` selection is ``nastech`` (or legacy ``use_gateway: true``), or never-configured with
    ...
    try:
        ...
        selected = None
    if selected is not None and selected != NASTECH_MANAGED_PROVIDER:
        return None
        ...
        if failure.kind == "http":
            ...
            if managed and 400 <= status < 500:
                hint = (
                    ...
                    f"Model '{model_id}' may not be enabled/priced on the Nastech Portal's Krea gateway. "
                    "Set KREA_API_KEY to use Krea directly, or pick a different model via "
                    "`nastech tools` → Image Generation.")
                return None, fail(
                    f"Nastech Subscription Krea gateway rejected '{model_id}' "
...
class KreaImageGenProvider(StaticImageGenProvider):
    ...
    setup = dict(
        ...
        tag="Krea 2 foundation model — Medium ($0.03), Large ($0.06), Medium Turbo ($0.015). Style transfer, moodboards, reference-guided generation. Direct key or managed Nastech Subscription gateway.",
        key="KREA_API_KEY", prompt="Krea API key", url="https://www.krea.ai/settings/api-tokens")

    def is_available(self) -> bool:
        # Direct key OR managed Nastech gateway (portal users without a Krea key).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        # Nastech access token); otherwise direct Krea with a BYO ``KREA_API_KEY``.
        managed = _resolve_managed_krea_gateway()
        if managed is not None:
            base_url = managed.gateway_origin.rstrip("/")
            auth_token = managed.nastech_user_token
        else:
            base_url = BASE_URL
            ...
            if not auth_token:
                return error_factory("krea", aspect)(
                    ...
                    "a Nastech account with the managed Krea gateway enabled "
                    "(`nastech setup`).",
                    "auth_required")
        ...
        if managed is not None:
            for what, arg in _MANAGED_UNSUPPORTED:
                if arg in payload:
                    return fail(
                        f"Managed Krea (Nastech Subscription) does not support {what}. "
        ...
        # 2. Poll — same principal as submit, so the managed path polls the gateway with the Nastech token.
