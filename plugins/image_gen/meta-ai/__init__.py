...
Output is base64 JSON (WebP) -> saved under ``$NASTECH_HOME/cache/images/``.

Selection precedence (first hit wins):
  1. ``model`` kwarg forwarded by the dispatcher (the ``nastech tools`` pick)
...
# Model catalog
# ---------------------------------------------------------------------------
# Catalog shown in `nastech tools` and matched against `image_gen.model`.
...
def _resolve_model(caller_model: Optional[str] = None) -> Tuple[str, Dict[str, Any]]:
    ...
    top-level ``image_gen.model`` config key (what ``nastech tools`` writes).
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    f"{API_KEY_ENV} not set. Run `nastech tools` -> Image "
