...
Every handler is a thin wrapper around ``nastech_cli.kanban_db`` (the same code paths the CLI
...
from nastech_cli import kanban_db_workspace as kbw
from nastech_cli import kanban_diagnostics as kd
from nastech_cli.kanban_db import KANBAN_ATTACHMENT_MAX_BYTES, _collision_free_path, _safe_attachment_name
...
def _ws_upgrade_authorized(ws: "WebSocket") -> bool:
    ...
    try:
        from nastech_cli import web_server_chat as _ws
    except Exception:
        return True
...
def _with_board_pinned(board: Optional[str], fn: Callable[[], Any]) -> Any:
    ...
    ``NASTECH_KANBAN_BOARD`` env var (concurrent requests for different boards would cross-write)."""
...
def _run_aux(board: Optional[str], module: str, fn: str, task_id: str, author: Optional[str]) -> Any:
    """Run a slow auxiliary-LLM task helper (``nastech_cli.<module>.<fn>``) with the board pinned;
    the module is imported lazily so a missing aux client can't break plugin load."""
    def _run():
        return getattr(importlib.import_module(f"nastech_cli.{module}"), fn)(task_id, author=(author or None))
...
def get_board(
    ...
    (``NASTECH_KANBAN_BOARD`` env → on-disk ``current`` pointer → ``default``)."""
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    with _board_conn(board) as (board, conn), _value_error_400():
        ...
        # dashboard backend may run under a different NASTECH_HOME than the board's profile.
        if task and task.status == "ready" and task.assignee:
            try:
                from nastech_cli.kanban import _check_dispatcher_presence
                from nastech_constants import get_nastech_home
                running, message = _check_dispatcher_presence(nastech_home=get_nastech_home())
...
class UpdateTaskBody(BaseModel):
    ...
    # Handoff fields forwarded to complete_task on -> 'done' (parity with ``nastech kanban complete``).
...
def list_diagnostics(
    ...
    consumed by ``nastech kanban diagnostics`` when the dashboard runs."""
    with _board_conn(board) as (board, conn):
...
def specify_task_endpoint(task_id: str, payload: SpecifyBody, board: Optional[str] = Query(None)):
    """Flesh out a triage task via the auxiliary LLM (``nastech kanban specify``). Non-OK is NOT
...
def reassign_task_endpoint(task_id: str, payload: ReassignBody, board: Optional[str] = Query(None)):
    ...
    (``nastech kanban reassign <task_id> <profile> [--reclaim]``)."""
    with _board_conn(board) as (board, conn):
...
def _active_profile_name() -> str:
    """Current Nastech profile name for notify-sub ownership."""
    try:
...
def model_options():
    ...
    (same substrate as the Models page) so it can't offer a pair Nastech rejects. Skips pricing
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    """``nastech profile describe <name> --auto``: persist with ``description_auto: true``.
...
def decompose_task_endpoint(task_id: str, payload: DecomposeBody, board: Optional[str] = Query(None)):
    """Fan a triage task out into child tasks via the auxiliary LLM (``nastech kanban decompose``).
