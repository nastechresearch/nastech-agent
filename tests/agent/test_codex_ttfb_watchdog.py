...
def _make_codex_agent(
    ...
    *,
    ...
    for name in ("NASTECH_CODEX_TTFB_TIMEOUT_SECONDS", "NASTECH_CODEX_TTFB_MAX_SECONDS",
...
def _shorten_implicit_idle_watchdog(monkeypatch, helpers, timeout=2.0):
    ...
    monkeypatch.delenv("NASTECH_CODEX_EVENT_STALE_TIMEOUT_SECONDS", raising=False)
...
def test_ttfb_installs_and_retires_the_codex_request_token(tmp_path, monkeypatch):
    ...
    agent = _make_codex_agent(tmp_path, monkeypatch)
    monkeypatch.setenv("NASTECH_CODEX_TTFB_TIMEOUT_SECONDS", "1")
...
def test_idle_phase_policy_is_narrow_and_preserves_operator_overrides(
    ...
    if idle_env is None:
        ...
        monkeypatch.setenv("NASTECH_CODEX_EVENT_STALE_TIMEOUT_SECONDS", idle_env)
...
def test_event_stale_phase_is_scoped_to_physical_stream_attempt(
    ...
    monkeypatch.setenv("NASTECH_CODEX_TTFB_TIMEOUT_SECONDS", "2")
    monkeypatch.setenv("NASTECH_CODEX_TTFB_STRICT", "1")
    monkeypatch.setenv("NASTECH_CODEX_HARD_TIMEOUT_SECONDS", "5")
...
def test_explicit_ttfb_max_seconds_still_caps(tmp_path, monkeypatch):
    """An explicit NASTECH_CODEX_TTFB_MAX_SECONDS override still bounds the
    scaled cutoff."""
    from agent import chat_completion_helpers as h

    agent = _make_codex_agent(tmp_path, monkeypatch)
    agent.reasoning_config = {"enabled": False}
    monkeypatch.setenv("NASTECH_CODEX_TTFB_MAX_SECONDS", "90")
