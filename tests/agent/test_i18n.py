...
def test_language_is_per_profile_under_multiplex(monkeypatch, tmp_path):
    """NASTECH_LANGUAGE in the DEFAULT profile's environ must not leak into a secondary profile's
    ...
    monkeypatch.setenv("NASTECH_LANGUAGE", "zh")  # default profile's .env, bridged into environ
    i18n.reset_language_cache()
