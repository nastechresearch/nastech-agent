...
def test_nastech_pkce_refresh_writes_back_to_singleton(tmp_path, monkeypatch):
    """A successful nastech_pkce refresh must update
    ~/.nastech/.anthropic_oauth.json, or ``_seed_from_singletons()`` on the
    ...
    oauth_file = nastech_home / ".anthropic_oauth.json"
    oauth_file.write_text(
        json.dumps({"accessToken": "sk-ant-oat-rt0", "refreshToken": "rt0", "expiresAt": 0}),
        encoding="utf-8",
    )
    _write_store(nastech_home / "auth.json", {"version": 1, "providers": {}})
    ...
    assert on_disk["refreshToken"] == "rt1", (
        "successful nastech_pkce refresh must write back to "
        "~/.nastech/.anthropic_oauth.json, or _seed_from_singletons() will "
        "revert the pool entry to the pre-refresh (spent) token on next load"
    )

    reloaded = load_pool("anthropic")
    reloaded_entries = [e for e in reloaded.entries() if e.source.endswith("nastech_pkce")]
    assert reloaded_entries, "nastech_pkce entry should still be present after reload"
...
def test_manual_nastech_pkce_refresh_does_not_create_duplicate_singleton(
    tmp_path, monkeypatch
):
    """A pool-owned manual:nastech_pkce entry must not create a second source."""
    nastech_home = tmp_path / "nastech"
    nastech_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.setattr("nastech_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    assert not oauth_file.exists(), (
        "manual:nastech_pkce is already pool-owned; refreshing it must not "
        "create a second nastech_pkce singleton source"
    )

    reloaded = load_pool("anthropic")
    matching = [e for e in reloaded.entries() if e.id == "manual-entry"]
    assert len(matching) == 1
    assert matching[0].source == "manual:nastech_pkce"
