...
def test_kanban_guidance_requires_worker_task_at_agent_init(monkeypatch, task_id, expected):
    ...
    if task_id is None:
        ...
        monkeypatch.setenv("NASTECH_KANBAN_TASK", task_id)
...
def test_kanban_guidance_fallback_requires_owned_worker_task(monkeypatch, task_id, owner, expected):
    ...
    is not identity, and an inherited NASTECH_KANBAN_TASK is not ownership (#112486)."""
...
class TestNamedProfileHintIntegration:
    ...
    ``TestNamedProfileHint`` mocks ``get_nastech_home``,
    ``get_default_nastech_root`` and ``_resolve_active_profile_name``, so it
    ...
    ``NASTECH_HOME`` and no resolver mocks.
    """

    def test_real_nastech_home_under_profiles_renders_correct_paths(
        ...
        assert "Active Nastech profile: coder." in prompt
    ...
    def test_real_default_home_renders_default_branch(self, tmp_path, monkeypatch):
        """NASTECH_HOME at the root resolves to the default profile, unchanged."""
        root = tmp_path / ".nastech"
        ...
        assert "Active Nastech profile: default." in prompt
...
def test_coding_prompt_orders_shared_context_before_workspace(monkeypatch):
    ...
    monkeypatch.setattr(system_prompt, "NASTECH_AGENT_HELP_GUIDANCE_NO_SKILLS", "HELP")
    monkeypatch.setattr(system_prompt, "STEER_CHANNEL_NOTE", "STEER")
    monkeypatch.setattr(system_prompt, "get_nastech_home", lambda: Path("/nastech"))

    # Production renders this as str(get_nastech_home()) + "/profiles/<name>/",
    # and str(Path("/nastech")) is platform-dependent (backslash on Windows) —
    # build the expectation the same way instead of hardcoding "/nastech".
    _home_str = str(Path("/nastech"))
    expected_profile = (
...
def test_conversation_start_uses_session_start_not_build_time(monkeypatch):
    ...
    with (
        ...
        patch("nastech_time.now", return_value=datetime(2026, 1, 2, 9, 0)),
    ):
        prompt = build_system_prompt(agent, system_message="SYSTEM_MESSAGE")
...
class TestConversationStartedTwoLine:
    ...
    def test_same_day_session_keeps_single_line(self):
        from nastech_time import now as nastech_now
        sid = nastech_now().strftime("%Y%m%d_%H%M%S_fresh")
