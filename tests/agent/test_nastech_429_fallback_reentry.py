"""Regression guard: a genuine Nastech 429 must re-enter the retry loop so the
top-of-loop Nastech rate-limit guard can activate the fallback chain.
...
``record_nastech_rate_limit()`` moments earlier and either activates a fallback
...
class TestGenuineNastech429ReentersLoop:
