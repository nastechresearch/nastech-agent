...
def test_api_key_rejection_chat_text_names_the_fix_and_the_provider_label():
    ...
    text = result["final_response"]
    assert "nastech setup" in text and "OpenRouter" in text
    assert "Provider said:" in text  # raw detail demoted to a trailing line
    assert text.index("nastech setup") < text.index("Provider said:")
...
def test_oauth_rejection_chat_text_names_the_provider_slug_and_the_failing_profile(tmp_path, monkeypatch):
    ...
    profile_home = tmp_path / ".nastech" / "profiles" / "codex"
    ...
    text = result["final_response"]
    assert "`nastech -p codex auth add openai-codex --type oauth`" in text
    assert "<provider>" not in text
    assert "token_revoked" in text  # the raw error survives for the judge to quote
    # The CLI 💡 hint names the same command; it no longer sends the user to a bare `nastech auth`.
    cli_hint = "\n".join(hints)
    assert "`nastech -p codex auth add openai-codex --type oauth`" in cli_hint, cli_hint
    assert "`nastech auth`" not in cli_hint, cli_hint
...
def test_max_retries_exhausted_chat_text_has_next_step_and_no_mechanism_lead():
    ...
    text = result["final_response"]
    assert "/retry" in text and "/model" in text and "nastech fallback add" in text
...
def test_outer_loop_error_copy_has_no_apology_and_routes_to_gateway_layer():
    ...
    assert "nastech doctor" in text and "/new" in text
...
def test_interpreter_shutdown_copy_substitutes_the_real_session_id():
    ...
    assert "nastech --resume 20260914_abc" in verdict.final_response
...
def test_model_caused_codes_stay_on_the_provider_layer_and_runtime_codes_on_gateway():
    ...
    per-code copy applies); a busy session or loop bug is Nastech-side (gateway layer, so the
