...
class TestBuildAnthropicClient:
    ...
    def test_disables_sdk_retries_for_api_key(self):
        ...
        double-retries inside nastech's outer loop. We delegate retry entirely
...
class TestResolveAnthropicToken:
    ...
    def test_falls_back_to_anthropic_credential_pool_oauth(self, monkeypatch, tmp_path):
        ...
        # returns nothing, mirroring a Nastech-PKCE-only setup.
...
class TestConvertToolsToAnthropicDedup:
    ...
    https://github.com/NastechResearch/nastech-agent/issues/18478
    """
...
def test_oauth_system_prompt_sanitizer_preserves_docs_url():
    ...
    assert "github.com/NastechResearch/nastech-agent" in system_text
    assert "nastech-agent@example.com" in system_text
    assert "skill_view(name='nastech-agent')" in system_text  # a quoted slug is an identifier
