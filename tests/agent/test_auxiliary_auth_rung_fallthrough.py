...
bare ``yield`` inside a ``return`` statement:
    ...
    if _is_auth_error(first_err) and client_is_nastech:
        ...
        if step is not None:
            return (yield step), None          # nastech rung
    ...
...
refreshed client also fails (e.g. an out-of-credit 404 on a stale Nastech runtime token),
...
exercises the real path end to end: a temp ``NASTECH_HOME`` whose ``config.yaml``
declares a ``fallback_chain``, the real client construction and HTTP layer against a
local endpoint, with only the credential sources stubbed (the Nastech portal account
...
class _FakeClient:
    api_key = "sk-test"
    base_url = "https://%s/v1" % NASTECH_HOST
...
def _ladder(base_info=("https://%s/v1" % NASTECH_HOST), resolved_provider="nastech"):
...
def hermetic(monkeypatch):
    ...
    monkeypatch.setattr(aux, "_nastech_portal_account_has_fresh_paid_access", lambda: False)
...
@pytest.mark.parametrize(
    "rung,retry_succeeds",
    [("nastech", False), ("nastech", True), ("provider_credential", False)],
)
def test_post_refresh_retry_owns_the_ladder_outcome(
    ...
    if rung == "nastech":
        monkeypatch.setattr(aux, "_refresh_nastech_auxiliary_client",
                            lambda **kwargs: (_FakeClient(), AUX_MODEL))
        expected_step, expected_base = "call", ("https://%s/v1" % NASTECH_HOST)
    else:
    ...
    ladder = _ladder(
        ...
        resolved_provider="nastech" if rung == "nastech" else "openrouter",
    )
    performed = []
...
@pytest.fixture
def nastech_ladder_endpoint(monkeypatch):
    """A local endpoint that answers the Nastech host, recording every request.

    The routing decision that selects the auth-refresh rung matches on the base URL
    host, so the endpoint is addressed as the real Nastech host and ``getaddrinfo`` is
    ...
    def local_nastech_address(host, *args, **kwargs):
        if host in (NASTECH_HOST, NASTECH_HOST.encode()):
            host = "127.0.0.1"
        return resolve_address(host, *args, **kwargs)

    monkeypatch.setattr(socket, "getaddrinfo", local_nastech_address)
    monkeypatch.setenv("NO_PROXY", "127.0.0.1,localhost,%s" % NASTECH_HOST)
    ...
    thread.start()
    try:
        yield "http://%s:%d" % (NASTECH_HOST, server.server_port), requests
    finally:
        aux.shutdown_cached_clients()
        server.shutdown()
        server.server_close()
...
def test_auth_refresh_retry_failure_reaches_the_configured_chain_over_http(
        tmp_path, monkeypatch, nastech_ladder_endpoint):
    """End to end: the configured chain must serve the retry the refresh could not."""
    host_url, requests = nastech_ladder_endpoint
    ...
    config = {
        "model": {"provider": "nastech", "default": AUX_MODEL},
    ...
    monkeypatch.setattr(aux, "_nastech_portal_account_has_fresh_paid_access", lambda: False)
    monkeypatch.setattr(
        aux, "_resolve_nastech_runtime_api",
        lambda **kwargs: ("fresh-nastech-key", host_url + "/v1"),
    )
