"""``auth.adopt_external_logins: false`` keeps Nastech off the Claude Code login (#113023).

Claude Code's OAuth refresh token is single-use: once Nastech borrows and refreshes it, Nastech and
Claude Code hold one token family and whichever refreshes first logs the other out. With the opt-out
set, Nastech must neither read nor refresh ``~/.claude/.credentials.json``, must drop the pool row an
earlier adopting process persisted, and must say so in ``nastech auth list``.
"""
...
def test_opt_out_never_reads_or_refreshes_claude_code_login(tmp_path, monkeypatch):
    ...
    _write_config(nastech_home, adopt=None)
    ...
    _write_config(nastech_home, adopt=False)
    ...
    _write_config(nastech_home, adopt=True)
