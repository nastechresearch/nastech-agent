...
Prior to this change, aggregator users (OpenRouter / Nastech Portal) had aux
...
class TestResolveVisionMainFirst:
    ...
    @staticmethod
    def _stub_nastech_portal(seen: dict):
        """Stub the Nastech network boundary, keeping the resolution chain real.

        Returns a ``_try_nastech`` replacement that answers with the Portal's
        tier-aware slots: a vision model for ``vision=True``, the text chat
        default otherwise.
        """
        nastech_client = MagicMock()
        nastech_client.api_key = "jwt-test"
        nastech_client.base_url = "https://inference-api.nastechresearch.com/v1"

        def fake_try_nastech(vision=False):
            seen["vision"] = vision
            return nastech_client, (
                "stepfun/step-3.7-flash:free" if vision else "tencent/hy3:free"
            )

        return nastech_client, fake_try_nastech

    def test_nastech_main_vision_uses_portal_pick_not_text_chat_model(self):
        """Nastech main → vision runs the Portal's vision slot, not the chat model.

        A Nastech chat default is routinely text-only (e.g. a ``:free`` chat SKU).
        Letting it reach the vision lane means the image goes to a model that
        cannot accept one and the Portal 404s. Only the Nastech network boundary
        ...
        """
        seen: dict = {}
        nastech_client, fake_try_nastech = self._stub_nastech_portal(seen)

        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="nastech",
        ), patch(
            ...
            "agent.auxiliary_client._try_nastech", side_effect=fake_try_nastech,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        assert provider == "nastech"
        assert client is nastech_client
        assert seen["vision"] is True
        assert model == "stepfun/step-3.7-flash:free"

    def test_nastech_main_vision_honours_explicit_vision_model(self):
        """An explicit auxiliary.vision.model still overrides the Portal pick."""
        seen: dict = {}
        _nastech_client, fake_try_nastech = self._stub_nastech_portal(seen)
    ...
    def test_nastech_explicit_vision_provider_also_skips_chat_model(self):
        """``auxiliary.vision.provider: nastech`` takes the same Portal pick.
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("nastech", None, None, None, None),
        ), patch(
            "agent.auxiliary_client._try_nastech", side_effect=fake_try_nastech,
        ):
            from agent.auxiliary_client import resolve_vision_provider_client
    ...
    def test_nastech_text_aux_still_uses_main_chat_model(self):
        """The vision carve-out must not leak into text aux resolution.

        Text auxiliary work on a Nastech main deliberately keeps the user's chat
        ...
        with patch(
            ...
            _client, model = resolve_provider_client("nastech")
...
class TestResolveVisionCustomProvider:
    ...
    it returns ``(None, None)`` and vision falls through to OpenRouter / Nastech,
