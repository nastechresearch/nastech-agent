...
def test_load_pool_api_key_path_skips_oauth_autodiscovery(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("agent.anthropic_credentials.read_nastech_oauth_credentials", _fake_pkce)
...
def test_load_pool_api_key_path_prunes_stale_oauth_entries(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    monkeypatch.setattr("agent.anthropic_credentials.read_nastech_oauth_credentials", lambda: None)
    monkeypatch.setattr("agent.anthropic_credentials.read_claude_code_credentials", lambda: None)
...
def test_load_pool_oauth_path_still_autodiscovers(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_credentials.read_nastech_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
        ...
        tmp_path,
        {
            ...
            "providers": {
                "nastech": {
                    ...
                    "inference_base_url": "https://inference.nastechresearch.github.io/v1",
                    "token_type": "Bearer",
...
def _disk_entry(tmp_path) -> dict:
    ...
    store = json.loads((tmp_path / "nastech" / "auth.json").read_text())
...
def test_reset_statuses_clears_a_cooldown_that_is_still_binding(tmp_path, monkeypatch):
    ...
    the cooldown was copied straight back. ``nastech auth reset`` printed "Reset
...
def test_reset_statuses_clears_the_classified_failure_reason(tmp_path, monkeypatch):
    ...
    status and no error code but still classified ``billing``. ``nastech auth
