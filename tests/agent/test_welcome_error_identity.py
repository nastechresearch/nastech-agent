...
from tests.nastech_cli.anon_portal import make_jwt
...
def agent_for(api_key, base_url):
    return SimpleNamespace(
        provider="nastech", api_key=api_key, base_url=base_url, model="stepfun/step-3.7-flash:free",
...
@pytest.mark.parametrize("api_key", [make_jwt(account_tier="free", client_id="nastech-cli"), "sk-named"],
...
def test_named_errors_do_not_offer_anonymous_recovery(api_key, base_url, case):
    ...
    classified = classify_api_error(error, provider="nastech", model=agent.model, base_url=base_url, api_key=api_key)
    common = dict(api_kwargs=None, api_messages=[], messages=[], conversation_history=[],
                  api_call_count=1, approx_tokens=10, provider="nastech", base_url=base_url, model=agent.model)
    if status == 403:
    ...
    surface = build_error_surface_from_result(result, provider="nastech", model=agent.model)
    assert surface["code"] == {403: "auth", 429: "rate_limit"}[status]
    assert "without signing in" not in result["final_response"]


def guard_for(agent):
    from agent.turn_api_call import nastech_rate_limit_guard
    return nastech_rate_limit_guard(
...
def test_signing_in_does_not_inherit_anonymous_cooldown(tmp_path, monkeypatch):
    ...
    from agent.nastech_rate_guard import clear_nastech_rate_limit, nastech_rate_limit_remaining, record_nastech_rate_limit
    from agent.turn_recovery import _is_genuine_nastech_rate_limit
    ...
    classified = classify_api_error(error, provider="nastech", base_url=WELCOME, api_key=guest.api_key)
    assert _is_genuine_nastech_rate_limit(guest, error, extract_api_error_context(error), classified)
    ...
    guest.api_key = make_jwt(account_tier="free", client_id="nastech-cli")
    assert guard_for(guest).action == "fallthrough"
    record_nastech_rate_limit(headers={"retry-after": "300"})
    ...
    assert nastech_rate_limit_remaining(anonymous=True) > 0
...
def test_auxiliary_anonymous_cooldown_does_not_outlive_signing_in(tmp_path, monkeypatch):
    ...
    assert aux._try_nastech() == (None, None)
    assert unhealthy and all(ttl <= 60 for ttl in unhealthy)
    runtime[:] = [make_jwt(account_tier="free", client_id="nastech-cli"), NAMED]
    assert aux._try_nastech()[0] is client
...
def test_401_diagnostics_follow_request_identity_on_welcome_host(tier, capsys, monkeypatch):
    ...
    from agent.turn_recovery import _print_nastech_401_diagnostics

    monkeypatch.setattr(loop, "_print_nastech_entitlement_guidance", lambda *a: False)
    _print_nastech_401_diagnostics(agent_for(make_jwt(account_tier=tier), WELCOME), Exception("unauthorized"))
    output = capsys.readouterr().out
    assert ("Nastech couldn't start a new one" in output) == (tier == "anonymous")
    assert ("nastech auth add nastech" in output) == (tier != "anonymous")


def test_anonymous_claim_does_not_classify_other_providers_as_nastech():
    error = Exception("refused")
    error.status_code = 429
...
def test_named_account_on_welcome_host_gets_reconnect_copy_without_signin_card():
    ...
    classified = classify_api_error(error, provider="nastech", model=agent.model, base_url=WELCOME, api_key=agent.api_key)
    result = nonretryable_client_error_result(
        agent, error, classified, status_code=400, api_kwargs=None, api_messages=[], messages=[],
        conversation_history=[], api_call_count=1, approx_tokens=10, provider="nastech", base_url=WELCOME, model=agent.model)
...
def test_escaped_exception_surface_keeps_the_anonymous_verdict():
    ...
    error = Exception("refused")
    error.status_code = 429
    error.body = {"status": 429, "message": "refused", "reason": "model_not_free", "alternates": ["nastech/welcome"]}
    anonymous = build_error_surface_from_exception(error, provider="nastech", model="gpt-5", api_key=make_jwt())
    named = build_error_surface_from_exception(error, provider="nastech", model="gpt-5", api_key=make_jwt(account_tier="free"))
