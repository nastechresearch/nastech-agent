...
The pool quarantines a dead ``openai-codex`` / ``xai-oauth`` / ``nastech`` / ``anthropic`` refresh
token — for the user this is the moment the login is lost, and a debug-only line made it look like
"I signed in once and Nastech keeps failing" (#113023). Two invariants: the WARNING carries the
``nastech auth add <provider>`` hint, and a row the quarantine does not drop (an independent
...
@pytest.mark.parametrize(
    ...
    [
        ("openai-codex", "_is_terminal_codex_oauth_refresh_error", "_sync_entry_from_auth_store",
         "_clear_terminal_tokens_state", "nastech auth add openai-codex"),
        ("nastech", "_is_terminal_nastech_refresh_error", "_sync_nastech_entry_from_auth_store",
         "_clear_terminal_nastech_state", "nastech auth add nastech"),
    ],
)
...
def test_anthropic_dead_grant_warns_and_marks_dead(monkeypatch, caplog):
    ...
    entry = _entry("anthropic", source="manual:nastech_pkce")
    pool._entries = [entry]
    ...
    warnings = [r for r in caplog.records if r.levelno == logging.WARNING and "terminally invalid" in r.getMessage()]
    assert len(warnings) == 1 and "nastech auth add anthropic" in warnings[0].getMessage()
...
@pytest.mark.parametrize(
    ("nastech_state", "expected_code"),
    [
        (None, "nastech_auth_missing"),
        ({"client_id": "nastech-cli", "scope": "inference:invoke", "access_token": _expired_invoke_jwt(),
          "refresh_token": "", "expires_at": "2026-02-01T00:00:00+00:00"}, "nastech_auth_missing_refresh_token"),
    ],
    ids=["not_logged_in", "expired_jwt_without_refresh_token"],
)
def test_nastech_login_missing_refresh_failure_is_terminal(tmp_path, monkeypatch, caplog, nastech_state, expected_code):
    ...
    NASTECH_HOME so the production raise site's code is what reaches the classifier."""
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    providers = {"nastech": nastech_state} if nastech_state else {}
    (tmp_path / "auth.json").write_text(json.dumps({"version": 1, "providers": providers}), encoding="utf-8")
    pool = _pool("nastech")
    entry = _entry("nastech", source="manual:device_code")
    pool._entries = [entry]
    cleared: list = []
    monkeypatch.setattr(pool, "_clear_terminal_nastech_state", lambda e, exc: cleared.append(e.id))
    ...
    assert "nastech auth add nastech" in warnings[0].getMessage()
    row = pool._entries[0]
...
def test_nastech_transient_error_still_benched(monkeypatch, caplog):
    """A failure that says nothing about the login stays transient: benched, not cleared."""
    pool = _pool("nastech")
    entry = _entry("nastech")
    pool._entries = [entry]
    cleared: list = []
    benched: list = []
    monkeypatch.setattr(pool, "_sync_nastech_entry_from_auth_store", lambda e: e)
    monkeypatch.setattr(pool, "_clear_terminal_nastech_state", lambda e, exc: cleared.append(e.id))
