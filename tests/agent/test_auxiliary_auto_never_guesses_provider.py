...
Regression for the Grok/Nastech incident: main provider ``xai-oauth`` with an expired token, Nastech
...
@pytest.fixture
def nastech_is_the_only_working_provider():
    """Main provider unusable; Nastech would win the discovery chain; no configured fallback policy."""
    ...
    with patch.object(aux, "_try_main_provider_route", return_value=None), \
         ...
         patch.object(aux, "_try_nastech", return_value=(MagicMock(name="nastech"), "nastech-model")):
        yield
...
def test_selected_main_provider_down_refuses_to_guess_another_account(
        nastech_is_the_only_working_provider, persisted_provider):
...
def test_quarantined_fallback_hands_over_to_the_next_configured_entry():
    ...
    with patch.object(aux, "_try_configured_fallback_chain", return_value=(healthy, "m2", "fallback_chain[1](nastech)")), \
    ...
    assert client is healthy and label == "fallback_chain[1](nastech)"
    discovery.assert_not_called()


def test_no_selected_main_provider_still_discovers(nastech_is_the_only_working_provider):
    ...
    assert client is not None and (model, label) == ("nastech-model", "nastech")
