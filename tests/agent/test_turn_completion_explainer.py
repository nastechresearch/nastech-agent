...
def test_explanation_persistence_turn_lease_cause_is_specific():
    ...
    assert "nastech doctor" not in lower
...
def test_explanation_persistence_corrupt_backups_dir_follows_nastech_home(monkeypatch, tmp_path):
    """Step 3 must name the backups dir under the ACTIVE home, not ~/.nastech (#104250).

    Pre-update backups live at ``<nastech_root>/backups`` (``nastech_cli/backup.py``), so a
    custom-NASTECH_HOME deployment told to restore from ``~/.nastech/backups/`` is misdirected
    mid data-loss incident: that directory may not exist at all, or may hold an unrelated
    install's backups.
    """
    custom_home = tmp_path / "custom-nastech-home"
    monkeypatch.setenv("NASTECH_HOME", str(custom_home / "profiles" / "research"))
    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "corrupt"
    )
    assert f"{custom_home / 'backups'}" in out
    assert "~/.nastech/backups" not in out
...
def test_deleted_wal_cause_is_plain_first_steps_not_a_forensic_runbook():
    """The WAL-generation runbook lives in the logger.error at nastech_state; the chat reply
    gives the two steps a user can take (stop, doctor) and points at the log."""
    from nastech_state_errors import PERSISTENCE_ERROR_CAUSES

    out = AIAgent._format_turn_completion_explanation(
        "session_persistence_failed", "deleted_wal"
    ).lower()
    assert "deleted_wal" in PERSISTENCE_ERROR_CAUSES
    assert "nastech gateway stop" in out and "nastech doctor" in out
    ...
    assert "~/.nastech" not in out  # display_nastech_home(), never a hardcoded path
...
def test_persistence_commands_are_pinned_to_the_failing_profile(monkeypatch, tmp_path, cause):
    """Every copy-pasteable ``nastech`` command in a persistence explanation names the profile
    whose store failed — a multi-profile backend serves sessions whose state.db is not the
    process default, and a bare ``nastech`` follows the sticky active_profile (#105887). The
    ...
    assert f"`nastech {selector}doctor" in out
    assert "`nastech doctor" not in out and "`nastech gateway" not in out
...
def test_explanation_persistence_one_arg_backward_compat():
    ...
    assert out.strip() != ""
    assert "couldn't save" in out.lower() and "nastech doctor" in out.lower()
...
def test_classify_persistence_error_reuses_disk_full_markers():
    """The disk bucket delegates to nastech_state_errors.is_disk_full_error, so
...
def test_classify_persistence_error_compression_busy_is_distinct():
    ...
    from nastech_state_errors import CompressionSessionBusyError
    from nastech_state import classify_persistence_error
...
def test_classify_persistence_error_turn_lease_lost_is_distinct():
    from nastech_state import classify_persistence_error
    from nastech_state_errors import SessionTurnLeaseLostError
...
def test_classify_persistence_error_fts_provenance_order():
    ...
    from nastech_state import SessionDB, classify_persistence_error
    from nastech_state_errors import SQLITE_CORRUPT_VTAB, is_fts_scoped_corruption_error
...
def test_explainer_config_read_once_then_cached():
    ...
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("NASTECH_TURN_COMPLETION_EXPLAINER", None)
        with patch("nastech_cli.config.load_config", counting_load):
...
def test_classify_persistence_error_quarantined_handle_is_corrupt() -> None:
    ...
    from nastech_state import StateDbCorruptError, classify_persistence_error
