"""Nastech credential hints and the Anthropic token-endpoint error shape (#113023).

A dead Nastech login must be repaired with ``nastech auth add <provider>``; hints that send the user to
an external CLI's login command do not touch Nastech' own credentials. The token endpoint's
...
def test_claude_code_refresher_warns_on_dead_grant(monkeypatch, caplog):
    """The auxiliary Claude Code refresher is a sibling path of the pool: same WARNING, same Nastech hint."""
    ...
    assert len(warnings) == 1 and "nastech auth add anthropic" in warnings[0]
...
def test_anthropic_401_troubleshooting_points_at_nastech_auth(capsys):
    ...
    assert "nastech auth add anthropic" in out and "nastech auth list anthropic" in out
    assert "/login" not in out


def test_no_anthropic_credentials_message_points_at_nastech_auth():
    from nastech_cli.runtime_provider import _NO_ANTHROPIC_CREDENTIALS_MSG

    assert "nastech auth add anthropic" in _NO_ANTHROPIC_CREDENTIALS_MSG
