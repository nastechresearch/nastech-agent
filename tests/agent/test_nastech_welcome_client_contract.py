"""The client side of the Nastech welcome tier's gateway contract: auxiliary calls on the welcome
host use its one model, the ``x-nastech-model-switch`` header moves a session off ``nastech/welcome``,
...
class TestAuxiliaryOnWelcomeHost:
    ...
    def _isolate(self):
        ...
        with patch.object(ac, "_read_nastech_auth", return_value={"auth_method": "anonymous"}), \
             patch.object(ac, "nastech_rate_limit_remaining", return_value=None, create=True):
            yield

    def test_text_aux_on_welcome_host_pins_the_welcome_model(self):
        import agent.auxiliary_client as ac
        with patch.object(ac, "_resolve_nastech_runtime_api", return_value=("jwt", WELCOME)), \
             patch.object(ac, "_create_openai_client", return_value="client") as create, \
             patch("nastech_cli.models.get_nastech_recommended_aux_model") as recommended:
            client, model = ac._try_nastech()
        ...
        assert ac.auxiliary_is_nastech is True
    ...
    def test_vision_aux_on_welcome_host_uses_the_same_model(self):
        ...
        with patch.object(ac, "_resolve_nastech_runtime_api", return_value=("jwt", WELCOME)), \
             ...
             patch("nastech_cli.models.get_nastech_recommended_aux_model") as recommended:
            assert ac._try_nastech(vision=True) == ("client", anon_auth.GUEST_MODEL)
        recommended.assert_not_called()

    def test_paid_host_keeps_the_portal_recommendation(self):
        import agent.auxiliary_client as ac
        with patch.object(ac, "_resolve_nastech_runtime_api", return_value=("jwt", PAID)), \
             patch.object(ac, "_create_openai_client", return_value="client"), \
             patch("nastech_cli.models.get_nastech_recommended_aux_model", return_value="some/free-model"):
            client, model = ac._try_nastech()
...
def _agent(model="nastech/welcome", base_url=PAID):
    return SimpleNamespace(model=model, base_url=base_url, provider="nastech", statuses=[])


class TestModelSwitchHeader:
    def test_header_is_recorded_not_applied_on_the_streaming_response(self):
        agent = _agent()
        assert anon_auth.note_model_switch(agent, {"X-Nastech-Model-Switch": "z-ai/glm-5.3-flash"}) == "z-ai/glm-5.3-flash"
        assert agent.model == "nastech/welcome"
        assert agent._nastech_pending_model_switch == ("nastech/welcome", "z-ai/glm-5.3-flash")

    def test_no_header_records_nothing(self):
        agent = _agent()
        assert anon_auth.note_model_switch(agent, {"content-type": "application/json"}) is None
        assert getattr(agent, "_nastech_pending_model_switch", None) is None

    def test_apply_moves_the_session_and_the_config_default(self, monkeypatch):
        agent = _agent()
        agent._buffer_status = agent.statuses.append
        anon_auth.note_model_switch(agent, {"x-nastech-model-switch": "z-ai/glm-5.3-flash"})
        writes = []
        monkeypatch.setattr("nastech_cli.config.load_config_readonly", lambda: {"model": {"default": "nastech/welcome"}})
        monkeypatch.setattr("nastech_cli.auth._update_config_for_provider",
        ...
        assert agent._nastech_model_switch == ("nastech/welcome", "z-ai/glm-5.3-flash")   # what the gateway cache check reads
        assert writes == [("nastech", PAID, "z-ai/glm-5.3-flash")]
        assert agent._nastech_pending_model_switch is None
    ...
    def test_apply_leaves_a_user_chosen_default_alone(self, monkeypatch):
        ...
        anon_auth.note_model_switch(agent, {"x-nastech-model-switch": "z-ai/glm-5.3-flash"})
        writes = []
        monkeypatch.setattr("nastech_cli.config.load_config_readonly", lambda: {"model": {"default": "openai/gpt-5"}})
        monkeypatch.setattr("nastech_cli.auth._update_config_for_provider",
    ...
    def test_mixin_capture_records_the_header(self):
        ...
        class _Agent(RateLimitCreditsMixin):
            model = "nastech/welcome"
            provider = "nastech"
        a = _Agent()
        a._capture_nastech_model_switch(SimpleNamespace(headers={"x-nastech-model-switch": "z-ai/glm-5.3-flash"}))
        assert a._nastech_pending_model_switch == ("nastech/welcome", "z-ai/glm-5.3-flash")
        a._capture_nastech_model_switch(None)   # fail-open
...
class TestRefusalCopy:
    def test_parse_reads_the_gateway_shape(self):
        body = {"status": 429, "message": "m", "reason": "model_not_free", "retry_after": 0,
                "alternates": ["nastech/welcome"], "upgrade_url": "https://portal.example/upgrade"}
        assert anon_auth.parse_welcome_refusal(body) == {
            "reason": "model_not_free", "retry_after": 0, "alternates": ["nastech/welcome"],
    ...
    def test_copy_names_the_served_model_and_the_sign_in(self):
        refusal = anon_auth.parse_welcome_refusal({"reason": "model_not_free", "alternates": ["nastech/welcome"]})
        chat = anon_auth.welcome_refusal_copy(refusal, model="gpt-5", in_chat=True)
        assert "gpt-5" in chat and "nastech/welcome" in chat and chat.endswith(anon_auth._SIGNIN_CHAT)
        ...
        assert "`nastech auth upgrade`" in terminal and "/login" not in terminal
...
class TestTurnRecoveryGuidance:
    def test_guidance_from_classified_context(self):
        ...
        assert "nastech/welcome" in _welcome_tier_guidance(classified, model="gpt-5", in_chat=True)
