"""Nastech free tier, inference side: the dark-tier 403 keyed on the route, the one-shot model move
...
def _classify(err: MockAPIError, *, model: str = "nastech/welcome", base_url: str = WELCOME):
    return classify_api_error(err, provider="nastech", model=model, base_url=base_url, api_key=make_jwt())
...
def _agent(**overrides):
    ...
    agent = SimpleNamespace(
        provider="nastech", api_key=make_jwt(), model="gpt-5", base_url=WELCOME, log_prefix="", _rate_limit_state=None,
        _vprint=lambda text, force=False, diagnostic=False: lines.append(text),
        _try_refresh_nastech_client_credentials=lambda **kw: True,
    )
...
class TestOneShotRecoveries:
    def test_model_not_free_moves_the_session_onto_the_alternate_and_retries_once(self):
        ...
        agent = _agent()
        classified = _classify(_refusal("model_not_free", alternates=["nastech/welcome"]), model="gpt-5")
        retry = TurnRetryState()
        assert _recover_welcome_tier(agent, classified, retry) is True
        assert agent.model == "nastech/welcome"
        assert agent._nastech_model_switch == ("gpt-5", "nastech/welcome")
    ...
    def test_a_wrong_host_refusal_re_reads_the_route_once(self):
        from agent.turn_recovery import _recover_welcome_tier
        calls = []
        agent = _agent(_try_refresh_nastech_client_credentials=lambda **kw: calls.append(kw) or True)
        body = {"status": 400, "message": "Anonymous accounts must use https://welcome-api.nastechresearch.github.io for inference."}
    ...
    def test_a_named_account_on_the_welcome_host_re_reads_the_route_once(self):
        from agent.turn_recovery import _recover_welcome_tier
        calls = []
        agent = _agent(api_key=make_jwt(account_tier="free", client_id="nastech-cli"),
                       _try_refresh_nastech_client_credentials=lambda **kw: calls.append(kw) or True)
        body = {"status": 400, "message": "This endpoint serves anonymous Nastech Agent accounts only. Use https://inference-api.nastechresearch.github.io with your API key or signed-in account."}
        classified = classify_api_error(_gateway_error(400, body), provider="nastech", base_url=WELCOME, api_key=agent.api_key)
    ...
    def test_a_wrong_host_refusal_whose_heal_fails_falls_through(self):
        from agent.turn_recovery import _recover_welcome_tier
        agent = _agent(_try_refresh_nastech_client_credentials=lambda **kw: False)
        body = {"status": 400, "message": "Anonymous accounts must use https://welcome-api.nastechresearch.github.io for inference."}
...
class TestLongWaitRule:
    ...
    def test_only_a_long_rate_limited_refusal_is_an_exhausted_allowance(self, reason, retry_after, expected):
        from agent.nastech_rate_guard import is_long_welcome_rate_limit
    ...
    def test_the_turn_records_a_long_refusal_from_the_classifiers_context(self, monkeypatch):
        ...
        import agent.nastech_rate_guard as guard
        from agent.turn_recovery import _is_genuine_nastech_rate_limit
        recorded = []
        monkeypatch.setattr(guard, "record_nastech_rate_limit", lambda **kw: recorded.append(kw))
        ...
        assert _is_genuine_nastech_rate_limit(_agent(), err, turn_ctx, classified) is True
        ...
        recorded.clear()
        assert _is_genuine_nastech_rate_limit(_agent(base_url=PAID, api_key=make_jwt(account_tier="paid")), err, turn_ctx, classified) is False
        assert recorded == []
        ...
        assert _is_genuine_nastech_rate_limit(_agent(), short, extract_api_error_context(short), _classify(short)) is False
        assert recorded == []
...
class TestOutageCopy:
    ...
    def test_a_spent_transport_failure_on_the_welcome_host_reads_as_one_sentence(self, reason):
        ...
        from nastech_cli.anon_auth import FREE_TIER_OUTAGE_COPY
...
class TestTerminalResultsCarryTheFreeTierBlock:
    ...
    def test_a_dark_tier_403_is_stamped_disabled_with_the_chat_sentence(self):
        ...
        result = nonretryable_client_error_result(
            ...
            provider="nastech", base_url=WELCOME, model="nastech/welcome")
    ...
    def test_an_exhausted_capacity_refusal_is_stamped_at_capacity(self):
        ...
        result = max_retries_exhausted_result(
            ...
            approx_tokens=10, provider="nastech", base_url=WELCOME, model="nastech/welcome")
    ...
    def test_the_same_outage_on_the_paid_host_is_not_stamped(self):
        ...
        result = max_retries_exhausted_result(
            ...
            api_call_count=3, approx_tokens=10, provider="nastech", base_url=PAID, model="nastech-4")
