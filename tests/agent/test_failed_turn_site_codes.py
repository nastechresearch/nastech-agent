...
def test_persistence_failure_default_copy_is_actionable_and_profile_aware(monkeypatch):
    monkeypatch.setenv("NASTECH_HOME", "/srv/nastech-profile")
    text = TurnExplainersMixin._format_turn_completion_explanation("session_persistence_failed", "replaced")
    assert "nastech gateway stop" in text and "nastech doctor" in text
    assert "~/.nastech" not in text and "/srv/nastech-profile" in text
    assert "manifest" not in text  # the runbook stays in logger.error at nastech_state
