...
class TestClassifyApiError:
        ...
        e = MockAPIError(
            ...
            body={
                ...
                "message": (
                    ...
                    "Upgrade at https://portal.nastechresearch.github.io or pick a free model."
                ),
            },
        )
        ...
        e = MockAPIError(
            ...
            body={
                ...
                "message": (
                    ...
                    "add credits at https://portal.nastechresearch.github.io or pick a free model."
                ),
            },
        )
        result = classify_api_error(e, provider="nastech", model="openai/gpt-5.5-pro")
        assert result.reason == FailoverReason.billing
        assert result.retryable is False
        assert result.should_fallback is True
    ...
    def test_reasoning_mandatory_400_is_retryable_not_format_error(self):
        ...
        result = classify_api_error(e, provider="nastech", model="z-ai/glm-5.3-flash")
...
class TestServerInjectedParameterRejection:
    ...
    so an identical request succeeds on retry ~80% of the time.  Nastech never
    ...
    def test_retention_rejection_from_meta_host_stays_non_retryable(self):
        """Boundary: on api.meta.ai / Bedrock Mantle Nastech DOES send
...
# ── Test: Nastech welcome tier (free tier) refusals ───────────────────────

class TestNastechWelcomeTier:
    """The Nastech gateway's welcome-tier contract: a structured 429 body carries ``reason`` /
    ...
    def test_model_not_free_is_a_non_retryable_gate_with_fallback(self):
        err = self._refusal("model_not_free", alternates=["nastech/welcome"], upgrade_url="https://portal.example/upgrade")
        result = classify_api_error(err, provider="nastech", api_key=make_jwt(), model="gpt-5")
        assert result.reason == FailoverReason.model_not_found
        assert result.retryable is False
        assert result.should_fallback is True
        assert result.should_rotate_credential is False
        ...
        assert refusal["alternates"] == ["nastech/welcome"]
        assert refusal["upgrade_url"] == "https://portal.example/upgrade"

    def test_feature_not_free_is_the_same_gate(self):
        result = classify_api_error(self._refusal("feature_not_free"), provider="nastech", api_key=make_jwt())
        assert result.reason == FailoverReason.model_not_found
        assert result.retryable is False
    ...
    def test_capacity_refusals_are_rate_limits_that_honour_retry_after(self, reason):
        result = classify_api_error(self._refusal(reason, retry_after=30), provider="nastech", api_key=make_jwt(), model="nastech/welcome")
        assert result.reason == FailoverReason.rate_limit
        assert result.retryable is True
        assert result.should_fallback is True
    ...
    def test_retry_after_zero_carries_no_reset(self):
        result = classify_api_error(self._refusal("at_capacity", retry_after=0), provider="nastech", api_key=make_jwt())
    ...
    def test_unknown_reason_is_not_the_welcome_shape(self):
        ...
        result = classify_api_error(err, provider="nastech", api_key=make_jwt())
    ...
    def test_anonymous_jwt_on_the_paid_host_is_deterministic(self):
        ...
        result = classify_api_error(err, provider="nastech", api_key=make_jwt(), model="nastech/welcome")
        assert result.reason == FailoverReason.format_error
    ...
    def test_named_caller_on_the_welcome_host_is_deterministic(self):
        ...
        err = MockAPIError(f"Error code: 400 - {body}", status_code=400, body=body)
        result = classify_api_error(err, provider="nastech", api_key=make_jwt(account_tier="free"))
    ...
    def test_ordinary_403_is_untouched(self):
        result = classify_api_error(MockAPIError("forbidden", status_code=403, body={"message": "forbidden"}), provider="nastech", api_key=make_jwt())
        assert result.reason == FailoverReason.auth
