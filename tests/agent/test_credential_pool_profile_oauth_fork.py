...
Real imports, real temp NASTECH_HOME root + named profile, real auth.json I/O.
...
def fleet(tmp_path, monkeypatch):
    """Root NASTECH_HOME with an expired-but-refreshable Anthropic pool row."""
    ...
    # against $HOME/.nastech/auth.json; our root is elsewhere, so writes go.
    import nastech_constants
    nastech_constants._default_nastech_root_memo = None  # type: ignore[attr-defined]
    ...
    store = {
        ...
        "credential_pool": {
            "anthropic": [{
                ...
                "priority": 0, "source": "manual:nastech_pkce",
...
def test_strip_helper_drops_device_code_blocks_and_reports(tmp_path):
    ...
    (pdir / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"openai-codex": {"access_token": "a", "refresh_token": "r"}, "nastech": {"agent_key": "k"}},
        "credential_pool": {
    ...
    assert "openai-codex" not in store["providers"] and "nastech" in store["providers"]
...
def test_borrower_prune_never_deletes_root_singleton_grant(fleet, tmp_path):
    """Root's nastech_pkce row is seeded from ROOT's .anthropic_oauth.json; a
    ...
    assert [e.source for e in root_rows] == ["nastech_pkce"]

    kid = _profile(fleet, "kid")
    fleet["use"](kid)
    pool = load_pool("anthropic")
    assert [e.source for e in pool.entries()] == ["nastech_pkce"], "borrowed root grant was pruned"
    assert fleet["rows"](root) and fleet["rows"](root)[0]["source"] == "nastech_pkce"
...
def test_profile_auth_add_owns_only_its_own_rows(fleet):
    ...
    pool.add_entry(PooledCredential(
        ...
        priority=0, source="manual:nastech_pkce", access_token="sk-ant-oat01-MINE",
...
def test_heal_consolidates_existing_forks_to_the_live_copy(fleet, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="nastech_cli.auth"):
        fleet["use"](forge)
        sel = load_pool("anthropic").select()
...
def test_heal_is_idempotent_and_logs_once(fleet, caplog):
    import logging
    from agent.credential_pool import load_pool
    from nastech_cli.auth import consume_oauth_heal_notices, heal_forked_single_use_oauth_grants
...
def test_heal_never_deletes_the_only_surviving_copy(fleet):
    """Root lost its grant (user ran `nastech auth remove` at root); the profile's
...
def test_heal_pkce_singleton_shape_commits_live_pair_to_root_singleton(fleet):
    """`nastech auth` PKCE shape: root + profile each have .anthropic_oauth.json +
    a nastech_pkce-seeded row; the profile's copy is the rotated (live) one."""
    from agent.credential_pool import load_pool
    ...
    fleet["use"](root)
    load_pool("anthropic")  # seeds root's nastech_pkce row from the singleton
...
def test_heal_is_a_noop_in_classic_mode(fleet):
    from nastech_cli.auth import heal_forked_single_use_oauth_grants
    fleet["use"](fleet["root"])
...
def test_heal_skips_profile_auth_json_symlinked_to_the_root_store(fleet):
    """#101356: `ln -s ~/.nastech/auth.json <profile>/auth.json` shares ONE store.
