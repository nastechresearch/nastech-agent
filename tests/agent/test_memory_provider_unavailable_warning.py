"""Regression tests for NastechResearch/nastech-agent#2765.
...
service that didn't inherit ``~/.nastech/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "nastech memory status" in msg
