"""Nastech agent keys live ~1 h. Every agent in a process must not learn about expiry from its own 401.
...
def test_key_far_from_expiry_is_left_alone_without_touching_the_store():
    ...
    with patch("nastech_cli.auth.resolve_nastech_runtime_credentials", side_effect=AssertionError("must not hit the store")):
        assert agent._adopt_nastech_key_before_expiry() is False
...
def test_key_inside_the_skew_adopts_the_stores_fresh_key_without_forcing_a_refresh():
    ...
    with patch("nastech_cli.auth.resolve_nastech_runtime_credentials", side_effect=resolve):
        assert agent._adopt_nastech_key_before_expiry() is True
    assert calls[0]["force_refresh"] is False  # the keepalive/peer refresh is adopted, never re-minted
    assert agent.adopted == [(fresh, "nastech_credential_refresh")]
...
def test_a_fresh_key_for_a_different_account_is_never_adopted():
    ...
    with patch("nastech_cli.auth.resolve_nastech_runtime_credentials", return_value={"api_key": other, "base_url": agent.base_url}):
        assert agent._adopt_nastech_key_before_expiry() is False
...
def test_same_key_back_from_the_store_is_not_readopted():
    ...
    agent = _Agent(key)
    with patch("nastech_cli.auth.resolve_nastech_runtime_credentials", return_value={"api_key": key, "base_url": agent.base_url}):
        assert agent._adopt_nastech_key_before_expiry() is False
    assert agent.adopted == []


def test_keepalive_thread_starts_when_an_agent_routes_to_nastech(monkeypatch, tmp_path):
