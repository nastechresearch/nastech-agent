...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.nastech/.env")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.nastech/config.yaml", "api_key: nastechConfigSecret123", "nastechConfigSecret123"),
            (
                "head ~/.nastech/profiles/work/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.nastech/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.nastech/config.example.yaml",
            "cat ~/.nastech/config.template.yaml",
            "cat ~/.bashrc.example",
...
class TestNastechHomePathClassification:
    """``_is_secret_file_arg`` must see the RESOLVED Nastech home: a managed Windows home
    (``%LOCALAPPDATA%\\nastech``) has no ``.nastech`` segment and a resolved path never spells
    ``$NASTECH_HOME``, so the literal test alone classified its ``config.yaml`` as ordinary YAML."""
    ...
    def test_resolved_home_config_is_secret_bearing_but_project_config_is_not(self, tmp_path, monkeypatch):
        ...
        home = tmp_path / "nastech"  # no ".nastech" segment
