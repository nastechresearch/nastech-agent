...
def _native_wire_selected(monkeypatch):
    """These contracts describe the native wire, which is now opt-in (``nastech.anthropic_wire:
    native``; default ``chat`` since 2026-09-06, see ``nastech_api_mode``). Select it here so the
