"""A Nastech 401 refresh must replace the auxiliary client under the SAME cache key
...
def _nastech_mock_client(*, async_mode, raises=None, returns=None):
    """A stand-in OpenAI client whose ``chat.completions.create`` 401s or returns."""
    client = MagicMock()
    client.base_url = NASTECH_BASE_URL
...
def test_call_llm_auto_provider_evicts_stale_client_end_to_end(monkeypatch):
    ...
    """
    task = "compression"
    stale = _nastech_mock_client(async_mode=False, raises=_Auth401("stale creds"))
    fresh = _nastech_mock_client(async_mode=False, returns={"ok": True})
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: (stale, "nastech-model"),
    )
    # The 401 refresh rebuilds a fresh client from refreshed runtime creds.
    monkeypatch.setattr(
        ac, "_resolve_nastech_runtime_api",
        lambda *, force_refresh=False, stale_access_token=None: ("fresh-key", NASTECH_BASE_URL),
    )
    monkeypatch.setattr(
...
async def test_async_call_llm_auto_provider_evicts_stale_client_end_to_end(monkeypatch):
    ...
    """
    task = "session_search"
    main_runtime = {"provider": "nastech", "model": "Nastech-4-405B"}
    stale = _nastech_mock_client(async_mode=True, raises=_Auth401("stale creds"))
    fresh = _nastech_mock_client(async_mode=True, returns={"ok": True})
    ...
    monkeypatch.setattr(ac, "_to_async_client", lambda *a, **k: (fresh, "nastech-model"))
    monkeypatch.setattr(ac, "_validate_llm_response", lambda resp, _task: resp)
