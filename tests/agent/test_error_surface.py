...
def test_auth_surface_names_oauth_vs_api_key_recovery():
    ...
    oauth = build_error_surface_from_result(_failed_result("auth"), provider="nastech")
    assert oauth["auth_kind"] == "oauth"
    assert oauth["provider_label"] == "Nastech Portal"
    ...
    assert "auth_kind" not in build_error_surface_from_result(_failed_result("rate_limit"), provider="nastech")


def test_result_billing_block_wins():
    surface = build_error_surface_from_result(
        _failed_result("rate_limit", billing_block={"provider": "nastech"})
    )
...
def test_free_tier_block_gets_its_own_code_and_carries_the_sentence():
    ...
    result = _failed_result("auth_permanent", error="HTTP 403: no permissions",
                            free_tier={"kind": "disabled", "message": "Using Nastech without signing in is switched off."})
    surface = build_error_surface_from_result(result, provider="nastech", model="nastech/welcome")
    ...
    assert surface["message"] == "Using Nastech without signing in is switched off."
...
def test_free_tier_kinds_say_whether_a_later_send_can_succeed(kind, retryable):
    surface = build_error_surface_from_result(_failed_result("rate_limit", free_tier={"kind": kind}), provider="nastech")
...
def test_a_free_tier_block_without_a_kind_is_ignored():
    surface = build_error_surface_from_result(_failed_result("auth_permanent", free_tier={}), provider="nastech")
