"""Concurrent Nastech 401 recovery must not stampede the shared OAuth grant.

Sep 2 2026 incident: ~120 subagent processes shared one Nastech OAuth pool entry
...
lost the auth-store flock race had their only entry benched ("matched no nastech
entry ... pool size 0") and surfaced the 401 to the user as "out of funds".

Two invariants pinned here:

1. ``resolve_nastech_runtime_credentials(force_refresh=True, stale_access_token=X)``
   does NOT POST a refresh when the store already holds a usable token that
   is not X — a peer already rotated; adopt it.
2. A lock-timeout during a pool-level Nastech refresh leaves the entry
...
def test_lock_timeout_during_nastech_refresh_does_not_bench_entry(monkeypatch, caplog):
    ...
    monkeypatch.setattr(auth_mod, "resolve_nastech_runtime_credentials", _busy)
    monkeypatch.setattr(auth_nastech, "resolve_nastech_runtime_credentials", _busy)
...
def test_agent_401_refresh_passes_failed_bearer_as_stale_hint(monkeypatch):
    ...
    monkeypatch.setattr(auth_mod, "resolve_nastech_runtime_credentials", _fake_resolve)
    monkeypatch.setattr(auth_nastech, "resolve_nastech_runtime_credentials", _fake_resolve)

    assert agent._try_refresh_nastech_client_credentials(force=True) is True
