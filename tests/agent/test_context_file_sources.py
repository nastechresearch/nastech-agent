...
def test_manifest_matches_what_the_prompt_actually_loads(project, tmp_path_factory):
    """Every context type present at once; the ladder picks .nastech.md, the chain lists both AGENTS files,
    CLAUDE.md/.cursorrules/.cursor/rules/*.mdc are shadowed, an empty file never wins, SOUL.md rides along."""
    (project / ".nastech.md").write_text("nastech rules")
    ...
    assert statuses == {
        ".nastech.md": "loaded", "../AGENTS.md": "shadowed", "AGENTS.override.md": "empty", "AGENTS.md": "shadowed",
    ...
    # Same walk, other winner: drop .nastech.md and the whole AGENTS chain loads while the rest stays shadowed.
    (project / ".nastech.md").unlink()
...
def test_truncated_and_suppressed_statuses_follow_the_builder(project, monkeypatch, tmp_path_factory):
    ...
    # Install-tree guard: a fallback cwd (cwd=None) inside the Nastech tree lists the file but never loads it.
