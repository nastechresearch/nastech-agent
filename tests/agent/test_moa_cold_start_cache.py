...
def test_slot_runtime_cache_expires_after_ttl(monkeypatch):
    ...
    key = (nastech_home_key(), "openai", "gpt-5")
...
def test_slot_runtime_cache_is_scoped_per_profile_home(monkeypatch, tmp_path):
    ...
    accounts; a cached api_key/base_url must never cross the per-turn NASTECH_HOME override."""
    import agent.moa_loop as moa
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: {"provider": "openai", "model": "gpt-5",
                      "api_key": f"key-{nastech_constants.get_nastech_home().name}",
                      "base_url": "https://x", "api_mode": None})

    slot = {"provider": "openai", "model": "gpt-5"}
    tok = nastech_constants.set_nastech_home_override(str(a))
    try:
        assert moa._slot_runtime(slot)["api_key"] == "key-a"
    finally:
        nastech_constants.reset_nastech_home_override(tok)
    tok = nastech_constants.set_nastech_home_override(str(b))
    try:
