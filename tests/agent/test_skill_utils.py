...
class TestParseConfigStringList:
    """#86661: `nastech config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `nastech config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    nastech_home = tmp_path / "da\\ta"
    subprocess_home = nastech_home / "home"
    subprocess_home.mkdir(parents=True)
    (nastech_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.setenv("HOME", str(nastech_home))
