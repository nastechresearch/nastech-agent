...
def test_entitlement_guidance_is_classified_at_direct_print(tmp_path, monkeypatch, suppress):
    ...
    (tmp_path / "config.yaml").write_text(yaml.safe_dump(
        {} if suppress is None else {"display": {"suppress_warning_notifications": suppress}}))
    monkeypatch.setattr(conversation_loop, "_nastech_entitlement_message", lambda capability: "entitlement detail\nnext step")
    agent = Emitter()
    printed = []
    agent._print_fn = lambda *a, **k: printed.append(a)
    assert conversation_loop._print_nastech_entitlement_guidance(agent, "model access") is True
