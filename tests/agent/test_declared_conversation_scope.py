...
conversation-affinity hint Nastech sends — ``prompt_cache_key`` on both
OpenAI-wire transports, the OpenRouter/Nastech sticky ``session_id``, and xAI's
``x-grok-conv-id`` — so the conversation never lands back on the routing
bucket it warmed. Nastech cannot infer the logical conversation from the id's
syntax (#79017's failure class), but it does not have to: the host declares
it through ``gateway_session_key`` (the ``X-Nastech-Session-Key`` /
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Nastech-internal token; a session KEY embeds the
...
class TestProviderStickyKeys:
    """OpenRouter / Nastech sticky ids and x-grok-conv-id read the same scope."""
    ...
    def test_nastech_sticky_key_follows_the_declared_scope(self):
        from providers import get_provider_profile

        scope = declared_conversation_scope(_agent(RUN_1, None, CHAT_KEY))
        token = set_affinity_scope(scope)
        try:
            body = get_provider_profile("nastech").build_extra_body(session_id=RUN_1)
            body_next = get_provider_profile("nastech").build_extra_body(session_id=RUN_2)
        finally:
            reset_affinity_scope(token)
...
class TestPeerIdentityIsSourceQualified:
    ...
    ``X-Nastech-Session-Key`` accepts any authenticated caller-supplied string,
...
class TestSourceOverrideDomain:
    ...
    ``NASTECH_SESSION_SOURCE``. Under an override both sides of a ``/new``
    ...
    def test_the_pre_row_source_matches_persistence(self, db, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_SESSION_SOURCE", "override-src")
