"""``nastech.anthropic_wire: auto`` decides a session's wire once, from its first response.
...
- one decision per session, only on call 1, only for nastech + anthropic/*, never for chat/native;
...
from agent import nastech_wire
...
class TestClassifier:
    def test_openrouter_by_provider_field_or_gen_id(self):
        assert nastech_wire.classify_upstream(_resp(provider="Anthropic", id="gen-1788708403-xXjDsFw")) == "openrouter"
        assert nastech_wire.classify_upstream(_resp(provider="Claude Platform on AWS", id="x")) == "openrouter"
        # native wire: no provider field, but the id is still OpenRouter-minted
        assert nastech_wire.classify_upstream(_resp(id="gen-1788680469-hyks5Nj9n7rmdAbMyQ5x")) == "openrouter"

    def test_gmi_by_anthropic_native_id_without_provider(self):
        assert nastech_wire.classify_upstream(_resp(id="msg_01XrffvmxsRUWwCbVpBsADHo")) == "gmi"
    ...
    def test_unknown_never_classifies(self, r):
        assert nastech_wire.classify_upstream(r) is None


class TestWireChoice:
    def test_chat_for_openrouter_and_unknown(self):
        assert nastech_wire.wire_for_upstream("openrouter") == "chat_completions"
        assert nastech_wire.wire_for_upstream(None) == "chat_completions"

    def test_gmi_is_chat_until_cleared_then_native(self, monkeypatch):
        monkeypatch.setattr(nastech_wire, "GMI_NATIVE_WIRE_CLEARED", False)
        assert nastech_wire.wire_for_upstream("gmi") == "chat_completions"
        monkeypatch.setattr(nastech_wire, "GMI_NATIVE_WIRE_CLEARED", True)
        assert nastech_wire.wire_for_upstream("gmi") == "anthropic_messages"
...
class TestHook:
    ...
    def _auto(self, monkeypatch):
        monkeypatch.setattr(_providers, "_nastech_anthropic_wire", lambda: "auto")
        self.switches = []
    ...
    def test_gmi_cleared_schedules_once_and_applies_at_next_iteration(self, monkeypatch):
        monkeypatch.setattr(nastech_wire, "GMI_NATIVE_WIRE_CLEARED", True)
        a = _agent()
        assert nastech_wire.maybe_switch_wire_after_first_response(a, _resp(id="msg_01abc"), 1) is True
        ...
        assert nastech_wire.apply_pending_wire_switch(a) is True
        assert a.api_mode == "anthropic_messages" and self.switches == ["anthropic_messages"]
        assert nastech_wire.apply_pending_wire_switch(a) is False  # nothing pending twice
        # a later call, even with a different-looking response, never flips again
        assert nastech_wire.maybe_switch_wire_after_first_response(a, _resp(provider="Anthropic", id="gen-1-x"), 2) is False
        assert nastech_wire.maybe_switch_wire_after_first_response(a, _resp(provider="Anthropic", id="gen-1-x"), 1) is False
    ...
    def test_gmi_uncleared_stays_on_chat(self, monkeypatch):
        monkeypatch.setattr(nastech_wire, "GMI_NATIVE_WIRE_CLEARED", False)
        a = _agent()
        assert nastech_wire.maybe_switch_wire_after_first_response(a, _resp(id="msg_01abc"), 1) is False
        assert a.api_mode == "chat_completions" and self.switches == []

    @pytest.mark.parametrize("mode", ["chat", "native"])
    def test_explicit_modes_never_auto_switch(self, monkeypatch, mode):
        monkeypatch.setattr(_providers, "_nastech_anthropic_wire", lambda: mode)
        monkeypatch.setattr(nastech_wire, "GMI_NATIVE_WIRE_CLEARED", True)
    ...
    def test_other_providers_and_models_untouched(self, monkeypatch):
        monkeypatch.setattr(nastech_wire, "GMI_NATIVE_WIRE_CLEARED", True)
        assert nastech_wire.maybe_switch_wire_after_first_response(_agent(provider="openrouter"), _resp(id="msg_01abc"), 1) is False
        assert nastech_wire.maybe_switch_wire_after_first_response(_agent(model="openai/gpt-5.6-sol"), _resp(id="msg_01abc"), 1) is False
        assert self.switches == []
    ...
    def test_switch_failure_is_swallowed_and_decision_is_final(self, monkeypatch):
        ...
        a = _agent()
        assert nastech_wire.maybe_switch_wire_after_first_response(a, _resp(id="msg_01abc"), 1) is True
        assert nastech_wire.apply_pending_wire_switch(a) is False
        assert a.api_mode == "chat_completions" and a._nastech_wire_decided is True and a._nastech_wire_pending is None
...
def test_real_agent_usage_recorder_calls_the_hook_once(tmp_path, monkeypatch):
    ...
    (tmp_path / "config.yaml").write_text("nastech:\n  anthropic_wire: auto\n", encoding="utf-8")
