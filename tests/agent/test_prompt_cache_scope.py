...
class TestPerResponseRunNonceIsolation:
    ...
    Nastech Studio group chat builds ``gc_run_<room>_<profile>_<name>_<uuid4hex>``
    for every reply and destroys it when the reply completes
    (``groupRuntimeSessionId``), so every conversation-affinity hint Nastech
    ...
    is an identity, and Nastech' public session API lets a client choose one
    ...
    def test_parentless_rows_resolve_to_their_own_scope(self, db):
        ...
        an owner Nastech was never told about must never be guessed. #96811
    ...
    @pytest.mark.parametrize("profile_name", ["openrouter", "nastech"])
    def test_distinct_ids_keep_distinct_provider_sticky_keys(self, profile_name):
        """OpenRouter/Nastech route by this key, and it isolates distinct ids.
