"""Secret stores under NASTECH_HOME are write-denied; control files stay writable (#110464).
...
@pytest.fixture()
def nastech_layout(tmp_path, monkeypatch):
    """Profile NASTECH_HOME plus a distinct global root, both patched."""
...
def test_read_denied_secret_stores_are_write_denied_on_profile_and_root(nastech_layout):
    root, profile = nastech_layout
...
def test_control_files_and_lookalikes_outside_home_stay_writable(nastech_layout, tmp_path):
    root, profile = nastech_layout
    for base in (profile, root):
...
class TestProfileHomeProcessHome:
    """With the process HOME pinned to ``{NASTECH_HOME}/home`` (TERMINAL_HOME_MODE=profile,
    ...
    def profile_home_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_nastech_home_path", lambda: profile)
        monkeypatch.setattr(fs, "_nastech_root_path", lambda: profile.parent)
        return profile
