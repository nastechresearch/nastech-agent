...
* ``urllib``-based ``/models`` catalog discovery probe
  (``nastech_cli.models.probe_api_models`` via ``_custom_provider_ssl_context``).
...
from nastech_cli.models import _custom_provider_ssl_context
...
class TestResolveRequestsVerifyProviderScoped:
    ...
    def test_no_base_url_does_not_consult_config(self, clean_env, bundle_file):
        ...
        with patch("nastech_cli.config.get_compatible_custom_providers", probe):
