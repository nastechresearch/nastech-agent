...
The official Codex endpoint must receive Nastech' own harness identity, rather
...
These tests pin:
- the required Nastech originator
- the versioned Nastech User-Agent
...
class TestCodexCloudflareHeaders:
    ...
    def test_user_agent_advertises_nastech_version(self):
...
class TestPrimaryClientWiring:
    ...
    def test_apply_client_headers_on_base_url_change(self):
        ...
        with patch("run_agent.OpenAI") as mock_openai:
            ...
            assert headers.get("originator") == "nastech-agent"
            assert headers.get("ChatGPT-Account-ID") == "acct-rotation"
            assert headers.get("User-Agent") == f"NastechAgent/{__version__}"
