...
class TestSharedPromptPath:
    ...
    def test_gates_suppress_auto_load(self, tmp_path, monkeypatch):
        """NASTECH_IGNORE_RULES, skip_context_files (delegate children / internal forks) and a session without
        the skills toolset all keep pinned skills out of the prompt."""
        _write_skill(tmp_path, "stable-skill", "ORIGINAL SKILL BYTES")
        cfg = {"skills": {"auto_load": ["stable-skill"]}}
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path), \
             patch("nastech_cli.config.load_config_readonly", return_value=cfg):
            monkeypatch.setenv("NASTECH_IGNORE_RULES", "true")
            agent = _bare_agent()
            ...
            monkeypatch.delenv("NASTECH_IGNORE_RULES")
