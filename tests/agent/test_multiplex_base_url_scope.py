...
them (OPENAI_BASE_URL, XAI_BASE_URL, NASTECH_INFERENCE_BASE_URL) and the gateway proxy URL / browser
...
def secondary_scope(monkeypatch):
    for name in ("OPENAI_BASE_URL", "XAI_BASE_URL", "NASTECH_XAI_BASE_URL", "NASTECH_INFERENCE_BASE_URL",
...
def test_unscoped_single_profile_reads_keep_environ(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setenv("NASTECH_INFERENCE_BASE_URL", "https://nastech.mine.example/v1/")
        assert GatewayTurnMixin._get_proxy_url(GatewayTurnMixin.__new__(GatewayTurnMixin)) == "https://proxy.mine.example"
        assert auth_nastech._nastech_inference_env_override() == "https://nastech.mine.example/v1"
    finally:
        secret_scope.reset_secret_scope(token)
