...
def test_single_profile_scoped_load_keeps_override_behavior(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_TEST_SHARED_ADAPTER_CONFIG", raising=False)
    other_home = tmp_path / "other"
    other_home.mkdir()
    (other_home / ".env").write_text("NASTECH_TEST_SHARED_ADAPTER_CONFIG=second\n")

    was_active = secret_scope.is_multiplex_active()
    secret_scope.set_multiplex_active(False)
    home_token = set_nastech_home_override(other_home)
    try:
        loaded = env_loader.load_nastech_dotenv(nastech_home=other_home)
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_nastech_home_override(home_token)

    try:
        assert os.environ.get("NASTECH_TEST_SHARED_ADAPTER_CONFIG") == "second"
        assert (other_home / ".env") in loaded
    finally:
        os.environ.pop("NASTECH_TEST_SHARED_ADAPTER_CONFIG", None)
...
def test_multiplex_dotenv_load_hydrates_sources_without_global_env(
    ...
    was_active = secret_scope.is_multiplex_active()
    home_token = set_nastech_home_override(tmp_path)
    secret_scope.set_multiplex_active(True)
    try:
        assert env_loader.load_nastech_dotenv(nastech_home=tmp_path) == []
    finally:
        secret_scope.set_multiplex_active(was_active)
        reset_nastech_home_override(home_token)
