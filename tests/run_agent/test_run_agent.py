...
class TestBuildSystemPrompt:
    ...
    def test_includes_nastech_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_nastech_subscription_prompt", lambda tool_names: "NASTECH SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "NASTECH SUBSCRIPTION BLOCK" in prompt
...
class TestConcurrentToolExecution:
    ...
    def test_sequential_registry_tool_forwards_request_middleware_trace(
        ...
        monkeypatch.setattr(
            "nastech_cli.plugins._dispatch_pre_tool_call_hooks",
...
class TestRunConversation:
    ...
    def test_nastech_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_nastech_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
...
class TestRetryExhaustion:
    ...
    def test_content_filter_refusal_surfaced_not_retried(self, agent):
        ...
        portal (Nastech Portal fronting Anthropic) returns ``message.refusal``
...
class TestNastechCredentialRefresh:
    """Verify Nastech credential refresh rebuilds the runtime client."""

    def test_try_refresh_nastech_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "nastech_cli.auth.resolve_nastech_runtime_credentials", _fake_resolve
        )
        ...
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_nastech_client_credentials(force=True)
        ...
        assert rebuilt["kwargs"]["api_key"] == "new-nastech-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.nastechresearch.github.io/v1"
        )
        assert "default_headers" not in rebuilt["kwargs"]
        assert isinstance(agent.client, _RebuiltClient)

    def test_try_refresh_nastech_client_credentials_rebuilds_anthropic_client(
        self, agent, monkeypatch
    ):
        ...
        agent.api_key = "stale-nastech-key"
        agent.base_url = "https://inference-api.nastechresearch.github.io/v1"
        agent._anthropic_api_key = "stale-nastech-key"
        agent._anthropic_base_url = "https://inference-api.nastechresearch.github.io/v1"
        ...
        assert agent.base_url == "https://inference-api.nastechresearch.github.io/v1"
        assert agent._anthropic_api_key == "fresh-portal-jwt"
        assert agent._anthropic_base_url == (
            "https://inference-api.nastechresearch.github.io/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_nastech_gpt5_stays_on_chat_completions(self, agent):
        """Nastech serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "nastech"
        agent.base_url = "https://inference-api.nastechresearch.github.io/v1"
        agent.api_mode = "chat_completions"
