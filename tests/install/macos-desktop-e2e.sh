...
# (Nastech-Setup.dmg from the website) can reach HEAD.
#
...
#              nastech-desktop-app-update  capture `nastech desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              nastech-update              CLI update from the installed venv
...
#     --update-method open-app-update|nastech-desktop-app-update
...
case "$UPDATE_METHOD" in
  open-app-update|nastech-desktop-app-update|nastech-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${NASTECH_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/nastech-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$NASTECH_HOME/nastech-agent"
}
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Nastech.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Nastech.app" \
    "/Applications/Nastech.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Nastech-Setup.dmg"

  local dmg="$WORK_ROOT/Nastech-Setup.dmg"
  ...
  # "Install Nastech" button with native input.
  local rc=0
  ...
  log_group "Nastech-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$nastech" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "nastech --version failed after install"
  ok "nastech --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Nastech.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    nastech-update)
      ...
      # update subcommand only in later releases; ask the installed nastech.
      local nastech="$INSTALL_DIR/venv/bin/nastech"
      local update_cmd=("$nastech" update)
      ...
      [ "$rc" -eq 0 ] || fail "nastech update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Nastech.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Nastech.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Nastech.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, nastech update) need.
      local app app_bin
...
PYEOF
    ...
    nastech-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "nastech desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "nastech desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.nastech/logs" "$ildest/nastech-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Nastech"
  ...
  "$INSTALL_DIR/venv/bin/nastech" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "nastech --version failed after update"
  ok "nastech --version works post-update"
