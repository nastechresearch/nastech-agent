...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Nastech the way
...
# actually touches:
#
#   INSTALL   - downloads the production Nastech-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Nastech.exe must appear.
...
#   installer's `git clone` and `nastech update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and nastech-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "nastech-desktop-app-update", "nastech-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Nastech-Setup.exe carries
...
$NastechHome  = Join-Path $WorkRoot "nastech-home"
$InstallDir  = Join-Path $NastechHome "nastech-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if nastech ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit nastech thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Nastech.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Nastech.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-NastechRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $NastechHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "nastech-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $NastechHome ".nastech-update-result.json"
    ...
    Get-ChildItem -LiteralPath $NastechHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "nastech-home-ls.txt")
}

function Test-NastechRuns([string]$Label) {
    Save-InstallSideState $Label
    $nastechExe = Join-Path $InstallDir "venv\Scripts\nastech.exe"
    Assert-True (Test-Path -LiteralPath $nastechExe) "$Label -- venv\Scripts\nastech.exe exists"
    & $nastechExe --version 2>&1 | ForEach-Object { Write-Host "    nastech --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- nastech --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-NastechHome", $NastechHome, "-InstallDir", $InstallDir)
...
function Invoke-NastechUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $nastechExe = Join-Path $InstallDir "venv\Scripts\nastech.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $nastechExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $nastechExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "nastech update transcript" $log
    Assert-True ($updateExit -eq 0) "nastech update exited $updateExit (expected 0)"
}

function Invoke-NastechDesktopAppUpdate([string]$TargetSha) {
    # The nastech-desktop launch surface: `nastech desktop` runs its whole
    ...
    $prevCap = $env:NASTECH_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:NASTECH_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $nastechExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:NASTECH_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "nastech desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "nastech desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $NastechHome ".nastech-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured nastech desktop spec; update completed"
}
...
function Stop-NastechAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Nastech.exe (Electron) -- never nastech.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Nastech" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Nastech.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `nastech update`/desktop builds use the Nastech-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $NastechHome "node\node.exe"),
        (Join-Path $NastechHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Nastech-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Nastech-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Nastech-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Nastech-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:NASTECH_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    New-Item -ItemType Directory -Path $NastechHome -Force | Out-Null
    ...
    try {
        ...
        Write-Host "  Nastech-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Nastech.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Nastech-Setup.exe", (Join-Path $NastechHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Nastech" -ErrorAction SilentlyContinue)) "Nastech.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Nastech-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $NastechHome "logs\bootstrap-installer.log"
    ...
    Stop-NastechAppProcesses "post-install"
    ...
    Test-NastechRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Nastech.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Nastech.exe present before update"
    ...
    try {
        ...
        # `nastech update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .nastech-update-result.json on every exit;
        #   * older checkouts -> the staged nastech-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/nastech/relaunch
        ...
        $updateLog = Join-Path $NastechHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-NastechRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Nastech.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Nastech.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Nastech" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Nastech window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Nastech" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $NastechHome "logs\desktop-update-handoff.log"
        ...
        Stop-NastechAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, nastech runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-NastechRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-NastechRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $NastechHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "nastech-desktop-app-update" {
            Invoke-NastechDesktopAppUpdate $state.current
        }
        "nastech-update" {
            Invoke-NastechUpdate
        }
        "installer-script" {
        ...
        "desktop-installer@latest" {
            # A user re-downloading Nastech-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $NastechHome "logs\desktop.log"))) {
