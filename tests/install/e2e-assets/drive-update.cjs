// drive-update.cjs — launch the INSTALLED Nastech.exe (real Electron desktop
...
//
//   node <this file> <path-to-Nastech.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Nastech.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: NASTECH_HOME (isolated install) and
  ...
  // The real backend (`nastech serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Nastech... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.nastechDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Nastech — this window will close" overlay should appear,
  ...
  // the staged binary), which claims NASTECH_HOME/.nastech-update-in-progress
  ...
  // proved the desktop had exited and `nastech update` was already running).
  //
  ...
  const nastechHome = process.env.NASTECH_HOME
  const markerPath = nastechHome ? path.join(nastechHome, '.nastech-update-in-progress') : null
  const resultPath = nastechHome ? path.join(nastechHome, '.nastech-update-result.json') : null
