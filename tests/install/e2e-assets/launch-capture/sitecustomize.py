"""Driver-side spawn interception for nastech desktop E2E legs.

The installed ``nastech`` is a venv console script, so its interpreter
imports ``sitecustomize`` at startup when this directory is on
``PYTHONPATH``. Behind an explicit env-var opt-in the module wraps
``subprocess.run`` so the FINAL electron launch call of ``nastech
...
Opt-in: ``NASTECH_E2E_CAPTURE_LAUNCH=<path>`` -- the spec is written
there, and the marker file ``<path>.captured`` distinguishes "nastech
...
_SPEC_PATH = os.environ.get("NASTECH_E2E_CAPTURE_LAUNCH")
...
if _SPEC_PATH:
    ...
    def _match_shape(argv: "list[str]") -> str:
        ...
        # apps/desktop/release/ (win-unpacked/Nastech.exe, linux-unpacked/...,
        # mac*/Nastech.app/Contents/MacOS/...).
