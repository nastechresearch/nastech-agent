...
#   mock_start <workroot>      start, write config into $NASTECH_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, NASTECH_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$NASTECH_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $NASTECH_HOME (api $url/v1)"
}
