...
; Drive the REAL Nastech-Setup.exe (Tauri bootstrap installer) window:
; click Install, wait for the install to finish, click Launch, and wait for
; the real Nastech.exe (Electron desktop) window to appear.
;
; Adapted from @ethernet8023's e2e/windows/install-nastech-desktop.ahk
...
setupExe := A_Args.Length >= 2 ? A_Args[2] : "Nastech-Setup.exe"
...
Log("Launch clicked; waiting for the Nastech desktop app window")
...
try {
    ...
    throw Error("Nastech.exe real-sized window did not appear within 300s of clicking Launch")
}
