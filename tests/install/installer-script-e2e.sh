...
#              install landed on OLD with a working `nastech`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `nastech`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method nastech-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  nastech-update|installer-script|installer-script+desktop|nastech-desktop-app-update) ;;
  *) echo "error: --update-method must be nastech-update, installer-script, installer-script+desktop or nastech-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/nastech-installer-script-e2e"
LOG_DIR="${NASTECH_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled nastech or a
# developer config, and old installer scripts hardcode $HOME/.nastech (the
# NASTECH_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export NASTECH_HOME="$HOME/.nastech"
...
assert_desktop_artifact() {
  ...
  for cand in \
    ...
    "$release_dir/linux-unpacked/nastech" \
    "$release_dir/mac-arm64/Nastech.app" \
    "$release_dir/mac/Nastech.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local nastech="$INSTALL_DIR/venv/bin/nastech"
  [ -x "$nastech" ] || fail "no nastech console script at $nastech"
  "$nastech" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "nastech --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "nastech --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `nastech desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `nastech update` itself makes. Probe the
  # INSTALLED nastech for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `nastech desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local nastech="$INSTALL_DIR/venv/bin/nastech"
  if ! "$nastech" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "nastech desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$nastech" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "nastech desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "nastech desktop --build-only ($1) exited $rc; transcript above"
  ok "nastech desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  nastech-update)
    ...
    # installed nastech; older ones read the prompt from stdin, so close it.
    NASTECH="$INSTALL_DIR/venv/bin/nastech"
    if "$NASTECH" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$NASTECH" update --yes)
    else
      update_cmd=("$NASTECH" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "nastech update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "nastech update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  nastech-desktop-app-update)
    # The real user surface: `nastech desktop` launches the app, the user
    ...
    # suite's own) is configured into NASTECH_HOME exactly like the dev:mock
    ...
    step "capturing the nastech desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      NASTECH_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$NASTECH" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "nastech desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "nastech desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "nastech desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$NASTECH_HOME/logs" "$ildest/nastech-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/nastech/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$NASTECH_HOME/.nastech-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$NASTECH_HOME" > "$ildest/nastech-home-ls.txt" 2>/dev/null || true
