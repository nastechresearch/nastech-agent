...
from nastech_state import FTS_STALE_KEY, SessionDB, _FTS_TRIGGERS
...
lock_cm = nastech_state._cross_process_repair_lock(Path({db!r}))
...
class TestNonContentionErrnoFailsFast:
    ...
    def test_repair_lock_non_contention_errno_fails_fast(
        ...
        monkeypatch.setattr(fcntl, "flock", _flock)
        t0 = time.monotonic()
        with nastech_state._cross_process_repair_lock(tmp_path / "state.db") as ok:
            assert ok is False
        assert time.monotonic() - t0 < 2.0
