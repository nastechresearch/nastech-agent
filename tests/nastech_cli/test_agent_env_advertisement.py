"""Tests for the AI_AGENT / NASTECH_AGENT harness-attribution env vars.
...
Nastech-specific marker, without clobbering an outer harness.

The AI_AGENT value must equal Nastech' id in the public agent-harness
registry (``nastech-agent`` in huggingface.js ``agent-harnesses.ts``) —
...
environment does not inherit the Nastech process env, and survives the
cross-session leak guard that strips ``NASTECH_SESSION_*`` from subprocess
...
class TestWrapCommandAdvertisesHarness:
    ...
    def _wrap(self, command: str) -> str:
        ...
        env._cwd_marker = "__NASTECH_CWD_testsession0__"
        env._snapshot_path = "/tmp/nastech-snap-testsession0.sh"
    ...
    def test_wrap_command_contains_export(self):
        ...
        assert 'NASTECH_AGENT="${NASTECH_AGENT:-true}"' in wrapped
    ...
    def test_shell_sets_default_and_preserves_outer(self):
        ...
        wrapped = self._wrap('echo "AI=$AI_AGENT NASTECH=$NASTECH_AGENT"')

        clean_env = {k: v for k, v in os.environ.items()
                     if k not in ("AI_AGENT", "NASTECH_AGENT")}
        out = subprocess.run(
        ...
        outer_env = dict(clean_env, AI_AGENT="pi", NASTECH_AGENT="false")
        out = subprocess.run(
            ["bash", "-c", wrapped], capture_output=True, text=True,
