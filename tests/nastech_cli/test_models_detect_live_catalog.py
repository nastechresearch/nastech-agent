...
class TestCurrentProviderCatalogWins:
    @pytest.mark.parametrize("provider,model", [
        ...
        ("nastech", "some-portal-only-model"),    # Portal serves it, static snapshot doesn't
    ...
    def test_bare_name_resolves_to_current_providers_full_slug(self, live_catalog):
        live_catalog["nastech"] = ["zai/glm-5.3-flash"]
        assert models.detect_provider_for_model("glm-5.3-flash", "nastech") == ("nastech", "zai/glm-5.3-flash")

    def test_unserved_model_still_walks_the_ladder(self, live_catalog, monkeypatch):
        from nastech_cli import models_detect

        monkeypatch.setattr(models_detect, "provider_has_credentials", lambda p: p == "openrouter")
        live_catalog["nastech"] = ["nastech-4-405b"]
        assert models.detect_provider_for_model("no-such-model", "nastech") == ("openrouter", "vendor/no-such-model")
