...
class TestCuratedModelsForProvider:
    ...
    def test_live_catalog_projected_to_tuples_else_static_fallback(self):
        with patch("nastech_cli.models.provider_model_ids", return_value=["m-live"]):
            assert curated_models_for_provider("nastech") == [("m-live", "")]
        with patch("nastech_cli.models.provider_model_ids", return_value=[]), patch.dict(
            "nastech_cli.models._PROVIDER_MODELS", {"nastech": ["m-static"]}
        ):
            assert curated_models_for_provider("nastech") == [("m-static", "")]
...
class TestValidateCodex900kVariants:
    """`-900k` is a Nastech picker convention: valid variants come from the
    ...
    def test_catalog_listed_variant_accepted(self):
        with patch("nastech_cli.models.provider_model_ids", return_value=self._CATALOG):
    ...
    def test_valid_variant_missing_from_catalog_still_accepted(self):
        ...
        with patch("nastech_cli.models.provider_model_ids", return_value=["gpt-5.6-sol"]):
            result = validate_requested_model("gpt-5.6-sol-900k", "openai-codex")
        assert result["accepted"] is True
...
class TestValidateRequestedModelNastechPortalRecommendations:
    """Regression tests for issue #71312: the Nastech Telegram picker (and any
    other messaging-platform /model validation, since they all share
    validate_requested_model()) rejected models that are live Nastech Portal
    recommendations (/api/nastech/recommended-models) but not yet in the
    hardcoded curated catalog -- even though `nastech chat` already accepts
    ...
    def _validate_nastech(self, model, api_models=None, portal_payload=None, portal_raises=False):
    ...
    def test_free_portal_recommendation_accepted(self):
        ...
        result = self._validate_nastech("inclusionai/ling-3.0-flash:free")
        assert result["accepted"] is True
        assert result["persist"] is True
        assert "Portal recommendation" in (result["message"] or "")

    def test_paid_portal_recommendation_accepted(self):
        result = self._validate_nastech("inclusionai/ling-3.0-pro")
        assert result["accepted"] is True
    ...
    def test_model_absent_from_portal_and_catalog_still_rejected(self):
        ...
        result = self._validate_nastech("totally-made-up-model-xyz")
        assert result["accepted"] is False
        assert result["recognized"] is False
    ...
    def test_non_string_model_name_entries_ignored(self):
        ...
        assert self._validate_nastech("5", portal_payload=payload)["accepted"] is False
        result = self._validate_nastech(
            "inclusionai/ling-3.0-flash:free", portal_payload=payload
        )
        assert result["accepted"] is True

    def test_non_nastech_provider_does_not_consult_portal_feed(self):
        """This fallback tier is Nastech-specific; a non-Nastech provider must
        not have its rejection changed by (or trigger a call to) the Nastech
        Portal feed."""
        probe_payload = {
        ...
        with patch("nastech_cli.models.fetch_api_models", return_value=["some/other-model"]), \
             patch("nastech_cli.models.probe_api_models", return_value=probe_payload), \
             patch("nastech_cli.models.fetch_nastech_recommended_models") as mock_portal, \
             patch("nastech_cli.models._model_in_provider_catalog", return_value=False):
    ...
    def test_curated_catalog_hit_short_circuits_before_portal_check(self):
        ...
        with patch("nastech_cli.models.fetch_api_models", return_value=api_models), \
             patch("nastech_cli.models.probe_api_models", return_value=probe_payload), \
             patch("nastech_cli.models._model_in_provider_catalog", return_value=True), \
             patch("nastech_cli.models.fetch_nastech_recommended_models") as mock_portal:
            result = validate_requested_model("inclusionai/ling-2.6-flash", "nastech")
        mock_portal.assert_not_called()
        assert result["accepted"] is True
...
class TestValidateCustomUnreachableFallback:
    ...
    def _validate(self, model, provider, models, **kw):
        ...
        with patch("nastech_cli.models.probe_api_models", return_value=probe):
