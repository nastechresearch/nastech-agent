...
class TestSessionDbCacheAfterProfileDelete:
    """``nastech profile delete`` force-closes every registry handle under the profile home
    ...
    def test_delete_then_recreate_gets_a_live_store(self, nastech_home):
        import shutil
        ...
        from nastech_cli.goals import GoalState, _get_session_db, load_goal, save_goal
        ...
        with patch.object(nastech_state, "DEFAULT_DB_PATH", nastech_state._IMPORT_DEFAULT_DB_PATH):
            profile = nastech_home / "profiles" / "p1"
...
class TestWaitBarrier:
    ...
    def test_wait_on_rejects_a_pid_not_alive_on_this_host(self, nastech_home, monkeypatch):
    ...
    def test_barrier_on_a_process_that_never_exits_expires(self, nastech_home):
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_on_dead_pid_continues_instead_of_parking(self, nastech_home):
    ...
    def test_judge_wait_on_pid_dying_between_check_and_park_continues(self, nastech_home):
...
class TestBlockedVerdict:
    ...
    def test_blocked_verdict_pauses_goal_instead_of_done(self, nastech_home):
        from unittest.mock import patch
        from nastech_cli.goals import GoalManager
...
def test_goal_session_db_is_the_registry_shared_handle(nastech_home):
