...
second Nastech holder the updater's drain did not stop — survives the copy and is
...
def test_torn_restore_is_what_the_guard_prevents(live_db_with_hot_wal, snapshot_db):
    ...
    """
    from nastech_cli.backup import verify_sqlite_integrity
...
def test_post_update_guard_covers_sibling_profiles(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(update_cmd, "get_nastech_home", lambda: root_home)
    monkeypatch.setattr(
        "nastech_cli.backup._sibling_profile_homes",
