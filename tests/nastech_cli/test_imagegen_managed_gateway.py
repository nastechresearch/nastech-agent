...
(non-managed) routing. When a user picked FAL through the Nastech-subscription
...
the user's personal FAL_KEY instead of the Nastech Tool Gateway (real incident:
...
exactly ONE provider string per category — ``image_gen.provider: nastech`` for
the managed "Nastech Subscription" row, ``image_gen.provider: fal`` for the
BYOK FAL row — and any legacy ``use_gateway`` key is popped so the
read-time shim (use_gateway: true ⇒ nastech) cannot override the fresh pick.
...
def _quiet(monkeypatch):
    import nastech_cli.tools_config_providers as tc
...
def test_image_gen_selector_preserves_managed_selection(monkeypatch):
    """Managed pick: the 'nastech' provider string must survive the selector."""
    _quiet(monkeypatch)
    config = {}
...
def test_image_and_video_selectors_share_the_selection_contract(monkeypatch):
    ...
    for use_gateway, expected in ((True, "nastech"), (False, "fal")):
        config = {
...
def _quiet_reconfigure(monkeypatch):
    ...
    monkeypatch.setattr(ns, "ensure_nastech_portal_access", lambda **k: True)
...
def test_reconfigure_managed_fal_row_keeps_managed_selection(monkeypatch):
    ...
    branch wrote the managed one — a Nastech Subscription user re-entering the
...
def test_reconfigure_direct_fal_row_writes_vendor_selection(monkeypatch):
    ...
    legacy use_gateway key so the read-time shim can't resurrect 'nastech'."""
    _quiet_reconfigure(monkeypatch)
    import nastech_cli.tools_config as tc
