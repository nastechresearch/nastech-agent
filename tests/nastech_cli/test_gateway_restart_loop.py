...
Covers:
- Defense 1: gateway stop/restart refuse when _NASTECH_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `python*` carries no nastech/gateway token yet takes it down — on every platform.
        ...
        "pkill -9 python3",
        "pkill -f python",
        'pkill -f "python -m nastech_cli.main"',
        "pkill -f nastech_cli",
        "killall -9 python3.12",
        "sudo pkill -9 python3",
        "pgrep python | xargs kill -9",
        "kill $(pidof python3)",
        # Windows spellings of the nastech-gateway forms (salvaged from #94379).
        "nastech.exe gateway restart",
        "nastech.cmd gateway stop",
        r'"C:\Program Files\nastech.exe" gateway restart',
        "taskkill /F /IM nastech-gateway.exe",
        "Stop-Process -Name nastech-gateway -Force",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        # Unrelated scripts that merely contain "nastech" cannot match the gateway cmdline.
        "pkill -f 'nastech-polis/run.sh'",
        "pkill -f my_nastech_bot.py",
        "pgrep python",
        "nastech.exe gateway start",
        "my-nastech.exe gateway restart",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.nastech.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.nastech/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l nastech-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.nastech.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.nastech.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.nastech.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.nastech/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.nastech.gateway-apollo:/a.plist' "
            "'ai.nastech.gateway-cronus:/c.plist' 'ai.nastech.gateway-plutus:/p.plist' "
            "'ai.nastech.gateway:/Users/botuser/Library/LaunchAgents/ai.nastech.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated nastech note on the next line"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.nastech.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.nastech.gateway',
        "launchctl kick'start' -k gui/501/ai.nastech.gateway",
        "launchctl kick\\start -k gui/501/ai.nastech.gateway",
        'launchctl "kickstart" -k gui/501/ai.nastech.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.nastech.gateway',
        "launchctl dis\\able gui/501/ai.nastech.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.nastech."gateway"',
        # Same class on the systemctl and nastech-CLI branches.
        'systemctl re"start" nastech-gateway',
        'nastech gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.nastech.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.nastech.update-checker',
        'systemctl re"start" nastech-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nastech cron list",
        "nastech update",
        "nastech config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `nastech gateway start` is benign — starting a gateway from inside a
        ...
        "nastech gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway nastech
        # services must not be falsely blocked (the old `.*nastech` matched any
        # nastech token).
        "launchctl unload ai.nastech.update-checker.plist",
        "launchctl restart ai.nastech.daemon",
        ...
        "systemctl restart nastech-meta.service",
        "systemctl restart nastech-cron-helper",
        ...
        "echo after the nastech gateway restarted cleanly",
        "the nastech gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "nastech skill view gateway-notes && echo nastech gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `nastech` is a path component
        # there, not a command.
        "cat '/docs/nastech gateway restart-notes.md'",
        "less /home/user/notes/nastech gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "nastech gateway restart; echo done",
        "nastech gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;nastech gateway restart",
        "true && nastech gateway stop",
        "echo $(nastech gateway restart)",
        "echo `nastech gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.nastech.gateway"])',
        'subprocess.run(["nastech", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.nastech.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["nastech", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: nastech gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `nastech -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry NASTECH_HOME/NASTECH_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("NASTECH_PROFILE", "zeus")
        monkeypatch.delenv("NASTECH_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "nastech -p zeus gateway stop",
        "nastech -p zeus gateway restart",
        "nastech --profile zeus gateway restart",
        "nastech --profile zeus gateway stop",
        "nastech --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "nastech -v -p zeus gateway restart",
        "nastech -p zeus -v gateway restart",
        "nastech --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "nastech -p venus gateway stop",
        "nastech -p venus gateway restart",
        "nastech --profile venus gateway restart",
        "nastech --profile=venus gateway stop",
        "nastech -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "nastech -p zeus gateway start",
        "nastech -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("nastech gateway restart")
        assert _contains_gateway_lifecycle_command("nastech gateway stop")

    def test_nastech_home_derived_profile(self, monkeypatch):
        # Without NASTECH_PROFILE the guard falls back to the NASTECH_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("NASTECH_PROFILE", raising=False)
        monkeypatch.delenv("NASTECH_PROFILE_NAME", raising=False)
        import nastech_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("nastech -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("nastech -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("nastech -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("nastech gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_nastech_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade nastech then run nastech gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_nastech_gateway_once.sh). The script must live under
        # NASTECH_HOME/scripts so the scheduler — and the guard — resolve it.
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_NASTECH_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _NASTECH_GATEWAY=1.

    Issue #37453: systemctl --user restart nastech-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart nastech-gateway",
        "systemctl --user restart nastech-gateway",
        "systemctl stop nastech-gateway.service",
        "nastech gateway restart",
        "nastech gateway uninstall",
        "launchctl kickstart gui/501/ai.nastech.gateway",
        "launchctl bootout gui/501/ai.nastech.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.nastech.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.nastech/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.nastech.gateway.restart-once.plist",
        "pkill -f nastech.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart nastech-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.nastech.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-nastech label: label-independent detection is the point
        # (#62891 second reproduction used `ai.nastech.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.nastech.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _NASTECH_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _NASTECH_GATEWAY=1 is in the
        ...
        result = json.loads(tt.terminal_tool(command="nastech gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["nastech gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.nastech.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.nastech.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-nastech systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run nastech gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("nastech gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under NASTECH_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("nastech gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/nastech\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 nastech gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_nastech_home()` falls back to Path.home(), which raises
        RuntimeError when neither NASTECH_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart nastech-gateway' .",
        "grep -rn 'nastech gateway restart' ./logs",
        "rg 'nastech gateway restart' ../archive",
        "grep -c 'systemctl stop nastech-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM nastech_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell nastech gateway restart"',
        'sqlite3 ./db ".system systemctl restart nastech-gateway"',
        'psql ./x -c "\\! systemctl restart nastech-gateway"',
        "grep -r 'nastech gateway restart' . | sh",
        "grep -r 'nastech gateway restart' ./logs | bash",
        "grep -r 'nastech gateway restart' . | sudo sh",
        "grep -r 'x' . ; nastech gateway restart",
        "grep -r 'x' . && systemctl restart nastech-gateway",
        'grep -r "$(nastech gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart nastech-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell nastech gateway restart"',
        'sqlite3 db ".system systemctl restart nastech-gateway"',
        'psql -c "\\! systemctl restart nastech-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    nastech_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run nastech gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / ".nastech"))
        (tmp_path / ".nastech").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run nastech gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop nastech-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart nastech-gateway' /var/log/syslog",
        "rg 'nastech gateway restart' /home/user/.nastech/logs/",
        "journalctl -u nastech-gateway --grep 'systemctl restart nastech-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'nastech_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell nastech gateway restart"',
        'psql -c "\\! systemctl restart nastech-gateway"',
        "grep 'systemctl restart nastech-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart nastech-gateway",
        'grep "$(systemctl restart nastech-gateway)" f',
        "grep 'restart' log; systemctl restart nastech-gateway",
        'sqlite3 db "SELECT 1"; nastech gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "nastech gateway restart",
        "sudo systemctl stop nastech-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart nastech-gateway%'\""
        )
