"""Narrowing the Nastech model lists to an org's policy.
...
from nastech_cli.models_pricing import _NASTECH_POLICY_APPEND_MAX, nastech_policy_allowed_ids, restrict_to_nastech_policy
...
class TestRestrictToNastechPolicy:
    ...
    def test_preserves_curated_order(self):
        ...
        assert restrict_to_nastech_policy(curated, allowed) == curated

    def test_keeps_a_free_sibling_when_its_base_is_reachable(self):
        """Portal free recommendations are ``:free`` ids."""
        assert restrict_to_nastech_policy(["vendor/model:free"], {"vendor/model"}) == [
            "vendor/model:free"
        ]
    ...
    def test_drops_a_free_sibling_whose_base_is_blocked(self):
        assert restrict_to_nastech_policy(["vendor/model:free"], {"other/model"}) == []


class TestNastechPolicyAllowedIds:
    ...
    def _patch(self, monkeypatch, *, policy_present, api_key="sk-test", pricing=None):
        calls = []
        monkeypatch.setattr(
            account_mod, "nastech_policy_present", lambda: policy_present
        )
        monkeypatch.setattr(models_pricing, "_resolve_nastech_pricing_credentials",
    ...
    def test_returns_the_authenticated_catalog_keys(self, monkeypatch):
        ...
        assert nastech_policy_allowed_ids() == {"a/one", "b/two"}
        assert len(calls) == 1
    ...
    def test_declines_to_filter_an_unrestricted_org(self, monkeypatch):
        ...
        assert nastech_policy_allowed_ids() is None
...
class TestNastechPolicyPresent:
...
class TestNastechPolicyNotice:

    def _patch(self, monkeypatch, present):
        monkeypatch.setattr(account_mod, "nastech_policy_present", lambda: present)

    def test_shows_a_line_for_a_governed_org(self, monkeypatch):
        self._patch(monkeypatch, True)
        assert "restricts which models" in account_mod.nastech_policy_notice(removed=True)
    ...
    def test_silent_otherwise(self, monkeypatch, present):
        ...
        assert account_mod.nastech_policy_notice(removed=True) == ""
    ...
    def test_silent_when_the_filter_removed_nothing(self, monkeypatch):
        ...
        self._patch(monkeypatch, True)
        assert account_mod.nastech_policy_notice(removed=False) == ""

    def test_names_no_models(self, monkeypatch):
        """The blocked set is most of the catalog under an allowlist."""
        self._patch(monkeypatch, True)
        notice = account_mod.nastech_policy_notice(removed=True)
...
class TestAllowlistOutsideTheCuratedList:
    ...
    def test_does_not_append_when_the_curated_overlap_is_non_empty(self):
        kept = restrict_to_nastech_policy(
            ["z/curated", "a/curated"],
    ...
    def test_does_not_rescue_a_catalog_sized_allowed_set(self):
        ...
        oversized = {f"cn/model-{i}" for i in range(_NASTECH_POLICY_APPEND_MAX + 1)}
        assert restrict_to_nastech_policy(["vendor/one"], oversized, rescue_empty=True) == []
...
class TestRescueIsOptIn:
    ...
    def test_rescue_only_when_asked(self):
        assert restrict_to_nastech_policy([], {"a/one"}, rescue_empty=True) == ["a/one"]
    ...
    def test_an_already_empty_unavailable_list_is_never_filled(self):
        ...
        assert restrict_to_nastech_policy([], reachable) == []
...
class TestPolicyRunsBeforeTierSplit:
    ...
    def test_a_rescued_paid_model_stays_unavailable_for_a_free_tier_user(self):
        from nastech_cli.models import partition_nastech_models_by_tier

        pricing = {
            "vendor/free": {"prompt": "0", "completion": "0"},
            "vendor/paid": {"prompt": "0.000002", "completion": "0.00001"},
        }
        narrowed = restrict_to_nastech_policy(
            list(pricing), {"vendor/paid"}, rescue_empty=True
        )
        selectable, unavailable = partition_nastech_models_by_tier(
