"""``nastech auth upgrade``: the free tier signs into a Nastech account, keeping its connectors.
...
class FakePortal:
    ...
    def handler(self, request: httpx.Request) -> httpx.Response:
        ...
        if path == "/api/oauth/token":
            ...
            self.token_grants += 1
            return httpx.Response(200, json={
                "access_token": _jwt(sub="nas_user:9", client_id="nastech-cli", account_tier="free"),
...
def portal(monkeypatch, tmp_path):
    fake = FakePortal()
    monkeypatch.setenv("NASTECH_PORTAL_BASE_URL", PORTAL)
    monkeypatch.setenv("NASTECH_ANON_API_SECRET", "test-secret")
    ...
    monkeypatch.setattr(auth_nastech, "_nastech_http_client", _client)
...
def free_account(monkeypatch):
    ...
    monkeypatch.setattr(m, "check_nastech_free_tier", lambda **kw: True)
    monkeypatch.setattr(m, "fetch_nastech_recommended_models", lambda *a, **kw: {
...
class TestSignInCompletionSettlesTheModel:
    ...
    def test_no_eligible_recommendation_leaves_no_default_rather_than_a_model_the_account_may_not_use(
        ...
        monkeypatch.setattr(m, "recommended_nastech_default_model", _portal_down)
        assert anon_auth.upgrade_guest(_args()) == 0
        model_cfg = _model_config()
        ...
        assert "run `nastech model` to pick one" in out
