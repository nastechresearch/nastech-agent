...
# behavior is covered by tests/nastech_cli/test_memory_setup_env_denylist.py,
# which exercises the current save_env_value-routed signature
# (env_writes, nastech_home=None) \u2014 these three tests pinned the prior direct
...
def test_cmd_status_memory_tool_gate_disabled(capsys, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.config.load_config", lambda: _cfg)
    # check_memory_requirements() reads the readonly loader, not load_config.
    monkeypatch.setattr(
        "nastech_cli.config.load_config_readonly", lambda: _cfg, raising=False
    )
