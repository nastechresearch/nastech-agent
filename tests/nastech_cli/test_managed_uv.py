...
from nastech_cli.managed_uv import _CandidateStageError
...
class TestMacOSManagedPythonSigning:
    def test_signs_with_stable_identifier_and_verifies(self, tmp_path, monkeypatch):
        ...
        assert calls[0][0] == [
            ...
            "--timestamp=none",
            "--identifier",
            "com.nastechresearch.nastech.managed-python",
            "--requirements",
            '=designated => identifier "com.nastechresearch.nastech.managed-python"',
            str(python),
        ]
        assert calls[1][0] == [
            "/usr/bin/codesign",
...
class TestEnsureUv:
    ...
    def test_installs_if_missing(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             ...
             patch("nastech_cli.managed_uv._uv_version", return_value="uv 0.1.2"), \
    ...
    def test_install_reports_runtime_repair_to_observer(self, tmp_path):
        ...
        with patch(
            ...
            "nastech_cli.managed_uv._uv_version",
...
class TestUpdateManagedUv:
    ...
    def test_fresh_stamp_skips_network_self_update_but_not_repair(self, tmp_path):
        ...
        uv = tmp_path / "bin" / _UV_BINARY_NAME
        _make_executable(uv)
        # The stamp reader imports get_nastech_home separately from the binary
        ...
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("nastech_cli.managed_uv.repair_vulnerable_runtime",
    ...
    def test_stale_stamp_runs_self_update_and_refreshes_stamp(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv.get_nastech_home", return_value=tmp_path), \
             patch("nastech_cli.managed_uv._uv_self_update_stamp", return_value=stamp), \
             patch("nastech_cli.managed_uv.repair_vulnerable_runtime", return_value=_RRR("not-applicable")), \
             patch("nastech_cli.managed_uv._uv_version", return_value="uv 0.2.0"), \
             patch("nastech_cli.managed_uv.subprocess.run") as mock_run:
...
class TestStageCandidateVenvCrossPlatform:
    ...
    def test_sync_keeps_uv_project_config_and_merges_stderr(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv.subprocess.run", side_effect=fake_run), \
             patch("nastech_cli.managed_uv.subprocess.Popen", side_effect=fake_popen), \
             patch(
                 "nastech_cli.managed_uv._smoke_candidate_venv",
    ...
    def test_sync_failure_reports_the_child_reason(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING), \
             patch("nastech_cli.managed_uv.subprocess.run", return_value=MagicMock(returncode=0)), \
             patch(
                 "nastech_cli.managed_uv.subprocess.Popen",
        ...
        assert not list((root / ".nastech-runtime").glob("venv-candidate-*"))
...
class TestInstallUvInternals:
    def test_installer_uses_host_branch_and_managed_directory(self, tmp_path):
        ...
        with patch("nastech_cli.managed_uv._install_uv_posix") as mock_posix, \
             patch("nastech_cli.managed_uv._install_uv_windows") as mock_windows:
...
class TestMinorLineFallForward:
    ...
    `nastech update` with no path to a fixed runtime.
    """
