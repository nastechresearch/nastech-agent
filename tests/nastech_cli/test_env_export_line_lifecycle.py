...
def test_remove_export_prefixed_token(nastech_home):
    """DELETE must clear an ``export KEY=...`` line, not 404 on it."""
    _write_env_raw(nastech_home, f"export GITHUB_TOKEN={OLD_PAT}\n")
...
def test_update_export_prefixed_token_does_not_duplicate(nastech_home):
...
def test_export_line_with_comment_untouched(nastech_home):
...
def test_remove_token_written_with_spaces_around_equals(nastech_home):
    ...
    """
    _write_env_raw(nastech_home, f"GITHUB_TOKEN = {OLD_PAT}\n")
...
def test_spaced_token_rotate_then_delete_does_not_resurrect(nastech_home):
...
def test_writer_matches_exactly_what_load_env_accepts(nastech_home):
    """Parity: the writers must recognise a line iff ``load_env()`` does."""
    from nastech_cli.config import _env_line_defines_key, load_env
    ...
    for line, expected in [
        ...
        _write_env_raw(nastech_home, line + "\n")
