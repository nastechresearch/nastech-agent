...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nastech_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nastech_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_bridges_ozone_hint_to_launch_env(tmp_path, monkeypatch):
    ...
    with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nastech_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
    ...
    with patch("nastech_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("nastech_cli.config.load_config", return_value=cfg), \
         patch("nastech_cli.linux_desktop_entry.install_desktop_entry", return_value=None), \
         patch("nastech_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run2, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns())
