...
def whatsapp_bridge(monkeypatch):
    """Route ``nastech send --to whatsapp:...`` through the real plugin standalone sender into a fake
    ...
    monkeypatch.setattr(send_cmd, "_load_nastech_env", lambda: None)
...
def test_load_nastech_env_latin1_fallback_overrides_shell(tmp_path, monkeypatch):
    ...
    (nastech_home / ".env").write_bytes(b"SEND_OVR_LABEL=caf\xe9-file\n")
...
def test_help_and_empty_list_hint_name_the_resolved_home(tmp_path, monkeypatch, capsys):
    ...
    hardcoded ``~/.nastech`` (absent on a Windows install or under a profile home)."""
    import argparse
    import sys
    import types
    ...
    assert "~/.nastech" not in help_text
    ...
    assert "~/.nastech" not in out


def test_empty_list_hint_names_default_root_directory_under_profile_home(tmp_path, monkeypatch, capsys):
    """Under ``NASTECH_HOME=<root>/profiles/<p>`` the ``--list`` empty state says the default root already holds
