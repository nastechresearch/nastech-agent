...
import nastech_cli.main_provider_setup as nastech_cli_main_provider_setup
from nastech_cli import model_setup_flows
...
def test_ensure_runtime_credentials_passes_cli_model_as_target_model(monkeypatch):
    """`nastech -m mimo-v2.5 --provider opencode-go` must resolve credentials for the model the
    ...
    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.NastechCLI(model="mimo-v2.5", provider="opencode-go", compact=True, max_turns=1)
...
def test_model_flow_nastech_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models_pricing.get_pricing_for_provider", lambda provider: {})
    ...
    assert model["base_url"] == "https://inference-api.nastechresearch.github.io/v1"
...
def test_startup_fallback_re_resolves_reasoning_for_the_fallback_model(monkeypatch, reasoning_flag, expected_effort):
    ...
    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.NastechCLI(compact=True, max_turns=1, reasoning=reasoning_flag)
...
def test_custom_entry_model_swap_re_resolves_reasoning(monkeypatch):
    """`nastech chat --model <custom-provider-name>`: the runtime's explicit `model` replaces the
    ...
    shell = cli.NastechCLI(model="my-lan", compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    ...
    monkeypatch.setattr("nastech_cli.main_provider_setup._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.main_provider_setup._save_custom_provider",
...
def test_cmd_model_forwards_nastech_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr(nastech_cli_main_provider_setup, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    nastech_main.cmd_model(
        SimpleNamespace(
            ...
            inference_url="https://inference.nastechresearch.github.io/v1",
...
def test_auto_provider_name_localhost():
    from nastech_cli.main_provider_setup import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from nastech_cli.main_provider_setup import _save_custom_provider
