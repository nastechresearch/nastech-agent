...
# ---------------------------------------------------------------------------
# _quarantine_running_nastech_exe — retry, then report
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    pairs = main_install_repair._quarantine_running_nastech_exe(tmp_path)
...
def test_quarantine_reports_a_lock_it_cannot_break(_winp, tmp_path, capsys, monkeypatch):
    ...
    monkeypatch.setattr(main_install_repair, "_nastech_exe_shims", lambda d: [shim])
...
def test_pause_and_resume_windows_gateway_service(
    ...
    import nastech_cli.update_cmd_windows as update_cmd_windows
    ...
    assert token == {
        ...
        "unmapped_pids": [],
        "unmapped": [],
        "services": ["NastechGateway"],
        "expected_services": ["NastechGateway"],
        "restarted_services": [],
        "service_profiles": {"NastechGateway": "default"},
    }
    assert stopped == ["NastechGateway"]

    cli_main._resume_windows_gateways_after_update(token)
    assert started == ["NastechGateway"]
...
def test_pause_windows_gateway_service_failure_restores_every_attempted_service(
    ...
    services = [
        SimpleNamespace(name="NastechGateway", profile="default", service_pid=11, service_create_time=11.0, gateway_pid=101, gateway_create_time=101.0, descendant_identities=()),
        SimpleNamespace(name="NastechGatewayPicasso", profile="picasso", service_pid=22, service_create_time=22.0, gateway_pid=202, gateway_create_time=202.0, descendant_identities=()),
    ]
    monkeypatch.setattr(gateway_mod, "find_gateway_pids", lambda **_k: [])
    monkeypatch.setattr(
    ...
    with pytest.raises(RuntimeError, match="NastechGatewayPicasso"):
        cli_main._pause_windows_gateways_for_update()

    assert restarted == ["NastechGatewayPicasso", "NastechGateway"]
...
def test_pause_windows_gateway_service_surfaces_rollback_start_failure(
    ...
    with pytest.raises(RuntimeError, match="rollback failures: NastechGateway"):
        cli_main._pause_windows_gateways_for_update()
...
def test_restore_windows_gateway_service_waits_out_stop_pending(monkeypatch):
    ...
    update_cmd._restore_windows_gateway_service("NastechGateway")

    assert restarted == ["NastechGateway"]
...
def test_resume_windows_gateway_service_failure_stays_retryable(
    ...
    with pytest.raises(RuntimeError, match="NastechGateway"):
        cli_main._resume_windows_gateways_after_update(token)

    assert token["resume_needed"] is True
    assert token["services"] == ["NastechGateway"]
...
# when every concurrent nastech.exe is a gateway runtime — the pause
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    launcher shape (python -m, nastech.exe shim, nastech-gateway.exe,
    gateway/run.py, bare `nastech gateway` which defaults to run)."""
    cases = [
        [r"C:\venv\Scripts\python.exe", "-m", "nastech_cli.main", "gateway", "run"],
        [r"C:\venv\Scripts\nastech.exe", "gateway", "run"],
        [r"C:\venv\Scripts\nastech-gateway.exe"],
        [r"C:\venv\Scripts\python.exe", "gateway/run.py"],
        ["nastech.exe", "GATEWAY", "RUN"],  # matcher is case-insensitive
        ["nastech.exe", "gateway"],  # bare `nastech gateway` defaults to run
        # profile selector before the subcommand — canonical matcher strips it
        ["nastech.exe", "--profile", "work", "gateway", "run"],
    ]
    for argv in cases:
...
def test_classify_concurrent_instance_recognises_non_gateways(monkeypatch):
    ...
    cases = [
        [r"C:\venv\Scripts\nastech.exe"],  # interactive REPL
        [r"C:\venv\Scripts\nastech.exe", "dashboard"],
        ["nastech.exe", "gateway", "status"],  # management, not runtime
        ["nastech.exe", "gateway", "stop"],
...
def test_filter_non_gateway_concurrent_instances_splits(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                100: ["nastech.exe", "gateway", "run"],
                200: ["nastech.exe"],  # REPL — keep
                300: ["nastech.exe", "dashboard"],  # keep
    ...
    assert kept == [(200, "nastech.exe"), (300, "nastech.exe"), (400, "nastech.exe")]
...
def test_filter_non_gateway_concurrent_instances_gateway_only(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        _fake_psutil_classify(
            {
                111: ["nastech.exe", "gateway", "run"],
                222: [r"C:\venv\Scripts\nastech-gateway.exe"],
            }
        ),
    )
    matches = [(111, "nastech.exe"), (222, "nastech-gateway.exe")]
...
def test_update_gate_skips_abort_when_only_concurrent_is_gateway(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_nastech_instances",
        return_value=[(1000, "nastech.exe"), (2000, "nastech-gateway.exe")],
    ), patch.object(
    ...
    captured = capsys.readouterr().out
    assert "Another nastech.exe is running" not in captured
...
def test_update_gate_still_aborts_on_non_gateway_concurrent(
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_nastech_instances",
        return_value=[(1000, "nastech.exe"), (3000, "nastech.exe")],
    ), patch.object(
        cli_main,
        "_filter_non_gateway_concurrent_instances",
        return_value=[(3000, "nastech.exe")],
    ), patch.object(
        cli_main, "_run_pre_update_backup"
    ) as mock_backup:
...
def test_update_impl_refuses_before_terminating_gateway_ancestor(
    ...
    holder = (
        300,
        "python.exe",
        r"C:\x\venv\Scripts\python.exe -m nastech_cli.main gateway run",
    )
    monkeypatch.setattr(
        gateway_cli,
        "_is_pid_ancestor_of_current_process",
...
def test_stop_service_refuses_pid_reuse_before_sc_stop(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="identity changed"):
        update_cmd._stop_windows_gateway_service(
            "NastechGateway", expected_service_identity=(11, 11.0)
        )
