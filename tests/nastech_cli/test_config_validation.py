...
class TestTimezoneValidation:
    ...
    schedule on server-local time (nastech_time._get_zoneinfo falls back with
    ...
    def test_invalid_or_non_string_zone_is_an_error(self):
        [issue] = _tz_issues({"timezone": "Asia/Tokio", "model": {"provider": "nastech"}})
        assert issue.severity == "error"
        assert "Asia/Tokio" in issue.message
        assert "IANA" in issue.hint and "NASTECH_TIMEZONE" in issue.hint
        [issue] = _tz_issues({"timezone": 9, "model": {"provider": "nastech"}})
    ...
    def test_valid_blank_missing_or_unverifiable_zone_is_silent(self, monkeypatch):
        for cfg in ({"timezone": "Asia/Tokyo"}, {}, {"timezone": ""}, {"timezone": "   "}, {"timezone": None}):
            assert _tz_issues({**cfg, "model": {"provider": "nastech"}}) == []
        ...
        assert _tz_issues({"timezone": "Asia/Tokio", "model": {"provider": "nastech"}}) == []
...
class TestUnknownTopLevelKeys:
    ...
    (gateway/run.py, nastech send) so users can feed skills and external apps
