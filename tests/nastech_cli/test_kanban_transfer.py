"""Tests for kanban board export / import (``nastech_cli.kanban_transfer``).
...
from nastech_cli import kanban_transfer as kt
from nastech_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("NASTECH_KANBAN_HOME", str(root))
        for var in ("NASTECH_KANBAN_DB", "NASTECH_KANBAN_WORKSPACES_ROOT",
                    "NASTECH_KANBAN_ATTACHMENTS_ROOT", "NASTECH_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Nastech"):
