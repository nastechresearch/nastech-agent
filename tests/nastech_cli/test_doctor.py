...
from nastech_cli.doctor_config import _has_provider_env_config
import shutil
from nastech_cli import doctor_tools
from nastech_cli import doctor_state
from nastech_cli import doctor_platform
from nastech_cli import doctor_config
...
class TestDoctorPlatformHints:
    def test_termux_package_hint(self, monkeypatch):
        ...
        assert nastech_constants.is_termux() is True
    ...
    def test_sqlite_upgrade_hint_recreates_docker_containers(self, monkeypatch):
        ...
        assert "docker pull nastechresearch/nastech-agent:latest" in hint
        assert "recreate all Nastech containers" in hint
        assert "nastech update" not in hint

    def test_sqlite_upgrade_hint_keeps_git_runtime_repair(self):
        hint = doctor_platform._sqlite_upgrade_hint("git")

        assert "run `nastech update`" in hint

    def test_sqlite_upgrade_hint_uses_pkg_for_apt_managed_install(self):
        hint = doctor_platform._sqlite_upgrade_hint("apt")

        assert "run `pkg upgrade nastech-agent`" in hint
        assert "nastech update" not in hint

    def test_sqlite_upgrade_hint_preserves_nix_guidance_as_prose(self):
        from nastech_cli.config import recommended_update_command_for_method
...
class TestDoctorToolAvailabilitySummary:
    ...
    def test_image_gen_without_provider_reports_setup_hint_not_system_dependency(self, monkeypatch):
        """image_gen declares no single env var (FAL / managed Nastech / plugin providers); an
        unconfigured backend is a setup problem and must say so, and it counts toward the
        'run nastech setup' summary like any missing key (#9516)."""
        ...
        assert "nastech tools" in image_line and "system dependency" not in image_line and "unavailable" in image_line
        assert "system dependency not met" in next(line for line in out.splitlines() if "homeassistant" in line)
        assert any("nastech setup" in issue for issue in f.issues)
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `nastech doctor` crashed on Windows
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("NASTECH_KANBAN_TASK", "probe")
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_nastech_home(self, tmp_path, provider="", memory_config=None):
        """Create a minimal NASTECH_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(
        ...
        *,
        ...
        home = self._make_nastech_home(tmp_path, provider, memory_config)
        ...
        try:
            ...
            monkeypatch.setattr(_auth_mod, "get_nastech_auth_status_local", lambda: {})
...
def test_run_doctor_accepts_nastech_provider_ids_that_catalog_aliases(
...
def _doctor_env_for_agent_browser(monkeypatch, tmp_path):
    ...
    branch in run_doctor (nastech_cli/doctor.py ~1557-1605)."""
    home = tmp_path / ".nastech"
    home.mkdir(parents=True, exist_ok=True)
    (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
    project = tmp_path / "project"
...
def test_run_doctor_fix_warms_npx_cache_when_agent_browser_resolves_via_npx(
    ...
    """`nastech doctor --fix` must actually call warm_agent_browser_npx_cache()
...
def _run_doctor_with_healthy_oauth_fallback(
    ...
    *,
    ...
    monkeypatch.setattr(_auth_mod, "get_nastech_auth_status_local", lambda: {"logged_in": True})
    monkeypatch.setattr(_auth_mod, "get_codex_auth_status", lambda: {})
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from nastech_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("nastech_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "nastech_cli.doctor", raising=False)
        from nastech_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestDoctorXaiOAuthStatus:
    ...
    failure (or runtime exception) cannot silence the Nastech / Codex / Gemini /
    ...
    def _run(self, monkeypatch, tmp_path, *, xai_auth_fn) -> str:
        ...
        from nastech_cli import auth as _auth_mod
        monkeypatch.setattr(_auth_mod, "get_nastech_auth_status_local", lambda: {"logged_in": False})
    ...
    def test_import_failure_does_not_affect_other_providers(self, monkeypatch, tmp_path):
        """Nastech / Codex / Gemini / MiniMax rows must survive an xAI import failure."""
        home = tmp_path / ".nastech"
        home.mkdir(parents=True, exist_ok=True)
        (home / "config.yaml").write_text("memory: {}\n", encoding="utf-8")
        project = tmp_path / "project"
        ...
        with contextlib.redirect_stdout(buf):
            doctor_mod.run_doctor(Namespace(fix=False))
        out = buf.getvalue()
        assert "Nastech Portal auth" in out
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale NASTECH_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        if ghost is not None:
            env_lines.append(f"NASTECH_MAX_ITERATIONS={ghost}\n")
        (nastech_home / ".env").write_text("".join(env_lines), encoding="utf-8")
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("NASTECH_MAX_ITERATIONS", str(os_environ_value))
        else:
    ...
    def test_detects_drift_warn_only(self, monkeypatch, tmp_path):
        out, nastech_home = self._run_config_section(
            monkeypatch, tmp_path, fix=False, ghost=90, cfg_turns=400,
            os_environ_value=400,  # bridge contaminated os.environ
        )
        assert "NASTECH_MAX_ITERATIONS=90" in out
        assert "shadows" in out
        # Warn-only must NOT mutate .env.
        assert "NASTECH_MAX_ITERATIONS=90" in (nastech_home / ".env").read_text(encoding="utf-8")
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale NASTECH_MAX_ITERATIONS" in out
        env_after = (nastech_home / ".env").read_text(encoding="utf-8")
        assert "NASTECH_MAX_ITERATIONS" not in env_after
...
class TestDoctorDeprecatedConfigAndEnv:
    ...
    def test_nastech_tool_progress_warning_says_unsupported_since_floor(self):
        """NASTECH_TOOL_PROGRESS lost its last consumer (the retired v3→4
        ...
        findings = dict(
            doctor_config.collect_deprecated_env_vars({"NASTECH_TOOL_PROGRESS": "true"})
        )
        assert "ignored/unsupported since config floor v12" in findings["NASTECH_TOOL_PROGRESS"]
        # The MODE variant is still read by the gateway fallback → keeps the
        # plain deprecation wording.
        mode = dict(
            doctor_config.collect_deprecated_env_vars({"NASTECH_TOOL_PROGRESS_MODE": "all"})
        )
        assert mode["NASTECH_TOOL_PROGRESS_MODE"] == "display.tool_progress in config.yaml"
...
class TestMacOSTCCGrants:
    ...
    def test_warns_on_cdhash_pinned_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nastechresearch.nastech" and cdhash H"97e692f3890f781fa0ad5ad6cb9d769cfaf42628"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
    ...
    def test_ok_and_repair_info_on_identifier_dr(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nastechresearch.nastech"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        ...
        assert "tccutil reset ScreenCapture com.nastechresearch.nastech" in out
        assert "toggle" in out
        assert "relaunch" in out

    def test_ok_on_certificate_anchored_dr(self, monkeypatch, capsys, tmp_path):
        """A cert-anchored DR (nastech desktop --setup-tcc-identity, or a
        ...
        monkeypatch.setattr(
            doctor_platform,
            "_macos_desktop_dr",
            lambda app: 'designated => identifier "com.nastechresearch.nastech" and certificate root = H"aabbcc"',
        )
        doctor_platform.check_macos_tcc_grants()
        out = capsys.readouterr().out
        assert "TCC signing identity is stable" in out
