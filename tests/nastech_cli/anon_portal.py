...
exercised through Nastech' real client code, never mocked away. Scenarios flip its behaviour
...
def install_portal(monkeypatch, tmp_path, fake: FakePortal | None = None) -> FakePortal:
    """Route every Nastech HTTP client at *fake*, isolate the stores, and reset the per-process memos.

    One transport seam: ``httpx.Client`` itself, which ``auth_nastech._nastech_http_client`` and
    ``resolve_nastech_access_token`` both construct."""
    from nastech_cli import anon_auth, free_tier_bootstrap
    ...
    # resolve_nastech_access_token memoises the last token for 5 s per profile home (dict); a token minted
