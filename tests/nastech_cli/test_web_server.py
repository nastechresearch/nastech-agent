...
import nastech_cli.web_routers.chat_ws as _rt_chat_ws
import nastech_cli.web_routers.status as _rt_status
import nastech_cli.web_server_chat as _web_server_chat
import nastech_cli.web_server_config as _web_server_config
import nastech_cli.web_server_dashboard as _web_server_dashboard
import nastech_cli.web_server_files as _web_server_files
import nastech_cli.web_server_gateway as _web_server_gateway
import nastech_cli.web_server_lifecycle as _web_server_lifecycle
import nastech_cli.web_server_memory as _web_server_memory
import nastech_cli.web_server_messaging as _web_server_messaging
import nastech_cli.web_server_sessions as _web_server_sessions
...
class TestWebServerEndpoints:
    ...
    def test_startup_eager_reconcile_heals_stale_store(self):
        ...
        #79531/#80037: after `nastech update` an old-schema state.db used to
    ...
    def test_startup_eager_reconcile_is_read_only_on_a_healthy_store(self, monkeypatch):
        ...
        SessionDB(db_path=get_nastech_home() / "state.db").close()
        ...
        monkeypatch.setattr(nastech_state.SessionDB, "__init__", spy)
        _web_server_lifecycle._eager_reconcile_own_session_db()
    ...
    def test_startup_eager_reconcile_never_raises(self, monkeypatch):
        ...
        monkeypatch.setattr(nastech_state, "SessionDB", boom)
    ...
    def test_generic_corruption_does_not_trigger_writable_heal(
        ...
        monkeypatch.setattr(nastech_state, "SessionDB", corrupt_open)
    ...
    def test_decode_error_triggers_writable_heal(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nastech_state, "SessionDB", scripted_open)
    ...
    def test_update_nastech_returns_docker_guidance_without_spawning(self, monkeypatch):
        ...
        # method through nastech_cli.config directly.
        monkeypatch.setattr(
            "nastech_cli.config.detect_install_method", lambda *_a, **_k: "docker"
        )
        monkeypatch.setattr(_cfg_mod, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(_web_server_gateway, "_spawn_nastech_action", fail_spawn)
        _web_server_gateway._ACTION_PROCS.pop("nastech-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("nastech-update", None)
    ...
    def test_update_nastech_returns_apt_guidance_without_spawning(self, monkeypatch):
        import nastech_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("APT-managed update guard should not spawn nastech update")

        monkeypatch.setattr(_web_server_files, "_dashboard_local_update_managed_externally", lambda: False)
        # The shared admission gate (#91277 Phase 3) resolves the install
        # method through nastech_cli.config directly, so patch it there (the
        # web_server module alias only feeds the /update/check endpoint).
        monkeypatch.setattr(
            "nastech_cli.config.detect_install_method", lambda *_a, **_k: "apt"
        )
        ...
        assert data["update_command"] == "pkg upgrade nastech-agent"
        assert spawned is False

        check = self.client.get("/api/nastech/update/check")
        assert check.status_code == 200
        check_data = check.json()
        assert check_data["install_method"] == "apt"
        assert check_data["can_apply"] is False
        assert check_data["update_command"] == "pkg upgrade nastech-agent"
    ...
    def test_update_status_recovers_completed_result_after_dashboard_restart(self, monkeypatch, tmp_path):
        ...
        action_id = "c" * 32
        (tmp_path / "nastech-update.log").write_text(
            "=== nastech-update started 2026-08-17 11:19:34 ===\n"
            "pulling updates...\n",
            encoding="utf-8",
        )
        (tmp_path / "update.log").write_text(
            "=== nastech update started 2026-08-17T11:19:35 ===\n"
            "✓ Update complete!\n"
            f"=== nastech-update completed {action_id} ===\n",
            encoding="utf-8",
        )
        ...
        status = self.client.get("/api/actions/nastech-update/status?lines=2000")
        ...
        assert f"=== nastech-update completed {action_id} ===" in data["lines"]
    ...
    def test_update_nastech_spawns_with_action_id(self, monkeypatch):
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_nastech_action", fake_spawn)
        _web_server_gateway._ACTION_PROCS.pop("nastech-update", None)
        _web_server_gateway._ACTION_RESULTS.pop("nastech-update", None)
    ...
    def test_update_nastech_reuses_running_action(self, monkeypatch):
        ...
        _web_server_gateway._ACTION_PROCS["nastech-update"] = Proc()
        _web_server_gateway._ACTION_IDS["nastech-update"] = "b" * 32

        try:
            resp = self.client.post("/api/nastech/update")
        finally:
            _web_server_gateway._ACTION_PROCS.pop("nastech-update", None)
            _web_server_gateway._ACTION_IDS.pop("nastech-update", None)
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        ...
        """
        from nastech_cli.web_server_messaging import _MESSAGING_KEYS_PAGE_KEYS, _build_catalog_entry, _channel_managed_env_keys
    ...
    def test_model_set_flips_a_stale_setup_record(self, monkeypatch):
        ...
        fb.reset_for_tests()
        monkeypatch.setattr("nastech_cli.model_cost_guard.expensive_model_warning", lambda *_a, **_k: None)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        monkeypatch.setattr(_web_server_gateway, "_spawn_nastech_action", fail_spawn_action)
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from nastech_cli.web_server_profiles import _parse_model_ids
    ...
    def test_numeric_yaml_provider_key_can_be_activated_and_deleted(self):
        ...
        """
        from nastech_cli.config import get_config_path, load_config
    ...
    def test_custom_endpoint_save_scopes_to_the_requested_profile(self):
        ...
        """
        from nastech_cli import profiles as profiles_mod
        from nastech_cli.config import custom_endpoint_key_env
        from nastech_constants import get_nastech_home
...
class TestBuildSchemaFromConfig:
    ...
    def test_timezone_field_is_searchable_select(self):
        ...
        """
        from nastech_cli.web_server_config import CONFIG_SCHEMA, _timezone_options
...
class TestNewEndpoints:
    ...
    def test_get_toolset_config_reports_truthful_provider_status(self, monkeypatch):
        ...
        import nastech_cli.tools_config_post_setup as tools_config_post_setup
    ...
    def test_select_managed_nastech_provider_reports_needs_nastech_auth(self, monkeypatch):
        ...
        # Managed rows store the single 'nastech' provider string (the runtime
        # maps it to the Browser Use cloud through the Nastech Tool Gateway).
        from nastech_cli.config import load_config
        cfg = load_config()
        assert cfg["browser"]["cloud_provider"] == "nastech"
...
class TestDesktopLoopbackAuthExemption:
    ...
    def test_exempt_with_desktop_env_and_session_token_on_loopback(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_DASHBOARD_SESSION_TOKEN", "desktop-minted")
    ...
    def test_not_exempt_without_desktop_env(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_DASHBOARD_SESSION_TOKEN", "tok")
    ...
    def test_not_exempt_without_any_credential(self, monkeypatch):
        ...
        # NASTECH_DESKTOP=1 alone is not enough: a plain serve with the env var
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from nastech_cli.web_server_config import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from nastech_cli.web_server_config import _denormalize_config_from_web
...
class TestDenormalizeProviderSwitch:
    ...
    def test_vendor_slug_switches_off_non_aggregator_provider(self):
        ...
        with _patch("nastech_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter"):
    ...
    def test_rejected_switch_is_400_and_leaves_the_model_block_byte_identical(self, monkeypatch):
        ...
        cfg_path = get_nastech_home() / "config.yaml"
        cfg_path.write_text(
            "model:\n"
            "  default: llama3.2\n"
            "  provider: ollama-local\n"
        ...
        monkeypatch.setattr("nastech_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter")
        monkeypatch.setattr("nastech_cli.model_switch.switch_model",
...
class TestStatusInstallId:
    ...
    Nastech home (not the profile home), survives a fresh process-cache read,
    ...
    def test_status_reports_persistent_install_id(self, monkeypatch):
        ...
        id_file = get_default_nastech_root() / "install_id"
    ...
    def test_all_profiles_of_one_install_share_the_id(self, monkeypatch, tmp_path):
        """NASTECH_HOME=<root> and NASTECH_HOME=<root>/profiles/<name> resolve to
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_non_dict(self):
        from nastech_cli.web_server_dashboard import _normalise_theme_definition
...
class TestDashboardPluginManifestExtensions:
    ...
    def test_unreadable_plugin_paths_do_not_block_discovery(self, tmp_path, monkeypatch, caplog):
        ...
        from nastech_cli import web_server_dashboard
...
import sys
from nastech_cli import main_tui_launch
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import nastech_cli.main_tui_launch as tui_launch
    import nastech_cli.web_server as ws
...
class TestServeIndexMissingIndex:
    ...
    def test_index_uses_ssh_token_applied_after_spa_mount(
        ...
        assert resp.status_code == 200
        assert 'window.__NASTECH_SESSION_TOKEN__="after-mount"' in resp.text


class TestHeadlessServeTokenPage:
    """Headless `nastech serve` must serve the Desktop token handshake page
    at `/` when the dashboard auth gate is off (#94227).

    The Electron renderer boots by fetching `/` and extracting
    ``window.__NASTECH_SESSION_TOKEN__`` for WebSocket auth. Headless serve
    ...
    def _headless_client(monkeypatch, *, gated: bool):
        ...
        monkeypatch.setenv("NASTECH_SERVE_HEADLESS", "1")
        spa_app = FastAPI()
    ...
    def test_root_serves_token_page_when_not_gated(self, monkeypatch):
        ...
        match = re.search(
            r'window\.__NASTECH_SESSION_TOKEN__\s*=\s*("(?:\\.|[^"\\])*")',
            resp.text,
        )
        ...
        assert "window.__NASTECH_AUTH_REQUIRED__=false" in resp.text
