"""Nastech free tier core: identity lifecycle, token-acquisition seam, routing pin, opt-out.
...
def _write_config(monkeypatch, **nastech):
    home = Path(os.environ["NASTECH_HOME"])
    (home / "config.yaml").write_text("nastech:\n" + "".join(f"  {k}: {str(v).lower()}\n" for k, v in nastech.items()))
...
def _shared_store(tmp_path) -> dict:
    p = tmp_path / "shared-store" / "nastech_auth.json"
...
class TestIdentityLifecycle:
    def test_fresh_install_mints_once_and_is_the_active_provider(self, portal, tmp_path):
        ...
        assert store["active_provider"] == "nastech"
        assert anon_auth.is_guest_state(store["providers"]["nastech"])
    ...
    def test_second_profile_under_same_root_adopts_from_shared_store(self, portal, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(other_home))
        before = len(portal.calls)
    ...
    def test_gate_closed_persists_nothing_and_raises_gate_code(self, portal):
        ...
        assert "nastech" not in _load_auth_store().get("providers", {})
    ...
    def test_launch_gate_off_means_no_free_tier_at_all(self, portal, monkeypatch):
        """Without ``NASTECH_GUEST_ONBOARDING=1`` the free tier does not exist: no mint, no portal
        traffic, ``nastech.guest``'s default is never consulted, and an identity already on disk is
        not treated as enabled. The env var is the only lever; ``0``/``true``/anything but ``1`` is off."""
        monkeypatch.setattr("agent.bedrock_adapter.has_aws_credentials", lambda: False)
        for raw in ("", "0", "true", "yes", "new"):
            monkeypatch.setenv("NASTECH_GUEST_ONBOARDING", raw)
...
class TestExplicitProvision:
    ...
    so two boots never create two identities. ``nastech.guest: false`` still wins."""
    ...
    def test_provision_mints_once_then_everything_adopts(self, portal, monkeypatch, tmp_path):
        ...
        assert portal.minted == 1
        token = _load_auth_store()["providers"]["nastech"]["anon_token"]
        # A second provision is idempotent, and the runtime now serves nastech/welcome on the welcome host.
        anon_auth.ensure_portal_identity(explicit=True)
        from nastech_cli.runtime_provider import resolve_runtime_provider
        runtime = resolve_runtime_provider(requested="nastech", target_model=anon_auth.GUEST_MODEL)
        assert runtime["base_url"].rstrip("/") == WELCOME
        assert resolve_provider("auto") == "nastech"
        # A sibling profile adopts the same identity implicitly; no second create call.
        sibling = tmp_path / "sibling-profile"
        sibling.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(sibling))
    ...
    def test_retired_identity_is_replaced(self, portal, monkeypatch):
        anon_auth.ensure_portal_identity(explicit=True)
        first = _load_auth_store()["providers"]["nastech"]["anon_token"]
        portal.dead_tokens.add(first)
        from nastech_cli.auth_nastech import resolve_nastech_runtime_credentials
        assert resolve_nastech_runtime_credentials(force_refresh=True)["api_key"]   # replaced, not refused
        assert _load_auth_store()["providers"]["nastech"]["anon_token"] != first
...
class TestResolverIsUnchanged:
    ...
    def test_runtime_routes_to_welcome_host(self, portal):
        ...
        assert runtime["provider"] == "nastech"
        assert runtime["base_url"].rstrip("/") == WELCOME
...
class TestRouteFallback:
    ...
    def test_exchange_without_inference_url_routes_to_welcome_literal(self, portal):
        ...
        runtime = resolve_runtime_provider()
        assert runtime["base_url"].rstrip("/") == WELCOME
        state = _load_auth_store()["providers"]["nastech"]
    ...
    def test_guest_state_without_url_never_resolves_to_the_paid_host(self, portal):
        from nastech_cli.auth_nastech import _nastech_effective_routing
        guest = {"auth_method": "anonymous", "anon_token": "anon_x"}
        _portal, stored, effective, _client = _nastech_effective_routing(guest)
        assert stored.rstrip("/") == WELCOME and effective.rstrip("/") == WELCOME
        _portal, stored, _effective, _client = _nastech_effective_routing({"refresh_token": "r"})
        assert stored.rstrip("/") == "https://inference-api.nastechresearch.github.io/v1"

    def test_shared_store_shape_keeps_a_guest_on_the_welcome_host(self, portal):
        from nastech_cli.auth_nastech import _nastech_shared_shape
        shape = _nastech_shared_shape({"auth_method": "anonymous", "anon_token": "anon_x"})
...
class TestTokenAcquisitionSeam:
    def test_expired_guest_jwt_reexchanges_and_never_hits_oauth_token(self, portal):
        anon_auth.ensure_portal_identity(explicit=True)
        from nastech_cli.auth import _auth_store_lock, _save_auth_store
        with _auth_store_lock():
            store = _load_auth_store()
            store["providers"]["nastech"]["access_token"] = _jwt(exp=int(time.time()) - 10)
            store["providers"]["nastech"]["expires_at"] = "2000-01-01T00:00:00+00:00"
    ...
    def test_dead_credential_is_replaced_by_a_fresh_identity(self, portal):
        ...
        from nastech_cli.auth_nastech import resolve_nastech_runtime_credentials
        creds = resolve_nastech_runtime_credentials(force_refresh=True)
    ...
    def test_tool_gateway_token_path_reexchanges(self, portal):
        anon_auth.ensure_portal_identity(explicit=True)
        from nastech_cli.auth import _auth_store_lock, _save_auth_store, resolve_nastech_access_token
        with _auth_store_lock():
            store = _load_auth_store()
            store["providers"]["nastech"]["expires_at"] = "2000-01-01T00:00:00+00:00"
            _save_auth_store(store)
        portal.calls.clear()
        token = resolve_nastech_access_token()
        assert token
...
class TestModelPin:
    ...
    def test_pin_keys_on_the_welcome_host_not_on_guest_state(self, portal):
        ...
        assert not anon_auth.route_is_welcome_host("https://inference-api.nastechresearch.github.io/v1")
        assert not anon_auth.route_is_welcome_host("")

    def test_agent_init_pins_only_on_welcome_route(self, portal):
        anon_auth.ensure_portal_identity(explicit=True)
        from run_agent import AIAgent
        welcome = AIAgent(provider="nastech", base_url=WELCOME, api_key="k", model="openai/gpt-5",
                          quiet_mode=True, skip_context_files=True, skip_memory=True)
        paid = AIAgent(provider="nastech", base_url="https://inference-api.nastechresearch.github.io/v1", api_key="k",
                       model="nastech/paid-model", quiet_mode=True, skip_context_files=True, skip_memory=True)
        assert welcome.model == anon_auth.GUEST_MODEL
        assert paid.model == "nastech/paid-model"


class TestLogout:
    def test_logout_with_only_free_tier_is_a_true_noop(self, portal, capsys):
        from types import SimpleNamespace
        from nastech_cli.auth import _auth_file_path, logout_command
        anon_auth.ensure_portal_identity(explicit=True)
    ...
    def test_logout_of_real_account_clears_shared_store(self, portal, tmp_path):
        ...
        from nastech_cli.auth_nastech import persist_nastech_credentials
        persist_nastech_credentials({"access_token": _jwt(client_id="nastech-cli", account_tier="free"),
        ...
        logout_command(SimpleNamespace(provider="nastech"))
...
class TestModelSwitchCopy:
    def test_switching_away_from_welcome_names_the_account_path_not_another_provider(self, portal, monkeypatch):
        ...
        result = model_switch.switch_model("gpt-5", "nastech", anon_auth.GUEST_MODEL, WELCOME)
...
class TestRotationNeverRewritesTheConversationModel:
    ...
    def _agent(self, api_mode="chat_completions", model="nastech/paid-model"):
        from types import SimpleNamespace
        return SimpleNamespace(provider="nastech", api_mode=api_mode, base_url="https://inference-api.nastechresearch.github.io/v1",
    ...
    def test_paid_conversation_refuses_a_welcome_route_on_every_wire_mode(self, portal):
        ...
        for mode, model in (("chat_completions", "nastech/paid-model"), ("anthropic_messages", "anthropic/claude-sonnet")):
    ...
    def test_welcome_conversation_may_move_to_the_portal_host(self, portal):
        ...
        ok = ClientLifecycleMixin._swap_credential(agent, SimpleNamespace(id="p2", runtime_api_key="key", runtime_base_url="https://inference-api.nastechresearch.github.io/v1"))
...
class TestBootstrapIsTheOneCreator:
    ...
    def _fresh(self):
        from nastech_cli import free_tier_bootstrap as fb
        fb.reset_for_tests()
        return fb
    ...
    def test_bootstrap_mints_once_records_and_a_second_run_is_free(self, portal):
        ...
        assert record.inference_provider == "nastech" and record.other_providers is False
        assert portal.minted == 1
    ...
    def test_own_key_keeps_inference_and_the_identity_stays_off_active_provider(self, portal, monkeypatch):
        ...
        assert record.inference_provider != "nastech"
        assert _load_auth_store().get("active_provider") != "nastech", "a mint beside an own key must not hijack inference"
        assert portal.minted == 1
    ...
    def test_reads_never_mint(self, portal, monkeypatch):
        ...
        monkeypatch.setattr("agent.bedrock_adapter.has_aws_credentials", lambda: False)
        from tools.managed_tool_gateway import read_nastech_access_token
        assert read_nastech_access_token() is None
        with pytest.raises(anon_auth.AuthError):
            resolve_provider("auto")
...
class TestIdentityOfRecordIsTheSharedStore:
    def test_stale_profile_guest_adopts_a_newer_shared_account(self, portal, tmp_path):
        ...
        _write_shared_nastech_state({"access_token": _jwt(client_id="nastech-cli", account_tier="free"),
        ...
        assert _load_auth_store()["providers"]["nastech"]["refresh_token"] == "rt-sibling"
    ...
    def test_clearing_a_dead_guest_leaves_a_sibling_identity_alone(self, portal, tmp_path):
        anon_auth.ensure_portal_identity(explicit=True)
        from nastech_cli.auth_nastech import _write_shared_nastech_state
        _write_shared_nastech_state({"access_token": _jwt(client_id="nastech-cli"), "refresh_token": "rt-sibling",
    ...
    def test_lock_order_is_profile_then_shared(self, portal, monkeypatch):
        order = []
        from nastech_cli import auth as auth_mod, auth_nastech
        real_profile, real_shared = auth_mod._auth_store_lock, auth_nastech._nastech_shared_store_lock
        ...
        monkeypatch.setattr(auth_nastech, "_nastech_shared_store_lock", shared)
        anon_auth.ensure_portal_identity(explicit=True)
...
class TestConnectorTokenPath:
    def test_opt_out_hides_the_free_tier_from_connectors_including_cached_tokens(self, portal, monkeypatch):
        anon_auth.ensure_portal_identity(explicit=True)
        from nastech_cli.auth_nastech import resolve_nastech_runtime_credentials
        resolve_nastech_runtime_credentials()  # now a cached, valid JWT exists
        from tools import managed_tool_gateway as mtg
        assert mtg.read_nastech_access_token()
        _write_config(monkeypatch, guest=False)
        assert mtg.peek_nastech_access_token() is None
        assert mtg.read_nastech_access_token() is None
    ...
    def test_connector_path_replaces_a_dead_credential_once(self, portal):
        ...
        with _auth_store_lock():
            store = _load_auth_store()
            store["providers"]["nastech"]["expires_at"] = "2000-01-01T00:00:00+00:00"
            store["providers"]["nastech"]["access_token"] = _jwt(exp=1)
            _save_auth_store(store)
        portal.dead_tokens.add(first["anon_token"])
        from tools import managed_tool_gateway as mtg
        token = mtg.read_nastech_access_token()
        assert token and token != _jwt(exp=1)
        assert _load_auth_store()["providers"]["nastech"]["anon_token"] != first["anon_token"]
        assert portal.minted == 2
