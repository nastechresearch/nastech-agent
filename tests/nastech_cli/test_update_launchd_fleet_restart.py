"""Regression for #41403 — ``nastech update`` must restart ALL macOS launchd gateways.
...
Sibling ``ai.nastech.gateway-<profile>`` services kept running pre-update
...
class TestLaunchdGatewayLabelsForInstall:
    def test_labels_derive_from_this_installs_profiles(self, monkeypatch):
        ...
        the shared per-user LaunchAgents dir. A sandboxed NASTECH_HOME (tests,
        ...
        assert launchd_gateway_labels_for_install() == [
            "ai.nastech.gateway",
            "ai.nastech.gateway-merit-ops",
            "ai.nastech.gateway-tfl-wiki",
        ]

    def test_no_profiles_means_no_fleet(self, monkeypatch):
        monkeypatch.setattr(nastech_cli.profiles, "list_profiles", lambda: [])
        assert launchd_gateway_labels_for_install() == []
...
class TestLocateLaunchdGatewayService:
    def test_domains_resolve_per_label_not_from_cache(self, monkeypatch):
        ...
        gui_loaded = {"ai.nastech.gateway-a"}
        ...
        assert _locate_launchd_gateway_service("ai.nastech.gateway-a") == (
            f"gui/{UID}",
            4242,
        )
        assert _locate_launchd_gateway_service("ai.nastech.gateway-b") == (
            f"user/{UID}",
            4242,
        )
    ...
    def test_loaded_without_live_process(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.nastech.gateway-x") == (
            f"gui/{UID}",
            None,
        )
    ...
    def test_not_loaded_in_either_domain(self, monkeypatch):
        ...
        assert _locate_launchd_gateway_service("ai.nastech.gateway-x") == (None, None)
    ...
    def test_timeout_propagates_to_caller(self, monkeypatch):
        ...
        with pytest.raises(subprocess.TimeoutExpired):
            _locate_launchd_gateway_service("ai.nastech.gateway-x")
...
class TestProbeLaunchdDomainForLabel:
    def test_unloaded_label_falls_back_to_managername(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.nastech.gateway-x") == f"gui/{UID}"
    ...
    def test_unloaded_label_defaults_to_user_domain(self, monkeypatch):
        ...
        monkeypatch.setattr(gw.subprocess, "run", fake_run)
        assert _probe_launchd_domain_for_label("ai.nastech.gateway-x") == f"user/{UID}"
...
class TestGetServicePidsScoping:
    def _wire(self, monkeypatch):
        ...
        # box with a live ai.nastech.gateway* fleet would leak its PIDs into the assertion.
        monkeypatch.setattr(gw.subprocess, "run", lambda *a, **k: _completed(0, ""))
        monkeypatch.setattr(gw, "get_launchd_label", lambda: "ai.nastech.gateway")
        monkeypatch.setattr(
            gw,
            "launchd_gateway_labels_for_install",
            lambda: ["ai.nastech.gateway", "ai.nastech.gateway-a", "ai.nastech.gateway-b"],
        )
        located = {
            "ai.nastech.gateway": (f"gui/{UID}", 100),
            "ai.nastech.gateway-a": (f"gui/{UID}", 200),
            "ai.nastech.gateway-b": (None, None),  # not bootstrapped
        }
        monkeypatch.setattr(
...
def _fleet(monkeypatch, tmp_path, *, current, labels, located,
    ...
    """Wire a fake launchd fleet through nastech_cli.gateway seams.
...
class TestRestartMacosLaunchdGateways:
    def test_current_delegates_and_siblings_kickstart_in_own_domains(
        ...
        current = "ai.nastech.gateway-merit-ops"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=["ai.nastech.gateway", current, "ai.nastech.gateway-user-scoped"],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                current: (f"gui/{UID}", 200),
                "ai.nastech.gateway-user-scoped": (f"user/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == [
            f"gui/{UID}/ai.nastech.gateway",
            f"user/{UID}/ai.nastech.gateway-user-scoped",
        ]
        assert rec.waits == [
            f"gui/{UID}/ai.nastech.gateway",
            f"user/{UID}/ai.nastech.gateway-user-scoped",
        ...
        assert restarted == [
            current,
            "ai.nastech.gateway",
            "ai.nastech.gateway-user-scoped",
        ]
        assert failed == []
    ...
    def test_current_profile_without_plist_makes_no_launchctl_calls(
        ...
        still processed."""
        current = "ai.nastech.gateway"
        rec = _fleet(
            monkeypatch,
            tmp_path,
            current=current,
            labels=[current, "ai.nastech.gateway-a"],
            located={"ai.nastech.gateway-a": (f"gui/{UID}", 200)},
            plist_exists=False,
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.nastech.gateway-a"]
        assert failed == []
    ...
    def test_unbootstrapped_sibling_is_skipped_not_failed(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nastech.gateway",
            labels=["ai.nastech.gateway", "ai.nastech.gateway-idle"],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-idle": (None, None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.kickstarts == []
        assert restarted == ["ai.nastech.gateway"]
        assert failed == []
    ...
    def test_loaded_but_not_running_sibling_is_kickstarted(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nastech.gateway",
            labels=["ai.nastech.gateway", "ai.nastech.gateway-dormant"],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-dormant": (f"gui/{UID}", None),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert rec.drains == []
        assert rec.kickstarts == [f"gui/{UID}/ai.nastech.gateway-dormant"]
        assert restarted == ["ai.nastech.gateway", "ai.nastech.gateway-dormant"]
        assert failed == []
    ...
    def test_graceful_drain_with_keepalive_respawn_skips_kickstart(
        ...
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nastech.gateway",
            labels=["ai.nastech.gateway", "ai.nastech.gateway-a"],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-a": (f"gui/{UID}", 200),
            },
        ...
        assert rec.kickstarts == []
        assert rec.waits == [f"gui/{UID}/ai.nastech.gateway-a"]
        assert restarted == ["ai.nastech.gateway", "ai.nastech.gateway-a"]
        assert failed == []
    ...
    def test_kickstart_failure_is_recorded_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.nastech.gateway",
                "ai.nastech.gateway-bad",
                "ai.nastech.gateway-good",
            ],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-bad": (f"gui/{UID}", 200),
                "ai.nastech.gateway-good": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.nastech.gateway-bad": subprocess.CalledProcessError(
        ...
        assert failed == ["ai.nastech.gateway-bad"]
        assert rec.kickstarts == [f"gui/{UID}/ai.nastech.gateway-good"]
        assert restarted == ["ai.nastech.gateway", "ai.nastech.gateway-good"]
    ...
    def test_timeout_during_discovery_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            labels=[
                "ai.nastech.gateway",
                "ai.nastech.gateway-wedged",
                "ai.nastech.gateway-after",
            ],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-wedged": subprocess.TimeoutExpired(
                    cmd=["launchctl", "print"], timeout=5
                ),
                "ai.nastech.gateway-after": (f"gui/{UID}", 300),
            },
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert failed == ["ai.nastech.gateway-wedged"]
        assert rec.kickstarts == [f"gui/{UID}/ai.nastech.gateway-after"]
        assert restarted == ["ai.nastech.gateway", "ai.nastech.gateway-after"]
    ...
    def test_timeout_during_kickstart_is_failed_and_rest_continue(
        ...
        rec = _fleet(
            ...
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-wedged": (f"gui/{UID}", 200),
                "ai.nastech.gateway-after": (f"gui/{UID}", 300),
            },
            kick_errors={
                "ai.nastech.gateway-wedged": subprocess.TimeoutExpired(
    ...
    def test_sibling_that_never_comes_back_is_failed(self, monkeypatch, tmp_path):
        rec = _fleet(
            ...
            tmp_path,
            current="ai.nastech.gateway",
            labels=["ai.nastech.gateway", "ai.nastech.gateway-zombie"],
            located={
                "ai.nastech.gateway": (f"gui/{UID}", 100),
                "ai.nastech.gateway-zombie": (f"gui/{UID}", 200),
            },
            wait_results={"ai.nastech.gateway-zombie": False},
        )
        restarted: list[str] = []
        failed: list[str] = []
        ...
        assert restarted == ["ai.nastech.gateway"]
        assert failed == ["ai.nastech.gateway-zombie"]
...
class TestWaitForLaunchdServicePid:
    def test_returns_true_once_pid_changes(self, monkeypatch):
        ...
        assert gw._wait_for_launchd_service_pid(
            "ai.nastech.gateway-x", old_pid=200, timeout=5.0, domain=f"gui/{UID}"
        )
    ...
    def test_returns_false_when_pid_never_changes(self, monkeypatch):
        ...
        assert not gw._wait_for_launchd_service_pid(
            "ai.nastech.gateway-x", old_pid=200, timeout=3.0, domain=f"gui/{UID}"
        )
...
class TestIncompleteWarningOnMacos:
    ...
    def test_launchd_labels_get_bootstrap_hint(self, capsys):
        _warn_incomplete_gateway_fleet_restart(["ai.nastech.gateway-merit-ops"])
