...
2. ``cli_main._refresh_windows_gateway_launchers`` — ``nastech
...
def test_update_launcher_refresh_reregisters_drifted_scheduled_task(monkeypatch):
    """``nastech update`` must not only rewrite the launcher scripts but also re-register a Scheduled
    ...
    assert reconciled == ["Nastech_Gateway"]
