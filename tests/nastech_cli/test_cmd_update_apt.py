"""APT-managed Nastech installs must never fall through to the git updater."""
...
def test_apt_stamp_is_detected_and_recommends_pkg_upgrade(tmp_path):
    ...
    assert recommended_update_command_for_method("apt") == "pkg upgrade nastech-agent"


@patch("nastech_cli.config.is_managed", return_value=False)
@patch("nastech_cli.config.detect_install_method", return_value="apt")
...
def test_cmd_update_apt_prints_pkg_guidance_without_git(
    ...
    assert "pkg upgrade nastech-agent" in capsys.readouterr().out
