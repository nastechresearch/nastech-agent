...
class TestPsFallbackBsdCompat:
    ...
    def test_ps_flags_are_bsd_compatible(self):
        ...
        with (
            patch("nastech_cli.gateway.is_windows", return_value=False),
...
class TestGetServicePidsAllProfiles:
    ...
    def test_default_scope_uses_current_profile_label(self):
        ...
        with (
            patch("nastech_cli.gateway.is_macos", return_value=True),
            patch("nastech_cli.gateway.supports_systemd_services", return_value=False),
            patch(
                "nastech_cli.gateway.get_launchd_label",
                return_value="ai.nastech.gateway.myprofile",
            ),
            patch(
                "nastech_cli.gateway._locate_launchd_gateway_service",
        ...
        assert located == ["ai.nastech.gateway.myprofile"]
        launchctl_calls = [
            c[0][0]
    ...
    def test_all_profiles_enumerates_all_gateway_labels(self):
        ...
        widens the EXCLUDE set with unmapped ai.nastech.gateway* agents
        (#74075 belt-and-suspenders)."""
        located = []
        label_pids = {
            "ai.nastech.gateway": 123,
            "ai.nastech.gateway-profile-b": 456,
        }
        ...
        with (
            ...
            patch(
                "nastech_cli.gateway.get_launchd_label",
                return_value="ai.nastech.gateway",
            ),
            patch(
                "nastech_cli.gateway.launchd_gateway_labels_for_install",
                return_value=["ai.nastech.gateway", "ai.nastech.gateway-profile-b"],
            ),
            patch(
                "nastech_cli.gateway._locate_launchd_gateway_service",
                side_effect=_fake_locate,
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "999\t0\tai.nastech.gateway-unmapped\n"
    ...
    def test_all_profiles_empty_when_no_gateway_labels(self):
        """When no ai.nastech.gateway* labels exist, all_profiles returns empty."""
        with (
            patch("nastech_cli.gateway.is_macos", return_value=True),
            patch("nastech_cli.gateway.supports_systemd_services", return_value=False),
            patch("subprocess.run") as mock_run,
    ...
    def test_all_profiles_handles_broken_pid_column(self):
        ...
        with (
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "-\t0\tai.nastech.gateway.broken\n"
                    "  123  \t0\tai.nastech.gateway.ok\n"
                ),
                stderr="",
            )
            pids = gateway_mod._get_service_pids(all_profiles=True)
    ...
    def test_all_profiles_preserves_systemd_behavior(self):
        ...
        lists every nastech-gateway* unit unconditionally."""
        with (
