...
and the ``nastech kanban repair`` CLI verb."""
...
# ---------------------------------------------------------------------------
# repair_db() API + `nastech kanban repair` CLI verb
# ---------------------------------------------------------------------------

def _run_kanban_cli(argv: list[str]) -> int:
    """Drive the real argparse surface exactly like `nastech kanban …`."""
    import argparse
...
def cli_home(tmp_path, monkeypatch):
    """Isolated NASTECH_HOME so kanban_db_path() resolves inside tmp_path."""
