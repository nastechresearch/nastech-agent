...
External secret backends are useful during normal Nastech startup, but the
...
""",
    ...
    probe = (
        "import json, sys\n"
        f"sys.argv = {argv!r}\n"
        "import nastech_cli.main as nastech_main\n"
        f"{dispatch}"
    ...
    result = subprocess.run(
        ...
        timeout=120,
        cwd=REPO_ROOT,
        env={**os.environ, "NASTECH_HOME": str(home), "BWS_ACCESS_TOKEN": ""},
    )
...
def test_normal_startup_still_loads_enabled_external_secret_source(tmp_path):
    loaded = _probe_startup_modules(tmp_path, ["nastech", "chat"])
...
def test_dotenv_loading_is_preserved_when_external_secrets_are_skipped(
    ...
    loaded = env_loader.load_nastech_dotenv(
        nastech_home=home,
