...
import nastech_cli.main_install_repair as nastech_cli_main_install_repair
...
def _stub_install_env(monkeypatch, m, seen):
    ...
    monkeypatch.setattr(nastech_cli_main_install_repair, "_is_termux_env", lambda *a, **k: False)
    monkeypatch.setattr("nastech_cli.managed_uv.ensure_uv", lambda: None)
    # The install executor moved to nastech_cli._install_repair (shared between
...
def test_recovery_self_lock_does_not_clear_core_marker_via_import_probes(
    ...
    monkeypatch.setattr(nastech_cli_main_install_repair, "_is_windows", lambda: True)
    monkeypatch.setattr(m, "_venv_scripts_dir", lambda: scripts_dir)
    monkeypatch.setattr(nastech_cli_main_install_repair, "_venv_scripts_dir", lambda: scripts_dir)
    ...
    monkeypatch.setattr(
        nastech_cli_main_install_repair, "_repair_venv_via_import_probes", lambda *a, **k: "healthy"
    )
