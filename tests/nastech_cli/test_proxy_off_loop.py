"""`nastech proxy` must resolve upstream credentials off the event loop.
...
blocking I/O:
  ...
  * ``NastechPortalAdapter.get_credential`` takes ``_auth_store_lock()``, a
    *cross-process* advisory lock with ``AUTH_LOCK_TIMEOUT_SECONDS = 15.0``
    (``nastech_cli/auth.py:110``), reads ``auth.json`` from disk, and may issue a
    token-refresh POST. Its terminal-error path takes that lock a second time to
    persist the quarantined state.
  * ``NastechPortalAdapter.get_retry_credential`` routes to that same
...
The harness mirrors ``tests/nastech_cli/test_proxy.py``: the proxy and a fake
...
def test_get_retry_credential_runs_off_the_event_loop():
    ...
    ``NastechPortalAdapter`` routes it into ``_get_credential(force_refresh=True)``,
...
def test_is_authenticated_runs_off_the_event_loop():
    ...
    calls", but ``NastechPortalAdapter`` implements it via ``_read_state()``, which
