...
def test_add_priority_places_reauthenticated_row_in_multi_entry_pool(monkeypatch):
    ...
    write_credential_pool("nastech", rows)
    monkeypatch.setattr(auth_commands.auth_mod, "_read_shared_nastech_state", lambda: None)
    monkeypatch.setattr(auth_commands.auth_mod, "_nastech_device_code_login", lambda **_kwargs: {
    ...
    auth_commands.auth_add_command(SimpleNamespace(
        provider="nastech", auth_type="oauth", priority=0, label="reauthenticated"))
    entries = read_credential_pool("nastech")
...
def test_refresh_rejects_ambiguous_and_non_oauth_targets():
    ...
    # Nastech's resolver refreshes only its singleton, never an independent pool grant.
    write_credential_pool("nastech", _rows())
    before = read_credential_pool("nastech")
    with pytest.raises(SystemExit, match="not a refreshable"):
        auth_commands.auth_refresh_command(SimpleNamespace(provider="nastech", target="row0"))
    assert read_credential_pool("nastech") == before
