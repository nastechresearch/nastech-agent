...
def _logged_out(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.get_auth_status", lambda p: {"logged_in": False})
...
def _free_tier(monkeypatch, *, on=True, identity=False):
    ...
    monkeypatch.setattr("nastech_cli.anon_auth.guest_enabled", lambda: on)
    monkeypatch.setattr("nastech_cli.anon_auth.has_guest", lambda: identity)
    monkeypatch.setattr("nastech_cli.anon_auth.ensure_portal_identity",
...
class TestFreeTierBeatsImplicitHostCredentials:
    ...
    @pytest.mark.parametrize("free_tier_on, identity, env_key, login, expected", [
        (True, True, None, None, "nastech"),                 # existing identity beats the AWS chain
