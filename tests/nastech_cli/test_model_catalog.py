"""Tests for nastech_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate NASTECH_HOME + reset any module-level catalog cache per test."""
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nastech": {
                "metadata": {"display_name": "Nastech Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from nastech_cli.model_catalog import _validate_manifest
...
class TestCuratedAccessors:
    ...
    def test_nastech_returns_none_when_catalog_empty(self, isolated_home):
        from nastech_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_nastech_models() is None
...
class TestDefaultModelFromCache:
    ...
    def _manifest_with_default(self) -> dict:
        ...
        m["providers"]["nastech"]["models"][1]["default"] = True  # kimi-k2.6
        return m
    ...
    def test_reads_label_from_disk_cache(self, isolated_home):
        ...
        with patch.object(model_catalog, "_fetch_manifest") as fetch:
            ...
            assert (
                model_catalog.get_default_model_from_cache("nastech")
    ...
    def test_shipped_manifest_labels_glm52_default(self, isolated_home):
        ...
        for provider in ("openrouter", "nastech"):
...
class TestIntegrationWithModelsModule:
    ...
    def test_picker_nastech_row_uses_curated_list(self, tmp_path, monkeypatch):
        """The /model picker surfaces the curated ``_PROVIDER_MODELS["nastech"]``
        ...
        try:
            ...
            # (``curated["nastech"] = get_curated_nastech_model_ids()``), so the test
            # stays an invariant — it can't rot as the curated/manifest list grows.
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("nastech_cli.models.check_nastech_free_tier", return_value=False), patch(
                ...
                expected = get_curated_nastech_model_ids()
                picker = list_picker_providers(
                    current_provider="nastech", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        nastech_row = next((r for r in picker if r["slug"] == "nastech"), None)
        assert nastech_row is not None, "nastech row must appear when authed"
        assert nastech_row["models"] == expected
    ...
    def test_picker_max_models_cap_semantics(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            with patch.object(
                ...
                expected = get_curated_nastech_model_ids()
                full = list_picker_providers(current_provider="nastech", max_models=None)
                one = list_picker_providers(current_provider="nastech", max_models=1)
                ...
                zero = list_authenticated_providers(
                    current_provider="nastech", max_models=0
                )
        finally:
            model_catalog.reset_cache()
        ...
        assert len(expected) > 1, "test needs a multi-model curated nastech list"

        full_row = _nastech(full)
        assert full_row is not None and full_row["models"] == expected

        one_row = _nastech(one)
        assert one_row is not None and one_row["models"] == expected[:1]

        zero_row = _nastech(zero)
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["nastech"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['nastech'] / OPENROUTER_MODELS. "
