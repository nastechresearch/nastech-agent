...
``.nastech-catalog.json`` sidecar) and the install endpoint has NO kill-list bypass."""
...
@pytest.fixture
def client(monkeypatch, tmp_path, _isolate_nastech_home):
    try:
...
def _install(name: str, sidecar: dict | None):
    from nastech_constants import get_nastech_home
    d = get_nastech_home() / "plugins" / name
    d.mkdir(parents=True)
    (d / "plugin.yaml").write_text(yaml.safe_dump({"name": name, "version": "1.0", "description": "x"}))
    if sidecar:
        (d / ".nastech-catalog.json").write_text(json.dumps(sidecar))
...
def test_install_endpoint_refuses_removed_plugins_with_no_bypass(client, monkeypatch):
    ...
    for body in ({"identifier": "https://github.com/evil/bad-plugin.git"},
        ...
        assert "removed" in resp.json()["detail"] or "not in the Nastech plugin catalog" in resp.json()["detail"]
