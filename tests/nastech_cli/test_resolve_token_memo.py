...
def test_memo_expires_after_ttl(monkeypatch, tmp_path):
    ...
    cache_key = auth.nastech_home_key()
...
def test_memo_does_not_leak_across_multiplex_profile_contexts(tmp_path):
    ...
    nastech_constants.set_nastech_home_override (gateway/run.py,
    tui_gateway/server.py), not the NASTECH_HOME env var — the memo must key
    on that same resolved home, or one profile's context can read another
    profile's already-cached Nastech access token for up to the TTL window.
    """
    ...
    reset_token = nastech_constants.set_nastech_home_override(str(profile_a))
    try:
        token_a = auth.resolve_nastech_access_token()
    finally:
        nastech_constants.reset_nastech_home_override(reset_token)

    # Still well within the 5s TTL — this is exactly the race window: profile
    # B's context calls resolve_nastech_access_token() shortly after profile A's.
    reset_token = nastech_constants.set_nastech_home_override(str(profile_b))
    try:
        token_b = auth.resolve_nastech_access_token()
    finally:
        nastech_constants.reset_nastech_home_override(reset_token)
