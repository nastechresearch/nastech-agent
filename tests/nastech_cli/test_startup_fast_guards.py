...
def test_literal_tilde_nastech_home_expands_before_any_reader(tmp_path):
    """A literal ``~`` in NASTECH_HOME (fish, or any quoted value) is expanded at process entry.
    ...
    fake_home.mkdir()
    cwd.mkdir()
    env = {**os.environ, "HOME": str(fake_home), "USERPROFILE": str(fake_home), "NASTECH_HOME": "~/.x"}
    env.pop("NASTECH_DEV", None)
    env["PYTHONPATH"] = str(REPO_ROOT)
    result = subprocess.run(
        [sys.executable, "-m", "nastech_cli.main", "config", "path"],
    ...
    # Raw-reader observable: the ~30 ``os.environ["NASTECH_HOME"]`` readers in nastech_cli/ never
    # call the resolver, so the entry-point hunk in main.py (not nastech_constants) must have
    # rewritten the env var by the time the module import finishes.
    probe = subprocess.run(
        [sys.executable, "-c", "import nastech_cli.main, os; print(os.environ['NASTECH_HOME'])"],
        capture_output=True, text=True, timeout=120, cwd=cwd, env=env,
    )
...
def test_normalize_nastech_home_env_rewrites_tilde_and_leaves_absolute_alone(tmp_path, monkeypatch):
    from nastech_cli import _startup_fast

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("USERPROFILE", str(tmp_path))
    monkeypatch.setenv("NASTECH_HOME", "~/.x")
    _startup_fast.normalize_nastech_home_env()
    assert os.environ["NASTECH_HOME"] == str(tmp_path / ".x")

    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "abs"))
    _startup_fast.normalize_nastech_home_env()
    assert os.environ["NASTECH_HOME"] == str(tmp_path / "abs")

    monkeypatch.delenv("NASTECH_HOME")
    _startup_fast.normalize_nastech_home_env()
    assert "NASTECH_HOME" not in os.environ
