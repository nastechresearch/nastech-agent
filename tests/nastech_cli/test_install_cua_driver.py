...
class TestCuaDriverRuntimeContract:
    def test_current_manifest_is_ready(self):
        from nastech_cli import tools_config_cua as tools_config
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_missing_explicit_override_does_not_install_standard_driver(
        ...
        monkeypatch.setenv("NASTECH_CUA_DRIVER_CMD", "/missing/custom/cua-driver")
        with patch.object(
                 tools_config,
                 "_resolved_cua_driver_cmd",
                 return_value=None,
...
class TestRequireConfirmedUpdate:
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Nastech' own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `nastech update` refresh must
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``nastech update``
...
class TestCuaVersionSummary:
    """`nastech computer-use status` prints one line, whatever the binary says.

    A binary chosen by NASTECH_CUA_DRIVER_CMD is under no obligation to answer
