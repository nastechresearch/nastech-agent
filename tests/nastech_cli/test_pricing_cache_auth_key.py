...
Nastech ``/v1/models`` answers each caller with the catalog their org may reach,
...
from nastech_cli.models_pricing import fetch_models_with_pricing, peek_cached_pricing
...
class TestNastechCatalogExpiry:
    """A Nastech catalog reflects the org's policy, which an admin can change while
    ...
    def test_entry_expires_so_a_policy_change_is_picked_up(self, catalog, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: now + _NASTECH_CATALOG_TTL_SECONDS + 1,
        )
        fetch_models_with_pricing(
            api_key="sk-test", base_url=BASE,
            cache_ttl_seconds=_NASTECH_CATALOG_TTL_SECONDS,
