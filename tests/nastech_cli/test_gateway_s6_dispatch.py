...
def test_redirect_falls_back_when_sleep_missing(
    ...
    monkeypatch.setattr("nastech_cli.gateway.os.execvp", _raise_missing_sleep)
...
def _armed_watchdog(monkeypatch: pytest.MonkeyPatch):
    """Arm the real watchdog as nastech_cli.main's argv fast-path does; the long timeout keeps the
...
class _UnregisteredRecorder(_CallRecorder):
    ...
    def _svc(self, action: str, name: str) -> None:
        from nastech_cli.service_manager import GatewayNotRegisteredError
...
def _arrange(monkeypatch, tmp_path, mgr, *, profile: str, seed_soul: bool):
    """Force the s6 branch and make ``tmp_path`` the shared NASTECH_HOME the slot maps back to."""
    from nastech_cli import gateway as gw
    from nastech_cli import service_manager as sm
