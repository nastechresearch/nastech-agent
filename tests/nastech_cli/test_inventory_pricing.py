...
def test_apply_pricing_free_models_get_flat_100_percent_sale(monkeypatch):
    ...
    rows = [{"slug": "nastech", "models": ["a/free", "b/natively-free"]}]
    inv._apply_pricing(rows)
...
def test_cold_nastech_entitlement_keeps_models_unselectable(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "get_cached_nastech_free_tier", lambda: None)
    rows = [{"slug": "nastech", "models": ["free/model", "paid/model"]}]
    ...
    rows = [{"slug": "nastech", "models": ["m"], "warning": "paste NASTECH_API_KEY to activate"}]
    inv._apply_pricing(rows, cached_only=True)
    assert rows[0]["warning"] == "paste NASTECH_API_KEY to activate"
...
def test_prewarm_preserves_context_and_runs_once_per_profile(tmp_path, monkeypatch):
    ...
    def capture_context(_rows):
        ...
        observed[label] = (nastech_home_key(), dict(scope))
        started[label].set()
    ...
    try:
        for label in ("a", "b"):
            home = tmp_path / label
            home_token = set_nastech_home_override(str(home))
        ...
        assert observed["a"] == (
            nastech_home_key(tmp_path / "a"),
            {"PROFILE_MARKER": "a"},
        )
        assert observed["b"] == (
            nastech_home_key(tmp_path / "b"),
            {"PROFILE_MARKER": "b"},
        )
    finally:
        release.set()
...
def test_prewarm_endpoint_rotation_starts_a_new_worker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        inv,
        "_apply_pricing",
        lambda _rows: models_pricing.get_pricing_for_provider("nastech"),
    )

    token = set_nastech_home_override(str(tmp_path / "profile"))
    threads = []
    try:
        threads.append(
            inv._prewarm_pricing_async(
                [{"slug": "nastech", "models": ["a/model"]}],
...
def test_prewarm_nastech_rotation_when_another_provider_is_current(tmp_path, monkeypatch):
    """Nastech endpoint identity must not depend on Nastech being selected."""
    from nastech_constants import (
        reset_nastech_home_override,
        set_nastech_home_override,
    )
    ...
    monkeypatch.setattr(models_pricing, "_pricing_cache_retry_after", {})
    monkeypatch.setattr(models_pricing, "_pricing_provider_cache_keys", {})
    monkeypatch.setattr(models_pricing, "get_cached_nastech_inference_base_url",
        lambda: active_endpoint["value"],
    )
    monkeypatch.setattr(models_pricing, "_resolve_nastech_pricing_credentials",
        lambda: ("", active_endpoint["value"]),
    ...
    def in_profile(home, endpoint, *, cached_only):
        ...
        try:
            return models_pricing.get_pricing_for_provider(
                "nastech", cached_only=cached_only
            )
        finally:
            reset_nastech_home_override(token)
