...
The bug class: on Windows, when `nastech.exe`/sibling shims could not be
...
import nastech_cli._install_repair as ir
import nastech_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("nastech", "nastech-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # nastech.exe cannot be renamed; nastech-gateway.exe can
    real_rename = Path.rename

    def deny_nastech(self, target):
        if self.name == "nastech.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_nastech)
    monkeypatch.setattr(main_install_repair, "_nastech_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "nastech.exe" in exc_info.value.failed_shims
    # The successful rename (nastech-gateway.exe) was rolled back.
    assert (scripts / "nastech-gateway.exe").exists()
    assert not list(scripts.glob("nastech-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("nastech",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("nastech",))
...
def test_refusal_writes_marker_and_exits_2(monkeypatch, capsys):
    ...
    exc = main_install_repair.ShimQuarantineError(["nastech.exe"])
    with pytest.raises(SystemExit) as exit_info:
        update_cmd._refuse_update_for_contended_shims(exc)
    assert exit_info.value.code == 2
    assert wrote == [1]
    out = capsys.readouterr().out
    assert "nastech.exe" in out
