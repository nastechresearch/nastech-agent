...
def _patch_gateway_discovery():
    ...
    """
    with patch("nastech_cli.gateway.find_gateway_pids", return_value=[]), \
         patch("nastech_cli.gateway.supports_systemd_services", return_value=False), \
         patch("nastech_cli.gateway.find_profile_gateway_processes", return_value=[]):
        yield
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "nastech_cli.update_cmd._reload_config_modules"
        ), patch(
...
class TestConfigVersionCheckUsesFreshModules:
    ...
    Before the fix, ``nastech update`` ran in the PRE-pull Python process.
    After ``git pull`` updated the source on disk, function-level imports
    returned the OLD cached ``nastech_cli.config`` module — so
    ...
    def test_run_config_check_fresh_reloads_modules(self):
        ...
        sys.modules held the OLD nastech_cli.config with the OLD
