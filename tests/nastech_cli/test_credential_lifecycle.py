...
# load_pool() or a separate `nastech auth add`.
# ---------------------------------------------------------------------------
...
def test_put_api_env_materializes_credential_pool_entry(nastech_home):
    ...
    the old cached credential until a separate ``nastech auth add opencode-go``
...
def test_put_api_env_writes_auth_json_for_provider(nastech_home):
    """Sentinel for #96058: PUT /api/env must modify auth.json on disk.

    The reported symptom was ``stat -c '%y' ~/.nastech/auth.json`` returning
...
def test_update_rotates_keyed_providers_mirror(nastech_home):
    old = "sk-kp-" + "n" * 24
    new = "sk-kp-" + "o" * 24
    _write_env(nastech_home, OPENAI_API_KEY=old)
    _write_config(
...
def test_delete_scrubs_keyed_providers_mirror(nastech_home):
    old = "sk-kp-" + "p" * 24
    _write_env(nastech_home, OPENAI_API_KEY=old)
    _write_config(
        nastech_home,
...
def test_scrub_never_touches_providers_base_url_alias(nastech_home):
