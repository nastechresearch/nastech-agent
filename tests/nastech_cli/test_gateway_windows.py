...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "nastech_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda home=None: (argv, cwd, {"NASTECH_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("nastech_cli.config.get_nastech_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda home=None: (
            argv,
            cwd,
            {"NASTECH_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("nastech_cli.config.get_nastech_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
def test_atomic_write_leaves_no_staging_file_when_swap_fails(monkeypatch, tmp_path):
    ...
    startup.mkdir()
    entry, staging = startup / "Nastech_Gateway.vbs", startup / "Nastech_Gateway.tmp"
...
def test_uninstall_and_reinstall_sweep_stale_startup_staging_file(monkeypatch, tmp_path):
    ...
    startup = tmp_path / "Startup"
    startup.mkdir()
    entry, staging = startup / "Nastech_Gateway_alice.vbs", startup / "Nastech_Gateway_alice.tmp"
    script = tmp_path / "task" / "Nastech_Gateway_alice.cmd"
    ...
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Nastech_Gateway_alice.cmd")
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>"C:\\Users\\me\\.nastech\\gateway-service\\Nastech_Gateway.vbs"</Arguments>
    </Exec>
  </Actions>
</Task>
"""
...
def test_scheduled_task_drift_names_missing_hardening_leaves(monkeypatch):
    ...
    ``nastech gateway status`` prints together with the ``nastech gateway install`` repair hint."""
    launcher = Path(r"C:\Users\me\.nastech\gateway-service\Nastech_Gateway.vbs")
    template = gateway_windows._build_scheduled_task_xml("Nastech_Gateway", launcher, r"PC\me")
...
def test_reconcile_scheduled_task_reregisters_only_on_drift(monkeypatch, tmp_path):
    ...
    assert gateway_windows.reconcile_scheduled_task("Nastech_Gateway") is True
    ...
    calls.clear()
    assert gateway_windows.reconcile_scheduled_task("Nastech_Gateway") is False
...
def _arrange_uninstalled_start(monkeypatch):
    ...
    installs, spawns = [], []
    monkeypatch.delenv("NASTECH_GATEWAY_INSTALL_START_ON_LOGIN", raising=False)
...
def test_stdin_interactive_only_when_isatty_and_a_console_answers_get_console_mode():
    """Windows CRT isatty() is True for the NUL device (`nastech gateway start < NUL`, stdin=DEVNULL), so
...
def test_start_with_nul_stdin_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    ...
    assert "nastech gateway install" in capsys.readouterr().out


def test_start_without_tty_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    """`nastech gateway start < /dev/null` must not answer the persistence question with a default Yes
    ...
    assert "nastech gateway install" in out and "did not complete" not in out
...
def test_start_on_tty_hands_both_answers_to_install_and_honours_the_env_opt_out(monkeypatch):
    ...
    again). NASTECH_GATEWAY_INSTALL_START_ON_LOGIN=0 → no question, no install, a plain start."""
    installs, spawns = _arrange_uninstalled_start(monkeypatch)
    monkeypatch.setattr(setup, "is_interactive_stdin", lambda: True)
    ...
    installs.clear()
    monkeypatch.setenv("NASTECH_GATEWAY_INSTALL_START_ON_LOGIN", "0")
