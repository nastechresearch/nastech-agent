...
Sending an express key to generativelanguage.googleapis.com always 403s, so ``nastech doctor`` and the
...
def test_doctor_gemini_probe_routes_express_key_to_aiplatform():
    from nastech_cli.doctor_connectivity import _apikey_request
...
def test_dashboard_gemini_key_probe_routes_express_key_to_aiplatform(monkeypatch):
    import nastech_cli.web_routers.config_env as mod
    from nastech_cli.web_models import EnvVarUpdate
