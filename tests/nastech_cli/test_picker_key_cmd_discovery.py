...
from nastech_cli import model_switch_providers as picker
...
def test_picker_command_auth_is_lazy_and_credential_scoped(monkeypatch, route, static):
    ...
    monkeypatch.setattr("nastech_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: False)
...
def test_setup_probe_credentials_never_become_saved_credentials(monkeypatch, route, discover, static):
    from agent import command_token_source
    from nastech_cli import config as config_module, models
    from nastech_cli import model_setup_flows_custom as setup
    ...
    from nastech_cli.main_provider_setup import _named_custom_provider_map
