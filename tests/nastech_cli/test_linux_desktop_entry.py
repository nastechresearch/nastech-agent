...
def xdg_home(tmp_path, monkeypatch) -> Path:
    ...
    # ~/.local/bin/nastech on the dev machine.
...
def _stub_install(tmp_path, monkeypatch) -> None:
    ...
    nastech_bin.parent.mkdir(exist_ok=True)
...
def test_install_prefers_themed_icon_from_hicolor(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "nastech.png"
    assert dest.is_file()
...
def test_installed_entry_is_executable(tmp_path, xdg_home, monkeypatch):
    root = _make_project(tmp_path)
    monkeypatch.setattr(
        "nastech_cli.relaunch.resolve_nastech_bin", lambda: "/usr/bin/nastech"
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_converges_from_repo_script_argv0_to_installed_wrapper(
    ...
    Launching with argv[0] = <checkout>/nastech (what the broken entry
    ...
    root = _make_project(tmp_path)
    repo_script = root / "nastech"  # checkout-internal launcher candidate
    repo_script.write_text(
        "#!/usr/bin/env python3\nimport nastech_cli\n", encoding="utf-8"
    )
    repo_script.chmod(0o755)
    wrapper = tmp_path / "installed" / "bin" / "nastech"
    ...
    monkeypatch.setattr(
        "shutil.which", lambda name: str(wrapper) if name == "nastech" else None
    )
    monkeypatch.setattr(lde, "refresh_desktop_databases", lambda _dir: [])
...
def test_exec_never_persists_a_bare_interpreter_command(
    ...
    """The `python -m nastech_cli.main` relaunch context must not write
...
def test_exec_keeps_resolver_fallback_when_no_wrapper_on_path(
    ...
    `sys.executable -m nastech_cli.main desktop` fallback. Persisting the
    ...
    def fake_resolve():
        # Mirror resolve_nastech_bin's chain: argv[0] → relative → PATH → None.
        return sys.argv[0] if sys.argv[0] else None

    monkeypatch.setattr("nastech_cli.relaunch.resolve_nastech_bin", fake_resolve)
...
def test_exec_uses_known_wrapper_when_path_lookup_misses(
    ...
    # checkout's nastech script (the aidiyet check requires it to target
    # the writing checkout).
    known_wrapper = tmp_path / "known-home" / ".local" / "bin" / "nastech"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "nastech"} "$@"\n',
        encoding="utf-8",
    )
...
def test_exec_never_persists_a_checkout_internal_path_hit(tmp_path, xdg_home, monkeypatch):
    ...
    cycle), and every flip rewrites nastech.desktop. A rewrite landing
...
def test_exec_finds_known_wrapper_when_resolver_has_no_candidate(
    ...
    `python -m`, and PATH has no `nastech`) makes resolve_nastech_bin return
    ...
    `<python> -m nastech_cli.main desktop` module form. Each flip between the
    wrapper and module forms rewrites nastech.desktop on the next launch; any
    ...
    known_wrapper = tmp_path / "cold-home" / ".local" / "bin" / "nastech"
    known_wrapper.parent.mkdir(parents=True)
    known_wrapper.write_text(
        f'#!/bin/bash\nexec {root / "venv" / "bin" / "python"} {root / "nastech"} "$@"\n',
        encoding="utf-8",
...
def test_exec_rejects_known_wrapper_from_another_checkout(
    ...
    On machines with multiple installs over time, ~/.local/bin/nastech may
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "user",
            {},
            "HOME-SET-BY-TEST/.local/bin/nastech",
            id="user-layout",
        ),
        pytest.param(
            "termux",
            {"PREFIX": "PREFIX-SET-BY-TEST"},
            "PREFIX-SET-BY-TEST/bin/nastech",
...
def test_known_wrapper_candidates_cover_installer_layouts(
    ...
    if layout == "root-fhs":
        ...
        assert candidates.index("/usr/local/bin/nastech") < candidates.index(
            f"{sentinel_home}/.local/bin/nastech"
        )
    if layout == "non-root-no-fhs":
        # Non-root euid: /usr/local/bin must be excluded outright.
        assert "/usr/local/bin/nastech" not in candidates
...
def test_can_import_probe_runs_and_caches(tmp_path):
    ...
    deliberately NOT asserted - a CI host with nastech pip-installed
    ...
    try:
        first = lde._can_import_nastech_cli(real)
        assert isinstance(first, bool)
        assert str(real) in lde._probe_cache
        t0 = time.monotonic()
        second = lde._can_import_nastech_cli(real)
...
def test_exec_falls_back_to_running_interpreter_when_probe_fails(
    ...
    monkeypatch.setattr(lde, "_can_import_nastech_cli", lambda p: False)
...
def test_wrapper_ownership_accepts_shim_via_symlinked_home(tmp_path, monkeypatch):
    ...
    lexical_checkout = home_link / "nastech-agent"
    (home_real / "nastech-agent").mkdir()

    shim = home_link / ".local" / "bin" / "nastech"
    shim.parent.mkdir(parents=True)
    shim.write_text(
        f"#!/bin/bash\n"
        f"exec {lexical_checkout}/venv/bin/python "
        f'{lexical_checkout}/nastech "$@"\n',
        encoding="utf-8",
    )
...
def test_needs_interpreter_case_insensitive_match(tmp_path, monkeypatch):
    ...
    console_script = venv_bin / "nastech"
...
def test_probe_skips_wrapper_with_escaping_python_shebang(
    ...
    broken_wrapper = xdg_home / ".local" / "bin" / "nastech"
...
def test_probe_accepts_shell_launcher_wrapper(tmp_path, xdg_home, monkeypatch):
    ...
    good_wrapper = xdg_home / ".local" / "bin" / "nastech"
...
def test_install_places_1024_png_in_256x256_not_scalable(
    ...
    dest = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "nastech.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "nastech.png"
    assert values["Icon"] == "nastech"
    assert dest.is_file()
    assert dest.read_bytes() == lde.icon_path(root).read_bytes()
...
def test_install_removes_stale_scalable_png(tmp_path, xdg_home, monkeypatch):
    """v2026.8.31 wrote the PNG into scalable/. A later nastech desktop
...
def test_install_exact_48_png_uses_48x48_dir(tmp_path, xdg_home, monkeypatch):
    ...
    dest = xdg_home / "icons" / "hicolor" / "48x48" / "apps" / "nastech.png"
    assert dest.is_file()
    assert not (
        xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "nastech.png"
    ).exists()
...
def test_install_resizes_decodable_png_to_panel_sizes(
    ...
    dest_24 = xdg_home / "icons" / "hicolor" / "24x24" / "apps" / "nastech.png"
    dest_256 = xdg_home / "icons" / "hicolor" / "256x256" / "apps" / "nastech.png"
    stale = xdg_home / "icons" / "hicolor" / "scalable" / "apps" / "nastech.png"
