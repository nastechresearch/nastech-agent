"""Invariants for ``nastech_cli.config_effective.load_user_config_effective`` — the one loader every
defaults-free config reader (gateway runtime, TUI gateway, cron, ``nastech send`` bridge, doctor,
...
def _reset_caches():
    ...
    from nastech_cli import config_effective, managed_scope
...
def _reset_caches_keep_last_good():
    import nastech_cli.config as cfg
    from nastech_cli import config_effective
