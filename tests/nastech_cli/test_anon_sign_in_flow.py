...
Driven directly against the same fake account service ``nastech auth upgrade`` is tested with, so the
...
from tests.nastech_cli.test_anon_upgrade import (  # noqa: F401  (fixtures used by name)
...
def test_a_completed_sign_in_yields_code_waiting_then_completed(portal, free_account):
    _seed_free_tier()
    _write_model_config({"provider": "nastech", "default": anon_auth.GUEST_MODEL, "base_url": WELCOME})
...
def test_a_timeout_yields_timed_out_and_keeps_the_enriched_detail(portal, monkeypatch):
    ...
    enriched = auth_device_flow._nastech_device_auth_timeout_message(PORTAL)
...
def test_a_persist_failure_yields_failed_rather_than_raising(portal, free_account, monkeypatch):
    ...
    monkeypatch.setattr(
        auth_nastech, "persist_nastech_credentials", lambda *a, **kw: (_ for _ in ()).throw(OSError("read-only home")))
...
def test_a_settle_failure_yields_failed_rather_than_raising(portal, free_account, monkeypatch):
    ...
    persists = []
    from nastech_cli import auth_nastech
    real_persist = auth_nastech.persist_nastech_credentials
    monkeypatch.setattr(
        auth_nastech, "persist_nastech_credentials",
...
def test_already_signed_in_short_circuits_before_any_network(portal):
    from nastech_cli.auth import _load_auth_store, _save_auth_store, _save_provider_state
    store = _load_auth_store()
    _save_provider_state(store, "nastech", {"auth_method": "oauth_device_code", "access_token": "x"})
