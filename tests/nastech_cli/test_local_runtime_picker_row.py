...
def test_no_staged_models_no_row(nastech_home):
    from nastech_cli.inventory import _local_runtime_row, load_picker_context

    assert _local_runtime_row(load_picker_context()) is None


def test_staged_models_make_a_selectable_row(nastech_home):
    from nastech_cli.inventory import _local_runtime_row, load_picker_context

    _stage(nastech_home, "Qwen3-4B-Instruct-2507-UD-Q8_K_XL", "Some-Other-Model")
...
def test_row_marks_current_when_config_points_at_llamacpp(nastech_home):
    from nastech_cli.inventory import _local_runtime_row, load_picker_context

    _stage(nastech_home, "M")
...
def test_full_payload_includes_local_row(nastech_home):
    ...
    _stage(nastech_home, "Local-Model-X")
...
def test_explicit_only_filter_keeps_local_row_on_any_profile(nastech_home):
    ...
    from nastech_cli.inventory import _filter_explicit_provider_rows, _local_runtime_row, load_picker_context

    _stage(nastech_home, "Qwen3.8-27B-UD-Q5_K_XL")
