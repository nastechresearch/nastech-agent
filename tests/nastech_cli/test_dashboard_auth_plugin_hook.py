...
from nastech_cli.dashboard_auth import registry as _auth_registry
...
def _real_ctx() -> tuple[PluginManager, PluginContext]:
    manager = PluginManager(scope_key=nastech_home_key())
...
def test_auth_provider_kept_out_of_manager_teardown_order():
    ...
    # Still attributed to the plugin for `nastech plugins list`.
...
def test_profile_manager_cannot_replace_launch_home_auth_provider():
    ...
    profile_scope = nastech_home_key(Path(manager.scope_key) / "profiles" / "bot")
...
def test_profile_directory_can_be_the_process_launch_home(monkeypatch, tmp_path):
    ...
    assert manager.scope_key == nastech_home_key(get_process_nastech_home())
...
def test_targeted_unload_disposes_persistent_auth_provider():
    ...
    # `nastech plugins disable basic` drives a targeted unload of that plugin.
