"""``nastech update`` / ``nastech migrate relay``: legacy ``NASTECH_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from nastech_cli.relay_plugin_migrate import (
...
NASTECH_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
NASTECH_NEMO_RELAY_ATOF_FILENAME=nastech-atof.jsonl
NASTECH_NEMO_RELAY_ATOF_MODE=append
NASTECH_NEMO_RELAY_ATIF_ENABLED=1
NASTECH_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
NASTECH_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
NASTECH_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "nastech-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file Nastech will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: NASTECH_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
