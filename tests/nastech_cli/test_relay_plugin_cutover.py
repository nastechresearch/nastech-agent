"""Regression tests for migration from the removed Nastech Relay plugin."""
...
def test_doctor_reports_legacy_exporter_env_without_new_config(monkeypatch):
    ...
    findings = dict(
        collect_relay_plugin_cutover_findings(
            {},
            {
                "NASTECH_NEMO_RELAY_ATIF_ENABLED": "true",
                "NASTECH_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S": "30",
            },
        )
    )

    assert "now ignored" in findings["NASTECH_NEMO_RELAY_ATIF_ENABLED"]
    assert RELAY_PLUGINS_CONFIG_ENV in findings["NASTECH_NEMO_RELAY_ATIF_ENABLED"]
    assert "now ignored" in findings["NASTECH_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    assert (
        RELAY_PLUGINS_CONFIG_ENV
        in findings["NASTECH_NEMO_RELAY_ATIF_EXPORT_TIMEOUT_S"]
    )
