...
def test_refresh_always_fires_before_expiry_for_observed_lifetimes():
    ...
    for lifetime in OBSERVED_LIFETIMES_SECONDS:
        tick = keepalive._tick_seconds(
            keepalive.NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS, lifetime
        )
        horizon = keepalive._refresh_horizon_seconds(
            tick, keepalive.NASTECH_INVOKE_JWT_MIN_TTL_SECONDS
        )
...
def test_interval_precedence_and_disable(monkeypatch):
    def _config(section):
        monkeypatch.setattr(keepalive, "_nastech_config", lambda: section)

    # An absent section leaves the module default in place.
    _config({})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )

    _config({keepalive.NASTECH_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 600})
    ...
    _config({keepalive.NASTECH_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: "not-a-number"})
    assert (
        keepalive._interval_seconds(None)
        == keepalive.NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS
    )
    ...
    _config({keepalive.NASTECH_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY: 0})
    assert keepalive._interval_seconds(None) == 0
    assert keepalive.start_nastech_auth_keepalive() is None
