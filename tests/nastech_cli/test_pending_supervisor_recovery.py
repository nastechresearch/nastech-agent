...
from nastech_cli import gateway, main, update_cmd_fleet as fleet, update_receipt
...
def test_pending_marker_requires_complete_systemd_recovery(monkeypatch, tmp_path, failure):
    ...
    def systemctl(cmd, **kw):
        if "list-units" in cmd:
            ...
            return SimpleNamespace(returncode=int(failure == "listing"), stdout=(
                "nastech-gateway-one.service loaded active running\n"
                + ("" if failure == "missing-owned" else "nastech-gateway-two.service loaded failed failed\n")), stderr="")
        bad = cmd[-1] == "nastech-gateway-two"
        if "restart" in cmd:
    ...
    monkeypatch.setattr(update_receipt, "collect_fleet_versions", lambda **kw: [
        {"profile": name.removeprefix("nastech-gateway-"), "state": "current", "code_sha": "pending"}
    ...
    if failure not in (None, "running"):
        ...
        assert set(recovered) == {"nastech-gateway-one", "nastech-gateway-two"}
...
def test_pending_launchd_requires_complete_supervision(monkeypatch, tmp_path, failure):
    ...
    current, sibling = "ai.nastech.gateway", "ai.nastech.gateway-two"
