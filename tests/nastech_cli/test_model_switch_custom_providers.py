...
from nastech_cli.model_switch_providers import (
...
def _disable_live_custom_provider_model_probe(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_local.fetch_ollama_local_models", lambda *_a, **_kw: None
    )


def test_picker_native_probe_failure_falls_back_to_openai_catalog(monkeypatch):
    monkeypatch.setattr(
        "nastech_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: True
    )
    monkeypatch.setattr(
        "nastech_cli.models._get_ollama_native_headers", lambda *a, **k: {}
    )
    monkeypatch.setattr(
        "nastech_cli.models_local.fetch_ollama_local_models", lambda *a, **k: None
    )
    monkeypatch.setattr(
        "nastech_cli.models.fetch_api_models", lambda *a, **k: ["fallback-model"]
    )
...
def test_picker_native_catalog_is_admitted_to_the_shared_model_cache(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models._get_ollama_native_headers", lambda *a, **k: {})
    monkeypatch.setattr(
        "nastech_cli.models_local.fetch_ollama_local_models", lambda *a, **k: ["qwen3:8b"]
    )
...
def _native_picker_probe(monkeypatch, models_by_call):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_local.fetch_ollama_local_models", lambda *a, **k: next(answers)
    )
...
def test_picker_native_catalog_uses_the_short_native_ttl(monkeypatch):
    ...
    """
    from nastech_cli.models_local import _OLLAMA_LOCAL_MODELS_CACHE_TTL
    ...
    refreshes = []
    monkeypatch.setattr(
        "nastech_cli.models._spawn_swr_refresh", lambda key, fn=None: refreshes.append(key)
    )
...
def test_picker_empty_native_catalog_is_not_stale_served(monkeypatch):
    ...
    """
    from nastech_cli.models import _PROVIDER_MODELS_CACHE_TTL
...
def test_picker_generic_discovery_preserves_api_mode(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: False
    )
    monkeypatch.setattr("nastech_cli.models.cached_fetch_api_models", cached)
...
def test_providers_singular_model_does_not_suppress_ollama_native_discovery(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "NASTECH_OVERLAYS", {})
    monkeypatch.setattr(
        "nastech_cli.models_local.fetch_ollama_local_models",
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models_validate.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
...
def test_switch_model_does_not_send_ollama_headers_to_unrelated_custom_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_local.should_use_ollama_native_catalog",
        fake_native_detection,
    )
    monkeypatch.setattr(
        "nastech_cli.models_local._get_ollama_request_headers",
        lambda: {"Authorization": "Bearer configured-ollama-secret"},
    )
    monkeypatch.setattr(
        "nastech_cli.models._get_provider_config_dict",
        lambda provider: (
    ...
    monkeypatch.setattr("nastech_cli.models_validate.validate_requested_model", fake_validation)
    monkeypatch.setattr("nastech_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("nastech_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_picker_selection_resolves_named_custom_provider_model_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_validate.validate_requested_model",
...
def test_custom_provider_no_key_singular_model_still_probes_live_models(monkeypatch):
    ...
    probed so /model matches the terminal ``nastech model`` flow. Ollama-native
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Nastech's own writer stores configured models as a dict
...
def test_picker_endpoint_authorization_overrides_inferred_bearer(monkeypatch):
    from nastech_cli.model_switch_providers import _fetch_picker_live_models
    ...
    monkeypatch.setattr("nastech_cli.models_local.should_use_ollama_native_catalog", lambda *a, **k: True)
    monkeypatch.setattr("nastech_cli.models_local.fetch_ollama_local_models", fake_native)
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models_local.fetch_lmstudio_models", _fake_fetch)
...
def test_lmstudio_bare_providers_block_does_not_hide_live_catalog(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_local.fetch_lmstudio_models",
...
def test_lmstudio_providers_block_with_explicit_endpoint_still_uses_section3(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.model_switch_providers._discover_endpoint_models", _fake_discover
    )
...
def test_discovered_models_auto_saved_to_cache(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.models.fetch_api_models", fake_fetch_api_models)
    monkeypatch.setattr(
        "nastech_cli.model_switch_providers._save_discovered_models_to_config",
...
def test_save_discovered_models_preserves_dict_form(monkeypatch):
    ...
    from nastech_cli.model_switch_providers import _save_discovered_models_to_config
...
def test_model_flow_named_custom_persists_discovered_models(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.main_provider_setup._save_custom_provider", lambda *a, **k: None)
    ...
    from nastech_cli.model_setup_flows_custom import _model_flow_named_custom
...
def test_legacy_sentinel_catalog_still_resolves_and_migrates(tmp_path, monkeypatch):
    ...
    Pre-fix Nastech wrote ``__discovered_model_catalog__: true`` (and
...
def test_same_provider_switch_on_session_only_custom_endpoint_keeps_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.model_switch.load_config", lambda: {"model": {"provider": "openrouter", "default": "x"}}, raising=False)
    monkeypatch.setattr(
