...
class TestShouldExclude:
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from nastech_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the NASTECH_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``nastech-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/nastech-agent/`` that the manual
        ...
        # nastech-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "nastech-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/nastech-agent/SKILL.md"))
...
class TestBackup:
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        nastech_home.mkdir()
        _make_nastech_tree(nastech_home)
        with sqlite3.connect(nastech_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from nastech_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup
        ...
        assert snap_id and (nastech_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Nastech configuration has been restored." in out
...
class TestBackupEdgeCases:
    ...
    def test_incomplete_archive_is_kept_but_reported_as_failure(self, tmp_path, monkeypatch, capsys):
        ...
        nastech_home.mkdir()
        _make_nastech_tree(nastech_home)
        unreadable = nastech_home / "skills" / "locked.md"
        ...
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        from nastech_cli.backup import _RUN_BACKUP_PREFIX, run_backup
        from nastech_cli.main import cmd_backup
...
class TestImportAtomicWrites:
    """`nastech import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the NASTECH_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (nastech_home / "config.yaml").write_text(original)
        monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (nastech_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(nastech_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside NASTECH_HOME."""
    ...
    def test_symlinked_target_keeps_its_symlink(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        store = nastech_home / "store"
        store.mkdir()
        real = store / "config.yaml"
        real.write_text("model: original\n")
        link = nastech_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        nastech_home = tmp_path / ".nastech"
        nastech_home.mkdir()
        target = nastech_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo nastech import`` onto a user-owned (or Docker/NAS volume-owned)
        NASTECH_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "nastech_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``nastech import`` is the
...
class TestQuickSnapshot:
    ...
    def test_restore_refused_db_is_not_counted(self, nastech_home, monkeypatch):
        """A refused live-safe restore (holder detected, backup leg failed) must
        not be counted as a restored file — `nastech import` reports it, and
        ...
        restore_quick_snapshot(snap_id, nastech_home=nastech_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(nastech_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, nastech_home):
        ...
        live_conn = sqlite3.connect(str(nastech_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, nastech_home=nastech_home)
        assert result is True
...
class TestPreUpdateBackup:
    ...
    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, nastech_home):
        """``nastech update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", nastech_home=nastech_home)
        assert snap_id and (nastech_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, nastech_home: Path) -> Path:
        nastech_home.mkdir(parents=True, exist_ok=True)
        cfg = nastech_home / "config.yaml"
    ...
    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        ...
        result = restore_config_model_settings_if_rewritten(
            snap_id, nastech_home=nastech_home
        )
        assert result is not None
        assert result["restored"] is True
        assert result["snapshot_id"] == snap_id
...
# ---------------------------------------------------------------------------
# run_import: NASTECH_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsNastechHomeOverride:
    """`nastech import` must restore into the home the command runs under.

    Resolving the target through get_default_nastech_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """NASTECH_HOME=<root>/profiles/<name> must restore INTO the profile,
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("NASTECH_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_nastech_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "nastech_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "nastech_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Nastech-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`nastech import` must not swap the inode of a database Nastech holds open.
