"""Contract tests for nastech_cli.local_runtime — Rollouts 1+2.

Per the design's verification plan: relationships and contracts, no
change-detector tests, real imports against temp NASTECH_HOME (the autouse
...
def test_llamacpp_endpoint_stale_state_falls_through(tmp_path, monkeypatch):
    ...
    with socket.socket() as s:
        ...
        dead_port = s.getsockname()[1]
    from nastech_cli.local_runtime import endpoint as ep
    from nastech_cli.local_runtime.detect import DEFAULT_PROBE_PORTS
    from nastech_cli.local_runtime.supervisor import state_path
...
def test_resolution_kicks_boot_when_no_thread_is_booting(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / ".nastech"))
    from nastech_cli.local_runtime import bootstrap as bs
    from nastech_cli.local_runtime import endpoint as ep
    from nastech_cli.local_runtime.supervisor import state_path
...
def test_staged_models_requires_every_split_part(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / ".nastech"))
    import nastech_cli.local_runtime.bootstrap as bs
...
def test_endpoint_identity_stable_across_supervisor_instances(tmp_path, monkeypatch):
    ...
    key_file = tmp_path / ".nastech" / "runtimes" / "llamacpp" / ".api_key"
