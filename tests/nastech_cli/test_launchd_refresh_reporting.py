...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.nastech.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.nastech.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("nastech_constants.display_nastech_home", lambda: "~/.nastech-work")
    ...
    assert "~/.nastech-work/logs/launchd-reload.log" in out
