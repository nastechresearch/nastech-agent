"""Tests for ``nastech doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from nastech_cli import doctor_live
from nastech_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `nastech doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("nastech_cli.doctor.NASTECH_HOME", tmp_path / "home")
        monkeypatch.setattr("nastech_cli.doctor.PROJECT_ROOT", tmp_path / "root")
    ...
    def test_false_on_termux_local_bare_npx(self, monkeypatch, tmp_path):
        ...
        advertise as ready — must not diverge from dep_ensure/nastech_subscription's
        same carve-out."""
        self._block_path_and_node_modules_checks(monkeypatch, tmp_path)
        import tools.browser_tool as bt
