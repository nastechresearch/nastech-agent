...
class TestCronCommandLifecycle:
    ...
    def test_edit_persists_user_owned_inference_pins(self, tmp_cron_dir, capsys):
        ...
        assert updated["provider"] == "nastech"
...
class TestUnverifiedDeliveryVisibility:
    ...
    state must be visible in ``nastech cron list`` and ``nastech cron doctor``,
...
class TestCronDoctor:
    def test_doctor_reports_cron_health_issues(self, tmp_cron_dir, capsys):
        ...
        assert "nastech cron edit" in out
...
class TestCronListStatusRendering:
    ...
    def test_delivery_failed_is_not_green_ok(self, tmp_cron_dir, capsys, monkeypatch):
        monkeypatch.setattr("nastech_cli.gateway.find_gateway_pids", lambda: [1])
        # capsys is not a tty, so force colors on to check the paint itself.
        monkeypatch.setattr("nastech_cli.colors.should_use_color", lambda: True)
        create_job(prompt="Daily digest", schedule="every 1h")
        jobs = load_jobs()
...
class TestCronRunBackgroundDispatch:
    """`nastech cron run` must not report 'failed' when the run was dispatched
...
class TestStatusSurfacesDeadScheduler:
    ...
    def _dead_gateway(self, monkeypatch):
        monkeypatch.setattr("nastech_cli.gateway.find_gateway_pids", lambda: [])
        monkeypatch.setattr(
            "nastech_cli.gateway.named_profile_served_by_running_multiplexer", lambda: None
        )
