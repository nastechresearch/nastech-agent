...
``nastech update`` exited 0 with zero rows — even when a gateway was
...
from nastech_cli.main import _fleet_probe_expected_runtimes
from nastech_cli.update_inventory import RuntimeRecord
...
class TestEmptySnapshotFailClosed:
    ...
    def test_incomplete_when_restart_phase_touched_gateways(self):
        ...
        assert (
            _fleet_probe_expected_runtimes(None, [], None, ["nastech-gateway"], set())
            is True
        )
