"""Tests for nastech_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `nastech setup` and `nastech import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("nastech_constants.is_container", lambda: container)
