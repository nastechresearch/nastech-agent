...
def test_env_var_names_the_managing_system(nastech_home, monkeypatch, env_value, expected):
    if env_value is None:
        monkeypatch.delenv("NASTECH_MANAGED", raising=False)
    else:
        monkeypatch.setenv("NASTECH_MANAGED", env_value)
...
def test_marker_file_names_the_managing_system(
    nastech_home, monkeypatch, marker_text, expected
):
    """An interactive shell reads .managed, not the NASTECH_MANAGED of the service."""
    (nastech_home / ".managed").write_text(marker_text, encoding="utf-8")
    monkeypatch.delenv("NASTECH_MANAGED", raising=False)
...
def test_env_var_wins_over_the_marker(nastech_home, monkeypatch):
    (nastech_home / ".managed").write_text("nixos", encoding="utf-8")
    monkeypatch.setenv("NASTECH_MANAGED", "home-manager")
...
def test_managed_install_names_its_system_and_offers_an_update(
    nastech_home, monkeypatch, tmp_path, managed_value
):
    """The message names the system, so the user knows what owns the install."""
    monkeypatch.setenv("NASTECH_MANAGED", managed_value)
    ...
    # `nastech update` cannot run on a managed install, so the advice must not
    # name it.
    assert config_mod.recommended_update_command() != "nastech update"
...
def test_unmanaged_install_offers_no_update_command(nastech_home, monkeypatch):
    monkeypatch.delenv("NASTECH_MANAGED", raising=False)
    assert config_mod.get_managed_update_command() is None


def test_unreadable_marker_still_reports_managed(nastech_home, monkeypatch):
    """A marker we cannot read is still a marker. Fail closed, not open."""
    marker = nastech_home / ".managed"
