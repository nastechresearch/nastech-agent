...
from nastech_cli.active_sessions import session_already_owned_message
from nastech_cli.cli_chat_error_copy import agent_init_failure_message, chat_error_response
...
@pytest.mark.parametrize("exc, pointer, absent", [
    (_StatusError("HTTP 401: Invalid API key", 401), "nastech model", "HTTP 401"),
    ...
    (Exception("Unknown error"), "nastech doctor", "Unknown error"),
])
...
def test_chat_error_response_accepts_plain_string_summary():
    text = chat_error_response("HTTP 401: Invalid API key", provider="nastech", model="m")
    assert "nastech model" in text.splitlines()[0]
...
def test_agent_init_failure_message_says_message_not_sent_and_points_to_doctor():
    ...
    assert "nastech doctor" in text and "/model" in text
...
def test_owned_message_first_line_is_plain_and_details_follow():
    ...
    assert first == ("This chat is open in another Nastech window/terminal. "
...
# ── cli-30: nastech sessions with a bad id / unopenable DB ──────────────────

def test_sessions_not_found_points_to_list(capsys):
    from nastech_cli.sessions_cmd import _not_found
    assert _not_found("abc") == 1
    out = capsys.readouterr().out
    assert "No session 'abc'" in out
    assert "nastech sessions list" in out
...
def test_sessions_db_open_failure_points_to_repair(monkeypatch, capsys):
    ...
    db_path = nastech_state._default_db_path()
    ...
    out = capsys.readouterr().out
    assert code == 1
    assert "nastech sessions repair" in out
