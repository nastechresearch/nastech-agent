...
def test_first_install_nastech_auto_configures_video_gen(monkeypatch):
    ...
    assert config["video_gen"]["provider"] == "nastech"
...
class TestAgentBrowserPostSetup:
    ...
    tools.browser_tool_install._find_agent_browser (PATH -> Homebrew/Nastech-managed
    ...
    def _stub_browser_use_install(self):
        ...
        logic under test."""
        with patch("nastech_cli.tools_config_post_setup._ensure_browser_use_cli") as stub:
            yield stub

    def test_warns_when_neither_npx_nor_agent_browser_on_path(self):
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch("nastech_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_chromium_already_installed_skips_subprocess(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "nastech_cli.tools_config_post_setup._print_success"
        ) as success:
            _run_post_setup("agent_browser")
    ...
    def test_docker_with_missing_chromium_warns_instead_of_installing(self):
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "nastech_cli.tools_config_post_setup._print_warning"
        ) as warn:
            _run_post_setup("agent_browser")
    ...
    def test_installs_chromium_via_npx_resolved_only_through_extended_path(self):
        """Nastech-managed-Node-only setups: npx resolves via
        ...
        nastech_npx = "/home/user/.nastech/node/bin/npx"
        with patch("shutil.which", return_value=None), patch(
            "subprocess.run"
        ) as run, patch(
            "tools.browser_tool_install._chromium_installed", return_value=False
            ...
            "tools.browser_tool_install._resolve_npx_bin", return_value=nastech_npx
        ), patch(
            "nastech_cli.tools_config_post_setup._print_success"
        ):
            run.return_value = SimpleNamespace(returncode=0, stdout="", stderr="")
        ...
        run.assert_called_once()
        assert run.call_args.args[0] == [
            nastech_npx, "--ignore-scripts", "-y", AGENT_BROWSER_NPX_SPEC, "install", "--with-deps",
        ]
    ...
    def test_install_failure_prints_stderr_tail_and_does_not_invalidate_cache(self):
        ...
        with patch("shutil.which", return_value="/usr/bin/npx"), patch(
            ...
            "nastech_cli.tools_config_post_setup._print_info"
        ) as info:
...
class TestBrowserUseCliInstalledForAllNonCamofoxBackends:
    ...
    def test_browser_post_setup_attempts_cli_install(self, key):
        with patch("nastech_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
    ...
    def test_camofox_post_setup_never_touches_browser_use(self):
        ...
        the CLI in."""
        with patch("nastech_cli.tools_config_post_setup._ensure_browser_use_cli") as ensure, patch(
            "nastech_constants.find_node_executable", return_value=None
        ), patch("subprocess.run"):
    ...
    def test_ensure_helper_always_delegates_to_install_cli(self):
        ...
        $NASTECH_HOME/bin when only side installs exist."""
        with patch(
            "nastech_cli.tools_config_post_setup.shutil.which", return_value="/usr/bin/browser-use"
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(True, "browser-use CLI already installed (/managed/bin/browser-use)"),
        ) as install:
            from nastech_cli.tools_config import _ensure_browser_use_cli
    ...
    def test_ensure_helper_install_failure_is_non_fatal(self):
        ...
        with patch(
            "nastech_cli.tools_config_post_setup.shutil.which", return_value=None
        ), patch(
            "tools.browser_use_cli.install_cli",
            return_value=(False, "`uv tool install browser-use` failed:\nboom"),
        ), patch("nastech_cli.tools_config_post_setup._print_warning") as warn:
...
class TestImagegenBackendRegistry:
    ...
    def test_image_gen_providers_tagged_with_registered_backend(self):
        ...
        from nastech_cli.tools_config import IMAGEGEN_BACKENDS, TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_plugin_picker_falls_back_when_default_is_missing_from_catalog(self):
        ...
        from nastech_cli.tools_config import _configure_imagegen_model_for_plugin
        ...
        with (
            patch(
                "nastech_cli.tools_config._plugin_image_gen_catalog",
                return_value=(catalog, "also-missing"),
            ),
            patch("nastech_cli.tools_config._prompt_choice", return_value=0),
        ):
...
def test_vision_picker_custom_endpoint(tmp_path, monkeypatch):
    ...
    import nastech_cli.tools_config_providers as tcp
...
#
# FAL, Krea and Portal models all live behind the single "Nastech Subscription" row; the stored
# model id picks the gateway. Before, the Portal plugin rendered its own row that also wrote
# `provider: nastech`, so two rows read active at once and the Portal pick generated on FAL.


def _managed_image_row() -> dict:
    return next(p for p in TOOL_CATEGORIES["image_gen"]["providers"] if p.get("managed_nastech_feature") == "image_gen")


def test_exactly_one_image_row_is_active_for_a_managed_selection(monkeypatch):
    import nastech_cli.tools_config as tools_config
    from nastech_cli.tools_config_providers import _plugin_image_gen_providers

    monkeypatch.setattr(
        tools_config, "get_nastech_subscription_features",
        lambda config, **kwargs: SimpleNamespace(features={"image_gen": SimpleNamespace(managed_by_nastech=True)}),
    )
    ...
    for model in ("fal-ai/flux-2/klein/9b", "krea-2-medium", "google/gemini-3-pro-image"):
        config = {"image_gen": {"provider": "nastech", "model": model}}
...
def test_gui_model_catalog_for_the_managed_row_spans_every_managed_gateway(monkeypatch):
    import nastech_cli.tools_config as tools_config
    from nastech_cli.web_routers.tools import _resolve_toolset_model_plugin, _toolset_model_catalog
    from plugins.image_gen.krea import KREA_MODEL_IDS
    from tools.image_generation_catalog import FAL_MODELS

    paid = NastechPortalAccountInfo(logged_in=True, source="jwt", fresh=False, paid_service_access=True)
    monkeypatch.setattr(
        tools_config, "get_nastech_subscription_features",
...
def test_pool_only_account_is_offered_fal_models_only(monkeypatch):
    import nastech_cli.tools_config as tools_config
    from nastech_cli.tools_config_providers import _managed_image_catalog

    pool = NastechPortalAccountInfo(
        logged_in=True, source="jwt", fresh=False, paid_service_access=False,
        tool_access=NastechToolAccessInfo(enabled=True, coverage={"fal": True, "krea": False}))
    monkeypatch.setattr(
        tools_config, "get_nastech_subscription_features",
...
def test_agent_disabled_toolsets_json_array_string_form_still_wins():
    ...
    `nastech config set agent.disabled_toolsets '["memory"]'`). It must be
...
def test_disabled_composite_debugging_prunes_constituent_platform_toolsets():
    ...
    toolsets on ``nastech tools --summary``, not only strip them at runtime."""
    config = {
        "platform_toolsets": {"cli": ["nastech-cli"]},
...
def test_explicit_plugin_toolset_admitted_in_platform_toolsets(monkeypatch):
    ...
    ``nastech-cli``), it MUST be admitted as a configurable key instead of
    ...
    import nastech_cli.tools_config as _tc_mod
    ...
    # standard composite — exactly the "I want nastech-cli AND a2a in my CLI
    # session" config the issue's user was trying to write.
    config = {"platform_toolsets": {"cli": ["nastech-cli", "dplat_client"]}}
...
def test_explicit_plugin_toolset_admitted_against_real_a2a_plugin(monkeypatch):
    ...
    a real config like ``platform_toolsets.cli: [nastech-cli, a2a]``, ``a2a``
    ...
    config = {"platform_toolsets": {"cli": ["nastech-cli", "a2a"]}}
    enabled = _get_platform_tools(config, "cli")
...
class TestLightpandaPostSetup:
    ...
    def test_reports_binary_when_found(self, _stub_browser_use_install):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value="/opt/lightpanda"), \
             patch("nastech_cli.tools_config_post_setup._print_success") as ok, \
             patch("nastech_cli.tools_config_post_setup._print_warning") as warn:
    ...
    def test_prints_install_hint_when_missing(self):
        ...
        with patch("tools.browser_lightpanda.find_lightpanda_binary", return_value=None), \
             patch("nastech_cli.tools_config_post_setup._print_warning") as warn, \
             patch("nastech_cli.tools_config_post_setup._print_info") as info:
            _run_post_setup("lightpanda")
