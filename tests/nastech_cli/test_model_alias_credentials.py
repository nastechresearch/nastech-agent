...
def _install_config(monkeypatch, alias_entry):
    ...
    monkeypatch.setattr("nastech_cli.config.load_config", lambda *a, **k: cfg)
    monkeypatch.setattr("nastech_cli.runtime_provider.load_config", lambda *a, **k: cfg)
    return cfg
...
def _switch_to_alias(monkeypatch, alias_entry):
    ...
    monkeypatch.setattr(
        "nastech_cli.models_validate.validate_requested_model", _fake_validate
    )
...
class TestDirectAliasApiKeyHelper:
    ...
    def test_resolves_literal_env_template_and_key_env(
        ...
        from nastech_cli.model_switch import DirectAlias, direct_alias_api_key
...
class TestSessionKeyIsHostScoped:
    ...
    def _switch(self, monkeypatch, session_base_url):
        ...
        monkeypatch.setattr("nastech_cli.config.load_config", lambda *a, **k: cfg)
        monkeypatch.setattr(
            "nastech_cli.runtime_provider.load_config", lambda *a, **k: cfg
        )
        monkeypatch.setattr(
...
class TestProviderLabelCannotSelectAKeyForAnArbitraryHost:
    ...
    def _switch(self, monkeypatch, alias, session_provider="openrouter",
        ...
        monkeypatch.setattr("nastech_cli.models_validate.validate_requested_model", _fake_validate)
        import nastech_cli.model_switch as ms
...
class TestSchemelessBaseUrls:
    ...
    def test_schemeless_loopback_alias_keeps_the_session_credential(self, url):
        from nastech_cli.model_switch import _may_reuse_session_credential
...
class TestOneShotUsesTheSameHostInvariant:
    """`nastech chat -m <alias>` must not trust the alias's provider label.
    ...
    def test_no_key_alias_on_a_foreign_host_gets_no_provider_token(
        ...
        monkeypatch.setenv(env_var, secret)
        monkeypatch.setattr(
            "nastech_cli.runtime_provider.load_config",
            lambda *a, **k: {"model": {"default": "m", "provider": "openrouter"}},
        )
        from nastech_cli.model_switch import DirectAlias, direct_alias_runtime_request
...
class TestNoProductionCodeMutatesTheAliasCacheInPlace:
    ...
    OWNER = ("nastech_cli/model_switch.py", "_ensure_direct_aliases")
    ...
    def test_the_scan_actually_detects_a_violation(self):
        ...
        offending = (
            "from nastech_cli.model_switch import DIRECT_ALIASES\n"
            "def warm():\n"
