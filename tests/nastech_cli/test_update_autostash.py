...
def _patch_gateway_discovery():
    ...
    ``_purge_stale_nastech_modules`` must also be stubbed: it evicts
    ``nastech_cli.gateway`` from ``sys.modules`` mid-update, and the restart
    phase's fresh ``from nastech_cli.gateway import ...`` then loads an
    ...
    with patch("nastech_cli.gateway.find_gateway_pids", return_value=[]), \
         ...
         patch("nastech_cli.update_inventory.collect_runtime_inventory", return_value=None), \
         patch("nastech_cli.update_inventory.report_unaccounted_runtimes", return_value=False), \
         patch.object(nastech_main, "_fleet_probe_expected_runtimes", lambda *a, **kw: False), \
         patch.object(nastech_main, "_purge_stale_nastech_modules", lambda *a, **kw: None), \
         patch("nastech_cli.update_receipt.collect_fleet_versions", return_value=[]):
        yield
...
def _make_update_side_effect(
    ...
    ``refs/nastech-update-backups/orphan-<branch>-*`` (oldest first) so the
...
def test_cmd_update_orphan_history_backs_up_before_reset(monkeypatch, tmp_path, capsys):
    ...
    ``refs/nastech-update-backups/orphan-*`` ref before the reset proceeds."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    assert ref_name.startswith("refs/nastech-update-backups/orphan-main-")
    ...
    assert f"expires after {nastech_main._ORPHAN_RESCUE_REF_MAX_AGE_DAYS} days" in out
...
def test_cmd_update_orphan_rescue_refs_pruned_beyond_keep_limit(monkeypatch, tmp_path, capsys):
    ...
    total = nastech_main._ORPHAN_RESCUE_REFS_TO_KEEP + 2
    stale_refs = [
        "refs/nastech-update-backups/orphan-main-"
    ...
    assert len(delete_calls) == total - nastech_main._ORPHAN_RESCUE_REFS_TO_KEEP
    deleted_refs = {c[c.index("-d") + 1] for c in delete_calls}
    assert deleted_refs == set(stale_refs[: total - nastech_main._ORPHAN_RESCUE_REFS_TO_KEEP])
...
def test_cmd_update_orphan_rescue_refs_expired_by_age(monkeypatch, tmp_path, capsys):
    ...
    now = datetime.now(timezone.utc)
    old = now - timedelta(days=nastech_main._ORPHAN_RESCUE_REF_MAX_AGE_DAYS + 5)
...
def test_prune_orphan_rescue_refs_leaves_unparseable_names_alone():
    ...
    weird = "refs/nastech-update-backups/orphan-main-not-a-timestamp-xyz"
    calls = []
    ...
    with mock_patch.object(nastech_main.subprocess, "run", side_effect=fake_run):
        nastech_main._prune_orphan_rescue_refs(["git"], ".", "main")
...
def _setup_keep_stash_test(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        nastech_main, "_park_stashed_changes",
    ...
    monkeypatch.setattr(
        "nastech_cli.gateway.find_gateway_pids", lambda **kw: [], raising=False
    )
...
def test_update_keep_stash_parks_instead_of_restoring(monkeypatch, tmp_path):
    ...
    nastech_main.cmd_update(SimpleNamespace(yes=True, keep_stash=True))
...
def test_update_without_keep_stash_still_restores(monkeypatch, tmp_path):
    ...
    nastech_main.cmd_update(SimpleNamespace(yes=True, keep_stash=False))
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    assert "made the Nastech agent unexecutable" in output
...
def test_restore_allows_preexisting_import_time_failure(monkeypatch, tmp_path):
    ...
    assert nastech_main._restore_stashed_changes(
        ["git"], tmp_path, stash_ref, prompt_user=False
    )
...
def test_prune_orphan_rescue_refs_with_real_git_unpins_objects(tmp_path):
    ...
    old_ref = "refs/nastech-update-backups/orphan-main-20200101-000000-" + snap_sha[:12]
    ...
    nastech_main._prune_orphan_rescue_refs(["git"], tmp_path, "main")
    remaining = git("for-each-ref", "refs/nastech-update-backups/").stdout
