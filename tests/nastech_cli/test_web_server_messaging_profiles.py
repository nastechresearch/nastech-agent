...
def test_named_current_home_matches_unscoped(client, isolated_profiles, monkeypatch):
    ...
    with _nastech_home_scope(isolated_profiles["worker_alpha"]):
        with _config_profile_scope("default") as scoped:
            assert scoped is None
            assert get_nastech_home() == isolated_profiles["default"]
...
def test_scoped_enablement_uses_only_own_credentials(client, isolated_profiles, monkeypatch):
    ...
    from nastech_cli.web_server_messaging import _messaging_platform_catalog
...
def test_credential_write_hot_serves_a_multiplexed_profile(client, isolated_profiles, monkeypatch, topology):
    ...
    ``?profile=`` and a pooled ``nastech --profile X serve`` that receives the PUT unscoped (#109088)."""
    import nastech_cli.gateway as gateway_cli
    import nastech_cli.gateway_multiplex_served as served_mod
    notified = []
