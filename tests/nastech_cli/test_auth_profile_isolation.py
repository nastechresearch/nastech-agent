...
A named profile (``NASTECH_HOME`` under ``profiles/<name>``) resolves provider state and the
credential pool from ITS OWN ``auth.json`` only. The root ``~/.nastech/auth.json`` is never a
...
def profile_env(tmp_path, monkeypatch):
    """Global root + an active named profile under Path.home()/.nastech/profiles/coder.
...
_ROOT_STORE = _make_auth_store(
    providers={
        "nastech": {"access_token": "nastech-root", "refresh_token": "rt-root"},
...
def test_named_profile_never_reads_the_root_store(profile_env):
    ...
    from nastech_cli.auth import get_provider_auth_state, read_credential_pool, resolve_codex_runtime_credentials
    from nastech_cli.auth_xai import _read_xai_oauth_tokens
    ...
    token = set_nastech_home_override(own)
    try:
        assert get_provider_auth_state("nastech")["access_token"] == "nastech-root"
...
def test_profile_refresh_never_writes_through_to_the_root_store(profile_env):
    ...
    from nastech_cli.auth_codex import _save_codex_tokens, clear_codex_pool_quota_cooldowns
    from nastech_cli.auth_xai import _save_xai_oauth_tokens
