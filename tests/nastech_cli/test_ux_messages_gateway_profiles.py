...
next command, and every ``nastech <sub> <cmd>`` it cites is a registered subcommand.
"""
...
from nastech_cli.update_lock import UpdateHolder, describe_holder
...
def test_systemctl_failure_is_explained_not_tracebacked(monkeypatch, capsys):
    exc = subprocess.CalledProcessError(1, ["systemctl", "--user", "start", "nastech-gateway"])
    out = _run_gateway_command_raising(monkeypatch, capsys, exc)
    assert not out.lstrip().startswith("Command '")
    assert "nastech gateway status --deep" in out
    assert "nastech gateway install --force" in out
...
def test_existing_gateway_guard_says_bots_are_online_and_names_status(monkeypatch, capsys):
    ...
    for cmd in ("nastech gateway status", "nastech gateway restart", "nastech gateway stop"):
        assert cmd in out
...
def test_unsupported_platform_copy_says_what_is_unsupported(subcommand):
    ...
    assert "nastech gateway run" in text or subcommand == "uninstall"
...
def test_invalid_profile_name_explains_rule_in_words_with_example():
    ...
    assert "nastech profile create my-work" in msg
...
def test_existing_profile_error_offers_use_and_list(tmp_path, monkeypatch):
    ...
    assert "nastech profile use work" in msg
    assert "nastech profile list" in msg
...
def test_describe_holder_avoids_jargon_and_names_next_steps():
    ...
    assert "nastech logs" in message
    assert "nastech update" in message
