...
Covers:
  ...
  - Namespace spoofing (nastech:, foreign, already-colon'd) is rejected
  ...
  - `nastech plugins show` output includes emits/listens
"""
...
@pytest.mark.parametrize(
    "bad_event",
    [
        "nastech:x",   # reserved core prefix
...
def test_emit_rejects_namespaced_names(bad_event):
    ...
    for name in (bad_event, f"b:{bad_event}", "nastech:x", "a:x", "b:x"):
...
def test_subscribe_is_unrestricted():
    """Any plugin may subscribe to any event, including nastech: and foreign."""
    manager = _fresh_manager()
    ctx_a = _make_ctx(manager, "plugin_a", key="a")
    got = []
    # None of these raise — only emit is namespace-gated.
    ctx_a.subscribe("nastech:core_event", lambda **p: got.append("nastech"))
    ctx_a.subscribe("b:ping", lambda **p: got.append("b"))
    assert "nastech:core_event" in manager._subscriptions
    assert "b:ping" in manager._subscriptions
...
def test_pending_budget_drops_new_event_without_blocking(monkeypatch, caplog):
    from nastech_cli import plugins_dispatch
