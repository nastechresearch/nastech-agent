...
"unknown" — and on that answer the Nastech profile drops a "thinking off" disable
rather than risk a 400. A short-lived process (``nastech -p``, a cron job, a
...
def cold_process(monkeypatch):
    ...
    def _reset():
        for name in (
            "_nastech_reasoning_caps_cache",
            "_nastech_reasoning_caps_failed_at",
            "_openrouter_reasoning_caps_cache",
            "_openrouter_reasoning_caps_failed_at",
        ):
            monkeypatch.setattr(models_mod, name, None)
        for name in (
            "_nastech_caps_disk_checked",
            "_nastech_caps_warm_started",
...
def test_fetched_catalog_answers_a_later_process_offline(
    ...
    caps = models_reasoning_caps.nastech_model_reasoning_capabilities("deepseek/deepseek-v4-pro")
    assert caps["mandatory"] is False
    mandatory = models_reasoning_caps.nastech_model_reasoning_capabilities(
...
def test_mirror_is_keyed_by_catalog_url(cold_process, offline, monkeypatch):
    ...
    models_reasoning_caps.nastech_model_reasoning_capabilities(
        "deepseek/deepseek-v4-pro", allow_fetch=True
    )
...
def test_stale_copy_is_still_served(cold_process, offline, monkeypatch):
    ...
    """
    url = models_reasoning_caps.nastech_catalog_url()
...
def test_unreadable_mirror_degrades_to_unknown(cold_process, offline, monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_urlopen_model_catalog_request", offline)
    assert models_reasoning_caps.nastech_model_reasoning_capabilities("deepseek/deepseek-v4-pro") is None
...
def test_a_missing_mirror_is_looked_for_once_per_process(cold_process, monkeypatch):
    ...
    """
    counts = {"url": 0, "read": 0}
    real_url = models_reasoning_caps.nastech_catalog_url
    ...
    monkeypatch.setattr(models_reasoning_caps, "nastech_catalog_url", _counting_url)
    monkeypatch.setattr(models_reasoning_caps, "_read_reasoning_caps_disk", _counting_read)
    for _ in range(5):
        models_reasoning_caps.nastech_model_reasoning_capabilities("deepseek/deepseek-v4-pro")
