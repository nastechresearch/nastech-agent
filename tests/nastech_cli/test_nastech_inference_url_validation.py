...
class TestCallSiteWiring:
    ...
    def _read_auth_source(self):
        # The Nastech refresh sites live in auth_nastech.py (split out of auth.py);
        # read both so the guard tolerates relocation but still fires on deletion.
        import nastech_cli.auth as _auth_mod
        import nastech_cli.auth_nastech as _nastech_mod
        from pathlib import Path
        return "".join(
            Path(m.__file__).read_text(encoding="utf-8") for m in (_auth_mod, _nastech_mod)
        )
    ...
    def test_validator_wired_at_all_known_call_sites(self):
        ...
        inference URL through ``_healed_nastech_inference_url`` (which applies the
        ...
        source = self._read_auth_source()
        assert (
            source.count('_validate_nastech_inference_url_from_network(refreshed.get("inference_base_url"))')
            == 1
        ), "the validator must be applied exactly once, inside _healed_nastech_inference_url"
        refresh_count = source.count("_healed_nastech_inference_url(refreshed)")
...
class TestHealsPoisonedStoredValue:
    ...
    def test_refresh_resets_rejected_url_to_default(self, monkeypatch):
        ...
        monkeypatch.setattr(nastech_cli_auth_nastech, "_nastech_invoke_jwt_status", lambda *a, **k: "needs_refresh")
        monkeypatch.setattr(
            auth,
        ...
        monkeypatch.setattr(nastech_cli_auth_nastech, "_assert_nastech_inference_jwt_usable", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_select_nastech_invoke_jwt", lambda *a, **k: None)
        monkeypatch.setattr(nastech_cli_auth_nastech, "_select_nastech_invoke_jwt", lambda *a, **k: None)
...
class TestEnvOverrideWins:
    ...
    def _patch_no_refresh(self, monkeypatch, auth, state):
        ...
        monkeypatch.setattr(nastech_cli_auth_nastech, "_nastech_invoke_jwt_status", lambda *a, **k: None)
        monkeypatch.setattr(
        ...
        monkeypatch.setattr(nastech_cli_auth_nastech, "_write_shared_nastech_state", lambda *a, **k: None)
        monkeypatch.setattr(auth, "_sync_nastech_pool_from_auth_store", lambda *a, **k: None)
        monkeypatch.setattr(nastech_cli_auth_nastech, "_sync_nastech_pool_from_auth_store", lambda *a, **k: None)
