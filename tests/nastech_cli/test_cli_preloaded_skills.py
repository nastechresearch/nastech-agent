...
def _real_finalize(cli_obj):
    """Call the real NastechCLI.finalize_preloaded_skills on a dummy object."""
    return _REAL_FINALIZE(cli_obj)


def _capture_real_finalize():
    import cli as cli_mod
    return cli_mod.NastechCLI.__dict__["finalize_preloaded_skills"]
...
def test_main_applies_preloaded_skills_to_system_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda skills, task_id=None, excluded_loaded_names=None: ("skill prompt", ["nastech-agent-dev", "github-auth"], []),
    )
...
def test_show_banner_does_not_print_skills():
    ...
    with patch("nastech_cli.banner.build_welcome_banner") as mock_banner, patch(
