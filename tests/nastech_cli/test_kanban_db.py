...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    import nastech_cli.kanban_db as _kb
    from nastech_cli import kanban_db_dispatch as _kbd
...
def test_infrastructure_spawn_refusal_never_charges_the_card(
    ...
    monkeypatch.setenv("NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")

    def spawn_via_real_boundary(task, workspace, board=None):
        kbd._restart_safe_worker_argv(task, ["nastech", "chat"])  # raises: real probe verdict, real _degrade()
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see nastech_state_wal.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(tmp_path, monkeypatch, caplog):
    ...
    import nastech_state_wal as _nastech_state_wal
    monkeypatch.setattr(
        _nastech_state_wal, "is_sqlite_wal_reset_vulnerable",
        lambda version_info=None: False,
    )
    _nastech_state_wal._wal_fallback_warned_paths.clear()

    # Clear module cache so a fresh connect() is attempted
    kb._INITIALIZED_PATHS.clear()
    nastech_state_wal._wal_fallback_warned_paths.clear()
...
# only falls back to the PATH shim when ``nastech_cli`` is not importable.
# ---------------------------------------------------------------------------


def test_resolve_nastech_argv_prefers_module_form_over_path_shim(monkeypatch):
    """A `nastech` on PATH must not shadow the running install (#111569):
    the module argv wins whenever ``nastech_cli`` is importable; only an
    explicit ``$NASTECH_BIN`` overrides it."""
    import shutil
    import sys
    from nastech_cli import kanban_db_dispatch as kbd

    monkeypatch.delenv("NASTECH_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/tmp/planted/nastech")
    monkeypatch.setattr(kbd, "_safe_which_no_cwd", lambda name: "/tmp/planted/nastech")
    assert kbd._resolve_nastech_argv() == [sys.executable, "-m", "nastech_cli.main"]

    monkeypatch.setenv("NASTECH_BIN", "/opt/nastech/bin/nastech")
    assert kbd._resolve_nastech_argv() == ["/opt/nastech/bin/nastech"]
...
def test_resolve_nastech_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    argv = kbd._resolve_nastech_argv()
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    """
    import struct
    from nastech_cli.kanban_db_connect import connect
