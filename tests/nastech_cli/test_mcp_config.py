...
class TestMcpTest:
    ...
    def test_exit_codes_distinguish_failure_from_unknown_server(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.mcp_config._probe_single_server", lambda name, cfg, **kw: [])
        ...
        monkeypatch.setattr("nastech_cli.mcp_config._probe_single_server", failing_probe)
    ...
    def test_cli_dispatcher_forwards_test_exit_code(self, tmp_path, monkeypatch):
        """``nastech mcp test`` reaches ``main()`` with the handler's code (the dispatcher used to drop it)."""
        _seed_config(tmp_path, {"ink": {"url": "https://mcp.ml.ink/mcp"}})
        from nastech_cli.main import cmd_mcp
...
class TestProbeEnvResolution:
    ...
    def test_probe_propagates_explicit_connect_timeout_to_config(self, monkeypatch):
        """An explicit `connect_timeout=` override (e.g. `nastech mcp login`'s 315s, extended so a
...
def test_tool_filters_keeps_explicit_empty_include():
    ...
    from nastech_cli.mcp_config import _tool_filters
