...
class TestClaudeCodeImport:
    ...
    def test_allowlist_lands_in_config_yaml(self, report, nastech_home):
        config = yaml.safe_load((nastech_home / "config.yaml").read_text(encoding="utf-8"))
...
class TestMalformedInputs:
    def test_bad_settings_json_reports_error(self, profile_env, nastech_home):
        ...
        assert "still importable" in (
            nastech_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
...
class TestMergeSemantics:
    ...
    def test_reimport_is_idempotent_for_memory(self, claude_tree, nastech_home):
        run_import("claude-code", claude_tree, nastech_home, execute=True)
        first = (nastech_home / "memories" / "MEMORY.md").read_text(encoding="utf-8")
        report = run_import("claude-code", claude_tree, nastech_home, execute=True)
        assert (nastech_home / "memories" / "MEMORY.md").read_text(encoding="utf-8") == first
...
class TestSyncManifest:
    ...
    def test_import_registers_source_and_unchanged_sync_is_noop(
            self, claude_tree, nastech_home, capsys):
        from nastech_cli.agent_import_sync import load_sync_manifest

        self._run_command("claude-code", claude_tree)
        entry = load_sync_manifest(nastech_home)["agents"]["claude-code"]
    ...
    def test_sync_reimports_changed_source_but_never_clobbers_user_skill(
        ...
        user_skill = nastech_home / "skills" / "claude-code-imports" / "hand-rolled"
        ...
        imports = nastech_home / "skills" / "claude-code-imports"
        ...
        # An imported skill the user then EDITED locally is no longer Nastech-owned: the next sync
    ...
    def test_sync_dry_run_previews_without_writing(self, claude_tree, nastech_home, capsys):
        from nastech_cli.agent_import_sync import load_sync_manifest

        self._run_command("claude-code", claude_tree)
        old_digest = load_sync_manifest(nastech_home)["agents"]["claude-code"]["digest"]
        (claude_tree / "CLAUDE.md").write_text(
            CLAUDE_MD + "\n- Dry sync entry\n", encoding="utf-8")
        before = snapshot_tree(nastech_home)
        capsys.readouterr()
        ...
        assert snapshot_tree(nastech_home) == before
        assert load_sync_manifest(nastech_home)["agents"]["claude-code"]["digest"] == old_digest
