...
def test_check_via_local_git_fetch_failure_returns_none(tmp_path, monkeypatch):
    ...
    def mock_git_stdout(args, *, cwd, timeout=5):
        if args[:2] == ["remote", "get-url"]:
            return "https://github.com/NastechResearch/nastech-agent.git"
...
def test_check_for_updates_does_not_cache_none(tmp_path, monkeypatch):
    ...
    cache_file = tmp_path / ".update_check"
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
    monkeypatch.delenv("NASTECH_REVISION", raising=False)
    ...
    monkeypatch.setattr(
        "nastech_cli.config.detect_install_method", lambda root: "git"
    )
    monkeypatch.setattr(
        "nastech_cli.config.get_project_root", lambda: repo_dir
    )
    ...
    # Simpler: just patch the get_nastech_home and the repo_dir resolution
    # by making check_for_updates find our fake repo via nastech_home fallback.
    # The code checks Path(__file__).parent.parent/.git first, then falls
    # back to nastech_home / "nastech-agent". We ensure the fallback hits.
    # To do this, we make Path(__file__).parent.parent.resolve() return
    # a path without .git, so it falls through to nastech_home / "nastech-agent".
    real_resolve = Path.resolve

    def fake_resolve(self, *args, **kwargs):
        s = str(self)
        if "banner.py" in s or s.endswith("nastech_cli"):
