"""Comprehensive tests for nastech_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and NASTECH_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.nastech/profiles)
    * NASTECH_HOME  -> tmp_path/.nastech  (so get_nastech_home() agrees)
    * Creates the bare-minimum ~/.nastech directory.
    """
...
class TestGetProfileDir:
    ...
    def test_default_returns_nastech_home(self, profile_env):
...
class TestCreateProfile:
    ...
    def test_fresh_profile_inherits_a_usable_model(self, profile_env):
        ...
        """
        default_home = profile_env / ".nastech"
        ...
        assert cfg["model"]["provider"] == "nastech"
    ...
    def test_clone_sync_imports_carries_manifest_but_never_links_profiles(self, profile_env):
        ...
        from nastech_cli.agent_import_sync import SYNC_MANIFEST_NAME, load_sync_manifest
...
class TestNoSkillsOptOut:
    """Tests for `nastech profile create --no-skills` and the opt-out marker."""
    ...
    def test_delete_marker_re_enables_seeding(self, profile_env, monkeypatch):
        ...
        called = []
        stdout_by_call = [
            '{"copied": ["nastech-agent"], "skipped_opt_out": true}',
            '{"copied": []}',
        ]
        monkeypatch.setattr(
            "subprocess.run",
        ...
        assert r1.get("copied") == ["nastech-agent"]
        assert len(called) == 1
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `nastech update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".nastech" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
...
class TestDeleteProfile:
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("nastech_cli.profiles._cleanup_gateway_service"), \
             patch("nastech_cli.profiles.time.sleep"), \
             patch("nastech_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_delete_purges_profile_keyed_identity(self, profile_env):
        ...
        scope = str(tmp_path / ".nastech" / "sessions")
        db = SessionDB(tmp_path / ".nastech" / "state.db")
        ...
        check = SessionDB(tmp_path / ".nastech" / "state.db")
        try:
    ...
    def test_delete_reports_pending_settlement_for_a_live_multiplexer(self, profile_env, capsys):
        ...
        """
        from nastech_state import SessionDB
        from nastech_cli.profiles import ProfileIdentitySettlementPending
        ...
        with patch("nastech_cli.profiles._cleanup_gateway_service"), \
             patch("nastech_cli.profiles._live_default_multiplexer", return_value=True):
        ...
        assert ei.value.retry_command == "nastech profile purge-identity gone"
        assert not ei.value.path.exists()
        assert isinstance(ei.value, RuntimeError)  # the CLI handler catches RuntimeError

        assert "nastech profile purge-identity gone" in capsys.readouterr().err
        check = SessionDB(tmp_path / ".nastech" / "state.db")
        try:
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "nastech_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "nastech_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "nastech_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "nastech_cli.main", "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_shebang_exec_of_nastech_shim(self, profile_env, monkeypatch):
        """A `nastech` console-script shim spawned directly (e.g. Electron's
        findOnPath('nastech') resolution) reports argv[0] as the interpreter
        (python3) and argv[1] as the shim's path -- not "nastech" -- because
        ...
        procs = [
            ...
            # being the python interpreter, not "nastech".
            FakeProc(201, ["/usr/bin/python3", "/Users/x/.local/bin/nastech", "--profile", "coder", "serve",
                            "--host", "127.0.0.1", "--port", "0"]),
            # Same shape but a different profile → skipped.
            FakeProc(202, ["/usr/bin/python3", "/Users/x/.local/bin/nastech", "--profile", "other", "serve"]),
            # Non-nastech script run by python3 → skipped.
    ...
    def test_backend_scan_rejects_unrelated_nastech_prefixed_script(self, profile_env, monkeypatch):
        """A user's own script that happens to start with "nastech" (e.g.
        nastech-notes.py, nastech-unrelated-tool) must NOT be misidentified as
        the console-script shim just because argv[0] is a python interpreter
        and argv[1]'s basename starts with "nastech" -- only the actual known
        console-script entry points (nastech, nastech-agent, nastech-acp) count.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        procs = [
            ...
            FakeProc(301, ["/usr/bin/python3", "/Users/x/scripts/nastech-notes.py",
                            "--profile", "coder", "serve"]),
            FakeProc(302, ["/usr/bin/python3", "/Users/x/scripts/nastech-unrelated-tool",
                            "--profile", "coder", "serve"]),
        ]
    ...
    def test_backend_scan_matches_all_known_console_script_shims(self, profile_env, monkeypatch):
        """The other two real console-script entry points (nastech-agent,
        nastech-acp -- see pyproject.toml [project.scripts]) must also be
        recognized via the shebang-exec path, not just the primary "nastech"
        shim.
        """
        create_profile("coder", no_alias=True)
        profile_dir = get_profile_dir("coder")
        ...
        self_pid = os.getpid()
        procs = [
            FakeProc(401, ["/usr/bin/python3", "/Users/x/.local/bin/nastech-agent",
                            "--profile", "coder", "serve"]),
            FakeProc(402, ["/usr/bin/python3", "/Users/x/.local/bin/nastech-acp",
                            "--profile", "coder", "serve"]),
        ]
...
class TestListProfiles:
    ...
    def test_lazy_skill_count_never_walks_in_the_polled_request(self, profile_env, monkeypatch):
        ...
        skills = profile_env / ".nastech" / "skills" / "cat"
        for i in range(3):
        ...
        first = _rpc()
        assert walks == [] or set(walks) == {"nastech-skill-count"}
        assert first[0]["skill_count"] in (0, 3)  # 0 until the refresh lands, never a stall
        for t in threading.enumerate():
            if t.name == "nastech-skill-count":
                t.join(timeout=10)
        assert walks == ["nastech-skill-count"]
        assert _rpc()[0]["skill_count"] == 3
        assert list_profiles(lazy_skill_count=True)[0].skill_count == 3
        assert walks == ["nastech-skill-count"]  # a second poll inside the window schedules nothing
        ...
        from nastech_cli.web_routers import profiles as profiles_router
    ...
    def test_skill_count_survives_subtree_vanishing_mid_walk(self, profile_env, monkeypatch):
        ...
        assert profiles._count_skills(profile_env / ".nastech") == 3
...
class TestActiveProfile:
    ...
    def test_set_to_default_removes_file(self, profile_env):
        ...
        create_profile("coder", no_alias=True)
        set_active_profile("coder")
        active_path = tmp_path / ".nastech" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom NASTECH_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("NASTECH_HOME", str(custom))
        # With Docker-aware roots, a custom NASTECH_HOME is the default —
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("nastech_cli.profiles.shutil.which", lambda name: "/opt/nastech/bin/nastech")
        from nastech_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/nastech/bin/nastech -p mybot" in content


    @pytest.mark.windows_only
    def test_remove_finds_bat_on_windows(self, profile_env):
        from nastech_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env):
        from nastech_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env):
        ...
        from nastech_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".nastech" / "profiles" / "oldname"
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "nastech.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["nastech_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["nastech_heimdall"]["peerName"] == "user-peer"
    ...
    def test_multiplexed_rename_unroutes_old_then_hot_serves_new(self, profile_env):
        ...
        create_profile("oldname", no_alias=True)
        old_dir = tmp_path / ".nastech" / "profiles" / "oldname"
        new_dir = tmp_path / ".nastech" / "profiles" / "newname"
        ...
        def _record_notify(name):
            ...
            if name == "oldname" and old_dir.exists():
                ...
                with pytest.raises(FileNotFoundError):
                    mkdir_under_nastech_home(old_dir / "logs")

        with patch("nastech_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("nastech_cli.profiles._live_default_multiplexer", return_value=True), \
             patch("nastech_cli.profiles._notify_multiplexer", side_effect=_record_notify):
    ...
    def test_unmultiplexed_rename_does_not_signal_multiplexer(self, profile_env):
        ...
        with patch("nastech_cli.profiles.check_alias_collision", return_value="skip"), \
             patch("nastech_cli.profiles._live_default_multiplexer", return_value=False), \
             patch("nastech_cli.profiles._notify_multiplexer") as notify:
            new_dir = rename_profile("oldname", "newname")

        notify.assert_not_called()
        assert not (tmp_path / ".nastech" / "profiles" / ".deleted").exists()
        assert not old_dir.exists() and new_dir.is_dir()
    ...
    def test_rename_migrates_session_identity_without_live_gateway(self, profile_env):
        ...
        pdb.close()
        root_db = SessionDB(tmp_path / ".nastech" / "state.db")
        ...
        moved_db.close()
        root_db2 = SessionDB(tmp_path / ".nastech" / "state.db")
    ...
    def test_migrate_identity_command_repairs_a_failed_live_migration(self, profile_env, capsys):
        """The failed-live-migration end state must be recoverable: `nastech profile
        ...
        with patch("nastech_cli.profiles.check_alias_collision", return_value="skip"), \
             ...
             patch("gateway.control_socket.migrate_gateway_profile_identity", return_value=None):
            rename_profile("oldname", "newname")
        assert "nastech profile migrate-identity oldname newname" in capsys.readouterr().err

        # Gateway restarted/stopped → the retry command repairs both stores.
        with patch("nastech_cli.profiles._live_default_multiplexer", return_value=False):
        ...
        moved_db = SessionDB(tmp_path / ".nastech" / "profiles" / "newname" / "state.db")
        row = moved_db._read_one(
            "SELECT session_key, profile_name FROM sessions WHERE id = ?", ("sess1",))
...
class TestExportImport:
    ...
    def test_export_default_includes_profile_data(self, profile_env, tmp_path):
        ...
        # Write through NASTECH_HOME, not get_profile_dir("default"): the latter resolves to the
        ...
        default_dir = profile_env / ".nastech"
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultNastechHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_nastech_home()."""



    def test_default_nastech_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_nastech_home() returns NASTECH_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(docker_home))
        home = _get_default_nastech_home()
        assert home == docker_home



    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under NASTECH_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("NASTECH_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="nastech gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with NASTECH_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("NASTECH_HOME", str(get_profile_dir("coder")))
...
class TestResolveProfileEnvSpelling:
    """resolve_profile_env() keeps the configured NASTECH_HOME spelling as
    ...
    def test_resolution_matrix_preserves_configured_spelling(self, monkeypatch, tmp_path):
        ...
        custom = tmp_path / "custom-nastech"
        ...
        for env_home, profile, expected in cases:
            monkeypatch.setenv("NASTECH_HOME", str(env_home))
    ...
    def test_unset_env_falls_back_to_default_root(self, monkeypatch):
        # No NASTECH_HOME: the platform default root applies (existing contract).
        monkeypatch.delenv("NASTECH_HOME", raising=False)
        assert Path(resolve_profile_env("default")) == _get_default_nastech_home()
...
class TestCloneAllExcludesRuntimeTrees:
    ...
    runtime trees the local-models flow puts under ``~/.nastech``: ``models/``
    ...
    def test_runtime_trio_is_one_constant_shared_with_backup(self):
        ...
        from nastech_cli import backup, profiles
        from nastech_constants import LOCAL_RUNTIME_ROOT_DIRS
