...
1. The standalone daemon path (``nastech kanban daemon --force`` /
   :func:`nastech_cli.kanban_db_dispatch.run_daemon`) never resolved
...
def test_run_daemon_resolves_and_passes_max_in_progress(
    ...
    ``nastech kanban dispatch`` paths both resolved the memory-derived
    default.
    """
    captured: dict = {}
...
def test_nonspawnable_review_does_not_tax_ready_budget(
    ...
    import nastech_cli.config as cfgmod
    import nastech_cli.profiles as profmod
