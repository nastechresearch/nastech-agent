...
from nastech_cli import process_identity
from nastech_cli import update_cmd_fleet as fleet
from nastech_cli import update_receipt
from nastech_cli.update_inventory import RuntimeRecord, UpdatePlan
...
def test_manual_deferral_survives_receipt_rotation(monkeypatch, capsys, kind, condition):
    ...
    monkeypatch.setattr("nastech_cli.update_cmd._finish_dashboard_update_cleanup", lambda *a, **k: None)
    monkeypatch.setattr("nastech_cli.gateway_migrate.maybe_auto_migrate_after_update", lambda: None)
    ...
    if condition == "unknown":
        ...
        (get_nastech_home() / "serve_restart_pending").write_text("not a directory")
    elif condition == "missing-identity":
        runtime.detail.clear()
    elif condition == "failed-unit":
        restart.failed_or_stale_units.append("nastech-serve-work.service")
    ...
    assert "nastech-serve.service" not in capsys.readouterr().out
    update_receipt.begin_update_receipt()
    ...
    assert "nastech gateway restart" not in warning
    monkeypatch.setattr(process_identity, "_pid_alive_matches", lambda *a: None)
    fleet._warn_pending_fleet_restart_on_startup()
...
def test_historical_manual_obligation_does_not_block_healthy_gateway(monkeypatch, capsys, alive, historical_fleet, marker, gateway_present):
    ...
    if not gateway_present:
        ...
        receipt["fleet"] = []
    root = get_nastech_home() / "logs" / "update_receipts"
    root.mkdir(parents=True, exist_ok=True)
    (root / "latest.json").write_text(json.dumps(receipt))
    monkeypatch.setattr(fleet, "_current_checkout_sha", lambda: "new")
    monkeypatch.setattr("nastech_cli.update_cmd._current_checkout_sha", lambda: "new")
    ...
    assert ("nastech gateway restart" in warning) is pending
...
def test_stamped_manual_only_history_has_no_gateway_obligation(monkeypatch, capsys, marker):
    ...
    assert ("nastech gateway restart" in warning) is marker
...
def test_historical_retention_is_independent_of_plan_order(monkeypatch, capsys, manual_first, unsupported):
    ...
    assert list((get_nastech_home() / "serve_restart_pending").glob("*.json"))
    update_receipt.begin_update_receipt()
    update_receipt.finalize_update_receipt("success", fleet=[])
    fleet._warn_pending_fleet_restart_on_startup()
...
def test_historical_retention_failure_warns_and_survives_rotation(monkeypatch, capsys, failure, gateway_state):
    ...
    from nastech_cli import update_serve_obligations as obligations
    ...
    directory = get_nastech_home() / "serve_restart_pending"
