"""Fixtures shared across nastech_cli tests."""
...
def _inline_post_swap_handoff(request, monkeypatch):
    """Run the post-swap tail in-process instead of re-executing ``nastech update --post-swap``.
    ...
    if request.node.get_closest_marker("real_post_swap_handoff"):
        return
    try:
        from nastech_cli import update_cmd, update_receipt
    except Exception:
        return
...
def isolated_update_runtime(monkeypatch, tmp_path, request):
    ...
    from nastech_cli import gateway, main, update_cmd, update_cmd_fleet
    from nastech_cli import update_inventory, update_receipt
