"""Every Nastech model list is narrowed to the org's policy before it is shown.
...
def policy(monkeypatch):
    ...
    monkeypatch.setattr(models_pricing, "nastech_policy_allowed_ids", lambda **_k: ALLOWED)
    return ALLOWED
...
def no_policy(monkeypatch):
    ...
    monkeypatch.setattr(models_pricing, "nastech_policy_allowed_ids", lambda **_k: None)


class TestLoginNastech:
    ...
    def _run(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(models_mod, "get_curated_nastech_model_ids", lambda: list(CURATED))
        monkeypatch.setattr(models_pricing, "get_pricing_for_provider", lambda _p: {})
        monkeypatch.setattr(models_mod, "check_nastech_free_tier", lambda **_k: None)
        monkeypatch.setattr(
            models_mod,
        ...
        auth_mod._login_nastech(args, auth_mod.PROVIDER_REGISTRY["nastech"])
        return seen
...
class TestModelSwitchPicker:
    """The ``/model`` picker's nastech branch (``list_authenticated_providers``)."""
    ...
    def _rows(self, monkeypatch):
        ...
        monkeypatch.setattr(
            auth_mod,
            "_load_auth_store",
            lambda *a, **k: {"providers": {"nastech": {"access_token": "tok"}}},
        )
        monkeypatch.setattr(models_mod, "get_curated_nastech_model_ids", lambda: list(CURATED))
        monkeypatch.setattr(models_pricing, "get_pricing_for_provider", lambda _p: {})
        monkeypatch.setattr(models_mod, "check_nastech_free_tier", lambda **_k: None)
        ...
        return next((r for r in rows if r["slug"] == "nastech"), None)

    def test_hidden_model_is_filtered(self, monkeypatch, policy):
        row = self._rows(monkeypatch)
        assert row is not None, "nastech row should be listed"
...
class TestRecommendedDefaultEndpoint:
    ...
    def _call(self, monkeypatch):
        ...
        return get_recommended_default_model(provider="nastech")
...
class TestAuxiliaryFastModel:
    ...
    def _pick(self, monkeypatch, *, catalog):
        ...
        monkeypatch.setattr(
            models_pricing, "_resolve_nastech_pricing_credentials",
            lambda: ("sk-nastech", "https://inference.example.com"),
        )
        monkeypatch.setattr(models_pricing, "fetch_models_with_pricing", _fake_fetch)
        picked = aux._fast_model_from_catalog("nastech")
        return picked, seen

    def test_reads_the_catalog_with_nastech_oauth_credentials(self, monkeypatch, no_policy):
        """The api-key resolver raises for OAuth providers."""
        _, seen = self._pick(monkeypatch, catalog=["vendor/haiku-fast"])
        assert seen["api_key"] == "sk-nastech"

    def test_hidden_model_is_not_selected(self, monkeypatch, policy):
        import agent.auxiliary_client as aux

        monkeypatch.setattr(
            models_pricing, "nastech_policy_allowed_ids", lambda **_k: {"vendor/allowed"}
        )
...
class TestNastechPrefetch:
    """The nastech disk-cache entry is write-only, so prefetching it is a round
    trip for nothing."""

    def test_nastech_is_not_collected_for_prefetch(self, monkeypatch):
        import nastech_cli.auth as auth_mod
        import nastech_cli.model_switch_providers as ms

        monkeypatch.setattr(
            auth_mod, "_load_auth_store",
            lambda *a, **k: {"providers": {"nastech": {"access_token": "tok"}}},
        )
        slugs = ms._collect_authed_provider_slugs({}, {"nastech": list(CURATED)}, [])
        assert "nastech" not in slugs


class TestPolicyNoticeIsShown:

    def test_login_prints_it(self, monkeypatch, tmp_path, policy, capsys):
        import nastech_cli.nastech_account as account_mod

        monkeypatch.setattr(account_mod, "nastech_policy_present", lambda: True)
        TestLoginNastech()._run(monkeypatch, tmp_path)
    ...
    def test_login_silent_for_an_ungoverned_org(
        ...
        monkeypatch.setattr(account_mod, "nastech_policy_present", lambda: False)
        TestLoginNastech()._run(monkeypatch, tmp_path)
...
class TestAuxFallbackRespectsPolicy:
    ...
    def test_blocked_recommendation_is_not_used(self, monkeypatch):
        ...
        assert aux._get_aux_model_for_provider("nastech", prefer_fast=True) == ""
    ...
    def test_allowed_recommendation_still_used(self, monkeypatch):
        ...
        assert (
            aux._get_aux_model_for_provider("nastech", prefer_fast=True)
            == "vendor/ok-haiku"
        )
...
def test_titling_seeds_the_shared_catalog_entry_like_the_pickers(monkeypatch):
    ...
    without the Nastech-only arguments costs the picker its sale chrome and leaves
    ...
    monkeypatch.setattr(models_pricing, "fetch_models_with_pricing", _fake_fetch)
    aux._fast_model_from_catalog("nastech")

    assert seen.get("include_sale_original") is True
    assert seen.get("cache_ttl_seconds") == models_pricing._NASTECH_CATALOG_TTL_SECONDS
