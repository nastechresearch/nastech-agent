"""Tests for nastech_cli configuration management."""
...
class TestEnsureNastechHome:
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from nastech_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``nastech logs`` surfaces it)
...
class TestLoadEnvInlineComments:
    def test_unquoted_hash_is_a_comment_quoted_hash_is_data(self, tmp_path):
        ...
        is a comment, a quoted value keeps its hash. Nastech' own writer (_quote_env_value) always quotes
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_quotes_values_containing_hash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSanitizeEnvLines:
    ...
    def test_migrate_reports_normalized_line_formatting(self, capsys):
        ...
        with (
            patch("nastech_cli.config.sanitize_env_file", return_value=2),
            patch(
                "nastech_cli.config.check_config_version",
                return_value=(latest_version, latest_version),
            ),
            patch("nastech_cli.config.read_raw_config", return_value={}),
            patch("nastech_cli.config.get_missing_env_vars", return_value=[]),
            patch("nastech_cli.config.get_missing_config_fields", return_value=[]),
            patch("nastech_cli.config.get_missing_skill_config_vars", return_value=[]),
        ):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `nastech setup`) lets a
        ...
        via config.yaml; NASTECH_MAX_ITERATIONS remains a read-only backward-compat
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `nastech setup` to regenerate" in captured.out
    ...
    def test_floor_message_uses_display_nastech_home(self):
        from nastech_cli.config_migrations import support_floor_message
        from nastech_constants import display_nastech_home

        msg = support_floor_message()
        assert f"{display_nastech_home()}/config.yaml" in msg
...
class TestRetiredMultiplexAllowlist:
    def test_v43_drops_multiplex_profile_allowlist_from_user_config(self, tmp_path, monkeypatch):
        ...
        from nastech_cli.config_migrations import run_migrations
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Nastech runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Nastech
    process on next startup via the dotenv → ``os.environ`` chain in
    ``nastech_cli/env_loader.py``.
    ...
    def _nastech_home(self, tmp_path, monkeypatch):
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
        ensure_nastech_home()
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NASTECH_SPOTIFY_CLIENT_ID",
            "NASTECH_QWEN_BASE_URL",
            "NASTECH_MAX_ITERATIONS",
        ],
    )
    def test_nastech_integration_keys_still_writable(self, allowed_key):
        """``NASTECH_*`` overall is NOT blocked.
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "NASTECH_YOLO_MODE",
            "NASTECH_ACCEPT_HOOKS",
            "NASTECH_REDACT_SECRETS",
            "NASTECH_INTERACTIVE",
            "NASTECH_EXEC_ASK",
            "NASTECH_GATEWAY_SESSION",
            "NASTECH_CRON_SESSION",
            "NASTECH_SINGLE_QUERY_SESSION",
            "NASTECH_SESSION_KEY",
            "NASTECH_SESSION_PLATFORM",
        ],
    )
    def test_nastech_security_control_keys_are_not_writable(self, protected_key):
    ...
    def test_preexisting_optional_mcps_override_still_loads(self, tmp_path):
        ...
        (tmp_path / ".env").write_text(
            f"NASTECH_OPTIONAL_MCPS={catalog}\n",
            encoding="utf-8",
        )
        invalidate_env_cache()

        assert load_env()["NASTECH_OPTIONAL_MCPS"] == str(catalog)

    @pytest.mark.parametrize(
        ("key", "expected"),
        [
            ("Path", "PATH"),
            ("Nastech_Yolo_Mode", "NASTECH_YOLO_MODE"),
            ("Nastech_Optional_Mcps", "NASTECH_OPTIONAL_MCPS"),
            ("Nastech_Copilot_Acp_Command", "NASTECH_COPILOT_ACP_COMMAND"),
            ("Nastech_Copilot_Acp_Args", "NASTECH_COPILOT_ACP_ARGS"),
        ],
    )
    def test_windows_policy_names_are_case_insensitive(self, key, expected):
        from nastech_cli.config import _env_var_policy_name
    ...
    def test_windows_env_assignment_matching_is_case_insensitive(self, prefix):
        from nastech_cli.config import _env_line_defines_key
...
class TestCodexAppServerAutoConfig:
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"NASTECH_HOME": str(tmp_path)}):
            ...
            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "nastech"
...
class TestConfigCommandFailClosedSurface:
    """`nastech config set/unset` must exit cleanly (no traceback) when the
    ...
    def test_config_command_set_exits_cleanly_on_broken_yaml(self, tmp_path, capsys):
        ...
        err = capsys.readouterr().err
        assert "formatting error" in err and "`nastech config edit`" in err
        assert config_path.read_text(encoding="utf-8") == original
...
def test_gateway_multiplex_keys_are_recognized_config_keys():
    """``nastech config set gateway.multiplex_profiles true`` used to warn 'not a recognized config
...
class TestCompatibleProvidersMalformedLegacyKey:
    ...
    def test_string_custom_providers_keeps_providers_view_and_warns(self, caplog):
        from nastech_cli.config_providers import get_compatible_custom_providers

        config = {
            "custom_providers": "- name: broken",
            "providers": {"exl3": {"api": "http://127.0.0.1:8290/v1", "default_model": "m"}},
        }
        with caplog.at_level(logging.WARNING, logger="nastech_cli.config"):
