...
can teach the ``nastech -p <profile> gateway <action>`` form that generated service units and
...
from nastech_cli._parser import PRE_ARGPARSE_INHERITED_FLAGS, build_top_level_parser
...
def test_rendered_help_documents_the_profile_scoped_command_form():
    ...
    assert "nastech -p <profile>" in help_text
    assert "--profile" in help_text
    assert "nastech -p coder gateway stop" in help_text
...
def test_rendered_help_documents_the_gateway_service_verbs():
    ...
    for verb in ("install", "start", "stop", "status"):
        assert f"nastech gateway {verb}" in help_text, verb
