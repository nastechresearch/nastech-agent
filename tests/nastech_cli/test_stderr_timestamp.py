"""Tests for nastech_cli.stderr_timestamp."""
...
from nastech_cli import stderr_timestamp
...
_LAUNCHD_ENV = {"PATH": "/usr/bin", "XPC_SERVICE_NAME": "ai.nastech.gateway-butler"}
...
def test_main_injects_flag_into_stale_gateway_child(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "ai.nastech.gateway-butler")
    ...
    stale = [sys.executable, "-c", code, "-m", "nastech_cli.main", "gateway", "run", "--replace"]
...
def test_main_maps_gateway_ex_config_to_clean_stop(tmp_path):
    ...
    log_path = tmp_path / "gateway.error.log"
    gateway_tail = ["-m", "nastech_cli.main", "gateway", "run"]
