"""Parity guard for the slash-command dispatch table in cli.NastechCLI.
...
def test_every_old_branch_resolves_to_a_handler():
    for name in OLD_CHAIN_COMMANDS:
        entry = NastechCLI._slash_handler(name)
        assert entry is not None, name
        method_name, pass_arg = entry
        assert callable(getattr(NastechCLI, method_name)), name
        assert isinstance(pass_arg, bool)
    # explicit table entries are only the ones the naming convention can't cover
    for name, (method_name, pass_arg) in NastechCLI._SLASH_DISPATCH.items():
...
def test_registry_names_resolve_into_the_table():
    ...
    for name in NastechCLI._SLASH_DISPATCH:
        cmd = resolve_command(name)
        assert cmd is not None and NastechCLI._slash_handler(cmd.name) is not None, name
    # registry commands the CLI never handled inline must still fall through
    dispatched = {c.name for c in COMMAND_REGISTRY if NastechCLI._slash_handler(c.name)}
