...
class TestServiceIdentityForForeignHome:
    """A NASTECH_HOME that is neither ``~/.nastech`` nor ``~/.nastech/profiles/<name>`` must never resolve to
    the default profile's ``nastech-gateway`` unit (a temp-home harness uninstalled the production gateway)."""
    ...
    def test_foreign_home_gets_its_own_unit(self, machine_home, tmp_path, monkeypatch):
        ...
        foreign.mkdir()
        monkeypatch.setenv("NASTECH_HOME", str(foreign))

        default_unit = machine_home / ".config" / "systemd" / "user" / "nastech-gateway.service"
        assert gateway_cli.get_service_name() != "nastech-gateway"
    ...
    def test_sudo_user_default_home_keeps_bare_service_name(self, machine_home, tmp_path, monkeypatch):
        ...
        # After unit sync, NASTECH_HOME points at the invoking user's native home.
        monkeypatch.setenv("NASTECH_HOME", str(sudo_default))
        assert gateway_cli.get_service_name() == "nastech-gateway"


class TestUninstallRefusesForeignUnit:
    """systemd_uninstall must not stop/disable/unlink a unit pinned to another NASTECH_HOME."""

    def test_unit_for_other_home_is_left_alone(self, tmp_path, monkeypatch, capsys):
        unit_path = tmp_path / "nastech-gateway.service"
        unit_path.write_text('[Service]\nEnvironment="NASTECH_HOME=/somewhere/else"\n', encoding="utf-8")
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "mine"))
        monkeypatch.setattr(gateway_cli, "get_systemd_unit_path", lambda system=False: unit_path)
...
class TestGetCronDrainTimeout:
    def test_missing_config_falls_back_to_default(self, monkeypatch):
        monkeypatch.delenv("NASTECH_CRON_DRAIN_TIMEOUT", raising=False)
    ...
    def test_env_overrides_config(self, monkeypatch):
        monkeypatch.setenv("NASTECH_CRON_DRAIN_TIMEOUT", "45")
        monkeypatch.setattr(
            gateway_cli,
            "read_raw_config",
...
class TestGeneratedSystemdUnits:
    ...
    def test_restart_exit_code_is_also_declared_a_success_status(self):
        ...
        and any OnFailure= alert unit fires on every routine restart (nastech update,
        nastech gateway restart, the in-app restart). SuccessExitStatus=75 keeps the same
    ...
    def test_launchd_plist_persists_configured_nofile_soft_limit(self, monkeypatch):
        ...
        plist rewrite by `nastech gateway start` cannot strip the FD floor and
        reintroduce EMFILE crashes (launchd default soft limit is 256)."""
        import nastech_cli.resource_limits as resource_limits
...
class TestLaunchdUnsupportedFallbackPolicy:
    ...
    Regression for the recurrence where ``nastech gateway install --force`` over the LIVE job
    ...
    def test_eio_on_a_supervised_label_does_not_degrade_to_detached(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.nastech.gateway")
        monkeypatch.setattr(
...
class TestGatewaySystemServiceRouting:
    ...
    def test_launchd_restart_uses_sigusr1_and_exit_wait_budget(self, monkeypatch, capsys):
        ...
        assert ("observe", "ai.nastech.gateway", 654, "gui/501") in calls
        out = capsys.readouterr().out
...
class TestSystemUnitNastechHome:
    ...
    def test_empty_managed_node_dir_uses_only_ambient_fallback(
        ...
        managed_bin = tmp_path / ".nastech" / "node" / "bin"
        ...
        gateway_cli._append_node_dir_for_service(entries, tmp_path / ".nastech")
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        target_nastech = target_home / ".nastech"
        root_home = tmp_path / "root"
        root_nastech = root_home / ".nastech"
        managed_bin = target_nastech / "node" / "bin"
        managed_bin.mkdir(parents=True)
        node = managed_bin / "node"
        node.write_text("#!/bin/sh\n")
        node.chmod(0o755)
        ...
        monkeypatch.setattr(gateway_cli, "get_nastech_home", lambda: root_nastech)
    ...
    def test_node_path_lookup_remains_fallback_without_managed_node(
        ...
        monkeypatch.setattr(
            "nastech_constants.iter_nastech_node_dirs", lambda root=None: []
        )
        monkeypatch.setattr(
            "nastech_constants.nastech_managed_node_tree_present",
...
class TestProfileArg:
    ...
    def test_launchd_plist_wraps_gateway_stderr_with_timestamps(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".nastech" / "profiles" / "mybot"
...
class TestLaunchdUnloadedJobStderrStaysOffTerminal:
    ...
    def fake_launchctl(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.nastech.gateway")
        monkeypatch.setattr(gateway_cli, "_launchd_domain", lambda: "gui/501")
        monkeypatch.setattr(gateway_cli, "get_launchd_plist_path", lambda: tmp_path / "ai.nastech.gateway.plist")
        monkeypatch.setattr(gateway_cli, "_clear_launchd_unsupported_marker", lambda: None)
...
class TestTimeoutStopSecCoversCronFloor:
    ...
    def _unit_with_config(self, tmp_path, monkeypatch, config_yaml, env=None):
        nastech = tmp_path / "home" / ".nastech"
        nastech.mkdir(parents=True)
        (nastech / "config.yaml").write_text(config_yaml, encoding="utf-8")
    ...
    def test_env_override_extends_the_leash(self, tmp_path, monkeypatch):
        unit = self._unit_with_config(
            ...
            monkeypatch,
            "agent:\n  restart_drain_timeout: 0\n",
            env={"NASTECH_CRON_DRAIN_TIMEOUT": "200"},
        )
        assert "TimeoutStopSec=240" in unit


class TestUnitAnchoredServiceIdentity:
    """The installed ``nastech-gateway.service`` owns the bare name: under ``sudo`` the naming basis moves
    mid-command when ``_sync_nastech_home_from_systemd_unit()`` adopts the unit's NASTECH_HOME (#108674).
    ...
    def test_home_not_pinned_by_unit_keeps_its_suffix(self, tmp_path, monkeypatch):
        alice_home = tmp_path / "alice" / ".nastech"
        alice_home.mkdir(parents=True)
        bob_home = tmp_path / "bob" / ".nastech"
        bob_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".nastech"
        root_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="NASTECH_HOME={alice_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(nastech_constants, "_get_platform_default_nastech_home", lambda: root_home)
        monkeypatch.setenv("NASTECH_HOME", str(bob_home))
    ...
    def test_unprivileged_profile_command_ignores_the_system_unit(self, tmp_path, monkeypatch):
        ...
        profile_home = tmp_path / "alice" / ".nastech" / "profiles" / "kimi"
        profile_home.mkdir(parents=True)
        unit_dir = tmp_path / "systemd"
        unit_dir.mkdir()
        (unit_dir / f"{gateway_cli._SERVICE_BASE}.service").write_text(
            f'[Service]\nEnvironment="NASTECH_HOME={profile_home}"\n', encoding="utf-8"
        )
        monkeypatch.setattr(gateway_cli, "_SYSTEM_UNIT_DIR", unit_dir)
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        monkeypatch.setattr(os, "geteuid", lambda: 1000)
        monkeypatch.setenv("NASTECH_HOME", str(profile_home))
        assert gateway_cli.get_service_name() == "nastech-gateway-kimi"
    ...
    def test_real_unit_sync_keeps_the_name_it_validated(self, tmp_path, monkeypatch):
        ...
        before ``_sync_nastech_home_from_systemd_unit()`` must survive the mutation it performs."""
        alice_home = tmp_path / "alice" / ".nastech"
        alice_home.mkdir(parents=True)
        root_home = tmp_path / "root" / ".nastech"
        root_home.mkdir(parents=True)
        ...
        gateway_cli._sync_nastech_home_from_systemd_unit(system=True)

        assert os.environ["NASTECH_HOME"] == str(alice_home)  # the sync really ran
