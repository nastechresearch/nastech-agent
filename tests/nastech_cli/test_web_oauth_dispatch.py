...
Nastech must not mint subscription OAuth tokens from an unattended HTTP
...
import nastech_cli.web_routers.oauth as _rt_oauth
import nastech_cli.web_server_oauth as _web_server_oauth
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        ...
        "nastech_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_minimax_start_route_honors_poller_mock_on_owning_module(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "nastech_cli.auth._minimax_poll_token",
        fail_poll_token,
    ), patch(
        "nastech_cli.web_server_oauth._minimax_poller",
        fake_poller,
...
def test_nastech_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    ...
    try:
        _web_server_oauth._nastech_plain_poller(session_id)
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "nastech auth add anthropic"
