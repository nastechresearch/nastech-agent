...
SessionDB.session_gateway_runtime (nastech_state.py) — the round trip that
makes `nastech --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare NastechCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.NastechCLI)
...
def test_session_gateway_runtime_falls_back_to_top_level_keys():
    ...
    meta = _row(model_config={"provider": "nastech", "api_mode": "chat_completions"})
    runtime = SessionDB.session_gateway_runtime(meta)
    assert runtime == {"provider": "nastech", "api_mode": "chat_completions"}
...
def test_restore_llamacpp_session_keeps_launch_base_url_for_same_provider(monkeypatch):
    """`nastech --provider llamacpp --base-url X --resume` is user intent for the SAME provider the
...
def test_session_gateway_runtime_explicit_provider_wins_over_billing():
    ...
    meta = _row(model_config={"provider": "nastech"})
