...
def test_offer_first_run_setup_re_resolves_reasoning_for_picked_model(monkeypatch):
    ...
    monkeypatch.setitem(cli.CLI_CONFIG, "agent", {
        **cli.CLI_CONFIG.get("agent", {}), "reasoning_effort": "medium",
        "reasoning_overrides": {"nastech-4-405b": "high"}})
    shell = _make_shell(cli, monkeypatch)
    assert shell.reasoning_config["effort"] == "medium"
    monkeypatch.setattr("nastech_cli.main.select_provider_and_model", lambda: None)
    monkeypatch.setattr("builtins.input", lambda *a, **k: "y")
...
def _bench_nastech_pool(monkeypatch, **entry_fields):
    import time
    from agent.credential_pool import STATUS_EXHAUSTED, CredentialPool, PooledCredential

    benched = PooledCredential(id="e1", provider="nastech", auth_type="oauth", access_token="x",
...
def _forbid_wizard(monkeypatch, shell):
    monkeypatch.setattr("nastech_cli.main.select_provider_and_model",
...
def test_benched_credential_prints_cooldown_instead_of_wizard(monkeypatch, capsys):
    ...
    cli = _import_cli()
    shell = _make_shell(cli, monkeypatch)
    shell.requested_provider = "nastech"

    def _raise(**kwargs):
        raise AuthError("Nastech is not logged into Nastech Portal.", provider="nastech",
                        code="nastech_auth_missing", relogin_required=True)

    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", _raise)
    _bench_nastech_pool(monkeypatch, last_error_code=429, last_error_reason="rate_limited")
    ...
    assert "re-authenticate" not in out and "nastech model" not in out


def test_auth_json_only_login_explains_instead_of_wizard(monkeypatch, capsys, tmp_path):
    """auth.json-only shape: logged into Nastech but no ``model.provider`` (requested "auto"). The
    ...
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **kw: "nastech")
    monkeypatch.setitem(rp._OAUTH_RUNTIME_PROVIDERS, "nastech",
                        dataclasses.replace(rp._OAUTH_RUNTIME_PROVIDERS["nastech"], resolve=_nastech_fail))
    ...
    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", lambda **kw: (_ for _ in ()).throw(
        AuthError("Nastech is not connected to any AI provider yet.", code="no_provider_configured")))
    shell._maybe_offer_first_run_setup()
    assert offered == [True]
