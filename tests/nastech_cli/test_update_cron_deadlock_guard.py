...
When `nastech update` runs INSIDE the gateway's own process tree (the
nastech-auto-update cron job), waiting for that gateway to exit is a
circular wait:
...
class TestSelfRestartFireAndForget:
    ...
    def test_refuses_non_ancestor_pid(self):
        from nastech_cli.gateway import _request_gateway_self_restart
...
class TestDrainOrSignalTriage:
    ...
    def test_ancestor_gateway_is_signalled_fire_and_forget(self, monkeypatch):
        ...
        from nastech_cli.update_cmd import _drain_or_signal_gateway_for_update
