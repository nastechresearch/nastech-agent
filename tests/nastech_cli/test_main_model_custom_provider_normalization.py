...
def test_unknown_vendor_still_uses_aggregator_fallback():
    with patch("nastech_cli.models_detect.provider_has_credentials", lambda p: p == "openrouter"):
...
def test_unknown_vendor_without_openrouter_key_is_not_reassigned():
    ...
    with patch("nastech_cli.models_detect.provider_has_credentials", lambda p: False):
