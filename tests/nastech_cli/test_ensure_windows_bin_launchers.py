"""``nastech`` must survive git operations on the checkout (launcher layout).

The Windows ``nastech`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``NASTECH_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``nastech-agent\\bin``) was swept by ``nastech update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``nastech`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``nastech update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``nastech_constants.venv_bin_dir``), so these
...
from nastech_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: NASTECH_HOME/nastech-agent/venv/Scripts + launchers."""
    home = tmp_path / "nastech"
    root = home / "nastech-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under NASTECH_HOME gains nothing anywhere."""
    home = tmp_path / "nastech-home"
    home.mkdir()
    monkeypatch.setenv("NASTECH_HOME", str(home))
    root = tmp_path / "src" / "nastech-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``nastech -p <name>`` NASTECH_HOME points inside profiles/<name>;
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `nastech update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "nastech.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "nastech.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\nastech\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/NASTECH/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``nastech update`` autostashes with
