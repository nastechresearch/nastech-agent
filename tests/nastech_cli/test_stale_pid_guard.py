...
- ``nastech_cli/_subprocess_compat.pid_is_nastech`` / ``kill_process_tree``
- ``nastech_cli/dashboard_procs._kill_stale_dashboard_processes`` (win32)
- ``nastech_cli/update_cmd._stop_process_trees``
...
class TestPidIsNastech:
    ...
    def test_non_windows_is_unconditional_pass(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False):
            assert _subprocess_compat.pid_is_nastech(1234) is True
    ...
    def test_non_windows_still_rejects_recycled_identity(self):
        ...
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", False), mock.patch.object(
            ...
            assert _subprocess_compat.pid_is_nastech(
                1234, expected_start_time=123
            ) is False

    def test_nastech_match_requires_token_boundary(self):
        # "nastech" buried inside an unrelated path segment must not match.
        assert _subprocess_compat._text_names_nastech(
            r"c:\users\shermesa\app.exe"
        ) is False
        assert _subprocess_compat._text_names_nastech(
            r"C:\Users\x\.nastech-runtime\python.exe -m nastech_cli.main"
        ) is True
        assert _subprocess_compat._text_names_nastech(
            "/opt/nastech-agent/venv/bin/python"
        ) is True

    def test_invalid_pid_inputs_do_not_crash(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True):
            assert _subprocess_compat.pid_is_nastech(-1) is False
            assert _subprocess_compat.pid_is_nastech(0) is False
            assert _subprocess_compat.pid_is_nastech("not-a-pid") is False
            assert _subprocess_compat.pid_is_nastech(True) is False

    def test_probe_matches_nastech_like_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_nastech", return_value=True
        ):
            assert _subprocess_compat.pid_is_nastech(1234) is True
    ...
    def test_probe_rejects_foreign_process(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            ...
            _subprocess_compat, "_process_command_is_nastech", return_value=False
        ):
            assert _subprocess_compat.pid_is_nastech(1234) is False
    ...
    def test_missing_pid_real_probe_fails_closed(self):
        # A PID that cannot exist must never be judged Nastech-owned.
        assert _subprocess_compat.pid_is_nastech(2**24) is False
...
class TestKillProcessTree:
    ...
    def test_retained_handle_is_taskkilled_without_probe(self):
        with mock.patch.object(_subprocess_compat, "IS_WINDOWS", True), mock.patch.object(
            _subprocess_compat, "pid_is_nastech"
...
class TestStopProcessTrees:
    ...
    def test_foreign_pids_only_probed(self):
        with mock.patch(
            ...
            "nastech_cli._subprocess_compat.pid_is_nastech", return_value=False
        ), mock.patch.object(update_cmd.subprocess, "run") as run:
            update_cmd._stop_process_trees([1111, 2222])
        run.assert_not_called()

    def test_nastech_pid_probed_then_taskkilled(self):
        with mock.patch(
            "gateway.status.get_process_start_time", return_value=123
        ), mock.patch(
            "nastech_cli._subprocess_compat.pid_is_nastech", return_value=True
        ), mock.patch.object(
...
class TestKillStaleDashboardProcesses:
    ...
    def test_foreign_pid_reported_not_killed(self):
        ...
        assert result["failed"] == [
            (12345, "not nastech-owned or process identity changed")
        ]
        run.assert_not_called()

    def test_nastech_pid_killed(self):
        with self._patch_find(), mock.patch.object(
            dashboard_procs.sys, "platform", "win32"
        ), mock.patch(
            "gateway.status.get_process_start_time", return_value=123
    ...
    def test_stop_only_targets_the_invoking_nastech_home(self, monkeypatch):
        """An argv match from another profile is never a ``--stop`` target."""
        own_home = "/tmp/nastech-own"
        foreign_home = "/tmp/nastech-foreign"
        monkeypatch.setenv("NASTECH_HOME", own_home)

        with mock.patch.object(
            dashboard_procs, "_scan_dashboard_processes",
            return_value=[(12345, "nastech serve"), (12346, "nastech serve"), (12347, "nastech serve")],
        ), mock.patch.object(dashboard_procs, "_caller_ancestor_pids", return_value=set()), mock.patch.object(
            dashboard_procs, "_nastech_home_for_pid",
...
class TestNastechHomeForPid:
    """Tri-state owner resolution: a readable environment always names a home."""

    @pytest.mark.skipif(sys.platform == "win32", reason="POSIX default home is $HOME/.nastech")
    def test_readable_env_without_var_resolves_to_that_process_default_home(self, monkeypatch, tmp_path):
        """The common install shape exports no NASTECH_HOME: the backend lives in its user's
        ...
        monkeypatch.setattr(main_dashboard, "_dashboard_cmdline_for_pid",
                            lambda pid: ["nastech", "--profile", "work", "serve"] if pid == 2 else ["nastech", "serve"])

        assert dashboard_procs._nastech_home_for_pid(1) == f"{home}/.nastech"
        # ``-p``/``--profile`` is applied to os.environ after exec, invisible in /proc environ.
        assert dashboard_procs._nastech_home_for_pid(2) == f"{home}/.nastech/profiles/work"
        assert dashboard_procs._pids_owned_by_nastech_home([1, 2], f"{home}/.nastech") == [1]

    def test_root_shaped_nastech_home_follows_the_flag_and_the_sticky_active_profile(self, monkeypatch, tmp_path):
        """Mirror ``_apply_profile_override``: an exported root ``NASTECH_HOME`` is the root, not the
        home — ``-p work`` and ``nastech profile use work`` both land in ``<root>/profiles/work``."""
        root = tmp_path / ".nastech"
        root.mkdir()
        monkeypatch.setattr(dashboard_procs, "_pid_environ",
                            lambda pid: {"HOME": str(tmp_path), "NASTECH_HOME": str(root)})
        from nastech_cli import main_dashboard
        monkeypatch.setattr(main_dashboard, "_dashboard_cmdline_for_pid",
                            lambda pid: ["nastech", "-p", "work", "serve"] if pid == 2 else ["nastech", "serve"])

        assert dashboard_procs._nastech_home_for_pid(2) == str(root / "profiles" / "work")
        assert dashboard_procs._nastech_home_for_pid(1) == str(root)  # no flag, no active_profile
        (root / "active_profile").write_text("work", encoding="utf-8")
        assert dashboard_procs._nastech_home_for_pid(1) == str(root / "profiles" / "work")
        # A profile-shaped NASTECH_HOME without a flag is the home itself (root = its grandparent).
        monkeypatch.setattr(dashboard_procs, "_pid_environ",
                            lambda pid: {"NASTECH_HOME": str(root / "profiles" / "ops")})
        assert dashboard_procs._nastech_home_for_pid(1) == str(root / "profiles" / "ops")
    ...
    def test_unreadable_env_is_none_and_spared(self, monkeypatch):
        ...
        assert dashboard_procs._nastech_home_for_pid(7) is None
        assert dashboard_procs._pids_owned_by_nastech_home([7], "/home/alice/.nastech") == []
