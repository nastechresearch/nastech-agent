...
def _run(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "nastech_cli.main", "computer-use", *args],
        capture_output=True,
        text=True,
        timeout=30,
    )
...
def _invoke(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    cli_main = import_module("nastech_cli.main")
    monkeypatch.setattr(sys, "argv", ["nastech", "computer-use", *args])
...
def test_computer_use_rejects_removed_browser_approve_command() -> None:
    ...
    assert "'browser-approve' is not a `nastech computer-use` command" in result.stderr
...
def test_computer_use_status_returns_zero_for_compatible_driver(
    ...
    import nastech_cli.tools_config_cua as tools_config_cua
...
def test_computer_use_status_returns_nonzero_for_incompatible_standard_driver(
    ...
    assert "Run: nastech computer-use install" in output
...
def test_computer_use_status_returns_nonzero_for_incompatible_custom_driver(
    ...
    monkeypatch.setenv("NASTECH_CUA_DRIVER_CMD", driver)
    monkeypatch.setattr(cua_backend_driver, "resolve_cua_driver_cmd", lambda: driver)
    monkeypatch.setattr(
        tools_config,
        "_cua_driver_contract_status",
    ...
    assert _invoke(monkeypatch, "status") == 1
    output = capsys.readouterr().out
    assert "custom binary from NASTECH_CUA_DRIVER_CMD" in output
