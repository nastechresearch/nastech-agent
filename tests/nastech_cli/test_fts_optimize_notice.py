...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            nastech_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(nastech_constants, "get_nastech_home", lambda: tmp_path)
    monkeypatch.setattr(nastech_state, "SessionDB", FakeSessionDB)
    ...
    assert "nastech sessions optimize-storage" in capsys.readouterr().out
    conn.close()
