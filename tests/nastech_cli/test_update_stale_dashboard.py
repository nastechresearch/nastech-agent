...
from nastech_cli.update_cmd import _finish_dashboard_update_cleanup
from nastech_cli.main_dashboard import _restart_managed_dashboard_service
from nastech_cli.dashboard_procs import _kill_stale_dashboard_processes as _warn_stale_dashboard_processes
...
def test_update_cleanup_spares_backend_owned_by_valid_ssh_lock(tmp_path, monkeypatch):
    ...
    with patch(
        "nastech_cli.main_dashboard._find_stale_dashboard_pids",
...
class TestKillStaleDashboardPosix:
    ...
    def test_sigterm_graceful_exit(self, capsys):
        ...
        with patch("nastech_cli.main_dashboard._find_stale_dashboard_pids",
    ...
    def test_user_scope_restart_never_falls_back_to_system_or_sudo(self, capsys):
        ...
        with patch("subprocess.run", side_effect=fake_run), \
             patch("nastech_cli.main_dashboard._find_stale_dashboard_pids", return_value=[]) as find_pids, \
...
class TestKillStaleDashboardWindows:
    ...
    def test_taskkill_invoked_for_each_pid(self, capsys):
        ...
        with patch("nastech_cli.main_dashboard._find_stale_dashboard_pids",
             ...
             patch("nastech_cli._subprocess_compat.pid_is_nastech", return_value=True), \
...
class TestDashboardUpdateCleanup:
    ...
    def test_all_failed_stops_do_not_claim_the_dashboard_was_stopped(self, capsys, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(own_home))
        with patch(
...
class TestWindowsWmicEncoding:
    ...
    def test_wmic_routed_through_bounded_probe_run_with_ignore_errors(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("nastech_cli._subprocess_compat.bounded_probe_run") as mock_probe:
    ...
    def test_probe_failure_fails_open_to_empty_list(self):
        ...
        with patch("sys.platform", "win32"), \
             patch("nastech_cli._subprocess_compat.bounded_probe_run",
                   return_value=None):
            assert _find_stale_dashboard_pids() == []
...
class TestSupervisedBackendRestart:
    ...
    def test_already_restarted_unit_is_left_untouched(self):
        """Review on #83595: nastech update's systemd fleet-restart loop may
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("time.sleep"):
            result = _kill_stale_dashboard_processes(
                restart_managed=True, already_restarted_units={"nastech-serve"}
            )
...
class TestManualBackendRespawn:
    ...
    def test_non_orphan_fixed_port_still_respawns(self, capsys):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("nastech_cli.dashboard_procs._nastech_home_for_pid", return_value=None), \
...
class TestFilterDashboardRespawnCandidates:
    ...
    def test_skips_serve_port_zero(self):
        ...
        assert _filter_dashboard_respawn_candidates([
            (42, argv, "/home/u/.nastech/profiles/mini-cat"),
        ]) == []
    ...
    def test_keeps_ppid1_fixed_port_for_repeat_update(self):
        ...
        argv = ["nastech", "dashboard", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([(10, argv, None)]) == [argv]

    def test_dedupes_identical_normalized_cmdlines(self):
        from nastech_cli.dashboard_procs import _filter_dashboard_respawn_candidates

        a = ["/usr/bin/python3", "-m", "nastech_cli.main", "dashboard", "--port", "8300"]
        b = ["/other/python", "-m", "nastech_cli.main", "dashboard", "--port", "8300"]
    ...
    def test_caps_one_per_profile(self):
        ...
        a = ["nastech", "--profile", "coder", "dashboard", "--port", "8300"]
        b = ["nastech", "--profile", "coder", "dashboard", "--port", "8301"]
        c = ["nastech", "--profile", "writer", "dashboard", "--port", "8302"]
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, None),
            (3, c, None),
    ...
    def test_caps_one_per_nastech_home(self):
        from nastech_cli.dashboard_procs import _filter_dashboard_respawn_candidates
    ...
    def test_profile_flag_and_profiles_home_share_cap(self):
        ...
        out = _filter_dashboard_respawn_candidates([
            (1, a, None),
            (2, b, "/home/u/.nastech/profiles/coder"),
        ])
        assert out == [a]
    ...
    def test_keeps_fixed_port_serve(self):
        ...
        argv = ["nastech", "serve", "--host", "0.0.0.0", "--port", "9119"]
        assert _filter_dashboard_respawn_candidates([
            (9, argv, None),
        ]) == [argv]
...
class TestPostUpdateDashboardCleanupIsolation:
    ...
    def test_cleanup_failure_is_isolated_and_recorded(self, capsys):
        ...
        import nastech_cli.update_receipt as ur
        from nastech_cli import update_cmd
        ...
        try:
            ...
            with patch(
                ...
                side_effect=AttributeError(
                    "module 'nastech_cli.main_dashboard' has no attribute '_loaded_launchd_backend_jobs'"
                ),
            ):
...
class TestLaunchdSupervisedBackends:
    ...
    ARGV = [
        "/opt/nastech/venv/bin/python", "-m", "nastech_cli.main",
    ...
    def _run(self, pid, jobs, *, restart_ok=True, restart_managed=True):
        ...
        with patch.object(main_dashboard, "_restart_managed_dashboard_service", return_value=False), \
             ...
             patch("nastech_cli.dashboard_procs._process_ancestors", return_value=[]), \
             patch("nastech_cli.dashboard_procs._nastech_home_for_pid", return_value=None), \
             patch.object(live, "_respawn_dashboard_processes", return_value=[]) as respawn, \
    ...
    def test_launchd_owned_backend_restarts_through_launchd_never_as_a_detached_respawn(self, capsys):
        ...
        job = ("system", "ai.nastech.dashboard", list(self.ARGV), None)
        result, restart, respawn = self._run(9102, [job])
        respawn.assert_not_called()
        restart.assert_called_once_with("system", "ai.nastech.dashboard", None)
        assert result["killed"] == [9102] and result["unrecovered"] == []
        out = capsys.readouterr().out
        assert "✓ restarted launchd job system/ai.nastech.dashboard" in out
        assert "when you're ready" not in out

        result, restart, respawn = self._run(9103, [("gui/501", "ai.nastech.dashboard", list(self.ARGV), 9103)],
                                             restart_ok=False)
        respawn.assert_not_called()
        assert result["unrecovered"] == [9103]
        assert "run: launchctl kickstart -k gui/501/ai.nastech.dashboard" in capsys.readouterr().out

        # A LaunchDaemon lives in the system domain: kickstart needs root, so a non-root hint says sudo.
        with patch("nastech_cli.dashboard_procs.os.geteuid", return_value=501, create=True):  # windows-footgun: ok — patch target string, create=True
            self._run(9105, [("system", "ai.nastech.serve", list(self.ARGV), None)], restart_ok=False)
        assert "run: sudo launchctl kickstart -k system/ai.nastech.serve" in capsys.readouterr().out

        other = ("gui/501", "ai.nastech.other", ["nastech", "dashboard", "--port", "8300"], 777)
    ...
    def test_loaded_backend_jobs_scans_plists_and_attributes_pids_host_independently(
        ...
        agents.mkdir()
        daemons.mkdir()
        backend_argv = ["/opt/nastech/venv/bin/python", "-m", "nastech_cli.main", "dashboard", "--port", "9119"]
        serve_argv = ["/opt/nastech/venv/bin/python", "-m", "nastech_cli.main", "serve", "--port", "8642"]
        gateway_argv = ["/opt/nastech/venv/bin/python", "-m", "nastech_cli.main", "gateway", "run"]
        for path, label, argv in (
            (agents / "ai.nastech.dashboard.plist", "ai.nastech.dashboard", backend_argv),
            (agents / "ai.nastech.gateway.plist", "ai.nastech.gateway", gateway_argv),
            (agents / "ai.nastech.serve-old.plist", "ai.nastech.serve-old", serve_argv),
            (daemons / "ai.nastech.serve.plist", "ai.nastech.serve", serve_argv),
        ):
        ...
        def fake_print(domain, label):
            ...
            if (domain, label) == (f"user/{uid}", "ai.nastech.dashboard"):
                return (True, 4242)
            if (domain, label) == ("system", "ai.nastech.serve"):
                return (True, None)
            return (False, None)
        ...
        with patch("nastech_cli.gateway._launchd_print_service_pid", side_effect=fake_print):
            jobs = _no_real_launchd_jobs([("agent", agents), ("daemon", daemons)])

        assert jobs == [
            (f"user/{uid}", "ai.nastech.dashboard", backend_argv, 4242),
            ("system", "ai.nastech.serve", serve_argv, None),
        ]
        assert not any(label == "ai.nastech.gateway" for _domain, label in probed)
        assert probed[:2] == [(f"gui/{uid}", "ai.nastech.dashboard"), (f"user/{uid}", "ai.nastech.dashboard")]
        assert not any(domain == "system" and label != "ai.nastech.serve" for domain, label in probed)

        owning = main_dashboard._launchd_job_owning_backend
        assert owning(4242, ["something", "else"], jobs) == (f"user/{uid}", "ai.nastech.dashboard", 4242)
        assert owning(9999, ["python"], jobs, ancestors=[4242, 1]) == (f"user/{uid}", "ai.nastech.dashboard", 4242)
        assert owning(9999, serve_argv, jobs) == ("system", "ai.nastech.serve", None)
        ...
        assert owning(9999, backend_argv + ["--no-open"], jobs) == (f"user/{uid}", "ai.nastech.dashboard", 4242)
