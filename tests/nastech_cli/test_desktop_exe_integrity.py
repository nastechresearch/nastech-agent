...
def test_rollback_restores_backup_and_keeps_corrupt_copy(tmp_path):
    ...
    with patch("nastech_cli.main_desktop._windows_native_machine", return_value="AMD64"):
...
def test_gate_fails_clearly_without_backup(tmp_path, capsys):
    ...
    with patch("nastech_cli.main_desktop._purge_electron_build_cache", return_value=[]), \
         patch("nastech_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"):
...
def test_build_only_fails_when_pack_produces_corrupt_exe(tmp_path, monkeypatch, capsys):
    ...
    must exit nonzero (so nastech-setup's retry-once kicks in) and must leave
    ...
    live_exe = desktop_dir / "release" / "win-unpacked" / "Nastech.exe"
    ...
    def pack_into_staging(cmd, *args, **kwargs):
        ...
        make_pe(staging / "win-unpacked" / "Nastech.exe", PE_AMD64, truncate_to=0x300)
        return subprocess.CompletedProcess(list(cmd), 0)

    with patch("nastech_cli.main_desktop.shutil.which", return_value="/usr/bin/npm"), \
         patch("nastech_cli.main_install_repair._resolve_node_runtime_npm", return_value="npm.cmd"), \
         ...
         patch("nastech_cli.main_desktop._desktop_stamp_path", return_value=tmp_path / "stamp.json"), \
         patch("nastech_cli.main_desktop._write_desktop_build_stamp") as mock_stamp, \
         patch("nastech_cli.main_desktop._windows_native_machine", return_value="AMD64"), \
         patch("nastech_cli.main_desktop.subprocess.run", side_effect=pack_into_staging), \
