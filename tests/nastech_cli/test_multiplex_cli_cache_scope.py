"""Multiplexed gateway: nastech_cli's process-wide caches must not hand profile A's value to profile B.

Every test builds two real profile homes (config.yaml / .env / cache files that differ), warms a
cache under ``set_nastech_home_override(A)`` and reads under B. Only the HTTP transport is canned —
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(a))
    for var in ("DEEPINFRA_API_KEY", "DEEPINFRA_BASE_URL", "NASTECH_INFERENCE_BASE_URL"):
...
def test_nastech_reasoning_caps_follow_each_profiles_portal(homes, monkeypatch):
    a, b = homes
    (a / ".env").write_text("NASTECH_INFERENCE_BASE_URL=https://portal-a.example/v1\n", encoding="utf-8")
    (b / ".env").write_text("NASTECH_INFERENCE_BASE_URL=https://portal-b.example/v1\n", encoding="utf-8")
    import nastech_cli.models as models
    import nastech_cli.models_reasoning_caps as caps

    for attr, value in (("_nastech_reasoning_caps_cache", None), ("_nastech_reasoning_caps_failed_at", None),
                        ("_nastech_caps_disk_checked", False), ("_nastech_caps_warm_started", False)):
        monkeypatch.setattr(models, attr, value)

    def transport(req, *, timeout, **kw):
        effort = "low" if "portal-a" in req.full_url else "high"
        return _json_resp({"data": [{"id": "nastech/m", "supported_parameters": ["reasoning"],
                                     "reasoning": {"supported_efforts": [effort]}}]})

    monkeypatch.setattr(models, "_urlopen_model_catalog_request", transport)
    with _Scoped(a):
        assert caps.nastech_model_reasoning_capabilities("nastech/m", allow_fetch=True)["supported_efforts"] == ["low"]
    with _Scoped(b):
        assert caps.nastech_model_reasoning_capabilities("nastech/m", allow_fetch=True)["supported_efforts"] == ["high"]
...
def test_failed_guest_mint_only_suppresses_that_profile(homes, monkeypatch, tmp_path):
    ...
    import nastech_cli.anon_auth as anon
    ...
    def client(timeout_seconds, verify):
        def handler(request):
            attempts.append(str(get_nastech_home()))
            if status["code"] == 429:
                return httpx.Response(429, json={"error": "rate"})
            return httpx.Response(201, json={"token": "anon_b", "user_id": "u", "org_id": "o"})
        return httpx.Client(transport=httpx.MockTransport(handler))

    monkeypatch.setattr(auth_nastech, "_nastech_http_client", client)
