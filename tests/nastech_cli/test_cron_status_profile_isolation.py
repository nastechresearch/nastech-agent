...
Issue #98790: `nastech cron status` in profile B reports profile A's gateway
PID as proof that B's jobs will fire. Root causes:

1. systemd branch of ``_get_service_pids()`` returns every nastech-gateway*
...
class TestCronStatusHeartbeatGuard:
    """Ensure `nastech cron status` correctly warns when the heartbeat file is absent.
    ...
    def test_no_heartbeat_triggers_yellow_warning(self, monkeypatch, capsys):
        ...
        with (
            patch("nastech_cli.gateway.find_gateway_pids", return_value={789}),
    ...
    def test_fresh_heartbeat_shows_green_checkmark(self, monkeypatch, capsys):
        ...
        with (
            patch("nastech_cli.gateway.find_gateway_pids", return_value={999}),
...
class TestGetServicePidsProfileScope:
    ...
    def test_default_scope_filters_current_profile_systemd_unit(self, monkeypatch):
        ...
        def _run_side_effect(args, **kwargs):
            ...
            if "list-units" in cmd_str:
                ...
                if "nastech-gateway-jarvis" in cmd_str:
                    stdout = "nastech-gateway-jarvis.service loaded active running\n"
                elif "nastech-gateway-coder" in cmd_str:
                    stdout = "nastech-gateway-coder.service loaded active running\n"
                elif "nastech-gateway*" in cmd_str:
                    stdout = (
                        "nastech-gateway-jarvis.service loaded active running\n"
        ...
        with (
            patch("nastech_cli.gateway.is_macos", return_value=False),
            patch("nastech_cli.gateway.supports_systemd_services", return_value=True),
            patch("nastech_cli.gateway.get_service_name", return_value="nastech-gateway-jarvis"),
    ...
    def test_all_profiles_true_enumerates_fleet(self, monkeypatch):
            ...
            if "list-units" in cmd_str:
                return MagicMock(
                    ...
                    stdout=(
                        ...
                        "nastech-gateway-profile-b.service loaded active running\n"
                    ),
                    stderr="",
                )
            if "show" in cmd_str and "MainPID" in cmd_str:
                if "nastech-gateway.service" in " ".join(args):
                    return MagicMock(returncode=0, stdout="111\n", stderr="")
                elif "nastech-gateway-profile-b.service" in " ".join(args):
        ...
        with (
            patch("nastech_cli.gateway.is_macos", return_value=False),
            patch("nastech_cli.gateway.supports_systemd_services", return_value=True),
            patch("nastech_cli.gateway.get_service_name", return_value="nastech-gateway"),
            patch("subprocess.run", side_effect=_run_side_effect),
        ):
...
class TestCronStatusMissingHeartbeat:
    ...
    def test_missing_heartbeat_warns_when_gateway_old(self, tmp_cron_dir, capsys, monkeypatch):
        ...
        with (
            ...
            patch("nastech_cli.gateway.find_gateway_pids", return_value=[4242]),
