...
def test_explicit_only_keeps_anthropic_row_with_oauth_credentials():
    ...
    Claude Code (~/.claude/.credentials.json) and Nastech' own device flow
    ...
    with (
        ...
        patch(
            "nastech_cli.inventory._anthropic_oauth_credentials_present",
            return_value=True,
        ),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_anthropic_oauth_presence_accepts_pool_only_oauth_entry():
    ...
    """
    from nastech_cli.inventory import _anthropic_oauth_credentials_present
...
def test_user_defined_rows_carry_alias_set_for_gui_current_match():
    ...
    assert "aliases" not in by_slug["nastech"]
