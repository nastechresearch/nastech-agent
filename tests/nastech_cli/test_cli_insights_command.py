...
from nastech_cli.main_agent_cmds import cmd_insights
...
def test_insights_skips_open_when_store_missing(capsys):
    ...
    for run in (lambda: NastechCLI.__new__(NastechCLI)._show_insights("/insights 7"),
                lambda: cmd_insights(SimpleNamespace(days=30, source=None))):
        with patch("nastech_state.SessionDB") as ctor:
            run()
...
def test_show_insights_opens_read_only():
    cli_obj = NastechCLI.__new__(NastechCLI)
    db = MagicMock()
    with patch("nastech_state.SessionDB", return_value=db) as ctor, \
         patch("agent.insights.InsightsEngine", _InsightsEngineStub):
        _InsightsEngineStub.calls = []
