...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        with patch("nastech_cli.dashboard_procs._scan_dashboard_processes", return_value=[]), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "No nastech dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        processes = [
            ...
            (12347, "nastech serve --host 100.94.65.93 --port 9119"),
        ]
        with patch("nastech_cli.dashboard_procs._scan_dashboard_processes", return_value=processes), \
             patch("gateway.status._pid_exists", return_value=True), \
             patch("nastech_cli.main_dashboard._dashboard_listening", return_value=True), \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(status=True))
        # Status is informational — always exits 0.
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "3 nastech dashboard/serve process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids",
             ...
             patch("nastech_cli.dashboard_procs._kill_stale_dashboard_processes",
    ...
    def test_stop_scopes_scan_and_kill_to_the_invoking_nastech_home(self, tmp_path, monkeypatch, capsys):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids", return_value=[12345]) as scan, \
             patch("nastech_cli.dashboard_procs._kill_stale_dashboard_processes",
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids", return_value=[]), \
             patch("nastech_cli.dashboard_procs._kill_stale_dashboard_processes") as kill, \
             pytest.raises(SystemExit) as exc:
            cmd_dashboard(_ns(stop=True))
    ...
    def test_stop_exits_nonzero_if_kill_leaves_survivors(self):
        ...
        with patch("nastech_cli.main._find_stale_dashboard_pids", return_value=[12345]), \
             patch("nastech_cli.dashboard_procs._kill_stale_dashboard_processes",
