...
``nastech*.exe`` shims aside (``nastech.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``nastech`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("nastech", "nastech-agent", "nastech-acp", "nastech-gateway"):
...
# ---------------------------------------------------------------------------
# nastech_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "nastech.exe" in names, "nastech.exe must be restored after a no-op install"
    assert "nastech-acp.exe" in names
    assert "nastech-gateway.exe" in names
    assert (scripts / "nastech.exe").read_bytes() == b"MZ-old-nastech"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "nastech.exe").read_bytes() == b"MZ-new-nastech"
...
# ---------------------------------------------------------------------------
# nastech_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "nastech.exe" in names, "nastech.exe must be restored after a no-op recovery install"
    assert (scripts / "nastech.exe").read_bytes() == b"MZ-old-nastech"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "nastech.exe").write_bytes(b"MZ-new-nastech")
