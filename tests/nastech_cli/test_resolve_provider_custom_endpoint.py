...
``nastech chat`` (which resolves the runtime directly) worked against the same config.
"""
...
def isolated_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("NASTECH_GUEST_ONBOARDING", raising=False)
    for var in ("OPENAI_API_KEY", "OPENROUTER_API_KEY", "ANTHROPIC_API_KEY", "OPENAI_BASE_URL",
                "OPENROUTER_BASE_URL", "NASTECH_INFERENCE_PROVIDER", "NASTECH_API_KEY"):
...
def test_a_provider_configured_after_boot_flips_the_stale_setup_record(isolated_home, monkeypatch):
    ...
    ``nastech setup`` in another process. A ``False`` record is reconciled with the config files
    ...
    token = set_nastech_home_override(str(profile_b))
    try:
