...
minimal ``NastechCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def nastech_home(tmp_path, monkeypatch):
    """Isolated NASTECH_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".nastech"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("NASTECH_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves NASTECH_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal NastechCLI stub with an active goal wired in."""
...
class TestInterruptAutoPause:
    ...
    def test_interrupted_turn_is_resumable(self, nastech_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, nastech_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, nastech_home):
        ...
        src = inspect.getsource(NastechCLI.chat)
