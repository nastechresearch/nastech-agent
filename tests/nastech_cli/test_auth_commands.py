...
def test_auth_add_configured_provider_uses_canonical_pool_key(tmp_path, monkeypatch):
    ...
    payload = json.loads((nastech_home / "auth.json").read_text(encoding="utf-8"))
...
def test_auth_add_migrates_display_name_derived_legacy_pool_key(
    ...
    with patch("nastech_cli.models.clear_provider_models_cache") as clear_cache:
        auth_add_command(_Args())
...
def test_auth_list_includes_non_registry_configured_provider(
    ...
    from nastech_cli.auth_commands import auth_list_command
...
def test_interactive_auth_add_accepts_non_registry_configured_provider(
    ...
    from nastech_cli import auth_commands
...
def test_auth_add_nastech_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "nastech"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("nastech@example.com")
    monkeypatch.setattr(
        "nastech_cli.auth._nastech_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["nastech"]
    device_code_entries = [
    ...
    # `nastech auth add nastech` must also populate providers.nastech so the
    # 401-recovery path (resolve_nastech_runtime_credentials) can refresh an
    # invoke JWT when the token expires. If this mirror is missing, recovery
    # raises "Nastech is not logged into Nastech Portal" and the agent dies.
...
def test_auth_add_nastech_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`nastech auth add nastech --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_nastech_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "nastech"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("nastech@example.com")
    ...
    pool_entry = payload["credential_pool"]["nastech"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-nastech"

    # … and survives in providers.nastech so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["nastech"]["label"] == "my-nastech"


def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``nastech auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``nastech auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("nastech_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from nastech_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from nastech_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_remove_codex_migrates_legacy_dict_suppression(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "nastech" / "auth.json").read_text(encoding="utf-8"))
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`nastech logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (nastech_home / "config.yaml").read_text()
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from nastech_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
# =============================================================================
# Unified credential-source stickiness — every source Nastech reads from has a
...
def test_seed_from_singletons_respects_nastech_pkce_suppression(tmp_path, monkeypatch):
    """anthropic nastech_pkce must not re-seed from ~/.nastech/.anthropic_oauth.json when suppressed."""
    nastech_home = tmp_path / "nastech"
    nastech_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))

    import yaml
    (nastech_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (nastech_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["nastech_pkce"]},
    }))

    # Stub the readers so only nastech_pkce is "available"; claude_code returns None
    import agent.anthropic_credentials as aa
    monkeypatch.setattr(aa, "read_nastech_oauth_credentials", lambda: {
    ...
    # nastech_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "nastech_pkce" not in active
...
def test_qwen_oauth_login_marks_active_through_moved_owner(monkeypatch):
    ...
    import nastech_cli.auth_commands as auth_commands
    import nastech_cli.auth_qwen as auth_qwen
...
def test_auth_add_openrouter_oauth_persists_pkce_key_without_touching_api_key_default(tmp_path, monkeypatch):
    """`nastech auth add openrouter --type oauth` stores the PKCE-minted key as an ``api_key`` pool row
    ...
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr("nastech_cli.auth._openrouter_pkce_login", lambda **kw: {"api_key": "sk-or-v1-from-pkce"})
...
def test_openrouter_loopback_callback_binds_nonce_path_and_rejects_forged_redirect(monkeypatch):
    ...
    import nastech_cli.auth_openrouter as orm
