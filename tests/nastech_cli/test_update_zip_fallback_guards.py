...
install``. A locked ``nastech.exe`` makes the install exit 2, the handler
...
def test_windows_dep_failure_does_not_zip_fallback(monkeypatch):
    monkeypatch.setattr(nastech_main, "_is_windows", lambda: True)
    exc = _cpe([r"C:\venv\Scripts\uv.exe", "pip", "install", "-e", "."])
...
def test_zip_overlay_blocked_on_modified_file(tmp_path, monkeypatch):
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(
        update_cmd.subprocess, "run", _porcelain_run(" M nastech_cli/update_cmd.py\n")
    )
...
def test_update_via_zip_aborts_before_download_when_dirty(
    ...
    with patch("urllib.request.urlretrieve") as download:
        with pytest.raises(SystemExit) as exc_info:
            nastech_main._update_via_zip(SimpleNamespace(branch=None))
...
def test_staging_artifact_lines_are_recognized():
    ...
    assert is_artifact("?? agent.nastech-update-staging/")
    assert is_artifact("?? cli.py.nastech-update-staging")
    assert is_artifact("?? tools.nastech-update-old/")
    ...
    assert not is_artifact(" M nastech_cli/update_cmd.py")
    assert not is_artifact("?? notes.nastech-update-staging.txt")
...
def test_recheck_ignores_own_staging_artifacts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        update_cmd.subprocess,
        "run",
        _porcelain_run("?? agent.nastech-update-staging/\n?? cli.py.nastech-update-old\n"),
    )
    assert (
