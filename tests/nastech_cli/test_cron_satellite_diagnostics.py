...
def served_root(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_PROFILE", "probe")
    ...
    monkeypatch.setattr("gateway.status._read_process_cmdline", lambda pid: "nastech gateway run")
...
def test_status_preserves_profile_health_contract(served_root, capsys, monkeypatch, mode):
    ...
    if mode == "local":
        monkeypatch.setattr("nastech_cli.gateway.find_gateway_pids", lambda: [os.getpid()])
    if mode == "external":
    ...
    if mode in {"missing", "stale"}:
        assert "nastech --profile default gateway restart" in output
    if mode == "missing":
    ...
    if mode in {"disabled", "excluded", "unrelated_pid"}:
        ...
        assert "nastech gateway install" in output
        assert "sudo nastech gateway install --system" in output
        assert "nastech gateway run" in output
        assert "nastech --profile default gateway restart" in output
    if mode == "external":
...
def test_satellite_list_and_create_require_own_heartbeat(served_root, capsys, monkeypatch, heartbeat):
    ...
    for output in (created, listed):
        assert ("Check status:  nastech cron status" in output) == (heartbeat != "fresh")
    cron.cron_status()
...
def test_doctor_bounds_persisted_fire_errors(served_root, capsys, detail):
    ...
    if detail:
        ...
        assert f"nastech cron run {records[0]['id']}" in line
...
def test_doctor_reports_persisted_dispatch_health(served_root, capsys, dispatch):
    ...
    assert "Review the findings above, then run `nastech cron doctor` again." in output
