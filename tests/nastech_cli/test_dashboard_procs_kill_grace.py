...
``nastech update`` / ``nastech dashboard --stop`` fall back to ``_kill_pids_posix`` for a
...
# Chat TUI): `nastech dashboard --stop` / `nastech update` must not SIGTERM their own process.
_STOPPER_CHILD = textwrap.dedent(
    """
    import os, pathlib, sys
    from nastech_cli import dashboard_procs as dp
