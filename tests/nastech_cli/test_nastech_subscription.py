...
def test_get_nastech_subscription_features_recognizes_keyless_tavily_backend(monkeypatch):
    ...
    features = ns.get_nastech_subscription_features({"web": {"backend": "tavily"}})
...
def test_logged_in_entitled_account_yields_a_state_for_every_feature(monkeypatch):
    ...
    crashing `nastech status`, `nastech tools`, and the dashboard toolsets API."""
    ...
    result = ns.get_nastech_subscription_features({"model": {"provider": "nastech"}})
...
def test_get_gateway_eligible_tools_treats_explicit_backend_as_configured(monkeypatch):
    ...
    but an explicit non-nastech selection must still keep it out of
    'unconfigured' — regression for #92647, where it was pre-checked and a
    single Enter during `nastech model` overwrote it to `web.backend: nastech`.
    """
    monkeypatch.setattr(ns, "get_nastech_portal_account_info", lambda **kw: _account(logged_in=True, paid=True))
    monkeypatch.setattr(
        ns,
        "_get_gateway_direct_credentials",
        lambda: {"web": False, "image_gen": False, "video_gen": False, "tts": False, "browser": False},
    ...
    config = {"model": {"provider": "nastech"}, "web": {"backend": "searxng"}}
...
def test_get_gateway_eligible_tools_treats_browser_use_selection_as_explicit(monkeypatch):
    ...
    managed selection, which is always stored as `cloud_provider: nastech`
    ...
    config = {"model": {"provider": "nastech"}, "browser": {"cloud_provider": "browser-use"}}
    unconfigured, has_direct, explicit_configured, already_managed = ns.get_gateway_eligible_tools(config)
...
def test_get_gateway_eligible_tools_not_entitled_returns_four_empty_lists(monkeypatch):
    """A logged-in Nastech account with no paid access and no free tool pool
    ...
    monkeypatch.setattr(ns, "get_nastech_portal_account_info", lambda **kw: _account(logged_in=True, paid=False))
...
def test_prompt_enable_tool_gateway_not_entitled_does_not_crash(monkeypatch):
    ...
    not raise when a Nastech account is logged in but not entitled to the Tool
...
def test_prompt_enable_tool_gateway_never_offers_explicit_backend(monkeypatch):
    ...
    an explicit non-nastech selection, so it can never be silently overwritten
...
def test_prompt_enable_tool_gateway_persists_decline(monkeypatch):
    ...
    captured = _capture_checklist(monkeypatch, selected_idx=[])
    monkeypatch.setattr(
        "nastech_cli.config.save_config", lambda cfg: saved.append(dict(cfg)), raising=False
    )
...
def test_apply_nastech_managed_defaults_writes_video_gen_config(monkeypatch):
    """apply_nastech_managed_defaults must store the managed 'nastech' selection
    when a Nastech subscriber selects video_gen without a direct FAL_KEY."""
    monkeypatch.setattr(tool_backend_helpers, "managed_nastech_tools_enabled", lambda **kw: True)
...
def _block_legacy_agent_browser_checks(monkeypatch):
    ...
    monkeypatch.setattr("nastech_constants.agent_browser_runnable", lambda path: False)
...
def test_has_agent_browser_import_failure_falls_back_to_nastech_managed_node_path(
    monkeypatch, tmp_path
):
    """If tools.browser_tool_install cannot be imported, the managed-Node rung must
    still find a runnable agent-browser under the Nastech Node dir even when
