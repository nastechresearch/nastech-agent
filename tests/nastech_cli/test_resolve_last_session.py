...
def test_resolve_last_session_real_db_prefers_workspace(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_state.SessionDB", lambda **kw: real_db(db_path=state_db, **kw))
    assert _resolve_last_session("cli") == "repo_a"


def test_resolve_last_session_cli_continues_a_oneshot(monkeypatch, tmp_path):
    """`nastech -z … --resume latest` / `nastech -c` chain on the previous one-shot: its distinct `oneshot`
    ...
    monkeypatch.setattr("nastech_cli.main._resolve_workspace_key", lambda: None)
    monkeypatch.setattr("nastech_state.SessionDB", lambda **kw: real_db(db_path=state_db, **kw))
