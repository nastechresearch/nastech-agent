...
``_write_profile_model`` used to enter only the NASTECH_HOME override. Once the dashboard
...
def homes(tmp_path, monkeypatch):
    """A throwaway NASTECH_HOME whose named profile carries its own ``.env`` credential.
...
def probe(monkeypatch):
    ...
    import nastech_cli.models_validate as mv
...
def test_model_pick_resolves_key_env_from_profile_scope(client, homes, probe):
    ...
    with _nastech_home_scope(homes[1]):
...
def test_model_pick_for_default_from_named_profile_launch(homes, probe, monkeypatch):
    ...
    root, demo = homes
    monkeypatch.setenv("NASTECH_HOME", str(demo))
    (root / ".env").write_text("ACME_RELAY_KEY=root-key\n", encoding="utf-8")
    from nastech_cli.config import invalidate_env_cache, load_config
    from nastech_cli.web_server_profiles import _nastech_home_scope
    invalidate_env_cache()
    from nastech_cli import web_server
    ...
    with _nastech_home_scope(root):
        assert (load_config().get("model") or {}).get("default") == "acme/mini"
    with _nastech_home_scope(demo):
