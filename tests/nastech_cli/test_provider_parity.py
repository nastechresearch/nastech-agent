...
the SAME provider universe as ``nastech model`` (the CLI/TUI picker).
...
def test_every_nastech_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `nastech model` universe."""
    ...
    assert not missing, (
        "providers shown in `nastech model` but not configurable in the desktop "
