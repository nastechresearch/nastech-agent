...
plain words and points at `nastech setup terminal`, never at raw TERMINAL_* env vars."""
...
def test_docker_missing_line_names_backend_and_setup_command(monkeypatch, capsys, issues):
    ...
    assert issues and "nastech setup terminal" in issues[0] and "Install Docker or Podman" in issues[0]
...
def test_docker_daemon_down_line_says_start_docker_or_switch(monkeypatch, capsys, issues):
    ...
    assert issues and "Start Docker" in issues[0] and "nastech setup terminal" in issues[0]
...
def test_podman_down_line_advises_the_machine_not_a_daemon(monkeypatch, capsys, issues):
    ...
    assert issues and "podman machine start" in issues[0] and "nastech setup terminal" in issues[0]
...
def test_ssh_host_missing_line_points_at_setup_terminal(monkeypatch, capsys, issues):
    ...
    assert issues and "nastech setup terminal" in issues[0] and "SSH host and user" in issues[0]
...
def test_daytona_key_missing_line_points_at_setup_terminal(monkeypatch, capsys, issues):
    ...
    assert issues and "nastech setup terminal" in issues[0]
