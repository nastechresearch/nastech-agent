...
def test_startup_route_uses_configured_nastech_provider(monkeypatch):
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    route = model_switch.resolve_startup_model_route(
        "nastech/deepseek-v4-pro",
        user_providers={"nastech": {"base_url": "https://inference.example/v1"}},
    )
    assert route == model_switch.StartupModelRoute("deepseek-v4-pro", "nastech", "")
...
def test_startup_route_aggregator_native_slug_stays_on_aggregator(monkeypatch):
    ...
    monkeypatch.setattr(model_switch, "DIRECT_ALIASES", {})
    monkeypatch.setattr(
        "nastech_cli.models._find_openrouter_slug",
...
def test_oneshot_and_tui_qualified_model_never_reaches_default_provider(tmp_path, monkeypatch):
    """``nastech -z -m custom:<name>:<model>`` and ``nastech --tui -m …`` route through the same
    startup owner, so provider auto-detection never hands the qualified string to the configured
    default (#73943)."""
    from nastech_cli.oneshot import _resolve_model_and_provider

    cfg = _write_named_provider(tmp_path, monkeypatch)
    from tui_gateway import server as tui_server  # binds the config path at import: after NASTECH_HOME
    ...
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: pytest.fail("auto-detection ran on a provider-qualified model"))
    monkeypatch.setenv("NASTECH_INFERENCE_MODEL", "custom:jetson-vllm:nemotron-nano-30b")
