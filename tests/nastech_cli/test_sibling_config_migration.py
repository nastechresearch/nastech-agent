...
`nastech update` migrated only the active profile's config.yaml; sibling
...
context-local NASTECH_HOME override.
...
import nastech_cli.update_cmd_config as update_cmd_config
...
def _write_profile(root: Path, name: str, version: int) -> Path:
    ...
    (home / "config.yaml").write_text(
        yaml.safe_dump({"_config_version": version, "model": {"provider": "nastech"}}),
        encoding="utf-8",
    )
    return home
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        nastech_constants, "get_process_nastech_home", lambda: active_home
    )
...
def test_sibling_behind_is_migrated_on_disk(monkeypatch, tmp_path):
    ...
    assert on_disk["model"]["provider"] == "nastech"
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_nastech_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_nastech_home_override() == before
