...
class TestConfigParseWarning:
    ...
    def test_warning_names_repair_commands_and_line(self, tmp_path, monkeypatch, capsys, fallback):
        ...
        assert "line " in err
        assert "`nastech config edit`" in err and "`nastech config check`" in err
...
class TestConfigGuardCopy:
    def test_require_parseable_user_config_says_what_happened_and_how_to_fix(self, tmp_path, monkeypatch):
        ...
        assert msg.startswith("Nastech stopped because your settings file")
        assert "`nastech config edit`" in msg and "--ignore-user-config" in msg
    ...
    def test_refuse_write_names_repair_commands_and_absolute_backup_dir(self, tmp_path, monkeypatch):
        ...
        assert "`nastech config edit`" in msg and "`nastech config check`" in msg
        assert f"{display_nastech_home()}/backups/config/" in msg
...
def test_no_provider_configured_points_at_model_login_and_auth_add(monkeypatch, tmp_path):
    ...
    assert "`nastech model`" in msg and "`/login`" in msg and "`nastech auth add <provider>`" in msg
    assert "~/.nastech/.env" not in msg or display_nastech_home() == "~/.nastech"
    assert f"{display_nastech_home()}/.env" in msg
    assert "inference provider" not in msg


# ── cli-07: Nastech device-code sign-in failures ────────────────────────────────────────────────


class TestSignInFailureCopy:
    @pytest.mark.parametrize("code, pointer", [
        ("expired_token", "`nastech portal`"),
        ("access_denied", "`nastech model`"),
    ])
    def test_poll_error_codes_map_to_plain_copy(self, code, pointer):
        from nastech_cli import auth_device_flow
        ...
        with pytest.raises(RuntimeError) as excinfo:
            auth_device_flow._poll_for_token(
                client=_Client(), portal_base_url="https://portal.nastechresearch.github.io", client_id="nastech-cli",
                device_code="device", expires_in=5, poll_interval=1)

        msg = str(excinfo.value)
        assert not msg.startswith(code)
        assert "`nastech portal`" in msg and pointer in msg
    ...
    def test_network_failure_names_host_and_retry_command(self):
        ...
        lines = sign_in_failure_lines(httpx.ConnectError("[Errno -2] Name or service not known"),
                                      service_host="portal.nastechresearch.github.io")

        assert lines[0].startswith("Could not sign in")
        assert "portal.nastechresearch.github.io" in lines[0] and "`nastech portal`" in lines[0]
    ...
    def test_run_login_prints_plain_copy_not_raw_exception(self, capsys):
        from nastech_cli.model_setup_flows_common import _run_login
    ...
    def test_login_nastech_prints_plain_copy(self, monkeypatch, capsys):
        from nastech_cli import auth_nastech

        def _boom(**_k):
            raise httpx.ConnectError("[Errno -2] Name or service not known")

        monkeypatch.setattr(auth_nastech, "_offer_shared_nastech_import", lambda *_a, **_k: None, raising=False)
        monkeypatch.setattr("nastech_cli.auth._nastech_device_code_login", _boom)
        ...
        with pytest.raises(SystemExit) as excinfo:
            auth_nastech._login_nastech(args, PROVIDER_REGISTRY["nastech"])

        assert excinfo.value.code == 1
        out = capsys.readouterr().out
        assert "Could not sign in" in out and "`nastech portal`" in out
...
class TestAuthCommandsCopy:
    def test_unknown_provider_offers_did_you_mean_and_pointers(self, monkeypatch):
        ...
        assert "`nastech auth`" in msg and "`nastech model`" in msg
    ...
    def test_refresh_failure_names_relogin_command(self, monkeypatch):
        ...
        matched = SimpleNamespace(id="abc", label="device-code", source="device_code", provider="nastech",
        ...
        with pytest.raises(SystemExit) as excinfo:
            auth_commands.auth_refresh_command(SimpleNamespace(provider="nastech", target=None))

        msg = str(excinfo.value)
        assert "refresh_failed" not in msg and "status now" not in msg
        assert "`nastech auth add nastech --type oauth`" in msg
...
def test_creds_for_target_provider_names_auth_add_and_model(monkeypatch):
    ...
    assert "`nastech auth add kimi-coding`" in msg and "/model" in msg
...
class TestProviderSetupFailureCopy:
    def test_setup_provider_step_reports_reason_and_that_nothing_changed(self, monkeypatch, capsys, tmp_path):
        ...
        monkeypatch.setattr("nastech_cli.main.select_provider_and_model", _boom)
    ...
    def test_quick_setup_error_hook_uses_same_copy(self, capsys):
        ...
        lines = provider_setup_failure_lines(RuntimeError("keyring locked"), retry_command="nastech portal")
        assert lines[0].startswith("Could not finish connecting a provider")
        assert "nastech portal" in " ".join(lines)
...
class TestRunLoginProviderAwareCopy:
    def test_minimax_failure_names_minimax_retry_not_portal(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "nastech auth add minimax-oauth" in out
        assert "nastech portal" not in out and "portal.nastechresearch.github.io" not in out
        assert "nastech login" not in out

    def test_nastech_failure_keeps_portal_retry(self, capsys):
        from nastech_cli.auth import PROVIDER_REGISTRY
        from nastech_cli.model_setup_flows_common import _run_login
        ...
        assert _run_login(_boom, SimpleNamespace(), PROVIDER_REGISTRY["nastech"]) is False
        out = capsys.readouterr().out
        assert "nastech portal" in out and "portal.nastechresearch.github.io" in out
    ...
    def test_silent_nonzero_system_exit_still_tells_user_how_to_retry(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "Sign-in did not complete" in out and "nastech auth add openai-codex" in out
    ...
    def test_system_exit_with_message_surfaces_the_message(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "token endpoint returned 400" in out and "nastech model" in out
