"""Tests for _purge_stale_nastech_modules — the class fix for stale
sys.modules breaking the gateway auto-restart after `nastech update`.

Field failure (2026-08-20, Teknium's Linux box): `nastech update` pulled a
checkout where nastech_cli/gateway.py newly imports `line_input` from
nastech_cli.cli_output, but the updater process had cli_output cached from
before that symbol existed. The function-level `from nastech_cli.gateway
...
the Nastech package prefixes so later imports rebuild a self-consistent
...
def _restore_sys_modules():
    ...
    The purge under test evicts real Nastech modules from the cache; later
...
def test_purge_evicts_nastech_prefixed_modules():
    victims = [
        "nastech_cli.cli_output",
        "nastech_cli.gateway",
        "gateway.status",
        "tools.ansi_strip",
        "tui_gateway.server",
    ...
    for name in victims:
        if name not in sys.modules:
            ...
            added.append(name)
    try:
        cli_main._purge_stale_nastech_modules()
        for name in victims:
...
def test_purge_protects_executing_modules():
    ...
    cli_main._purge_stale_nastech_modules()
    assert sys.modules.get("nastech_cli.update_cmd") is update_cmd
    assert sys.modules.get("nastech_cli.main") is cli_main
    assert "nastech_cli" in sys.modules
...
def test_purge_never_raises_on_weird_sys_modules():
    ...
    sys.modules["nastech_cli._purge_test_none"] = None  # type: ignore[assignment]
    try:
        cli_main._purge_stale_nastech_modules()
    finally:
        sys.modules.pop("nastech_cli._purge_test_none", None)
...
def test_stale_symbol_scenario_end_to_end():
    ...
    try:
        ...
        try:
            from nastech_cli.cli_output import line_input  # noqa: F401
            raised = False
        except ImportError:
            raised = True
