...
``nastech_cli.memory_setup._write_env_vars`` persists provider plugin
credentials to ``~/.nastech/.env``. It previously called ``Path.write_text``
...
other subprocess-influencing or Nastech-runtime-location name) could
otherwise plant a value into ``.env`` via the interactive memory-setup
wizard. The next Nastech process would load it through the
...
from nastech_cli.config import ensure_nastech_home, get_env_path, load_env
from nastech_cli.memory_setup import _write_env_vars


def _env_file_keys() -> set[str]:
    """Parse ``~/.nastech/.env`` directly and return the set of keys present.
...
def _nastech_home(tmp_path, monkeypatch):
...
def test_denylisted_key_does_not_block_other_writes(capsys):
    ...
    _write_env_vars({
        ...
        "NASTECH_LANGFUSE_PUBLIC_KEY": "pk-test-123",
        "OPENROUTER_API_KEY": "sk-or-test-456",
    })

    assert "LD_PRELOAD" not in _env_file_keys()
    env = load_env()
    assert env["NASTECH_LANGFUSE_PUBLIC_KEY"] == "pk-test-123"
    assert env["OPENROUTER_API_KEY"] == "sk-or-test-456"


def test_legitimate_nastech_integration_key_still_writable():
    """``NASTECH_*`` overall is NOT blocked — only the four runtime
    location names (HOME/PROFILE/CONFIG/ENV). Integration credentials
    following the ``NASTECH_*`` convention (NASTECH_LANGFUSE_*,
    NASTECH_SPOTIFY_*, NASTECH_QWEN_BASE_URL, ...) must keep working or
    the memory-setup wizard regresses for every plugin that follows
    the convention."""
    _write_env_vars({
        "NASTECH_LANGFUSE_PUBLIC_KEY": "pk-lf-789",
        "NASTECH_QWEN_BASE_URL": "https://example.com/v1",
    })

    env = load_env()
    assert env["NASTECH_LANGFUSE_PUBLIC_KEY"] == "pk-lf-789"
    assert env["NASTECH_QWEN_BASE_URL"] == "https://example.com/v1"
...
def test_explicit_nastech_home_writes_to_that_env_file(tmp_path):
    """Plugin ``post_setup`` hooks (e.g. Supermemory) pass an explicit
    Nastech home; keep that target while still routing through
    ``save_env_value`` validation instead of writing directly."""
    home = tmp_path / "plugin-home"

    _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"}, nastech_home=home)
