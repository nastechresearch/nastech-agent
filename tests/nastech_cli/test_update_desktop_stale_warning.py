...
#88251: ``nastech update`` treated a failed desktop pack as non-fatal, printed
...
import nastech_cli.update_cmd_maint as update_cmd_maint
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_constants.with_nastech_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "nastech_constants.display_nastech_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_nastech_home", lambda: tmp_path)
