"""Unit tests for the Nastech Portal profile's reasoning wiring.
...
@pytest.fixture
def nastech_profile():
    """Resolve the registered Nastech profile through the real discovery path."""
    # ``model_tools`` triggers plugin discovery on import, which is what
    # registers the Nastech profile in the global provider registry.
    import model_tools  # noqa: F401
    import providers

    profile = providers.get_provider_profile("nastech")
    assert profile is not None, "nastech provider profile must be registered"
    return profile
...
def portal_catalog(monkeypatch):
    ...
    monkeypatch.setattr(models_mod, "_nastech_reasoning_caps_failed_at", None)
    monkeypatch.setattr(models_mod, "_nastech_reasoning_caps_cache", {
...
class TestNastechReasoningWireShape:
    """``build_api_kwargs_extras`` produces the Portal's wire format."""

    def test_disable_reaches_optional_reasoning_model(self, nastech_profile, portal_catalog):
        """The knob the user set is the knob the Portal receives."""
        extra_body, top_level = nastech_profile.build_api_kwargs_extras(
    ...
    def test_disable_dropped_for_mandatory_reasoning_model(self, nastech_profile, portal_catalog):
        """Mandatory routes 400 on a disable — send nothing instead."""
        extra_body, _ = nastech_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
            supports_reasoning=True,
    ...
    def test_disable_dropped_for_unknown_model(self, nastech_profile, portal_catalog):
    ...
    def test_disable_dropped_for_non_reasoning_route(self, nastech_profile, portal_catalog):
        """A route the catalog says takes no reasoning parameter gets none.

        Nastech' own ``supports_reasoning`` can disagree with the Portal about a
    ...
    def test_enabled_config_always_forwarded(self, nastech_profile, portal_catalog, model):
    ...
    def test_no_config_defaults_to_medium(self, nastech_profile, portal_catalog):
        extra_body, _ = nastech_profile.build_api_kwargs_extras(
    ...
    def test_nothing_emitted_without_reasoning_support(self, nastech_profile, portal_catalog):
        extra_body, top_level = nastech_profile.build_api_kwargs_extras(
            reasoning_config={"enabled": False},
    ...
    def test_caller_config_not_mutated(self, nastech_profile, portal_catalog):
        cfg = {"enabled": False}
        nastech_profile.build_api_kwargs_extras(
