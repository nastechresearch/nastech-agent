...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "nastech_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "nastech_cli.web_server_chat", stub)
    monkeypatch.setattr(nastech_cli, "web_server_chat", stub, raising=False)
...
def test_dashboard_board_project_binding_is_exposed_in_ui():
    ...
    assert "nastech-kanban-board-project-unbind" in bundle
...
# tests/nastech_cli/test_kanban_db.py. The OUTER fallback here is not, which
