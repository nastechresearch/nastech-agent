...
from nastech_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(nastech_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <nastech_home>/plugins/<name> and opt it in."""
    plugin_dir = nastech_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "nastech_empty"))
