"""Plugin data paths follow the active profile's NASTECH_HOME, including the ContextVar override.

Several plugins carried a ``~/.nastech`` fallback (guarding an ImportError of ``nastech_constants``
...
def _openviking_log(home):
    ...
    return ov.get_nastech_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
