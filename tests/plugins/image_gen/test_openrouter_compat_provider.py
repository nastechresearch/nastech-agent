...
class TestLiveCatalog:
    ...
    def test_nastech_portal_picker_excludes_image_api_catalog(self):
        """Nastech Portal has no /images route; its picker must not offer
        Image-API-only models it cannot serve."""
        from plugins.image_gen.openrouter import _build_providers

        nastech = {p.name: p for p in _build_providers()}["nastech"]
        with patch(_RUNTIME, side_effect=RuntimeError("no creds")):
            ids = [m["id"] for m in nastech.list_models()]
        from plugins.image_gen.openrouter import DEFAULT_MODEL, _FALLBACK_MODEL
...
class TestImageApiSurface:
    ...
    def test_nastech_never_uses_the_image_api(self):
        """Nastech Portal proxies chat-completions and has no /images route."""
        from plugins.image_gen.openrouter import _build_providers
    ...
    def test_catalog_models_are_offered_only_by_openrouter(self):
        ...
        nastech_ids = {m["id"] for m in by_name["nastech"].list_models()}
        assert "openai/gpt-image-2" in openrouter_ids
        assert set(_IMAGE_API_MODELS) <= openrouter_ids
        assert not (set(_IMAGE_API_MODELS) & nastech_ids)
