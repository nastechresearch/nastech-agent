...
class TestRequestMessageCoercion:
    ...
    def test_messages_for_langfuse_includes_anthropic_system_param(self):
        ...
        out = mod._messages_for_langfuse_input(
            request_messages=[{"role": "user", "content": "hi"}],
            system_prompt="You are Nastech.",
        )
        assert out[0]["role"] == "system"
        assert out[0]["content"] == "You are Nastech."
...
# ---------------------------------------------------------------------------
# Capture modes: metadata | sanitized | full  (NASTECH_LANGFUSE_CAPTURE)
# ---------------------------------------------------------------------------
...
class TestCaptureModes:
    ...
    def test_default_mode_is_sanitized(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.delenv("NASTECH_LANGFUSE_CAPTURE", raising=False)
        assert mod._capture_mode() == "sanitized"

    def test_invalid_mode_falls_back_and_warns_once(self, monkeypatch, caplog):
        mod = self._fresh_plugin()
        monkeypatch.setenv("NASTECH_LANGFUSE_CAPTURE", "everything")
        with caplog.at_level(logging.WARNING):
            assert mod._capture_mode() == "sanitized"
            assert mod._capture_mode() == "sanitized"
        warnings = [r for r in caplog.records if "NASTECH_LANGFUSE_CAPTURE" in r.getMessage()]
        assert len(warnings) == 1

    def test_metadata_mode_omits_content(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("NASTECH_LANGFUSE_CAPTURE", "metadata")
    ...
    def test_sanitized_mode_redacts_secrets(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("NASTECH_LANGFUSE_CAPTURE", "sanitized")
        samples = {
    ...
    def test_full_mode_keeps_secret_shaped_text(self, monkeypatch):
        mod = self._fresh_plugin()
        monkeypatch.setenv("NASTECH_LANGFUSE_CAPTURE", "full")
...
class TestAtexitFinalization(TestTurnTraceIsolation):
    """Short-lived processes (kanban workers, `nastech chat -q`, cron) can exit
    ...
    def test_atexit_hook_is_registered_on_client_init(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_LANGFUSE_PUBLIC_KEY", "pk-lf-0123456789abcdef")
        monkeypatch.setenv("NASTECH_LANGFUSE_SECRET_KEY", "sk-lf-0123456789abcdef")
    ...
    def test_finalize_flushes_every_profile_without_an_ambient_scope(self, monkeypatch, tmp_path):
        ...
        for profile in ("alpha", "beta"):
            ...
            home.mkdir()
            home_token = set_nastech_home_override(home)
            scope_token = secret_scope.set_secret_scope({
                "NASTECH_LANGFUSE_PUBLIC_KEY": f"pk-lf-{profile}-0123456789",
                "NASTECH_LANGFUSE_SECRET_KEY": f"sk-lf-{profile}-0123456789",
            })
            try:
...
class TestSystemPromptInGenerationInput:
    ...
    and the Responses/Codex API (``instructions``).  Nastech forwards it to
    ...
    def test_string_system_prompt_prepended(self, monkeypatch):
        ...
        assert captured["input"][0]["content"] == "You are Nastech."
    ...
    def test_metadata_records_chars(self, monkeypatch):
        ...
        assert captured["metadata"]["system_prompt_chars"] == len("You are Nastech.")
...
class TestSystemPromptCrossesHookBoundary:
    ...
    no Nastech internals mocked (only the Langfuse client is faked)."""
