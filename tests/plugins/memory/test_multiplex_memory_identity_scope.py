...
OPENVIKING_* / HINDSIGHT_BANK_ID / NASTECH_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "NASTECH_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def secondary_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(prof_b))
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("nastech", "", "platform")
    ...
    provider.initialize("s1", nastech_home=str(secondary_profile))
    assert provider._client.project == "nastech-b"
    ...
    assert (hcfg["banks"]["nastech"]["bankId"], hcfg["mode"]) == ("nastech", "cloud")
