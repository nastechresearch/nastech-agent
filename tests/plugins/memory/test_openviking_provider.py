...
_EXPECTED_USER_AGENT = f"openviking-memory-nastech/{_NASTECH_VERSION}"
...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Nastech's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Nastech
    venv instead of its own, and on Windows locks Nastech venv DLLs so the
    venv cannot be rebuilt during `nastech update`.
    """
    popen_calls = []
    ...
    monkeypatch.setattr(openviking_module.shutil, "which", lambda name: "/usr/local/bin/openviking-server")
    monkeypatch.setattr(openviking_module.subprocess, "Popen", fake_popen)
    monkeypatch.setenv("PYTHONPATH", "/opt/nastech/.venv/Lib/site-packages")
    monkeypatch.setenv("NASTECH_PROFILE", "test-profile")
    ...
    assert child_env.get("NASTECH_PROFILE") == "test-profile"
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Nastech and
