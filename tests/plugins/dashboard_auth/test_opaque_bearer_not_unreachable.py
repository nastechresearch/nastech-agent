...
HTTP 503 ``{"detail": "Auth provider 'nastech' unreachable"}`` while Portal was
perfectly healthy: ``NastechDashboardAuthProvider._verify_jwt`` folded *every*
...
Real ``NastechDashboardAuthProvider`` + real ``SelfHostedOIDCProvider`` JWKS path,
...
def _nastech(portal_url: str) -> nastech_plugin.NastechDashboardAuthProvider:
    return nastech_plugin.NastechDashboardAuthProvider(client_id="agent:test-instance", portal_url=portal_url)
...
def test_opaque_bearer_with_healthy_portal_is_not_unreachable(empty_jwks_server):
    provider = _nastech(empty_jwks_server)
...
def test_real_jwt_with_unreachable_portal_still_raises_provider_error():
    provider = _nastech("http://127.0.0.1:9")  # discard port: connection refused
...
def test_opaque_bearer_with_unreachable_portal_is_still_just_not_ours():
    ...
    provider = _nastech("http://127.0.0.1:9")
    assert provider.verify_session(access_token=OPAQUE_PEER_KEY) is None
...
@pytest.fixture
def _gated_nastech(empty_jwks_server):
    clear_providers()
    ...
    register_provider(_nastech(empty_jwks_server))
...
def test_gated_api_rejects_opaque_bearer_with_401_not_503(_gated_nastech):
    r = _gated_nastech.get("/api/auth/me", headers={"Authorization": f"Bearer {OPAQUE_PEER_KEY}"})
    assert r.status_code != 503, r.text
    assert r.status_code == 401
    assert "unreachable" not in r.text.lower()


def test_gated_api_rejects_opaque_cookie_with_401_not_503(_gated_nastech):
    _gated_nastech.cookies.set(SESSION_AT_COOKIE, OPAQUE_PEER_KEY)
    r = _gated_nastech.get("/api/auth/me")
    assert r.status_code != 503, r.text
    assert "unreachable" not in r.text.lower()
