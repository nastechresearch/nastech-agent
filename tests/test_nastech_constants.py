...
class TestGetDefaultNastechRoot:
    ...
    def test_expanded_custom_profile_returns_custom_root(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv(
            "NASTECH_HOME", f"{home_token}/deployment/profiles/research"
        )
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "native-home")

        assert get_default_nastech_root() == custom_root
    ...
    def test_result_memoised_until_env_or_home_changes(self, tmp_path, monkeypatch):
        ...
        # _get_platform_default_nastech_home() on every call (even hits), so
        ...
        monkeypatch.setattr(
            nastech_constants, "_default_nastech_root_memo", None, raising=False
        )

        first = get_default_nastech_root()
        ...
        # NASTECH_HOME change invalidates the memo (fresh resolution).
        other_profile = docker_root / "profiles" / "writer"
        other_profile.mkdir(parents=True)
        monkeypatch.setenv("NASTECH_HOME", str(other_profile))
        before = resolve_calls["n"]
        assert get_default_nastech_root() == docker_root
        assert resolve_calls["n"] > before, (
            "NASTECH_HOME change must force a fresh resolution"
        )
...
class TestGetProcessNastechHome:
    ...
    def test_process_and_context_homes_expand_environment_and_user_syntax(
        ...
        for syntax in (home_token, "~"):
            ...
            monkeypatch.setenv("NASTECH_HOME", f"{syntax}/process-home")
            assert get_process_nastech_home() == process_home

            override_home = tmp_path / "override-home"
            token = set_nastech_home_override(f"{syntax}/override-home")
            try:
                assert get_nastech_home() == override_home
                assert get_process_nastech_home() == process_home
            finally:
...
class TestSecureParentDir:
    ...
    def test_install_tree_skipped(self, monkeypatch):
        ...
        Regression test for #93050: secure_parent_dir() chmod'd /opt/nastech to
        0700 because it has 3 path parts and passed the ``< 3`` guard, locking
        out UID 10000 (nastech user) from traversing the install dir.
        """
        install_root = Path(nastech_constants.__file__).resolve().parent

        # Directly under the install root (e.g. /opt/nastech/auth.json)
    ...
    def test_install_tree_siblings_still_hardened(self, monkeypatch):
        ...
        # Prefix-named sibling of the install root (/opt/nastech-data/...).
...
class TestManagedNodeTreeInUse:
    ...
    def _install_fake_psutil(self, monkeypatch, procs):
        ...
        monkeypatch.setattr(nastech_constants.sys, "platform", "win32")
    ...
    def test_always_false_off_windows(self, tmp_path, monkeypatch):
        monkeypatch.setattr(nastech_constants.sys, "platform", "darwin")
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path))
        assert nastech_constants.managed_node_tree_in_use() is False
    ...
    def test_exe_under_node_dir_counts(self, tmp_path, monkeypatch):
        ...
        assert nastech_constants.managed_node_tree_in_use() is True
...
class TestWindowsHealStageSwap:
    ...
    def _stub_env(
        ...
        monkeypatch.setenv("NASTECH_HOME", str(home))
        monkeypatch.setenv(
            "NASTECH_NODE_TARGET_MAJOR",
            str(nastech_constants._NASTECH_NODE_TARGET_MAJOR),
        )
        monkeypatch.setattr(nastech_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(
            nastech_constants, "_managed_node_in_use_notice_printed", False
        )
        monkeypatch.setattr(
            nastech_constants,
        ...
        monkeypatch.setattr(
            nastech_constants, "node_tool_runnable", lambda path: True
        )
    ...
    def test_in_use_defers_without_touching_tree(self, tmp_path, monkeypatch):
        ...
        zip_name, zip_bytes = _make_node_zip(nastech_constants._NASTECH_NODE_TARGET_MAJOR)
        ...
        result = nastech_constants._heal_managed_node_windows()
...
class TestHealAttemptFlagSemantics:
    ...
    def test_deferral_keeps_flag_clear_and_retries(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(nastech_constants, "_heal_managed_node_windows", fake_heal)

        assert heal_nastech_managed_node() is False
        assert nastech_constants._managed_node_heal_attempted is False
    ...
    def test_real_failure_records_attempt(self, tmp_path, monkeypatch):
        ...
        assert heal_nastech_managed_node() is False
        assert nastech_constants._managed_node_heal_attempted is True
