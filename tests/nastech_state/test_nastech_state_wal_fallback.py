...
from nastech_state import SessionDB, format_session_db_unavailable, get_last_init_error
from nastech_state_wal import WalUnsupportedError, apply_wal_with_fallback
...
def _reset_wal_fallback_warned_paths():
    ...
    nastech_state_wal._wal_delete_fallback_failed_paths.clear()
    yield
    nastech_state_wal._wal_fallback_warned_paths.clear()
    nastech_state_wal._wal_delete_fallback_failed_paths.clear()
...
def _assume_fixed_sqlite(monkeypatch):
    ...
    nastech_state_wal._wal_reset_bug_warned_paths.clear()
    yield
    nastech_state_wal._wal_reset_bug_warned_paths.clear()
...
class TestFormatSessionDbUnavailable:
    ...
    def test_adds_nfs_hint_for_locking_protocol(self):
        ...
        # a move-it action (`nastech doctor --fix` cannot repair a mount; a WAL-docs link is not
        # something a chat user can act on).
        lead, details = msg.splitlines()
        assert "network" in lead
        assert "local disk" in lead
        assert "nastech doctor --fix" not in lead
    ...
    def test_known_bucket_keeps_table_action_and_appends_network_hint(self):
        ...
        nastech_state._set_last_init_error("OperationalError: disk I/O error")
        msg = format_session_db_unavailable()
