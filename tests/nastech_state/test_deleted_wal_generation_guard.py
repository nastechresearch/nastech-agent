...
from nastech_state_dbfile import _pread_db_header, iter_deleted_sqlite_sidecar_holders
from tests.nastech_state._wal_generation_harness import (
...
def test_iter_holders_empty_on_non_linux(monkeypatch, tmp_path):
    monkeypatch.setattr(nastech_state.sys, "platform", "win32")
...
def test_delete_journal_two_writers_still_work(tmp_path, monkeypatch):
    monkeypatch.setattr(nastech_state_wal, "resolve_journal_mode", lambda: "delete")
    monkeypatch.setattr(
...
def test_iter_holders_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(nastech_state_dbfile.os, "readlink", fake_readlink)
    try:
...
def test_iter_holders_ignores_descriptor_closed_during_scan(tmp_path, monkeypatch, vanish_errno):
    ...
    monkeypatch.setattr(nastech_state_dbfile.sys, "platform", "linux")
    monkeypatch.setattr(
        nastech_state_dbfile,
    ...
    monkeypatch.setattr(nastech_state_dbfile.os, "stat", stat_vanished)
...
def test_write_path_ignores_live_unhashed_dentry(tmp_path, force_wal, monkeypatch):
    ...
    monkeypatch.setattr(nastech_state_readpool.os, "readlink", fake_readlink)
    try:
