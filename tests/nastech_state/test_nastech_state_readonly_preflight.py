...
- files inside the Nastech home tree are repaired with ``chmod u+rw``
...
from nastech_state import SessionDB, preflight_db_writability
...
class TestRepairScope:
    def test_repairs_readonly_db_inside_home(self, nastech_home):
        db = nastech_home / "state.db"
        _make_db(db)
        os.chmod(db, 0o444)
    ...
    def test_repairs_readonly_sidecars(self, nastech_home):
        db = nastech_home / "state.db"
        _make_wal_db(db)
    ...
    def test_repairs_readonly_parent_directory(self, nastech_home):
        sub = nastech_home / "kanban"
        sub.mkdir()
        db = sub / "kanban.db"
        _make_db(db)
        os.chmod(sub, 0o555)
...
class TestRefusalOutsideScope:
    def test_actionable_error_names_file_and_chmod(self, nastech_home, tmp_path):
    ...
    def test_wal_error_warns_against_deletion(self, nastech_home, tmp_path):
        outside = tmp_path / "elsewhere"
        outside.mkdir()
        db = outside / "custom.db"
        _make_wal_db(db)
...
class TestSkips:
    ...
    def test_healthy_db_untouched(self, nastech_home):
        db = nastech_home / "state.db"
        _make_db(db)
...
class TestSessionDBIntegration:
    def test_sessiondb_selfheals_readonly_db_in_home(self, nastech_home):
        db_path = nastech_home / "state.db"
