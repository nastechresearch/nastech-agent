...
from nastech_state_wal import apply_wal_with_fallback, is_sqlite_wal_reset_vulnerable, sqlite_source_id
...
class TestApplyWalWalResetGate:
    def test_fresh_db_uses_delete_when_vulnerable(self, tmp_path, monkeypatch, caplog):
        monkeypatch.setattr(
            nastech_state_wal, "is_sqlite_wal_reset_vulnerable", lambda version_info=None: True
        )
...
class TestNoDowngradeUnderConcurrentOpeners:
    ...
    def test_probe_unreadable_touches_nothing(self, tmp_path, monkeypatch, caplog):
        ...
        nastech_state_wal._wal_probe_unknown_paths.clear()
        ...
        try:
            with caplog.at_level(logging.WARNING, logger="nastech_state_wal"):
...
def test_doctor_warns_without_adding_issues(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(nastech_state_wal, "sqlite_source_id", lambda: "testid-abc")
