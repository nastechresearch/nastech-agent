...
import nastech_state_schema
from nastech_state import SessionDB
from nastech_state_common import FTS_REBUILD_DEFERRAL_KEY, FTS_STALE_KEY, LEGACY_FTS_SQL, LEGACY_FTS_TRIGRAM_SQL, SCHEMA_SQL, _FTS_TRIGGERS
from nastech_state_dbfile import _concrete_state_db_holder_pids, _is_inactive_orphan_desktop_holder
...
class TestRuntimeFtsRebuild:
    def test_reap_candidates_exclude_uninspectable_holder_suspicions(
        ...
        assert _concrete_state_db_holder_pids(
            db_path,
            [
                (222, "uninspectable holder: python -m nastech_cli.main serve --port 0"),
    ...
    @pytest.mark.parametrize(
        "argv",
        (
            ("journalctl", "-u", "nastech-agent.service"),
            ("grep", "nastech-agent", "/var/log/syslog"),
            (
                "/usr/sbin/tailscaled",
                "be-child",
                "ssh",
                "--cmd=python -m nastech_cli.main gateway",
            ),
            ("tmux", "new-session", "/opt/nastech-agent/.venv/bin/nastech gateway"),
            ("python3", "/opt/nastech-agent/tools/check_state.py"),
            ("nastech-monitor", "gateway"),
            ("hermesctl", "serve"),
            ("python3", "worker.py", "nastech_cli.main"),
            ("python3", "-m", "other.module", "nastech_cli.main"),
            ("python3", "-c", "nastech_cli.main"),
            ("python3", "-Icprint('nastech_cli.main')", "nastech_cli/main.py"),
        ),
    )
    def test_uninspectable_non_nastech_process_is_not_a_holder(self, argv):
        assert not nastech_state_holders._looks_like_nastech(argv)

    @pytest.mark.parametrize(
        "argv",
        (
            ("/usr/local/bin/nastech", "gateway"),
            ("/usr/local/bin/nastech-agent", "serve"),
            ("/usr/local/bin/nastech-acp", "--stdio"),
            ("/usr/bin/python3", "-m", "nastech_cli.main", "gateway"),
            ("/usr/bin/python3", "-m", "acp_adapter"),
            ("/usr/bin/python3", "-Im", "nastech_cli.main", "gateway"),
            ("/usr/bin/python3", "-mnastech_cli.main", "gateway"),
            ("/usr/bin/python3", "-W", "ignore", "-m", "nastech_cli.main"),
            ("/usr/bin/python3", "-Xdev", "-m", "nastech_cli.main"),
            (
                "/opt/nastech-agent/.venv/bin/python",
                "/opt/nastech-agent/nastech_cli/main.py",
                "gateway",
            ),
            ("python.exe", "--", "nastech_cli/main.py", "gateway"),
            ("python3", "/opt/nastech-agent/run_agent.py", "--query", "hello"),
        ),
    )
    def test_uninspectable_nastech_process_remains_a_holder(self, argv):
        assert nastech_state_holders._looks_like_nastech(argv)
    ...
    def test_foreign_holder_detection_proc_readlink_deleted_wal(
        ...
        monkeypatch.setattr(nastech_state_holders.os, "stat", _stat)
    ...
    def test_foreign_holder_ignores_same_path_with_different_file_identity(
        ...
        assert nastech_state_holders.foreign_state_db_holders(db_path) == []
    ...
    def test_foreign_holder_uninspectable_process_cmdline_fallback(
        ...
        flagged when /proc/<pid>/cmdline identifies it as a Nastech process."""
        db_path = tmp_path / "state.db"
        ...
        # PID 222's cmdline is world-readable and looks like Nastech
        cmdline_path = proc_root / "222" / "cmdline"
        cmdline_path.write_bytes(
            b"python3\x00-m\x00nastech_cli.main\x00chat\x00"
        )
        ...
        def _fake_argv(pid):
            ...
            try:
                ...
                return None
        monkeypatch.setattr(nastech_state_holders, "_read_proc_argv", _fake_argv)

        holders = nastech_state_holders.foreign_state_db_holders(db_path)
        # Should include PID 222 with the cmdline info
        assert len(holders) == 1
        assert holders[0][0] == 222
        assert "nastech_cli.main" in holders[0][1]
    ...
    def test_structural_corruption_propagates_without_live_fts_mutation(
        ...
        from nastech_state import StateDbCorruptError
    ...
    def test_repeated_deferrals_reap_inactive_orphan_then_rebuild(
        ...
        monkeypatch.setattr(nastech_state_schema.time, "time", lambda: 120.0)
    ...
    def test_same_holder_set_across_futile_window_names_holder_and_gateway_safe_remedy(
        ...
        monkeypatch.setattr(
            nastech_state_schema, "_read_proc_argv", lambda pid: ["python", "-m", "nastech_cli.main", "serve"], raising=False,
        )
        clock = [1000.0]
        monkeypatch.setattr(nastech_state_schema.time, "time", lambda: clock[0])

        from nastech_cli.doctor_state import _render_state_db_stats
        from nastech_state_dbfile import collect_state_db_stats
        ...
        futile_attempts = getattr(nastech_state_schema, "_FTS_HOLDER_FUTILE_ATTEMPTS", 10)
        futile_seconds = getattr(nastech_state_schema, "_FTS_HOLDER_FUTILE_SECONDS", 1800.0)
        reopened = SessionDB(db_path=db_path)
        try:
            ...
            assert "pid 4242: python -m nastech_cli.main serve" in msg
    ...
    def test_retry_backoff_resets_when_the_blocking_holder_set_changes(
        ...
        try:
            ...
            reopened._fts_stale_retry_after = time.monotonic() + nastech_state_schema._FTS_STALE_RETRY_MAX_SECONDS
            reopened._fts_stale_retry_interval = nastech_state_schema._FTS_STALE_RETRY_MAX_SECONDS
