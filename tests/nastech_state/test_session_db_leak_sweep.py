...
A raw single-process ``pytest tests/nastech_cli/`` used to accumulate every
...
* ``nastech_state_guard._register_test_instance`` adds every successfully
  constructed SessionDB to a WeakSet registry when the
  ``NASTECH_TEST_ISOLATION`` marker is set (test-isolation runs only).
...
def test_constructed_sessiondb_is_registered(tmp_path):
    ...
    try:
        assert db in nastech_state_guard._test_instance_registry
    finally:
        db.close()
