...
child at the same moment the child lost the ``NASTECH_HOME`` redirect. That
pairing is exactly how fixture rows (dm:123 / chat-1 / wx-chat) landed in a
developer's production state.db.

``NASTECH_TEST_ISOLATION`` is Nastech's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``nastech_state`` honors it as a
...
def test_conftest_exports_the_marker():
    ...
    assert os.environ.get("NASTECH_TEST_ISOLATION"), (
        "NASTECH_TEST_ISOLATION must be exported by tests/conftest.py so "
...
def test_child_with_rebuilt_env_keeping_marker_refuses_production_db():
    ...
    stripped, NASTECH_HOME lost) but which keeps the marker must still refuse
    to open the production state.db."""
    env = _minimal_env(NASTECH_TEST_ISOLATION="/tmp/pytest-isolation-root")
...
def test_child_bypass_env_disarms_guard_even_with_marker():
    ...
    DB: NASTECH_STATE_DB_GUARD_BYPASS=1, not marker-stripping."""
    env = _minimal_env(
        NASTECH_TEST_ISOLATION="/tmp/pytest-isolation-root",
