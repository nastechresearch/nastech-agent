...
# Must match the root the guard itself computes.  Hardcoding ``~/.nastech``
# silently disarmed every assertion below on Windows, where the real root is
# ``%LOCALAPPDATA%\nastech``: the paths under test were then *correctly*
...
REAL_ROOT = nastech_state._real_platform_state_root()
