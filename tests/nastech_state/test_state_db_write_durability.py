...
The defect: nastech_state already knows macOS `fsync()` does not guarantee
...
from nastech_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in nastech_state_repair; nastech_state only
    # re-imports them.
    import nastech_state_repair

    source = Path(nastech_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(nastech_state_repair.__file__))
