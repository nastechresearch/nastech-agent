...
work on a file several Nastech processes share (gateway service, the Desktop
app's `nastech serve` backend, CLI sessions, the TUI slash worker):

* `nastech_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `nastech_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
