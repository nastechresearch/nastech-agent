...
VALID_ENTRY = {
    "name": "example-plugin",
    "repo": "https://github.com/NastechResearch/nastech-example-plugins",
    "sha": "38fe0fb53eff98d477f807432e965429e665ca33",
    "subdir": "",
    "description": "One-line description.",
    "maintainer": "NastechResearch",
    "tier": "official",
    "requires_nastech": ">=0.19",
    "docs_url": "",
    "platforms": [],
    "capabilities": {
...
def test_bad_requires_nastech_spec_fails(tmp_path):
    _expect_error(tmp_path, {"requires_nastech": "banana"}, "requires_nastech")


def test_comma_separated_requires_nastech_passes(tmp_path):
    entry = {**VALID_ENTRY, "requires_nastech": ">=0.19, <2.0"}
    path = write_entry(tmp_path, entry)
    result = run_validator(str(path))
    assert result.returncode == 0, result.stdout + result.stderr
