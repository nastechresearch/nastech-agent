...
`scripts/desktop-update/windows.ps1` drives `nastech update` for the in-app
Desktop updater. It used to invoke the update through the venv's
`venv\\Scripts\\nastech.exe` console-script launcher. On Windows that launcher is
a real process that keeps `nastech.exe` mapped as its running image and spawns
`python.exe` as a child. The update ends in `uv pip install -e .`, which rewrites
the console-script shims -- including the `nastech.exe` the launcher still has
...
Driving the update as `python.exe -m nastech_cli.main update` puts the inherited
...
hand-off. The invariant it guards is that every `Invoke-NastechStep` call site
(the update, its retry, and the desktop rebuild) drives `$pythonExe`, never the
`$nastechExe` shim. `nastech.exe` may still be *named* in the file for the
...
def _handoff_source() -> str:
    ...
    fixture runs a synthetic PowerShell step through ``Invoke-NastechStep`` to
...
def test_invoke_nastech_step_calls_drive_python_not_the_shim() -> None:
    source = _handoff_source()

    invocations = re.findall(r"Invoke-NastechStep\s+(\$\w+)", source)
    assert invocations, (
        "Expected at least one Invoke-NastechStep call in "
    ...
    assert not offenders, (
        "Every Invoke-NastechStep call in scripts/desktop-update/windows.ps1 "
        "must drive $pythonExe, not the nastech.exe shim. Driving the update "
        "through the shim keeps nastech.exe mapped as a running image, so uv's "
...
def test_update_invocation_uses_module_entrypoint() -> None:
    ...
    assert '@("-m", "nastech_cli.main", "update"' in source, (
        "The update step must invoke `python.exe -m nastech_cli.main update ...` "
        "so the inherited image handle lands on python.exe, which uv never has "
        "to replace."
    )
    assert (
        '@("-m", "nastech_cli.main", "desktop", "--force-build", "--build-only")'
        in source
    ), (
        "The desktop rebuild step must also go through "
        "`python.exe -m nastech_cli.main desktop ...` for the same reason."
    )


def test_update_no_longer_invokes_the_nastech_exe_shim() -> None:
    source = _read()

    assert "Invoke-NastechStep $nastechExe" not in source, (
        "scripts/desktop-update/windows.ps1 still invokes the update through "
        "the nastech.exe shim (`Invoke-NastechStep $nastechExe`). That is the "
