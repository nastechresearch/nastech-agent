...
def gateway_session(monkeypatch):
    ...
    for k in ("NASTECH_CRON_SESSION", "NASTECH_YOLO_MODE", "NASTECH_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("NASTECH_GATEWAY_SESSION", "1")
    monkeypatch.setenv("NASTECH_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from NASTECH_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("NASTECH_CRON_SESSION", "NASTECH_YOLO_MODE", "NASTECH_GATEWAY_SESSION", "NASTECH_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
