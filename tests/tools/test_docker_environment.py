...
def test_snap_compat_drops_only_init_and_no_new_privileges(monkeypatch):
    ...
    def strip(argv):  # everything except the two flags and the random container name
        return [a for a in argv if a not in ("--init", "--security-opt", "no-new-privileges") and not a.startswith("nastech-")]
...
def test_shared_container_key_replaces_profile_identity(monkeypatch):
    ...
    assert a._labels["nastech-profile"] == b._labels["nastech-profile"]
    assert a._labels["nastech-profile"] != "research"
    assert a._labels["nastech-profile"].startswith("team_workspace-")
...
def test_distinct_shared_keys_never_collide(monkeypatch):
    ...
    assert c._labels["nastech-profile"] != d._labels["nastech-profile"]
    # Both stay within Docker's 63-char label-value bound.
    assert len(c._labels["nastech-profile"]) <= 63
    assert len(d._labels["nastech-profile"]) <= 63
...
def test_empty_shared_container_key_preserves_profile_isolation(monkeypatch):
    ...
    assert env._labels["nastech-profile"] == "research"
        ...
        if isinstance(cmd, list) and len(cmd) >= 2:
            ...
            if sub == "ps":
                if "--format" in cmd:
                    ...
                    assert any(
                        str(part) == "label=nastech-egress=off" for part in cmd
