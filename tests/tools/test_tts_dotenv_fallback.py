...
class TestDotenvFallbackPerProvider:
    ...
    dotenv-backed lookup by mocking ``nastech_cli.config.get_env_value`` directly;
    ...
    def test_elevenlabs_reads_dotenv_key(self, tmp_path):
        ...
        with patch("nastech_cli.config.get_env_value", return_value="el-dotenv-key"), \
    ...
    def test_xai_reads_dotenv_key(self, tmp_path):
        ...
        canonical ``nastech_cli.config.get_env_value`` — the dotenv contract from #17140.
        """
        from tools import tts_tool
        ...
        with patch("nastech_cli.config.get_env_value", return_value="xai-dotenv-key"), \
    ...
    def test_gemini_reads_dotenv_key(self, tmp_path):
        ...
        with patch("nastech_cli.config.get_env_value", side_effect=fake_get_env_value), \
             patch("requests.post", side_effect=fake_post):
