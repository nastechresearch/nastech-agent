...
from tools.environments.local_env_policy import _ALWAYS_STRIP_KEYS, _NASTECH_PROVIDER_ENV_FORCE_PREFIX
...
class TestStripByDefault:
    ...
    def test_buzz_platform_vars_stripped_by_default(self):
        ...
        for var in buzz_sample:
            assert var not in result, f"{var} leaked via nastech_subprocess_env"
...
class TestDelegatedChildMarker:
    def test_delegated_child_context_scrubs_parent_kanban_keys_and_sets_marker(self):
        ...
        assert env["NASTECH_KANBAN_DB"] == "/tmp/parent-kanban.db"
        assert env["NASTECH_KANBAN_WORKSPACE"] == "/tmp/parent-workspace"
