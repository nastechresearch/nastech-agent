...
Nastech exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_nastech_cache_not_tmp(tmp_path, monkeypatch):
    ...
    assert result == str(tmp_path / ".nastech" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "nastech_bg_dead1.log").exists()
