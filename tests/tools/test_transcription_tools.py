...
class TestExplicitOpenaiSelectionError:
    ...
    with the ``nastech tools`` remediation for managed-Nastech users), the old
    ...
    def test_get_provider_openai_none_not_generic_when_managed_route_down(
        ...
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_nastech_tools_enabled", lambda: True
        )
        monkeypatch.setattr(
    ...
    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        ...
        its nastech tools remediation instead of the all-provider install
        hint."""
        self._no_openai_credentials(monkeypatch)
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_nastech_tools_enabled", lambda: True
        ...
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
             patch("tools.transcription_tools._HAS_FASTER_WHISPER", False), \
             patch(
                 "tools.tool_backend_helpers.nastech_tool_gateway_unavailable_message",
                 lambda what: f"managed route down for {what}; run `nastech tools`",
             ):
        ...
        assert "nastech tools" in result["error"]
