...
def test_delegate_child_execute_code_env_bridges_contextvar_and_scrubs_kanban(
    ...
    assert env["NASTECH_DELEGATED_CHILD_CONTEXT"]  # fenced board root (path), not a bare flag
    ...
    assert env["NASTECH_KANBAN_DB"] == str(home / "kanban.db")
    assert env["NASTECH_KANBAN_WORKSPACE"] == str(tmp_path / "parent-workspace")
...
def test_auto_heartbeat_reports_failure_without_mutating_fenced_child_board(
    ...
    try:
        ...
        monkeypatch.setenv("NASTECH_DELEGATED_CHILD_CONTEXT", str(tmp_path / ".nastech"))
        ...
        with caplog.at_level(logging.WARNING, logger="tools.kanban_tools"):
            ...
            monkeypatch.setattr(kanban_tools, "_auto_heartbeat_last_attempt", 0.0)
            assert kanban_tools.heartbeat_current_worker_from_env() is False
        fence_warnings = [r for r in caplog.records if "NASTECH_DELEGATED_CHILD_CONTEXT" in r.getMessage()]
        assert len(fence_warnings) == 1 and tid in fence_warnings[0].getMessage()

        monkeypatch.delenv("NASTECH_DELEGATED_CHILD_CONTEXT")
        monkeypatch.setattr(kanban_tools, "_auto_heartbeat_last_attempt", 0.0)
