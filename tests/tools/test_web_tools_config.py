...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        self._managed_patchers = [
            patch("tools.tool_backend_helpers.managed_nastech_tools_enabled", return_value=True),
    ...
    def test_no_config_raises_with_helpful_message(self):
        ...
        with patch("plugins.web.firecrawl.provider.Firecrawl"):
            with patch("tools.managed_tool_gateway.read_nastech_access_token", return_value=None):
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "nastechresearch.github.io"}):
...
class TestBackendSelection:
    ...
    def test_nastech_backend_maps_to_firecrawl(self):
        """The managed 'nastech' selection is serviced by the firecrawl
        provider (whose client resolver routes managed)."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={"backend": "nastech"}):
            assert _get_backend() == "firecrawl"
    ...
    def test_managed_gateway_does_not_preempt_explicit_tavily(self):
        """A Nastech OAuth token must not beat an explicit TAVILY_API_KEY."""
        from tools.web_tools import _get_backend
        with patch("tools.web_tools._load_web_config", return_value={}), \
             patch("tools.web_tools._is_tool_gateway_ready", return_value=True), \
             patch.dict(os.environ, {"TAVILY_API_KEY": "tvly-test"}):
...
class TestCheckWebApiKey:
    ...
    def test_configured_firecrawl_backend_accepts_managed_gateway(self):
        with patch("tools.web_tools._load_web_config", return_value={"backend": "firecrawl"}):
            with patch("tools.managed_tool_gateway.peek_nastech_access_token", return_value="nastech-token"):
...
class TestNonBuiltinProviderAvailability:
    ...
    def test_check_web_api_key_returns_true_for_custom_provider(self):
        ...
        with patch("tools.web_tools._ddgs_package_importable", return_value=False), \
             patch("tools.managed_tool_gateway.peek_nastech_access_token", return_value=None):
            from tools.web_tools import check_web_api_key
            assert check_web_api_key() is True
...
class TestSiblingProvidersEnvResolution:
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "nastech-agent"
