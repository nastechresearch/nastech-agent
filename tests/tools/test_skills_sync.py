...
class TestCallTimeDirResolution:
    ...
    NASTECH_HOME at import, so a long-lived dashboard/TUI process serving a
    ...
    The accessors must follow set_nastech_home_override() at call time, while
    an explicitly patched module global (tests, _profile_scope retargeting)
    still wins.
    """

    def test_accessors_follow_nastech_home_override(self, tmp_path):
        ...
        try:
            assert ss._nastech_home() == profile_home
    ...
    def test_explicit_module_patch_wins_over_override(self, tmp_path):
        ...
        token = set_nastech_home_override(str(tmp_path / "other-profile"))
        try:
