...
When ``NASTECH_EXEC_ASK`` leaks into a classic CLI process (historically via
...
class TestExecuteCodeGuardCliApprovalSurvivesExecAskLeak:
    ...
    def test_cli_callback_used_for_platform_marker_leak_without_exec_ask(
        ...
        ``NASTECH_SESSION_PLATFORM`` is set, so the whole-script gate is
        reached with ``NASTECH_EXEC_ASK`` entirely absent. That path must
