...
class TestProviderEnvBlocklist:
    ...
    def test_bare_nastech_resolves_from_sanitized_subprocess_path(self):
        """Cron children can resolve Nastech even when the gateway PATH cannot."""
        from tools.environments.local import _sanitize_subprocess_env

        with patch(
            "tools.environments.local._resolve_nastech_bin_dir",
    ...
    def test_bare_nastech_path_does_not_duplicate_existing_install_dir(self):
...
class TestTerminalFirstPartyPlatformEnv:
    ...
    (and the other BUZZ_* vars) are stripped by _NASTECH_PROVIDER_ENV_BLOCKLIST
    ...
    children then get the BUZZ_* vars; execute_code, nastech_subprocess_env,
    ...
    def test_buzz_vars_stay_in_blocklist(self):
        ...
        nastech_subprocess_env, env_passthrough registration)."""
...
class TestTerminalFirstPartySnapshotIsolation:
    ...
    an exclusion, profile A's BUZZ_PRIVATE_KEY lands in nastech-snap-<id>.sh
...
def _physical_repo_root(tmp_path: Path) -> Path:
    ...
    physical_root = tmp_path / "physical-home" / "nastech-agent"
    physical_root.mkdir(parents=True)
    return physical_root


class TestPythonpathSelectiveStrip:
    """PYTHONPATH Nastech-owned entry stripping (#74817).

    The Desktop Electron app injects the Nastech repo root and the Nastech
    ...
    ``_strip_nastech_owned_pythonpath`` surgically removes only the
    entries Nastech itself owns (repo root, own venv site-packages),
    ...
    def test_owned_entries_stripped_matrix(self):
        """Exact Nastech-owned entries are removed; everything else survives
        verbatim (ordering, duplicates, empty components).

        Covers: the running venv's site-packages, the repo root (computed
        independently via parents[2] so an off-by-one in _nastech_repo_root
        cannot silently pass), duplicate Nastech entries, all-owned input
        (PYTHONPATH key removed), and mixed user/Nastech ordering with an
    ...
    def test_non_owned_entries_preserved(self, user_pp):
        """Anything not proven Nastech-owned is preserved byte-for-byte.
    ...
    def test_non_owned_runtime_shaped_entries_preserved(self):
        ...
        different interpreter version, a descendant of the Nastech venv
        ...
        off-by-one in _nastech_repo_root cannot silently pass; no launcher
    ...
    def test_windows_backslash_paths(self):
        """Windows-style backslash paths are handled for Nastech-owned entries.
        ...
        Nastech-owned — the critical invariant is that user Windows paths
        (including site-packages paths for another Python version) are
        never destroyed.  On a real Windows host, Path splits on
        backslashes and Nastech venv site-packages entries are stripped
        by the same Nastech-owned check (covered by the Windows-only test
        below).
        """
        from tools.environments.local_pythonpath import _strip_nastech_owned_pythonpath
        import sys
        ...
        pyver = f"python{sys.version_info[0]}.{sys.version_info[1]}"
        nastech_win = f"C:\\\\Users\\\\u\\\\.nastech\\\\nastech-agent\\\\venv\\\\lib\\\\{pyver}\\\\site-packages"
        user_win = "D:\\\\user\\\\lib"
        env = {
            "PYTHONPATH": ";".join([nastech_win, user_win]),
        }
        ...
        # the Nastech-owned check cannot match a backslash path here.
        assert nastech_win in entries
        assert user_win in entries

    @pytest.mark.windows_only
    def test_windows_nastech_owned_paths_stripped(self):
        """On Windows, a Nastech venv site-packages entry written with
        backslashes is stripped by the same Nastech-owned check, while a
        ...
        _strip_nastech_owned_pythonpath(env)
        entries = env["PYTHONPATH"].split(";")
        assert nastech_win not in entries
        assert user_win in entries
    ...
    def test_base_python_sanitizer_uses_validated_separate_runtime_venv(self, tmp_path, monkeypatch):
        ...
        This deliberately uses a synthetic Nastech venv separate from the test
        runner: sys.prefix represents base Python, while validated VIRTUAL_ENV
        identifies ``<repo>/venv`` as the Nastech runtime producer contract.
        """
        import tools.environments.local as local
        from tools.environments import local_pythonpath

        repo_root = tmp_path / "nastech-agent"
        ...
        monkeypatch.setattr(local, "_nastech_repo_root_aliases", (repo_root,))
        monkeypatch.setattr(local, "_in_venv", False)
        monkeypatch.setattr(local, "_nastech_site_packages", None)
    ...
    def test_unrelated_virtual_env_is_not_runtime_provenance(self, tmp_path, monkeypatch):
        ...
        local_pythonpath._strip_nastech_owned_pythonpath(env)
    ...
    def test_builders_strip_nastech_venv_pythonpath(self, builder):
        """Every subprocess env builder applies the same sanitation contract:
        Nastech venv site-packages is stripped, user entries survive.
        """
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod.nastech_subprocess_env()
    ...
    def test_scrub_child_env_strips_nastech_venv_pythonpath(self):
        """execute_code's _scrub_child_env path: after scrubbing, Nastech venv
        site-packages entries should be stripped when
        _strip_nastech_owned_pythonpath is applied (as the spawn path does),
        ...
        _strip_nastech_owned_pythonpath(scrubbed)
    ...
    def test_execute_code_composition_strips_inherited_nastech_entries(self, same_env):
        """Integration: execute_code's real spawn path composes a clean PYTHONPATH.

        Seeds a contaminated inherited PYTHONPATH (Nastech repo root + Nastech
        ...
        * inherited Nastech venv site-packages never survive into the sandbox;
        ...
        * user entries survive after the controlled entries.
        ...
        nastech_root = str(Path(cet.__file__).resolve().parents[1])
        venv_sp = str(_running_venv_site_packages())
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch("tools.code_execution_env._uses_nastech_python_environment",
                   return_value=same_env), \
             patch("subprocess.Popen", side_effect=_fake_popen), \
             patch.dict(os.environ, {
                 "PYTHONPATH": os.pathsep.join(
                     [nastech_root, venv_sp, user_a, user_b]),
             }):
        ...
        norm_root = os.path.normcase(nastech_root)
        ...
        assert norm_venv not in norm_parts, \
            "inherited Nastech venv site-packages must be stripped"
    ...
    def test_configured_home_alias_matches_launcher_output(self, tmp_path, monkeypatch):
        ...
        import tools.environments.local as local
        from tools.environments import local_pythonpath
        from nastech_cli.gateway_windows import _preserve_nastech_home_path
        ...
        monkeypatch.setenv("NASTECH_HOME", str(configured_home))

        launcher_entry = Path(_preserve_nastech_home_path(physical_root))
        aliases = local_pythonpath._build_nastech_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        )

        assert launcher_entry == configured_home / "nastech-agent"
        assert launcher_entry in aliases

        monkeypatch.setattr(local, "_nastech_repo_root_aliases", aliases)
    ...
    def test_profile_rehome_keeps_junction_lexical_alias(self, tmp_path, monkeypatch):
        ...
        The desktop/CLI spawn children with NASTECH_HOME and PYTHONPATH in the
        configured (junction) spelling, but --profile / sticky active_profile
        re-home NASTECH_HOME through resolve_profile_env() before the
        ...
        physical_home = tmp_path / "physical-home"
        physical_root = physical_home / "nastech-agent"
        physical_root.mkdir(parents=True)
        ...
        monkeypatch.setenv("NASTECH_HOME", str(configured_home))
        lexical_root = configured_home / "nastech-agent"
        ...
        # The sanitizer now runs under the re-homed (profile) NASTECH_HOME.
        aliases = local_pythonpath._build_nastech_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
    ...
    def test_repo_level_junction_recovers_lexical_alias(self, tmp_path, monkeypatch):
        ...
        # repo-level link: <configured-home>/nastech-agent -> physical repo
        try:
            _make_directory_link(configured_home / "nastech-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")
    ...
    def test_same_named_non_owned_directories_preserved(self, tmp_path, monkeypatch):
        ...
        physical_root = _physical_repo_root(tmp_path)
        configured_home = tmp_path / "configured-home"
        (configured_home / "nastech-agent").mkdir(parents=True)
        unrelated = tmp_path / "user-tools" / "nastech-agent"
        ...
        for lookalike in (configured_home / "nastech-agent", unrelated):
    ...
    def test_profile_home_with_repo_level_junction(self, tmp_path, monkeypatch):
        ...
        is <root>/profiles/<name> while the repo is a link at <root>/nastech-agent.
        ...
        try:
            _make_directory_link(configured_root / "nastech-agent", physical_root)
        except OSError as exc:
            pytest.skip(f"directory link unavailable on this host: {exc}")

        configured_home = configured_root / "profiles" / "coder"
        lexical_root = configured_root / "nastech-agent"
        aliases = local_pythonpath._build_nastech_repo_root_aliases(
            physical_root.resolve(),
            physical_root,
            configured_home,
        ...
        assert any(local_pythonpath._same_path(a, lexical_root) for a in aliases)
        assert not any(local_pythonpath._same_path(a, configured_home / "nastech-agent") for a in aliases)
    ...
    def test_validated_runtime_venv_lexical_after_repo_recovery(self, tmp_path, monkeypatch):
        ...
        local._nastech_site_packages = None
...
class TestPythonhomeSanitized:
    """PYTHONHOME must not leak from the Nastech runtime into subprocesses.
    ...
    inherits it redirects its stdlib search to the Nastech venv and crashes
    ...
    def test_builders_strip_pythonhome(self, builder):
        ...
        with patch.dict(os.environ, seed, clear=True):
            if builder == "_make_run_env":
                ...
                result = local_mod._sanitize_subprocess_env(dict(os.environ))
            elif builder == "nastech_subprocess_env":
                result = local_mod.nastech_subprocess_env()
            else:
    ...
    def test_build_subprocess_env_no_scrub_preserves_pythonhome(self):
        ...
        base = {
            ...
            "HOME": "/home/user",
            "PYTHONHOME": "/opt/nastech-venv",
            "VIRTUAL_ENV": "/opt/nastech-venv",
            "SERVICE_TOKEN": "s3cr3t",
        }
        result = build_subprocess_env(base, scrub_secrets=False)
        assert result.get("PYTHONHOME") == "/opt/nastech-venv"
        assert result.get("VIRTUAL_ENV") == "/opt/nastech-venv"
...
class TestNastechInternalDynamicSecrets:
    ...
    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local_env_policy import _is_nastech_internal_secret
