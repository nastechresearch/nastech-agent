...
class TestStaleSyncBackTempCleanup:
    ...
    def test_removes_only_stale_prefixed_entries(self, tmp_path, monkeypatch):
        stale_tar = tmp_path / "nastech-sync-back-stale.tar"
        stale_dir = tmp_path / "nastech-sync-back-stale-staging"
        recent = tmp_path / "nastech-sync-back-recent.tar"
    ...
    def test_sync_back_sweeps_leaked_entry_and_uses_identifiable_tar(self, tmp_path, monkeypatch):
        ...
        leaked = tmp_root / "nastech-sync-back-leaked.tar"
        ...
        def download(dest: Path):
            seen["tar"] = dest
            _make_tar({"root/.nastech/x.txt": b"hi"}, dest)
...
class TestSyncBackSizeCap:
    ...
    def test_sync_back_refuses_oversized_tar(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="tools.environments.file_sync"):
            with patch("nastech_cli.config.load_config", return_value={"terminal": {"sync_back_max_bytes": 1}}):
                mgr.sync_back(nastech_home=tmp_path / ".nastech")
    ...
    def test_cap_override_config_key_raises_the_cap(self, tmp_path, monkeypatch, caplog):
        ...
        host_file = _write_file(tmp_path / "host_skill.md", b"original")
        files = {"root/.nastech/skill.md": b"remote_version"}
        mgr = _make_manager(tmp_path, file_mapping=[(host_file, "/root/.nastech/skill.md")],
                            bulk_download_fn=_make_download_fn(files))

        monkeypatch.setenv("NASTECH_SYNC_BACK_MAX_BYTES", "1")  # the first cut's env var: must be ignored
...
class TestSyncBackWindowsHost:
    ...
    def test_backend_can_reopen_the_tar_path_for_writing(self, tmp_path):
        ...
        def download(dest: Path) -> None:
            ...
            with tarfile.open(fileobj=buf, mode="w") as tar:
                info = tarfile.TarInfo(name="root/.nastech/skill.py")
                info.size = 2
        ...
        mgr = _make_manager(tmp_path, [(str(host_file), "/root/.nastech/skill.py")], bulk_download_fn=download)
        mgr._pushed_hashes["/root/.nastech/skill.py"] = _sha256_bytes(b"v1")
        mgr.sync_back(nastech_home=tmp_path / ".nastech")
    ...
    def test_posix_remote_keys_match_on_windows(self, tmp_path):
        host_file = tmp_path / "host" / "skill.py"
        _write_file(host_file, b"v1")
        mapping = [(str(host_file), "/root/.nastech/skills/a/skill.py")]
        mgr = _make_manager(tmp_path, mapping, bulk_download_fn=_make_download_fn({
            "root/.nastech/skills/a/skill.py": b"v2", "root/.nastech/skills/a/new.md": b"new"}))
        mgr._pushed_hashes["/root/.nastech/skills/a/skill.py"] = _sha256_bytes(b"v1")
        mgr.sync_back(nastech_home=tmp_path / ".nastech")
        assert host_file.read_bytes() == b"v2"  # relpath key was 'root\\.nastech\\...' → skipped
