...
def _flow(flow_id: str):
    ...
    return DashboardOAuthFlow(
        flow_id=flow_id, server_name="asana", profile=None, nastech_home="/tmp/nastech-test",
...
def test_preregistered_pinned_redirect_port_keeps_loopback_listener_under_dashboard_flow(tmp_path, monkeypatch):
    ...
    flow = DashboardOAuthFlow(flow_id="flow-5", server_name="asana", profile=None, nastech_home=str(tmp_path),
    ...
    with dashboard_oauth_flow(flow), force_interactive_oauth():
        assert _configure_callback_port(cfg, NastechTokenStorage("asana")) == port
