...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        NASTECH_YOLO_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --yolo Nastech session (or any shell exporting
        NASTECH_YOLO_MODE=1) freezes True and every guard auto-approves.
