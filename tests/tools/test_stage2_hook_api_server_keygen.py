...
was gated on ``[ -f "$NASTECH_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'NASTECH_HOME="{home}"\n'
        # In tests we run unprivileged; as_nastech is a passthrough then.
        'as_nastech() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Nastech loads $NASTECH_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/nastech/.env.example -> $NASTECH_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $NASTECH_HOME/.env from it on first boot"
    )
