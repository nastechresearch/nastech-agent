...
def test_local_delivery_command_and_ack(tmp_path, monkeypatch):
    ...
    assert command.startswith("nastech -p researcher chat --in ~ -c \"Bot Chat\"")
...
def test_peer_delivery_command(tmp_path, monkeypatch):
    ...
    assert command.startswith("nastech peer dm spark/researcher < ")
    ...
    assert calls[1]["command"].startswith("nastech peer dm spark < ")
