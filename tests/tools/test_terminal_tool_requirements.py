...
class TestTerminalRequirements:
    ...
    def test_terminal_and_execute_code_tools_resolve_for_managed_modal(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("tools.terminal_tool_backends.managed_nastech_tools_enabled", lambda: True)
...
class TestCheckFnTransientFailureSuppression:
    ...
    def test_core_tool_drop_after_success_warns_once_never_configured_stays_info(self, monkeypatch, caplog):
        """A check_fn that drops a ``_NASTECH_CORE_TOOLS`` member it had previously admitted logs at
        ...
        clock = {"now": 1000.0}
        monkeypatch.setattr(reg.time, "monotonic", lambda: clock["now"])
        monkeypatch.setattr(toolsets, "_NASTECH_CORE_TOOLS",
                            [*toolsets._NASTECH_CORE_TOOLS, "core_probe_tool", "never_probe_tool"])
