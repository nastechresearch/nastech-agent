...
def test_read_nastech_provider_state_reads_only_the_profiles_own_store(tmp_path, monkeypatch):
    # Every profile owns its credentials (#111724): a named profile with an empty auth.json has
    # no Nastech identity, even when the root is signed in; its own login is what the gate sees.
    root = tmp_path / ".nastech"
    profile = root / "profiles" / "nastech-setup"
    profile.mkdir(parents=True)
    (root / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"nastech": {"auth_method": "anonymous", "access_token": "root-tok"}},
    }))
    ...
    monkeypatch.setattr(auth_mod, "get_nastech_home", lambda: profile)
    monkeypatch.setattr(auth_mod, "_auth_file_path", lambda: profile / "auth.json")

    assert managed_tool_gateway._read_nastech_provider_state() is None

    (profile / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"nastech": {"auth_method": "anonymous", "access_token": "profile-tok"}},
    }))
    state = managed_tool_gateway._read_nastech_provider_state()
