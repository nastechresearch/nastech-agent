"""Regression tests for the stage2 Nastech routing-override sync.

Hosted deploys carry ``NASTECH_PORTAL_BASE_URL`` / ``NASTECH_INFERENCE_BASE_URL`` only in the container
...
def _sync_block(text: str) -> str:
    start = text.index("# --- Sync deploy-injected Nastech routing overrides")
...
def test_container_values_reach_home_and_every_profile_env(stage2_text: str, tmp_path: Path) -> None:
    """All three overrides land in $NASTECH_HOME/.env and each profiles/*/.env (created 0600 when
    ...
    result = _run_sync(stage2_text, home, {"NASTECH_PORTAL_BASE_URL": PORTAL, "NASTECH_PORTAL_BASE_URL": PORTAL,
                                           "NASTECH_INFERENCE_BASE_URL": INFERENCE})

    assert result.returncode == 0, result.stderr
    for env_file in (home / ".env", home / "profiles" / "work" / ".env", home / "profiles" / "ops" / ".env"):
        parsed = load_env_file(env_file)
        assert parsed["NASTECH_PORTAL_BASE_URL"] == parsed["NASTECH_PORTAL_BASE_URL"] == PORTAL, env_file
        assert parsed["NASTECH_INFERENCE_BASE_URL"] == INFERENCE, env_file
        assert len(_assignments(env_file, "NASTECH_PORTAL_BASE_URL")) == 1, env_file
...
def test_container_wins_over_stale_line_then_idempotent_then_removed_when_unset(stage2_text: str, tmp_path: Path) -> None:
    ...
    assert load_env_file(env_file)["NASTECH_PORTAL_BASE_URL"] == PORTAL
    assert len(_assignments(env_file, "NASTECH_PORTAL_BASE_URL")) == 1
    assert "Synced NASTECH_PORTAL_BASE_URL" in first.stdout
    ...
    second = _run_sync(stage2_text, home, {"NASTECH_PORTAL_BASE_URL": PORTAL})
    ...
    with env_file.open("a") as fh:
        fh.write("NASTECH_PORTAL_BASE_URL=https://by-hand\n")  # operator line beside the managed one
    third = _run_sync(stage2_text, home, {})
    assert third.returncode == 0, third.stderr
    parsed = load_env_file(env_file)
    assert _assignments(env_file, "NASTECH_PORTAL_BASE_URL") == ["NASTECH_PORTAL_BASE_URL=https://by-hand"], \
        "only the stage2-written line goes when the platform unsets it"
    assert parsed["NASTECH_INFERENCE_BASE_URL"] == "https://by-hand/v1", "a hand-set line is never touched"
    assert parsed["OTHER"] == "1"
    assert "Removed NASTECH_PORTAL_BASE_URL" in third.stdout

    if os.geteuid() != 0:
        env_file.chmod(0o444)
        frozen = env_file.read_text()
        readonly = _run_sync(stage2_text, home, {"NASTECH_PORTAL_BASE_URL": PORTAL})
        env_file.chmod(0o600)
        assert readonly.returncode == 0, "a read-only volume degrades to a warning under set -e"
        assert "could not write NASTECH_PORTAL_BASE_URL" in readonly.stdout
    ...
    fourth = _run_sync(stage2_text, home, {"NASTECH_PORTAL_BASE_URL": PORTAL})
