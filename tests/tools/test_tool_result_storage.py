...
class TestWriteToSandbox:
    ...
    def test_size_probe_decides_lossless(self, stdin_mode, probed, ok):
        ...
        assert _write_to_sandbox(content, "/tmp/nastech-results/p.txt", env) is ok
        if not ok:
            rm_cmd = env.execute.call_args_list[2][0][0]
            assert rm_cmd.startswith("rm -f ") and "/tmp/nastech-results/p.txt" in rm_cmd
        else:
    ...
    def test_unprobeable_backend_is_best_effort_success(self):
        ...
        assert _write_to_sandbox("data", "/tmp/nastech-results/np.txt", env) is True
...
# ── Host-side spillover ($NASTECH_HOME/cache/spillover) ────────────────
...
class TestSpillover:
    ...
    def test_remote_env_probe_failure_falls_back_to_sandbox_write(self):
        ...
        assert PERSISTED_OUTPUT_TAG in result
        assert "/tmp/nastech-results/tc_remote_2.txt" in result
...
class TestRecoveryHint:
    def test_preview_teaches_recovery_not_refetch(self):
        msg = _build_persisted_message(
            ...
            file_path="/tmp/nastech-results/r.txt",
        )
