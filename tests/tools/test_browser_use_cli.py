...
* Mode detection — ``browser.backend: browser-use`` in config (set via the
  ``nastech tools`` picker); off by default.
...
class TestModeDetection:
    def test_default_on_when_cli_available(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.config.read_raw_config", lambda: {})
    ...
    def test_config_read_failure_uses_default(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.config.read_raw_config", boom)
        monkeypatch.setattr(bu_cli, "_find_cli", lambda: None)
        assert bu_cli.is_browser_use_cli_mode() is False
...
class TestSubprocessEnvironment:
    ...
    def test_subprocess_env_strips_parent_python_import_paths(self, monkeypatch):
        ...
        own Python — inherited PYTHONPATH/PYTHONHOME pointing at Nastech's
        ...
        browser_tool = ModuleType("tools.browser_tool")
        browser_tool._build_browser_env = lambda: {
            "PYTHONPATH": "/nastech:/nastech/venv/lib/site-packages",
            "PYTHONHOME": "/nastech/venv",
            "KEEP_ME": "yes",
        }
        monkeypatch.setitem(sys.modules, "tools.browser_tool", browser_tool)
...
class TestToolSurfaceSwap:
    ...
    def test_browser_exec_in_browser_toolsets(self):
        ...
        assert "browser_exec" in _NASTECH_CORE_TOOLS
...
class TestVaultSupervisorAttach:
    def test_exec_attaches_supervisor_to_the_browser_it_drives(self, tmp_path, monkeypatch, _fake_supervisor_registry):
        ...
        monkeypatch.setattr("nastech_cli.config.read_raw_config", lambda: {"browser": {"backend": "browser-use"}})
...
class TestLegacyCloudMigration:
    ...
    Nastech-gateway users stay on the legacy provider path."""
    ...
    def test_picker_highlights_cli_row_for_migrated_config(self, monkeypatch):
        from nastech_cli.tools_config import TOOL_CATEGORIES, _is_provider_active
...
class TestBackendCdpResolution:
    ...
    def test_picker_managed_selection_resolves_gateway_provider(self, monkeypatch):
        """``cloud_provider: nastech`` (the `nastech tools` managed row) must resolve through the
        ...
        monkeypatch.setattr(bu_cli, "_read_browser_cfg", lambda: {"cloud_provider": "nastech"})
        env = {}
        assert bu_cli._resolve_backend_cdp(env, "t1") is None
...
class TestOwnTabPreamble:
    ...
    def test_named_shared_browser_gets_preamble(self, tmp_path, monkeypatch):
        ...
        assert result["success"] is True
        assert "_nastech_ensure_own_tab" in result["output"]
        # model code still present, after the preamble
        assert result["output"].index("_nastech_ensure_own_tab") < result["output"].index("print('payload')")
    ...
    def test_named_packaged_chromium_skips_preamble(self, tmp_path, monkeypatch):
        ...
        assert result["success"] is True
        assert "_nastech_ensure_own_tab" not in result["output"]
    ...
    def test_sentinel_never_reaches_subprocess_env(self, tmp_path, monkeypatch):
        ...
        cli = _fake_cli(tmp_path, 'cat > /dev/null\necho "sentinel:${_NASTECH_BU_PRIVATE_BROWSER:-unset}"\n')
        monkeypatch.setattr(bu_cli, "_find_cli", lambda: [cli])
        result = json.loads(bu_cli.browser_exec("print(1)", session="r7k2"))
...
class TestProviderPickerIntegration:
    ...
    def test_picker_row_names_stay_unique(self):
        ...
        from nastech_cli.tools_config import TOOL_CATEGORIES, _plugin_browser_providers
...
class TestFindCliManagedBin:
    """MANAGED-FIRST: _find_cli probes $NASTECH_HOME/bin before PATH and
    ~/.local/bin, so the Nastech-installed copy always wins."""
    ...
    def test_managed_bin_precedes_user_local_bin(self, tmp_path, monkeypatch):
        """MANAGED-FIRST: Nastech' managed copy wins over a user-level side
...
class TestInstallCli:
    def test_path_install_does_not_short_circuit(self, tmp_path, monkeypatch):
        ...
        otherwise resolution stays pinned to a binary Nastech can't update."""
        ...
        fake = _types.ModuleType("nastech_cli.managed_uv")
        fake.ensure_uv = lambda **kw: None
        monkeypatch.setitem(_sys.modules, "nastech_cli.managed_uv", fake)
...
class TestDefaultDowngradeNotice:
    ...
    def test_notice_when_default_and_cli_missing(self, tmp_path, monkeypatch):
        ...
        assert "nastech tools" in notice
...
class TestLightpandaBackendResolution:
    """browser.engine: lightpanda in Browser Use mode — Nastech spawns
...
class TestLightpandaStatusLine:
    ...
    def test_status_reports_lightpanda_in_use(self, monkeypatch):
        out = self._status(monkeypatch, used=True, reason="Browser Use mode: Nastech spawns `lightpanda serve` per session")
