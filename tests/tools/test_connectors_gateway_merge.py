...
def test_account_gate_reads_the_portal_claim_not_entitlement(monkeypatch, claims, rolled_out):
    ...
    monkeypatch.setattr(
        "nastech_cli.auth._decode_jwt_claims", lambda token: {"exp": time.time() + 3600, **claims})
    account = nastech_account._info_from_valid_jwt("tok", {}, None, 60)
    assert account is not None and account.logged_in

    monkeypatch.setattr(nastech_account, "get_nastech_portal_account_info", lambda **kw: account)
