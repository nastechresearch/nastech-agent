...
class TestPermanentAllowlistReload:
    ...
    def test_load_permanent_allowlist_clears_when_config_is_empty(self):
        with mock_patch.object(approval_module, "_permanent_approved", {"stale-pattern"}):
            with mock_patch("nastech_cli.config.load_config_readonly", return_value={"command_allowlist": []}):
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_SESSION_PLATFORM", "webhook")
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("NASTECH_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("NASTECH_INTERACTIVE", raising=False)
        monkeypatch.setenv("NASTECH_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("NASTECH_SESSION_KEY", "test-api-session")
...
class TestLaunchctlGatewayLifecycle:
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a nastech-gateway identifier, so quoted prose and non-gateway nastech
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "nastech"/"ai.nastech" to appear AFTER the verb and missed this
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.nastech.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.nastech.gateway.plist",
    "launchctl stop ai.nastech.gateway",
    "launchctl restart ai.nastech.gateway",
    "launchctl bootout gui/501/ai.nastech.gateway",
    "launchctl remove ai.nastech.gateway",
    "launchctl disable gui/501/ai.nastech.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_NASTECH_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Nastech ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.nastech.update-checker",
