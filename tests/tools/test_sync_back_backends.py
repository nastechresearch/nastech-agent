...
class TestSSHBulkDownload:
    ...
    def test_ssh_bulk_download_tolerates_only_socket_ignored_exit_2(self, ssh_mock_env, tmp_path):
        ...
        dest = tmp_path / "backup.tar"
        stderr = b"tar: home/testuser/.nastech/gateway.sock: socket ignored\n"
    ...
    def test_ssh_bulk_download_still_fails_on_every_other_status(self, ssh_mock_env, tmp_path):
        ...
        dest = tmp_path / "backup.tar"
        failures = (
            subprocess.CompletedProcess([], 1, stderr=b"tar: home/testuser/.nastech/state.db: file changed as we read it"),
            subprocess.CompletedProcess([], 2, stderr=(b"tar: home/testuser/.nastech/gateway.sock: socket ignored\n"
                                                      b"tar: home/testuser/.nastech/state.db: Cannot open: Permission denied\n")),
