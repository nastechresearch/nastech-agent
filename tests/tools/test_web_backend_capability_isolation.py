...
def test_ladder_derived_firecrawl_takes_managed_path_on_gateway_ready_install(monkeypatch):
    ...
    _ladder_env(monkeypatch, {"extract_backend": "keenable"})
    gw = SimpleNamespace(nastech_user_token="tok", gateway_origin="https://gw.example")
    ...
    # Broken before the fix: ValueError "web is configured to use firecrawl (set via nastech tools)".
