...
def test_windows_managed_node_root_prefers_cmd_launchers(tmp_path):
    """Managed Windows Node lives directly in ``<NASTECH_HOME>/node`` (no ``bin``) as ``npx.cmd`` /
...
def test_node_fallback_uses_active_profile_home(tmp_path, monkeypatch):
    """The managed-Node lookup follows ``get_nastech_home()`` (context override), not raw ``NASTECH_HOME``:
    ...
    npx_path.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
    npx_path.chmod(0o755)
    monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "launch-home"))
