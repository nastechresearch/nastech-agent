...
class TestSensitivePathCheck:
    ...
    def test_nastech_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("tools.file_tools_write_guards._nastech_config_resolved", str(fake_config))
        monkeypatch.setattr("tools.file_tools_write_guards._nastech_config_resolved_loaded", True)
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("tools.file_tools_write_guards._nastech_config_resolved", "/some/other/path")
        monkeypatch.setattr("tools.file_tools_write_guards._nastech_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("tools.file_tools_write_guards._nastech_config_resolved", "/home/user/.nastech/config.yaml")
        monkeypatch.setattr("tools.file_tools_write_guards._nastech_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()
...
class TestSecretFileReadRedaction:
    ...
    def nastech_home(self, tmp_path, monkeypatch):
        ...
        home.mkdir()
        monkeypatch.setattr(file_safety, "_nastech_home_path", lambda: home)
        monkeypatch.setattr(file_safety, "_nastech_root_path", lambda: home)
        return home
    ...
    @patch("tools.file_tools._get_file_ops")
    def test_read_file_of_nastech_config_masks_opaque_token(self, mock_get, nastech_home):
        ...
        from tools.file_tools import read_file_tool
        out = json.loads(read_file_tool(str(nastech_home / "config.yaml"), task_id="secret-read"))
        ...
        out = json.loads(read_file_tool(str(nastech_home.parent / "proj-config.yaml"), task_id="plain-read"))
        assert self.SYNTH in out["content"]

    @patch("tools.file_tools._get_file_ops")
    def test_search_in_nastech_home_masks_opaque_token(self, mock_get, nastech_home):
        config = nastech_home / "config.yaml"
        ops = MagicMock()
        ...
        from tools.file_tools import search_tool
        raw = search_tool(pattern="ADS_API_TOKEN", path=str(nastech_home),
