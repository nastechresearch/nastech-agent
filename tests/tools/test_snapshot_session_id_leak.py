...
run stamps per command (NASTECH_DELEGATED_CHILD_CONTEXT, NASTECH_CRON_SESSION), or
...
def test_export_snippet_shape():
    ...
    assert "${!NASTECH_BROWSER_CONTROL_*}" in snippet
...
def test_export_dump_drops_every_bridged_var_and_the_delegation_marker():
    ...
    exports = "; ".join([f'export {n}="x"' for n in scoped] + ['export NASTECH_HOME="/h"', 'export MYVAR="keep"'])
    out = subprocess.run(
    ...
    assert 'declare -x NASTECH_HOME="/h"' in out
