...
class TestCreateSession:
    ...
    def test_make_agent_prefers_passed_toolsets_over_config_servers(self, monkeypatch):
        ...
        monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {})
        monkeypatch.setattr("nastech_cli.mcp_startup.ensure_mcp_discovery_before_agent_build", lambda **_kw: None)
        monkeypatch.setattr("acp_adapter.session._register_task_cwd", lambda task_id, cwd: None)
        manager = SessionManager(db=None)

        manager._make_agent(session_id="fresh", cwd=".")
        manager._make_agent(
            session_id="rebuilt", cwd=".", enabled_toolsets=["nastech-acp", "mcp-acp-server"], disabled_toolsets=["browser"],
        )

        assert (seen[0]["enabled_toolsets"], seen[0]["disabled_toolsets"]) == (["nastech-acp", "mcp-cfg-server"], None)
        assert (seen[1]["enabled_toolsets"], seen[1]["disabled_toolsets"]) == (["nastech-acp", "mcp-acp-server"], ["browser"])
...
class TestSymlinkAliasNormalization:
    ...
    def test_missing_path_keeps_lexical_normalization(self):
        ...
        assert acp_session._normalize_cwd_for_compare(
            "/nonexistent-nastech-test/x/../y"
        ) == "/nonexistent-nastech-test/y"
