...
Driven through the real ``NastechACPAgent.prompt()`` with a real ``AIAgent`` and ``SessionDB``
...
def acp(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("NASTECH_DISABLE_PLUGINS", "1")
    ...
    from acp_adapter.server import NastechACPAgent, TextContentBlock
    from acp_adapter.session import SessionManager
    from nastech_state import SessionDB

    db_path = tmp_path / "state.db"
    db = SessionDB(db_path)
    server = NastechACPAgent(session_manager=SessionManager(db=db))
    conn = _RecordingConn()
...
def test_acp_refusal_closes_the_turn_and_is_not_replayed_into_the_next_prompt(acp):
    ...
    Invariant: the durable tail after a failed turn is a Nastech-authored assistant row (never
