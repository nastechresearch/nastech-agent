...
def _runner(monkeypatch, *, extra=None):
    ...
    monkeypatch.setattr(anon_auth, "current_nastech_state", lambda: None)
    return runner
...
async def test_already_signed_in_starts_no_task(monkeypatch):
    runner = _runner(monkeypatch)
    monkeypatch.setattr(anon_auth, "current_nastech_state", lambda: {"auth_method": "oauth"})
    flow = MagicMock()
    monkeypatch.setattr(anon_auth, "run_sign_in", flow)
...
async def test_a_completion_evicts_welcome_and_clears_its_override(monkeypatch):
    runner = _runner(monkeypatch)
    agent = SimpleNamespace(provider="nastech", model=anon_auth.GUEST_MODEL)
...
def _durable_sweep_runner(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(nastech_state, "SessionDB", no_sqlite)
    runner = _runner(monkeypatch)
    ...
    runner._agent_cache[key] = (
        SimpleNamespace(provider="nastech", model=anon_auth.GUEST_MODEL), "signature")
...
async def test_the_sweep_leaves_sessions_on_other_models_alone(monkeypatch):
    runner = _runner(monkeypatch)
    runner._agent_cache = {
        "k1": (SimpleNamespace(provider="nastech", model=anon_auth.GUEST_MODEL), "signature"),
