...
class TestIsIntentionalModelSwitch:
    ...
    default for a reason the system produced: a /model override, or the Nastech gateway moving the
    session off the ``nastech/welcome`` alias the config still carries."""
    ...
    def test_server_model_switch_off_the_welcome_alias_is_intentional(self):
        ...
        agent = SimpleNamespace(model="z-ai/glm-5.3-flash", _nastech_model_switch=("nastech/welcome", "z-ai/glm-5.3-flash"))
        assert runner._is_intentional_model_switch(sk, agent, "nastech/welcome") is True
        ...
        assert runner._is_intentional_model_switch(sk, agent, "nastech/welcome") is False
