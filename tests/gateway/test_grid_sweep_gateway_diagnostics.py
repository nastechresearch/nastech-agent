"""Code-flow grid sweep (NASTECH-368, wave 1): three pre-existing automatic gateway diagnostics that
...
async def test_shutdown_notice_to_active_chats_and_home_channel_honors_policy(tmp_path, monkeypatch, setting):
    ...
    if setting is True:
        ...
        assert all("Nastech is shutting down" in m for _c, m, _meta in adapter.sent_calls)
...
async def test_in_chat_restart_ack_to_requester_is_never_suppressed(tmp_path, monkeypatch):
    ...
    assert chat_id == source.chat_id and "Nastech is restarting" in message
