...
def test_real_retry_producers_keep_final_failure_and_persistence(tmp_path, monkeypatch, caplog, enabled):
    ...
    monkeypatch.setattr(run, "_nastech_home", tmp_path)
    sent = []
    ...
    sent.clear()
    with patch("agent.nastech_rate_guard.nastech_rate_limit_remaining", return_value=60), \
         patch("nastech_cli.anon_auth.apply_model_switch"), \
         patch("nastech_cli.anon_auth.route_is_welcome_host", return_value=False):
        verdict = nastech_rate_limit_guard(agent, _retry=SimpleNamespace(), api_messages=[], messages=[],
