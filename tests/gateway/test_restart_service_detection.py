...
def test_supervised_child_marker_is_a_launch_not_a_restart_route():
    """The Windows Scheduled-Task launcher exports only ``NASTECH_SUPERVISED_CHILD``: that must make
    ...
    from nastech_cli.gateway_windows import _GATEWAY_ENV

    task_env = dict(_GATEWAY_ENV)
    assert task_env["NASTECH_SUPERVISED_CHILD"] == "1"
