...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_nastech_home(self):
        config = GatewayConfig()
        source = SessionSource(
            platform=Platform.LOCAL, chat_id="cli",
            chat_name="CLI terminal", chat_type="dm",
        ...
        with patch("nastech_constants.display_nastech_home", return_value="~/.nastech/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.nastech/profiles/coder/cron/output/" in prompt
