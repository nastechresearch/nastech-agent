...
class TestScopedGatewayPidQuery:
    ...
    def _write_scoped_profile(self, tmp_path):
        ...
        record = {
            "pid": 4242,
            "kind": "nastech-gateway",
            "argv": ["python", "-m", "nastech_cli.main", "gateway", "--profile", "wiki"],
            "start_time": 123,
            "nastech_home": str(profile_dir.resolve()),
        }
    ...
    def test_scoped_query_reports_live_foreign_profile_pid(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "default-home"))
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "python -m nastech_cli.main gateway --profile wiki",
        )
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_stamps_profile_label(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "profiles" / "lead-gen-outreach"))
    ...
    def test_acquire_scoped_lock_omits_profile_when_not_inferable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("NASTECH_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
        monkeypatch.setenv("NASTECH_HOME", str(tmp_path / "some-custom-dir"))
...
class TestScopedLockOwnerLabel:
    ...
    def test_profile_label_for_root_home_is_default(self, monkeypatch):
        monkeypatch.setenv("NASTECH_HOME", "/opt/data")
    ...
    def test_owner_label_prefers_explicit_profile_field(self):
        record = {"profile": "coder", "nastech_home": "/opt/data/profiles/other"}
    ...
    def test_owner_label_ignores_invalid_profile_and_uses_home(self):
        ...
        # nastech_home fallback — attribution degrades, never corrupts.
        record = {
            "profile": "bad; rm -rf /",
            "nastech_home": "/opt/data/profiles/zerocool",
        }
        assert status.scoped_lock_owner_label(record) == "zerocool"

    def test_owner_label_falls_back_to_nastech_home(self):
        # Locks written before the profile field existed still attribute.
        record = {"pid": 559, "nastech_home": "/opt/data/profiles/zerocool"}
        assert status.scoped_lock_owner_label(record) == "zerocool"
...
def test_strict_gateway_identity_rejects_reused_pid(tmp_path, monkeypatch):
    pid_path = tmp_path / "gateway.pid"
    lock_path = tmp_path / "gateway.lock"
    record = {"pid": 123, "start_time": 10.0, "kind": "nastech-gateway"}
...
def test_retained_gateway_state_keeps_watchdog_degraded_like_startup_failed():
    ...
    ``nastech gateway stop`` records the intent. The startup-time ``degraded`` (retryable platforms, no
