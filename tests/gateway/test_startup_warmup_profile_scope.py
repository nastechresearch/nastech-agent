...
``get_tool_definitions`` runs every ``check_fn``; the vision probe resolves Nastech runtime
...
def multiplex_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    home.mkdir()
    (home / ".env").write_text(f"NASTECH_PORTAL_BASE_URL={PORTAL}\n")
...
def _probe(seen: dict):
    def probe() -> int:
        ...
        seen["portal_override"] = _nastech_portal_env_override()
        return 1
    return probe
...
def _run_warmup(monkeypatch, *, multiplex: bool) -> dict:
    ...
    async def drive() -> None:
        ...
        seen["home_override_after"] = get_nastech_home_override()
