...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/NastechResearch/nastech-agent/issues/87051:
    ...
    response in a different chat lane than the Nastech topic the session runs
    in. The no-anchor branch must prefer the Nastech topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_nastech_topic_thread_id(self):
