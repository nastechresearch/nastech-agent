...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /nastech, /btw, /stop,
        ...
        for expected in ("/nastech", "/btw", "/stop", "/model", "/help"):
...
class TestSlackConnectCleanup:
    ...
    async def test_disconnect_closes_workspace_clients_and_clears_runtime_state(self):
        ...
        ``nastech gateway run --replace`` takes the old process through the
...
class TestSlackProxyBehavior:
    ...
    async def test_connect_uses_proxy_when_not_bypassed(self):
        ...
        assert "nastech_feedback" in created_apps[0].registered_actions
        assert "nastech_clarify_other" in created_apps[0].registered_actions
        ...
        assert any(
            pattern.fullmatch("nastech_clarify_choice_0")
            for pattern in clarify_choice_patterns
        )
        assert not any(
            pattern.fullmatch("nastech_clarify_choice")
            for pattern in clarify_choice_patterns
        )
    ...
    async def test_channel_delivery_honors_unfurl_config(self):
        ...
        with patch.object(_slack_mod.aiohttp, "ClientSession", return_value=session):
            result = await _slack_mod._standalone_send(
                config,
                "C123",
                "[Nastech](https://example.com/nastech)",
            )

        assert result["success"] is True
        payload = session.post.call_args.kwargs["json"]
        assert payload["text"] == "<https://example.com/nastech|Nastech>"
...
class TestSlashCommands:
    ...
    # instead of as /nastech subcommands. This is the Discord/Telegram parity
    ...
    @pytest.mark.asyncio
    async def test_legacy_nastech_prefix_still_works(self, adapter):
        """Backward compat: /nastech btw foo must still route to /btw foo.

        Old workspace manifests only declared /nastech as the single slash.
...
class TestMessageSplitting:
    ...
    async def test_send_coerces_string_unfurl_options(self, adapter):
        """`nastech config set` / Railway persist YAML booleans as strings.
...
class TestMissingCredentials:
    ...
    async def test_missing_app_token_sets_fatal_error(self):
        ...
        assert "nastech gateway setup" in fatal_errors[0]["message"].lower() or ".env" in fatal_errors[0]["message"]
...
class TestTrackingStructureBounds:
    ...
    async def test_slash_command_contexts_bounded(self, adapter):
        ...
        adapter.handle_nastech_command = AsyncMock(return_value=None)
        for i in range(10):
            command = {
                "command": "/nastech",
                "text": "/status",
...
class TestThreadImageContext:
    ...
    def _prep(self, adapter_with_session_store):
        ...
        a._download_slack_file = AsyncMock(return_value="/tmp/nastech-cached.png")
        return a
    ...
    async def test_cold_start_delivers_thread_root_image(
        ...
        assert msg_event.media_urls == ["/tmp/nastech-cached.png"]
...
class TestSlackUserAgent:
    ...
    ``user_agent_prefix=_NASTECH_SLACK_USER_AGENT_PREFIX`` on every
    ...
    def test_nastech_slack_user_agent_prefix_format(self):
        """Module constant matches the NastechAgent/<version> convention used
        elsewhere in the codebase for platform-partner attribution."""
        assert _slack_mod._NASTECH_SLACK_USER_AGENT_PREFIX.startswith("NastechAgent/")
