...
class TestStoredSessionModelFilter:
    ...
    "no stored model" — replaying "nastech-agent" upstream 400s. Found live
    (Aug 2026): the first cross-gateway `nastech peer dm` against a fresh
    api_server failed every turn with "nastech-agent is not a valid model ID".
    """
...
class TestSessionDbOffEventLoop:
    ...
    async def test_create_session_without_model_does_not_persist_virtual_alias(self, auth_adapter):
        ...
        virtual model alias (self._model_name, e.g. "nastech-agent") as if it
        ...
        "nastech-agent" as an invalid model identifier on every turn.
        """
        app = _create_app(auth_adapter)
    ...
    async def test_create_session_with_explicit_virtual_alias_does_not_persist_it(self, auth_adapter):
        """Sending ``model: "nastech-agent"`` explicitly (the virtual alias
    ...
    async def test_create_session_with_provider_prefixed_virtual_alias_does_not_persist_it(self, auth_adapter):
        ...
        "openrouter::nastech-agent" never matched self._model_name and leaked
...
class TestCreateAgentModelRecovery:
    ...
    def test_create_agent_does_not_cache_virtual_alias(self, monkeypatch):
        """Write-side guard: the advertised virtual model (``nastech-agent``)
