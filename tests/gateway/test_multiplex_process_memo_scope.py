...
(NASTECH_HOME override), not the launch home the module constants were frozen from."""
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (NASTECH_HOME) and a routed profile B with different config everywhere."""
    a = tmp_path / ".nastech"
    b = a / "profiles" / "b"
...
def test_max_turns_and_fallback_chain_follow_routed_profile(two_homes, monkeypatch):
    ...
    monkeypatch.setattr(gateway_run, "_nastech_home", a)
...
def test_aux_nastech_auth_reads_routed_profile_auth_json(two_homes, monkeypatch):
    a, b = two_homes
    import agent.auxiliary_client as aux

    for home, token in ((a, "TOKEN_A"), (b, "TOKEN_B")):
        (home / "auth.json").write_text(json.dumps({
            "version": 1, "active_provider": "nastech",
            "providers": {"nastech": {"agent_key": token, "access_token": token}},
        }), encoding="utf-8")
    monkeypatch.setattr(aux, "_select_pool_entry", lambda _provider: (False, None))

    assert (aux._read_nastech_auth() or {}).get("access_token") == "TOKEN_A"
    assert (_under(b, aux._read_nastech_auth) or {}).get("access_token") == "TOKEN_B"
...
async def test_gateway_hooks_fire_per_routed_profile(two_homes):
    ...
    token = set_nastech_home_override(str(b))
    try:
        ctx_b: dict = {}
        ...
        reset_nastech_home_override(token)
    assert ctx_a.get("seen") == ["nastech_hook_hook-a"]
    assert ctx_b.get("seen") == ["nastech_hook_hook-b"]
...
def test_media_policy_reads_routed_profile_config_not_env(two_homes, monkeypatch):
    ...
    for var in ("NASTECH_MEDIA_DELIVERY_STRICT", "NASTECH_MEDIA_ALLOW_DIRS"):
