...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_session_stores_but_not_neighbouring_artifacts(self, tmp_path, monkeypatch):
        ...
        pasted into a chat; ``MEDIA:~/.nastech/state.db`` must not exfiltrate them (#41071). Named
        ...
        monkeypatch.setattr("gateway.platforms.base._NASTECH_HOME", nastech_dir)
        monkeypatch.setattr("gateway.platforms.base._NASTECH_ROOT", nastech_dir)
        board = nastech_dir / "kanban" / "boards" / "team-a"
        ...
        for rel in denied + allowed:
            path = nastech_dir / rel
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_bytes(b"SQLite format 3\x00")
        assert [rel for rel in denied if BasePlatformAdapter.validate_media_delivery_path(str(nastech_dir / rel))] == []
        assert [rel for rel in allowed if not BasePlatformAdapter.validate_media_delivery_path(str(nastech_dir / rel))] == []
    ...
    def test_denylist_covers_every_profile_home_not_just_the_launch_home(self, tmp_path, monkeypatch):
        ...
        turn is the launch (default) profile's or the secondary's own (NASTECH_HOME override), while
        ...
        self._patch_roots(monkeypatch)
        fake_home = tmp_path / "home"
        nastech_root = fake_home / ".nastech"
        profile_b = nastech_root / "profiles" / "beta"
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._NASTECH_HOME", nastech_root)
        monkeypatch.setattr("gateway.platforms.base._NASTECH_ROOT", nastech_root)
        ...
        for scope in (None, profile_b):  # default profile's turn, then beta's own turn
            token = set_nastech_home_override(scope)
            try:
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.nastech exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".nastech"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.nastech/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
