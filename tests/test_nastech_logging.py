...
class TestSetupLogging:
    ...
    def test_profile_routing_follows_context_home(self, nastech_home, tmp_path):
        ...
        profile_home.mkdir()
        nastech_logging.setup_logging(nastech_home=nastech_home)
        assert nastech_logging.enable_profile_log_routing(
            [nastech_home, profile_home]
        ) is True
        ...
        assert "profile-routed cron record" not in (
            nastech_home / "logs" / "agent.log"
        ).read_text()

    def test_release_profile_log_handlers_closes_only_deleted_profile(self, nastech_home, tmp_path):
        ...
        nastech_logging.setup_logging(nastech_home=nastech_home)
        assert nastech_logging.enable_profile_log_routing(
            [nastech_home, deleted_home, other_home]
        ) is True

        logger = logging.getLogger("agent.profile-delete-log-release")
        token = set_nastech_home_override(deleted_home)
        try:
            logger.warning("deleted profile log handles")
        finally:
            reset_nastech_home_override(token)
        token = set_nastech_home_override(other_home)
        try:
        ...
        routers = [
            handler for handler in nastech_logging._queued_file_handlers
            if isinstance(handler, nastech_logging._ProfileRoutingFileHandler)
        ]
        ...
        assert nastech_logging.release_profile_log_handlers(deleted_home) == 2
    ...
    def test_a_second_home_routes_instead_of_stacking_an_unfiltered_handler(self, nastech_home, tmp_path):
        ...
        profile_home.mkdir()
        nastech_logging.setup_logging(nastech_home=nastech_home)
        nastech_logging.setup_logging(nastech_home=profile_home)

        assert not [h for h in nastech_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)], (
        ...
        a_log = (nastech_home / "logs" / "agent.log").read_text()
    ...
    def test_setup_for_an_already_routed_home_adds_no_duplicate_writer(self, nastech_home, tmp_path):
        ...
        profile_home.mkdir()
        nastech_logging.setup_logging(nastech_home=nastech_home)
        assert nastech_logging.enable_profile_log_routing([nastech_home, profile_home]) is True
        nastech_logging.setup_logging(nastech_home=profile_home)
        ...
        assert "once please" not in (nastech_home / "logs" / "agent.log").read_text()

    def test_a_component_log_added_after_routing_is_routed_too(self, nastech_home, tmp_path):
        ...
        a_log = (nastech_home / "logs" / "gateway.log").read_text()
...
class TestAddRotatingHandler:
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        handlers = [h for h in nastech_logging._queued_file_handlers if isinstance(h, RotatingFileHandler)]
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, nastech_home):
        ...
        assert any(
            ...
            for h in nastech_logging._queued_file_handlers
        )
