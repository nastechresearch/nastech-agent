...
Regression for the silent cross-profile secret leak class: ``nastech serve`` hosted many profile
...
only NASTECH_HOME. And the launch-profile asymmetry: a default-member hosted-room turn in a
...
def two_homes(tmp_path, monkeypatch):
    ...
    (root / ".env").write_text(
        f"A_ONLY_TOKEN={A_VAL}\nNASTECH_API_KEY={A_API_KEY}\nNASTECH_BASE_URL={A_BASE_URL}\n",
        encoding="utf-8")
    (b / ".env").write_text(
        f"B_ONLY_TOKEN={B_VAL}\nNASTECH_API_KEY={B_API_KEY}\nNASTECH_BASE_URL={B_BASE_URL}\n",
        encoding="utf-8")
    for home in (root, b):
    ...
    monkeypatch.setenv("NASTECH_API_KEY", A_API_KEY)
    monkeypatch.setenv("NASTECH_BASE_URL", A_BASE_URL)
    monkeypatch.setenv("INJECTED_TOKEN", ENV_VAL)  # systemd / op run credential injection
    monkeypatch.setattr(server, "_nastech_home", root)
...
def test_rpc_scope_reaches_llm_oneshot_and_model_options(two_homes, monkeypatch):
    ...
    def fake_oneshot(**kwargs):
        seen["oneshot"] = (Path(os.environ.get("NASTECH_HOME", "")), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        from nastech_constants import get_nastech_home
        seen["oneshot_home"] = Path(get_nastech_home())
        return "t"
    ...
    def build_payload(ctx, **kwargs):
        from nastech_constants import get_nastech_home
        seen["options"] = (Path(get_nastech_home()), get_secret("B_ONLY_TOKEN"), get_secret("A_ONLY_TOKEN"))
        return {"providers": []}

    monkeypatch.setattr("nastech_cli.inventory.build_model_options_payload", build_payload)
...
def test_launch_profile_agent_build_is_scoped_once_multiplexing(two_homes, monkeypatch):
    ...
    try:
        ...
        assert Path(get_nastech_home()) == root
    finally:
...
class _MemoryManager:
    ...
    def build_system_prompt(self):
        from agent.secret_scope import get_secret
        from nastech_constants import get_nastech_home
        return f"{get_nastech_home()}|{get_secret('MEM_PROVIDER_KEY')}"
