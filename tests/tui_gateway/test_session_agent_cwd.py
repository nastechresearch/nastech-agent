...
def workspace_runtime(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_cli.banner.prefetch_update_check", lambda: None)
    ...
    monkeypatch.setattr("nastech_cli.config.load_config_readonly", lambda: {"memory": {"provider": "honcho"}})
