...
* the Desktop/dashboard backend serves ``?profile=B`` by installing a NASTECH_HOME override WITHOUT
...
def test_ambient_only_launch_credential_never_reaches_a_routed_child(homes):
    ...
    assert env["NASTECH_HOME"] == str(b) and env["B_MARKER"] == "b"
...
def test_routed_home_with_multiplex_flag_off_gets_no_launch_residue(homes, monkeypatch):
    """Desktop/dashboard topology: B served via the NASTECH_HOME override only. The slash-worker /
    ...
    try:
        ...
        assert env["NASTECH_HOME"] == str(b) and env["B_MARKER"] == "b"
        assert "A_MARKER" not in env and "NASTECH_MODEL" not in env and "OPENAI_API_KEY" not in env
        browser_env = _build_browser_env()
        assert "FIRECRAWL_API_KEY" not in browser_env and browser_env["NASTECH_HOME"] == str(b)
    finally:
    ...
    assert served_profile_child_env(inherit_credentials=True)["NASTECH_MODEL"] == "a-model"
