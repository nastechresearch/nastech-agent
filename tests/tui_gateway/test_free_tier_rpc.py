...
def _fresh_process_memos():
    ...
    from nastech_cli import free_tier_bootstrap
...
def guest(tmp_path, monkeypatch):
    ...
    with _auth_store_lock():
        ...
        store.setdefault("providers", {})["nastech"] = {
...
def test_status_is_pull_from_local_state_and_ack_persists_on_the_identity(guest, monkeypatch):
    ...
    assert status == {"has_guest": True, "enabled": True, "available": True, "notice_pending": True,
                      "model": "nastech/welcome", "label": anon_auth.FREE_TIER_LABEL}

    assert _call("free_tier.ack_notice") == {"acked": True}
    assert _call("free_tier.status")["notice_pending"] is False
    assert _load_auth_store()["providers"]["nastech"][anon_auth.GUEST_NOTICE_FLAG] is True

    # nastech.guest: false -> the identity exists but carries nothing; no notice either.
...
def test_billing_state_answers_the_free_tier_locally(guest, monkeypatch):
    ...
    assert res["free_tier"] is True and res["free_tier_model"] == "nastech/welcome"
...
def test_provision_sets_the_free_tier_up_through_the_lifecycle_primitive(tmp_path, monkeypatch):
    ...
    def fake_provision(**kw):
        ...
        with _auth_store_lock():
            ...
            _save_auth_store(store)
        return store["providers"]["nastech"]
    ...
    def refused(**kw):
        raise anon_auth.AuthError("Nastech free tier is not open on this portal.", code="anon_gate_closed")

    with _auth_store_lock():
        store = _load_auth_store(); store["providers"].pop("nastech"); _save_auth_store(store)
