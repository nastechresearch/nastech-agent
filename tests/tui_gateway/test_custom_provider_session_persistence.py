...
# what left room bots stuck on a stale provider (e.g. "out of Nastech credits"
...
#
# Regression: GH #89497 (room bots hang then report "out of Nastech credits").
...
class TestRoomPlumbingRuntimeOverrides:
    def test_marked_row_returns_no_overrides(self):
        ...
        row = {
            ...
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "nastech", "room_plumbing": True}
            ),
        }
    ...
    def test_marked_row_dict_model_config(self):
        ...
        row = {
            "model": "openai/gpt-5.6-luna-pro",
            "model_config": {"model": "openai/gpt-5.6-luna-pro", "provider": "nastech", "room_plumbing": True},
        }
        assert _stored_session_runtime_overrides(row) == {}
    ...
    def test_legacy_group_title_shape_still_skipped(self):
        ...
        row = {
            ...
            "model": "openai/gpt-5.6-luna-pro",
            "billing_provider": "nastech",
            "model_config": json.dumps({"model": "openai/gpt-5.6-luna-pro", "provider": "nastech"}),
        }
        assert _stored_session_runtime_overrides(row) == {}
...
# "out of Nastech credits" after the profile was switched to ollama-cloud) while
...
class TestFollowProfileConfigRuntimeOverrides:
    ...
    def test_unmarked_row_still_restores_stored_runtime(self):
        ...
        row = {
            ...
            "billing_provider": "nastech",
            "model_config": json.dumps(
                {"model": "openai/gpt-5.6-luna-pro", "provider": "nastech"}
            ),
        }
        overrides = _stored_session_runtime_overrides(row)
        assert overrides["model_override"]["model"] == "openai/gpt-5.6-luna-pro"
        assert overrides["model_override"]["provider"] == "nastech"
...
# VeniceAI/empero endpoint under a model that should run on Nastech). The sibling
...
class TestRuntimeModelConfigDropsStaleKeys:
    ...
    def test_falsy_model_drops_stale_existing_model(self):
        ...
        agent = _agent_like(model="", provider="nastech")
        existing = {"model": "meituan/longcat-2.0:free", "provider": "nastech"}
        config = _runtime_model_config(agent, existing)

        assert "model" not in config, config
        assert config["provider"] == "nastech"
    ...
    def test_truthy_provider_overwrites_stale_existing(self):
        ...
        config = _runtime_model_config(_agent_like(provider="nastech"), existing)
    ...
    def test_resume_overrides_get_no_stale_provider(self):
        ...
        assert overrides["model_override"]["provider"] == "nastech"
        assert overrides["provider_override"] == "nastech"
    ...
    def test_existing_none_returns_only_agent_identity(self):
        ...
        config = _runtime_model_config(_agent_like(provider="nastech"), None)

        assert config == {"model": "deepseek/deepseek-v4-flash-0731", "provider": "nastech"}
