...
``NASTECH_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def nastech_root(tmp_path, monkeypatch):
    """A temp NASTECH_HOME root with two named profiles: 'work' and 'other'.

    Pointing NASTECH_HOME at a dir outside ~/.nastech makes it the profile ROOT
    (get_default_nastech_root's Docker/custom branch), so named profiles live at
...
def test_add_lands_in_named_profile_only(nastech_root):
    root = nastech_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(nastech_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_status_is_profile_scoped_and_credential_safe(nastech_root):
    _result(
        _call(
            "mcp.servers.add",
            {"profile": "work", "name": "svc-a", "config": {"command": "svc-a-bin"}},
...
def test_status_includes_named_profile_runtime_in_multiplex(nastech_root):
    ...
    work_token = set_nastech_home_override(nastech_root / "profiles" / "work")
    try:
        work_scope = nastech_home_key()
    finally:
        reset_nastech_home_override(work_token)
...
def test_set_api_key_writes_env_and_header_to_right_profile(nastech_root):
    root = nastech_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(nastech_root):
    root = nastech_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(nastech_root):
    root = nastech_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(nastech_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(nastech_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(nastech_root):
    root = nastech_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == NASTECH_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
...
def test_test_resolves_env_refs_from_requested_profile_secret_scope(nastech_root, monkeypatch):
