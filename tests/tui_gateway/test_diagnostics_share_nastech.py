"""diagnostics.share_nastech RPC — Desktop "Send Diagnostics" upload path.

Contract pinned:
* Reuses the CLI ``--nastech`` pipeline (collect_share_bundle → build_nastech_bundle
  → share_to_nastech) with redaction FORCED on — the client cannot disable it.
...
def _handler():
    fn = server._methods.get("diagnostics.share_nastech")
    assert fn is not None, "diagnostics.share_nastech not registered"
    return fn
...
def captured_upload(monkeypatch, tmp_path):
    ...
    import nastech_cli.diagnostics_upload as du

    monkeypatch.setattr(du, "share_to_nastech", _fake_share)
    return captured
...
def test_share_nastech_uploads_redacted_bundle(captured_upload):
    ...
    assert envelope["format"].startswith("nastech-debug-share/")
    assert envelope["redacted"] is True
    assert "report" in envelope["files"]


def test_share_nastech_attaches_redacted_error_context(captured_upload):
...
def test_share_nastech_client_text_gets_upload_safe_log_redaction(captured_upload):
...
def test_share_nastech_linkless_success_is_a_failure(monkeypatch):
    ...
    monkeypatch.setattr(du, "share_to_nastech", lambda blob: {})
...
def test_share_nastech_extra_files_sanitized_and_redacted(captured_upload):
    secret = "sk-abc123def456ghi789jkl012mno345pqr678"
    result = _handler()(
        "rid-3",
        {
...
def test_share_nastech_upload_failure_is_structured(monkeypatch):
    import nastech_cli.diagnostics_upload as du

    def _boom(blob: bytes) -> dict:
        raise RuntimeError("NAS unavailable")

    monkeypatch.setattr(du, "share_to_nastech", _boom)
