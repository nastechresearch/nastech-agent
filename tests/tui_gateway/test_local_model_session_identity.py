...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("nastech_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_nastech_home", launch)
    monkeypatch.setattr(server, "_probe_credentials", lambda agent: None)
    monkeypatch.setattr("nastech_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("nastech_cli.banner.get_available_skills", lambda: {})
    ...
    assert get_nastech_home() == launch
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_nastech_home() == secondary
    assert get_nastech_home() == launch
