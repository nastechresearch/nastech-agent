...
def _two_repo_project_skill_sessions(tmp_path, monkeypatch) -> tuple[Path, Path]:
    ...
    def repo(name: str, skill: str) -> Path:
        ...
        (r / ".nastech" / "skills" / skill).mkdir(parents=True)
        (r / ".nastech" / "skills" / skill / "SKILL.md").write_text(
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_state_registry.acquire", lambda db_path=None: profile_db)
    monkeypatch.setattr(server, "_get_db", lambda: launch_db)
    monkeypatch.setattr(server, "_enable_gateway_prompts", lambda: None)
    monkeypatch.setattr(
        server,
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "nastech_cli.model_switch.persist_model_selection",
...
def test_superseded_by_resume_is_recoverable_end_reason():
    from nastech_state_common import _RECOVERABLE_END_REASONS
...
def test_ensure_session_db_row_stamps_profile_name(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_state_registry.acquire", _ProfileDB)
    monkeypatch.setattr(server, "_resolve_model", lambda: "test-model")
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("nastech_cli.main._has_any_provider_configured", lambda **_kw: False)
...
def test_setup_status_answers_from_the_bootstrap_record_once_it_exists(monkeypatch):
    """Under ``nastech serve`` the boot bootstrap owns the free-tier identity; ``setup.status`` reports
    ...
    fb.reset_for_tests()
    monkeypatch.setattr("nastech_cli.main._has_any_provider_configured",
    ...
    def slow_bootstrap():
        release.wait(5)
        with fb._lock:
            fb._record = fb.SetupRecord(provider_configured=True, inference_provider="nastech", free_tier=True,
    ...
    try:
        ...
        assert resp["result"]["inference_provider"] == "nastech"
    finally:
        fb.reset_for_tests()
...
def test_invalid_params_and_unknown_method_name_the_version_skew_fix():
    ...
    assert "nastech update" in resp["error"]["message"]
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("nastech_cli.main._has_any_provider_configured", lambda **_kw: True)
...
def test_setup_runtime_check_agrees_with_session_fallback_chain(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", fake_resolve)
...
def test_setup_runtime_check_scopes_launch_profile_in_multiplex_backend(monkeypatch, tmp_path):
    ...
    launch_home.mkdir()
    monkeypatch.setenv("NASTECH_CODEX_BASE_URL", "https://codex.launch.test/v1")
    monkeypatch.setattr(server, "_nastech_home", launch_home)
    ...
    def resolve_codex(requested=None, **_kwargs):
        ...
        return {
            ...
            "base_url": secret_scope.get_secret("NASTECH_CODEX_BASE_URL"),
            "source": "credential-pool",
        }

    monkeypatch.setattr("nastech_cli.runtime_provider.resolve_runtime_provider", resolve_codex)
...
def test_setup_readiness_scopes_to_requested_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("nastech_cli.profiles.profile_exists", lambda name: name == "bot")
    monkeypatch.setattr(server, "_profile_home", lambda profile: bot_home if profile == "bot" else None)
    seen = {}

    def fake_resolve(requested=None, **kwargs):
        seen["home"] = Path(str(get_nastech_home())).resolve()
    ...
    assert Path(str(get_nastech_home())).resolve() != bot_home.resolve()
...
def test_setup_readiness_unknown_profile_never_answers_for_launch_profile(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.profiles.profile_exists", lambda name: False)
...
def test_config_set_model_recovers_failed_profile_resume_after_build_completes(
    ...
    monkeypatch.setattr(
        "nastech_cli.model_selection_guards.combined_selection_warning",
        lambda *args, **kwargs: None,
    )
    monkeypatch.setattr("nastech_state_registry.acquire", FakeDb)
    monkeypatch.setattr(server, "_make_agent", fake_make_agent)
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "nastech_cli.commands_completion.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    ...
    fake_registry = types.ModuleType("nastech_state_registry")
    fake_registry.acquire = _broken_shared
    monkeypatch.setitem(sys.modules, "nastech_state_registry", fake_registry)
...
def test_prompt_submit_fails_loudly_when_store_unavailable(monkeypatch):
    ...
    assert "not saved" in msg and "nastech doctor --fix" in msg
...
def test_session_list_returns_clean_error_when_state_db_is_unavailable(monkeypatch):
    ...
    assert "nastech doctor --fix" in resp["error"]["message"]
    assert resp["error"]["data"]["code"] == "storage_unavailable"
...
def test_session_list_honors_params_profile_opens_profile_db(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_get_db", lambda: LaunchDB())
    monkeypatch.setattr(
        "nastech_cli.web_server_sessions._open_session_db_at_path",
...
def test_session_delete_honors_params_profile_sessions_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(server, "_profile_home", lambda p: profile_home if p == "mlperf" else None)
    monkeypatch.setattr(server, "_get_db", lambda: None)
    monkeypatch.setattr("nastech_state_registry.acquire", ProfileDB)
...
def test_model_save_key_reconciles_the_launch_profiles_stale_setup_record(monkeypatch):
    ...
    monkeypatch.setattr("nastech_cli.auth.PROVIDER_REGISTRY", {"test-provider": types.SimpleNamespace(
        name="Test Provider", auth_type="api_key", api_key_env_vars=("TEST_PROVIDER_API_KEY",))})
    monkeypatch.setattr("nastech_cli.config.is_managed", lambda: False)
    monkeypatch.setattr("nastech_cli.credential_lifecycle.save_provider_env_credential", Mock())
    monkeypatch.setattr("nastech_cli.inventory.build_models_payload", Mock(return_value={"providers": []}))
...
def test_verification_status_returns_recorded_evidence(tmp_path, monkeypatch):
    monkeypatch.setenv("NASTECH_VERIFY_ON_STOP", "1")  # ledger is inert when the guard is off
...
def test_persist_live_session_system_prompt_uses_profile_home(monkeypatch, tmp_path):
    ...
    NASTECH_HOME to the session's profile before rebuilding the system
    ...
    # to the profile home, not the root ~/.nastech.
...
def test_persist_live_session_system_prompt_restores_pre_existing_override(tmp_path):
    """reset_nastech_home_override() restores the previous ContextVar state,
    ...
    class FakeAgent:
        ...
        def _build_system_prompt(self, system_message=None):
            from nastech_constants import get_nastech_home
            built_homes.append(str(get_nastech_home()))
            return "inner prompt"
    ...
    outer_token = set_nastech_home_override(outer_home)
    try:
        server._persist_live_session_system_prompt(session)
        ...
        # to None.
        assert get_nastech_home_override() == str(outer_home)
    finally:
