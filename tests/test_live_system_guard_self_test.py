...
def test_subprocess_popen_real_gateway_restart_blocked():
    """``python -m nastech_cli.main gateway restart`` is a detached child that
    inherits the pytest-tmp NASTECH_HOME, resolves the developer's real
    ``nastech-gateway`` unit, and outlives the test (39 six-day orphans squatted
    the webhook port, 2026-09-03). Blocked at the spawn primitive."""
    with pytest.raises(RuntimeError, match="live-system guard"):
        subprocess.Popen(
            [sys.executable, "-m", "nastech_cli.main", "gateway", "restart"],
...
def test_subprocess_run_gateway_status_passes_through():
    ...
    result = subprocess.run(
        [sys.executable, "-c", "import sys; print(sys.argv[1:])", "-m", "nastech_cli.main", "gateway", "status"],
