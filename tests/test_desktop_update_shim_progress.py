...
def progress(tmp_path):
    ...
    status = tmp_path / "nastech-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Nastech to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `nastech update`, and reports the stage that was on screen
...
FAKE_NASTECH = """#!/bin/bash
n="$(cat "$NASTECH_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$NASTECH_TEST_CALLS"
for f in "$TMPDIR"/nastech-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$NASTECH_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$NASTECH_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "nastech-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    nastech = install_root / "venv" / "bin" / "nastech"
    nastech.write_text(FAKE_NASTECH)
    nastech.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "NASTECH_TEST_CAPTURE": str(capture),
        "NASTECH_TEST_CALLS": str(calls),
        "NASTECH_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".nastech-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`nastech update` is the longest wait in the hand-off and the one the
