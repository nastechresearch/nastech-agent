"""Shared fixtures for the nastech-agent test suite.
...
def _nastech_home_points_at_production(value: str) -> bool:
    ...
    if not value:
        return True
    try:
        # The platform-default root, not a hardcoded ``~/.nastech``: Windows installs live under
        # ``%LOCALAPPDATA%\nastech``, and a dev shell exporting that path used to be honored as
        # "custom", pinning import-time paths (``tui_gateway.server._nastech_home``) to the live
        # install so the state.db guard tripped on every store-touching test (#112692).
        from nastech_state_guard import _real_platform_state_root

        resolved = Path(value).expanduser().resolve()
        real_root = _real_platform_state_root() or (Path.home() / ".nastech").resolve()
    except Exception:
        return True
...
# like a real CLI") — which used to disarm nastech_state's live-DB guard in
# the child at the same moment the child lost the NASTECH_HOME redirect.
# NASTECH_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# nastech_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export NASTECH_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["NASTECH_TEST_ISOLATION"] = os.environ.get("NASTECH_HOME", "") or "1"
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    #    fixture. Any code in the codebase reading ``~/.nastech/*`` via
    #    ``Path.home() / ".nastech"`` instead of ``get_nastech_home()``
    #    is a bug to fix at the callsite.
    fake_nastech_home = tmp_path / "nastech_test"
    fake_nastech_home.mkdir()
    (fake_nastech_home / "sessions").mkdir()
    ...
    # nastech_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("NASTECH_TEST_ISOLATION", str(fake_nastech_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("NASTECH_STATE_DB_GUARD_BYPASS", raising=False)
    ...
    #     test file with a top-level ``from nastech_state import ...``) that
    ...
    nastech_state_mod = sys.modules.get("nastech_state")
    if nastech_state_mod is not None and hasattr(nastech_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            nastech_state_mod, "DEFAULT_DB_PATH", fake_nastech_home / "state.db"
        )
    ...
    try:
        import nastech_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path NASTECH_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_nastech_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from nastech_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _close_leaked_session_dbs():
    ...
    Root cause of OOM incident 20260816: ~40 files under tests/nastech_cli/
    ...
    ``pytest tests/nastech_cli/`` accumulated 16-25 GB RSS and had to be
    OOM-killed three times in one day.

    Rather than editing every test file, ``SessionDB.__init__`` registers each
    instance in ``nastech_state_guard._test_instance_registry`` (a WeakSet,
    populated only when the ``NASTECH_TEST_ISOLATION`` marker is set — i.e.
    ...
    Instances opened through ``nastech_state_registry.acquire()`` are skipped:
    ...
    yield
    try:
        from nastech_state_guard import _test_instance_registry as registry
    except Exception:
        return
    if not registry:
        return
...
# invocation), kanban writes silently pollute the real ~/.nastech. This autouse
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    Only patches when ``nastech_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("nastech_cli.kanban_db")
    _kdbc = sys.modules.get("nastech_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import nastech_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.nastech. See #69283."
        )
...
# ``nastech_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Nastech root. This fixture wires the
# test-side knobs:
...
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("nastech_state")
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Nastech home override redirects every later
    # ``get_nastech_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from nastech_constants import get_nastech_home_override, set_nastech_home_override

        if get_nastech_home_override() is not None:
            set_nastech_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal nastech config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``nastech-gateway`` process
...
# correlated with a ``tests/nastech_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> nastech-gateway``
...
def _wal_is_usable() -> bool:
    """True when Nastech will actually put a database into WAL mode here.

    Nastech refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Nastech can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Nastech managed runtime on 3.53.1
...
#      ``nastech_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``nastech_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``nastech_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``nastech_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Nastech runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.nastech``.
#
...
def _relocate_basetemp_outside_operator_home(config) -> None:
    """Move pytest's basetemp out of the operator's platform-native Nastech home.

    Every per-test sandbox is ``<basetemp>/.../nastech_test``. ``get_default_nastech_root()``
    prefers the platform-native home whenever ``NASTECH_HOME`` sits *under* it, so a basetemp
    inside ``~/.nastech`` (or ``%LOCALAPPDATA%\\nastech``, where ``TEMP`` commonly lives on
    ...
    native = _get_platform_default_nastech_home().resolve()
    ...
    # Nastech home). The repo is no escape either: the default install checks it
    # out *inside* the home (~/.nastech/nastech-agent). A sibling of the native
    ...
    safe = Path(tempfile.mkdtemp(prefix="nastech-pytest-basetemp-", dir=safe_root))
    assert not safe.resolve().is_relative_to(native), (
        f"pytest basetemp {safe} still resolves inside the operator's Nastech home {native}; "
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Nastech falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Nastech uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart nastech-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting nastech/python patterns are also blocked.
    """
    ...
    def _is_blocked_systemctl(cmd) -> bool:
        ...
        if "systemctl" not in cmd_str:
            return False
        if not _matches_nastech_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch nastech-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m nastech_cli.main".
                if (
                    "nastech" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live nastech-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting nastech/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `nastech update` (or the
        # equivalent `python -m nastech_cli.main update`).  These commands
        ...
        if "update" in low and (
            # nastech update / nastech update --gateway / setsid bash -c ... nastech update
            ("nastech" in low and "update" in low.split())
            or
            # python -m nastech_cli.main update --gateway
            ("nastech_cli" in low and "update" in low.split())
            or
            # venv/bin/nastech update  (absolute path variant used in tests)
            (".venv/bin/nastech" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`nastech update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m nastech_cli.main
        # gateway run|start|restart``). ``_spawn_nastech_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp NASTECH_HOME, resolves the DEVELOPER's
        # ``nastech-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … nastech gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "nastech gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (nastech_cli.web_server_gateway."
                "_spawn_nastech_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    try:
        import nastech_cli.voice as _voice
    except Exception:
