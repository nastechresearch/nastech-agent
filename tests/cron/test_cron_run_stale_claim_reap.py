"""Regression for #86721 — a one-shot `nastech cron run` invocation's
...
def test_recover_interrupted_executions_reaps_the_stale_claim_from_a_dead_process(
    ...
    # exactly what a subsequent `nastech cron run` invocation would trigger
...
def test_try_dispatch_background_run_calls_recovery_before_claiming(monkeypatch):
    ...
    # caller ("nastech cron run", tests) early return documented just
...
def test_one_shot_cli_path_reaps_dead_owner_row_before_returning_sync(tmp_path, monkeypatch):
    """The real one-shot `nastech cron run` shape (``_SESSION_ASYNC_DELIVERY`` scoped to False,
    as nastech_cli/cron.py::_job_action does) must still reap a 'running' row whose owner pid
