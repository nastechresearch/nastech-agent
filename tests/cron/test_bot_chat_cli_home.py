...
def test_cli_keeps_discovered_home_when_launch_selection_changes(tmp_path, monkeypatch, profile):
    ...
    token = set_nastech_home_override(str(root))
    ...
    def run(argv, **kwargs):
        ...
        code = ('import json,sys; sys.argv=["nastech"]+json.loads(sys.argv[1]); '
                'import nastech_cli.main; from nastech_constants import get_nastech_home; '
                'print(json.dumps(str(get_nastech_home())))')
        # Everything after the launcher (binary or ``python -m nastech_cli.main``) is the CLI argv.
        cli_argv = argv[3:] if argv[1:3] == ["-m", "nastech_cli.main"] else argv[1:]
    ...
    try:
        ...
        assert seen == [home]
        assert not (tmp_path / ".nastech").exists()
    finally:
