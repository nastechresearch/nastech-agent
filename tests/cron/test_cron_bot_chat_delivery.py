...
from nastech_cli.quiet_single_query import TURN_REPORT_FILE_ENV, write_turn_report
...
def test_named_profile_resolves_when_exists():
    with mock.patch("nastech_cli.profiles.profile_exists", return_value=True):
...
def test_unknown_profile_resolves_to_none():
    with mock.patch("nastech_cli.profiles.profile_exists", return_value=False):
...
def _child_env() -> dict:
    """The stand-in child imports ``nastech_cli`` from this checkout, like the real ``-m nastech_cli.main``."""
