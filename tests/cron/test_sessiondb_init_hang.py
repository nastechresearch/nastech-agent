...
class TestSessionDbInitTimeout:
    def test_sessiondb_init_preserves_multiplex_profile_context(
        ...
        try:
            with patch("cron.scheduler._nastech_home", None), \
                 ...
                 patch("nastech_state_registry.acquire", side_effect=make_session_db), \
                 patch(
    ...
    def test_run_job_does_not_hang_when_sessiondb_init_wedges(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._nastech_home", tmp_path), \
             ...
             patch("nastech_cli.env_loader.load_nastech_dotenv"), \
             patch("nastech_cli.env_loader.reset_secret_source_cache"), \
             patch("nastech_state_registry.acquire"), \
             patch(
                 "nastech_cli.runtime_provider.resolve_runtime_provider",
                 return_value=_RUNTIME,
             ), \
    ...
    def test_late_session_db_result_is_closed(self, tmp_path, monkeypatch):
        ...
        try:
            with patch("cron.scheduler._nastech_home", tmp_path), \
                 ...
                 patch("nastech_cli.env_loader.load_nastech_dotenv"), \
                 patch("nastech_cli.env_loader.reset_secret_source_cache"), \
                 patch("nastech_state_registry.acquire", side_effect=_hanging_then_capture), \
                 patch(
                     "nastech_cli.runtime_provider.resolve_runtime_provider",
                     return_value={
                         "api_key": "test-key",
...
class TestSessionDbInitAfterEarlyReturns:
    ...
    def test_wake_gate_false_never_opens_session_db(self, tmp_path, monkeypatch):
        ...
        with patch("cron.scheduler._nastech_home", tmp_path), \
             ...
             patch("nastech_cli.env_loader.load_nastech_dotenv"), \
             patch("nastech_cli.env_loader.reset_secret_source_cache"), \
             patch("nastech_state_registry.acquire") as mock_db_cls, \
             patch(
