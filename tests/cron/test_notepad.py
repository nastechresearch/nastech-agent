...
class TestNotepadProfileIsolation:
    def test_profile_override_routes_writes_to_current_home(self, tmp_path):
        ...
        import_token = set_nastech_home_override(profile_a)
        try:
            importlib.reload(notepad_mod)
        finally:
            reset_nastech_home_override(import_token)

        runtime_token = set_nastech_home_override(profile_b)
        try:
            notepad_mod.set_note("job-1", "cursor", "page=7")
        finally:
            reset_nastech_home_override(runtime_token)
