...
def cron_store(tmp_path, monkeypatch):
    ...
    (nastech_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("NASTECH_HOME", str(nastech_home))
    monkeypatch.setattr(J, "NASTECH_DIR", nastech_home)
    monkeypatch.setattr(J, "CRON_DIR", nastech_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", nastech_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", nastech_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_nastech_now", lambda: now):
            due = J.get_due_jobs()
