...
resolve_runtime_provider against a temp NASTECH_HOME) and the create_job snapshot capture.
"""
...
def _run(job, tmp_path, *, current_provider="openrouter", current_model=None, cron_model=None,
    ...
    with patch("cron.scheduler._nastech_home", tmp_path), \
         ...
         patch("nastech_cli.runtime_provider.resolve_runtime_provider", side_effect=_resolve), \
...
class TestResnapshot:
    ...
    def test_resnapshot_unpinned_refreshes_to_current(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr("cron.jobs.get_nastech_home", lambda: tmp_path, raising=True)
        with patch(
            "nastech_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
        ):
