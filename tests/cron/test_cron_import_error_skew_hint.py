...
Field incident: an interrupted `nastech update` (pull done, restart never ran)
...
had no way to know the fix was one `nastech gateway restart`. The failure
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "nastech gateway restart" not in msg
