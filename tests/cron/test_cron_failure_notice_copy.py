"""User-facing cron failure notices: plain words, the real output path, and the exact `nastech cron`
...
def test_generic_failure_names_runs_and_pause_commands_and_the_real_output_dir():
    ...
    for cmd in ("nastech cron runs ab12cd34", "nastech cron run ab12cd34", "nastech cron pause ab12cd34"):
        assert f"`{cmd}`" in msg
    assert f"{display_nastech_home()}/cron/output/ab12cd34/" in msg
...
def test_auth_failure_points_at_login_and_a_retry_command(monkeypatch):
    ...
    assert "/login" in msg and "`nastech auth add <provider>`" in msg
    assert "nastech login" not in msg  # that command was removed
    assert "`nastech cron run ab12cd34`" in msg
...
def test_auth_failure_names_the_pinned_provider_and_the_failing_profile(monkeypatch, tmp_path):
    ...
    _no_chain(monkeypatch)
    profile_home = tmp_path / ".nastech" / "profiles" / "ops"
    ...
    assert "`nastech -p ops auth add openai-codex --type oauth`" in msg, msg
    assert "<provider>" not in msg
    unpinned = _summarize_cron_failure_for_delivery(JOB, "Error code: 401 - Unauthorized")
    assert "`nastech -p ops auth add <provider>`" in unpinned, unpinned
...
def test_rate_and_usage_limit_phrases_still_yield_a_provider_notice(monkeypatch):
    ...
    Nastech Portal limit turns into a raw generic notice."""
    _no_chain(monkeypatch)
    for text in (
        "Nastech Portal rate limit active until 15:00",
        ...
        assert "`nastech cron run ab12cd34`" in msg or "`nastech cron edit ab12cd34" in msg, msg
...
def test_cron_cause_gloss_is_the_shared_table():
    ...
    for reason in FAILURE_CAUSE_GLOSS:
        ...
        assert notice is not None and "`nastech cron" in notice, reason
...
def test_transient_provider_failures_never_lead_with_jargon(monkeypatch):
    ...
    for err in ("Request timed out.", "HTTP 429: Too Many Requests"):
        ...
        assert "`nastech cron run ab12cd34`" in msg
        assert "`nastech cron runs ab12cd34`" in msg
...
def test_blocked_config_notice_says_it_did_not_run_and_will_self_heal():
    ...
    assert "`nastech cron doctor`" in text
