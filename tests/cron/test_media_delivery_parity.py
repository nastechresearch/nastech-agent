...
text-only on manual ``nastech cron run <job-id>``. Same box, same token, same
...
def _install_fake_slack_sender(monkeypatch, result_factory):
    ...
    if entry is None:
        ...
        import nastech_cli.plugins as hp_boot
    ...
    import nastech_cli.plugins as hp
...
def slack_platform_config(monkeypatch, tmp_path):
    ...
    # Config caches are process-global; clear them so the temp NASTECH_HOME wins.
    try:
...
class TestMediaPolicyEnvBridge:
    ...
    def test_bridge_helper_exists_and_applies_config(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("NASTECH_HOME", str(home))
        for var in (
            "NASTECH_MEDIA_DELIVERY_STRICT",
            "NASTECH_MEDIA_ALLOW_DIRS",
            "NASTECH_MEDIA_TRUST_RECENT_FILES",
        ):
        ...
        assert os.environ.get("NASTECH_MEDIA_DELIVERY_STRICT") == "1"
        assert str(allow_dir) in os.environ.get("NASTECH_MEDIA_ALLOW_DIRS", "")
        assert os.environ.get("NASTECH_MEDIA_TRUST_RECENT_FILES") == "0"
    ...
    def test_deliver_result_runs_bridge(self, monkeypatch, tmp_path, media_file):
        ...
        Realistic enterprise shape: NASTECH_MEDIA_DELIVERY_STRICT=1 arrives via
