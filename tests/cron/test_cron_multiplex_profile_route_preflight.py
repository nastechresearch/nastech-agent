...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_nastech_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "nastech_constants.get_default_nastech_root", lambda: root
    )
    token = set_nastech_home_override(str(grant_home))
