...
def test_deferred_destination_does_not_follow_root_changes(tmp_path, monkeypatch, recipient):
    ...
    monkeypatch.setenv("NASTECH_HOME", str(source))
    monkeypatch.setattr("nastech_cli.profiles.get_profile_dir", lambda _: home)
    ...
    try:
        ...
        lease.release()
        db.close()
    monkeypatch.setattr("nastech_cli.profiles.get_profile_dir", lambda _: other)
    ...
    try:
        with monkeypatch.context() as changed:
            changed.setenv("NASTECH_HOME", str(tmp_path / "new-source"))
        ...
        if recipient == "cli":
            ...
            assert Path(run.call_args.kwargs["env"]["NASTECH_HOME"]) == home
