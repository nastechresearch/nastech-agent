...
class TestGetTimezone:
    ...
    def test_multiplex_prefers_routed_profile_config_over_env(self, tmp_path, monkeypatch):
        """Under the multiplexed gateway NASTECH_TIMEZONE holds only the DEFAULT profile's value
        ...
        monkeypatch.setenv("NASTECH_TIMEZONE", "America/New_York")

        # Single-profile process: env stays authoritative.
        assert nastech_time.get_timezone_name() == "America/New_York"

        set_multiplex_active(True)
        try:
            assert nastech_time.get_timezone_name() == "America/New_York"  # default profile turn
            token = set_nastech_home_override(str(routed_home))
            try:
                assert nastech_time.get_timezone_name() == "Asia/Tokyo"
                assert str(nastech_time.get_timezone()) == "Asia/Tokyo"
            finally:
