...
def test_create_job_no_agent_requires_script(nastech_env):
...
def test_update_job_roundtrips_no_agent_flag(nastech_env):
    from cron.jobs import create_job, update_job, get_job

    script_path = nastech_env / "scripts" / "w.sh"
...
def test_cronjob_tool_create_no_agent_without_script_errors(nastech_env):
...
def test_run_job_no_agent_success_returns_script_stdout(nastech_env):
    """Happy path: script exits 0 with output, delivered verbatim."""
    from cron.jobs import create_job
    from cron.scheduler import run_job

    script_path = nastech_env / "scripts" / "alert.sh"
...
def test_run_job_no_agent_reloads_dotenv_before_script(nastech_env, monkeypatch):
    ...
    def fake_load(*, nastech_home=None, project_env=None):
        loaded_homes.append(nastech_home)
        return []

    monkeypatch.setattr(env_loader, "load_nastech_dotenv", fake_load)

    script_path = nastech_env / "scripts" / "probe.sh"
    script_path.write_text('#!/bin/bash\necho "ok"\n')
    ...
    assert success is True
    assert error is None
    assert loaded_homes, "load_nastech_dotenv was not called on the no_agent path"
    assert str(loaded_homes[0]) == str(nastech_env)
...
def test_no_agent_script_gets_owning_profiles_declared_secret_never_launch_residue(
    nastech_env, monkeypatch, tmp_path,
):
    ...
    (nastech_env / ".env").write_text("LAUNCH_ONLY_TOKEN=launch-secret\n", encoding="utf-8")
    monkeypatch.setenv("LAUNCH_ONLY_TOKEN", "launch-secret")  # what load_nastech_dotenv() did at startup
    ...
    home_token = set_nastech_home_override(str(routed))
...
def test_no_agent_script_of_launch_profile_keeps_its_own_env_credential(nastech_env, monkeypatch):
    ...
    monkeypatch.delenv("JOB_SVC_TOKEN", raising=False)
    (nastech_env / "scripts" / "probe.sh").write_text(_PRESENCE_PROBE, encoding="utf-8")
...
def test_agent_provider_timeout_delivery_keeps_fallback_guidance(nastech_env, monkeypatch):
...
def test_run_job_script_nul_path_fails_cleanly(nastech_env):
...
def test_run_job_script_nul_rejected_before_any_path_call(nastech_env, monkeypatch):
...
def test_run_job_script_accepts_pathlike_script_path(nastech_env):
...
@pytest.mark.parametrize(
    "error",
    [
        "Script timed out after 900s: /home/u/.nastech/scripts/nightly.sh",
