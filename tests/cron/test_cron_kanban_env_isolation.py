...
def test_dispatcher_grants_only_the_assigned_worker_scope(tmp_path, monkeypatch):
    ...
    from nastech_cli.kanban_db_dispatch import _default_spawn

    monkeypatch.setenv("HOME", str(tmp_path))
    db = tmp_path / "board.db"
    monkeypatch.setenv("NASTECH_KANBAN_DB", str(db))
    conn = connect(db)
    ...
    worker.chmod(0o700)
    monkeypatch.setenv("NASTECH_BIN", str(worker))
    # Building a new worker under an existing task must replace, not inherit, its scope.
    monkeypatch.setenv("NASTECH_KANBAN_TASK", "prior-task")
    ...
    monkeypatch.setenv("NASTECH_DELEGATED_CHILD_CONTEXT", str(tmp_path))
    ...
    assert os.environ["NASTECH_KANBAN_TASK"] == "prior-task"
    conn.close()
