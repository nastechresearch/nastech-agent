...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("nastech_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._nastech_home", tmp_path), \
             ...
             patch("nastech_state_registry.acquire", return_value=fake_db), \
             patch(
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("NASTECH_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("NASTECH_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("NASTECH_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("NASTECH_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("NASTECH_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("NASTECH_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("NASTECH_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("NASTECH_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("NASTECH_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_heartbeats_oneshot_claim_in_both_wait_modes(
        ...
        monkeypatch.setenv("NASTECH_CRON_TIMEOUT", timeout_value)
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this NASTECH_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_nastech_dotenv`` (which then re-applies external secret sources).
        """
        ...
        with patch("cron.scheduler._nastech_home", tmp_path), \
             patch("cron.scheduler_delivery._resolve_origin", return_value=None), \
             patch("nastech_cli.env_loader.reset_secret_source_cache", _record_reset), \
             patch("nastech_cli.env_loader.load_nastech_dotenv", _record_load), \
             patch("nastech_state_registry.acquire", return_value=fake_db), \
             patch(
                 "nastech_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("NASTECH_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("NASTECH_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("NASTECH_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobModelResolution:
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``nastech_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """NASTECH_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("NASTECH_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``nastech cron create --deliver=<platform>`` silently
...
class TestFailureStreakNudge:
    ...
    def test_nudges_at_threshold(self):
        ...
        assert "nastech cron pause scout" in out
