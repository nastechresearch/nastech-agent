...
class TestSecureHelpers(unittest.TestCase):
    ...
    def test_secure_dir_preserves_operator_mode_in_container(self):
        ...
        an explicit NASTECH_HOME_MODE is still honored (#10757)."""
        from cron.jobs import _secure_dir
        with tempfile.TemporaryDirectory() as tmp:
            d = Path(tmp) / "cron"
            d.mkdir()
            os.chmod(d, 0o755)
            with patch.dict(os.environ, {"NASTECH_CONTAINER": "1"}, clear=False):
                os.environ.pop("NASTECH_HOME_MODE", None)
                _secure_dir(d)
                self.assertEqual(stat.S_IMODE(os.stat(d).st_mode), 0o755)
                os.environ["NASTECH_HOME_MODE"] = "0701"
                _secure_dir(d)
