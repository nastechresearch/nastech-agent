...
def _assert_path_preserved(env):
    """Original PATH entries survive sanitization; the nastech console-script
    dir may be prepended (see _sanitize_subprocess_env, issue #92998) so we
    assert the contract, not byte equality."""
    from tools.environments.local import _resolve_nastech_bin_dir

    path_val = env.get("PATH", "")
    assert path_val.endswith("/usr/bin:/bin"), path_val
    nastech_bin = _resolve_nastech_bin_dir()
    if nastech_bin and path_val != "/usr/bin:/bin":
        assert path_val.startswith(nastech_bin + os.pathsep), path_val
