...
class TestOpenVikingAutoRecallPrefetch:
    @pytest.mark.parametrize("peer", ["", "nastech"])
    def test_prefetch_e2e_sends_limit_and_reads_l2_content(self, monkeypatch, peer):
                    ...
                    if payload.get("context_type") == "memory":
                        self._send_json({
                            "result": {
                                "memories": [
                                    {
                                        "uri": "viking://user/user/peers/nastech/memories/e2e-full.md",
                                        "score": 0.9,
                                        "level": 2,
                                        "category": "events",
        ...
        assert records["reads"] == ["viking://user/user/peers/nastech/memories/e2e-full.md"]
        ...
        assert all(
            headers.get("user-agent") == f"openviking-memory-nastech/{_NASTECH_VERSION}"
...
class TestEnsureClientReloadsEnv:
    ...
    def test_rebuilt_client_resolves_its_own_user_space(self, monkeypatch):
        ...
        assert alice_uri.startswith("viking://user/alice/peers/nastech/")
        assert bob_uri.startswith("viking://user/bob/peers/nastech/")
    ...
    def test_handle_tool_call_reconnects_after_startup_health_failure(self, monkeypatch):
        ...
        assert out["session_id"].startswith("nastech-remember-")
    ...
    def test_remember_uses_a_distinct_one_shot_session_for_each_call(self, monkeypatch):
        ...
        assert first["session_id"].startswith("nastech-remember-")
        assert second["session_id"].startswith("nastech-remember-")
    ...
    def test_remember_reports_commit_failure_with_recovery_command(self, monkeypatch):
        ...
        assert out["error"].startswith(
            "Memory message was accepted, but commit failed for session nastech-remember-"
        )
        ...
        assert out["session_uri"].endswith(f"/sessions/{out['session_id']}")
        assert out["recovery_command"] == f"ov session commit {out['session_id']}"
        assert "same OpenViking profile and credentials as Nastech" in out["recovery_note"]
        assert len(posts) == 2
        assert posts[0][0].endswith("/messages")
