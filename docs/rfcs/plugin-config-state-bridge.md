...
or change arbitrary Nastech configuration.
...
small host fact requested by the RFC. Settings writes use Nastech'
...
`NASTECH_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<NASTECH_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
- **Profile isolation:** the data root resolves from the active context-local
  Nastech home on every operation.
...
The implementation is covered with real temporary-Nastech-home tests for:
...
- [Issue #64227](https://github.com/NastechResearch/nastech-agent/issues/64227)
- [RFC PR #58542](https://github.com/NastechResearch/nastech-agent/pull/58542) by Topher Ross
