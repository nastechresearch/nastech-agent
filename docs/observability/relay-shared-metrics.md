...
into Nastech and does not require a Nastech observability plugin. Nastech remains
...
> [!WARNING]
> This removes the Nastech `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `NASTECH_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `NASTECH_NEMO_RELAY_ATOF_*` and `NASTECH_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Nastech does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Nastech requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Nastech
...
opt-in. Set `NASTECH_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Nastech does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Nastech reports the error and does not invoke
...
retain the same `session_id` and add `nastech.session.segment` plus
`nastech.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Nastech
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Nastech process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Nastech logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Nastech
process and share the same policy boundary.

Run profiles in separate Nastech processes when they require different trust
...
`$NASTECH_HOME/telemetry/shared_metrics/outbox/` to the Nastech telemetry ingest
...
telemetry:
  shared_metrics:
    ...
    send: false      # NEW: transmit to the Nastech telemetry service
```
...
Removing `$NASTECH_HOME/telemetry/shared_metrics` still resets local identity,
