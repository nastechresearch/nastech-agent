...
def _stat_db_file_identity(path: Path) -> Optional[Tuple[int, int]]:
    ...
    Mirrors the nastech_state helper of the same name; kept local so this
    module has no import-time dependency on nastech_state (which imports
...
def acquire(db_path: Optional[Path] = None) -> "SessionDB":
    ...
    shared generation was opened — e.g. by ``nastech sessions recover`` or
...
# ── Backwards-compatible aliases (nastech_state re-exports) ──
# Kept so call sites and tests can import either from nastech_state
