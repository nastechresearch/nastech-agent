...
export const APPROVAL_COMMAND = 'rm -rf /tmp/nastech-e2e-approval-probe'
 ...
 * `E2E_SAY(code-farmer)[{at}nastech Reply with B.] E2E_SAY(nastech)[B]`.
...
/** Create an isolated NASTECH_HOME + Electron user-data dir in the OS temp dir. */
function createDevSandbox(): DevSandbox {
  const root = fs.mkdtempSync(nodePath.join(os.tmpdir(), `nastech-dev-mock-${Date.now()}`))
  const nastechHome = nodePath.join(root, 'nastech-home')
...
function writeMockConfig(nastechHome: string, mockUrl: string): void {
  fs.writeFileSync(
    nodePath.join(nastechHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(nodePath.join(nastechHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function runDevLaunch(): Promise<void> {
  ...
  const env: Record<string, string> = {
    ...
    NASTECH_DESKTOP_NASTECH_ROOT: repoRoot,
