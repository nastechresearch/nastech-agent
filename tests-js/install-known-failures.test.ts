...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'nastech-update',
  error: 'E2E ASSERTION FAILED: nastech update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('nastech.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'nastech-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured nastech desktop spec; update completed',
      logs: { desktop: '[nastech] [updates] no staged updater; surfacing manual `nastech update` for CLI install at C:/install\n[nastech] [updates] manual: nastech update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: nastech update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> nastech-update (v2026.3.12 -> HEAD)'
