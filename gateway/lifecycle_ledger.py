...
``<NASTECH_HOME>/state/gateway.lifecycle.json``:
...
def check_state_db_integrity(home: Optional[Path] = None) -> str:
    ...
    ``_enforce_macos_synchronous_full`` in :mod:`nastech_state` — macOS
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``nastech_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
