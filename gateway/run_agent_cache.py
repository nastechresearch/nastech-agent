...
class GatewayAgentCacheMixin:
    ...
    def _is_intentional_model_switch(self, session_key: str, agent: Any, config_model: str) -> bool:
        ...
        override names that model, or the Nastech gateway moved the session off the ``nastech/welcome``
        ...
        return getattr(agent, "_nastech_model_switch", None) == (config_model, agent.model)
    ...
    def _bind_adapter_run_generation(self, adapter: Any, session_key: str, generation: int | None) -> None:
        ...
        with suppress(Exception):
            ...
            if interrupt_event is not None:
                interrupt_event._nastech_run_generation = int(generation)
    ...
    def _ephemeral_change_key(context, redact_pii: bool) -> str:
        ...
        try:
            ...
            home_display = str(display_nastech_home())
        except Exception:
            home_display = ""
        key_tuple = (
    ...
    def _run_release_in_profile_scope(self, target, args: tuple, session_key: Optional[str]) -> None:
        ...
        root Nastech dir even when the gateway was launched under a named profile. Its scope is
            ...
            if session_key and store is not None:
                try:
                    ...
                    logger.warning("Could not resolve the owning profile for %s; releasing under the default profile",
                                   session_key, exc_info=True)
            owner_home = Path(owner) if owner else get_default_nastech_root()
            if current_secret_scope() is None or nastech_home_key(get_nastech_home()) != nastech_home_key(owner_home):
