...
Hooks live in ~/.nastech/hooks/<name>/ with HOOK.yaml (name, description, events) and
...
def _resolve_hooks_dir() -> Path:
    ...
    else ``get_nastech_home()/hooks``. The import-time constant is the LAUNCH profile's; under
    ...
    return configured if configured != _HOOKS_DIR_AT_IMPORT else get_nastech_home() / "hooks"
...
class ProfileHookRegistries:
    """``HookRegistry`` per served profile home, picked at emit time from the active NASTECH_HOME.
    ...
    ``get_nastech_home()`` there gives each profile its own ``hooks/`` and keeps the default
    profile's handlers from seeing other profiles' messages. Each home's registry is loaded on its
    first emit, i.e. inside that profile's scope (handler imports see its NASTECH_HOME); multiplexing off
    ...
    def _active(self) -> HookRegistry:
        key = nastech_home_key(get_nastech_home())
