...
from nastech_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: nastech gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Nastech commands.
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _nastech_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from nastech_cli.context_switch_guard import (
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from nastech_cli.route_identity import should_clear_context_pin_async
                        ...
                        from nastech_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        # Persist the selection only — nastech_cli.personality never writes
        # agent.system_prompt (user-owned manual overlay). persist_personality
        # writes get_nastech_home()/config.yaml, i.e. the routed profile under
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower == "gate" or lower.startswith("gate "):
            ...
            if gate_lower.startswith("add "):
                ...
                # Nastech process account — and with no admin list configured
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from nastech_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `nastech cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from nastech_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Nastech itself changed. ``--stat`` limits output to the summary.
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.nastech/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``nastech skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _gateway_config_home
        from nastech_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `nastech skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.nastech/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Nastech is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while Nastech is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while Nastech is busy."
            return EphemeralReply(
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from nastech_cli.session_export import (
            SAVE_USAGE,
        ...
        temp_dir = tempfile.mkdtemp(prefix="nastech_save_")
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (nastech_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Nastech balance and hand off to the portal.
        ...
        lines: list[str] = ["💳 **Nastech balance**"]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # ── Nastech credits magnitudes + monthly-grant % gauge ─────────────
        ...
        # Run off the event loop. The helper gates on "a Nastech account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # Nastech-credentialled user running inference elsewhere (or with none resident)
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            from nastech_state import get_shared_session_db, release_shared_session_db
            ...
            # with no explicit path resolves ``get_nastech_home()`` at call
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # get_nastech_home()/skills, a contextvar override under multiplex.
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        # profile's logs/config off ``get_nastech_home()`` and uploads them to a
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through nastech_cmd (venv\Scripts\nastech.exe): the shim
