...
_SESSION_SOURCE: ContextVar = ContextVar("NASTECH_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("NASTECH_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_TYPE: ContextVar = ContextVar("NASTECH_SESSION_CHAT_TYPE", default=_UNSET)
...
_SESSION_USER_ID_ALT: ContextVar = ContextVar("NASTECH_SESSION_USER_ID_ALT", default=_UNSET)
...
_SESSION_SCOPE_ID: ContextVar = ContextVar("NASTECH_SESSION_SCOPE_ID", default=_UNSET)
...
# intentionally separate from NASTECH_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("NASTECH_UI_SESSION_ID", default=_UNSET)
...
_SESSION_PROFILE: ContextVar = ContextVar("NASTECH_SESSION_PROFILE", default=_UNSET)
_BROWSER_CONTROL_PRINCIPAL: ContextVar = ContextVar(
    "NASTECH_BROWSER_CONTROL_PRINCIPAL", default=_UNSET
)
_BROWSER_CONTROL_TRANSPORT_FAMILY: ContextVar = ContextVar(
    "NASTECH_BROWSER_CONTROL_TRANSPORT_FAMILY", default=_UNSET
)
...
_CRON_SESSION: ContextVar = ContextVar("NASTECH_CRON_SESSION", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("NASTECH_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_VAR_MAP = {
    ...
    "NASTECH_SESSION_CHAT_TYPE": _SESSION_CHAT_TYPE,
    ...
    "NASTECH_SESSION_USER_ID_ALT": _SESSION_USER_ID_ALT,
    "NASTECH_SESSION_USER_NAME": _SESSION_USER_NAME,
    "NASTECH_SESSION_SCOPE_ID": _SESSION_SCOPE_ID,
    "NASTECH_SESSION_KEY": _SESSION_KEY,
    "NASTECH_SESSION_ID": _SESSION_ID,
    "NASTECH_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "NASTECH_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "NASTECH_SESSION_PROFILE": _SESSION_PROFILE,
    "NASTECH_BROWSER_CONTROL_PRINCIPAL": _BROWSER_CONTROL_PRINCIPAL,
    "NASTECH_BROWSER_CONTROL_TRANSPORT_FAMILY": _BROWSER_CONTROL_TRANSPORT_FAMILY,
    "NASTECH_CRON_SESSION": _CRON_SESSION,
...
def set_current_session_id(session_id: str) -> None:
    ...
    ``NASTECH_SESSION_ID`` for the rest of the process — leaking the child id
    ...
    # parent's session identity. See NastechPRDelegationSessionContext task.
    try:
...
def set_session_vars(
    ...
    ``os.environ["NASTECH_CRON_SESSION"]`` fallback, ``"1"`` marks a cron job,
...
def reset_session_vars() -> None:
    ...
    *A's* ``NASTECH_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``NASTECH_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
# value (``telegram``) to NASTECH_SESSION_PLATFORM, while the CLI, TUI, and
# desktop bind NASTECH_SESSION_SOURCE (``cli``, ``tui``, ``desktop``) and leave
...
# NASTECH_SESSION_PLATFORM but have no attachment channel behind them.
...
def session_is_messaging_surface() -> bool:
    ...
    Resolves ``NASTECH_PLATFORM``, then the session platform, then the session
    source, and reports messaging when any of them names a surface outside
    :data:`NON_MESSAGING_SESSION_SURFACES`.
    """
    import os

    platform = os.getenv("NASTECH_PLATFORM") or get_session_env("NASTECH_SESSION_PLATFORM", "")
...
def declare_stateless_channel() -> None:
    ...
    See NastechResearch/nastech-agent#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    or a one-shot runner that exits after its final response (``nastech -z``,
    cron — see :func:`declare_stateless_channel`) — and dispatcher-spawned
    Kanban workers (identified by ``NASTECH_KANBAN_TASK``), which are one-shot
