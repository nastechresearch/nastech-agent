...
Replaces the old ``os.environ``-based ``NASTECH_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "NASTECH_SESSION_PLATFORM", "NASTECH_SESSION_SOURCE", "NASTECH_SESSION_CHAT_ID",
    "NASTECH_SESSION_CHAT_TYPE", "NASTECH_SESSION_CHAT_NAME", "NASTECH_SESSION_THREAD_ID",
    "NASTECH_SESSION_USER_ID", "NASTECH_SESSION_USER_ID_ALT", "NASTECH_SESSION_USER_NAME",
    "NASTECH_SESSION_SCOPE_ID", "NASTECH_SESSION_KEY", "NASTECH_SESSION_ID",
    "NASTECH_UI_SESSION_ID", "NASTECH_SESSION_MESSAGE_ID", "NASTECH_SESSION_PROFILE",
    "NASTECH_BROWSER_CONTROL_PRINCIPAL", "NASTECH_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "NASTECH_CRON_SESSION", "NASTECH_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("NASTECH_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("NASTECH_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("NASTECH_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("NASTECH_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("NASTECH_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``NASTECH_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``NASTECH_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds NASTECH_SESSION_PLATFORM, CLI/TUI/
# desktop bind NASTECH_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``NASTECH_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("NASTECH_PLATFORM") or get_session_env("NASTECH_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("NASTECH_SESSION_SOURCE", ""))
...
def async_delivery_supported() -> bool:
    ...
    (``NASTECH_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
