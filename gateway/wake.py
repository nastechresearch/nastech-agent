...
* Stateless request/response adapters (the API server,
  ...
  ``X-Nastech-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Nastech-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Nastech-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Nastech-Session-Id`` is 403-gated on
    ...
    payload = {
        "model": str(getattr(adapter, "_model_name", "") or "nastech-agent"),
