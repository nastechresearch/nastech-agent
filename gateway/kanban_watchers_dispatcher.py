...
def _kbd():
    from nastech_cli import kanban_db_dispatch
...
def _default_profile_secret_scope():
    ...
    LLM reads ``auxiliary.*`` from ``get_nastech_home()``, so its credentials come
    ...
    if not is_multiplex_active():
        yield
        return
    token = set_secret_scope(build_profile_secret_scope(Path(get_nastech_home())))
    try:
        yield
    finally:
