...
Built on gateway startup, refreshed every 5 min, saved to ``<NASTECH_HOME>/channel_directory.json``.
...
def _directory_path() -> Path:
    return DIRECTORY_PATH or get_nastech_home() / "channel_directory.json"


def _aliases_path() -> Path:
    return CHANNEL_ALIASES_PATH or get_nastech_home() / "channel_aliases.json"
...
def format_directory_for_display(platforms: Optional[Dict[str, Any]] = None) -> str:
    ...
    ``platforms`` overrides the on-disk directory (``nastech send --list`` merges in
