...
def resolve_whatsapp_bridge_dir() -> Path:
    ...
    /opt/nastech) is mirrored to NASTECH_HOME so npm install works."""
    import shutil
    from nastech_constants import get_nastech_home
    install_bridge = Path(__file__).resolve().parents[2] / "scripts" / "whatsapp-bridge"
    nastech_home_bridge = get_nastech_home() / "scripts" / "whatsapp-bridge"
    try:
    ...
    try:
        ...
        shutil.copytree(install_bridge, nastech_home_bridge, dirs_exist_ok=False)
        return nastech_home_bridge
    except Exception:
        return install_bridge
