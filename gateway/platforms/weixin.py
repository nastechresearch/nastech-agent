...
def save_weixin_account(nastech_home: str, *, account_id: str, token: str, base_url: str, user_id: str = "") -> None:
    path = _account_dir(nastech_home) / f"{account_id}.json"
...
def load_weixin_account(nastech_home: str, account_id: str) -> Optional[Dict[str, Any]]:
    return _read_json(_account_dir(nastech_home) / f"{account_id}.json")
...
def _load_sync_buf(nastech_home: str, account_id: str) -> str:
    data = _read_json(_account_dir(nastech_home) / f"{account_id}.sync.json")
...
def _save_sync_buf(nastech_home: str, account_id: str, sync_buf: str) -> None:
    atomic_json_write(_account_dir(nastech_home) / f"{account_id}.sync.json", {"get_updates_buf": sync_buf})
...
async def qr_login(nastech_home: str, *, bot_type: str = "3", timeout_seconds: int = 480) -> Optional[Dict[str, str]]:
    if not AIOHTTP_AVAILABLE:
        ...
        while time.monotonic() < deadline:
            ...
            if status == "wait":
                ...
                if not creds["account_id"] or not creds["token"]:
                    ...
                    return None
                save_weixin_account(nastech_home, **creds)
...
class WeixinAdapter(OwnAccessPolicyMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._nastech_home = nastech_home = str(get_nastech_home())
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if self._group_policy != "disabled":
            logger.warning(
                ...
                "ordinary-group events for these accounts, so group messages may never reach Nastech regardless of "
                "this policy. If group delivery doesn't work, the limitation is on the iLink side, not in Nastech.",
