...
- GET  /v1/models                  — lists nastech-agent and any configured model_routes aliases
...
def _nastech_version() -> str:
    """Return the canonical Nastech Agent version string.

    ``nastech_cli.__version__`` is the runtime source of truth used by the CLI,
    ...
    ``nastech_agent-*.dist-info`` after a source update until the environment is
...
# config.yaml accept (nastech_constants.VALID_REASONING_EFFORTS); wire-level
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    max_tokens = None
    env_max_tokens = os.environ.get("NASTECH_MAX_TOKENS")
    if env_max_tokens:
        try:
...
def _request_agent_overrides(
    ...
    *,
    ...
    ``/v1/models`` advertises a stable virtual model (usually ``nastech-agent``)
    ...
    ``direct_model_requests`` config value here, while Nastech-native
    endpoints (session chat, /v1/runs) always allow it.  A request that
    sends an explicit ``provider`` is unambiguously Nastech-aware and is
    always honored.
    """
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send an explicit ``provider`` — and the Nastech-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        ...
        :func:`nastech_cli.model_switch.resolve_effective_model` (the shared
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        ...
        if not getattr(cfg, "multiplex_profiles", False):
            ...
            # `nastech peer dm mini/researcher` answered by the mini's default
    ...
    def _declared_conversation_session(
        ...
        """Resolve the live session a client declared with ``X-Nastech-Session-Key``.
        ...
        every conversation-affinity hint Nastech sends off that id
        (``prompt_cache_key`` on both OpenAI-wire transports, the
        OpenRouter/Nastech sticky ``session_id``, and xAI's ``x-grok-conv-id``)
    ...
    def _ensure_session_db(self):
        ...
        redirects ``get_nastech_home()``, so each profile gets its own DB —
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        The advertised virtual model (usually ``nastech-agent``) means "use
        ...
        400s ("nastech-agent is not a valid model ID") — the same filter
    ...
    def _session_runtime_request_from_body(self, body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # The virtual model alias (self._model_name, e.g. "nastech-agent") is
    ...
    def _create_agent(
        ...
        # nastech_cli.model_switch.resolve_effective_model (session /model
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "nastech-agent", "version": _nastech_version()}
        )
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list nastech-agent and any configured model_routes aliases.
    ...
    async def _handle_model_options(self, request: "web.Request") -> "web.Response":
        """GET /api/model/options — return Nastech provider/model inventory.
        ...
        Nastech provider catalog instead of scraping the single OpenAI-compatible
        `/v1/models` alias.
        """
        auth_err = self._check_auth(request)
        if auth_err:
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<NASTECH_HOME>/plugin-data/.../artifacts``-style controlled root),
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        ...
        # Exact-title lookup, used by `nastech peer dm` to resolve a peer's
        ...
        if title_filter:
            ...
            if not sessions:
                ...
                # which would fail `nastech peer dm` resolution and mint
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        ...
        # (e.g. "provider::nastech-agent") are split, and the virtual model
        # alias (self._model_name, e.g. "nastech-agent") is nulled out
        # there — a bare "nastech-agent" is not a model_routes alias, so a
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "nastech.session.model_lock",
            "session_id": session_id,
            "runtime": runtime,
        })
    @_admit_api_agent_request
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Nastech-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Nastech-Error"] = _redact_api_error_text(err_msg, limit=200)
        ...
        try:
            ...
            async def _emit(item):
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    await response.write(_sse_frame(item[1], event="nastech.tool.progress"))
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
                        "created": created, "model": model,
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # the conversation it declared via ``X-Nastech-Session-Key`` before
        ...
        # response and the X-Nastech-Session-Id header.  Without this,
        ...
        response_headers = {"X-Nastech-Session-Id": _effective_session_id}
        if gateway_session_key:
            response_headers["X-Nastech-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        # `nastech cron run` / cronjob(action='run', prompt=...) — same length
            ...
            if role == "assistant" and msg.get("tool_calls"):
                for tc in msg["tool_calls"]:
                    ...
                    items.append({
                        ...
                        # Nastech agent; they are replayed for structured tool
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # that is the exact surface the nastech-0day campaign abused to write
            # ~/.nastech/config.yaml and plant persistence. Sandboxing (Docker /
