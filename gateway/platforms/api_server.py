...
_BROWSER_CONTROL_WS_PROTOCOL = "nastech-browser-control-v1"
_BROWSER_CONTROL_TICKET_PROTOCOL_PREFIX = "nastech-browser-control-ticket."
...
def _nastech_version() -> str:
    """Canonical Nastech version: ``nastech_cli.__version__`` (dist-info can be stale on
...
def _resolve_request_runtime_agent_kwargs(provider: str, target_model: Optional[str] = None) -> Dict[str, Any]:
    ...
    from nastech_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error, _get_model_config
    try:
...
def _request_agent_overrides(
    ...
    The virtual model (``nastech-agent``) means "gateway default". A bare ``model`` without
    ``provider`` is honored only when ``allow_bare_model`` (generic clients hardcode "gpt-4o";
    OpenAI-compatible handlers pass the ``direct_model_requests`` opt-in, Nastech-native
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        from nastech_state_wal import apply_wal_with_fallback
...
_CORS_HEADERS = {
    ...
    "Access-Control-Allow-Headers": "Authorization, Content-Type, Idempotency-Key, X-Nastech-Session-Id"}
_SECURITY_HEADERS = {
...
def _derive_chat_session_id(system_prompt: Optional[str], first_user_message: str) -> str:
    ...
    turns of an Open WebUI-style conversation), so one Nastech session/sandbox is reused."""
...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    """aiohttp server routing OpenAI-format requests through nastech-agent's AIAgent."""
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # executing model. Requests that send an explicit ``provider`` — and the Nastech-native session-chat
    ...
    def _resolve_model_name(explicit: str) -> str:
        """Advertised /v1/models name: explicit override > active profile name > "nastech-agent"
        (precedence owned by ``nastech_cli.model_switch.resolve_effective_model``)."""
        from nastech_cli.model_switch import resolve_effective_model
        profile_name = ""
        with suppress(Exception):
        ...
        return resolve_effective_model(explicit, profile_name, "nastech-agent")
        ...
        if not profile:
            with suppress(Exception):
                ...
                if is_multiplex_active():
                    ...
                    return _profile_runtime_scope(get_nastech_home())
            return nullcontext()
        from gateway.run import _profile_runtime_scope
    ...
    def _declared_conversation_session(self, gateway_session_key: Optional[str]) -> Optional[str]:
        """Resolve the live session a client declared with ``X-Nastech-Session-Key`` (the key
    ...
    def _parse_session_key_header(
        ...
        """Validate ``X-Nastech-Session-Key`` (per-channel memory scope) -> ``(key_or_None, None)``
    ...
    def _ensure_session_db(self):
        """SessionDB for the active profile home (the runtime scope redirects ``get_nastech_home()``
        per profile). Sync, for ``_create_agent``; handlers use ``_ensure_session_db_async``."""
        if self._session_db is not None:
            return self._session_db
        try:
            from nastech_constants import get_nastech_home
            return self._open_and_cache_session_db(get_nastech_home())
    ...
    def _stored_session_model(self, session: Any) -> Optional[str]:
        ...
        "nastech-agent" upstream as a provider model id 400s)."""
    ...
    def _normalize_session_source(value: Any) -> str:
        ...
        allowed = {"api_server", "nastech_browser", "browser", "cli", "telegram", "discord", "slack", "desktop", "dashboard"}
        if text not in allowed:
            return "api_server"
        return "nastech_browser" if text == "browser" else text
    ...
    def _recover_or_record_model(self, model: str, runtime_kwargs: Dict[str, Any], gateway_session_key) -> str:
        ...
        # No model.default but a provider resolved (e.g. `nastech auth add` without `nastech model`).
    ...
    def _select_agent_runtime(
        ...
        # nastech_cli.model_switch.resolve_effective_model.
        from nastech_cli.model_switch import resolve_effective_model
        if session_override:
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "nastech-agent", "version": _nastech_version()})
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "status": readiness["status"], "readiness": readiness, "platform": "nastech-agent",
            "version": _nastech_version(), "gateway_state": gw_state,
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — nastech-agent plus configured model_routes aliases (alias + resolved
        ...
        def _model(mid: str, root: str, parent) -> Dict[str, Any]:
            return {"id": mid, "object": "model", "created": now, "owned_by": "nastech", "permission": [],
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "nastech.api_server.capabilities", "platform": "nastech-agent",
    ...
    def _artifact_store_for(self, profile: str) -> ArtifactStore:
        ...
        (``<NASTECH_HOME>/plugin-data/.../artifacts``-style controlled root), so artifacts never escape the
        ...
        try:
            ...
            # Unscoped fallback (tests/manual wiring): controlled root under the Nastech home.
            try:
                from nastech_state import get_nastech_home
                root = Path(get_nastech_home()) / "artifacts" / "browser-control"
            except Exception:
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        ...
        # Exact-title lookup (`nastech peer dm` -> canonical "Bot Chat"). include_hidden is honored
        ...
        if title_filter and not sessions:
            # A canonical Bot Chat auto-archived by the orphan reaper would make `nastech peer dm`
            ...
            try:
                ...
                # which would fail `nastech peer dm` resolution and mint transient sessions — same accident
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions -- create an empty Nastech session row. Existence check, insert and
        ...
        # would persist "nastech-agent" and later send it to the provider literally.
    ...
    def _session_headers(session_id: str, gateway_session_key: Optional[str]) -> Dict[str, str]:
        """``X-Nastech-Session-Id`` (+ ``X-Nastech-Session-Key`` when declared) response headers."""
    ...
    def _effective_turn_runtime(self, runtime_request: Dict[str, Any], result: Any, usage: Any) -> Dict[str, Any]:
        ...
        return self._sanitize_runtime_metadata(
            ...
            # gateway/status readers resolve process-level paths and do NOT follow the NASTECH_HOME
    ...
    async def _handle_session_model_lock(self, request: "web.Request") -> "web.Response":
        ...
        runtime = self._sanitize_runtime_metadata(
            ...
            model_lock="accepted")
        return web.json_response(
            {"object": "nastech.session.model_lock", "session_id": session_id, "runtime": runtime})
    ...
    async def _handle_run_job(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        # Optional transient per-run context (standalone `nastech cron run` /
    ...
    async def _handle_cron_fire(self, request: "web.Request") -> "web.Response":
        ...
        with _reserve_pending_api_work(self) as reservation:
            ...
            # `nastech pause` ESTOP: refuse the fire and ask NAS to retry later.
            ...
            with suppress(ImportError):
                ...
                if _estop_check_paused("cron-webhook", logger):
                    return web.json_response(
                        {"error": "nastech is paused (ESTOP)", "job_id": job_id},
                        status=503,
    ...
    def _bind_api_server_session(
        *, chat_id: str = "", session_key: str = "", session_id: str = "", profile: str = "",
        ...
        reach ``NASTECH_SESSION_PROFILE``: the persistent-Docker container key is derived from it, so an
    ...
    def _turn_runtime_metadata(
        ...
        runtime = dict(getattr(agent, "_nastech_api_runtime", {}) or {})
