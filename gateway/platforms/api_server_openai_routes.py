...
def _nastech_extras(completed, is_partial, is_failed, err_msg, finish_reason: str) -> Dict[str, Any]:
    return {
...
class OpenAICompatRoutesMixin:
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # X-Nastech-Session-Key scopes long-term memory per channel; independent of
        # X-Nastech-Session-Id (the key persists across transcripts, the id rotates on /new).
        gateway_session_key, key_err = self._parse_session_key_header(request)
        if key_err is not None:
            return key_err
        # X-Nastech-Session-Id continues an existing session (history from state.db, not the body);
        ...
        run_kwargs = dict(
            ...
            # #98619: only an explicitly provided X-Nastech-Session-Id is wake-capable (the
        ...
        if provided_session_id and body.get("nastech_notification_category") == "diagnostic":
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """``nastech.tool.progress`` status=running; ``_``-prefixed tools stay off the wire."""
        ...
        response_headers = {"X-Nastech-Session-Id": (provided_session_id or result.get("session_id", session_id))}
        ...
        # Soft partial (some text, run incomplete): 200 + finish_reason="length"/Nastech extras.
        response_data = {
        ...
        if is_partial or is_failed or not completed:
            response_data["nastech"] = _nastech_extras(
        ...
        try:
            ...
            async for delta in _iter_stream_items(stream_q, agent_task, response):
                ...
                if isinstance(delta, tuple) and len(delta) == 2 and delta[0] == "__tool_progress__":
                    ...
                    await response.write(_sse_frame(delta[1], event="nastech.tool.progress"))
                else:
            ...
            if finish_reason != "stop":
                ...
                finish_chunk["nastech"] = _nastech_extras(
                    completed, is_partial, is_failed, "" if presentation_muted else err_msg, finish_reason)
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        # Session precedence: previous_response_id chain > declared X-Nastech-Session-Key > fresh
