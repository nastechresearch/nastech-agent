...
class QQAdapter(OwnAccessPolicyMixin, BasePlatformAdapter):
    ...
    async def _send_identify(self) -> None:
        ...
        payload = {"op": 2, "d": {
            ...
            "shard": [0, 1],
            "properties": {"$os": "macOS", "$browser": "nastech-agent", "$device": "nastech-agent"}}}
    ...
    async def _default_interaction_dispatch(self, event: InteractionEvent) -> None:
        ...
        ``~/.nastech/.update_response``; anything else is ignored at DEBUG."""
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``.update_response``, polled by the detached ``nastech update --gateway`` watcher."""
        try:
    ...
    async def send_update_prompt(
        ...
        are written to ``~/.nastech/.update_response`` by the interaction callback."""
        del session_key, metadata  # present for contract parity only.
        default_hint = f" (default: {default})" if default else ""
        content = f"𓄃 **Update Needs Your Input**\n\n{prompt}{default_hint}"
