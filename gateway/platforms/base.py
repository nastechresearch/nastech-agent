...
# as MPEG-2 Layer II are audio to Nastech but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    # Routed Nastech profile for shared state.db namespaces (topic bindings
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``NASTECH_SESSION_PLATFORM`` contextvar — that contextvar is
...
def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_nastech_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_nastech_dir(new_subpath, old_name)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Nastech root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* NASTECH_HOME's cache, so a gateway running at
    the root (e.g. ``NASTECH_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when NASTECH_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _NASTECH_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("NASTECH_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _NASTECH_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Nastech profile and shared Nastech root both contain control
    ...
    # NASTECH_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.nastech
    ...
    for nastech_root in (_NASTECH_HOME, _NASTECH_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(nastech_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Nastech
    secrets (``~/.nastech/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Nastech cache dirs.

    The agent legitimately sees generated artifacts at ``/root/.nastech/...``
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Nastech cache dirs (``/root/.nastech/...``), the session's
    ...
    # In-process gateways (Desktop backend, `nastech serve`) may not have
    ...
    # are longer prefixes, so /root/.nastech/... still translates to the host
    # cache — this only catches stray home writes like /root/out.png.
    if not any(c.as_posix() == "/root" for _, c in mounts):
        # /root/.nastech/* that did NOT match a cache mount is the container's
        ...
        if not candidate.as_posix().startswith("/root/.nastech"):
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    # credential/secret stores under the Nastech root (~/.nastech/.env,
    ...
    # ``MEDIA:~/.nastech/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``NASTECH_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Nastech commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-NASTECH_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("NASTECH_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Nastech sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-NASTECH_HOME holder may be replaced only when the runner
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(nastech --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Nastech home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Nastech home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.nastech/media/generated/stale.png"}
            ...
            if response:
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                        ...
                        and not self._streaming_tts_turn_completed(
                            session_key,
                            getattr(interrupt_event, "_nastech_run_generation", None),
                            event=event,
                        )):
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # NASTECH_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile NASTECH_HOME isolation.
        """
