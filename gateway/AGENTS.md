...
(`nastech_cli/AGENTS.md`). Each adapter picks a base toolset (Telegram → `"messaging"`).
...
`if canonical == ...` chains. Registry + adding a command: `nastech_cli/AGENTS.md`.
...
duplicate-final incident (NS-658 canary ledger, nastech#85796 / gateway-gateway#210); violating any
...
(or `NASTECH_BACKGROUND_NOTIFICATIONS`): `concise` (default; one line, failures append an output
...
`/login` is registered in `nastech_cli/commands.py` with `busy_policy="dispatch"` and
...
**It binds the whole install.** The sign-in writes the singleton `providers.nastech`, so whoever
...
On completion the handler **evicts** every cached agent still on `nastech/welcome` and clears any
...
`nastech serve` (control plane, desktop-spawned child) dies with the app — by design. The messaging
gateway (`gateway run`) SURVIVES the app: the serve backend's `/api/gateway/*` endpoints spawn it
detached (`_spawn_nastech_action` — `start_new_session` / `DETACHED_PROCESS`), so `before-quit`'s
