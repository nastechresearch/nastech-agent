...
# (`nastech update` re-runs it in the terminal) and only a short tail is quoted in chat.
_UPDATE_FAILED_NOTICE = (
    "❌ Nastech update failed; the previous version is still running. Run `nastech update` on the "
...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``nastech update --gateway`` and its watcher exchange under NASTECH_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_nastech_home / ".update_pending.json",
            claimed=_nastech_home / ".update_pending.claimed.json", output=_nastech_home / ".update_output.txt",
            exit_code=_nastech_home / ".update_exit_code",
            prompt=_nastech_home / ".update_prompt.json", response=_nastech_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Nastech update finished." if exit_code == 0 else _UPDATE_FAILED_NOTICE
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Nastech update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _nastech_home, _non_conversational_metadata
    ...
    def _free_tier_startup_line(self) -> Optional[str]:
        """Extra startup line when the gateway's inference is carried by the Nastech free tier; None otherwise.
        ...
        try:
            ...
            from nastech_cli.anon_auth import guest_carries_inference
            if not guest_carries_inference():
                return None
            if resolve_provider("auto") != "nastech":
                return None
        except Exception as exc:
            logger.debug("Free tier startup line skipped: %s", exc)
            return None
        return "Inference: Free tier (nastech/welcome). Sign in for more: /login"
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from nastech_constants import get_default_nastech_root, profile_cli_selector
        from nastech_state import _default_db_path, classify_persistence_error
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `nastech` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_nastech_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `nastech {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   nastech {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) nastech {profile_arg}sessions recover "
                ...
                f"Run `nastech {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `nastech {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                f"recovery tools or restore a backup unless `nastech {profile_arg}doctor` confirms damage."
            )
        else:
            ...
            message = (
                ...
                f"empty. Cause: {failure.gloss}. Run `nastech {profile_arg}doctor --fix` on the "
                f"gateway machine, then `nastech {profile_arg}gateway restart`."
            )
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Nastech-Session-Id key, not a structured ``agent:...`` key.
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_nastech_home()/state.db``) resolve from the ambient scope.
        ...
        if get_nastech_home_override() == str(profile_home):
            return contextlib.nullcontext()
