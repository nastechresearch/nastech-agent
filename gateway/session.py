...
# ``NASTECH_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``NASTECH_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
...
# ``sessions_dir / f"{session_id}.json"`` in nastech_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `nastech tools` (opt-in, default
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Nastech session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_nastech_home()`` for the turn (its docstring lists "sessions"
        ...
        # profile name -> its NASTECH_HOME (or None to use the ambient scope).
        ...
        try:
            ...
            self._routing_home: Optional[Path] = Path(get_nastech_home())
        except Exception:
    ...
    def _open_session_db_for_active_scope(self, db_path: Optional[Path] = None):
        ...
        time, and that helper follows the context-local NASTECH_HOME override
        ...
        """
        from nastech_state import SessionDB, _default_db_path, get_shared_session_db
    ...
    def _profile_home_for_key(self, session_key: Optional[str]) -> Optional[Path]:
        """NASTECH_HOME of the profile that owns *session_key*, or None.
    ...
    def _db_for_key(self, session_key: Optional[str]):
        ...
        ``_db`` follows the ambient NASTECH_HOME, and only the inbound message
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.nastech/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.nastech/state.db "
                "and are shown by `nastech sessions list` and `/sessions`. "
    ...
    def _append_to_transcript_serialized(
        ...
        while True:
            try:
                ...
                from nastech_state import CompressionSessionClosedError, StateDbReplacedError
                ...
                if isinstance(exc, StateDbReplacedError):
                    ...
                    try:
                        from nastech_state import divert_session_transcript_jsonl
