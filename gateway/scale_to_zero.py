...
Design constraints (decisions.md):
  - Per-instance enable is gated SOLELY by the NAS "Labs" toggle, carried to the
    gateway as the ``NASTECH_SCALE_TO_ZERO`` env stamp (D11/Q8=A). NOT a user
...
SCALE_TO_ZERO_ENV = "NASTECH_SCALE_TO_ZERO"
...
def scale_to_zero_enabled(environ: Optional[dict] = None) -> bool:
    """Whether the per-instance Labs toggle is on (the NASTECH_SCALE_TO_ZERO stamp).
...
def dashboard_client_heartbeat_path(nastech_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under NASTECH_HOME."""
    ...
    return Path(nastech_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
