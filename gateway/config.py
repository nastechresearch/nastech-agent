...
class PlatformConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "PlatformConfig":
        ...
        # write them where the docs and ``nastech config set platforms.webhook.port`` put them:
...
class GatewayConfig:
    ...
    # ``None``: a request the gateway settles at boot, not a verdict. ``nastech_cli.gateway_multiplex_mode
    ...
    # route); NASTECH_ROOM_LINK_URL overrides.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        return cls(
            ...
            sessions_dir=Path(data["sessions_dir"]) if "sessions_dir" in data else get_nastech_home() / "sessions",
...
def load_gateway_config() -> GatewayConfig:
    """Load gateway configuration. Priority: env > ~/.nastech/config.yaml > legacy gateway.json > defaults."""
