...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from nastech_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (nastech_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Nastech is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # gateway behaves exactly as before — single NASTECH_HOME, no profile stamping.
    ...
    # advertise a route. NASTECH_ROOM_LINK_URL remains the operator override.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        # set to a recognized value. Hosted deployments (Nastech Portal / Fly) stamp
...
def load_gateway_config() -> GatewayConfig:
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through nastech_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `nastech config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `nastech config set gateway.multiplex_profiles true`
