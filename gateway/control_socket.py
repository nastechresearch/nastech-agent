...
(the updater, `nastech serve`/dashboard, the Desktop app) currently discovers
...
- ``identify`` → pid, profile label, nastech_home, code_sha/code_version
...
- POSIX: Unix domain socket at ``$NASTECH_HOME/gateway.sock``. When the home
  path is too long for ``sun_path`` (~104 bytes on macOS/BSD), the socket is
  bound in the system temp dir and a pointer file
  ``$NASTECH_HOME/gateway.sock.path`` records the real location; clients
  follow the pointer transparently.
- Windows: named pipe ``\\\\.\\pipe\\nastech-gateway-<home-hash>`` served via
...
Consumers (``nastech update --plan`` inventory, the post-update fleet version
...
def windows_pipe_name(home: Path) -> str:
    """Per-NASTECH_HOME named pipe path (Windows transport)."""
    return rf"\\.\pipe\nastech-gateway-{_home_hash(home)}"
...
def _fallback_socket_path(home: Path) -> Path:
    ...
    """
    name = f"nastech-gw-{_home_hash(home)}.sock"
...
def _detect_supervisor() -> str:
    ...
    if sys.platform == "darwin" and (
        env.get("XPC_SERVICE_NAME", "").startswith("ai.nastech")
        or env.get("LAUNCHD_SOCKET")
    ):
        return "launchd"
    if env.get("NASTECH_DESKTOP_MANAGED"):
        return "desktop"
...
def build_identify_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "nastech_home": record.get("nastech_home"),
        "profile": _profile_label_for_home(record.get("nastech_home") or ""),
...
class GatewayControlServer:
    ...
    its NASTECH_HOME), ``stop()`` on shutdown. All failures are non-fatal —
