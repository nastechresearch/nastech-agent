...
class GatewayModelCommandsMixin:
    ...
    async def _perform_model_switch(
        ...
        try:
            from nastech_cli.context_switch_guard import enrich_model_switch_warnings_for_gateway
    ...
    async def _model_switch_confirmation(
        ...
        from gateway.run import _load_gateway_config
        from nastech_cli.model_switch import format_model_for_display, resolve_display_context_length_async
        ...
        # Provider-aware chain: Codex OAuth, Copilot and Nastech caps win over the raw models.dev entry.
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        ctx = _ModelSwitchContext(
            ...
            # the caller's). user_id is critical for the fallback lookup path (nastech_state.py:1994-2009)
            ...
            source=source,
            config_path=(profile_home or _nastech_home) / "config.yaml",
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality — list or set a personality (nastech_cli.personality owns the state)."""
        from gateway.run import _load_gateway_config
