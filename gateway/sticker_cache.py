...
(``~/.nastech/sticker_cache.json``) so the same image is never re-analyzed.
"""
...
def _resolve_cache_path() -> Path:
    ...
    it, else live profile-scoped NASTECH_HOME — under the multiplexed gateway one process serves
    every profile, so the import-time constant would pin every profile to the launch home."""
    return CACHE_PATH if CACHE_PATH != _CACHE_PATH_AT_IMPORT else get_nastech_home() / "sticker_cache.json"
