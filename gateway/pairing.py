...
# path eagerly freezes it to whatever NASTECH_HOME/profile context existed
# at that exact import moment for the rest of the process's lifetime --
# even if a context-local override (see nastech_constants.set_nastech_home_override)
# is established afterward. A freshly-started, short-lived process (e.g. the
# ``nastech pairing`` CLI) re-imports this module later with the final
...
# working (NastechResearch/nastech-agent#93449).
#
...
def _default_pairing_dir() -> Path:
    ...
    if PAIRING_DIR is not None:
        return PAIRING_DIR
    return get_nastech_dir("platforms/pairing", "pairing")
...
def _read_allowlist_env(env_var: str) -> str:
    ...
    The grant mirror below writes through ``nastech_cli.config.save_env_value``
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{NASTECH_HOME}/pairing`` while newer code/docs may
    write ``{NASTECH_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    profile's own NASTECH_HOME using the same legacy/consolidated layout rules
    as ``nastech -p <name> pairing ...``. This keeps multiplex gateways and
    profile-scoped CLI approvals on one whitelist. Without a profile, storage
    is the global pairing directory for the current NASTECH_HOME.
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp NASTECH_HOME
        ...
        if profile:
            ...
            # ``nastech -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `nastech` after
    ...
    def approve_request(self, platform: str, request_id: str) -> Optional[dict]:
        ...
        This is the grant path for authenticated admin surfaces (``nastech
