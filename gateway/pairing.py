...
failed approvals, chmod 0600 data files, codes never logged. Storage: ~/.nastech/pairing/
"""
...
# path would freeze the NASTECH_HOME/profile context at gateway boot, ignoring later
# context-local overrides, so the gateway and ``nastech pairing`` CLI wrote different
...
# computing the path eagerly freezes it to whatever NASTECH_HOME/profile context existed at that exact import
# moment for the rest of the process's lifetime -- even if a context-local override (see
# nastech_constants.set_nastech_home_override) is established afterward. A freshly-started, short-lived
# process (e.g. the ``nastech pairing`` CLI) re-imports this module later with the final environment already
...
# (NastechResearch/nastech-agent#93449). ``_default_pairing_dir()`` below resolves this fresh on every call in
...
def _default_pairing_dir() -> Path:
    return PAIRING_DIR if PAIRING_DIR is not None else get_nastech_dir("platforms/pairing", "pairing")
...
def _load_json_file(path: Path) -> dict:
    ...
    try:
        ...
        try:
            ...
            # exec` runs as root and writes the file, then the gateway process — running as `nastech` after
        ...
        logger.warning(
            ...
            "If you ran `docker exec <container> nastech pairing approve ...` as root, "
            "re-run with `docker exec -u nastech <container> ...` and "
            "chown the existing file to the nastech user, or restart the "
...
class PairingStore:
    ...
    profile's NASTECH_HOME exactly as ``nastech -p <name> pairing ...`` does, so multiplex
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        self._dir = get_nastech_dir("platforms/pairing", "pairing", home=profile_home) if profile else _default_pairing_dir()
