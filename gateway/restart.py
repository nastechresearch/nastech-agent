...
# Set by ``nastech gateway run --external-supervisor``. Unlike systemd's INVOCATION_ID
# and launchd's XPC_SERVICE_NAME, this survives wrappers that replace the child
# environment (e.g. ``sudo env -i``).
EXTERNAL_GATEWAY_SUPERVISOR_ENV = "NASTECH_GATEWAY_EXTERNAL_SUPERVISOR"
...
def is_gateway_supervisor_process(environ: Mapping[str, str] | None = None) -> bool:
    ...
    ``--external-supervisor`` opt-in. The generalized ``NASTECH_SUPERVISED_CHILD`` launcher marker is
    ...
    return bool(env.get("INVOCATION_ID") or env.get("NASTECH_S6_SUPERVISED_CHILD") or (xpc_service and xpc_service != "0")
...
def is_supervised_gateway_launch(environ: Mapping[str, str] | None = None) -> bool:
    ...
    Superset of :func:`is_gateway_supervisor_process` that also honours ``NASTECH_SUPERVISED_CHILD``,
    the marker every generated launcher exports (systemd unit, launchd plist, s6 run script, Windows
    Scheduled Task — see ``nastech_cli.main._apply_profile_override``). This is the identity the
    ...
    """
    env = os.environ if environ is None else environ
    if env.get("NASTECH_SUPERVISED_CHILD"):
        return True
