...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(
        material,
        b"nastech-hosted-room-installation-grant-v1",
        hashlib.sha256,
    ).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local NASTECH_HOME override. The process
...
def derive_room_grant_secret(api_key: str) -> bytes:
    ...
    return hmac.new(
        ...
        b"nastech-hosted-room-grant-v1",
        hashlib.sha256,
    ).digest()
    ...
    checked_policy = RoomExecutionPolicy.from_mapping(
        ...
        or execution_policy_mapping(
            target_profile=(
                ...
                or (os.getenv("NASTECH_PROFILE") or "default").strip()
                or "default"
            )
        )
    )
...
def _configured_room_link_url() -> str | None:
    ...
    override = os.getenv("NASTECH_ROOM_LINK_URL")
