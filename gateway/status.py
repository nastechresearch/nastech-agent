"""Gateway runtime status helpers: PID/lock/marker files under ``{NASTECH_HOME}`` (one set per
...
from nastech_constants import _get_platform_default_nastech_home, get_nastech_home, get_process_nastech_home
...
def record_start_and_check_storm(
    ...
    try:
        path = get_nastech_home() / "gateway-starts.log"
...
def _get_process_nastech_home() -> Path:
    """Launch-home NASTECH_HOME for identity files (PID, lock, status, markers):
    ``get_nastech_home()`` honors the per-session ``_NASTECH_HOME_OVERRIDE`` and would misroute
    them."""
    return get_process_nastech_home()


def _canonical_nastech_home(path: Path | str) -> Path:
    """Stable absolute NASTECH_HOME path for persisted identity data."""
    return Path(path).expanduser().resolve(strict=False)


def _same_nastech_home(left: Path | str, right: Path | str) -> bool:
    """Compare NASTECH_HOME paths with the host platform's case semantics."""
    left_c = os.path.normcase(str(_canonical_nastech_home(left)))
    return left_c == os.path.normcase(str(_canonical_nastech_home(right)))


def recorded_gateway_home_conflicts(
    record: Optional[dict[str, Any]], *, expected_home: Optional[Path | str] = None
) -> bool:
    """True when a persisted gateway record names a DIFFERENT NASTECH_HOME (cross-profile kill guard:
    profile B's stop must never SIGTERM profile A). ``expected_home`` overrides the comparison base.
    Legacy records without ``nastech_home`` prove nothing -> False; a comparison failure fails
    closed -> True."""
    recorded_home = record.get("nastech_home") if isinstance(record, dict) else None
    if not isinstance(recorded_home, str) or not recorded_home.strip():
        return False
    try:
        base = expected_home if expected_home is not None else _get_process_nastech_home()
        return not _same_nastech_home(recorded_home, base)
    except Exception:
        return True


# Mirrors nastech_cli.profiles._PROFILE_ID_RE -- duplicated so gateway identity code
# stays import-light (nastech_constants + stdlib only).
...
def _profile_label_for_home(home: Path | str) -> Optional[str]:
    ...
    try:
        canonical = _canonical_nastech_home(home)
    except Exception:
        return None
    if canonical.parent.name == "profiles" and _PROFILE_LABEL_RE.match(canonical.name):
        return canonical.name
    import nastech_constants
    default_homes = (nastech_constants.get_default_nastech_root, _get_platform_default_nastech_home)
    for default_home in default_homes:
        with contextlib.suppress(Exception):
            if _same_nastech_home(canonical, default_home()):
                return "default"
    return None
...
def _get_runtime_status_path() -> Path:
    return _get_process_nastech_home() / _RUNTIME_STATUS_FILE


def _get_lock_dir() -> Path:
    """Machine-local dir for token-scoped gateway locks; ``NASTECH_GATEWAY_LOCK_DIR`` overrides."""
...
def retained_gateway_state(runtime: Any) -> str:
    ...
    ``nastech gateway stop`` keeps the last ``startup_failed`` + ``exit_reason`` on disk for
    ...
    under the same rule, so the dashboard agrees with ``nastech gateway status``. Any other retained
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Nastech gateway lifecycle subcommand from a command line, or None. No loose substring matches
    (``"gateway" in cmdline`` also matched ``gateway status`` / ``python -m tui_gateway``): needs a
    Nastech entrypoint plus the ``gateway`` subcommand, or a gateway-dedicated entrypoint. Tokenizes
    ...
    if any(t == "gateway/run.py" or t.endswith("/gateway/run.py") for t in tokens):
        return "run"
    if any(b in ("nastech-gateway", "nastech-gateway.exe") for b in basenames):
        return "run"
    joined = " ".join(tokens)
    if "nastech_cli.main" not in joined and "nastech_cli/main.py" not in joined and not any(
        b in ("nastech", "nastech.exe") for b in basenames
    ):
        return None
    ...
    for i, token in enumerate(filtered):
        if token == "gateway":
            # Bare `nastech gateway` defaults to `run`.
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    Nastech-owned records / cleanup scans only; ``looks_like_gateway_command_line`` stays strict."""
    return _gateway_command_subcommand(command) in {"run", "restart"}


def _looks_like_gateway_process(pid: int) -> bool:
    """True when the live PID still looks like the Nastech gateway."""
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    ``nastech_cli.gateway._matches_current_profile``): a stale state file can record a PID recycled
    onto ANOTHER profile's live gateway. Named profiles carry ``-p``/``--profile <name>`` or
    ``NASTECH_HOME=`` on argv; the default gateway runs bare. Separators normalized."""
    command_lc = command.lower().replace("\\", "/")
    profile_name = _profile_name_for_home(profile_home)
    home_lc = str(profile_home).lower().replace("\\", "/")
    if profile_name is not None and profile_name != "default":
        return profile_flag_value(command_lc) == profile_name.lower() or f"nastech_home={home_lc}" in command_lc
    ...
    # or a conflicting explicit NASTECH_HOME= (its absence is not disqualifying -- NASTECH_HOME usually
    # arrives via the env).
    if profile_flag_value(command_lc) is not None:
        return False
    return not ("nastech_home=" in command_lc and f"nastech_home={home_lc}" not in command_lc)
...
def _build_pid_record() -> dict:
    return {
        ...
        "nastech_home": str(_canonical_nastech_home(_get_process_nastech_home())),
    }
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    Lazy import keeps ``gateway.status`` free of ``nastech_cli`` at import time. Never raises.

    A gateway keeps serving the module versions it imported at startup, so stamping the identity into
    ``gateway_state.json`` lets `nastech update` (and the dashboard) prove whether a running gateway actually
...
def _pid_record_belongs_to_current_profile(record: Optional[dict[str, Any]]) -> bool:
    """True when the record's ``nastech_home`` matches the current process (legacy records: True);
    another NASTECH_HOME's record must be ignored or the default gateway assumes its identity."""
    if not isinstance(record, dict):
        return False
    record_home = record.get("nastech_home")
    return not record_home or _same_nastech_home(record_home, _get_process_nastech_home())
...
def multiplexer_liveness_for_profile(profile_dir: Path) -> Optional[tuple[int, dict[str, Any]]]:
    ...
    dashboard ladder reports it stopped while ``nastech -p X status`` says running — the two must agree.
    """
    name = _profile_name_for_home(Path(profile_dir))
    if not name:
        return None
    from nastech_cli.gateway import named_profile_served_by_running_multiplexer
    from nastech_cli.gateway_multiplex_served import live_default_gateway_pid
    ...
    if pid is None:
        return None
    return pid, read_runtime_status(get_default_nastech_root() / "gateway_state.json") or {}
...
def resolve_gateway_liveness(
    *, profile_dir: Optional[Path] = None, runtime: Any = _UNSET,
    ...
    # the multiplexer IS its gateway (mirrors `nastech -p X status` / `gateway list`). Unscoped, the
    # question is about the process's OWN home — which is a named profile inside a pooled
    # `nastech --profile X serve` (the Desktop's per-profile backend answers its REST without
    # `?profile=`), so it takes the same rung instead of reporting the served profile stopped.
    own_home = profile_dir if scoped else _get_process_nastech_home()
...
def get_runtime_status_running_pid(
    ...
    if pid is None:
        return None
    # Active-profile context: the record's nastech_home must match this process so a stale record
...
def acquire_scoped_lock(
    ...
    # does not say WHICH profile). Omitted when not inferable; readers fall back to nastech_home.
    profile = _profile_label_for_home(_get_process_nastech_home())
    if profile:
...
# token. ``nastech.service`` + ``nastech- gateway.service``). See #5646.
...
def _get_takeover_marker_path(nastech_home: Optional[Path] = None) -> Path:
    """Takeover marker path; ``nastech_home`` is given only for a verified cross-home handoff."""
    home = _canonical_nastech_home(nastech_home or _get_process_nastech_home())
    return home / _TAKEOVER_MARKER_FILENAME


def _get_planned_stop_marker_path() -> Path:
    return _get_process_nastech_home() / _PLANNED_STOP_MARKER_FILENAME
...
def _pid_marker_names_self(target_pid: int, target_start_time: Any) -> bool:
    ...
    ``nastech gateway stop`` as an unexpected exit revived by the service manager."""
...
def _consume_pid_marker_for_self(path: Path, *, ttl_s: int) -> bool:
    ...
    # cross-NASTECH_HOME --replace while ignoring a marker accidentally written into another
    ...
    target_home = record.get("target_nastech_home")
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_nastech_home(target_home, our_home):
            return False
    else:
        replacer_home = record.get("replacer_nastech_home")
        if replacer_home is not None and not _same_nastech_home(replacer_home, our_home):
            return False
...
def write_takeover_marker(
    ...
    try:
        marker_home = _canonical_nastech_home(target_home or _get_process_nastech_home())
        ...
        return _write_marker(_get_takeover_marker_path(marker_home), {
            ...
            "target_nastech_home": str(marker_home), "replacer_pid": os.getpid(),
            "replacer_nastech_home": str(_canonical_nastech_home(_get_process_nastech_home())),
...
def _validated_scoped_lock_gateway_owner(record: dict[str, Any]) -> Optional[tuple[int, int, Path]]:
    ...
    raw_home = record.get("nastech_home")
    if (
        ...
        return None
    target_home = _canonical_nastech_home(raw_home)
    ...
    pid_record_home = pid_record.get("nastech_home")
    if (
        ...
        or not _same_nastech_home(pid_record_home, target_home)
    ):
        return None
