...
from nastech_constants import get_nastech_home, _get_platform_default_nastech_home
...
def _get_starts_log_path() -> Path:
    ...
    collision."""
    return get_nastech_home() / "gateway-starts.log"
...
def _get_process_nastech_home() -> Path:
    """Return the process-level NASTECH_HOME, skipping context-local overrides.
    ...
    ``get_nastech_home()`` honors ``_NASTECH_HOME_OVERRIDE`` contextvar used for
...
def _canonical_nastech_home(path: Path | str) -> Path:
    """Return a stable absolute NASTECH_HOME path for persisted identity data."""
...
def _same_nastech_home(left: Path | str, right: Path | str) -> bool:
    ...
    return os.path.normcase(str(_canonical_nastech_home(left))) == os.path.normcase(
        str(_canonical_nastech_home(right))
    )
...
def recorded_gateway_home_conflicts(
    ...
    *,
    ...
    """True when a persisted gateway record names a DIFFERENT NASTECH_HOME.
    ...
    gateway (its ``nastech_home`` stamp records the real owner). Any
    ...
    Legacy records without a ``nastech_home`` stamp return False — they prove
...
# Mirrors nastech_cli.profiles._PROFILE_ID_RE — duplicated here because gateway
# identity code must stay import-light (nastech_constants + stdlib only).
_PROFILE_LABEL_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")


def _profile_label_for_home(home: Path | str) -> Optional[str]:
    """Best-effort profile label for a NASTECH_HOME path.

    Returns the profile name for ``<root>/profiles/<name>`` layouts (both
    ``~/.nastech/profiles/coder`` and Docker ``/opt/data/profiles/coder``),
    ...
    try:
        ...
        if _same_nastech_home(canonical, get_default_nastech_root()):
            return "default"
    except Exception:
        pass
    try:
        if _same_nastech_home(canonical, _get_platform_default_nastech_home()):
            return "default"
    except Exception:
        pass
    return None
...
def scoped_lock_owner_label(record: Optional[dict[str, Any]]) -> Optional[str]:
    ...
    NASTECH_HOME profile than the caller.  Prefers the explicit ``profile``
    field stamped by :func:`acquire_scoped_lock`; falls back to inferring the
    label from the persisted ``nastech_home`` for locks written before the
...
def _get_pid_path() -> Path:
    ...
    home = _get_process_nastech_home()
...
# Reject epoch values before 2000-01-01T00:00:00Z: nothing in Nastech' lifetime
...
def _gateway_command_subcommand(command: str | None) -> str | None:
    """Return the Nastech gateway lifecycle subcommand from a command line.
    ...
    test also matched ``nastech_cli.main gateway status`` and even unrelated
    ...
    (``"C:\\Program Files\\...\\nastech-gateway.exe"``) survive, and strips
    ``--profile``/``-p`` selectors from anywhere in argv -- Nastech's
    ...
    for token in tokens:
        ...
        if basename in ("nastech-gateway", "nastech-gateway.exe"):
            return "run"

    joined = " ".join(tokens)
    has_gateway_entry = (
        "nastech_cli.main" in joined
        or "nastech_cli/main.py" in joined
        or any(t.rsplit("/", 1)[-1] in ("nastech", "nastech.exe") for t in tokens)
    )
    ...
    for i, token in enumerate(filtered):
        if token != "gateway":
            continue
        if i + 1 >= len(filtered):
            return "run"  # bare `nastech gateway` defaults to `run`
        return filtered[i + 1]
    return None
...
def looks_like_gateway_runtime_command_line(command: str | None) -> bool:
    ...
    use this broader matcher only when validating Nastech-owned runtime records
...
def _profile_name_for_home(profile_home: Path) -> Optional[str]:
    """Return the profile id a NASTECH_HOME directory represents, or None.

    A named profile's home is ``<root>/profiles/<name>`` (immediate parent is
    ``profiles``).  The root/default home (``~/.nastech`` or ``$NASTECH_HOME``)
...
def _command_line_belongs_to_profile(command: str, profile_home: Path) -> bool:
    ...
    Mirrors ``nastech_cli.gateway._matches_current_profile`` so the dashboard's
    ...
    explicit ``NASTECH_HOME=<path>``) on its argv; the default/root gateway runs
    bare with no profile flag.
    """
    # Normalize separators before the substring match: on Windows,
    # str(Path) renders backslashes while a NASTECH_HOME= value on the argv
    ...
    if profile_name is not None and profile_name != "default":
        ...
        return (
            ...
            or f"nastech_home={home_lc}" in command_lc
        )
    ...
    # a non-matching explicit NASTECH_HOME= on the argv. NASTECH_HOME is usually
    ...
    if "--profile " in command_lc or " -p " in command_lc:
        return False
    if "nastech_home=" in command_lc and f"nastech_home={home_lc}" not in command_lc:
        return False
    return True
...
def _build_pid_record() -> dict:
    return {
        ...
        # NASTECH_HOME-local.  Persist the owning gateway's process home so an
...
def _get_code_identity_fields() -> dict[str, Any]:
    ...
    ``nastech_cli``; the helper itself is cached per process. A gateway
    keeps serving the module versions it imported at startup, so stamping
    the identity into ``gateway_state.json`` lets `nastech update` (and the
...
def _pid_record_belongs_to_current_profile(
    ...
    """Return True when the PID record's ``nastech_home`` matches the current process.

    PID records written by ``_build_pid_record()`` include the gateway's
    ``nastech_home`` at write time. If a profile gateway was started (or recorded)
    under a different NASTECH_HOME, the record belongs to a different profile
    ...
    Records that predate the ``nastech_home`` field (pre-#74872 gateways) are
    ...
    if not record_home:
        # Records without nastech_home belong to a pre-#74872 gateway;
        # accept them conservatively.
        return True
    return _same_nastech_home(record_home, _get_process_nastech_home())
...
def write_runtime_status(
    *,
    ...
    if served_profiles is not _UNSET:
        ...
        # for a single-profile gateway. Lets `nastech status` show per-profile
...
def read_runtime_status(path: Optional[Path] = None) -> Optional[dict[str, Any]]:
    ...
    can do so without mutating ``NASTECH_HOME`` in-process.  Defaults to
...
def resolve_gateway_liveness(
    *,
    ...
    passes its ``nastech_cli.web_server`` bindings so the long-standing
...
def get_runtime_status_running_pid(
    ...
    *,
    ...
    NASTECH_HOME.  Pass it when validating *another* profile's state file (the
    ...
    # verify the persisted record's nastech_home matches the current process.
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    # is inferable; readers fall back to deriving it from nastech_home.
...
def _get_takeover_marker_path(nastech_home: Optional[Path] = None) -> Path:
    ...
    ``nastech_home`` is supplied only for a verified cross-home handoff.  The
    target process always consumes the marker from its own process-level home.
    """
    home = nastech_home or _get_process_nastech_home()
    return _canonical_nastech_home(home) / _TAKEOVER_MARKER_FILENAME
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # TARGET home.  That permits a deliberate cross-NASTECH_HOME --replace while
    ...
    if target_home is not None:
        if not isinstance(target_home, str) or not _same_nastech_home(
            target_home, our_home
        ):
            return False
    else:
        replacer_home = record.get("replacer_nastech_home")
        if replacer_home is not None and not _same_nastech_home(
...
def write_takeover_marker(
    ...
    *,
    ...
    gateway's NASTECH_HOME rather than the replacer's.  Same-home callers omit
    ...
    """
    try:
        marker_home = _canonical_nastech_home(
            target_home or _get_process_nastech_home()
        )
        ...
        record = {
            ...
            "target_nastech_home": str(marker_home),
            "replacer_pid": os.getpid(),
            "replacer_nastech_home": str(
                _canonical_nastech_home(_get_process_nastech_home())
            ),
...
def _validated_scoped_lock_gateway_owner(
    ...
    the lock record, the target NASTECH_HOME's gateway PID record, and the live
    ...
    if not isinstance(pid_record_home, str) or not _same_nastech_home(
