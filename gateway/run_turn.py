...
class GatewayTurnMixin:
    ...
    def _resolve_session_agent_runtime(
        ...
        # Provider resolved but no model.default (`nastech auth add` without `nastech model`): use the
    ...
    async def _hmwa_acquire_turn_lease(self, _quick_key, run_generation, session_entry, _session_env_tokens):
        ...
        try:
            _lease_token = await _lease_registry.acquire(
                ...
                timeout=_float_env("NASTECH_TURN_LEASE_TIMEOUT", DEFAULT_LEASE_WAIT),
            )
        ...
        if _hyg_aborted:
            ...
            if not _hyg_fence_cancelled:
                ...
                await self._hmwa_hygiene_notify(
                    ...
                    "run `nastech doctor` on the host.",
    ...
    async def _hmwa_hygiene_detached_attempt(
        ...
        try:
            ...
            attempt.future = loop.run_in_executor(
                ...
                # secret scope / NASTECH_HOME override live in ContextVars, and a bare run_in_executor worker
    ...
    async def _hmwa_first_contact_notes(self, source, history, turn_sidecar_notes):
        ...
        from gateway.run import _nastech_home, _home_target_env_var, _load_gateway_config
        if history:
            return
        ...
        if not home_env:
            with suppress(Exception):
                ...
                if prof and prof != "default" and _lgc().get_home_channel(source.platform):
                    home_env = "set"
        if not home_env:
            # Slack routes every command through the parent `/nastech`; bare `/sethome` would fail.
            sethome_cmd = "/nastech sethome" if source.platform == Platform.SLACK else "/sethome"
            await self._deliver_platform_notice(
                source, f"📬 No home channel is set for {platform_name.title()}. "
                f"A home channel is where Nastech delivers cron job results and cross-platform "
    ...
    def _hmwa_apply_message_timestamp(self, event, message_text):
        ...
        try:
            from nastech_time import get_timezone as _get_evt_tz
    ...
    # Chat-side next steps keyed by HTTP status; Nastech commands only (/login is the gateway's own
    # sign-in, `nastech auth add <provider>` the host equivalent).
    _STATUS_HINTS = {
        401: (" Your sign-in to the AI model service has expired or the API key is wrong. "
              "Use /login here, or run `nastech auth add <provider>` on the host."),
    ...
    async def _hmwa_agent_error_reply(self, e, event, source, session_entry, session_key, prepared):
        ...
        return self._hmwa_add_failed_turn_notice(
            ...
            "Technical details are in the gateway log (`nastech logs`).",
    ...
    def _standalone_launch_scope():
        ...
        return launch_profile_runtime_scope(get_process_nastech_home())
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    ...
                    "configured. Use /login, or run `nastech setup` on the host.",
    ...
    async def _execute_mcp_reload(self, event: MessageEvent) -> str:
        ...
        if multiplex and not get_nastech_home_override():
        ...
        try:
            ...
            # a browser OAuth flow either: an expired token parks with a `nastech mcp login` hint.
    ...
    async def _run_agent_via_proxy(
        ...
        """Forward the message to a remote Nastech API server instead of running a local AIAgent.
        ...
        # OpenAI chat format. The remote keeps continuity via X-Nastech-Session-Id; send the current
        ...
        body = {"model": "nastech-agent", "messages": api_messages, "stream": True}
    ...
    def _run_agent_display_settings(self, source: SessionSource) -> "GatewayRunner._RunAgentDisplay":
        ...
        progress_mode, _tool_progress_explicit = resolve_tool_progress(
            user_config, platform_key, os.getenv("NASTECH_TOOL_PROGRESS_MODE"),
        )
        ...
        # "log" mode: tool calls go to ~/.nastech/logs/tool_calls.log instead of the chat. Gateway-only.
    ...
    async def _run_agent_write_tool_log(self, log_queue: Any) -> None:
        ...
        tool_logger = logging.getLogger(f"nastech.tool_calls.{id(log_queue)}")
    ...
    def _run_agent_start_turn_worker(self, turn_ctx: TurnContext, run_sync: Callable[[], Any]) -> "GatewayRunner._RunAgentWorker":
        ...
        *Inactivity* timeout (agent.gateway_timeout / NASTECH_AGENT_TIMEOUT, env wins; 0 = unlimited),
        ...
        _agent_timeout, _agent_warning = (
            v if v > 0 else None
            for v in (_float_env("NASTECH_AGENT_TIMEOUT", 1800), _float_env("NASTECH_AGENT_TIMEOUT_WARNING", 900))
        )
    ...
    def _run_agent_evict_on_fallback(self, turn_ctx: TurnContext) -> None:
        ...
        with suppress(Exception):
            from nastech_cli.model_normalize import _AGGREGATOR_PROVIDERS, normalize_model_for_provider
    ...
    async def _run_agent_notify_long_running(
        ...
        Interval: agent.gateway_notify_interval / NASTECH_AGENT_NOTIFY_INTERVAL (default 180s; 0 or
        ...
        _NOTIFY_INTERVAL = _float_env("NASTECH_AGENT_NOTIFY_INTERVAL", 180)
