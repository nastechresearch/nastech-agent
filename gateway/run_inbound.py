...
class GatewayInboundMixin:
    ...
    async def _hm_report_ignored_dm(self, source: SessionSource) -> None:
        ...
        hint = unauthorized_owner_hint(
            platform_name, source.user_id, source.user_name or "", nastech_home=display_nastech_home(),
        )
    ...
    async def _hm_admit_event(
        ...
        # spawning context, which may carry ANOTHER message's NASTECH_SESSION_* ContextVars; until
        ...
        if (
            ...
            and not is_internal
            and not getattr(event, "_nastech_startup_restore_replay", False)
        ):
    ...
    def _hm_estop_turn_allowed(self, event: "MessageEvent", source: SessionSource) -> bool:
        ...
        with suppress(Exception):
            ...
            if _estop_cmd:
                from nastech_cli.commands import resolve_command as _resolve_estop_cmd
    ...
    def _hm_estop_gate(
        ...
        """Global emergency-stop (`nastech pause`) notice when this turn must be blocked, else None.
    ...
    def _hm_write_update_response(response_text: str) -> Optional[str]:
        ...
        try:
            ...
            (_nastech_home / ".update_prompt.json").unlink(missing_ok=True)
        except OSError as e:
            return str(e)
        return None
    ...
    def _hm_busy_telegram_grace_queue(
        ...
        _grace = float(os.getenv("NASTECH_TELEGRAM_FOLLOWUP_GRACE_SECONDS", "3.0"))
    ...
    async def _hm_resolve_command(
        ...
        from nastech_cli.commands import is_gateway_known_command, resolve_command as _resolve_cmd
            ...
            if _plat and (_skill_name or extra_keys):
                ...
                if _disabled_extra:
                    return (
                        ...
                        f"Enable them with: `nastech skills config`"
                    )
    ...
    async def _dispatch_plugin_message_injection(
        ...
        await adapter.handle_message(MessageEvent(
            ...
            metadata={
                "nastech_plugin_id": plugin_id, "nastech_plugin_injection": True,
