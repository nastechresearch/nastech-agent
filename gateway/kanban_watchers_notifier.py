...
def _kbc():
    from nastech_cli import kanban_db_connect
    return kanban_db_connect


def _kbn():
    from nastech_cli import kanban_db_notify
...
def _warn_anchorless_thread_sub_once(sub: dict, platform: str) -> None:
    ...
    logger.warning(
        ...
        "profile route; it will not be delivered. Re-subscribe with `nastech kanban notify-subscribe ... "
...
class _Collector:
    ...
    def collect(self) -> list[dict]:
        ...
        # NASTECH_KANBAN_DB pins the board path.
        kb = self.kb
...
def _fmt_gave_up(ev, n) -> tuple:
    ...
    return (
        f"⛔ {n.head} is now blocked: {count}{last}. Fix the cause, then `nastech kanban unblock "
        f"{n.task_id}` (or `nastech kanban reassign {n.task_id}`). Logs: `nastech kanban log {n.task_id}`.",
        None, None,
    )
...
class _KanbanNotification:
    ...
    def _served_wake_profile(self) -> Optional[str]:
        ...
        ``None`` keeps the historical path: a standalone ``nastech -p <name>`` gateway owns its own
