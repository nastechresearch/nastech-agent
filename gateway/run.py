...
# nastech_bootstrap must be the very first import (UTF-8 stdio on Windows; no-op on POSIX).
try:
    import nastech_bootstrap  # noqa: F401
except ModuleNotFoundError:
    pass  # a partial ``nastech update`` can leave the bootstrap unregistered; only Windows UTF-8 stdio suffers
...
from nastech_cli.config import _is_ssh_remote_tilde_cwd, cfg_get
...
def _hygiene_compression_timeout_message(
    *, total_exhausted: bool, elapsed: float, idle_timeout: float, progress_observed: bool) -> str:
    ...
    model config, so the copy names /compress, /new and `nastech doctor`, never a config key or the
    ...
    if total_exhausted:
        return lead
    return lead + " If this keeps happening, run `nastech doctor` on the host."
...
async def run_codex_hygiene_compaction(
    ...
    * Evicting the cached live agent afterwards destroys the only real context: the next turn spawns an
    EMPTY thread and the model starts blank while Nastech still mirrors a full history (abrupt amnesia — the
    user-facing damage documented on #73503).
    """
    mode = str(auto_mode or "native").lower()
    if mode not in {"native", "nastech", "off"}:
        mode = "native"
    if mode != "nastech":
    ...
    # copy_context carries profile secret scope / NASTECH_HOME override (executors don't propagate ContextVars).
...
def _ensure_windows_gateway_venv_imports() -> None:
    """Make detached Windows gateway runs see the Nastech venv packages.
...
# slash command the chat user can run; raw provider text stays in the gateway log (`nastech logs`).
_PROVIDER_ERROR_REPLIES = (
    (_GATEWAY_AUTH_ERROR_RE, "⚠️ Sign-in to the AI model service failed. Use /login to sign in again, "
                             "or ask whoever runs this bot to run `nastech doctor` on the host."),
    ...
    (_GATEWAY_CONNECTION_ERROR_RE, "⚠️ The AI model service isn't reachable right now — the configured model "
                                   ...
                                   "if it persists, run `nastech doctor` on the host."))
...
def _gateway_provider_error_reply(text: str) -> str:
    ...
    return (
        ...
        "models. Details are in the gateway log (`nastech logs`).")
...
def _telegramize_command_mentions(text: str, platform: Any) -> str:
    ...
    from nastech_cli.commands_platforms import _sanitize_telegram_name
...
def _startup_restore_drain_timeout_secs() -> float:
    ...
    """
    return _float_env("NASTECH_STARTUP_RESTORE_DRAIN_TIMEOUT", _STARTUP_RESTORE_DRAIN_TIMEOUT_SECS_DEFAULT)
...
def _startup_warmup_timeout_secs() -> float:
    ...
    return _float_env("NASTECH_STARTUP_WARMUP_TIMEOUT", _STARTUP_WARMUP_TIMEOUT_SECS_DEFAULT)
...
def _build_gateway_agent_history(
    ...
    from nastech_time import get_timezone as _get_msg_tz
...
# Load ~/.nastech/.env first: user-managed env files must override stale shell exports on restart.
...
def _reload_runtime_env_preserving_config_authority() -> None:
    ...
    stale NASTECH_MAX_ITERATIONS wins). Multiplex never reloads .env globally: secrets come from the
    ...
    if not is_multiplex_active():
        load_nastech_dotenv(
            nastech_home=_nastech_home, project_env=Path(__file__).resolve().parents[1] / '.env')
    _bridge_max_turns_from_config(_nastech_home)
...
def _bridge_max_turns_from_config(home: "Path") -> None:
    ...
    nastech_state reads the routed profile's ``sessions.*`` from its own config under scope."""
...
def _current_max_iterations() -> int:
    ...
    ``int()`` crash. A routed profile (NASTECH_HOME override, multiplexed turns) reads ITS
    ``agent.max_turns`` straight from config: the ``NASTECH_MAX_ITERATIONS`` bridge is one process-wide
    slot holding the launch profile's value, so every secondary would inherit the default's budget."""
    _reload_runtime_env_preserving_config_authority()
    from nastech_cli.config import resolve_turn_limit as _resolve_turn_limit
    ...
    return _resolve_turn_limit(os.getenv("NASTECH_MAX_ITERATIONS"))
...
def _cron_tick_profile_homes(config: object) -> list[tuple[str, "Path"]]:
    ...
    NASTECH_HOME). Adapter startup already skips ``active``."""
    from nastech_cli.profiles import get_active_profile_name, get_profile_dir
...
def _load_profile_secret_scope(profile_home: "Path") -> dict:
    ...
    from nastech_cli.env_loader import hydrate_profile_secret_sources

    home_token = set_nastech_home_override(str(profile_home))
    try:
...
def _profile_runtime_scope(
    ...
    ``set_nastech_home_override`` is a contextvar (reaches the agent worker via ``copy_context()``);
...
def load_gateway_config_for_runner() -> "GatewayConfig":
    ...
    See #64674.
    """
    from nastech_cli.gateway_multiplex_mode import log_multiplex_decision, resolve_multiplex_mode
...
async def _discover_gateway_mcp_tools(config: object) -> None:
    ...
    ``mcp_servers`` from ``get_nastech_home()``'s config, so an unscoped call only connects the launch
    ...
    actionable ``nastech mcp login`` warning instead of opening an authorize tab.
    """
...
def _platform_has_bot_credential(platform: "Platform", platform_config: "PlatformConfig") -> bool:
    ...
    # ~/.nastech/.env, so an env fallback would report "has credential" for every Matrix config on the box —
...
from nastech_cli.config_defaults import DEFAULT_CONFIG as _DEFAULT_CONFIG
os.environ["NASTECH_TURN_LEASE_TIMEOUT"] = str(_DEFAULT_CONFIG["agent"]["gateway_turn_lease_timeout"])
...
_AGENT_ENV_BRIDGE = {
    "gateway_timeout": "NASTECH_AGENT_TIMEOUT",
    "gateway_turn_lease_timeout": "NASTECH_TURN_LEASE_TIMEOUT",
    "gateway_timeout_warning": "NASTECH_AGENT_TIMEOUT_WARNING",
    "gateway_notify_interval": "NASTECH_AGENT_NOTIFY_INTERVAL",
    "session_stall_timeout": "NASTECH_SESSION_STALL_TIMEOUT",
    # Internal bridge only — config.yaml (agent.reconnect_attention_after) is the documented setting.
    "reconnect_attention_after": "NASTECH_RECONNECT_ATTENTION_AFTER_SECONDS",
    "restart_drain_timeout": "NASTECH_RESTART_DRAIN_TIMEOUT",
    "cron_drain_timeout": "NASTECH_CRON_DRAIN_TIMEOUT",
    "gateway_auto_continue_freshness": "NASTECH_AUTO_CONTINUE_FRESHNESS",
    "gateway_startup_restore_drain_timeout": "NASTECH_STARTUP_RESTORE_DRAIN_TIMEOUT",
    "gateway_startup_warmup_timeout": "NASTECH_STARTUP_WARMUP_TIMEOUT"}
# config-authoritative knobs for the session-search index (env stays the cross-process carrier).
_SESSIONS_ENV_BRIDGE = {"cjk_fts": "NASTECH_CJK_FTS", "search_slow_ms": "NASTECH_SEARCH_SLOW_MS"}
_DISPLAY_ENV_BRIDGE = {
    "busy_input_mode": "NASTECH_GATEWAY_BUSY_INPUT_MODE",
    "busy_text_mode": "NASTECH_GATEWAY_BUSY_TEXT_MODE",
    "busy_ack_enabled": "NASTECH_GATEWAY_BUSY_ACK_ENABLED"}
...
def _bridge_max_turns_to_env(agent_cfg: Any) -> None:
    ...
    if raw is not None:
        os.environ["NASTECH_MAX_ITERATIONS"] = str(raw)
    elif "NASTECH_MAX_ITERATIONS" in os.environ:
        del os.environ["NASTECH_MAX_ITERATIONS"]
...
def _bridge_config_to_env(_cfg: dict) -> None:
    ...
    # (e.g. NASTECH_MAX_ITERATIONS=60 written by an old `nastech setup` run) silently shadow the user's
    ...
    if (isinstance(_display_cfg, dict) and "busy_steer_ack_enabled" in _display_cfg
            and "NASTECH_GATEWAY_BUSY_STEER_ACK_ENABLED" not in os.environ):
        os.environ["NASTECH_GATEWAY_BUSY_STEER_ACK_ENABLED"] = str(_display_cfg["busy_steer_ack_enabled"])
    ...
    if isinstance(_security_cfg, dict) and _security_cfg.get("redact_secrets") is not None:
        os.environ["NASTECH_REDACT_SECRETS"] = str(_security_cfg["redact_secrets"]).lower()
    # Media policy uses the shared bridge so standalone entrypoints (`nastech cron run`) match.
    ...
    if isinstance(_gateway_cfg, dict):
        ...
        if ("platform_connect_timeout" in _gateway_cfg
                and not os.environ.get("NASTECH_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()):
            os.environ["NASTECH_GATEWAY_PLATFORM_CONNECT_TIMEOUT"] = str(_gateway_cfg["platform_connect_timeout"])
...
os.environ["NASTECH_QUIET"] = "1"  # gateway runs quiet: no debug output, cwd used directly

# NASTECH_EXEC_ASK is set in start_gateway(), NOT at import: CLI tools importing this module must not
...
def _resolve_runtime_agent_kwargs_for_provider(provider: str, target_model: Optional[str] = None) -> dict:
    ...
    from nastech_cli.runtime_provider import resolve_runtime_provider, format_runtime_provider_error
    try:
...
def _deep_merge_request_overrides(base: Optional[dict], override: Optional[dict]) -> dict:
    ...
    from nastech_cli.config import _deep_merge
...
def _try_resolve_fallback_provider() -> dict | None:
    ...
    try:
        ...
        for entry in fb_list:
            try:
                from nastech_cli.fallback_config import effective_runtime_provider, resolve_entry_api_key
                runtime = resolve_runtime_provider(
...
def _gateway_config_home() -> Path:
    """Return the Nastech home that gateway config reads should use."""
    override = get_nastech_home_override()
    return Path(override) if override else _nastech_home
...
def _load_gateway_config(config_path: "Path | None" = None) -> dict:
    ...
    (``_nastech_home`` monkeypatches apply); multiplexers pass a path.
    """
...
def _resolve_nastech_bin() -> Optional[list[str]]:
    """Nastech update/restart argv: the running interpreter's ``python -m nastech_cli.main``
    (exactly this install), else ``nastech`` on PATH, else None. The module argv must win: a
    PATH-first lookup lets an attacker-planted ``nastech`` shadow the running install when
...
_RECONNECT_ATTENTION_AFTER_SECONDS = _float_env("NASTECH_RECONNECT_ATTENTION_AFTER_SECONDS", 7200)
...
_BUILTIN_ADAPTERS: dict[Platform, tuple[str, str, str, str]] = {
    Platform.WHATSAPP_CLOUD: ("whatsapp_cloud", "WhatsAppCloudAdapter", "check_whatsapp_cloud_requirements",
                              "WhatsApp Cloud: aiohttp/httpx missing — reinstall nastech-agent"),
...
class GatewayRunner(
    ...
    def _init_session_db(self) -> None:
        ...
        try:
            ...
            # WARNING (not DEBUG) so it lands in errors.log; else an NFS NASTECH_HOME silently loses /resume etc.
        ...
        # restart of the day (the ``nastech update`` one) looked hung and failed fleet verification.
    ...
    def _open_session_db_for_active_scope(self, raise_on_error: bool = False) -> Any:
        ...
        ``SessionDB()`` reads the context-local NASTECH_HOME; one handle cached per path. Construction
        ...
        resolves ``_default_db_path()`` at call time through the context-local NASTECH_HOME override
        ...
        #88235 broadcast.
        """
        from nastech_state import AsyncSessionDB, _default_db_path
        ...
        def _open():
            ...
            if borrowed is not None:
                ...
                wrapper.__dict__["_nastech_borrowed_handle"] = True
                return wrapper
    ...
    def close_all_session_db_handles(self) -> None:
        ...
        """
        def _close(db) -> None:
            if getattr(db, "__dict__", {}).get("_nastech_borrowed_handle"):
                return
            ...
            # instead (#90837).
            from nastech_state_registry import release_or_close
            try:
    ...
    def _update_runtime_status(self, gateway_state: Optional[str] = None, exit_reason: Optional[str] = None) -> None:
        # ``active_work`` names each unit only while draining — that is when an observer (``nastech
    ...
    def _thread_metadata_for_source(
        ...
        if profile and metadata is not None:
            ...
            metadata["nastech_profile"] = profile
        return metadata
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(
                    max_workers=10, thread_name_prefix="nastech-gateway")
    ...
    def _resolve_profile_home_for_source(self, source: SessionSource) -> "Path":
        """Resolve which profile's NASTECH_HOME serves this source: the pinned identity's runtime
        ...
        from nastech_cli.profiles import get_active_profile_name, get_profile_dir, profile_exists
        ...
        try:
            ...
            if explicit_profile and not profile_exists(name):
                logger.warning(
                    ...
                    "falling back to global NASTECH_HOME",
            ...
            logger.warning(
                ...
                "falling back to global NASTECH_HOME: %s",
...
def _run_planned_stop_watcher(
    ...
    ``add_signal_handler``, so ``nastech gateway stop`` would never drain). Runs everywhere; on POSIX
    ...
    standard signal-driven shutdown path never runs when ``nastech gateway stop`` signals the gateway. The
...
def _housekeeping_deferred_fts_retry() -> None:
    ...
    from nastech_state_registry import borrow_live_shared_session_dbs
...
def _drain_restart_safe_cron_deliveries(adapters, loop, runner=None) -> None:
    ...
    for profile_name, profile_home in _handoff_watch_scopes(runner):
        ...
        if profile_adapters is None:
            continue
        with _profile_runtime_scope(profile_home or get_nastech_home()):
...
def _start_gateway_housekeeping(
    ...
    chores: list[tuple[int, str, Any]] = [
        ...
        # ``nastech gateway status`` / ``/api/status`` warn when it ages past 2x ``interval`` with the
...
def _replace_target_belongs_to_other_profile(existing_pid: int) -> bool:
    ...
    by exact PID + start-time; live argv can never PROVE ownership (no NASTECH_HOME), it is only a
    ...
    # (no service supervisor — e.g. `nastech gateway run` on Windows) is at risk: its PID never joins the
    ...
    try:
        from gateway.status import (
            _get_pid_path, _get_process_nastech_home, _get_process_start_time, _pid_from_record,
            _read_pid_record, _record_looks_like_gateway, _read_process_cmdline, _same_nastech_home)
        our_home = _get_process_nastech_home()
        ...
        if _get_process_start_time(existing_pid) != recorded_start:
            return refuse("pid record start-time does not match the live process %s (stale/PID-reuse record).",
                          existing_pid)
        recorded_home = record.get("nastech_home")
        if not isinstance(recorded_home, str) or not recorded_home.strip():
            return refuse("pid record predates nastech_home stampings; ownership of PID %s unprovable.",
                          existing_pid)
        if not _same_nastech_home(recorded_home, our_home):
            return refuse("pid record belongs to a different NASTECH_HOME (%s, ours %s). Remove the stale PID "
                          "record or stop the owning profile explicitly.", recorded_home, our_home,
                          level=logging.ERROR)
        # Argv never proves ownership; an explicit contradicting --profile / NASTECH_HOME= still refuses.
        ...
        if live_cmdline and _looks_like_profile_conflict_from_cmdline(live_cmdline, our_home):
            return refuse("target PID %s command line explicitly advertises a different profile than "
                          "NASTECH_HOME %s.", existing_pid, our_home, level=logging.ERROR)
        return False
    except Exception:
...
def _looks_like_profile_conflict_from_cmdline(command: str, our_home) -> bool:
    ...
    def _env_home_value() -> Optional[str]:
        """NASTECH_HOME=<path> env-style assignment on the argv, token-exact."""
        prefix = "NASTECH_HOME="
    ...
    for flag in ("--profile", "-p"):
        ...
        if profile_name is None or profile_name == "default" or value != profile_name:
            return True
    home_value = _flag_value("--nastech-home") or _env_home_value()
...
async def _start_gateway_replace_existing_instance(existing_pid: int, replace: bool) -> bool:
    """Handle a live gateway PID under this NASTECH_HOME: replace it (``--replace``) or refuse.
    ...
    if _replace_target_belongs_to_other_profile(existing_pid):
        from gateway.status import _get_process_nastech_home
        logger.error(
            "Refusing --replace: PID %d cannot be proven to belong "
            "to this profile's gateway (NASTECH_HOME %s). Remove the "
            "stale PID record or stop the owning profile explicitly.",
            existing_pid, _get_process_nastech_home())
        return False
...
def _start_gateway_configure_logging(verbosity: Optional[int]) -> None:
    ...
    def _security_audit() -> None:
        ...
        from nastech_cli.security_audit_startup import log_startup_security_warnings

        def _raw_cfg():
            from nastech_cli.config import read_raw_config
            return read_raw_config()

        log_startup_security_warnings(nastech_home=_nastech_home, config=_best_effort(_raw_cfg))
        ...
        if _shutdown_ctx is not None:
            ...
            def _diagnostic() -> None:
                ...
                spawn_async_diagnostic(
                    _nastech_home / "logs" / "gateway-shutdown-diag.log", _shutdown_ctx["signal"], timeout_seconds=5.0)
...
async def _start_gateway_start_control_socket(runner):
    ...
    try:
        ...
        # becomes the authoritative gateway for its NASTECH_HOME, so from here on "does a socket answer?" is
        ...
        def _rescan_profiles_handler() -> dict:
            """``nastech profile create/delete`` asks the multiplexer to reconcile ``profiles/`` now
    ...
    if not isinstance(cron_provider, InProcessCronScheduler):
        ...
        if not _has_api_server:
            logger.warning(
                ...
                "the gateway through its supervisor (`nastech gateway "
...
async def _start_gateway_shutdown_tail(
    ...
    def _stop_keepalive() -> None:
        from nastech_cli.nastech_auth_keepalive import stop_nastech_auth_keepalive
        stop_nastech_auth_keepalive()
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Duplicate-instance guard scoped to NASTECH_HOME; distinct-home multi-profile setups coexist.
    ...
    # Windows has no add_signal_handler, so `nastech gateway stop`'s SIGTERM would never drain; poll the
    # planned-stop marker (written BEFORE the kill) instead. Runs everywhere so masked-SIGTERM drains.
    # Windows fallback: asyncio.add_signal_handler raises NotImplementedError on Windows, so `nastech gateway
    ...
    # sessions are silently lost across restarts (issue #33778). The fix is a marker-polling thread: `nastech
    ...
    _best_effort(_start_keepalive, "Nastech auth keepalive did not start: %s")
...
def _guard_corrupt_user_config() -> None:
    ...
    named. Same policy and escape hatch (``NASTECH_IGNORE_USER_CONFIG=1``) as ``nastech_cli/main.py``."""
    from nastech_cli.config import InvalidUserConfigError, require_parseable_user_config
...
def main():
    ...
    # Advertise the harness to children (mirrors _advertise_agent_env in nastech_cli/main.py, inlined to
    # avoid its startup side effects). Value must equal registry id ``nastech-agent`` exactly.
    ...
    def _arm_watchdog() -> None:
        ...
        from nastech_startup_watchdog import arm_startup_watchdog
...
def _exit_after_graceful_shutdown(exit_code: int) -> None:
    ...
    def _drain_logs() -> None:
        ...
        from nastech_logging import drain_log_queue
