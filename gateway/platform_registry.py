...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``nastech setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``nastech gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # `nastech` invocation, so the import happens only when a lookup asks for it.
