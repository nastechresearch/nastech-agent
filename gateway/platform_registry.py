...
class PlatformEntry:
    ...
    # status displays (``nastech setup``, ``nastech status``, the dashboard
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved NASTECH_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `nastech`
        # invocation -- including plain `nastech chat`, which never touches any
        ...
        # `nastech setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``nastech setup``/``gateway status``, channel
