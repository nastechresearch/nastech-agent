...
from nastech_cli.status_report import build_status_fields
...
HISTORY_UNREADABLE = ("⚠️ I can't read this conversation's history right now (your earlier messages "
                      "exist but cannot be loaded). Run `nastech doctor --fix` on the host, or use /new "
                      "to start fresh.")
...
class GatewayStatusCommandsMixin:
    ...
    async def _handle_status_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            free_tier_active = await self._run_in_executor_with_context(
                lambda: resolve_provider("auto") == "nastech" and guest_carries_inference()
            )
            if free_tier_active:
    ...
    async def _handle_topup_command(self, event: MessageEvent) -> str:
        """Handle /topup -- show the Nastech balance and hand off to the portal. Does NOT charge, confirm,
        ...
        lines = ["💳 **Nastech balance**"] + [ln for ln in view.balance_lines if not ln.lstrip().startswith("📈")]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # Nastech credits + monthly-grant gauge (shared with CLI/TUI). Gates on "a Nastech account is
        # logged in" — NOT the inference provider — so a Nastech user inferring elsewhere still sees
        # a balance. Fail-open: never break /usage.
        from agent.account_usage import nastech_credits_lines
        credits_lines = await _quiet(lambda: asyncio.to_thread(nastech_credits_lines, markdown=True), [])
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            # Not a bare hop: ``SessionDB()`` resolves ``get_nastech_home()`` at call time, a
