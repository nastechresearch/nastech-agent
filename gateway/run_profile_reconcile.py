...
verb (``nastech_cli/profiles.py`` create/delete fire it through the control socket) and by the supervised
...
def migrate_profile_identity_verb(runner):
    """Build the ``migrate-profile-identity`` control-verb handler for ``nastech profile rename``
...
def purge_profile_identity_verb(runner):
    """Build the ``purge-profile-identity`` control-verb handler for ``nastech profile delete``
