...
def relay_explicitly_disabled() -> bool:
    ...
    try:
        cfg = read_yaml_layers(get_nastech_home())
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # prefix every reply "**Nastech Agent:**" and shadow the connector's
...
def _resolve_relay_identity_token() -> str:
    ...
    Canonical resolver shared by runtime self-provision and ``nastech gateway enroll``.
    Modes, in precedence order:
      ...
      2.  Nastech Portal (default): ``resolve_nastech_access_token()``.
...
def self_provision_relay() -> bool:
    ...
    ``~/.nastech/.env``), so a hosted container re-provisions every boot; the
