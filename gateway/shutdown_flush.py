...
All hooks write atomic JSON payloads under ``<nastech_home>/pending_messages/``:
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active NASTECH_HOME."""
    from nastech_constants import get_nastech_home
    flush_dir = get_nastech_home() / "pending_messages"
...
def spool_dropped_transcript_message(session_id: str, message: Dict[str, Any]) -> Optional[Path]:
    ...
    message under ``<nastech_home>/pending_messages/``), so a runtime cap rotation no longer silently
