...
# node:26 image instead (Nastech pins its toolchain to Node 26 everywhere).
...
# Do not write .pyc files at runtime: /opt/nastech is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/nastech/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when nastech
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# nastech process, the dashboard, and per-profile gateways.
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
...
sys.exit('SQLite FTS5 trigram self-test failed') if db.execute(\"SELECT count(*) FROM docs WHERE docs MATCH 'ast'\").fetchone()[0] != 1 else None; \
db.close()"

# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main nastech process, the dashboard,
...
# 'Nastech WebUI' catalog, NAS compose projects that preserve an old
...
#
# ui-tui/packages/nastech-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/nastech-ink/ ui-tui/packages/nastech-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as nastech-ink above).
...
# lock on the @nastech/ink entry, tripped the TUI launcher's
...
#
# The [otlp] extra contains the SDK/exporter imported by Nastech when Gateway
# Health export is enabled. Collector and observability-backend dependencies
# remain external and are not part of the Nastech production image.
#
# The hindsight memory provider's client (hindsight-client) is baked in
# for the same reason: it lazy-installs into /opt/nastech/.venv at first
...
# NASTECH_LAZY_INSTALL_TARGET when the extra is not present.
#
...
# gives the non-root nastech user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link nastech-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/nastech are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the nastech user (go-w from the --chmod above).
...
# lives outside both /opt/nastech (which operators sometimes bind-mount as a
# checkout) and /opt/data (the mutable NASTECH_HOME volume).
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `nastech dump` reports
...
ARG NASTECH_GIT_SHA=
RUN set -eu; \
    if [ -n "${NASTECH_GIT_SHA}" ]; then \
        printf '%s\n' "${NASTECH_GIT_SHA}" > /opt/nastech/.nastech_build_sha; \
    fi; \
    mkdir -p /etc/nastech; \
    NASTECH_GIT_SHA="${NASTECH_GIT_SHA}" python3 -c 'import json, os, pathlib, tomllib; project = tomllib.loads(pathlib.Path("/opt/nastech/pyproject.toml").read_text(encoding="utf-8"))["project"]; marker = pathlib.Path("/etc/nastech/image-provenance.json"); marker.write_text(json.dumps({"schema": 1, "deployment_kind": "image", "manager": "docker", "image": "nastechresearch/nastech-agent", "version": project["version"], "revision": os.environ.get("NASTECH_GIT_SHA") or None}, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8"); marker.chmod(0o444)'

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-nastech + dashboard.
...
# `exec nastech`. Wired in as cont-init.d/01- so it
...
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/nastech/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-nastech-setup && \
    chmod +x /etc/cont-init.d/01-nastech-setup
...
# bin globally so `docker exec <container> nastech ...` and any
# subprocess that doesn't activate the venv first still find nastech.
#
# /opt/nastech/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/nastech/bin:/opt/nastech/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. nastech
# subcommand vs. no-args), drops to the nastech user via s6-setuidgid,
...
ENTRYPOINT [ "/opt/nastech/docker/entrypoint-dispatch.sh" ]
CMD [ ]
