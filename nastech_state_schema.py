...
``nastech_state.SessionDB``. It defines no ``__init__`` and no state of its
...
``SessionDB.__init__``. It must never import nastech_state (cycle) — shared
module-level constants live in nastech_state_common.
"""
...
# Moved methods logged under the "nastech_state" logger before the split;
...
def schema_read_probe_statements() -> tuple:
    ...
    ``nastech_cli/web_server.py``) run these probes right after a read-only
    ...
    served an empty session list after `nastech update` until the user's
...
class SessionSchemaMixin:
    ...
    def _migrate_trigram_cron_exclusion(self, cursor: sqlite3.Cursor) -> bool:
        ...
        if self._db_has_trigram_tool_calls_projection(cursor):
            ...
            # ``nastech sessions optimize-storage`` path (it recreates the
            ...
            if escalated:
                ...
                if foreign_holders:
                    logger.error(
                        ...
                        "`nastech sessions optimize-storage` with the gateway stopped. "
                        "`nastech doctor` reports this degraded state.",
                        attempts,
                        foreign_holders,
                    )
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        try:
            from nastech_constants import get_nastech_home
            cache_path = get_nastech_home() / "cache" / "schema_columns.json"
    ...
    def _init_schema(self):
        ...
        if row is None:
            ...
            if current_version < 23:
                ...
                # optimization is available. `nastech sessions optimize-storage`
        ...
        if fts5_available:
            ...
            # not yet opted into `nastech db optimize`) must keep its EXISTING
