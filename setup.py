...
Nastech Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``NASTECH_NIX_BUILD=1`` on the
Nastech package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("NASTECH_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "the Nastech Nix derivation sets NASTECH_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``nastech_state``, ``toolsets``...)
