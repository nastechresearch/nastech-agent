...
3. Pip-installed plugins: distributions exposing a ``nastech_agent.plugins``
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$NASTECH_HOME/plugins/`` if it exists.

    This is where ``nastech plugins install`` clones a plugin — flat, one
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``nastech_agent.plugins``
    ...
    * **Provider targets only.** The ``nastech_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$NASTECH_HOME``) keep their
    ...
    try:
        from nastech_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2b. Plugins installed by ``nastech plugins install`` at
          ``$NASTECH_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``nastech_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$NASTECH_HOME`` profile of the
    ...
    # 2b. Plugins installed by ``nastech plugins install`` / the plugin index.
    #     Those clone into $NASTECH_HOME/plugins/<name>/ — flat, NOT under
