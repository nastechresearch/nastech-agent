...
def _bundle(description, extras=()):
    """A `nastech-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _NASTECH_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_NASTECH_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _NASTECH_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `nastech tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``nastech tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _NASTECH_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        [t for t in _NASTECH_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in nastech_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Nastech toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. nastech-acp is the
    ...
    "nastech-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors nastech-cli; `nastech tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "nastech-cron": _bundle("Default cron toolset - same core tools as nastech-cli; gated by `nastech tools`"),
    "nastech-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "nastech-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "nastech-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    ...
    "nastech-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "nastech-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "nastech-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "nastech-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "nastech-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "nastech-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "nastech-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _NASTECH_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "nastech-sms": _bundle("SMS bot toolset - interact with Nastech via SMS (Twilio)"),
    "nastech-webhook": _ts("Webhook toolset - receive and process external webhook events", _NASTECH_WEBHOOK_SAFE_TOOLS),
    "nastech-gateway": _ts(
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _NASTECH_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only nastech-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_NASTECH_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `nastech-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("nastech-"):
        return []
