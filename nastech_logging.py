...
# essentially always in Nastech (TUI, gateway, ``hy_memory`` server, MCP
...
COMPONENT_PREFIXES = {
    ...
    # ``nastech logs --component gateway``.
...
class _ProfileRoutingFileHandler(logging.Handler):
    ...
    def __init__(
        ...
        *,
        ...
        self._nastech_routed_log_path = Path(self.baseFilename)
...
# the module header): when several Nastech processes log to the same file, an
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="nastech-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_nastech_routed_log_path", None) == resolved:
...
def _read_logging_config():
    ...
    try:
        ...
        # reuses the parse nastech_cli.main's early bridge already did (one
        # config.yaml parse per process instead of 3-4). Fall back to a
        # direct parse when nastech_cli.config isn't importable (bare
        # nastech_logging consumers).
        try:
            from nastech_cli.config import read_raw_config as _rrc
            cfg = _rrc() or {}
        except Exception:
