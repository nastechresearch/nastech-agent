...
# in Nastech (TUI, gateway, hy_memory, MCP servers, CLI commands all log) —
...
from nastech_constants import get_config_path, get_nastech_home, mkdir_under_nastech_home
...
def _install_session_record_factory() -> None:
    ...
    def _session_record_factory(*args, **kwargs):
        ...
        try:
            record.nastech_home = str(get_nastech_home().resolve())  # type: ignore[attr-defined]
        except Exception:
            record.nastech_home = ""  # type: ignore[attr-defined]
        return record
...
# Logger name prefixes per component; used by _ComponentFilter and ``nastech logs --component``.
COMPONENT_PREFIXES = {
    ...
    "gateway": ("gateway", "nastech_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("nastech_cli", "cli"),
    "cron": ("cron",),
    "gui": ("nastech_cli.web_server", "nastech_cli.pty_bridge", "nastech_cli.desktop", "tui_gateway", "uvicorn"),
}
...
def setup_logging(
    *,
    ...
    """Configure the Nastech logging subsystem; returns the ``logs/`` directory.
    ...
    log_dir = mkdir_under_nastech_home(home / "logs")
    # A second Nastech home in a process that already logs for another one — a dashboard or
    # ``nastech serve`` backend building agents for several profiles, a multiplexed gateway —
...
def setup_verbose_logging() -> None:
    ...
    if any(getattr(h, "_nastech_verbose", False) for h in root.handlers):
        return
        ...
        if _is_unavailable_log_stream(exc):
            ...
            if not self._unavailable_reported:
                ...
                _quietly(lambda: print(
                    f"nastech_logging: {self.baseFilename} unavailable ({exc}); "
...
class _ProfileRoutingFileHandler(logging.Handler):
    """Route queued records to the log file for their Nastech home.
    ...
    def __init__(self, existing: RotatingFileHandler, profile_homes: Sequence[Path]) -> None:
        ...
        self._nastech_routed_log_path = resolved
    ...
    def _home_for_record(self, record: logging.LogRecord) -> Path:
        raw_home = getattr(record, "nastech_home", "")
        try:
...
def _register_queued_handler(handler: logging.Handler) -> None:
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._nastech_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    if listener is None:
        return
    t = threading.Thread(target=lambda: _quietly(listener.stop), name="nastech-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_nastech_queue", False):
...
def _add_rotating_handler(
    ...
    *,
    ...
    for existing in _queued_file_handlers:
        ...
        if getattr(existing, "_nastech_routed_log_path", None) == resolved or (
...
def _read_logging_config():
    ...
    try:
        ...
        # can pin logging.*) so this reuses nastech_cli.main's early parse (one config.yaml parse
        # per process); fall back to a direct parse for bare nastech_logging consumers.
        try:
