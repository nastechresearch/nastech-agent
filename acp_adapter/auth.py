...
TERMINAL_SETUP_AUTH_METHOD_ID = "nastech-setup"
...
def build_auth_methods() -> list[Any]:
    ...
    handshake. A fresh Zed install may have no Nastech credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Nastech using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Nastech provider", type="terminal", args=["--setup"],
        description=("Open Nastech' interactive model/provider setup in a terminal. "
