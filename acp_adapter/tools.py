...
# Nastech tool name -> ACP ToolKind (anything unlisted is "other").
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some Nastech tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def build_tool_title(tool_name: str, args: Args) -> str:
    ...
    every other Nastech surface, so ACP clients never show a different summary than the CLI/TUI;
