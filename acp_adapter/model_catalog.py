"""ACP model picker: deduplicated ``provider:model`` rows from the Nastech inventory + named endpoints."""
...
def _named_custom_provider_catalogs() -> list[tuple[str, str, list[tuple[str, str]]]]:
    ...
    try:
        from nastech_cli.config import (get_compatible_custom_providers, is_provider_enabled, load_config)
        from nastech_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
        from nastech_cli.model_switch_providers import _NativePickerModelList, _fetch_picker_live_models
        from nastech_cli.model_switch_providers import _discover_flag
