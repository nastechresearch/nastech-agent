...
# ACP permission option id -> Nastech approval result. Ids are stable across the
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True, smart_denied: bool = False,
    ...
    # DENY override — offering a scope Nastech discards would re-prompt every write.
    # See #81887.
    once_only = smart_denied or not allow_session
    options = [PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once")]
    if not once_only:
        # ACP has no session-scoped kind: closest persistent hint, Nastech semantics in the id.
...
def _map_outcome_to_nastech(outcome: object, *, allowed_option_ids: set[str]) -> str:
    ...
    if outcome.option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_NASTECH.get(outcome.option_id, "deny")
...
def make_approval_callback(request_permission_fn: Callable, loop: asyncio.AbstractEventLoop,
    ...
    """Return a Nastech approval callback (``command, description, **kw`` as used by
    ...
    def _callback(command: str, description: str, *, allow_permanent: bool = True,
        ...
        if response is None:
            return "deny"
        return _map_outcome_to_nastech(response.outcome, allowed_option_ids={option.option_id for option in options})
