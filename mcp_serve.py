"""
Nastech MCP Server — expose messaging conversations as MCP tools (`nastech mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Nastech-specific
channels_list. Client config: {"mcpServers": {"nastech": {"command": "nastech", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("nastech.mcp_serve")
...
def _nastech_home() -> Path:
    try:
        ...
        return Path(os.environ.get("NASTECH_HOME", Path.home() / ".nastech"))


def _get_sessions_dir() -> Path:
    return _nastech_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_nastech_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_nastech_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Nastech analogue of OpenClaw's WebSocket
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Nastech MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
