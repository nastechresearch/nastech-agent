...
Nastech is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Nastech ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`nastech tools`, `nastech setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `NASTECH_HOME` — two of them (A→B→A) when the
...
- **New `NASTECH_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `nastech tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.nastech/plugins/` or pip
  entry point), promoted in the Nastech Research Discord `#plugins-skills-and-skins`. This is a
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `nastech <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`nastech webhook`, `nastech cron`, `nastech tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.nastech/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Nastech Cloud, and
only the first two carry `NASTECH_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Nastech desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_NASTECH_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `NASTECH_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`nastech --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.nastech/nastech-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _NASTECH_CORE_TOOLS
├── cli.py                # NastechCLI (REPL, slash dispatch) + nastech_cli/cli_*_mixin.py
├── nastech_state.py       # SessionDB facade; nastech_state_*.py siblings
├── nastech_constants.py   # get_nastech_home(), display_nastech_home() — profile-aware paths
├── nastech_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── nastech_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
**User state:** `~/.nastech/config.yaml` (settings), `~/.nastech/.env` (secrets only),
`~/.nastech/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_nastech_home()`. Browse logs with `nastech logs [--follow] [--level] [--session]`.
...
families: `nastech_state.py` (21), `gateway/run.py` (15), `tools/mcp_tool.py` (15),
`nastech_cli/kanban.py` (14), `nastech_cli/web_server.py` (13 + 24 routers), `nastech_cli/auth.py`
(12), `tools/browser_tool.py` (11), `cli.py` (12 `nastech_cli/cli_*_mixin.py`), `run_agent.py`
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::nastech_cli.plugin_compat.NastechPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `nastech_cli.update_cmd._nastech_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `nastech_cli/AGENTS.md`.
- **Never hardcode `~/.nastech`.** `get_nastech_home()` for code paths, `display_nastech_home()`
  for user-facing text (both from `nastech_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.nastech/profiles`) so `nastech -p x profile list`
...
- **One process may serve many profiles; code that runs outside a turn binds the owning
  ...
  module constants are a bug class — key slots by `nastech_home_key()` or resolve at call time.
  ...
  `NASTECH_HOME`. Advisory lint: `scripts/check_profile_scope_patterns.py`.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `nastech webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `NASTECH_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `NASTECH_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `NASTECH_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/nastech_cli/`,
  `tests/agent/`, `tests/nastech_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `nastech_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.nastech/`.** The autouse `_isolate_nastech_home` fixture in
  `tests/conftest.py` redirects `NASTECH_HOME`; never hardcode `~/.nastech/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_nastech_home()` stay
  in the temp dir (pattern: `tests/nastech_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".nastech"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("NASTECH_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `NASTECH_HOME` — code reads the
  env var, not `Path.home()/.nastech`.
...
`tests/nastech_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `nastech_cli/`, `main.py` | `nastech_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `nastech update` pipeline, profiles / multiplex |
...
| `plugins/`, `nastech_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `nastech_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`nastech_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`nastech-agent-dev` skill, not here.
