...
bundled, `$NASTECH_HOME/plugins/`, `./.nastech/plugins/` (opt-in via
`NASTECH_ENABLE_PROJECT_PLUGINS`), and `nastech_agent.memory_providers` entry
...
### Bot Mode (`apps/desktop/src/plugins/nastech-bots/`)

The desktop "Bots" experience ships bundled in-tree. Each bot is a Nastech
...
`ui_meta['nastech-bots'].chat` and verified it per click; five hardening
...
## Update Pipeline (`nastech update`)
...
- **Plan** (`nastech_cli/update_inventory.py`, `nastech update --plan`):
...
- **Snapshot** (`nastech_cli/backup.py`): pre-update quick snapshot for
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE
  (every `nastech-gateway*` unit / `ai.nastech.gateway*` LaunchAgent),
...
- **Report**: every run writes a machine-readable receipt to
  `~/.nastech/logs/update_receipts/` (`latest.json` pointer; steps,
...
`nastech serve` (control plane, desktop-spawned child) dies with the app
— by design. The messaging gateway (`gateway run`) SURVIVES the app: the
serve backend's `/api/gateway/*` endpoints spawn it detached
(`_spawn_nastech_action` — `start_new_session` / `DETACHED_PROCESS`), so
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  (gateway run), `nastech_cli.update_cmd._nastech_holder_subcommand`
  (top-level subcommand of any Nastech argv). Never hand-roll token scans.
...
Interactive menus must use `nastech_cli/curses_ui.py`. See `nastech_cli/tools_config.py` for an example.
...
canary ledger, nastech#85796 / gateway-gateway#210). Violating any of them
...
`tests/nastech_cli/test_venv_holder_windows_live.py` on a real
