# Nastech Plugin Catalog

Curated, Nastech-approved Nastech plugins. Each YAML file in this directory
(except `removed.yaml`) is one catalog entry, discoverable via
`nastech plugins catalog` / `nastech plugins search` and installable with
...
1. **Human-merged gate.** Entries are added *only* via a PR to the
   `nastech-agent` repository, reviewed and merged by a maintainer. There is
...
3. **No self-updating code.** A listed plugin must not fetch and replace
   ...
   `nastech plugins update <name>`. Keep the updater in the standalone
...
5. **Owner-or-major-contributor submissions, or a maintainer-curated sweep.**
   ...
   Nastech maintainers may also add entries in batches from a reviewed sweep
...
7. **The install scanner runs at admission.** `nastech plugins validate` includes
...
requires_nastech: ">=0.19"   # optional
docs_url: ""                # optional
