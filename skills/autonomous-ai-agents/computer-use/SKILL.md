...
This skill teaches the Nastech `computer_use` **action vocabulary**, which is
NOT the driver's raw MCP vocabulary. Call the actions documented below and
never the driver's tools by name: `capture` is a Nastech action that maps to
the driver's `get_window_state`; `element=N` is a Nastech argument that the
...
`mode` decides what Nastech hands back to you, not what the driver does.
...
rejects image tool results), Nastech routes the screenshot through the
...
| "bare element_index is not accepted" / `snapshot_id_required` | The driver saw a raw index without its token. This is a wrapper defect, not something you fix by passing `snapshot_id` (Nastech has no such argument). Re-capture once; if it repeats, tell the user to run `nastech update` and fall back to `coordinate=[x, y]` from the capture's bounds meanwhile |
| "tool 'capture' has no reviewed risk classification" / `Unknown tool` | Something called the driver's MCP vocabulary directly (`capture`, `screenshot`, `get_window_state`, `click` with raw args). Only the `computer_use(action=…)` vocabulary in this file exists on the Nastech side |
...
| `nastech computer-use doctor` says "could not be started … Access is denied" (Windows) | The Nastech venv interpreter can't execute a binary under `C:\Program Files\WindowsApps`; the tool itself may still work because the shell resolves another copy on PATH. Fix once: reinstall cua-driver with the upstream installer (lands under the user profile) or set `NASTECH_CUA_DRIVER_CMD` to a copy outside `WindowsApps`. The same denial spams `errors.log` for any other `WindowsApps` binary Nastech spawns (e.g. `bws.exe`) |
...
links the pack into `~/.nastech/skills/cua-driver` (Nastech is a detected
...
calling the Nastech actions from this file — the wrapper does the translation.
