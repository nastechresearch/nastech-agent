...
Set via `nastech model` (picker) or `nastech setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://nastechresearch.github.io/nastech-agent/docs/integrations/providers
...
| nastech | OAuth device code | `nastech auth add nastech` (or `NASTECH_API_KEY`) |
| openai-codex | OAuth | `nastech auth add openai-codex` |
| qwen-oauth | OAuth | `nastech auth add qwen-oauth` |
| minimax-oauth | OAuth | `nastech auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`nastech auth`).
Fallback chain when the primary fails: `nastech fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`nastech_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   nastech config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
