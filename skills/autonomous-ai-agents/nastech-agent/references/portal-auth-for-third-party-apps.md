# Nastech Portal — authenticating third-party apps against the subscription

Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, OpenViking,
LangChain pipeline, n8n flow, etc.) use my Nastech Portal subscription without me
...
## Layer 1 — Is this thing a Nastech plugin, or a separate app?
...
| **Karakeep, n8n, LibreChat, OpenWebUI, any self-hosted app** | Different process, often different machine. Makes its own HTTPS calls to `inference-api.nastechresearch.github.io`. | Layer 2/3 below. |
...
via Nastech's provider config. The plugin's own server auth (e.g.
...
Portal at `https://inference-api.nastechresearch.github.io/v1` is an OpenAI-compatible
inference endpoint. It accepts **bearer-token authentication only**: either

1. **A static API key** from `portal.nastechresearch.github.io → API Keys`, or
...
"Sign in with Nastech Portal" SSO that third-party apps can register as clients
against. There is no shared cookie or session that browser-Portal-login
extends to other apps on the same machine.

What Nastech Agent has that *feels* like OAuth — `nastech login --provider nastech`
...
3. Forward incoming requests to `inference-api.nastechresearch.github.io/v1` with that
   bearer attached.
...
rides on the credential Nastech already holds.

Where this could live in Nastech:
...
Only when the consumer is *another first-party Nastech thing* (a future SDK, a
Nastech-branded extension, a Discord-bot integration that needs per-user
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Nastech itself.
...
2. If separate app: today, paste the static API key from Portal → API Keys.
   Base URL `https://inference-api.nastechresearch.github.io/v1`. Rate limits are
