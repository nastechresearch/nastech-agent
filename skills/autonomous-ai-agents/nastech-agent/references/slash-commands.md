...
Registry of record: `nastech_cli/commands.py` (`COMMAND_REGISTRY`) — every
...
/snapshot [sub]          Create/restore Nastech config+state snapshots (CLI)
...
/subscription (/upgrade) Nastech plan info (CLI)
/topup                   Nastech balance / billing
/copy [N]                Copy last response to clipboard (CLI)
/paste                   Attach clipboard image (CLI)
/image <path>            Attach a local image file (CLI)
/update                  Update Nastech to latest
/version (/v)            Show version
/debug [nastech|local]      Upload debug report, get shareable links
```
