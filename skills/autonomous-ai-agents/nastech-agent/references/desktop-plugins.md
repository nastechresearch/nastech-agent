...
Write plugins for the Nastech desktop app: statusbar items, layout panes,
...
system (`~/.nastech/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$NASTECH_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$NASTECH_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.nastech-package.json` marker) — the ONLY root the renderer loads from — so
...
The ONLY import surface is `@nastech/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Nastech
  ...
  (`nastech://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.nastech/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@nastech/plugin-sdk`, `react`, and
