...
~/.nastech/pets/             Installed pet mascots (see references/petdex.md)
~/.nastech/state.db          Canonical session store (SQLite + FTS5)
~/.nastech/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
...
| AGENTS.md / .nastech.md / CLAUDE.md project rules | `references/project-context-files.md` |
...
| MCP servers (add, catalog, `nastech mcp`) | `references/native-mcp.md` |
...
- **"Can $external_app use my Nastech Portal subscription / OAuth?"** — see `references/portal-auth-for-third-party-apps.md`. Walk the user through three layers (plugin-vs-app, what Portal actually exposes, local-broker-proxy option).

## Surfaces (quick orientation)

- **Desktop app** (`nastech desktop` / `nastech gui`) — native Electron app for macOS/Linux/Windows: streaming chat, session list, Cmd+K palette, drag-and-drop files, native notifications, per-profile remote-gateway login. Extend it with UI plugins — `references/desktop-plugins.md`.
- **Web dashboard** (`nastech dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `nastech --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`nastech --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
- **OpenAI-compatible proxy** (`nastech proxy`) — a local OpenAI API backed by whichever OAuth provider you're signed into. Point Codex CLI, Aider, Cline, or any script at it — no API key.
