...
def refresh_token(token_data: dict) -> dict:
    ...
    params = urllib.parse.urlencode({
        ...
        # The refresh token goes in BOTH the body and the ``x-nastech-refresh-token`` header. Portal's token
        ...
        # name must match Portal's ``REFRESH_TOKEN_HEADER`` exactly (``x-nastech-refresh- token``); any other
