...
def _db_path() -> Path:
    ...
    return get_nastech_home().resolve() / "cron" / "executions.db"
...
def _initialize_schema(conn: sqlite3.Connection) -> None:
    from nastech_cli.sqlite_util import add_column_if_missing
...
def close_incidents_for_recovered_job(job_id: str) -> int:
    ...
    outage) stayed ``detected``/``alerted`` forever after the job recovered, so ``nastech cron
