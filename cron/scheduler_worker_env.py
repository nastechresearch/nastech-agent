...
``cron.scheduler``, not ``nastech_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips Nastech-owned PYTHONPATH entries because user
children must not see our tree. This child IS Nastech, so the pin is applied *after* the
...
def pin_nastech_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
