...
A cron job that restarts/stops the gateway from inside the gateway (``nastech gateway restart``,
``launchctl kickstart ai.nastech.gateway``, ``systemctl restart nastech-gateway``) kills the process,
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `nastech gateway` ops. `start` is excluded: starting from inside a
    # gateway is benign and a job may legitimately start a sibling profile. The lookbehind keeps
    # `nastech` from being a path component or word tail (`/docs/nastech gateway restart-notes.md`)
    # while every real command position (text start, whitespace, `;`/`&`/`|`, `$(`, backtick,
    # U+FFFD) still matches.
    # See #77173.
    # Windows spells the CLI with a launcher suffix (`nastech.exe`, npm-style `nastech.cmd`/`.ps1`);
    # same command, so the suffix is optional here.
    r"(?:(?<![/\w.\-])nastech(?:\.(?:exe|cmd|bat|com|ps1))?\s+gateway\s+(?:restart|stop|uninstall)\b)"
    # Branch B: launchctl ops anchored on a nastech-gateway label so unrelated nastech services stay
    ...
    # -l ai.nastech.gateway-<suffix> -- <helper-script>` (or `launchctl bootstrap gui/<uid> <plist>`) creates
...
# gateway is literally `python.exe` / `python3.12` (`python -m nastech_cli.main gateway run`), so
# `taskkill /F /IM python.exe`, `pkill -9 python3` or `killall python` carry no nastech/gateway token
...
# (`python -m nastech_cli.main gateway run` / `nastech gateway run`) only through its own tokens;
# an unrelated script that merely contains "nastech" (`nastech-polis/run.sh`, `my_nastech_bot.py`)
# cannot match it. Same nastech+gateway pairing as Branch D, plus the module path.
_GATEWAY_CMDLINE_TOKEN_RE = re.compile(r"(?i)nastech_cli|\bnastech\b[^\n]*\bgateway\b|\bgateway\b[^\n]*\bnastech\b")
...
HOST_INTERPRETER_KILL_REJECTION = (
    ...
    "for a background job Nastech started, or kill/taskkill by its explicit PID."
)
...
def _pattern_reaches_host_interpreter(pattern: str, *, full_cmdline: bool, exact: bool) -> bool:
    ...
    command line). `python -m nastech_cli.main …` is the gateway's own cmdline, so a `-f` pattern
    that names the interpreter and then only wildcards or a `nastech` token reaches it, while
    ...
    return not rest.strip() or bool(_ERE_WILDCARD_ONLY.match(rest)) or "nastech" in rest.lower()
...
# split across continuation lines (e.g. `launchctl submit \` / `  -l ai.nastech.gateway-... \` / `  -- ...`,
...
# Branch A2: `nastech -p <profile> gateway restart|stop` (also `--profile <name>` /
...
# `nastech -p venus gateway restart` operates on a sibling profile's gateway and is a legitimate fleet
...
# `$label` next to the verb. These verbs act on an EXISTING job, so the nastech-gateway label anchor
...
_NASTECH_GATEWAY_LABEL_RE = re.compile(r"(?i)\bnastech[.\-]?gateway\b")
...
def _current_profile_name() -> Optional[str]:
    """Profile running the guard: ``NASTECH_PROFILE_NAME``/``NASTECH_PROFILE`` env first, then
    ``nastech_cli.profiles.get_active_profile_name`` (from ``NASTECH_HOME``); ``None`` if neither."""
...
def _contains_launchctl_gateway_lifecycle(normalized_text: str) -> bool:
    ...
    return bool(_LAUNCHCTL_LIFECYCLE_VERBS_RE.search(normalized_text)) and bool(
        _NASTECH_GATEWAY_LABEL_RE.search(normalized_text)
    )
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    ``nastech``/``gateway`` token) — this function is the single choke point
    ...
    # <<'EOF'`) are masked before scanning (#88336): a runbook line "a human can run: nastech gateway
    ...
    # Profile-flag form (#78028): `nastech -p <profile> gateway restart|stop` bypasses Branch A because the
    # selector sits between `nastech` and `gateway`. It is only the same foot-gun when the named profile IS
    ...
    # Branch E (#113667): killers aimed at the interpreter image itself carry no nastech/gateway token.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    ``<NASTECH_HOME>/scripts/``) so the guard scans the file that will actually run."""
    ...
    if raw.is_absolute():
        return raw
    try:
        return get_nastech_home() / "scripts" / raw
    except (RuntimeError, OSError):
        # get_nastech_home() falls back to Path.home(), which raises when neither NASTECH_HOME nor
...
def check_gateway_lifecycle(prompt: Optional[str], script: Optional[str] = None) -> None:
    ...
    if script:
        ...
        if resolved_script is not None and _on_cloud_path(resolved_script):
            raise GatewayLifecycleBlocked(
                ...
                "(e.g. ~/.nastech/scripts/) and recreate the job."
            )
    ...
    if refusal:
        ...
        # The direct command regex below still scans the full text, so a literal `nastech gateway restart`
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `nastech gateway restart` from a shell outside "
