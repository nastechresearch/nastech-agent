...
``nastech cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``nastech gateway``, ``launchctl ... nastech-gateway``,
``systemctl ... nastech-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_NASTECH_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``nastech gateway
...
The profile-flag form (``nastech -p <profile> gateway restart|stop``, #78028)
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: destructive `nastech gateway` operations.
    # The destructive operations are restart, stop, and uninstall.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    # The lookbehind (#77173): `nastech` must not be a path component or a
    # word tail. Excluding `/`, word chars, `.` and `-` keeps file paths
    # with embedded spaces (`/docs/nastech gateway restart-notes.md`) from
    # matching via the `/nastech` tail, while every real command position
    # (start of text, whitespace, `;`/`&`/`|`, `$(`, backtick, even a
    # U+FFFD from binary-content decoding) still matches.
    r"(?:(?<![/\w.\-])nastech\s+gateway\s+(?:restart|stop|uninstall)\b)"
    ...
    # (kickstart/etc.): `launchctl submit -l ai.nastech.gateway-<suffix> --
...
# lines (e.g. `launchctl submit \` / `  -l ai.nastech.gateway-... \` / `  -- ...`,
...
# selector — `nastech -p <profile> gateway restart|stop` / `--profile <name>`
# / `--profile=<name>`. The selector token between `nastech` and `gateway`
...
# `nastech -p venus gateway restart` operates on a sibling profile's gateway
...
def _current_profile_name() -> Optional[str]:
    ...
    Prefers the explicit ``NASTECH_PROFILE_NAME`` / ``NASTECH_PROFILE`` env
    (set by the profile launcher and kanban worker spawns), falling back to
    ``nastech_cli.profiles.get_active_profile_name`` (derived from
    ``NASTECH_HOME``, which the gateway process inherits from its launch
...
# Branch B only catches `launchctl <verb> ... nastech[.-]?gateway` when the
...
# command — `for item in 'ai.nastech.gateway-apollo:...' 'ai.nastech.gateway:...';
...
# is the unexpanded variable `$label`, not the string "nastech.gateway". This
...
# EXISTING job, so anchoring to the nastech-gateway label is still correct —
# `test_safe_commands` requires unrelated-label ops (e.g. `launchctl unload
# ai.nastech.update-checker.plist`) to stay unblocked. The fix is checking
...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    a ``nastech``/``gateway`` token) — this function is the single choke
    ...
    # a runbook line "a human can run: nastech gateway restart" inside such a
    ...
    if _GATEWAY_LIFECYCLE_PATTERN.search(normalized):
        return True
    # Profile-flag form (#78028): `nastech -p <profile> gateway restart|stop`
    # bypasses Branch A because the selector sits between `nastech` and
    ...
    if profile_match:
        ...
        if named:
            # Profile ids cannot contain quotes (nastech_cli.profiles
...
# for `systemctl restart nastech-gateway` in syslog, a SQL LIKE literal over a
...
# blocking `grep -r 'systemctl restart nastech-gateway' .` outright. Require a
...
# to execution (`grep 'systemctl restart nastech-gateway' f | sh`); never mask
# such a line.
...
def contains_launchctl_submit_command(command: str) -> bool:
    ...
    chosen by whoever writes it, so a neutral name (``ai.nastech.svc-reload-tmp``)
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart nastech-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart nastech-gateway%'"
...
def _references_at(
    ...
    # (e.g. `Path.home() / ".nastech"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    try:
        ...
        # get_nastech_home() falls back to Path.home(), which raises when
        # neither NASTECH_HOME nor HOME is resolvable (launchd/systemd
...
def _read_script_for_scanning(script_path: str) -> str:
    ...
    if unsafe:
        return "nastech gateway restart"
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `nastech gateway restart` embedded in a .py script is still
