...
# store under its own NASTECH_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same NASTECH_HOME — so a job authored in profile
# `coder` lives in `~/.nastech/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_nastech_home()` (the active profile home), NOT `get_default_nastech_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever NASTECH_HOME the
...
# `_get_nastech_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
...
# gateway process and the (separate) ``nastech cron status`` process share it
...
# threshold in `nastech cron status` (nastech_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_nastech_home()
       (context-local override, then the NASTECH_HOME env var) — so a test
       or embedder that re-points NASTECH_HOME after this module was
...
# the cron inactivity timeout is disabled (NASTECH_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. NASTECH_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``NASTECH_CRON_TIMEOUT`` (the cron inactivity timeout the
...
def _jobs_lock():
    ...
    and standalone ``nastech`` CLI invocations, which previously shared no lock
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec nastech nastech cron create ...`` — ``docker exec``
...
def _is_named_profile_path(path: Path) -> bool:
    ...
    Named profiles live under ``<nastech_home>/profiles/<name>/``.  The
    default profile lives at ``<nastech_home>`` directly (no ``profiles``
    parent), as do custom ``NASTECH_HOME`` paths outside ``~/.nastech``.
...
def _ensure_cron_dir(cron_dir: Path) -> None:
    ...
    Default and custom Nastech homes keep ``parents=True`` so first-run
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Nastech timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `nastech_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                nastech_tz = _nastech_now().tzinfo
                dt = dt.replace(tzinfo=nastech_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Nastech/system local time
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``nastech cron status``) never sees a torn/truncated
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `nastech cron status` can tell a thread that is merely *alive and looping*
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``nastech cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``nastech cron
...
def _normalize_reasoning_effort(value: Any) -> Optional[str]:
    ...
    every other effort surface uses (``nastech_constants.parse_reasoning_effort``)
...
def create_job(
    ...
    Args:
        ...
        monitor_script: Optional path to a cheap monitor source script (same
                ...
                ~/.nastech/scripts/, .sh/.bash via bash, else Python). Each
        ...
        reasoning_effort: Optional per-job reasoning effort pin. One of the
                canonical Nastech levels (none|minimal|low|medium|high|xhigh|
    ...
    # covered, not just `nastech cron create`.
...
def _machine_id() -> str:
    ...
    Uses ``NASTECH_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from NASTECH_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                        ...
                        if times is not None and times > 0 and completed >= times:
                            ...
                            if job.get("last_run_at") is not None:
                                ...
                                logger.warning(
                                    ...
                                    "'nastech cron resume <job> --run-now' "
                                    "(#93524).",
                                    job.get("name", job.get("id", "?")),
                                    completed,
                                    times,
                ...
                # desktop both run in-process 60s tickers on one NASTECH_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from NASTECH_CRON_TIMEOUT),
                ...
                # dispatch timing on the job record so `nastech cron list` /
                # `nastech cron status` (separate CLI processes) can show a
...
# execution. Unlike the quick-snapshot store (`nastech_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``nastech_cli.backup._prune_quick_snapshots``:
...
def _canonical_skill_ref(raw: Any) -> str:
    ...
    A job may store an absolute path under ``NASTECH_HOME/skills`` or an
