...
def _guarded_store_write(action, description, *args, **kwargs):
    ...
    markers are diagnostics for ``nastech cron status`` — losing one write to a broken store
...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    back on disk and in ``nastech profile list`` (#47368). Filtering on directory existence leaves a deleted
...
def fire_overdue_jobs(
    ...
    """
    # `nastech pause` ESTOP: skip the sweep entirely. No state to unwind — the
    # next housekeeping pass after `nastech resume` catches overdue work up
    ...
    from cron.jobs import (
        ONESHOT_GRACE_SECONDS, _ensure_aware, _nastech_now, is_job_runnable, load_jobs,
    )
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        # the process-global NASTECH_HOME (the default profile) is ticked. Heartbeats and recovery are also
        # scoped per profile so `nastech cron status` reflects liveness for every profile independently.
        ...
        try:
            ...
            # Heartbeat before the first sleep so `nastech cron status` sees a live ticker
            # immediately.
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # Persist the reason so `nastech cron status` (separate process) shows WHY.
                _guarded_store_write(
