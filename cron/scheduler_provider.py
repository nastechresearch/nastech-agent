...
def _existing_profile_homes(profile_homes: list) -> list:
    ...
    the ticker runs — via ``nastech profile delete``, the desktop's DELETE
    ...
    and in ``nastech profile list`` (#47368). Filtering on directory existence
...
def fire_overdue_jobs(
    ...
    *,
    ...
    from cron.jobs import _ensure_aware, _nastech_now, is_job_runnable, load_jobs
...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global NASTECH_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `nastech cron
        ...
        # Heartbeat once before the first sleep so `nastech cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `nastech cron status`/`list` (separate processes) can show
    ...
    def _start_multiplex(
        ...
        *,
        ...
        Each profile uses ``set_nastech_home_override()`` + ``use_cron_store()``
            ...
            for entry in cycle_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `nastech cron status` can show WHY ticks fail
                        # (#68483).
                        if _home_ok:
                            clear_ticker_error()
