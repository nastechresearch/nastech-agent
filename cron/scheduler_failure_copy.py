...
Every notice says WHAT happened and WHAT TO DO, and names the exact ``nastech cron`` command plus
...
def cron_output_dir_display(job_id: str) -> str:
    ...
    return f"{display_nastech_home()}/cron/output/{job_id}/"
...
_PROVIDER_FAILURE_ACTION: dict[str, str] = {
    ...
    "model_not_found": "Pick another model with `nastech cron edit {job_id} --model <name>`.",
    "context_overflow": "Shorten the job's prompt with `nastech cron edit {job_id} --prompt <text>`.",
}
...
_PROVIDER_FAILURE_ACTION["content_policy_blocked"] = (
    "Reword the job's prompt with `nastech cron edit {job_id} --prompt <text>`, or pick another "
    "model with `nastech cron edit {job_id} --model <name>`."
)
_DEFAULT_FAILURE_ACTION = "Run it again with `nastech cron run {job_id}`, or edit it with `nastech cron edit {job_id}`."
...
def generic_failure_notice(job_name: str, job_id: str, cleaned_error: str) -> str:
    ...
    return (
        ...
        f"See the full run with `nastech cron runs {job_id}` (output saved under "
        f"{cron_output_dir_display(job_id)}); run it again with `nastech cron run {job_id}`, "
        f"edit it with `nastech cron edit {job_id}`, or pause it with `nastech cron pause {job_id}`."
    )
...
def script_timeout_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"Check the script's output under {cron_output_dir_display(job_id)} or `nastech cron runs {job_id}`, "
        f"then run it again with `nastech cron run {job_id}`."
    )
...
def inactivity_notice(job_name: str, job_id: str) -> str:
    return (
        ...
        f"{cron_output_dir_display(job_id)} (`nastech cron runs {job_id}`), then run it again with "
        f"`nastech cron run {job_id}`."
    )
...
def blocked_config_notice(job_name: str, reason: str) -> str:
    ...
    if reason and reason[-1] not in ".!?":
        reason += "."
    return (
        f"⛔ Cron '{job_name}' did not run: {reason} Nothing was charged. Nastech will try again at "
