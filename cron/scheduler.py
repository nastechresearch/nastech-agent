...
def _close_late_session_db_result(future: "concurrent.futures.Future") -> None:
    ...
    try:
        db = future.result()
        if db is not None:
            from nastech_state import release_or_close
            release_or_close(db)
    except Exception:
        pass
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "Fallback chain was exhausted or unavailable."
    return (
        "No fallback chain configured — add one with `nastech fallback add`, "
...
def _failure_streak_nudge(job: dict) -> str:
    ...
    return (
        ...
        f"Fix its prompt/config, or pause it with `nastech cron pause {job_ref}` "
...
class CronTickYielded(RuntimeError):
    ...
    (``nastech cron status`` must not show green while jobs only fire from the
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    if "skipped to prevent unintended spend: global inference config drifted" in lower:
        if "finite one-shot job is consumed" in lower:
            ...
            remediation = (
                "On the host running Nastech, pin it explicitly: "
                f"`nastech cron edit {job_id} --provider <provider> "
                "--model <model>`."
            )
        return (
    ...
    # checkout was updated underneath it (interrupted `nastech update`, manual
    ...
    if provider_reachable and re.search(
        ...
        if skew is not None:
            ...
            message += (
                ...
                "`nastech gateway restart` to fix it."
            )
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy nastech_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from nastech_cli.tools_config import enabled_mcp_server_names
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str) -> dict | None:
    ...
    """
    from nastech_constants import parse_reasoning_effort, resolve_reasoning_config
...
def _inflight_min_allowance_minutes() -> float:
    ...
    ``NASTECH_CRON_INFLIGHT_MAX_MINUTES`` env var is kept as an internal
    escape hatch only.
    """
    try:
        ...
        pass
    raw = os.getenv("NASTECH_CRON_INFLIGHT_MAX_MINUTES", "").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
            ...
            logger.warning(
                "Invalid NASTECH_CRON_INFLIGHT_MAX_MINUTES=%r; using default %s",
                raw,
...
def _record_forced_release(job_id: str, name: str, age_seconds: float, allowance_seconds: float) -> None:
    ...
    entry = {
        ...
        "at": _nastech_now().isoformat(),
    }
    with _running_lock:
        _forced_releases.append(entry)
        del _forced_releases[:-_FORCED_RELEASE_HISTORY]
    try:
        path = _get_nastech_home() / "cron" / "inflight_forced_releases.jsonl"
    ...
    *,
    ...
    with _running_lock:
        ...
        if only_owners is None:
            active_fires.extend(
                (None, job_id, None, _get_nastech_home())
                for job_id in (
...
def _cron_inactivity_seconds() -> float:
    """Parse NASTECH_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600.
    ...
    try:
        ...
        logger.warning("Invalid NASTECH_CRON_TIMEOUT=%r; using default 600s", raw)
        return 600.0
...
# Resolves through _get_nastech_home() so profile-scoped paths work correctly.
def _usage_audit_path() -> Path:
    return _get_nastech_home() / "cron" / "usage_audit.jsonl"
...
def _write_usage_audit(record: dict) -> None:
    """Append a single JSONL line to ~/.nastech/cron/usage_audit.jsonl.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``nastech update`` / ``nastech gateway stop`` / systemd restart, or an
...
def _get_nastech_home() -> Path:
    ...
    profile-scoped gateway, so resolving the active NASTECH_HOME at call time
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Nastech — {task_name}"
    try:
...
def cron_delivery_targets() -> list[dict]:
    ...
    try:
        from nastech_cli.profiles import list_profile_names
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str) -> Optional[str]:
    ...
    Runs ``nastech [-p <profile>] chat --in ~ -c "Bot Chat" --create-if-missing
    ...
    scheduler's NASTECH_HOME) or a validated local profile name.  Returns None
    ...
    nastech_bin = _shutil.which("nastech")
    if nastech_bin:
        argv = [nastech_bin]
    else:
        try:
            ...
            if _ilu.find_spec("nastech_cli") is not None:
                argv = [sys.executable, "-m", "nastech_cli.main"]
            else:
                return "bot-chat delivery failed: nastech CLI not resolvable"
        except Exception:
            return "bot-chat delivery failed: nastech CLI not resolvable"
    ...
    if profile:
        argv += ["-p", profile]
        # -p owns profile resolution in the child; a leftover NASTECH_HOME
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="nastech-cron-botchat-",
            delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
...
def _resolve_bot_chat_target(job: dict, profile_arg: str) -> Optional[dict]:
    ...
    ``profile_arg`` is ``""`` for the job's own profile (the NASTECH_HOME
    ...
    ``~/.nastech/profiles/`` tree, so same-named profiles on other gateways
    can never be targeted by accident.
    """
    if not profile_arg:
        # Own profile: chat subprocess inherits NASTECH_HOME, no name needed.
        return {"platform": BOT_CHAT_PLATFORM, "chat_id": "", "thread_id": None}
    try:
        from nastech_cli.profiles import normalize_profile_name, profile_exists
    ...
    for media_path, _is_voice in media_files:
        try:
            ...
            try:
                ...
                # NASTECH_CRON_MEDIA_SEND_TIMEOUT env override).
...
def _deliver_result(
    ...
    # another job in-process (``nastech cron run``), and that nested delivery
    # must not be keyed under the outer execution id.
    external_execution = os.environ.get("_NASTECH_CRON_EXTERNAL_WORKER", "")
    if (
        external_execution
    ...
    # ``last_delivery_unverified`` so `nastech cron list` shows the state
    ...
    # at boot; a standalone process (manual `nastech cron run` from the CLI,
    ...
    for target in targets:
        ...
        if transport is not None:
            ...
            # (`nastech cron run`) has no live relay adapter, so surface the
...
def _get_media_send_timeout() -> int:
    ...
    NASTECH_CRON_MEDIA_SEND_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("NASTECH_CRON_MEDIA_SEND_TIMEOUT", "").strip()
    if env_value:
        try:
            timeout = int(float(env_value))
            if timeout > 0:
            ...
            logger.warning(
                "Invalid NASTECH_CRON_MEDIA_SEND_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _get_session_db_timeout() -> float:
    ...
    Mirrors the ``script_timeout_seconds`` resolution pattern: the
    NASTECH_CRON_SESSION_DB_TIMEOUT env var wins, then
    ...
    """
    env_value = os.getenv("NASTECH_CRON_SESSION_DB_TIMEOUT", "").strip()
    if env_value:
        try:
            return float(env_value)
        except (ValueError, TypeError):
            logger.warning(
                "Invalid NASTECH_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                env_value,
            )
...
def _run_job_script(
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Nastech-managed secrets are not inherited
...
def _preflight_check_provider_key(job: dict, cfg: dict) -> Optional[str]:
    ...
    try:
        ...
        return (
            ...
            "Set the provider API key in .env (or `nastech setup`), or pin a "
            "working provider via `nastech cron edit "
...
def _primary_profile_routes_for_current_home() -> list:
    ...
    """
    try:
        from nastech_constants import get_default_nastech_root, get_nastech_home

        primary_home = get_default_nastech_root()
        current_home = Path(get_nastech_home())
        if (
        ...
        from nastech_cli.profiles import profile_matches_home
    ...
    for platform_name in platform_parts:
        ...
        if platform_name.lower() not in connected:
            ...
            return (
                ...
                "`nastech setup` or change the job's `deliver` target."
            )
    return None
...
def run_job(
    ...
    *,
    ...
    # AIAgent or spend tokens. Escape hatch: NASTECH_IGNORE_USER_CONFIG=1.
    ...
    if job_has_monitor(job):
        ...
        _mon_now = _nastech_now().strftime("%Y-%m-%d %H:%M:%S")
        if not _mon.ok:
    ...
    _ctx_tokens = set_session_vars(
        ...
        # NASTECH_SESSION_* routing keys just below, so an async delegation's
        ...
        # os.environ NASTECH_SESSION_KEY, which risks routing a cron subagent's output
    ...
    _cron_session_var = _VAR_MAP["NASTECH_CRON_SESSION"]
    ...
    try:
        ...
        #
        # A kanban worker is a normal `nastech chat -q` CLI agent whose default
        # toolset includes `cronjob`, running with NASTECH_KANBAN_TASK
        ...
        # and kanban_complete defaults task_id to $NASTECH_KANBAN_TASK -- letting
        ...
        # load_nastech_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this NASTECH_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_nastech_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from nastech_cli.env_loader import (
            load_nastech_dotenv,
        ...
        # cron-fleet default) > NASTECH_MODEL env > config.yaml ``model:``
        # (string or ``{default: ...}``). The per-job value is intentionally
        # re-read from storage every tick so a ``nastech cron edit --model``
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"NASTECH_MODEL={os.getenv('NASTECH_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`nastech cron edit {job_id} --model <name>` or set a "
                "default with `nastech model <name>`."
            )
        ...
        prefill_file = (
            os.getenv("NASTECH_PREFILL_MESSAGES_FILE", "")
        ...
        # to a paid PROVIDER (e.g. nastech) OR a paid MODEL on the same provider
        ...
        if cron_model_drift_guard_enabled(_cfg):
            ...
            if _drift:
                ...
                if _finite_oneshot:
                    ...
                    _remediation = (
                        "To run on the new config, on the host running Nastech "
                        "pin it explicitly: "
                        f"`nastech cron edit {job_id} --provider <provider> "
        ...
        #
        # Bounded with its own timeout (separate from NASTECH_CRON_TIMEOUT,
        ...
        try:
            from nastech_state import get_shared_session_db
        ...
        if final_response.strip() and turn_exit_reason:
            ...
            _explainer_variants = []
            try:
                from nastech_state import PERSISTENCE_ERROR_CAUSES as _causes
            except Exception:
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_nastech_now().strftime('%b %d %H:%M')}"
            ...
            # nastech_state's session_lifecycle_statuses docstring — keep the
...
def run_one_job(
    ...
    *,
    ...
    external_owner = os.environ.get("_NASTECH_CRON_EXTERNAL_WORKER") == execution_id
    ...
    if extra_prompt is None:
        # A gateway-forwarded manual run (`nastech cron run --prompt` /
    ...
    profile_home = _get_nastech_home().resolve()
    with _running_lock:
    ...
    *,
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_nastech_home())
        )
        ...
        if delivery_error:
            ...
            # exact signature via `nastech cron incidents ack`.
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    execution_id = str(job["execution_id"])
    job_id = str(job["id"])
    handoff_dir = _get_nastech_home() / "cron" / "external-workers"
    ...
    try:
        with os.fdopen(fd, "w", encoding="utf-8") as payload_file:
            json.dump(
                {
                    "job": job,
                    "profile_home": str(_get_nastech_home().resolve()),
    ...
    worker_env = build_subprocess_env(
        ...
        extra={"NASTECH_HOME": str(_get_nastech_home().resolve())},
    )
    try:
        process = subprocess.Popen(
            scoped_command,
    ...
    try:
        with use_cron_store(profile_home):
            ...
            try:
                ...
                return False
            old_external_execution = os.environ.get("_NASTECH_CRON_EXTERNAL_WORKER")
            os.environ["_NASTECH_CRON_EXTERNAL_WORKER"] = execution_id
            try:
                return run_one_job(job, adapters=None, loop=None, verbose=False)
            finally:
                if old_external_execution is None:
                    os.environ.pop("_NASTECH_CRON_EXTERNAL_WORKER", None)
                else:
                    os.environ["_NASTECH_CRON_EXTERNAL_WORKER"] = old_external_execution
    finally:
...
# `nastech -w` launches, so on gateway-driven boxes (where sessions arrive via
...
def _worktree_maintenance_repos() -> List[str]:
    ...
    Candidates: the nastech install checkout itself (where ``nastech -w``
    ...
    # The nastech source checkout (editable/git installs). Wheel installs have
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    ``nastech -w`` startup path uses — dirty/unpushed/live-locked trees are
...
def tick(
    ...
    *,
    ...
    # ``git pull``, interrupted ``nastech update``) serves MIXED sys.modules:
    ...
    try:
        # Global emergency stop (`nastech pause`): skip dispatch entirely while
        # the ESTOP sentinel exists. Never touches in-flight runs — due jobs
        # simply wait for the next tick after `nastech resume`. Logged once per
        ...
        # scheduler STARTUP. A one-shot `nastech cron run` that claimed a job
        ...
        # boxes never hit the `nastech -w` startup pruner, so this is the only
...
if __name__ == "__main__":
    if "--external-worker-file" in sys.argv:
        ...
        try:
            ...
            setup_logging(nastech_home=_get_nastech_home(), mode="cron")
        except Exception:
            pass
        raise SystemExit(
