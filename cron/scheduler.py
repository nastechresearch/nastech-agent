...
A file lock (~/.nastech/cron/.tick.lock) keeps overlapping processes to one tick at a time.
"""
...
# `nastech update`) otherwise fail with ModuleNotFoundError for nastech_time et al.
...
def _fallback_chain_phrase() -> str:
    ...
    if chain:
        return "No backup provider succeeded either."
    return (
        "No backup provider is configured — add one with `nastech fallback add`, "
...
class CronTickYielded(RuntimeError):
    ...
    open). Raised, not returned, so ``record_ticker_error`` sees it and ``nastech cron status``
    isn't green.
    """
...
def _summarize_cron_failure_for_delivery(job: dict, error: str | None) -> str:
    ...
    # underneath it (interrupted `nastech update`, manual git pull) serves MIXED modules and every
...
def _repeat_alert_withheld(incident: dict) -> bool:
    ...
    try:
        ...
        return False
    return _nastech_now() - last < timedelta(hours=hours)
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy: avoid heavy nastech_cli import at module load; shares MCP-membership with gateway/CLI
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"tool. Check `platform_toolsets.cron` in config.yaml (`nastech cron doctor`): {exc}"
        ) from exc
...
def get_running_job_details() -> list[dict]:
    ...
    runs). The drain wait publishes this so ``nastech update`` can say WHICH job it is waiting on."""
    now = time.time()
    with _running_lock:
        return [
...
def try_register_running_job(job_id: str) -> bool:
    ...
    """
    from nastech_cli.backend_retirement import retirement
...
def _inflight_min_allowance_minutes() -> float:
    ...
    with contextlib.suppress(Exception):
        ...
        if _cfg_val is not None:
            ...
            if val > 0:
                return val
    raw = cron_env_setting("NASTECH_CRON_INFLIGHT_MAX_MINUTES").strip()
    if raw:
        try:
            val = float(raw)
            if val > 0:
...
def _cron_inactivity_seconds() -> float:
    """Parse NASTECH_CRON_TIMEOUT (seconds). 0 = unlimited; bad input = 600. Shared by the
    ...
    raw = cron_env_setting("NASTECH_CRON_TIMEOUT").strip()
    if not raw:
        return 600.0
    try:
        return float(raw)
...
# Per-fire usage audit log; resolves via _get_nastech_home() so profile-scoped paths work.
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    A cron tick can fire while the gateway is tearing down — SIGTERM from ``nastech update`` / ``nastech
...
def _get_nastech_home() -> Path:
    """Nastech home at call time (honouring the test override). Cron is per-profile: never freeze
    ...
    resolving the active NASTECH_HOME at call time means a profile's jobs are stored AND executed under that
...
def _snapshot_pin(job: dict, axis: str, current: str, job_id: str) -> str:
    ...
    if snapshot and current and snapshot.lower() != current.lower():
        logger.info(
            ...
            "`nastech cron resnap %s` adopts the new default (stays unpinned), "
            "`nastech cron edit %s --%s <value>` or cron.%s in config.yaml pins it.",
...
def _load_cron_job_config(job: dict, job_id: str, job_name: str) -> _CronJobConfig:
    ...
    creation snapshot > NASTECH_MODEL > config ``model:``. Re-read every tick (no cache) so
    ``nastech cron edit --model`` applies next tick."""
    model = job.get("model") or cron_env_setting("NASTECH_MODEL") or ""
            ...
            if not job.get("model"):
                if _cron_default_model:
                    ...
                    _, _global_model = resolve_cron_model_drift_defaults(
                        _cfg, environ={"NASTECH_MODEL": cron_env_setting("NASTECH_MODEL")})
    ...
    if not (isinstance(model, str) and model.strip()):
        raise RuntimeError(
            ...
            f"NASTECH_MODEL={cron_env_setting('NASTECH_MODEL')!r}, "
...
def _load_prefill_messages(cfg: dict, job_id: str) -> Optional[list]:
    ...
    prefill_file = (
        cron_env_setting("NASTECH_PREFILL_MESSAGES_FILE")
...
def _blocked_config_result(job_id: str, job_name: str, _pf_reason: str) -> tuple:
    ...
    blocked_doc = (
        ...
        "(nothing was charged).\n\n"
        f"**Reason:** {_pf_reason}\n\n"
        "Nastech tries again at the next scheduled time and clears this state on the first healthy "
        "run; this alert is not repeated. Check with `nastech cron doctor`. Set `cron.preflight: "
...
def _resolve_job_runtime(job: dict, job_id: str, jc: _CronJobConfig) -> tuple[dict, str]:
    ...
    try:
        # Do NOT pass NASTECH_INFERENCE_PROVIDER as `requested`: it would override persisted config
...
def _open_cron_session_db(job: dict):
    """Open the SQLite session store under its own timeout (NASTECH_CRON_TIMEOUT only watches
    ...
    # handle (#96290). Bounded with its own timeout (separate from NASTECH_CRON_TIMEOUT, which only watches
...
def _run_agent_with_watchdog(
    ...
    watchdog: default 600s, override NASTECH_CRON_TIMEOUT, 0 = unlimited."""
...
def _final_response_from_result(result: dict, job_id: str, job_name: str, AIAgent) -> str:
    ...
    if final_response.strip() and turn_exit_reason:
        ...
        _explainer_variants = []
        try:
            from nastech_state_errors import PERSISTENCE_ERROR_CAUSES as _causes
        except Exception:
...
def _finalize_cron_session(session_db, agent, job_id: str, job_name: str, cron_session_id: str) -> None:
    ...
    try:
        ...
        with contextlib.suppress((Exception, KeyboardInterrupt)):
            ...
            # NASTECH_SESSION_KEY is not in the CLI environment, so the key resolves empty here. Since #64240
    ...
    # vocabulary in nastech_state's session_lifecycle_statuses docstring — keep the tuple below in sync when
...
def _prepare_job_prompt(
    ...
    # user never chose. no_agent jobs are exempt. Escape hatch: NASTECH_IGNORE_USER_CONFIG=1.
    if not job.get("no_agent"):
...
class _CronRunScope:
    ...
    NASTECH_SESSION_* are deliberately NOT seeded from job["origin"]: it is delivery metadata, not
    a sender, and terminal/tts/skills/send_message tools would act as if the origin user were
    driving the agent. Delivery reads job["origin"] / NASTECH_CRON_AUTO_DELIVER_* directly.
    """
    ...
    def __init__(self, job: dict, job_id: str, execution_id: Optional[str]):
        ...
        self._ctx_tokens = set_session_vars(
            ...
            # We clear the NASTECH_SESSION_* routing keys just below, so an async delegation's completion
            ...
            # NASTECH_SESSION_KEY, which risks routing a cron subagent's output into an unrelated user chat.)
        ...
        self._cron_session_var = _VAR_MAP["NASTECH_CRON_SESSION"]
    ...
    def enter(self) -> None:
        ...
        # NASTECH_KANBAN_TASK in env, and an unrelated job could close the worker's task. Must be a
...
def _reload_dotenv_and_publish_delivery_target(job: dict) -> None:
    ...
    from nastech_cli.env_loader import load_nastech_dotenv, reset_secret_source_cache
    from gateway.session_context import _VAR_MAP

    reset_secret_source_cache(_get_nastech_home())
    load_nastech_dotenv(nastech_home=_get_nastech_home())
...
def _run_one_job_body(
    ...
    try:
        ...
        _scope_token = set_secret_scope(build_profile_secret_scope(_get_nastech_home()))
        ...
        _terminal_scope_token = install_profile_terminal_scope(_get_nastech_home())
...
def _launch_external_cron_worker(job: dict) -> bool:
    ...
    try:
        worker_env = strip_launch_profile_env(build_subprocess_env(
            ...
            extra={"NASTECH_HOME": str(profile_home)},
        ))
    finally:
        reset_secret_scope(secret_token)
    worker_env = systemd_user_bus_env(worker_env)
    # Unattended worker: the gateway sets NASTECH_EXEC_ASK at startup (interactive launches set
    ...
    # `-m cron.scheduler` has no nastech_cli.main bootstrap; pin this checkout explicitly
    # (PYTHONSAFEPATH / stale editable mapping, #112729). See cron/scheduler_worker_env.py.
    from cron.scheduler_worker_env import pin_nastech_tree_on_pythonpath
    repo_root = Path(__file__).resolve().parent.parent
    worker_env = pin_nastech_tree_on_pythonpath(worker_env, repo_root)
    try:
...
def _worktree_maintenance_repos() -> List[str]:
    """Repos whose ``.worktrees/`` to keep pruned: the nastech checkout plus job workdir repo roots,
    filtered to those that actually have a ``.worktrees/`` dir."""
    repos: set = set()

    # Nastech source checkout (git installs only; wheel installs have no .git).
    with contextlib.suppress(Exception):
...
def _maybe_run_worktree_maintenance() -> None:
    ...
    git. Same conservative pruner as ``nastech -w`` startup (dirty/unpushed/locked trees untouched).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # recovery previously ran only at scheduler STARTUP. A one-shot `nastech cron run` that claimed a job and
...
def _resolve_max_parallel_workers() -> Optional[int]:
    """Max workers: env > config.yaml > unbounded (NASTECH_CRON_MAX_PARALLEL=1 restores serial)."""
    try:
        _env_par = cron_env_setting("NASTECH_CRON_MAX_PARALLEL").strip()
        if _env_par:
