...
def _open_continuable_cron_thread(job: dict, adapter, chat_id: str, loop) -> Optional[str]:
    ...
    if not callable(create_thread) or loop is None:
        return None
    thread_name = f"Nastech — {_cron_display_name(job)}"
    try:
...
def _run_bot_chat_turn(argv: list, env: dict, report_path: str, timeout: float) -> subprocess.CompletedProcess:
    """Run one ``nastech chat -Q`` delivery child; the cap bounds the TURN, not the process.

    The child records its turn outcome at *report_path* (``nastech_cli.quiet_single_query``)
    ...
    """
    from nastech_cli.quiet_single_query import read_turn_report
...
def _deliver_to_bot_chat(job: dict, content: str, profile: str, *, deferred: Optional[dict] = None,
    ...
    try:
        source_home = get_nastech_home().resolve()
    ...
    # The running install first (same trust order as gateway.run._resolve_nastech_bin): the
    # scheduler lives in the long-running gateway, so a PATH-first lookup would hand delivery
    # to whatever `nastech` PATH names — another install, or a planted one — instead of this one.
    try:
        import importlib.util as _ilu
        found = _ilu.find_spec("nastech_cli") is not None
    except Exception:
        found = False
    if found:
        ...
        if not nastech_bin:
            return ("Nastech could not deliver this result to Bot Chat: the `nastech` command was not found. "
                    "The result is saved; run `nastech cron runs` to see it, or `nastech doctor` if this keeps happening")
        argv = [nastech_bin]
    ...
    try:
        with tempfile.NamedTemporaryFile(
            "w", encoding="utf-8", suffix=".txt", prefix="nastech-cron-botchat-", delete=False,
        ) as fh:
            fh.write(message)
            query_file = fh.name
        ...
        from nastech_cli.quiet_single_query import TURN_REPORT_FILE_ENV
        ...
        if result.returncode != 0:
            ...
            return (
                f"Nastech could not deliver this result to Bot Chat (profile '{profile_label}'). "
                "The result is saved; run `nastech cron runs` to see it, or `nastech doctor` if this keeps happening"
                f". Details: {tail}")
...
def _deliver_result(
    ...
    # job's own attempt: a worker's script may dispatch another job in-process (`nastech cron run`),
    ...
    # persisted as ``last_delivery_unverified`` so `nastech cron list` shows it.
    ...
    # at boot; standalone runs (`nastech cron run`) did not, silently dropping files. Idempotent.
