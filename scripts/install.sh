...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://nastechresearch.github.io/nastech-agent/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Nastech currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Nastech requires Python >=3.11,<3.14"
...
# replaced with the Nastech-managed Node $NODE_VERSION.
...
check_node() {
    ...
    if [ -x "$NASTECH_HOME/node/bin/node" ] && [ -x "$NASTECH_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$NASTECH_HOME/node/bin/node" --version)"; then
    ...
    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Nastech-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Nastech requires Node 22.22+, 24.11+, or 26+) — installing Nastech-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.nastech/node/.
#
...
install_node_line() {
    ...
    local installed_ver
    if ! installed_ver=$("$NASTECH_HOME/node/bin/node" --version 2>&1); then
...
# install — the same closure `nastech update` installs
# (nastech_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_browser_use_cli() {
    ...
    # `nastech tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$NASTECH_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Nastech' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$NASTECH_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'nastech tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `nastech tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `nastech computer-use install` on their own).
    ...
    # Same upstream installer `nastech computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Nastech install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Nastech'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'nastech tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: nastech computer-use install"
...
install_desktop() {
    ...
    # supported Node lines and prepends the Nastech-managed Node to PATH, so
