...
# immediately), retaining NASTECH_UPDATE_STARTED_AT from the Desktop hand-off.
# nastech_cli/update_lock.py's ancestry rule lets our
...
$NastechHome = if ($InstallRoot) { Split-Path -Parent $InstallRoot } else { $TempDir }
...
$script:Ui = $null
$script:UiStage = "Nastech will open once done."   # until the first gate; matches ui.html
...
function Stop-UiServer([switch]$LeaveWindow) {
    ...
    # profile plus any stale nastech-update-ui-* leftovers from interrupted
    ...
    # update marker (.nastech-update-in-progress) serialises hand-offs, so no
    # other run's profile can be in active use here.
    try {
        $profileDirs = @()
        if ($script:UiServer.Profile) { $profileDirs += $script:UiServer.Profile }
        Get-ChildItem -LiteralPath $TempDir -Directory -Filter "nastech-update-ui-*" -ErrorAction SilentlyContinue |
...
function Publish-UiProgress([string]$Message) {
    ...
    # stderr remain asynchronously drained in Invoke-NastechStep and are never
    ...
    if ($htmlPath -and $browser) {
        ...
        if ($server) {
            try {
                ...
                $browserProfile = Join-Path $TempDir ("nastech-update-ui-{0}" -f $PID)
                $browserArgs = @(
...
function Show-ErrorFinale([string]$Message) {
    ...
    # log tail -- `nastech debug share` uploads the real evidence and the
    ...
    try {
        ...
        $ui.Sub.Text = "Run `"nastech debug share`" in a terminal to send a report."
        ...
        try {
            $ui.Form.Activate()
            if ($script:Win32) { [NastechHandoff.Win32]::SetForegroundWindow($ui.Form.Handle) | Out-Null }
        } catch {}
...
function Show-ManualFinale([string]$Message) {
    ...
    # they reopen Nastech themselves.
    if ($script:UiServer) {
...
function Start-DesktopRelaunch {
    ...
    # update it can remove the old Nastech.exe before writing the replacement,
    ...
    if (-not $spawned) {
        ...
        # verify by watching for a fresh Nastech process.
        try {
            ...
            while ((Get-Date) -lt $explorerDeadline) {
                ...
                if ($fresh.Count -gt 0) {
                    ...
                    try {
                        if ($script:Win32) {
                            [NastechHandoff.Win32]::AllowSetForegroundWindow([int]$fresh[0].Id) | Out-Null
...
# closes it. `nastech update` deliberately runs its build steps with stdout
# inherited (nastech_cli/main.py, the tee-stderr runner), so the tree under a
...
if ($env:NASTECH_UPDATE_PIPE_DRAIN_SECONDS) {
    $parsedGrace = 0
    if ([int]::TryParse($env:NASTECH_UPDATE_PIPE_DRAIN_SECONDS, [ref]$parsedGrace) -and $parsedGrace -ge 0) {
...
if ($env:NASTECH_UPDATE_STEP_IDLE_SECONDS) {
    $parsedIdle = 0
    if ([int]::TryParse($env:NASTECH_UPDATE_STEP_IDLE_SECONDS, [ref]$parsedIdle) -and $parsedIdle -gt 0) {
        $script:StepIdleTimeoutSeconds = $parsedIdle
    }
}

# Silence on the pipes is NOT silence in the update. `nastech update` captures
# the (very loud) Electron/vite build into logs/update.log instead of its own
# stdout (nastech_cli/update_cmd.py, the update-log tee), so a real update is
...
if ($env:NASTECH_UPDATE_PROGRESS_LOG) {
    $script:StepProgressLogPath = $env:NASTECH_UPDATE_PROGRESS_LOG
}
...
if (-not ("NastechUpdateJob" -as [type])) {
...
public static class NastechUpdateJob {
...
function Invoke-NastechStep([string]$Exe, [string[]]$NastechArgs, [string]$Tag) {
    ...
    # afterwards, where `nastech debug share` picks it up.
    #
    ...
    # .nastech-update-result.json, clearing .nastech-update-in-progress,
    ...
    # Nastech" until the user killed something by hand. Losing the tail of a
    ...
    $arguments = ($NastechArgs | ForEach-Object { '"{0}"' -f ($_ -replace '"', '\"') }) -join ' '
    ...
    try {
        ...
        $started = [NastechUpdateJob]::StartAssigned($Exe, $arguments)
    } finally {
    ...
    while ($true) {
        ...
        if ($proc.HasExited) {
            ...
            # Quiet pipes are how a healthy `nastech update` looks for 40+
            ...
            if ($currentLogStamp -ne $progressLogStamp) {
                ...
                $stalled = [NastechUpdateJob]::TerminateAndWait($job, 124, 10000)
                if (-not $stalled) {
...
function Resolve-NastechVenvDir([string]$Root) {
    # Match nastech_constants.project_venv_dir(): installer-created venv wins,
...
# the page can be QA'd in any browser; NASTECH_SELFTEST_FAIL=1 exercises the
# error state, NASTECH_SELFTEST_HOLD_SECONDS delays the terminal event.
if ($SelfTestUi) {
    ...
    $hold = 6
    if ($env:NASTECH_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:NASTECH_SELFTEST_HOLD_SECONDS }
    Publish-UiProgress "Testing quiet update"
    Start-Sleep -Seconds $hold
    if ($env:NASTECH_SELFTEST_FAIL) {
...
# -SelfTestPipeDrain: prove Invoke-NastechStep survives a leaked pipe ------
# The #90455 deadlock needs no update, no checkout and no Nastech install to
...
#            update log, the shape of every real `nastech update` build (output
...
if ($SelfTestPipeDrain) {
    ...
    if ($env:NASTECH_SELFTEST_HOLD_SECONDS) { $hold = [int]$env:NASTECH_SELFTEST_HOLD_SECONDS }
    $floodKb = 8192
    if ($env:NASTECH_SELFTEST_FLOOD_KB) { $floodKb = [int]$env:NASTECH_SELFTEST_FLOOD_KB }
    # $PSHOME is this interpreter's own directory -- no hardcoded system path.
    $powershell = Join-Path $PSHOME "powershell.exe"
    $stamp = [Guid]::NewGuid().ToString("N")
    $childPs1 = Join-Path $TempDir "nastech-pipe-drain-$stamp.ps1"
    $floodPs1 = Join-Path $TempDir "nastech-pipe-flood-$stamp.ps1"
    $pidFile = Join-Path $TempDir "nastech-pipe-drain-$stamp.pid"
    $stallPs1 = Join-Path $TempDir "nastech-step-stall-$stamp.ps1"
    $stallPidFile = Join-Path $TempDir "nastech-step-stall-$stamp.pid"
    $stallGrandchildPidFile = Join-Path $TempDir "nastech-step-stall-grandchild-$stamp.pid"
    $logStallPs1 = Join-Path $TempDir "nastech-step-logstall-$stamp.ps1"
    $logStallProgress = Join-Path $TempDir "nastech-step-logstall-$stamp.update.log"
...
'@
    ...
    # merely loud. `nastech update` is this shape -- the Electron/vite build
...
'@
    ...
    $res = Invoke-NastechStep $powershell @(
    ...
    $flood = Invoke-NastechStep $powershell @(
    ...
    $stall = Invoke-NastechStep $powershell @(
    ...
    try {
        $logstall = Invoke-NastechStep $powershell @(
    ...
    if ($stallAlive) { $problems += "stalled child pid $stallPid remained alive after Invoke-NastechStep returned" }
    if ($stallGrandchildAlive) { $problems += "stalled descendant pid $stallGrandchildPid remained alive after Invoke-NastechStep returned" }
...
try {
    ...
    try {
        ...
        $startedAt = 0L
        $hasStartedAt = [int64]::TryParse($env:NASTECH_UPDATE_STARTED_AT, [ref]$startedAt)
    ...
    # Desktop launches us from NASTECH_HOME; pin the process directory to the
    ...
    $VenvDir = Resolve-NastechVenvDir $InstallRoot
    ...
    if ($SelfTestWorkingDirectory) {
        ...
        $probe = Invoke-NastechStep $probeExe @("-NoProfile", "-Command", "[Environment]::CurrentDirectory") "cwd"
    ...
    Publish-UiProgress "Waiting for Nastech to close"
    if ($DesktopPid -gt 0) {
    ...
    Publish-UiProgress "Preparing Nastech files"
    $shim = Join-Path $VenvDir "Scripts\nastech.exe"
    if (Test-Path -LiteralPath $shim) {
        ...
        if (-not $unlocked) {
            ...
            $finalCode = 5
            $finalMsg = "Update aborted: another process is still holding the Nastech install open ($shim locked after 20s). Nothing was changed. Close other Nastech windows/terminals and try again."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    ...
    #
    # DRIVE THE UPDATE THROUGH venv\Scripts\python.exe, NOT venv\Scripts\nastech.exe.
    # `uv pip install -e .` has to replace the console-script shims, so
    # _quarantine_running_nastech_exe must first rename the running nastech.exe
    # out of the way. On Windows that rename fails whenever ANY child process
    # spawned from that nastech.exe is still alive: a child inherits a handle on
    # the parent image, and the resulting sharing violation is indistinguishable
    # from a user leaving a second Nastech window open. It is the inherited
    ...
    # install whose Start Menu shortcut points at a Nastech.exe that no longer
    ...
    #
    # Running the same code as `python.exe -m nastech_cli.main update` puts the
    ...
    if (-not (Test-Path -LiteralPath $pythonExe)) {
        $finalCode = 3
        $finalMsg = "Update aborted: $pythonExe is missing. The install needs repair (run the Nastech installer or `nastech doctor`)."
        Write-HandoffLog $finalMsg
        exit $finalCode
    }
    $updateArgs = @("-m", "nastech_cli.main", "update", "--yes", "--gateway", "--force", "--branch", $Branch)
    ...
    # collides with the "close all Nastech windows" sentinel.
    try {
        $updateHelp = & $pythonExe -m nastech_cli.main update --help 2>$null | Out-String
        if ($updateHelp -match "--keep-stash") {
            $updateArgs += "--keep-stash"
        } else {
            Write-HandoffLog "installed nastech predates --keep-stash; running without it"
        }
    } catch {
    ...
    $res = Invoke-NastechStep $pythonExe $updateArgs "update"
    ...
    if (Test-Path -LiteralPath $retryPolicyPath) {
        . $retryPolicyPath
        $shouldRetry = Test-NastechUpdateShouldRetry -ExitCode $res.Code -InstallRoot $InstallRoot
    } else {
    ...
    if ($res.Code -eq 0 -and $res.Output -match "Desktop build failed") {
        ...
        $rebuild = Invoke-NastechStep $pythonExe @("-m", "nastech_cli.main", "desktop", "--force-build", "--build-only") "rebuild"
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        $verifyCode = "import nastech_cli.main; from nastech_cli.desktop_update_verify import verify_windows_desktop_update; verify_windows_desktop_update()"
        $verify = Invoke-NastechStep $pythonExe @("-c", $verifyCode) "verify"
        if ($verify.Code -ne 0) {
            $finalCode = 8
            $finalMsg = "The updated Nastech runtime or Desktop build failed verification. Repair the installation and review antivirus quarantine before retrying."
            Write-HandoffLog $finalMsg
            exit $finalCode
        }
    }
    ...
    if ($res.Code -eq 0 -and -not $desktopBuildFailed) {
        ...
        $finalCode = $res.Code
        $finalMsg = "Update failed (exit $($res.Code)). Run `nastech debug share` in a terminal to send a report."
    }
    exit $finalCode
} finally {
    ...
    #   3. only then the terminal UI state — done means "Nastech is back",
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        $finalCode = 7
        $finalMsg = "Update recovery could not stop every updater process. Nastech was not restarted to avoid overlapping the active install. Wait for it to finish or restart Windows, then reopen Nastech."
        ...
        if ($finalCode -ne 0) {
            ...
            Publish-UiProgress "Opening Nastech"
            ...
            if (-not $cameBack -and $RelaunchExe) {
                ...
                $finalMsg = "Update complete. Reopen Nastech to finish (it could not restart itself)."
