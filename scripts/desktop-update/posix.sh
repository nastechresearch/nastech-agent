...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  ...
  "$NASTECH_BIN" desktop --force-build --build-only >> "$LOG" 2>&1 || {
...
else FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run nastech debug share in a terminal to send a report."; fi
exit "$FINAL_CODE"
