...
# changing PATH, installing software, or touching the user's Nastech home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("nastech-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$NastechHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -NastechHome $NastechHome -InstallDir $InstallDir
