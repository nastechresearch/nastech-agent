...
param(
    ...
    #     `nastech desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:NastechResearch/nastech-agent.git"
$RepoUrlHttps = "https://github.com/NastechResearch/nastech-agent.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by Nastech Research.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $NastechHome)) {
        New-Item -ItemType Directory -Force -Path $NastechHome | Out-Null
    }
    $envFile = Join-Path $NastechHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `nastech update`'s npx
    ...
    $prefixDir = Join-Path $NastechHome "node"
...
function Install-Uv {
    ...
    Write-Info "Installing managed uv into $NastechHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $NastechHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $NastechHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $NastechHome "bin"
        ...
        # (nastech_cli/managed_uv.py looks only at $NastechHome\bin\uv.exe).
...
# Upgrade the Nastech-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Nastech-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Nastech belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # nastech_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".nastech-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Nastech-managed interpreter
    ...
    #
    # Under Nastech-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Nastech-managed Python $ver`: $_"
        } finally {
...
function Install-Git {
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\nastech\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Write-Info "Git not found -- downloading PortableGit to $NastechHome\git\ ..."
...
function Set-GitBashEnvVar {
    ...
    #
    # Layouts:
    #   PortableGit (our default): $NastechHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $NastechHome\git\usr\bin\bash.exe
    $candidates += "$NastechHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$NastechHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
...
# range used to provision the Nastech-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Nastech requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Nastech requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Nastech-managed Node.js installation instead..."
    ...
    # The portable zip path drops node.exe + npm into $NastechHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $NastechHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$NastechHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$NastechHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$NastechHome\node") {
                    try {
                        Rename-Item "$NastechHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Nastech-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\nastech-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `nastech
                    # update` path (#4735).
                    ...
                    $stashName = "nastech-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
            ...
            try {
                ...
                Write-Info "terminals, running nastech processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NastechResearch/nastech-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NastechResearch/nastech-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NastechResearch/nastech-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\nastech-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\nastech-agent-extract"
    ...
    # next `nastech update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    if (-not $resolvedPython) {
        throw "Nastech-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running nastech process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running nastech processes before recreating venv..."
            ...
            # The launcher CLI (nastech.exe) plus its child tree.
            & taskkill /F /T /IM nastech.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM nastech.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m nastech_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not nastech.exe.
            ...
            # Nastech children re-exec through .nastech-runtime, so killing only
            ...
            # user (see nastech_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Nastech " +
...
function Restore-VenvBackup {
    ...
    # venv so Nastech (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `nastech.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install nastech-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and nastech.exe broken with
    ...
    # here, not 30 seconds later when the user runs `nastech`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Nastech processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Nastech processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Nastech and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register nastech.exe in dist-info/RECORD but fail to
        ...
        # `nastech` command while nastech-agent.exe / nastech-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m nastech_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `nastech dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `nastech dashboard` should now work."
            } else {
...
function Install-NastechCommandLaunchers {
    ...
    # Requiring nastech.exe before creating the destination keeps the PATH
    ...
    # nastech_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("nastech", "nastech-acp")) {
    ...
    $requiredExe = Join-Path $Destination "nastech.exe"
    $requiredCmd = Join-Path $Destination "nastech.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the nastech command: launcher was not installed: $requiredExe"
    }
    return $Destination
}
...
function Set-PathVariable {
    ...
    if ($NoVenv) {
        ...
        # $NastechHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `nastech update`'s autostash
        ...
        # installer staged under nastech-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-NastechCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $nastechBin = "$NastechHome\bin"
        Install-NastechCommandLaunchers -Root $InstallDir -Destination $nastechBin | Out-Null
    }
    ...
    #   nastech-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The nastech-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept nastech via $nastechBin)"
        }
    }
    
    if ($currentPath -notlike "*$nastechBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$nastechBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $nastechBin"
    } else {
    ...
    Write-Success "nastech command ready"
}
...
function Write-BootstrapMarker {
    ...
    $markerPath = Join-Path $InstallDir ".nastech-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    New-Item -ItemType Directory -Force -Path "$NastechHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NastechHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NastechHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NastechHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NastechHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$NastechHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$NastechHome\.env"
    ...
    # Create config.yaml
    $configPath = "$NastechHome\config.yaml"
    ...
    # PowerShell version.
    $soulPath = "$NastechHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in nastech_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $NastechHome"
    
    # Seed bundled skills into $NastechHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $NastechHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            $userSkills = "$NastechHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $NastechHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'nastech setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\nastech-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\nastech-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
...
# failure is non-fatal (browser_exec can still run via uvx, and `nastech tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $NastechHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Nastech' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Nastech' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'nastech tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `nastech
...
# paths still lazy-install via install_cua_driver() (nastech_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `nastech computer-use install` runs. Bounded
        ...
        # Nastech' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'nastech tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: nastech computer-use install"
            }
        } else {
...
function Install-Desktop {
    ...
    # enforces the supported Node lines and prepends the Nastech-managed Node to
...
function Install-PlatformSdks {
    ...
    # ~/.nastech/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'nastech setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m nastech_cli.main setup
    } else {
...
function Start-GatewayIfConfigured {
    ...
    $nastechCmd = "$InstallDir\venv\Scripts\nastech.exe"
    if (-not (Test-Path $nastechCmd)) {
        $nastechCmd = "nastech"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$NastechHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'nastech whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: nastech gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$NastechHome\logs\gateway.log"
            Start-Process -FilePath $nastechCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$NastechHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: nastech gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: nastech gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$NastechHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$NastechHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NastechHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$NastechHome\nastech-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   nastech              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   nastech setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   nastech config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   nastech config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   nastech gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   nastech update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    # Managed Python lives under $InstallDir\.nastech-runtime, so the checkout
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://nastechresearch.github.io/nastech-agent/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
