...
Run after changing any ``desktop=`` value or alias in ``nastech_cli/commands.py``;
``tests/nastech_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from nastech_cli.commands import desktop_surface_registry
