 ...
 *   The artifact published on the website right now -- Nastech-Setup.exe has
 ...
 * @typedef {InstallMethod | 'nastech-update' | 'open-app-update' | 'nastech-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; nastech-desktop-app-update starts the app
 *   via `nastech desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      ...
      // Website Nastech-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `nastech desktop`.
      { method: 'nastech-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Nastech-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `nastech desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'nastech-desktop-app-update';
}
