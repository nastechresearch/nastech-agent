...
NASTECH_HOME so the operator's own telemetry state is untouched.
...
def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="nastech-telemetry-e2e-"))
    os.environ["NASTECH_HOME"] = str(scratch)
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "nastech.shared_metrics.v2",
    ...
    print(f"scratch NASTECH_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://nastech-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
