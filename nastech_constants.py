...
_UNSET = object()
_NASTECH_HOME_OVERRIDE: ContextVar[str | object] = ContextVar("_NASTECH_HOME_OVERRIDE", default=_UNSET)
...
def sudo_invoker_default_home() -> Path | None:
    """The invoking user's native ``~/.nastech`` when this process is root under ``sudo``, else None.

    sudo strips NASTECH_HOME and sets HOME=/root, so the process's own default is root's; the profile
    ...
    try:
        return Path(pwd.getpwnam(sudo_user).pw_dir) / ".nastech"
    except KeyError:  # SUDO_USER not in passwd (chroot/container)
        return None


def _warn_profile_fallback_once() -> None:
    """Warn once when NASTECH_HOME is unset but a non-default profile is sticky-active (wrong fallback)."""
...
def nastech_home_key(path: str | Path | None = None) -> str:
    """Stable registry key for a Nastech home/profile dir.
...
def reset_nastech_home_key_cache() -> None:
...
def get_process_nastech_home() -> Path:
    """Nastech home of the running process, ignoring task overrides.
...
# ``nastech backup`` (excludes them) and ``profile create --clone-all`` (skips them from the
...
# Files marking a real Nastech home; arbitrary dirs with a ``profiles`` segment lack them.
_NASTECH_HOME_MARKERS = ("config.yaml", ".env", "state.db")


def _is_nastech_profiles_root(profiles_dir: Path) -> bool:
    """True when *profiles_dir* is provably ``<nastech-home>/profiles``.

    Accepts the classic ``~/.nastech`` layout, a root carrying Nastech-home marker files, a
    ``profiles/.deleted`` tombstone dir (only ``profile delete`` creates it), or the default root.
    """
    root = profiles_dir.parent
    if root.name == ".nastech":
        return True
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir() or any(
            (root / marker).exists() for marker in _NASTECH_HOME_MARKERS
        ):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_nastech_root().resolve(strict=False)
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Requires ``<name>`` not to start with ``.`` and the ``profiles`` parent to be a real Nastech home;
    ...
    for candidate in (current, *current.parents):
        if (candidate.parent.name == "profiles" and not candidate.name.startswith(".")
                and _is_nastech_profiles_root(candidate.parent)):
            return candidate
        if candidate.name == ".nastech":  # default home: a coincidental profiles/ ancestor is not a root
            return None
    return None
...
def profile_name_for_home(path: str | Path | None) -> str | None:
    ...
    The default home is the Nastech root itself, so its basename is an installation detail (``.nastech``
    on POSIX and commonly ``nastech`` on Windows), not the profile id ``default``.
    """
...
def named_profile_is_live(profile_home: str | Path) -> bool:
    ...
    never be started as a backend (whose ``ensure_nastech_home`` would rebuild the full tree)."""
    home = Path(profile_home)
...
def mkdir_under_nastech_home(path: str | Path) -> Path:
...
def _packaged_dir(env_var: str, default: Path | None, subdir: str) -> Path:
    ...
    return Path(override) if override else default if default is not None else get_nastech_home() / subdir


def get_optional_skills_dir(default: Path | None = None) -> Path:
    """Return the optional-skills directory, honoring package-manager wrappers."""
    return _packaged_dir("NASTECH_OPTIONAL_SKILLS", default, "optional-skills")


def get_optional_mcps_dir(default: Path | None = None) -> Path:
    """Return the optional-mcps directory, honoring package-manager wrappers (``NASTECH_OPTIONAL_MCPS``)."""
    return _packaged_dir("NASTECH_OPTIONAL_MCPS", default, "optional-mcps")


def get_bundled_skills_dir(default: Path | None = None) -> Path:
    """Return the bundled skills directory, honoring package-manager wrappers (``NASTECH_BUNDLED_SKILLS``)."""
    return _packaged_dir("NASTECH_BUNDLED_SKILLS", default, "skills")


def get_nastech_dir(new_subpath: str, old_name: str, *, home: Path | None = None) -> Path:
    """Resolve a Nastech subdirectory, honouring a populated legacy ``<old_name>/`` (no migration).
...
def iter_nastech_node_dirs(home: Path | None = None) -> list[Path]:
    """Nastech-managed Node dirs in lookup order; both Windows and POSIX shapes so migrated installs work.

    Keep in sync with nastechManagedNodePathEntries() in apps/desktop/electron/backend-env.ts.
    """
    node_dir = (home or get_nastech_home()) / "node"
...
def _version_probe_ok(path: str) -> bool:
    """True when ``<path> --version`` exits 0 under the Nastech-managed Node PATH."""
    result = _run_version_probe([path, "--version"], env=with_nastech_node_path())
...
# Install tree root (this file lives at <install_root>/nastech_constants.py). Used by secure_parent_dir() to
# skip chmod on the install dir — chmodding it 0700 breaks nastech-user traversal in Docker (UID 10000). See
# #25821, #93050.
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        "→ Nastech-managed Node.js is in use by a running app; deferring its "
        "upgrade until the app is closed (re-run `nastech update` afterwards).", flush=True,
    )
...
def _stage_windows_node_zip(home: Path, node_arch: str) -> Path | None:
    ...
    if index_bytes is None:
        return None
    pattern = rf"node-v{_NASTECH_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip"
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    ...
    (images are mapped with ``FILE_SHARE_DELETE`` — the same mechanism as the nastech.exe quarantine); when
...
def _run_node_bootstrap(func: str, *, timeout: int, **extra_env: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "NASTECH_HOME": str(get_nastech_home()), **extra_env},
...
def bootstrap_nastech_managed_node() -> str | None:
    """Install a Nastech-managed Node tree under ``$NASTECH_HOME/node`` and return its npm path.

    Nastech never modifies a user-owned toolchain (system, nvm, brew, Nix) that fails ``engines``.
    """
    ...
    if sys.platform == "win32":
        ...
        # NASTECH_NODE_SKIP_LINKS=1 keeps node/npm/npx out of ~/.local/bin: never shadow the user toolchain.
        ok = _run_node_bootstrap("_nb_install_bundled_node", timeout=600, NASTECH_NODE_SKIP_LINKS="1")
    if not ok:
        return None
...
def heal_nastech_managed_node() -> bool:
    """Redownload Nastech-managed Node when the tree exists but is broken; at most once per process.
    ...
    if _managed_node_heal_attempted or not nastech_managed_node_tree_present():
        return False
    if sys.platform == "win32":
...
def find_nastech_node_executable(command: str) -> str | None:
    """Nastech-managed Node/npm path, healing broken/outdated trees; heal failure still returns old Node."""
...
def find_node_executable(command: str) -> str | None:
    ...
    """
    managed = find_nastech_node_executable(command)
    if managed:
        return managed
    if nastech_managed_node_tree_present():
        return None
...
def with_nastech_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    for entry in reversed([str(path) for path in iter_nastech_node_dirs() if path.is_dir()]):
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    local ``node_modules/agent-browser/bin/...`` binary, which then disappears on the next ``nastech update``
...
def display_nastech_home(home: Path | None = None) -> str:
    """User-facing ``~/`` display string for NASTECH_HOME (``~/.nastech/profiles/coder``).

    ``home`` overrides the lookup for callers that run before the CLI has applied the sticky
    ``active_profile`` (``get_nastech_home()`` would emit the wrong-profile fallback warning there).
    """
    if home is None:
        home = get_nastech_home()
    try:  # as_posix(): str() on Windows yields chimeras like ~/AppData\Local\nastech/skills/
...
def profile_cli_selector() -> str:
    """``-p <name> `` (trailing space) pinning copy-pasteable ``nastech ...`` guidance to the
    active NAMED profile, else ``""``: a bare ``nastech`` follows the sticky ``active_profile``
    file, which can name a different database than the one that failed (#105887). A custom
    home outside the profile tree has no selector (only NASTECH_HOME names it)."""
    name = profile_name_for_home(get_nastech_home())
    return f"-p {name} " if name and name != "default" else ""


def secure_parent_dir(path: Path) -> None:
    """Chmod ``0o700`` on *path*'s parent, refusing ``/`` and top-level dirs (misresolved NASTECH_HOME)."""
    parent = path.parent.resolve()
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree: chmod 0700 breaks nastech-user traversal in Docker (UID 10000).
    # A credential file here means NASTECH_HOME misresolved; surface it (caused production lockouts).
    # See #25821, #93050.
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        ...
        logging.getLogger(__name__).warning(
            ...
            "nastech-agent install directory (%s). Credential files are "
            "normally stored under the nastech home directory instead.", parent, _INSTALL_ROOT,
        )
        return
    with contextlib.suppress(OSError):
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates = [_env_get(env, "NASTECH_REAL_HOME"), _env_get(env, "HOME")]
    with contextlib.suppress(Exception):
        import pwd
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """The OS user's real home, avoiding the Nastech profile HOME.

    ``HOME`` belongs to the OS account and external CLIs keeping credentials under ``~``; a parent
    already running with ``HOME={NASTECH_HOME}/home`` is repaired back when possible.
    """
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    ``{NASTECH_HOME}/home``; ``real``: always real HOME; ``profile``: always the profile home.
    """
    env = env or {}
    profile_home = _profile_home_path(env)
...
def translate_cwd_for_wsl_backend(cwd: str) -> str:
    """Map a Windows-host cwd (drive path or ``\\\\wsl.localhost\\`` UNC) to POSIX when Nastech runs in WSL.
...
def is_container() -> bool:
    ...
    See: NastechResearch/nastech-agent#47111
    """
...
def openrouter_variant_base(model_id: str) -> str | None:
    ...
    Lives here rather than in ``nastech_cli.models`` so the metadata layer
...
def venv_bin_dir(venv_dir, *, windows: bool | None = None) -> Path:
    ...
    Canonical helper for venv layout. This was open-coded in seven places across four ``nastech_cli`` modules
    ...
    lived 2400 lines away in another function. A few sites outside ``nastech_cli``
...
def is_first_party_module(name: str | None) -> bool:
    """True when *name* ships with Nastech (exact first segment; ``startswith`` would claim ``agentops``)."""
    root = str(name).split(".")[0] if name else ""
    return bool(root) and (root in FIRST_PARTY_MODULE_ROOTS or root.startswith("nastech_"))
