...
def get_nastech_home() -> Path:
    ...
    ``nastech_cli/kanban_db.py``).  See https://github.com/NastechResearch/nastech-agent/issues/18594.
    """
...
# Process-level memo for get_default_nastech_root(). The function resolves
# NASTECH_HOME against the native home on every call (~80us of path
...
# Its result depends only on (NASTECH_HOME, platform native home), which are
# compared for free on each call, so the memo is freshness-correct even if a
# test or plugin mutates NASTECH_HOME mid-process.
...
def get_default_nastech_root() -> Path:
    ...
    if _default_nastech_root_memo is not None:
        memo_native, memo_env, memo_result = _default_nastech_root_memo
...
# Files whose presence marks a directory as a real Nastech home. A fresh home
...
def _is_nastech_profiles_root(profiles_dir: Path) -> bool:
    """Return True when *profiles_dir* is a canonical ``<nastech-home>/profiles``.

    Anchors named-profile recognition so it only fires for directories that
    provably live under a Nastech home: the classic ``~/.nastech`` layout, a
    root carrying Nastech-home marker files (Docker/custom ``NASTECH_HOME``
    like ``/opt/data``), a ``profiles/.deleted`` tombstone directory (only
    ever created by ``nastech profile delete``), or the process's resolved
    default Nastech root.
    """
    ...
    try:
        if (profiles_dir / _DELETED_PROFILES_DIR).is_dir():
            return True
        if any((root / marker).exists() for marker in _NASTECH_HOME_MARKERS):
            return True
    except OSError:
        pass
    try:
        return root.resolve(strict=False) == get_default_nastech_root().resolve(
            strict=False
        )
    except OSError:
        return False
...
def named_profile_home(path: str | Path) -> Path | None:
    ...
    Nastech home (see :func:`_is_nastech_profiles_root`). A default Nastech home
    whose path merely contains a ``profiles`` segment
    (e.g. ``/tmp/foo/profiles/notahome/.nastech``) is not a named profile,
    ...
    for candidate in (current, *current.parents):
        if (
            ...
            return candidate
        # Stop at a default Nastech home so a coincidental ``profiles/``
        # ancestor is not treated as a named-profile root.
        if candidate.name == ".nastech":
            return None
    return None
...
def assert_named_profile_home_live(path: str | Path) -> None:
    ...
    Default ``NASTECH_HOME`` (not under ``profiles/``) is unchanged.
    """
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    Mirrors :func:`get_optional_skills_dir` for the MCP catalog (Nastech-approved
...
# Install tree root (this file lives at <install_root>/nastech_constants.py).
# Used by secure_parent_dir() to skip chmod on the install dir — chmodding it
# 0700 breaks nastech-user traversal in Docker (UID 10000). See #25821, #93050.
...
def _print_managed_node_in_use_notice() -> None:
    ...
    print(
        ...
        "upgrade until the app is closed (re-run `nastech update` afterwards).",
        flush=True,
    )
...
def _heal_managed_node_windows(home: Path | None = None) -> bool | None:
    ...
    ``FILE_SHARE_DELETE`` — the same mechanism as the nastech.exe quarantine);
...
def heal_nastech_managed_node() -> bool:
    ...
    if _managed_node_heal_attempted:
        return False
    if not nastech_managed_node_tree_present():
        return False
...
def with_nastech_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    ...
    managed = [str(path) for path in iter_nastech_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``nastech update`` — leaving a **dangling symlink** that ``which`` still
...
def display_nastech_home() -> str:
    ...
    try:
        ...
        # ``~/AppData\Local\nastech/skills/`` once callers append
...
def secure_parent_dir(path: Path) -> None:
    ...
    Also refuses to chmod the nastech-agent install tree (the directory this
    ...
    See https://github.com/NastechResearch/nastech-agent/issues/25821 and
    https://github.com/NastechResearch/nastech-agent/pull/93050.
    """
    parent = path.parent.resolve()
    # Refuse root and its direct children (/usr, /home, /var, /tmp, …).
    if parent == Path("/") or len(parent.parts) < 3:
        return
    # Refuse the install tree root. chmodding it 0700 breaks nastech-user
    ...
    if parent == _INSTALL_ROOT or _INSTALL_ROOT in parent.parents:
        # A credential file inside the install tree usually means NASTECH_HOME
        ...
        logging.getLogger(__name__).warning(
            ...
            "normally stored under the nastech home directory instead.",
            parent,
            _INSTALL_ROOT,
        )
        return
