...
# (nastech_state._rows_to_conversation), so any code that mutates a loaded or
...
class AIAgent:
    ...
    def _should_emit_quiet_tool_messages(self) -> bool:
        ...
        ``nastech chat -Q``) always wins: those flows neutralize the rendering
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        the ``NASTECH_API_CALL_STALE_TIMEOUT`` env var. Reasoning-model floors
        ...
        if get_provider_stale_timeout(self.provider, self.model) is not None:
            return True
        return os.getenv("NASTECH_API_CALL_STALE_TIMEOUT") is not None
    ...
    def _record_file_mutation_result(
        ...
        if landed:
            ...
            # can tell Nastech-authored content from later user hand-edits.
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "replaced":
                return (
                    ...
                    "`nastech doctor --fix` or in-place FTS repair — stop "
            ...
            if cause == "corrupt":
                ...
                # NASTECH_HOME do not live under ~/.nastech).
                ...
                return (
                    ...
                    "further\n"
                    "3. Restore from a backup in ~/.nastech/backups/\n"
    ...
    def _anthropic_messages_create(self, api_kwargs: dict, *, client: Any = None):
        ...
        return create_anthropic_message(
            ...
            # matching header families (x-ratelimit-* / x-nastech-credits-*).
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        """
        if base_url_host_matches(self._base_url_lower, "nastechresearch.github.io"):
            return True
    ...
    def run_conversation(
        ...
        try:
            ...
            # Serialize the full load -> run -> flush region across Nastech
            ...
            if (
                ...
                def _on_session_turn_lease_wait(elapsed: float) -> None:
                    ...
                    if elapsed < 1.0:
                        self._emit_status(
                            "⏳ Another Nastech process is using this session; "
                            "waiting for it to finish before starting your turn..."
                        )
                    else:
                        self._emit_status(
                            "⏳ Still waiting for the other Nastech process on "
                ...
                if not _turn_db.acquire_session_turn_lease(
                    ...
                    if getattr(self, "_interrupt_requested", False):
                        ...
                        interrupt_msg = (
                            "Stopped waiting for another Nastech process on "
                    ...
                    timeout_msg = (
                        "⏳ Another Nastech process kept this session busy too "
            ...
            if getattr(relay_turn, "relay_enabled", True):
                ...
                task_started = True
            # Publish the conversation id for ambient Nastech Portal tagging. Every
