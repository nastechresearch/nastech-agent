...
# 5. Symlinks the 'nastech' CLI command into a user-facing bin dir
...
if is_termux; then
    # Nastech currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Nastech requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink nastech into a user-facing bin dir
# ============================================================================
...
echo "Other commands:"
echo "  nastech status        # Check configuration"
if is_termux; then
    echo "  nastech gateway       # Run gateway in foreground"
else
    echo "  nastech gateway install # Install gateway service (messaging + cron)"
fi
echo "  nastech cron list     # View scheduled jobs"
echo "  nastech doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m nastech_cli.main setup
fi
