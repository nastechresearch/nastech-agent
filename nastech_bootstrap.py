"""Process bootstrap for Nastech entry points: Windows UTF-8 stdio, import-path
...
point (``nastech``, ``nastech-agent``, ``nastech-acp``, ``gateway.run``, ``batch_runner``,
...
here may pull in a Nastech package that a project-local directory could shadow.
"""
...
def _patch_urllib3_create_connection(module) -> None:
    ...
    if getattr(module.create_connection, "_nastech_happy_eyeballs", False):
        return
    ...
    _urllib3_racer._nastech_happy_eyeballs = True  # type: ignore[attr-defined]
...
class _Urllib3ConnectionPatcher(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    Importing urllib3 eagerly costs ~50 ms on every CLI start, and ``nastech`` / the TUI
...
def install_happy_eyeballs_socket_connect() -> None:
    ...
    """
    if getattr(socket.create_connection, "_nastech_happy_eyeballs", False):
        return
    ...
    _socket_racer._nastech_happy_eyeballs = True  # type: ignore[attr-defined]
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``nastech_cli._subprocess_compat.suppress_platform_ver_console`` for callers
    that never import ``nastech_cli.main``; double application is harmless.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    ...
    Nastech ships top-level modules with common names (``utils``, ``proxy``, ``ui``); a
    ...
    dropped and the Nastech root is *relocated* to the front, not merely inserted when
...
def activate_durable_lazy_target() -> None:
    """Put the durable lazy-install dir (``NASTECH_LAZY_INSTALL_TARGET``) on ``sys.path``.
...
# Apply on import — entry points only need ``import nastech_bootstrap`` first.
