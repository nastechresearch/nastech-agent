...
Extracted verbatim from nastech_state.py so the SessionDB mixin modules
(nastech_state_search / nastech_state_schema / nastech_state_portability) can
reference them without importing nastech_state (which would be a cycle).
nastech_state re-imports every name here for backward compatibility.
"""
...
# `nastech sessions optimize-storage` rebuilds it on a capable host.
...
# service, the Desktop app's `nastech serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `nastech_state._cross_process_repair_lock` (the schema-
...
# rebuilder cannot wedge future rebuilds. It lives here (not nastech_state)
# because the search/schema mixins cannot import nastech_state (cycle).
#
