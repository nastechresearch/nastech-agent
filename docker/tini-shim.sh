...
# Nastech WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: NASTECH_TINI_SHIM_TARGET overrides the `/init` path (and
# NASTECH_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${NASTECH_TINI_SHIM_TARGET:-/init}"
WRAPPER="${NASTECH_TINI_SHIM_WRAPPER:-/opt/nastech/docker/main-wrapper.sh}"
