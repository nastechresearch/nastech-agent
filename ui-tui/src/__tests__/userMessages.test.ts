import { JsonRpcGatewayError } from '@nastech/shared/json-rpc-channel'
...
describe('backend lifecycle copy', () => {
  it('names the exit code, the last real stderr line, /logs and nastech doctor', () => {
    const tail =
    ...
    expect(text).toContain('/logs')
    expect(text).toContain('nastech doctor')
    expect(text).toContain('/resume')
    expect(text).not.toMatch(/\bgateway\b/)
  })
  ...
  it('only failure-looking stderr earns an activity row', () => {
    ...
    expect(stderrLooksLikeProblem('INFO nastech.mcp: discovered 3 servers')).toBe(false)
  })
