import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@nastech/shared/color'
import type { SkinBranding, SkinColors } from '@nastech/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @nastech/shared/color (the primitives,
...
// Mirrors @nastech/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @nastech/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Nastech navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Nastech look was never hand-authored: for years the
 ...
 *  answer when known (cached in NASTECH_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.NASTECH_TUI_BACKGROUND ?? '').trim()
 ...
 * the OSC-11 background answer cached into NASTECH_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into NASTECH_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Nastech navy
  ...
  // the derived value already carries the identity seeds (e.g. Nastech navy).
  const activeRow =
