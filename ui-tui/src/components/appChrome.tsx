...
const renderIndicator = (style: IndicatorStyle, tick: number): IndicatorRender => {
  ...
  if (style === 'emoji') {
    return {
      frame: EMOJI_FRAMES[tick % EMOJI_FRAMES.length] ?? '𓄃 ',
...
export function StatusRule({
  ...
  // Dev-only readout (NASTECH_DEV_CREDITS). The server omits the key entirely unless the
  ...
  // Dev-gated readout (NASTECH_DEV_CREDITS), lowest priority,
