...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo NastechResearch/nastech-agent "<your terms>"
  gh search prs --repo NastechResearch/nastech-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/NastechResearch/nastech-agent/issues?q=) · [PRs (all states)](https://github.com/NastechResearch/nastech-agent/pulls?q=is%3Apr).
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nastech Research Discord](https://discord.gg/NastechResearch). Users can install it with `nastech skills install`.
...
├── cli.py                    # NastechCLI class — interactive CLI orchestrator (~4.6k LOC + nastech_cli/cli_*_mixin.py)
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (nastech-cli, nastech-telegram, etc.)
├── nastech_state.py           # SessionDB facade (~1.4k LOC); implementation in nastech_state_*.py (21 siblings) — FTS5 search, session titles
...
│   ├── cli_*_mixin.py            # NastechCLI mixins (slash commands, display, session, ...)
│   ├── config.py                 # Config management, migration, env var definitions
│   ├── setup.py                  # Interactive setup wizard
│   ├── auth.py                   # Provider resolution, OAuth, Nastech Portal (facade + auth_*.py siblings)
...
| `~/.nastech/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and explicit `/save` exports. Automatic per-session JSON snapshots are no longer written; state.db is canonical. |
...
- **Session persistence**: All conversations are stored in SQLite (`nastech_state.py`) with full-text search and unique session titles. Automatic per-session JSON snapshots have been removed. Existing files are left untouched; use `/save json` or `nastech sessions export` for an explicit export.
...
- **Discord**: [discord.gg/NastechResearch](https://discord.gg/NastechResearch) — for questions, showcasing projects, and sharing skills
