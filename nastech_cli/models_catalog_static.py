...
maps. Split out of ``nastech_cli.models``.
"""
...
# OpenRouter entries the Nastech Portal does not carry (routing/fast variants, free tier —
...
def _codex_curated_models() -> list[str]:
    ...
    synthesis) so the gateway /model picker and the CLI ``nastech model`` flow share one source."""
...
# Mirrors the xAI-direct IDs from $NASTECH_HOME/models_dev_cache.json; the cache overrides it on the
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nastech-curated xAI models missing from models.dev, right after the pinned headline."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    """Offline curated floor for xAI / xAI OAuth pickers: $NASTECH_HOME/models_dev_cache.json
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    "moa": ["default"],
    "nastech": [mid for mid, _ in OPENROUTER_MODELS if mid not in _OPENROUTER_ONLY and not mid.endswith(":free")],
    ...
    # (see nastech_cli/model_setup_flows.py); validated live against a GCP project (global region).
    "vertex": [
...
# lists, displays, or iterates providers (nastech model, /model, list_authenticated_providers)
# derives from it. slug = internal ID (config.yaml, --provider); label = short display name;
# tui_desc = longer description for the `nastech model` picker.
# ---------------------------------------------------------------------------
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [ProviderEntry(*row) for row in (
    ("nastech", "Nastech Portal", "Nastech Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
# ...) fold under one top-level row in the INTERACTIVE PICKERS (``nastech model``, setup wizard,
...
# Offline/fresh-install fallback for the model Nastech silently lands on when the user never picked
...
# (GUI onboarding / ``nastech model``) uses the tier-aware ``get_recommended_default_model`` in
# nastech_cli/web_server.py + ``partition_nastech_models_by_tier``, which may hit the Portal.
...
_AGGREGATOR_PROVIDERS = frozenset({"nastech", "openrouter", "ai-gateway", "copilot", "kilocode"})
...
# agentic subset of 400+ models — merging would dump everything), "nastech" (curated list + Portal
...
# OpenRouter-style ids -> Copilot ids. Dash-notation Claude ids are accepted too: Nastech' default
