...
- ``nastech_cli._early_recovery.recover_if_needed`` — stdlib-only, runs BEFORE
  ``nastech_cli.main``'s third-party imports, so it can complete a pending
  update while no native extension is mapped yet (#83569).
- ``nastech_cli.main._recover_core_update_marker_locked`` — the historical
...
the corrupted-venv state it exists to repair. ``nastech_cli.main`` imports
``managed_uv``, ``nastech_constants``, and friends only in its late path; the
...
:func:`nastech_cli._early_recovery._find_uv_binary` lookup and falls back to
...
from nastech_cli import _early_recovery as _er
...
def _is_termux_env(env: dict | None = None) -> bool:
    """Stdlib Termux probe (nastech_cli.main's version lives behind imports)."""
...
def _stdout_to_stderr():
    ...
    ``nastech acp`` speaks JSON-RPC on stdout; an inherited-fd install child
...
def _venv_scripts_dir(root: Path) -> Path | None:
    ...
    # nastech_constants is stdlib-only, so the canonical layout helpers are safe
...
#: managed binary dir (the default Nastech root's ``bin``, next to uv.exe)
#: on the user PATH. Keep in lockstep with the launcher list in
#: scripts/install.ps1.
_WINDOWS_BIN_LAUNCHERS = ("nastech", "nastech-acp")


def _venv_is_relocatable(venv_dir: Path) -> bool:
    """True when the venv's pyvenv.cfg declares ``relocatable = true``.

    uv writes the flag; ``nastech_cli.managed_uv`` builds its replacement
...
def ensure_windows_bin_launchers(
    ...
    *,
    ...
    """Re-stage the Windows ``nastech`` launchers when they vanish.

    On Windows, ``nastech`` resolves through launchers derived from the venv
    ...
    the managed binary dir — the default Nastech root's ``bin``
    (``%LOCALAPPDATA%\\nastech\\bin``, next to the managed uv) — which lives
    OUTSIDE the git checkout so no git operation can ever touch it. It is
    a per-machine dir shared by every profile: ``get_nastech_home()`` would
    point inside ``profiles\\<name>`` under ``nastech -p``, so the anchor
    here is :func:`nastech_constants.get_default_nastech_root`.

    Earlier installer versions staged them at ``<checkout>\\bin`` instead —
    inside the git working tree — where ``nastech update``'s pre-update
    ...
    nothing restored them and ``nastech`` stopped resolving in every new
    ...
    - canonical managed binary dir: only when *root* is the managed clone
      (``root.parent == get_default_nastech_root()``), so source checkouts
    ...
    *windows* and *user_path_entries* are injectable for tests, same pattern
    as ``nastech_constants.venv_bin_dir``.
    """
    if windows is None:
        windows = _is_windows()
    if not windows:
    ...
    # Per-machine anchor: the DEFAULT Nastech root, not get_nastech_home() —
    # under ``nastech -p <name>`` that returns ``profiles\\<name>``, which
    ...
    try:
        home = Path(get_default_nastech_root())
    except Exception:
        return []
    ...
    # every nastech_cli.main process start (right after the profile
    ...
    if restored:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                "  ✓ Restored nastech launcher(s): " + ", ".join(restored),
                file=sys.stderr,
            )
    return restored
...
def _read_user_path_raw() -> tuple[list[str], int]:
    ...
    (same discipline as ``nastech_cli.uninstall``). Only called on Windows.
    """
    import winreg
...
def migrate_windows_bin_path(
    ...
    *,
    ...
    """One-time PATH migration to the ``NASTECH_HOME\\bin`` launcher layout.

    Runs from the ``nastech update`` tail (and mirrors what install.ps1's
    ...
    *read_user_path*/*write_user_path* are injectable for tests.
    ...
    from nastech_constants import get_default_nastech_root, venv_bin_dir
    ...
    if kept != entries:
        ...
        with contextlib.suppress(OSError, ValueError):
            print(
                f"  ✓ nastech launchers now resolve from {home_bin} "
...
def _quarantine_running_nastech_exe(
    ...
    """Rename live nastech*.exe shims aside so the installer can rewrite them.
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    ``nastech acp`` speaks JSON-RPC on stdout.
    """
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    failed: list[str] = []
    moved = (
        _quarantine_running_nastech_exe(scripts_dir, failed_out=failed)
        if scripts_dir
        else []
    )
    if failed:
    ...
    try:
        ...
        # the quarantined shims renamed aside and `nastech` gone from PATH
...
def run_core_install(root: Path) -> None:
    ...
    - quarantine live ``nastech*.exe`` shims on Windows so they can be replaced
