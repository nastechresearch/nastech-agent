...
def _sync_windows_cli_launchers(root: Path) -> list[Path]:
    """Copy the venv's Nastech launchers into the dedicated PATH directory.
    ...
    ``nastech.exe`` is required; ``nastech-acp.exe`` is copied when available.
    Existing files are left alone because ``bin\\nastech.exe`` may be the
    ...
    required_source = scripts_dir / "nastech.exe"
    if not required_source.is_file():
        raise FileNotFoundError(
            f"required Nastech launcher not found: {required_source}"
        )
    ...
    copied: list[Path] = []
    for name in ("nastech.exe", "nastech-acp.exe"):
    ...
    required_destination = bin_dir / "nastech.exe"
    if not required_destination.is_file():
        raise FileNotFoundError(
            f"Nastech launcher was not installed: {required_destination}"
        )
    return copied
...
def _quarantine_running_nastech_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
...
def _run_install_cmd(cmd: list[str], *, env: dict | None, root: Path) -> None:
    ...
    moved = _quarantine_running_nastech_exe(scripts_dir) if scripts_dir else []
    try:
