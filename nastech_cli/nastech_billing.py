"""Nastech Portal Remote Spending HTTP client.

Thin, fail-loud client for the ``/api/billing/*`` endpoints the terminal billing screens drive.
``nastech_account.py`` owns read-only entitlement/balance; this module owns the *write* side: buy
...
# Short-lived cache for the resolved (token, base): `resolve_nastech_access_token` takes two
...
# Keyed by nastech_home_key() so a multiplex profile never bills through a sibling's token.
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError("Not logged into Nastech Portal — run `nastech portal` to log in.", status=401, error="invalid_token")
    if exc is not None:
        err.__cause__ = exc
    return err
...
def _request(
    ...
    try:
        ...
        raise BillingError(f"Could not reach Nastech Portal: {exc.reason}", error="network_error") from exc
        ...
        raise BillingError("Could not reach Nastech Portal: timed out", error="network_error") from exc
