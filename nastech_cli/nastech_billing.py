"""Nastech Portal Remote Spending HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``nastech_cli/nastech_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Nastech already holds for inference
  (``get_provider_auth_state("nastech")["access_token"]``). No API-key auth on these.
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``NASTECH_PORTAL_BASE_URL`` → ``NASTECH_PORTAL_BASE_URL`` → the
...
DEFAULT_PORTAL_BASE_URL = "https://portal.nastechresearch.github.io"
...
# nastech_cli.auth.NASTECH_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``NASTECH_PORTAL_BASE_URL`` → ``NASTECH_PORTAL_BASE_URL`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    """
    env = os.getenv("NASTECH_PORTAL_BASE_URL") or os.getenv("NASTECH_PORTAL_BASE_URL")
...
# Short-lived cache for the resolved (token, base). `resolve_nastech_access_token`
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Nastech Portal — run `nastech portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    (``resolve_nastech_access_token``), so a short-lived (~15 min) access token that
    ...
    when there is no usable Nastech session at all.
...
def _request(
    ...
    *,
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Nastech Portal: {exc.reason}", error="network_error"
        ) from exc
        ...
        raise BillingError(
            "Could not reach Nastech Portal: timed out", error="network_error"
        ) from exc
