...
A provider API key can live in up to THREE stores at once:
    ...
    1. ``~/.nastech/.env``                     — the canonical secret store
    2. ``~/.nastech/auth.json`` →
       ``credential_pool.<provider>[*]``      — env-seeded pool entries
       (``source == "env:<VAR>"``) persisted by the pool loader
    3. ``~/.nastech/config.yaml``              — inline mirrors written by the
...
def purge_env_credential_references(
    ...
    # Make the removal sticky the same way `nastech auth remove` does: a
...
def save_provider_env_credential(env_var: str, value: str) -> Dict[str, Any]:
    ...
    re-add through the UI behaves like ``nastech auth add``.
    ...
    ran ``nastech auth add <provider> --type api-key`` separately. This makes
    the Desktop save's effect on disk match what ``nastech auth add`` does.
    """
    ...
    # that reads this var.
    try:
        from nastech_cli.auth import unsuppress_credential_source
