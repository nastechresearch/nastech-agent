...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: nastech pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(f"  To reset sooner, delete the '_lockout:{platform}' entry from ~/.nastech/platforms/pairing/_rate_limits.json\n")
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'nastech pairing list' to see pending requests.\n")
