...
Split out of :mod:`nastech_cli.plugins`. Names that tests patch on the origin
...
from nastech_cli.plugin_capabilities import VALID_CAPABILITY_IDS
from nastech_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
from nastech_cli.plugins_manifest import (
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    ``nastech_agent.plugin_capabilities`` group (``<plugin-id>.<capability-id>`` entries pointing at the same
...
def collect_directory_manifests() -> List[PluginManifest]:
    ...
    from nastech_cli import plugins as _origin  # patched names resolve through the origin
    manifests: List[PluginManifest] = []
    ...
    if _origin._env_enabled("NASTECH_ENABLE_PROJECT_PLUGINS"):
        project_dir = Path.cwd() / ".nastech" / "plugins"
        logger.debug("Scanning project plugins: %s", project_dir)
        _scan("project", project_dir, "project")
    else:
        logger.debug("Project plugins disabled (set NASTECH_ENABLE_PROJECT_PLUGINS=1 to enable)")
    return manifests
...
def gate_manifest(
    ...
    if names & LEGACY_RELAY_PLUGIN_KEYS:
        ...
        return _placeholder(error, logging.WARNING, "Refusing to load removed Nastech Relay plugin '%s'; %s", error)
    if names & disabled:
    ...
    if manifest.source == "bundled":
        ...
        # Bundled platforms register LAZILY: eagerly importing ~20 heavy SDKs added seconds to every `nastech`
    ...
    if enabled is None or not names & enabled:
        return _placeholder(
            f"not enabled in config (run `nastech plugins enable {lookup_key}` to activate)", logging.DEBUG,
