"""Unified self-relaunch for Nastech CLI: preserves inherited flags (--tui, --dev, --profile, --model…)
across process replacement so ``nastech sessions browse`` / post-setup relaunch keep the user's mode."""
...
def resolve_nastech_bin() -> Optional[str]:
    """Nastech entry point: ``sys.argv[0]`` if a real executable, else ``which nastech``, else ``None``
    (caller falls back to ``python -m nastech_cli.main``).
    ...
    lose the venv. Falling through to ``sys.executable -m nastech_cli.main`` keeps the venv.
    """
    argv0 = sys.argv[0]
    ...
    def _is_unsafe_python_launcher(p: str) -> bool:
        ...
        from nastech_cli.linux_desktop_entry import _needs_interpreter
...
def build_relaunch_argv(
    ...
    argv = [bin_path] if bin_path else [sys.executable, "-m", "nastech_cli.main"]
...
def relaunch(
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # Raw ``[Errno 8] Exec format error`` is cryptic; usual causes are ``nastech`` not on
