...
NASTECH_OVERLAYS: Dict[str, NastechOverlay] = {
    ...
    "tencent-tokenplan": NastechOverlay(
        transport="anthropic_messages",
    ...
    "nebius-token-factory": NastechOverlay(
        transport="openai_chat",
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Nastech overlays (for providers not in models.dev: nastech, openai-codex, etc.)
    ...
    # tencent-tokenhub) are absent from models.dev and NASTECH_OVERLAYS, so
...
def nastech_api_mode(model: str = "") -> str:
    """Resolve the wire protocol for a Nastech Portal model.
    ...
    historical Nastech transport — so callers that don't yet know the model
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    Resolution order:
      ...
      2. Nastech Portal dual-wire (model-derived; overlay alone is openai_chat).
      3. Known provider → transport → TRANSPORT_TO_API_MODE.
      4. Direct provider checks (bedrock).
      5. Default: 'chat_completions'.

    *model* is optional but required for dual-wire providers (Nastech) whose
    ...
    # Nastech is dual-wire: anthropic/* → Messages, everything else →
    # chat_completions. The Nastech overlay still advertises openai_chat
...
def resolve_provider_full(
    ...
    # 0.5 Exact Nastech provider IDs must win over LOSSY alias collapsing.
