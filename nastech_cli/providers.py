...
# -- Nastech overlay: metadata models.dev doesn't provide ----------------------
...
NASTECH_OVERLAYS: Dict[str, NastechOverlay] = {
    ...
    "xai-oauth": NastechOverlay(transport="codex_responses", auth_type="oauth_external",
                               base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "qwen-oauth": NastechOverlay(auth_type="oauth_external", base_url_override="https://portal.qwen.ai/v1",
                                base_url_env_var="NASTECH_QWEN_BASE_URL"),
    "lmstudio": NastechOverlay(extra_env_vars=("LM_API_KEY",), base_url_override="http://127.0.0.1:1234/v1",
                              base_url_env_var="LM_BASE_URL"),
    "copilot-acp": NastechOverlay(transport="codex_responses", auth_type="external_process",
                                 base_url_override="acp://copilot", base_url_env_var="COPILOT_ACP_BASE_URL"),
    "github-copilot": NastechOverlay(extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN")),
    "anthropic": NastechOverlay(transport="anthropic_messages", extra_env_vars=("ANTHROPIC_TOKEN", "CLAUDE_CODE_OAUTH_TOKEN")),
    "zai": NastechOverlay(extra_env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url_env_var="GLM_BASE_URL"),
    "kimi-for-coding": NastechOverlay(base_url_env_var="KIMI_BASE_URL"),
    "stepfun": NastechOverlay(extra_env_vars=("STEPFUN_API_KEY",),
                             base_url_override="https://api.stepfun.ai/step_plan/v1",
                             base_url_env_var="STEPFUN_BASE_URL"),
    "minimax": NastechOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_BASE_URL"),
    "minimax-oauth": NastechOverlay(transport="anthropic_messages", auth_type="oauth_external",
                                   base_url_override="https://api.minimax.io/anthropic"),
    "minimax-cn": NastechOverlay(transport="anthropic_messages", base_url_env_var="MINIMAX_CN_BASE_URL"),
    "deepseek": NastechOverlay(base_url_env_var="DEEPSEEK_BASE_URL"),
    "alibaba": NastechOverlay(base_url_env_var="DASHSCOPE_BASE_URL"),
    "alibaba-coding-plan": NastechOverlay(base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL"),
    "vercel": NastechOverlay(is_aggregator=True),
    "opencode": NastechOverlay(is_aggregator=True, base_url_env_var="OPENCODE_ZEN_BASE_URL"),
    "opencode-go": NastechOverlay(is_aggregator=True, base_url_env_var="OPENCODE_GO_BASE_URL"),
    "kilo": NastechOverlay(is_aggregator=True, base_url_env_var="KILOCODE_BASE_URL"),
    "huggingface": NastechOverlay(is_aggregator=True, base_url_env_var="HF_BASE_URL"),
    "novita": NastechOverlay(is_aggregator=True, base_url_env_var="NOVITA_BASE_URL"),
    "xai": NastechOverlay(transport="codex_responses", base_url_override="https://api.x.ai/v1", base_url_env_var="XAI_BASE_URL"),
    "nvidia": NastechOverlay(base_url_override="https://integrate.api.nvidia.com/v1", base_url_env_var="NVIDIA_BASE_URL"),
    "xiaomi": NastechOverlay(base_url_env_var="XIAOMI_BASE_URL"),
    "tencent-tokenhub": NastechOverlay(base_url_env_var="TOKENHUB_BASE_URL"),
    "tencent-tokenplan": NastechOverlay(transport="anthropic_messages",
                                       base_url_override="https://api.lkeap.cloud.tencent.com/plan/anthropic",
                                       base_url_env_var="TOKENPLAN_BASE_URL"),
    "arcee": NastechOverlay(base_url_override="https://api.arcee.ai/api/v1", base_url_env_var="ARCEE_BASE_URL"),
    "gmi": NastechOverlay(extra_env_vars=("GMI_API_KEY",), base_url_override="https://api.gmi-serving.com/v1",
                         base_url_env_var="GMI_BASE_URL"),
    "fireworks": NastechOverlay(extra_env_vars=("FIREWORKS_API_KEY",),
                               base_url_override="https://api.fireworks.ai/inference/v1"),
    "actual": NastechOverlay(transport="chat_completions", extra_env_vars=("ACTUAL_API_KEY",),
                            base_url_override="https://api.actual.inc/v1", base_url_env_var="ACTUAL_BASE_URL"),
    "upstage": NastechOverlay(extra_env_vars=("UPSTAGE_API_KEY",), base_url_override="https://api.upstage.ai/v1",
                             base_url_env_var="UPSTAGE_BASE_URL"),
    "nebius-token-factory": NastechOverlay(extra_env_vars=("NEBIUS_API_KEY", "NEBIUS_TOKEN_FACTORY_API_KEY"),
                                          base_url_override="https://api.tokenfactory.nebius.com/v1",
                                          base_url_env_var="NEBIUS_BASE_URL"),
    "ollama-cloud": NastechOverlay(base_url_override="https://ollama.com/v1", base_url_env_var="OLLAMA_BASE_URL"),
    # Azure Foundry serves OpenAI- and Anthropic-style endpoints; transport comes from model.api_mode.
    "azure-foundry": NastechOverlay(base_url_env_var="AZURE_FOUNDRY_BASE_URL"),
    "bedrock": NastechOverlay(transport="bedrock_converse", auth_type="aws_sdk"),
    ...
    "vertex": NastechOverlay(auth_type="vertex"),
}
...
_LABEL_OVERRIDES: Dict[str, str] = {
    "moa": "Mixture of Agents", "nastech": "Nastech Portal", "openai-codex": "ChatGPT or Codex Subscription",
...
def _overlay_pdef(canonical, ov: NastechOverlay, name, env_vars, base_url, doc, source) -> ProviderDef:
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    """Look up a built-in provider by id or alias: models.dev catalog merged with the Nastech overlay;
    Nastech-only overlay (nastech, openai-codex, …); plugin provider profiles with a concrete endpoint."""
    canonical = normalize_provider(name)
    mdev_info = _models_dev_info(canonical, allow_network)
    overlay = NASTECH_OVERLAYS.get(canonical)
    if mdev_info is not None:
        ov = overlay or NastechOverlay()
    ...
    if overlay is not None:
        return _overlay_pdef(canonical, overlay, _LABEL_OVERRIDES.get(canonical, canonical), overlay.extra_env_vars,
                             overlay.base_url_override, "", "nastech")
    # Plugin-registered profiles (plugins/model-providers/<name>/) absent from models.dev and
    # NASTECH_OVERLAYS would otherwise be "Unknown provider" in /model, --provider and model-switch
...
def nastech_api_mode(model: str = "") -> str:
    """Wire protocol for a Nastech Portal model. Portal serves its ``anthropic/*`` catalog on a native
    Messages route alongside OpenAI-compatible chat/completions for everything else.

    ``anthropic/*`` rides chat/completions by default for now (``nastech.anthropic_wire``). Measured
    ...
    (NastechResearch/api#227 carries the diagnostics); flip the default back to ``native`` when it is
    ...
    Empty/unknown model defaults to ``chat_completions`` (the historical Nastech transport)."""
    if str(model or "").strip().lower().startswith("anthropic/"):
        # ``auto`` starts on chat too: it is safe on every upstream, and ``agent/nastech_wire.py``
        # promotes the session to native from the first response when the upstream allows it.
        return "anthropic_messages" if _nastech_anthropic_wire() == "native" else "chat_completions"
    return "chat_completions"


def _nastech_anthropic_wire() -> str:
    """``nastech.anthropic_wire``: ``"chat"`` (default), ``"native"``, or ``"auto"`` (chat, then per-session
    promotion decided from the first response; see ``agent/nastech_wire.py``). Anything else reads as ``chat``."""
    try:
        from nastech_cli.config import load_config_readonly
        value = str(((load_config_readonly().get("nastech") or {}).get("anthropic_wire")) or "chat").strip().lower()
    except Exception:
        return "chat"
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    """API mode (wire protocol) for a provider/endpoint: host-mandated mode, then Nastech dual-wire
    ...
    if mandated is not None:
        return mandated
    if (provider or "").strip().lower() in {"nastech", "nastech-portal", "nastechresearch"}:
        return nastech_api_mode(model)
...
def _lossy_alias_registry_pdef(raw: str, canonical: str) -> Optional[ProviderDef]:
    """Exact Nastech registry ids win over LOSSY alias collapsing (kimi-coding-cn must stay distinct
    ...
    try:
        from nastech_cli.auth import PROVIDER_REGISTRY as _AUTH_PROVIDER_REGISTRY
