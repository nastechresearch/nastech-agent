"""Background keepalive for long-lived Nastech Portal sessions."""
...
from nastech_cli.auth import (
    ...
    NASTECH_INVOKE_JWT_MIN_TTL_SECONDS,
    AuthError,
...
NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS = 15 * 60
NASTECH_AUTH_KEEPALIVE_MIN_INTERVAL_SECONDS = 60
# Ticks per credential lifetime: four keeps refresh comfortably ahead of expiry without chatter.
NASTECH_AUTH_KEEPALIVE_TICKS_PER_LIFETIME = 4
NASTECH_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS = 60
NASTECH_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY = "keepalive_interval_seconds"
...
def _timeout_seconds(value: Optional[float]) -> float:
    if value is not None:
        return float(value)
    try:
        return float(os.getenv("NASTECH_NASTECH_TIMEOUT_SECONDS", "15"))
    except (TypeError, ValueError):
        return 15.0


def _nastech_config() -> dict:
    """The ``nastech:`` section of config.yaml, or {} on any failure (config loader imported lazily)."""
    try:
        from nastech_cli.config import load_config

        section = load_config().get("nastech")
...
def _interval_seconds(value: Optional[int]) -> int:
    """Tick interval: explicit argument, then ``nastech.keepalive_interval_seconds`` in config.yaml,
    ...
    if value is not None:
        try:
            ...
            return NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS
    raw = _nastech_config().get(NASTECH_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY)
    ...
    try:
        ...
        logger.warning(
            "Ignoring invalid nastech.%s=%r; using %ds",
            NASTECH_AUTH_KEEPALIVE_INTERVAL_CONFIG_KEY, raw, NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS,
        )
        return NASTECH_AUTH_KEEPALIVE_INTERVAL_SECONDS


def _observed_lifetime_seconds() -> Optional[int]:
    """Server-issued lifetime (seconds) of the current Nastech credentials; the shorter of the access
...
def _tick_seconds(configured_interval: int, lifetime: Optional[int]) -> int:
    ...
    derived = lifetime // NASTECH_AUTH_KEEPALIVE_TICKS_PER_LIFETIME
    return max(NASTECH_AUTH_KEEPALIVE_MIN_INTERVAL_SECONDS, min(configured_interval, derived))
...
def _refresh_selected_pool_entry(*, min_key_ttl_seconds: int, min_access_ttl_seconds: Optional[int] = None) -> Optional[bool]:
    ...
    no usable entry; None = no Nastech pool.
    """
    try:
        ...
        logger.debug("Nastech auth keepalive: credential pool unavailable: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Nastech auth keepalive: credential pool selection failed: %s", exc)
        return False
    if entry is None:
        return False
    ...
    if access_expiring or not key_usable:
        if pool.try_refresh_current() is None:
            return False
        logger.debug("Nastech auth keepalive: refreshed credential pool entry")
    return True


def refresh_nastech_auth_keepalive_once(
    *, min_key_ttl_seconds: int = NASTECH_INVOKE_JWT_MIN_TTL_SECONDS,
    min_access_ttl_seconds: Optional[int] = None, timeout_seconds: Optional[float] = None,
) -> bool:
    """Refresh Nastech auth once if credentials are configured (pool entry first, then singleton state)."""
    ...
    try:
        ...
        if isinstance(exc, AuthError) and exc.relogin_required:
            logger.info("Nastech auth keepalive requires re-login: %s", exc)
        else:
            logger.debug("Nastech auth keepalive failed: %s", exc)
        return False
...
def _keepalive_loop(
    ...
    while not stop_event.is_set():
        ...
        refresh_nastech_auth_keepalive_once(
            min_key_ttl_seconds=horizon, min_access_ttl_seconds=horizon, timeout_seconds=timeout_seconds
        )
        stop_event.wait(tick)


def start_nastech_auth_keepalive(
    *, interval_seconds: Optional[int] = None,
    initial_delay_seconds: int = NASTECH_AUTH_KEEPALIVE_INITIAL_DELAY_SECONDS,
    min_key_ttl_seconds: int = NASTECH_INVOKE_JWT_MIN_TTL_SECONDS, timeout_seconds: Optional[float] = None,
) -> Optional[threading.Thread]:
    """Start the process-wide Nastech auth keepalive thread (idempotent; None when disabled)."""
    ...
    try:
        if is_guest_state(get_provider_auth_state("nastech")):
            logger.debug("Nastech auth keepalive skipped: free tier has no refresh token")
            return None
    except Exception:
        pass
    ...
    with _keepalive_lock:
        ...
        _keepalive_thread = threading.Thread(
            target=_keepalive_loop, args=(_keepalive_stop,), daemon=True, name="nastech-auth-keepalive",
            kwargs={
        ...
        logger.debug("Nastech auth keepalive started")
        return _keepalive_thread


def stop_nastech_auth_keepalive(timeout: float = 5.0) -> None:
