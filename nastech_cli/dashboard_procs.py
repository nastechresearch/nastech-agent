...
``_detect_concurrent_nastech_instances``) are lifted verbatim. References to
helpers that STAY in ``nastech_cli.main`` (``_find_stale_dashboard_pids``,
``_respawn_dashboard_processes``, ``_is_windows``, ...) are routed through a
lazy ``_m()`` main reference so existing test monkeypatches on
``nastech_cli.main.<name>`` keep reaching this code path, and imports stay
...
patches on ``nastech_cli.main`` resolve unchanged.
"""
...
def _m():
    """Lazy ``nastech_cli.main`` reference (call-time; keeps patches working)."""
...
def _scan_dashboard_processes(
    *,
    ...
    ``nastech-dashboard.service`` systemd unit.  Managed units are restarted
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``nastech serve`` as
    ...
    patterns = [
        ...
        # The headless backend (`nastech serve`) is the same long-lived server
    ...
    # miss profiled launches — `nastech --profile p serve --host <ip>` contains
    # neither "nastech serve" nor "nastech_cli.main serve". Every serve/
...
def _nastech_home_for_pid(pid: int) -> str | None:
    """Best-effort ``NASTECH_HOME`` from *pid*'s environment."""
    try:
        import psutil

        home = psutil.Process(pid).environ().get("NASTECH_HOME")
        if home:
            return home
    except Exception:
        pass
    ...
    for part in raw.split(b"\x00"):
        if part.startswith(b"NASTECH_HOME="):
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    Ephemeral-port backends are owned by Nastech Desktop (or become PPID-1
    orphans after a prior update respawn).  Replaying them after
    ``nastech update`` multiplies listening backends because ``--port 0``
...
def _profile_key_for_respawn(
    ...
    if nastech_home:
        try:
            home_path = Path(nastech_home).resolve()
        except (OSError, RuntimeError, ValueError):
            home_path = Path(nastech_home)
        ...
        try:
            ...
            return f"home:{os.path.normcase(nastech_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    *,
    ...
    """Select which killed manual backends to respawn after ``nastech update``.

    Each candidate is ``(pid, argv, nastech_home)``.  *own_home* is the
    updating install's home; it defaults to this process's
    ``get_nastech_home()`` and exists as a parameter so tests can pin it.

    Rules (#78821, #94030):
    1. Never resurrect Desktop ephemeral ``serve|dashboard --port 0``
       backends — Desktop (``NASTECH_DESKTOP_CHILD_PID``) owns their
       lifecycle.  These are also the PPID-1 orphans that previously
       multiplied across updates because ``--port 0`` always binds a
       fresh free port.
    2. Never replay a backend from a **foreign** ``NASTECH_HOME``.  The
    ...
    4. Cap at most one managed backend per profile / ``NASTECH_HOME``.

    Intentionally does **not** blanket-skip every PPID-1 process: a prior
    ``nastech update`` respawn detaches with ``start_new_session=True``, so
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``nastech dashboard`` / ``nastech serve`` processes.
    ...
    When ``restart_managed`` is true (the ``nastech update`` path), a detected
    ``nastech-dashboard.service`` is restarted through systemd; any OTHER
    killed PID that was supervised by a systemd unit (custom unit names —
    e.g. a remote backend's ``nastech-serve.service``) has its owning unit
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the
    caller already restarted directly — e.g. ``nastech update``'s systemd
    fleet-restart loop, which restarts ``nastech-serve*`` units before this
    ...
    if restart_managed and _m()._restart_managed_dashboard_service(reason):
        ...
        # ``nastech-dashboard.service`` and ``nastech-serve.service`` -- the
        ...
        _dash_unit = getattr(
            _m(), "_DASHBOARD_SYSTEMD_UNIT", "nastech-dashboard.service"
        )
    ...
    # caller asked for restarts (the `nastech update` path) — `--stop` must
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                ...
                # Snapshot NASTECH_HOME before the kill so per-profile caps
        ...
        if already_restarted_units:
            # Already handled directly by the caller (e.g. nastech update's
    ...
    #    remote backend (nastech serve) under Restart=on-failure never comes
...
def _detect_concurrent_nastech_instances(
    ...
    for shim in _m()._nastech_exe_shims(scripts_dir):
        try:
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    ...
    Desktop primary/pool backends launch as::
        ...
        nastech serve --host 127.0.0.1 --port 0
        nastech serve --isolated --host 127.0.0.1 --port 0 ...
    ...
    if "serve" not in cmd:
        return False
    if "nastech" not in cmd and "nastech_cli" not in cmd:
        return False
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (NASTECH_DESKTOP_CHILD_PID)."""
    raw = os.environ.get("NASTECH_DESKTOP_CHILD_PID", "")
    out: set[int] = set()
...
# on the *remote* host for every ``nastech serve`` backend it spawns over SSH
# (see apps/desktop/electron/remote-lifecycle.ts). A backend started from
# another client/machine — e.g. a MacBook driving a ``nastech serve`` on a Mac
...
def _nastech_home_dir() -> Path:
    """Resolved Nastech home (NASTECH_HOME override or ~/.nastech)."""
    override = os.environ.get("NASTECH_HOME", "").strip()
    if override:
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    *adds* lock-owned PIDs to its spare-set.
    ...
    for field in ("profile", "nastechPath", "nastechHome", "logPath", "startedAt"):
        value = parsed.get(field)
        if not isinstance(value, str) or len(value) > 1024:
            return False
    # logPath is written as ``{lock_root}/{ownershipId}/{spawnNonce}.log``. We
    # only check the suffix so a relocated NASTECH_HOME (different leading path)
...
def _lock_owned_serve_pids(base_dir: Path | None = None) -> set[int]:
    ...
    Scans ``{nastech_home}/desktop-ssh/<ownershipId>/backend.lock.json`` (the
    ...
    root = base_dir if base_dir is not None else (
        _nastech_home_dir() / _REMOTE_LOCK_SUBDIR
    )
    owned: set[int] = set()
    if not root.is_dir():
        return owned
...
def _reap_orphaned_desktop_local_serves(
    *,
    reason: str = "orphaned desktop-local nastech serve",
    signal_term=None,
    signal_kill=None,
    sleep_fn=None,
    ...
    """Kill leftover Desktop-local ``nastech serve`` backends with no parent.
    ...
    running under NASTECH_PARENT_PID; this helper clears *already* orphaned
    ...
    - never self / never NASTECH_DESKTOP_CHILD_PID entries
    ...
    - never a candidate younger than ``_REAP_MIN_AGE_SECONDS`` (or whose age
      ...
      only after the backend reports NASTECH_BACKEND_READY, so during
    ...
    for pid, cmd in scanned:
        ...
        # NASTECH_BACKEND_READY, so a sibling spawned seconds ago is not yet
