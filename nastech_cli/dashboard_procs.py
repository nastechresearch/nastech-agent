"""Dashboard process-hygiene helpers — extracted from ``nastech_cli/main.py``.

Helpers defined in ``nastech_cli.main_dashboard`` / ``nastech_cli.main_install_repair`` are imported at
...
def _scan_dashboard_processes(*, exclude_pids: set[int] | None = None) -> list[tuple[int, str]]:
    ...
    ``nastech update`` (every API call 401s). *exclude_pids* (Desktop's NASTECH_DESKTOP_CHILD_PID
    backends) are never returned.

    *exclude_pids* is an optional set of PIDs that must never be returned. This is used by the Nastech
    Desktop Electron app to protect its own backend child process: when the desktop spawns ``nastech serve``
    as a backend and triggers an auto-update, the update must not kill the backend that the desktop itself
    manages. The desktop sets the environment variable ``NASTECH_DESKTOP_CHILD_PID`` on the spawned backend
...
def _nastech_home_for_pid(pid: int) -> str | None:
    ...
    if env is None:
        return None
    from nastech_cli.main_dashboard import _dashboard_cmdline_for_pid
    from nastech_cli.profiles import get_active_profile, normalize_profile_name, profile_root_for_env_home
    argv = _dashboard_cmdline_for_pid(pid) or []
    env_home = env.get("NASTECH_HOME", "").strip()
    profile = _profile_flag_value(argv)
    if profile is None and env_home and (
        Path(env_home).parent.name == "profiles" or env.get("NASTECH_UPDATE_POST_SWAP") == "1"
    ):
        return env_home
    if sys.platform == "win32":
...
def _is_ephemeral_port_zero_backend(argv: list[str]) -> bool:
    ...
    ``nastech update`` multiplies listening backends because ``--port 0`` binds a fresh port.
...
def _pids_owned_by_nastech_home(pids: list[int], home: str) -> list[int]:
    """Return only *pids* whose resolved Nastech home (``_nastech_home_for_pid``) is ``home``.

    Dashboard argv is discovery-only: it is not an ownership proof because
    several Nastech installs and profiles can run the same command on one
    ...
    return [
        pid for pid in pids
        if (pid_home := _nastech_home_for_pid(pid))
        and _normalized_home_for_compare(pid_home) == target
    ]


def _profile_key_for_respawn(argv: list[str], nastech_home: str | None = None) -> str:
    """Stable owner key: ``NASTECH_HOME`` when known, else ``--profile`` / ``-p``.
    ...
    """
    if nastech_home:
        parts = _resolved_home(nastech_home).parts
        if len(parts) >= 2 and parts[-2] == "profiles" and parts[-1]:
            return f"profile:{parts[-1]}"
        return f"home:{_normalized_home_for_compare(nastech_home)}"
...
def _filter_dashboard_respawn_candidates(
    ...
    """Select which killed manual backends ``(pid, argv, nastech_home)`` to respawn after update.

    Rules: never resurrect Desktop ``--port 0`` backends; never replay a backend from a
    **foreign** ``NASTECH_HOME`` (the argv-only respawn would come back on this install's home
    ...
    (``NASTECH_DESKTOP_CHILD_PID``) owns their lifecycle. These are also the PPID-1 orphans that previously
    ...
    Intentionally does **not** blanket-skip every PPID-1 process: a prior ``nastech update`` respawn detaches
    ...
    for _pid, argv, nastech_home in candidates:
        if not argv or _is_ephemeral_port_zero_backend(argv):
            continue
        if own_key and nastech_home and _normalized_home_for_compare(nastech_home) != own_key:
            continue
        norm = _normalize_dashboard_cmdline(argv)
        profile_key = _profile_key_for_respawn(argv, nastech_home)
...
def _exclude_pids_from_env() -> set[int]:
    """PIDs Desktop marks as live backends (``NASTECH_DESKTOP_CHILD_PID``, comma-separated)."""
    out: set[int] = set()
    for part in os.environ.get("NASTECH_DESKTOP_CHILD_PID", "").split(","):
        with contextlib.suppress(ValueError):
            out.add(int(part))
    return out


#: Executables that only *carry* a nastech command line. A process headed by one of these
#: never serves traffic itself; when its argv matches the dashboard patterns it is a
#: wrapper around the command (``bash -c 'nastech dashboard --stop'``), not a backend.
...
def _is_caller_wrapper_shell(pid: int, ancestors: set[int]) -> bool:
    ...
    ``bash -c 'nastech dashboard --stop'`` wrapper) matches on its own argv. Ancestor alone
...
def _kill_pids_windows(pids: list[int], killed: list[int], failed: list[tuple[int, str]]) -> None:
    ...
    for pid in pids:
        try:
            ...
            elif not pid_is_nastech(pid, expected_start_time=expected_start_time):
...
def _is_detached_session_leader(pid: int, tty: str) -> bool:
    ...
    ``nastech --tui`` child is a session leader too (``pty.fork``) but owns the pts whose master the
...
def _posix_descendants(roots: list[int]) -> dict[int, tuple[int, int | None]]:
    ...
    subtrees. So is the calling process with its subtree and its ancestor chain: ``nastech dashboard
    --stop`` / ``nastech update`` run from a shell escape inside the hosted Chat TUI are same-session
...
def _kill_stale_dashboard_processes(
    ...
    ``NASTECH_HOME`` are candidates; unknown ownership fails closed. This is
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    e.g. ``nastech update``'s systemd fleet-restart loop, which restarts ``nastech-serve*`` units before this
    ...
    """
    from nastech_cli import main_dashboard as _dash

    if restart_managed and _dash._restart_managed_dashboard_service(reason):
        # The dashboard unit is handled but other backends (e.g. nastech-serve.service) are not:
        # mark the unit handled so the filter below drops its PIDs, and keep going.
        _dash_unit = getattr(_dash, "_DASHBOARD_SYSTEMD_UNIT", "nastech-dashboard.service")
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if launchd_jobs and (job := _launchd_owner(pid, cmdline)):
                ...
                # Manual process: exact argv + NASTECH_HOME for the respawn and its profile cap.
                # Manually-started process: preserve its exact argv so we can respawn it after the update
                # (#40449, #68934). Snapshot NASTECH_HOME before the kill so per-profile caps still work
                # after the process is gone (#78821).
                pid_cmdline[pid] = cmdline
                pid_home[pid] = _nastech_home_for_pid(pid)
...
def _restart_killed_backends(
    ...
    # Two categories: Without this, a remote backend (nastech serve) under Restart=on-failure never comes
...
def _is_desktop_local_serve_cmdline(command: str) -> bool:
    """True for the Desktop-local shape ``nastech serve [--isolated] --host 127.0.0.1 --port 0``.
    ...
    # ``vim notes about nastech serve`` contains both markers — this predicate decides a kill.
    if _nastech_holder_subcommand(command) != "serve":
        return False
...
# (apps/desktop/electron/remote-lifecycle.ts) for every ``nastech serve`` it spawns. Such a backend
...
def _nastech_home_dir() -> Path:
    """The process's Nastech home: remote-backend locks are a process-level asset, so a request scoped
...
def _valid_lockfile_payload(parsed: object, ownership_id: str) -> bool:
    ...
    if any(not isinstance(parsed.get(f), str) or len(parsed[f]) > 1024
           for f in ("profile", "nastechPath", "nastechHome", "logPath", "startedAt")):
        return False
    # Suffix-only check of logPath so a relocated NASTECH_HOME can't reject a legitimate backend.
...
def _remote_lock_roots(base_dir: Path | None) -> list[Path]:
    ...
    The Desktop writes SSH locks beneath the ROOT home (``~/.nastech/desktop-ssh``), but a profile
    backend (``nastech --profile X serve``) runs with ``NASTECH_HOME=<root>/profiles/X`` — scanning only
    ...
    roots: list[Path] = []
    for home in (_nastech_home_dir(), get_default_nastech_root()):
...
def _reap_orphaned_desktop_local_serves(
    *, reason: str = "orphaned desktop-local nastech serve", signal_term=None, signal_kill=None,
    sleep_fn=None, lock_owned_pids_fn=None, process_age_seconds_fn=None) -> dict[str, list]:
    """Kill leftover Desktop-local ``nastech serve`` backends with no parent. Never raises.
    ...
    0/1; not self / parent / NASTECH_DESKTOP_CHILD_PID; not claimed by a valid
    ...
    after NASTECH_BACKEND_READY, so a live sibling mid-startup is briefly unowned).
    """
    import signal as _signal
    import time as _time
