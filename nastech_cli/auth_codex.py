...
Tokens live in ~/.nastech/auth.json, NOT ~/.codex/: Nastech keeps its own Codex OAuth session
...
_NO_CREDENTIALS_MSG = "No Codex credentials stored. Run `nastech auth` to authenticate."
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from nastech_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("NASTECH_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from nastech_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    from nastech_cli.auth import _load_provider_state, _nonempty_str
...
def _sync_codex_pool_entries(
    ...
    ``device_code`` (the singleton-seeded entry from ``nastech setup`` / the model picker) is always
    synced. ``manual:device_code`` (``nastech auth add openai-codex``) is synced only when its
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    ...
    """
    from nastech_cli.auth import _provider_state_transaction, _save_auth_store, _store_provider_state, _utc_now_z
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    ...
    from nastech_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def refresh_codex_oauth_pure(
    ...
    from nastech_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    """
    from nastech_cli.auth import _provider_state_transaction, _save_codex_tokens, refresh_codex_oauth_pure
    ...
    with _provider_state_transaction("openai-codex", lock_timeout) as (_store, state, _source):
        ...
        try:
            ...
            # (or another Nastech process) rotates the shared token this frozen copy fails with a
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from nastech_cli.auth import _codex_access_token_is_expiring
    ...
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    if _should_refresh(access_token):
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout):
            ...
            access_token = _stripped(tokens.get("access_token"))
    return _codex_runtime_result(
        access_token, source="nastech-auth-store", last_refresh=data.get("last_refresh"))
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from nastech_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    from nastech_cli.auth import _nonempty_str, read_credential_pool
...
def _codex_login_rate_limited_error(response: "httpx.Response", *, during: str = "") -> AuthError:
    ...
    # callers surface a "retry later" notice instead of a misleading "run nastech auth" prompt (see issue
    # #32790).
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from nastech_cli.auth import _utc_now_z
