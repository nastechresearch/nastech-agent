...
``nastech-cli`` to the non-existent ``nastech``. ``resolve_toolset('nastech')``
...
def _platform_default_is_valid(
    ...
    # though their synthesized nastech-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``nastech-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``nastech`` instead of ``nastech-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `nastech tools` to configure explicitly."
            )
            if platform_valid_count == 0:
                warnings.append(
                    ...
                    f"Run `nastech tools` to reconfigure."
                )
            continue
        names = raw
        for name in names:
        ...
        if platform_valid_count == 0:
            ...
            warnings.append(
                ...
                f"tools on this platform. Run `nastech tools` to reconfigure."
            )

    if valid_count == 0:
        warnings.append(
            "platform_toolsets resolves to zero valid toolsets — the agent will "
            "have no tools. Run `nastech tools` to reconfigure."
        )
    return warnings
