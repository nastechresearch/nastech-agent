...
Supports OAuth device code flows (Nastech Portal, future: OpenAI Codex) and
...
# httpx is imported lazily: it costs ~30ms at import time and nastech_cli.auth
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # ("nastech_cli.auth.httpx.Client", ...) keeps working in tests.
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from nastech_cli import __version__ as _NASTECH_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _NASTECH_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"nastech-cli/{_NASTECH_CLI_VERSION}"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "anthropic": ProviderConfig(
        ...
        # from it), so removing it would stop Nastech finding a setup-token
    ...
    "opencode-free": ProviderConfig(
        ...
        # secret to configure. Select via `nastech model` / `/model free`.
        api_key_env_vars=(),
    ),
...
try:
    ...
    for _pp in _list_providers_for_registry():
        ...
        if _pp.auth_type == "external_process":
            ...
            # without it, `nastech -m <that provider>` dies with
...
def get_anthropic_key() -> str:
    ...
    ``~/.nastech/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.nastech/.env over os.environ so a deliberate key rotation
...
def _format_nastech_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        message = format_nastech_portal_entitlement_message(
            account_info,
            capability="Nastech model access",
        )
        if message:
            return message
    except Exception:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``_nastech_auth_status_cache``): read_credential_pool() -> load_pool() runs
    this once per provider row in the /model picker, and the path resolution
    (``_global_auth_file_path()`` -> ``get_default_nastech_root()``) + JSON
...
def _file_lock(
    ...
    # A stress test with 20 concurrent Nastech processes reproduced this
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        ...
        if "nastech_portal" in systems:
            providers["nastech"] = systems["nastech_portal"]
...
def _save_auth_store(auth_store: Dict[str, Any], target_path: Optional[Path] = None) -> Path:
    ...
    # nastech-agent install tree (#25821, #93050).
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated Nastech refresh token only to
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    profile can see providers (e.g. ``nastech``) that were only authenticated at
...
def strip_cloned_single_use_oauth_grants(profile_dir: Path) -> Dict[str, Any]:
    ...
    into another (``nastech profile create --clone-all``, the dashboard/TUI
...
# (an independent ``nastech -p <p> auth add`` grant, or the only surviving
...
def consume_oauth_heal_notices() -> List[str]:
    ...
    ``nastech auth list`` / ``nastech auth status`` print them so the user sees
...
def _find_root_counterpart(
    ...
    Anthropic ``nastech_pkce`` grant uses one client id and carries no claims,
    ...
    (an independent ``nastech -p <p> auth add`` login for another account).
    """
...
def _heal_forked_single_use_oauth_grants(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        # user's ~/.nastech/auth.json from a test that forgot to isolate HOME.
        real_home_env = os.environ.get("HOME", "")
        if real_home_env and _same_path(root_path, Path(real_home_env) / ".nastech" / "auth.json"):
            return None
    ...
    with _auth_store_lock():
        ...
        with _auth_store_lock(target_path=root_path):
            ...
            for row in p_rows:
                ...
                # .anthropic_oauth.json (the ``nastech auth`` PKCE shape); a
                # profile nastech_pkce-family row is that grant's copy.
                is_pkce = str(row.get("source") or "").endswith("nastech_pkce")
                    ...
                    if root_singleton_row is not None:
                        ...
                        idx = next(
                            (i for i, r in enumerate(r_rows)
                             ...
                             and str(r.get("source") or "").endswith("nastech_pkce")),
                            None,
                        )
            ...
            if summary["adopted"] and root_singleton is not None and root_singleton_row is not None:
                # Keep root's singleton and its ``nastech_pkce``-seeded pool row
                ...
                pkce_idx = next(
                    (i for i, r in enumerate(r_rows)
                     if _is_oauth_pool_payload(r) and r.get("source") == "nastech_pkce"),
                    None,
                )
            ...
            if summary["adopted"] and root_singleton is not None and root_singleton_row is not None:
                from agent.anthropic_credentials import _write_nastech_oauth_credentials
                _write_nastech_oauth_credentials(
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Nastech device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # explicit for users who never set Nastech up for it. Only Nastech-scoped
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    When ``~/.nastech/config.yaml`` EXISTS but fails to parse, ``load_config()``
    ...
    """
    try:
        from nastech_cli.config import get_active_config_parse_failure, get_config_path
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(_scoped_key_env(env_var)):
                ...
                # OAuth but has a stale key in ~/.nastech/.env isn't silently
...
_NASTECH_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.nastechresearch.github.io",
})

# Allowlist of valid Nastech Portal hosts. A portal_base_url outside this
...
_NASTECH_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
    "portal.nastechresearch.github.io",
    "localhost",
    "127.0.0.1",
})
...
def _migrate_stale_nastech_portal_url(providers: Dict[str, Any]) -> None:
    ...
    if stored:
        ...
        if parsed.hostname in _NASTECH_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale nastech portal_base_url %s -> %s",
                stored, DEFAULT_NASTECH_PORTAL_URL,
            )
            nastech["portal_base_url"] = DEFAULT_NASTECH_PORTAL_URL


# Allowlist of hosts the Nastech Portal proxy is willing to forward inference
...
def _validate_nastech_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "nastech: refusing non-https inference URL scheme %r from Portal response",
            parsed.scheme,
        )
        return None
...
def _nastech_inference_env_override() -> Optional[str]:
    """Return the user-set ``NASTECH_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("NASTECH_INFERENCE_BASE_URL"))
...
def _nastech_portal_env_override() -> Optional[str]:
    ...
    ``NASTECH_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Nastech at a non-production Nastech Portal (e.g. a hosted agent
    provisioned on nastech-account-service's `staging` environment, which stamps
    ``NASTECH_PORTAL_BASE_URL=https://portal.staging-nastechresearch.github.io`` into
    ...
    gated by ``_NASTECH_PORTAL_ALLOWED_HOSTS``: that allowlist exists to reject
    ...
    return _optional_base_url(
        os.getenv("NASTECH_PORTAL_BASE_URL") or os.getenv("NASTECH_PORTAL_BASE_URL")
    )
...
def _assert_nastech_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Nastech Portal access token is not a usable inference JWT "
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the Nastech / xAI-OAuth / Codex-OAuth / MiniMax pattern.
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("NASTECH_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``nastech auth status``, …),
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("NASTECH_XAI_REFRESH_TIMEOUT_SECONDS", 20)
...
def _nastech_device_auth_timeout_message(portal_base_url: str) -> str:
    """Actionable timeout text for Nastech device-code login failures.
    ...
    """
    portal = (portal_base_url or DEFAULT_NASTECH_PORTAL_URL).rstrip("/")
    return (
...
def _poll_for_token(
    ...
    # the CLI login (_nastech_device_code_login) and the dashboard/desktop
    # poller (web_server._nastech_poller, which surfaces str(e) to the UI).
    raise TimeoutError(_nastech_device_auth_timeout_message(portal_base_url))


# =============================================================================
# Nastech Portal — token refresh and model discovery
# =============================================================================
...
def _write_shared_nastech_state(state: Dict[str, Any]) -> None:
    ...
    try:
        with _nastech_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Nastech
...
def _quarantine_nastech_oauth_state(
    ...
    *,
    ...
    forensic: Dict[str, Any] = {
        ...
        # No session_id field exists on Nastech state; provenance is client_id +
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="nastech", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="nastech", relogin_required=True) from exc
    ...
    raise AuthError(description, provider="nastech", code=code, relogin_required=relogin)
...
def fetch_nastech_models(
    *,
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="nastech", code="models_fetch_failed")
...
# Per-process memo for resolve_nastech_access_token. Startup runs
...
# separate flows (force_fresh / refresh_nastech_oauth_pure) and are unaffected.
        ...
        with _RESOLVE_TOKEN_CACHE_LOCK:
            if _RESOLVE_TOKEN_CACHE is not None:
                ...
                if (time.monotonic() - cached_at) < _RESOLVE_TOKEN_CACHE_TTL_S:
                    return cached_token
    with _provider_state_transaction("nastech") as (
        auth_store,
        state,
        state_source_path,
    ):
        ...
        if not state:
            raise AuthError(
                "Nastech is not logged into Nastech Portal.",
                provider="nastech",
                relogin_required=True,
            )

        # NASTECH_PORTAL_BASE_URL / NASTECH_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors NASTECH_INFERENCE_BASE_URL) and
        ...
        if env_portal_override:
            ...
            if parsed_portal_url.hostname and parsed_portal_url.hostname not in _NASTECH_PORTAL_ALLOWED_HOSTS:
                logger.warning(
        ...
        with _nastech_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Nastech Portal login.",
                    provider="nastech",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "nastech", state, state_source_path)
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Nastech Portal.",
                    provider="nastech",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            state["inference_base_url"] = refreshed_url or DEFAULT_NASTECH_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
...
def persist_nastech_credentials(
    ...
    *,
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "nastech", state)
        _save_auth_store(auth_store)
...
def resolve_nastech_runtime_credentials(
    *,
    ...
    """
    Resolve Nastech inference credentials for runtime use.
    ...
    with _provider_state_transaction("nastech") as (
        ...
        if not state:
            raise AuthError("Nastech is not logged into Nastech Portal.",
                            provider="nastech", relogin_required=True)
        ...
        def _already_rotated_by_peer(token: Any) -> bool:
            return bool(
                ...
                and _nastech_invoke_jwt_status(
                    token,
                    scope=state.get("scope"),
                    expires_at=state.get("expires_at"),
                ) is None
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _NASTECH_PORTAL_ALLOWED_HOSTS
            ...
            effective_inference_url = (
                _nastech_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_NASTECH_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Nastech Portal login.",
                                provider="nastech", relogin_required=True)
            ...
            if force_refresh or invoke_jwt_status is not None:
                with _nastech_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_nastech_oauth_pure):
                        ...
                        # persisted to auth.json below. The NASTECH_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_nastech_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_NASTECH_INFERENCE_URL
                        inference_base_url = (
                            _nastech_inference_env_override() or stored_inference_base_url
                        )
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Nastech inference API key",
                        provider="nastech", code="server_error")
...
# which does a synchronous OAuth refresh POST to portal.nastechresearch.github.io. That can take
...
# Same invalidation contract as _nastech_auth_status_cache — the global auth
...
def get_nastech_auth_status_local() -> Dict[str, Any]:
    ...
    Unlike :func:`get_nastech_auth_status`, this NEVER calls
    ``resolve_nastech_runtime_credentials()`` and therefore never performs an
    ...
    need a live credential) should keep using ``get_nastech_auth_status()``.
    ...
    access_token = state.get("access_token")
    jwt_reason = _nastech_invoke_jwt_status(
        access_token,
        scope=state.get("scope"),
        expires_at=state.get("expires_at"),
    )
...
# Enum values reported on the dashboard /api/status as ``nastech_session_valid``.
...
def get_nastech_session_validity() -> str:
    ...
    Returns one of:
      - ``"valid"``    — a usable Nastech credential is present (login healthy).
      - ``"terminal"`` — the Nastech session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no Nastech provider state, or a transient/
    ...
    # into the Nastech provider state when it clears dead tokens (the exact path
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    # the NastechOverlay keyless flag — the same source the provider catalog
    # and GUI contract tests use.
    try:
        from nastech_cli.providers import NASTECH_OVERLAYS
        _overlay = NASTECH_OVERLAYS.get(provider_id)
    except Exception:
        _overlay = None
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    CLI may hold its session in an OS keychain Nastech can't read. Callers
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    Nastech can actually see some — absence of evidence is not absence of auth.
    """
    pconfig = PROVIDER_REGISTRY.get(provider_id)
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    # NASTECH_COPILOT_ACP_COMMAND / COPILOT_CLI_PATH / NASTECH_COPILOT_ACP_ARGS
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    if provider in {"nastech", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _confirm_selection_guards(
    ...
    *,
    ...
    :mod:`nastech_cli.model_selection_guards` and shows one [y/N] confirm with
...
def _prompt_model_selection(
    ...
    # Sale chrome (★ / -N% / was) is Nastech Portal-only — never for OpenRouter
    # or other providers even if pricing.original is somehow present.
    sale_chrome = (confirm_provider or "").strip().lower() == "nastech"

    def _confirmed_selection(mid: str) -> Optional[str]:
        if not mid:
            return None
        # Unified guard registry (nastech_cli.model_selection_guards): the cost
    ...
    # Leave room for a leading "★ " on sale rows (Nastech only).
...
def _login_xai_oauth(
    ...
    *,
    ...
    # xAI credential re-enabled. ``nastech auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``nastech auth list`` would show
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the Nastech / xAI-OAuth / Codex-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def step_up_nastech_billing_scope(
    *,
    ...
    like ``_login_nastech`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [NASTECH_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if NASTECH_BILLING_MANAGE_SCOPE not in requested:
        requested.append(NASTECH_BILLING_MANAGE_SCOPE)
    ...
    with _auth_store_lock():
        auth_store = _load_auth_store()
        _save_provider_state(auth_store, "nastech", auth_state)
        _save_auth_store(auth_store)

    # Mirror to shared store + reseed the pool (best-effort), same as _login_nastech.
    try:
...
def _login_nastech(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        # overwrites it to "nastech".  If the user picks "Skip (keep current)"
        ...
        # leave the config in a half-updated state (provider=nastech but model
        ...
        try:
            ...
            from nastech_cli.models import (
                ...
                nastech_policy_allowed_ids, restrict_to_nastech_policy,
                ...
                if free_tier:
                    try:
                        ...
                        unavailable_message = (
                            format_nastech_portal_entitlement_message(
                                _account_info,
                                capability="paid Nastech models",
                            )
                            or ""
                        )
                    except Exception:
        ...
        # them to Nastech with a mismatched model.  The Nastech OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "nastech".  config.yaml model.provider is left
