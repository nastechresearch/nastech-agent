...
- The auth store (``~/.nastech/auth.json``) holds per-provider state, the credential pool and
...
- ``OAUTH_PROVIDER_FLOWS`` maps each OAuth provider to its resolver/status builder; the flows live in
  ``auth_nastech``/``auth_codex``/``auth_xai``/``auth_qwen``/``auth_minimax``/``auth_spotify``/``auth_openrouter`` and are
  re-imported here so ``nastech_cli.auth.<name>`` stays the public/patchable surface."""
...
from nastech_cli.config import (
    get_nastech_home, get_config_path, read_raw_config, require_readable_config_before_write)
from nastech_constants import OPENROUTER_BASE_URL, nastech_home_key, secure_parent_dir
...
from nastech_cli.auth_zai_kimi import (  # noqa: F401  re-exported
...
from nastech_cli.auth_model_picker import (  # noqa: F401  re-exported
    _prompt_model_selection, _save_model_choice)
from nastech_cli.auth_device_flow import (  # noqa: F401  re-exported
    _can_open_graphical_browser, _default_verify, _is_remote_session,
    _nastech_device_auth_timeout_message, _offer_existing_oauth_credentials,
...
from nastech_cli.auth_oauth_grants import (  # noqa: F401  re-exported
    SINGLE_USE_REFRESH_POOL_PROVIDERS, strip_cloned_single_use_oauth_grants)
from nastech_cli.auth_nastech import (  # noqa: F401  re-exported
    NASTECH_SESSION_TERMINAL, NASTECH_SESSION_UNKNOWN, NASTECH_SESSION_VALID, _ALLOWED_NASTECH_INFERENCE_HOSTS,
    _agent_key_is_usable, _apply_nastech_refreshed_tokens, _assert_nastech_inference_jwt_usable,
    _compute_nastech_auth_status, _format_nastech_entitlement_auth_error, _healed_nastech_inference_url,
    _login_nastech, _merge_shared_nastech_oauth_state, _migrate_stale_nastech_portal_url,
    _nastech_device_code_login, _nastech_inference_env_override, _nastech_invoke_jwt_is_usable,
    _nastech_invoke_jwt_status, _nastech_portal_env_override, _nastech_shared_store_lock,
    _nastech_shared_store_path, _pool_first_oauth_status, _quarantine_nastech_oauth_state,
    _quarantine_nastech_pool_entries, _read_shared_nastech_state, _refresh_access_token,
    _refresh_nastech_or_quarantine, _select_nastech_invoke_jwt, _sync_nastech_pool_from_auth_store,
    _token_fingerprint, _try_import_shared_nastech_state, _validate_nastech_inference_url_from_network,
    _write_shared_nastech_state, fetch_nastech_models, get_nastech_auth_status_local,
    get_nastech_session_validity, persist_nastech_credentials, refresh_nastech_oauth_from_state,
    resolve_nastech_runtime_credentials, step_up_nastech_billing_scope)
from nastech_cli.auth_minimax import (  # noqa: F401  re-exported
...
from nastech_cli.auth_xai import (  # noqa: F401  re-exported
...
from nastech_cli.auth_codex import (  # noqa: F401  re-exported
...
from nastech_cli.auth_spotify import (  # noqa: F401  re-exported
    _refresh_spotify_oauth_state, get_spotify_auth_status, login_spotify_command,
    resolve_spotify_runtime_credentials)
from nastech_cli.auth_openrouter import _openrouter_pkce_login  # noqa: F401  re-exported
from nastech_cli.auth_qwen import (  # noqa: F401  re-exported
...
from nastech_cli.auth_constants import (  # noqa: F401  re-exported
    _decode_jwt_claims, AUTH_STORE_VERSION, AUTH_LOCK_TIMEOUT_SECONDS, DEFAULT_NASTECH_PORTAL_URL,
    DEFAULT_NASTECH_INFERENCE_URL, DEFAULT_NASTECH_CLIENT_ID, NASTECH_BILLING_MANAGE_SCOPE,
    DEFAULT_NASTECH_SCOPE, NASTECH_DEVICE_CODE_SOURCE, NASTECH_AUTH_PATH_INVOKE_JWT,
    ACCESS_TOKEN_REFRESH_SKEW_SECONDS, NASTECH_INVOKE_JWT_MIN_TTL_SECONDS, DEFAULT_CODEX_BASE_URL,
    ...
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, CODEX_RATE_LIMITED_CODE, AuthError, _nastech_err, httpx)
...
_REGISTRY_ROWS: Tuple[Any, ...] = (
    ProviderConfig(
        "nastech", "Nastech Portal", "oauth_device_code", portal_base_url=DEFAULT_NASTECH_PORTAL_URL,
        inference_base_url=DEFAULT_NASTECH_INFERENCE_URL, client_id=DEFAULT_NASTECH_CLIENT_ID,
...
def get_anthropic_key() -> str:
    ...
    Checks both the ``.env`` file and the process environment, preferring ``~/.nastech/.env`` so a deliberate
    key rotation isn't shadowed by a stale shell export (matches the api-key resolution path — see #20591).
    """
    from nastech_cli.config import get_env_value_prefer_dotenv
...
def _model_level_key_env(provider_id: str) -> str:
    ...
    (``model.key_env`` → ``$NASTECH_HOME/.env``) instead of the registry's canonical env var,
...
def _resolve_api_key_provider_secret(provider_id: str, pconfig: ProviderConfig) -> tuple[str, str]:
    ...
    # Prefer ~/.nastech/.env over os.environ so a deliberate key rotation in .env isn't shadowed by
    ...
    # model.key_env → $NASTECH_HOME/.env (e.g. NASTECH_CUSTOM_LMSTUDIO_API_KEY) while keeping
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    re-authenticating cannot fix it, so callers should say "retry later", not ``nastech auth``."""
    return (isinstance(error, AuthError) and not error.relogin_required
            and error.code == CODEX_RATE_LIMITED_CODE)


# Entitlement failures: Nastech gets a Portal-aware message; other providers a fixed generic one (or
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        # Profile-aware: a bare `nastech model` from a named profile re-signs the ROOT store (#114012).
        from nastech_constants import profile_cli_selector

        return f"{error} Run `nastech {profile_cli_selector()}model` to re-authenticate."
    if error.code in _ENTITLEMENT_ERROR_CODES:
        if error.provider == "nastech":
...
def _auth_file_path() -> Path:
    ...
    # monkeypatch NASTECH_HOME or escaped the hermetic conftest). In production: one dict lookup.
    if (os.environ.get("PYTEST_CURRENT_TEST")
            and _same_path(path, Path.home() / ".nastech" / "auth.json")):
        raise RuntimeError(
...
def _file_lock(
    ...
    the shared Nastech store) track reentrancy separately."""
...
def _auth_store_lock(
    ...
    ``_nastech_shared_store_lock`` SECOND (inner), else deadlock against a concurrent shared import."""
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_nastech_portal_url(raw["providers"])
        return raw

    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):  # legacy "systems" format
        systems = raw["systems"]
        providers = {"nastech": systems["nastech_portal"]} if "nastech_portal" in systems else {}
        return {**_empty_auth_store(), "providers": providers,
                "active_provider": "nastech" if providers else None}
    return _empty_auth_store()
...
def _save_private_json(target: Path, data: Any, *, fsync_dir: bool = False, **dump_kwargs: Any) -> None:
    ...
    from nastech_constants import mkdir_under_nastech_home
    mkdir_under_nastech_home(target.parent)
...
def _save_provider_state_to_source(
    ...
    left as it is (a Nastech free-tier identity refreshed for a connector call must not become the
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    """Set ``active_provider`` only when none is set yet: the first ``nastech auth add`` credential must
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    not inherited (#111724) — ``nastech -p <name> auth add <provider>`` gives the profile its own."""
...
def write_credential_pool(
    ...
    *status_cleared_ids* were cleared deliberately (``nastech auth reset``) and skip the
...
def nastech_token_has_billing_scope() -> bool:
    """Return True if the currently-held Nastech token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return NASTECH_BILLING_MANAGE_SCOPE in scope.split()
...
def _explicit_pool_entry_present(normalized: str) -> bool:
    """Pool rows from EXPLICIT Nastech flows (manual add / device-code / PKCE) or live env keys;
...
# Set by Claude Code itself, not by the user explicitly configuring anthropic in Nastech.
_IMPLICIT_ENV_VARS = frozenset({"CLAUDE_CODE_OAUTH_TOKEN"})
_EXPLICIT_POOL_SOURCES = frozenset({"device_code", "loopback_pkce", "nastech_pkce", "manual"})
...
def _pool_entry_is_explicit(entry: Any) -> bool:
    """True for pool rows the user created via an explicit Nastech flow (or a still-live env key)."""
...
def _keyless_provider_has_explicit_config(normalized: str) -> bool:
    """Vertex / Bedrock count as explicit when Nastech-scoped routing config is present.
    ...
    Nastech-scoped signals (VERTEX_PROJECT_ID / vertex.project_id / VERTEX_CREDENTIALS_PATH) count
    here."""
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    Nastech-initiated flow, or Nastech-scoped routing config for keyless cloud-SDK providers. Ambient
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    ...
    ``nastech logout``. Returns True if something was cleared."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    if str(provider_name or "").strip().lower() in {"opencode-free", "free", "opencode_free"}:
        return ("OpenCode discontinued anonymous free-tier access outside its own client "
                ...
                "($10/mo subscription, OPENCODE_GO_API_KEY) via 'nastech model'.")
    try:
...
def _refuse_env_adoption_if_config_corrupt() -> None:
    ...
    try:
        from nastech_cli.config import get_active_config_parse_failure
    ...
    raise AuthError(
        ...
        f"was saved next to it) or run nastech setup.",
        code="corrupt_config")
...
def _openrouter_auto_detected(scoped_key_env: Callable[[str], str]) -> bool:
    ...
    `nastech auth add openrouter` has no env var; without the pool check it is invisible to
    ...
    if any(has_usable_secret(scoped_key_env(v)) for v in ("OPENAI_API_KEY", "OPENROUTER_API_KEY")):
        return True
    try:
        # Auto-detect an OpenRouter credential added via `nastech auth add openrouter` (manual pool entry, no
        # env var). Without this, a key that only lives in the credential pool is invisible to
        # auto-detection — the user sees `nastech auth list` showing the credential while requests go out
...
def _config_model_provider() -> Tuple[Any, Optional[str]]:
    ...
    dashboard's Ink chat parked every session on Setup Required while ``nastech chat`` worked
    (#108383)."""
    try:
        from nastech_cli.config import load_config
        model_cfg = (load_config() or {}).get("model")
        ...
        if base_url:
            from nastech_cli.runtime_provider import _config_base_url_trustworthy_for_bare_custom
...
def _env_key_auto_detected(
    ...
    preempts a logged-in OAuth provider so a stale key in ~/.nastech/.env never switches silently."""
            ...
            if has_usable_secret(scoped_key_env(env_var)):
                if oauth_active and oauth_active != pid:
                    logger.warning(
                        ...
                        # ~/.nastech/.env isn't silently switched without knowing why.
...
def resolve_provider(
    ...
    *,
    ...
    7. Nastech free tier when it is on and its identity exists (never created here);
    ...
    if normalized != "auto":
        ...
        tail = (f"\n\n{hint}" if hint else " Check 'nastech model' for available providers, "
                "or run 'nastech doctor' to diagnose config issues.")
    ...
    # Nastech free tier, when it is on and its identity already exists. This rung sits ABOVE the Bedrock
    ...
    from nastech_constants import display_nastech_home, profile_cli_selector, profile_name_for_home
    ...
    profile_name = profile_name_for_home(get_nastech_home()) if sel else None
    where = f"Profile '{profile_name}' is" if profile_name else "Nastech is"
    raise AuthError(
        f"{where} not connected to any AI provider yet. Run `nastech {sel}model` to pick one (the free "
        f"Nastech tier needs no API key), type `/login` in chat, or add a key with "
        f"`nastech {sel}auth add <provider>`. (Advanced: put an API key such as OPENROUTER_API_KEY in "
        f"{display_nastech_home()}/.env.)",
...
def _quarantine_flat_oauth_state(state: Dict[str, Any], provider: str, exc: "AuthError") -> None:
    ...
    subsequent calls fail fast without a network retry (mirrors the Nastech / xAI / Codex pattern)."""
...
# the burst into one round-trip. Callers needing freshness use force_fresh/refresh_nastech_oauth_pure.
# Keyed by nastech_home_key(): the resolution itself is profile-scoped (_auth_file_path reads the
# per-turn NASTECH_HOME override a multiplex gateway sets), so a single slot would hand profile A's
...
def _nastech_portal_base_url(state: Dict[str, Any]) -> str:
    """NASTECH_PORTAL_BASE_URL / NASTECH_PORTAL_BASE_URL is the trusted operator override and wins
    ...
    portal_base_url = _optional_base_url(state.get("portal_base_url")) or DEFAULT_NASTECH_PORTAL_URL
    portal_base_url = portal_base_url.rstrip("/")
    host = urlparse(portal_base_url).hostname
    if host and host not in _NASTECH_PORTAL_ALLOWED_HOSTS:
        logger.warning(
            "auth: ignoring invalid portal_base_url %r (host %r not in allowlist), using default",
            portal_base_url, host)
        return DEFAULT_NASTECH_PORTAL_URL
...
def resolve_nastech_access_token(
    *,
    ...
    with _provider_state_transaction("nastech") as (auth_store, state, state_source_path):
        ...
        portal_base_url = _nastech_portal_base_url(state)
        ...
        persist = lambda: _save_provider_state_to_source(  # noqa: E731
            auth_store, "nastech", state, state_source_path)

        lock_timeout = max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)
        with _nastech_shared_store_lock(timeout_seconds=lock_timeout):
            ...
            if not isinstance(refresh_token, str) or not refresh_token:
                raise _nastech_err(
                    "Session expired and no refresh token is available.", "nastech_auth_missing_refresh_token",
                    relogin=True)
            ...
            _apply_nastech_refreshed_tokens(state, refreshed, refresh_token)
...
# Process-level memo for get_nastech_auth_status(): it validates via a synchronous refresh POST
...
_nastech_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None
...
def invalidate_nastech_auth_status_cache() -> None:
    """Clear the get_nastech_auth_status() memo (for code paths that mutate Nastech auth state without
...
def get_nastech_auth_status() -> Dict[str, Any]:
    """Status snapshot for Nastech auth, memoised ~15s keyed on the auth.json mtime.
    ...
    if (cached is not None and cached[1:3] == (auth_file_key, mtime)
            and (now - cached[0]) < _NASTECH_AUTH_STATUS_CACHE_TTL):
        return dict(cached[3])
    status = _compute_nastech_auth_status()
    _nastech_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status
...
class OAuthProviderFlow:
    ...
    ``monkeypatch.setattr("nastech_cli.auth.resolve_codex_runtime_credentials", ...)`` applies."""
    provider_id: str
    resolve_fn: str
    status_fn: str
    terminal_refresh_codes: FrozenSet[str] = frozenset()  # retrying the same refresh token cannot succeed
    # ``nastech logout`` with no active provider falls back to config.yaml ``model.provider`` only
...
# Nastech state-shape failures raised BEFORE any refresh POST (no login, no token pair): retrying
# cannot succeed either, so the pool must not bench them as a transient outage (#113718).
_NASTECH_AUTH_MISSING_CODES = frozenset({
    "nastech_auth_missing", "nastech_auth_missing_access_token", "nastech_auth_missing_refresh_token"})

OAUTH_PROVIDER_FLOWS: Dict[str, OAuthProviderFlow] = {
    "nastech": OAuthProviderFlow(
        "nastech", "resolve_nastech_runtime_credentials", "get_nastech_auth_status",
        terminal_refresh_codes=_OAUTH_GRANT_DEAD_CODES | _NASTECH_AUTH_MISSING_CODES, logout_from_config=True),
...
_is_terminal_nastech_refresh_error = partial(_is_terminal_refresh_error, provider="nastech")
...
def _external_process_auth_evidence(provider_id: str) -> tuple[bool, Optional[str]]:
    ...
    Nastech can't read). Deliberately subprocess-free: spawning ``gh auth token`` from status
    ...
    try:
        from nastech_cli.copilot_auth import COPILOT_ENV_VARS, validate_copilot_token
...
def _external_process_spec(
    ...
    Launch details come from the provider's own profile (copilot-acp: NASTECH_COPILOT_ACP_COMMAND /
    COPILOT_CLI_PATH / NASTECH_COPILOT_ACP_ARGS), so out-of-tree providers describe their binary."""
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    tests that patch ``nastech_cli.auth.get_*_auth_status`` still apply."""
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    CLI startup flat; ``nastech doctor`` runs the live probe). ``api_key`` (default): usable
    ``AZURE_FOUNDRY_API_KEY``."""
    info: Dict[str, Any] = {"provider": "azure-foundry"}
    try:
        from nastech_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def login_command(args) -> None:
    ...
    print("The 'nastech login' command has been removed.\nUse 'nastech auth' to manage credentials,\n"
          "'nastech model' to select a provider, or 'nastech setup' for full setup.")
    raise SystemExit(0)
...
def logout_command(args) -> None:
    ...
    if not target:
        ...
        return
    if target == "nastech":
        from nastech_cli.anon_auth import FREE_TIER_NOT_SIGNED_IN, is_guest_state
        if is_guest_state(get_provider_auth_state("nastech")):
    ...
    if target == "nastech":
        ...
        from nastech_cli.auth_nastech import _clear_shared_nastech_state
        _clear_shared_nastech_state("logout")
...
_PLUGIN_COMPAT_LAZY = {
    'CODEX_OAUTH_USER_AGENT': ('nastech_cli.auth_constants', 'CODEX_OAUTH_USER_AGENT'),
    'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS': ('nastech_cli.auth_codex', 'CODEX_QUOTA_PROBE_MIN_INTERVAL_SECONDS'),
    'DEFAULT_SPOTIFY_REDIRECT_URI': ('nastech_cli.auth_constants', 'DEFAULT_SPOTIFY_REDIRECT_URI'),
    'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS': ('nastech_cli.auth_constants', 'DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS'),
    'MINIMAX_OAUTH_GRANT_TYPE': ('nastech_cli.auth_constants', 'MINIMAX_OAUTH_GRANT_TYPE'),
    'NASTECH_INFERENCE_INVOKE_SCOPE': ('nastech_cli.auth_constants', 'NASTECH_INFERENCE_INVOKE_SCOPE'),
    'NASTECH_SHARED_STORE_FILENAME': ('nastech_cli.auth_nastech', 'NASTECH_SHARED_STORE_FILENAME'),
    'OAUTH_OVER_SSH_DOCS_URL': ('nastech_cli.auth_constants', 'OAUTH_OVER_SSH_DOCS_URL'),
    'QWEN_OAUTH_CLIENT_ID': ('nastech_cli.auth_constants', 'QWEN_OAUTH_CLIENT_ID'),
    'QWEN_OAUTH_TOKEN_URL': ('nastech_cli.auth_constants', 'QWEN_OAUTH_TOKEN_URL'),
    'SINGLE_USE_OAUTH_SINGLETON_FILES': ('nastech_cli.auth_oauth_grants', 'SINGLE_USE_OAUTH_SINGLETON_FILES'),
    'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS': ('nastech_cli.auth_constants', 'SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS'),
    'SPOTIFY_DASHBOARD_URL': ('nastech_cli.auth_constants', 'SPOTIFY_DASHBOARD_URL'),
    'XAI_OAUTH_DEVICE_CODE_URL': ('nastech_cli.auth_constants', 'XAI_OAUTH_DEVICE_CODE_URL'),
    'XAI_OAUTH_DISCOVERY_URL': ('nastech_cli.auth_constants', 'XAI_OAUTH_DISCOVERY_URL'),
    'XAI_OAUTH_ISSUER': ('nastech_cli.auth_constants', 'XAI_OAUTH_ISSUER'),
    'refresh_nastech_oauth_pure': ('nastech_cli.auth_nastech', 'refresh_nastech_oauth_pure'),
}
