...
from nastech_constants import get_nastech_home  # noqa: F401 — config paths

from nastech_cli.local_runtime.binaries import runtimes_root

logger = logging.getLogger(__name__)

_SUPERVISOR = None  # process-wide singleton; one router per Nastech process
...
def models_dir() -> Path:
    ...
    return get_default_nastech_root() / "models"
...
def _stop_state_server(state: dict) -> None:
    ...
    from nastech_cli.local_runtime.endpoint import _pid_alive
...
def ensure_local_runtime(config: dict, force: bool = False) -> "object | None":
    ...
    # Another Nastech process may already be supervising — reuse via state,
    ...
    try:
        ...
        from nastech_cli.local_runtime.presets import generate_presets
        ...
        # Nastech-release default when unpinned) is preferred; when it isn't
