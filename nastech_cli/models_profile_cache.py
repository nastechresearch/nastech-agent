"""Per-profile view of ``nastech_cli.models``' module-level catalog slots.
...
files. In a multiplexed gateway every turn runs under a NASTECH_HOME override, so a single module slot
...
def profile_slot_get(module: Any, attr: str, default: Any = None) -> Any:
    ...
    return _SLOTS_BY_HOME.get((nastech_home_key(), attr), default)
...
def profile_slot_set(module: Any, attr: str, value: Any) -> None:
    if get_nastech_home_override() is None:
        ...
        _SLOTS_BY_HOME[(nastech_home_key(), attr)] = value
