...
The Python backing the install is shared by every Nastech profile because the checkout's ``venv``
is shared. Runtime repair therefore uses an install-scoped store under
``<checkout>/.nastech-runtime/python``. A vulnerable interpreter is never reinstalled in place: a
...
def managed_uv_path() -> Path:
    """Path of Nastech' own uv binary (``$NASTECH_HOME/bin/uv[.exe]``); may not exist yet."""
    return get_nastech_home() / "bin" / ("uv.exe" if platform.system() == "Windows" else "uv")
...
def pip_install_hint(package: str) -> str:
    ...
    Names Nastech' own uv when it exists: the installer drops it in ``$NASTECH_HOME/bin``
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print("    Sessions stay protected meanwhile: Nastech keeps databases "
...
def _record_runtime_repair(repair: RuntimeRepairResult) -> None:
    """Put the repair outcome into the update receipt (no-op outside ``nastech update``).
    ...
    """
    from nastech_cli.update_receipt import record_skip, record_step
...
def _uv_self_update_stamp() -> Path:
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "cache" / ".uv_self_update_stamp"


def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    """True when ``uv self update`` ran recently enough to skip.

    uv releases roughly weekly while many users run ``nastech update`` daily; a blocking network
...
def _reload_nastech_constants():
    """Re-execute ``nastech_constants`` from disk (the imported one may predate venv_python_path)."""
...
def _attempt_install_generation(
    ...
    try:
        ...
        return reject("uv resolved Python outside the Nastech generation: %s", python)
...
def _install_safe_python_generation(
    ...
    # supported minor (e.g. 3.11 → 3.12) so the user isn't stuck on every `nastech update`. The
...
def _stream_sync(argv: list[str], *, cwd: Path, env: dict[str, str]) -> tuple[int, str]:
    ...
    scrollback only, and the rejection carried a bare exit code — "nastech update says the SQLite
...
def _install_uv(target: Path) -> None:
    ...
    into ``$NASTECH_HOME/bin/`` instead of ``~/.local/bin/``.
    """
