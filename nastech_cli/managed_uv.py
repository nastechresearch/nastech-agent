"""Nastech-managed uv and Python runtime repair.
...
The Python backing the install is different: it is shared by every Nastech
profile because the checkout's ``venv`` is shared.  Runtime repair therefore
uses an install-scoped store under ``<checkout>/.nastech-runtime/python``. A
...
from nastech_cli.sqlite_runtime import SQLiteRuntimeInfo, probe_sqlite_runtime

logger = logging.getLogger(__name__)

_PROJECT_ROOT = Path(__file__).resolve().parents[1]
_RUNTIME_DIR_NAME = ".nastech-runtime"
_VENV_NAME = "venv"
_ALT_VENV_NAME = ".venv"
_REPAIR_LOCK_NAME = "runtime-repair.lock"
_MACOS_MANAGED_PYTHON_IDENTIFIER = "com.nastechresearch.nastech.managed-python"
...
def managed_python_env(
    ...
    *,
    ...
    """Return a sanitized environment for Nastech-private uv Python commands."""
    target = (
        Path(install_dir)
...
def _macos_sign_managed_python(python: Path) -> bool:
    ...
    TCC with a cdhash-only identity that changes whenever Nastech provisions a
...
def _report_runtime_repair_failure(repair: RuntimeRepairResult) -> None:
    if repair.backup_venv is None:
        ...
        print(
            "    Sessions stay protected meanwhile: Nastech keeps databases "
            "out of WAL mode on this SQLite build. The next `nastech update` "
            "will retry."
        )
        return
...
def _uv_self_update_is_fresh(now: float | None = None) -> bool:
    ...
    uv releases roughly weekly while many users run ``nastech update`` daily;
    re-running a blocking network self-update on every invocation is waste
    and, offline, an unbounded hang risk. A stamp file under NASTECH_HOME
    caches the last successful self-update time.
    """
    try:
        from nastech_constants import get_nastech_home

        stamp = get_nastech_home() / "cache" / ".uv_self_update_stamp"
...
def _reload_nastech_constants():
    """Re-execute ``nastech_constants`` from disk and return the fresh module.

    ``nastech update`` imports ``nastech_constants`` from the OLD checkout,
    ...
    return importlib.reload(nastech_constants)
...
def _attempt_install_generation(
    ...
    *,
    ...
    try:
        ...
        logger.warning("uv resolved Python outside the Nastech generation: %s", python)
        _remove_tree(generation, boundary=python_root)
        return None
...
def _install_safe_python_generation(
    ...
    *,
    ...
    # user isn't stuck on every `nastech update` with no path to a fixed
...
def _windows_runtime_holders() -> tuple[bool, str]:
    if platform.system() != "Windows":
        return False, ""
    main_module = sys.modules.get("nastech_cli.main")
    ...
    if holders:
        ...
        return True, f"other Nastech processes still hold the venv (PID {pids})"
    return False, ""
...
def _windows_runtime_self_lock(live: Path) -> tuple[bool, str]:
    ...
    ancestors on purpose — a CLI ``nastech update`` itself runs from the
    ...
    updater's own ``python.exe`` (or a waiting ``nastech.exe`` launcher
    ...
    # Belt-and-braces: the venv\Scripts\nastech.exe launcher stays mapped
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    ...
    unmanaged installs.  Only the Nastech-managed binary is ever refreshed;
...
def _default_live_venv(root: Path) -> Path:
    ...
    ``not-applicable`` on every ``nastech update`` and stayed on
    journal_mode=DELETE forever — even though the WAL fallback warning
    promises that ``nastech update`` repairs the runtime (issue class:
...
def repair_vulnerable_runtime(
    ...
    *,
    ...
    if self_locked:
        ...
        print(
            "    Retrying `nastech update` from inside this venv cannot help: "
        ...
        print(f"      cd {root}")
        print("      <system Python> -m nastech_cli.main update")
        print(
            "    Sessions stay protected meanwhile: Nastech keeps databases "
