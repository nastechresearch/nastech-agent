...
Backs ``nastech kanban export|import``, the ``/boards/{slug}/export`` and
...
ARCHIVE_FORMAT = "nastech-kanban-board"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manifest = {
            ...
            "nastech_version": str(nastech_version),
...
def _read_manifest(root: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise ValueError("archive is not a Nastech kanban board export (no manifest.json)")
    try:
    ...
    if not isinstance(manifest, dict) or manifest.get("format") != ARCHIVE_FORMAT:
        raise ValueError(
            "archive is not a Nastech kanban board export "
    ...
    if not isinstance(version, int) or version > ARCHIVE_FORMAT_VERSION:
        raise ValueError(
            f"archive format version {version!r} is newer than this Nastech "
            f"understands (max {ARCHIVE_FORMAT_VERSION}) — update Nastech and retry"
        )
    return manifest
    ...
    *,
    ...
    return {
        ...
        "source": {
            ...
            "nastech_version": manifest.get("nastech_version"),
        },
        "counts": counts,
