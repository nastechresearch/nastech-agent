"""Agent construction + session-resume display for ``NastechCLI``: credential resolution,
...
def _single_query_clarify_callback(question: str, choices=None, multi_select=False) -> str:
    """Headless clarify answer for ``nastech chat -q``.
...
def _credential_pool_notice(provider: str) -> tuple:
    ...
    if dead:
        ...
        lines.append(f"The {provider} sign-in was lost ({reason}); run `nastech auth add {provider}` "
                     "to sign in again.")
...
class CLIAgentSetupMixin:
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        try:
            # target_model: the ladder's model-keyed rungs (Zen/Go api_mode, Copilot/Nastech
            # api_mode) must see the model this CLI will actually send, not config's `default`,
            # or `nastech -m mimo-v2.5 --provider opencode-go` resolves an api_mode/base_url the
        ...
        # is just the provider slug/display name (`nastech chat --model <provider-name>`
        ...
        # Still empty (e.g. `nastech auth add` without `nastech model`): fall back to the
        ...
        if self.model != _model_at_entry and getattr(self, "_explicit_reasoning_config", None) is None:
            from nastech_cli.cli_model_switch_mixin import _resolve_cli_reasoning
    ...
    def _offer_first_run_setup(self) -> bool:
        ...
        startup, TTY). Runs the same flow as ``nastech model`` so onboarding has a single
    ...
    def _load_resumed_history_late(self) -> bool:
        ...
        # stderr so stdout stays machine-readable for `$(nastech chat -Q --resume ...)`.
        ...
        if not session_meta:
            hint = "Use a session ID from a previous CLI run (nastech sessions list)."
            if _quiet_mode:
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        try:
            ...
            console = ChatConsole()
            from nastech_cli.cli_chat_error_copy import agent_init_failure_message
    ...
    def _preload_resumed_session(self) -> bool:
        ...
        if not session_meta:
            ...
            self._console_print("[dim]Use a session ID from a previous CLI run (nastech sessions list).[/]")
            return False
        session_meta = self._follow_compression_chain(
            session_meta,
    ...
    def _display_resumed_history(self):
        ...
        role_styles = {
            ...
            "assistant": ("  ◆ Nastech: ", f"dim bold {_assistant_label_c}", "dim", " " * 12),
            "assistant_last": ("  ◆ Nastech: ", f"bold {_assistant_label_c}", "", " " * 12),  # full, non-dim
        }
        lines = Text()
        if skipped:
