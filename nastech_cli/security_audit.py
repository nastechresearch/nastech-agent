...
def _discover_plugins(nastech_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.nastech/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, nastech_home: Optional[Path] = None
) -> list[Component]:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    components = _discover_components(nastech_home=home, **skips)
    ...
    try:
        findings = run_audit(nastech_home=home, components=components)
