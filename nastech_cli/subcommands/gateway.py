...
def build_gateway_parser(
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy", help="Remove legacy nastech.service units from pre-rename installs",
        description="Stop, disable, and remove legacy Nastech gateway unit files "
    ...
    gateway_enroll = gateway_subparsers.add_parser("enroll",
        ...
        description="Redeem a single-use enrollment token with a relay connector. "
            "Authenticates as your Nastech Portal account (the connector derives the "
            ...
            "Requires being logged in (nastech setup). Not available in managed installs.")
    ...
    gateway_enroll.add_argument("--wake-url", dest="wake_url", default=None,
        help="Phase 5 §5.2 wake URL: a reachable URL the connector pokes "
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.nastech/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description="Run a local HTTP server that forwards OpenAI-compatible requests "
            "to an OAuth-authenticated provider (e.g. Nastech Portal). External "
    ...
    proxy_start.add_argument("--provider", default="nastech",
        help="Upstream provider: nastech or xai (default: nastech). See `nastech proxy providers`.")
