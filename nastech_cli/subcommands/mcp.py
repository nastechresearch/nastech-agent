...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp", help="Manage MCP servers and run Nastech as an MCP server",
        description="Manage MCP server connections and run Nastech as an MCP server.\n\n"
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve", help="Run Nastech as an MCP server (expose conversations to other agents)")
    ...
    # Catalog (Nastech-approved MCPs shipped with the repo)
    mcp_sub.add_parser(
        "picker", help="Interactive catalog picker (also the default for `nastech mcp`)")
    mcp_sub.add_parser("catalog", help="List Nastech-approved MCPs available for one-click install")
    mcp_install_p = mcp_sub.add_parser(
        "install", help="Install a catalog MCP by name (e.g. `nastech mcp install n8n`)")
