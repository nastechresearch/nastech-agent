...
def _add_server_runtime_args(parser) -> None:
    ...
    # manager / PID file: they scan the process table for `nastech dashboard|serve`
    # cmdlines and SIGTERM them — the same path `nastech update` uses.
    parser.add_argument(
        "--stop", action="store_true", help="Stop all running Nastech web server processes and exit")
    parser.add_argument(
        "--status", action="store_true", help="List running Nastech web server processes and exit")
...
def build_serve_parser(
    *, cmd_dashboard: Callable, add_help: bool = True, exit_on_error: bool = True,
    ...
    parser = argparse.ArgumentParser(
        prog="nastech serve",
        description="Run the Nastech backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    # Compat shim: desktop shells <= 0.15.x spawn `nastech dashboard --no-open --tui ...`;
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Nastech backend server (headless; powers the desktop app and remote backends)",
        description="Run the Nastech backend server — the JSON-RPC/WebSocket gateway the "
            "desktop app and remote clients connect to. Headless: it never opens "
            "a browser UI.")
    _configure_serve_parser(serve_parser, cmd_dashboard=cmd_dashboard)

    # `register` is nested so bare `nastech dashboard` keeps launching the server.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        "register",
        help="Register a self-hosted dashboard with Nastech Portal (writes the OAuth client ID to .env)",
        description="Register this install as a self-hosted dashboard with your Nastech "
    ...
    dashboard_register_parser.add_argument(
        ...
        help="Override the Nastech Portal base URL for registration (default: the "
