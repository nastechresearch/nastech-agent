...
def _add_server_runtime_args(parser) -> None:
    ...
    # `nastech dashboard` / `nastech serve` cmdlines and SIGTERM them directly —
    # the same path `nastech update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Nastech web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Nastech web server processes and exit",
    )
...
def build_serve_parser(
    *,
    ...
    parser = argparse.ArgumentParser(
        prog="nastech serve",
        description=(
            "Run the Nastech backend server - the JSON-RPC/WebSocket gateway the "
...
def build_dashboard_parser(
    ...
    # browser. It exists so the Nastech Desktop app (and headless remote
    ...
    # client with Nastech Portal and write the client_id into ~/.nastech/.env.
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            "Register this install as a self-hosted dashboard with your Nastech "
    ...
    dashboard_register_parser.add_argument(
        ...
        default=None,
        help=(
            "Override the Nastech Portal base URL for registration (default: the "
