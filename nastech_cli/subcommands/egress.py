"""``nastech egress`` subcommand parser."""
...
def build_egress_parser(subparsers) -> None:
    ...
    # OUTBOUND egress firewall (iron-proxy); `nastech proxy` is the separate INBOUND
    ...
    def _dispatch_egress(args):  # noqa: ANN001
        # dest='egress_command' stays disjoint from ``nastech proxy`` (dest='proxy_command').
