"""``nastech logs`` subcommand parser."""
...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Nastech log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    nastech logs                    Show last 50 lines of agent.log
    nastech logs -f                 Follow agent.log in real time
    nastech logs errors             Show last 50 lines of errors.log
    nastech logs gateway -n 100     Show last 100 lines of gateway.log
    nastech logs gui -f             Follow gui.log in real time
    nastech logs desktop -f         Follow desktop.log (Electron app boot/backend)
    nastech logs --level WARNING    Only show WARNING and above
    nastech logs --session abc123   Filter by session ID
    nastech logs --component tools  Only show tool-related lines
    nastech logs --since 1h         Lines from the last hour
    nastech logs --since 30m -f     Follow, starting from 30 min ago
    nastech logs list               List available log files with sizes
""")
