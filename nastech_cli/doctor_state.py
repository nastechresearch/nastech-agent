"""NASTECH_HOME state checks for nastech doctor: directories, memory files, state.db health, skills hub, memory provider, profiles.
...
def _doctor_memory_config(nastech_home: Path | None = None) -> dict:
    ...
    try:
        ...
        config_path = (nastech_home if nastech_home is not None else NASTECH_HOME) / "config.yaml"
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        if deferral.get("futile"):
            ...
            lines.append(("warn", f"state.db FTS repair is blocked after {deferral.get('attempts') or '?'} deferral(s) "
                          ...
                          "'nastech sessions optimize-storage' with every holder stopped)"))
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += "; run 'nastech sessions optimize-storage' offline (with the gateway stopped) to compact FTS storage"
...
def _memory_store_flags(nastech_home: Path) -> tuple:
    from tools.memory_tool import get_builtin_memory_store_flags
    return get_builtin_memory_store_flags({"memory": _doctor_memory_config(nastech_home)})


@doctor_check()
def _check_directory_structure(should_fix: bool, f: Finding) -> None:
    """NASTECH_HOME, expected subdirs, SOUL.md, and the enabled built-in memory files."""
    from nastech_cli.doctor import NASTECH_HOME, _DHH
    nastech_home = NASTECH_HOME
    ensure_dir(f, should_fix, nastech_home, f"{_DHH} directory exists", f"Created {_DHH} directory", f"{_DHH} not found")
    _memory_enabled, _user_profile_enabled = _memory_store_flags(nastech_home)
    ...
    for subdir_name in ["cron", "sessions", "logs", "skills"] + (["memories"] if memory_on else []):
        ensure_dir(f, should_fix, nastech_home / subdir_name, f"{_DHH}/{subdir_name}/ exists",
...
def _write_health_reason(state_db_path: Path, *, should_fix: bool):
    ...
    from nastech_state_repair import _db_opens_cleanly, _live_writer_holds_db
    ...
    if not should_fix and state_db_path.stat().st_size > _WRITE_PROBE_SNAPSHOT_MAX_BYTES:
        check_info("state.db write-health probe skipped: store is held by a live writer and larger than 1 GB "
                   "(run 'nastech doctor --fix' to probe it)")
        return None
    import sqlite3
    import tempfile
...
_STATE_DB_REPAIRS = {
    "fts": ("Repaired state.db FTS write health",
            ...
            "state.db FTS write corruption — run 'nastech doctor --fix' (or 'nastech sessions repair') to rebuild the FTS index"),
    "schema": ("Repaired state.db schema ({count} sessions recovered)",
               ...
               "state.db schema malformed — run 'nastech doctor --fix' (or 'nastech sessions repair') to recover hidden sessions"),
}
_STATE_DB_STRUCTURAL_ISSUE = (
    ...
    "cannot repair it. Stop the gateway, then run 'nastech {profile_arg}sessions recover --source {db_path} "
    "--inspect-only' and, if it reports recoverable, 'nastech {profile_arg}sessions recover --source {db_path} "
...
def _repair_state_db(f: Finding, should_fix: bool, state_db_path: Path, kind: str) -> None:
    ...
    from nastech_state_repair import repair_state_db_schema
...
def _report_structural_damage(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, reason) -> bool:
    ...
    from nastech_state_repair import state_db_has_structural_damage
...
def _classify_unreadable_state_db(f: Finding, should_fix: bool, state_db_path: Path, _DHH: str, exc: Exception) -> None:
    ...
    from nastech_state import is_malformed_db_error
...
def _state_db_stats(issues: list, state_db_path: Path) -> None:
    ...
    with warn_on_error("state.db stats unavailable ({e})", "", report=lambda t, _d: check_info(t)):
        from nastech_state_dbfile import collect_state_db_stats, count_db_holders
        ...
        for _kind, _text, _detail in rows:
            ...
            if "auto_prune" in _detail:
                issues.append("state.db is large — enable sessions.auto_prune in config.yaml"
                              + (" and run 'nastech sessions optimize-storage' offline (gateway stopped)" if "optimize-storage" in _detail else ""))
...
def _state_db_wal(f: Finding, should_fix: bool, state_db_path: Path) -> None:
    ...
    with warn_on_error(""):
        ...
        if size > 50 * 1024 * 1024:  # 50 MB
            ...
            from nastech_state_repair import _exclusive_repair_db_guard, _live_writer_holds_db
            ...
            _SKIP = ("Large WAL file — cannot prove state.db is quiet (stop the profile's gateway first, then "
                     "run 'nastech doctor --fix' to checkpoint)")
            ...
            if not should_fix:
                return f.issues.append(
                    "Large WAL file — stop the profile's gateway, then run 'nastech doctor --fix' to checkpoint")
            with _exclusive_repair_db_guard(state_db_path) as (guard, guard_error):
                if guard is None:
                    check_warn("WAL checkpoint skipped: could not take exclusive ownership of state.db",
                               f"({guard_error}; stop the profile's gateway and re-run 'nastech doctor --fix')")
                    return f.issues.append(_SKIP)
...
def _check_skills_hub(should_fix: bool, f: Finding) -> None:
    ...
    if check_bool(hub_dir.exists(), "Skills Hub directory exists", ("Skills Hub directory not initialized", "(run: nastech skills list)")):
...
def _memory_provider_honcho(issues: list) -> None:
    ...
    if not cfg_path.exists():
        ...
        check_bool(hcfg.api_key or hcfg.base_url,
                   ...
                   ("Honcho config not found", "run: nastech memory setup"))
    elif not hcfg.enabled:
        check_info(f"Honcho disabled (set enabled: true in {cfg_path} to activate)")
    elif not (hcfg.api_key or hcfg.base_url):
        _fail_and_issue("Honcho API key or base URL not set", "run: nastech memory setup",
                        "No Honcho API key — run 'nastech memory setup'", issues)
    else:
...
def _memory_provider_mem0(issues: list) -> None:
    ...
    if mem0_cfg.get("api_key", ""):
        ...
        _fail_and_issue("Mem0 API key not set", "(set MEM0_API_KEY in .env or run nastech memory setup)",
...
def _memory_provider_generic(name: str) -> None:
    ...
    if _provider and _provider.is_available():
        ...
        check_warn(f"{name} configured but not available", "run: nastech memory status")
    else:
        check_warn(f"{name} plugin not found", "run: nastech memory setup")


@doctor_check()
def _check_memory_provider(should_fix: bool, f: Finding) -> None:
    from nastech_cli.doctor import NASTECH_HOME
    name = _doctor_memory_config(NASTECH_HOME).get("provider", "")
    if not name:
    ...
    if wrapper_dir.is_dir():
        for wrapper in wrapper_dir.iterdir():
            if not wrapper.is_file():
                continue
            with warn_on_error(""):
                _m = _re.search(r"nastech -p (\S+)", wrapper.read_text(encoding="utf-8"))
