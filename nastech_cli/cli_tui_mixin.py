...
interactive CLI. Mixin bound onto ``NastechCLI`` via the MRO; cli.py-internal symbols are imported
...
class CLITuiMixin:
    ...
    def _render_scroll_list_panel(self, state, title, hint, labels, *, min_width, max_width, indent):
        ...
        """
        from cli import NastechCLI, _panel_box_width, _wrap_panel_text
    ...
    def _get_model_picker_display_fragments(self):
        ...
        if state.get("stage", "provider") == "provider":
            ...
            from nastech_cli.cli_model_switch_mixin import _picker_reasoning_rows
        ...
        if code := self._sudo_state.get("vault_code"):
            return self._render_sudo_style_panel(
                ...
                [f'{code["site"]} is asking for a one-time code (text message, email or authenticator app).',
                 'Type the code and press Enter; Nastech enters it into the page for you.',
    ...
    def _tui_handle_ignored_terminal_sequence(self, event):
        ...
        nastech_cli.pt_input_extras registers focus reports (CSI I / CSI O) as Keys.Ignore at the
    ...
    def _tui_enter_while_busy(self, text: str, images: list, payload) -> None:
        ...
        """
        from cli import CLI_CONFIG, _ACCENT, _DIM, _RST, _cprint, _nastech_home
        ...
        if _effective_mode == "queue":
            ...
            if redirected:
                ...
                try:
                    with open(_nastech_home / "interrupt_debug.log", "a", encoding="utf-8") as _f:
                        _f.write(
    ...
    def _tui_collapse_paste(self, text: str, line_count: int, *, fallback: bool) -> str:
        """Save a large paste under ~/.nastech/pastes and return the placeholder for the buffer."""
        from cli import _nastech_home, datetime, logger
    ...
    def _tui_handle_prompt_stash(self, event):
        ...
        """
        from nastech_cli.prompt_stash import ACTION_RESTORED, ACTION_STASHED, resolve_ctrl_s
        buf = event.app.current_buffer
    ...
    def _tui_build_key_bindings(self):
        ...
        from nastech_cli.cli_subagent_monitor import modal_prompt_active, open_monitor, toggle_dock
        for key in ('c-t', 'f6'):
    ...
    def _tui_build_layout(self, kb):
        ...
        cli_ref = self
        from nastech_cli.cli_subagent_monitor import install_dock
        install_dock(self)
