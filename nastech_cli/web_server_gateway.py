...
from nastech_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from nastech_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
# ~/.nastech/logs/<action>.log which the dashboard tails.
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from nastech_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``nastech <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``nastech -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_nastech_dotenv``
    ...
    Named-profile actions therefore start from Nastech' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``NASTECH_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from nastech_cli.web_server_profiles import _resolve_profile_dir
        from nastech_constants import apply_subprocess_home_env, get_default_nastech_root
        ...
        try:
            source_homes = {str(get_default_nastech_root()), str(get_nastech_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["NASTECH_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["NASTECH_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _NASTECH_GATEWAY=1;
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    ``nastech update`` fleet restart (``update_cmd_fleet._needs_sudo`` / ``_sudo_noninteractive_ok``).
    """
    from nastech_cli.update_cmd_fleet import _needs_sudo
    ...
    from nastech_cli.gateway import _select_systemd_scope
    from nastech_constants import reset_nastech_home_override, set_nastech_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from NASTECH_HOME, so a selector-bearing action must be resolved
    ...
    from nastech_cli.web_server_profiles import _resolve_profile_dir
    token = set_nastech_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_nastech_action(
    ...
    """Spawn ``nastech <subcommand>`` detached (via ``nastech_cli.main``) and record the handle."""
    from nastech_cli.web_server import PROJECT_ROOT
    ...
    if _action_targets_system_gateway(subcommand):
        ...
        # NASTECH_HOME past sudo's env_reset and reads SUDO_USER for the service identity.
        ...
        # two-step gate as the ``nastech update`` fleet restart: a refused blanket probe falls
        # back to ``sudo -l`` on the exact argv, so a command-scoped NOPASSWD entry qualifies.
        from nastech_cli.update_cmd_fleet import _sudo_noninteractive_ok
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo nastech {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _NASTECH_GATEWAY).
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``nastech --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_nastech_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``nastech [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``NASTECH_HOME`` and hit the same exit-78 refusal."""
    from nastech_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from nastech_constants import get_process_nastech_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_nastech_home()) == "default" else ["-p", "default"]
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    The spawned ``nastech -p X gateway <verb>`` would only print exit-78 / "no gateway running for this
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from nastech_cli.web_server import _spawn_gateway_restart
    try:
