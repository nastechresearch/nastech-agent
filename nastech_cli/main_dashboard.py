...
def _find_stale_dashboard_pids(*, exclude_pids: set[int] | None = None,
    ...
    *scope_home*: keep only backends whose resolved Nastech home (see
    ``_nastech_home_for_pid``) is this home; unreadable ownership is spared, never guessed.
    ...
    # 'nastech dashboard --stop'``); killing it takes down the invoking terminal.
    ancestors = _caller_ancestor_pids()
    pids = [pid for pid in pids if not _is_caller_wrapper_shell(pid, ancestors)]
    return _pids_owned_by_nastech_home(pids, scope_home) if scope_home else pids
...
def _parse_dashboard_runtime(command: str) -> tuple[str, str, int] | None:
    ...
    for candidate in ("dashboard", "serve"):
        patterns = (f"nastech {candidate}", f"nastech_cli.main {candidate}", f"nastech_cli/main.py {candidate}")
...
def _restart_managed_dashboard_service(reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT) -> bool:
    ...
    # User manager first (Nastech installs Linux services in the user scope by
...
def _get_systemd_service_for_pid(pid: int) -> str | None:
    """The systemd service unit name *pid* belongs to (``nastech-serve.service``), or None.
...
# launchd plist directories that can supervise a ``nastech dashboard`` / ``nastech serve`` backend on
...
def _loaded_launchd_backend_jobs(
    ...
    ``ProgramArguments`` is a ``nastech dashboard`` / ``nastech serve`` backend. macOS only (empty
    ...
    from nastech_cli.gateway import _launchd_print_service_pid
...
def _restart_launchd_job(domain: str, label: str, old_pid: int | None, *, timeout: float = 15.0) -> bool:
    ...
    from nastech_cli.gateway import _wait_for_launchd_service_pid
    try:
...
def _respawn_dashboard_processes(commands: list[list[str]]) -> list[list[str]]:
    """Respawn manually-started dashboards after ``nastech update``, detached, logging to
...
class _UpdateOutputStream:
    """stdout/stderr wrapper for ``nastech update``: mirrors to ``logs/update.log`` and, once the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    try:
        ...
        stage = "continued on the pulled code" if os.environ.get("NASTECH_UPDATE_POST_SWAP") == "1" else "started"
        log_file.write(f"\n=== nastech update {stage} {_dt.datetime.now().isoformat(timespec='seconds')} ===\n")
...
def _report_dashboard_status() -> int:
    ...
    """
    from nastech_cli.dashboard_procs import _scan_dashboard_processes
...
def _read_ssh_session_token_file(path: str) -> str:
    ...
    # The Desktop client writes the token under the account's $HOME/.nastech/
    # desktop-ssh, independent of NASTECH_HOME and the active profile. Anchor
    # validation there, NOT get_nastech_home(): a non-default profile or a Docker
    # /opt/data root re-homes get_nastech_home() and would reject every token.
    # See #69551.
    token_root = Path.home() / ".nastech" / "desktop-ssh"
    try:
...
def _is_electron_packaged_web_dist(path: str) -> bool:
    ...
    A standalone ``nastech dashboard`` inheriting that ``NASTECH_WEB_DIST`` would
...
def _route_named_profile_dashboard(
    ...
    (NASTECH_DESKTOP=1) stay per-profile. Returns normally when no routing applies.
    """
    try:
    ...
    if (
        ...
        or os.environ.get("NASTECH_DESKTOP") == "1"
    ):
        return
    ...
    # NASTECH_HOME is pinned to the machine root below — the factory must not
    # re-inject a profile home.
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=False)
    # Pin the child to the machine ROOT, resolved explicitly rather than by
    # dropping NASTECH_HOME: in the Docker layout the root is /opt/data, and an
    # unset NASTECH_HOME would fall back to $HOME/.nastech = /opt/data/.nastech — an
    # empty auto-seeded home with only the default profile and no install stamp.
    # get_default_nastech_root() strips a trailing profiles/<name> for both layouts.
    try:
        from nastech_constants import get_default_nastech_root
        env["NASTECH_HOME"] = str(get_default_nastech_root())
    except Exception:
        env.pop("NASTECH_HOME", None)  # prior behaviour rather than blocking the reroute
...
def _resolve_dashboard_web_dist(args, _headless_backend: bool) -> None:
    ...
    ``serve`` sets NASTECH_SERVE_HEADLESS so mount_spa() stays off. Otherwise build
    unless NASTECH_WEB_DIST / --skip-build promise a dist — then verify index.html
    (else the server serves 404s). --skip-build on the default location gets ONE
    recovery build; a caller-managed NASTECH_WEB_DIST can't be populated.
    """
    from nastech_cli.main import PROJECT_ROOT
    from nastech_cli.main_web_build import _build_web_ui
    skip_build = getattr(args, "skip_build", False)
    if _headless_backend:
        os.environ["NASTECH_SERVE_HEADLESS"] = "1"  # set before web_server import
    elif "NASTECH_WEB_DIST" not in os.environ and not skip_build:
        ...
        if not (_dist_root / "index.html").exists():
            ...
            # --build-mode skip after a wipe of web_dist); a custom NASTECH_WEB_DIST
        ...
        # NASTECH_WEB_DIST without --skip-build: the env var points at a
        # caller-managed dist, so validate it like the --skip-build branch.
        # NASTECH_WEB_DIST is set without --skip-build: the build is skipped (the env var points at a
        ...
        if not (_dist_root / "index.html").exists():
            ...
            sys.exit(1)
        # web_server reads NASTECH_WEB_DIST raw at import (no expanduser), so a
