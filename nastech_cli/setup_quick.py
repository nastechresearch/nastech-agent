"""Streamlined setup flows: the Nastech Portal one-shot (`nastech portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``nastech_cli.setup`` take effect."""
...
logger = logging.getLogger("nastech_cli.setup")
...
def _blank_slate_done(config: dict, nastech_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from nastech_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         nastech skills opt-in --sync",
          "  Add MCP servers:     nastech mcp add", *extra, "  Tune agent settings: nastech setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from nastech_cli.setup import load_config
...
def _run_nastech_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    """Run ``_model_flow_nastech`` (login, model pick, provider switch, Tool Gateway opt-in) — the
    single source of truth shared with ``nastech model``. False when cancelled or failed (the
    message is already printed)."""
    from nastech_cli.setup import _info
    try:
        from nastech_cli.model_setup_flows import _model_flow_nastech
        _model_flow_nastech(config)
        return True
    except cancel_exc:
        # _login_nastech raises SystemExit(130)/(1) on cancel/failure; the expired-session re-login
        # path inside _model_flow_nastech only catches Exception, so SystemExit would kill the CLI.
        _info(*cancel_lines)
    except Exception as exc:
        logger.debug("_model_flow_nastech error during %s: %s", context, exc)
        print_error(exc)
    return False
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print()
        lead, *rest = provider_setup_failure_lines(exc, retry_command="nastech portal")
        print_error(f"  {lead}")
        for line in rest:
            print_info(f"  {line}")

    if not _run_nastech_flow(config, context="`nastech portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `nastech portal`."),
...
def _run_first_time_quick_setup(config: dict, nastech_home, is_existing: bool):
    ...
    # Step 1: Nastech Portal — OAuth login + model selection (provider set to "nastech" by the save).
    ...
    _run_nastech_flow(config, context="quick setup", cancel_exc=(KeyboardInterrupt, EOFError),
                   cancel_lines=(None, "Nastech Portal setup cancelled."), print_error=_on_error)
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'nastech setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `nastech import`).
        from nastech_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    nastech setup")
    if gateway_choice != 0:
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Nastech.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Nastech update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``nastech-agent`` skill
    ...
    try:
        ...
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "nastech-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("nastech-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``nastech setup agent``."""
...
def _run_blank_slate_setup(config: dict, nastech_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `nastech-agent` skill is always kept so the agent",
          "can help you drive and configure Nastech itself.", None)
    ...
    if path != 0:
        ...
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `nastech update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, nastech_home, "  Enable tools:        nastech tools", "  Enable plugins:      nastech plugins",
...
def _blank_slate_walkthrough(config: dict, nastech_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `nastech-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`nastech update` runs from re-injecting them. Opt back in any",
              "time with `nastech skills opt-in --sync`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `nastech plugins list` / `nastech plugins install`.",
         "No plugins enabled. Add later with `nastech plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `nastech mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `nastech mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, nastech_home, "  Enable more tools:   nastech tools")
...
def _run_quick_setup(config: dict, nastech_home):
    ...
    from nastech_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'nastech setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Nastech to messaging apps to chat from anywhere.",
              "You can configure these later with 'nastech setup gateway'.")
