"""Nastech free-tier identity: the ``anonymous`` auth method of the ``nastech`` provider.

The identity is created in exactly one place, at boot (``nastech_cli.free_tier_bootstrap``), and only
while ``NASTECH_GUEST_ONBOARDING=1`` (see ``guest_enabled``). The bootstrap mints an anonymous Nastech
account (``POST /api/anonymous/create``); its ``anon_`` credential is later exchanged for short-lived
JWTs (``POST /api/anonymous/token``). The result is persisted as the singleton ``providers.nastech``; it
...
(the welcome inference host, single model ``nastech/welcome``).

Users are never shown the words guest / anonymous / account for this state: surfaces say
"Nastech · free tier". Two user-facing verbs reach the same flow, both keeping the identity's
connectors: ``nastech auth upgrade`` in a terminal and ``/login`` inside a chat.
...
from nastech_cli.auth_constants import (
    AuthError, DEFAULT_NASTECH_PORTAL_URL, DEFAULT_NASTECH_WELCOME_URL, _decode_jwt_claims, httpx)
...
ANON_SECRET_ENV = "NASTECH_ANON_API_SECRET"
...
GUEST_ONBOARDING_ENV = "NASTECH_GUEST_ONBOARDING"
...
FREE_TIER_LABEL = "Nastech · free tier"
UPGRADE_HINT = "Run `nastech auth upgrade` to sign in with a Nastech account, or /login inside a chat."
FREE_TIER_NOT_SIGNED_IN = (
    "You're not signed in. Free inference and connectors are always on. "
    "Run `nastech auth` to sign in with a Nastech account.")
...
# fallback (no ``/login``, no ``nastech`` verb, no guest / anonymous / credential). ``retryable``
# says whether a later attempt can succeed at all; ``retry_after`` is the wait the server named.
#
# What NAS actually sends (nastech-account-service ``api/anonymous/gate.ts`` and the routes behind it):
...
# what is unavailable is using Nastech without signing in, and signing in is free.
_SIGNIN_IS_FREE = "Signing in is free."
ANON_FAILURE_COPY = {
    ANON_GATE_CLOSED: f"This version can't be used without an account. {_SIGNIN_IS_FREE}",
    ANON_GATE_PAUSED: f"Using the free tier without signing in is paused for a moment. {_SIGNIN_IS_FREE}",
    ANON_RATE_LIMITED: "Lots of people are getting started right now. Try again in {wait}. "
                       "Signing in is free and skips the wait.",
    ANON_POW_REQUIRED: "The server asked for a proof of work, but that isn't implemented in your "
    ...
    ANON_UNREACHABLE: "The service couldn't be reached. Check your internet connection and try again.",
    ANON_SERVER_ERROR: "The service had a hiccup. Try again in a moment.",
}
...
def guest_enabled() -> bool:
    """The free tier is on for this process: the launch gate is set AND ``nastech.guest`` (default
    ...
    try:
        ...
        nastech_cfg = load_config_readonly().get("nastech")
    except Exception as exc:  # config unreadable: keep today's behaviour (no guest) rather than mint
        logger.debug("guest: config unreadable, treating nastech.guest as false: %s", exc)
        return False
    if not isinstance(nastech_cfg, dict):
        return True
    return bool(nastech_cfg.get("guest", True))
...
def is_anonymous_request(provider: Any, api_key: Any) -> bool:
    ...
    """
    from nastech_cli.auth_constants import _decode_jwt_claims
    return provider == "nastech" and _decode_jwt_claims(api_key).get("account_tier") == ANON_ACCOUNT_TIER
...
def current_nastech_state() -> Optional[Dict[str, Any]]:
    """The profile's ``providers.nastech`` state without locking or network (status/picker reads)."""
    from nastech_cli.auth import _load_auth_store, _load_provider_state
    try:
        return _load_provider_state(_load_auth_store(), "nastech")
...
def has_guest() -> bool:
    return is_guest_state(current_nastech_state())


def guest_carries_inference() -> bool:
    """True when the profile's Nastech identity is the free tier and the free tier is on.
    ...
    credential-pool entry can pick a paid Nastech key while the profile singleton is still a guest.
    """
...
# itself; it never widens the NETWORK-side allowlist in ``auth_nastech``.
EXTRA_WELCOME_HOSTS_ENV = "NASTECH_EXTRA_WELCOME_HOSTS"


def welcome_hosts() -> frozenset[str]:
    """``WELCOME_HOSTS`` plus any ``NASTECH_EXTRA_WELCOME_HOSTS`` entries (lowercased hostnames)."""
...
def pin_model_for_route(provider: Any, base_url: Any, model: Any) -> Any:
    """Model policy at agent START: on the Nastech welcome host the model is ``nastech/welcome``; anywhere
    ...
    """
    if provider == "nastech" and route_is_welcome_host(base_url):
        if model and model != GUEST_MODEL:
            logger.info("Nastech free tier: using %s instead of configured model %s", GUEST_MODEL, model)
        return GUEST_MODEL
    return model


def route_can_serve_model(provider: Any, base_url: Any, model: Any) -> bool:
    """Eligibility for a credential ROTATION: the welcome host serves only ``nastech/welcome``, so a
    conversation on any other model must not be rotated onto it (and a ``nastech/welcome`` conversation
    may move to the portal host, which serves it too). Non-Nastech routes are always eligible."""
    if provider != "nastech" or not route_is_welcome_host(base_url):
        return True
    return not model or model == GUEST_MODEL


def route_is_welcome_host(base_url: Any) -> bool:
    """The routing predicate for the free tier: the welcome host serves exactly ``nastech/welcome``.
...
def _raise_for_anon_status(response: httpx.Response, *, action: str) -> Dict[str, Any]:
    ...
    if code == ANON_SERVER_ERROR:
        logger.info("Nastech free tier %s failed (%s%s)", action, status, f": {error}" if error else "")
...
def mint_guest(client: httpx.Client, portal_base_url: str) -> Dict[str, Any]:
    ...
    if not isinstance(token, str) or not token.startswith("anon_"):
        logger.info("Nastech free tier sign-up returned no credential")
...
def exchange_anon_jwt(client: httpx.Client, portal_base_url: str, anon_token: str) -> Dict[str, Any]:
    ...
    if not isinstance(payload.get("access_token"), str) or not payload["access_token"]:
        logger.info("Nastech free tier token exchange returned no token")
        raise _anon_err(ANON_FAILURE_COPY[ANON_SERVER_ERROR], ANON_SERVER_ERROR)
    return payload
...
def apply_exchange_to_state(state: Dict[str, Any], exchanged: Dict[str, Any]) -> None:
    ...
    from nastech_cli.auth_nastech import _validate_nastech_inference_url_from_network
    ...
    # (a staging host without NASTECH_INFERENCE_BASE_URL set), the literal stands in. Never the paid
    # host: the gateway cross-refuses an anonymous JWT there.
    inference_url = (_validate_nastech_inference_url_from_network(exchanged.get("inference_base_url"))
                     or DEFAULT_NASTECH_WELCOME_URL)
...
def _portal_base_url() -> str:
    from nastech_cli.auth_nastech import _nastech_portal_env_override
    return (_nastech_portal_env_override() or DEFAULT_NASTECH_PORTAL_URL).rstrip("/")


def _shared_identity_key(state: Any) -> Optional[str]:
    """Stable identity of a Nastech credential: the anon_ token for a guest, the refresh token for an
...
def _mint_locked(
    ...
    from nastech_cli.auth import _store_provider_state, _save_auth_store
    from nastech_cli.auth_nastech import _write_shared_nastech_state
    ...
    _store_provider_state(auth_store, "nastech", state, set_active=carries_inference)
    _save_auth_store(auth_store)
    _write_shared_nastech_state(state)
    logger.info("Nastech free tier ready (identity minted)")
    return state
...
def _mint_memo_key() -> str:
    from nastech_constants import get_nastech_home_override, nastech_home_key
    return "" if get_nastech_home_override() is None else nastech_home_key()
...
def _reconcile_and_provision(*, timeout_seconds: float, carries_inference: bool = True) -> Optional[Dict[str, Any]]:
    ...
    1. The shared store is the identity of record for this Nastech root. If it holds an identity
    ...
    from nastech_cli.auth_nastech import (
        _nastech_http_client, _nastech_shared_store_lock, _read_shared_nastech_state, _write_shared_nastech_state)
    portal = _portal_base_url()
    with _auth_store_lock():
        auth_store = _load_auth_store()
        profile_state = _load_provider_state(auth_store, "nastech")
        with _nastech_shared_store_lock(timeout_seconds=max(timeout_seconds, 5.0)):
            shared = _read_shared_nastech_state()
            if shared and _shared_identity_key(shared) != _shared_identity_key(profile_state):
                state = dict(shared)
                _store_provider_state(
                    auth_store, "nastech", state,
                    set_active=carries_inference or not is_guest_state(state))
                _save_auth_store(auth_store)
                logger.debug("Nastech identity adopted from the shared store")
                return state
            if profile_state:
                if not shared:
                    _write_shared_nastech_state(profile_state)
...
def ensure_portal_identity(
    *, explicit: bool, timeout_seconds: float = GUEST_MINT_TIMEOUT_SECONDS,
    ...
    """Make sure this profile has a Nastech identity (guest or account); mint a guest only if the shared
    store has none. Returns the ``providers.nastech`` state, or None (disabled / failed once already).
    ...
    dead-credential replacements (``auth_nastech.resolve_nastech_runtime_credentials``,
    ...
    first, then shared, matching every other Nastech path. ``carries_inference=False`` leaves
    ...
    if not guest_enabled():
        return None
    failure = _mint_failure_for_profile()
    if failure and not force and not current_nastech_state() and time.monotonic() < failure.not_before:
    ...
    try:
        ...
        logger.info("Nastech free tier not set up (%s, attempt %d%s)", noted.code, noted.attempts,
...
def refresh_guest_state(state: Dict[str, Any], client: httpx.Client) -> None:
    ...
    from nastech_cli.auth import _nastech_portal_base_url
    apply_exchange_to_state(state, exchange_anon_jwt(client, _nastech_portal_base_url(state), anon_token))
...
def clear_dead_guest(reason: str, *, dead_token: Optional[str] = None) -> None:
    ...
    *dead_token* is None the profile's current guest is treated as the failed one.
    ...
    from nastech_cli.auth_nastech import _clear_shared_nastech_state, _nastech_shared_store_lock, _read_shared_nastech_state
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "nastech")
        if is_guest_state(state):
            token = dead_token or state.get("anon_token")
            if state.get("anon_token") == token:
                _store_section(auth_store, "providers").pop("nastech", None)
                _store_section(auth_store, "credential_pool").pop("nastech", None)
                if auth_store.get("active_provider") == "nastech":
    ...
    _clear_mint_failure()
    logger.info("Nastech free-tier identity retired (%s); a new one is set up on next use", reason)
...
# account that still asks for ``nastech/welcome`` is served the id's backing model and told what to
# switch to in the ``x-nastech-model-switch`` response header. Every rule for reading those lives here;
# the error classifier and the turn loop only call in.

MODEL_SWITCH_HEADER = "x-nastech-model-switch"
...
_WELCOME_ROUTE_REFUSALS = (
    ...
    ("serves anonymous nastech agent accounts only", "named_on_welcome_host"),
...
_WELCOME_ROUTE_COPY = {
    ...
    # the session: the one cause left is a user-set NASTECH_INFERENCE_BASE_URL naming the paid host.
    "anon_on_paid_host": "This install is set to use a different Nastech server (NASTECH_INFERENCE_BASE_URL). "
                         "Unset it to use the free model, or sign in. {signin}",
    "named_on_welcome_host": "This Nastech account needs to reconnect. {model_hint}",
    "tier_disabled": "Using Nastech without signing in is switched off right now. "
...
_SIGNIN_TERMINAL = "To sign in: `nastech auth upgrade`."
_MODEL_HINT_CHAT = "Run /model and pick the Nastech row again."
_MODEL_HINT_TERMINAL = "Run `nastech model` and pick the Nastech row again."
...
def welcome_refusal_copy(refusal: Dict[str, Any], *, model: str = "", in_chat: bool = True, door: bool = True) -> str:
    ...
    if reason == "model_not_free":
        ...
        return (f"{what} available without signing in, so Nastech uses {serves} for now. "
    ...
    return f"Nastech couldn't send that without signing in. Signing in is free. {signin}".rstrip()
...
def welcome_route_refusal_copy(kind: str, *, in_chat: bool = True, door: bool = True) -> str:
    template = _WELCOME_ROUTE_COPY.get(kind) or "Nastech couldn't reach the free model on this route."
    return template.format(
        host=DEFAULT_NASTECH_WELCOME_URL, signin=(_SIGNIN_CHAT if in_chat else _SIGNIN_TERMINAL) if door else "",
...
def note_model_switch(agent: Any, headers: Any) -> Optional[str]:
    """Record the gateway's ``x-nastech-model-switch`` header on *agent* for the next call, if present.

    The header arrives on a NAMED account's response that asked for ``nastech/welcome`` (the gateway
    ...
    if backing == requested:
        return None
    try:
        agent._nastech_pending_model_switch = (requested, backing)
    except Exception:
        return None
    return backing
...
def apply_model_switch(agent: Any) -> Optional[str]:
    ...
    map. The config write is the same one a sign-in completion uses, so ``nastech model`` and the
    gateway's config re-read agree with the live session.
    """
    pending = getattr(agent, "_nastech_pending_model_switch", None)
    if not pending:
        return None
    agent._nastech_pending_model_switch = None
    ...
    agent._nastech_model_switch = (requested, backing)
    logger.info("Nastech gateway asked to switch %s -> %s; applied for this session", requested, backing)
    try:
        from nastech_cli.config import load_config_readonly
        ...
        if str(model_cfg.get("default") or "").strip() == requested:
            ...
            _update_config_for_provider(
                "nastech", str(getattr(agent, "base_url", "") or ""), default_model=backing)
...
def guest_notice_pending() -> bool:
    ...
    state = current_nastech_state()
    return is_guest_state(state) and not bool(state.get(GUEST_NOTICE_FLAG))


def mark_guest_notice_shown() -> bool:
    """Persist ``guest_notice_shown`` on the guest's ``providers.nastech`` state (the active store).

    Returns True when a flag was written; False when there is no guest to mark."""
    from nastech_cli.auth import _provider_state_transaction, _save_auth_store, _store_section
    with _provider_state_transaction("nastech") as (auth_store, state, source_path):
        ...
        _store_section(auth_store, "providers")["nastech"] = state
        _save_auth_store(auth_store)
    return True


# --- ``nastech auth upgrade``: sign the guest into a real Nastech account, keeping its connectors ---------
#
...
# ``completed`` promotion is followed by the token grant, which ``persist_nastech_credentials`` writes
...
def register_promotion_intent(
    ...
    if not isinstance(payload.get("claim_code"), str) or not payload["claim_code"]:
        logger.info("Nastech free tier sign-in returned no transfer code")
        raise _anon_err(ANON_FAILURE_COPY[ANON_SERVER_ERROR], ANON_SERVER_ERROR)
    return payload
...
def _account_state_from_token(
    ...
    """The ``providers.nastech`` shape for the signed-in account (same fields the device-code login writes)."""
    from nastech_cli.auth import PROVIDER_REGISTRY, _coerce_ttl_seconds, _optional_base_url, _tls_state_from_verify
    from nastech_cli.auth_nastech import _NASTECH_EMPTY_AGENT_KEY_FIELDS, _iso_after, refresh_nastech_oauth_from_state
    now = datetime.now(timezone.utc)
    ttl = _coerce_ttl_seconds(token_data.get("expires_in", 0))
    inference_url = (
        _optional_base_url(token_data.get("inference_base_url"))
        or PROVIDER_REGISTRY["nastech"].inference_base_url.rstrip("/"))
    state = {
        ...
        "tls": _tls_state_from_verify(verify), **_NASTECH_EMPTY_AGENT_KEY_FIELDS}
    state = refresh_nastech_oauth_from_state(state, timeout_seconds=timeout_seconds, force_refresh=False)
...
def settle_after_upgrade(account_state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Picking the free-tier row may have written ``model.default: nastech/welcome`` and ``model.base_url``
    = welcome host. An account cannot keep either: the welcome host refuses account tokens, and the
    portal host serves ``nastech/welcome`` as a paid model. When the config is on the free tier's route,
    ``model.base_url`` becomes the account's inference host and ``model.default`` the recommended
    default for the account's tier (:func:`nastech_cli.models.recommended_nastech_default_model`, the
    same pick as ``GET /api/model/recommended-default``), through the same config write a plain Nastech
    login uses. A config on the user's own model and host is left alone.

    Every sign-in completion (CLI ``nastech auth upgrade``, the desktop poller) calls this once, after
    ``persist_nastech_credentials``. Returns ``{"model": str, "changed": bool}``: ``model`` is the default
    ...
    model = current
    if on_welcome_model:
        from nastech_cli.models import recommended_nastech_default_model
        try:
            model = str(recommended_nastech_default_model().get("model") or "")
        except Exception as exc:
    ...
    try:
        ...
        # same write; the runtime's silent default applies until the user picks one with `nastech model`.
        _update_config_for_provider(
            "nastech", str(account_state.get("inference_base_url") or ""),
...
def _poll_for_token(*args, **kwargs) -> Dict[str, Any]:
    ...
    from nastech_cli.auth_device_flow import _poll_for_token as poll
    return poll(*args, **kwargs)


def persist_nastech_credentials(*args, **kwargs):
    """Keep the existing auth_nastech persistence seam behind the sign-in entry point."""
    from nastech_cli.auth_nastech import persist_nastech_credentials as persist
...
from nastech_cli.anon_sign_in import (  # noqa: E402
...
from nastech_cli.anon_sign_in_cli import (  # noqa: E402
