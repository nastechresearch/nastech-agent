...
Split out of ``nastech_cli.models``. OpenRouter and
Nastech Portal share one implementation parametrized by :class:`_CapsSource`; the per-source module
globals (``_openrouter_reasoning_caps_cache``, ``_nastech_caps_disk_checked``, ...) stay defined on
``nastech_cli.models`` — tests reset them there — and are read/written by attribute name.
...
logger = logging.getLogger("nastech_cli.models")
...
# that must never block on HTTP — so without a disk copy, `nastech -p`, a cron job, or a freshly
...
def _read_reasoning_caps_disk() -> dict[str, Any]:
    from nastech_cli.models import _read_json_cache
...
def _save_reasoning_caps_disk(url: str, caps: Caps) -> None:
    ...
    from nastech_cli.models import _write_json_cache
    try:
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._NASTECH_USER_AGENT})
...
# ── Per-source cache (OpenRouter, Nastech Portal) ─────────────────────────

@dataclass(frozen=True)
class _CapsSource:
    """One catalog's cache slots on ``nastech_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from nastech_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from nastech_cli.models_profile_cache import profile_slot_set
...
def refresh_reasoning_caps_async(provider: Optional[str]) -> None:
    ...
    """
    src = {"nastech": _NASTECH_CAPS, "nastech-portal": _NASTECH_CAPS, "nastechresearch": _NASTECH_CAPS,
...
# Nastech Portal serves OpenRouter's catalog schema, so the same parser and contract apply. Its own
# cache because the two catalogs list different models (and different capabilities for shared ids).
_NASTECH_CAPS = _CapsSource(
    "_nastech_reasoning_caps_cache", "_nastech_reasoning_caps_failed_at",
    "_nastech_caps_disk_checked", "_nastech_caps_warm_started",
    lambda: nastech_catalog_url(), per_profile=True,
)


def nastech_catalog_url() -> str:
    """The Portal ``/v1/models`` URL for the endpoint we actually talk to (``NASTECH_INFERENCE_BASE_URL``
...
def nastech_model_reasoning_capabilities(
    ...
    """Nastech Portal counterpart of :func:`openrouter_model_reasoning_capabilities`; warm the cache
    with :func:`warm_nastech_reasoning_caps_async` from hot paths."""
    return _model_caps(_NASTECH_CAPS, model_id, timeout=timeout, allow_fetch=allow_fetch)
...
def warm_nastech_reasoning_caps_async() -> None:
    ...
    _warm_caps_async(_NASTECH_CAPS)
