...
class Advisory:
    ...
    ``{nastech_home}``; ``full_remediation_text`` fills it with ``display_nastech_home()`` at render
    time so a profile / NASTECH_HOME user is sent to their own .env.
    """
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in {nastech_home}/.env (OpenRouter, Anthropic, OpenAI, "
            "Nastech, GitHub, AWS, Google, Mistral, etc.).",
...
def full_remediation_text(hit: AdvisoryHit) -> list[str]:
    ...
    return [
        ...
        "",
        "Remediation:",
        *(f"  {i}. {step.format(nastech_home=display_nastech_home())}" for i, step in enumerate(a.remediation, 1)),
    ]


# ─── Startup-banner gating ────────────────────────────────────────────────────
# Once the banner is seen we cache that in ``~/.nastech/cache/advisory_banner_seen`` (one
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active (IDs: {', '.join(h.advisory.id for h in fresh)}). "
            "Run `nastech doctor` on the gateway host for details.")
