...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``nastech_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``nastech_state._backup_db_file``.
    """
...
def _snapshot_and_inspect(
    ...
    temp_dir = tempfile.TemporaryDirectory(
        prefix="nastech-session-recovery-",
        dir=str(work_root),
    )
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Nastech process using this profile and retry. "
                "This includes the interactive `nastech` CLI session this "
                ...
                "Nastech process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `nastech` session open, "
