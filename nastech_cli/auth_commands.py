...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "nastech", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth", "openrouter"}
# ...and default to it when ``--type`` is omitted. OpenRouter stays API-key-first: the documented
# ``nastech auth add openrouter --api-key sk-or-...`` must keep working with no ``--type``.
_OAUTH_DEFAULT_PROVIDERS = _OAUTH_CAPABLE_PROVIDERS - {"openrouter"}
# Providers whose sibling CLI login Nastech may borrow (``auth.adopt_external_logins``).
...
def _unknown_provider_exit(provider: str) -> SystemExit:
    ...
    return SystemExit(
        f"Unknown provider '{provider}'.{hint} Run `nastech auth` to see the provider list, or "
        "`nastech model` to pick one interactively.")
...
def _qwen_oauth_login(args) -> dict:
    from nastech_cli.auth_qwen import _mark_qwen_oauth_active
...
class _OAuthAddSpec:
    """Per-provider parameters for the generic ``nastech auth add <provider> --type oauth`` path."""
...
def _add_nastech_oauth_credential(args, provider: str) -> PooledCredential:
    """``nastech auth add nastech --type oauth``: shared-credential import, else device-code login."""
    custom_label = (getattr(args, "label", None) or "").strip() or None
    timeout = getattr(args, "timeout", None) or 15.0

    def _persist(creds: dict, what: str) -> PooledCredential:
        # `--label` is embedded into providers.nastech so label_from_token doesn't overwrite it on every
        # subsequent load_pool("nastech").
    ...
    # Codex-style auto-import: a shared Nastech credential at <nastech-root>/shared/nastech_auth.json
    # (written by any previous login) makes `nastech --profile <name> auth add nastech --type oauth`
    # a one-tap operation for multi-profile users.
    if auth_mod._read_shared_nastech_state():
        try:
            found = f"Found existing Nastech OAuth credentials at {auth_mod._nastech_shared_store_path()}"
        except RuntimeError:
            found = "Found existing shared Nastech OAuth credentials"
        print()
        print(found)
        ...
        if do_import is None or do_import.lower() in {"", "y", "yes"}:
            ...
            rehydrated = auth_mod._try_import_shared_nastech_state(timeout_seconds=timeout)
...
def _add_credential(args, provider: str, pool, requested_type: str) -> PooledCredential:
    ...
    if provider == "nastech":
        return _add_nastech_oauth_credential(args, provider)
...
def _report_priority(provider: str, pool, moved, requested: int, verb: str, prep: str) -> None:
    ...
    print(f'{verb} {provider} credential "{moved.label}" {prep} priority {moved.priority} '
          f"(#{moved.priority + 1} in `nastech auth list {provider}`)")
...
def auth_priority_command(args) -> None:
    """`nastech auth priority <provider> <target> <priority>`: reorder one pooled credential."""
    provider = _normalize_provider(getattr(args, "provider", ""))
    pool = load_pool(provider)
...
def auth_list_command(args) -> None:
    ...
    for provider in providers:
        ...
        if not entries:
            continue
        current = pool.peek()
        if provider == "nastech" and all(_is_free_tier_entry(e) for e in entries):
...
def auth_remove_command(args) -> None:
    ...
    # Every credential source Nastech reads from (env vars, external OAuth files, auth.json blocks,
...
def auth_refresh_command(args) -> None:
    """`nastech auth refresh <provider> [target]`: force one pooled OAuth entry to refresh.
    ...
    if target is None or not str(target).strip():
        if len(entries) != 1:
            raise SystemExit(
                ...
                f"label (see `nastech auth list {provider}`).")
    ...
    if (provider not in REFRESHABLE_OAUTH_PROVIDERS or matched.auth_type != AUTH_TYPE_OAUTH
        ...
        raise SystemExit(
            ...
            f"credential.")
    # Nastech's resolver is singleton-bound, not an independent-account refresher.
    if provider == "nastech" and matched.source != "device_code":
        raise SystemExit(
            f"nastech credential #{index} ({matched.label}) is not a refreshable OAuth "
            "credential: only the device_code singleton supports refresh. "
            "Reauthenticate with `nastech auth add nastech --type oauth`.")
    ...
    if refreshed is None:
        ...
        raise SystemExit(
            ...
            f"Sign in again with `nastech auth add {provider} --type oauth`.")
...
def auth_upgrade_command(args) -> None:
    """``nastech auth upgrade``: sign the free tier into a Nastech account, keeping its connectors."""
    from nastech_cli.anon_auth import upgrade_guest
