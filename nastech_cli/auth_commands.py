...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "nastech", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "nastech":
        # Codex-style auto-import: if a shared Nastech credential lives at
        ...
        # auth add nastech --type oauth` a one-tap operation for users who
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing Nastech OAuth credentials at {path}")
            else:
        ...
        # Honor `--label <name>` so nastech matches other providers' UX.  The
        # helper embeds this into providers.nastech so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("nastech").
        custom_label = (getattr(args, "label", None) or "").strip() or None
