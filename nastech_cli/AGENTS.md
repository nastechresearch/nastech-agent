# nastech_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `NastechCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`nastech_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`NastechCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.nastech/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`nastech_cli/curses_ui.py`; example
`nastech_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
`nastech_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`nastech_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `nastech config
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`nastech tools/setup`, most subcommands, `nastech_cli/config.py`, merges `DEFAULT_CONFIG`);
  `nastech_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `nastech send`, doctor,
  `nastech_time`/`nastech_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' nastech_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`nastech_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.nastech/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`nastech update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `nastech update --plan`): read-only inventory — install kind, all
...
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `nastech-gateway*` unit /
  `ai.nastech.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.nastech/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `nastech update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `nastech update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `nastech_cli/main.py` sets `NASTECH_HOME` before any module import for
single-profile commands (`nastech -p x <cmd>`), so there `get_nastech_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["NASTECH_HOME"]` keeps the launch
...
carries an identity marker (`nastech_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_nastech_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `nastech_constants.nastech_home_key()`, never a single flag.
...
Migration from per-profile gateways: `nastech_cli/gateway_migrate.py` (`nastech gateway migrate
...
other service domain / UNIX user / NASTECH_HOME outside `profiles/` — notices for the explicit command,
...
`nastech gateway restart` for a gateway Nastech did not install (custom launchd agent / unit running
...
profile's `NASTECH_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nastech free tier (`nastech_cli/anon_auth.py`)
...
(`models.recommended_nastech_default_model`, shared with `GET /api/model/recommended-default`).
...
which never contains a raw exception, a URL or a `nastech` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`nastech auth upgrade`), the CLI `/login` handler and the desktop
promotion poller are renderers over it; a surface that needs the cancel check and the save to be
atomic passes `persist_guard`. The desktop's plain "connect another Nastech account" device-code login
is a separate path (`_nastech_plain_poller`) and must stay one.
