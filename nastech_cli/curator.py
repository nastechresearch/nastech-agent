...
def _print_unmanaged_summary() -> None:
    ...
    print("  never auto-staled or archived — `nastech curator adopt <name>` hands one over")
...
_PIN_MESSAGES = {
    True: (
        ...
        "external). `nastech curator list-unmanaged` shows which skills the curator tracks.",
        ...
        "it. Run `nastech curator adopt {skill}` to put it under curator management)",
...
def _cmd_list_unmanaged(args) -> int:
    ...
    print("\nadopt one with `nastech curator adopt <name>`, "
          "or all with `nastech curator adopt --all-unmanaged`")
    return 0
...
def _cmd_ledger(args) -> int:
    ...
    for r in rows:
        ...
        print(
            ...
            f"{r.get('skill', '?')}{extra}")
    print(
        "\nRoll back a single mutation with `nastech curator rollback <id>`; "
        "whole-tree snapshots remain available via `nastech curator rollback --list`.")
    return 0
...
def _rollback_ledger_entry(args, entry_id: str) -> int:
    ...
    if entry is None:
        print(
            ...
            "See `nastech curator ledger` for entry ids, or use "
...
_SUBCOMMANDS = (
    ...
    (
        "backup",
        "Take a manual tar.gz snapshot of ~/.nastech/skills/ "
    ...
    (
        "rollback",
        "Restore ~/.nastech/skills/ from a curator snapshot, or a single "
        "mutation by ledger entry id (see `nastech curator ledger`)",
        _cmd_rollback,
        _arg("entry_id", nargs="?", default=None,
             help="Ledger entry id for single-mutation rollback (from "
                  "`nastech curator ledger`). Omit for whole-tree snapshot rollback."),
