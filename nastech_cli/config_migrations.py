...
``nastech_cli.config.migrate_config``. Each step is a function
...
``nastech_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``nastech_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``nastech_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``nastech_cli.config`` (e.g.
``patch("nastech_cli.config.read_raw_config", ...)``) keep working, because
...
#: NASTECH_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def _cfg():
    """Return the live ``nastech_cli.config`` module (lazy, cycle-free)."""
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_nastech_home = _c.get_nastech_home
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `nastech config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `nastech config set`"
                )
...
# behavior set it to true explicitly via `nastech config set`.
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # period that has since ended server-side, leaving every Nastech-signed-in
    ...
    # provider surface (`video_generate`, `nastech tools` → Video Generation).
