...
in :func:`run_migrations`. Every write goes through ``nastech_cli.config._persist_migration`` so a
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_nastech_home()}/config.yaml and run `nastech setup` to "
        f"regenerate, or manually set _config_version: {SUPPORT_FLOOR_VERSION} "
        "after reviewing the changelog.")


def _cfg():
    """Return the live ``nastech_cli.config`` module (lazy, cycle-free, monkeypatch-friendly)."""
...
def _installed_user_plugins(disabled: set) -> List[str]:
    """Names of plugins under ``$NASTECH_HOME/plugins/`` with a manifest, minus *disabled*."""
    _c = _cfg()
    found: List[str] = []
    try:
        user_plugins_dir = _c.get_nastech_home() / "plugins"
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # section; deep-merge made it work but users could not see/edit it and `nastech curator status`
    # had no stable logs dir. Only keys the user hasn't set are written.
    _c = _cfg()
    DEFAULT_CONFIG = _c.DEFAULT_CONFIG

    try:
        curator_dir = _c.get_nastech_home() / "logs" / "curator"
        ...
        for label, added in (("curator", added_curator), ("auxiliary.curator", added_aux)):
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added)}) — edit via `nastech config set`")
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from nastech_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if changed:
        _commit(
            ...
            "lists — video generation now lives under `nastech tools` → "
            "Video Generation (Nastech Subscription or FAL).")
...
def _migrate_to_41(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from tools.bot_mode_probe import _PROTOCOL_HEADING, _nastech_root, _roster, strip_legacy_protocol

    cleaned: List[str] = []
    for name, profile_dir in _roster(_nastech_root(get_nastech_home())):
...
def _migrate_to_45(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from nastech_cli.tools_config import _configurable_keys, _get_plugin_toolset_keys
    ...
    for platform, toolsets in saved.items():
        ...
        if not toolset_allowed_for_platform("connections", platform):
            continue
        # A composite like [nastech-cli] already inherits every core tool at read time.
    ...
    _commit(
        ...
        "Uncheck Connections in `nastech tools` to turn it off.")
...
MIGRATIONS: Tuple[Tuple[int, Callable[[Dict[str, Any], bool], None]], ...] = (
    ...
    (44, _rewrite_stale_default(
        ...
        message=(
            ...
            "skills/.archive/ (recoverable with `nastech curator restore`). Set it back to 90 to keep the old window."))),
