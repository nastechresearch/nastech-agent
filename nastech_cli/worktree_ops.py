"""Git worktree isolation for ``nastech -w`` sessions: create, classify, prune.
...
from nastech_cli._subprocess_compat import kill_process_tree
...
# One repack attempt per clone per interval, box-wide. Every ``nastech -w`` launch on a shared clone
...
_REPACK_LOCK = "nastech-repack.lock"
...
def _maintain_pack_health(repo_root: str) -> None:
    ...
    try:
        ...
        if not _claim_repack_slot(pack_dir.parent.parent):
            return
        from nastech_cli.gitlock import clear_stale_tmp_packs
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True,
    ...
    *sync_base* branches from the fetched remote tip (``_resolve_worktree_base``), else local
    HEAD. *name* replaces the random ``nastech-<id>``; named trees lack the ``nastech-`` prefix so
    ...
    try:
        _git(["worktree", "lock", "--reason", f"nastech pid={os.getpid()}", str(wt_path)], repo_root)
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    """Lock state: ``"live"`` (owning pid runs), ``"dead"`` (pid gone / non-nastech reason), None (unlocked).

    ``nastech -w`` locks with reason ``nastech pid=<pid>``; ``worktree remove --force`` refuses
...
def _prune_candidates(worktrees_dir: Path, max_age_hours: int, now: float) -> list:
    ...
    Kanban trees (``t_<hex>``) belong to the kanban gc. ``nastech-*`` trees age on *max_age_hours*,
    ...
    for entry in sorted(worktrees_dir.iterdir()):
        if not entry.is_dir() or kanban_re.match(entry.name):
            continue
        tier_hours = max_age_hours if entry.name.startswith("nastech-") else max_age_hours * 3
        try:
...
def _classify_prune_candidates(repo_root: str, candidates: list) -> list:
    ...
    def _classify(item):
        ...
        # Live lock = running nastech; a dead lock is unlocked in phase 3.
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    try:
        ...
        if count >= 10 or (size_mb or 0) >= 5120:
            ...
            logger.warning(".worktrees/ holds %d tree(s) (%s) — run `nastech worktree list` "
                           "to audit and `nastech worktree prune` to reclaim safely.", count, size_txt)
    except Exception:
        pass


def _prune_orphaned_branches(repo_root: str, protect: Optional[set] = None) -> None:
    """Delete local ``nastech/nastech-*`` and ``pr-*`` branches with no worktree, except *protect*."""
    try:
