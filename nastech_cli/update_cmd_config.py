"""Post-``nastech update`` config-schema migration for the active profile and every sibling.
Names are re-imported by ``update_cmd`` (``nastech_cli.update_cmd.<name>`` resolves/monkeypatches);
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    sibling (active skipped): scope via the context-local NASTECH_HOME override (never ``os.environ``)
    ...
    profile, but ``nastech update`` historically migrated only the active profile's config — siblings drifted
    versions until their gateway hit a config the new code couldn't read.
    """
    from nastech_cli.update_cmd import _run_config_check_fresh, _run_migrate_config_fresh
    migrated: list[tuple[str, int, int]] = []
    with _best_effort('Sibling profile enumeration failed: %s'):
        from nastech_constants import (
            get_process_nastech_home, reset_nastech_home_override, set_nastech_home_override)
...
def _ask_configure_new_options(*, assume_yes: bool, gateway_mode: bool) -> str:
    ...
    from nastech_cli.update_cmd import _gateway_prompt
    if assume_yes:
...
def _check_and_apply_config_migration(
    *, assume_yes: bool = False, gateway_mode: bool = False, pre_update_snapshot_id: str | None = None
    ...
    try:
        from nastech_cli.config import get_missing_env_vars, get_missing_config_fields
    ...
    # migration per sibling home via the context-local NASTECH_HOME override (never os.environ).
