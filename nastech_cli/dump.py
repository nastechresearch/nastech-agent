...
def _dotenv_key_names() -> set[str]:
    """Env-var names assigned a non-empty value in ~/.nastech/.env — what the managed backends (launchd /
    systemd / desktop ``serve``) load, as opposed to the shell exports ``os.getenv`` reflects here.

    ``nastech debug share`` runs in a terminal, so ``os.getenv`` reflects the shell's environment, which can
...
def _get_git_commit(project_root: Path) -> str:
    ...
    the Dockerfile bakes into ``<project_root>/.nastech_build_sha``."""
    value = _git_output(project_root, "rev-parse", "--short=8", "HEAD")
    if value:
        return value
    try:
        from nastech_cli.build_info import get_build_sha  # deferred: keeps dump cheap on non-dump paths
...
_API_KEYS = [
    ...
    ("NASTECH_API_KEY", "nastech"), ("GOOGLE_API_KEY", "google/gemini"), ("GEMINI_API_KEY", "gemini"),
...
def _api_key_lines(show_keys: bool) -> list[str]:
    ...
    for env_var, label in _API_KEYS:
        ...
        # Set in this shell but absent from ~/.nastech/.env: a managed backend loads .env, not the login
        ...
        # `nastech auth add openrouter` credentials live in the pool, not env — don't read "not set".
        # A credential added via `nastech auth add openrouter` lives in the credential pool, not as an env
        # var — surface it so the dump doesn't misleadingly read "not set" while `nastech auth list` shows it
        # (#42130).
...
def run_dump(args):
    ...
    load_nastech_dotenv(nastech_home=get_env_path().parent, project_env=get_project_root() / ".env")
    project_root, nastech_home = get_project_root(), get_nastech_home()
    try:
        config = load_config()
    except Exception:
        config = {}
    ...
    lines = [
        ...
        f"profile:          {profile}",
        f"nastech_home:      {display_nastech_home()}",
        f"model:            {model}",
        f"provider:         {provider}",
        ...
        f"  cron_jobs:          {_cron_summary(nastech_home)}",
        f"  skills:             {_count_skills(nastech_home)}",
    ]
