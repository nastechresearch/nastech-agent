...
def _anthropic_oauth_status() -> Dict[str, Any]:
    """Status for the "Anthropic API Key" card: Nastech-managed PKCE file first, then the
    ...
    """
    try:
        from agent.anthropic_credentials import read_nastech_oauth_credentials, _get_nastech_oauth_file
        nastech_creds = read_nastech_oauth_credentials()
    except Exception:
        nastech_creds = None
    if nastech_creds and nastech_creds.get("accessToken"):
        return _token_status("nastech_pkce", f"Nastech PKCE ({_get_nastech_oauth_file()})", nastech_creds)
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    store); the CLI may hold its session in an OS keychain Nastech can't read, so the unverified
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    (``NASTECH_COPILOT_ACP_COMMAND`` / ``COPILOT_CLI_PATH``); others get ``default`` untouched."""
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    {"id": "nastech", "name": "Nastech Portal", "flow": "device_code", "cli_command": "nastech auth add nastech",
     "docs_url": "https://portal.nastechresearch.github.io", "status_fn": None},
    {"id": "openai-codex", "name": "ChatGPT or Codex Subscription", "flow": "device_code",
     "cli_command": "nastech auth add openai-codex", "docs_url": "https://platform.openai.com/docs",
     "status_fn": None},
    {"id": "qwen-oauth", "name": "Qwen (via Qwen CLI)", "flow": "external",
     "cli_command": "nastech auth add qwen-oauth", "docs_url": "https://github.com/QwenLM/qwen-code",
     "status_fn": None},
    ...
    {"id": "minimax-oauth", "name": "MiniMax (OAuth)", "flow": "device_code",
     "cli_command": "nastech auth add minimax-oauth", "docs_url": "https://www.minimax.io", "status_fn": None},
    ...
    {"id": "xai-oauth", "name": "xAI Grok OAuth (SuperGrok / Premium+)", "flow": "device_code",
     ...
     "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/guides/xai-grok-oauth", "status_fn": None},
    ...
    # Login works via the terminal (`nastech auth add anthropic`) or a plain API key.
    {"id": "anthropic", "name": "Anthropic API Key", "flow": "external", "cli_command": "nastech auth add anthropic",
...
@_oauth_poller("nastech")
def _nastech_promotion_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """Drain the sign-in the start route began: one shared flow, rendered onto the session.

    The generator was created and advanced to its ``Code`` state by ``_start_nastech_device_code``, so
...
@_oauth_poller("nastech")
def _nastech_plain_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """Background poller for a plain Nastech device-code login (no free-tier identity to transfer).

    A sign-in that carries the free tier's connectors runs through ``anon_auth.run_sign_in`` and
    ``_nastech_promotion_poller`` instead; this is the "connect another Nastech account" path.
    """
    from nastech_cli.web_server_profiles import _profile_scope
    from nastech_cli.auth import _poll_for_token, persist_nastech_credentials, refresh_nastech_oauth_from_state
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        full_state = refresh_nastech_oauth_from_state(auth_state, timeout_seconds=15.0, force_refresh=False)
...
def _minimax_poller(session_id: str, sess: Dict[str, Any]) -> None:
    """MiniMax poller: PKCE-style ``code_verifier`` + ``user_code`` instead of Nastech's
    ``device_code``. Builds the same auth_state as the CLI's ``_minimax_oauth_login`` and persists
    via ``_minimax_save_auth_state`` so the system ends up as after ``nastech auth add minimax-oauth``.
...
def _xai_device_poller(session_id: str, sess: Dict[str, Any]) -> None:
    ...
    with _profile_scope(_oauth_session_profile(session_id)):
        ...
        # Mirror `nastech auth add xai-oauth`: first credential may become active; never overwrite.
