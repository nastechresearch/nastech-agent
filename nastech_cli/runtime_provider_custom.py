...
:mod:`nastech_cli.runtime_provider`; origin-internal collaborators
...
logger = logging.getLogger("nastech_cli.runtime_provider")

_LLAMACPP_ALIASES = ("llamacpp", "llama.cpp", "llama-cpp")


def _rp():
    """Origin module, late-bound so test patches on ``nastech_cli.runtime_provider.*`` apply."""
    import nastech_cli.runtime_provider as origin
    return origin
...
def _shadowed_by_builtin(requested_norm: str) -> bool:
    ...
    canonical provider (``nastech``); an entry matching merely an alias (``kimi`` → ``kimi-coding``)
...
def _resolve_named_custom_runtime(*, requested_provider: str, explicit_api_key: Optional[str] = None,
    ...
    if family is not None and not custom_provider.get("api_mode"):
        from nastech_cli.models import normalize_opencode_base_url, opencode_model_api_mode
