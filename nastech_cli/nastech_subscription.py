...
# Maps a tools_config provider's ``managed_nastech_feature`` to the tool-pool
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    an in-memory section dict: ``"nastech"`` for the managed selection (stored
    ``nastech`` value or legacy ``use_gateway: true``), a vendor name for BYOK
    picks, or ``None`` when no selection is stored. Keeping this in lockstep
    with the runtime resolver is what stops ``nastech status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
class NastechSubscriptionFeatures:
    ...
    @property
    def web(self) -> NastechFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> NastechFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> NastechFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> NastechFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> NastechFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> NastechFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> NastechFeatureState:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_nastech_managed_defaults`` from flipping a working local setup
...
def get_nastech_subscription_features(
    ...
    *,
    ...
    provider_is_nastech = str(model_cfg.get("provider") or "").strip().lower() == "nastech"

    try:
        if force_fresh:
            account_info = get_nastech_portal_account_info(force_fresh=True)
        else:
            account_info = get_nastech_portal_account_info()
    except Exception:
        account_info = None
    ...
    subscribed = provider_is_nastech or nastech_auth_present
    ...
    # requires `pip install faster-whisper`. For Nastech subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_nastech_managed_defaults below.
    ...
    # "nastech" (stored value or legacy use_gateway: true) = managed gateway;
    ...
    web_use_gateway = web_selected == "nastech"
    tts_use_gateway = tts_selected == "nastech"
    stt_use_gateway = stt_selected == "nastech"
    browser_use_gateway = browser_selected == "nastech"
    image_use_gateway = image_selected == "nastech"
    video_use_gateway = video_selected == "nastech"

    # The "nastech" selection is serviced by a concrete vendor implementation —
    # normalize the current-provider labels so downstream vendor checks hold.
    if web_backend == "nastech" or web_use_gateway:
        web_backend = "firecrawl"
    if tts_provider == "nastech" or tts_use_gateway:
        tts_provider = "openai"
    if stt_provider == "nastech" or stt_use_gateway:
        stt_provider = "openai"
    if browser_provider == "nastech" or browser_use_gateway:
    ...
    # Keyless Tavily is opt-in: selecting it in `nastech tools` / setup writes
    ...
    features = {
        "web": NastechFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_nastech=web_managed,
        ...
        "image_gen": NastechFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_nastech=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if (image_selected not in (None, "nastech") or (image_selected is None and direct_fal)) else ("Nastech Subscription" if (image_managed or image_use_gateway) else ""),
            explicit_configured=image_selected is not None or direct_fal,
        ),
        "video_gen": NastechFeatureState(
            key="video_gen",
            label="Video generation",
            included_by_default=False,
            available=video_available,
            active=video_active,
            managed_by_nastech=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if (video_selected not in (None, "nastech") or (video_selected is None and direct_fal_video)) else ("Nastech Subscription" if (video_managed or video_use_gateway) else ""),
            explicit_configured=video_selected is not None or direct_fal_video,
        ),
        "tts": NastechFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_nastech=tts_managed,
        ...
        "stt": NastechFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_nastech=stt_managed,
        ...
        "browser": NastechFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_nastech=browser_managed,
        ...
        "modal": NastechFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_nastech=modal_managed,
    ...
    return NastechSubscriptionFeatures(
        ...
        nastech_auth_present=nastech_auth_present,
        provider_is_nastech=provider_is_nastech,
        features=features,
...
def apply_nastech_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_nastech:
        return set()
    ...
    if "tts" in selected_toolsets and not features.tts.explicit_configured and not (
        ...
        tts_cfg["provider"] = "nastech"
        tts_cfg.pop("use_gateway", None)
        changed.add("tts")

    # STT: same pattern as TTS. The DEFAULT_CONFIG seed is "local"
    # (requires `pip install faster-whisper`); for Nastech subscribers we
    ...
    if "browser" in selected_toolsets and not features.browser.explicit_configured and not (
        ...
        browser_cfg["cloud_provider"] = "nastech"
...
# ``get_nastech_subscription_features`` reads (web uses "backend", browser
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    - unconfigured: tools with no direct credentials and no explicit
      non-nastech selection (easy switch, safe to pre-check)
    - has_direct: tools where the user has their own API keys
    - explicit_configured: tools with an explicit non-nastech selection stored
    ...
    All lists are empty when the user is not a paid Nastech subscriber or
    is not using Nastech as their provider.
    """
    ...
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "nastech":
        return [], [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_nastech which fires implicitly when
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        if opted_in.get(key):
            ...
            # An explicit non-nastech selection (e.g. a keyless local backend
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    # already_managed, just for a non-nastech vendor.
    ...
    try:
        account_info = get_nastech_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    # pre-checked checklist re-fired on every Nastech model swap.
    ...
    # so the next Nastech model swap doesn't pre-check it again. Cancel paths
...
def ensure_nastech_portal_access(
    *,
    ...
    backend (e.g. "Firecrawl (Nastech Portal)").  Unlike ``nastech model``'s Nastech
    login, this:
...
def _run_nastech_portal_login_only(*, capability: str) -> bool:
    ...
    try:
        ...
        # switches the user's inference provider to Nastech.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing Nastech OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_nastech_state(timeout_seconds=15.0)
        ...
        # _nastech_device_code_login raises SystemExit on subscription_required;
