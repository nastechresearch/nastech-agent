...
from nastech_cli.nastech_account import (
    NastechPortalAccountInfo, format_nastech_portal_entitlement_message, get_nastech_portal_account_info,
)
...
_DEFAULT_PLATFORM_TOOLSETS = {"cli": "nastech-cli"}
...
class _FeatureSpec:
    ...
    # Tool-pool coverage category (nastech_account.TOOL_COVERAGE_CATEGORIES) gating per backend:
    ...
    offer_label: str = ""
    direct_label: str = ""
    # Direct-credential env vars that stop apply_nastech_managed_defaults switching the category to
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    """Stored provider for a section (``read_selection`` semantics): ``"nastech"`` for the managed
    selection (stored ``nastech`` or legacy ``use_gateway: true``), a vendor name for BYOK, else None."""
...
@dataclass(frozen=True)
class NastechFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_nastech: bool
...
class NastechSubscriptionFeatures:
    subscribed: bool
    nastech_auth_present: bool
    provider_is_nastech: bool
    features: Dict[str, NastechFeatureState]
    account_info: Optional[NastechPortalAccountInfo] = None

    def __getattr__(self, name: str) -> NastechFeatureState:  # ``features.web`` -> per-key state
        if name in _FEATURE_ORDER:
            return self.features[name]
        raise AttributeError(name)

    def items(self) -> Iterable[NastechFeatureState]:
...
def _select_nastech(config: Dict[str, object], key: str) -> None:
    """Store the managed ``nastech`` selection in the ``key`` section (field per _GATEWAY_SECTION_FIELDS)."""
    section_key, field = _GATEWAY_SECTION_FIELDS[key]
    section = _ensure_section(config, section_key)
    section[field] = "nastech"
...
def _provider_is_nastech(config: Dict[str, object]) -> bool:
    return _norm(_section(config, "model").get("provider")) == "nastech"
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Nastech-managed Node dirs ($NASTECH_HOME/node, prepended to PATH at runtime but usually absent
        ...
        search_paths = [None, with_nastech_node_path().get("PATH", ""), str(local_bin_dir) if local_bin_dir.is_dir() else ""]
        return any(
...
def _account_info_or_none(**kwargs) -> Optional[NastechPortalAccountInfo]:
    """``get_nastech_portal_account_info(**kwargs)``, failing closed to ``None`` on any error."""
    try:
        return get_nastech_portal_account_info(**kwargs)
    except Exception:
        return None


def _state(key: str, **fields) -> NastechFeatureState:
    spec = _FEATURES[key]
    fields.setdefault("direct_override", fields["active"] and not fields["managed_by_nastech"])
    return NastechFeatureState(key, spec.label, spec.included_by_default, **fields)


def _web_feature(web_cfg: Dict[str, object], tool_enabled: bool, managed: bool, web_gw: bool, direct_firecrawl: bool) -> NastechFeatureState:
    ...
    # The "nastech" selection is serviced by Firecrawl — normalize so downstream vendor checks hold.
    if backend == "nastech" or web_gw:
        backend = "firecrawl"
    ...
    return _state(
        ...
        managed_by_nastech=web_managed, toolset_enabled=tool_enabled,
...
def _fal_feature(key: str, tool_enabled: bool, direct: bool, managed: bool, selected: Optional[str]) -> NastechFeatureState:
    ...
    if selected not in (None, "nastech") or (selected is None and direct):
        label = "FAL"
    else:
        label = "Nastech Subscription" if (fal_managed or selected == "nastech") else ""
    return _state(
        key, available=bool(managed or direct), active=bool(tool_enabled and (fal_managed or direct)),
        managed_by_nastech=fal_managed, toolset_enabled=tool_enabled, current_provider=label,
...
def _audio_provider(cfg: Dict[str, object], default: str, gw: bool) -> str:
    ...
    return "openai" if (provider == "nastech" or gw) else (provider or default)
...
def _audio_features(
...
) -> tuple[NastechFeatureState, NastechFeatureState]:
    tts_gw, stt_gw = use_gateway["tts"], use_gateway["stt"]
    # STT default is "local" (faster-whisper, needs a pip install); Nastech subscribers are routed to the
    # managed audio gateway by apply_nastech_managed_defaults. The "nastech" selection is serviced by OpenAI
    ...
    tts = _state(
        ...
        managed_by_nastech=tts_tool_enabled and tts_current == "openai" and managed["tts"] and not direct_openai_tts,
    ...
    stt = _state(
        ...
        managed_by_nastech=stt_current == "openai" and managed["stt"] and not direct_openai_stt,
...
def _browser_feature(
    ...
    if provider == "nastech" or browser_gw:
    ...
    return _state(
        "browser", available=available, active=bool(tool_enabled and available), managed_by_nastech=managed_now,
        toolset_enabled=tool_enabled, current_provider=_provider_label("browser", current), explicit_configured=explicit,
    )


def _modal_feature(terminal_cfg: Dict[str, object], tool_enabled: bool, managed: bool, managed_tools_flag: bool) -> NastechFeatureState:
    ...
    return _state(
        "modal", available=available, active=active, managed_by_nastech=managed_now, direct_override=direct_override,
...
def get_nastech_subscription_features(config: Optional[Dict[str, object]] = None, *, force_fresh: bool = False) -> NastechSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    provider_is_nastech = _provider_is_nastech(config)
    ...
    nastech_auth_present = bool(account_info and account_info.logged_in)
    managed_tools_flag = nastech_auth_present and account_info.tool_gateway_entitled
    enabled = {key: _toolset_enabled(config, key) for key in ("web", "image_gen", "video_gen", "tts", "browser", "terminal")}
    # Stored selections (strict model): "nastech" = managed gateway; vendor name = that vendor direct;
    ...
    use_gateway = {key: value == "nastech" for key, value in selected.items()}
    ...
    def _fal(key: str) -> NastechFeatureState:
    ...
    return NastechSubscriptionFeatures(
        subscribed=provider_is_nastech or nastech_auth_present, nastech_auth_present=nastech_auth_present,
        provider_is_nastech=provider_is_nastech, features=features, account_info=account_info,
    )
...
def apply_nastech_managed_defaults(config: Dict[str, object], *, enabled_toolsets: Optional[Iterable[str]] = None, force_fresh: bool = False) -> set[str]:
    features = get_nastech_subscription_features(config, force_fresh=force_fresh)
    account_info = features.account_info
    if not (account_info and account_info.logged_in and account_info.tool_gateway_entitled and features.provider_is_nastech):
        return set()
...
def get_gateway_eligible_tools(config: Optional[Dict[str, object]] = None, *, force_fresh: bool = False) -> tuple[list[str], list[str], list[str], list[str]]:
    ...
    and no explicit non-nastech selection (safe to pre-check) / own API keys / explicit non-nastech selection
    ...
    if config is None:
        config = load_config() or {}
    if not _provider_is_nastech(config):
        return [], [], [], []
    ...
    for key in _ALL_GATEWAY_KEYS:
        ...
        elif selected is not None and selected != "nastech":
...
def prompt_enable_tool_gateway(config: Dict[str, object], *, force_fresh: bool = True) -> set[str]:
    ...
    # every Nastech model swap. See #92647.
    ...
    title = (
        "Your free Nastech tool pool — pick the tools to enable:" if pool_only
        else "Your Nastech subscription includes the Tool Gateway — pick the tools to enable:"
    )
    try:
...
def ensure_nastech_portal_access(*, capability: str = "the Nastech Tool Gateway", coverage_category: Optional[str] = None) -> bool:
    """Make sure the user is entitled to the Nastech Tool Gateway, logging in if needed.
    ...
    if not _entitled(info) and (info is None or not info.logged_in):
        if not _run_nastech_portal_login_only(capability=capability):
            return False
        info = _account_info_or_none(force_fresh=True)
    if _entitled(info):
        return True
    # Logged in but not entitled for this capability — neutral billing guidance, do not enable.
    message = format_nastech_portal_entitlement_message(info, capability=capability, coverage_category=coverage_category)
...
def _run_nastech_portal_login_only(*, capability: str) -> bool:
    """Run the Nastech Portal device-code OAuth and persist credentials only (no model selection, no
    ...
    proceed = _confirm("  Log in to Nastech Portal now? [Y/n]: ")
    if proceed is None:
        print()
        return False
    if not proceed:
        ...
        return False
    try:
        # Snapshot active_provider so a tool-config login never silently switches inference to Nastech.
        ...
        if auth._read_shared_nastech_state() and _confirm("  Found existing Nastech OAuth credentials. Import them? [Y/n]: ") is not False:
            auth_state = auth._try_import_shared_nastech_state(timeout_seconds=15.0)
        if auth_state is None:
            auth_state = auth._nastech_device_code_login()
        with auth._auth_store_lock():
            auth_store = auth._load_auth_store()
            auth._save_provider_state(auth_store, "nastech", auth_state)
            if prior_active_provider:
                auth_store["active_provider"] = prior_active_provider
            else:
                auth_store.pop("active_provider", None)
            auth._save_auth_store(auth_store)
        auth._write_shared_nastech_state(auth_state)
        auth._sync_nastech_pool_from_auth_store()
        ...
        # _nastech_device_code_login raises SystemExit on subscription_required (guidance already printed).
        return False
...
_PLUGIN_COMPAT_LAZY = {
    'managed_nastech_tools_enabled': ('tools.tool_backend_helpers', 'managed_nastech_tools_enabled'),
}
