...
"not connected to any AI provider" setup prompt instead of the owner's credentials. ``nastech update``
...
def _profile_has_own_credentials(profile_dir: Path) -> bool:
    ...
    from nastech_cli.main import _dotenv_has_provider_key
...
def profiles_without_own_credentials() -> List[str]:
    ...
    from nastech_cli.profiles import _iter_named_profile_dirs
    if not _auth_store_has_credentials(get_default_nastech_root() / "auth.json"):
        return []
...
def print_profiles_without_credentials_notice() -> None:
    ...
    for name in names:
        print(f"     nastech -p {name} model        # or: nastech -p {name} auth add <provider>")
