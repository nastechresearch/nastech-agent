...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not AIOHTTP_AVAILABLE:
        _err("nastech proxy requires aiohttp. Run `nastech setup` to install it.")
        return 1
    provider = getattr(args, "provider", None) or "nastech"
    try:
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``nastech proxy <subcommand>``; no/unknown subcommand prints the short help."""
    ...
    _err(
        ...
        "\n"
        "Subcommands:\n"
        "  nastech proxy start [--provider nastech|xai] [--host 127.0.0.1] [--port 8645]\n"
