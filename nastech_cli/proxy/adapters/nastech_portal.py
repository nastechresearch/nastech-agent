"""Nastech Portal upstream adapter."""
...
# Endpoints inference-api.nastechresearch.github.io actually serves; anything else is a 404 so stray
...
class NastechPortalAdapter(UpstreamAdapter):
    """Proxy upstream for the Nastech Portal inference API."""

    def __init__(self) -> None:
        # In-process serialization; cross-process refresh/persistence is resolve_nastech_runtime_credentials().
    ...
    def get_retry_credential(
        ...
        if status_code != 401:
            return None
        logger.info("proxy: Nastech upstream rejected bearer; force-refreshing invoke JWT")
    ...
    def _get_credential(
        ...
        with self._lock:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to refresh Nastech Portal credentials: {exc}") from exc
            runtime_key = refreshed.get("api_key")
            if not runtime_key:
                raise RuntimeError(
                    "Nastech Portal refresh did not return a usable inference JWT. "
                    "Try `nastech auth add nastech` to re-authenticate."
                )
            # The returned base_url already honors the NASTECH_INFERENCE_BASE_URL override (documented
            ...
            base_url = (
                _nastech_inference_env_override()
                or _validate_nastech_inference_url_from_network(refreshed.get("base_url"))
    ...
    # auth.json access — kept local so nastech_cli.auth's public surface does not grow.
    ...
    def _read_state(self) -> Optional[Dict[str, Any]]:
        try:
            ...
            return None
        state = (store.get("providers") or {}).get("nastech")
        ...
        try:
            with _auth_store_lock():
                ...
                if quarantine_error is not None and quarantine_reason:
                    _quarantine_nastech_pool_entries(store, quarantine_error, reason=quarantine_reason)
                providers = store.setdefault("providers", {})
                providers["nastech"] = state
                _save_auth_store(store)
            _write_shared_nastech_state(state)
        except Exception as exc:
            logger.warning("proxy: failed to persist Nastech quarantine state: %s", exc)


__all__ = ["NastechPortalAdapter"]
