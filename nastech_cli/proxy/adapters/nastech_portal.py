...
Reads the user's Nastech OAuth state from ``~/.nastech/auth.json`` through the
...
# Endpoints inference-api.nastechresearch.github.io actually serves. Anything else
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nastech Portal. Run `nastech auth add nastech` first."
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f"Failed to refresh Nastech Portal credentials: {exc}"
                ) from exc
            ...
            # base_url returned by resolve_nastech_runtime_credentials() already
            # honors the NASTECH_INFERENCE_BASE_URL env override (the documented
