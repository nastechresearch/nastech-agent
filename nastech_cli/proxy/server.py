...
:mod:`nastech_cli.proxy.sse_done`, issue #90848).
...
from nastech_cli.proxy.sse_done import (
    DONE_SSE_FRAME,
    SseDoneTracker,
    content_type_is_sse,
)
...
def create_app(adapter: UpstreamAdapter) -> "web.Application":
    ...
    if not AIOHTTP_AVAILABLE:
        raise RuntimeError(
            "aiohttp is required for `nastech proxy`. Run `nastech setup` to install it."
        )
    ...
    async def handle_health(request: "web.Request") -> "web.Response":
        ...
        # but both shipped adapters read auth state off disk, and the Nastech one
        ...
        # a concurrent ``nastech auth`` command.
    ...
    async def handle_proxy(request: "web.Request") -> "web.StreamResponse":
        ...
        # the Nastech adapter takes ``_auth_store_lock()`` (a cross-process lock
        ...
        if upstream_resp.status in {401, 429}:
            ...
            # most expensive: the Nastech adapter routes this straight into
