...
_ARG_SYNONYMS = {
    ...
    "off": "auto", "default": "auto", "disable": "auto", "nastech": "auto"}

_NASTECH_TOOLS_CALLBACK_NOTE = (
...
def _migration_lines(config: dict) -> list[str]:
    ...
    lines: list[str] = []
    try:
        from nastech_cli.codex_runtime_plugin_migration import NASTECH_TOOLS_MCP_SERVER_NAME, migrate
        mig_report = migrate(config)
        # The nastech-tools callback is internal plumbing — surfaced separately below.
        user_servers = [s for s in mig_report.migrated if s != NASTECH_TOOLS_MCP_SERVER_NAME]
        if user_servers:
        ...
        if NASTECH_TOOLS_MCP_SERVER_NAME in mig_report.migrated:
            lines.extend(_NASTECH_TOOLS_CALLBACK_NOTE)
...
def apply(
    ...
    # correct but the world state (managed block in ~/.codex/config.toml, nastech-tools MCP
    ...
    if new_value == "codex_app_server":
        ...
        # Migrate Nastech' MCP servers + Codex's curated plugins into ~/.codex/config.toml so the
        # spawned codex subprocess sees the same tool surface AND can call back into Nastech.
