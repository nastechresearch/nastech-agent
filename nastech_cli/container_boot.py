...
01-nastech-setup (the stage2 hook) has chowned the volume and seeded $NASTECH_HOME, but
...
def reconcile_profile_gateways(
    *, nastech_home: Path, scandir: Path, dry_run: bool = False,
    ...
    the top of ``$NASTECH_HOME``): ``nastech_cli.gateway`` maps an empty profile suffix to it,
    so it is what ``nastech gateway start`` (no ``-p``) targets.

    Without it, bare ``nastech gateway start`` inside the container would land on ``s6-svc -u
    ...
    legacy_default_state = _maybe_migrate_legacy_gateway_run_state(
        nastech_home, container_argv=container_argv, dry_run=dry_run)
    default_prior_state = legacy_default_state or _read_desired_state(nastech_home)
    ...
    actions.append(_slot_action("default", nastech_home, default_prior_state, default_should_start))

    profiles_root = nastech_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            # SOUL.md (seeded by `nastech profile create`) is the "real profile" marker.
...
def _maybe_migrate_legacy_gateway_run_state(
    nastech_home: Path, *, container_argv: Sequence[str] | None, dry_run: bool) -> str | None:
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the nastech args.
    ...
    if args and args[0].endswith("entrypoint-dispatch.sh"):  # non-PID-1 dispatch shim
        args = args[1:]
    if args and Path(args[0]).name == "nastech":  # the wrapper re-execs `nastech <subcommand>`
        args = args[1:]
    return args
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        if not start:  # `nastech -p <profile> gateway start` brings it up later (s6-svc -u)
            (tmp_dir / "down").touch()
        # Pre-create supervise/ with nastech ownership BEFORE publishing so s6-supervise inherits
        # it and runtime s6-svc calls as the nastech user won't EACCES.
...
def _write_reconcile_log(nastech_home: Path, actions: list[ReconcileAction]) -> None:
    """Append one line per profile to $NASTECH_HOME/logs/container-boot.log (rotated to ``.1``) —
...
def main() -> int:
    ...
    # A dashboard-only container must not reconcile: with a shared bind-mounted NASTECH_HOME both
    ...
    actions = reconcile_profile_gateways(nastech_home=nastech_home, scandir=scandir)
    for a in actions:
