...
def cmd_gateway_enroll(args) -> None:
    ...
    # is set (air-gapped / self-hosted IdP), otherwise Nastech Portal.
    try:
        ...
        if getattr(exc, "relogin_required", False):
            _fail(
                "✗ You're not logged into Nastech Portal.",
                "  Run `nastech setup` (or `nastech auth add nastech`) first, then retry.",
            )
        _fail(f"✗ Could not resolve a Nastech Portal access token: {exc}")
    ...
    # Persist idempotently; save_env_value writes the sensitive values to ~/.nastech/.env and never
