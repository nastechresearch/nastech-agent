"""``nastech gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nastech Portal access token from the existing login
     (``~/.nastech/auth.json``) — the same path ``nastech dashboard register``
     uses (``resolve_nastech_access_token``). This proves *which Nastech org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.nastech/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Nastech Portal).
    ...
    self-hosted-IdP path; otherwise Nastech Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Nastech Portal "
                "token could not be verified — try `nastech auth add nastech` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `nastech gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Nastech Portal); otherwise the Nastech Portal token.
    try:
        access_token = _resolve_identity_token()
    except AuthError as exc:
        if getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nastech Portal.")
            print("  Run `nastech setup` (or `nastech auth add nastech`) first, then retry.")
        else:
    ...
    #    save_env_value writes them to ~/.nastech/.env (0600 dir) and never logs.
    to_write = {
    ...
    # process is launched under (load_nastech_dotenv exports that .env), so
...
def _warn_if_secondary_multiplex_profile() -> bool:
    ...
    in nastech_cli/gateway.py. Best-effort: any failure to determine the
    ...
    try:
        ...
        default_root = Path(get_default_nastech_root()).resolve()
