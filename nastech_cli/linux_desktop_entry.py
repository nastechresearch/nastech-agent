...
``nastech desktop`` builds and launches the Electron app. On Linux, a
...
def desktop_entry_path() -> Path:
    """Where the ``nastech.desktop`` entry lives."""
...
def _running_interpreter_fallback() -> str:
    ...
    The RUNNING interpreter by definition has ``nastech_cli`` importable
...
def resolve_exec_command(project_root: Optional[Path] = None) -> str:
    ...
    Prefer the real ``nastech`` executable (argv[0] or PATH). When Nastech
    ...
    bytes. ``resolve_nastech_bin()`` prefers ``sys.argv[0]``, which differs
    ...
    bin_path = _resolve_nastech_bin_for_desktop_entry(
        resolve_nastech_bin, checkout_root=project_root
    )
    interpreter = _running_interpreter()
    if not _can_import_nastech_cli(Path(interpreter)):
        # The candidate interpreter cannot actually import nastech_cli.main
    ...
    if bin_path:
        ...
        if _needs_interpreter(resolved):
            ...
            # a NON-venv interpreter (e.g. the repo's `nastech` script with
            ...
            # sys.executable is the interpreter actually running Nastech (the
...
def _resolve_nastech_bin_for_desktop_entry(
    ...
    Wraps :func:`nastech_cli.relaunch.resolve_nastech_bin` with one
    desktop-entry-specific rule: an ``argv[0]`` that points inside this
    checkout is a launch-context artifact (the repo ``nastech`` script the
    ...
    def _inside_checkout(candidate: str) -> bool:
        try:
            ...
            return False
        # The repo `nastech` script and anything else shipped in the tree is
        ...
        for root in {checkout_root, resolved_root}:
            if root is not None and (path == root or root in path.parents):
                return True
        # The `python -m nastech_cli.main` relaunch context surfaces the
    ...
    # interpreter is an external launcher (e.g. /opt/.../bin/nastech from
    ...
    if rerouted is None and primary:
        # argv[0] was checkout-internal AND PATH had no `nastech` — common
...
def _wrapper_targets_checkout(wrapper: Path, checkout_root: Path) -> bool:
    ...
    ``<checkout>/venv/bin/python <checkout>/nastech``; a venv console
    ...
    for root in roots:
        ...
        # <root>/nastech …): a path-continuation boundary is also a match.
        if root + "/" in text:
            return True
    return False
...
def _known_wrapper_candidates():
    ...
    (``$PREFIX/bin``). The wrapper is always named ``nastech``.
    """
    candidates = []
    home = Path.home()
    ...
    candidates.append(home / ".local" / "bin" / "nastech")
    return candidates


def _project_root() -> Path:
    """This file lives at ``<checkout>/nastech_cli/linux_desktop_entry.py``.
...
def _needs_interpreter(bin_path: Path) -> bool:
    ...
    ``sys.executable`` to see Nastech' venv (rather than its own shebang)."""
    try:
...
def _install_icon_to_hicolor(icon: Path) -> bool:
    ...
    The freedesktop icon lookup finds an installed ``apps/nastech.png``
    by the unqualified name ``nastech``, so the entry can reference the
...
def install_desktop_entry(project_root: Path) -> Optional[Path]:
    """Write (or refresh) the Nastech desktop entry. Return its path.
