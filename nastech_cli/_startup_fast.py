...
Imported by ``nastech_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
nastech_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
__all__ = [
    "project_root_str", "normalize_nastech_home_env", "ensure_project_root_on_path", "is_termux_env",
...
def normalize_nastech_home_env() -> None:
    """Expand ``~``/``$VAR`` in ``NASTECH_HOME`` once, at process entry, and write it back.

    fish does not expand ``~`` inside ``VAR=~/...`` and every shell passes a quoted value
    through verbatim, so a literal tilde reaches the process. ``Path("~/.nastech")`` is
    *relative*: the many raw ``os.environ["NASTECH_HOME"]`` readers (this fast path, the
    active_profile probe, profile re-home, the dotenv loader) would each resolve it against
    cwd and scaffold a full home under ``<cwd>/~/.nastech``. One expansion here gives every
    reader the same absolute spelling; ``nastech_constants`` expands as well for non-CLI
    entry points. A relative value that is not tilde/variable-shaped is left alone.
    """
    raw = os.environ.get("NASTECH_HOME", "")
    if not raw.strip():
        return
    expanded = os.path.expanduser(os.path.expandvars(raw.strip()))
    if expanded != raw:
        os.environ["NASTECH_HOME"] = expanded
...
def active_profile_may_override_home(nastech_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect NASTECH_HOME?"""
    active = (_read_text(os.path.join(nastech_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".nastech")


def _resolved_home() -> str:
    return os.environ.get("NASTECH_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("NASTECH_DEV") == "1" or is_container_startup_environment():
        return False
    nastech_home = os.environ.get("NASTECH_HOME", "").strip()
    if nastech_home:
        if os.path.exists(os.path.join(nastech_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(nastech_home)))
        return parent_name != "profiles" and active_profile_may_override_home(nastech_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``nastech --version`` output (also used by /version).
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``nastech --version`` before the heavy import wall.
    ...
    NASTECH_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("NASTECH_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
