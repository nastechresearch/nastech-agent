...
This module is imported by ``nastech_cli/main.py`` BEFORE its heavy import
wall (config, argparse tree, logging, providers). Everything here must stay
**stdlib-only and cheap** (os/sys file probes; no yaml, no nastech_cli.config,
...
``nastech_cli/config.py``'s ``get_container_exec_info()`` reads the same
...
def active_profile_may_override_home(nastech_root: str) -> bool:
    ...
    active_profile = os.path.join(nastech_root, "active_profile")
    try:
...
def container_mode_may_be_active() -> bool:
    ...
    if nastech_home:
        ...
        return (
            ...
            and active_profile_may_override_home(nastech_home)
        )

    default_home = os.path.join(os.path.expanduser("~"), ".nastech")
...
def read_install_method() -> str | None:
    ...
    non-default profile might redirect NASTECH_HOME.
    """
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    ...
    ``nastech version`` subcommand. Every lazy block degrades gracefully —
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    ...
    container. Termux keeps the NASTECH_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
