...
def defer_manual_serve(runtime: dict, *, require_alive: bool = False) -> bool:
    ...
    from nastech_cli.process_identity import _pid_alive_matches
...
def warn_pending_manual_serves(*, startup: bool = False, pending_manual: list[dict] | None = None) -> None:
    ...
    for row in pending_manual:
        ...
        print("    Ask its owner to relaunch `nastech serve` / `nastech dashboard`; check reminder storage permissions and free space.", file=stream)
    directory = get_nastech_home() / "serve_restart_pending"
    for path in sorted(directory.glob("*.json")):
        try:
            ...
            print("    Ask its owner to relaunch `nastech serve` / `nastech dashboard` (reconnect Desktop for an SSH backend).", file=stream)
