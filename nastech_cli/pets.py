...
def _cmd_list(args) -> int:
    ...
    if args.installed:
        ...
        if not pets:
            print("No pets installed. Try: nastech pets install boba")
            return 0
...
def _cmd_install(args) -> int:
    ...
    if args.select or not _has_active_pet():
        ...
        print(f"  Make it active with: nastech pets select {slug}")
    return 0
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        print("  → no pets installed. Run: nastech pets install boba")
        ok = False
    elif active is None:
        print("  → active pet unresolved. Run: nastech pets select <slug>")
        ok = False
    elif not enabled:
        print("  → pet display is disabled. Run: nastech pets select " + active.slug)
