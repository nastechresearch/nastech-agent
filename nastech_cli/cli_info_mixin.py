...
Mixin split out of ``cli.py``; bound onto ``NastechCLI`` via the MRO. cli.py-internal symbols are
...
from nastech_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        if is_nastech_nastech_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
        ...
        # `nastech skills trust`. Never raises.
        try:
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/nastech_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
    ...
    def _show_usage(self):
        ...
        The Nastech credits block is agent-independent (portal fetch), so it runs even with no live
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.nastech/skills/ and queue a note for the next user turn.
