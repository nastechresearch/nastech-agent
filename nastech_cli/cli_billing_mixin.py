"""Billing and subscription handlers for the interactive CLI (mixed into ``NastechCLI``).
...
_CHARGE_ERROR_COPY = {
    ...
    "cli_billing_disabled": "  Remote spending is off for this account — a billing admin can turn it on from the portal's Nastech Agent page.",
...
_KILLSWITCH_REASON_OVERVIEW = "A billing admin can turn it on from the portal's Nastech Agent page to add funds here."
_KILLSWITCH_REASON_BUY = "A billing admin can turn it on from the portal's Nastech Agent page before adding funds."
...
class CLIBillingMixin:
    ...
    def _print_logged_out(self, state, load_failed, cmd) -> None:
        ...
        if state.error:
            ...
            self._dim("Not logged into Nastech Portal.", icon="💳 ", lead=True)
            print(f"  Run `nastech portal` to log in, then {cmd}.")
    ...
    # ── /usage — Nastech balance block ──

    def _print_nastech_credits_block(self) -> bool:
        """Nastech dollar balance block (two bars); True if anything printed. Shared dollar model first, then
        legacy ``nastech_credits_lines``. Agent-independent (TUI slash-worker has no live agent). Fail-open."""
    ...
    def _print_usage_cta(self) -> None:
        """The `/usage` call-to-action; mirrors the TUI's ``USAGE_CTA``. Nastech-account only."""
    ...
    def _show_subscription(self):
        ...
        if state.context == "team":  # no personal plan — teams run on a shared balance
            self._block_header("𓄃", "Team subscription")
    ...
    def _subscription_free_catalog(self, state, manage_url):
        ...
        if not tiers:
            ...
            return
        self._block_header("𓄃", "Choose a plan")
    ...
    def _subscription_handle_scope_required(self, state, *, retry, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `nastech portal` and allow Remote Spending, then re-run /subscription.",
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {_RULE}")
        self._dim('By confirming, you allow Nastech Research to charge your card.')
        confirm_choices = [
    ...
    def _billing_submit_and_poll(self, state, amount, key, *, missing_msg, status_msg, on_scope=None):
        ...
        from cli import _cprint, _d
        from nastech_cli.nastech_billing import BillingError, BillingScopeRequired, post_charge
        try:
    ...
    def _billing_poll_charge(self, state, charge_id, amount):
        ...
        from nastech_cli.nastech_billing import BillingError, BillingTransient, get_charge_status
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from nastech_cli.nastech_billing import BillingTransient, BillingRemoteSpendingRevoked, BillingSessionRevoked
        code = exc.error
    ...
    def _billing_handle_scope_required(self, state, *, amount=None, idempotency_key=None):
        ...
        granted = self._step_up_remote_spending(
            ...
            noninteractive_msg="  Run `nastech portal` and allow Remote Spending, then retry.",
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        if reload_amt is None or threshold_amt is None or reload_amt <= threshold_amt:
            ...
            return
        self._dim(f"By confirming, you authorize Nastech Research to charge {card.masked} whenever your balance reaches {format_money(threshold_amt)}. Turn off any time here or on the portal.", lead=True)
    ...
    def _billing_patch_auto_top_up(self, state, **kwargs) -> bool:
        ...
        from nastech_cli.nastech_billing import BillingError, BillingScopeRequired, patch_auto_top_up
        try:
