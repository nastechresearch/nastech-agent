...
Toolsets = explicit --toolsets, else the user's "cli" toolsets from `nastech tools`. Rules /
memory / AGENTS.md / preloaded skills = same as a normal chat turn. Approvals are auto-bypassed
(NASTECH_YOLO_MODE=1). Model/provider mirror `nastech chat`: both optional; only --model → auto-detect
...
def run_oneshot(
    ...
    Model/provider fall back to ``NASTECH_INFERENCE_MODEL`` and config.yaml. ``usage_file`` gets a
    ...
    # Same finite-chat marker as `nastech chat -q` (cli.py): the session-source resolver uses it to drop an
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB — oneshot bypasses ``NastechCLI._init_agent()``, so it must wire the
...
def _resolve_model_and_provider(cfg: dict, model: Optional[str], provider: Optional[str]) -> _ModelChoice:
    ...
    if direct is None:
        ...
        current_provider = cfg_provider or os.getenv("NASTECH_INFERENCE_PROVIDER", "").strip().lower() or "auto"
        # Same owner as NastechCLI startup: a provider-qualified string (``custom:<name>:<model>``,
...
def _load_resume_target(session_db, resume: Optional[str]) -> tuple[Optional[str], list, Optional[dict]]:
    ...
    nothing but is recorded under the requested session — ``nastech -z "hello" -c <title>
...
def _apply_stored_session_runtime(
    ...
    if explicit_model:
        return choice
    from nastech_cli.cli_model_switch_mixin import stored_session_route
...
def _run_agent(
    ...
    try:
        agent = AIAgent(
            ...
            # NASTECH_INTERACTIVE (never set), hook approval via NASTECH_ACCEPT_HOOKS=1, dangerous
            # commands via NASTECH_YOLO_MODE=1, skill secret capture degrades gracefully.
