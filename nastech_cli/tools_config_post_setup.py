"""Post-setup install hooks and installed-state predicates for `nastech tools` provider rows."""
...
from nastech_cli.tools_config_cua import (
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Nastech-managed ``$NASTECH_HOME/bin`` copy does."""
...
def _post_setup_agent_browser(post_setup_key: str) -> None:
    ...
    global/Nastech-managed install), so there is no ``npm install`` step here."""
    ...
    try:
        ...
        # PATH/node_modules probes above. Mirror the rung nastech_cli.doctor uses so this probe can't diverge
        ...
        # agent-browser is no longer a root package.json dependency (#43564) — it resolves lazily via npx
        # (or a global/Nastech-managed install) instead of a local `npm install`, so there's no node_modules/
    ...
    # Reuse the runtime resolution cascade (PATH -> Homebrew/Nastech-managed node -> npx) rather than
    # a bare shutil.which — Nastech-managed-Node-only setups resolve agent-browser/npx only that way.
    try:
...
_PIP_POST_SETUP_HOOKS: dict = {
    "faster_whisper": _pip_hook(
        ...
        on_install=("Model sizes: tiny, base (default), small, medium, large-v3",
                    "Change via stt.local.model in ~/.nastech/config.yaml")),
    "kittentts": _pip_hook(
...
def _post_setup_spotify() -> None:
    # Full `nastech auth spotify` flow: no client_id yet → interactive wizard (persists to ~/.nastech/.env)
    ...
    try:
        ...
        _info_lines("Run manually: nastech auth spotify")
        return
    ...
    try:
        ...
        _info_lines("Run later: nastech auth spotify")
    except Exception as exc:
...
def _post_setup_langfuse() -> None:
    ...
    try:
        ...
        _info_lines("Run manually: nastech plugins enable observability/langfuse")
    _info_lines("Restart Nastech for tracing to take effect.", "Verify: nastech plugins list")
...
def _post_setup_xai_grok() -> None:
    ...
    try:
        from nastech_cli.setup import prompt_choice, prompt as _setup_prompt
        ...
        _info_lines("Run later: nastech auth add xai-oauth   (or set XAI_API_KEY)")
        return
    ...
    if idx == 0:
        if _run_xai_oauth_login_from_setup():
            ...
            _print_warning("    xAI Grok OAuth login did not complete. Run later: nastech auth add xai-oauth")
    elif idx == 1:
        ...
        if api_key:
            ...
            _print_warning("    No API key provided. Run later: nastech auth add xai-oauth")
    else:
...
def run_post_setup_command(args) -> int:
    """``nastech tools post-setup <key>`` — non-interactive runner the dashboard spawns so the GUI can drive
...
# Python deps installed via ``nastech tools`` aren't in the managed runtime's locked ``all`` sync, so a
