...
def _warn_config_parse_failure(
    ...
    if fallback == "last-known-good":
        ...
        msg = (
            ...
            f"Fix the YAML (nastech config edit) and retry."
        )
    else:
        msg = (
            f"Failed to parse {config_path}: {exc}. "
...
def get_active_config_parse_failure() -> Optional[str]:
    ...
    Consumers: ``nastech_cli.auth.resolve_provider`` uses this to refuse
...
def get_managed_system() -> Optional[str]:
    ...
    if raw:
        ...
        # NASTECH_MANAGED variable of the service. A marker with content
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nastech was installed: 'apt', 'docker', 'nix', 'nixos',
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "apt":
        ...
        return "pkg upgrade nastech-agent"
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``nastech config set`` uses ``.`` as the nesting separator, so a key that
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    # Nastech (``_save_discovered_models_to_config``), not hand-curated.
    # Older Nastech versions wrote an in-mapping ``__discovered_model_catalog__``
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "tool_gateway_declined_tools",  # per-tool Tool Gateway offer declines (nastech_cli/nastech_subscription.py, #92647)
...
def split_model_config_default(raw_default: Any) -> tuple[str, str]:
    ...
    together through ``NastechCLI`` construction: the model becomes a plain
...
#   - survives the ``str() → int()`` round-trip through the NASTECH_MAX_ITERATIONS
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "intended values with `nastech cron edit <job_id> --provider <provider> "
        "--model <model>`."
    )
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    ``nastech config set platforms.telegram.streaming false`` landed on a key
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if not isinstance(_default_value_for_key(key), str):
        ...
        if _lower in {'true', 'yes', 'on'}:
            ...
            #   nastech config set platform_toolsets.line '["file","web"]'
            # or a multi-line YAML block:
            #   nastech config set custom_providers '- name: foo
            #     base_url: https://...'
