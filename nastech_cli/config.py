"""Configuration management for Nastech Agent: config.yaml / .env loading, saving,
validation, migration, and the ``nastech config`` command."""
...
# dies with ImportError before any later heal point is reached. See nastech_cli.stale_modules.
...
from nastech_constants import get_nastech_home, get_process_nastech_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    "last-known-good": "Nastech is running on the settings it loaded before the edit until it is fixed, so recent changes are not applied.",
    "last-known-good-backup": "Nastech is running on your last good settings until it is fixed, so recent changes are not applied.",
    "refuse-write": "Nothing was written, so the existing file is preserved."}
_PARSE_FAILURE_DEFAULTS_MSG = (
    "Nastech is running on default settings until it is fixed, so none of your saved settings are applied.")
_PARSE_FAILURE_REPAIR_MSG = "Open it with `nastech config edit`, fix {where}, then run `nastech config check`."
...
def format_config_parse_failure(config_path: Path, exc: Exception, *, fallback: str = "defaults") -> str:
    """User copy for an unparseable config.yaml: what happened, what Nastech is doing, how to fix.
...
def _warn_config_parse_failure(
    ...
    from nastech_cli.config_backups import backup_config
...
# interpreter init (PYTHON*, NODE_*: Nastech restarts through them), PATH (fix tool lookup
...
# and Nastech runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``NASTECH_*`` overall is NOT blocked — many integration credentials use that prefix
# (NASTECH_LANGFUSE_PUBLIC_KEY, NASTECH_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Nastech runtime location and security policy (NASTECH_HOME, NASTECH_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.nastech/.env "
            "directly.")
...
# acquires it again (nastech_cli/plugins.py: `with ..., config_mod._CONFIG_LOCK:` then
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean NASTECH_TOOL_PROGRESS variant is unsupported (its only
    ...
    "NASTECH_LANGFUSE_ENV", "NASTECH_LANGFUSE_RELEASE", "NASTECH_LANGFUSE_SAMPLE_RATE",
    "NASTECH_LANGFUSE_MAX_CHARS", "NASTECH_LANGFUSE_CAPTURE", "NASTECH_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "NASTECH_ACP_AUTH_METHOD", "NASTECH_ACP_AUTO_APPROVE", "NASTECH_COPILOT_ACP_COMMAND",
    "NASTECH_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# NASTECH_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``NASTECH_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `nastech update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system() -> Optional[str]:
    """Return the package manager owning this install, if any.
    Signals: NASTECH_MANAGED env var (systemd service) or a ``.managed`` marker file in
    NASTECH_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("NASTECH_MANAGED", "").strip().lower() or None
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Nastech was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$NASTECH_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because NASTECH_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``nastech update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``nastechresearch/nastech-agent`` image bakes a ``docker`` stamp into
    ``/opt/nastech`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of nastech_cli/), NOT of $NASTECH_HOME,
    ...
    method = _install_method_stamp(get_nastech_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``nastech_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nastechresearch/nastech-agent:latest",
    "apt": "pkg upgrade nastech-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "nastech update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (nastech_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Nastech.
...
def get_container_exec_info() -> Optional[dict]:
    ...
    already inside the container, or when NASTECH_DEV=1 is set. Only FileNotFoundError is
    ...
    try:
        info = {}
        with open(get_nastech_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
...
# ---- Config paths / NASTECH_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / NASTECH_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("NASTECH_IGNORE_USER_CONFIG") == "1":
        return
    ...
    message = (
        f"Nastech stopped because your settings file ({config_path}) has a formatting error"
        f"{f' at {where}' if where else ''}. Fix it with `nastech config edit` and check with "
        "`nastech config check`, or add --ignore-user-config to run once with default settings.")
    if backup_path is not None:
...
def _resolve_nastech_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read NASTECH_UID / NASTECH_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns NASTECH_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Nastech commonly set these to map the in-container user to a host user so
    ...
    return _env_int("NASTECH_UID"), _env_int("NASTECH_GID")


def _chown_to_nastech_uid(path) -> None:
    """Chown ``path`` to ``NASTECH_UID:NASTECH_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_nastech_home` on Docker deployments. See #34107.
    """
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply NASTECH_UID/GID ownership. No-op when managed;
    in a container only an explicit NASTECH_HOME_MODE is applied. NASTECH_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse NASTECH_HOME to a served subdirectory without
    directory listings.

    Also applies ``NASTECH_UID``/``NASTECH_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("NASTECH_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit NASTECH_HOME_MODE is the operator's choice and is still applied.
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or NASTECH_CONTAINER / NASTECH_SKIP_CHMOD opt-out).
...
_NASTECH_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_nastech_home():
    """Ensure the ~/.nastech directory skeleton exists with secure permissions.
    ...
    from nastech_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _NASTECH_HOME_ENSURED and home.is_dir():
        return
    from nastech_cli.config_home import initialize_home
    initialize_home(home, _NASTECH_HOME_SUBDIRS, _NASTECH_HOME_ENSURED)


# ---- Config loading/saving ----

from nastech_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from nastech_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use nastech_cli.config.<name>)
...
# Back-compat re-exports — :mod:`nastech_cli.personality` owns personality/overlay semantics.
from nastech_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``nastech config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `nastech config set` routes this credential-shaped key to .env through the
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `nastech update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "known_plugin_toolsets", # nastech_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    ``nastech_time._get_zoneinfo()`` swallows an invalid name behind a single WARNING in the
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. NASTECH_TIMEZONE overrides "
            "this key when set.")
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'nastech tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from nastech_cli.config_home import config_load_issue
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from nastech_cli.toolset_scope import toolset_allowed_for_platform
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_nastech_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``nastech config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``nastech status``/``dump``) read ``name`` and
...
# the NASTECH_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _backups_dir_display() -> str:
    from nastech_constants import display_nastech_home
    return f"{display_nastech_home()}/backups/config/"


_FIX_PERMS = "Fix the file permissions or move it aside first."
_FIX_YAML = (
    "Fix it with `nastech config edit` and check with `nastech config check`, or copy the newest good "
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Nastech host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `nastech config get`): nothing loaded yet in this process, so
        ...
        from nastech_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:8] == cache_sig:
            ...
            # load_nastech_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_nastech_dotenv() pins unexpanded literals
...
def load_env() -> Dict[str, str]:
    """Load ~/.nastech/.env as a dict. Memoised inside ``load_env_file`` (``get_env_value()`` runs
...
def sanitize_env_file() -> int:
    """Rewrite ~/.nastech/.env with normalized line formatting; returns the number of changed lines."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.nastech/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``NASTECH_CUSTOM_`` prefix keeps the
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.nastech/.env and os.environ; True if it was found and removed."""
...
def reload_env() -> int:
    """Re-read ~/.nastech/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Nastech (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.nastech/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Nastech-managed credential preferring ``~/.nastech/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale NASTECH_MAX_ITERATIONS ghost is caught even when the
    ...
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale NASTECH_MAX_ITERATIONS={env_ghost} "
                    f"(run 'nastech doctor --fix' to remove)", Colors.YELLOW))
...
def show_config():
    ...
    print(color("│              𓄃 Nastech Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``NASTECH_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("NASTECH_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`nastech cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `nastech config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``nastech config set`` string to bool/None/int/float/list/dict.
...
def _refuse_container_type_mismatch(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> Any:
    ...
    _exit_invalid(
        ...
        f"  Pass a YAML/JSON literal, e.g.:\n    nastech config set {key} '{literal}'\n"
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``nastech config set platforms.telegram.streaming false`` landed on a key
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Nastech may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from nastech_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (NASTECH_YOLO_MODE, PATH, ...)
    ...
    if key == "model.provider" and _old_provider.lower() != str(value).strip().lower():
        from nastech_cli.route_identity import drop_stale_model_route
        ...
        if _popped:
            ...
            _route_notice = color(
                ...
                "`nastech config unset model.base_url` (and model.api_mode).", Colors.YELLOW)
    # api_base -> base_url alias at set-time too (mirrors _normalize_root_model_keys).
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `nastech config
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from nastech_cli.config_env_routing import is_env_setting_key, read_env_setting
    ...
    if _split_key_path(key)[0] in _known_top_level_keys():
        ...
        if not is_known:
            print(color(
                f"⚠ '{key}' is not a recognized config key — Nastech may not read it; the value "
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        return
    from nastech_cli.config_env_routing import is_env_setting_key, remove_env_setting
...
_USAGE_GET = ("Usage: nastech config get <key> [--json] [--raw]", [
    "nastech config get model", "nastech config get terminal.backend",
    "nastech config get skills.config --json"], None)
_USAGE_SET = ("Usage: nastech config set [--force] <key> <value>", [
    "nastech config set model anthropic/claude-sonnet-4", "nastech config set terminal.backend docker",
...
_USAGE_UNSET = ("Usage: nastech config unset <key>", [
    "nastech config unset model", "nastech config unset terminal.backend",
    "nastech config unset OPENROUTER_API_KEY"], None)
...
_CONFIG_USAGE = """Available commands:
  ...
  nastech config get <key>          Print a resolved config value
  nastech config set <key> <value>   Set a config value
  nastech config unset <key>        Remove a config value
  nastech config check     Check for missing/outdated config
  nastech config migrate   Update config with new options
  nastech config path      Show config file path
  nastech config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``nastech config`` UI without the core knowing they exist.
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('nastech_cli.route_identity', 'normalize_route_base_url'),
}
