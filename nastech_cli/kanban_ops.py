"""Dispatcher and maintenance verbs for ``nastech kanban``: ``dispatch``,
...
from nastech_cli.kanban_output import _err, _fmt_ts, _print_json
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                ...
                print(
                    ...
                    f"successfully.{held} Check profile health (venv, PATH, credentials) and `nastech "
                    f"kanban list --status ready` / `nastech kanban list --status blocked` for "
