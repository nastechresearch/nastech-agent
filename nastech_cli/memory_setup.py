...
def _provider_pip_dependencies(provider_name: str, declared: list) -> list:
    ...
    ``nastech memory setup`` — if the update-time refresh only reinstalled
    ...
    if provider_name == "hindsight":
        try:
            import json
            cfg_path = get_nastech_home() / "hindsight" / "config.json"
...
def _install_dependencies(provider_name: str, *, force: bool = False) -> None:
    ...
    ranges. This is how ``nastech update`` heals the active memory provider
    ...
    # data volume (NASTECH_LAZY_INSTALL_TARGET). install_specs handles the
...
def _write_env_vars(
    env_writes: dict,
    nastech_home: str | os.PathLike[str] | None = None,
) -> None:
    """Persist memory-provider env vars through the canonical ``.env`` writer.

    Delegates to ``nastech_cli.config.save_env_value`` so every key flows
    ...
    ``NASTECH_HOME`` / etc.), CR/LF stripping on the value, and the atomic
    ...
    ``.env`` -> ``os.environ`` chain on the next Nastech startup, and the
    ...
    ``nastech_home`` may be supplied by plugin ``post_setup`` hooks that
    already received an explicit home directory (e.g. a non-default
    profile). It is applied through the context-local Nastech home override
            ...
            if provider.is_available():
                ...
                if required_fields:
                    ...
                    for f in required_fields:
                        ...
                        print(line)
                print(
                    "  Note: systemd/gateway services do not inherit ~/.nastech/.env —"
                )
                print(
