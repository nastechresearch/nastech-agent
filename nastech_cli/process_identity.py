...
Three layers make every long-lived Nastech process positively identifiable, so reapers (``nastech
update``, Desktop startup sweeps) never guess lineage from PPID archaeology or cmdline matching:
1. spawn tags (``NASTECH_SPAWN`` env stamped by the spawner); 2. a ``(pid, create_time)`` ledger;
...
def register_self(purpose: str, *, project_root: Optional[Path] = None, detail: Optional[dict] = None) -> bool:
    ...
    try:
        ...
        # 10 tokens: enough for `nastech serve --host X --port N --profile P` while bounding
...
def _desktop_spawner_identity() -> tuple[Optional[int], Optional[float]]:
    """Spawner ``(pid, create_time)`` from the Electron app's NASTECH_PARENT_PID (+ optional
    ...
    try:
        spawner_pid = int(os.environ.get("NASTECH_PARENT_PID", ""))
    except (TypeError, ValueError):
        spawner_pid = 0
    if spawner_pid <= 0:
        return None, None
...
def register_child(pid: int, purpose: str, *, project_root: Optional[Path] = None) -> bool:
    ...
    helpers: arbitrary ``npx``/binary servers never import Nastech code). Records the child's
