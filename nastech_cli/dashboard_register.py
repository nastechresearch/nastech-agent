...
Automates the Nastech Portal ``/local-dashboards`` flow: resolve a fresh Nastech access token, POST
``{portal}/api/oauth/self-hosted-client`` (the ``agent:`` prefix is applied server-side),
write ``NASTECH_DASHBOARD_OAUTH_CLIENT_ID`` (+ portal/public URL when warranted) into ``.env``
...
_DEFAULT_PORTAL = "https://portal.nastechresearch.github.io"
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    try:
        ...
        base = (get_provider_auth_state("nastech") or {}).get("portal_base_url")
        chosen = base if isinstance(base, str) and base.strip() else str(DEFAULT_NASTECH_PORTAL_URL)
    ...
    *, access_token: str, portal_base_url: str, name: Optional[str], custom_redirect_uri: Optional[str],
    ...
    try:
        ...
        try:  # structured JSON errors: {error, error_description}
            ...
            detail = ""
        if exc.code == 401:
            message = ("Nastech Portal rejected the access token (401). "
                       "Try `nastech auth add nastech` to re-authenticate.")
        elif exc.code == 403:
        ...
        raise RuntimeError(f"Could not reach Nastech Portal at {portal_base_url}: {exc.reason}") from exc
    ...
    *, client_id: str, portal_base_url: str, custom_redirect_uri: Optional[str],
    ...
    if custom_redirect_uri:
        try:  # example host matches the one the user registered
            ...
            host = "your-host"
        print(
            "  To require Nastech login on your registered host, run the dashboard\n"
            f"  bound publicly (it must be reachable at https://{host}) and log in\n"
            "  at its /login page.")
    else:
        print(
            "  To require Nastech login (e.g. exposing on your LAN or a public host):\n"
...
def _public_url_from_redirect(redirect_uri: Optional[str]) -> str:
    ...
    ``/auth/callback`` to NASTECH_DASHBOARD_PUBLIC_URL, so the raw URI would double the path."""
    try:
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print("✗ `nastech dashboard register` is not available in a managed/hosted install.\n"
    ...
    try:
        ...
        if isinstance(exc, AuthError) and getattr(exc, "relogin_required", False):
            print("✗ You're not logged into Nastech Portal.\n"
                  "  Run `nastech setup` (or `nastech auth add nastech`) first, then retry.")
        else:
            print(f"✗ Could not resolve a Nastech Portal access token: {exc}")
        sys.exit(1)
    ...
    portal_override = getattr(args, "portal_url", None) or os.environ.get("NASTECH_DASHBOARD_PORTAL_URL")
    ...
    stored = _env_value("NASTECH_DASHBOARD_OAUTH_CLIENT_ID")
    ...
    existing_portal = _env_value("NASTECH_DASHBOARD_PORTAL_URL")
    should_write_portal = (
    ...
    wrote_portal_url = should_write_portal and _save_env_quietly("NASTECH_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    wrote_public_url = bool(
        public_url
        and _env_value("NASTECH_DASHBOARD_PUBLIC_URL") != public_url
        and _save_env_quietly("NASTECH_DASHBOARD_PUBLIC_URL", public_url))
