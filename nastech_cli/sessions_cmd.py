"""``nastech sessions`` command — extracted from ``nastech_cli/main.py``.
...
Helpers that stay in ``nastech_cli.main`` (``get_nastech_home``,
...
``nastech_cli.main.<name>`` keep reaching this code path, and so imports stay
...
#: Default age floor for `nastech sessions prune --never-active`.  Deliberately
...
def _prune_never_active_keyed(db, args):
    """`nastech sessions prune --never-active` — drop leaked/dead keyed rows.
...
def cmd_sessions(args, sessions_parser=None):
    ...
    if action == "repair":
        ...
        if report.get("repaired"):
            ...
            print(f"    nastech sessions recover --source {source_hint} \\")
            print("        --inspect-only")
    ...
    if action == "list":
        from nastech_state import workspace_key as _ws_key
            ...
            try:
                if len(ids) == 1:
                    ...
                    out_dir = (
                        ...
                        else get_nastech_home() / "session-exports"
                    )
        ...
        if args.output == "-":
            ...
            db.close()
            return
        output_dir = Path(args.output).expanduser() if args.output else get_nastech_home() / "session-exports"
        ...
        # `nastech sessions prune`: no time window and no filters at all
        ...
        if not _include_pinned and callable(_count_matches):
            ...
            if _pinned_skipped:
                ...
                _optin = (
                    ...
                    "first with `nastech sessions unpin <id>`."
                    if action == "prune"
                    else "Unpin first with `nastech sessions unpin <id>` to include them."
                )
                print(
        ...
        if skipped_open:
            ...
            print(
                ...
                "sessions. Use `nastech sessions delete <id>` "
        ...
        if not pinned_rows:
            print(
                "No pinned sessions. Pin one with: nastech sessions pin <session_id>"
            )
            return
        ...
        if result.get("vacuumed") is False:
            print("  (VACUUM was skipped or failed — run "
                  "`nastech sessions optimize` later to reclaim freed space.)")
