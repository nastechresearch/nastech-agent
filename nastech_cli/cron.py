...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: nastech gateway install\n"
...
def _missed_fire_line(job: Dict[str, Any], fire_err: Dict[str, Any]) -> str:
    ...
    return (f"{color('⚠ A scheduled run was skipped', Colors.RED)} at {fire_err.get('at', '?')}: the messaging "
            f"gateway was unreachable. Run `nastech gateway restart`, then `nastech cron run {job.get('id', '<id>')}` "
...
def _print_ticker_health(pids: list) -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              ...
              "  If heartbeat never appears, restart: nastech gateway restart")
        ...
        print("  Cron jobs may NOT be firing. Restart: nastech gateway restart")
...
def cron_status():
    ...
    if provider != "builtin":
        ...
        print(color("✓ Gateway is running via the default-profile multiplexer — it ticks this profile's jobs.",
                    Colors.GREEN))
        print(color("  Ticker health is reported by `nastech cron status` on the default profile.", Colors.DIM))
    else:
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Next: fix the listed job config, then run `nastech cron doctor` again.", Colors.DIM))
    return 1
