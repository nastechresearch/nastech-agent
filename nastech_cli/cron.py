...
# commands at execution time when ``_NASTECH_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Check status:  nastech cron status", Colors.DIM))
...
def cron_incidents(args) -> int:
    ...
    ``nastech cron incidents [--state <s>]`` lists incidents (the stored error
    is redacted and truncated at write time, safe for terminal display);
    ``nastech cron incidents ack <id>`` closes one so its failure ping stays
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(
                color(
                    "✗ Incident ID required: nastech cron incidents ack <incident_id>",
                    Colors.RED,
                )
            )
            return 1
...
def cron_status():
    ...
    if pids or gateway_alive_via_lock:
        ...
        if hb_age is None:
            ...
            print("  If the gateway just started, wait ~60s and re-run `nastech cron status`.")
            print("  If heartbeat never appears, restart: nastech gateway restart")
            ...
            if last_error:
                ...
                if "Permission denied" in last_error:
                    print(color(
                        ...
                        "(e.g. rewritten by a root `docker exec nastech "
                        "nastech cron ...`). Fix ownership to match the "
...
def _print_active_jobs_summary(jobs) -> None:
    ...
    if jobs:
        ...
        # status level, not just buried per-job in `nastech cron list`.
        late = [
            j for j in jobs
...
def _scripts_dir_for_cron() -> Path:
    ...
    Prefer ``cron.jobs.CRON_DIR.parent`` over a fresh ``get_nastech_home()`` call
    so tests and profile-aware callers that monkeypatch cron storage inspect the
    same Nastech home the jobs were loaded from.
    """
...
def _script_health_issue(script: str) -> Optional[str]:
    ...
    try:
        ...
        return f"script resolves outside NASTECH_HOME/scripts: {script!r}"
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # the CLI inherits a gateway/desktop session env (NASTECH_SESSION_KEY);
...
def cron_resume(args) -> int:
    ...
    run_at = _nastech_now().isoformat() if args.run_now else args.run_at
    try:
...
def cron_notepad(args) -> int:
    """Handle ``nastech cron notepad <job_id> [get|set|delete|list]``.
    ...
    try:
        if action == "set":
            if key is None or value is None:
                print(color("Usage: nastech cron notepad <job_id> set <key> <value>", Colors.RED))
                return 1
        ...
        if action == "get":
            if key is None:
                print(color("Usage: nastech cron notepad <job_id> get <key>", Colors.RED))
                return 1
        ...
        if action == "delete":
            if key is None:
                print(color("Usage: nastech cron notepad <job_id> delete <key>", Colors.RED))
                return 1
...
def cron_command(args):
    ...
    print("Usage: nastech cron [list|create|edit|pause|resume|run|remove|status|runs|doctor|tick]")
    sys.exit(1)
