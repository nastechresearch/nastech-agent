...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     If no gateway is running: nastech gateway install\n"
                "                    sudo nastech gateway install --system  # Linux servers\n"
                "     Check status:  nastech cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'nastech cron create ...' "
...
def _delivery_fix_hint(job: Dict[str, Any]) -> str:
    return (f"Check the target with `nastech cron status` or change it with "
            f"`nastech cron edit {job.get('id', '<id>')} --deliver <target>`.")
...
def _missed_fire_issue(job: Dict[str, Any], fire_err: Dict[str, Any]) -> str:
    return (f"missed scheduled fire at {fire_err.get('at', '?')}: {_short_reason(fire_err['detail'])}. "
            "The messaging gateway was unreachable. Run `nastech gateway restart`, then "
            f"`nastech cron run {job.get('id', '<id>')}` to run it now.")
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `nastech cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: nastech cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: nastech cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec nastech nastech cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list, restart_command: str = "nastech gateway restart") -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `nastech cron status`.\n"
...
def cron_status():
    ...
    from nastech_cli.gateway import find_gateway_pids, named_profile_served_by_running_multiplexer
    ...
    if provider != "builtin":
        ...
        if pids or gateway_alive_via_lock or served_by_multiplexer:
            if served_by_multiplexer:
                ...
                _print_ticker_health([], restart_command="nastech --profile default gateway restart")
            else:
            ...
            print("\n  To enable automatic execution for this profile:\n"
                  ...
                  "    nastech gateway run        # Or run in foreground")
            if active not in ("default", "custom"):
                print("\n  Alternatives for this named profile:\n"
                      ...
                      "      nastech --profile default config set gateway.multiplex_profiles true\n"
                      "      nastech --profile default gateway restart\n"
                      "    To migrate existing per-profile services with preflight checks:\n"
                      "      nastech --profile default gateway migrate --multiplex\n"
                      "  Check: nastech cron status from this profile should show its ticker heartbeat.\n")
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Review the findings above, then run `nastech cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits NASTECH_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (NASTECH_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _nastech_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _nastech_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``nastech cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: nastech cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: nastech cron [list|create|edit|pause|resume|run|remove|resnap|status|runs|doctor|tick]")
    sys.exit(1)


def _cron_resnap(args) -> int:
    """Handle `nastech cron resnap [job_id] [--all]`."""
    ...
    if not job_id:
        ...
        print("Usage: nastech cron resnap <job_id> | nastech cron resnap --all")
        return 1
