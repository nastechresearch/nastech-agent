...
Pure leaf: imports nothing from ``nastech_cli.auth`` so the per-provider modules
(``auth_nastech``, ``auth_codex``, ...) can import it at module scope without cycles."""
...
# httpx is imported lazily (~30ms) because nastech_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("nastech_cli.auth.httpx.Client")
...
_nastech_err = _provider_error_factory("nastech")
