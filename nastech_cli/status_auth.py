"""Credential sections of `nastech status`, run through ``status._SECTIONS`` with its shared context.
...
_OAUTH_BLOCKS = (
    ...
    ("OpenAI Codex", "get_codex_auth_status", "nastech model", _FILE_REFRESH_ROWS),
    ...
    ("MiniMax OAuth", "get_minimax_oauth_auth_status", "nastech auth add minimax-oauth", (
        ("Region:", "region", None, True),
        ("Access exp:", "expires_at", None, None),
        ("Error:", "error", None, False))),
    ("xAI OAuth", "get_xai_oauth_auth_status", "nastech auth add xai-oauth", _FILE_REFRESH_ROWS))
...
# Nastech Tool Gateway per-feature state: first matching (predicate(feature, nastech_auth), text(feature)).
_FEATURE_STATES = (
    (lambda f, _: f.managed_by_nastech, lambda f: "active via Nastech subscription"),
...
def _render_auth_providers(ctx):
    ...
    try:
        # Read-only display: the refresh-free snapshot, so `nastech status` never performs an OAuth
        # refresh or burns a single-use refresh token.
        nastech_status = auth.get_nastech_auth_status_local()
        statuses = {getter: getattr(auth, getter)() for _, getter, _, _ in _OAUTH_BLOCKS[:3]}
    except Exception:
        nastech_status, statuses = {}, {}
    ...
    info = None
    if any(nastech_status.get(k) for k in ("logged_in", "access_token", "portal_base_url",
                                        "inference_credential_present", "error_code")):
        try:
            info = get_nastech_portal_account_info()
        except Exception:
            pass
    ctx.nastech_account_info = info
    ctx.nastech_logged_in = logged_in = bool(nastech_status.get("logged_in") or (info and info.logged_in))
    ctx.nastech_inference_present = inference = bool(
        nastech_status.get("inference_credential_present") or (info and info.inference_credential_present)
    )
    if nastech_status.get("free_tier"):
        # Free tier: never rendered as an account login (no account ids, no refresh row).
        from nastech_cli.anon_auth import FREE_TIER_LABEL, GUEST_MODEL, UPGRADE_HINT
        _status._row("Nastech Portal", True, f"{FREE_TIER_LABEL} · {GUEST_MODEL}")
        _status._detail("", UPGRADE_HINT)
        inference_url = nastech_status.get("inference_base_url")
        if inference_url:
    ...
    _status._row("Nastech Portal", logged_in,
         "logged in" if logged_in else "not logged in (Nastech inference key configured)" if inference
         else "not logged in (run: nastech portal)")
    portal_url = nastech_status.get("portal_base_url") or "(unknown)"
    inference_url = nastech_status.get("inference_base_url") or (info.inference_base_url if info else None)
    for label, value, show in (
        ("Portal URL:", portal_url, logged_in or portal_url != "(unknown)" or nastech_error),
        ("Inference:", inference_url, inference and inference_url),
        ("Access exp:", _format_iso_timestamp(nastech_status.get("access_expires_at")),
         logged_in or nastech_status.get("access_expires_at")),
        ("Key exp:", _format_iso_timestamp(nastech_status.get("agent_key_expires_at")),
         logged_in or inference or nastech_status.get("agent_key_expires_at")),
        ("Refresh:", "yes" if nastech_status.get("has_refresh_token") else "no",
         logged_in or nastech_status.get("has_refresh_token")),
        ("Error:", nastech_error, nastech_error)):
        if show:
...
def _render_nastech_gateway(ctx):
    if managed_nastech_tools_enabled():
        features = get_nastech_subscription_features(ctx.config)
        _status._section("Nastech Tool Gateway")
        print("  Nastech Portal   ✓ managed tools available" if features.nastech_auth_present
              else "  Nastech Portal   ✗ not logged in")
        for f in features.items():
            state = next((text(f) for match, text in _FEATURE_STATES if match(f, features.nastech_auth_present)),
                         "not configured")
            _status._row(f.label, f.available or f.active or f.managed_by_nastech, state, 15, " ")
    elif ctx.nastech_logged_in or ctx.nastech_inference_present:
        # Nastech OAuth without entitlement, or an opaque inference key without Portal account
        # information, cannot enable the Tool Gateway.
        _status._section("Nastech Tool Gateway")
        message = format_nastech_portal_entitlement_message(
            ctx.nastech_account_info, capability="managed web, image, TTS, STT, browser, and Modal tools"
        )
...
def _render_apikey_providers(ctx):
    ...
    for pname, env_vars in _APIKEY_PROVIDERS.items():
        ...
        _status._row(pname, configured, "configured" if configured else "not configured (run: nastech model)", 16, " ")
    ...
    if _status._effective_provider_label() == "LM Studio":
        from nastech_cli.models_local import probe_lmstudio_models
...
import nastech_cli.status as _status  # noqa: E402  (bottom: nastech_cli.status imports this module)
