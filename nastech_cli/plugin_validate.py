"""``nastech plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``NASTECH_HOME``), so a crashing or
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "NASTECH_VALIDATE_JSON:"
...
def _requires_nastech_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_nastech`` spec.

    Unlike :func:`nastech_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from nastech_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_nastech(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_nastech") or "").strip()
    if not spec:
        report.add("requires_nastech", True, "not declared")
        return
    if _requires_nastech_spec_valid(spec):
        report.add("requires_nastech", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_nastech",
            False,
            f"requires_nastech spec {spec!r} does not parse "
...
# from nastech so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "nastech_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="nastech-validate-") as scratch:
        env = dict(os.environ)
        env["NASTECH_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_nastech(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that Nastech can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from nastech_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from nastech_cli.plugin_python_deps import applicable_specs, unsupported_specs
