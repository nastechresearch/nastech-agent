"""Argparse tree for ``nastech kanban …`` (``build_parser``).
...
def _triage_sweep_args(verb: str, Verb: str, noun: str):
    ...
    return (
        ...
        _arg("--author",
             help=f"Author name recorded on the audit comment (default: $NASTECH_PROFILE or '{noun}')"),
...
_BOARD_SPECS = [
    ...
    _cmd("import", [
        ...
        "Import a .tar.gz produced by `nastech kanban boards export`. The board always lands as a "
...
# Top-level ``nastech kanban <action>`` records, in ``--help`` order.
_SPECS = [
    ...
    _cmd("create", [
        ...
        _arg("--project",
             help="Link to a project (id or slug). Anchors the task's "
                  ...
                  "deterministic branch. See `nastech project list`."),
        _TENANT,
        _PRIORITY,
    ...
    _cmd("list", [
        _arg("--mine", action="store_true", help="Filter by $NASTECH_PROFILE as assignee"),
        _arg("--assignee"),
    ...
    _cmd("comment", [
        ...
        _arg("--author", help="Author name (default: $NASTECH_PROFILE or 'user')"),
    ...
    _cmd("attach", [
        ...
        _arg("--author", help="uploaded_by label (default: $NASTECH_PROFILE or 'user')"),
    ...
    _cmd("assignees", [_json_flag()],
         help="List known profiles + per-profile task counts (union of ~/.nastech/profiles/ and current assignees on the board)"),
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description="Durable SQLite-backed task board shared across Nastech profiles. "
                    "Tasks are claimed atomically, can depend on other tasks, and "
                    "are executed by a named profile in an isolated workspace. "
                    "See https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/kanban.",
    )
    # --board scopes every subcommand to one board's DB; when omitted the
    # resolution is NASTECH_KANBAN_BOARD, then the persisted current-board
    ...
    kanban_parser.add_argument("--board", default=None, metavar="<slug>",
                               help="Board slug to operate on. Defaults to the current board (set "
                                    "via `nastech kanban boards switch <slug>` or the "
                                    "NASTECH_KANBAN_BOARD env var). Use `nastech kanban boards "
