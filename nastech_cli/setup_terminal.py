...
setup.py names are resolved through the module object so test patches on ``nastech_cli.setup.<name>``
...
def _pip_install_vercel(package):
    """uv when Nastech has one ($NASTECH_HOME/bin is never on PATH, so which() misses it and
...
def _setup_backend_local(config: dict) -> None:
    ...
    # Gateway cwd defaults to home; sudo stays off. Both configurable via `nastech setup terminal`.
...
def _setup_backend_docker(config: dict) -> None:
    ...
    if _setup.prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
        ...
        _setup.print_info(
            "Run `nastech egress setup` then `nastech egress start` to mint tokens and launch the proxy.")
    else:
        _setup.print_info("Skipping egress firewall. You can enable it later with `nastech egress setup`.")
...
def _setup_backend_modal(config: dict) -> None:
    ...
    managed_modal_available = bool(
        tool_backend_helpers.managed_nastech_tools_enabled()
        and nastech_subscription.get_nastech_subscription_features(config).nastech_auth_present
    ...
    if managed_modal_available:
        ...
        use_managed_modal = _setup.prompt_choice(
            ...
            ["Use my Nastech subscription", "Use my own Modal account"], default_idx) == 0
    if use_managed_modal:
        config["terminal"]["modal_mode"] = "managed"
        _setup.print_info("Modal execution will use the managed Nastech gateway and bill to your subscription.")
...
def _setup_backend_vercel(config: dict) -> None:
    ...
    _setup._info("Cloud microVM sandboxes with snapshot-backed filesystem persistence.",
                 "Requires the optional SDK: pip install 'nastech-agent[vercel]'")
    _ensure_sdk("vercel", "pip install 'nastech-agent[vercel]'", show_stderr=True, install=_pip_install_vercel)
...
def setup_terminal_backend(config: dict):
    ...
    _setup._info("Choose where Nastech runs shell commands and code.",
    ...
    # Plugin-registered backends (~/.nastech/plugins/). Fail-soft: a broken plugin must not take
    # the wizard down.
