...
# Windows launcher self-heal — the ``nastech`` command users run is a COPY of
# the venv console script, staged into the managed binary dir (the default
# Nastech root's ``bin``, next to the managed uv) by install.ps1. That dir
# lives OUTSIDE the git checkout precisely because an earlier layout staged
# the copies at ``<checkout>\bin``, where ``nastech update``'s autostash
# (``git stash push --include-untracked``) swept them off disk; with the
# desktop updater's ``--keep-stash`` nothing restored them and ``nastech``
...
# ``python -m nastech_cli.main``. Costs a few stat calls when healthy; gates
# fail toward inaction so source checkouts are untouched. Sits AFTER the
# profile override on purpose — no nastech module may be imported before
# profiles resolve. The launcher dir itself is per-machine (the helper
# anchors on the DEFAULT root, not NASTECH_HOME), so profile sessions heal
# the same shared dir.
    ...
    *starts* is the set of workspace keys the launch install explicitly scopes
    ...
    while stack:
        ...
        if not isinstance(entry, dict):
            continue
        # Workspace symlink (e.g. node_modules/@nastech/ink → ui-tui/packages/…):
    ...
    if (
        ...
        def _run_tui_install() -> subprocess.CompletedProcess:
            ...
            return subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                env=_npm_lifecycle_env(with_nastech_node_path()),
            )
...
def cmd_chat(args):
    ...
    #
    # FIRST RUN is the exception: with an empty ~/.nastech/skills the banner
...
# surface so argparse wiring and test monkeypatches on nastech_cli.main.<name>
# keep resolving unchanged. Importing those modules eagerly costs ~50ms on
# every `nastech` invocation, including fast paths like `nastech --version`
...
def _self():
    ...
    keeps monkeypatches on nastech_cli.main.<name> working, exactly like a
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # `nastech --version` pre-import fast path (the `version` subcommand
...
# Update pipeline lives in nastech_cli/update_cmd.py (main.py decomposition,
...
# monkeypatches on nastech_cli.main.<name> keep resolving unchanged without
...
def _do_build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    if not npm:
        ...
        return not fatal
    build_env = _npm_lifecycle_env(with_nastech_node_path())
    ...
    # as `nastech update`'s _update_node_dependencies() (ui-tui + web +
...
def _renderer_bundle_dir(desktop_dir: Path, *, source_mode: bool) -> Optional[Path]:
    ...
    # macOS: …/Nastech.app/Contents/MacOS/Nastech → …/Contents/Resources
    resources = (
...
def _renderer_bundle_torn(dist_dir: Path) -> bool:
    ...
    the intact SOURCE tree, ``nastech desktop`` skips the rebuild that would fix
...
# unpacked`` (or the mac ``Nastech.app``) and the Electron unpack + asar + rename
...
# ``nastech update`` used to report "partially complete" over an empty
...
def _desktop_macos_setup_tcc_identity(identity: str = "Nastech Local Signing") -> bool:
    """Create/import a self-signed code-signing cert and configure Nastech to use it.

    One-shot setup for ``nastech desktop --setup-tcc-identity``. Creates a
    ...
    if not already_imported:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="nastech-tcc-"))
        try:
    ...
    # Point Nastech at the identity (config.yaml, not .env — it's not a secret).
    try:
    ...
    print(
        ...
        "stuck, reset it with:  tccutil reset All com.nastechresearch.nastech"
    )
    return True
...
def _detect_linux_password_store() -> str | None:
    ...
    fails under `nastech desktop` because the launcher environment doesn't look
...
def _desktop_launch_options() -> tuple[list[str], str, str, str]:
    ...
    is one of "auto"/"1"/"0" (normalized for the NASTECH_DESKTOP_DISABLE_GPU
...
def cmd_gui(args: argparse.Namespace):
    ...
    # `NASTECH_DESKTOP_DISABLE_GPU=... nastech desktop` and
    # `ELECTRON_OZONE_PLATFORM_HINT=... nastech desktop` keep working.
    ...
    # `NASTECH_DESKTOP_PASSWORD_STORE=... nastech desktop` keeps working.
    if sys.platform == "linux" and "NASTECH_DESKTOP_PASSWORD_STORE" not in os.environ:
        password_store = (
        ...
        if password_store:
            env["NASTECH_DESKTOP_PASSWORD_STORE"] = password_store
    ...
    if getattr(args, "setup_tcc_identity", False):
        identity = getattr(args, "identity", None) or "Nastech Local Signing"
...
# Dashboard process-hygiene helpers live in nastech_cli/dashboard_procs.py
...
# nastech_cli.main.<name> keep resolving unchanged.
...
def _recover_core_update_marker_locked() -> None:
    ...
    try:
        from nastech_cli import _install_repair as _ir
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\nastech.exe`` is a launcher that runs the interpreter with
    ...
    ``nastech ...`` command holds its own shim, and an editable install run
    ...
    ``nastech.exe`` belonging to some other install never matches.
    """
    if not _is_windows():
        return None
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _nastech_exe_shims(scripts_dir)}
    if not shims:
        return None
...
_UPDATE_REEXEC_ENV = "NASTECH_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    The child re-runs ``nastech update``, so the whole remaining flow — the
    ...
    cmd = [str(python_exe), "-m", "nastech_cli.main", *sys.argv[1:]]
...
def _venv_scripts_dir() -> Path | None:
    ...
    from nastech_constants import project_venv_dir, venv_bin_dir
...
def _quarantine_running_nastech_exe(
    ...
    2. If all retries fail, print a clear warning naming the most likely
       culprit (running Nastech Desktop / gateway / REPL).

    The updater's own launcher is no longer one of those culprits: an update
    started from ``nastech.exe`` re-runs itself under the venv Python before
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Nastech versions queued for our shims.

    Nastech used to fall back to ``MoveFileExW(MOVEFILE_DELAY_UNTIL_REBOOT)``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            kept, removed = _filter_pending_shim_renames(
                entries, _nastech_exe_shims(scripts_dir)
            )
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    ...
    update; a failed *restore* leaves the install with no ``nastech`` on PATH,
...
class ShimQuarantineError(RuntimeError):
    """A live ``nastech*.exe`` shim could not be renamed aside (#87331).
...
def _run_quarantined_install(
    ...
    *,
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_nastech_exe(scripts_dir, failed_out=failed)
    ...
    try:
        ...
        # quarantined aside and `nastech` missing from PATH after a green
...
def _quarantine_stamp_ms(stale: Path) -> int | None:
    ...
    :func:`_quarantine_running_nastech_exe`. We neither rescue nor delete those:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``nastech.exe.old.*`` from updates.

    Called early on every nastech invocation. Two cases the old unconditional
    ``unlink()`` got wrong, both ending with ``nastech`` gone from PATH:

    1. **Orphan rescue.** If ``nastech.exe`` is missing while
       ``nastech.exe.old.*`` is present, that .old file is the ONLY surviving
    ...
    2. **Concurrency.** A fresh quarantine file may belong to an update in
       ...
       ``nastech update`` does exactly this). Leave anything inside the grace
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    if (
        ...
        # Quarantining the wrong dir means the running nastech.exe stays locked
...
def cmd_update(args):
    ...
    if getattr(args, "plan", False):
        ...
        # Nastech runtime across profiles, its supervisor, and its running
...
def cmd_profile(args):
    ...
    if action is None:
        ...
        from nastech_cli.profiles import format_profile_label
    ...
    if action == "list":
        ...
        from nastech_cli.profiles import normalize_profile_name, rename_profile
...
def _report_dashboard_status() -> int:
    ...
    if not live:
        print("No nastech dashboard or serve processes running.")
        return 0

    print(f"{len(live)} nastech dashboard/serve process(es) running:")
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    provider on the spot — or point them at ``nastech dashboard register`` for
    OAuth.
    ...
    try:
        from nastech_cli.web_server import should_require_dashboard_auth
...
def cmd_dashboard(args):
    ...
    _mcp_discovery_after_bind = _headless_backend and os.environ.get("NASTECH_DESKTOP") == "1"
...
def _try_fast_serve_launch() -> bool:
    ...
    for unrelated Nastech commands performs thousands of filesystem-backed
...
def _try_fast_chat_launch() -> bool:
    ...
    ``nastech`` / ``nastech -w -s foo --yolo`` / ``nastech chat`` don't need the
...
def _cmd_skills_trust(args):
    """``nastech skills trust [path]`` / ``nastech skills untrust [path]``.
...
def _advertise_agent_env() -> None:
    ...
    (``nastech-agent`` in huggingface.js ``agent-harnesses.ts``): standard-var
    matching is exact, so any other value is counted as "unknown".
    ``NASTECH_AGENT`` is the Nastech-specific marker. setdefault: never
    clobber an outer harness (e.g. Nastech running inside another agent's
    terminal).
    """
...
def main():
    ...
    # never restarted the fleet. Do NOT restart here — that is ``nastech
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    worktree_parser = subparsers.add_parser(
        ...
        description=(
            "Attended reclaim for the .worktrees/ directory nastech -w sessions "
            "accumulate. Never deletes uncommitted tracked changes, unique "
            "unpushed commits, or in-use trees; untracked-only scratch is "
            "archived to ~/.nastech/archive/worktree-prune/ before removal. See: "
            "https://nastechresearch.github.io/nastech-agent/docs/user-guide/cli#worktree-cleanup"
        ),
    )
    ...
    def _dispatch_worktree(_args):
        from nastech_cli.worktree_cmd import cmd_worktree
    ...
    browser_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "default profile so Nastech can copy it — DESTRUCTIVE (unsaved tabs "
    ...
    # =========================================================================
    # peer command — bot-to-bot DMs across machines (peer Nastech gateways)
    # =========================================================================
    ...
    # subcommand — ``nastech --help``, ``nastech logs``,
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import os as _os
            import subprocess
            from nastech_cli.tools_config import _cua_driver_contract_status
            ...
            override = _os.environ.get("NASTECH_CUA_DRIVER_CMD", "").strip()
            if path:
                version = ""
                try:
                ...
                if not contract.get("ready"):
                    ...
                    if override:
                        print(
                            "    Update the binary selected by NASTECH_CUA_DRIVER_CMD, or unset "
                            "the override and run: nastech computer-use install --upgrade"
                        )
                    else:
    ...
    sessions_import = sessions_subparsers.add_parser(
        "import",
        help="Import a Claude Code or Codex CLI session into Nastech",
        description=(
            "Pull a conversation started in Claude Code (~/.claude/projects) "
            "or Codex CLI (~/.codex/sessions) into the Nastech session store "
            "so it can be resumed with 'nastech --resume <id>'. The foreign "
    ...
    # and lets monkeypatches on nastech_cli.main.cmd_sessions keep working.
    ...
    # NOTE: the `nastech version` subcommand was removed — `nastech --version`
