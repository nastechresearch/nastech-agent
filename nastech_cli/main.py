...
Usage:
    ...
    nastech chat / gateway / setup / status / cron / doctor / update / ...
    nastech --version           # Show version and update status
    nastech <cmd> --help        # Per-command help
"""

# nastech_bootstrap must be the very first import — it sets up UTF-8 stdio on
# Windows (no-op on POSIX). Guarded: after a ``git pull`` / interrupted
# ``nastech update`` the editable install's ``.pth`` may not list it yet; crashing
# here would block ``nastech update``.
try:
...
# Inline path math so ``python nastech_cli/main.py`` (script mode: sys.path[0]
# is nastech_cli/, not the repo root) can import nastech_cli._startup_fast.
...
# A literal ``~``/``$VAR`` in NASTECH_HOME (fish, or any quoted value) must become absolute
# before the first reader — otherwise it resolves against cwd and scaffolds <cwd>/~/.nastech.
_startup_fast.normalize_nastech_home_env()

# Early venv self-heal — MUST run before any third-party import below. A prior
# ``nastech update`` may have left a recovery marker with a core package wiped;
# the nastech_cli.config/env_loader imports further down would then crash before
...
# PID" class it exists for. ``nastech_startup_watchdog`` is stdlib-only so it
...
def _warn_if_unsupervised_pid1(pid: "int | None" = None) -> None:
    ...
    children). A Compose service that overrides ``entrypoint:`` to invoke nastech directly makes
    nastech itself PID 1 — nothing then ``wait()``s on those orphans and they accumulate as
    ...
    if (pid if pid is not None else os.getpid()) != 1:
        return
    print(
        "[nastech] WARNING: this process is PID 1 with no init above it "
...
def _set_process_title() -> None:
    """Cosmetic: show 'nastech' instead of 'python3.xx' in ps/top/htop.

    Order: opt-in ``setproctitle`` dep; ctypes ``prctl(PR_SET_NAME)`` (Linux,
    15-char limit); ``pthread_setname_np`` (macOS — lldb/top only, not ``ps
    aux``); no-op on Windows (the .exe is already ``nastech.exe``). Never fatal.
    """
    try:
...
# nastech_cli.config is importable. Cached so early callers don't re-parse YAML.
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    Precedence: ``--cli`` wins, then ``--tui``/``NASTECH_TUI=1``, then a
...
# `resetTerminalModes()` is the later cousin. ``NASTECH_TUI_NO_EARLY_DISABLE``
...
# ``nastech --version`` is answered before config/logging imports.
...
from nastech_cli.subcommands.whatsapp import build_whatsapp_parser, build_whatsapp_cloud_parser
...
from nastech_cli.subcommands.dashboard import build_dashboard_parser, build_serve_parser
...
from nastech_cli.subcommands.vault import build_vault_parser
from nastech_cli.subcommands.moa import build_moa_parser
from nastech_cli.subcommands.fallback import build_fallback_parser
from nastech_cli.subcommands.worktree import build_worktree_parser
from nastech_cli.subcommands.browser import build_browser_parser
from nastech_cli.subcommands.secrets import build_secrets_parser
from nastech_cli.subcommands.egress import build_egress_parser
from nastech_cli.subcommands.migrate import build_migrate_parser
from nastech_cli.subcommands.checkpoints import build_checkpoints_parser
from nastech_cli.subcommands.bundles import build_bundles_parser
from nastech_cli.subcommands.curator import build_curator_parser
from nastech_cli.subcommands.pets import build_pets_parser
from nastech_cli.subcommands.journey import build_journey_parser
from nastech_cli.subcommands.computer_use import build_computer_use_parser
from nastech_cli.subcommands.sessions import build_sessions_parser
from nastech_cli.subcommands.completion import build_completion_parser
...
# Profile override — MUST happen before any nastech module import: many modules
# cache NASTECH_HOME at import time. --profile/-p is pre-parsed from sys.argv,
# NASTECH_HOME set, and the flag stripped so argparse never sees it. Falls back
# to ~/.nastech/active_profile for the sticky default.
_PROFILE_NAME_RE = r"^[a-z0-9][a-z0-9_-]{0,63}$"  # mirrors nastech_cli.profiles._PROFILE_ID_RE
...
def _looks_like_nastech_invocation() -> bool:
    """False when ``sys.argv`` belongs to a test runner rather than a ``nastech`` run.

    pytest's own ``-p no:xdist`` reaches ``_scan_profile_flag`` through ``sys.argv`` at import
    time; it must stay a silent skip, while a real ``nastech -p 'Work Bot'`` must fail loudly.
    """
    return "pytest" not in (sys.argv[0] or "")


def _exit_invalid_profile_name(value: str) -> None:
    from nastech_cli.profiles import _invalid_profile_name_error

    print(f"Error: {_invalid_profile_name_error(value)}", file=sys.stderr)
    print("Run `nastech profile list` to see your profiles.", file=sys.stderr)
    sys.exit(2)
...
def _scan_profile_flag(argv: list) -> tuple:
    ...
    Historically the flag worked even after the subcommand (`nastech chat -p
    ...
    ``nastech`` run — after a subcommand, ``-p`` may belong to that subcommand or a
    plugin (`nastech kanban ... -p 8080`), and option-looking values (``no:xdist``,
    ``--flag``) are always a silent skip.
    """
    from nastech_cli._parser import top_level_value_flag_sets
    ...
    while i < len(argv):
        ...
        if arg in {"--profile", "-p"} and i + 1 < len(argv):
            ...
            if re.match(_PROFILE_NAME_RE, value):
                return value, 2, i
            if not saw_subcommand and not _looks_like_option_value(raw) and _looks_like_nastech_invocation():
...
def _resolve_sudo_user_profile_env(name: str) -> str | None:
    ...
    if name == "default":
        return None
    from nastech_constants import named_profile_is_live, sudo_invoker_default_home
...
def _under_gateway_supervisor(argv: list) -> bool:
    ...
    ``-p <name>`` or pin NASTECH_HOME to the profile dir; a bare invocation
    means "the root NASTECH_HOME profile". If a supervised default-profile
    child read active_profile, switching the active profile (dashboard,
    ``nastech profile use``) would silently redirect the default gateway into
    ...
    NASTECH_SUPERVISED_CHILD (systemd unit / launchd plist / Windows task),
    NASTECH_S6_SUPERVISED_CHILD (legacy s6 container), INVOCATION_ID (systemd
    ...
    self-hosted CI runners), NASTECH_GATEWAY_EXTERNAL_SUPERVISOR (explicit
    opt-in). XPC_SERVICE_NAME is deliberately NOT consulted: interactive macOS
    terminals set it too.
    """
    if os.environ.get("NASTECH_SUPERVISED_CHILD") or os.environ.get("NASTECH_S6_SUPERVISED_CHILD"):
        return True
...
def _apply_profile_override() -> None:
    ...
    # NASTECH_HOME already set with no explicit flag: trust it only when it
    # points at a specific profile dir ("profiles" as immediate parent). If it
    # points at the nastech root (systemd hardcodes NASTECH_HOME=/root/.nastech)
    # we must still read active_profile — the user may have run
    # `nastech profile use` and the gateway should honour it (#22502).
    nastech_home_env = os.environ.get("NASTECH_HOME", "")
    if profile_name is None and nastech_home_env and Path(nastech_home_env).parent.name == "profiles":
        return
    ...
    if profile_name is None and nastech_home_env and os.environ.get("NASTECH_UPDATE_POST_SWAP") == "1":
        return
...
# Windows launcher self-heal — the ``nastech`` command is a COPY of the venv
# console script staged into the managed bin dir (outside the checkout, since
# ``nastech update``'s autostash once swept ``<checkout>\bin`` copies off disk;
...
# app's ``python -m nastech_cli.main`` spawn. Gates fail toward inaction. Sits
# AFTER the profile override on purpose — no nastech module may import before
...
# ``<checkout>\bin``, where ``nastech update``'s autostash (``git stash push --include-untracked``) swept
# them off disk; with the desktop updater's ``--keep-stash`` nothing restored them and ``nastech`` stopped
...
if sys.platform == "win32":
    try:
        from nastech_cli import _install_repair as _install_repair_mod
...
# Bridge security.redact_secrets → NASTECH_REDACT_SECRETS BEFORE nastech_logging
...
try:
    ...
    # serves nastech_logging, nastech_time and later raw reads: 3-4 config.yaml parses become one.
    # Managed overlay included: administrator-pinned redact_secrets / force_ipv4 win here too.
    from nastech_cli.config_effective import load_user_config_effective as _load_effective_early
...
from nastech_cli.main_agent_cmds import (
    cmd_acp,
    cmd_insights,
    cmd_memory,
    cmd_monitoring,
...
from nastech_cli.main_platform_setup import (
    cmd_slack,
    cmd_sync,
    cmd_whatsapp,
    cmd_whatsapp_cloud,
)
from nastech_cli.main_dashboard import (
...
from nastech_cli.main_dashboard import (  # frozen updater surface: update_cmd*.py resolve these via _m()
...
from nastech_cli.main_install_repair import (
    _cleanup_quarantined_exes,
    _recover_from_interrupted_install,
)
from nastech_cli.main_install_repair import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    ShimQuarantineError,
    _UPDATE_REEXEC_ENV,
...
from nastech_cli.main_desktop import (
    cmd_gui,
)
from nastech_cli.main_desktop import (  # frozen updater surface: update_cmd*.py resolve these via _m()
...
from nastech_cli.main_web_build import (
    _sweep_stale_bytecode_if_checkout_changed,
)
from nastech_cli.main_web_build import (  # frozen updater surface: update_cmd*.py resolve these via _m()
    _build_web_ui,
    _nixos_build_env,
    _record_bytecode_fingerprint,
    _run_npm_install_deterministic,
)
from nastech_cli.main_tui_launch import (
    _launch_tui,
...
def _dotenv_has_provider_key(env_file: Path, provider_env_vars: set) -> bool:
    """True if ~/.nastech/.env assigns a non-empty value to any provider key."""
...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    """Check if at least one inference provider is usable. Never creates one: the Nastech free tier
    counts only once its identity exists, and the boot bootstrap (``nastech_cli.free_tier_bootstrap``)
    ...
    """
    from nastech_cli.config import DEFAULT_CONFIG, get_env_path, get_nastech_home, load_config
    ...
    if isinstance(model_cfg, dict):
        ...
        if isinstance(_model_name, dict):
            from nastech_cli.config import split_model_config_default
    ...
    _has_nastech_config = _model_name and _model_name != DEFAULT_CONFIG.get("model", "")
    ...
    if _auth_store_logged_in(get_nastech_home() / "auth.json", PROVIDER_REGISTRY, strict_profile_scope):
        return True
    ...
    # Claude Code OAuth credentials count only once Nastech is explicitly
    # configured — having Claude Code installed isn't consent to use its tokens.
    if _has_nastech_config and not strict_profile_scope:
        try:
    ...
    # Nothing explicit anywhere: an existing Nastech free-tier identity counts while the tier is on.
    try:
        from nastech_cli.anon_auth import guest_enabled, has_guest
...
def _confirm_startup_expensive_model_override(args) -> None:
    ...
    try:
        ...
        from nastech_cli.model_selection_guards import (
            combined_message,
            selection_warnings,
        )
    except Exception as exc:
...
def _session_db():
    """Yield a read-only ``SessionDB`` (lazy import, so test patches on ``nastech_state``
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    """
    # A finite `nastech -z`/`chat -q` run is CLI history too: `nastech -z … --resume latest` chains on it.
    if source == "cli":
...
def _create_titled_session(title: str) -> Optional[str]:
    ...
    db = None
    try:
        from nastech_state_ids import new_session_id as mint_session_id
        from nastech_state_registry import acquire
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(
                    "Use 'nastech sessions list' to see available sessions, or "
            ...
            try:
                from nastech_cli.terminal_breadcrumbs import resolve_breadcrumb_session
            ...
            if _crumb_id:
                ...
                if last_id:
                    ...
                    print(
                        ...
                        "`nastech`, or list sessions with `nastech sessions list`.",
                        file=sys.stderr,
                    )
                    sys.exit(1)
...
def _import_foreign_resume(args) -> None:
    ...
    if not (isinstance(_resume_foreign, str) and _resume_foreign.strip().lower() in ("@claude", "@codex")):
        return
    from nastech_cli.foreign_sessions import import_foreign_session, pick_foreign_session
    ...
    print(f"  (later: nastech --resume {_imported_id})")
...
def _start_chat_background_prefetch() -> None:
    ...
    The ONE exception is an unseeded ~/.nastech/skills: there the banner
    ...
    if _termux_should_prefetch_update_check():
        try:
            from nastech_cli.banner import prefetch_banner_data, prefetch_update_check
    ...
    def _skills_dir_is_unseeded() -> bool:
        try:
            from nastech_cli.config import get_nastech_home
            skills_dir = Path(get_nastech_home()) / "skills"
    ...
    if _skills_dir_is_unseeded():
        ...
        try:
            import nastech_cli.banner as _banner_mod
...
def _first_run_setup_guard(args) -> None:
    """No provider configured: offer `nastech setup` (TTY) or exit 1 with guidance."""
    print()
    print(
...
def cmd_chat(args):
    ...
    from nastech_cli.stream_json import stream_json_requested
    # Structured stdout is a non-interactive protocol: it overrides NASTECH_TUI/display.interface too.
    ...
    from nastech_cli.free_tier_bootstrap import run_bootstrap
    ...
    if getattr(args, "source", None):
        ...
        os.environ["NASTECH_SESSION_SOURCE_EXPLICIT"] = "1"
...
def _forward_command(name: str, module: str, attr: str, *, forward_return: bool = False, doc: str = ""):
    """A ``nastech <cmd>`` handler that hands ``args`` to ``<module>.<attr>``.
...
cmd_setup = _forward_command("cmd_setup", "nastech_cli.setup", "run_setup_wizard", doc='Interactive setup wizard.')
cmd_login = _forward_command("cmd_login", "nastech_cli.auth", "login_command", doc='Authenticate Nastech CLI with a provider.')
cmd_logout = _forward_command("cmd_logout", "nastech_cli.auth", "logout_command", doc='Clear provider authentication.')
cmd_auth = _forward_command("cmd_auth", "nastech_cli.auth_commands", "auth_command", doc='Manage pooled credentials.')
cmd_status = _forward_command("cmd_status", "nastech_cli.status", "show_status", doc='Show status of all components.')
cmd_cron = _forward_command("cmd_cron", "nastech_cli.cron", "cron_command", forward_return=True, doc='Cron job management.')
cmd_webhook = _forward_command("cmd_webhook", "nastech_cli.webhook", "webhook_command", doc='Webhook subscription management.')
cmd_kanban = _forward_command("cmd_kanban", "nastech_cli.kanban", "kanban_command", forward_return=True, doc='Multi-profile collaboration board.')
cmd_project = _forward_command("cmd_project", "nastech_cli.projects_cmd", "projects_command", forward_return=True, doc='Manage projects (named, multi-folder workspaces).')
cmd_hooks = _forward_command("cmd_hooks", "nastech_cli.hooks", "hooks_command", doc='Shell-hook inspection and management.')
cmd_doctor = _forward_command("cmd_doctor", "nastech_cli.doctor", "run_doctor", doc='Check configuration and dependencies.')
cmd_dump = _forward_command("cmd_dump", "nastech_cli.dump", "run_dump", doc='Dump setup summary for support/debugging.')
cmd_debug = _forward_command("cmd_debug", "nastech_cli.debug", "run_debug", doc='Debug tools (share report, etc.).')
cmd_skin = _forward_command("cmd_skin", "nastech_cli.skin_cmd", "skin_command", doc='Skin management (list / use / set).')
cmd_import = _forward_command("cmd_import", "nastech_cli.backup", "run_import", doc='Restore a Nastech backup from a zip file.')
cmd_dashboard_register = _forward_command("cmd_dashboard_register", "nastech_cli.dashboard_register", "cmd_dashboard_register", doc='Register a self-hosted dashboard OAuth client with Nastech Portal.')
cmd_gateway_enroll = _forward_command("cmd_gateway_enroll", "nastech_cli.gateway_enroll", "cmd_gateway_enroll", doc='Enroll a self-hosted gateway with a relay connector.')
cmd_prompt_size = _forward_command("cmd_prompt_size", "nastech_cli.prompt_size", "cmd_prompt_size", doc='Show a byte/char breakdown of the system prompt + tool schemas.')
cmd_pairing = _forward_command("cmd_pairing", "nastech_cli.pairing", "pairing_command")
cmd_plugins = _forward_command("cmd_plugins", "nastech_cli.plugins_cmd", "plugins_command")
cmd_mcp = _forward_command("cmd_mcp", "nastech_cli.mcp_config", "mcp_command", forward_return=True)
cmd_claw = _forward_command("cmd_claw", "nastech_cli.claw", "claw_command")
cmd_import_agent = _forward_command("cmd_import_agent", "nastech_cli.agent_import", "import_agent_command")
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            ...
            pass
    from nastech_cli.setup import run_setup_action_with_navigation
...
_PROVIDER_MODEL_FLOWS = {
    ...
    "nastech": lambda c, m, a: _model_flow_nastech(c, m, args=a),
...
def _resolve_active_provider(config, model_cfg, effective_provider, custom_provider_map):
    ...
    """
    from nastech_cli.auth import AuthError, format_auth_error, resolve_provider
    from nastech_cli.config import get_compatible_custom_providers, get_env_value
    from nastech_cli.providers import custom_provider_aliases, resolve_provider_full
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            print(
                f"Warning: Unknown provider '{effective_provider}'. Check 'nastech model' for "
    ...
    if not active:
        try:
            ...
            if exc.code == "no_provider_configured":
                ...
                # "run `nastech model`" from inside `nastech model` is circular.
                print("No provider is set up yet — pick one below. (Nastech Portal works without an API key.)")
...
def select_provider_and_model(args=None):
    ...
    effective_provider = config_provider or os.getenv("NASTECH_INFERENCE_PROVIDER") or "auto"
    ...
    # nastech_cli.main keep intercepting.
    ...
    # "custom") leaves a stale OPENAI_BASE_URL in ~/.nastech/.env that poisons
...
# ``nastech_cli/update_cmd*.py`` files resolve these names via ``_m().<name>``
# on nastech_cli.main; importing update_cmd eagerly would cost every ``nastech``
...
_FROZEN_UPDATER_SURFACE: dict[str, tuple[str, ...]] = {
    ...
    "nastech_cli.dashboard_procs": (
        "_detect_concurrent_nastech_instances", "_kill_stale_dashboard_processes",
    ),
}
...
def _print_version_info(*, check_updates: bool = True) -> None:
    # Shared with the `nastech --version` pre-import fast path.
...
def _finalize_update_receipt(code: int, reason: str) -> None:
    ...
    try:
        ...
        from nastech_cli.update_receipt import finalize_pending_update_receipt
...
def _update_preflight_handled(args) -> bool:
    ...
    if getattr(args, "plan", False):
        # Read-only plan phase (#91277 Phase 2): inventory every running Nastech runtime across profiles, its
        # supervisor, and its running code version — without mutating anything. Safe on a live fleet.
        from nastech_cli.update_inventory import (
...
def cmd_update(args):
    """Update Nastech Agent: hangup protection + update lock around ``_cmd_update_impl``."""
    ...
    from nastech_cli.update_cmd import _cmd_update_impl
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    ``nastech -c Pokemon Agent Dev`` → ``['-c', 'Pokemon Agent Dev']``; tokens
...
from nastech_cli.profile_cmd import cmd_profile
...
def _dashboard_lifecycle_flags(args, token_file) -> None:
    ...
    if getattr(args, "stop", False):
        ...
        own_home = str(get_nastech_home())
        if not _find_stale_dashboard_pids(scope_home=own_home):
            print("No nastech dashboard processes running for this profile.")
            sys.exit(0)
        ...
        from nastech_cli.dashboard_procs import _kill_stale_dashboard_processes
...
def _dashboard_validate_serve_args(args, headless_backend, token_file):
    ...
    # `nastech serve` is headless/non-interactive: fail closed on a corrupt
...
def _dashboard_sanitize_desktop_env(headless_backend) -> None:
    ...
    NASTECH_SERVE_HEADLESS=1). A shell inheriting those then running
    `nastech dashboard` would serve the desktop renderer ("Desktop IPC bridge
    ...
    (NASTECH_DESKTOP=1) keeps its dist. Headless `serve` re-sets
    NASTECH_SERVE_HEADLESS itself.
    """
    if os.environ.get("NASTECH_DESKTOP") != "1":
        if _is_electron_packaged_web_dist(os.environ.get("NASTECH_WEB_DIST", "")):
...
def _dashboard_prepare_runtime(args, headless_backend) -> bool:
    ...
    try:
        ...
        from nastech_cli.main_dep_hints import missing_optional_deps_message
    ...
    mcp_discovery_after_bind = headless_backend and os.environ.get("NASTECH_DESKTOP") == "1"
...
def cmd_completion(args, parser=None):
    ...
    from nastech_cli import completion
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    """
    first = _first_positional_argv()  # None = bare ``nastech`` → chat
...
def _resolve_deferred_platform_cli_command(command_name: str | None) -> None:
    ...
    on import; ``discover_plugins()`` alone leaves ``nastech photon`` failing
    ...
    not import it, so the CLI registration never happens and ``nastech photon`` fails with argparse ``invalid
    choice`` (issue #54678).
    """
    if not command_name:
        return
...
def _is_tui_chat_launch(args) -> bool:
    ...
    # flags, TTY gate, NASTECH_TUI env, display.interface config). Bare
    # `nastech`/`nastech chat` with a TUI display config was previously missed
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo chokepoint: NASTECH_YOLO_MODE must be set before any discovery
    ...
    if not _is_tui_chat_launch(args):
        ...
        try:
            from nastech_cli.plugins import start_background_plugin_discovery
    ...
    try:
        from nastech_cli.mcp_startup import set_mcp_server_filter
    ...
    if _run_inline_mcp_discovery:
        try:  # synchronous for entrypoints without a later bounded startup path
            from nastech_cli.mcp_startup import get_mcp_server_filter
...
def _try_fast_serve_launch() -> bool:
    ...
    """
    if os.environ.get("NASTECH_DISABLE_FAST_SERVE_LAUNCH") == "1":
        return False
...
def _try_fast_chat_launch() -> bool:
    ...
    """
    if os.environ.get("NASTECH_DISABLE_FAST_CHAT_LAUNCH") == "1":
        return False
    argv = sys.argv[1:]
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `nastech --tui` is the hot path on phones and the TUI immediately execs
...
def _advertise_agent_env() -> None:
    ...
    (``nastech-agent``) — matching is exact. ``NASTECH_AGENT`` is the
    Nastech-specific marker. setdefault: never clobber an outer harness.
    ...
    harness that spawned them. Nastech running inside another agent's terminal).
    """
    os.environ.setdefault("AI_AGENT", "nastech-agent")
    os.environ.setdefault("NASTECH_AGENT", "true")
...
def _cmd_sessions_lazy(args, **kwargs):
    """``nastech sessions`` handler; sessions_cmd imports only when the subcommand runs."""
    from nastech_cli.sessions_cmd import cmd_sessions

    return cmd_sessions(args, **kwargs)


def _build_cli_parser():
    """Build the full ``nastech`` argparse tree -> ``(parser, subparsers)``.

    Registration ORDER is the ``nastech --help`` order; keep it stable. Groups
    live in ``nastech_cli/subcommands/<group>.py`` with handlers injected so
    ...
    # OUTBOUND egress firewall; ``nastech proxy`` (gateway group) is the INBOUND one.
    ...
    from nastech_cli.subcommands.peer import build_peer_parser
    ...
    # "desktop" is canonical (Nastech-Setup.exe tells users to run it, so it
...
def _parse_cli_args(parser, subparsers, argv):
    ...
    routing; if that fails (``nastech -c model`` — 'model' is the session name)
...
def main():
    ...
    # Sweep stale ``nastech.exe.old.*`` quarantine files from previous Windows
    # updates (see ``_quarantine_running_nastech_exe``). No-op elsewhere.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass
    ...
    # Self-heal a venv left half-built by an interrupted ``nastech update``, and
    ...
    # substring match is deliberately loose: over-matching (``nastech skills
    # install update``) only defers recovery one launch; under-matching
    # (``nastech -p work update``) would race. Never raises.
    # See #95294.
    if "update" not in sys.argv[1:]:
        try:
            ...
            pass
        try:
            from nastech_cli.update_cmd_fleet import _warn_pending_fleet_restart_on_startup
...
_PLUGIN_COMPAT_LAZY = {
    'line_input': ('nastech_cli.cli_output', 'line_input'),
}
