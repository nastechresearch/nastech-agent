"""Serve-start bootstrap for the Nastech free tier: the ONE place a free-tier identity is created.

Every Nastech process that may need the free tier runs this once at boot (``nastech serve`` on a
...
(:func:`nastech_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
``auth_nastech.resolve_nastech_runtime_credentials``. Ruling: NS-845 Q1.2 (recorded on NS-847).
"""
...
class SetupRecord:
    ...
    has_identity: bool             # a Nastech identity (free tier or account) is on disk
...
def reconcile_record() -> Optional[SetupRecord]:
    ...
    writes this process never saw (``nastech setup`` / ``nastech model`` from a shell, a hand edit).
    ...
    if record is None or record.provider_configured:
        return record
    from nastech_constants import get_process_nastech_home, nastech_home_key
    if nastech_home_key() != nastech_home_key(get_process_nastech_home()) or _inventory_stamp == _config_stamp():
        return record
...
def _inventory_other_providers() -> bool:
    ...
    try:
        return resolve_provider("auto", skip_free_tier=True) != "nastech"
...
def _build_record(*, other: bool, force: bool) -> SetupRecord:
    ...
    state: Optional[Dict[str, Any]] = anon_auth.current_nastech_state()
    if anon_auth.guest_enabled():
        try:
            ...
            error = str(exc)
            logger.info("Nastech free tier not set up at boot: %s", exc)
        if state is None:
...
def _retry_until_settled() -> None:
    ...
    for _ in range(BOOTSTRAP_RETRY_ATTEMPTS):
        ...
        if record.has_identity:
            logger.info("Nastech free tier set up after a boot-time retry")
            return
...
def start_background_bootstrap() -> threading.Thread:
    """``nastech serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
