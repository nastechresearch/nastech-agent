...
cycle and lets tests patch ``nastech_cli.config.load_config`` etc. at call time. The shared skeleton
lives in :mod:`nastech_cli.model_setup_flows_common`; the custom / Azure / Bedrock flows live in their
...
from nastech_cli.model_setup_flows_custom import _model_flow_custom, _model_flow_named_custom
from nastech_cli.model_setup_flows_azure import _model_flow_azure_foundry
from nastech_cli.model_setup_flows_bedrock import _model_flow_bedrock
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from nastech_cli.auth import ProviderConfig, _prompt_model_selection
    ...
    from nastech_cli.models import model_ids
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from nastech_cli.main_provider_setup import _prompt_api_key
    from nastech_cli.auth import PROVIDER_REGISTRY, _prompt_model_selection
    from nastech_cli.config import get_env_value
    ...
    from nastech_cli.models import ai_gateway_model_ids
    from nastech_cli.models_pricing import get_pricing_for_provider
...
def _model_flow_moa(config, current_model=""):
    ...
    if not presets:
        print("No MoA presets configured. Run `nastech moa configure <name>` first.")
        return
...
def _nastech_login_args(args) -> argparse.Namespace:
...
def _nastech_model_catalog(free_tier: bool, portal_url: str, model_ids: list, pricing: dict):
    """Free/paid-tier catalog for the Nastech picker: ``(model_ids, pricing, unavailable_models,
    ...
    if free_tier:
        try:
            from nastech_cli.nastech_account import format_nastech_portal_entitlement_message, get_nastech_portal_account_info
            _account_info = get_nastech_portal_account_info(force_fresh=True)
            unavailable_message = format_nastech_portal_entitlement_message(_account_info, capability="paid Nastech models") or ""
        except Exception:
            unavailable_message = ""
    ...
    if free_tier:
        model_ids, unavailable_models = partition_nastech_models_by_tier(model_ids, pricing, free_tier=True)

    if not model_ids and not unavailable_models:
        print("No models available for Nastech Portal after filtering.")
        return None
    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from nastech_cli.auth import DEFAULT_NASTECH_PORTAL_URL
            _url = (portal_url or DEFAULT_NASTECH_PORTAL_URL).rstrip("/")
            print(unavailable_message or f"Upgrade at {_url} to access paid models.")
        return None
    return model_ids, pricing, unavailable_models, unavailable_message, _policy_narrowed


def _nastech_verified_credentials(creds_or_none=None):
    """Resolve Nastech runtime credentials; on failure print the diagnosis (re-login when the
    session expired) and return None."""
    from nastech_cli.auth import (
        AuthError, PROVIDER_REGISTRY, _login_nastech, format_auth_error, resolve_nastech_runtime_credentials)

    try:
        return resolve_nastech_runtime_credentials()
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            _say(f"Session expired: {msg}", "Re-authenticating with Nastech Portal...\n")
            try:
                _login_nastech(_nastech_login_args(None), PROVIDER_REGISTRY["nastech"])
...
def _nastech_persist_selection(selected: str, creds: dict) -> dict:
    """Nastech persist step: model choice + provider state, then rewrite ``model`` on a fresh
    ...
    from nastech_cli.auth import _save_model_choice, _update_config_for_provider
    from nastech_cli.config import get_env_value, load_config, save_config, save_env_value
    _save_model_choice(selected)
    inference_url = creds.get("base_url", "")
    _update_config_for_provider("nastech", inference_url)
    config = load_config()
...
def _model_flow_nastech(config, current_model="", args=None):
    """Nastech Portal provider: ensure logged in, then pick model."""
    from nastech_cli.auth import get_provider_auth_state, _prompt_model_selection, _login_nastech, PROVIDER_REGISTRY
    ...
    if not state or not state.get("access_token"):
        _say("Not logged into Nastech Portal. Starting login...", "")

        def _login_then_offer_gateway(login_args, pconfig):
            _login_nastech(login_args, pconfig)
            # Offer Tool Gateway enablement for paid subscribers
            with contextlib.suppress(Exception):
                prompt_enable_tool_gateway(load_config() or {})

        # login_nastech already handles model selection + config update
        _run_login(_login_then_offer_gateway, _nastech_login_args(args), PROVIDER_REGISTRY["nastech"])
        return
    ...
    from nastech_cli.models import check_nastech_free_tier, get_curated_nastech_model_ids
    from nastech_cli.models_pricing import get_pricing_for_provider
    from nastech_cli.model_switch_providers import _free_tier_nastech_row
    tier_row = _free_tier_nastech_row({"name": "Nastech Portal", "models": []})
    if tier_row is None:
        print("The Nastech free tier is off for this install; sign in with `nastech auth upgrade` to use Nastech models.")
        return
    if tier_row["models"]:
        # Free-tier identity: the welcome host serves the single pinned model; no Portal catalog,
        # pricing, or account lookups apply.
        creds = _nastech_verified_credentials()
        if creds is None:
            return
        selected = tier_row["models"][0]
        _nastech_persist_selection(selected, creds)
    ...
    if not free_tier:
        ...
        try:
            creds = resolve_nastech_runtime_credentials(force_refresh=True) or creds
        except Exception:
    ...
    _nastech_portal_url = ""
    with contextlib.suppress(Exception):
        _nastech_portal_url = (get_provider_auth_state("nastech") or {}).get("portal_base_url", "")

    catalog = _nastech_model_catalog(free_tier, _nastech_portal_url, model_ids, pricing)
    if catalog is None:
        return
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_nastech_portal_url, unavailable_message=unavailable_message, confirm_provider="nastech",
        confirm_base_url=creds.get("base_url", ""), confirm_api_key=creds.get("api_key", ""))
    if not selected:
        print("No change.")
        return
    config = _nastech_persist_selection(selected, creds)
    print(f"Default model set to: {selected} (via Nastech Portal)")
    # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    # Prefer the credential pool (where `nastech auth` stores device_code tokens),
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # credentials may live only in the pool (``nastech auth add xai-oauth``) — fall back to
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from nastech_cli.main_provider_setup import _DEFAULT_QWEN_PORTAL_MODELS
    from nastech_cli.auth import (
...
def _copilot_catalog(api_key: str):
    ...
    from nastech_cli.models import fetch_github_model_catalog, normalize_copilot_model_id
...
def _model_flow_copilot(config, current_model=""):
    ...
    from nastech_cli.models import fetch_api_models, copilot_model_api_mode
    provider_id = "copilot"
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    _say("  GitHub Copilot ACP delegates Nastech turns to `copilot --acp`.",
         "  Nastech currently starts its own ACP subprocess for each request.",
         "  Nastech uses your selected model as a hint for the Copilot ACP session.",
         f"  Command: {resolved_command}", f"  Backend marker: {effective_base}", "")
    try:
        creds = resolve_external_process_provider_credentials(provider_id)
    except Exception as exc:
        _say(f"  ⚠ {exc}", "  Set NASTECH_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from nastech_cli.auth import PROVIDER_REGISTRY, KIMI_CODE_BASE_URL
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from nastech_cli.main_provider_setup import _infer_stepfun_region, _prompt_provider_choice, _stepfun_base_url_for_region
...
def _model_flow_vertex(config, current_model=""):
    ...
    if sa_path:
        ...
        _say("  Vertex credentials: Application Default Credentials (ADC)",
             ...
             "      • set VERTEX_CREDENTIALS_PATH in ~/.nastech/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    """Picker for the official Z.AI endpoints (``ZAI_ENDPOINTS`` in ``nastech_cli.auth``, kept in
    sync with the probe list) plus a custom-proxy option. Returns the selected base URL;
    *current_base* on cancel/error."""
    from nastech_cli.main_provider_setup import _prompt_provider_choice
...
_GEMINI_FREE_TIER_NOTICE = (
    ...
    "   Nastech typically makes 3-10 API calls per user turn (tool iterations + auxiliary tasks),",
    "   so the free tier is exhausted after a handful of messages and cannot sustain",
    "   an agent session.", "",
    "   To use Gemini with Nastech, enable billing on your Google Cloud project and regenerate",
    "   the key in a billing-enabled project: https://aistudio.google.com/apikey", "",
    "   Alternatives with workable free usage: DeepSeek, OpenRouter (free models), Groq, Nastech.", "",
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from nastech_cli.auth import PROVIDER_REGISTRY
    from nastech_cli.config import save_env_value, load_config
    from nastech_cli.models import opencode_model_api_mode, normalize_opencode_model_id
    pconfig = PROVIDER_REGISTRY[provider_id]
...
def _anthropic_authenticate() -> bool:
    ...
    from nastech_cli.main_provider_setup import _run_anthropic_oauth_flow
    from nastech_cli.config import save_env_value, save_anthropic_api_key
...
_PLUGIN_COMPAT_LAZY = {
    'BEDROCK_GEO_PREFIXES': ('nastech_cli.model_setup_flows_bedrock', 'BEDROCK_GEO_PREFIXES'),
    'bedrock_model_routable_from_region': ('nastech_cli.model_setup_flows_bedrock', 'bedrock_model_routable_from_region'),
    'bedrock_region_geo_prefix': ('nastech_cli.model_setup_flows_bedrock', 'bedrock_region_geo_prefix'),
