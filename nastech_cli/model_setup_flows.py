...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.nastech/.env by hand.
    ...
    from nastech_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nastech(config, current_model="", args=None):
    ...
    if not state or not state.get("access_token"):
        print("Not logged into Nastech Portal. Starting login...")
        print()
        try:
            ...
            _login_nastech(mock_args, PROVIDER_REGISTRY["nastech"])
    ...
    try:
        ...
        if relogin:
            ...
            print("Re-authenticating with Nastech Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    _nastech_portal_url = ""
    try:
        _nastech_state = get_provider_auth_state("nastech")
        if _nastech_state:
    ...
    if selected:
        _save_model_choice(selected)
        # Reactivate Nastech as the provider and update config
...
def _model_flow_named_custom(config, provider_info):
    ...
    from nastech_cli.auth import _save_model_choice, deactivate_provider
    from nastech_cli.config import load_config, normalize_extra_headers, save_config
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    # Check env var and credential pool (keys added via `nastech auth`)
    from nastech_cli.auth import _resolve_api_key_provider_secret, ProviderConfig
...
def _model_flow_vertex(config, current_model=""):
    ...
    *path* lives in .env (VERTEX_CREDENTIALS_PATH / GOOGLE_APPLICATION_CREDENTIALS);
    ...
    from nastech_cli.config import load_config, save_config, get_env_value
    from nastech_cli.models import _PROVIDER_MODELS
    ...
    if sa_path:
        ...
        print("      • set VERTEX_CREDENTIALS_PATH in ~/.nastech/.env to a service account JSON")
    print()
...
def _select_zai_endpoint(current_base: str) -> str:
    ...
    sourced from ``ZAI_ENDPOINTS`` in ``nastech_cli.auth`` so it stays in
    sync with the probe list.

    Returns the selected base URL.  Falls back to *current_base* on cancel
    or error.
    """
    from nastech_cli.main import _prompt_provider_choice
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Nastech."
                )
                print()
