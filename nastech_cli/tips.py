...
TIPS = [
    ...
    "nastech chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    ...
    "/plan writes a markdown implementation plan to .nastech/plans/ without executing anything.",
    ...
    # --- MCP ---
    "nastech mcp opens an interactive picker of Nastech-approved MCPs you can install in one keystroke.",
    "nastech mcp catalog lists Nastech-approved MCP servers shipped with the repo.",
    ...
    "nastech auth add nastech or nastech auth add openai-codex sets up OAuth-based providers.",
    ...
    "Profiles scope Nastech state via NASTECH_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Skills from trusted repos (NastechResearch) get a 'trusted' security level; community skills get extra scanning.",
    ...
    'use_gateway: true per-tool routes web, image, tts, or browser through your Nastech subscription — no extra keys.',
    ...
    'NASTECH_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs concise/all/error/off).',
    'NASTECH_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
...
# any project or none — Nastech is not a coding-only agent.
# ---------------------------------------------------------------------------
