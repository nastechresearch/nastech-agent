...
def _subscriptions_path() -> Path:
    ...
    return get_nastech_home() / _SUBSCRIPTIONS_FILENAME
...
def _cmd_subscribe(args):
    ...
    if profile_arg is None:
        ...
        from nastech_cli.profiles import normalize_profile_name, profile_exists, validate_profile_name
        try:
    ...
    if cron_job:
        ...
        try:
            ...
            return
        if job is None:
            print(f"Error: no cron job matches '{cron_job}'. List jobs with: nastech cron list")
            return
    ...
    print("  The gateway must be running to receive events (nastech gateway run).\n")
