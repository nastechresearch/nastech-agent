...
The provider list shown by ``nastech model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`nastech_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``nastech model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("nastech_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("nastech_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    NASTECH_OVERLAYS = _safe_import("nastech_cli.providers", "NASTECH_OVERLAYS", {})
    try:
