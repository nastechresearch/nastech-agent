"""CLI subcommand: ``nastech send`` — pipe text from shell scripts to any configured messaging platform
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                ...
                f'  nastech send --to telegram "MEDIA:{file_path}"\n'
                f'  nastech send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  nastech send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"nastech send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"nastech send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"nastech send: failed to read channel directory: {exc}")
    ...
    if not platforms:
        ...
        from nastech_constants import get_default_nastech_root, get_nastech_home, nastech_home_key
        home, root = get_nastech_home(), get_default_nastech_root()
        ...
        if nastech_home_key(root) != nastech_home_key(home) and (root / "channel_directory.json").exists():
...
def _load_nastech_env() -> None:
    """Populate the credential environment from ``<NASTECH_HOME>/.env`` AND bridge top-level ``config.yaml``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_nastech_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `nastech send --list telegram` lands "telegram" in the `message` positional.
    ...
    if mentions and target.split(":", 1)[0].strip().lower() != "whatsapp":
        _fail("nastech send: --mention is only supported for WhatsApp targets.", _USAGE_EXIT)
    invalid_mentions = _invalid_whatsapp_mentions(mentions)
    if invalid_mentions:
        _fail(
            "nastech send: invalid --mention value(s): "
    ...
    # Lazy import keeps `nastech send --help` fast (no tool registry / gateway config stack).
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            f"credentials ({nastech_home / '.env'} + "
            f"{nastech_home / 'config.yaml'}) — no LLM, "
