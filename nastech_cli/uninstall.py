...
def _strip_nastech_path_lines(content: str) -> str:
    """Drop the ``# Nastech Agent`` marker (+ its PATH line) and any nastech PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
...
def remove_path_from_shell_configs():
    ...
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            ...
            new_content = _strip_nastech_path_lines(content)
...
def remove_wrapper_script():
    ...
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a nastech_cli / nastech-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('nastech_cli' in content or 'nastech-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("nastech", "nastech-acp", "nastech-agent"))
...
def _remove_windows_gateway() -> bool:
    ...
    from nastech_cli import gateway_windows as gw
...
# NASTECH_HOME / NASTECH_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\nastech\git\{cmd,bin,usr\bin}, ...\nastech\node), PortableGit + Node copies
...
def _nastech_path_markers(nastech_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Nastech-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    subs = ("nastech-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(nastech_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Nastech-owned entries from User-scope PATH in the registry (see ``_nastech_path_markers``)."""
    markers = tuple(m.lower() for m in _nastech_path_markers(nastech_home, include_managed_bin=include_managed_bin))
...
def remove_portable_tooling_windows(nastech_home: Path) -> list[Path]:
    ...
    ``nastech_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (nastech_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``nastech`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from nastech_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from nastech_constants import get_default_nastech_root
        bin_dir = get_default_nastech_root() / "bin"
    except Exception as e:
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``nastech -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current NASTECH_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m nastech_cli.main` (the `nastech` wrapper may be half-gone).
    nastech_invocation = [sys.executable, "-m", "nastech_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                nastech_invocation + ["gateway", subcmd], capture_output=True, text=True,
...
def run_gui_uninstall(args):
    """``nastech uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from nastech_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    nastech_home = get_nastech_home()
    summary = gui_install_summary(nastech_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         𓄃 Nastech Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()
...
def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.nastech/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, nastech_home=nastech_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie NASTECH_HOMEs and systemd units behind.
    ...
    print()
    _print_box("│            𓄃 Nastech Agent Uninstaller                  │", Colors.MAGENTA)
    print()
    ...
    # services, alias wrappers, NASTECH_HOME dirs) — otherwise they leave zombie services behind.
    ...
    _perform_uninstall(
        project_root=project_root, nastech_home=nastech_home, full_uninstall=full_uninstall,
...
def _print_uninstall_dry_run(*, project_root: Path, nastech_home: Path, full_uninstall: bool) -> None:
    ...
    if not full_uninstall:
        ...
        profiles = _discover_named_profiles() if _is_default_nastech_home(nastech_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$NASTECH_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: nastech_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; nastech\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_nastech_home(nastech_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(nastech_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Nastech-owned PATH entries in User environment"),
        (windows, "Removing NASTECH_HOME / NASTECH_GIT_BASH_PATH User env vars...",
         remove_nastech_env_vars_windows, "Removed User env var: {}", "No Nastech-set User env vars to remove"),
        (True, "Removing nastech command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows nastech launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows nastech launchers found"),
        (True, "Removing Nastech-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(nastech_home), "Removed {}", "No Nastech-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE NASTECH_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from nastech_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(nastech_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(nastech_home), "Removed {}",
...
def main(argv=None) -> int:
    """``python -m nastech_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``nastech_constants`` + ``nastech_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
