...
def _nastech_path_markers(nastech_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    ...
    holding the nastech launchers and the managed uv) — only wanted when
...
def remove_path_from_windows_registry(nastech_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    ...
    ``include_managed_bin`` adds ``<nastech_home>\\bin`` (the managed binary
    dir holding the nastech launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _nastech_path_markers(nastech_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``nastech`` launchers install.ps1 staged in the managed
    binary dir (the default Nastech root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``nastech`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_nastech_exe`` relies on), so
...
def _perform_uninstall(
    *,
    ...
    if _is_windows():
        ...
        # The managed binary dir (nastech\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_nastech_home(nastech_home)
    ...
    #     would dangle — `nastech` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows nastech launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows nastech launchers found")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://nastechresearch.github.io/nastech-agent/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://nastechresearch.github.io/nastech-agent/install.sh | bash", Colors.DIM))
        print()
