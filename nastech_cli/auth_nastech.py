"""Nastech Portal OAuth: device-code login, refresh, shared-store mirroring, JWT selection, status.

Split out of ``nastech_cli/auth.py``; origin helpers are imported lazily inside each function
so ``nastech_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from nastech_cli.auth_codex import _pool_entries
from nastech_cli.auth_constants import (
    _decode_jwt_claims, AUTH_LOCK_TIMEOUT_SECONDS, AuthError, DEFAULT_NASTECH_CLIENT_ID,
    DEFAULT_NASTECH_INFERENCE_URL, DEFAULT_NASTECH_PORTAL_URL, DEFAULT_NASTECH_SCOPE, DEFAULT_NASTECH_WELCOME_URL,
    DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS, NASTECH_AUTH_PATH_INVOKE_JWT, NASTECH_BILLING_MANAGE_SCOPE,
    NASTECH_DEVICE_CODE_SOURCE, NASTECH_INFERENCE_INVOKE_SCOPE, NASTECH_INVOKE_JWT_MIN_TTL_SECONDS,
    _nastech_err, httpx)

if TYPE_CHECKING:  # annotation-only; the runtime import would be a cycle
    from nastech_cli.auth import ProviderConfig

# Log-record parity with the origin module (caplog tests pin "nastech_cli.auth").
logger = logging.getLogger("nastech_cli.auth")

_UNUSABLE_JWT_RELOGIN = "Re-authenticate with: nastech auth add nastech"
...
def _unusable_invoke_jwt_error(reason: str, *, no_refresh_token: bool = False) -> AuthError:
    ...
    terminal ``nastech_auth_missing_refresh_token`` code the pool recognises instead of the JWT
    ...
    return _nastech_err(
        f"Nastech Portal access token is not a usable inference JWT ({reason}){detail}. "
        f"{_UNUSABLE_JWT_RELOGIN}",
        "nastech_auth_missing_refresh_token" if no_refresh_token else reason, relogin=True)
...
def _oauth_trace(event: str, *, sequence_id: Optional[str] = None, **fields: Any) -> None:
    if os.getenv("NASTECH_OAUTH_TRACE", "").strip().lower() not in {"1", "true", "yes", "on"}:
        return
...
# Nastech agent-key slots; a fresh login persists them as None, quarantine strips them.
_NASTECH_EMPTY_AGENT_KEY_FIELDS: Dict[str, Any] = {
...
def _portal_entitlement_message(capability: str) -> str:
    ...
    from nastech_cli.nastech_account import (
        format_nastech_portal_entitlement_message, get_nastech_portal_account_info)
    account_info = get_nastech_portal_account_info(force_fresh=True)
    return format_nastech_portal_entitlement_message(account_info, capability=capability) or ""


def _format_nastech_entitlement_auth_error(error: AuthError) -> str:
    with suppress(Exception):
        if message := _portal_entitlement_message("Nastech model access"):
            return message
    return f"{error} Check credits or billing in Nastech Portal, then retry."


def _migrate_stale_nastech_portal_url(providers: Dict[str, Any]) -> None:
    nastech = providers.get("nastech")
    if not isinstance(nastech, dict):
        return
    stored = (nastech.get("portal_base_url") or "").strip()
    if stored and urlparse(stored).hostname in _NASTECH_STALE_PORTAL_HOSTS:
        logger.warning(
            "auth: migrating stale nastech portal_base_url %s -> %s", stored, DEFAULT_NASTECH_PORTAL_URL)
        nastech["portal_base_url"] = DEFAULT_NASTECH_PORTAL_URL


# Allowlist of hosts the Nastech Portal proxy will forward inference JWTs to — a bearer sent anywhere
...
def _nastech_inference_host_allowed(hostname: Optional[str]) -> bool:
    ...
    its own — not even for a Nastech-owned host: the operator's explicit override is the authority,
    ...
    """
    if hostname in _ALLOWED_NASTECH_INFERENCE_HOSTS:
        return True
    if not hostname:
        return False
    override = _nastech_inference_env_override()
...
def _validate_nastech_inference_url_from_network(url: Optional[str]) -> Optional[str]:
    ...
    if parsed.scheme != "https":
        logger.warning(
            "nastech: refusing non-https inference URL scheme %r from Portal response", parsed.scheme)
        return None
    if not _nastech_inference_host_allowed(parsed.hostname):
        logger.warning(
            "nastech: refusing inference URL host %r from Portal response "
...
def _scoped_operator_override(*names: str) -> Optional[str]:
    """The first set operator routing override among ``names`` (``NASTECH_INFERENCE_BASE_URL``,
    ``NASTECH_PORTAL_BASE_URL`` / its ``NASTECH_PORTAL_BASE_URL`` alias), resolved through the profile
    ...
    try:
        ...
        logger.warning(
            "nastech: %s unreadable — no profile secret scope on a multiplexed call; treating the "
            "override as absent (default routing applies). The caller needs a profile scope binding.",
            "/".join(names))
        return None


def _nastech_inference_env_override() -> Optional[str]:
    """User-set ``NASTECH_INFERENCE_BASE_URL`` override (trailing slash stripped) or None.
    ...
    """
    from nastech_cli.auth import _optional_base_url
    return _optional_base_url(_scoped_operator_override("NASTECH_INFERENCE_BASE_URL"))


def _nastech_portal_env_override() -> Optional[str]:
    """``NASTECH_PORTAL_BASE_URL`` / ``NASTECH_PORTAL_BASE_URL`` override or None.

    Documented dev/staging escape hatch (e.g. hosted agents on the staging Portal). Trusted env
    source: must NOT be gated by ``_NASTECH_PORTAL_ALLOWED_HOSTS``, which rejects untrusted
    NETWORK-provided values persisted to auth.json, not operator config. Read through the
    profile secret scope like ``_nastech_inference_env_override``: it is reached on every routed
    turn (``_nastech_effective_routing``), and a raw environ read would POST a multiplexed
    secondary's refresh token to the DEFAULT profile's Portal.
    """
    from nastech_cli.auth import _optional_base_url
    return _optional_base_url(_scoped_operator_override("NASTECH_PORTAL_BASE_URL", "NASTECH_PORTAL_BASE_URL"))
...
def _nastech_invoke_jwt_status(
    ...
    min_ttl_seconds: int = NASTECH_INVOKE_JWT_MIN_TTL_SECONDS) -> Optional[str]:
    """Return None when the token can be used for inference, else a reason."""
    from nastech_cli.auth import _is_expiring
...
def _nastech_invoke_jwt_is_usable(
    token: Any, *, scope: Any = None, expires_at: Any = None,
    min_ttl_seconds: int = NASTECH_INVOKE_JWT_MIN_TTL_SECONDS) -> bool:
    from nastech_cli.auth import _nastech_invoke_jwt_status
    return _nastech_invoke_jwt_status(
...
def _state_invoke_jwt_status(state: Dict[str, Any], token: Any) -> Optional[str]:
    """``_nastech_invoke_jwt_status`` for *token* using *state*'s scope / expires_at (patchable)."""
    from nastech_cli.auth import _nastech_invoke_jwt_status
    return _nastech_invoke_jwt_status(
        token, scope=state.get("scope"), expires_at=state.get("expires_at"))


def _assert_nastech_inference_jwt_usable(state: Dict[str, Any], *, access_token: Any = None) -> None:
...
def _remaining_ttl(expires_at: Any, fallback_expires_in: Any) -> int:
    ...
    from nastech_cli.auth import _coerce_ttl_seconds, _parse_iso_timestamp
...
def _set_nastech_agent_key_from_invoke_jwt(
    ...
    from nastech_cli.auth import _nonempty_str
...
def _select_nastech_invoke_jwt(
    ...
    logger.debug("Nastech inference auth: using NAS invoke JWT")
    _oauth_trace(
        "nastech_invoke_jwt_selected", sequence_id=sequence_id,
...
_NASTECH_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({"expires_in", "agent_key_expires_in"})


def _nastech_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {k: v for k, v in state.items() if k not in _NASTECH_EFFECTIVE_STATE_IGNORED_KEYS}
...
def _nastech_shared_auth_dir() -> Path:
    """Directory of the shared Nastech token store: ``NASTECH_SHARED_AUTH_DIR`` or ``<root>/shared/``.

    Outside any named profile so all profiles share it (``nastech --profile X auth add nastech --type
...
def _nastech_shared_store_path() -> Path:
    ...
    # user's Nastech root so a test that forgot NASTECH_SHARED_AUTH_DIR fails loudly instead of
    # corrupting cross-profile state.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from nastech_constants import get_default_nastech_root
        real_home_shared = (
            get_default_nastech_root() / "shared" / NASTECH_SHARED_STORE_FILENAME).resolve(strict=False)
        try:
...
def _nastech_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    """
    from nastech_cli.auth import _file_lock
    try:
        lock_path = _nastech_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        yield  # No NASTECH_HOME yet (pre-setup): fall through without locking.
        return
    with _file_lock(
        lock_path, _nastech_shared_lock_holder, timeout_seconds,
...
# OAuth fields mirrored between a profile's Nastech state and the shared cross-profile store.
_NASTECH_SHARED_STATE_KEYS = (
...
def _merge_shared_nastech_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Nastech state."""
    from nastech_cli.auth import _nonempty_str, _parse_iso_timestamp, _read_shared_nastech_state
    shared = _read_shared_nastech_state() or {}
    ...
    if not refresh_changed and not shared_access_exp > local_access_exp:
        return False
    for key in _NASTECH_SHARED_STATE_KEYS:
...
def _nastech_shared_shape(src: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "scope": src.get("scope") or DEFAULT_NASTECH_SCOPE,
        "client_id": src.get("client_id") or DEFAULT_NASTECH_CLIENT_ID,
        "portal_base_url": src.get("portal_base_url") or DEFAULT_NASTECH_PORTAL_URL,
        ...
        "inference_base_url": src.get("inference_base_url") or (
            DEFAULT_NASTECH_WELCOME_URL if src.get("auth_method") == "anonymous" else DEFAULT_NASTECH_INFERENCE_URL),
...
def _write_shared_nastech_state(state: Dict[str, Any]) -> None:
    ...
    """
    from nastech_cli.auth import _nonempty_str, _save_private_json
    ...
    if not is_guest and not (_nonempty_str(refresh_token) and _nonempty_str(state.get("access_token"))):
        return
    shared = {
        "_schema": 1, **_nastech_shared_shape(state),
    ...
    try:
        ...
        _oauth_trace(
            "nastech_shared_store_written", path=str(path),
...
def _read_shared_nastech_state() -> Optional[Dict[str, Any]]:
    """Shared Nastech OAuth state when present and well-formed, else None.
...
def _clear_shared_nastech_state(reason: str) -> None:
    ...
    try:
        with _nastech_shared_store_lock():
            _nastech_shared_store_path().unlink(missing_ok=True)
...
def _quarantine_forensics(state: Dict[str, Any], error: AuthError, reason: str) -> Dict[str, Any]:
    ...
    provenance is client_id + agent_key_id (Nastech state has no session_id).
    """
    from nastech_cli.auth import _auth_file_path
...
def _quarantine_nastech_oauth_state(state: Dict[str, Any], error: AuthError, *, reason: str) -> None:
    ...
    from nastech_cli.auth import (
        _FLAT_OAUTH_TOKEN_KEYS, _last_auth_error_marker, invalidate_nastech_auth_status_cache)
    ...
    logger.warning(
        "Nastech OAuth state quarantined (terminal auth death): %s",
        json.dumps(_quarantine_forensics(state, error, reason), sort_keys=True, ensure_ascii=False))
    for key in (*_FLAT_OAUTH_TOKEN_KEYS, *_NASTECH_EMPTY_AGENT_KEY_FIELDS):
        state.pop(key, None)
    state["last_auth_error"] = _last_auth_error_marker("nastech", error, reason=reason)
...
def _quarantine_nastech_pool_entries(
    ...
    """Remove singleton-seeded Nastech pool entries that contain dead OAuth state."""
    entries = _pool_entries(auth_store, "nastech")
    if entries is None:
        return False
    ...
    if removed:
        auth_store["credential_pool"]["nastech"] = retained
        _oauth_trace("nastech_pool_device_code_quarantined", reason=reason, error_code=error.code)
    return removed


def _try_import_shared_nastech_state(*, timeout_seconds: float = 15.0) -> Optional[Dict[str, Any]]:
    """Rehydrate Nastech OAuth state from the shared store via a forced refresh.

    Returns auth_state ready for ``persist_nastech_credentials()``; None on any failure (expired
    token, portal unreachable) so the caller falls through to device-code.
    """
    from nastech_cli.auth import (
        _read_shared_nastech_state, _write_shared_nastech_state, refresh_nastech_oauth_from_state,
        _is_terminal_nastech_refresh_error)
    try:
        with _nastech_shared_store_lock(timeout_seconds=_shared_lock_timeout(timeout_seconds)):
            shared = _read_shared_nastech_state()
            if not shared:
                return None
            # Full state dict so refresh_nastech_oauth_from_state has every field it needs.
            state: Dict[str, Any] = {
                **_nastech_shared_shape(shared), "agent_key": None, "agent_key_expires_at": None,
                "tls": {"insecure": False, "ca_bundle": None}}
            refreshed = refresh_nastech_oauth_from_state(
                state, timeout_seconds=timeout_seconds, force_refresh=True,
                on_state_update=lambda updated, _reason: _write_shared_nastech_state(updated))
            _write_shared_nastech_state(refreshed)
    except Exception as exc:
        is_auth = isinstance(exc, AuthError)
        _oauth_trace(
            "nastech_shared_import_failed", error_type=type(exc).__name__,
            **({"error_code": getattr(exc, "code", None)} if is_auth else {}))
        if is_auth and _is_terminal_nastech_refresh_error(exc):
            _clear_shared_nastech_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Nastech import failed: %s", exc)
        return None
    return refreshed
    ...
    *, client: httpx.Client, portal_base_url: str, client_id: str, refresh_token: str,
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise _nastech_err("Refresh response missing access_token", "invalid_token", relogin=True)
        return payload
    try:
        error_payload = response.json()
    except Exception as exc:
        raise _nastech_err("Refresh token exchange failed", relogin=True) from exc
    ...
    # self-heal hook) redeemed Nastech's refresh_token without persisting the rotated token, so the
    ...
    if code == "refresh_token_reused" or "reuse" in description.lower():
        description = (
            "Nastech Portal detected refresh-token reuse and revoked this session.\n"
        ...
        relogin = True
    raise _nastech_err(description, code, relogin=relogin)


def _refresh_nastech_or_quarantine(
    *, client: httpx.Client, auth_store: Dict[str, Any], state: Dict[str, Any],
    ...
    from nastech_cli.auth import _refresh_access_token, _is_terminal_nastech_refresh_error
    try:
        ...
        if _is_terminal_nastech_refresh_error(exc):
            _quarantine_nastech_oauth_state(state, exc, reason=reason)
            _quarantine_nastech_pool_entries(auth_store, exc, reason=reason)
            persist()
        raise


def _apply_nastech_refreshed_tokens(
    state: Dict[str, Any], refreshed: Dict[str, Any], refresh_token: str, *,
    inference_base_url: Optional[str] = None) -> None:
    """Write a successful Nastech token-refresh payload into *state* (tokens + expiry fields).
    ...
    *inference_base_url*, when given, is the healed network-provenance URL to persist alongside
    ...
    """
    from nastech_cli.auth import _coerce_ttl_seconds
...
def _healed_nastech_inference_url(refreshed: Dict[str, Any]) -> str:
    ...
    """
    url = _validate_nastech_inference_url_from_network(refreshed.get("inference_base_url"))
    return url or DEFAULT_NASTECH_INFERENCE_URL


def _nastech_http_client(timeout_seconds: float, verify: Any) -> httpx.Client:
    return httpx.Client(
...
def fetch_nastech_models(
    *, inference_base_url: str, api_key: str, timeout_seconds: float = 15.0,
    ...
    """Fetch available model IDs from the Nastech inference API."""
    from nastech_cli.auth import _nonempty_str
    with _nastech_http_client(timeout_seconds, verify) as client:
        response = client.get(
    ...
    if response.status_code != 200:
        ...
        raise _nastech_err(description, "models_fetch_failed")
    ...
    for item in data:
        ...
        # Nastech models aren't reliable for agentic tool-calling
        if _nonempty_str(model_id) and "nastech" not in model_id.lower():
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    return _nonempty_str(key) and _nastech_invoke_jwt_is_usable(
...
def refresh_nastech_oauth_pure(
    ...
    inference_base_url: str, *, token_type: str = "Bearer", scope: str = DEFAULT_NASTECH_SCOPE,
...
def refresh_nastech_oauth_from_state(
    ...
    on_state_update: Optional[Callable[[Dict[str, Any], str], None]] = None) -> Dict[str, Any]:
    """Refresh Nastech OAuth from a state dict (defaults filled in) without mutating auth.json."""
    from nastech_cli.auth import (
        _assert_nastech_inference_jwt_usable, _refresh_access_token, _resolve_verify,
    ...
    state: Dict[str, Any] = {
        "access_token": src.get("access_token", ""), "refresh_token": src.get("refresh_token", ""),
        "client_id": src.get("client_id") or DEFAULT_NASTECH_CLIENT_ID,
        "portal_base_url": (src.get("portal_base_url") or DEFAULT_NASTECH_PORTAL_URL).rstrip("/"),
        "inference_base_url": (
            src.get("inference_base_url") or DEFAULT_NASTECH_INFERENCE_URL).rstrip("/"),
        "token_type": src.get("token_type") or "Bearer",
        "scope": src.get("scope") or DEFAULT_NASTECH_SCOPE,
        "obtained_at": src.get("obtained_at"), "expires_at": src.get("expires_at"),
    ...
    with _nastech_http_client(timeout_seconds or 15.0, verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise _nastech_err(
                    "No refresh token is available for Nastech Portal.", "nastech_auth_missing_refresh_token",
                    relogin=True)
            refreshed = _refresh_access_token(
                client=client, portal_base_url=state["portal_base_url"],
                client_id=state["client_id"], refresh_token=refresh_token_value)
            _apply_nastech_refreshed_tokens(
                state, refreshed, refresh_token_value,
                inference_base_url=_healed_nastech_inference_url(refreshed))
...
def persist_nastech_credentials(creds: Dict[str, Any], *, label: Optional[str] = None):
    """Persist Nastech OAuth credentials as the singleton provider state.

    Nastech credentials are read from ``providers.nastech`` (401 recovery, pool seeding) AND
    ``credential_pool.nastech`` (runtime ``pool.select()``); a pool-only write broke expiry recovery.
    So: write the singleton, mirror to the shared store, then ``load_pool("nastech")`` upserts the
    canonical ``device_code`` entry in place. ``label`` rides in the singleton so re-seeding keeps
    it.
    """
    from nastech_cli.auth import _save_active_provider_state, _write_shared_nastech_state
    from agent.credential_pool import load_pool
    state = dict(creds)
    if label and str(label).strip():
        state["label"] = str(label).strip()
    _save_active_provider_state("nastech", state)
    _write_shared_nastech_state(state)
    pool = load_pool("nastech")
    return next((e for e in pool.entries() if e.source == NASTECH_DEVICE_CODE_SOURCE), None)
...
def _nastech_effective_routing(state: Dict[str, Any]) -> tuple[str, str, str, str]:
    ...
    layers the runtime-only ``NASTECH_INFERENCE_BASE_URL`` override on top and is never persisted.
    """
    from nastech_cli.auth import _NASTECH_PORTAL_ALLOWED_HOSTS, _optional_base_url
    portal_url = (_optional_base_url(state.get("portal_base_url")) or DEFAULT_NASTECH_PORTAL_URL).rstrip("/")
    ...
    env_portal_override = _nastech_portal_env_override()
    if env_portal_override:
        ...
        if not portal_host or portal_host not in _NASTECH_PORTAL_ALLOWED_HOSTS or not trusted_scheme:
            logger.warning(
                "auth: ignoring invalid portal_base_url %r "
                "(host %r or scheme not allowed), using default",
                portal_url, portal_host)
            portal_url = DEFAULT_NASTECH_PORTAL_URL
    ...
    from nastech_cli.anon_auth import is_guest_state
    stored_inference_url = (
        _validate_nastech_inference_url_from_network(
            _optional_base_url(state.get("inference_base_url")))
        or (DEFAULT_NASTECH_WELCOME_URL if is_guest_state(state) else DEFAULT_NASTECH_INFERENCE_URL))
    return (
        portal_url, stored_inference_url, _nastech_inference_env_override() or stored_inference_url,
        str(state.get("client_id") or DEFAULT_NASTECH_CLIENT_ID))


class _NastechRuntimeResolve:
    """Working set for one ``resolve_nastech_runtime_credentials`` call.
    ...
    def _reload_routing(self) -> None:
        (self.portal_base_url, self.stored_inference_base_url, self.inference_base_url,
         self.client_id) = _nastech_effective_routing(self.state)

    def persist(self, reason: str) -> None:
        from nastech_cli.auth import _save_provider_state_to_source, _write_shared_nastech_state
        state = self.state
        persisted = _nastech_effective_provider_state(self._persisted_state)
        if _nastech_effective_provider_state(state) == persisted:
            _oauth_trace("nastech_state_persist_skipped", sequence_id=self.sequence_id, reason=reason)
            return
        try:
            _save_provider_state_to_source(self.auth_store, "nastech", state, self._source_path)
        except Exception as exc:
            _oauth_trace(
                "nastech_state_persist_failed", sequence_id=self.sequence_id, reason=reason,
                error_type=type(exc).__name__)
            raise
        _oauth_trace(
            "nastech_state_persisted", sequence_id=self.sequence_id, reason=reason,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
            access_token_fp=_token_fingerprint(state.get("access_token")))
        self._persisted_state = dict(state)
    ...
    def shared_lock(self):
        return _nastech_shared_store_lock(timeout_seconds=_shared_lock_timeout(self.timeout_seconds))
    ...
    def merge_shared(self) -> bool:
        ...
        if not _merge_shared_nastech_oauth_state(self.state):
            return False
    ...
    def refresh(self, client: httpx.Client, invoke_jwt_status: Optional[str]) -> None:
        ...
        refreshed = _refresh_nastech_or_quarantine(
        ...
        # NASTECH_INFERENCE_BASE_URL env override is layered on for the client/return value only.
        self.stored_inference_base_url = _healed_nastech_inference_url(refreshed)
        self.inference_base_url = _nastech_inference_env_override() or self.stored_inference_base_url
        _apply_nastech_refreshed_tokens(
    ...
    def ensure_usable_access_token(self, client: httpx.Client) -> None:
        ...
        from nastech_cli.anon_auth import is_guest_state, refresh_guest_state
        ...
        if not self.has_access_token():
            raise _nastech_err(
                "No access token found for Nastech Portal login.", "nastech_auth_missing_access_token", relogin=True)
...
def resolve_nastech_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ...
    """Resolve Nastech inference credentials for runtime use (refreshing under the auth-store lock).
    ...
    """
    from nastech_cli.anon_auth import AnonCredentialDead, clear_dead_guest, ensure_portal_identity
    try:
        return _resolve_nastech_runtime_credentials(
        ...
        from nastech_cli.anon_auth import ANON_ACCOUNT_LOCKED
        dead = get_provider_auth_state("nastech") or {}
...
def _resolve_nastech_runtime_credentials(
    *, timeout_seconds: float = 15.0, insecure: Optional[bool] = None,
    ca_bundle: Optional[str] = None, force_refresh: bool = False,
    stale_access_token: Optional[str] = None) -> Dict[str, Any]:
    """Resolve Nastech inference credentials for runtime use (refreshing under the auth-store lock).
    ...
    """
    from nastech_cli.auth import (
        _assert_nastech_inference_jwt_usable, _auth_file_path, _provider_state_transaction,
        _resolve_verify, _select_nastech_invoke_jwt, _sync_nastech_pool_from_auth_store,
        _tls_state_from_verify)
    with _provider_state_transaction("nastech") as (auth_store, state, state_source_path):
        if not state:
            raise _nastech_err("Nastech is not logged into Nastech Portal.", "nastech_auth_missing", relogin=True)
        run = _NastechRuntimeResolve(
        ...
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)
        _oauth_trace(
            "nastech_runtime_credentials_start", sequence_id=run.sequence_id,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")))
        with _nastech_http_client(timeout_seconds or 15.0, verify) as client:
            run.ensure_usable_access_token(client)
            _assert_nastech_inference_jwt_usable(state, access_token=run.access_token)
            _select_nastech_invoke_jwt(
        ...
        run.persist("resolve_nastech_runtime_credentials_final")
    if run.persisted_any:
        _sync_nastech_pool_from_auth_store()
    api_key = state.get("agent_key")
    if not isinstance(api_key, str) or not api_key:
        raise _nastech_err("Failed to resolve a Nastech inference API key", "server_error")
    expires_at = state.get("agent_key_expires_at")
    return {
        "provider": "nastech", "base_url": run.inference_base_url, "api_key": api_key,
...
def _snapshot_nastech_pool_status() -> Dict[str, Any]:
    ...
    ``resolve_nastech_runtime_credentials()`` refreshes.
    """
    from nastech_cli.auth import _parse_iso_timestamp
    try:
        from agent.credential_pool import load_pool
        pool = load_pool("nastech")
        ...
        return {
            ...
            "portal_base_url": (
                (attr("portal_base_url") or DEFAULT_NASTECH_PORTAL_URL) if is_portal_oauth else None),
            "inference_base_url": (
...
def _nastech_status_from_state(
    state: Dict[str, Any], *, logged_in: bool, source: str) -> Dict[str, Any]:
    """Auth-store-backed Nastech status snapshot (shared by the live and refresh-free variants)."""
    from nastech_cli.anon_auth import is_guest_state
    access_token = state.get("access_token")
...
def _compute_nastech_auth_status() -> Dict[str, Any]:
    ...
    from nastech_cli.auth import get_provider_auth_state, resolve_nastech_runtime_credentials
    state = get_provider_auth_state("nastech")
    if not state:
        return _snapshot_nastech_pool_status()
    base_status = _nastech_status_from_state(
        state, logged_in=bool(state.get("access_token")), source="auth_store")
    try:
        creds = resolve_nastech_runtime_credentials()
        refreshed_state = get_provider_auth_state("nastech") or state
        base_status.update({
            "logged_in": True,
            "portal_base_url": (
...
def get_nastech_auth_status_local() -> Dict[str, Any]:
    """Refresh-free Nastech auth snapshot for read-only display surfaces.

    NEVER calls ``resolve_nastech_runtime_credentials()`` (no refresh POST / single-use token spent);
    ...
    status = _nastech_status_from_state(state, logged_in=logged_in, source="auth_store_local")
...
# Enum values reported on the dashboard /api/status as ``nastech_session_valid``. NAS's health sweep
...
NASTECH_SESSION_VALID = "valid"
NASTECH_SESSION_TERMINAL = "terminal"
NASTECH_SESSION_UNKNOWN = "unknown"


def get_nastech_session_validity() -> str:
    """Classify the Nastech bootstrap session for the dashboard /api/status probe.
    ...
    try:
        ...
        state = None
    if not state:
        return NASTECH_SESSION_UNKNOWN
...
def _pool_first_oauth_status(
    ...
    Pool first (where `nastech auth` / `nastech model` store device_code tokens), then
...
def _nastech_device_code_login(
    *, portal_base_url: Optional[str] = None, inference_base_url: Optional[str] = None,
    ...
    """Run the Nastech device-code flow and return full OAuth state without persisting."""
    from nastech_cli.auth import (
        ...
        _tls_state_from_verify, format_auth_error, refresh_nastech_oauth_from_state)
    pconfig = PROVIDER_REGISTRY["nastech"]
    portal_base_url = (
        portal_base_url or os.getenv("NASTECH_PORTAL_BASE_URL") or os.getenv("NASTECH_PORTAL_BASE_URL")
        or pconfig.portal_base_url).rstrip("/")
    requested_inference_url = (
        inference_base_url or os.getenv("NASTECH_INFERENCE_BASE_URL")
    ...
    auth_state = {
        ...
        **_NASTECH_EMPTY_AGENT_KEY_FIELDS}
    try:
        return refresh_nastech_oauth_from_state(
            auth_state, timeout_seconds=timeout_seconds, force_refresh=False)
    except AuthError as exc:
        if exc.code == "subscription_required":
            portal_url = auth_state.get("portal_base_url", DEFAULT_NASTECH_PORTAL_URL).rstrip("/")
            print()
...
def _mirror_nastech_state_best_effort(auth_state: Dict[str, Any]) -> None:
    """Mirror to the shared store + reseed the pool, swallowing all errors (same as _login_nastech)."""
    from nastech_cli.auth import _sync_nastech_pool_from_auth_store, _write_shared_nastech_state
    with suppress(Exception):
...
def step_up_nastech_billing_scope(
    *, open_browser: bool = True, timeout_seconds: float = 15.0,
    ...
    server silently downscopes and this returns False. Persists like ``_login_nastech`` minus the
    model picker.
    """
    from nastech_cli.auth import (
        PROVIDER_REGISTRY, _nastech_device_code_login, _save_active_provider_state,
        get_provider_auth_state)
    prior = get_provider_auth_state("nastech") or {}
    pconfig = PROVIDER_REGISTRY["nastech"]
    ...
    requested = list(dict.fromkeys([
        *(prior_scope or [NASTECH_INFERENCE_INVOKE_SCOPE, "tool:invoke"]), NASTECH_BILLING_MANAGE_SCOPE]))
    ...
    _save_active_provider_state("nastech", auth_state)
    _mirror_nastech_state_best_effort(auth_state)
    granted = auth_state.get("scope")
    return isinstance(granted, str) and NASTECH_BILLING_MANAGE_SCOPE in granted.split()


def _pick_nastech_model_after_login(
    auth_state: Dict[str, Any], inference_base_url: str) -> Optional[str]:
    """Fetch the curated Nastech model list (tier/policy-filtered) and run the interactive picker.
    ...
    if not isinstance(runtime_key, str) or not runtime_key:
        raise _nastech_err("No runtime API key available to fetch models", "invalid_token")
    ...
    from nastech_cli.models_pricing import (
    ...
    if model_ids:
        pricing = get_pricing_for_provider("nastech")
        ...
        if free_tier:
            with suppress(Exception):
                unavailable_message = _portal_entitlement_message("paid Nastech models")
    ...
    if model_ids:
        from nastech_cli.nastech_account import nastech_policy_notice
        _policy_notice = nastech_policy_notice(removed=_policy_narrowed)
        if _policy_notice:
        ...
        return _prompt_model_selection(
            ...
            unavailable_message=unavailable_message, confirm_provider="nastech",
    ...
    if unavailable_models:
        ...
        print("No curated models available for Nastech Portal.")
    return None


def _offer_shared_nastech_import(timeout_seconds: float) -> Optional[Dict[str, Any]]:
    """Codex-style auto-import: offer to rehydrate a Nastech credential from another profile.
    ...
    """
    from nastech_cli.auth import _prompt_yes_no, _read_shared_nastech_state
    from nastech_cli.anon_auth import is_guest_state
    shared = _read_shared_nastech_state()
    if not shared or is_guest_state(shared):
        ...
        # (persist_nastech_credentials overwrites the singleton and the shared store).
        return None
    try:
        ...
        shared_path = None
    print()
    print(f"Found existing Nastech OAuth credentials at {shared_path}" if shared_path
          else "Found existing shared Nastech OAuth credentials")
    if not _prompt_yes_no("Import these credentials? [Y/n]: ", default="y"):
        return None
    print("Rehydrating Nastech session from shared credentials...")
    auth_state = _try_import_shared_nastech_state(timeout_seconds=timeout_seconds)
...
def _restore_active_provider(prior_active_provider: Any) -> None:
    """Undo the ``active_provider="nastech"`` that ``_save_provider_state`` wrote during login."""
...
def _login_nastech(args, pconfig: ProviderConfig) -> None:
    """Nastech Portal device authorization flow."""
    from nastech_cli.auth import (
        _auth_store_lock, _load_auth_store, _nastech_device_code_login, _save_active_provider_state,
        _save_model_choice, _sync_nastech_pool_from_auth_store, _update_config_for_provider,
        _write_shared_nastech_state, format_auth_error)
    timeout_seconds = getattr(args, "timeout", None) or 15.0
    ca_bundle = (
        getattr(args, "ca_bundle", None) or os.getenv("NASTECH_CA_BUNDLE")
        or os.getenv("SSL_CERT_FILE"))
    try:
        auth_state = _offer_shared_nastech_import(timeout_seconds)
        if auth_state is None:
            auth_state = _nastech_device_code_login(
        ...
        # Snapshot BEFORE _save_provider_state overwrites active_provider to "nastech", so a
        ...
        saved_to = _save_active_provider_state("nastech", auth_state)
        ...
        selected_model = None
        try:
            selected_model = _pick_nastech_model_after_login(auth_state, inference_base_url)
        except Exception as exc:
        ...
        # switch to Nastech with a mismatched model; the Nastech tokens stay saved for future use.
        if not selected_model:
            _restore_active_provider(prior_active_provider)
            print()
            print("No provider change. Nastech credentials saved for future use.")
            print("  Run `nastech model` again to switch to Nastech Portal.")
            return
        config_path = _update_config_for_provider(
            "nastech", inference_base_url, default_model=selected_model)
        _save_model_choice(selected_model)
        print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=nastech)")
    except KeyboardInterrupt:
        print("\nLogin cancelled.")
        raise SystemExit(130)
    except Exception as exc:
        from nastech_cli.auth_error_copy import sign_in_failure_lines
        logger.debug("nastech login failed: %r", exc)
        print()
