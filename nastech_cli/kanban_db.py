...
# ``heartbeat_claim`` or raise it via NASTECH_KANBAN_CLAIM_TTL_SECONDS.
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    """Explicit ``ttl_seconds`` > ``NASTECH_KANBAN_CLAIM_TTL_SECONDS`` > default."""
    if ttl_seconds is not None:
        return max(1, int(ttl_seconds))

    return _env_int("NASTECH_KANBAN_CLAIM_TTL_SECONDS", DEFAULT_CLAIM_TTL_SECONDS, minimum=1)
...
def _resolve_crash_grace_seconds() -> int:
    """``NASTECH_KANBAN_CRASH_GRACE_SECONDS`` (0 = immediate, for tests) else default."""
    return _env_int("NASTECH_KANBAN_CRASH_GRACE_SECONDS", DEFAULT_CRASH_GRACE_SECONDS)


def _resolve_rate_limit_cooldown_seconds() -> int:
    """``NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` (0 = next tick, for tests) else default."""
    return _env_int("NASTECH_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", DEFAULT_RATE_LIMIT_COOLDOWN_SECONDS)
...
def kanban_home() -> Path:
    """``NASTECH_KANBAN_HOME`` else ``get_default_nastech_root()``. Shared across
    profiles BY DESIGN: resolving through the active profile's NASTECH_HOME would
...
def get_current_board() -> str:
    """Active slug: context override -> ``NASTECH_KANBAN_BOARD`` -> ``<root>/kanban/current``
    ...
    for candidate in (
        ...
        os.environ.get("NASTECH_KANBAN_BOARD", "").strip(),
    ):
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    """``kanban.db`` path: ``NASTECH_KANBAN_DB`` pins it (injected into workers);
    ``default`` -> ``<root>/kanban.db`` (back-compat), else the board dir."""
    return _board_path("NASTECH_KANBAN_DB", board, ("kanban.db",), "kanban.db")


def workspaces_root(board: Optional[str] = None) -> Path:
    """Per-board scratch workspace root (``NASTECH_KANBAN_WORKSPACES_ROOT`` wins);
    ``default`` keeps the legacy ``<root>/kanban/workspaces/``."""
    return _board_path("NASTECH_KANBAN_WORKSPACES_ROOT", board, ("kanban", "workspaces"), "workspaces")


def attachments_root(board: Optional[str] = None) -> Path:
    """Per-board attachments root (``NASTECH_KANBAN_ATTACHMENTS_ROOT`` wins). Workers
    read attachments by absolute path, so remote terminal backends must mount it."""
    return _board_path("NASTECH_KANBAN_ATTACHMENTS_ROOT", board, ("kanban", "attachments"), "attachments")
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    """Per-board worker log dir (logs follow the board so ``nastech kanban log``
...
class Task:
    ...
    session_id: Optional[str] = None         # originating NASTECH_SESSION_ID; NULL from CLI/dashboard
...
def create_task(
    ...
    """
    from nastech_cli.kanban_db_graph import initial_task_state, inherit_creator_origin
    from nastech_cli.kanban_pr_acceptance import validate_contract
...
# dispatcher tick or a manual `nastech kanban recompute` (issue #22459).
...
def promote_task(
    ...
    if unsatisfied:
        return False, (
            ...
            f"`nastech kanban unlink <parent_id> {task_id}`)"
        )
...
def _ctx_comments(lines: list[str], comments: list[Comment], now: int) -> None:
    ...
    worker" framing stops an operator-controlled NASTECH_PROFILE like
    "nastech-system" being read as a system directive above an
    ...
    if omitted_note:
        lines.append(omitted_note)
    for c in shown:
        # Render author with explicit "comment from worker" framing so operator-controlled NASTECH_PROFILE
        # values like "nastech-system" or "operator" can't be misread by the next worker as a system
...
def list_profiles_on_disk() -> list[str]:
    ...
    directly to avoid importing ``nastech_cli.profiles`` at startup."""
    try:
        from nastech_constants import get_default_nastech_root
...
from nastech_cli.kanban_db_workspace import (  # noqa: E402
    _cleanup_workspace,
    _is_managed_scratch_path,
    _managed_scratch_path_info,
    _scratch_workspace,
)
from nastech_cli.kanban_db_dispatch import (  # noqa: E402
