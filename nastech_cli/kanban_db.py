...
# Assumed per-worker memory footprint for the derived default cap. Nastech
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``nastech kanban dispatch``)
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so the ``NASTECH_KANBAN_DB``
    ...
    *,
    ...
    def _any_spawnable_review() -> bool:
        if not review_rows:
            return False
        try:
            from nastech_cli.profiles import profile_exists as _rpe
        except Exception:
...
def run_daemon(
    *,
    ...
    dispatcher and ``nastech kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `nastech kanban dispatch` do (OOF-30): explicit
