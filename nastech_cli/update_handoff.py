"""Post-swap hand-off: finish ``nastech update`` in an interpreter born on the pulled code.

``nastech update`` starts in an interpreter that imported the PRE-pull tree. Once ``git merge``
...
into a hand-off file and re-executes ``nastech update --post-swap <file>`` under the venv
...
POST_SWAP_ENV = "NASTECH_UPDATE_POST_SWAP"
...
def _running_from_windows_shim() -> bool:
    from nastech_cli.main_install_repair import _windows_shim_in_process_chain
...
def post_swap_command(handoff_path: Path, argv_tail: list[str]) -> list[str]:
    """``python -m nastech_cli.main update <original flags> --post-swap <file>``."""
    return [str(post_swap_python()), "-m", "nastech_cli.main", "update", *argv_tail, "--post-swap", str(handoff_path)]


def post_swap_child_env() -> dict[str, str]:
    """Environment for the child. ``NASTECH_UPDATE_REEXEC`` marks it as already off the Windows
    ...
    from nastech_cli.main_install_repair import _UPDATE_REEXEC_ENV
    from nastech_cli.update_lock import HANDOFF_PID_ENV
...
def continue_update_in_fresh_interpreter(payload: dict[str, Any], *, argv_tail: list[str]) -> int | None:
    ...
    ``nastech.exe``, the child cannot be awaited: the shim is one of the files the dependency
    ...
    if _running_from_windows_shim():
        try:
            ...
            return None
        print("→ Windows: nastech.exe cannot replace itself while it runs; the update")
