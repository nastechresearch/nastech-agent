...
...) to the *resolved absolute path* of the client binary.  Nastech' interpreter
...
stable across ``nastech update``, and because it is a regular file there is no
...
The anchor self-heals: when ``nastech update`` / ``nastech doctor`` runs and the
...
#   - CVE-repair generations:    .../.nastech-runtime/python/generation-*/
...
_STORE_ROOT_MARKERS = ("/uv/python/", "/.nastech-runtime/python/")
...
def _install_anchor(venv_dir: Path, source_file: Path) -> None:
    ...
    try:
        ...
        try:
            from nastech_cli.managed_uv import _macos_sign_managed_python
...
def tcc_anchor_state(project_root: Path | None = None) -> tuple[str, str]:
    """Report the anchor state for ``nastech doctor``.
