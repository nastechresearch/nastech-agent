"""MCP catalog — curated, Nastech-approved MCP servers shipped with the repo.

Entries are added only by merging a PR into nastech-agent; presence in ``optional-mcps/`` = Nastech
...
from nastech_cli.config import load_config, save_config, get_env_value, save_env_value
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.nastech/.env.
...
def _parse_suggest(path: Path, suggest_raw: Any) -> Optional[SuggestSpec]:
    ...
    from nastech_cli.mcp_app_detection import validate_applications
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Nastech".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Nastech) or ``invalid`` (malformed)."""
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.nastech/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    """Prompt for each env spec; secrets and non-secrets alike go to ~/.nastech/.env."""
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by nastech_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from nastech_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``nastech mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`nastech mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `nastech mcp configure <name>` and unselect a tool (switches to include-mode).
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "oauth" and entry.auth.provider:
        # Provider-mediated OAuth relies on the existing `nastech auth <provider>` flow; surface
    ...
    from nastech_cli.mcp_config import _save_mcp_server
