...
from nastech_constants import INDICATOR_STYLES
...
# _nastech_auth_status_cache). Falls back to a fresh load when the file cannot
# be stat'ed.
...
class CommandDef:
    ...
    # Names a key in ``nastech_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``nastech_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("plan", "Write a markdown implementation plan to .nastech/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("wake", "Toggle the 'Hey Nastech' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Nastech is working", "Configuration",
    ...
    CommandDef("subscription", "View your Nastech plan and change it in the browser", "Info",
               cli_only=True, aliases=("upgrade",)),
    CommandDef("topup", "Show your Nastech balance and manage billing on the portal", "Info"),
...
# Telegram allows up to 100 BotCommands. Nastech ships ~50 built-in commands;
...
# unrelated command landed. These claim their slots right after /nastech,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /nastech <command>).
...
# native slot, the alias spelling stays reachable via /nastech reset).
...
# ``/nastech <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/nastech" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /nastech topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /nastech moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /nastech debug on Slack.
#   - egress: Docker-only proxy status; reachable as /nastech egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /nastech init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /nastech version on
...
#   - diff: git working-tree diff; reached via /nastech diff on Slack so it
...
#     /nastech update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /nastech heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /nastech refine on
...
#     /nastech platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_NASTECH_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_NASTECH_ONLY:
            # Intentionally Slack-via-/nastech only (see _SLACK_VIA_NASTECH_ONLY).
            return
    ...
    # everything except /nastech, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    ...
    the same as /nastech bg do stuff.
...
class SlashCommandCompleter(Completer):
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        """Yield completions for /personality via nastech_cli.personality."""
        try:
            ...
            # freshness-correct (same pattern as load_env / _nastech_auth_status_cache).
