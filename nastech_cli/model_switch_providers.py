...
Split out of ``nastech_cli/model_switch.py``; every moved name is re-imported there so
``nastech_cli.model_switch.<name>`` keeps resolving (and monkeypatching) as before."""
...
from nastech_cli.providers import custom_provider_aliases, custom_provider_slug, get_label
from utils import base_url_host_matches

# Log-record parity with the origin module.
logger = logging.getLogger("nastech_cli.model_switch")
...
def _save_discovered_models_to_config(
    ...
    from nastech_cli.model_switch import _extra_headers_from_config
...
def _discovered_catalog_stale(entry: dict, model_ids: list[str]) -> bool:
    ...
    A mapping Nastech itself discovered (entry flag or legacy in-mapping sentinel) is ours to
...
def _fetch_picker_live_models(
    ...
    from nastech_cli.models import _get_ollama_native_headers, cached_fetch_api_models, fetch_api_models
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from nastech_cli.models import update_provider_cache_entry
...
def _iter_builtin_candidates(models_dev_data: dict, excluded: set, seen: set):
    """Yield ``(nastech_id, mdev_id, pconfig, env_vars)`` for section-1 rows.
    ...
    from nastech_cli.auth import PROVIDER_REGISTRY, is_runtime_provider_routable
    ...
    for nastech_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        alias_target = ALIASES.get(nastech_id)
        if alias_target and alias_target != nastech_id and alias_target in _AGGREGATOR_PROVIDERS:
            continue
        try:
            from providers import get_provider_profile
            prof = get_provider_profile(nastech_id)
            if prof is not None and prof.name != nastech_id:
                continue
        except Exception:
            pass
        if nastech_id.lower() in seen:
            continue
        if nastech_id.lower() in excluded or mdev_id.lower() in excluded:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(nastech_id)
        if (pconfig and pconfig.auth_type != "api_key") or not is_runtime_provider_routable(nastech_id):
            continue
        env_vars = list(pconfig.api_key_env_vars) if pconfig and pconfig.api_key_env_vars else pdata.get("env", [])
        if isinstance(env_vars, list):
            yield nastech_id, mdev_id, pconfig, env_vars
...
def _raw_pool_usable(nastech_id: str) -> bool:
    ...
    try:
        ...
        if store and store.get("credential_pool", {}).get(nastech_id):
            return _credential_pool_is_usable(nastech_id, raw_pool_present=True)
    except Exception:
        pass
    return False
...
def _overlay_has_env_creds(pid: str, nastech_slug: str, overlay, read_env) -> bool:
...
def _live_or_curated_ids(slug: str, curated: dict, *fallback_keys: str, merge_models_dev: bool = True,
    ...
    """``cached_provider_model_ids`` (the SAME disk-cached list ``nastech model`` builds), falling
    ...
    from nastech_cli.models import _MODELS_DEV_PREFERRED, _merge_with_models_dev, cached_provider_model_ids
...
def _nastech_picker_model_ids(curated: dict, force_fresh_nastech_tier: bool) -> list:
    """Nastech serves a huge live catalog; the picker shows ONLY the curated agentic list, augmented
    with the Portal's free/paid recommendations (new models surface without a CLI release) and
    narrowed by org policy. Mirrors ``_model_flow_nastech`` so GUI pickers match the CLI. A failed
    recommendation fetch still yields a policy-filtered curated list."""
    model_ids = curated.get("nastech", [])
    try:
        from nastech_cli.models_pricing import get_pricing_for_provider
        from nastech_cli.models import (
        ...
        pricing = get_pricing_for_provider("nastech", cached_only=True) or {}
        try:
            portal = (get_provider_auth_state("nastech") or {}).get("portal_base_url", "") or ""
        except Exception:
            portal = ""
        if check_nastech_free_tier(force_fresh=force_fresh_nastech_tier):
    ...
    try:
        ...
        model_ids = restrict_to_nastech_policy(model_ids, nastech_policy_allowed_ids(), rescue_empty=True)
    except Exception:
        pass
    return model_ids


def _free_tier_nastech_row(row: dict) -> dict | None:
    """The one free-tier rule for a Nastech picker row, shared by every row builder.

    ``row`` carries at least ``name`` and ``models``. A guest identity carrying inference turns
    it into "Nastech · free tier" with the single model ``nastech/welcome`` (the welcome host serves
    nothing else). A guest that ``nastech.guest: false`` has switched off yields ``None``: no Nastech
    row at all, since there is nothing selectable. A real account (or no Nastech state) passes the
...
def _absorb_entry_models(grp: dict, entry: dict, active_model: Any) -> None:
    ...
    from nastech_cli.model_switch import _declared_model_ids, _entry_models_discovered, _models_config_is_allowlist
...
def _display_prefix(name: str) -> str:
    """Text before the per-model separator Nastech's own writer uses ("—" / " - ")."""
...
def _discover_endpoint_models(
    ...
    if probe_live:
        ...
        try:
            from nastech_cli.models import cached_fetch_api_models
...
def _collect_authed_provider_slugs(
    ...
    for nastech_id, _mdev_id, _pconfig, env_vars in _iter_builtin_candidates(models_dev_data, excluded_set, seen):
        if _any_env(env_vars, _scoped_key_env) or _raw_pool_usable(nastech_id):
            _emit(nastech_id, nastech_id)

    mdev_to_nastech = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in NASTECH_OVERLAYS.items():
        nastech_slug = mdev_to_nastech.get(pid, pid)
        if _skip(seen, excluded_set, pid, nastech_slug) or overlay.auth_type == "aws_sdk":
            continue
        if (
            _overlay_has_env_creds(pid, nastech_slug, overlay, _scoped_key_env)
            or _auth_store_has_provider(pid, nastech_slug) or _pool_usable(nastech_slug)):
            _emit(nastech_slug, pid, nastech_slug)
    ...
    # Nastech excluded: its picker branch builds from the curated list and never reads the
    # api_key-only cache entry a prefetch would write.
    return [s for s in slugs if s != "nastech"]


@dataclass
class _PickerBuild:
    """State threaded through the ``list_authenticated_providers`` sections: 1 built-ins mapped to
    models.dev, 2 Nastech-only overlays, 2b canonical providers missed by 1/2, 3 ``providers:``
    entries + 3b the bare active custom endpoint, 4 ``custom_providers:`` entries. Row-builder
    imports of ``nastech_cli.auth/models`` stay lazy so tests can patch those modules."""
    current_provider: str
    current_base_url: str
    current_model: str
    max_models: int | None
    for_picker: bool
    force_fresh_nastech_tier: bool
    ...
    def add_builtin_row(
        ...
        if slug == "nastech":
            # Free-tier identity: one row "Nastech · free tier" / nastech/welcome, or no row when
            # nastech.guest is off. Still marks the slug seen so a later lap cannot re-emit it.
            row = _free_tier_nastech_row(row)
        if row is not None:
...
def _lap_lmstudio_row(b: _PickerBuild, user_providers: dict) -> None:
    ...
    if not (is_current or isinstance(configured, dict)):
        return
    from nastech_cli.model_switch import _declared_model_ids
    ...
    b.add_builtin_row("lmstudio", get_label("lmstudio"), is_current, model_ids, "nastech")
...
def _lap_builtin_rows(b: _PickerBuild, data: dict, user_providers: dict) -> None:
    ...
    from nastech_cli.model_switch import _declared_model_ids, _scoped_key_env
    from agent.models_dev import get_provider_info
    for nastech_id, mdev_id, pconfig, env_vars in _iter_builtin_candidates(data, b.excluded, b.seen_slugs):
        ...
        if not (_any_env(env_vars, _scoped_key_env) or _raw_pool_usable(nastech_id)):
            continue
        model_ids = _live_or_curated_ids(nastech_id, b.curated, non_blocking=b.non_blocking_catalogs)
        ...
        configured = user_providers.get(nastech_id) if isinstance(user_providers, dict) else None
        configured_models = _declared_model_ids(configured.get("models")) if isinstance(configured, dict) else []
        ...
        b.add_builtin_row(
            nastech_id, display_name, b.current_provider in (nastech_id, mdev_id), model_ids, "built-in")


def _overlay_has_creds(b: _PickerBuild, pid: str, nastech_slug: str, overlay) -> bool:
    ...
    if overlay.auth_type == "aws_sdk":
        has_creds = _has_aws_sdk_creds_for_listing(nastech_slug, b.current_provider)
    else:
        from nastech_cli.model_switch import _scoped_key_env
        has_creds = _overlay_has_env_creds(pid, nastech_slug, overlay, _scoped_key_env)
    ...
    if not has_creds and overlay.auth_type == "external_process":
        try:
            ...
            _ext_status = get_auth_status(nastech_slug) or {}
    ...
    has_creds = has_creds or _auth_store_has_provider(pid, nastech_slug)
    if not has_creds:
        # Full auto-seeding pool check catches external stores (Codex CLI ~/.codex/auth.json)
        # not yet in auth.json.
        try:
            if _credential_pool_is_usable(nastech_slug):
                has_creds = True
            elif b.for_picker:
                ...
                try:
                    from agent.credential_pool import load_pool
                    has_creds = load_pool(nastech_slug).has_credentials()
                except Exception:
                    pass
        except Exception as exc:
    ...
    if not has_creds and nastech_slug == "anthropic":
        ...
        try:
            from agent.anthropic_credentials import read_claude_code_credentials, read_nastech_oauth_credentials
            nastech_creds = read_nastech_oauth_credentials()
            cc_creds = read_claude_code_credentials()
            if (nastech_creds and nastech_creds.get("accessToken")) or (cc_creds and cc_creds.get("accessToken")):
                has_creds = True
        except Exception as exc:
...
def _lap_overlay_rows(b: _PickerBuild, data: dict) -> None:
    """Section 2: Nastech-only providers (nastech, openai-codex, copilot, opencode-go, ...)."""
    from agent.models_dev import PROVIDER_TO_MODELS_DEV
    from nastech_cli.providers import NASTECH_OVERLAYS

    # NASTECH_OVERLAYS keys may be models.dev IDs ("github-copilot") while config.yaml uses
    # Nastech IDs ("copilot").
    mdev_to_nastech = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in NASTECH_OVERLAYS.items():
        nastech_slug = mdev_to_nastech.get(pid, pid)
        if _skip(b.seen_slugs, b.excluded, pid, nastech_slug):
            continue
        if not _overlay_has_creds(b, pid, nastech_slug, overlay):
            continue
        if nastech_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = _live_or_curated_ids(nastech_slug, b.curated, merge_models_dev=False,
                                             non_blocking=b.non_blocking_catalogs)
        elif overlay.auth_type == "aws_sdk":
            model_ids = _aws_live_or_curated_ids(nastech_slug, b.curated, nastech_slug, pid,
                                                 non_blocking=b.non_blocking_catalogs)
        elif nastech_slug == "nastech":
            # A guest identity never needs the Portal catalog: add_builtin_row pins nastech/welcome
            # (or drops the row when nastech.guest is off), so only a real account fetches.
            tier_row = _free_tier_nastech_row({"name": get_label(nastech_slug), "models": []})
            real_account = tier_row is not None and not tier_row["models"]
            model_ids = _nastech_picker_model_ids(b.curated, b.force_fresh_nastech_tier) if real_account else []
        else:
            model_ids = _live_or_curated_ids(nastech_slug, b.curated, nastech_slug, pid,
                                             non_blocking=b.non_blocking_catalogs)
        b.add_builtin_row(
            nastech_slug, get_label(nastech_slug), b.current_provider in (nastech_slug, pid), model_ids, "nastech")
...
def _lap_user_provider_rows(b: _PickerBuild, user_providers: dict) -> None:
    ...
    from nastech_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from nastech_cli.config import coerce_provider_id, is_provider_enabled
...
def _lap_custom_provider_rows(b: _PickerBuild, custom_providers: list) -> None:
    ...
    their own rows."""
    from nastech_cli.model_switch import _extra_headers_from_config, _scoped_key_env
    from nastech_cli.config import coerce_provider_id
...
def _build_curated_lists(current_provider: str, current_base_url: str, current_model: str,
    ...
    """Curated model lists keyed by nastech provider id, plus the dynamic ones (nastech manifest,
    ...
    from nastech_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS, get_curated_nastech_model_ids
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # Remote manifest so new Portal models surface without a release; in-repo snapshot fallback.
    curated["nastech"] = get_curated_nastech_model_ids()
    ...
    if "lmstudio" not in curated and (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL") or is_current_lmstudio):
        from nastech_cli.models_local import fetch_lmstudio_models
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None, *, force_fresh_nastech_tier: bool = False,
    ...
    ("built-in", "nastech", "canonical", "user-config", "model-config").
    ``force_fresh_nastech_tier`` bypasses the short Nastech tier cache (account-sensitive flows only);
    ...
    from nastech_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # as (nastech_id / mdev_id / canonical slug).
    b = _PickerBuild(
        current_provider=current_provider, current_base_url=current_base_url, current_model=current_model,
        max_models=max_models, for_picker=for_picker, force_fresh_nastech_tier=force_fresh_nastech_tier,
    ...
    if current_model:
        for row in results:
            ...
            if current_model not in models:
                from nastech_cli.models import _model_requires_account_discovery
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    try:
        from nastech_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    OpenRouter's list is replaced with :func:`nastech_cli.models.fetch_openrouter_models` (curated
