...
def _dashboard_local_update_managed_externally() -> bool:
    """True when the dashboard should not offer ``nastech update``.

    Containerized dashboards are updated by the outer launcher/image — except a
    ``git`` install (bind-mounted checkout, e.g. the nastech-webui image), where
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # install still browses its home). Lock to /opt/data only when the Nastech
    # root actually IS /opt/data or NASTECH_DASHBOARD_FILES_ROOT is set.
    if _default_nastech_root_is_opt_data():
