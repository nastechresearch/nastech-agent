...
from nastech_cli.vercel_auth import describe_vercel_auth
from nastech_constants import OPENROUTER_MODELS_URL
from nastech_state_common import FTS_STORAGE_VERSION
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    if method == "docker":
        ...
        action = f"run `{command}`, then recreate all Nastech containers"
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    locks a live connection is holding (see ``nastech_cli.sqlite_safe_read``).
...
def _report_database_journal_modes(
    ...
    from nastech_state import _wal_reset_repair_hint, is_sqlite_wal_reset_vulnerable

    vulnerable = is_sqlite_wal_reset_vulnerable(version_info)
    home = nastech_home if nastech_home is not None else NASTECH_HOME
    try:
        databases = _nastech_database_paths(home)
...
def _doctor_memory_config(nastech_home: Path | None = None) -> dict:
...
from nastech_cli.sizefmt import format_bytes as _human_bytes
...
def _render_state_db_stats(stats: dict, holders=None) -> list:
    ...
    if isinstance(deferral, dict):
        ...
        lines.append((
            ...
            "(stop the listed processes, then run 'nastech sessions "
    ...
    if logical is not None and logical > STATE_DB_SIZE_WARN_BYTES:
        ...
        if stats.get("fts_rebuild_pending") or stale_trigram:
            detail += (
                "; run 'nastech sessions optimize-storage' offline "
...
_DEPRECATED_ENV_VARS: tuple[tuple[str, str], ...] = (
    ...
    # ignored. NASTECH_TOOL_PROGRESS_MODE is still read by the gateway as a
    # back-compat fallback but remains deprecated.
    ("NASTECH_TOOL_PROGRESS", "display.tool_progress in config.yaml — ignored/unsupported since config floor v12"),
    ("NASTECH_TOOL_PROGRESS_MODE", "display.tool_progress in config.yaml"),
...
def collect_relay_plugin_cutover_findings(
    ...
    """Return actionable findings for the removed Nastech Relay plugin."""
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        ...
        if issues is not None:
            issues.append(
                "Repair the CA bundle: run `nastech doctor --fix`, or "
...
def managed_scope_check() -> None:
    ...
    resolved from the NASTECH_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("NASTECH_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via NASTECH_MANAGED_DIR={managed_dir}")
...
def check_macos_tcc_grants() -> None:
    ...
    Post-#73681 builds pin ``designated => identifier "com.nastechresearch.nastech"``
    ...
    if "cdhash" in dr.lower():
        check_warn(
            ...
            "Run `nastech update` to get the stable identifier-pinned signing "
            "identity, then re-grant permissions once.",
        )
        return
    if "certificate" in dr.lower():
        # Certificate-anchored DR (nastech desktop --setup-tcc-identity, or a
        ...
        check_ok(
            ...
            "anchor, see `nastech desktop --setup-tcc-identity`)",
        )
    check_info(
        "If macOS still re-prompts for permissions (toggle shows ON): the stored "
        "grant is stale — run `tccutil reset ScreenCapture com.nastechresearch.nastech` "
        "(repeat per affected service), toggle it ON in System Settings, then "
        "fully quit & relaunch Nastech once."
    )
...
def _desktop_app_bundle() -> Path | None:
    ...
    (``apps/desktop/release/mac-<arch>/Nastech.app``) — the only layout whose
    ...
    ``_desktop_packaged_executable``'s selection. ``/Applications/Nastech.app``
    is deliberately not probed: it is the separately-signed Nastech-Setup
    launcher (``com.nastechresearch.nastech.setup``, certificate-anchored), whose
...
def check_macos_full_disk_access() -> None:
    ...
    check_info(
        ...
        "app Full Disk Access and Nastech will never trip per-folder dialogs "
        ...
        "    then enable your terminal (and Nastech.app if you use Desktop), "
        "and restart them once. With Nastech' stable signing identities the "
...
def run_doctor(args):
    ...
    try:
        import sqlite3
        from nastech_state import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        ...
        if is_sqlite_wal_reset_vulnerable():
            # Warn-only: Nastech already refuses to enable WAL on fresh DBs.
    ...
    if env_path.exists():
        ...
        # matches nastech_cli.env_loader._load_dotenv_with_fallback.
        try:
    ...
    if config_path.exists():
        ...
        try:
            from nastech_cli.config import load_env, read_user_config_raw, remove_env_value
        ...
        try:
            from nastech_cli.config import load_env as _load_env_depr
            from nastech_cli.config import read_user_config_raw as _read_raw_depr
    ...
    try:
        ...
        nastech_status = get_nastech_auth_status_local()
        if nastech_status.get("logged_in"):
            check_ok("Nastech Portal auth", "(logged in)")
        else:
            check_warn("Nastech Portal auth", "(not logged in)")
    ...
    # disrupt the already-printed Nastech/Codex/Gemini/MiniMax rows above.
    try:
        from nastech_cli.auth import get_xai_oauth_auth_status
    ...
    _memory_config = _doctor_memory_config(nastech_home)
    ...
    if state_db_path.exists():
        try:
            ...
            from nastech_state import _db_opens_cleanly, repair_state_db_schema
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'nastech doctor --fix' "
                        "(or 'nastech sessions repair') to rebuild the FTS index"
                    )
        except Exception as e:
            from nastech_state import is_malformed_db_error, repair_state_db_schema
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'nastech doctor --fix' "
                        "(or 'nastech sessions repair') to recover hidden sessions"
                    )
            else:
                check_warn(f"{_DHH}/state.db exists but has issues: {e}")

        # Health/stats snapshot (#statedb-visibility): a multi-GB state.db
        # with a runaway WAL was previously invisible to every Nastech
        ...
        try:
            from nastech_state import collect_state_db_stats, count_db_holders
                ...
                if _kind == "warn":
                    ...
                    if "auto_prune" in _detail:
                        issues.append(
                            ...
                            "in config.yaml"
                            + (
                                " and run 'nastech sessions optimize-storage' "
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            _fail_and_issue(
                ...
                "Install the Vercel optional dependency: pip install 'nastech-agent[vercel]'",
                issues,
            )
    ...
    if _safe_which("node"):
        ...
        # — it resolves lazily via npx (or a global/Nastech-managed install)
        ...
        if _resolved_ab and _is_npx_agent_browser_sentinel(_resolved_ab):
            ...
            if should_fix:
                ...
                # `nastech update` does, so a session's first browser call
            ...
            # `nastech update` wiped node_modules (issue #48521).
            check_warn(
    ...
    try:
        ...
        if _using_lightpanda_engine():
            ...
            if not _lp_used:
                ...
                check_info(
                    "Fix: pick Lightpanda in `nastech tools` → Browser Automation, "
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable NASTECH_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Nastech one.
                        check_info(
    ...
    # `nastech doctor --live` (real network calls; bounded + read-only).
    try:
        from nastech_cli.doctor_live import maybe_run_live_checks
