...
Mixed into :class:`nastech_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from nastech_cli.middleware import OBSERVER_SCHEMA_VERSION
...
class PluginDispatchMixin:
    ...
    def _invoke_hook_callback(callback: Callable, payload: Dict[str, Any]) -> Any:
        ...
        """
        from nastech_cli.plugins import resolve_plugin_command_result
        try:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from nastech_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"nastech-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="nastech-plugin-events",
            daemon=True,
        )
