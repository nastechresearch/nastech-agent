...
from nastech_cli.urllib_security import open_credentialed_url, url_origin
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from nastech_cli.config import get_custom_provider_tls_settings
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from nastech_cli.codex_models import DEFAULT_CODEX_MODELS, _finalize_codex_models
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Nastech-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $NASTECH_HOME/models_dev_cache.json directly (no network). Falls
...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # endpoint expects (see nastech_cli/model_setup_flows.py).
...
# ---------------------------------------------------------------------------
# Nastech Portal free-model helper
# ---------------------------------------------------------------------------
# The Nastech Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Nastech Portal account tier detection
# ---------------------------------------------------------------------------
...
def partition_nastech_models_by_tier(
    ...
    """Split Nastech models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/nastech/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["nastech"]`` list happens to contain or whether the
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/nastech/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["nastech"]`` list happens to contain or whether
...
def _pricing_profile_key() -> str:
    ...
    return nastech_home_key()


def get_cached_nastech_free_tier() -> Optional[bool]:
...
def check_nastech_free_tier(
    *, force_fresh: bool = False, cached_only: bool = False
) -> bool:
    """Check if the current Nastech Portal user is on a free (unpaid) tier.
    ...
    if not force_fresh:
        cached_result = get_cached_nastech_free_tier()
...
# ---------------------------------------------------------------------------
# Nastech Portal recommended models
#
...
NASTECH_RECOMMENDED_MODELS_PATH = "/api/nastech/recommended-models"
...
def _write_nastech_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logging.getLogger(__name__).debug(
            "nastech recommended-models disk cache write failed: %s", exc
        )
...
def fetch_nastech_recommended_models(
    ...
    *,
    ...
    """Fetch the Nastech Portal's curated recommended-models payload.

    Hits ``<portal>/api/nastech/recommended-models``. The endpoint is public —
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("nastech",           "Nastech Portal",              "Nastech Portal (Everything your agent needs, 300+ models with bundled tool use)"),
...
# In-repo fallback for the model Nastech silently lands on when the user never
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    """
    try:
        from nastech_cli.model_catalog import get_default_model_from_cache
...
# curated-list entry [0]. Metered aggregators (Nastech Portal, OpenRouter) order
...
# ``nastech model``) uses the richer free/paid-tier-aware resolver — see
# ``get_recommended_default_model`` in nastech_cli/web_server.py and
# ``partition_nastech_models_by_tier`` — which can hit the Portal.
_SILENT_DEFAULT_PROVIDERS: frozenset[str] = frozenset({"nastech", "openrouter"})
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Nastech Portal, private mirrors, older catalog snapshots) don't populate
...
# copy, `nastech -p`, a cron job, or a freshly booted gateway answers
...
# the Nastech Portal list different models, and a staging Portal must not answer
# for production.
...
def _reasoning_caps_disk_path() -> Path:
    from nastech_constants import get_nastech_home
    return get_nastech_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(
    ...
    (OpenRouter itself, Nastech Portal). Returns None when the catalog is
    ...
    """
    headers = {"Accept": "application/json", "User-Agent": _NASTECH_USER_AGENT}
    try:
...
# Nastech Portal serves OpenRouter's catalog schema, so the same parser and
...
_nastech_reasoning_caps_cache: dict[str, Optional[dict[str, Any]]] | None = None
_nastech_reasoning_caps_failed_at: float | None = None


def nastech_catalog_url() -> str:
    ...
    ``NASTECH_INFERENCE_BASE_URL`` → resolved credential base → prod — so a
    ...
    """
    return f"{_resolve_nastech_pricing_credentials()[1]}/v1/models"


def _fetch_nastech_reasoning_caps(
    timeout: float = 6.0, *, force: bool = False
) -> Optional[dict[str, Optional[dict[str, Any]]]]:
    """Nastech Portal counterpart of :func:`_fetch_openrouter_reasoning_caps`."""
    global _nastech_reasoning_caps_cache, _nastech_reasoning_caps_failed_at
    if _nastech_reasoning_caps_cache is not None and not force:
        return _nastech_reasoning_caps_cache
    if (
        _nastech_reasoning_caps_failed_at is not None
        and (time.monotonic() - _nastech_reasoning_caps_failed_at) < 60
    ):
        return None
    caps_by_id = _fetch_reasoning_caps_catalog(nastech_catalog_url(), timeout)
    if caps_by_id is None:
        _nastech_reasoning_caps_failed_at = time.monotonic()
        return None
    _nastech_reasoning_caps_cache = caps_by_id
    return caps_by_id


def _refresh_nastech_reasoning_caps() -> None:
    _fetch_nastech_reasoning_caps(force=True)


def nastech_model_reasoning_capabilities(
    model_id: Optional[str],
    *,
    timeout: float = 6.0,
    allow_fetch: bool = False,
) -> Optional[dict[str, Any]]:
    """Return live-catalog reasoning capabilities for a Nastech Portal model.

    Same tri-state contract and cache-only default as
    :func:`openrouter_model_reasoning_capabilities`; warm the cache with
    :func:`warm_nastech_reasoning_caps_async` from hot paths.
    """
    model = str(model_id or "").strip()
    if not model:
        return None
    caps_by_id = _nastech_caps_cached()
    if caps_by_id is None and allow_fetch:
        caps_by_id = _fetch_nastech_reasoning_caps(timeout=timeout)
    if caps_by_id is None:
        return None
    return caps_by_id.get(model)


_nastech_caps_disk_checked = False
_nastech_caps_warm_started = False


def _nastech_caps_cached() -> Optional[dict[str, Optional[dict[str, Any]]]]:
    ...
    """
    global _nastech_reasoning_caps_cache, _nastech_caps_disk_checked
    if _nastech_reasoning_caps_cache is None and not _nastech_caps_disk_checked:
        _nastech_caps_disk_checked = True
        _nastech_reasoning_caps_cache = _hydrate_reasoning_caps_from_disk(
            nastech_catalog_url(), _refresh_nastech_reasoning_caps
        )
    return _nastech_reasoning_caps_cache


def warm_nastech_reasoning_caps_async() -> None:
    """Nastech Portal counterpart of :func:`warm_openrouter_reasoning_caps_async`."""
    global _nastech_caps_warm_started
    if _nastech_caps_warm_started or _nastech_caps_cached() is not None:
        return
    _nastech_caps_warm_started = True
    _warm_reasoning_caps_async(_refresh_nastech_reasoning_caps)
    ...
    *,
    ...
    for preferred_id in preferred_ids:
        ...
        if preferred_id == silent_default:
            ...
            # picker shows which model Nastech lands on when none is selected.
            desc = "default"
        else:
...
def get_curated_nastech_model_ids() -> list[str]:
    """Return the curated Nastech Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["nastech"]`` when the manifest is
    ...
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("nastech", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from nastech_constants import AI_GATEWAY_BASE_URL
...
def compute_sale_discount(
    ...
    Nastech Portal-only feature: callers gate on the provider; this helper only
    sees ``original`` because the Nastech fetch path opted in via
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Nastech Portal).

    When *include_sale_original* is true (Nastech Portal only) and the gateway
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Nastech Portal-only. Never copy pricing.original for
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return nastech-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; nastech's picker expects
...
def _resolve_nastech_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Nastech Portal pricing.

    The Nastech inference ``/v1/models`` endpoint exposes pricing without
    ...
    1. ``NASTECH_INFERENCE_BASE_URL`` env override (staging / preview)
    ...
    try:
        ...
        env_base = _nastech_inference_env_override()
    except Exception:
        env_base = None
    ...
    base_url = (env_base or creds_base or _DEFAULT_NASTECH_INFERENCE_BASE).rstrip("/")
...
def nastech_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    """The Nastech model ids the caller's org may reach, or ``None`` to not filter.
    ...
    """
    try:
        from nastech_cli.nastech_account import nastech_policy_present

        if nastech_policy_present() is not True:
            return None
    except Exception:
        return None
    ...
    # Same arguments as get_pricing_for_provider's nastech branch, so a caller
    ...
    pricing = fetch_models_with_pricing(
        ...
        include_sale_original=True,
        cache_ttl_seconds=_NASTECH_CATALOG_TTL_SECONDS,
    )
...
_NASTECH_POLICY_APPEND_MAX = 64

# How long a Nastech catalog stays trusted. Its contents depend on the org's
...
_NASTECH_CATALOG_TTL_SECONDS = 300.0


def restrict_to_nastech_policy(
    model_ids: list[str],
    ...
    *,
    ...
    if rescue_empty and not kept and len(allowed) <= _NASTECH_POLICY_APPEND_MAX:
        return sorted(allowed)
    return kept


def get_cached_nastech_inference_base_url() -> str:
    """Return the profile's persisted Nastech endpoint without refreshing auth."""
    try:
        from nastech_cli.auth import (
            _load_auth_store,
        ...
        state = _load_provider_state(_load_auth_store(), "nastech") or {}
        return (
            _validate_nastech_inference_url_from_network(
...
def pricing_cache_scope(
    ...
    *,
    ...
    if normalized == "nastech":
        ...
        if normalize_provider(current_provider) == "nastech" and current_base_url:
            return current_base_url.rstrip("/").removesuffix("/v1")
        persisted_base = get_cached_nastech_inference_base_url()
        if persisted_base:
            return persisted_base
        return _pricing_provider_cache_keys.get(
            (_pricing_profile_key(), normalized), _DEFAULT_NASTECH_INFERENCE_BASE
        )
    return ""
    ...
    if normalized == "nastech":
        ...
        if base_url:
            ...
            return fetch_models_with_pricing(
                ...
                base_url=base_url,
                force_refresh=force_refresh,
                # Sale chrome (pricing.original) is Nastech Portal-only.
                include_sale_original=True,
                cache_ttl_seconds=_NASTECH_CATALOG_TTL_SECONDS,
            )
    return {}
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from nastech_cli.providers import custom_provider_slug
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nastech:nastech-3                           →  ("nastech", "nastech-3")
...
def probe_ollama_local_models(
    ...
    try:
        ...
        request_headers = {"User-Agent": _NASTECH_USER_AGENT}
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Nastech, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"nastech", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model nastech` or `/model anthropic`, treat it as a
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            ...
            # curated list is ordered most-capable-first (e.g. Nastech Portal),
            # entry [0] is the priciest flagship, and typing ``/model nastech``
...
def _configured_provider_ids() -> set[str]:
    ...
    Includes both top-level ids (``ollama``, ``nastech``) and ``custom:*``
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    ...
    ``nastech/deepseek-v4-pro`` or ``ollama/qwen3.5:4b`` should route to the
    ...
    ``TestDenormalizeProviderSwitch`` in tests/nastech_cli/test_web_server.py).
...
def _fast_mode_route_supported(
    ...
    OpenRouter, Nastech, Copilot, Azure, Bedrock, and custom base_urls either
...
#   - "nastech": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Nastech),
    ...
    if normalized == "nastech":
        # Try live Nastech Portal /models endpoint
        try:
        ...
        # — NOT the in-repo _PROVIDER_MODELS["nastech"] snapshot — so newly
...
def _credential_fingerprint(provider: str) -> str:
    ...
    (codex, copilot, anthropic-via-claude-code, nastech portal), the
    ...
    # env var, so it must change the fingerprint too or `nastech config set
...
def copilot_model_api_mode(
    ...
    *,
    ...
    # endpoint, not through Nastech' native Anthropic adapter. The live catalog may
...
def _fetch_opencode_free_models(
    ...
    from nastech_cli.urllib_security import open_credentialed_url
...
def probe_api_models(
    ...
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"nastech-agent/{_NASTECH_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=nastech"
...
def validate_requested_model(
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `nastech moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
        ...
        if ollama_models is None:
            return {
                ...
                "message": (
                    ...
                    "Nastech will save the model name, but local Ollama model discovery could not verify it."
                ),
            }
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        # BEFORE the hidden-slug soft-accept below: the suffix is a Nastech
    ...
    if api_models is not None:
        ...
        if _variant_base is not None and _variant_base in set(api_models):
            ...
            # fetch) so we only accept models Nastech actually ships.  (#46850)
            #
            ...
            # Nastech provider: also check the Portal's live
            # /api/nastech/recommended-models feed. That feed can list a model
            # (e.g. a newly-promoted free/paid recommendation) before it's
            # been added to the hardcoded _PROVIDER_MODELS["nastech"] curated
            # list or the docs-hosted catalog manifest has been rebuilt.
            # `nastech chat` already accepts these models via
            ...
            if normalized == "nastech":
                try:
                    portal_payload = fetch_nastech_recommended_models(
                        _resolve_nastech_portal_url()
                    )
                    portal_model_names = {
                        name.lower()
                ...
                if requested_for_lookup.lower() in portal_model_names:
                    return {
                        ...
                        "message": (
                            ...
                            f"listing but is a current Nastech Portal recommendation — accepted."
                        ),
                    }
