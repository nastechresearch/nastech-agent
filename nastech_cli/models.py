...
``nastech_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from nastech_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from nastech_cli.models_reasoning_caps import (
    _OPENROUTER_CATALOG_URL,
    _seed_reasoning_caps)
from nastech_cli.models_local import (
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from nastech_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_nastech_home() / "cache" / "openrouter_curated_catalog.json"
...
# ---------------------------------------------------------------------------
# Nastech Portal free-model helpers — the Portal models endpoint is the source of truth for what is
...
def partition_nastech_models_by_tier(
    ...
    """Split Nastech models into (selectable, unavailable): free-tier users may only select free models
...
def _union_with_portal_recommendations(
    ...
    *, force_refresh: bool, synthesize_free_pricing: bool,
    ...
    """
    try:
        payload = fetch_nastech_recommended_models(portal_base_url, force_refresh=force_refresh)
    except Exception:
        payload = None
...
def check_nastech_free_tier(*, force_fresh: bool = False, cached_only: bool = False) -> bool:
    """True only when the Nastech Portal user is KNOWN to be free-tier (unknown/error → False so this
    ...
    try:
        ...
        result = get_nastech_portal_account_info(force_fresh=force_fresh).is_free_tier
    except Exception:
...
# ---------------------------------------------------------------------------
# Nastech Portal recommended models — curated paid/free suggestions plus dedicated compaction (aux)
...
def _read_nastech_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    blob = _read_json_cache(_nastech_recommended_disk_path(), errors=(OSError, json.JSONDecodeError))
...
def _write_nastech_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.debug("nastech recommended-models disk cache write failed: %s", exc)


def fetch_nastech_recommended_models(
    portal_base_url: str = "", timeout: float = 5.0, *, force_refresh: bool = False
) -> dict[str, Any]:
    """Fetch the Portal's public ``/api/nastech/recommended-models`` payload (no auth).

    Cached per portal URL for ``_NASTECH_RECOMMENDED_CACHE_TTL`` seconds in process (``force_refresh``
    ...
    if not force_refresh and cached is not None and now - cached[1] < _NASTECH_RECOMMENDED_CACHE_TTL:
        return cached[0]
    try:
        data = _get_json(
            f"{base}{NASTECH_RECOMMENDED_MODELS_PATH}", timeout=timeout, headers={"Accept": "application/json"}
        )
    ...
    if data:
        ...
        data = _read_nastech_recommended_disk(base) or data
...
def recommended_nastech_default_model() -> dict[str, Any]:
    """The model a Nastech account lands on without choosing one, honouring the account's tier.
    ...
    ``{"provider": "nastech", "model": str, "free_tier": bool}``; ``model`` may be ``""`` when nothing
    is selectable (callers degrade). Shared by ``GET /api/model/recommended-default`` and the
    sign-in completion in ``nastech_cli.anon_auth`` so both land on the same model.
    """
    from nastech_cli import models_pricing as mp
    from nastech_cli.auth import get_provider_auth_state

    model_ids = get_curated_nastech_model_ids()
    pricing = mp.get_pricing_for_provider("nastech") or {}
    free_tier = check_nastech_free_tier(force_fresh=True)
    try:
        portal_url = (get_provider_auth_state("nastech") or {}).get("portal_base_url", "") or ""
    except Exception:
        portal_url = ""
    # Narrow to policy BEFORE the tier split, so a rescued id still has to pass the free/paid predicate.
    policy_allowed = mp.nastech_policy_allowed_ids()
    ...
    model_ids = mp.restrict_to_nastech_policy(model_ids, policy_allowed, rescue_empty=True)
    if free_tier:
        model_ids, _unavailable = partition_nastech_models_by_tier(model_ids, pricing, free_tier=True)
    return {"provider": "nastech", "model": pick_silent_default_model(model_ids, provider="nastech"),
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (nastech-agent is tool-calling-first).
    Permissive when the field is absent/malformed: some OpenRouter-compatible gateways (Nastech Portal,
...
# Reasoning-capability cache slots, one set per catalog (OpenRouter, Nastech Portal). The logic
...
def fetch_openrouter_models(
    ...
    from nastech_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — nastech-agent requires it and surfacing
...
def get_curated_nastech_model_ids() -> list[str]:
    """Curated Nastech Portal model ids: the remote catalog manifest, else the in-repo
    ``_PROVIDER_MODELS["nastech"]`` snapshot. Always a list."""
    try:
        from nastech_cli.model_catalog import get_curated_nastech_models
        remote = get_curated_nastech_models()
    except Exception:
        remote = None
    return list(remote or _PROVIDER_MODELS.get("nastech", []))
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``nastech model`` and ``/model``)."""
...
def detect_static_provider_for_model(
    ...
    # Step 0: a bare provider name typed as the model (`/model nastech`) is a provider switch to that
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if resolved_provider in _PROVIDER_LABELS and default_models and resolved_provider not in current_keys:
            ...
            # so [0] would silently escalate `/model nastech` to the priciest flagship.
...
def _resolve_provider_prefix(model_name: str) -> Optional[tuple[str, str]]:
    """Route an explicit ``vendor/model`` prefix (``nastech/deepseek-v4-pro``, ``ollama/qwen3.5:4b``) to
    a provider the user defined in ``providers:`` (by raw name or alias) instead of the default.

    ``nastech/deepseek-v4-pro`` or ``ollama/qwen3.5:4b`` should route to the named provider instead of falling
...
def detect_provider_for_model(
    ...
    NAMED the provider (``/model nastech``), or there is no current provider yet (``auto``) — then the
    first guess is returned so the credential step fails loudly instead of silently ignoring input."""
    from nastech_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``nastech_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from nastech_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _nastech_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
    ...
    # Portal models still surface without a Nastech release.
    return get_curated_nastech_model_ids() or None
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``nastech model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Nastech actually ships. (#46850)
...
# every authed provider's /v1/models). One JSON file at $NASTECH_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    inflight_key = cache_key if get_nastech_home_override() is None else (nastech_home_key(), cache_key)
...
def _credential_fingerprint(provider: str) -> str:
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_nastech_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``nastech model --refresh``."""
    try:
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _NASTECH_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from nastech_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('nastech_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('nastech_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('nastech_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('nastech_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('nastech_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('nastech_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('nastech_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('nastech_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('nastech_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nastech_inference_base_url': ('nastech_cli.models_pricing', 'get_cached_nastech_inference_base_url'),
    'get_pricing_for_provider': ('nastech_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('nastech_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('nastech_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nastech_catalog_url': ('nastech_cli.models_reasoning_caps', 'nastech_catalog_url'),
    'nastech_model_reasoning_capabilities': ('nastech_cli.models_reasoning_caps', 'nastech_model_reasoning_capabilities'),
    'nastech_policy_allowed_ids': ('nastech_cli.models_pricing', 'nastech_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('nastech_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('nastech_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('nastech_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('nastech_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('nastech_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('nastech_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('nastech_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('nastech_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('nastech_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nastech_policy': ('nastech_cli.models_pricing', 'restrict_to_nastech_policy'),
    'should_use_ollama_native_catalog': ('nastech_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('nastech_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('nastech_cli.models_validate', 'validate_requested_model'),
    'warm_nastech_reasoning_caps_async': ('nastech_cli.models_reasoning_caps', 'warm_nastech_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('nastech_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
