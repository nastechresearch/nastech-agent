"""nastech fallback — manage the fallback provider chain (tried in order when the primary fails).

Subcommands: ``list`` (default), ``add`` (same picker as `nastech model`), ``remove``, ``clear``.
"""
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    print("  Docs: https://nastechresearch.github.io/nastech-agent/docs/user-guide/features/fallback-providers\n")
...
def cmd_fallback_add(args) -> None:
    ...
    print("\n  Adding a fallback provider.  The picker below is the same one used by\n"
          "  `nastech model` — select the provider + model you want as a fallback.\n")
