"""Helpers shared by the Bitwarden and 1Password ``nastech secrets`` CLIs.

Import-light on purpose: ``nastech_cli.secrets_cli`` must stay free of the Bitwarden backend
...
def require_enabled(console: Console, cfg: dict, product: str, command: str) -> bool:
    ...
    console.print(f"[yellow]{product} integration is disabled.  Run "
                  f"`nastech secrets {command} setup` first.[/yellow]")
    return False
