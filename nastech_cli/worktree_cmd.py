"""``nastech worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``nastech_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    nastech worktree list             # audit: verdict + reason per tree
    nastech worktree prune            # reap safe trees + merged branches
    nastech worktree prune --dry-run  # show the plan, change nothing
    nastech worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `nastech worktree prune`."
        )
