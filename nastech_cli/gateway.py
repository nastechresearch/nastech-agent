...
import asyncio
import contextlib
from nastech_cli.cli_output import line_input
import json
import logging
import os
import shlex
...
from nastech_cli import setup_platforms
...
# display_nastech_home is imported lazily: nastech_constants may be a cached pre-update version.
...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``all_profiles`` widens the current profile's unit/label to the whole ``nastech-gateway*`` /
    ``ai.nastech.gateway*`` fleet so update/reaper never kill a sibling's service gateway as "manual".

    ``all_profiles`` widens the launchd branch to every installed ``ai.nastech.gateway*`` LaunchAgent — the
    ...
    the ``nastech-gateway*`` fleet glob.
    """
    pids: set = set()

    # --- systemd (Linux): user and system scopes ---
    if supports_systemd_services():
        pattern = "nastech-gateway*" if all_profiles else get_service_name()
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                # Belt-and-suspenders for the EXCLUDE use case (#74075): a bare ``launchctl list`` prefix
                # scan also catches ai.nastech.gateway* agents the label derivation can't map (renamed
    ...
    if is_macos():
        ...
        if all_profiles:
            # Whole fleet, mirroring the systemd ``nastech-gateway*`` glob above.
            # Every gateway LaunchAgent, not just the invoking profile's — mirrors the systemd branch's
            # ``nastech-gateway*`` pattern above. The update path restarts the whole fleet, and its
        ...
        for label in sorted(labels):
            ...
            if pid is not None and pid > 0:
                pids.add(pid)
        if all_profiles:
            # Prefix scan also catches ai.nastech.gateway* agents the label derivation can't map
            ...
            try:
                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().splitlines():
                        parts = line.split()
                        if len(parts) >= 3 and parts[-1].startswith("ai.nastech.gateway"):
                            try:
                                pid = int(parts[0])
                                if pid > 0:
                                    pids.add(pid)
...
# its full budget and `nastech update` can deadlock. Two witnesses classify the loop BEFORE any
...
# fail", and `nastech update` can deadlock behind it. The loop publishes a liveness signal precisely for this
...
def _get_ancestor_pids() -> set[int]:
    """PIDs of this process and its ancestors, so scans never count the invoking ``nastech`` CLI as a gateway.

    Walks from the current PID up to PID 1 (init) so that process-table scans never match the calling CLI
    process or any of its parents. This prevents ``nastech gateway status`` from falsely counting the
    ``nastech`` CLI that invoked it as a running gateway instance (see #13242).
    """
...
def _scan_gateway_pids(
    ...
    # Exclude the entire ancestor chain so the CLI process that invoked this scan (e.g. ``nastech gateway
    ...
    # Forward slashes on both sides of the NASTECH_HOME= match (mirrors gateway.status).
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        # fallback stop could SIGTERM the named gateway). NASTECH_HOME may come via env (invisible to
        # wmic/CIM), so only a non-matching explicit NASTECH_HOME= disqualifies.
        if profile_flag_value(command_lc) is not None:
            return False
        return not ("nastech_home=" in command_lc and f"nastech_home={current_home_lc}" not in command_lc)
...
def _windows_process_listing() -> str | None:
    ...
    # failure or timeout (result is None) trips the fallback. ``nastech update`` hung exactly there on
    ...
    from nastech_cli._subprocess_compat import bounded_probe_run
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    """Find running gateway PIDs for the current profile, or every profile with ``all_profiles`` (``nastech update``)."""
...
def find_windows_gateway_services(
    *, psutil_module=None, profile_processes: list[ProfileGatewayProcess] | None = None
    ...
    command lines, so identity = Nastech's own PID file + a parent chain ending at a running SCM service
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``nastech update``: ``--external-supervisor`` gateways
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    if old_pid <= 0 or not run_argv:
        return False
    from nastech_cli._subprocess_compat import windows_detach_flags_without_breakaway, windows_detach_popen_kwargs
    ...
    # normalizes the interpreter and captures a stable cwd + env overlay (NASTECH_HOME,
    ...
    if sys.platform == "win32":
        try:
            from nastech_cli.gateway_windows import windowless_gateway_restart_spec
    ...
    watcher = textwrap.dedent(
        ...
        try:
            ...
            from pathlib import Path
            _log_dir = Path(get_nastech_home()) / "logs"
        ...
        # NASTECH_HOME) the windowless base interpreter needs to import nastech_cli. Empty on POSIX.
        if _respawn_cwd:
...
def _nastech_home_pinned_by_unit(unit_path: Path) -> str | None:
    """``NASTECH_HOME`` pinned by the unit file at *unit_path*, or None when absent/unreadable."""
    try:
    ...
    for line in text.splitlines():
        ...
        if body.startswith("Environment="):
            ...
            if body.startswith("NASTECH_HOME="):
                return body.split("=", 1)[1].strip().strip('"') or None
    return None


def _nastech_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """``NASTECH_HOME`` from the on-disk unit file - what refresh/compare already read, and reliable under ``sudo``."""
    return _nastech_home_pinned_by_unit(get_systemd_unit_path(system=system))


def _sync_nastech_home_from_systemd_unit(system: bool) -> None:
    """Adopt a system-scope unit's ``NASTECH_HOME``: under ``sudo`` it is stripped and HOME=/root, so
    get_nastech_home() would pick the wrong profile for runtime-status/PID reads."""
    if not system:
        return
    # On-disk unit first; ``systemctl show`` for units that only exist in the manager.
    unit_home = (_nastech_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        env_line = _systemctl_show(("Environment",), system=True).get("Environment", "")
        unit_home = _parse_kv_pairs(env_line.split()).get("NASTECH_HOME", "").strip()
    if unit_home and os.environ.get("NASTECH_HOME", "").strip() != unit_home:
...
def _wait_for_systemd_service_restart(
    *,
        ...
        if active_state == "active" and new_pid and (previous_pid is None or new_pid != previous_pid):
            ...
            if gateway_state in ("running", "degraded"):
                ...
                if gateway_state == "degraded":
                    ...
                    print(f"⚠ {scope_label} gateway is DEGRADED — see `nastech gateway status`")
                return True
    ...
    print(
        ...
        f"  Check status: {sudo}nastech gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {sudo}nastech gateway restart{scope_flag}")
...
def _s6_gateway_snapshot(gateway_pids: tuple[int, ...]) -> GatewayRuntimeSnapshot | None:
    ...
    from nastech_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_multiplex_standalone_reason() -> None:
    ...
    if reason:
        ...
        print("  Fold every profile onto this gateway: nastech gateway migrate --multiplex")
        print("  Keep per-profile gateways: nastech config set gateway.multiplex_profiles false")
...
def _print_served_ingress_urls(profile: str | None = None) -> None:
    ...
    try:
        from nastech_cli.gateway_multiplex_served import format_ingress_url_lines, served_profile_ingress_urls
...
def _print_unserved_shared_ingress(profile: str | None) -> None:
    ...
    try:
        from nastech_cli.gateway_multiplex_served import served_profile_unserved_platforms
...
def _print_other_profiles_gateway_status() -> None:
    """Print other profiles' running gateways at the bottom of ``nastech gateway status``."""
    try:
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    if is_windows():
        try:
            from nastech_cli.gateway_windows import get_task_name  # profile-aware task name
            _task_name = get_task_name()
        except Exception:
            _task_name = "Nastech_Gateway"
...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int]:
    ...
    with contextlib.suppress(Exception):
        ...
        # cover the whole ai.nastech.gateway* fleet — not just the current profile's label — or a sibling
...
def stop_profile_gateway() -> bool:
    ...
    if is_windows():
        ...
        # wedged process.
        from gateway.status import get_process_start_time
        from nastech_cli.gateway_windows import (
            _drain_gateway_pid,
...
def _gw_windows():
    """Lazily import :mod:`nastech_cli.gateway_windows` (Windows-only service backend)."""
...
def _gateway_detached_env() -> bool:
    return _truthy_env(os.getenv("NASTECH_GATEWAY_DETACHED"))
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    """True for detached Windows gateway runs that should ignore Ctrl+C (``NASTECH_GATEWAY_DETACHED=1``
...
def _windows_gateway_breakaway_state() -> bool | None:
    ...
    if not is_windows():
        return None
    from nastech_cli._subprocess_compat import _WINDOWS_GATEWAY_BREAKAWAY_ENV
...
def _native_service_homes() -> set[Path]:
    ...
    from nastech_constants import _get_platform_default_nastech_home, sudo_invoker_default_home

    homes = {_get_platform_default_nastech_home().resolve()}
...
def _bare_unit_pinned_home() -> Path | None:
    """Resolved ``NASTECH_HOME`` pinned by an installed ``nastech-gateway.service``, or None. The unit is the
    ...
    if not is_linux() or os.geteuid() != 0:  # windows-footgun: ok — behind is_linux()
        return None
    pinned = _nastech_home_pinned_by_unit(_SYSTEM_UNIT_DIR / f"{_SERVICE_BASE}.service")
    if not pinned:
        return None
    try:
...
def _profile_suffix() -> str:
    """Service-name suffix for NASTECH_HOME: "" for a home that owns the bare name, the profile name for
    ``<root>/profiles/<name>``, else a short hash of the path.

    Bare-name owners: this process's platform-native default (``~/.nastech``), under sudo the invoking
    user's native default, and the home pinned by an installed ``nastech-gateway.service``. Under sudo the
    naming basis moves MID-COMMAND — sudo strips NASTECH_HOME and sets HOME=/root, then
    ``_sync_nastech_home_from_systemd_unit()`` adopts the unit's own NASTECH_HOME into ``os.environ`` — so a
    basis derived from the process alone names one unit before the adoption and another after it. The
    unit-pinned check must precede the profile branch: ``sudo nastech gateway install --system`` resolves
    ...
    The bare name is deliberately NOT tied to ``get_default_nastech_root()``: that helper treats any
    NASTECH_HOME outside ``~/.nastech`` (Docker ``/opt/data``, a temp dir) as "the root itself", which let a
    temp-home harness resolve to the default profile's ``nastech-gateway`` unit and uninstall the
    ...
    if home in _native_service_homes() or home == _bare_unit_pinned_home():
        return ""
    name = _profile_name_from_home(home, get_default_nastech_root().resolve())
...
def _current_profile_name() -> str:
    ...
    from nastech_constants import profile_name_for_home
    return profile_name_for_home(get_nastech_home()) or _profile_suffix()


def _profile_arg(nastech_home: str | None = None, default_root: str | Path | None = None) -> str:
    """``--profile <name>`` for ``<root>/profiles/<name>``, else "". *nastech_home*/*default_root* let a
    ...
    default = Path(default_root).resolve() if default_root else get_default_nastech_root().resolve()
    if home == default:
        return ""
...
def get_service_name() -> str:
    """Systemd service name: ``nastech-gateway`` for default NASTECH_HOME, ``nastech-gateway-<profile>``
    or ``-<hash>`` otherwise."""
...
def _run_systemctl(args: list[str], *, system: bool = False, **kwargs) -> subprocess.CompletedProcess:
    ...
    try:
        ...
        from nastech_cli.gateway_command_errors import SystemctlUnavailableError
...
def _find_legacy_nastech_units() -> list[tuple[str, Path, bool]]:
    """``[(unit_name, unit_path, is_system)]`` for legacy gateway units (e.g. ``nastech.service``), which
    ...
    Detects unit files installed by older Nastech versions that used a different service name (e.g. When both
    a legacy unit and the current ``nastech-gateway.service`` are active, they fight over the same bot token
...
def remove_legacy_nastech_units(interactive: bool = True, dry_run: bool = False) -> tuple[int, list[Path]]:
...
def refuses_container_user_scope_install(system: bool) -> bool:
    ...
    _print_info_lines(
        ...
        "",
        "  nastech gateway run                                # run as the container's main process",
        ...
        "  sudo nastech gateway install --system --run-as-user <user>",
    )
    return True
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: nastech gateway install"
            )
            return scope, False
...
def ensure_gateway_service(context: str = "setup") -> bool:
    """Install and start a user-scope gateway service without prompting (``nastech setup``/``import``).
    ...
    if not (supports_systemd or is_macos() or is_windows()):
        ...
        print_info("  Run the gateway in the foreground with: nastech gateway")
        return False
    ...
    try:
        ...
        print_info("  You can retry manually: nastech gateway install")
...
def get_launchd_plist_path() -> Path:
    """``~/Library/LaunchAgents/ai.nastech.gateway[-<profile>].plist`` under the real account home."""
    import pwd
    suffix = _profile_suffix()
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    the profile layout, NOT by globbing ``~/Library/LaunchAgents``, so a sandboxed NASTECH_HOME never
    ...
    for profile in list_profiles():
        if profile.is_default:
            root_label.append("ai.nastech.gateway")
        elif _re.match(r"^[a-z0-9][a-z0-9_-]{0,63}$", profile.name):
            profile_labels.append(f"ai.nastech.gateway-{profile.name}")
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # Update-boundary: a gateway restarted mid-update can hold a stale nastech_constants
            # without this symbol; see _reload_nastech_constants() in nastech_cli/managed_uv.py.
            from nastech_cli.managed_uv import _reload_nastech_constants
            venv_python_path = _reload_nastech_constants().venv_python_path
...
def _nastech_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current NASTECH_HOME (root's, under sudo) to the target user's equivalent:
    ``/root/.nastech[/profiles/x]`` → ``/home/alice/.nastech[/profiles/x]``; custom paths kept as-is."""
    current_nastech_raw = os.environ.get("NASTECH_HOME", "").strip()
    current_nastech = Path(current_nastech_raw).expanduser() if current_nastech_raw else get_nastech_home()
    # Keep paths lexical: resolving a non-existent path can bake a different NASTECH_HOME into the unit.
    current_default = Path.home() / ".nastech"
    target_default = Path(target_home_dir) / ".nastech"
    try:
        # Default ~/.nastech or a profile/subdir of it → preserve the relative structure under the target.
        return str(target_default / current_nastech.relative_to(current_default))
    except ValueError:
        return str(current_nastech)  # Completely custom path (not under ~/.nastech) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    extras = (project_root / "node_modules" / ".bin", nastech_home / "node" / "bin", nastech_home / "node_modules" / ".bin")
    for extra in extras:
        if _is_dir(extra):
...
def _stable_service_working_dir() -> str:
    """WorkingDirectory that won't disappear under systemd (NASTECH_HOME, else PROJECT_ROOT). cwd is
...
def _systemd_watchdog_seconds(nastech_home: str | Path | None = None) -> int:
    ...
    if nastech_home is not None:
        ...
        override_token = set_nastech_home_override(nastech_home)
        reset_home_override = reset_nastech_home_override
    try:
...
def _append_node_dir_for_service(path_entries: list[str], nastech_root: Path | None = None) -> None:
    """Append the Node dir a service unit should use: managed ``<nastech_root>/node`` (profile-scoped)
    first — a unit survives reboots, so baking a shell-PATH Node is permanent breakage — else PATH lookup."""
    from nastech_constants import (nastech_managed_node_tree_present, iter_nastech_node_dirs)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Profile arg relative to the TARGET user's ~/.nastech when nastech_home lives under it.
        target_root = Path(home_dir) / ".nastech"
        try:
            Path(nastech_home).resolve().relative_to(target_root.resolve())
            profile_arg = _profile_arg(nastech_home, default_root=target_root)
        except ValueError:
        ...
        _append_node_dir_for_service(_target_node_entries, Path(nastech_home) if nastech_home else None)
    ...
    watchdog_seconds = _systemd_watchdog_seconds(nastech_home)
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}{identity_lines}ExecStart={python_path} -m nastech_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
{env_lines}Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="NASTECH_HOME={nastech_home}"
Environment="NASTECH_SUPERVISED_CHILD=1"
Restart=always
RestartSec=5
...
def _normalize_launchd_plist_for_comparison(text: str) -> str:
    ...
    import re
    return re.sub(
        r"(<key>PATH</key>\s*<string>)(.*?)(</string>)", r"\1__NASTECH_PATH__\3",
        _normalize_service_definition(text), flags=re.S,
    )


def systemd_unit_is_current(system: bool = False) -> bool:
    # NASTECH_HOME sync chokepoint for every compare/regenerate path: under `sudo … --system` it is often
    # stripped to /root/.nastech, so refresh would rewrite a correct unit and status warn forever.
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Temp-dir NASTECH_HOME baked into a systemd unit / launchd plist, or None. A temp home means a
    test/E2E harness generated it; installing it leaves the gateway "running" but deaf to every platform."""
    import re
    import tempfile
    candidates = re.findall(r'NASTECH_HOME=([^"\n]+)', definition)
    candidates += re.findall(r"<key>NASTECH_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S)
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp NASTECH_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(f"✗ Refusing to write the gateway {kind}: NASTECH_HOME resolves to a temporary directory ({temp_home}).")
    print(
        "  This usually means a test/E2E environment exported NASTECH_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # systemd_unit_is_current is the NASTECH_HOME-sync chokepoint; its env mutation persists for the regenerate below.
    ...
    # pytest-tmp NASTECH_HOME baked into the developer's real unit breaks their gateway on next reboot.
    if not system and any(m in new_unit for m in ("/pytest-of-", '/nastech_test"', "/nastech_test/")):
        return False

    # Structural variant: refuse ANY temp-dir NASTECH_HOME (manual E2E homes lack the pytest markers).
    if _refuse_temp_home_service_write(new_unit, "systemd unit"):
        return False

    unit_path.write_text(new_unit, encoding="utf-8")
    _run_systemctl(["daemon-reload"], system=system, check=True, timeout=30)
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Nastech install")
    return True
...
def _get_cron_drain_timeout() -> float:
    ...
    See #82161.
    """
    return _agent_timeout_setting("NASTECH_CRON_DRAIN_TIMEOUT", "cron_drain_timeout", parse_cron_drain_timeout)
...
def _get_restart_exit_wait_budget() -> float:
    ...
    return resolve_restart_exit_wait_budget(
        ...
        _agent_timeout_setting(
            "NASTECH_RESTART_AFTER_TURN_TIMEOUT", "restart_after_turn_timeout", parse_restart_after_turn_timeout
        ),
    )
...
def systemd_install(
    ...
    # Existing system units already pin NASTECH_HOME; adopt it before any regenerate.
    if unit_path.exists():
        _sync_nastech_home_from_systemd_unit(system=system)
    ...
    print()
    print("Next steps:")
    print(f"  {sudo}nastech gateway start{scope_flag}              # Start the service")
    print(f"  {sudo}nastech gateway status{scope_flag}             # Check status")
...
def _systemd_unit_belongs_to_current_home(system: bool = False) -> bool:
    """False (with a warning) when the installed unit pins a NASTECH_HOME other than this process's: the
    service name then resolved to ANOTHER install's gateway, and stop/disable/unlink would take it down."""
    _sync_nastech_home_from_systemd_unit(system=system)  # sudo strips NASTECH_HOME; adopt the unit's first
    unit_home = _nastech_home_from_systemd_unit_file(system=system)
    if unit_home is None or Path(unit_home).expanduser().resolve() == get_nastech_home().resolve():
        return True
    print_warning(
        f"Refusing to remove {get_systemd_unit_path(system=system)}: it runs NASTECH_HOME={unit_home}, "
        f"but this process has NASTECH_HOME={get_nastech_home()}"
    )
    return False
...
def _print_service_not_installed(system: bool) -> None:
    ...
    print(f"  Run: {sudo}nastech gateway install{scope_flag}")
...
def systemd_start(system: bool = False):
    ...
    # NASTECH_HOME sync happens in refresh's systemd_unit_is_current gate; the unit is guaranteed to exist here.
    refresh_systemd_unit_if_needed(system=system)
...
def systemd_restart(system: bool = False):
    ...
    # NASTECH_HOME sync happens in refresh's systemd_unit_is_current gate; its os.environ mutation
    ...
    if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
        ...
        print(
            ...
            # graceful shutdown, so waiting the full drain budget only stalls the restart (and `nastech
...
def _systemd_graceful_restart_action(system: bool, pid: int) -> str | None:
    ...
    from nastech_cli.update_cmd_drain_report import drain_progress_reporter
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {sudo}nastech gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if result.stdout.strip() == "active":
        ...
        print(f"  Run: {sudo}nastech gateway start{scope_flag}")
    ...
    if active_state == "activating" and unit_props.get("SubState", "") == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {sudo}nastech gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {user_flag}reset-failed {svc} && {sudo}nastech gateway start{scope_flag}")
...
# at all and we degrade to a detached background process (the `nohup nastech gateway run` workaround). See
# #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_nastech_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_nastech_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build ``python -m nastech_cli.main [--profile X] gateway run --replace``, honoring the active profile."""
    return [get_python_path(), "-m", "nastech_cli.main", *_profile_arg().split(), "gateway", "run", "--replace"]
...
def _timestamped_stderr_gateway_command(error_log: Path, *, external_supervisor: bool = False) -> list[str]:
    ...
    (launchd ProgramArguments only) adds ``--external-supervisor`` so ``nastech update`` hands back to
    ...
    ``--external-supervisor`` so ``nastech update`` sees the flag on the live grandchild argv and hands the
    ...
    return [get_python_path(), "-m", "nastech_cli.stderr_timestamp", "--error-log", str(error_log), "--", *inner]
...
def _spawn_detached_gateway() -> bool:
    ...
    the `nohup nastech gateway run --replace` workaround but keeps it CLI-managed: stdout goes to
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(f"  Try manually: nohup nastech gateway run --replace > {_dhh()}/logs/gateway.log 2>&1 &")
    if exit_on_failure:
        sys.exit(1)
    return False
...
def _launchd_degrade_or_raise(exc: subprocess.CalledProcessError, what: str) -> None:
    ...
    if _launchctl_label_supervising_process(label):
        ...
        print("  Apply the definition with: nastech gateway stop && nastech gateway install --force")
        raise exc
...
def generate_launchd_plist() -> str:
    ...
    nofile_block = ""
    try:
        from nastech_cli.resource_limits import configured_nofile_soft_limit
...
<dict>
    ...
    <dict>
        ...
        <key>NASTECH_SUPERVISED_CHILD</key>
        <string>1</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if not _retry_launchctl_bootstrap_until_registered(domain, plist_path, label, deadline=_deadline):
        ...
        return False
    print("↻ Updated gateway launchd service definition to match the current Nastech install")
    return True
        ...
        if not launchd_plist_is_current():
            ...
            if refresh_launchd_plist_if_needed():
                ...
                print(
                    ...
                    "Run 'nastech gateway install --force' or check "
                    f"{display_nastech_home()}/logs/launchd-reload.log for details."
                )
            return
...
def launchd_stop():
    ...
    _mark_planned_stop()
    # bootout unloads the definition so KeepAlive doesn't respawn; `nastech gateway start` re-bootstraps.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    NASTECH_HOMEs work) to exit; SIGKILL it after ``force_after`` s of graceful waiting."""
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # only stalls the restart (and `nastech update`). Bounded SIGTERM → SIGKILL, ~10s.
...
def launchd_status(deep: bool = False):
    ...
    # Nastech PID may be a detached fallback process; when launchd IS supervising both PIDs match — don't double-count.
    from gateway.status import get_running_pid
    ...
    if not service_listed:
        ...
        if fallback_pid:
            ...
            print("  Cron jobs will fire. Stop with: nastech gateway stop")
        else:
...
def _is_official_docker_checkout() -> bool:
    return str(PROJECT_ROOT) == "/opt/nastech" and (PROJECT_ROOT / "docker" / "entrypoint.sh").is_file()
...
def _running_under_gateway_supervisor() -> bool:
    ...
    (shells inherit "0"), s6 NASTECH_S6_SUPERVISED_CHILD, or ``--external-supervisor``."""
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    try:
        from nastech_cli.gateway_multiplex_served import live_default_gateway_pid, recorded_served_profiles
        ...
        # default profile (`nastech -p X` loads X's .env) and a config edit after start is not live yet.
        ...
        from nastech_cli.gateway_multiplex_mode import explicit_multiplex_flag
...
def _served_profile_needs_no_service() -> bool:
    ...
    Shared by ``nastech setup gateway`` / ``nastech setup`` / ``nastech import`` (``ensure_gateway_service``)
    and the ``nastech gateway setup`` wizard. See #111958."""
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    "already running". Same user-facing contract; never scans other NASTECH_HOME roots."""
    ...
    try:
        ...
        return
    if pid is None:
        # get_running_pid() filters by the current profile's NASTECH_HOME; warn if the PID file
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                logger.warning(
                    "PID file belongs to another profile (nastech_home=%s). "
                    "The old gateway may still be running under that profile.",
                    stale.get("nastech_home", "<unknown>"),
                )
        except Exception:
            pass
        return
    ...
    print("  Check with `nastech gateway status`.")
    print("  To restart it: `nastech gateway restart`. To stop it: `nastech gateway stop`.")
    print("  To replace it from here: `nastech gateway run --replace`.")
    sys.exit(1)


def _guard_official_docker_root_gateway() -> None:
    """Refuse gateway startup when the official Docker privilege drop was bypassed."""
    if not hasattr(os, "geteuid") or os.geteuid() != 0 or _truthy_env(os.getenv("NASTECH_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error("Refusing to run the Nastech gateway as root inside the official Docker image.")
    print(
    ...
    print("  Set NASTECH_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def _apply_startup_watchdog_config() -> None:
    ...
    try:
        from nastech_startup_watchdog import (
        ...
        try:
            from nastech_cli.config import load_config as _sw_load_config
...
def _make_exit_diag():
    ...
    ``asyncio.run()`` can return, for chasing silent Windows gateway deaths. NASTECH_GATEWAY_EXIT_DIAG=0 opts out."""
...
def _respawn_storm_backoff() -> None:
    ...
    DEFAULT_CONFIG ``gateway.respawn_storm``; NASTECH_GATEWAY_MAX_STARTS / NASTECH_GATEWAY_START_WINDOW_S
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            _max_starts = int(os.environ["NASTECH_GATEWAY_MAX_STARTS"])
        except (KeyError, ValueError):
            pass
        try:
            _win = float(os.environ["NASTECH_GATEWAY_START_WINDOW_S"])
        except (KeyError, ValueError):
            pass
        ...
        if _storm is not None:
            ...
            try:
                from nastech_startup_watchdog import kick_startup_watchdog
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # Detached Windows runs (NASTECH_GATEWAY_DETACHED=1, or non-TTY for older wrappers) ignore
    ...
    # exit-code-75 respawn (stale-code or /restart), which bypasses `nastech gateway restart`.
    if supports_systemd_services():
        try:
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           𓄃 Nastech Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    {
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL",
             ...
             "password": False, "help": "Channel ID where Nastech delivers cron results and notifications."},
...
def _runtime_health_lines() -> list[str]:
    ...
    if gateway_state == "startup_failed" and exit_reason:
        ...
        if isinstance(work, list) and work:
            from nastech_cli.update_cmd_drain_report import describe_active_work_unit
...
def _setup_standard_platform(platform: dict):
    ...
    from nastech_cli.setup_hidden_env import is_setup_hidden_env as _is_setup_hidden_env
...
def _setup_weixin():
    ...
    print()
    _print_info_lines(
        "  1. Nastech will open Tencent iLink QR login in this terminal.",
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown users can request access; approve with `nastech pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    """Resolve a built-in platform's setup function; late-bound to dodge the nastech_cli.setup cycle."""
...
_WIZARD_NO_SERVICE_LINES = {
    "wsl": (
        "  WSL detected but systemd is not running.", "  Run in foreground: nastech gateway run",
    ...
    "termux": (
        "  Termux does not use systemd/launchd services.", "  Run in foreground: nastech gateway run",
        "  Or start it manually in the background (best effort): nohup nastech gateway run >{home}/logs/gateway.log 2>&1 &",
    ),
    "unsupported": (
        "  Service install not supported on this platform.", "  Run in foreground: nastech gateway run",
    ),
}
...
def gateway_command(args):
    ...
    try:
        ...
        from nastech_cli.gateway_command_errors import explain_service_failure
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    ``NASTECH_S6_SUPERVISED_CHILD`` (set by ``S6ServiceManager._render_run_script``) marks the supervised
    child, which must run in foreground or we'd recurse run → start → run; ``--no-supervise`` /
    NASTECH_GATEWAY_NO_SUPERVISE=1 opts out (CI smoke, debugging)."""
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("NASTECH_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    # NASTECH_S6_SUPERVISED_CHILD: we ARE the supervised child; fall through so the gateway starts.
    if no_supervise or os.environ.get("NASTECH_S6_SUPERVISED_CHILD"):
        return False
    if not _dispatch_via_service_manager_if_s6("start"):
        return False
    # This process never reaches a GatewayRunner, so the watchdog armed by nastech_cli.main's argv
    ...
    from nastech_startup_watchdog import disarm_startup_watchdog
    ...
    try:
        ...
        # down the supervised gateway cleanly). Prefer `sleep infinity` (matches the static main-nastech
        # service's pattern in docker/s6-rc.d/main-nastech/run, and frees the Python interpreter — the
...
def _refuse_from_inside_gateway(verb: str, reason: str) -> None:
    ...
    if _is_supervised_gateway_process():
        print_error(
            ...
            f"Use `nastech gateway {verb}` from a shell outside the running gateway."
        )
        sys.exit(1)
...
_NO_BACKEND_MESSAGES = {
    ("install", "termux"): (1,
        "Gateway service installation is not supported on Termux.", "Run manually: nastech gateway"),
    ("install", "wsl"): (1,
    ...
    ("install", "s6"): (None,
        ...
        "  nastech -p <name> gateway start   # bring it up via s6",
    ...
    ("install", "container"): (0,
        ...
        "To run the gateway: nastech gateway run"),
    ("install", "unsupported"): (1,
        "Service installation not supported on this platform.", "Run manually: nastech gateway run"),
    ("uninstall", "termux"): (1,
        "Gateway service uninstall is not supported on Termux because there is no managed service to remove.",
        "Stop manual runs with: nastech gateway stop"),
    ("uninstall", "s6"): (None,
    ...
    ("start", "container"): (0,
        ...
        "Or run the gateway directly: nastech gateway run"),
    ("start", "unsupported"): (1,
        "Running the gateway as a background service is not available on this platform "
        "(no systemd, launchd or Scheduled Tasks).",
        "Run it directly with: nastech gateway run"),
}
...
def _cmd_stop(args):
    ...
    if not stop_all and not find_gateway_pids() and named_profile_served_by_running_multiplexer():
        ...
        print()
        print("    nastech gateway stop      # takes every served profile offline")
        print("    nastech gateway restart")
        sys.exit(GATEWAY_FATAL_CONFIG_EXIT_CODE)
...
def _cmd_restart(args):
    ...
    # detached run below is the restart; bailing here left `nastech gateway restart` a
    ...
    if kind == "systemd" and supports_systemd_services():
        ...
        if linger_ok is not True:
            ...
            _print_lines(
                ...
                "  Then restart the gateway:", "    nastech gateway restart",
            )
            return
    ...
    if service_configured:
        _print_lines(
            ...
            "  Fix the service, then retry: nastech gateway start",
        )
        sys.exit(1)
    ...
    from gateway.status import get_running_pid
    from nastech_cli.gateway_supervised_restart import (
...
# ``nastech gateway status`` hints for a manually-run / stopped gateway, keyed by host kind.
_STATUS_RUNNING_HINTS = {
    ...
    "windows": ("To install as a Windows Scheduled Task (auto-start on login):", "  nastech gateway install"),
    "other": (
        "To install as a service:", "  nastech gateway install", "  sudo nastech gateway install --system",
    ),
}
_STATUS_STOPPED_HINTS = {
    ...
    "windows": ("  nastech gateway install  # Install as Windows Scheduled Task (auto-start on login)",),
    "other": (
...
def _cmd_status(args):
    ...
    if not snapshot.running and named_profile_served_by_running_multiplexer():
        ...
        print("  Manage it from the default profile: nastech gateway status")
...
def _cmd_migrate_legacy(args):
    """Stop, disable, and remove legacy Nastech gateway unit files (e.g. nastech.service)."""
...
def _cmd_migrate(args):
    from nastech_cli.gateway_migrate import cmd_migrate
    cmd_migrate(args)
