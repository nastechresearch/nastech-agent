...
def _get_service_pids(all_profiles: bool = False) -> set:
    ...
    ``ai.nastech.gateway*`` LaunchAgent — the update path needs the whole
    ...
    if is_macos():
        ...
        if all_profiles:
            ...
            # mirrors the systemd branch's ``nastech-gateway*`` pattern above.
        ...
        if all_profiles:
            ...
            # ``launchctl list`` prefix scan also catches ai.nastech.gateway*
...
# — restart may fail", and `nastech update` can deadlock behind it.  The loop
...
def _scan_gateway_pids(
    ...
    # Forward slashes on both sides of the NASTECH_HOME= match — see
    ...
    try:
        if is_windows():
            ...
            # wedges the caller forever. ``nastech update`` hung exactly there
...
def find_windows_gateway_services(
    *,
    ...
    validated profile gateway PID comes from Nastech's own PID file, and its
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m nastech_cli.main gateway
...
def _prepare_profile_gateway_update_restart(profile: str, pid: int) -> str | None:
    """Choose who relaunches a profile gateway after ``nastech update``.

    A gateway started with ``--external-supervisor`` must exit back to that
    manager. Starting Nastech's detached watcher as well would escape the
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose NASTECH_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # interpreter and captures the stable cwd + env overlay (NASTECH_HOME,
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / NASTECH_HOME) the windowless
        # base interpreter needs to import nastech_cli.  Empty on POSIX, where
...
def _nastech_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``NASTECH_HOME`` from the on-disk unit file (not ``systemctl show``).
...
def _sync_nastech_home_from_systemd_unit(system: bool) -> None:
    ...
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("NASTECH_HOME", "").strip()
    if not unit_home:
        return
...
def _reap_unsupervised_gateway_orphans(extra_exclude: set | None = None) -> bool:
    ...
    in-process under a ``gateway restart`` argv (nastech_cli/gateway.py restart
    ...
    if is_windows():
        try:
            # The install-time task name is profile-aware (Nastech_Gateway /
            # Nastech_Gateway_<profile>) — never hardcode it, or the guard is
            # dormant on every standard `nastech gateway install` deployment.
            from nastech_cli.gateway_windows import get_task_name
    ...
    try:
        ...
        # whole ai.nastech.gateway* fleet — not just the current profile's
    ...
    # healthy standalone gateway (no service supervisor — e.g. `nastech
...
def _profile_arg(nastech_home: str | None = None, default_root: str | Path | None = None) -> str:
    ...
    Args:
        ...
        default_root: Optional Nastech root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_nastech_root()``
...
def _profile_arg_for_target_user(nastech_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".nastech"
    try:
        Path(nastech_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(nastech_home, default_root=target_root)
    except ValueError:
        return _profile_arg(nastech_home)
...
def ensure_gateway_service(context: str = "setup") -> bool:
    ...
    The zero-decision service path used by ``nastech setup`` (end of wizard)
    and ``nastech import``: if this host supports a service manager and no
    ...
    ``nastech gateway install --system`` explicitly (that path prompts and
...
def launchd_gateway_labels_for_install() -> list[str]:
    ...
    ``get_default_nastech_root()``), NOT by globbing ``~/Library/LaunchAgents``:
    the LaunchAgents directory is shared per-user, so a sandboxed
    ``NASTECH_HOME`` (tests, capture sandboxes, side-by-side installs) must
...
def get_python_path() -> str:
    ...
    if venv is not None:
        try:
            ...
            # nastech_constants cached from before this symbol existed. See
            # _reload_nastech_constants() in nastech_cli/managed_uv.py.
...
def _nastech_home_for_target_user(target_home_dir: str) -> str:
    ...
    current_nastech = (
        Path(current_nastech_raw).expanduser()
        if current_nastech_raw
        else get_nastech_home()
    )
    ...
    # different NASTECH_HOME into the generated service unit.
...
def _systemd_watchdog_service_fields(
    ...
    seconds = _systemd_watchdog_seconds(nastech_home)
    if seconds <= 0:
        return "simple", ""
...
def _append_node_dir_for_service(
    path_entries: list[str], nastech_root: Path | None = None
) -> None:
    """Add the Node directory a generated service unit should use to *path_entries*.

    The Nastech-managed Node under ``$NASTECH_HOME/node`` goes first when it
    ...
    *nastech_root* is the Nastech home the unit will run against. System units
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if not system:
        ...
        # user's Nastech home is known — probing here would stat the calling
    ...
    if system:
        ...
        profile_arg = _profile_arg_for_target_user(nastech_home, home_dir)
        ...
        # the remapped nastech_home, not the calling user's. Prepend — the
        ...
        _append_node_dir_for_service(
            _target_node_entries, Path(nastech_home) if nastech_home else None
        )
        path_entries = [
...
[Service]
Type={systemd_type}
{systemd_watchdog_directives}ExecStart={python_path} -m nastech_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
WorkingDirectory={working_dir}
Environment="PATH={sane_path}"
Environment="VIRTUAL_ENV={venv_dir}"
Environment="NASTECH_HOME={nastech_home}"
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned NASTECH_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo nastech gateway … --system``, NASTECH_HOME is often stripped
    # and falls back to ``/root/.nastech``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / NASTECH_HOME comparisons
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir NASTECH_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved NASTECH_HOME
    ...
    exported a throwaway ``NASTECH_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``nastech gateway restart``
    with ``NASTECH_HOME=/tmp/nastech-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="NASTECH_HOME=..."`` lines and launchd
    ``<key>NASTECH_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile
    ...
    candidates += re.findall(
        r"<key>NASTECH_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    ...
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: NASTECH_HOME resolves to a "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single NASTECH_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # NASTECH_HOME into the unit (manual E2E homes like /tmp/nastech-e2e-NNN
...
def _get_cron_drain_timeout() -> float:
    ...
    env_raw = os.getenv("NASTECH_CRON_DRAIN_TIMEOUT")
...
def _get_restart_after_turn_timeout() -> float:
    ...
    env_raw = os.getenv("NASTECH_RESTART_AFTER_TURN_TIMEOUT")
    if env_raw is not None and str(env_raw).strip() != "":
...
def systemd_install(
    ...
    # Existing system units already pin NASTECH_HOME; adopt it before any
    ...
    # ``sudo nastech gateway install --system --force`` would bake /root/.nastech
...
def systemd_start(system: bool = False):
    ...
    # NASTECH_HOME sync happens inside refresh_systemd_unit_if_needed's
...
# process invocation (each ``nastech gateway start/stop/status`` call).
...
#      background process (the `nohup nastech gateway run` workaround). See #23387.
...
def _timestamped_stderr_gateway_command(
    ...
    *,
    ...
    ``nastech update`` sees the flag on the live grandchild argv and hands
...
def generate_launchd_plist() -> str:
    ...
    # without this block every plist rewrite (e.g. `nastech gateway start`)
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.nastech/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See nastech-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_nastech_home() / "logs" / "launchd-reload.log"
        try:
...
def launchd_restart():
    ...
    try:
        ...
        if pid is not None and probe_gateway_loop_liveness(pid) == GATEWAY_LOOP_WEDGED:
            ...
            # full drain budget only stalls the restart (and `nastech update`
...
def launchd_status(deep: bool = False):
    ...
    # Nastech PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Nastech PID file).
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``NASTECH_S6_SUPERVISED_CHILD``.
...
def named_profile_served_by_running_multiplexer(profile_name: str | None = None) -> bool:
    ...
    platforms. ``profile_name`` defaults to the current NASTECH_HOME profile.
    """
    try:
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``nastech gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``nastech gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``nastech gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other NASTECH_HOME roots.
    """
    ...
    if pid is None:
        ...
        # NASTECH_HOME (via _pid_record_belongs_to_current_profile). When no
        ...
        try:
            ...
            if stale is not None and not _pid_record_belongs_to_current_profile(stale):
                stale_home = stale.get("nastech_home", "<unknown>")
                logger.warning(
    ...
    print("  Use 'nastech gateway restart' to replace it,")
    print("  or 'nastech gateway stop' first.")
    print("  Or use 'nastech gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # ``nastech gateway run`` invocations already armed in nastech_cli.main's
    ...
    #
    # The argv fast-path has ALREADY armed on the standard `nastech gateway
    ...
    _gateway_detached = (
        os.getenv("NASTECH_GATEWAY_DETACHED", "").strip().lower()
        in {"1", "true", "yes", "on"}
    )
    ...
    # the env vars ``NASTECH_GATEWAY_MAX_STARTS`` /
    # ``NASTECH_GATEWAY_START_WINDOW_S`` override for escape-hatch use.
    ...
    try:
        ...
        try:
            _env_starts = os.getenv("NASTECH_GATEWAY_MAX_STARTS")
            if _env_starts is not None:
                _max_starts = int(_env_starts)
        except ValueError:
            pass
        try:
            _env_win = os.getenv("NASTECH_GATEWAY_START_WINDOW_S")
    ...
    def _hard_exit_after_gateway_teardown(code: int) -> None:
        # ``nastech gateway run`` enters through this CLI wrapper, not through
...
def gateway_setup():
    ...
    print(
        color(
            "│             𓄃 Gateway Setup                            │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if subcmd == "install":
        ...
        if _is_supervised_gateway_process():
            print_error(
                ...
                "Use `nastech gateway uninstall` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        # The supervised probe also PASSES a plain foreground `nastech gateway run`
