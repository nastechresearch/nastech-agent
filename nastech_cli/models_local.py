...
disk cache). Split out of ``nastech_cli.models``; helpers still defined there are looked up on
``nastech_cli.models`` at call time so ``patch("nastech_cli.models.<name>")`` mocks keep intercepting.
"""
...
from nastech_cli.urllib_security import url_origin
...
def _get_ollama_base_url() -> str:
    ...
    from nastech_cli.models import _get_model_config_dict
...
def probe_ollama_local_models(
    ...
    from nastech_cli.models import _NASTECH_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _NASTECH_USER_AGENT, **(headers or {})}
...
_NEVER_OLLAMA_PROVIDERS = frozenset({"openrouter", "nastech", "anthropic", "openai", "openai-codex", "gemini", "ollama-cloud"})
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from nastech_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    return {"User-Agent": _NASTECH_USER_AGENT, **({"Authorization": f"Bearer {token}"} if token else {})}
