...
Pre-reexec updaters (Nastech ≤ v2026.9.14) purged only package prefixes
(``nastech_cli``, ``gateway``, ``tools``, ``tui_gateway``, ``agent``) and left
root modules like ``utils`` cached in the updater process. The post-pull
gateway-restart phase then imports new ``nastech_cli.gateway`` /
``nastech_cli.config`` into that process; those need symbols the stale
``utils`` lacks (``file_signature``), and ``nastech update`` exits 1 with
``gateway auto-restart failed: cannot import name 'file_signature' from
'utils'``.

Post-swap hand-off (``nastech_cli.update_handoff``) makes this class dead for
...
freshly imported ``nastech_cli`` code drops the incomplete cache before
importing ``utils``.
"""
