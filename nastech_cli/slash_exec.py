...
``CommandDef.execute`` (nastech_cli/commands.py) names a key in
...
for a fixed context (enforced by tests/nastech_cli/test_commands_execute.py).

Import discipline: this module imports nothing heavy at module level and
``nastech_cli.commands`` does NOT import this module (the ``execute`` field is
...
def _exec_profile(ctx: CommandContext) -> CommandReply:
    ...
    if not home_display:
        ...
        home_display = display_nastech_home()
...
def _exec_bundles(ctx: CommandContext) -> CommandReply:
    ...
    if not bundles:
        return CommandReply(
            ...
            "Create one with: nastech bundles create <name> --skill <s1> --skill <s2>\n"
