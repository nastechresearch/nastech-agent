...
def detect_service_manager() -> ServiceManagerKind:
    ...
    backend-agnostic code (profile hooks, the s6 dispatch in ``nastech gateway``).
    """
    # Deferred so importing this module (Protocol type, validate_profile_name) doesn't drag in
    # the whole gateway dependency graph.
    from nastech_cli.gateway import is_macos, is_windows, supports_systemd_services
    ...
    # made the s6 dispatch inert on Fly, so `nastech gateway start` spawned a foreground gateway
...
def _s6_running() -> bool:
    ...
    Must work for the unprivileged nastech user too: ``/proc/1/exe`` is unreadable for other UIDs
    ...
    resolved name is the literal ``"exe"`` and detection always fails. Since every Nastech runtime call
    inside the container drops to nastech via ``s6-setuidgid``, that silent failure made the entire
...
# ---------------------------------------------------------------------------
# Host backends: thin facades over ``nastech_cli.gateway`` (systemd/launchd) and
# ``nastech_cli.gateway_windows``. The protocol's ``name`` parameter is unused here — host backends
# operate on the currently active profile (``nastech -p <profile>``); the shape exists for s6 where
# each profile maps to a distinct service directory.
# ---------------------------------------------------------------------------


class _HostServiceManager:
    """``start``/``stop``/``restart`` resolve to ``<_fn_prefix><op>`` on ``nastech_cli.<_backend>``
    ...
    def _backend_module(self):
        import importlib
        import nastech_cli
        importlib.import_module(f"nastech_cli.{self._backend}")
        return getattr(nastech_cli, self._backend)
...
class SystemdServiceManager(_HostServiceManager):
    """Wraps the ``systemd_*`` functions in nastech_cli.gateway (host call sites still use those
...
class LaunchdServiceManager(_HostServiceManager):
    """Wraps the ``launchd_*`` functions in nastech_cli.gateway."""
...
class WindowsServiceManager(_HostServiceManager):
    """Wraps ``nastech_cli.gateway_windows`` (Scheduled Task / Startup-folder fallback).
...
# `nastech profile create` inside the container. Static services (main-nastech, dashboard) live in
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    the caller's NASTECH_HOME blindly: derive the shared profile root and map the suffix to the root
    ...
    root = nastech_home.parent.parent if nastech_home.parent.name == "profiles" else nastech_home
...
def register_unregistered_profile_gateway(mgr: ServiceManager, profile: str) -> bool:
    ...
    `nastech profile create` can only register a slot when it runs inside the container; created
...
# UID/GID of the in-image ``nastech`` user; hardcoded to match what ``stage2-hook.sh`` enforces
# (tests/docker/test_uid_remap.py). s6-supervise starts as root and drops via ``s6-setuidgid``.
_NASTECH_UID = 10000
_NASTECH_GID = 10000


def _chown_nastech(path: Path) -> None:
    try:
        os.chown(path, _NASTECH_UID, _NASTECH_GID)
    except PermissionError:
        # Already running as nastech → the dir is nastech-owned by default; swallowing keeps root
        # and unprivileged callers on one code path.
        pass


def _seed_supervise_skeleton(svc_dir: Path) -> None:
    """Pre-create nastech-owned ``supervise/`` and top-level ``event/`` inside a service directory.

    s6-supervise (root) creates ``event/``/``supervise/`` 0700 and the control FIFO 0600, so the
    nastech user gets EACCES on every ``s6-svc``/``s6-svstat``. s6 treats EEXIST as success and skips
    ...
    dispatched as the nastech user.
...
class S6ServiceManager:
    ...
    def _render_run_script(profile: str, extra_env: dict[str, str]) -> str:
        ...
        Sources NASTECH_HOME via with-contenv (run time, not baked in), resets ``HOME`` before the
        privilege drop so root's HOME does not leak, activates the venv, drops to nastech.
        ``profile == "default"`` emits NO ``-p`` flag: it is the sentinel for the root NASTECH_HOME
        ...
        allocator: because each supervised profile gateway loads its own ``NASTECH_HOME``, two profiles that
        ...
        deterministic SHA-256 allocator in ``nastech_cli.profiles._allocate_gateway_port``). PR #30136 review
        ...
        # nastech_cli.main._apply_profile_override; kept alongside the s6 one for back-compat.
        lines.append("export NASTECH_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its NASTECH_HOME. Without it
        # a gateway started OUTSIDE s6 (stray ``nastech gateway run``, an agent action, the Open
        ...
        if profile == "default":
            gateway_cmd = "nastech gateway run --replace"
        else:
            gateway_cmd = f"nastech -p {shlex.quote(profile)} gateway run --replace"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        return (
            ...
            # Create the leaf and clear a stale s6-log lock AS NASTECH when starting as root. Never
            # chown/unlink nastech-writable volume paths from this restartable root-context script:
            # an unprivileged user can race a pathname op through a symlink swap (CWE-59/CWE-367).
            # Parent logs/gateways is seeded nastech-owned at stage2 boot (test_log_dir_seed.py).
            # See #45258.
            f'if [ "$(id -u)" = 0 ]; then\n'
    ...
    def register_profile_gateway(
        ...
        try:
            ...
            # Seed nastech-owned supervise/ BEFORE publishing so the nastech-user s6-svc/s6-svstat/
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        root-owned but the parent ``supervise/`` is nastech-owned (see ``_seed_supervise_skeleton``)
