...
def detect_service_manager() -> ServiceManagerKind:
    ...
    # `nastech gateway start/stop/restart` fell through to host code that spawns
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's NASTECH_HOME blindly;
    derive the shared profile root from the current NASTECH_HOME and map the
    ...
    if nastech_home.parent.name == "profiles":
        root = nastech_home.parent.parent
    else:
        root = nastech_home
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        each supervised profile gateway loads its own ``NASTECH_HOME``,
        ...
        allocator in ``nastech_cli.profiles._allocate_gateway_port``).
        ...
        # the Open WebUI helper) grabs the per-NASTECH_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The NASTECH_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own NASTECH_HOME and s6 guarantees a
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``nastech gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``nastech -p <profile> gateway start``.
