"""Pydantic request/response models for the Nastech dashboard web server.

Extracted verbatim from ``nastech_cli/web_server.py`` (pure schema move).
``web_server`` re-exports every name here, so existing imports like
``from nastech_cli.web_server import ConfigUpdate`` keep working.
"""
...
class ModelAssignment(BaseModel):
    ...
    # ``nastech model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `nastech import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`nastech -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the NASTECH_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under NASTECH_HOME.
    output: str = ""
