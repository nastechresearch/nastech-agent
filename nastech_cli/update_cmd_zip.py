"""ZIP-download fallback for ``nastech update`` (Windows with broken git): two-phase stage/commit swap, dirty-tree guard.

Split out of ``update_cmd.py``; every name is re-imported there so ``nastech_cli.update_cmd.<name>`` keeps
...
_STASH_HINT = "  Stash or commit your changes, then rerun `nastech update`."
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    Thin alias over the two-phase helpers; retained for the ``nastech_cli.main`` re-export surface.
...
def _zip_overlay_block_reason(root: Path, *, ignore_staging_artifacts: bool = False) -> Optional[str]:
    ...
    is for the pre-swap re-check: phase 1 leaves our own ``*.nastech-update-staging`` siblings that git
...
def _extracted_root(tmp_dir: str, branch: str) -> str:
    """GitHub ZIPs extract to ``nastech-agent-<branch>/``; fall back to the first non-``__MACOSX`` dir."""
...
def _require_staging_space(extracted: str, entries: list[str], project_root: str) -> None:
    ...
    need = sum(
        ...
        # root holds 20 first-party modules (run_agent.py, cli.py, nastech_constants.py, ...). Check up front
...
def _stage_entries(extracted: str, entries: list[str], project_root: str) -> list[tuple[str, str]]:
    ...
    try:
        for item in entries:
            ...
            # desktop app, win-unpacked/ Nastech.exe) exists only in the LIVE tree. Graft the live release
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False, _windows_gateway_resume=None) -> bool:
    ...
    from nastech_cli.update_cmd import _hand_off_post_swap, _m, _read_project_version, _resolve_update_options, _sweep_bytecode_after_update
    ...
    _download_and_swap_zip(branch, f"https://github.com/NastechResearch/nastech-agent/archive/refs/heads/{branch}.zip")
