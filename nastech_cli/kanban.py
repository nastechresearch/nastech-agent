...
def _check_dispatcher_presence(
    ...
    ``nastech_home`` scopes the probe to a named profile's directory. The
    dashboard plugin API passes it because the dashboard backend process can
    be running under a different NASTECH_HOME than the profile the request
    ...
    try:
        ...
        liveness = resolve_gateway_liveness(
            profile_dir=nastech_home, use_cache=False
        )
    except Exception:
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    b_import = boards_sub.add_parser(
        ...
        description=(
            "Import a .tar.gz produced by `nastech kanban boards export`. "
    ...
    p_attach.add_argument("--author", default=None,
                          help="uploaded_by label (default: $NASTECH_PROFILE or 'user')")
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # nastech_cli.kanban_db, because children can import DB mutators directly.
    ...
    with board_scope:
        ...
        # every `nastech kanban repair` into "could not initialize database"
...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        return bool(os.environ.get("NASTECH_DELEGATED_CHILD_CONTEXT"))
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    print("Import it with `nastech kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `nastech kanban boards switch {res['board']}`.")
    return 0
