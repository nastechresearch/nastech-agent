...
Mixin on ``NastechCLI``; cli.py symbols are imported lazily inside methods (import cycle)."""
...
class CLILoopsMixin:
    ...
    def _cmd_plugins(self, cmd_original: str):
        ...
        try:
            # Discover from disk (bundled + user) like `nastech plugins list`, so
            ...
            # on one line (full catalog behind `nastech plugins list`).
