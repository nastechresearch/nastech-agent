...
``nastech kanban specify [task_id | --all]`` asks the auxiliary LLM for a
tightened title + concrete body for a Triage task, then flips it
``triage -> todo`` via ``kanban_db.specify_triage_task``.

Mirrors ``nastech_cli/goals.py``: same aux-client pattern, same "empty config
...
NASTECH_KANBAN_SPECIFY_MAX_TOKENS = max(1500, env_int("NASTECH_KANBAN_SPECIFY_MAX_TOKENS", 6000))
...
def _profile_author(default: str = "specifier") -> str:
    ...
    return os.environ.get("NASTECH_PROFILE") or os.environ.get("USER") or default
...
def specify_task(
    ...
    *,
    ...
    raw, reason = _call_aux(
        ...
        max_tokens=NASTECH_KANBAN_SPECIFY_MAX_TOKENS, timeout=timeout or 120,
    )
    if raw is None:
        return SpecifyOutcome(task_id, False, reason)
    raw = raw.strip()
