...
# Logger name parity with the origin module (tests capture "nastech_cli.commands").
logger = logging.getLogger("nastech_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``nastech skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/nastech <command>`` on Slack only: the registry
...
_SLACK_VIA_NASTECH_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_NASTECH_ONLY are skipped. ``/nastech`` is always first for anything dropped."""
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/nastech`` handler, plugin commands included."""
