...
from nastech_cli.auth_constants import DEFAULT_NASTECH_PORTAL_URL
...
def _confirm_selection_guards(
    ...
    """
    try:
        from nastech_cli.model_selection_guards import combined_message, selection_warnings
...
class _ModelPickerRows:
    """Column-aligned picker rows (name + $/Mtok + Nastech sale chrome).
    ...
    def __init__(
        ...
        from nastech_cli.models_pricing import _format_price_per_mtok, compute_sale_discount
...
def _prompt_model_selection(
    ...
    *unavailable_models* render grayed out and unselectable with an upgrade link to *portal_url*.
    ...
    # Sale chrome is Nastech Portal-only, even if pricing.original is present for another provider.
