...
dependency/config-schema warnings. Mixed into :class:`nastech_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``nastech_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from nastech_constants import get_nastech_home, reset_nastech_home_override, set_nastech_home_override
from registration_lifecycle import replacement_coordinator
from nastech_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from nastech_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from nastech_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from nastech_cli.plugins import LoadedPlugin

logger = logging.getLogger("nastech_cli.plugins")
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``nastech plugins list`` until then."""
        from nastech_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``nastech tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``nastech tools`` checklist, and even an explicit
        ...
        """
        from nastech_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `nastech plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        ``nastech plugins install``/``enable`` and after ``nastech update`` (``nastech_cli.plugin_python_deps``)
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Run `nastech plugins enable %s` to install them, "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from nastech_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_nastech gate: skip cleanly (no import, no traceback) on a version mismatch.
        from nastech_cli.plugins_manifest import requires_nastech_error
        reason = requires_nastech_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (nastech_cli.plugin_compat).
        from nastech_cli.plugin_compat import disable_reason
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.enabled = True
                from nastech_cli.plugins_ledger import _hook_source_of
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from nastech_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `nastech plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        try:
            ...
            package = load_agent_plugin(
                Path(manifest.path), get_nastech_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``nastech_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``nastech_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Nastech home, or an earlier force
