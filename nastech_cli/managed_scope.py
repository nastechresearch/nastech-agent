...
DISTINCT from ``nastech_cli.config.is_managed()`` / ``NASTECH_MANAGED`` (a coarse package-manager
...
# unless the stale ``utils`` is dropped first. See nastech_cli.stale_modules.
...
def _under_pytest() -> bool:
    """True inside the test suite: ignore the system ``/etc/nastech`` so a real managed scope on a
    dev/CI box can't leak policy into the suite. An explicit ``NASTECH_MANAGED_DIR`` still wins."""
...
def get_managed_dir() -> Optional[Path]:
    ...
    Priority: ``$NASTECH_MANAGED_DIR`` (IT-only bootstrap override; never persisted to any .env;
    honored only when non-empty AND the directory exists), then ``/etc/nastech`` when it exists.
