"""Client for uploading ``nastech debug share`` bundles to Nastech-internal S3.

This is the opt-in (``--nastech``) destination for ``nastech debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a Nastech-owned
S3 bucket via a short-lived signed URL minted by the Nastech account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by Nastech staff (and allowlisted Discord mods) through a
...
# Base URL of the Nastech account service that mints the signed upload URL.
...
def share_to_nastech(report_bundle: bytes) -> dict:
    """Orchestrate the full Nastech-S3 upload of a gzipped *report_bundle*.
