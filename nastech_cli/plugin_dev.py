"""Runtime-backed validation behind ``nastech plugins doctor``: every manifest/import/registration
...
def _doctor_runtime(plugin_path: Path):
    ...
    NASTECH_HOME with outbound socket connects blocked.
    """
    stack = ExitStack()
    try:
        # The temp dir enters the stack FIRST so any failure below (ENOSPC / KeyboardInterrupt in
        # copytree) removes it instead of stranding a nastech-plugin-doctor-* directory.
        home = Path(stack.enter_context(tempfile.TemporaryDirectory(prefix="nastech-plugin-doctor-")))
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(f"Nastech discovery found no valid plugin manifest under {copied}")
        if len(manifests) != 1:
...
def _is_plugin_module(name: str) -> bool:
    return name == "nastech_plugins" or name.startswith("nastech_plugins.")
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        candidates.append(get_nastech_home() / "plugins" / raw)
        try:
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    if missing:
        report.warning(
            ...
            + " — Nastech never auto-installs plugin dependencies; install manually: pip install "
