...
# the real profile into ``~/.nastech/browser-profile/`` and launch the user's real binary on
...
def _secure_snapshot(path: str, *, contents: bool = False) -> None:
    ...
    NixOS group-share carve-out, NASTECH_UID/GID) as every Nastech secret dir — via ``_secure_dir``/
    ...
    ``NASTECH_HOME_MODE`` hatch. Best-effort; never blocks a launch."""
    try:
...
def _unavailable_auth_dbs_error(browser: str, failed: dict[str, str]) -> str:
    ...
    if all(reason == _AUTH_DB_LOCKED for reason in failed.values()):
        return (f"{browser} is running and holds the profile's {names} with a write lock, so their "
                "SQLite backup made no progress within five seconds. Nastech does not fall back to a "
...
def _locked_profile_error(browser: str) -> str:
    ...
    if _real_profile_autoclose():
        msg = (
            ...
            "yet. Nastech can close it for you (this quits the browser — you'll lose unsaved "
        ...
        msg = (
            ...
            "browser.use_real_profile off. (Enable browser.real_profile_autoclose to let Nastech "
...
def snapshot_real_profile(browser: str, src: str | None = None) -> tuple[str | None, str | None]:
    """Snapshot ``browser``'s real ACTIVE profile into the nastech copy dir; returns ``(dst, err)``.
