...
def _start_parent_death_watchdog() -> None:
    ...
    The desktop passes its own PID via NASTECH_PARENT_PID. When that process
    ...
    No-op for standalone `nastech serve` (env unset). Poll interval tunable via
    NASTECH_SERVE_WATCHDOG_POLL_S.
    """
    raw = os.environ.get("NASTECH_PARENT_PID")
    if not raw:
        return
    try:
        desktop_pid = int(raw)
