...
from nastech_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from nastech_cli.profiles import profiles_to_serve
...
def _warm_gateway_module() -> None:
    ...
    ``nastech_cli.gateway``. But the first WS connection and its initial
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from nastech_cli.gateway import _get_restart_drain_timeout
...
def _eager_reconcile_own_session_db() -> None:
    ...
    bringing a store left behind by `nastech update` current before the
    ...
    """
    try:
        from nastech_state import SessionDB, _default_db_path
...
async def _lifespan(app: "FastAPI"):
    ...
    # so a store left behind by `nastech update` kept 500ing every
    ...
    # Import nastech_cli.gateway eagerly *before* the lifespan yield so the
    ...
    # model picker) can detect when `nastech update` replaced the code
    ...
    if os.getenv("NASTECH_DESKTOP") == "1":
        ...
        # a healthy standalone gateway (launched via `nastech gateway run`,
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Nastech-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    ``public_url`` is already Nastech' canonical browser-facing URL behind a
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``nastech-0day``
...
def _desktop_loopback_auth_exempt(
    ...
    for EVERY ``nastech serve`` on the machine — including the private loopback
    backends the Desktop app spawns for itself. Those backends authenticate
    with the per-spawn session token (injected via
    ``NASTECH_DASHBOARD_SESSION_TOKEN`` for local spawns, ``--ssh-session-token
    ...
    Exemption requires ALL of: loopback bind, ``NASTECH_DESKTOP=1`` (set by
    ...
    ``nastech serve`` with ``NASTECH_DESKTOP=1`` exported but no credential is
    NOT exempt.
    """
    ...
    if os.environ.get("NASTECH_DESKTOP") != "1":
        return False
    return bool(
        os.environ.get("NASTECH_DASHBOARD_SESSION_TOKEN")
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "proxy.enabled": {
        "type": "boolean",
        "description": (
            "Docker-only egress credential firewall. Requires `nastech egress setup` "
            "and `nastech egress start`; Modal/SSH/Daytona are not wired yet."
        ),
        "category": "security",
    },
...
_CATEGORY_MERGE: Dict[str, str] = {
    ...
    # fold it into general rather than spawning a one-field orphan category.
    "session": "general",
    # `nastech.keepalive_interval_seconds` is the only schema-surfaced nastech field
...
from nastech_cli.web_models import (  # noqa: F401
    ConfigUpdate,
    EnvVarUpdate,
    EnvVarDelete,
    EnvVarReveal,
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Nastech provider slug — fine.
    ...
    1. Vendor-name → Nastech-provider mapping: when the provider string is
       not a known Nastech provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    from nastech_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from nastech_cli.model_normalize import normalize_model_for_provider
    from nastech_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    # not be treated as a stray vendor prefix -- it isn't a known Nastech
    ...
    if (
        ...
        try:
            ...
            cur_provider = ""
        from nastech_cli.models import _AGGREGATOR_PROVIDERS
...
def _count_status_active_sessions() -> int:
    ...
    Nastech process is using it.
    """
    from nastech_state import _default_db_path
...
_MANAGED_FILES_ROOT_ENV = "NASTECH_DASHBOARD_FILES_ROOT"
...
# NASTECH_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to NASTECH_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Nastech credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no NASTECH_HOME resolution, so it blocks these trees wherever
...
def _default_nastech_root_is_opt_data() -> bool:
    raw = os.environ.get("NASTECH_HOME", "").strip()
    if not raw:
        return False
    try:
        from nastech_constants import get_default_nastech_root

        root = get_default_nastech_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``nastech update``.
    ...
    a container — e.g. the nastech-webui image sharing the Nastech source tree),
    the dashboard's ``nastech update`` button is the correct update path and
    ...
    """
    if _default_nastech_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Nastech root is actually /opt/data
    # (the container/hosted layout) or when NASTECH_DASHBOARD_FILES_ROOT is set.
    if _default_nastech_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Nastech inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``nastech:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.nastech-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``nastech_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from nastech_cli import web_git as _web_git  # noqa: E402
...
from nastech_cli.web_routers import git as _git_routes  # noqa: E402

app.include_router(_git_routes.router)

from nastech_cli.web_routers import local_models as _local_models_routes  # noqa: E402

app.include_router(_local_models_routes.router)
from nastech_cli.web_routers.git import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    git_status_route,
    git_worktrees_route,
    git_branches_route,
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``profile_platforms`` — ``{profile: platforms}`` runtime platform maps
    ...
    """
    try:
        from nastech_cli.profiles import _check_gateway_running, profiles_to_serve
...
# install, minted on first read and persisted under the ROOT Nastech home
# (get_default_nastech_root()) — NOT the profile-scoped NASTECH_HOME — so every
...
_PROFILE_PLATFORM_STATUS_KEY_RE = re.compile(
    # Profile segment mirrors nastech_cli.profiles._PROFILE_ID_RE.  Platform
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_nastech_home() at call time
    ...
    try:
        ...
        # NASTECH_HOME (see issue #69143). Plain /api/status keeps the exact
        ...
        # Windows install the first import of nastech_cli.gateway blocks the
        ...
        # SPA's StatusPage can show "OAuth gate ON via Nastech Research" or
        ...
        # Nastech bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose Nastech auth dies terminally (invalid_grant / quarantine)
        ...
        nastech_session_valid = "unknown"
        try:
            from nastech_cli.auth import get_nastech_session_validity
            nastech_session_valid = get_nastech_session_validity()
        except Exception:
            nastech_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_nastech": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "nastech_session_valid": nastech_session_valid,
        }
        ...
        try:
            ...
            storage_check = await run_in_threadpool(_probe_state_db, get_nastech_home())
        ...
        # nastech_session_valid above. Deliberately NOT folded into
        ...
        try:
            ...
            status["memory"] = await run_in_threadpool(
                ...
                profile_dir if profile_dir else get_nastech_home(),
            )
        except Exception:
            status["memory"] = {"pressure": "unknown"}

        # Disk-usage rollup (NS-656, same lineage as OOF-2/OOF-107 fleet
        # disk-exhaustion incidents). One statvfs call on NASTECH_HOME's
        ...
        try:
            from nastech_state import SessionDB as _SDB
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Nastech Cloud
...
# ---------------------------------------------------------------------------
# Portal endpoint — Nastech Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
    ...
    try:
        ...
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_nastech", False):
                    state = "via Nastech Portal"
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(
    r"^=== nastech-update completed ([0-9a-f]{32}) ===$"
)
...
def _dashboard_spawn_executable() -> str:
    ...
    runpy.run_module('nastech_cli.main', ...)"``) — so ``sys.executable`` is
...
def _spawn_nastech_action(
    ...
    *,
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "nastech_cli.main", *subcommand]

    # The dashboard runs *inside* the gateway process, so os.environ carries
    # _NASTECH_GATEWAY=1. Inheriting it makes a spawned `nastech gateway restart`
    ...
    action_env = {**os.environ, "NASTECH_NONINTERACTIVE": "1"}
    action_env.pop("_NASTECH_GATEWAY", None)
    ...
    action_id = (env_overrides or {}).get("NASTECH_ACTION_ID")
    if action_id:
...
def _durable_completed_update_action_id(lines: List[str]) -> Optional[str]:
    ...
    for index, line in enumerate(lines):
        if line.startswith("=== nastech update started "):
            last_start = index
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["nastech", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``nastech gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``nastech gateway restart`` children race each other on the
    ...
    proc = _spawn_nastech_action(subcommand, "gateway-restart")
...
async def gateway_drain(request: Request):
    ...
    active (no ``NASTECH_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
async def update_nastech():
    ...
    if _dashboard_local_update_managed_externally():
        message = (
            "Nastech updates are managed outside this dashboard in "
    ...
    # already keys on.
    from nastech_cli.update_contract import (
    ...
    if refusal is not None:
        _record_completed_action("nastech-update", refusal.message, exit_code=1)
    ...
    existing = _ACTION_PROCS.get("nastech-update")
    if existing is not None and existing.poll() is None:
        response = {
            "ok": True,
            "pid": existing.pid,
        ...
        action_id = _ACTION_IDS.get("nastech-update")
        if action_id:
    ...
    try:
        proc = _spawn_nastech_action(
            ["update"],
            "nastech-update",
            env_overrides={"NASTECH_ACTION_ID": action_id},
        )
    except Exception as exc:
...
async def check_nastech_update(force: bool = False):
    ...
    if install_method == "apt":
        payload["message"] = (
            "Nastech is managed by Termux APT; run `pkg upgrade nastech-agent`."
        )
        return payload
...
async def get_action_status(name: str, lines: int = 200):
    ...
    if name == "nastech-update":
        ...
        if durable_update_action_id:
            marker = f"=== nastech-update completed {durable_update_action_id} ==="
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    ...
    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``nastech
    ...
    """
    try:
        from nastech_cli.update_receipt import read_latest_receipt
...
@app.get("/api/nastech/update/receipt")
async def get_update_receipt():
    ...
    if not receipt:
        raise HTTPException(
            status_code=404,
            detail="No update receipt found (no `nastech update` run recorded).",
        )
...
from nastech_cli.web_routers import sessions as _sessions_routes  # noqa: E402

app.include_router(_sessions_routes.list_router)
from nastech_cli.web_routers.sessions import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_sessions,
)


from nastech_cli.web_routers import profiles as _profiles_routes  # noqa: E402

app.include_router(_profiles_routes.sessions_router)
from nastech_cli.web_routers.profiles import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
...
def _flat_json_path(provider: ProviderConfigSchema) -> Path:
    return get_nastech_home() / provider.name / "config.json"
...
def _install_memory_provider_pip_dependencies(dependencies: List[str]) -> List[Dict[str, Any]]:
    ...
    # under /opt/nastech is sealed read-only, and installs must be redirected
    # to the writable durable target on the data volume
    # (NASTECH_LAZY_INSTALL_TARGET, e.g. /opt/data/lazy-packages) — the same
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        nastech_home / f"{name}.json",
        nastech_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_nastech_home()))
            return
...
async def get_egress_status():
    ...
    from nastech_cli.proxy_cli import format_status_text
...
def _dashboard_code_skew_guard() -> Optional[str]:
    ...
    The dashboard and Desktop-owned ``nastech serve`` are long-lived; their
    ``sys.modules`` is frozen at boot.  When ``nastech update`` (or a manual
...
def _dashboard_skew_restart_hint() -> str:
    ...
    ``nastech serve --isolated`` (local or SSH). Hardcoding a systemd unit
    misleads macOS/launchd hosts and Desktop SSH backends, which have no
    ``nastech-dashboard`` unit (#97046).
    """
    if os.environ.get("NASTECH_SERVE_HEADLESS") == "1":
        return (
            "restart the Desktop-owned backend to load the new code "
            "(use Restart backend in Nastech Desktop, or quit and reopen the app)"
        )
    return (
        "restart this Nastech process to load the new code "
        "(nastech dashboard --port <port>, or the equivalent service restart for this install)"
    )
...
async def get_model_options(
    ...
    try:
        ...
        from nastech_cli.inventory import build_model_options_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Nastech this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for Nastech and None otherwise. `model` may be
    ...
    if slug == "nastech":
        try:
            ...
            pricing = get_pricing_for_provider("nastech") or {}
            ...
            # predicate.
            _policy_allowed = nastech_policy_allowed_ids()

            if free_tier:
                model_ids, pricing = union_with_portal_free_recommendations(
                    model_ids, pricing, portal_url
                )
                model_ids = restrict_to_nastech_policy(
            ...
            model = pick_silent_default_model(model_ids, provider="nastech")
            return {"provider": "nastech", "model": model, "free_tier": bool(free_tier)}
        except Exception:
            _log.exception("GET /api/model/recommended-default (nastech) failed")
            return {"provider": "nastech", "model": "", "free_tier": None}

    # Non-Nastech: preferred silent default when the provider's curated list
    ...
    try:
        ...
        from nastech_cli.models import pick_silent_default_model
...
def set_moa_models(body: MoaConfigPayload, profile: Optional[str] = None):
    ...
    try:
        from nastech_cli.moa_config import normalize_moa_config, validate_moa_payload
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from nastech_cli.model_selection_guards import combined_selection_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to Nastech, mirror the CLI's
        # post-model-selection behaviour (nastech_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_nastech_managed_defaults):
        # auto-route any *unconfigured* tools through the Nastech Tool Gateway.
        ...
        if provider.strip().lower() == "nastech":
            try:
                ...
                # Portal lookup hiccups / non-subscriber / non-nastech gating
        ...
        # endpoint, mirroring the ``nastech model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from nastech_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. nastech with $0 balance) keeps paying 402s
...
async def update_config(body: ConfigUpdate, profile: Optional[str] = None):
    def _run():
        ...
        # different NASTECH_HOME than this process's gateway sessions.
...
def _is_other_profile(profile: Optional[str]) -> bool:
    ...
    try:
        ...
        return True
    return target.resolve() != get_process_nastech_home().resolve()
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``nastech model`` universe). This is what lets the desktop Keys tab
    ...
    try:
        from nastech_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `nastech model` provider — would be
        ...
        # Vertex — a `nastech model` provider — would be invisible in the desktop
...
def _get_env_vars_sync(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `nastech model` picker uses (not desktop-only prefix guesses).
...
def list_custom_endpoints(profile: Optional[str] = None):
    ...
    the process-level NASTECH_HOME. Mirrors ``/api/config``'s profile scoping.
    """
    try:
        with _config_profile_scope(profile):
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Nastech from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "email": {
        ...
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/",
        "env_vars": (
            "EMAIL_ADDRESS",
            "EMAIL_PASSWORD",
            "EMAIL_IMAP_HOST",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Nastech to Google Chat via Cloud Pub/Sub.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/google_chat",
    },
    "wecom": {
    ...
    "weixin": {
        ...
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/weixin/",
    ...
    "teams": {
        "description": "Connect Nastech to Microsoft Teams chats via the Bot Framework.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/teams",
    },
    ...
    "irc": {
        "description": "Relay messages between an IRC channel (or DMs) and Nastech.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/irc",
    },
    "line": {
        "description": "Use Nastech from LINE via the LINE Messaging API webhook.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/line",
    },
    "ntfy": {
        "description": "Chat with Nastech over ntfy push topics (ntfy.sh or self-hosted).",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/ntfy",
    },
    "photon": {
        "description": "Use Nastech through iMessage via Photon's managed Spectrum platform.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/photon",
    },
    "raft": {
        "description": "Join a Raft workspace as an external agent.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/raft",
    },
    "simplex": {
        "description": "Talk to Nastech over SimpleX Chat via a local simplex-chat daemon.",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/simplex",
    },
    "yuanbao": {
        "name": "Yuanbao (元宝)",
    ...
    "webhook": {
        ...
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/webhooks/",
    ...
    "msgraph_webhook": {
        ...
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/msgraph-webhook",
        "required_env": (),
    },
    "whatsapp_cloud": {
        "name": "WhatsApp Cloud API",
        "description": "Use Nastech via Meta's hosted WhatsApp Cloud API (no local bridge).",
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/whatsapp-cloud",
    },
    "relay": {
        "name": "Relay (experimental)",
        "description": "Generic relay adapter fronted by the Nastech Relay connector.",
        "docs_url": "",
        "required_env": (),
    },
}
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in nastech gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in nastech gateway setup",
...
# nastech_cli/setup_hidden_env.py. Shared with the `nastech setup gateway`
# wizard so the surfaces ask for the same things.
from nastech_cli.setup_hidden_env import (  # noqa: E402
...
def _messaging_platform_payload(
    ...
    # NASTECH_HOME contextvar override (#56986 / #69143), so the profile's
...
def _whatsapp_session_path() -> Path:
    ...
    return get_nastech_dir("platforms/whatsapp/session", "whatsapp/session")
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Nastech appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # load_env() honors the NASTECH_HOME contextvar override; the gateway
...
def _multiplex_port_binding_conflict(
    ...
    *enabling* is blocked; disabling/clearing stays allowed so users can
    ...
    if not requested or requested.lower() == "current":
        ...
        # The dashboard's own profile. "custom" (an unrecognized NASTECH_HOME)
...
# the canonical ``nastech auth add <provider>`` command so the dashboard can
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.nastech/.anthropic_oauth.json`` — Nastech-managed terminal PKCE
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_nastech_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            ...
            _get_nastech_oauth_file,
        )
    except ImportError:
        read_nastech_oauth_credentials = None  # type: ignore
        _get_nastech_oauth_file = None  # type: ignore
    ...
    if nastech_creds and nastech_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "nastech_pkce",
            "source_label": f"Nastech PKCE ({_get_nastech_oauth_file() if _get_nastech_oauth_file else None})",
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    hold its session in an OS keychain Nastech can't read, so the unverified
    state is presented as "managed by the Copilot CLI" — never as signed out.
    """
    try:
        from nastech_cli.auth import get_external_process_provider_status
...
def _external_process_cli_command(provider_id: str, default: str) -> str:
    ...
    Nastech at a custom binary (``NASTECH_COPILOT_ACP_COMMAND`` /
    ``COPILOT_CLI_PATH``) would otherwise be told to run a command that isn't
    the one Nastech spawns. Non-external-process providers get ``default`` back
    untouched.
    """
    try:
        from nastech_cli.auth import PROVIDER_REGISTRY, get_external_process_provider_status
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nastech Portal",
        "flow": "device_code",
        "cli_command": "nastech auth add nastech",
        "docs_url": "https://portal.nastechresearch.github.io",
    ...
    },
    {
        ...
        # as Nastech's device-code flow; the PKCE bit is a security
    ...
    },
    {
        ...
        "docs_url": "https://nastechresearch.github.io/nastech-agent/docs/guides/xai-grok-oauth",
    ...
    # server). Nastech previously reimplemented that subscription-OAuth PKCE
    ...
    # credentials. Login still works via the terminal (`nastech auth add
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Nastech, so the disconnect
    ...
    user sees exactly what executes, and Nastech then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # catalog entry) but, unlike other external providers, Nastech still OWNS
    # the credential it can show here: the Nastech-managed PKCE file
    # (~/.nastech/.anthropic_oauth.json) and its credential-pool entry, both
    # written by `nastech auth add anthropic` in the terminal. Those are ours
    ...
    if provider.get("flow") == "external" and provider.get("id") != "anthropic":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Nastech — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``nastech model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `nastech model` universe (zero-edit for new plugins).
    try:
        from nastech_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"nastech auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    sync with the `nastech model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    def _run():
        with _profile_scope(profile):
            ...
            # Anthropic clears only the Nastech-managed PKCE file and auth-store entry.
            ...
            if provider_id == "anthropic":
                cleared = False
                try:
                    from agent.anthropic_adapter import _get_nastech_oauth_file
                    oauth_file = _get_nastech_oauth_file()
            ...
            try:
                from nastech_cli.auth import clear_provider_auth, invalidate_nastech_auth_status_cache
...
# points at `nastech auth add anthropic` (terminal PKCE, unaffected) instead.
#
#   Device code (Nastech, OpenAI Codex):
#     1. POST /api/providers/oauth/{nastech|openai-codex}/start
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (Nastech, OpenAI Codex, MiniMax, or xAI).
    ...
    if provider_id == "nastech":
        ...
        sid, sess = _new_oauth_session("nastech", "device_code", profile=profile)
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Nastech's device-code
    ...
    if provider_id == "xai-oauth":
        from nastech_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nastech_poller(session_id: str) -> None:
    """Background poller that drives a Nastech device-code flow to completion."""
    from nastech_cli.auth import (
        _poll_for_token,
    ...
    try:
        ...
        _log.info("oauth/device: nastech login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("nastech device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    the ``device_code`` field used by Nastech. On success, builds the same
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # Mirror `nastech auth add xai-oauth`: first credential may become
            ...
            # prior ``nastech auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``nastech model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Nastech "
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (Nastech, OpenAI Codex, MiniMax, xAI).
...
def _session_db_read_probe_statements() -> tuple:
    ...
    after `nastech update` until the first message forced a writable open.
    """
    from nastech_state_schema import schema_read_probe_statements
...
def _open_session_db_at_path(db_path: Path, *, read_only: bool):
    ...
    from nastech_state import SessionDB, is_malformed_schema_error
...
def _maybe_auto_archive_for_profile(profile: Optional[str]) -> None:
    ...
    The Desktop backend is spawned as ``nastech serve`` — it runs neither the
...
def _prune_sessions(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `nastech sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_nastech_home()
...
def _cron_default_profile() -> str:
    ...
    A desktop pool backend runs one process per profile (NASTECH_HOME already
    ...
    fallback would write a non-default profile's job into ``~/.nastech``.
    Resolve the process's own profile instead. ``custom`` (an unrecognized
    NASTECH_HOME outside the profiles tree) has no profile-dir equivalent, so
...
from nastech_cli.web_routers import cron as _cron_routes  # noqa: E402

app.include_router(_cron_routes.router)
from nastech_cli.web_routers.cron import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_cron_jobs,
    get_cron_job,
    list_cron_job_runs,
    create_cron_job,
...
def _gateway_fire_endpoint(profile: str, home: Path) -> str:
    ...
    if multiplex and profile != "default":
        ...
        listener_profile, listener_home = "default", get_default_nastech_root()
        _log.info(
    ...
    try:
        ...
        # yaml.safe_load of config.yaml (tests/nastech_cli/
        # test_config_read_guard.py). The NASTECH_HOME override scopes
        ...
        token = set_nastech_home_override(str(listener_home))
        try:
...
def _gateway_intentionally_stopped(profile: Optional[str]) -> bool:
    ...
    commands (``nastech gateway stop`` persists ``"stopped"``; start and
...
def _normalize_mcp_server_create(
    ...
    """
    from nastech_cli.mcp_config import (
...
from nastech_cli.web_routers import mcp as _mcp_routes  # noqa: E402

app.include_router(_mcp_routes.router)
from nastech_cli.web_routers.mcp import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    list_mcp_servers,
    add_mcp_server,
    replace_mcp_servers,
    remove_mcp_server,
...
def _mcp_oauth_callback_url(request: Request, server_name: str) -> str:
    ...
    from nastech_cli.dashboard_auth.prefix import prefix_from_request, resolve_public_url
...
def _mcp_oauth_transaction(flow) -> threading.Lock:
    key = (flow.nastech_home, flow.server_name)
...
def _run_dashboard_mcp_oauth(flow, cfg: dict) -> None:
    ...
    try:
        ...
        from tools.mcp_oauth import NastechTokenStorage, force_interactive_oauth
        from tools.mcp_oauth_manager import get_manager

        home_token = set_nastech_home_override(flow.nastech_home)
        secret_token = set_secret_scope(build_profile_secret_scope(Path(flow.nastech_home)))
        try:
            transaction = _mcp_oauth_transaction(flow)
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                manager = get_manager()
                storage = NastechTokenStorage(flow.server_name)
                backup = storage.snapshot()
                previous_entry = None
                try:
                    previous_entry = manager.remove(
                        flow.server_name,
                        nastech_home=flow.nastech_home,
                    )
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_nastech_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_nastech_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    ...
    def _run():
        try:
            ...
            # (via DELETE below or `nastech auth add`) can seed again.
            # Mirrors the `nastech auth add` behaviour in auth_commands.py.
...
async def remove_credential_pool_entry(provider: str, index: int):
    ...
    registry the CLI ``nastech auth remove`` uses: each source cleans up its
...
def _dashboard_backup_dir() -> Path:
    return get_nastech_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"nastech-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_nastech_action(args, "import")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``nastech`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_nastech_action`` tracks one process/log per name, so a shared
...
from nastech_cli.web_routers import skills as _skills_routes  # noqa: E402

app.include_router(_skills_routes.hub_router)
from nastech_cli.web_routers.skills import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    install_skill_hub,
    uninstall_skill_hub,
    update_skills_hub,
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local NASTECH_HOME override (same mechanism as
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the NASTECH_HOME override. Returns the
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_nastech_home()`` at call
       time — the context-local override from ``set_nastech_home_override``
    ...
    retargeted to the *current* ``get_nastech_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a NASTECH_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_nastech_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_nastech_home()`` at call time
...
from nastech_cli.web_routers import tools as _tools_routes  # noqa: E402

app.include_router(_tools_routes.router)
from nastech_cli.web_routers.tools import (  # noqa: E402,F401 — legacy re-exports; tests call these via web_server.<name>
    get_toolsets,
    toggle_toolset,
    get_toolset_config,
    get_toolset_models,
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "Nastech Subscription" image row instead carries the legacy
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Nastech,
...
from nastech_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
_GATEWAY_WS_PROTOCOL = "nastech-gateway-v1"
_GATEWAY_WS_TICKET_PROTOCOL_PREFIX = "nastech-gateway-ticket."
    ...
    if auth_required:
        ...
        try:
            ...
            if protocol_ticket:
                ...
                ws._nastech_ws_subprotocol = _GATEWAY_WS_PROTOCOL
...
def _resolve_chat_argv(
    ...
    ``NASTECH_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``NASTECH_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_nastech_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``nastech -p <name>`` performs. The in-process
    ``NASTECH_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from nastech_cli.main import PROJECT_ROOT, _apply_tui_python_env, _make_tui_argv
    ...
    # Nastech TUI child: build via the single spawn-env factory (profile-home
    # contract applied; secrets kept — the spawned agent needs provider creds).
    # An explicit profile scope still overrides NASTECH_HOME before config is
    # bridged into the child environment.
    from tools.environments.local import build_subprocess_env
    env = build_subprocess_env(scrub_secrets=False, inherit_profile_home=True)
    if profile_dir is not None:
        env["NASTECH_HOME"] = str(profile_dir)
    try:
        from nastech_cli.config import (
    ...
    # inherits the profile NASTECH_HOME set above.
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``NASTECH_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("NASTECH_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="nastech-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Nastech Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Nastech CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="nastech-console",
                )
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from nastech_cli.console_engine import NastechConsoleEngine

        engine = NastechConsoleEngine(output_limit=_CONSOLE_OUTPUT_LIMIT)
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Nastech Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if raw_resume and env:
        registry_resume = env.get("NASTECH_TUI_RESUME") or raw_resume
...
async def gateway_ws(ws: WebSocket) -> None:
    ...
    await handle_ws(
        ws,
        auth_identity=getattr(ws, "_nastech_auth_identity", None),
        subprotocol=getattr(ws, "_nastech_ws_subprotocol", None),
    )
...
def _render_active_theme_bootstrap_css() -> str:
    ...
    default Nastech Teal canvas before the SPA flips the configured user
    theme into place.
    ...
    try:
        ...
        for theme in _discover_user_themes():
            ...
            return (
                '<style id="nastech-theme-bootstrap">'
                ":root{"
...
def mount_spa(application: FastAPI):
    ...
    """
    # `nastech serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("NASTECH_SERVE_HEADLESS") == "1"
    if _headless:
        _msg = (
            "Headless backend (nastech serve): web UI disabled — use "
            "`nastech dashboard` for the browser UI."
        )

        @application.get("/{full_path:path}")
        async def no_frontend(full_path: str):
            # Desktop token handshake (#94227): the Electron shell boots by
            # fetching `/` and extracting ``window.__NASTECH_SESSION_TOKEN__``
            ...
            # `nastech update` replaced the backend) had no way to adopt the
            ...
            if full_path == "" and not gated:
                ...
                return HTMLResponse(
                    ...
                    f"window.__NASTECH_SESSION_TOKEN__={token_js};"
                    "window.__NASTECH_AUTH_REQUIRED__=false;"
                    "</script></head><body>"
                    "Headless backend (nastech serve): web UI disabled — use "
                    "`nastech dashboard` for the browser UI."
                    "</body></html>",
    ...
    # (#82614): a long-lived `nastech dashboard --skip-build` process that
    ...
    def _serve_index(prefix: str = ""):
        ...
        # (``NASTECH_HOME/dashboard-themes/<name>.yaml``), inject a minimal
        # critical-CSS block so the first paint uses the target palette.
        # Without this the SPA paints the default Nastech Teal canvas, then
...
_BUILTIN_DASHBOARD_THEMES = [
    ...
    {"name": "nastech-blue",     "label": "Nastech Blue",           "description": "Light mode — vivid Nastech-blue accents on cream canvas"},
...
def _discover_user_themes() -> list:
    ...
    Uses the dashboard process launch home, not ``get_nastech_home()``, so a
    transient profile override from embedded chat does not hide themes that
    live under the server's own ``NASTECH_HOME``.
    """
    themes_dir = get_process_nastech_home() / "dashboard-themes"
...
def _discover_dashboard_plugins() -> list:
    ...
    # NASTECH_HOME override (e.g. embedded /chat under --open-profile).
    #
    # #87197: when the process itself is profile-scoped (``--profile <name>``
    # sets ``NASTECH_HOME=<root>/profiles/<name>``), the launch home is the
    # profile directory, which has no ``plugins/`` — user plugins are
    # installed in the nastech root (``~/.nastech/plugins``). Scan the default
    # root as well (``get_default_nastech_root()`` unwraps
    # ``<root>/profiles/<name>`` → ``<root>`` and returns a custom
    # ``NASTECH_HOME`` unchanged when it *is* the root), mirroring how
    # ``nastech_cli.plugins`` resolves plugin install locations. The
    # ``seen_names`` dedupe below keeps profile-local plugins (if any)
    # authoritative over same-named root plugins.
    from nastech_constants import get_default_nastech_root

    user_plugin_roots = [get_process_nastech_home() / "plugins"]
    root_plugins = get_default_nastech_root() / "plugins"
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``NASTECH_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("NASTECH_DESKTOP_READY_FILE")
    if not target:
        return
...
def _is_serve_orphaned(
    ...
    *,
    ...
    ``NASTECH_PARENT_PID`` is the Electron Desktop PID, not necessarily this
    Python process's immediate PPID. On Windows the venv ``nastech.exe`` launcher
...
def _start_parent_death_watchdog() -> None:
    ...
    Desktop versions that provide only ``NASTECH_PARENT_PID`` retain PID-only
    tracking.
    """
    raw_pid = os.environ.get("NASTECH_PARENT_PID")
    start_marker = os.environ.get("NASTECH_PARENT_START_MARKER")
    nonce = os.environ.get("NASTECH_PARENT_NONCE")
    ...
    try:
        poll = max(0.5, float(os.environ.get("NASTECH_SERVE_WATCHDOG_POLL_S", "2.0")))
    except (TypeError, ValueError):
        poll = 2.0
...
# spawn (and any script wrapping ``nastech serve``) cannot tell "port occupied"
...
# NASTECH_BACKEND_READY sentinel (which is NOT changed by any of this).
...
def _write_machine_sentinel_line(line: str) -> None:
    ...
    NASTECH_DESKTOP_READY_FILE channel) for port discovery instead. Never
...
def _report_port_in_use(host: str, port: int) -> None:
    ...
    print(
        ...
        "'nastech serve' / 'nastech dashboard' backend or the Nastech gateway. "
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``NASTECH_SERVE_HEADLESS``), so
    ...
    # the nastech-0day MCP-persistence campaign abused unauthenticated public
        ...
        if not list_providers():
            ...
            try:
                ...
                if _nastech_plugin.LAST_SKIP_REASON:
                    skip_reasons.append(
                        f"  • nastech: {_nastech_plugin.LAST_SKIP_REASON}"
                    )
            except Exception:
                pass
    ...
    # NASTECH_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            if os.getenv("NASTECH_DESKTOP") == "1":
                try:
                    from nastech_cli.dashboard_procs import (
            ...
            # for standalone `nastech serve` (no NASTECH_PARENT_PID env).
            ...
            # is what lets `nastech update` relaunch a manually-started serve
            # on its real endpoint instead of dropping it (#63206).
            try:
                from nastech_cli.process_identity import (
