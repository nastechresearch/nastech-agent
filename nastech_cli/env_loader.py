...
# load_nastech_dotenv can call _sanitize_env_file_if_needed multiple times
...
def _known_nastech_env_keys() -> set[str]:
    """Return the combined set of known Nastech env-var keys.
    ...
    ``load_nastech_dotenv()`` calls.
    """
    from nastech_cli.config import _EXTRA_ENV_KEYS
    from nastech_cli.config_defaults import OPTIONAL_ENV_VARS

    return set(OPTIONAL_ENV_VARS.keys()) | set(_EXTRA_ENV_KEYS)


# Behavioral routing keys a parent Nastech process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Nastech keys absent from ``.env``.
    ...
    parent Nastech process injects and that silently change *which provider
    ...
    ``tests/nastech_cli/test_dump_env_visibility.py``), and a startup scrub
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_nastech_dotenv()'s .op.env bootstrap: the 1Password
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    ``nastech_cli.config._sanitize_env_lines`` normalizes line endings while
...
def load_nastech_dotenv(
    *,
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Nastech keys
    ...
    #   EnvironmentFile=-/path/to/.nastech/.op.env
    ...
    # 2. A fresh ``nastech update`` retry just completed a deferred dependency
    ...
    # TERMINAL_ENV=docker left in ~/.nastech/.env (e.g. written by an older
    # `nastech setup` before the user switched terminal.backend in config.yaml)
    ...
    # cron standalone runs) call load_nastech_dotenv() repeatedly and used to
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    Delegates to ``nastech_cli.config.apply_terminal_config_to_env`` — the
    ...
    Scoped to the process NASTECH_HOME: the shared bridge reads the
    process-global config, so re-applying it for a *different* profile's
    ``load_nastech_dotenv(nastech_home=...)`` call would bridge the wrong
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of NASTECH_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Nastech reads
    ...
    owns the once-per-NASTECH_HOME guard, the post-apply ASCII
    ...
    if not cfg:
        ...
        # on its next load_nastech_dotenv() call instead of never.
        return
    ...
    # a generic dict entry must not force crypto load on every nastech launch.
    ...
    # so the 3-5 import-time load_nastech_dotenv() calls per startup don't
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `nastech model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `nastech` startup, so populating the
    # shared cache here lets main.py's early bridge and nastech_logging reuse
...
def _process_nastech_home() -> Path:
    """The NASTECH_HOME the shared config cache is keyed to."""
    try:
