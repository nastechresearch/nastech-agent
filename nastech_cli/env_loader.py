...
# Once-per-process guards: load_nastech_dotenv() runs repeatedly (user + project env, gateway hot-reload,
...
# env-var name → source label ("bitwarden", …) for externally injected credentials; setup / `nastech
...
# NASTECH_HOME paths already pulled external secrets for: load_nastech_dotenv() runs at import time from
...
# Behavioral routing keys a parent Nastech process injects into child env that silently redirect a profile
...
def get_secret_source_values(nastech_home: str | os.PathLike) -> dict[str, str]:
    """Return the external-secret value snapshot for ``nastech_home``."""
    return dict(_SECRET_SOURCE_VALUES_BY_HOME.get(str(Path(nastech_home).resolve()), {}))


def hydrate_profile_secret_sources(nastech_home: str | os.PathLike) -> dict[str, str]:
    ...
    with _SECRET_SOURCE_CACHE_LOCK:
        return _hydrate_profile_secret_sources(Path(nastech_home))
...
def _hydrate_profile_secret_sources(home: Path) -> dict[str, str]:
    ...
    try:
        ...
        # Mirror load_nastech_dotenv()'s .op.env bootstrap (1Password token lives in gitignored .op.env)
        ...
        local_env["NASTECH_HOME"] = str(home)
...
def reset_secret_source_cache(nastech_home: str | os.PathLike | None = None) -> None:
    """Forget applied homes so the next load re-pulls (tests, long-running processes after config edits).

    ``nastech_home`` limits the reset to ONE home: a multiplex gateway keeps every profile's snapshot in
    ...
    (#102041)."""
    if nastech_home is None:
        _APPLIED_HOMES.clear()
        _SECRET_SOURCES.clear()
        _SECRET_SOURCE_VALUES_BY_HOME.clear()
        return
    home_key = str(Path(nastech_home).resolve())
...
def _load_dotenv_with_fallback(path: Path, *, override: bool, load_pass: int | None = None) -> None:
    ...
    ``load_pass`` groups the layered files of one ``load_nastech_dotenv`` call: within a pass a later layer
    (project, managed) still sees the earlier layer's output, as it always did; only OTHER passes' output
    is peeled. A bare call (``nastech send``'s direct reload) is its own pass."""
    raw = path.read_bytes()
    try:
...
def load_nastech_dotenv(
    *,
    ...
    """Load Nastech env files: ``~/.nastech/.env`` overrides stale shell exports; project ``.env`` is a dev
    ...
    home_path = Path(nastech_home) if nastech_home else get_process_nastech_home()
    ...
    if is_multiplex_active() and get_nastech_home_override() is not None:
    ...
    # Windows (#73381, #86735). 2. A fresh ``nastech update`` retry just completed a deferred dependency
    ...
    # ~/.nastech/.env win on every reload and flip the backend mid-session in long-lived processes.
    ...
    # with override=True — so a stale TERMINAL_ENV=docker left in ~/.nastech/.env (e.g. written by an older
    # `nastech setup` before the user switched terminal.backend in config.yaml) silently wins again on every
    # reload. Startup launchers bridge config→env once, but long-lived processes (gateway per-turn reload,
    # cron standalone runs) call load_nastech_dotenv() repeatedly and used to flip the effective backend back
...
def _reapply_terminal_config_bridge(home_path: Path) -> None:
    ...
    try:
        if Path(home_path).resolve() != _process_nastech_home().resolve():
            return
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    tokens), BEFORE Nastech reads credentials; failures never block startup. Precedence/conflicts/provenance
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # populating it lets main.py's early bridge and nastech_logging reuse one parse instead of 3-4.
    if home_path == _process_nastech_home():
        try:
...
def _process_nastech_home() -> Path:
    """The NASTECH_HOME the running process was launched under.

    Must be the *true* process home, ignoring any context-local
    ``set_nastech_home_override`` a per-request task has installed. Both
    ...
    * ``_load_secrets_config`` uses it to decide whether the shared
    ...
    ``nastech_constants.get_process_nastech_home()`` is the override-immune
