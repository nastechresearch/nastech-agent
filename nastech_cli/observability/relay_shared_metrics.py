...
class _Runtime:
        ...
        with session.lock:
            ...
            if existing is not None:
                ...
                if task is not None:
                    ...
                    # physical attempt. Provider fallback resets Nastech's
            ...
            if task is not None:
                ...
                if retry_ordinal is not None and retry_ordinal > 0:
                    # A real Nastech retry can advance api_request_id while
    ...
    def finish_task(self, event: dict[str, Any]) -> None:
        ...
        if finished:
            try:
                ...
                logger.warning(
                    "Nastech shared-metrics task flush failed",
                    exc_info=True,
                )
            else:
                self._export()
    ...
    def close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Nastech shared-metrics session %s closed with errors: %s",
                session.session_id,
                "; ".join(failures),
            )
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning(
                "Nastech shared-metrics shutdown flush failed",
                exc_info=True,
            )
        else:
            self._export()
    ...
    def _finish_model_call(
        ...
        try:
            ...
            logger.warning(
                "Nastech shared-metrics model call close failed", exc_info=True
            )
    ...
    def _finish_task(
        ...
        try:
            ...
            logger.warning("Nastech shared-metrics task close failed", exc_info=True)
        finally:
    ...
    def _send_exported_packages(self) -> None:
        from nastech_cli.observability.shared_metrics_send_config import (
            resolve_send_config,
        )
        ...
        with self._send_lock:
            ...
            thread = threading.Thread(
                ...
                name="nastech-shared-metrics-send",
                daemon=True,
            )
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Nastech shared metrics operation failed", exc_info=True)
            return None
...
def _reconcile_send_consent_once() -> None:
    ...
    and creating ``~/.nastech/telemetry`` for every fully-disabled user would
    ...
    try:
        ...
        # fully-disabled user was getting a ~/.nastech/telemetry directory.
        default_path = (
            get_nastech_home() / "telemetry" / "shared_metrics" / "metrics.sqlite3"
        )
...
def observe_lifecycle(hook_name: str, **kwargs: Any) -> None:
    ...
    try:
        ...
        logger.warning(
            "Nastech shared metrics hook failed: %s", hook_name, exc_info=True
        )
