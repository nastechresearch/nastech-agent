"""Bounded product contract for the first Nastech shared-metrics slice."""
...
SCHEMA_KEY = "nastech.metrics.schema_version"
SCHEMA_VERSION = "nastech.metrics.event.v2"
MODEL_CALL_SCOPE = "nastech.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "nastech.task_run"
TOOL_CALL_SCOPE = "nastech.tool_call"
CLIENT_ACTIVE_MARK = "nastech.client.active"
TOOL_APPROVAL_MARK = "nastech.tool_approval"
SKILL_LIFECYCLE_MARK = "nastech.skill.lifecycle"
SKILL_LOAD_MARK = "nastech.skill.load"
SUBSCRIBER_NAME = "nastech.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "nastech.client.active"
LEGACY_MODEL_CALL_METRIC = "nastech.model_call.count"
MODEL_ROUTE_METRIC = "nastech.model_route.count"
TASK_STARTED_METRIC = "nastech.task_run.started"
TASK_FINISHED_METRIC = "nastech.task_run.finished"
TOOL_CALL_METRIC = "nastech.tool_call.count"
TOOL_APPROVAL_METRIC = "nastech.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "nastech.skill.lifecycle.count"
SKILL_LOAD_METRIC = "nastech.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "nastech_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Nastech installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    nastech_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(nastech_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "nastech_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("nastech_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Nastech logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "nastech.call_role"
    )
    call_role = (metadata or {}).get("nastech.call_role")
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Nastech terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "nastech-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Nastech registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Nastech tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Nastech."""
