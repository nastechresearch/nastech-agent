"""Nastech update pipeline: dispatchers (``_cmd_update_impl``/``_cmd_update_check``) + git plumbing.

Each concern lives in ``update_cmd_<concern>.py`` and is re-imported here so
``nastech_cli.update_cmd.<name>`` keeps resolving (and stays monkeypatchable). Imports are one-way:
main -> update_cmd -> update_cmd_*; ``_m()`` resolves ``nastech_cli.main`` at call time.
"""
...
from nastech_cli.config import get_nastech_home  # noqa: F401  (re-exported; patched via update_cmd)
from nastech_cli.update_cmd_common import _best_effort
from nastech_constants import get_default_nastech_root, project_venv_dir, venv_python_path

# Re-exports: every split-module name stays reachable (and monkeypatchable) as update_cmd.<name>.
from nastech_cli.update_abort_recovery import (  # noqa: F401
...
from nastech_cli.update_cmd_windows import (  # noqa: F401
    ...
    _handoff_reapable_backend_pids, _nastech_holder_subcommand, _holder_value_flags,
...
from nastech_cli.update_cmd_fleet import (  # noqa: F401
...
from nastech_cli.update_cmd_zip import (  # noqa: F401
...
from nastech_cli.update_cmd_stash import (  # noqa: F401
...
from nastech_cli.update_cmd_config import (  # noqa: F401
...
from nastech_cli.update_cmd_deps import (  # noqa: F401
...
from nastech_cli.update_cmd_git import (  # noqa: F401
...
from nastech_cli.update_cmd_maint import (  # noqa: F401
...
def _m():
    """Lazy ``nastech_cli.main`` handle: keeps main-side test patches effective, import one-way."""
...
_UPDATE_CRITICAL_FILES = (
    "nastech_cli/main.py", "nastech_cli/config.py", "nastech_cli/__init__.py",
    "nastech_cli/web_server.py", "cli.py", "run_agent.py", "model_tools.py", "toolsets.py",
...
# otherwise leaves `nastech update` on "Fetching updates..." forever (#93759, #95777). Five
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from nastech_constants import get_nastech_home  # noqa: F811  (deliberate: constants variant)
...
def _format_concurrent_instances_message(matches: list[tuple[int, str]], scripts_dir: Path) -> str:
    """Explanation + remediation hint for the Windows concurrent-nastech.exe gate."""
...
def _log_only_write(text: str) -> None:
    ...
    with suppress(Exception):
        if log_file is None:
            log_path = get_nastech_home() / "logs" / "update.log"
...
def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    ...
    import codecs
    import io
    from nastech_cli._subprocess_compat import kill_process_tree, windows_hide_flags
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """``nastech update --check``: fetch and report without installing. ``branch_explicit`` is
    ...
    from nastech_cli.update_contract import evaluate_update_admission, record_refusal_receipt
    ...
    if is_shallow:
        ...
        from nastech_cli.gitlock import repair_broken_shallow_boundaries, prune_stale_shallow_grafts
...
def _print_update_check_result(behind: int | None, compare_branch: str) -> None:
    ...
    if behind is not None:
        print(f"𓄃 Update available: {behind} {'commit' if behind == 1 else 'commits'} behind {compare_branch}.")
    else:
    ...
    *, assume_yes, gateway_mode, pre_update_snapshot_id,
    ...
    if handed_off_sync or not healthy:
        ...
        if runtime_repaired is not None:
            ...
            # imports pass while lazily-installed backends and ``nastech tools`` dependencies
            ...
            # must leave the breadcrumb so the next `nastech` run finishes the repair.
            ...
            if _m()._refresh_active_lazy_features(
                ...
                print("  ⚠ Lazy-refresh recovery incomplete — run `nastech` again "
...
def _pull_updates(
    ...
    # critical-path file (PR #28452 incident: orphan merge-conflict markers in nastech_cli/config.py bricked
    # every user who ran ``nastech update`` for the 7 minutes between the bad commit and the fix landing).
...
class _UpdateOptions:
    """Resolved ``nastech update`` inputs (flags, config, pre-update snapshots)."""
...
def _begin_update_receipt_and_plan(args):
    ...
    pre-update plan (None if the probe failed); ``sys.exit(2)`` when a non-gateway nastech.exe
    holds the venv shim."""
    ...
    # Plan phase (#91277 Phase 2): snapshot the pre-update fleet — every running Nastech runtime, its
    ...
    with _best_effort('Update plan phase failed: %s'):
        from nastech_cli.update_inventory import collect_runtime_inventory, record_plan_in_receipt
    ...
    # Windows: another nastech.exe holding the venv shim means WinError 32 spam and a
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        ...
        concurrent = _m()._detect_concurrent_nastech_instances(scripts_dir) if scripts_dir is not None else []
...
def _verify_head_after_pull(
    ...
    # step re-detaches to the SHA). Before this guard, ``nastech update`` printed "✓ Code updated!" and
    # reinstalled deps + rebuilt the desktop app against the stale tree — no error, no warning, ``nastech
...
def _handle_update_called_process_error(
    ...
    if _should_zip_fallback_on_update_error(e):
        ...
        if _called_process_error_is_python_dep_install(e):
            ...
            print()
            print("  Nastech may not start until the dependencies are installed. Fix the error above")
            print("  (usually network or disk space), then run `nastech update` again.")
            if _m()._is_windows():
                print("  If `nastech update` itself will not start, retry through the venv interpreter:")
                print(
...
def _post_swap_payload(
    *, swap: str, branch: str, opts, gateway_mode: bool, had_desktop_app_before_update: bool,
    ...
    from nastech_cli.update_receipt import detach_update_receipt
...
def _sibling_snapshots_module():
    ...
    import nastech_cli.update_cmd_config as _cfg
    return _cfg


def _hand_off_post_swap(args, **payload_kwargs) -> None:
    """Re-execute ``nastech update --post-swap`` on the pulled tree and exit with its code.

    The parent detaches from the receipt and its Windows resume hook — the child owns both —
    and only relays the exit code (``nastech_cli/update_handoff.py``).
    """
    from nastech_cli.update_handoff import continue_update_in_fresh_interpreter
    from nastech_cli.update_receipt import resume_update_receipt
...
def _run_post_swap_phase(args, gateway_mode: bool) -> None:
    ...
    pulled code."""
    from nastech_cli.update_handoff import read_handoff
    from nastech_cli.update_receipt import resume_update_receipt
...
def _execute_post_swap(payload: dict, args, gateway_mode: bool) -> None:
    ...
    import nastech_cli.update_cmd_config as _cfg
    from nastech_cli.update_inventory import UpdatePlan
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Any venv python still running (typically the Desktop `nastech serve` backend) keeps .pyd
    ...
    # Self-lock deferral moved: the venv-holder sweep above excludes this process by design (a CLI `nastech
