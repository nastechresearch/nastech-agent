...
from nastech_constants import get_default_nastech_root, venv_python_path
...
#: ``_purge_stale_nastech_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "nastech_cli.nastech_logging",
    }
)


def _purge_stale_nastech_modules() -> None:
    """Evict every cached Nastech module after the checkout changed in-place.

    ``nastech update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from nastech_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``nastech_cli.gateway`` does
    ``from nastech_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Nastech
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Nastech module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Nastech modules: %s", exc)
...
def _reload_config_modules() -> None:
    ...
    It also reloads ``nastech_cli._subprocess_compat`` and
    ``nastech_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    for mod_name in (
        ...
        "nastech_cli.config",
        "nastech_cli.config_migrations",
        "nastech_cli._subprocess_compat",
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``nastech update`` historically
    ...
    caller): scope config reads/writes via the context-local NASTECH_HOME
    ...
    try:
        ...
        from nastech_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_nastech_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_nastech_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'nastech config migrate' to check manually.")
        return
    ...
    # context-local NASTECH_HOME override (never os.environ — other
    ...
    try:
        from nastech_cli.backup import restore_config_model_settings_if_rewritten
    ...
    try:
        from nastech_cli.backup import restore_cron_jobs_all_profiles
    ...
    try:
        from nastech_cli.backup import restore_config_model_settings_all_profiles
...
def _prune_orphan_rescue_refs(
    ...
    ``refs/nastech-update-backups/orphan-<branch>-<ts>-<sha>``. A rescue ref
    ...
    try:
        list_result = subprocess.run(
            git_cmd + [
                ...
                "--format=%(refname)",
                "--sort=refname",
                f"refs/nastech-update-backups/orphan-{branch}-*",
            ],
            cwd=cwd,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        ...
        if max_age_days > 0:
            ...
            prefix = f"refs/nastech-update-backups/orphan-{branch}-"
            for ref in refs:
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``nastech.exe`` has
    ...
    Skipping is safe because Nastech pins its editable finder to a *static*
...
def _critical_module_import_failures(
    ...
    marker = f"__NASTECH_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        "        if missing in %r or missing.startswith('nastech_') or %r:\n"
...
def _reload_process_scan_modules() -> None:
    ...
    ``from nastech_cli._subprocess_compat import bounded_probe_run``. If the
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``nastech update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && nastech update")
    print(
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Nastech launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["nastech.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Nastech")
    print("  Desktop, a gateway, or another nastech REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `nastech` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``nastech.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.nastech-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".nastech-update-staging", ".nastech-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `nastech update`.")
...
def _post_update_sqlite_runtime_status():
    ...
    from nastech_constants import project_venv_dir
    from nastech_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Nastech runtime."""
    ...
    print(
        "  Rebuild the Nastech venv with a uv-managed Python, restart Nastech, "
        "then verify with `nastech doctor`."
    )
    return False
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Nastech process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `nastech desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `nastech "
                "update` again; if SQLite is unchanged, rebuild the Nastech venv "
                "with a uv-managed Python, restart Nastech, then verify with "
                "`nastech doctor`."
            )
    else:
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_nastech_home() / ".update_exit_code"
    try:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from nastech_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from nastech_cli.sqlite_safe_read import LiveConnectionError, offline_file_access

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (nastech gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Nastech) and retry."
        )
        return False
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    try:
        from nastech_cli.backup import _sibling_profile_homes
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    ...
    try:
        ...
        try:
            for item in entries:
                ...
                # Nastech.exe) exists only in the LIVE tree. Swapping `apps`
    ...
    try:
        from nastech_cli.update_receipt import finalize_update_receipt
...
_AUTOSTASH_NAME_PREFIX = "nastech-update-autostash-"

#: Age past which a leftover ``nastech-update-autostash-*`` entry is called out
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    Nastech never drops one automatically.
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``nastech update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Nastech agent unexecutable.")
    ...
    *,
    ...
    if not has_upstream:
        ...
        if assume_yes or (
            ...
            print(
                "  Add it later with: git remote add upstream https://github.com/NastechResearch/nastech-agent.git"
            )
            return False
...
# ``fleet_restart_pending`` lives under NASTECH_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """NASTECH_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_nastech_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from nastech_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `nastech update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `nastech update` or `nastech gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every nastech-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_nastech_modules()
    except Exception:
        pass
    try:
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``nastech update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: nastech gateway restart")
    sys.exit(1)
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from nastech_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``nastech update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Nastech Desktop backend child, an unrelated ``nastech`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``nastech tools``."""
    try:
        from nastech_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Nastech Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``nastech tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Nastech Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Nastech Tools dependency set(s)...")
    restored: list[str] = []
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from nastech_cli.banner import _github_compare_behind
            ...
            if counted is not None:
                ...
                print(f"𓄃 Update available: {counted} {commits_word} behind {compare_branch}.")
            else:
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``nastech-acp`` launcher next to the ``nastech`` one.
    ...
    No-op on Windows (install.ps1 stages the ``nastech`` / ``nastech-acp``
    launchers into the managed binary dir ``$NastechHome\bin`` and puts THAT
    ...
    ``nastech_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``nastech-acp`` is already present next to the
    ``nastech`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    try:
        ...
        try:
            from nastech_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from nastech_cli.update_receipt import record_step
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Nastech candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "nastech.exe"}
        ):
            continue
...
# ``cryptography`` is the canonical case: ``nastech_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``nastech.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``nastech`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `nastech` launch will complete it")
...
def _nastech_holder_subcommand(cmdline: str) -> str | None:
    """The actual Nastech SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``nastech_cli.main`` / ``nastech(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("nastech_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("nastech", "nastech.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``nastech dashboard`` must not be labeled as the
    ...
    hint_by_subcommand = {
        "serve": "  ← Nastech backend (if the Desktop app is open, close it)",
        "dashboard": "  ← nastech dashboard (stop it: nastech dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _nastech_holder_subcommand(cmdline)
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``nastech update`` through its terminal tool.
    ...
    try:
        from nastech_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `nastech update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Nastech spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from nastech_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _serve_relaunch_commands(entries: list[dict]) -> list[list[str]]:
    ...
    try:
        ...
        if scripts_dir is not None:
            for name in ("nastech.exe", "nastech"):
                candidate = scripts_dir / name
                if candidate.is_file():
                    nastech = str(candidate)
                    break
    except Exception:
        nastech = None
    if nastech is None:
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (nastech serve --host <ip> --port <port>)."
        )
    try:
        from nastech_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_nastech at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``nastech_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Nastech ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``nastech.exe``) is open, NOTHING legitimate
    ...
    - Only Nastech backends (``nastech_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``nastech.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    from nastech_cli._subprocess_compat import pid_is_nastech, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            if expected_start_time is None:
                ...
                continue
            if not pid_is_nastech(
                pid,
                expected_start_time=expected_start_time,
            ):
                logger.debug(
                    "Skipping taskkill of non-Nastech or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``nastech serve`` / ``nastech dashboard`` argv.
    ...
    """
    if "nastech_cli.main" not in (cmdline or "").lower():
        return False
    return _nastech_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``nastech-gateway*.service``/``nastech-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("nastech-serve")`` alone would also accept the
        # unrelated ``nastech-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "nastech-gateway.service"
            or unit.startswith("nastech-gateway-")
            or unit == "nastech-serve.service"
            or unit.startswith("nastech-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``nastech-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``nastech-serve*`` units (#83438) don't, so sending them
    ...
    (``nastech-gateway-helper`` is fine — profile units are
    ``nastech-gateway-<profile>`` — but ``nastech-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "nastech-gateway" or svc_name.startswith("nastech-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from nastech_cli.gateway import is_macos
    ...
    print("    nastech gateway status")
    if any(not name.startswith("ai.nastech.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.nastech.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: nastech gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: nastech gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.nastech.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``nastech_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``nastech-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"nastech-gateway-{profile}",
        f"ai.nastech.gateway-{profile}",
        f"gateway-{profile}",
    }


# Abort recovery lives in its own bounded module (review on #96235). Re-exported
# here because `nastech_cli.main` and the update flow below address these names
# through `update_cmd`.
from nastech_cli.update_abort_recovery import (  # noqa: E402
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``nastech_cli.gateway`` in the failing
    ...
    unrecoverable.  The fresh child runs a separate ``nastech-serve*`` systemd
    ...
    ``nastech serve``: it records no spawner, so it reads as ``manual-serve``.
        ...
        for runtime in getattr(plan, "runtimes", ()) or ():
            ...
            if kind == "gateway":
                ...
                if supervisor == "desktop":
                    ...
                    # `nastech serve` sets neither NASTECH_SPAWN nor
                    # NASTECH_PARENT_PID, so it lands here as "manual-serve".
                    # Unit-backed serve runtimes are recovered by the
                    # fresh child's systemd pass, which enumerates
                    # `nastech-serve*` from systemd instead of trusting this
                    ...
                    reason = (
                        ...
                        " unit pass when it owns a nastech-serve* unit, else"
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``nastech_cli.gateway`` from the freshly pulled checkout)
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    if relaunched or unmapped_relaunched:
        ...
        if not ready_pids:
            ...
            print("    Recover with: nastech gateway restart")
            raise RuntimeError(
...
def _portable_git_candidates() -> list:
    ...
    The Nastech-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped NASTECH_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``nastech update`` must
    ...
    candidates = []
    try:
        for root in (get_default_nastech_root(), Path(get_nastech_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    Nastech-managed PortableGit copy; accept the first candidate that runs and
...
def _rebuild_desktop_after_update(
    ...
    # `nastech desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "nastech_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Nastech-managed Node on PATH: when
    # `nastech update` runs inside the desktop updater chain (Desktop →
    # nastech-setup → nastech update), the shell PATH customizations are lost,
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `nastech desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo nastech`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``nastech update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/nastech`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Nastech broken.")
    print("  This usually happens after running nastech or pip with sudo. Offending paths:")
    for p, uid in foreign:
        print(f"    - {p} (owner uid {uid})")
    print("\n  Fix ownership, then re-run the update:")
    print(f"    sudo chown -R $(id -un): {project_root}")
    print("    nastech update")
...
def _drain_or_signal_gateway_for_update(
    ...
    """Decide how ``nastech update`` hands a running gateway over to new code.
    ...
    1. **Gateway is an ancestor of this process** — THREE-WAY DEADLOCK BREAK
       (#100179). When ``nastech update`` runs INSIDE the gateway's own
       process tree (the nastech-auto-update cron job is the canonical case),
       waiting for that gateway to exit is a circular wait::

           gateway  waits on all in-flight work units (#77184)
             └─ cron agent session waits on the `nastech update` process
                  └─ `nastech update` waits on the gateway to exit  ← back to A
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from nastech_cli.update_receipt import begin_update_receipt
    ...
    # running Nastech runtime, its supervisor, and its running code version —
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Nastech backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Nastech backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            # `nastech serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Nastech backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_nastech_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Nastech backend process(es) "
    ...
    # process by design (a CLI `nastech update` IS the venv python), and an
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by nastech.exe...")
            elif not healthy:
        ...
        # Before this guard, ``nastech update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``nastech doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && nastech update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``nastech update`` can catch up even when git is already up to date.
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Nastech re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.nastechresearch.nastech` (repeat "
        ...
        try:
            from nastech_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        # Migrate the Windows nastech launchers to the managed binary dir
        # (the default Nastech root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (nastech-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `nastech`
        ...
        # the managed clone under the default Nastech root).
        try:
            from nastech_cli._install_repair import migrate_windows_bin_path
        ...
        # ``user/nastech-serve`` and ``system/nastech-serve`` are two different
        ...
        #
        # Purge stale cached Nastech modules FIRST: the import below pulls
        ...
        try:
            ...
            # Discover all nastech-gateway* units (default + profiles) plus
            # nastech-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # nastech-serve has no such handler (it isn't
            ...
            # Restart EVERY ai.nastech.gateway* LaunchAgent, not only the
            ...
            try:
                from nastech_cli.update_receipt import record_gateway_restart
            ...
            # the reporter hit through tui_gateway (hosted by `nastech serve`,
        ...
        #
        # Forward the systemd units restarted above (includes nastech-serve*,
        ...
        # `nastech serve` (no unit) is left running its pre-update
        ...
        try:
            from nastech_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import nastech_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``nastech update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from nastech_cli.main import main; main()" update --yes'
                    )
            try:
                from nastech_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``nastech_cli.gateway`` no longer imports,
