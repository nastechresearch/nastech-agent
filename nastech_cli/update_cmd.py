...
def _ensure_acp_launcher() -> None:
    ...
    On Windows, restore missing ``nastech.exe`` / ``nastech-acp.exe`` copies in
    the dedicated ``<install>\\bin`` directory.  Existing files are not
    overwritten because ``bin\\nastech.exe`` may be the currently running
    update launcher.
    ...
    On POSIX, the ACP shim is skipped wherever a ``nastech-acp`` is already
    present next to the ``nastech`` command.  Unwritable POSIX directories
    (e.g. ``/usr/local/bin`` as non-root) are skipped silently.  Idempotent.
    """
    if _m().sys.platform == "win32":
        from nastech_cli._install_repair import _sync_windows_cli_launchers
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Self-heal the launchers exposed on PATH: the POSIX nastech-acp shim
