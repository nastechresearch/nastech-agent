...
Proxies mounting the dashboard at a path inject ``X-Forwarded-Prefix: /nastech``
so the backend can build prefixed URLs (Location headers, OAuth redirect_uri,
cookie Path, SPA asset URLs). An operator-declared ``NASTECH_DASHBOARD_PUBLIC_URL``
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    _warn_once(
        ...
        source, cleaned, cleaned.split("://")[-1] or "nastech.example.com")
...
def normalise_prefix(raw: Optional[str]) -> str:
    """``"/nastech"`` form (no trailing slash) or ``""`` when unset/malformed. ``..``, ``//`` and
...
def resolve_public_url() -> str:
    ...
    ``NASTECH_DASHBOARD_PUBLIC_URL`` env (blank counts as unset so a provisioned-but-blank secret
    ...
    if env_clean:
        return env_clean
    _warn_if_malformed("NASTECH_DASHBOARD_PUBLIC_URL env var", env_raw)
