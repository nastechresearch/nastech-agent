...
All HttpOnly, ``SameSite=Lax`` unless noted, Path = proxy prefix or /: ``nastech_session_at``
(access token; Max-Age = token TTL), ``nastech_session_rt`` (rotating refresh token; written only
when the provider returned one, always cleared on logout/expiry), ``nastech_session_provider``
(non-secret routing hint so an RT is not handed to the wrong provider), ``nastech_session_pkce``
...
cookies set on such a 302, crbug 40508226), ``nastech_sso_attempt`` (auto-SSO loop guard, 60 s).
...
SSO_ATTEMPT_COOKIE = "nastech_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    """``Path=/nastech`` under a proxy prefix (no leak to sibling apps), else ``/``."""
...
def set_session_cookies(
    ...
    Nastech Portal issues a 24h rotating refresh token (nastech #37247); a provider that omits it returns
