...
class Session:
    ...
    mandatory; providers without orgs set ``org_id=""``. The tokens are opaque to Nastech."""
    user_id: str
    email: str
    display_name: str
    org_id: str
...
def classify_jwks_lookup_error(exc: BaseException) -> Exception:
    ...
    * ``jwt.DecodeError`` — the bearer is not a JWT at all (an opaque peer key, a legacy session token,
    garbage). #94558: hosted agents answered every non-JWT bearer with a fast 503 ``Auth provider 'nastech'
