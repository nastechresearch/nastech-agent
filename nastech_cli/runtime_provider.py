...
Custom-provider lookup lives in :mod:`nastech_cli.runtime_provider_custom`; Azure Foundry,
OpenRouter/bare-custom, Bedrock and external-process builders in
:mod:`nastech_cli.runtime_provider_backends` — both re-exported here so
``nastech_cli.runtime_provider.<name>`` imports and test patches keep working."""
...
from nastech_cli.auth import (  # resolve_external_process_provider_credentials is read via origin by runtime_provider_backends
    ACTUAL_LOCAL_NOAUTH_PLACEHOLDER, AuthError, DEFAULT_CODEX_BASE_URL, DEFAULT_QWEN_BASE_URL, DEFAULT_XAI_OAUTH_BASE_URL,
    PROVIDER_REGISTRY, _agent_key_is_usable, _nastech_inference_env_override, format_auth_error, resolve_provider,
    resolve_nastech_runtime_credentials, resolve_codex_runtime_credentials, resolve_xai_oauth_runtime_credentials,
...
from nastech_cli import models as _models  # attribute access keeps ``nastech_cli.models.<name>`` patches effective
from nastech_constants import OPENROUTER_BASE_URL
from nastech_cli.providers import determine_api_mode, get_provider, is_actual_route, is_official_openai_host, nastech_api_mode
...
# lazily, so its first import can happen while a test has ``nastech_cli.config.load_config`` patched
...
# Hosts that only speak one wire protocol. Mirrors host_mandated_api_mode in nastech_cli/providers.py
...
def _api_key_provider_api_mode(provider: str, model_cfg: Dict[str, Any], api_key: str, base_url: str, effective_model: Any, *,
    ...
    if provider == "xai":
        ...
        # the host_mandated_api_mode clause in nastech_cli/providers.py so the runtime resolver stays in
...
_NO_ANTHROPIC_CREDENTIALS_MSG = ("No Anthropic credentials found. Run 'nastech auth add anthropic' to sign in, "
    ...
    if not token:
        ...
        if model and resolve_anthropic_token():
            raise AuthError(f"Anthropic credentials are rate-limited for {model}; "
                            "other Claude models remain available (see `nastech auth list`).")
...
def _nastech_entry_key_usable(entry: Any, min_ttl: int) -> bool:
    return _agent_key_is_usable({k: getattr(entry, k, None) for k in ("agent_key", "agent_key_expires_at", "scope")}, min_ttl)


def _nastech_min_key_ttl() -> int:
    return max(60, env_int("NASTECH_NASTECH_MIN_KEY_TTL_SECONDS", 1800))


def _resolve_nastech_creds() -> Dict[str, Any]:
    return resolve_nastech_runtime_credentials(timeout_seconds=float(get_secret_str("NASTECH_NASTECH_TIMEOUT_SECONDS", "15")))
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    """Provider request from explicit arg, then config, then ``NASTECH_INFERENCE_PROVIDER``, else
    ...
    return get_secret_str("NASTECH_INFERENCE_PROVIDER", "").strip().lower() or "auto"


# ── extracted collaborators (re-exported; see module docstring) ────────────────────────────

from nastech_cli.runtime_provider_custom import (  # noqa: E402,F401
...
from nastech_cli.runtime_provider_backends import (  # noqa: E402,F401
...
def _pool_entry_mode_and_url(provider, entry, model_cfg, effective_model, base_url) -> tuple:
    ...
    if provider in _POOL_ENTRY_SIMPLE_MODES:
        ...
        if provider == "openai-codex":
            ...
            # NASTECH_CODEX_BASE_URL override must apply consistently to every
            # credential source, including pooled OAuth credentials.
            override_url = get_secret_str("NASTECH_CODEX_BASE_URL", "").strip().rstrip("/")
            if override_url:
    ...
    if provider == "nastech":
        return nastech_api_mode(effective_model), (_nastech_inference_env_override() or "") or base_url
    if provider == "copilot":
...
def _openrouter_should_use_pool(requested_provider, model_cfg, explicit_api_key, explicit_base_url) -> bool:
    ...
    # OpenRouter host — `nastech setup` persists https://openrouter.ai/api/v1 for plain installs,
...
def _refresh_nastech_pool_entry(pool: CredentialPool, entry: Any, pool_api_key: str):
    """Nastech pool entries carry the agent_key (an invoke JWT) which the pool does not refresh on
    selection (avoids network calls in `nastech auth list`); refresh here before falling back to
    singleton auth resolution. Returns (entry, pool_api_key) — key "" when still unusable."""
    min_ttl = _nastech_min_key_ttl()
    if _nastech_entry_key_usable(entry, min_ttl):
        return entry, pool_api_key
    logger.debug("Nastech pool entry agent_key expired/missing, refreshing selected pool entry")
    try:
        refreshed = pool.try_refresh_current()
    except Exception as exc:
        logger.debug("Nastech pool entry refresh failed: %s", exc)
        refreshed = None
    if refreshed is not None:
        entry, pool_api_key = refreshed, _pool_entry_api_key(refreshed)
    if not pool_api_key or not _nastech_entry_key_usable(entry, min_ttl):
        logger.debug("Nastech pool entry agent_key still unavailable, falling through to runtime resolution")
        pool_api_key = ""
    return entry, pool_api_key
...
def _resolve_from_pool(provider: str, requested_provider: str, model_cfg: Dict[str, Any], explicit_api_key, explicit_base_url,
    ...
    if provider == "nastech":
        entry, pool_api_key = _refresh_nastech_pool_entry(pool, entry, pool_api_key)
...
def _explicit_nastech(requested_provider, model_cfg, api_key, explicit_base_url, target_model):
    state = auth_mod.get_provider_auth_state("nastech") or {}
    base_url = (explicit_base_url or _nastech_inference_env_override()
    ...
    # raw OAuth access_token fallback is handled by resolve_nastech_runtime_credentials().
    api_key = api_key or (str(state.get("agent_key") or "").strip() if _agent_key_is_usable(state, _nastech_min_key_ttl()) else "")
    api_key, base_url, expires_at = _creds_fallback(api_key, explicit_base_url, base_url,
                                                    ...
                                                    _resolve_nastech_creds)
    return _runtime("nastech", nastech_api_mode(_effective_model(model_cfg, target_model)), base_url, api_key, source="explicit",
...
_EXPLICIT_RESOLVERS: Dict[str, Callable[..., Dict[str, Any]]] = {
    "anthropic": _explicit_anthropic, "openai-codex": _explicit_codex, "nastech": _explicit_nastech,
...
_OAUTH_RUNTIME_PROVIDERS: Dict[str, _OAuthRuntimeSpec] = {
    "nastech": _OAuthRuntimeSpec(_resolve_nastech_creds, nastech_api_mode, "portal", "expires_at",
                              "Auto-detected Nastech provider but credentials failed"),
    "openai-codex": _OAuthRuntimeSpec(lambda: resolve_codex_runtime_credentials(), "codex_responses", "nastech-auth-store",
                                      "last_refresh", "Auto-detected Codex provider but credentials failed"),
    "xai-oauth": _OAuthRuntimeSpec(lambda: resolve_xai_oauth_runtime_credentials(), "codex_responses", "nastech-auth-store",
...
def _resolve_vertex_runtime(requested_provider: str) -> Dict[str, Any]:
    ...
    if not token or not base_url:
        raise AuthError("Vertex AI credentials could not be resolved. Vertex uses OAuth2 (not a static API key): provide a "
                        "service-account JSON via GOOGLE_APPLICATION_CREDENTIALS (or VERTEX_CREDENTIALS_PATH) in ~/.nastech/.env, "
                        "or run 'gcloud auth application-default login' for ADC. Set the GCP project/region under vertex: in "
                        "config.yaml if they aren't embedded in the credentials. Run `nastech setup` to install Vertex support.")
...
def resolve_runtime_provider(*, requested: Optional[str] = None, explicit_api_key: Optional[str] = None,
    ...
    rung returns or raises, else falls to the next):
      ...
      7. OAuth specs (nastech/codex/xai/qwen; "auto" swallows AuthError, logs, and stamps it on a
...
_PLUGIN_COMPAT_LAZY = {
    'custom_provider_aliases': ('nastech_cli.providers', 'custom_provider_aliases'),
