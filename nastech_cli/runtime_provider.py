...
from nastech_cli.auth import (
    ...
    _nastech_inference_env_override,
    format_auth_error,
    resolve_provider,
...
from nastech_cli.providers import custom_provider_aliases, custom_provider_slug
from nastech_constants import OPENROUTER_BASE_URL
from nastech_cli.providers import is_official_openai_host


def load_config():
    """Late-bound delegate to :func:`nastech_cli.config.load_config`.

    Deliberately NOT a module-level ``from nastech_cli.config import
    load_config``: this module is often imported lazily (inside functions),
    so its first import can happen while a test has
    ``nastech_cli.config.load_config`` patched — a from-import would then
    ...
    patching ``nastech_cli.config.load_config`` OR
    ``nastech_cli.runtime_provider.load_config`` behaves as expected.
    """
...
def _detect_api_mode_for_url(base_url: str) -> Optional[str]:
    ...
    # host_mandated_api_mode clause in nastech_cli/providers.py so the
...
def _parse_api_mode(raw: Any) -> Optional[str]:
    ...
    if isinstance(raw, str):
        from nastech_cli.config import _canonical_api_mode
...
def _nastech_inference_base_url_override() -> str:
    """Return the trusted Nastech runtime base URL override, if configured.

    Delegates to ``auth._nastech_inference_env_override`` so every
    ``NASTECH_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _nastech_inference_env_override() or ""
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = nastech_api_mode(effective_model)
        base_url = _nastech_inference_base_url_override() or base_url
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("NASTECH_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``nastech``, ``openrouter``, …) so
...
def canonical_custom_identity(
    *,
    ...
    3. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``NASTECH_INFERENCE_PROVIDER`` equivalent). When neither
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("NASTECH_INFERENCE_PROVIDER", "").strip()
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "nastech":
        ...
        base_url = (
            explicit_base_url
            or _nastech_inference_base_url_override()
        ...
        if not api_key:
            creds = resolve_nastech_runtime_credentials(
                timeout_seconds=float(_getenv("NASTECH_NASTECH_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
        ...
        return {
            "provider": "nastech",
            "api_mode": nastech_api_mode(target_model or model_cfg.get("default") or ""),
            "base_url": base_url,
            "api_key": api_key,
            "source": "explicit",
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # For Nastech, the pool entry's runtime_api_key is the agent_key
        ...
        if provider == "nastech" and entry is not None:
            ...
            if not _agent_key_is_usable(nastech_state, min_ttl):
                ...
                if not pool_api_key or not _agent_key_is_usable(nastech_state, min_ttl):
    ...
    if provider == "nastech":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Nastech but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected Nastech provider but credentials failed; "
