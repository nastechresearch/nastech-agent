...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``nastech mcp login`` and
    ``nastech mcp reauth`` so both behave identically for a single server.
    """
    ...
    #
    # force_interactive_oauth: `nastech mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Nastech desktop / agent-
...
def cmd_mcp_reauth(args):
    ...
    ``nastech mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``nastech mcp reauth --all`` discovers every ``auth: oauth`` server in
...
def mcp_command(args):
    ...
    if handler:
        ...
        _info("nastech mcp catalog                            List Nastech-approved MCPs")
        ...
        _info("nastech mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
