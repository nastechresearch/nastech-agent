...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    Mirrors ``_load_mcp_config()`` in ``tools/mcp_tool.py``: load ``~/.nastech/.env`` into ``os.environ`` and
...
def _probe_single_server(
    ...
    # an OAuth browser flow (e.g. `nastech mcp login`'s 315s) left that inner bound at the
...
def _configure_http_auth(
    ...
    if needs_auth and (auth_type == "header" or not auth_type):
        ...
        if get_env_value(env_key):
            ...
            from nastech_cli.cli_output import prompt
...
def cmd_mcp_add(args):
    ...
    # --command uses dest="mcp_command" (see nastech_cli/main.py for why the dest is renamed).
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(
            f"Saved '{name}' to {display_nastech_home()}/config.yaml ({tool_count}/{len(tools)} tools enabled)"
        )
...
def cmd_mcp_remove(args):
    ...
    # an earlier `nastech mcp test`) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def _probe_failure_next_step(name: str, exc: BaseException) -> str:
    ...
    if _is_auth_error(root) or getattr(getattr(root, "response", None), "status_code", None) in (401, 403):
        return f"The server rejected the sign-in. Run: nastech mcp login {name}"
    if "missing executable" in _format_connect_error(exc):
        return (f"Install that command, or set mcp_servers.{name}.command in {display_nastech_home()}/config.yaml "
                "to its full path.")
    return f"Check the server is running and the URL/command in its config, then run: nastech mcp test {name}"
...
def _reauth_oauth_server(name: str, server_config: dict, *, flow: str | None = None) -> bool:
    ...
    # re-auth path in web_server.py. force_interactive_oauth: `nastech mcp login` is explicitly
...
def cmd_mcp_reauth(args):
    ...
    if not name:
        ...
        _info("Usage: nastech mcp reauth <name>   |   nastech mcp reauth --all")
        return
...
_MCP_USAGE = (
    "nastech mcp                                    Open the catalog picker (default)",
    "nastech mcp catalog                            List Nastech-approved MCPs",
    "nastech mcp install <name>                     Install a catalog MCP",
    "nastech mcp serve                              Run as MCP server",
    "nastech mcp add <name> --url <endpoint>        Add a custom MCP server",
    "nastech mcp add <name> --command <cmd>         Add a stdio server",
    "nastech mcp add <name> --preset <preset>       Add from a known preset",
    "nastech mcp remove <name>                      Remove a server",
    "nastech mcp list                               List configured servers",
    "nastech mcp test <name>                        Test connection",
    "nastech mcp configure <name>                   Toggle tools",
    "nastech mcp login <name>                       Re-authenticate OAuth",
    "nastech mcp reauth <name> | --all              Re-auth one or all OAuth servers",
)
...
def mcp_command(args):
    ...
    if action in ("picker", "catalog", "install"):
        ...
        from nastech_cli import mcp_picker
    ...
    if handler:
        ...
        return handler(args)
    # No subcommand — drop the user into the catalog picker (same UX as `nastech plugin`).
