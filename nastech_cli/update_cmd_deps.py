"""Post-``nastech update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``nastech_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
from nastech_constants import project_venv_dir, venv_python_path
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``nastech.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "nastech_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from nastech_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from nastech_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__NASTECH_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        "import importlib, json, sys\n"
        # Importing nastech_cli.main runs the startup dotenv load, which pulls external secret
        ...
        # only our own packages count. Roots come from nastech_constants so the user hint can't drift.
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``nastech tools`` dependencies (from a pre-rebuild probe) into a rebuilt
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Nastech extras / ``LAZY_DEPS``), so the core reinstall can strip them;
...
def _reapply_plugin_python_dependencies() -> None:
    ...
    ``uv sync``/reinstall strips anything Nastech' own lock does not know). Non-memory plugins whose
    deps no longer resolve are disabled loudly, memory providers last. Never raises."""
    from nastech_cli.plugin_python_deps import reapply_all
    from nastech_cli.update_cmd import _m
    ...
    for name, reason in report.dropped:
        print(f"  ✗ Plugin '{name}' DISABLED: {reason}. Fix the plugin's declared dependencies, "
              f"then `nastech plugins enable {name}`.")
    if report.failed:
...
def _migrate_removed_memory_providers() -> None:
    ...
    try:
        from nastech_cli.memory_provider_migration import migrate_all_homes
        migrate_all_homes()
...
def _npm_lockfile_changed(nastech_root: Path) -> bool:
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(nastech_root)
...
def _npm_lock_cache_file(nastech_root: Path) -> Path:
...
def _record_npm_lockfile_hash(nastech_root: Path) -> None:
    ...
    if digest is None:
        return
    try:
        _npm_lock_cache_file(nastech_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run nastech
...
def _update_node_dependencies() -> list[str]:
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `nastech update` case.
    # See #43564.
    ...
    # The chatty npm-deprecation noise during `nastech update` comes from the *desktop* build, not this step;
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".nastech-bootstrap-complete", _m()._update_marker_path())
...
# canonical case: ``nastech_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``nastech.exe`` shim
...
def _rebuild_desktop_after_update(
    ...
    # rebuild window), then surface the tail. Put Nastech-managed Node on PATH: the desktop
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo nastech`` dies mid-update with ``venv/bin/nastech`` already
    ...
    See #83529.
    A later normal ``nastech update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/nastech`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from nastech_cli.update_cmd import _path_uid
    try:
