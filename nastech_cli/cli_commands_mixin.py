"""Slash-command handlers for the interactive CLI (``NastechCLI`` inherits ``CLICommandsMixin``).
...
class _TTYBuf(StringIO):
    """StringIO that claims to be a TTY so ``nastech_cli.colors.color()`` still emits ANSI escapes."""
...
# /busy mode -> what Enter does while Nastech is working (status line / post-set explanation).
_BUSY_MODE_SHORT = {
    "queue": "queues for next turn", "steer": "steers into current run (after next tool call)",
    "interrupt": "redirects current run immediately"}
_BUSY_MODE_LONG = {
    "queue": "Enter will queue follow-up input while Nastech is busy.",
    "steer": "Enter will steer your message into the current run (after the next tool call).",
    "interrupt": "Enter will redirect the current run while Nastech is busy; /stop still cancels it.",
}
...
# /bg AIAgent provider-routing kwargs -> NastechCLI attribute carrying the value.
_BG_PROVIDER_KWARGS = {
...
def _print_side_result_panel(cli, *, header_lines, body, title_suffix, empty_note, console=None) -> None:
    ...
    try:
        ...
        label, _resp_color, _resp_text = "𓄃 Nastech", "#CD7F32", "#FFF8DC"
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        mgr = self._checkpoint_manager((
            "  Checkpoints are not enabled.", "  Enable with: nastech --checkpoints",
    ...
    def _handle_diff_command(self, command: str):
        ...
        cwd; ``session`` is everything Nastech changed since the checkpoint baseline."""
        stat_only = False
        mode = "working"
        paths: list[str] = []
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        mgr = self._checkpoint_manager((
            ...
            "  Enable with: nastech --checkpoints",
            "  Or in config.yaml: checkpoints: { enabled: true }",
        ...
        if result.get("empty") or (not stat and not diff):
            return print("  No changes — Nastech hasn't edited any files here yet.")
        if stat:
    ...
    def _snapshot_list(self, parts) -> None:
        from nastech_cli.backup import list_quick_snapshots
    ...
    def _snapshot_restore(self, parts) -> None:
        from nastech_cli.backup import list_quick_snapshots, restore_quick_snapshot
        if len(parts) < 3:
    ...
    def _snapshot_prune(self, parts) -> None:
        from nastech_cli.backup import prune_quick_snapshots
        keep = 20
        if len(parts) > 2:
            try:
                keep = int(parts[2])
    ...
    def _handle_export_command(self, command: str):
        ...
        from nastech_cli.profiles import export_profile, get_active_profile_name, get_profile_export_path
        ...
        try:
            ...
            _pr(f"  ✓ Exported '{name}' to {result}",
                "  Share it: the other user runs /import or `nastech profile import <archive>`.")
    ...
    def _handle_import_command(self, command: str):
        ...
        new profile."""
        from nastech_cli.profiles import check_alias_collision, create_wrapper_script, import_profile
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `nastech journey`). Read-only views render
    ...
    def _handle_copy_command(self, cmd_original: str) -> None:
        ...
        try:
            from nastech_cli.clipboard import is_remote_shell_session, write_clipboard_text
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            tip = f'Tip: type your next message, or run nastech chat -q --image {example} "What do you see?"'
            _cp(_dim_line(tip))
    ...
    def _handoff_wait(self, platform_name: str, session_title: str) -> bool:
        ...
        try:  # pending timed out: CAS-clear so the user can retry
            ...
            pass
        return self._handoff_keep(
            "  Timed out waiting for the gateway. Is `nastech gateway` running?",
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            return _cp("  Tip:   Use /history or `nastech sessions list` to find sessions.")
        ...
        # /sessions <id>, which delegates here) lands in the same directory as a startup `nastech
    ...
    def _resolve_resume_target(self, target: str):
        ...
        if not session_meta:
            return _cp(f"  Session not found: {target}",
                       "  Use /sessions or `nastech sessions list` to see available sessions.")
        try:
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=new_session_id, source=os.environ.get("NASTECH_SESSION_SOURCE", "cli"),
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        ``new`` moves this session into the tree (as ``nastech -w``: kept on exit only with
    ...
    def _worktree_prune(self, repo_root: str, rest: str) -> None:
        import cli as _cli
        from nastech_cli import worktree_gc
        rest = rest.lower()
    ...
    def _worktree_new(self, repo_root: str, rest: str) -> None:
        ...
        # Retarget the session's terminal/file tools at the new tree (as `nastech -w` does).
        try:
            os.chdir(wt_info["path"])
        except OSError as e:
            print(f"  ⚠ Created worktree but could not enter it: {e}")
        os.environ["TERMINAL_CWD"] = wt_info["path"]
        # Same keep-if-unpushed cleanup as `nastech -w`. Only one tree is "active" per process;
    ...
    def _handle_personality_command(self, cmd: str):
        ...
        persistence goes through nastech_cli.personality, the single owner of personality state."""
    ...
    def _cron_overview(self) -> None:
        ...
        if jobs:
            from nastech_cli.cron import _next_run_row
            _pr("  Current Jobs:", "  " + "-" * 63)
            for job in jobs:
    ...
    def _cron_job_action(self, subcommand: str, opts: dict) -> None:
        ...
        if action == "resume":
            ...
            from nastech_cli.cron import _run_outcome
    ...
    def _handle_curator_command(self, cmd: str):
        """Handle /curator — delegates to nastech_cli.curator so the CLI and the `nastech curator`
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to nastech_cli.skills_hub, after intercepting the
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        inspects the workspace read-only and saves the plan under ``.nastech/plans/``."""
    ...
    def _handle_login_command(self, cmd_original: str) -> None:
        ...
        def _settle_session_model(state) -> None:
            ...
            the portal serves ``nastech/welcome`` as a paid model, so a session still carrying it must
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        """In-session ``/bundles`` — show installed skill bundles (``nastech bundles list`` rendered
        ...
        if not bundles:
            return _cp("  No skill bundles installed.",
                       _dim_line('Create one with: nastech bundles create <name> --skill <s1> --skill <s2>'),
        ...
        _cp("\n" + _dim_line("Invoke a bundle with /<slug>. Manage with `nastech bundles`."))
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        Session-scoped and in-process — use `nastech cron` for durable schedules."""
        from nastech_cli.heartbeat import format_interval
        arg = _command_arg(cmd)
        lower = arg.lower()
    ...
    def _heartbeat_set(self, mgr, arg: str) -> None:
        ...
        _cp(f"  ♥ Heartbeat set (every {format_interval(state.interval_seconds)}): {state.prompt}",
            _dim_line("Fires as a normal turn whenever the session is idle and the interval has "
                      ...
                      "Nastech process runs — use `nastech cron` for durable schedules."))
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        from nastech_cli.goal_command import dispatch_goal_command
        from nastech_cli.goals import last_user_message_content
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        if not new_skin:  # show current skin and list available
            ...
            return _pr("\n  Usage: /skin <name>",
                       f"  Custom skins: drop a YAML file in {display_nastech_home()}/skins/\n")
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy [status|queue|steer|interrupt] — what Enter does while Nastech is working."""
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        anthropic = _probe("nastech_cli.models", "_is_anthropic_fast_model", None, model)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        """Handle /debug [nastech|local] — upload debug report + logs and print share URLs.
        Default: public paste service; ``nastech``: Nastech-internal (staff-only); ``local``: render to
        ...
        run_debug_share(SimpleNamespace(
            lines=200, expire=7, local=local, nastech="nastech" in words and not local, yes=True))

    def _handle_update_command(self) -> bool:
        """Handle /update — exit the session and relaunch as ``nastech update``. Returns True when
        ...
        choices = [("once", "Update Now", "exit the current session and update Nastech Agent"),
                   ("cancel", "Cancel", "keep the current session")]
        raw = self._prompt_text_input_modal(
            title="𓄃  Update Nastech Agent",
            detail="This will exit the current session and run `nastech update`.", choices=choices)
        if raw is None or self._normalize_slash_confirm_choice(raw, choices) != "once":
            print("  🟡 /update cancelled.")
            return False
        _say_block("  𓄃 Launching update...")
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Nastech' hotword listener. The toggle IS the
