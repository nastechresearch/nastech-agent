...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        Syntax:
            ...
            /rollback <N> --all       — classic full restore (may overwrite
                                        files you edited after Nastech did)
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        # "Is `nastech gateway` running?" is the correct diagnosis, and the
        ...
        # CLI printed "Is `nastech gateway` running?" while the gateway was
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not session_meta:
            ...
            _cprint("  Use /sessions or `nastech sessions list` to see available sessions.")
            return
    ...
    def _handle_worktree_command(self, cmd_original: str) -> None:
        ...
        commits, same as ``nastech -w``).

        ``prune`` is the same attended reclaim as ``nastech worktree prune``
        (nastech_cli/worktree_gc.py): never deletes tracked changes, unique
        ...
        if sub in {"new", "add", "create"}:
            ...
            # same way `nastech -w` and session-resume cwd restore do.
            try:
                os.chdir(wt_info["path"])
            except OSError as e:
                print(f"  ⚠ Created worktree but could not enter it: {e}")
            os.environ["TERMINAL_CWD"] = wt_info["path"]
            # Register for the same keep-if-unpushed cleanup as `nastech -w`.
    ...
    def _handle_plan_command(self, cmd: str):
        ...
        ``.nastech/plans/`` via ``write_file``. No engine, no model-tool
