"""Default configuration data for Nastech Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from nastech_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    # journal_mode: SQLite journal mode for every Nastech DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `nastech -c`: each CLI session writes a breadcrumb under
        # $NASTECH_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `nastech
        ...
        # Nastech-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Nastech (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env NASTECH_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `nastech doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`nastech status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # NASTECH_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else NASTECH_HOME/cache/terminal (auto-pruned
        ...
        # (bytes); raise it for a ~/.nastech tree that legitimately exceeds 2 GiB.
        "sync_back_max_bytes": 2 * 1024 * 1024 * 1024,
        # HOME for host tool subprocesses: "auto" = host keeps the real OS-user HOME, containers use
        # NASTECH_HOME/home; "real" = force real HOME; "profile" = force NASTECH_HOME/home when it
        ...
        # ["/home/user/.nastech/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Nastech image,
        # which drops to `nastech` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `nastech tools`, not a ring
        # member). Set by the `nastech tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Nastech-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Nastech' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.nastech/browser-profile/ so credentials
        ...
        # runs `nastech browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `nastech chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.nastech/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `nastech checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .nastech.md,
    ...
    # Same bound for single-query mode (``nastech -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Nastech' summarizer cannot shrink it. native = codex decides; nastech = Nastech' threshold
        ...
        # scope it to one route ({"openai-codex:astra": 0.85} leaves Astra on OpenRouter/Nastech at the
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `nastech model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `nastech`/`nastech chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / NASTECH_TUI=1 forces the TUI.
        "interface": "cli",
        # `nastech --tui` auto-resumes the most recent human-facing session (like `nastech -c`).
        # NASTECH_TUI_RESUME=<id> always wins.
        ...
        # Nastech credits status-bar notices (usage bands, grant-spent, depleted/restored). False mutes
        ...
        # `path`/`paths` = NASTECH_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/nastech`.
        ...
        # CLI/TUI/desktop, managed with `nastech pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_nastech_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: NASTECH_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than NASTECH_TUI_SESSION_TTL_S, default 6h) with
        ...
        # OAuth gate (engaged when --host is set and --insecure is not), read by the Nastech Portal
        # plugin. Env NASTECH_DASHBOARD_OAUTH_CLIENT_ID / NASTECH_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env NASTECH_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # NASTECH_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env NASTECH_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/nastech) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.nastech/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Nastech turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Nastech (any model / provider you have
    ...
    # "Hey Nastech" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_nastech" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.nastech/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.nastech/cache/delegation/ with a head+tail window + read_file offset footer, nothing
    ...
    "moa": {
        ...
        # <nastech_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.nastech/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # NASTECH_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.nastech/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `nastech skills trust` / `untrust`.
        ...
        # skip; NASTECH_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${NASTECH_SKILL_DIR} / ${NASTECH_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `nastech skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.nastech/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.nastech/.curator_backups/blobs/); powers `nastech
    ...
    # daemon. `nastech curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `nastech curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `nastech update` restoring them);
        ...
        "prune_builtins": True,
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `nastech curator purge`
        # delete older archived skills (never automatic; logged in the ledger).
        "archive_ttl_days": 0,
        # Before every real (non-dry-run) pass, snapshot ~/.nastech/skills/ to
        # ~/.nastech/skills/.curator_backups/<utc-iso>/skills.tar.gz (`nastech curator rollback`).
        "backup": {
            "enabled": True,
    ...
    # peerName, sessions, enabled); nastech-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "𓄃 *Nastech Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (NASTECH_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export NASTECH_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. NASTECH_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `nastech plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `nastech plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.nastech/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # NASTECH_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "auth": {  # Login policy (credentials themselves live in auth.json / .env).
        ...
        # logins automatically when Nastech has no usable login of its own. Their refresh tokens are single-use
        # and rotate, so two programs on one login can log each other out; set false to make Nastech use only
        # its own logins (`nastech auth add <provider>`). `nastech auth add openai-codex` still offers the import
        # interactively.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .nastech config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `nastech doctor --ack <id>`; remove by editing the list. Catalog:
        # nastech_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $NASTECH_HOME/plugins/ <name>/),
        ...
        # holds NO scheduler credentials (provision reuses the Nastech Portal token).
        "chronos": {
        ...
        # Env override: NASTECH_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: NASTECH_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: NASTECH_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: NASTECH_CRON_MEDIA_SEND_TIMEOUT.
        ...
        # before one reminder ping (the run is still recorded; `nastech cron incidents` shows it).
        # A green run or a different error alerts again immediately; `nastech cron incidents ack`
    ...
    # promotes dependency-satisfied todos to ready, and fires `nastech -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across Nastech homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `nastech kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with nastech-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Nastech tools
    ...
    "tools": {
        ...
        # Remote connector discovery/lifecycle through the Nastech tool gateway.
        # The flag is the user's off switch; availability additionally requires
        # the portal sign-in every managed tool gates on.
        "connectors": {"enabled": True},
    },
    "logging": {  # File logging to ~/.nastech/logs/: agent.log captures INFO+, errors.log WARNING+.
        "level": "INFO",       # minimum level for agent.log: DEBUG, INFO, WARNING
        "max_size_mb": 5,      # max size per log file before rotation
        "backup_count": 3,     # rotated backups to keep
    },
    # Remote model-catalog manifest: curated OpenRouter / Nastech Portal model lists fetched from this
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: Nastech
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "nastech-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to NASTECH_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to NASTECH_STARTUP_WATCHDOG / NASTECH_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.nastech/sessions/sessions.json mirror of the routing index
        ...
        # `false` keeps per-profile gateways for good. `nastech gateway migrate --multiplex` folds a
        ...
        # adapter is parked; `nastech profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `nastech update` fold this install onto a multiplexed default gateway by itself?
        ...
        # `nastech gateway migrate --multiplex` is an explicit request and always proceeds.
        ...
        # (NASTECH_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.nastech/.env, auth.json). True:
        # files must be under the Nastech cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to NASTECH_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Nastech cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to NASTECH_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to NASTECH_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # NASTECH_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.nastech/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `nastech sessions prune`).
        ...
        # stay until `nastech sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `nastech update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.nastech/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to NASTECH_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`nastech sessions export`); checked
    ...
    # and transmission to Nastech (`send`) are SEPARATE opt-ins; see
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `nastech doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `nastech update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <NASTECH_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``nastech backup`` zip in
        # <NASTECH_HOME>/backups/ (``nastech import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `nastech update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `nastech update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <NASTECH_HOME>/lsp/bin/ on
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `nastech tools`.
    "x_search": {
    ...
    # encrypted vault (`nastech vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.nastech/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.nastech/cache/,
            ...
            # Auto-download bws into ~/.nastech/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Nastech sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Nastech also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.nastech/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Nastech Desktop (Electron) launch options; only affect `nastech desktop`.
        ...
        # Bridged to NASTECH_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to NASTECH_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nastech": {
        # Upper bound (seconds) on the Nastech auth keepalive tick, which derives from the
        ...
        # NastechResearch/api#227), so chat is the default until that is fixed.
        "anthropic_wire": "chat",
        # Nastech free tier: with no other provider configured, Nastech sets up a free Nastech identity on
        # first use (inference on nastech/welcome + connectors) and offers `/login` (terminal:
        # `nastech auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Nastech still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Nastech releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "NASTECH_BASE_URL": _base_url("Nastech Portal"),
    "NASTECH_ANON_API_SECRET": _env(
        "Shared secret for the Nastech free-tier sign-up endpoints while they are in their gated "
        "integration phase (not needed once the gate is removed)",
        "Nastech free-tier shared secret (leave empty unless given one)", password=True,
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Nastech mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'nastech model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": _tool(
        "Exact Firecrawl tool-gateway origin override for Nastech Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        "Explicit Nastech Subscriber access token for tool-gateway requests (optional; otherwise "
        "read from the Nastech auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Nastech' wake word engine (optional; "
    ...
    "NASTECH_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "NASTECH_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "NASTECH_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Nastech, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @nastech:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. NASTECH_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #nastech)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: nastech-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'nastech-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Nastech API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Nastech API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Nastech API server (proxy mode). Must "
    ...
    # NASTECH_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. NASTECH_TOOL_PROGRESS is unsupported.
    "NASTECH_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "NASTECH_EPHEMERAL_SYSTEM_PROMPT": _setting(
