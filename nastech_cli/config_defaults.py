...
DEFAULT_CONFIG = {
    ...
    "session": {
        # Per-terminal `nastech -c`: each CLI session drops a breadcrumb file
        # under $NASTECH_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by nastech_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `nastech chat --run-budget N`.
        ...
        # an interactive `nastech gateway restart` must never block for hours
        ...
        # `nastech doctor`. Delivered via the status channel only —
    ...
    "terminal": {
        ...
        # Root directory for Nastech' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Nastech uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at NASTECH_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `nastech -p <bot> chat -Q` recipient
    ...
    "web": {
        ...
        # `nastech tools`, not a ring member). Set by the `nastech tools`
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes Nastech spawn
        ...
        # `nastech doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a Nastech-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Nastech' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.nastech/browser-profile/) so copied
        ...
        # approval does it run `nastech browser close-profile` (terminates the
    ...
    "auxiliary": {
        ...
        "review": {
            "provider": "auto",    # auto (= inherit main model) | openrouter | nastech | anthropic | ...
    ...
    "dashboard": {
        ...
        # NASTECH_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (NASTECH_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``portal_url`` defaults to https://portal.nastechresearch.github.io).
        "oauth": {
    ...
    # goes to ~/.nastech/skills/ unless create_dir (below) redirects it.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Where agent-created skills (skill_manage action=create) are written.
        # Empty = the profile-local skills dir (~/.nastech/skills/). When set,
        ...
        # against NASTECH_HOME. The directory is scanned for skills alongside
        # the local dir. e.g. "/opt/brain/skills"
        "create_dir": "",
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.nastech/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``nastech skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``nastech skills trust`` / ``nastech skills untrust``.
        ...
        # (`nastech skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.nastech/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.nastech/.curator_backups/blobs/. Enables `nastech curator ledger`
        # and single-mutation `nastech curator rollback <entry-id>`.
    ...
    "curator": {
        ...
        # skills are kept forever. When > 0, `nastech curator purge` deletes
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "𓄃 *Nastech Agent*" header.
    ...
    # dangerous command. -q runs export NASTECH_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # modal; NASTECH_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # ``nastech plugins enable|disable`` and intentionally omitted here so an
    ...
    "cron": {
        ...
        # overridable via NASTECH_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    "model_catalog": {
        ...
        # /model or `nastech model` invocation once the cache is older than
    ...
    # #8731).
    #
    # Provider keys accept the Nastech provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # bridges these keys to the internal NASTECH_STARTUP_WATCHDOG /
        # NASTECH_STARTUP_WATCHDOG_TIMEOUT_S env vars AND applies them to the
    ...
    # directory. Collection is opt-in (``enabled``). Transmission to the Nastech
    ...
    "telemetry": {
        "shared_metrics": {
            "enabled": False,
            # Transmit exported packages to the Nastech telemetry service.
            ...
            # redirect telemetry a user consented to send to Nastech. Non-HTTPS
    ...
    "updates": {
        ...
        # When `nastech update` finds the source checkout parked on a feature
        ...
        #                         merge. `nastech update --switch-branch`
    ...
    "computer_use": {
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Nastech computer-use runtime.
        ...
        # manifest used when permission_mode is "bounded". Nastech passes the
    ...
    "nastech": {
        # Upper bound on the Nastech auth keepalive tick, in seconds. The tick
    ...
    # Nastech downloads official llama.cpp release binaries, then spawns and
    ...
    "local_runtime": {
        ...
        # (Nastech still finds an external llama-server you run yourself).
        "enabled": False,
        # Pinned llama.cpp release tag (rolling bNNNN). Bumped by Nastech
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nastech Subscribers only, used to derive vendor hosts, e.g. nastechresearch.github.io -> firecrawl-gateway.nastechresearch.github.io",
