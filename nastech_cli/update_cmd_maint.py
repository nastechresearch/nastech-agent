"""Post-update maintenance for ``nastech update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary.

Split out of ``update_cmd.py``, which re-imports every name so ``nastech_cli.update_cmd.<name>``
...
def _sqlite_partial_completion_lines(sqlite_version: str) -> list[str]:
    ...
    return [
        ...
        "corruption bug. Nastech works, but sessions could be damaged.",
        f"  Fix: run the installer again ({_REINSTALL_ONE_LINER[bool(_m()._is_windows())]}) "
        "which installs a safe Python, then run `nastech doctor` to confirm.",
    ]
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: nastech curator status)")
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    try:
        ...
        stop_result = _m()._kill_stale_dashboard_processes(
            ...
            scope_home=str(get_nastech_home()),
        )
        ...
        print("    nastech dashboard --port <port>   (or: systemctl --user restart nastech-dashboard)")
        return
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from nastech_cli.update_cmd import _branch_head_suffix
...
def _print_verified_update_completion(message: str) -> bool:
    ...
    from nastech_cli.update_cmd import _post_update_sqlite_runtime_status
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from nastech_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: nastech skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v nastech'`` resolves."""
    from nastech_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/nastech).
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `nastech gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``nastech-acp`` launcher next to ``nastech`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``nastech acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$NastechHome\bin``, never
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_nastech_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_nastech_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from nastech_cli.update_cmd import _record_update_step
...
def _run_full_backup() -> None:
    """Zip NASTECH_HOME under ``backups/`` (restorable via ``nastech import``). Never raises."""
    try:
    ...
    # display_nastech_home so the user sees ~/.nastech/...
    try:
        from nastech_constants import get_nastech_home, display_nastech_home
        display_path = f"{display_nastech_home()}/{out_path.relative_to(get_nastech_home())}"
    except Exception:
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of NASTECH_HOME under ``backups/`` (``nastech import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in nastech_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # NASTECH_HOME, so sync_skills()'s module-level NASTECH_HOME cache can't skew it.
    with suppress(Exception):
        from nastech_cli.profiles import list_profiles, seed_profile_skills
...
def _print_plugin_compat_notice() -> None:
    ...
    from nastech_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_profiles_without_credentials_notice() -> None:
    from nastech_cli.profile_credential_audit import print_profiles_without_credentials_notice
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from nastech_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    for message, step in (
        ...
        ('nastech-acp launcher self-heal failed: %s', _ensure_acp_launcher),
        ...
        # Legacy NASTECH_NEMO_RELAY_ATIF_*/ATOF_* vars produce no traces since the Relay cutover;
...
def _migrate_relay_exporter_env() -> None:
    from nastech_cli.relay_plugin_migrate import run_relay_migration_after_update
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from nastech_cli.update_cmd import _check_and_apply_config_migration, _m
