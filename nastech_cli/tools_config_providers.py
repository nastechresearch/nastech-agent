"""Provider rows, active-provider detection, model pickers and provider-selection persistence for `nastech tools`."""
...
from nastech_cli.config import cfg_get, get_env_value, load_config, save_config, save_env_value
from nastech_cli.nastech_account import format_nastech_portal_entitlement_message
from nastech_cli.nastech_subscription import MANAGED_FEATURE_COVERAGE_CATEGORY, NastechSubscriptionFeatures
from tools.tool_backend_helpers import NASTECH_MANAGED_PROVIDER, fal_key_is_configured
from utils import base_url_hostname, is_truthy_value

logger = logging.getLogger("nastech_cli.tools_config")
...
def _visible_providers(
    cat: dict, config: dict, *, force_fresh: bool = False, features: Optional[NastechSubscriptionFeatures] = None,
) -> list[dict]:
    """Provider entries visible for the current auth/config state.
    Nastech-managed rows (``managed_nastech_feature``) are always shown, even logged-out/unentitled, to
    advertise the capability."""
    from nastech_cli.tools_config import get_nastech_subscription_features
    ...
    for provider in cat.get("providers", []):
        managed = provider.get("managed_nastech_feature")
        # Managed rows stay visible regardless of auth (selecting one drives an inline Portal login); a
        # `requires_nastech_auth` row without a managed feature hides until logged in.
        if provider.get("requires_nastech_auth") and not managed and not features.nastech_auth_present:
            continue
...
def provider_readiness_status(provider: dict, config: dict, *, features=None, is_active: Optional[bool] = None) -> str:
    ...
    from nastech_cli.tools_config import (
        _POST_SETUP_READY, _provider_env_ready, _xai_credentials_present, get_nastech_subscription_features,
    )
...
def _toolset_needs_configuration_prompt(ts_key: str, config: dict, *, force_fresh: bool = False) -> bool:
    ...
    from nastech_cli.tools_config import TOOL_CATEGORIES, _post_setup_already_installed, _toolset_has_keys
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    ``reconfigure`` ("Reconfigure an existing tool"): no setup note / skip row / Nastech marker, and the
    chosen provider goes through the key-update prompts instead of the new-enable prompts."""
    from nastech_cli.tools_config import _prompt_choice, _provider_env_ready, get_nastech_subscription_features
    ...
    # Logged-in Nastech users get a marker on rows included in their subscription (cost-extra vs. included).
    _nastech_logged_in = False
    if not reconfigure:
        try:
            _nastech_logged_in = bool(get_nastech_subscription_features(config, force_fresh=force_fresh).nastech_auth_present)
        except Exception:
            pass
    ...
    for p in providers:
        ...
        sub_marker = ""
        if not reconfigure and p.get("managed_nastech_feature"):
            sub_marker = "  ★ Included with your Nastech subscription" if _nastech_logged_in else "  ★ via Nastech Portal (login on select)"
...
def _managed_provider_active(provider: dict, config: dict, managed_feature: str, force_fresh: bool) -> bool:
    """Active check for a Nastech-managed row: the feature must be managed AND the category's selected
    provider must be the row's vendor or ``nastech``."""
    from nastech_cli.tools_config import get_nastech_subscription_features

    feature = get_nastech_subscription_features(config, force_fresh=force_fresh).features.get(managed_feature)
    if feature is None:
        return False
    if managed_feature in ("image_gen", "video_gen"):
        ...
        if isinstance(gen_cfg, dict):
            ...
            if configured_provider not in {None, "", "fal", NASTECH_MANAGED_PROVIDER}:
                return False
            if (
                configured_provider != NASTECH_MANAGED_PROVIDER
    ...
    for marker, section, key in _MANAGED_SELECTION_KEYS:
        if _has_marker(provider, marker):
            ...
            selected = current in {provider[marker], NASTECH_MANAGED_PROVIDER}
            if marker == "web_backend":
                selected = selected and _web_tier_matches(provider, config)
            return feature.managed_by_nastech and selected
    return feature.managed_by_nastech
...
def _detect_active_provider_index(providers: list, config: dict, *, force_fresh: bool = False) -> int:
    ...
    from nastech_cli.tools_config import _provider_env_ready
...
def _managed_image_catalog(config: dict):
    ...
    acct = get_nastech_subscription_features(config).account_info
...
# picker time. "nastech" is the single managed row: one catalog spanning the FAL, Krea and Portal gateways.
IMAGEGEN_BACKENDS = {
    "fal": {"display": "FAL.ai", "config_key": "image_gen", "catalog_fn": _fal_model_catalog},
    "nastech": {"display": "Nastech Subscription", "config_key": "image_gen", "catalog_fn": _managed_image_catalog}}
...
def _pick_model_from_catalog(
    ...
    selection."""
    from nastech_cli.tools_config import _cfg_section, _prompt_choice
...
def _select_into(config: dict, section: str, key: str, vendor, managed) -> dict:
    """Write ``config[section][key] = vendor`` (``nastech`` for a managed pick) and drop any legacy ``use_gateway``
    key so the old read-time shim cannot override the new choice. Returns the section dict."""
    from nastech_cli.tools_config import _cfg_section

    cfg = _cfg_section(config, section)
    cfg[key] = NASTECH_MANAGED_PROVIDER if managed else vendor
...
def _select_plugin_gen_provider(section: str, plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    """Persist a plugin-backed image/video gen provider selection (``nastech`` for a Nastech-managed pick, else the
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    Pure, non-interactive core of :func:`_configure_provider` (no env prompts, post-setup hooks, Nastech
    auth gating or model pickers) shared by the CLI and the GUI ``PUT .../provider`` endpoint. Each pick
    writes exactly ONE provider string per category (``nastech`` for managed rows) and removes any legacy
    ...
    if managed_feature and managed_feature not in {"web", "tts", "stt", "browser"}:
        # Managed rows without a marker above (image_gen/video_gen "Nastech Subscription" rows carry only
        # managed_nastech_feature) still persist the "nastech" selection.
        section = config.setdefault(managed_feature, {})
        if isinstance(section, dict):
            section["provider"] = NASTECH_MANAGED_PROVIDER
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    # GUI flow); managed picks store "nastech". The in-tree FAL BYOK row always persists an explicit
...
def _nastech_provider_gate(provider: dict, config: dict, managed_feature, *, force_fresh: bool) -> bool:
    """Return False (after printing why) when a Nastech-gated row cannot be selected.
    Managed Tool Gateway rows are always listed but only *activate* with paid Nastech Portal access —
    selecting one runs an inline Portal login (auth + entitlement only, no inference-provider switch).
    Pure pre-auth UX rows (``requires_nastech_auth`` without a managed feature) keep the older logged-in +
    entitled gate."""
    from nastech_cli.tools_config import get_nastech_subscription_features

    if managed_feature:
        from nastech_cli.nastech_subscription import ensure_nastech_portal_access
        ...
        if not ensure_nastech_portal_access(
            ...
            _print_warning("  Not enabled — Nastech Portal access is required for this backend.")
            return False
        return True

    if provider.get("requires_nastech_auth"):
        features = get_nastech_subscription_features(config, force_fresh=force_fresh)
        ...
        if not features.nastech_auth_present or not entitled:
            message = format_nastech_portal_entitlement_message(
                features.account_info, capability=f"{provider.get('name', 'Nastech Subscription')}")
            _print_warning(f"  {message or 'Nastech Subscription is only available after logging into Nastech Portal.'}")
            return False
    return True
...
def _finish_provider_selection(provider: dict, config: dict, managed_feature) -> None:
    ...
    if backend:
        ...
        # "nastech" for the managed row (the picked model id chooses the FAL / Krea / Portal gateway at run time),
...
def _print_provider_selection(provider: dict, managed_feature, *, reconfigure: bool) -> None:
    ...
    if "browser_provider" in provider:
        ...
        if reconfigure and managed_feature:
            _print_success(f"  Browser cloud provider set to: {bp or 'nastech'}")
        elif bp == "local":
...
def _show_portal_hint(provider: dict, config: dict, managed_feature, force_fresh: bool) -> bool:
    """True when a BYOK row shares its category with a Nastech-managed sibling and the user is not authed to
    Nastech — a single dim hint tells them the key is avoidable via a Portal subscription."""
    from nastech_cli.tools_config import TOOL_CATEGORIES, get_nastech_subscription_features

    if managed_feature or provider.get("requires_nastech_auth"):
        return False
    try:
        for _cat in TOOL_CATEGORIES.values():
            _providers = _cat.get("providers", [])
            if provider in _providers and any(sib.get("managed_nastech_feature") for sib in _providers):
                return not get_nastech_subscription_features(config, force_fresh=force_fresh).nastech_auth_present
    except Exception:
        pass
    return False
...
def _configure_provider(provider: dict, config: dict, *, force_fresh: bool = True, reconfigure: bool = False):
    ...
    hook."""
    from nastech_cli.tools_config import _run_post_setup

    env_vars = provider.get("env_vars", [])
    managed_feature = provider.get("managed_nastech_feature")

    if not _nastech_provider_gate(provider, config, managed_feature, force_fresh=force_fresh):
        return
...
def _configure_vision_backend() -> None:
    ...
    Offers any authenticated provider + model (same surface as ``nastech model``) or a custom endpoint
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    Rows come from ``build_aux_picker_rows()`` so this lists exactly what the ``nastech model`` aux-task
    picker lists, including user-defined ``providers:`` / ``custom_providers:`` endpoints. Persists
    ``auxiliary.vision.provider`` + ``.model``."""
    from nastech_cli.tools_config import _prompt_choice
...
def _configure_simple_requirements(ts_key: str, *, reconfigure: bool = False):
    ...
    from nastech_cli.tools_config import TOOLSET_ENV_REQUIREMENTS, _toolset_has_keys, _toolset_label
