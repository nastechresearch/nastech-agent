"""Normalized Nastech Portal account entitlement helpers."""
...
@dataclass(frozen=True)
class NastechPaidServiceAccessInfo:
...
FREE_TIER_NEEDS_ACCOUNT = "This needs a Nastech account. Run `nastech auth upgrade`."
FREE_TIER_NEEDS_ACCOUNT_CHAT = "This needs an account. Use /login to sign in."


def _is_anonymous_tier(account_info: Optional["NastechPortalAccountInfo"]) -> bool:
...
class NastechPortalAccountInfo:
    ...
    nastech_client: Optional[str] = None
    ...
    paid_service_access_info: Optional[NastechPaidServiceAccessInfo] = None
    tool_access: Optional[NastechToolAccessInfo] = None
    raw_claims: Optional[dict[str, Any]] = None
    raw_account: Optional[dict[str, Any]] = None
    error: Optional[str] = None
    # NAS account tier claim; ``"anonymous"`` is the free tier (no Nastech account behind it).
    ...
    def is_anonymous_tier(self) -> bool:
        """The free tier: no Nastech account, so no billing, credits, or entitlement to speak of."""
...
def nastech_portal_billing_url(account_info: Optional[NastechPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized Nastech account snapshot."""
    try:
    ...
    if not _nonblank(base):
        base = DEFAULT_NASTECH_PORTAL_URL
    return f"{base.rstrip('/')}/billing"


def nastech_portal_topup_url(account_info: Optional[NastechPortalAccountInfo] = None) -> str:
    ...
    ``/orgs/None/billing``.
    """
    base = nastech_portal_billing_url(account_info)[: -len("/billing")]
...
def format_nastech_portal_entitlement_message(
    account_info: Optional[NastechPortalAccountInfo], *, capability: str = "this feature",
    include_refresh_hint: bool = True, coverage_category: Optional[str] = None,
    in_chat: bool = False,
) -> Optional[str]:
    """User-facing guidance for a missing Nastech tool-gateway entitlement; ``None`` when entitled.
    ...
    billing_url = nastech_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                return (
                    f"{capability} isn't included with your current Nastech Portal access. "
    ...
    if account_info is None:
        return (
            f"Nastech could not verify your Nastech Portal entitlement, so {capability} is unavailable. "
            f"Run `nastech model` to refresh your login, or check billing at {billing_url}."
        )
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nastech inference credentials are configured, but Nastech cannot verify your Nastech Portal "
                f"paid access for {capability}. Log in with `nastech model` to enable Portal-managed "
    ...
    if account_info.paid_service_access is None:
        detail = f"Nastech could not verify your Nastech Portal paid access, so {capability} is unavailable."
    ...
    if reason == "account_missing":
        return (
            f"Nastech could not find a Nastech Portal account or organisation for this login, so {capability} "
            f"is unavailable. Run `nastech model` to authenticate again; if the problem persists, contact Nastech support."
        )
...
def _no_paid_access_message(
    account_info: NastechPortalAccountInfo, capability: str, billing_url: str, *, in_chat: bool = False,
) -> str:
    if _is_anonymous_tier(account_info):
        return FREE_TIER_NEEDS_ACCOUNT_CHAT if in_chat else FREE_TIER_NEEDS_ACCOUNT
    access = account_info.paid_service_access_info or NastechPaidServiceAccessInfo()
    ...
    if access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Nastech Portal access is paused because you've exceeded the per-member spend cap set by "
            f"your organisation.{cap_detail}{credit_detail} Ask your organisation admin to raise the "
            f"member spend cap at {billing_url}, then run `nastech model` to refresh."
        )
    if active and paid:
        return (
            f"Your Nastech Portal credits are exhausted{credit_detail}, so {capability} is unavailable. "
    ...
    if active is False:
        return (
            f"Your Nastech Portal account has no active subscription or usable credits{credit_detail}, "
...
def reset_nastech_portal_account_info_cache() -> None:
    """Clear the short-lived account-info cache used by tests."""
    global _account_info_cache
    _account_info_cache = None


def get_nastech_portal_account_info(*, force_fresh: bool = False, min_jwt_ttl_seconds: int = 60) -> NastechPortalAccountInfo:
    """Normalized Nastech Portal account entitlement.
    ...
    if not _nonblank(access_token):
        return (
            ...
            or NastechPortalAccountInfo(logged_in=False, source="none", fresh=False, portal_base_url=portal_base_url)
        )
    if not force_fresh:
...
def nastech_policy_present() -> Optional[bool]:
    ...
    """
    try:
        from nastech_cli.auth import get_provider_auth_state, _decode_jwt_claims

        access_token = (get_provider_auth_state("nastech") or {}).get("access_token")
        if not _nonblank(access_token):
            return None
...
def nastech_policy_notice(*, removed: bool) -> str:
    ...
    """
    if not removed or nastech_policy_present() is not True:
        return ""
    return (
...
def _fresh_account_info(state: dict[str, Any], force_fresh: bool, portal_base_url: Optional[str]) -> NastechPortalAccountInfo:
    global _account_info_cache

    try:
        from nastech_cli.auth import get_provider_auth_state, resolve_nastech_access_token
...
def _info_from_inference_key_pool(portal_base_url: Optional[str]) -> Optional[NastechPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque Nastech keys."""
    try:
        entry = _select_nastech_pool_entry()
        if entry is None:
            return None
...
def _select_nastech_pool_entry() -> Optional[Any]:
...
def _info_from_valid_jwt(
    ...
    access_info = NastechPaidServiceAccessInfo(
    ...
    return NastechPortalAccountInfo(
        ...
        nastech_client=_coerce_str(claims.get("nastech_client")),
...
def _info_from_account_payload(
    ...
    access = _coerced_dataclass(NastechPaidServiceAccessInfo, payload.get("paid_service_access"))
    paid_access = None
...
def _tool_access_from_value(value: Any) -> Optional[NastechToolAccessInfo]:
    ...
    return NastechToolAccessInfo(enabled=value.get("enabled") is True, coverage=coverage)
...
def _subscription_from_payload(value: Any) -> Optional[NastechPortalSubscriptionInfo]:
    return _coerced_dataclass(NastechPortalSubscriptionInfo, value)
