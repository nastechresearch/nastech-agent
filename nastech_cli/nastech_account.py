...
def nastech_portal_topup_url(account_info: Optional[NastechPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = nastech_portal_billing_url(account_info)  # {base}/billing
...
def format_nastech_portal_entitlement_message(
    ...
    *,
    ...
    """Return user-facing guidance for a missing Nastech tool-gateway entitlement.
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nastech inference credentials are configured, but Nastech cannot verify "
...
def _no_paid_access_message(
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            f"Your Nastech Portal access is paused because you've exceeded the"
...
def get_nastech_portal_account_info(
    *,
    force_fresh: bool = False,
    min_jwt_ttl_seconds: int = 60,
) -> NastechPortalAccountInfo:
    """Return normalized Nastech Portal account entitlement information.
...
def nastech_policy_notice(*, removed: bool) -> str:
    ...
    A blocked model is omitted rather than marked, which reads as "Nastech does
...
def _tool_access_from_value(value: Any) -> Optional[NastechToolAccessInfo]:
    ...
    API) into :class:`NastechToolAccessInfo`. Fails closed: a non-object value
    ...
    return NastechToolAccessInfo(enabled=enabled, coverage=coverage)
...
def _paid_service_access_from_payload(value: Any) -> Optional[NastechPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return NastechPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
