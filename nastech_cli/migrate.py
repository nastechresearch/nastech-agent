...
Currently exposes only ``nastech migrate xai`` — diagnoses and (with --apply) rewrites references to
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    print()
    print(color("Run `nastech doctor` to confirm no retired xAI models remain.", Colors.DIM))
    return 0
