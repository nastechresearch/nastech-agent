"""Plugin catalog — curated, Nastech-approved Nastech plugins shipped with the repo.
...
an installed Nastech sees new entries and removals without updating. Any fetch failure falls back to the
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_nastech": self.requires_nastech,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_nastech=str(data.get("requires_nastech") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_nastech_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``NASTECH_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_nastech_home(cache.parent)
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_nastech:
        bits.append(f"Requires Nastech {entry.requires_nastech}.")
    return " ".join(bits)
