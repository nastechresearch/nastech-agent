"""Migrate Nastech MCP server config and Codex's installed curated plugins into ~/.codex/config.toml.
"""
...
from agent.transports.nastech_tools_mcp_server import NASTECH_TOOLS_MCP_SERVER_NAME
...
MIGRATION_END_MARKER = "# end nastech-agent managed section"
...
# Nastech MCP keys codex understands (transport stdio/http, timeouts, general). Any other key is
# dropped with a warning: ``sampling`` has no codex equivalent; the rest are unknown Nastech keys.
_KNOWN_NASTECH_KEYS = {
...
# (nastech key, codex key, skip note) — timeouts are emitted as floats or skipped when non-numeric.
_TIMEOUT_KEYS = (
...
def _translate_one_server(name: str, nastech_cfg: dict) -> tuple[Optional[dict], list[str]]:
    """Translate one Nastech MCP server config to codex's inline-table dict.

    Returns ``(codex_entry, skipped_keys)``; ``codex_entry`` is None when the config is unusable.
    stdio (``command``) wins over ``url`` when both are set. Nastech' ``transport: sse`` hint is
    ...
    skipped: list[str] = []
    out: dict[str, Any] = {}
    if nastech_cfg.get("command"):
        if nastech_cfg.get("url"):
            skipped.append("url (both command and url set; preferring stdio)")
        out["command"] = str(nastech_cfg["command"])
        if nastech_cfg.get("args"):
            out["args"] = [str(a) for a in nastech_cfg["args"]]
        if nastech_cfg.get("env"):
            out["env"] = _str_map(nastech_cfg["env"])
        if nastech_cfg.get("cwd"):
            out["cwd"] = str(nastech_cfg["cwd"])
    elif nastech_cfg.get("url"):
        out["url"] = str(nastech_cfg["url"])
        if nastech_cfg.get("headers"):
            out["http_headers"] = _str_map(nastech_cfg["headers"])
        if nastech_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]
    for nastech_key, codex_key, note in _TIMEOUT_KEYS:
        if nastech_key in nastech_cfg:
            try:
                out[codex_key] = float(nastech_cfg[nastech_key])
...
# env-var passthrough (NASTECH_HOME, PYTHONPATH) could carry one in pathological cases.
_TOML_ESCAPES = (
    ("\\", "\\\\"), ('"', '\\"'), ("\b", "\\b"), ("\t", "\\t"),
    ("\n", "\\n"), ("\f", "\\f"), ("\r", "\\r"))
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out += ["# (no MCP servers, plugins, or permissions configured by Nastech)", MIGRATION_END_MARKER]
        return "\n".join(out) + "\n"
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    Such dirs are reaped between sessions; a NASTECH_HOME pointing there burned into
...
def _build_nastech_tools_mcp_entry() -> dict:
    """Codex stdio entry launching Nastech' own tool surface as an MCP server (browser/web/
    delegate_task/vision/memory/skills call-backs).

    NASTECH_HOME passes through only IF SET, read from os.environ (not get_nastech_home()): when
    unset the codex subprocess must inherit its launcher's runtime NASTECH_HOME (systemd, gateway,
    kanban), not a migrate-time default burned into config.toml that pins the wrong profile. The
    pytest-tempdir guard keeps a sibling test's monkeypatched NASTECH_HOME out of the user's real
    config. PYTHONPATH passes through so a worktree-launched nastech finds the branch's modules.
    """
    import sys
    env: dict[str, str] = {}
    # NASTECH_HOME passes through IF SET so the MCP subprocess sees the same config / auth / sessions DB as
    # the parent CLI. Read from os.environ (not get_nastech_home()) on purpose: when the env var is unset we
    # want codex's subprocess to inherit whatever NASTECH_HOME its launcher sets at runtime (systemd unit,
    # gateway, kanban dispatcher, custom shell), rather than burning the migrate-time resolved default into
    # config.toml — that would override the launcher's NASTECH_HOME and pin the subprocess to the wrong
    # profile. The pytest-tempdir guard below catches the issue #26250 Bug C scenario: a sibling test's
    # monkeypatch.setenv("NASTECH_HOME", tmp_path) would otherwise leak a transient pytest tempdir into the
    ...
    if nastech_home and not _looks_like_test_tempdir(nastech_home):
...
def migrate(
    nastech_config: dict, *, codex_home: Optional[Path] = None, dry_run: bool = False,
    discover_plugins: bool = True, default_permission_profile: Optional[str] = ":workspace",
    expose_nastech_tools: bool = True) -> MigrationReport:
    """Translate Nastech mcp_servers config + Codex curated plugins into ~/.codex/config.toml.
    ...
    approval prompt on every write. ``expose_nastech_tools`` registers Nastech' own tool surface
    (agent/transports/nastech_tools_mcp_server.py, launched on demand by codex over stdio) as an MCP
    ...
    if not isinstance(nastech_servers, dict):
        report.errors.append("mcp_servers in Nastech config is not a dict; cannot migrate.")
        return report
    translated: dict[str, dict] = {}
    for raw_name, cfg in nastech_servers.items():
        name = str(raw_name)
    ...
    if expose_nastech_tools:
        translated[NASTECH_TOOLS_MCP_SERVER_NAME] = _build_nastech_tools_mcp_entry()
        if NASTECH_TOOLS_MCP_SERVER_NAME not in report.migrated:
            report.migrated.append(NASTECH_TOOLS_MCP_SERVER_NAME)
