...
Split out of ``nastech_cli/auth.py`` and re-exported there; origin helpers are imported lazily
inside each function so ``nastech_cli.auth.<name>`` patches still intercept (and no import cycle).
"""
...
from nastech_cli.auth_constants import (
    AuthError, DEFAULT_NASTECH_PORTAL_URL, DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS,
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(
        ...
        f"Nastech is waiting for the OAuth callback on {redirect_uri}\n"
...
def _resolve_verify(
    *, insecure: Optional[bool] = None, ca_bundle: Optional[str] = None,
    ...
    from nastech_cli.auth import _default_verify
    ...
    effective_ca = (
        ca_bundle or tls_state.get("ca_bundle") or os.getenv("NASTECH_CA_BUNDLE")
...
def _poll_device_token_generic(
    ...
    """RFC 8628 device-code polling loop shared by the Nastech and xAI flows.
...
def _poll_for_token(
    ...
    """Poll the Nastech token endpoint until the user approves or the code expires."""
    ...
    def _error(_response, error_payload) -> Exception:
        ...
        from nastech_cli.auth_error_copy import device_flow_error
    ...
    return _poll_device_token_generic(
        ...
        # (web_server_oauth._nastech_promotion_poller surfaces it to the UI) both inherit the guidance.
        on_timeout=lambda: TimeoutError(_nastech_device_auth_timeout_message(portal_base_url)))
    ...
    *resolve* attempts a refresh, so a resolved token should be valid — but double-check the
    ...
    try:
        ...
        if isinstance(api_key, str) and api_key and not is_expiring(api_key, 60):
            print(f"Existing {display_name} credentials found in Nastech auth store.")
