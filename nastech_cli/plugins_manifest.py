...
Split out of :mod:`nastech_cli.plugins`; validation warns and never fails a load.
"""
...
from nastech_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``nastech``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "nastech", "depends",
    "requires_nastech", "python_runtime",
}
...
def _plugins_debug() -> bool:
    from nastech_cli import plugins as _origin
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `nastech plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `nastech plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Nastech version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_nastech_error``.
    requires_nastech: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Nastech never auto-installs these (isolation design for the install seam
...
# ── requires_nastech version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_nastech_version() -> str:
    """Installed ``nastech-agent`` distribution version, else ``nastech_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("nastech-agent")
    except Exception:
...
def requires_nastech_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_nastech`` rejects the running version."""
    if not manifest.requires_nastech:
        return None
    current = running_nastech_version()
    if version_satisfies(manifest.requires_nastech, current):
        return None
    return f"requires nastech {manifest.requires_nastech}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_nastech=str(data.get("requires_nastech") or "").strip(),
