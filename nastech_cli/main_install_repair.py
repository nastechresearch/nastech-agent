...
def _stdout_to_stderr():
    ...
    a protocol stream (``nastech acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_core_update_marker_locked() -> None:
    ...
    # Windows: a ``nastech.exe`` launch has the launcher as an ancestor; the quarantined full
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\nastech.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``nastech.exe`` never matches.
...
def _windows_running_nastech_launcher_locked() -> bool:
    """True when a venv ``nastech*.exe`` shim is this process or an ancestor (best-effort)."""
...
def _reexec_dependency_sync_off_windows_shim() -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``nastech update`` so the sync and its tail happen
    ...
    user's own console.
    ``venv\\Scripts\\nastech.exe`` is a launcher that runs the interpreter with the shim as its script and
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `nastech update` starts, so an update running
...
def _quarantine_running_nastech_exe(
    ...
    """Pre-empt the Windows file lock on the running ``nastech.exe``.
    ...
    transient; Nastech Desktop backend child: until closed) — retry with backoff, then warn
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Nastech versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _nastech_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_nastech_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``nastech``
...
class ShimQuarantineError(RuntimeError):
    """A live ``nastech*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    Every editable install rewrites the entry-point shims; on Windows the live ``nastech.exe``
    can be neither deleted nor overwritten, so without quarantine ``nastech`` drops off PATH.
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `nastech` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    ...
    (1) orphan rescue — ``nastech.exe`` missing while ``nastech.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``nastech.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``nastech.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``nastech*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves nastech.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``nastech.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``nastech`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``nastech-agent.exe`` and ``nastech-acp.exe`` present but ``nastech.exe`` missing, so
    ``nastech`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    _report_still_missing(
        _missing(), "Workaround: python -m nastech_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``nastech update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `nastech update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        return
    _report_still_missing(
        _missing_deps(), "Run `nastech update --force` after closing other nastech processes.",
...
def _is_termux_env(env: dict[str, str] | None = None) -> bool:
    from nastech_cli.main import _is_termux_startup_environment
