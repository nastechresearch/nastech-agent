...
Nastech Portal). This module fetches, validates, and caches that manifest,
...
def get_catalog(*, force_refresh: bool = False) -> dict[str, Any]:
    ...
    # this via get_curated_nastech_model_ids on every open) never block on the
...
def refresh_catalogs() -> bool:
    ...
    free-pricing filter) and the Nastech Portal recommendations, writing each
...
def get_curated_nastech_models() -> list[str] | None:
    """Return Nastech Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("nastech")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
