...
``~/.nastech/cache/model_catalog.json`` → master URL fetch; any fetch failure keeps the stale copy
(or ``{}``). ``get_curated_openrouter_models()`` / ``get_curated_nastech_models()`` are thin accessors
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NastechResearch/nastech-agent/main/website/static/api/model-catalog.json",
)
...
def _fetch_manifest(url: str, timeout: float) -> dict[str, Any] | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": _NASTECH_USER_AGENT})
...
def refresh_catalogs() -> bool:
    """Force-refresh every remote catalog the picker reads (manifest, OpenRouter live list, Nastech Portal
    ...
    try:
        from nastech_cli.models import fetch_nastech_recommended_models, fetch_openrouter_models

        fetch_openrouter_models(force_refresh=True)
        fetch_nastech_recommended_models(force_refresh=True)
    except Exception:
...
def get_curated_nastech_models() -> list[str] | None:
    """Nastech Portal's curated model ids from the manifest."""
    return [mid for mid, _ in _block_ids(_get_provider_block("nastech"))] or None
...
def get_default_model_from_cache(provider: str) -> str | None:
    """The manifest's labeled default for ``provider`` (the model Nastech silently lands on when the
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    After ``nastech update`` that file IS the newest catalog, so the picker stays current even when
