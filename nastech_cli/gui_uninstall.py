"""Nastech Desktop (Chat GUI) uninstaller: removes only GUI state — built Electron artifacts, the packaged
...
def desktop_userdata_dir() -> Path:
    """Electron ``app.getPath('userData')`` for an app named "Nastech" on each platform (GUI-only state)."""
    home = Path.home()
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Nastech"
    if sys.platform == "win32":
        return _env_dir("APPDATA", home / "AppData" / "Roaming") / "Nastech"
...
def source_built_gui_artifacts(nastech_home: Path) -> "list[Path]":
    ...
    return [desktop_dir / "dist", desktop_dir / "release", desktop_dir / "node_modules",
            agent_root / "node_modules", nastech_home / "desktop-build-stamp.json"]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    home = Path.home()
    if sys.platform == "darwin":
        return [Path("/Applications/Nastech.app"), home / "Applications" / "Nastech.app"]
    if sys.platform == "win32":
        ...
        return [local_base / "Programs" / "Nastech", local_base / "nastech-desktop"] + (
            [Path(program_files) / "Nastech"] if program_files else [])
    ...
    return [desktop_entry_path(), data_base / "applications" / "Nastech.desktop"] + [
        icons / size / "apps" / "nastech.png"
        for size in ("scalable", "24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024")]


def agent_is_installed(nastech_home: Path) -> bool:
    """True when a usable Python agent install exists under NASTECH_HOME (gates the desktop UI's options).
    Package source or a venv alone is enough — a source checkout without a venv is still "the agent is here"."""
    return any((nastech_home / "nastech-agent" / sub).is_dir() for sub in ("nastech_cli", "venv", ".venv"))


def gui_is_installed(nastech_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    return any(p.exists() for p in (*source_built_gui_artifacts(nastech_home), *packaged_gui_app_paths(), desktop_userdata_dir()))
...
def gui_install_summary(nastech_home: "Path | None" = None) -> dict:
    ...
    return {"nastech_home": str(home), "agent_installed": agent_is_installed(home),
...
def uninstall_gui(nastech_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    if sys.platform.startswith("linux"):
        # The desktop entry was removed above but the menu caches still list it; reindex so Nastech
        # disappears from the launcher.
        try:
            from nastech_cli.linux_desktop_entry import desktop_entry_path, refresh_desktop_databases
        ...
        log_info("If you installed the desktop via a .deb / .rpm package, remove it with your package manager "
                 "(e.g. 'sudo apt remove nastech' or 'sudo dnf remove nastech'). AppImage builds are a single "
