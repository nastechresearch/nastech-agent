"""Gateway fleet restart + post-update verification for ``nastech update``.

Split out of ``nastech_cli/update_cmd.py``; every name is re-imported there so
``nastech_cli.update_cmd.<name>`` keeps resolving/monkeypatching. Origin helpers are
...
from nastech_cli.update_inventory import _gateway_service_matches_profile

# Log-record parity with the origin module.
logger = logging.getLogger("nastech_cli.update_cmd")

# Under NASTECH_HOME (not next to the venv): records the fleet-restart obligation
...
_LIST_GATEWAY_UNITS = ["list-units", "nastech-gateway*", "nastech-serve*", "--plain", "--no-legend", "--no-pager"]


def _write_gateway_update_exit_code(ok: bool) -> None:
    from nastech_cli.update_cmd import get_nastech_home
...
def _write_fleet_restart_pending_marker(*, expected_sha: str = "", runtimes: list[dict] | None = None) -> None:
    ...
    from nastech_cli.update_cmd import _m
...
def _current_checkout_sha() -> str | None:
    ...
    from nastech_cli.update_cmd import _capture_head_sha, _m
    try:
...
def _receipt_looks_unfinished(receipt: dict) -> bool:
    ...
    successful receipt look unfinished, or the next ``nastech update`` retriggers
...
def _receipt_reports_stale_runtime(receipt: dict, expected_sha: str | None = None) -> bool:
    ...
    *before* the pull, so a finished update's plan always looks stale and must not
    ...
    See #95294.
    """
    from nastech_cli.update_cmd import _current_checkout_sha
...
def _live_fleet_covers_receipt(expected_sha: str | None, receipt: dict, owed: set[tuple[str, str]] | None, *, accept_states: tuple = ("current",)) -> bool:
    ...
    if not expected_sha:
        return False
    from nastech_cli.update_receipt import collect_fleet_versions
...
def _marker_only_restart_obsolete() -> bool:
    ...
    """
    from nastech_cli.update_serve_obligations import defer_manual_serve
...
def _pending_fleet_restart_needed(*, receipt: dict | None = None, pending_manual: list[dict] | None = None) -> bool:
    ...
    from nastech_cli.update_serve_obligations import retain_receipt_manual_serves
...
def _update_owes_fleet_restart(*, receipt: dict | None = None, pending_manual: list[dict] | None = None) -> bool:
    ...
    # A fleet an operator has since restarted onto a moved checkout (``nastech gateway restart`` —
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print("⚠ A previous `nastech update` pulled new code but did not restart running gateways.", file=stream)
    print("  Gateways may still be serving pre-update modules (mixed sys.modules).", file=stream)
    if startup:
        print("  Run `nastech update` or `nastech gateway restart`.", file=stream)


def _warn_pending_fleet_restart_on_startup() -> None:
    """Cheap CLI-startup hint. Never restarts; never raises."""
    from nastech_cli.update_receipt import read_latest_receipt
    from nastech_cli.update_serve_obligations import retain_receipt_manual_serves, warn_pending_manual_serves
...
def _run_pending_fleet_restart() -> bool:
    ...
    # Warn if legacy Nastech gateway unit files are still installed. When both nastech.service (from a
    # pre-rename install) and the current nastech-gateway.service are enabled, they SIGTERM-fight for the
    # same bot token (see PR #11909). Flagging here means every `nastech update` surfaces the issue until the
    # user migrates.
    try:
    ...
    try:
        ...
        # --- Systemd services (Linux) --- Discover all nastech-gateway* units (default + profiles) plus
        ...
        # --- Launchd services (macOS) --- Restart EVERY ai.nastech.gateway* LaunchAgent, not only the
...
def _defer_fleet_restart_after_update(*, update_complete: bool, resume_incomplete: bool = False) -> None:
    ...
    ``nastech update --no-gateway-restart`` (cron running inside the gateway's
    ...
    next normal update (or ``nastech gateway restart``) catches up.
    ...
    print("  Restart them separately: `nastech gateway restart` or a daily-restart cron.")
    print("  (fleet restart deferred — marker kept for catch-up)")
    with suppress(Exception):
        from nastech_cli.update_receipt import record_skip
...
def _apply_pending_fleet_restart_catchup(*, defer: bool = False) -> None:
    ...
    """
    from nastech_cli.update_cmd import _run_pending_fleet_restart
    ...
    if defer:
        ...
        print("  Restart separately: `nastech gateway restart` or next non-cron update.")
        return
    print()
    _warn_pending_fleet_restart()
...
def _is_nastech_gateway_unit(unit: str) -> bool:
    """Exact base unit or hyphenated profile family only: ``startswith("nastech-serve")``
    would accept ``nastech-server.service``."""
    return (
...
def _for_each_systemd_gateway_unit(list_units_stdout: str, *, process_unit, on_unit_timeout) -> None:
    """Process each nastech-gateway*/nastech-serve* unit from ``systemctl list-units``.
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        if not parts:
            continue
        unit = parts[0]
        if not unit.endswith(".service") or not _is_nastech_gateway_unit(unit):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``nastech-gateway*`` runs ``gateway/run.py`` (the handler); SIGUSR1 would just
    kill ``nastech-serve*`` and burn the drain budget, so those go straight to the blunt
    restart. Same exact/hyphenated shape as ``_for_each_systemd_gateway_unit`` so a
    near-prefix unit like ``nastech-gatewayd`` is never signalled.
...
def _restart_macos_launchd_gateways(
    ...
    The pull is shared across profiles, so every ``ai.nastech.gateway*`` LaunchAgent
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    ``None`` when undeterminable (notably ``nastech_cli.gateway`` no longer importing
...
# NOT a claim that no supervisor exists: a systemd-launched serve sets neither NASTECH_SPAWN
# nor NASTECH_PARENT_PID ("manual-serve"). Unit-backed serves are recovered by the fresh
...
_SERVE_SKIP_REASON = (
    ...
    " systemd unit pass when it owns a nastech-serve* unit, else left running for explicit"
    " operator restart"
)
...
def _gateway_recovery_partition(plan, *, skip_profiles: set[str] | None = None) -> tuple[dict[str, str], list[dict]]:
    ...
    Uses only the pre-checkout inventory: re-importing ``nastech_cli.gateway`` in the
    ...
    child's ``nastech-serve*`` systemd pass enumerates units from systemd; leftovers are
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    logged at debug level, so an early failure (e.g. importing ``nastech_cli.gateway`` from the freshly
...
def _gateway_home_for_pid(pid: int):
    """NASTECH_HOME of the gateway ``pid`` per the fleet inventory, else None (own profile's file)."""
    with suppress(Exception):
        from nastech_cli.update_receipt import _profile_homes
...
def _resolve_manage_cmd(cache: dict, scope_: str, scope_cmd_: list, svc_name_: str):
    ...
    so a least-privilege sudoers entry scoped to nastech-gateway* qualifies (idempotent
...
def _restart_one_systemd_gateway_unit(
    ...
    # → drain → exit, Restart=always respawns. nastech-serve has no handler → blunt restart below.
    _main_pid = 0
    ...
    # budget, restart-policy mismatch). Mirrors `nastech gateway restart` (`systemd_restart()`).
...
def _restart_systemd_gateway_units(restarted_services, failed_or_stale_units, restarted_scoped_units, drain_budget):
    """Restart every active nastech-gateway*/nastech-serve* systemd unit (user + system).
    ...
    """
    from nastech_cli.gateway import supports_systemd_services, _ensure_user_systemd_env
...
def _restart_manual_gateways(out: _GatewayRestartOutcome, _drain_budget) -> None:
    ...
    if out.restarted_services or out.killed_pids:
        ...
        if unmapped_count:
            ...
            if unmapped_count > 1:
                print("    (or: nastech -p <profile> gateway run  for each profile)")
...
def _force_kill_stuck_gateways(killed_pids) -> None:
    ...
    with _best_effort('Post-restart survivor sweep failed: %s'):
        from nastech_cli.gateway import find_gateway_pids, _get_service_pids
...
def _recover_after_restart_phase_abort(
    ...
    from nastech_cli.update_abort_recovery import _owed_stale_serve_rows
    from nastech_cli.update_cmd import (
...
def _restart_gateway_fleet_after_update(_pre_update_plan, gateway_mode: bool):
    ...
    """
    from nastech_cli.update_cmd import _m, _write_gateway_update_exit_code
    ...
    # Scope-qualified twin (``user/nastech-serve`` vs ``system/nastech-serve`` are different
    ...
    try:
        ...
        from nastech_cli.gateway import (  # noqa: F401
            is_macos,
            find_gateway_pids,
        ...
        # macOS: EVERY ai.nastech.gateway* LaunchAgent (systemd parity).
        if is_macos():
...
def _print_legacy_units_warning() -> None:
    """Legacy nastech.service fights nastech-gateway.service over the bot token; warn on
    every update until migrated."""
    from nastech_cli.gateway import (has_legacy_nastech_units, _find_legacy_nastech_units, supports_systemd_services)
    if not (supports_systemd_services() and has_legacy_nastech_units()):
        return
    print()
...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    # units, so a unit-less `nastech serve` keeps stale sys.modules. Runs AFTER
    ...
    # units, so an sshd-spawned `serve --isolated` or a manual `nastech serve` (no unit) is left running its
    ...
    with _best_effort('Fleet version verification failed: %s'):
        from nastech_cli.update_receipt import print_fleet_version_matrix
    ...
    with _best_effort('Runtime-outcome reconciliation failed: %s'):
        ...
        if _pre_update_plan is not None and _pre_update_plan.runtimes:
            from nastech_cli.update_inventory import (match_runtime_outcomes, report_unaccounted_runtimes)
    ...
    with _best_effort('Multiplex auto-migration after update failed: %s'):
        from nastech_cli.gateway_migrate import maybe_auto_migrate_after_update
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state (typically the freshly-pulled
    ``nastech_cli.gateway`` no longer imports, one of the ways the phase aborts). That is proof-of-safety
