...
from nastech_cli.curses_ui import MenuNavigationEvent, MenuNavigationStart
...
_DOCS_BASE = "https://nastechresearch.github.io/nastech-agent/docs"
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar(
    "nastech_setup_navigation", default=None
)
...
def is_noninteractive() -> bool:
    ...
    ``NASTECH_NONINTERACTIVE=1`` (see ``nastech_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("NASTECH_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``NASTECH_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, nastech_home):
    ...
    if not _provider_ready:
        ...
        print_info("    nastech setup --portal   (Nastech Portal OAuth, no API key)")
    ...
    if subscription_features.web.managed_by_nastech:
        tool_status.append(("Web Search & Extract (Nastech subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_nastech:
        tool_status.append(("Browser Automation (Nastech Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Nastech), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_nastech:
        tool_status.append(("Image Generation (Nastech subscription)", True, None))
    ...
    if subscription_features.video_gen.managed_by_nastech:
        tool_status.append(("Video Generation (FAL via Nastech subscription)", True, None))
    else:
        try:
    ...
    if subscription_features.tts.managed_by_nastech:
        tool_status.append(("Text-to-Speech (OpenAI via Nastech subscription)", True, None))
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nastech:
        tool_status.append(("Speech-to-Text (OpenAI via Nastech subscription)", True, None))
    ...
    if subscription_features.modal.managed_by_nastech:
        tool_status.append(("Modal Execution (Nastech subscription)", True, None))
        ...
        tool_status.append(("Modal Execution (optional via Nastech subscription)", True, None))
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # Tool Gateway prompt is already shown by _model_flow_nastech() above.
    save_config(config)
...
def _setup_tts_provider(config: dict):
    ...
    if managed_nastech_tools_enabled() and subscription_features.nastech_auth_present:
        choices.append("Nastech Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    selected_via_nastech = selected == "nastech-openai"
    if selected == "nastech-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Nastech gateway and bill to your subscription.")
    ...
    elif selected == "openai" and not selected_via_nastech:
...
def setup_terminal_backend(config: dict):
    ...
    # under ~/.nastech/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Nastech gateway and bill to your subscription.")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.nastech/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        profile_name = _profile_name_from_nastech_home(Path(get_nastech_home()))
    except Exception:
        pass
...
def _profile_name_from_nastech_home(nastech_home) -> str | None:
    """Return the active profile name when NASTECH_HOME is a profile dir."""
    if nastech_home.parent.name == "profiles":
        return nastech_home.name
    return None
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://nastechresearch.github.io/nastech-agent/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
...
def setup_gateway(config: dict):
    ...
    # via `nastech import` or `nastech setup gateway`). Gating this on
...
def setup_telemetry(config: dict):
    ...
    print_info("Collection is local. Sending them to Nastech is a separate opt-in.")
    ...
    print_info("")
    print_info("Sending uploads each daily package to the Nastech telemetry")
    ...
    shared_metrics["send"] = prompt_yes_no(
        "Send shared metrics to Nastech?",
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        ...
        from nastech_cli.observability.shared_metrics_sender import (
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      ...
      * ``active_provider`` in the auth store — covers OAuth device-code /
        external-OAuth providers (Nastech, Codex, Qwen, Gemini CLI, ...).
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nastech Portal setup — OAuth + model pick + provider + Tool Gateway.

    Wired into ``nastech setup --portal`` and ``nastech portal``. This is the
    Nastech-Portal slice of the first-time quick setup, collapsed into a single
    ...
    delegated to ``_model_flow_nastech`` — the exact same flow quick setup uses
    (``_run_first_time_quick_setup``) and the same one ``nastech model`` runs
    when you pick Nastech. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``nastech portal`` always offers a model picker,
    and there is a single source of truth for the Nastech onboarding steps.
    """
    ...
    print_info("    — all routed through your Nastech Portal sub.")
    print()
    print_info("  Sign up: https://portal.nastechresearch.github.io/manage-subscription")
    print()

    # _model_flow_nastech handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nastech model picker), then offers the Tool Gateway opt-in and sets
    # provider=nastech via the login/model save. This is the same routine quick
    # setup calls, so `nastech portal` == quick setup's Nastech step.
    try:
        from nastech_cli.main import _model_flow_nastech

        _model_flow_nastech(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_nastech raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_nastech catches it, but the
        ...
        logger.debug("_model_flow_nastech error during `nastech portal`: %s", exc)
        print()
        print_error(f"  Nastech Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `nastech portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_nastech (and the
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``nastech model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    # --portal: one-shot Nastech Portal setup. Skips the rest of the wizard.
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps(
                [("Quick Setup", lambda: _run_quick_setup(config, nastech_home))]
            )
            return
        ...
        setup_mode = prompt_choice(
            ...
            [
                "Quick Setup (Nastech Portal) — free OAuth login, no API keys, model + tools (recommended)",
                ...
                [
                    (
                        ...
                        lambda: _run_first_time_quick_setup(
                            config, nastech_home, is_existing
                        ),
                    )
                ]
            )
    ...
    def _gateway_step() -> None:
        ...
        from nastech_cli.gateway import ensure_gateway_service
...
def _run_first_time_quick_setup(config: dict, nastech_home, is_existing: bool):
    """Streamlined first-time setup via Nastech Portal: OAuth, model, terminal & messaging.

    Routes straight to the Nastech Portal provider — runs the device-code OAuth
    login, picks a Nastech model, then configures the terminal backend and (optionally)
    ...
    # Step 1: Nastech Portal — OAuth login + model selection.
    # _model_flow_nastech() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # Nastech model picker). Provider is set to "nastech" by the login/model save.
    print()
    print_header("Nastech Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: https://portal.nastechresearch.github.io/manage-subscription")
    print()
    try:
        ...
        print()
        print_info("Nastech Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_nastech error during quick setup: %s", exc)
        print_warning(f"Nastech Portal setup encountered an error: {exc}")
        print_info("You can try again later with: nastech model")

    # Re-sync the wizard's config dict from disk — _model_flow_nastech (and the
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `nastech import` from another machine).
        from nastech_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  your terminal (and Nastech.app if you use Desktop), or run:")
    print_info("    open \"x-apple.systempreferences:com.apple.preference"
               ".security?Privacy_AllFiles\"")
    print_info("  The grant is permanent — it survives every Nastech update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``nastech-agent`` skill (the agent's operating manual for driving,
    configuring, and troubleshooting Nastech) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``nastech tools`` (which
    ...
    try:
        ...
        from nastech_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``nastech setup agent`` /
    ``nastech config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, nastech_home, is_existing: bool):
    ...
    print_info("essential `nastech-agent` skill is always kept so the agent")
    print_info("can help you drive and configure Nastech itself.")
    print()
    ...
    if path == 0:
        ...
        # `nastech update` runs don't re-inject them. Essential skills (the
        # `nastech-agent` operating manual) are still seeded by the sync.
        try:
        ...
        print_info("  Enable tools:        nastech tools")
        print_info("  Seed skills:         nastech skills opt-in --sync")
        print_info("  Add MCP servers:     nastech mcp add")
        print_info("  Enable plugins:      nastech plugins")
        print_info("  Tune agent settings: nastech setup agent")
        print()
        _print_setup_summary(config, nastech_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, nastech_home)


def _blank_slate_walkthrough(config: dict, nastech_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            set_bundled_skills_opt_out(True)
            # Essential skills (the `nastech-agent` operating manual) are
            # still seeded even for an opted-out profile.
            sync_skills(quiet=True)
            print_info("No skills seeded (except the essential `nastech-agent`")
            print_info("skill). A .no-bundled-skills marker keeps future")
            print_info("`nastech update` runs from re-injecting them. Opt back in any")
            print_info("time with `nastech skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `nastech tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `nastech plugins list` / `nastech plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `nastech plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `nastech mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `nastech mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   nastech tools")
    print_info("  Seed skills:         nastech skills opt-in --sync")
    print_info("  Add MCP servers:     nastech mcp add")
    print_info("  Tune agent settings: nastech setup agent")
    print()
