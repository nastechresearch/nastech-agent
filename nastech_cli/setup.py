"""Interactive setup wizard for Nastech Agent (config lives in ~/.nastech/).
...
``nastech_cli.setup.<name>`` keep working.
"""
...
# Config helpers are re-exported (tests patch them on this module). display_nastech_home is
# imported lazily at call sites (stale-module safety during nastech update).
from nastech_cli.config import (
    cfg_get, DEFAULT_CONFIG, get_nastech_home, get_config_path, get_env_path, load_config, save_config,
    save_env_value, remove_env_value, get_env_value, ensure_nastech_home,
)
from nastech_cli.colors import Colors, color
from nastech_cli.cli_output import print_error, print_info, print_success, print_warning
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    _info("The interactive wizard cannot be used here.", None,
          "Configure Nastech using environment variables or config commands:",
          "  nastech config set model.provider custom",
          "  nastech config set model.base_url http://localhost:8080/v1",
          "  nastech config set model.default your-model-name", None,
          "Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.",
          "Run 'nastech setup' in an interactive terminal to use the full wizard.", None)
...
_SETUP_NAVIGATION: ContextVar[_SetupNavigationState | None] = ContextVar("nastech_setup_navigation", default=None)
...
def _setup_navigation_scope():
    ...
    from nastech_cli.curses_ui import reset_menu_navigation_handler, set_menu_navigation_handler
...
def run_setup_action_with_navigation(
    ...
    ``nastech model`` that use the wizard's pickers outside ``run_setup_wizard``."""
...
def is_noninteractive() -> bool:
    ...
    with ``stdin=DEVNULL`` and ``NASTECH_NONINTERACTIVE=1`` (``nastech_cli/web_server.py``), where a
    prompt that aborts on EOF would kill the spawned action — callers fall back to their default."""
    return os.environ.get("NASTECH_NONINTERACTIVE", "").strip().lower() in {"1", "true", "yes", "on"}


def prompt_yes_no(question: str, default: bool = True) -> bool:
    """Prompt for yes/no. Ctrl+C exits; empty input, ``NASTECH_NONINTERACTIVE=1`` or a
...
def _prompt_api_key(var: dict):
    ...
    print()
    _prompt_and_save_env_var(var, "  ✓ Saved", "  Skipped (configure later with 'nastech setup')")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    """Configure the inference provider and default model via the ``nastech model`` flow (one code
    ...
    try:
        ...
        lead, *rest = provider_setup_failure_lines(exc, retry_command="nastech model")
        print_warning(lead)
        for line in rest:
            print_info(line)
    ...
    # keep safe defaults (configure via `nastech auth add` / `nastech setup tts`).
    config.clear()
...
def _apply_default_agent_settings(config: dict):
    ...
    # config.yaml is authoritative for max_turns (the gateway bridges it into NASTECH_MAX_ITERATIONS);
    ...
    _info("  Max iterations: 150", "  Tool progress: all", "  Compression threshold: 0.50",
          "  Run `nastech setup agent` later to customize.")
...
_TOOL_PROGRESS_HELP = (
    ...
    "  log     — Silent in chat; write every tool call to ~/.nastech/logs/tool_calls.log (gateway only)",
)
def setup_agent_settings(config: dict):
    ...
    if max_iter is None:
        ...
        # config.yaml only; gateway/run.py derives NASTECH_MAX_ITERATIONS from agent.max_turns.
...
def setup_tools(config: dict, first_install: bool = False):
    """`nastech setup tools` == `nastech tools`: platform selection → toolset toggles → provider keys.
...
_SEND_CONSENT_EXPLAINER = (
    "", "Sending uploads each daily package to the Nastech telemetry",
...
def setup_telemetry(config: dict):
    ...
    _info("Shared metrics contain only bounded counters and histograms.",
          "Collection is local. Sending them to Nastech is a separate opt-in.")
    ...
    shared_metrics["send"] = prompt_yes_no("Send shared metrics to Nastech?", default=shared_metrics.get("send") is True)
...
def _record_send_consent_change(*, enabled: bool) -> None:
    ...
    try:
        ...
        from nastech_cli.observability.shared_metrics_sender import reconcile_send_consent
...
# nastech_cli.setup. They import this module lazily inside bodies, so this is cycle-free.

from nastech_cli.setup_tts import setup_tts  # noqa: E402
from nastech_cli.setup_terminal import setup_terminal_backend  # noqa: E402
from nastech_cli.setup_platforms import setup_gateway  # noqa: E402
from nastech_cli.setup_summary import _print_setup_summary  # noqa: E402,F401
from nastech_cli.setup_migration import _offer_openclaw_migration, _skip_configured_section  # noqa: E402
from nastech_cli.setup_quick import _run_portal_one_shot, _run_quick_setup  # noqa: E402
...
def _run_setup_section(config: dict, section: str) -> None:
    """``nastech setup <section>``: run one SETUP_SECTIONS entry under the banner."""
    ...
    if entry is None:
        ...
        return
    label, func = entry
    _print_banner(f"│     𓄃 Nastech Setup — {label:<34s} │")
...
def _run_full_setup(config: dict, nastech_home, *, is_existing: bool, migration_ran: bool) -> None:
    ...
    _info(f"Config file:  {get_config_path()}", f"Secrets file: {get_env_path()}",
          f"Data folder:  {nastech_home}", f"Install dir:  {PROJECT_ROOT}", None,
          "You can edit these files directly or use 'nastech config edit'")
    if migration_ran:
...
def _run_setup_wizard_impl(args):
    ...
    ``nastech setup <section>`` from SETUP_SECTIONS."""
    ...
    if getattr(args, 'non_interactive', False) or not is_interactive_stdin():
        ...
        return
    if getattr(args, "portal", False):  # one-shot Nastech Portal setup; skips the rest
    ...
    _print_banner("│             𓄃 Nastech Agent Setup Wizard                │",
                  "├─────────────────────────────────────────────────────────┤",
                  "│  Let's configure your Nastech Agent installation.       │",
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_setup_steps([("Quick Setup", lambda: _run_quick_setup(config, nastech_home))])
            return
        ...
        migration_ran = _offer_openclaw_migration(nastech_home)  # before configuration begins
        if migration_ran:
            config = load_config()
        setup_mode = prompt_choice("How would you like to set up Nastech?", [label for label, _ in _FIRST_TIME_MODES], 0)
        label, runner = _FIRST_TIME_MODES[setup_mode]
        if runner is not None:
            from nastech_cli import setup_quick
            _run_setup_steps([(label, lambda: getattr(setup_quick, runner)(config, nastech_home, is_existing))])
            return
    _run_full_setup(config, nastech_home, is_existing=is_existing, migration_ran=migration_ran)
...
_PLUGIN_COMPAT_LAZY = {
    'get_nastech_subscription_features': ('nastech_cli.nastech_subscription', 'get_nastech_subscription_features'),
    'get_optional_skills_dir': ('nastech_constants', 'get_optional_skills_dir'),
