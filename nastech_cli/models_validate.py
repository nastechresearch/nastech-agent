...
Split out of ``nastech_cli.models``. Catalog fetchers defined in ``nastech_cli.models`` are looked up
there at call time (``_m.<name>``) so ``patch("nastech_cli.models.<name>")`` mocks keep intercepting;
local-server probes are looked up on ``nastech_cli.models_local`` (``_ml.<name>``) the same way.
...
def _validate_moa(req: _Request) -> dict[str, Any]:
    try:
        ...
        if req.requested in cfg["presets"]:
            return _accept()
        return _reject(f"MoA preset `{req.requested}` was not found. Run `nastech moa list`.")
...
def _validate_lmstudio(req: _Request) -> dict[str, Any]:
    from nastech_cli import models_local as _ml
...
def _ollama_probe_headers(req: _Request) -> dict[str, str]:
    ...
    from nastech_cli import models as _m
    from nastech_cli import models_local as _ml
    from nastech_cli.models_local import _configured_ollama_base_url, _drop_authorization
...
def _validate_static_catalog(req: _Request) -> Optional[dict[str, Any]]:
    ...
    if req.normalized == "openai-codex":
        ...
        # the suffix is a Nastech picker convention, so an unknown `*-900k` can never be a real
...
def _nastech_portal_recommended_names() -> set[str]:
    """Lower-cased ids from the Portal's live recommended-models feed (empty on any failure)."""
    from nastech_cli import models as _m

    try:
        payload = _m.fetch_nastech_recommended_models(_m._resolve_nastech_portal_url())
        return {
            name.lower()
...
def _validate_live_listing(req: _Request) -> Optional[dict[str, Any]]:
    ...
    # Nastech: the Portal's recommended-models feed can list a model before the curated list or the
    # docs-hosted manifest catches up; `nastech chat` already accepts those at model-list build
    # time, so mirror that source of truth for per-message /model validation.
    if req.normalized == "nastech" and req.lookup.lower() in _nastech_portal_recommended_names():
        return _accept_with_note(f"Note: `{req.requested}` was not found in the live /v1/models listing "
                                 "but is a current Nastech Portal recommendation — accepted.")
