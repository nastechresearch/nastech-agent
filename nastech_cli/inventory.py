...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (Nastech /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for Nastech, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``nastech model`` CLI picker. Adds network calls
      (pricing fetch + Nastech tier check); only set for interactive pickers.
    ...
    - ``force_fresh_nastech_tier``: bypass the short Nastech free-tier cache when
      selecting Portal-recommended Nastech models and applying tier gating. Keep
    ...
    try:
        from nastech_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
...
def build_aux_picker_rows(
    *,
    ...
    back. Mirrors the same filter in ``nastech_cli/moa_cmd.py``.
...
def _reasoning_catalog_reader(slug: str):
    ...
    if slug == "nastech":
        warm_nastech_reasoning_caps_async()
        return nastech_model_reasoning_capabilities
    if slug == "openrouter":
        warm_openrouter_reasoning_caps_async()
...
def _apply_featured(rows: list[dict]) -> None:
    ...
    Aggregator providers (nastech, openrouter) serve dozens of models across many
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`nastech_cli.providers.custom_provider_aliases` — lets the frontend
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `nastech model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _anthropic_oauth_credentials_present() -> bool:
    ...
    model.provider / API-key env vars: Nastech' own Anthropic device flow
    ...
    try:
        ...
        nastech_creds = read_nastech_oauth_credentials() or {}
        if nastech_creds.get("accessToken"):
            return True
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Nastech.
    """
    ...
    for row in rows:
        ...
        if slug == "anthropic" and _anthropic_oauth_credentials_present():
            # Anthropic OAuth logins (Nastech device flow / Claude Code) are
...
def _provider_is_keyless(slug: str) -> bool:
    """True when the provider's Nastech overlay declares it keyless."""
    try:
        from nastech_cli.providers import NASTECH_OVERLAYS
        overlay = NASTECH_OVERLAYS.get(slug)
...
def _apply_pricing(
    ...
    *,
    ...
    """Enrich each provider row with per-model pricing + Nastech tier gating.

    Mutates ``rows`` in-place. For every row whose provider supports live
    pricing (openrouter / nastech / novita) adds::

        row["pricing"] = {model_id: {"input": "$3.00", "output": "$15.00",
                                     "cache": "$0.30" | None, "free": bool}}

    For Nastech additionally adds::
    ...
    # Resolve Nastech free-tier once (cached in models.py for the TTL window).
    ...
    for row in rows:
        ...
        if slug == "nastech" and cached_only:
            ...
            if cached_nastech_tier is None:
                ...
                if not row.get("warning"):
                    row["warning"] = (
                        "Checking Nastech plan entitlement… models unlock on the "
        ...
        if not raw_pricing:
            if slug == "nastech":
                ...
                row["unavailable_models"] = (
                    list(models) if cached_nastech_tier else []
                )
            continue
        ...
        for mid in models:
            ...
            # Sale chrome is Nastech Portal-only. Other providers (OpenRouter,
        ...
        if slug == "nastech":
            try:
                if nastech_free_tier is None:
                    if cached_only:
                        nastech_free_tier = cached_nastech_tier
                    else:
...
def _prewarm_pricing_async(
    ...
    *,
    ...
    from nastech_cli.models import pricing_cache_scope

    profile_key = nastech_home_key()
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(
            ...
            args=(_worker,),
            name="nastech-picker-pricing-prewarm",
            daemon=True,
        )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`nastech_cli.model_switch.list_picker_providers`) so the
