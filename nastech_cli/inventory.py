...
def build_models_payload(
    ...
    capabilities: bool = False, featured: bool = False, force_fresh_nastech_tier: bool = False,
    ...
    rows = list_authenticated_providers(
        ...
        custom_providers=ctx.custom_providers, force_fresh_nastech_tier=force_fresh_nastech_tier,
...
def _reasoning_catalog_reader(slug: str):
    ...
    try:
        from nastech_cli.models_reasoning_caps import (
            nastech_model_reasoning_capabilities,
    ...
    readers = {
        "nastech": (warm_nastech_reasoning_caps_async, nastech_model_reasoning_capabilities),
...
def _apply_custom_aliases(rows: list[dict]) -> None:
    ...
    :func:`nastech_cli.providers.custom_provider_aliases` — lets the frontend do a membership check instead.
    """
    from nastech_cli.providers import custom_provider_aliases
...
def _append_unconfigured_rows(
    ...
    from nastech_cli.models import CANONICAL_PROVIDERS, _model_requires_account_discovery
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else f"Configured provider is not authenticated; run `nastech model` to reactivate. {tail}"
            )
...
def _anthropic_oauth_credentials_present() -> bool:
    """True when the user explicitly authenticated Anthropic via OAuth (Nastech device flow or Claude Code
    ...
    try:
        ...
        readers = (read_nastech_oauth_credentials, read_claude_code_credentials)
...
def _apply_pricing(rows: list[dict], *, force_fresh_nastech_tier: bool = False, cached_only: bool = False) -> None:
    """Set ``row["pricing"] = {model_id: {input, output, cache | None, free}}``; for Nastech also
    ``free_tier`` (account is free-tier) and ``unavailable_models`` (paid models a free user can't pick).
    ``cached_only`` never hits the network: unknown Nastech entitlement fails closed (``free_tier_pending``,
    ...
    from nastech_cli.models import (
        ...
        get_cached_nastech_free_tier,
        partition_nastech_models_by_tier,
    )

    nastech_free_tier: Optional[bool] = None  # resolved once (cached in models.py for the TTL window)
    ...
    for row in rows:
        ...
        try:
            ...
            raw_pricing = {}
        cached_nastech_tier: Optional[bool] = None
        if slug == "nastech" and cached_only:
            cached_nastech_tier = get_cached_nastech_free_tier()
            if cached_nastech_tier is None:
                ...
                if not row.get("warning"):  # say why every model renders locked
                    row["warning"] = ("Checking Nastech plan entitlement… models unlock on the "
                                      "next picker open or refresh.")
                continue
        if not raw_pricing:
            if slug == "nastech":
                row["free_tier"] = bool(cached_nastech_tier)
                row["pricing_pending"] = True
                row["unavailable_models"] = list(models) if cached_nastech_tier else []
            continue
        ...
        for mid in models:
            ...
            # Sale chrome is Nastech Portal-only (other catalogs' nested pricing.original is ignored); free
        ...
        if slug == "nastech":
            try:
                if nastech_free_tier is None:
                    nastech_free_tier = (cached_nastech_tier if cached_only
                                      else check_nastech_free_tier(force_fresh=force_fresh_nastech_tier))
                row["free_tier"] = bool(nastech_free_tier)
                row["unavailable_models"] = (
                    partition_nastech_models_by_tier(list(models), raw_pricing, free_tier=True)[1]
                    if nastech_free_tier else [])
...
def _local_runtime_row(ctx: "ConfigContext") -> dict | None:
    ...
    try:
        ...
        if not current:
            ...
            try:
                from nastech_cli.local_runtime.endpoint import _state_endpoint
...
def _prewarm_pricing_async(
    ...
    with _pricing_prewarm_lock:
        ...
        thread = Thread(target=copy_context().run, args=(_worker,),
                        name="nastech-picker-pricing-prewarm", daemon=True)
