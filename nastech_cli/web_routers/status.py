...
Extracted from ``nastech_cli.web_server``; app state and helpers are late-bound through
:mod:`nastech_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from nastech_cli.web_deps import LateState, late
from nastech_cli.web_server_gateway import _display_system_platform
...
from nastech_cli.config import get_config_path, get_env_path
from nastech_constants import get_process_nastech_home, profile_name_for_home
from nastech_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from nastech_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "nastech_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "nastech_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "nastech_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "nastech_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "nastech_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "nastech_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "nastech_cli.web_server_lifecycle")
_spawn_nastech_action = late("_spawn_nastech_action", "nastech_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "nastech_cli.config")
get_nastech_home = late("get_nastech_home", "nastech_cli.config")
...
load_config = late("load_config", "nastech_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "nastech_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Nastech process uses it."""
...
async def get_ssh_ownership(request: Request):
    from nastech_cli.web_server import _SSH_OWNER_NONCE
...
async def get_health_idle(request: Request):
    ...
    from nastech_cli.web_server_idle_proof import idle_proof
    _require_token(request)
...
# Profile segment mirrors nastech_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level NASTECH_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``nastech --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_nastech_home())
    ...
    if runtime:
        ...
        if not gateway_running:
            ...
            # as ``nastech gateway status`` so the sidebar strip and the CLI agree.
...
def _nastech_session_validity() -> str:
    """Nastech bootstrap-session validity for the NAS health sweep: a hosted agent whose Nastech
    ...
    try:
        ...
        return get_nastech_session_validity()
    except Exception:
        return "unknown"
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from nastech_cli.web_server import DASHBOARD_HEALTH
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from nastech_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of nastech_cli.gateway blocks
        ...
        status = {
            ...
            **auth, "nastech_session_valid": _nastech_session_validity()}
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_nastech_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Nastech Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "nastech_home": str(get_nastech_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond nastech home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "nastech_version": __version__, "cpu_count": os.cpu_count()}
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``nastech <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_nastech_action(argv, name)
    except Exception as exc:
...
# Portal — Nastech Portal auth + Tool Gateway routing status (read-only).
