...
Extracted from ``nastech_cli.web_server``; helpers/state that tests monkeypatch on
...
_profile_scope = late("_profile_scope", "nastech_cli.web_server_profiles")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "nastech_cli.web_server_profiles")
_OAUTH_PROVIDER_CATALOG = LateState("_OAUTH_PROVIDER_CATALOG", "nastech_cli.web_server_oauth")
...
_nastech_plain_poller = late("_nastech_plain_poller", "nastech_cli.web_server_oauth")
_nastech_promotion_poller = late("_nastech_promotion_poller", "nastech_cli.web_server_oauth")
_minimax_poller = late("_minimax_poller", "nastech_cli.web_server_oauth")
_xai_device_poller = late("_xai_device_poller", "nastech_cli.web_server_oauth")
...
def _codex_post(httpx, url: str, **kwargs: Any) -> Any:
    ...
    """
    from nastech_cli.auth_codex import _is_transient_transport_error
...
def _codex_request_user_code(httpx) -> Dict[str, Any]:
    ...
    from nastech_cli.auth import CODEX_OAUTH_CLIENT_ID
...
def _codex_exchange_tokens(httpx, code_resp: Dict[str, Any]) -> Dict[str, str]:
    ...
    from nastech_cli.auth import CODEX_OAUTH_CLIENT_ID, CODEX_OAUTH_TOKEN_URL
...
# nastech: refresh-free local snapshot so listing providers never performs an OAuth
...
_PROVIDER_STATUS: Dict[str, tuple[str, Callable[[dict], dict]]] = {
    "nastech": ("get_nastech_auth_status_local", lambda r: {**_status_card(
        r, "nastech_portal", r.get("portal_base_url") or "Nastech Portal",
...
async def _start_nastech_device_code(profile: Optional[str]) -> Dict[str, Any]:
    """Start a Nastech sign-in. Over a free-tier identity (``nastech.guest`` on) the whole sign-in is the
    ...
    from nastech_cli.auth import PROVIDER_REGISTRY, _request_device_code
    from nastech_cli.web_server_profiles import _config_profile_scope, _profile_scope
    pconfig = PROVIDER_REGISTRY["nastech"]
    portal_base_url = (
        os.getenv("NASTECH_PORTAL_BASE_URL") or os.getenv("NASTECH_PORTAL_BASE_URL") or pconfig.portal_base_url
    ).rstrip("/")
    with _profile_scope(_oauth_profile_name(profile)):
        guest = anon_auth.current_nastech_state() if anon_auth.guest_enabled() else None
    ...
    if not anon_auth.is_guest_state(guest):
        ...
        return _device_session_started(
            "nastech", profile, _nastech_plain_poller, fields, str(device_data["user_code"]),
    ...
    _start_poller(_nastech_promotion_poller, sid)   # last: nothing observes `sess` before it is complete
    return {
...
_DEVICE_CODE_STARTERS = {
    "nastech": _start_nastech_device_code, "openai-codex": _start_codex_device_code,
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    # "anthropic" is flow == "external" (no in-dashboard login) but Nastech still
    # OWNS its credential (the PKCE file ~/.nastech/.anthropic_oauth.json and its
    # credential-pool entry, written by `nastech auth add anthropic`), so it is
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    in ``nastech model`` order, so plugin-added OAuth/external providers appear automatically."""
    ...
    try:
        ...
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "cli_command": f"nastech auth add {d.slug}", "docs_url": d.signup_url or "", "status_fn": None,
            })
    except Exception:
        pass
    return rows
...
def _clear_anthropic_auth() -> bool:
    """Clear only the Nastech-managed PKCE file and auth-store entry (never ~/.claude/*)."""
    cleared = False
    try:
        from agent.anthropic_credentials import _get_nastech_oauth_file
...
async def poll_oauth_session(provider_id: str, session_id: str, profile: Optional[str] = None):
    ...
    return {
        ...
        # Nastech over a free-tier identity: why a transfer ended, who signed in, and the default model
