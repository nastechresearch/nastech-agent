...
from nastech_cli.pty_session import RegistryFull
from nastech_cli.web_deps import LateState, late
from nastech_cli.web_routers.chat_ws_errors import chat_start_failure_message
from nastech_cli.web_server_chat import (
...
_active_session_file_for_channel = late("_active_session_file_for_channel", "nastech_cli.web_server_chat")
_profile_scope = late("_profile_scope", "nastech_cli.web_server_profiles")
_resolve_chat_argv_async = late("_resolve_chat_argv_async", "nastech_cli.web_server_chat")
_resolve_profile_dir = late("_resolve_profile_dir", "nastech_cli.web_server_profiles")
_ws_auth_reason = late("_ws_auth_reason", "nastech_cli.web_server_chat")
_ws_client_reason = late("_ws_client_reason", "nastech_cli.web_server_chat")
_ws_host_origin_reason = late("_ws_host_origin_reason", "nastech_cli.web_server_chat")
...
def _ws_auth_mode() -> str:
    ...
    from nastech_cli.web_server_chat import _LOOPBACK_HOSTS
        ...
        try:
            ...
            if command_id == command_generation:
                ...
                await out.error_then_complete(
                    "Command timed out. Nastech Console returned to the prompt.", line, command_id, "timeout",
                )
        except Exception as exc:
            if command_id == command_generation:
                pending_confirmation = None
...
async def pty_ws(ws: WebSocket) -> None:
    from nastech_cli.web_server_chat import PTY_REGISTRY, PtyBridge, PtyUnavailableError, _PTY_BRIDGE_AVAILABLE, _RESIZE_RE
...
# (NASTECH_TUI_SIDECAR_URL from /api/pty's env) and writes every dispatcher emit
