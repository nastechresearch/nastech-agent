...
late-binding seam in :mod:`nastech_cli.web_deps` so tests that
...
def get_profiles_projects_tree(preview_limit: int = 3, session_limit: int = 2000):
    ...
    the repo-scan policy, the NASTECH_HOME junk filters — through the
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's NASTECH_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    def _run():
        ...
        # NASTECH_HOME against the profiles root. Batched into one hop so the
...
# ── Export / Import ──────────────────────────────────────────────────────────
# Profile sharing for the desktop: wraps nastech_cli.profiles.export_profile /
# import_profile (the same machinery behind `nastech profile export|import`).
