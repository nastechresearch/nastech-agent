...
Shared helpers are reached via the late-binding seam in :mod:`nastech_cli.web_deps`
...
import copy
import functools
from nastech_cli.web_read_coalescing import coalesced_read
import inspect
import json
import logging
import re
...
from nastech_cli.config import get_process_nastech_home
from nastech_cli.profiles import ProfileIdentitySettlementPending
from nastech_cli.web_server_config import (
    _apply_main_model_assignment, _normalize_main_model_assignment, _validated_main_model_selection,
)
from nastech_cli.web_server_gateway import _strip_session_list_rows
from nastech_cli.web_routers._common import _CONFIG_MUTATION_LOCK
from nastech_cli.web_server_profiles import (
...
from nastech_cli.web_models import (
    ...
    SessionPrScanBody)
from nastech_cli.web_server_profiles import _config_profile_scope, _nastech_home_scope
...
_cron_profile_home = late("_cron_profile_home", "nastech_cli.web_server_cron")
...
def _scope_profile_name(path: Path) -> Optional[str]:
    ...
    ``"default"`` for the default root (its basename -- ``.nastech`` or a custom root -- is not a
    ...
    if resolved == get_process_nastech_home().resolve():
        return None
    if resolved == get_default_nastech_root().resolve():
        return "default"
    return path.name


def _write_profile_model(profile_dir: Path, provider: str, model: str, validate_in: Optional[Path] = None) -> None:
    """Write the main model assignment into ``profile_dir``'s config.yaml (NASTECH_HOME-scoped)
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    with _nastech_home_scope(profile_dir), _CONFIG_MUTATION_LOCK:  # RMW span
...
def _profile_errors(log_msg: str, *args, not_found=(FileNotFoundError,),
    ...
    """Map nastech_cli.profiles exceptions to HTTP: ``not_found`` -> 404, ``bad_request`` -> 400
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    def _set_model() -> bool:
        ...
        try:
            _write_profile_model(path, provider, model, validate_in=get_process_nastech_home())
    ...
    def _spawn_install(ident: str):
        return _spawn_nastech_action(["-p", body.name, "skills", "install", ident, "--yes"],
...
async def get_active_profile_endpoint():
    """``active`` is the sticky default written by ``nastech profile use`` (what new CLI
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``nastech profile use``); does not retarget the
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    own profile — ``POST /api/model/set`` (main scope) via the NASTECH_HOME override."""
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    """Auto-generate a profile's description via the auxiliary LLM (mirrors ``nastech profile
...
# ── Export / Import ── wraps nastech_cli.profiles.export_profile / import_profile. Paths are
