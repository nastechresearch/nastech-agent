...
from nastech_cli.web_models import ModelAssignment, MoaConfigPayload, MoaModelSlot
from nastech_cli.web_routers._common import _CONFIG_MUTATION_LOCK, config_write_scope, http_failure
...
def _nastech_recommended_default() -> dict:
    from nastech_cli.models import recommended_nastech_default_model
    return recommended_nastech_default_model()
...
def get_recommended_default_model(provider: str = ""):
    ...
    ``nastech model``'s curation so GUI onboarding lands on a sensible default.
    Nastech honors the user's free/paid tier. Any other provider gets the preferred
    ...
    Nastech; ``model`` may be empty (caller degrades gracefully)."""
    slug = (provider or "").strip().lower()

    if slug == "nastech":
        try:
            return _nastech_recommended_default()
        except Exception:
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    ``~/.nastech/config.yaml`` — applies to **new** sessions only; a running chat
