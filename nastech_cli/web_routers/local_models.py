...
def _variant_files_on_disk(model_id: str) -> "list[Path]":
    ...
    from nastech_cli.local_runtime.bootstrap import assets_dir
    from nastech_cli.local_runtime.catalog import find_entry_for_model
...
def _models_dir() -> Path:
    from nastech_cli.local_runtime.bootstrap import models_dir
...
def _engine_too_old(min_engine: str) -> bool:
    ...
    if not min_engine:
        return False
    try:
        from nastech_cli.local_runtime.binaries import default_tag, installed_tags
...
def local_models_status():
    ...
    # the Nastech-release default — is newer than anything on disk. The
    ...
    mdir = _models_dir()
    if mdir.exists():
        from nastech_cli.local_runtime.bootstrap import staged_models
...
def _loading_progress() -> Dict[str, Any]:
    try:
        from nastech_cli.local_runtime.load_progress import get_loading_progress
...
def local_models_hardware():
    ...
    from nastech_cli.local_runtime.hardware import probe_budget, _nvidia_vram, _ram_bytes
    ...
    try:
        ...
        from nastech_cli.local_runtime.hardware import _nvidia_smi_path
...
def local_models_catalog():
    ...
    from nastech_cli.local_runtime.catalog import (
        CATALOG,
        recommended_entry,
        refresh_catalog_soon,
        select_variant,
    )
    from nastech_cli.local_runtime.context_policy import (
        RUNTIME_OVERHEAD_BYTES,
        initial_window,
        ub_logits_bytes,
    )
    from nastech_cli.local_runtime.estimator import PhysicsRefusal
...
async def local_models_runtime_install(body: RuntimeInstallBody):
    ...
    from nastech_cli.local_runtime.bootstrap import _detect_gpu_vendor
    ...
    def _run():
        try:
            ...
            restarted = False
            try:
                from nastech_cli.local_runtime.bootstrap import (
...
async def local_models_download(body: ModelDownloadBody):
    ...
    from nastech_cli.local_runtime.bootstrap import assets_dir, staged_model_ids
    ...
    def _run():
        try:
            ...
            try:
                from nastech_cli.local_runtime.bootstrap import refresh_local_runtime
...
async def local_models_delete(model_id: str):
    ...
    try:
        from nastech_cli.local_runtime.growth import clear_window_override
...
async def local_models_quickstart(body: QuickstartBody):
    ...
    def _run():
        try:
            if need_runtime:
                from nastech_cli.local_runtime.binaries import ensure_runtime_installed
...
def local_models_eject(body: ModelEjectBody):
    ...
    from nastech_cli.local_runtime.bootstrap import get_supervisor
...
async def local_models_search(q: str, limit: int = 20):
    ...
    from nastech_cli.local_runtime.hf_browse import search_models
...
async def local_models_search_files(repo: str):
    ...
    from nastech_cli.local_runtime.hardware import probe_budget
    from nastech_cli.local_runtime.hf_browse import priced_repo_files
...
async def local_models_sideload(body: SideloadBody):
    ...
    it is; delete-from-Nastech removes only our link."""
    import os
    import shutil
