...
late-binding seam in :mod:`nastech_cli.web_deps` (``late`` for callables,
...
from nastech_cli.web_deps import late, LateState
...
_spawn_nastech_action = late("_spawn_nastech_action")
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `nastech tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `nastech computer-use install`
            ...
            if cat:
                ...
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # including logged-out Nastech Subscription rows and never-run
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `nastech tools` runs after a
...
async def select_toolset_provider(
    ...
    Managed Nastech rows (``managed_nastech_feature``) additionally report the
    Portal entitlement state: the CLI flow gates these selections on
    ``ensure_nastech_portal_access`` (inline login), but the GUI has no inline
    ...
    ``managed_by_nastech``). The response now carries an additive
    ``needs_nastech_auth: true`` + ``feature`` so the client can drive the
    existing Nastech Portal OAuth flow (``POST /api/providers/oauth/nastech/start``)
    and refetch.
    """
    from nastech_cli.tools_config import (
        TOOL_CATEGORIES,
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Nastech rows — mirrors the gate the CLI
            # applies via ensure_nastech_portal_access at selection time. This hits
...
async def run_toolset_post_setup(
    ...
    ``profile`` spawns the hook as ``nastech -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same NASTECH_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``nastech computer-use permissions grant`` as a background action.
