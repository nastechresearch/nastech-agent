...
from nastech_cli.web_routers._common import (
    _CONFIG_MUTATION_LOCK, _profile_cli_args, _profile_scope, _spawn_nastech_action,
...
def _terminal_backend_rows() -> List[Dict[str, str]]:
    ...
    from nastech_cli.web_server_profiles import _TERMINAL_BACKENDS
...
def _probe_docker_backend(_cfg) -> tuple:
    ...
    ``find_docker()`` honors ``NASTECH_DOCKER_BINARY``, then ``docker`` / ``podman``
...
def _toolset_model_catalog(ts_key: str, plugin_name: str, config: dict):
    ...
    ``imagegen_backend`` (``fal``, the managed ``nastech`` union), that backend's catalog."""
    from nastech_cli.tools_config import IMAGEGEN_BACKENDS, _plugin_image_gen_catalog, _plugin_video_gen_catalog
...
def _category_providers(ts_key: str, config: dict) -> list:
    ...
    from nastech_cli.tools_config import TOOL_CATEGORIES, _visible_providers
...
async def get_toolsets(profile: Optional[str] = None):
    from nastech_cli.tools_config import (
        ...
        _toolset_configuration_platform, _toolset_has_keys, get_nastech_subscription_features,
        gui_toolset_label)
    from nastech_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # install `nastech tools` runs — otherwise the toggle "saves" but the tool
    ...
    def _read():
        with _profile_scope(profile):
            ...
            if cat:
                ...
                features = get_nastech_subscription_features(config, force_fresh=True)
                for prov in _visible_providers(cat, config, force_fresh=True):
                    ...
                    row = {
                        ...
                        # automatically ready (logged-out Nastech rows, never-run
                        # post_setup installs).
...
async def select_toolset_provider(
    ...
    ``nastech tools``, so both write identical keys).

    ``web`` only: ``capability`` ('search' | 'extract') writes
    ``web.<capability>_backend`` (the override the dispatchers resolve first);
    omitted -> legacy ``web.backend``.  Managed Nastech rows report Portal
    entitlement (``needs_nastech_auth`` + ``feature``): the GUI has no inline
    login, so an unentitled selection would write config and never activate.
    """
    from nastech_cli.tools_config import apply_provider_selection, web_provider_capabilities
    from nastech_cli.nastech_subscription import (
        MANAGED_FEATURE_COVERAGE_CATEGORY, get_nastech_subscription_features)
    ...
    def _run():
        with _profile_scope(body.profile or profile):
            ...
            # Entitlement check for managed Nastech rows (mirrors the CLI's
            # ensure_nastech_portal_access gate).  Hits the Portal, so it runs AFTER
            # releasing the mutation lock — still in the worker thread + scope.
            row = _provider_row(config)
            managed_feature = (row or {}).get("managed_nastech_feature")
            if managed_feature:
                features = get_nastech_subscription_features(config, force_fresh=True)
                ...
                if not entitled:
                    response["needs_nastech_auth"] = True
...
async def run_toolset_post_setup(
    ...
    """Spawn ``nastech tools post-setup <key>`` (long-running installs) as a
    background action tailed via ``GET /api/actions/tools-post-setup/status``;
    ``profile`` is threaded so hooks see the drawer's NASTECH_HOME."""
    from nastech_cli.tools_config import valid_post_setup_keys
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``nastech computer-use permissions grant`` (macOS-only: launches
...
_PLUGIN_COMPAT_LAZY = {
    'LateState': ('nastech_cli.web_deps', 'LateState'),
}
