"""Gateway restart/drain, Nastech update and background-action status dashboard routes.
...
from nastech_cli.config import format_docker_update_message, recommended_update_command_for_method
from nastech_cli.web_deps import LateState, late
from nastech_cli.web_server_gateway import _ACTION_LOG_FILES
from nastech_cli.web_routers._common import http_failure
...
detect_install_method = late("detect_install_method", "nastech_cli.config")
get_nastech_home = late("get_nastech_home", "nastech_cli.config")
_ACTION_COMMANDS = LateState("_ACTION_COMMANDS", "nastech_cli.web_server_gateway")
_ACTION_IDS = LateState("_ACTION_IDS", "nastech_cli.web_server_gateway")
_ACTION_PROCS = LateState("_ACTION_PROCS", "nastech_cli.web_server_gateway")
_ACTION_RESULTS = LateState("_ACTION_RESULTS", "nastech_cli.web_server_gateway")
...
def _server_path(name: str) -> Path:
    ...
    import nastech_cli.web_server_gateway as gw
...
_UPDATE_ACTION_COMPLETED_RE = re.compile(r"^=== nastech-update completed ([0-9a-f]{32}) ===$")

_MANAGED_EXTERNALLY_MESSAGE = "Nastech updates are managed outside this dashboard in containerized environments."
...
async def gateway_migrate_plan():
    ...
    from nastech_cli.gateway_migrate import build_migration_plan
...
async def gateway_migrate():
    """Run ``nastech gateway migrate --multiplex --yes`` detached; the CLI re-runs the preflight and
    refuses (exit 1 into the action log) when blocked, so the UI should gate on the plan first."""
    from nastech_cli.web_server_gateway import _spawn_nastech_action
    with http_failure("Failed to spawn gateway migrate", 500, "Failed to start gateway migration"):
        proc = _spawn_nastech_action(["gateway", "migrate", "--multiplex", "--yes"], "gateway-migrate")
...
def _update_refused(error: str, message: str, update_command: str) -> Dict[str, Any]:
    ...
    return {
        "ok": False, "pid": None, "name": "nastech-update", "error": error, "message": message,
        "update_command": update_command,
    }


@router.post("/api/nastech/update")
async def update_nastech():
    ...
    if existing is not None and existing.poll() is None:
        response = {"ok": True, "pid": existing.pid, "name": "nastech-update", "already_running": True}
    ...
    with http_failure("Failed to spawn nastech update", 500, "Failed to start update"):
        proc = _spawn_nastech_action(["update"], "nastech-update", env_overrides={"NASTECH_ACTION_ID": action_id})
    return {"ok": True, "pid": proc.pid, "name": "nastech-update", "action_id": action_id}


_NON_APPLYABLE_MESSAGES = {
    "docker": format_docker_update_message,
    "apt": lambda: "Nastech is managed by Termux APT; run `pkg upgrade nastech-agent`.",
}


@router.get("/api/nastech/update/check")
async def check_nastech_update(force: bool = False):
    ...
    try:
        from nastech_cli.banner import check_for_updates, upstream_commits_behind
...
def _latest_update_receipt_summary() -> Optional[Dict[str, Any]]:
    """Compact summary of the latest receipt (written by EVERY ``nastech update`` run,
    incl. refused/failed), or None; never raises. Steps/skips stay in the full endpoint.

    Phase-1 bullet 3 (#91277): the receipt (written by EVERY ``nastech update`` run since #91283, including
...
@status_router.get("/api/nastech/update/receipt")
async def get_update_receipt():
    ...
    receipt = _read_latest_receipt()
    if not receipt:
        raise HTTPException(status_code=404, detail="No update receipt found (no `nastech update` run recorded).")
