...
from nastech_cli.colors import Colors
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `nastech mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: nastech mcp install <name>    Picker: nastech mcp", Colors.DIM)
    # Manifest-version warnings: the user's Nastech is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Nastech — run `nastech update` to install this entry.", Colors.YELLOW)
        print()
    print()
...
def run_picker() -> None:
    """`nastech mcp picker` (and default `nastech mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    ...
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `nastech mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
...
_PLUGIN_COMPAT_LAZY = {
    'color': ('nastech_cli.colors', 'color'),
}
