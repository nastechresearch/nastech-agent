...
# Set by an orchestrating updater (Tauri `nastech-setup --update`) to its own pid before
# spawning `nastech update` as a child stage; the parent holds the marker for its whole run,
...
# Tauri updater (UPDATE_EXIT_CONCURRENT in update.rs) to show "Nastech is still running".
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Nastech home (never the context-local profile override): the Rust
    updater resolves ``$NASTECH_HOME`` or the platform default and the desktop pins that same
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``nastech update`` as a (grand)child, so a live marker
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Nastech update is already running (started {elapsed} ago, "
        f"process {holder.pid}).\n"
        "\n"
        "  Running two at once would corrupt the install. Wait for it to finish\n"
        "  (watch `nastech logs`), or close the Desktop/dashboard window that\n"
        "  started it, then run `nastech update` again."
    )
