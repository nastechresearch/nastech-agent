"""Host-platform checks for nastech doctor: interpreter, SQLite, certificates, macOS TCC, gateway supervision, command install.
Split out of ``nastech_cli/doctor.py``, which re-exports every name so ``nastech_cli.doctor.<name>`` keeps resolving (and monkeypatching)."""
...
from nastech_cli.config import is_nix_install_method, recommended_update_command_for_method
...
def _sqlite_upgrade_hint(install_method: str | None = None) -> str:
    ...
    from nastech_cli.doctor import PROJECT_ROOT
    ...
    action = cmd if is_nix_install_method(method) else {  # nix: prose guidance, not a shell command
        "docker": f"run `{cmd}`, then recreate all Nastech containers", "apt": f"run `{cmd}`"}.get(method, "run `nastech update`")
    return f"({action}; fixed versions: 3.51.3+ / 3.50.7 / 3.44.6 — see https://sqlite.org/wal.html#walresetbug)"


def _nastech_database_paths(nastech_home: Path) -> list[tuple[str, Path]]:
    """(display name, path) pairs for Nastech-managed SQLite databases: backup.py's per-profile store list + per-board kanban.db."""
    from nastech_cli.backup import _QUICK_STATE_FILES
    entries = [(name, nastech_home / name) for name in _QUICK_STATE_FILES if name.endswith(".db")]
    for board_db in sorted((nastech_home / "kanban" / "boards").glob("*/kanban.db")):
        entries.append((str(board_db.relative_to(nastech_home)), board_db))
    return entries
...
def _read_journal_mode(db_path: Path) -> tuple[str | None, str | None]:
    ...
    process's POSIX advisory locks (see ``nastech_cli.sqlite_safe_read``), and the dashboard console runs
...
def _report_database_holders(name: str, db_path: Path) -> None:
    ...
    from nastech_state_holders import _read_proc_argv, foreign_state_db_holders, psutil
...
def _report_database_journal_modes(nastech_home: Path | None = None, version_info: tuple[int, ...] | None = None) -> None:
    ...
    from nastech_cli.doctor import NASTECH_HOME
    ...
    try:
        databases = _nastech_database_paths(nastech_home if nastech_home is not None else NASTECH_HOME)
    except Exception as exc:
        check_warn(f"Could not list Nastech databases: {exc}")
        return
    exposed = []
    for name, path in databases:
        ...
        if error is None and mode == "wal" and configured == "delete":
            ...
            check_warn(f"{name} is in WAL mode ({size}) despite database.journal_mode=delete",
                       ...
                       + ". Stop every Nastech process for this profile, then run a one-time offline "
            ...
            check_warn(f"{name} is in WAL mode on a cross-VM filesystem (virtiofs/9p, {size})",
                       "(WAL can silently corrupt across the VM boundary; stop every Nastech process and run a one-time "
...
def _check_version_consistency(issues: list[str]) -> None:
    """Detect pyproject.toml vs nastech_cli.__version__ drift (a conflict resolution can revert one but not the
    ...
    _fail_and_issue("Version mismatch between source files", f"(pyproject.toml {pyproject_version} != nastech_cli/__init__.py {init_version})",
                    "Re-sync version files (e.g. run 'nastech update', or set nastech_cli/__init__.py __version__ to match pyproject.toml)", issues)
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    try:
        from nastech_cli.service_manager import S6ServiceManager, detect_service_manager
    except Exception:
        return
    ...
    for static in ("main-nastech", "dashboard"):  # s6-rc symlinks under /run/service/, same s6-svstat probe
    ...
    if not profiles:
        return check_info("No per-profile gateways registered yet — create one with `nastech profile create <name>`")
...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    ...
    if not should_fix:
        issues.append(f"Repair the CA bundle: run `nastech doctor --fix`, or `{pip_cmd}`")
        return
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    try:
        from nastech_cli.gateway import get_systemd_linger_status, get_systemd_unit_path, is_linux
...
_TCC_CDHASH_DETAIL = (
    ...
    "all permission grants. Run `nastech update` to get the stable identifier-pinned signing identity, "
    "then re-grant permissions once.")
_TCC_STABLE_DETAIL = {
    True: "(certificate-anchored DR; grants survive rebuilds)",
    False: "(identifier-pinned DR; grants survive rebuilds — for the strongest anchor, see `nastech desktop --setup-tcc-identity`)",
}
...
def check_macos_tcc_grants() -> None:
    ...
    check_info("If macOS still re-prompts for permissions (toggle shows ON): the stored grant is stale — run "
               "`tccutil reset ScreenCapture com.nastechresearch.nastech` (repeat per affected service), toggle it ON in "
               "System Settings, then fully quit & relaunch Nastech once.")


def _desktop_app_bundle() -> Path | None:
    """Locate the locally-built desktop bundle (``apps/desktop/release/mac-<arch>/Nastech.app``), newest first.

    The only layout whose ad-hoc re-signed bundle can invalidate TCC grants. ``/Applications/Nastech.app`` is
    deliberately not probed: it is the separately-signed, certificate-anchored Nastech-Setup launcher.
    """
    release_dir = Path(__file__).resolve().parents[1] / "apps" / "desktop" / "release"
    candidates = [p for p in release_dir.glob("mac*/Nastech.app") if p.is_dir()]
...
def check_macos_tcc_anchor(should_fix: bool = False) -> None:
    ...
    with warn_on_error("macOS TCC anchor check failed"):
        from nastech_cli import macos_tcc_anchor as tcc
...
def check_macos_full_disk_access() -> None:
    ...
    try:
        ...
        check_info("One switch silences all macOS folder prompts: grant your terminal app Full Disk Access and Nastech "
                   ...
                   "    then enable your terminal (and Nastech.app if you use Desktop), and restart them once. "
                   "With Nastech' stable signing identities the grant survives every update.")
    except OSError:
...
def _check_security_advisories(should_fix: bool, f: Finding) -> None:
    ...
    from nastech_cli.security_advisories import detect_compromised, filter_unacked, full_remediation_text, get_acked_ids
    ...
    for hit in fresh_hits:
        ...
        _fail_and_issue(f"{hit.advisory.title}", f"({hit.package}=={hit.installed_version})",
                        ...
                        f"and rotate credentials, then run `nastech doctor --ack {hit.advisory.id}`.", f.manual_issues)
...
def _check_python_environment(should_fix: bool, f: Finding) -> None:
    ...
    with warn_on_error("SQLite version probe failed: {e}", ""):
        import sqlite3
        from nastech_state_wal import is_sqlite_wal_reset_vulnerable, sqlite_source_id
        src = sqlite_source_id()
        # Warn-only: Nastech already refuses WAL on fresh DBs and runtime repair is best-effort.
...
def _check_command_installation(should_fix: bool, f: Finding) -> None:
    ...
    venv_bin = next((c for c in (PROJECT_ROOT / n / "bin" / "nastech" for n in ("venv", ".venv")) if c.exists()), None)
    if venv_bin is None:
        check_warn("Venv entry point not found", "(nastech not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')")
    ...
    link = link_dir / "nastech"
    if link.is_symlink():
        target, expected = link.resolve(), venv_bin.resolve()
        if target == expected:
            return check_ok(f"{display}/nastech → correct target")
        check_warn(f"{display}/nastech points to wrong target", f"(→ {target}, expected → {expected})")
        if not should_fix:
            return f.issues.append(f"Broken symlink at {display}/nastech — run 'nastech doctor --fix'")
        link.unlink()
        verb = "Fixed"
    elif link.exists():  # regular file (wrapper script), not a symlink
        return check_ok(f"{display}/nastech exists (non-symlink)")
    else:
        check_fail(f"{display}/nastech not found", "(nastech command may not work outside the venv)")
        if not should_fix:
            return f.issues.append(f"Missing {display}/nastech symlink — run 'nastech doctor --fix'")
        link_dir.mkdir(parents=True, exist_ok=True)
        verb = "Created"
    link.symlink_to(venv_bin)
    check_ok(f"{verb} symlink: {display}/nastech → {venv_bin}")
    f.fixed += 1
