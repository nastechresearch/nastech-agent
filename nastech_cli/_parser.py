...
gateway, sessions, …) is built by ``nastech_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``NASTECH_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/nastech_cli/
...
def _cfg_path() -> str:
    """``~/.nastech/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_nastech_home()`` would emit the
    "[NASTECH_HOME fallback] ... wrong profile" warning on every ``nastech`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from nastech_constants import display_nastech_home, get_process_nastech_home

    return f"{display_nastech_home(get_process_nastech_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``nastech --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``nastech_cli.relaunch``."""
...
Examples:
    ...
    nastech --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    ...
    nastech auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    nastech auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    nastech auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``nastech -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/nastech_cli/
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Nastech itself.")
...
class NastechArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "nastech" at the top level and "nastech gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = NastechArgumentParser(
        prog="nastech", description="Nastech Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``nastech [...] <command>`` instead of the brace list of
    # every subcommand name; ``nastech --help`` still lists each command with its help row.
