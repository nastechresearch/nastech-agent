...
def _browser_available() -> bool:
    ...
    try:
        ...
        if (PROJECT_ROOT / "node_modules" / "agent-browser").exists():
            return True
        for candidate in (NASTECH_HOME / "node" / "bin",
                          NASTECH_HOME / "node",
                          NASTECH_HOME / "node_modules" / ".bin"):
    ...
    # nastech_cli.doctor uses so this probe can't diverge from it, including
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``nastech mcp test``.
    """
