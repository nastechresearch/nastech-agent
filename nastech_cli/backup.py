...
# Where ``nastech backup --quick`` / ``/snapshot`` / the pre-update safety net
...
# ``nastech-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/nastech-agent/``
...
# NASTECH_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    "browser-profile",
    # Python dependency trees (plugin / MCP-server venvs under NASTECH_HOME) —
...
# Nastech-managed runtime downloads that only exist at the top of a profile
...
# ONLY at the root of NASTECH_HOME and at ``profiles/<name>/`` — a deeper
...
def _in_excluded_root_dir(rel_path: Path) -> bool:
    """True when *rel_path* (relative to NASTECH_HOME) is, or sits inside, a
    Nastech-managed runtime tree at the top of a profile home."""
...
# ``state.db.pre-update-emergency-<timestamp>.bak`` at the NASTECH_HOME root
...
# File names that ``nastech import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Nastech portal
...
# Reserved archive subtree for provider state that lives OUTSIDE NASTECH_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside NASTECH_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``nastech-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/nastech-agent/`` — must be preserved.
        if part == "nastech-agent" and part != parts[0]:
            continue
        return True
...
def _iter_backup_files(
    ...
    never descends a multi-GB excluded tree), the root-only ``nastech-agent``
    carve-out, profile-home-root runtime trees, and the per-file exclusion
    rules — shared by the manual ``nastech backup`` path and the automatic
    ...
    for dirpath, dirnames, filenames in os.walk(nastech_root, followlinks=False):
        ...
        # ``nastech-agent`` is only pruned at the root level; nested dirs
...
# to the user as a hung `nastech update` (#70553 follow-up). Sessions databases
...
def verify_sqlite_integrity(
    ...
    *,
    ...
    if check_header:
        ...
        # nastech_cli.sqlite_safe_read). Verification targets snapshots and
    ...
    try:
        ...
        try:
            ...
            logger.error(
                ...
                "database handles (or restart Nastech) and retry.",
                dst, exc2,
            )
            return False
...
def _run_backup_locked(args, nastech_root: Path) -> None:
    ...
    files_to_add: list[tuple[Path, Path]] = list(
        _iter_backup_files(nastech_root, out_path, skipped_dirs)
    )

    # External memory-provider state (e.g. ~/.honcho, ~/.hindsight) lives
    # outside NASTECH_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_nastech_home()}."
        )
...
def _extract_member_atomically(
    ...
    with nothing behind it — during ``nastech import``, which is the
    ...
    so without the chown a ``sudo nastech import`` would silently re-own every
    ...
    if mode is None:
        ...
        # take over an existing setuid/setgid file, so ``nastech import`` would
        # hand whoever produced the zip the identity that file runs as.  Nothing
        # constrains that to Nastech' own state either: the ``_external/`` branch
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    database that is not a Nastech session store, or one that cannot be read.
...
def _import_db_member(
    ...
    ``nastech import`` is the disaster-recovery path, so that failure mode lands
...
def run_import(args) -> None:
    ...
    # same path printed as "Target:" via display_nastech_home(). Resolving
    # through get_default_nastech_root() instead maps a profile home
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under NASTECH_HOME.
            ...
            # Nastech portal. Matched by basename so both the root profile and
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: nastech profile list  (after installing nastech)")
        ...
        if nastech_root != native_default and default_has_install:
            print(
                "\nRestored into a non-default home; leaving the gateway service "
                "alone to avoid clashing with the install at "
                f"{native_default}."
            )
            print("To start a gateway for this home, run:  nastech gateway install")
        else:
            try:
...
_QUICK_STATE_FILES = (
    ...
    # are at $NASTECH_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside NASTECH_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: nastech snapshot list"
        )
        logger.error(
...
def restore_config_model_settings_if_rewritten(
    ...
    Args:
        snapshot_id: The pre-update quick-snapshot id (from
            :func:`create_quick_snapshot`).
        nastech_home: Override for the Nastech home directory (tests/siblings).
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        try:
            result = restore_config_model_settings_if_rewritten(
                snap_id, nastech_home=profile_home
            )
        except Exception as exc:
            logger.debug(
...
def restore_cron_jobs_all_profiles(
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_cron_jobs_if_emptied(snap_id, nastech_home=profile_home)
        except Exception as exc:
