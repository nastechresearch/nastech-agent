...
from nastech_constants import (
    LOCAL_RUNTIME_ROOT_DIRS, _get_platform_default_nastech_home, get_default_nastech_root, get_nastech_home,
    display_nastech_home,
)
from nastech_state_dbfile import RETIRED_GENERATION_DIR_SUFFIX
from utils import (
...
# Where ``nastech backup --quick`` / ``/snapshot`` / the pre-update safety net write state
...
def _snapshot_recovery_hint() -> str:
    """How to restore a state snapshot. There is no `nastech snapshot` subcommand — only the /snapshot
    slash command inside a `nastech` session (nastech_cli/commands.py)."""
    return ("To restore a newer snapshot, start `nastech` in a terminal and run `/snapshot list`, then "
            "`/snapshot restore <id>` (CLI only).")

# Directory names to skip (matched against each path component). ``nastech-agent`` only matches at
# the root (``_should_exclude``) so skill dirs like ``skills/.../nastech-agent/`` survive. The
# dependency/cache entries matter: one plugin venv or pip/uv cache under NASTECH_HOME walked
...
# Nastech-managed runtime downloads (see ``LOCAL_RUNTIME_ROOT_DIRS``). Matched ONLY at the root of
# NASTECH_HOME and at ``profiles/<name>/`` — a deeper dir of the same name (a skill's ``models/``)
# is user data.
...
# Files ``nastech import`` must never overwrite, matched by basename so root and named profiles are
...
# disconnected from the Nastech portal); PID/lock/registry files reference source PIDs. Mirrors
...
# Reserved archive subtree for memory-provider state OUTSIDE NASTECH_HOME (e.g. ~/.honcho, via
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Nastech backup slot."""
...
@contextmanager
def _backup_operation_lock(nastech_home: Path, timeout_seconds: float = 0.25):
    """Acquire one cross-process backup slot for full and quick snapshots."""
    lock_path = nastech_home / ".backup.lock"
        ...
        while not acquired:
            try:
                ...
                if time.monotonic() >= deadline:
                    raise BackupInProgressError("another Nastech backup is already running")
                time.sleep(0.05)
        yield
    finally:
        if acquired:
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    if _in_excluded_root_dir(rel_path):
        return True
    # ``nastech-agent`` only matches at the root level; nested same-named dirs are preserved.
    if any(p in _EXCLUDED_DIRS and (p != "nastech-agent" or p == parts[0]) for p in parts):
        return True
    name = rel_path.name
    return name in _EXCLUDED_NAMES or name.startswith(_EXCLUDED_PREFIXES) or name.endswith(_EXCLUDED_SUFFIXES)


def _iter_backup_files(nastech_root: Path, out_path: Path, skipped_dirs: Optional[set] = None):
    ...
    excluded tree, the root-only ``nastech-agent`` carve-out, root runtime trees, per-file rules),
    shared by ``nastech backup`` and the pre-update / pre-migration path so they can never drift.
    """
    for dirpath, dirnames, filenames in os.walk(nastech_root, followlinks=False):
        rel_dir = Path(dirpath).relative_to(nastech_root)
        is_root = rel_dir == Path(".")
        kept = [
            d for d in dirnames
            if (d not in _EXCLUDED_DIRS or (d == "nastech-agent" and not is_root))
        ...
        for fname in filenames:
            rel = rel_dir / fname
            fpath = nastech_root / rel
            # zipfile.write() follows file symlinks, so skip links before any archive write can
            # copy data from outside NASTECH_HOME; never archive the output zip into itself.
...
# 30 GB state.db, reading as a hung ``nastech update``) is replaced by the O(1) header+schema probe.
...
def _unlink_move_restore_db(src: Path, dst: Path) -> bool:
    ...
    try:
        ...
        logger.error("Refusing unlink+move restore of %s: %s Close the in-process "
                     "database handles (or restart Nastech) and retry.", dst, exc2)
        return False
...
_RUN_BACKUP_PREFIX = "nastech-backup-"
...
def run_backup(args) -> bool:
    ...
    try:
        with _backup_operation_lock(nastech_root):
            return _run_backup_locked(args, nastech_root)
    except BackupInProgressError as exc:
        print(f"Error: {exc}")
        raise SystemExit(2) from exc


def _run_backup_locked(args, nastech_root: Path) -> bool:
    ...
    files_to_add: list[tuple[Path, Path]] = list(_iter_backup_files(nastech_root, out_path, skipped_dirs))
    ...
    print(f"\nBackup {'incomplete' if errors else 'complete'}: {out_path}\n"
          ...
          f"  Time:        {elapsed:.1f}s")
    if external_to_add:
        print(f"\n  Included {len(external_to_add)} memory-provider file(s) stored outside {display_nastech_home()}.")
    if skipped_external:
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    ...
    # Telltale files a nastech home has — at the root or one level deep (zipped directory).
    if not any(Path(n).name in {"config.yaml", ".env", "state.db"} for n in names):
        return False, "zip does not appear to be a Nastech backup (no config.yaml, .env, or state databases found)"
    return True, ""
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    if len(first_parts) == 1 and first_parts <= {".nastech", "nastech"}:
...
def _count_session_rows(path: Path) -> Optional[Tuple[int, int]]:
    ...
    ``None`` means "unknown" (missing, not a Nastech session store, unreadable) — never "zero":
...
def _confirm_import_overwrite(nastech_root: Path) -> bool:
    """Prompt before importing over an existing installation; True when import may proceed."""
    if not any((nastech_root / m).exists() for m in ("config.yaml", ".env")):
        return True
    print("\nWarning: Target directory already has Nastech configuration.\n"
...
def _import_members(
    zf: zipfile.ZipFile, members: List[str], prefix: str, nastech_root: Path, file_count: int
    ...
    for member in members:
        ...
        # NOT under NASTECH_HOME; provider configs commonly hold credentials, so tighten to 0600.
...
def run_import(args) -> None:
    ...
    # ``get_default_nastech_root()`` would silently retarget a profile restore at the live root.
    nastech_root = get_nastech_home()
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Backup contains {file_count} files\nTarget: {display_nastech_home()}")
        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")
        if not args.force and not _confirm_import_overwrite(nastech_root):
            return
        ...
        restored, restored_external, errors, skipped_runtime, db_shrunk = _import_members(
            zf, members, prefix, nastech_root, file_count)
        elapsed = time.monotonic() - t0
        ...
        if restored_external:
            print(f"\n  Restored {restored_external} memory-provider file(s) to "
                  f"their original location(s) outside {display_nastech_home()}.")
        if errors:
...
def _restore_profile_wrappers(nastech_root: Path) -> List[str]:
    ...
    except ImportError:  # nastech_cli.profiles unavailable (fresh install)
        if any(profiles_dir.iterdir()):
            print("\n  Profiles detected but aliases could not be created.\n"
                  "  Run: nastech profile list  (after installing nastech)")
    return [n for n, _ in restored_profiles]


def _revive_gateway_after_import(nastech_root: Path) -> None:
    ...
    """
    native_default = _get_platform_default_nastech_home()
    if nastech_root != native_default and any(
        ...
        print("\nRestored into a non-default home; leaving the gateway service alone to avoid clashing "
              ...
              "To start a gateway for this home, run:  nastech gateway install")
        return
    try:
        from nastech_cli.gateway import ensure_gateway_service, _is_service_running
...
# Critical state files (relative to NASTECH_HOME) for quick snapshots; everything else is
...
# outside state.db, so it is listed explicitly — ``nastech update`` snapshots this set (#15733).
_QUICK_STATE_FILES = (
    ...
    # schema-init re-creates an empty one (#52889). Skipped when outside NASTECH_HOME.
...
def create_quick_snapshot(
    label: Optional[str] = None, nastech_home: Optional[Path] = None, keep: Optional[int] = None,
...
def _create_quick_snapshot_locked(
    ...
    multi-GB ``state.db`` never stalls ``nastech update`` while the small files are always captured.
    """
...
def list_quick_snapshots(limit: int = 20, nastech_home: Optional[Path] = None) -> List[Dict[str, Any]]:
    """List existing quick state snapshots, most recent first."""
    results = []
    for d in _snapshot_dirs(_quick_snapshot_root(nastech_home)):
...
def restore_quick_snapshot(snapshot_id: str, nastech_home: Optional[Path] = None) -> bool:
...
def restore_cron_jobs_if_emptied(snapshot_id: str, nastech_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
...
def create_pre_update_snapshots_all_profiles(
    ...
    home = invoking_home or get_nastech_home()
    for name, profile_home in _sibling_profile_homes(home):
        try:
            snap_id = create_quick_snapshot(
                label="pre-update", nastech_home=profile_home, keep=keep, max_file_size=max_file_size)
            if snap_id:
...
def restore_config_model_settings_if_rewritten(
    snapshot_id: str, nastech_home: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """Safety net for silent config.yaml model/MoA loss across ``nastech update``.
...
def _restore_all_sibling_profiles(
    ...
    """Run ``restore_fn(snap_id, nastech_home=...)`` for every sibling against ITS OWN
    ...
    for name, snap_id in profile_snapshots.items():
        ...
        if profile_home is None:
            continue
        try:
            result = restore_fn(snap_id, nastech_home=profile_home)
        except Exception as exc:
...
def prune_quick_snapshots(keep: int = _QUICK_DEFAULT_KEEP, nastech_home: Optional[Path] = None) -> int:
    """Remove oldest quick snapshots beyond the keep limit. Returns count deleted."""
    return _prune_oldest(_snapshot_dirs(_quick_snapshot_root(nastech_home)), keep, shutil.rmtree, "snapshot")
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        print(f"State snapshot created: {snap_id}\n"
              f"  {len(list_quick_snapshots())} snapshot(s) stored in {display_nastech_home()}/state-snapshots/\n"
...
def _write_full_zip_backup(out_path: Path, nastech_root: Path) -> Optional[Path]:
    """Full zip snapshot of ``nastech_root`` to ``out_path`` under the backup slot (same rules as
    :func:`run_backup`); None when nothing to back up, another backup running, or write error."""
    try:
        with _backup_operation_lock(nastech_root):
            return _write_full_zip_backup_locked(out_path, nastech_root)
    except BackupInProgressError as exc:
        logger.warning("Full-zip backup skipped: %s", exc)
        return None


def _write_full_zip_backup_locked(out_path: Path, nastech_root: Path) -> Optional[Path]:
    scan_started = time.monotonic()
    logger.info("automatic backup phase=scan status=started")
    try:
        files_to_add = list(_iter_backup_files(nastech_root, out_path))
    except OSError as exc:
...
def _create_prefixed_full_backup(
    nastech_home: Optional[Path], prefix: str, keep: int, what: str, prune_what: str) -> Optional[Path]:
    """Write ``<NASTECH_HOME>/backups/<prefix><timestamp>.zip`` and prune older same-prefix zips.
    ...
    if not nastech_root.is_dir():
        return None
    backup_dir = nastech_root / _PRE_UPDATE_BACKUPS_DIR
    try:
    ...
    if _write_full_zip_backup(out_path, nastech_root) is None:
        return None
...
def create_pre_update_backup(
    nastech_home: Optional[Path] = None, keep: int = _PRE_UPDATE_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-update-<timestamp>.zip``, auto-pruned; ``None`` if nothing
    was found or the backup failed. Never raises — ``nastech update`` continues anyway."""
    return _create_prefixed_full_backup(nastech_home, _PRE_UPDATE_PREFIX, max(keep, 1), "pre-update", "backup")


def create_pre_migration_backup(
    nastech_home: Optional[Path] = None, keep: int = _PRE_MIGRATION_DEFAULT_KEEP) -> Optional[Path]:
    """Full zip backup to ``backups/pre-migration-<timestamp>.zip`` before ``nastech claw migrate``
    (same dir as update backups so listings/``nastech import`` find it); ``None`` if nothing was
    found or the write failed. Never raises."""
    return _create_prefixed_full_backup(
        nastech_home, _PRE_MIGRATION_PREFIX, max(keep, 0), "pre-migration", "pre-migration backup")
