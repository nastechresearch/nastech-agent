"""Directory initialization and storage diagnostics for the active Nastech home."""
...
def _ensure_directory(path: Path, *, create: bool, secure: bool, home: Path) -> None:
    ...
    try:
        ...
        raise HomeInitializationError(
            f"Cannot initialize Nastech directory {path}: {exc}. "
            ...
            "Nastech has not replaced the link or created its missing target."
        ) from exc


def initialize_home(home: Path, subdirs: tuple[str, ...], ensured: set[str]) -> None:
    from nastech_cli.config import _ensure_default_soul_md, is_managed
    ...
    try:
        ...
        try:
            ...
            raise HomeInitializationError(
                f"Cannot initialize Nastech home {home}: {exc}. "
...
def config_load_issue(exc: Exception):
    from nastech_cli.config import ConfigIssue

    if isinstance(exc, (HomeInitializationError, OSError)):
        return ConfigIssue(
            "error", f"Nastech storage is unavailable: {exc}",
