...
Wraps the same config layer the CLI uses (nastech_cli.mcp_config); stdio ``env``
...
def _normalize_mcp_server_create(body: MCPServerCreate) -> tuple[str, Dict[str, Any], Optional[str]]:
    ...
    """
    from nastech_cli.mcp_config import _bearer_auth_headers, _strip_bearer_prefix
    ...
    try:
        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                previous_entry = None
                try:
                    previous_entry = manager.remove(flow.server_name, nastech_home=flow.nastech_home)
                    ...
                    manager.restore_entry(flow.server_name, previous_entry, nastech_home=flow.nastech_home)
                    raise
        finally:
