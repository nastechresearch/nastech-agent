...
``nastech update`` performs its fleet restart in the interpreter that started
...
itself lives in :mod:`nastech_cli.update_restart_recovery`.
"""
...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``nastech-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``nastech-serve.service`` can exist in BOTH the user and the system
...
def _recover_gateway_restart_after_abort(
    ...
    *,
    ...
    The same child additionally restarts active ``nastech-serve*`` systemd
    units (#92145).  ``nastech serve`` hosts ``tui_gateway.server`` and is
    ...
    """
    from nastech_cli.update_cmd import _gateway_recovery_partition
...
def _warn_stale_serve_runtimes(rows) -> None:
    ...
    terminal naming the responsible process. ``nastech serve`` hosts
    ...
    print(
        "    Restart them before using Nastech again, e.g."
        " `systemctl --user restart nastech-serve.service`"
        " or by relaunching `nastech serve` / the Desktop app."
    )
...
def _abort_recovery_is_complete(
    *,
    ...
    ``gateway_state.json`` / control socket — so a ``nastech serve`` still
    ...
    * no ``nastech-serve*`` unit failed to produce a verified replacement; and
