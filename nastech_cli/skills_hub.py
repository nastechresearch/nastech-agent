...
# serves everything from "nastech-index", so that entry MUST exceed the whole catalog or browse
...
_BROWSE_LIMITS = {
    "nastech-index": 1000000, "official": 200, "skills-sh": 200, "well-known": 50,
    "github": 200, "clawhub": 500, "lobehub": 500, "browse-sh": 500}
# Programmatic (TUI gateway) browse keeps its own, lower caps.
_BROWSE_API_LIMITS = {
    "nastech-index": 5000, "official": 100, "skills-sh": 100, "well-known": 25,
...
def _ident_col(style: str) -> tuple:
    ...
    # copy into `nastech skills install`.
...
def _existing_categories() -> List[str]:
    """Sorted category buckets under ``~/.nastech/skills/`` (children without their own SKILL.md)."""
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print("[bold]Category[/] "
                f"[dim](optional — press Enter to install flat at {display_nastech_home()}/skills/<name>/)[/]")
...
def _render_browse_page(c: Console, deduped, page_items, page: int, total_pages: int,
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from Nastech Research[/]")
    c.print()
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    # nastech-index AND the external APIs at once would double-count every skill.
...
def _scan_block_message(result, identifier: str) -> str:
    ...
    can help, and the read-only next step (``nastech skills inspect``). The hard-block rule mirrors
    ...
    policy = ("Nastech never installs unverified skills with high-risk findings, even with --force."
    ...
    return (f"the security scan found {findings} in '{identifier}' (listed above). "
            ...
            f"installing, run `nastech skills inspect {identifier}`.")
...
def _announce_blueprint(c: Console, skill_name: str) -> None:
    """Offer an installed skill's ``metadata.nastech.blueprint`` via /suggestions — never
    ...
    try:
        ...
        if register_blueprint_suggestion(spec) is not None:
            ...
            c.print("[dim]You can still schedule it any time by asking the agent "
                    "or via[/] [bold]nastech cron add[/][dim].[/]\n")
...
def _print_fetch_failure(c: Console, sources, identifier: str, meta=None, source=None) -> None:
    ...
    if meta is not None and not rate_limited:
        ...
        c.print("[dim]Stale index entry: the skill was likely renamed or removed by "
                "its author. Try `nastech skills search` for an alternative.[/]\n")
        return
    ...
    if rate_limited:
        ...
        c.print(f"Check the name with [bold]nastech skills search {identifier.rsplit('/', 1)[-1]}[/] "
                "and check your internet connection. If it keeps failing, run [bold]nastech doctor[/].\n")
...
_INSTALL_PANELS = {
    "official": (
        "[bold bright_cyan]This is an official optional skill maintained by Nastech Research.[/]\n\n"
...
def _confirm_install(c: Console, bundle, category: str) -> bool:
    ...
    files_at = f"Files will be at: [cyan]{display_nastech_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]"
...
def do_list(source_filter: str = "all", enabled_only: bool = False,
    ...
    profile's config — ``-p`` swaps NASTECH_HOME at process start, so no profile flag here."""
    from tools.skills_hub import HubLockFile, ensure_hub_dirs
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    if orphaned:
        c.print(f"[yellow]Orphaned:[/] {', '.join(orphaned)} — lock-file entries whose local "
                ...
                "remove the stale entry with: nastech skills uninstall <name>\n")
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None,
    ...
    Mirrors the user-modified protection bundled skills already get from ``nastech update`` (ported from
    ...
    if skipped_local:
        ...
        c.print("[dim]Overwrite with: nastech skills update <name> --force[/]\n")
...
def do_list_modified(console: Optional[Console] = None, as_json: bool = False) -> None:
    """List bundled skills the user has edited (which `nastech update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `nastech update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   nastech skills diff <name>[/]")
    c.print("[dim]Resume updates: nastech skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: nastech skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: nastech skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            _print_error(c, f"Repo required. Usage: nastech skills tap {action} owner/repo")
            return
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            _print_error(c, "GitHub authentication required.\n"
                            f"Set GITHUB_TOKEN in {display_nastech_home()}/.env "
...
# `nastech skills <action>` -> handler(args). Lambdas late-bind the do_* names so
# tests that patch("nastech_cli.skills_hub.do_install") still intercept.
_CLI_ACTIONS = {
...
def skills_command(args) -> None:
    ...
    if handler is None:
        _console.print("Usage: nastech skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
