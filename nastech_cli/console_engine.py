...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (prev.startswith("Run 'nastech doctor'") and last.startswith("Run 'nastech setup'")):
        return text.rstrip()
    lines = lines[:-2]
    _drop_trailing_blank(lines)
...
def _split_line(line: str) -> list[str]:
    ...
    # See #78293.
    # See #83934.
    from nastech_cli._subprocess_compat import split_command_line
    try:
...
def _clean_summary(text: str | None) -> str:
    ...
    return "" if summary.startswith("Run `nastech ") else summary
...
class _CliSurface:
    ...
    ``extracted``: ``builder(subparsers, <handler>=fn)``, fn from nastech_cli.main.
    ``registered``: ``register(subparsers.add_parser(root))``, optional module ``handler``.
    ``builder``: ``top = builder(subparsers)``, func from nastech_cli.main.
    ``adder``: self-wiring."""
    ...
    def build(self, root: str, *, live: bool) -> _ArgumentParser:
        ...
        main_handler = (
            (lambda: getattr(importlib.import_module("nastech_cli.main"), self.handler))
...
def _sub(module: str, builder: str, handler: str) -> _CliSurface:
    return _CliSurface("extracted", f"nastech_cli.subcommands.{module}", builder, handler)


def _reg(module: str, handler: str | None = None) -> _CliSurface:
    return _CliSurface("registered", f"nastech_cli.{module}", "register_cli", handler)
...
_CLI_FAMILIES: dict[str, tuple[_CliSurface, str]] = {
    ...
    "portal": (_CliSurface("adder", "nastech_cli.portal_cli", "add_parser"), "info, tools"),
    "project": (
        _CliSurface("builder", "nastech_cli.projects_cmd", "build_parser", "cmd_project"),
        "list, show, *create, *add-folder, *remove-folder, *rename, *set-primary, *use, "
        "*archive, *restore, *bind-board"),
    "kanban": (
        _CliSurface("builder", "nastech_cli.kanban", "build_parser", "cmd_kanban"),
...
_SEND_SURFACE = _CliSurface("adder", "nastech_cli.send_cmd", "register_send_subparser")


def _register_command_family(
    engine: "NastechConsoleEngine", root: str, surface: _CliSurface, paths: str) -> None:
    ...
    for child_path, mutating in _paths(paths):
        ...
        def handler(_engine: NastechConsoleEngine, args: list[str], fixed=child_path) -> str:
            return _dispatch(surface, root, fixed, args, namespace_update)

        engine.register(
            full_path, usage, summaries.get(full_path) or f"Run `nastech {usage}`.", handler,
            mutating=mutating, confirmation=f"Run `nastech {usage}`?")
...
_BLOCKED_PAIRS = {
    ...
    ("sessions", "delete"): "`sessions delete` and `sessions prune` are not available in Nastech Console.",
    ("sessions", "prune"): "`sessions delete` and `sessions prune` are not available in Nastech Console.",
}
...
class NastechConsoleEngine:
    ...
    def execute(self, line: str, *, confirmed: bool = False) -> ConsoleResult:
        ...
        try:
            ...
            if tokens and tokens[0] == "nastech":
                tokens = tokens[1:]
            if not tokens:
    ...
    def help_text(self, subject: str | None = None) -> str:
        ...
        lines = ["Nastech Console", "", "Supported commands:"]
...
def _captured(fn):
    ...
    @functools.wraps(fn)
    def wrapper(engine: NastechConsoleEngine, args: list[str]) -> str:
...
def _apply_confirmed_defaults(args: argparse.Namespace) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError(
                "auth add --type api-key requires --api-key in Nastech Console.")
...
_version = _simple_command(
    "version", "nastech_cli._startup_fast", "print_fast_version_info", check_updates=True)
...
_doctor = _simple_command(
    "doctor", "nastech_cli.doctor", "run_doctor", lambda: (SimpleNamespace(fix=False, ack=None),))
_config_show = _simple_command("config show", "nastech_cli.config", "show_config")
_cron_status = _simple_command("cron status", "nastech_cli.cron", "cron_status")
...
def _sessions_list(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    from nastech_state_sessions import INTERNAL_LISTING_SOURCES
...
@_captured
def _config_migrate(_engine: NastechConsoleEngine, args: list[str]) -> None:
...
def _guard_exports(db, session_ids: list[str]) -> None:
    ...
    from nastech_state import SessionExportTooLargeError, resolved_max_export_messages
...
@_captured
def _sessions_export(_engine: NastechConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_rename(_engine: NastechConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_optimize(_engine: NastechConsoleEngine, args: list[str]) -> None:
...
@_captured
def _sessions_repair(_engine: NastechConsoleEngine, args: list[str]) -> int | None:
    ns = _parse(
    ...
    from nastech_state_repair import _db_opens_cleanly, repair_state_db_schema
...
def _cron_list(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    from nastech_cli.cron import cron_list
...
def _cron_pause(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    return _cron_job_action(
        ...
        lambda ref: pause_job(ref, reason="paused from nastech console"))
...
def _cron_resume(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    from cron.jobs import AmbiguousJobReference, _nastech_now, rearm_oneshot, resume_job
    try:
        job = (
            rearm_oneshot(ns.job, _nastech_now().isoformat() if ns.run_now else ns.at)
...
_BUILTIN_COMMANDS = (
    (("status",), "status", "Show Nastech component status.", _status, ""),
    (("version",), "version", "Show Nastech version information.", _version, ""),
    (("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, ""),
    (("logs",), "logs [name] [-n N]", "Show recent Nastech logs.", _logs, ""),
    ...
    (("config", "migrate"), "config migrate", "Update config with new options.", _config_migrate,
     "Update Nastech configuration with missing defaults?"),
...
def run_console_repl(
    *, stdin=None, stdout=None, stderr=None, interactive: bool | None = None) -> int:
    """Run the local ``nastech console`` REPL."""
    ...
    say("Nastech Console. Type `help` for commands, `exit` to quit.")
    while True:
        say("nastech> ", end="", flush=True)
        line = stdin.readline()
        if line == "":
            say("")
            return 0
