...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'nastech doctor'")
        and last.startswith("Run 'nastech setup'")
    ):
        return text.rstrip()
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `nastech "):
        return ""
    return summary
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("nastech_cli.main")
    builder = getattr(module, builder_name)
...
def _register_command_family(
    ...
    *,
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `nastech {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
...
class NastechConsoleEngine:
    ...
    def _register_broad_cli_surface(self) -> None:
        ...
        extracted = {
            ...
            "prompt-size": (
                "nastech_cli.subcommands.prompt_size",
            ...
            "insights": (
                "nastech_cli.subcommands.insights",
            ...
            "security": (
                "nastech_cli.subcommands.security",
            ...
            "backup": (
                "nastech_cli.subcommands.backup",
                "build_backup_parser",
                "cmd_backup",
                [()],
                {()},
            ...
            "import": (
                "nastech_cli.subcommands.import_cmd",
            ...
            "config": (
                "nastech_cli.subcommands.config",
                "build_config_parser",
                "cmd_config",
                [("env-path",), ("check",)],
                set(),
            ...
            "plugins": (
                "nastech_cli.subcommands.plugins",
            ...
            "skills": (
                "nastech_cli.subcommands.skills",
                "build_skills_parser",
                "cmd_skills",
                [
                    ("browse",),
            ...
            "memory": (
                "nastech_cli.subcommands.memory",
                "build_memory_parser",
                "cmd_memory",
                [("status",), ("off",), ("reset",)],
                {("off",), ("reset",)},
            ...
            "pairing": (
                "nastech_cli.subcommands.pairing",
            ...
            "webhook": (
                "nastech_cli.subcommands.webhook",
            ...
            "profile": (
                "nastech_cli.subcommands.profile",
        ...
        self.register(
            ...
            _adder_handler("send", (), "nastech_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("nastech_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "nastech_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("nastech_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("nastech_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
...
def _sessions_repair(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        from nastech_state import DEFAULT_DB_PATH, _db_opens_cleanly, repair_state_db_schema
...
def _cron_pause(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    try:
        job = pause_job(args[0], reason="paused from nastech console")
...
def _cron_resume(_engine: NastechConsoleEngine, args: list[str]) -> str:
    ...
    try:
        if ns.at or ns.run_now:
            job = rearm_oneshot(ns.job, _nastech_now().isoformat() if ns.run_now else ns.at)
        else:
...
def run_console_repl(
    *,
    ...
    while True:
        if interactive:
            print("nastech> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)
