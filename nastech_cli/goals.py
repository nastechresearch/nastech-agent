...
# (schema DDL, FTS tables, first nastech_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per NASTECH_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``nastech profile delete`` force-closes every handle under the profile home
        # (``nastech_state_registry.close_all_under``) before rmtree; a same-name recreate in this
...
def judge_goal(
    ...
    *,
    ...
    try:
        ...
        # No client at all (e.g. a dead Nastech refresh token): name the cause so the user is sent to
