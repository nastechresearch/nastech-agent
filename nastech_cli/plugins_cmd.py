...
from nastech_cli.plugin_capabilities import _child_dict
...
def _missing_env_specs(manifest: dict) -> list[dict]:
    ...
    normalised to dicts; nameless entries dropped) whose variable is unset in ``~/.nastech/.env``."""
    env_specs = [
...
def _refuse_conflicting_python_deps(tmp_target: Path, plugin_name: str) -> None:
    ...
    from nastech_cli.plugin_python_deps import DependencyConflict, refuse_conflicting_candidate
    try:
        refuse_conflicting_candidate(tmp_target, home=get_nastech_home())
        ...
        raise PluginOperationError(
            ...
            "its Python packages yourself: nastech plugins install <identifier> --no-deps") from exc
...
def _install_python_dependencies_quietly(target: Path, warnings: list[str]) -> list[str]:
    ...
    from nastech_cli.plugin_python_deps import install_for_plugin_dir
...
def _clone_failure_message(git_url: str, git_error: str) -> str:
    ...
    return (f"Could not download the plugin from {git_url}. Check the address (browse the catalog "
            "with `nastech plugins search`), check your internet connection, or, if the repository "
...
def _unknown_plugin_message(name: str, *, downloaded_only: bool = False) -> str:
    ...
    return (f"[red]No plugin named '{name}'.[/red] Run `nastech plugins list` to see the exact names "
            f"(nested plugins use their full key, e.g. web/firecrawl).{scope} "
            "To add one: `nastech plugins install <owner/repo>`.")
...
def _install_metadata_path() -> Path:
    return get_nastech_home() / "plugins" / ".install-metadata.json"
...
def _ensure_tree_readable(root: Path, plugins_dir: Path) -> None:
    """Refuse to ship a tree Nastech cannot read back. A clone can land unreadable (Windows ACL
...
def cmd_install(
    ...
    a ``.nastech-catalog.json`` sidecar; URLs/shorthand are flagged as custom (unreviewed) sources. Every
    ...
    *enable* None prompts "Enable now? [y/N]"; True/False skip the prompt.
    """
    from nastech_cli import plugins_cmd_catalog as catalog
    console = _console()
    entry = None
    if catalog.looks_like_catalog_name(identifier):
        ...
        console.print("[yellow]Warning:[/yellow] custom (unreviewed) source — not from the Nastech catalog.")
    if allow_removed:
        console.print(
    ...
    if not _looks_like_plugin_dir(target):
        console.print(
            ...
            f"plugin.json, or __init__.py. It may not be a valid Nastech plugin.")
...
def cmd_update(name: str) -> None:
    ...
    try:
        output = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"`nastech plugins install {escape(str(rec.get('source', '<source>')))} --force "
    ...
    if declared_caps:
        from nastech_cli.plugin_capabilities import declared_set_changed, pending_capabilities
...
def _rescan_after_update(target: Path, name: str, console) -> None:
    ...
    if scan_result.verdict == "dangerous":
        ...
        console.print(
            ...
            f"findings, then re-enable with `nastech plugins enable {name}` "
            f"if you trust them.")
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``nastech setup`` / ``nastech plugins disable basic`` can park it there while
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    """
    from nastech_cli.relay_plugin_cutover import LEGACY_RELAY_PLUGIN_KEYS, RELAY_PLUGINS_CONFIG_ENV
    console = _console()
    ...
    def _refuse_legacy_relay(plugin: str) -> None:
        if plugin in LEGACY_RELAY_PLUGIN_KEYS:
            _fail(console, (
                ...
                f"by Nastech core; configure {RELAY_PLUGINS_CONFIG_ENV} instead."))
...
def _declared_capabilities_from_manifest(manifest: dict, plugin_name: str = "?") -> list:
    ...
    from nastech_cli.plugin_capabilities import parse_declared_capabilities
...
def _declared_capabilities_for_key(key: str) -> list:
    ...
    if entry[3] == "entrypoint":
        from nastech_cli.plugins import discover_entrypoint_manifests
...
def _run_capability_consent(console, plugin_id: str, declared: list, *, context: str = "install") -> bool:
    ...
    """
    from nastech_cli.plugin_capabilities import CAPABILITY_REGISTRY, pending_capabilities, record_consent
    ...
    if not _is_tty():
        console.print(
            ...
            f"`nastech plugins capabilities {plugin_id}` to review and "
            f"`nastech plugins enable {plugin_id}` to grant interactively.")
        return False
    ...
    console.print(
        ...
        f"`nastech plugins enable {plugin_id}` to grant later.[/dim]")
    return False


def cmd_capabilities(name: Optional[str] = None) -> None:
    """``nastech plugins capabilities [<id>]`` — declared vs granted."""
...
def _resolve_tool_override_grant(console, key: str, allow_tool_override: Optional[bool]) -> None:
    ...
    if allow_tool_override:
        ...
        console.print(
            ...
            f"`nastech plugins enable {key} --allow-tool-override` to grant "
            "this later.[/dim]")
...
def _discover_all_plugins() -> list:
    ...
    from nastech_cli.plugins import discover_entrypoint_manifests, get_bundled_plugins_dir
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        _fail(console, "[dim]List installed plugins:[/dim] nastech plugins list")
...
def _run_composite_ui(curses, plugin_keys, plugin_labels, plugin_selected, disabled, categories, console):
    ...
    from nastech_cli.curses_ui import _addnstr, flush_stdin
...
def dashboard_install_plugin(
    ...
    if catalog_name:
        ...
        if entry is None:
            return {"ok": False, "error": f"'{catalog_name}' is not in the Nastech plugin catalog."}
        identifier = entry.install_identifier
    else:
        warnings.append("Custom (unreviewed) source — not from the Nastech catalog.")
    try:
    ...
    try:
        ...
        msg = _pull_plugin_update(
            ...
            lambda rec: (
                ...
                f"run `nastech plugins install {rec.get('source', '<source>')} --force "
...
def _run_plugin_git(
    ...
    from nastech_cli.git_credentials import run_git_with_credential_fallback
...
def _autostash_dirty_tree(git_exe: str, target: Path) -> tuple[bool, str]:
    ...
    push = _run_plugin_git(
        git_exe, target, "stash", "push", "--include-untracked", "-m", "nastech-plugin-update-autostash")
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    marketplace now succeeds when its checkout has local changes"), and the same autostash approach ``nastech
...
def cmd_plugin_doctor(target: str = ".", *, ci: bool = False) -> None:
    ...
    from nastech_cli.plugin_dev import doctor_plugin
...
def _catalog():
    from nastech_cli import plugins_cmd_catalog
    return plugins_cmd_catalog


def _action_pack(args):
    from nastech_cli.plugin_packs import pack_command
    pack_command(args)


def cmd_compat(args: Any | None = None) -> None:
    """``nastech plugins compat`` — which installed plugins import paths scheduled for removal, and where."""
    import sys
    from pathlib import Path
    from nastech_cli.plugin_compat import (
