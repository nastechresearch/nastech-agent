...
def _print_python_dependencies(manifest: dict, console) -> None:
    ...
    Declaration seam ONLY — Nastech never auto-installs plugin pip
...
def _install_metadata_path() -> Path:
    return get_nastech_home() / "plugins" / _INSTALL_METADATA_FILE
...
def _resolve_index_name(identifier: str, console) -> tuple[str, Optional[str]]:
    ...
    """
    from nastech_cli.plugin_index import SECURITY_FOOTER, load_index, resolve_name
    ...
    if entry is None:
        if len(candidates) > 1:
            ...
            console.print(
                ...
                f"community index ({source}). Use `nastech plugins search <term>` to "
...
def cmd_update(name: str) -> None:
    ...
    if install_record.get("pinned") is True:
        ...
        console.print(
            ...
            f"`nastech plugins install {recorded_source} --force "
...
def ensure_basic_auth_plugin_enabled_in_config(cfg: dict) -> bool:
    ...
    ``nastech setup`` / ``nastech plugins disable basic`` can park the plugin
...
def cmd_enable(name: str, allow_tool_override: Optional[bool] = None) -> None:
    ...
    # Nastech core and are trusted; every other source needs operator opt-in.
    if source == "bundled":
        return
...
def _print_capability_list(console, capabilities: list) -> None:
    ...
    from nastech_cli.plugin_capabilities import CAPABILITY_REGISTRY
...
def cmd_capabilities(name: Optional[str] = None) -> None:
    ...
    from nastech_cli.plugin_capabilities import granted_capabilities
...
def _discover_entrypoint_plugins() -> list[tuple[str, str, str, str]]:
    """Return plugin entries advertised through ``nastech_agent.plugins``.

    Entry-point plugins are installed as Python packages, so they do not have a
    plugin directory under ``~/.nastech/plugins``. Include package metadata here
    so ``nastech plugins list`` can show and enable them.
    """
    from nastech_cli.plugins import ENTRY_POINTS_GROUP
...
def cmd_show(name: str) -> None:
    ...
    if match is None:
        ...
        console.print("[dim]List installed plugins:[/dim] nastech plugins list")
        sys.exit(1)
...
def cmd_toggle() -> None:
    ...
    # plugins.disabled, but ``nastech plugins enable web/firecrawl`` cleared
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    ...
    if install_record.get("pinned") is True:
        ...
        return {
            ...
            "error": (
                ...
                f"run `nastech plugins install {recorded_source} --force "
    ...
    # Sibling of the CLI ``nastech plugins update`` path: drop bytecode
...
def _clear_plugin_bytecode(target: Path) -> int:
    ...
    fingerprint sweep in ``nastech_cli.main`` never covers them. After a
...
def _git_pull_plugin_dir(target: Path) -> tuple[bool, str]:
    ...
    has local changes"), and the same autostash approach ``nastech update``
    ...
    try:
        ...
        if dirty:
            ...
            push = _run_plugin_git(
                ...
                "-m", "nastech-plugin-update-autostash",
            )
...
def cmd_search(
    ...
    *,
    ...
    console.print(table)
    console.print(f"[dim]Index source: {source}. Install: nastech plugins install <name>[/dim]")
    console.print(f"[dim]{SECURITY_FOOTER}[/dim]")
