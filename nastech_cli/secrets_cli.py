...
# module makes the ``nastech update`` self-lock preflight defer. This module is registered
# parse-time from ``nastech_cli.main``, so the backend import stays lazy (nothing touches ``bw``
...
def __getattr__(name: str):
    ...
    Existing callers (and upstream tests) monkeypatch attributes on ``nastech_cli.secrets_cli.bw`` directly.
    ...
    so ``import nastech_cli.secrets_cli`` stays crypto-free while ``secrets_cli.bw.find_bws`` still resolves.
    """
    if name == "bw":
        return _load_bw()
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print("  Status:  [cyan]nastech secrets bitwarden status[/cyan]\n"
...
def cmd_token(args: argparse.Namespace) -> int:
    ...
    def verify(token: str) -> bool:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `nastech secrets bitwarden setup` to pick a different project.[/yellow]")
        return True
    ...
    return rotate_token(
        ...
        disabled_note=None if bw_cfg.get("enabled") else (
            ...
            "run `nastech secrets bitwarden setup` (or set "
