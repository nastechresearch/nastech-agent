...
# that process is ``nastech update``, the self-lock preflight detects the
# loaded native module and defers (#86781).  Keep the backend import lazy:
# this module is registered parse-time from ``nastech_cli.main`` and must not
...
def __getattr__(name: str):
    ...
    ``nastech_cli.secrets_cli.bw`` directly.  Resolving that attribute at
    ...
    the first actual attribute access, so ``import nastech_cli.secrets_cli``
    ...
    if not args.no_verify:
        ...
        if project_id and projects and project_id not in {p["id"] for p in projects}:
            console.print(
                ...
                "app or re-run `nastech secrets bitwarden setup` to pick a "
