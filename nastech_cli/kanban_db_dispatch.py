...
Split out of ``nastech_cli.kanban_db``; origin-resident helpers are reached
...
from nastech_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER

if TYPE_CHECKING:
    from nastech_cli.kanban_db import Task
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Nastech profile. Expected steady-state
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``nastech kanban tail`` is the only place the
...
# children): a per-tick ``nastech kanban dispatch`` process finds it empty, so
...
def _worker_log_exit_code(task_id: str, board: Optional[str] = None) -> Optional[int]:
    ...
    (``nastech_cli.quiet_single_query.exit_single_query``), so it is there whether
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|𓄃\s*Nastech")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``nastech kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``nastech_cli.profiles.profile_exists``, or ``None`` when it cannot be
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several Nastech homes),
...
def dispatch_profile_allowlist_summary() -> str:
    ...
    Surfaced by ``nastech kanban diagnostics`` so an operator on a shared board
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Nastech profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Nastech ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``NASTECH_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``nastech -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``nastech kanban tail`` shows why the task looks stuck.
...
def _module_nastech_argv() -> list[str]:
    """Interpreter-bound Nastech CLI invocation (``nastech_cli.main`` is the
    console-script target — there is no top-level ``nastech`` package)."""
...
def _nastech_path_argv(path: str) -> list[str]:
    """argv for a resolved Nastech executable path. Windows batch shims
...
def _resolve_nastech_argv() -> list[str]:
    """Resolve the ``nastech`` invocation as argv for ``Popen``: ``$NASTECH_BIN``
    ...
    nastech_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("nastech")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``nastech_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``nastech`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_nastech_bin``; local because ``nastech_cli``
    ...
    nastech_bin = _safe_which_no_cwd("nastech") if _kb._IS_WINDOWS else shutil.which("nastech")
    if nastech_bin:
...
def _resolve_worker_cli_toolsets(nastech_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``NASTECH_KANBAN_TASK`` is set.
    """
    if not nastech_home:
        return None
    try:
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(nastech_home)))
...
def _worker_argv(task: Task, profile_arg: str, nastech_home: Optional[str]) -> list[str]:
    """Build the ``nastech -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_nastech_argv(),
        ...
        "--cli",
        # Workers run under a profile-scoped NASTECH_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(nastech_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `nastech kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    ...
    ``NASTECH_KANBAN_DB`` / ``NASTECH_KANBAN_BOARD`` / workspaces_root to the
    ...
    try:
        ...
        # NASTECH_PROFILE (set below) instead.
        profile_home = None
    ...
    # without it the child's get_nastech_home() falls back to the DEFAULT
    # profile root because `nastech -p` applies its override before
    # nastech_constants is imported.
    if profile_home:
        env["NASTECH_HOME"] = profile_home
    ...
    # match after `nastech -p` rewrites NASTECH_HOME (symlink / Docker layouts).
    env["NASTECH_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["NASTECH_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    ...
    env["NASTECH_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads NASTECH_PROFILE for its default author; `-p` alone
    ...
    # `--cli` is the highest-precedence TUI override; dropping NASTECH_TUI covers
    # older nastech builds on PATH that predate the flag's precedence.
    env.pop("NASTECH_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("NASTECH_HOME"))
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``nastech kanban dispatch`` — the standalone
...
from nastech_cli import kanban_db_connect as _kbc  # noqa: E402
from nastech_cli import kanban_db_workspace as _kbw  # noqa: E402
