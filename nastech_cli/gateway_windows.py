...
_GATEWAY_ENV = (("PYTHONIOENCODING", "utf-8"), ("NASTECH_GATEWAY_DETACHED", "1"), ("NASTECH_SUPERVISED_CHILD", "1"))
...
def _nastech_home() -> Path:
    ...
    return Path(get_nastech_home())


def _preserve_nastech_home_path(path: str | Path) -> str:
    r"""Render Nastech-owned paths under the configured NASTECH_HOME spelling.

    ``%LOCALAPPDATA%\nastech`` may be a symlink/junction to another drive; launcher files must not
    bake in the resolved target for paths under NASTECH_HOME.
    """
    candidate = Path(path)
    try:
        home = _nastech_home()
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "nastech_cli.main", *_current_profile_cli_args(), "gateway", command, *(extra_args or [])]
...
def _launch_elevated_install(force: bool = False, *, start_now: bool | None = None, start_on_login: bool | None = None) -> bool:
    ...
    overrides = {"NASTECH_GATEWAY_ELEVATED_HANDOFF": "1"}
    extra_args = ["--elevated-handoff"]
    if force:
        extra_args.append("--force")
    for choice, env_key, flag in (
        (start_now, "NASTECH_GATEWAY_INSTALL_START_NOW", "start-now"),
        (start_on_login, "NASTECH_GATEWAY_INSTALL_START_ON_LOGIN", "start-on-login"),
    ):
...
def get_task_name() -> str:
    ...
    _assert_windows()
    from nastech_cli.gateway import _profile_suffix  # local: avoids circular init during boot
...
def get_task_script_path() -> Path:
    """The generated ``gateway.cmd`` wrapper under ``<NASTECH_HOME>/gateway-service/`` (per-profile
    installs stay self-contained); the VBS launcher lives beside it."""
    _assert_windows()
    script_dir = _nastech_home() / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    """Stable cwd for detached/startup runs: anchor at NASTECH_HOME when it exists (mirrors the POSIX
...
def _gateway_run_argv(python_exe: str, profile_arg: str) -> list[str]:
    """``python -m nastech_cli.main [--profile X] gateway run`` — shared by every launcher renderer."""
...
def _launcher_settings(home: Path | None = None) -> tuple[str, str, str, str]:
    """Return (python_path, working_dir, nastech_home, profile_arg) for generated launchers.
    ``home`` targets another profile's NASTECH_HOME (per-profile cold-start, #110959)."""
    from nastech_cli.gateway import PROJECT_ROOT, _profile_arg, get_python_path  # avoid circular init

    nastech_home = str(home if home is not None else _nastech_home())
    return (
        _preserve_nastech_home_path(get_python_path()),
        _stable_gateway_working_dir(PROJECT_ROOT),
        nastech_home,
        _profile_arg(nastech_home),
    )


def _launcher_pythonpath_entries(extra_pythonpath: list[str]) -> list[str]:
    return [
        _preserve_nastech_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_nastech_home_path(entry) for entry in extra_pythonpath],
    ]


def _build_gateway_cmd_script(python_path: str, working_dir: str, nastech_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f'set "NASTECH_HOME={nastech_home}"',
        *[f'set "{k}={v}"' for k, v in _GATEWAY_ENV],
        # VIRTUAL_ENV lets the gateway's own python detection find the venv.
        f'set "VIRTUAL_ENV={_preserve_nastech_home_path(venv_dir)}"',
...
def _build_gateway_vbs_script(python_path: str, working_dir: str, nastech_home: str, profile_arg: str) -> str:
    ...
    lines = [
        ...
        f"env.Item({q('NASTECH_HOME')}) = {q(nastech_home)}",
        *[f"env.Item({q(k)}) = {q(v)}" for k, v in _GATEWAY_ENV],
        f"env.Item({q('VIRTUAL_ENV')}) = {q(_preserve_nastech_home_path(venv_dir))}",
...
def _atomic_write(path: Path, content: str, tmp: Path) -> None:
    ...
    ``Nastech_Gateway.tmp`` pops up in Notepad after every sign-in (#114093).
    """
    try:
...
def _build_gateway_argv(home: Path | None = None) -> tuple[list[str], str, dict[str, str]]:
    ...
    _assert_windows()
    from nastech_cli.gateway import PROJECT_ROOT

    python_path, working_dir, nastech_home, profile_arg = _launcher_settings(home)
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    env_overlay = {"NASTECH_HOME": nastech_home, **dict(_GATEWAY_ENV), "VIRTUAL_ENV": _preserve_nastech_home_path(venv_dir)}
    _prepend_pythonpath(env_overlay, [_preserve_nastech_home_path(p) for p in (PROJECT_ROOT, *extra_pythonpath)])
...
def windowless_gateway_restart_spec(run_argv: list[str]) -> tuple[list[str], str, dict[str, str]]:
    ...
    interpreter via ``_resolve_detached_python`` and supplies the stable cwd + env overlay (NASTECH_HOME,
    ...
    try:
        nastech_home = str(_nastech_home().resolve())
    except Exception:
        nastech_home = ""
    env_overlay: dict[str, str] = {"PYTHONIOENCODING": "utf-8", "NASTECH_GATEWAY_DETACHED": "1", "VIRTUAL_ENV": str(venv_dir)}
    if nastech_home:
        env_overlay["NASTECH_HOME"] = nastech_home
...
def _stdin_is_interactive(*, isatty: bool, console_mode_ok: bool | None) -> bool:
    ...
    every character device — the NUL device included (`nastech gateway start < NUL`, stdin=DEVNULL) — so
...
def _prompt_install_choices(start_now: bool | None = None, start_on_login: bool | None = None) -> tuple[bool, bool]:
    ...
    if start_now is None:
        start_now = _install_choice_from_env("NASTECH_GATEWAY_INSTALL_START_NOW")
    if start_on_login is None:
        start_on_login = _install_choice_from_env("NASTECH_GATEWAY_INSTALL_START_ON_LOGIN")
...
def install(
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run in the foreground later with: nastech gateway run")
        return

    task_name = get_task_name()
    script_path = _write_task_script()
    # A pre-fix install that failed its Startup-folder swap left `Nastech_Gateway.tmp` there, and the
...
def _start_attestation_path(home: Path | None = None) -> Path:
    ...
    return (home if home is not None else _nastech_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _write_start_attestation(pids: list[int], via: str, home: Path | None = None) -> None:
    ...
    newer one written by a concurrent ``nastech gateway start`` (#110020 review)."""
    try:
        path = _start_attestation_path(home)
        path.parent.mkdir(parents=True, exist_ok=True)
        from nastech_cli.process_identity import _process_create_time
...
# meant to bridge the seconds between a ✓ and the next ``nastech gateway status``/``update``; a
...
def _attested_pid_exited_cleanly(pid: int, create_time: float | None = None, home: Path | None = None) -> bool:
    ...
    try:
        ...
        sentinel = get_lifecycle_sentinel_path(home if home is not None else _nastech_home())
...
def attested_death_generation(current_pids: list[int], home: Path | None = None) -> str | None:
    ...
    one-shot marker — ``nastech update`` consults it to decide whether a Desktop-owned install
...
def _format_attestation_warning(attested: list[int], data: dict) -> str:
    ...
    lines = [
        ...
        "shutdown record.",
        "  This usually means the shell that ran `nastech gateway start` was inside "
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        print(f"  Check the log for startup errors:\n    type {_nastech_home()}\\logs\\gateway.log\n    type {_nastech_home()}\\logs\\gateway-stdio.log")
...
def _print_next_steps() -> None:
    ...
    print(f"  type {_nastech_home()}\\logs\\gateway.log       # View logs")
...
def _print_scheduled_task_drift(task_name: str) -> None:
    ...
    repair runs from ``start()`` / ``nastech update`` via ``reconcile_scheduled_task``)."""
    drift = scheduled_task_drift(task_name)
    if drift:
        print(f"⚠ Scheduled Task registration predates the current template ({'; '.join(drift)})")
        print("  Repair: nastech gateway start  (or: nastech gateway install)")
...
def reconcile_scheduled_task(task_name: str) -> bool:
    ...
    ``schtasks`` refused (typically Access Denied — the elevating ``nastech gateway install`` is the fallback)."""
    drift = scheduled_task_drift(task_name)
    if not drift:
        return False
    ...
    if not ok:
        print("  Repair manually: nastech gateway install")
    return ok
...
def start() -> None:
    ...
    if not is_task_registered() and not is_startup_entry_installed():
        ...
        # answer — the NASTECH_GATEWAY_INSTALL_START_ON_LOGIN override or a real TTY prompt — never on a
        # non-TTY default (#113977). Declining still starts the gateway; the command is ``start``.
        start_on_login = _install_choice_from_env("NASTECH_GATEWAY_INSTALL_START_ON_LOGIN")
        if start_on_login is None:
            from nastech_cli.setup import is_interactive_stdin, is_noninteractive, prompt_yes_no
        ...
        if start_on_login:
            ...
            return
        print("ℹ Login auto-start not installed; add it later with: nastech gateway install")
...
def restart() -> None:
    ...
    from nastech_cli.gateway import _wait_for_api_server_port_free  # avoid circular init
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `nastech gateway status`."
        )
