...
def _preserve_nastech_home_path(path: str | Path) -> str:
    """Render Nastech-owned paths under the configured NASTECH_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\nastech`` as a symlink/junction to
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    source checkout only if ``NASTECH_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - invokes ``python -m nastech_cli.main [--profile X] gateway run``
    ...
    to that terminal like a normal foreground ``nastech gateway run``.
    ...
    lines.append('set "NASTECH_SUPERVISED_CHILD=1"')
    ...
    lines.append(f'set "VIRTUAL_ENV={_preserve_nastech_home_path(venv_dir)}"')
    pythonpath_entries = [
    ...
    prog_args = [python_exe_path, "-m", "nastech_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_nastech_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_nastech_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('NASTECH_HOME')}) = {_quote_vbs_string(nastech_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('NASTECH_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('NASTECH_SUPERVISED_CHILD')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_nastech_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_nastech_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    nastech_home = str(Path(get_nastech_home()))
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, nastech_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    project_root = _preserve_nastech_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    nastech_home = str(Path(get_nastech_home()))
    profile_arg = _profile_arg(nastech_home)
    ...
    env_overlay = {
        ...
        "NASTECH_SUPERVISED_CHILD": "1",
        "VIRTUAL_ENV": _preserve_nastech_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_nastech_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    supplies the stable cwd + env overlay (NASTECH_HOME, VIRTUAL_ENV,
    PYTHONPATH) so the respawn doesn't depend on the watcher's transient
    working directory.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m nastech_cli.main
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``python.exe -m nastech_cli.main gateway run`` directly — NOT
...
def _start_attestation_path() -> Path:
    ...
    return Path(get_nastech_home()).joinpath(*_START_ATTESTATION_RELATIVE)
...
def _attested_pid_exited_cleanly(pid: int) -> bool:
    ...
    try:
        ...
        sentinel_path = get_lifecycle_sentinel_path(Path(get_nastech_home()))
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from nastech_cli.config import get_nastech_home
        print(f"    type {Path(get_nastech_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_nastech_home())}\\logs\\gateway-stdio.log")
        try:
            if is_task_registered():
                print(
                    f"  Recovery: schtasks /Run /TN {get_task_name()}   "
...
def _print_next_steps() -> None:
    ...
    nastech_home = Path(get_nastech_home())
    print()
    print("Next steps:")
