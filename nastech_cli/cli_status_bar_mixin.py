...
class CLIStatusBarMixin:
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if _fields is not None and "git_branch" in _fields:
                from nastech_cli.status_bar_git import current_git_branch
    ...
    def _pet_resolve_config(self) -> None:
        """(Re)resolve the active pet from config so ``/pet`` / ``nastech pets`` changes apply
    ...
    def _status_bar_segments(
        ...
        if _ok("model"):
            if styled:
                segs.append([(_SB, " 𓄃 "), (_STRONG, model_short)])
            else:
                segs.append([("", f"𓄃 {model_short}")])
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            parts = ["".join(t for _, t in seg) for seg in segs] or [f"𓄃 {model_short}"]
    ...
    def _get_status_bar_fragments(self):
        ...
        try:
            ...
            frags: list = []
            for seg in segs or [[(_SB, " 𓄃 "), (_STRONG, snapshot["model_short"])]]:
                if frags:
            ...
            # Battery is the first element when enabled: prepend ahead of the 𓄃 marker.
            battery_label = snapshot.get("battery_label") or ""
