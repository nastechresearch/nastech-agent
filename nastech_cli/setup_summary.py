...
resolved through the module object so test patches on ``nastech_cli.setup.<name>`` take effect."""
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'nastech setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'nastech setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'nastech tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("nastech setup", "          Re-run the full wizard"), ("nastech setup model", "    Change model/provider"),
    ("nastech setup terminal", " Change terminal backend"), ("nastech setup gateway", "  Configure messaging"),
    ("nastech setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("nastech config", "         View current settings"), ("nastech config edit", "    Open config in your editor"),
    ("nastech config set <key> <value>", ""))
_READY_ROWS = (
    ("nastech", "              Start chatting"), ("nastech gateway", "      Start messaging gateway"),
    ("nastech doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'nastech setup' to configure")


def _managed_or_provider_row(feature, name: str, managed_label: str, missing_hint: str):
    """Row for a Nastech-manageable feature: managed > available (with provider) > missing hint."""
...
def _web_row(config, feats):
    ...
    return _managed_or_provider_row(feats.web, "Web Search & Extract", "Nastech subscription", _WEB_MISSING)
...
def _browser_row(config, feats):
    ...
    return _managed_or_provider_row(feats.browser, "Browser Automation", "Nastech Browser Use", hint)


def _image_gen_row(config, feats):
    # FAL (direct or via Nastech), or any plugin-registered provider (OpenAI, etc.)
    if feats.image_gen.managed_by_nastech:
        return ("Image Generation (Nastech subscription)", True, None)
...
def _video_gen_row(config, feats):
    # Opt-in via `nastech tools` → Video Generation. Only show the row when a plugin reports
    # available so we don't badger users who don't care about video gen with a "missing" line.
    if feats.video_gen.managed_by_nastech:
        return ("Video Generation (FAL via Nastech subscription)", True, None)
...
def _tts_row(config, feats):
    ...
    if feats.tts.managed_by_nastech:
        return ("Text-to-Speech (OpenAI via Nastech subscription)", True, None)
...
def _stt_row(config, feats):
    ...
    if stt_feature is not None and stt_feature.managed_by_nastech:
        return ("Speech-to-Text (OpenAI via Nastech subscription)", True, None)
...
def _modal_row(config, feats):
    if feats.modal.managed_by_nastech:
        return ("Modal Execution (Nastech subscription)", True, None)
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'nastech setup terminal'")
    if tool_backend_helpers.managed_nastech_tools_enabled() and feats.nastech_auth_present:
        return ("Modal Execution (optional via Nastech subscription)", True, None)
    return None
...
def _spotify_row(config, feats):
    # OAuth via nastech auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, nastech_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Nastech cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    nastech model            (pick any provider/model)",
              "    nastech setup --portal   (Nastech Portal OAuth, no API key)")

    print()
    _setup.print_header("Tool Availability Summary")

    tool_status = []
    subscription_features = nastech_subscription.get_nastech_subscription_features(config)
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'nastech setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {nastech_home}/cron/, sessions/, logs/")):
