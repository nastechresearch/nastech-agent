...
from nastech_cli.status_auth import (  # renderers wired into _SECTIONS below
    _render_api_keys, _render_apikey_providers, _render_auth_providers, _render_nastech_gateway)
...
def _estop_status_line():
    """One-line pause banner for `nastech status`, or None when not paused."""
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{f' — reason: {reason}' if reason else ''}; `nastech resume` to lift)"
...
def _render_terminal(ctx):
    ...
    if terminal_env in _TERMINAL_ENV_ROWS:
        ...
        _kv_flag("SDK:", importlib.util.find_spec("vercel") is not None, "installed",
                 "missing (install: pip install 'nastech-agent[vercel]')")
        ...
        # (fail-soft — never break `nastech status`).
        try:
...
def _render_gateway(ctx):
    ...
    try:
        ...
        from nastech_cli.gateway_multiplex_served import multiplexer_served_secondaries
        ...
        if not snapshot.running and named_profile_served_by_running_multiplexer():
            ...
            _kv("Manage with:", "nastech gateway status   # from the default profile")
            return
        ...
        if snapshot.running and (served := multiplexer_served_secondaries()):
            ...
            from nastech_cli.gateway_multiplex_served import served_profile_ingress_urls
        ...
        if snapshot.has_process_service_mismatch:
            ...
            _kv("Start with:", "nastech gateway")
...
def _render_sessions(ctx):
    ...
    elif not (sessions_file := get_nastech_home() / "sessions" / "sessions.json").exists():
        _kv("Active:", 0)
    else:
        try:
...
def _render_footer(ctx):
    _banner(("─" * 60, "  Run 'nastech doctor' for detailed diagnostics", "  Run 'nastech setup' to configure"),
            Colors.DIM)
    print()


# Print order of `nastech status`; each renderer takes the shared _StatusContext.
_SECTIONS = (
    _render_header, _render_environment, _render_api_keys, _render_auth_providers, _render_nastech_gateway,
...
def show_status(args):
    ...
    # Shared by section renderers: config, --deep, and the Nastech login facts Auth Providers derives
    # for the later Nastech Tool Gateway section.
    ctx = SimpleNamespace(deep=getattr(args, 'deep', False), config={}, nastech_logged_in=False,
                          nastech_inference_present=False, nastech_account_info=None)
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_nastech_portal_account_info': ('nastech_cli.nastech_account', 'get_nastech_portal_account_info'),
