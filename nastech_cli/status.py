...
def _estop_status_line():
    ...
    Cheap: a single stat on $NASTECH_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `nastech resume` to lift)"
...
def show_status(args):
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `nastech status`
    ...
    print(
        f"  {'Nastech Portal':<12}  {check_mark(nastech_logged_in)} "
        f"{nastech_label}"
    )
    ...
    # disrupt the already-printed Nastech/Codex/Qwen/MiniMax rows above.
    try:
    ...
    if managed_nastech_tools_enabled():
        ...
        for feature in features.items():
            ...
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_nastech)} {state}")
    elif nastech_logged_in or nastech_inference_present:
        # Nastech OAuth without entitlement, or an opaque inference key without
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'nastech-agent[vercel]')"
        ...
        # provider's doctor rows (fail-soft — never break `nastech status`).
        try:
