...
from nastech_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See nastech_constants.get_subprocess_home().
    "home",
]
...
#   backups             — `nastech backup` archives
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Nastech home
    (:func:`nastech_constants.get_nastech_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, nastech subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    try:
        ...
        # not be migrated. The next `nastech doctor --fix` can still surface the
...
# Real wrappers are a few hundred bytes of shell; the needle (``nastech -p X``)
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        # Creation must not fail over this; `nastech model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``NASTECH_HOME``.
    """
    try:
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, nastech_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own NASTECH_HOME. When
    ...
    The returned ``nastech_home`` is the path to pass to
    ``set_nastech_home_override`` when scoping a turn to that profile.
    """
...
def create_profile(
    ...
    # Keys pages, `nastech -p <name> auth add`) had no file until first
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    only the essential skills (e.g. ``nastech-agent``), reporting
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    ...
    # Never terminate ourselves or a parent (e.g. `nastech -p <canon> profile
    ...
    nastech_markers = ("nastech_cli.main", "nastech-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('nastech')` resolution) spawns the
    ...
    # interpreter, not "nastech", so the checks below would otherwise miss it.
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Nastech process: either an entrypoint marker in argv, a
            # resolved executable named `nastech`, or a python interpreter
            # directly exec'ing a `nastech`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_nastech = (
                any(marker in joined for marker in nastech_markers)
                or exe_name == "nastech"
                or exe_name.startswith("nastech")
            )
            if not is_nastech and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: nastech, nastech-agent,
                # nastech-acp) rather than a bare `startswith("nastech")` --
                ...
                # "nastech-notes.py" or "nastech-unrelated-tool" as the shim,
                ...
                is_nastech = script_stem in _NASTECH_CONSOLE_SCRIPT_NAMES
            if not is_nastech:
                continue
            ...
            # ...or by NASTECH_HOME env pointing at this profile dir.
            if not bound:
                try:
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_nastech_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``NASTECH_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal (#89315): the record's nastech_home stamp
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    ``NASTECH_HOME`` points inside a checkout but the process runs from
...
def _profile_export_directory() -> Path:
    ...
    # A custom deployment may point NASTECH_HOME at its source checkout.  Do
    ...
    # Per-uid temp name: a fixed /tmp/nastech-profile-exports is a predictable
    # shared path another local user could pre-create (or symlink) before us.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".nastech-profile-exports",
        Path(tempfile.gettempdir()) / f"nastech-profile-exports{uid_suffix}",
    )
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``nastech sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``NASTECH_REDACT_SECRETS`` — share archives
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    When NASTECH_HOME is already set, the configured spelling IS the
    ...
    it to match Nastech-owned PYTHONPATH entries written in the same
    ...
    if env_home:
        ...
        # (mirrors get_default_nastech_root()).
