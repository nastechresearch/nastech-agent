...
from nastech_cli.archive_safe import archive_root_dirs, make_targz, normalize_archive_parts, safe_extract_targz
...
# stay visible; containers persist HOME state here). See nastech_constants.get_subprocess_home().
...
# (``~/.nastech``): git checkout (+ ~3 GB venv), worktrees, sibling profiles, shared bins,
...
# ``nastech_cli.backup._EXCLUDED_ROOT_DIRS`` so the two lists cannot drift.
...
# (+wal/shm, can reach many GB), session dirs, `nastech backup` archives, quick-backup
...
# Marker written by `nastech profile create --no-skills`. When present at a profile root,
# seed_profile_skills() callers (fresh-create, `nastech update` all-profile sync, the
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    if source_resolved == _get_default_nastech_home().resolve():
...
# Allow-list for ``export_profile("default")``: when NASTECH_HOME equals the cwd
# (Docker/custom deployments) the default home holds arbitrary user files that must NOT
# be bundled. Only known Nastech profile artifacts at the root survive; sensitive runtime
...
# when introduced in ``nastech_constants``.
# See #58394.
...
_RESERVED_NAMES = frozenset({"nastech", "default", "test", "tmp", "root", "sudo"})
...
def _get_profiles_root() -> Path:
    """Named-profiles root, anchored to the nastech root (NOT the current NASTECH_HOME, which
...
def _get_default_nastech_home() -> Path:
    """Default (pre-profile) NASTECH_HOME: ``~/.nastech``, or NASTECH_HOME itself in
...
def _is_our_wrapper(path: Path) -> bool:
    """True when *path* reads as a Nastech-generated wrapper (contains ``nastech -p``)."""
    try:
        return "nastech -p" in path.read_text(encoding="utf-8")
    except Exception:
        return False


def _missing_profile_error(canon: str) -> FileNotFoundError:
    return FileNotFoundError(f"Profile '{canon}' does not exist. Create it with: nastech profile create {canon}")
...
def _unknown_profile_error(canon: str) -> FileNotFoundError:
    ...
    return FileNotFoundError(f"No profile named '{canon}'. See your profiles with: nastech profile list")


def _profile_exists_error(canon: str) -> FileExistsError:
    return FileExistsError(
        f"A profile named '{canon}' already exists. Switch to it with `nastech profile use {canon}`, "
        "see all profiles with `nastech profile list`, or choose a different name."
    )
...
def _invalid_profile_name_error(name: str) -> ValueError:
    ...
    return ValueError(
        ...
        f"Then run `nastech profile create {suggestion}`."
    )
...
def get_profile_dir(name: str) -> Path:
    ...
    # profiles/nastech may still exist and must keep resolving.
...
def _iter_named_profile_dirs(*, live_only: bool = True) -> List[Path]:
    ...
    shell into a "real" profile on the next ``nastech update`` (#95188, #94823, #99392)."""
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    if not (profile_dir / "config.yaml").exists():
        return
    # Creation must not fail over an unmigratable old config; `nastech doctor --fix` surfaces
...
# wrappers are a few hundred bytes with the ``nastech -p X`` needle near the top; the wrapper
...
class ProfileInfo:
    ...
    display_name: str = ""
    # Bot Mode title (``profile.yaml`` ``ui_meta['nastech-bots'].title``) — the name
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    if config_path.exists():
        return
    with contextlib.suppress(Exception):  # creation must not fail over this; `nastech model` sets it later
        import yaml
        from nastech_constants import get_nastech_home
        from nastech_cli.config import read_user_config_raw
        source = get_nastech_home() / "config.yaml"
...
def _check_gateway_running(profile_dir: Path) -> bool:
    """Gateway liveness for a profile dir, never mutating NASTECH_HOME.
...
def _served_by_running_multiplexer(profile_name: str) -> bool:
    ...
    """
    try:
        from nastech_cli.gateway import named_profile_served_by_running_multiplexer
...
def _count_skills(profile_dir: Path) -> int:
    ...
    synchronously when stale — detail surfaces (``nastech profile info``, ``profiles.describe``)
...
def _cached_skill_count(profile_dir: Path) -> int:
    ...
    if due:
        threading.Thread(target=_count_skills, args=(profile_dir,),
                         name="nastech-skill-count", daemon=True).start()
    cached = _SKILL_COUNT_CACHE.get(key)
    return cached[2] if cached is not None else 0


# profile.yaml — per-profile metadata (description, role, etc.)
# Deliberately tiny and separate from ``config.yaml`` (user-facing Nastech config, ~5000
...
def read_profile_meta(profile_dir: Path) -> dict:
    ...
    break ``nastech profile list``."""
    data = _load_yaml_dict(profile_dir / "profile.yaml") or {}
    ui_meta = data.get("ui_meta")
    bot_title = ""
    if isinstance(ui_meta, dict):
        nastech_bots = ui_meta.get("nastech-bots")
        if isinstance(nastech_bots, dict):
            bot_title = str(nastech_bots.get("title") or "").strip()
    return {
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """``(profile_name, nastech_home)`` pairs a gateway should serve — the single chokepoint
    ...
    serve: List[Tuple[str, Path]] = [("default", _get_default_nastech_home())]
...
def _clone_all_into(source_dir: Path, profile_dir: Path, canon: str) -> None:
    ...
    from nastech_cli.auth import strip_cloned_single_use_oauth_grants
    ...
    if any(stripped.values()):
        logger.info(
            ...
            "(run `nastech -p %s auth add <provider>` to sign in)", canon, stripped, canon,
        )
...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path],
    ...
    ``sync_imports`` also copies the source's ``import-sync.json`` (the ``nastech import-agent``
    manifest) so the clone stays registered against the same external Claude Code / Codex trees
    and ``nastech -p <clone> import-agent --sync`` keeps pulling from them. The link is to the
    ...
    for relpath in _CLONE_SUBDIR_FILES:
        _clone_file(source_dir, profile_dir, relpath)
    if sync_imports:
        from nastech_cli.agent_import_sync import SYNC_MANIFEST_NAME  # lazy: keeps yaml/utils off the hot startup path
...
def create_profile(
    ...
    makes two gateways fight over one bot (``nastech_cli.profile_channels``; callers list what
    was left behind with ``channel_platforms_configured(source_dir)``).
    ``no_skills`` creates an empty profile and writes a marker so ``nastech update`` skips
    ...
    canon = _canon_valid(name)
    if canon == "default":
        raise ValueError("Cannot create a profile named 'default' — it is the built-in profile (~/.nastech).")
    profile_dir = get_profile_dir(canon)
    ...
    if source_dir is not None and clone_channels:
        from nastech_cli.profile_channels import clone_channels_refusal
    ...
    try:
        ...
        if source_dir is not None and not clone_channels:
            from nastech_cli.profile_channels import strip_channel_settings
    ...
    # `nastech -p <profile> gateway start` supervises via `s6-svc -u` instead of a bare
...
def _finish_profile_layout(profile_dir: Path, *, no_skills: bool, clone_all: bool,
    ...
    # profile-scoped env writes (dashboard Channels/Keys pages, `nastech -p <name> auth add`)
    ...
    # Opt-out marker read by seed_profile_skills() and `nastech update`'s all-profile sync
    # (the feature still works via the empty skills/ dir if this fails).
    if no_skills:
        _seed_file_if_missing(
            profile_dir / NO_BUNDLED_SKILLS_MARKER,
            "This profile opted out of bundled-skill seeding (`nastech profile create --no-skills`).\n"
    ...
    if description and description.strip():
        with contextlib.suppress(Exception):  # non-fatal — `nastech profile describe` works later
...
def _notify_multiplexer(canon: str) -> None:
    from nastech_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
...
def _purge_identity(canon: str) -> bool:
    ...
    from nastech_cli.profile_identity import purge_profile_identity
...
def _live_default_multiplexer() -> bool:
    ...
    try:
        from nastech_cli.gateway_multiplex_served import recorded_served_profiles
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    """Seed bundled skills into a profile via subprocess (sync_skills() caches NASTECH_HOME at
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    """
    backfilled: List[str] = []
    default_env = _get_default_nastech_home() / ".env"
...
_NASTECH_ARGV_MARKERS = ("nastech_cli.main", "nastech-gateway", "tui_gateway")
...
# `findOnPath('nastech')`) spawns the shim by handing the interpreter its path — then the
# OS-reported argv[0] is the interpreter, not "nastech".
...
# argv[1] is matched against exact names, not ``startswith("nastech")``: with a bare
# interpreter argv[0], argv[1] can be ANY user script ("nastech-notes.py").
_NASTECH_CONSOLE_SCRIPT_NAMES = frozenset({"nastech", "nastech-agent", "nastech-acp"})


def _is_nastech_argv(argv: list) -> bool:
    """True for a Nastech process: entrypoint marker in argv, executable named ``nastech*``,
    or a python interpreter directly exec'ing a known ``nastech`` console-script shim."""
    joined = " ".join(argv)
    exe_name = os.path.basename(argv[0]).lower()
    if any(marker in joined for marker in _NASTECH_ARGV_MARKERS) or exe_name.startswith("nastech"):
        return True
    if len(argv) >= 2 and _PYTHON_INTERPRETER_RE.match(exe_name):
        script_name = os.path.basename(str(argv[1])).lower()
        return script_name.rsplit(".", 1)[0] in _NASTECH_CONSOLE_SCRIPT_NAMES
    return False
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Nastech *backends* bound to this profile (``gateway.pid`` only tracks
    ...
    # Never terminate ourselves or a parent (`nastech -p <canon> profile delete` runs under
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            if not argv or not _is_nastech_argv(argv):
                continue
            if not ({tok.lower() for tok in argv} & _BACKEND_TOKENS):
                continue

            # Bound to THIS profile by selector flag, or by NASTECH_HOME pointing at its dir.
            ...
            if not bound:
                with contextlib.suppress(Exception):  # environ() can raise AccessDenied even same-user
                    env_home = (proc.environ() or {}).get("NASTECH_HOME", "")
...
class ProfileIdentitySettlementPending(RuntimeError):
    ...
    settled (``nastech_cli.profile_identity.purge_profile_identity`` returned False).

    Subclasses ``RuntimeError`` so delete-failure handling that treats the error as fatal (the
    CLI's ``nastech profile delete``) keeps working unchanged; surfaces that can report a partial
    ...
    def __init__(self, profile: str, path: Path):
        ...
        self.path = path
        self.retry_command = f"nastech profile purge-identity {profile}"
        super().__init__(
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    canon = normalize_profile_name(name)
    if canon == "default":
        raise ValueError("Cannot delete the default profile (~/.nastech).\nTo remove everything, use: nastech uninstall")
    ...
    shutdown_mcp_servers(scope=nastech_home_key(profile_dir))
    ...
    with contextlib.suppress(Exception):
        from nastech_state_registry import close_all_under as _close_session_dbs_under
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    Port selection: each supervised profile gateway loads its own ``NASTECH_HOME`` and binds the port
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # NASTECH_HOME is set temporarily so _profile_suffix resolves the service name.
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal: the record's nastech_home stamp names the gateway's TRUE
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded NASTECH_HOME "
...
def get_active_profile(root: Path | None = None) -> str:
    """Read the sticky active profile name (of *root*, default: this process's Nastech root)."""
...
def get_active_profile_name() -> str:
    """Profile name inferred from NASTECH_HOME: ``"default"`` when unset or ``~/.nastech``, the
    name under ``~/.nastech/profiles/<name>``, ``"custom"`` for any other path."""
    from nastech_constants import get_nastech_home
    resolved = get_nastech_home().resolve()
    if resolved == _get_default_nastech_home().resolve():
        return "default"
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    (not cwd) so the check holds when NASTECH_HOME sits in a checkout but the process runs
...
def _profile_export_directory() -> Path:
    ...
    import tempfile
    export_dir = _get_default_nastech_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point NASTECH_HOME at its source checkout: use a sibling store,
    # falling back to the OS temp dir only when the user's home itself is a checkout (dotfiles
    # repo). Per-uid temp name: a fixed /tmp/nastech-profile-exports is a predictable shared
    # path another local user could pre-create (or symlink) first.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".nastech-profile-exports", Path(tempfile.gettempdir()) / f"nastech-profile-exports{uid_suffix}"
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in ``_DEFAULT_EXPORT_INCLUDE_ROOT``
    ...
    NASTECH_HOME equals the cwd) is excluded. Blacklisting was tried first and proved unable to anticipate
    every non-Nastech file the user may have lying alongside NASTECH_HOME (#58394). * **Universal exclusions
...
def _scrub_export_secrets(staged: Path) -> None:
    """Force-redact secret-shaped strings in a staged export tree (same pass as ``nastech
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    # The default profile IS ~/.nastech (dir name ".nastech"), so both paths stage a filtered
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json", _get_default_nastech_home() / "honcho.json", Path.home() / ".honcho" / "config.json"
    ]
    seen: set[Path] = set()
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            block["aiPeer"] = old_name  # source_host is ``nastech_<old>`` or legacy ``nastech.<old>``
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    # ``mkdir_under_nastech_home`` does not refuse it) and the periodic reconcile re-adopts the
    ...
    from nastech_constants import nastech_home_key
    from tools.mcp_tool_lifecycle import shutdown_mcp_servers
    shutdown_mcp_servers(scope=nastech_home_key(old_dir))
    ...
    from nastech_cli.profile_identity import _migrate_profile_identity
...
def profile_root_for_env_home(env_home: str, default_root: Path) -> Path:
    """Nastech root named by an exported ``NASTECH_HOME``: the grandparent of a profile-shaped value
    (``<root>/profiles/<name>``, mirrors ``get_default_nastech_root()``), the value itself otherwise,
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a NASTECH_HOME path string. Called early in the CLI entry
    point, before nastech modules are imported, to set NASTECH_HOME.

    When NASTECH_HOME is already set, the configured spelling IS the launch root (it may be a
    junction/symlink alias of the platform default). Keep that spelling so profile re-home does not destroy
    the launcher's lexical provenance -- the subprocess sanitizer needs it to match Nastech-owned PYTHONPATH
    ...
    root = profile_root_for_env_home(os.environ.get("NASTECH_HOME", ""), _get_default_nastech_home())
    if canon == "default":
        return str(root)
