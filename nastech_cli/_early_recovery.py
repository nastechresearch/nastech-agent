"""Dependency-light venv recovery that runs BEFORE nastech_cli.main's imports.

Deliberately **stdlib-only** so importing it can never fail on a corrupted venv. ``nastech_cli.main``
calls :func:`recover_if_needed` at the very top of its module body, before any third-party import.
Scope: repair only enough for ``nastech_cli.main`` to become importable again (force-reinstall of
...
# ``nastech update`` renames the live ``nastech*.exe`` shims aside (``nastech.exe.old.<unix-ms>``) so
# uv can write replacements. Putting them BACK is the safety-critical direction: losing that rename
# leaves no ``nastech`` on PATH, and the command that would repair it IS ``nastech update``. The
...
def _should_skip_external_secret_sources() -> bool:
    """True inside any ``nastech update`` process (and its import probes).

    Every dotenv load in the process — ``nastech_cli.main``, ``run_agent``, ``cli`` — consults
    ...
    Profile flags are stripped before ``nastech_cli.main`` loads dotenv, so ``argv[1]`` is
...
def _find_uv_binary() -> str | None:
    ...
    fallback refuses to touch them; the only sanctioned installer is then uv itself, which Nastech
    vendors (``~/.nastech/bin/uv.exe``) or the user has on PATH.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    for sub in ((".nastech", "bin"), (".local", "bin"), (".cargo", "bin")):
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``nastech acp`` speaks JSON-RPC on stdout). uv is preferred when the
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``nastech_cli.main`` with recovery armed and
...
def recover_if_needed(project_root: Path | None = None, argv: list[str] | None = None) -> None:
    """Repair wiped core packages so ``nastech_cli.main`` can import at all.

    Fast path (no marker present) is two ``lstat`` calls. Only acts when a recovery marker from a
    prior ``nastech update`` exists AND an import probe confirms a core package is actually broken.
    ...
    try:
        ...
        # recovered even when this launch is itself `nastech update`: CLI and Desktop retries keep
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    ``recover_if_needed`` invokes this when ``.update-incomplete`` exists — a prior ``nastech update`` (or
    ...
    venv extension mapped; this process, running before ``nastech_cli.main``'s third-party imports, maps
    nothing yet, so the installer can replace ``.pyd`` files without hitting the lock.
    """
    try:
        from nastech_cli import _install_repair as ir
        ...
        if not _claim_recovery_lock(root):
            return False
        try:
            print("⚠ A previous `nastech update` was interrupted mid-install — "
            ...
            print("  The next launch will retry; nastech will keep working from "
