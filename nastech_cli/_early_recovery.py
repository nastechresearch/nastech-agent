...
The ``nastech`` console entry point is ``nastech_cli.main:main``.  Importing
``nastech_cli.main`` pulls in third-party packages at module level (``dotenv``
via ``nastech_cli.env_loader``, ``yaml`` via ``nastech_cli.config``, ...).  In
...
a corrupted venv.  ``nastech_cli.main`` imports and calls
:func:`recover_if_needed` at the very top of its module body, before any
third-party import.

Scope: this early pass only repairs enough for ``nastech_cli.main`` to become
...
#
# ``nastech update`` renames the live ``nastech*.exe`` shims aside
# (``nastech.exe.old.<unix-ms>``) so uv can write replacements. Putting them BACK
# is the safety-critical direction: losing that rename leaves the install with
# no ``nastech`` on PATH, and the command that would repair it IS ``nastech
# update`` (#75584).
#
...
def restore_quarantined_shims(
    ...
    *,
    ...
    Messages go to stderr by default -- the startup sweep runs on EVERY nastech
    invocation, and ``nastech acp`` speaks JSON-RPC on stdout.
    """
    if stream is None:
        stream = sys.stderr
...
def _find_uv_binary() -> str | None:
    ...
    the only sanctioned installer is uv itself, which Nastech already vendors
    (``~/.nastech/bin/uv.exe``) or the user has on PATH.  Stdlib-only.
    """
    exe = "uv.exe" if sys.platform == "win32" else "uv"
    candidates = [
        Path.home() / ".nastech" / "bin" / exe,
...
def _run_repair_install(specs: list[str], project_root: Path) -> bool:
    ...
    Streams nothing to stdout (``nastech acp`` speaks JSON-RPC on stdout);
...
def _pytest_owns_live_checkout(root: Path) -> bool:
    ...
    Lifecycle tests spawn real subprocesses that import ``nastech_cli.main``
...
def recover_if_needed(
    ...
    recovery marker from a prior ``nastech update`` exists AND an import probe
    ...
    try:
        ...
        # launch is itself `nastech update`: CLI and Desktop retries preserve
...
# every `nastech` launch: past this many attempts the early pass hands the
...
def _complete_pending_core_install(root: Path, core_marker: Path) -> bool:
    ...
    a prior ``nastech update`` (or the self-lock preflight, #83569) left the
    ...
    ``nastech_cli.main``'s third-party imports, maps nothing yet, so the
    ...
    failure does not block every launch (``nastech acp`` included).
    ...
    try:
        ...
        try:
            ...
            print(
                "  The next launch will retry; nastech will keep working from "
