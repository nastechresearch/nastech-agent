...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    API-key prompt for users who logged in via ``nastech model`` -> xAI Grok OAuth."""
    try:
...
def _xai_api_key_path():
    ...
    return ("No xAI API key provided for TTS. Configure XAI_API_KEY via nastech setup model "
...
def _setup_tts_provider(config: dict):
    ...
    if tool_backend_helpers.managed_nastech_tools_enabled() and nastech_subscription.get_nastech_subscription_features(config).nastech_auth_present:
        options.insert(0, ("nastech-openai",
                           "Nastech Subscription (managed OpenAI TTS, billed to your subscription)"))
    ...
    if selected == "nastech-openai":
        selected = "openai"
        _setup.print_info("OpenAI TTS will use the managed Nastech gateway and bill to your subscription.")
        if _setup.get_env_value("VOICE_TOOLS_OPENAI_KEY") or _setup.get_env_value("OPENAI_API_KEY"):
            _setup.print_warning("Direct OpenAI credentials are still configured and may take precedence "
                                 "until removed from ~/.nastech/.env.")
