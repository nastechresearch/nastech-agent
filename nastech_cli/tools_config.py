...
from nastech_cli.config import cfg_get, load_config, save_config, get_env_value
from nastech_cli.nastech_subscription import (
    NastechSubscriptionFeatures, apply_nastech_managed_defaults, get_nastech_subscription_features)
from nastech_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
from nastech_cli.toolset_scope import (
    _TOOLSET_PLATFORM_RESTRICTIONS, toolset_allowed_for_platform as _toolset_allowed_for_platform)
# Re-exports: keep ``nastech_cli.tools_config.X`` callers and test patch targets resolving.
from nastech_cli.tools_config_cua import (  # noqa: F401
...
from nastech_cli.tools_config_post_setup import (  # noqa: F401
...
from nastech_cli.tools_config_providers import (  # noqa: F401
...
from nastech_cli.tools_config_mcp import (  # noqa: F401
...
# OFF by default for new installs (still in _NASTECH_CORE_TOOLS; the checklist won't pre-select them). x_search
...
# Config-only capabilities: provider setup in `nastech tools` (TOOL_CATEGORIES) but not model toolsets — zero
...
def _get_plugin_toolset_keys() -> set:
    ...
    try:
        ...
        from nastech_cli.plugins import get_plugin_toolset_keys_nowait
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Toolset keys the ``nastech tools`` checklist offers for ``platform`` (mirrors ``_prompt_toolset_checklist``);
...
def _platform_default_toolset(platform: str) -> str:
    """Composite toolset a platform falls back to (plugin platforms derive ``nastech-<platform>``)."""
    return PLATFORMS[platform]["default_toolset"] if platform in PLATFORMS else f"nastech-{platform}"
...
def _row(name: str, badge: str = "", tag: str = "", env_vars: list = (), **markers) -> dict:
    """One TOOL_CATEGORIES provider row; ``markers`` are the ``*_provider`` / ``post_setup`` / Nastech keys."""
    row = {"name": name}
    if badge:
        row["badge"] = badge
    if tag:
...
_NASTECH = {"requires_nastech_auth": True}
...
_LANGFUSE_PUBLIC = ("NASTECH_LANGFUSE_PUBLIC_KEY", "Langfuse public key (pk-lf-...)")
_LANGFUSE_SECRET = ("NASTECH_LANGFUSE_SECRET_KEY", "Langfuse secret key (sk-lf-...)")
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            _row("Nastech Subscription", "subscription", "Managed OpenAI TTS billed to your subscription", tts_provider="openai",
                 **_NASTECH, managed_nastech_feature="tts", override_env_vars=["VOICE_TOOLS_OPENAI_KEY", "OPENAI_API_KEY"]),
    ...
    "stt": {
        ...
        "providers": [
            ...
            _row("Nastech Subscription", "subscription", "Managed OpenAI transcription billed to your subscription",
                 stt_provider="openai", **_NASTECH, managed_nastech_feature="stt",
    ...
    "web": {
        ...
        # non-provider firecrawl setup-flow rows live here: managed via Nastech subscription, and self-hosted.
        "providers": [
            {"name": "Nastech Subscription", "badge": "subscription", "tag": "Managed Firecrawl billed to your subscription",
             "web_backend": "firecrawl", "env_vars": [], **_NASTECH, "managed_nastech_feature": "web",
    ...
    "image_gen": {
        ...
        # _plugin_image_gen_providers(). Only the managed "Nastech Subscription" row lives here: ONE row for the
        # FAL, Krea and Portal gateways, whose union catalog is `imagegen_backend: "nastech"`; the stored model id
        # picks the gateway at run time (tools/image_generation_managed.py).
        "providers": [
            _row("Nastech Subscription", "subscription",
                 "Managed image generation (FAL, Krea 2, Nastech Portal models) billed to your subscription", **_NASTECH,
                 managed_nastech_feature="image_gen", override_env_vars=["FAL_KEY"], imagegen_backend="nastech"),
        ],
    },
    "video_gen": {
        "name": "Video Generation", "icon": "🎬",
        # Mirrors image_gen: managed FAL video billed via the Nastech Portal. Plugin-backed rows (FAL BYOK, xAI, …)
        ...
        "providers": [
            _row("Nastech Subscription", "subscription", "Managed FAL video generation billed to your subscription", **_NASTECH,
                 managed_nastech_feature="video_gen", override_env_vars=["FAL_KEY"], video_gen_plugin_name="fal"),
        ],
    },
    "x_search": {
        "name": "X (Twitter) Search", "setup_title": "Select xAI Credential Source",
        "setup_note": (
            "Nastech routes X searches through xAI's built-in x_search Responses tool for read-only public X "
    ...
    "browser": {
        ...
        # first so a fresh install's Enter lands on the free local backend (index 0), never on the paid Nastech row.
        ...
        "providers": [
            ...
            _row("Lightpanda", "free · local · no Chromium", "Zig headless browser spawned by Nastech, text-only (no screenshots)",
            ...
            _row("Nastech Subscription (Browser Use cloud)", "subscription", "Managed Browser Use billed to your subscription",
                 browser_provider="browser-use", **_NASTECH, managed_nastech_feature="browser",
    ...
    "computer_use": {
        ...
        # cua-driver reads HOME/TMPDIR from the process env; NASTECH_CUA_DRIVER_CMD selects a specific
        ...
        "providers": [
            ...
            _row("Langfuse Self-Hosted", tag="Self-hosted Langfuse instance", post_setup="langfuse",
                 env_vars=[_key(*_LANGFUSE_PUBLIC), _key(*_LANGFUSE_SECRET),
                           _key("NASTECH_LANGFUSE_BASE_URL", "Langfuse server URL (e.g. http://localhost:3000)", default="http://localhost:3000")]),
        ],
    },
}
...
def enabled_mcp_server_names(config: dict) -> Set[str]:
    ...
    try:
        from nastech_cli.plugins import get_portable_mcp_server_names_nowait
...
#: "declined": saving ``nastech tools`` freezes a platform's composite into an explicit list nothing adds to, so
#: a later toolset stays off forever for picker users while ``[nastech-cli]`` users inherit it.
...
def _enable_recently_shipped_toolsets(enabled_toolsets: Set[str], config: dict, platform: str) -> None:
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # composites (nastech-acp, nastech-webhook) stay narrow.
...
def _explicit_toolsets(
    ...
    ``nastech-cli`` would re-enable disabled toolsets). A mixed list (``[nastech-cli, spotify]``) still expands the
...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly saved list (even a composite like ``nastech-discord``) is an opt-in to the platform's
    ...
    # Plugin toolsets are first-class on a saved list: ``[nastech-cli, a2a]`` must survive filtering.
    # Plugin-provided toolsets are first-class on a platform-toolsets list — explicit config like
    # ``[nastech-cli, a2a]`` must survive filtering just like a built-in configurable toolset would. See
    # issue #81163.
    ...
    # above. It may arrive as a JSON-array string ("['memory']") from `nastech config set` or a JSON-mode editor.
...
def _prune_toolsets_stripped_by_disabled(enabled_toolsets: Set[str], disabled_names: List[str]) -> Set[str]:
    ...
    never appear in the list. A name-only subtraction here left inspection surfaces (``nastech tools
...
def _recover_platform_native_toolsets(enabled_toolsets: Set[str], platform: str, *, skip: Set[str]) -> None:
    ...
    skip = skip | {k for k in TOOLSETS if k.startswith("nastech-")} | (set(_DEFAULT_OFF_TOOLSETS) - {platform})
...
def _warn_all_invalid_platform_toolsets(platform: str, explicit: list) -> None:
    """Warn once when an explicit platform list has only invalid names (``nastech`` for ``nastech-cli`` → no
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # names): platform defaults (nastech-cli, ...) resolve to ALL tools and would silently override the user's
...
def _toolset_has_keys(
    ts_key: str, config: dict = None, *, force_fresh: bool = False, features: Optional[NastechSubscriptionFeatures] = None,
) -> bool:
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    state = "off" if not enabled else ("collecting + sending to Nastech" if send else "collecting locally")
...
def _print_tools_summary(config: dict, enabled_platforms: List[str]) -> None:
    """``nastech tools --summary``: enabled toolsets per platform, non-interactive."""
    total = len(_get_effective_configurable_toolsets())
    print(color("𓄃 Tool Summary", Colors.CYAN, Colors.BOLD))
    print()
...
def _first_install_flow(config: dict, enabled_platforms: List[str]) -> None:
    ...
    for pkey in enabled_platforms:
        ...
        auto_configured = apply_nastech_managed_defaults(config, enabled_toolsets=new_enabled, force_fresh=True)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `nastech tools` / `nastech setup tools`. ``first_install`` skips the menu (checklist + key
...
_PLUGIN_COMPAT_LAZY = {
    'MANAGED_FEATURE_COVERAGE_CATEGORY': ('nastech_cli.nastech_subscription', 'MANAGED_FEATURE_COVERAGE_CATEGORY'),
    'NASTECH_MANAGED_PROVIDER': ('tools.tool_backend_helpers', 'NASTECH_MANAGED_PROVIDER'),
    'base_url_hostname': ('utils', 'base_url_hostname'),
    'fal_key_is_configured': ('tools.tool_backend_helpers', 'fal_key_is_configured'),
    'format_nastech_portal_entitlement_message': ('nastech_cli.nastech_account', 'format_nastech_portal_entitlement_message'),
