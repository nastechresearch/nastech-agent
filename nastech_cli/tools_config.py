    ...
    "browser": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "tag": "Zig headless browser spawned by Nastech, text-only (no screenshots)",
                "env_vars": [],
                "browser_provider": "local",
...
def _cua_version_summary(raw: str, *, limit: int = 120) -> str:
    ...
    A binary selected by ``NASTECH_CUA_DRIVER_CMD`` is not obliged to answer
...
def _cua_driver_contract_status(binary: Optional[str] = None) -> dict:
    """Inspect whether an installed driver supports Nastech' runtime contract."""
    import time
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
    ...
    ``nastech update`` and must not inherit the explicit install command's
    ...
    override = os.environ.get("NASTECH_CUA_DRIVER_CMD", "").strip()
    if override and not binary:
        _print_warning(
            "    NASTECH_CUA_DRIVER_CMD does not resolve to an executable: "
            f"{override}"
        )
        _print_info(
    ...
    # An installed driver that fails Nastech' runtime contract (version floor,
    # missing manifest verbs) is repaired regardless of the caller's mode.
    # Nastech' own minimum requirement IS the confirmation that an upgrade is
    ...
    # contract landed, with `nastech update` declining to refresh).
    ...
    if repair_existing:
        ...
        _print_warning(
            f"    Found cua-driver {version}, but Nastech cannot use its current "
            f"runtime contract: {reason}."
        )
        if os.environ.get("NASTECH_CUA_DRIVER_CMD", "").strip():
            _print_info(
        ...
        if is_windows and require_confirmed_update:
            ...
            _print_info(
                ...
                "nastech computer-use install --upgrade"
            )
            return False
    ...
    if ok and repair_existing:
        ...
        if not repaired.get("ready"):
            ...
            _print_info("    Run: nastech computer-use doctor")
            return False
    if ok and before:
        try:
...
# Optional refreshes launched by ``nastech update`` are quiet and unattended.
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    # Unattended refreshes (installer_timeout set by `nastech update`) fail
    ...
    def _reap_after_timeout(proc):
        ...
        ceiling stops bounding anything and ``nastech update`` hangs past its
...
def _ensure_browser_use_cli(*, verbose_hints: bool = False) -> None:
    ...
    Firecrawl, and the Nastech-managed cloud rows all execute through
    ...
    check — only the Nastech-managed ``$NASTECH_HOME/bin`` copy does.
    ``install_cli()`` short-circuits on the managed copy and otherwise
    provisions it, so resolution always lands on a binary Nastech installs
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key == "lightpanda":
        # Browser Use mode drives Lightpanda directly (Nastech spawns
        ...
        if lightpanda_bin:
            ...
            if os.name == "nt":
                _print_info("    Lightpanda has no native Windows build; run Nastech under WSL2.")
        return
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # (PATH -> Homebrew/Nastech-managed node -> npx) rather than a bare
        # shutil.which — Nastech-managed-Node-only setups resolve agent-browser
        ...
        # Nastech-managed-node -> npx cascade _find_agent_browser uses at
        ...
        if _is_npx_agent_browser_sentinel(browser_cmd):
            ...
            # — Nastech-managed-Node-only setups resolve npx only through the
...
def _get_platform_tools(
    ...
    *,
    ...
    # explicit config like ``[nastech-cli, a2a]`` must survive filtering just
...
# Python dependencies installed explicitly through ``nastech tools`` are not
...
def active_restorable_python_tool_dependencies() -> list[str]:
    """Return ``nastech tools`` Python dependencies present in this runtime."""
    return [
        name
    ...
    if managed_feature:
        ...
        if provider.get("tts_provider"):
            return (
                ...
                in {provider["tts_provider"], NASTECH_MANAGED_PROVIDER}
            )
        if provider.get("stt_provider"):
            return (
                feature.managed_by_nastech
                and cfg_get(config, "stt", "provider")
                in {provider["stt_provider"], NASTECH_MANAGED_PROVIDER}
            )
        if "browser_provider" in provider:
            ...
            return feature.managed_by_nastech and current in {
                provider["browser_provider"],
                NASTECH_MANAGED_PROVIDER,
            }
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return (
                feature.managed_by_nastech
                and current in {provider["web_backend"], NASTECH_MANAGED_PROVIDER}
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``use_gateway=True`` marks a provider picked through the Nastech-managed
    flow: the stored selection becomes ``image_gen.provider: nastech`` (the
    ...
    img_cfg["provider"] = NASTECH_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _select_plugin_video_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    ``video_gen.provider: nastech``; BYOK picks store the plugin name; any
    ...
    vid_cfg["provider"] = NASTECH_MANAGED_PROVIDER if use_gateway else plugin_name
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    post-setup hooks, gate on Nastech auth, or run interactive model pickers.
    ...
    category. Managed "Nastech Subscription" rows write ``nastech``; BYOK rows
    ...
    legacy shim (use_gateway: true ⇒ nastech) cannot override the new choice.
    """
    def _set_selection(section_key: str, name_key: str, vendor_value) -> None:
        ...
        section[name_key] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else vendor_value
        )
    ...
    # image_gen/video_gen "Nastech Subscription" rows carry only
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    # flow). Managed picks store the "nastech" selection.
    plugin_name = provider.get("image_gen_plugin_name")
    if plugin_name:
        img_cfg = config.setdefault("image_gen", {})
        if not isinstance(img_cfg, dict):
            ...
            config["image_gen"] = img_cfg
        img_cfg["provider"] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else plugin_name
        )
        img_cfg.pop("use_gateway", None)
    ...
    if video_plugin:
        ...
        if not isinstance(vid_cfg, dict):
            ...
            config["video_gen"] = vid_cfg
        vid_cfg["provider"] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else video_plugin
        )
        vid_cfg.pop("use_gateway", None)
...
def _configure_provider(
    ...
    *,
    ...
    if provider.get("tts_provider"):
        ...
        tts_cfg["provider"] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
        )
    ...
    if not env_vars:
        ...
        if backend:
            ...
            # explicit selection: "nastech" for a managed row, "fal" for BYOK.
            img_cfg = config.setdefault("image_gen", {})
            if isinstance(img_cfg, dict):
                img_cfg["provider"] = (
                    NASTECH_MANAGED_PROVIDER if managed_feature else "fal"
                )
                img_cfg.pop("use_gateway", None)
...
def _reconfigure_provider(
    ...
    *,
    ...
    # provider string per category — "nastech" for managed rows, the vendor name
    # for BYOK rows — and drops any legacy use_gateway key so the read-time
    # shim (use_gateway: true ⇒ nastech) cannot override the fresh pick.
    if provider.get("tts_provider"):
        tts_cfg = config.setdefault("tts", {})
        tts_cfg["provider"] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else provider["tts_provider"]
    ...
    if provider.get("stt_provider"):
        stt_cfg = config.setdefault("stt", {})
        stt_cfg["provider"] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else provider["stt_provider"]
        )
    ...
    if "browser_provider" in provider:
        ...
        browser_cfg = config.setdefault("browser", {})
        if managed_feature:
            browser_cfg["cloud_provider"] = NASTECH_MANAGED_PROVIDER
    ...
    if provider.get("web_backend"):
        ...
        web_cfg["backend"] = (
            NASTECH_MANAGED_PROVIDER if managed_feature else provider["web_backend"]
        )
        ...
        if backend:
            ...
            if backend == "fal":
                ...
                if isinstance(img_cfg, dict):
                    ...
                    # imagegen_backend="fal" — store the "nastech" selection
...
def _shared_metrics_menu_label(config: dict) -> str:
    ...
    if not enabled:
        ...
        state = "collecting + sending to Nastech"
    else:
...
def _configure_shared_metrics_interactive(config: dict) -> None:
    """Toggle shared-metrics collection and sending from `nastech tools`.
