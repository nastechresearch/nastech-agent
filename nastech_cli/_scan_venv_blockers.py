"""``nastech_cli/_scan_venv_blockers.py`` — Standalone venv-process scan for JSON consumption.

Invoked by the Desktop Electron app::

    venv\\Scripts\\python.exe -m nastech_cli._scan_venv_blockers
...
def _is_pausable_gateway(cmdline: str) -> bool:
    ...
    nastech_cli.main gateway run`` and the uv-side interpreter re-running the
    same argv). Reporting those as blockers dead-ends the Desktop update:
    the preflight aborts with ``venv-blocked`` *before* spawning
    ``nastech-setup``, so the CLI updater's own
    ``_pause_windows_gateways_for_update()`` — which exists precisely to
    stop these processes (and is always active: ``nastech-setup`` invokes
    ``nastech update --yes --gateway``) — never gets the chance to run.
...
def _is_updater_owned_backend(pid: int, cmdline: str) -> bool:
    """Return True when *pid* is a Nastech backend the CLI updater can stop.
    ...
    them. ``nastech serve`` / ``nastech dashboard`` backends had no such
...
def _updater_owned_backend_entry(pid: int, cmdline: str) -> dict | None:
    ...
    """
    try:
        from nastech_cli.update_cmd import _nastech_holder_subcommand  # noqa: PLC0415

        purpose = _nastech_holder_subcommand(cmdline)
    except Exception:
        return None
    if purpose not in ("serve", "dashboard"):
        return None
    try:
        from nastech_cli.process_identity import (  # noqa: PLC0415
            ledger_entries,
            spawner_is_dead,
        )
...
def main() -> None:
    ...
    try:
        from nastech_cli.main import _detect_venv_python_processes  # noqa: PLC0415
