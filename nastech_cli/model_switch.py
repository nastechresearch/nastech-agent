...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Nastech wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Nastech.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Nastech versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``nastech model`` wizard — not a
    ...
    while ``nastech model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Nastech itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Nastech itself discovered (``models_discovered: true``
...
_NASTECH_MODEL_WARNING = (
    "Nastech Research Nastech 3 & 4 models are NOT agentic and are not designed "
    "for use with Nastech Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nastech Research Nastech 3 / Nastech 4 chat families.
...
#   NastechResearch/Nastech-3-Llama-3.1-70B, nastech-4-405b, openrouter/nastech3:70b
...
def is_nastech_nastech_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nastech Nastech 3/4 chat model.
...
def _check_nastech_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nastech Nastech 3/4 chat model."""
...
# switch (NASTECH_HOME moves, so the path moves) and a config/key rotation
...
def resolve_startup_model_route(
    ...
    *,
    ...
    ``NastechCLI`` is constructed before the interactive ``/model`` pipeline
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "nastech")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "nastech")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Nastech, and falls back to models.dev for the
    rest.
...
async def resolve_display_context_length_async(
    ...
    ``/v1/models``, Copilot, Nastech, Codex, GMI, Ollama, models.dev and
    ...
    ``nastech_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    # =================================================================
        ...
        try:
            from nastech_cli.config import load_config
            from nastech_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from nastech_cli.models import should_use_ollama_native_catalog
    ...
    # --- Nastech Portal dual-wire override ---
    ...
    if target_provider in {"nastech", "nastech-portal", "nastechresearch"}:
        from nastech_cli.providers import nastech_api_mode

        api_mode = nastech_api_mode(new_model)
    ...
    # nastech_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from nastech_cli.models import opencode_provider_family as _oc_family_fn
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Nastech provider IDs to prefetch (e.g. ``["openrouter",
...
def _collect_authed_provider_slugs(
    ...
    from nastech_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from nastech_cli.providers import NASTECH_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from nastech_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Nastech-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for nastech_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        _canonical = nastech_id
        try:
            from providers import get_provider_profile as _gpp
            _prof = _gpp(nastech_id)
            if _prof is not None:
                _canonical = _prof.name
        except Exception:
            pass
        if _canonical != nastech_id:
            continue
        if nastech_id.lower() in seen:
            continue
        if nastech_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        ...
        if not is_runtime_provider_routable(nastech_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(nastech_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        nastech_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(nastech_id)
            seen.add(nastech_id.lower())

    # --- Section 2: Nastech-only providers (NASTECH_OVERLAYS) ---
    ...
    for pid, overlay in NASTECH_OVERLAYS.items():
        ...
        if nastech_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or nastech_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or nastech_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                ...
                pass
        if has_creds:
            slugs.append(nastech_slug)
            seen.add(pid.lower())
            seen.add(nastech_slug.lower())
    ...
    # Nastech excluded: its picker branch builds from the curated list and it
...
def list_authenticated_providers(
    ...
    *,
    ...
    ``force_fresh_nastech_tier`` bypasses the short Nastech tier cache for explicit
    ...
    from nastech_cli.models import (
        ...
        clear_provider_models_cache, get_curated_nastech_model_ids,
    )
    ...
    # Compared against nastech_id / mdev_id (section 1), pid / nastech_slug
    ...
    # "nastech" pulls from the remote model-catalog manifest published at
    # https://nastechresearch.github.io/nastech-agent/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Nastech release. Falls back to the in-repo
    # _PROVIDER_MODELS["nastech"] snapshot when the manifest is unreachable.
    ...
    for nastech_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip nastech_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose nastech_id matches the canonical profile name so
        ...
        # same nastech_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if nastech_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Nastech may not route yet.
        ...
        results.append({
            ...
            "is_current": (
                ...
                or nastech_id == current_provider
    ...
    # --- 2. Check Nastech-only providers (nastech, openai-codex, copilot, opencode-go) ---
    ...
    for pid, overlay in NASTECH_OVERLAYS.items():
        ...
        if nastech_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or nastech_slug.lower() in _excluded:
            continue
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # Nastech serves a large live /v1/models catalog (vendor-prefixed
            ...
            # order. Mirror that exactly (see _model_flow_nastech in main.py) so
            ...
            try:
                ...
                _pricing = _nastech_pricing("nastech") or {}
                _portal = ""
                try:
                    _st = _nastech_state("nastech") or {}
            ...
            try:
                from nastech_cli.models import (
                    nastech_policy_allowed_ids as _nastech_policy,
                    restrict_to_nastech_policy as _nastech_restrict,
                )

                model_ids = _nastech_restrict(
                    model_ids, _nastech_policy(), rescue_empty=True,
                )
            except Exception:
                pass
        else:
        ...
        total = len(model_ids)
        if nastech_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Nastech writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Nastech's own writer uses (mirrors section-4 grouping).
            ...
            # ``nastech model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``nastech model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for grp in groups.values():
            ...
            #   ``nastech model``. Pin a dict catalog with
