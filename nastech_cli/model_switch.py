...
# Re-exported: callers/tests patch nastech_cli.model_switch.<name>.
from nastech_cli.model_switch_providers import list_authenticated_providers
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, str):
        ...
        # Pre-fix Nastech wrote sentinel keys inside the user-facing ``models`` mapping.
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    ``discover_models: false``. A catalog Nastech itself persisted (``discovered``) is never a pin."""
    if discovered:
        return False
    if isinstance(value, str):
...
# Match only the real Nastech Research Nastech 3 / 4 chat families; a bare substring check
# false-positived on tool-capable local Modelfiles like ``nastech-brain:qwen3-14b-ctx16k``.
#   match:    NastechResearch/Nastech-3-Llama-3.1-70B, nastech-4-405b, openrouter/nastech3:70b
#   no match: nastech-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NASTECH_NASTECH_NON_AGENTIC_RE = re.compile(r"(?:^|[/:])nastech[-_ ]?[34](?:[-_.:]|$)", re.IGNORECASE)
...
def is_nastech_nastech_non_agentic(model_name: str) -> bool:
    """True if *model_name* is a real Nastech Nastech 3/4 chat model (single owner; cli.py uses it too)."""
    return bool(model_name and _NASTECH_NASTECH_NON_AGENTIC_RE.search(model_name))


def _check_nastech_model_warning(model_name: str) -> str:
    """Warning string if *model_name* is a Nastech Nastech 3/4 chat model, else ""."""
    return _NASTECH_MODEL_WARNING if is_nastech_nastech_non_agentic(model_name) else ""
...
def resolve_startup_model_route(
    ...
    ``NastechCLI`` is constructed before the interactive ``/model`` pipeline runs; resolving here
    ...
    if current_provider:
        try:
            from nastech_cli.providers import is_routing_aggregator, normalize_provider as _norm_prov
...
def parse_model_flags_detailed(raw_args: str) -> ModelFlagParseResult:
    ...
    is the raw level word (validated by :func:`nastech_constants.parse_reasoning_effort` at apply
...
def _configured_provider_identity(slug: str, cfg: dict) -> tuple[str, str, str, str]:
    ...
    from nastech_cli.config_providers import _canonical_api_mode, _normalize_custom_provider_entry
...
def _ollama_configured_base() -> tuple[dict, str]:
    from nastech_cli.models import _get_provider_config_dict
...
def _unknown_provider_message(explicit_provider: str) -> str:
    msg = (
        f"Unknown provider '{explicit_provider}'. Check 'nastech model' for available "
    ...
    try:  # Surface common config issues that cause provider resolution failures
        ...
        if issues:
            msg += "\n\nRun 'nastech doctor' — config issues detected:" + "".join(f"\n  • {ci.message}" for ci in issues[:3])
    except Exception:
        pass
    return msg
...
def _apply_direct_alias_endpoint(st: "_Switch", da: DirectAlias) -> None:
    ...
    from nastech_cli.models_local import _same_ollama_native_root
...
def _convert_vendor_colon_slug(st: _Switch) -> None:
    ...
    ``left`` names a provider Nastech knows, so ``/model alibaba:qwen3.6-plus`` routes like
...
def _route_from_model_input(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.resolved_alias or resolved_in_current_catalog or st.target_provider != current_provider:
        return None
    if current_provider == "nastech":
        # The welcome host serves nastech/welcome only; a model outside it needs an account or a key.
        ...
        if route_is_welcome_host(st.current_base_url) and st.new_model != GUEST_MODEL:
            return st.fail(
                f"{st.new_model} needs a Nastech account or an API key. "
...
def _creds_for_switched_provider(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    if st.explicit_provider and st.user_providers:
        from nastech_cli.providers import resolve_user_provider
    ...
    if user_pdef is not None and user_pdef.base_url:
        ...
        from nastech_cli.runtime_provider import _resolves_to_custom
...
def _creds_for_current_provider(st: _Switch) -> None:
    ...
    from nastech_cli.models_local import _get_ollama_request_headers, _same_ollama_native_root
    keep_current_ollama_endpoint = False
    ollama_headers: dict[str, str] = {}
    if st.current_provider == "custom" and st.current_base_url:
        try:
            from nastech_cli.models_local import should_use_ollama_native_catalog
...
def _validate_switch(st: _Switch) -> Optional[ModelSwitchResult]:
    ...
    from nastech_cli.models_local import _get_ollama_request_headers
...
def _nastech_api_mode(provider: str, model: str, api_key: str) -> str:
...
def _build_switch_result(st: _Switch) -> ModelSwitchResult:
    ...
    from nastech_cli.models import normalize_opencode_base_url, opencode_provider_family
    ...
    warnings = [w for w in (st.validation.get("message"), _check_nastech_model_warning(st.new_model)) if w]
    ...
    try:
        from nastech_cli.runtime_provider import _get_named_custom_provider, _custom_provider_request_overrides
...
def persist_model_selection(result: ModelSwitchResult, config_path: Any = None) -> None:
    ...
    ``NASTECH_HOME/config.yaml`` — the context override or ``NASTECH_HOME`` at call time).
...
def _scoped_key_env(name: str) -> str:
    ...
    ``$NASTECH_HOME/.env`` authenticates the ``/model`` verification probe (#109315) and a rotated
