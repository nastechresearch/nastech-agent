...
from nastech_cli.plugin_capabilities import (  # noqa: F401 — re-exported
    CAPABILITY_REGISTRY,
    VALID_CAPABILITY_IDS,
    plugin_capability_granted,
)
from nastech_cli.plugin_capabilities import (
    parse_declared_capabilities as _parse_declared_capabilities,
)
from nastech_cli.relay_plugin_cutover import (
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Nastech' shipped guidance lives in the evidence-based
    ...
    "pre_transcription",
    # Kanban task lifecycle hooks. Fired by nastech_cli.kanban_db when a task
    ...
    # separate `nastech -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `nastech kanban dispatch`),
    ...
    # ``nastech kanban dispatch``); on_kanban_task_updated fires in whichever
    ...
    #   result: nastech_cli.kanban_db.DispatchResult (spawned, reclaimed,
...
ENTRY_POINT_CAPABILITIES_GROUP = "nastech_agent.plugin_capabilities"
...
def discover_entrypoint_manifests() -> List["PluginManifest"]:
    ...
    * **Capability declarations** — read from the companion
      ``nastech_agent.plugin_capabilities`` entry-point group (declarations
...
PLUGIN_SECTIONS_START = "<!-- nastech-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- nastech-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- nastech-plugin-section-chars:{len(content)} -->\n\n"
        f"{content}"
    )
...
# Reserved event namespace prefix — only core may publish ``nastech:<event>``.
NASTECH_EVENT_NAMESPACE = "nastech"
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Nastech home."""
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "capabilities", "emits", "listens", "nastech", "depends",
}

# Highest manifest schema version this Nastech understands.
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Nastech "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `nastech plugins enable %s` if "
                    "it is installed.",
                    k, dep_id, dep_id,
                )
                continue
...
class PluginManifest:
    ...
    # ``nastech_cli.plugin_capabilities.CAPABILITY_REGISTRY``. Declaration is
    ...
    # Declared pip dependencies. VALIDATED AND SURFACED ONLY — Nastech never
    ...
    # (``nastech plugins show``); a plugin may emit/subscribe without declaring.
...
def _plugin_relative_segments(key: str) -> tuple[str, ...]:
    ...
    ``retry.policy``).  Full Nastech paths, traversal syntax, and the security-
...
class PluginState:
    ...
    def data_dir(self) -> Path:
        ...
        return get_nastech_home() / "plugin-data" / self._data_namespace
...
class PluginContext:
    ...
    def set_config(self, key: str, value: Any) -> None:
        ...
        # threads. Other Nastech config writers still retain their existing
    ...
    def platform_actions(self):
        ...
        if self._platform_actions is None:
            from nastech_cli.platform_actions import PlatformActions
    ...
    def profile_name(self) -> str:
        """Return the active Nastech profile name (e.g. ``"default"``).

        Derived from ``NASTECH_HOME`` via
        :func:`nastech_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.nastech/profiles/<name>``, or ``"custom"`` when
        ``NASTECH_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Nastech core) are trusted by default —
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        ...
        """
        from nastech_cli.dashboard_auth import DashboardAuthProvider
    ...
    def register_secret_source(self, source) -> Optional[PluginRegistration]:
        ...
        sources run during ``load_nastech_dotenv()`` startup — after
        ``~/.nastech/.env`` loads, before Nastech reads credentials — when
        ...
        NOTE ON TIMING: ``load_nastech_dotenv()`` usually runs at import
        *before* plugin discovery.  After discovery completes, the plugin
        ...
        + ``load_nastech_dotenv``) so the first process sees them (#64177).
    ...
    def register_slack_action_handler(
        ...
        Nastech' Slack adapter wires registered handlers into its
    ...
    def register_telegram_handler(self, factory: Callable) -> None:
        ...
        Nastech' Telegram adapter invokes registered factories at ``connect()``
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        including ``nastech:x`` or a foreign ``other:x``) is rejected with a
        ``ValueError`` and a logged warning — fail-closed. The ``nastech:``
        ...
        if ":" in event:
            logger.warning(
                ...
                plugin_key, event, plugin_key, NASTECH_EVENT_NAMESPACE,
            )
            raise ValueError(
                f"Plugin '{plugin_key}' may not emit '{event}': emit only the "
                f"bare event name; the namespace is forced to '{plugin_key}:' "
                f"and the '{NASTECH_EVENT_NAMESPACE}:' prefix is reserved for core"
            )
    ...
    def subscribe(self, event: str, callback: Callable) -> None:
        ...
        *event* is the full ``<plugin_key>:<event>`` name (or ``nastech:<event>``
...
class PluginManager:
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # original scope.
        self.scope_key = scope_key or nastech_home_key()
        ...
        # by resolved nastech home). The ledger is therefore keyed per manager
        # — i.e. per (nastech_home, plugin_id) — and every release/restore
        ...
        # contributed, so `nastech plugins list` still attributes them once the
        # full plugin loads.
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            if env_var_enabled("NASTECH_SAFE_MODE"):
                logger.info("NASTECH_SAFE_MODE=1 — plugin discovery skipped")
            ...
            try:
                ...
                # load_nastech_dotenv() already ran at import time. Re-pull so the
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning(
                "Removed Nastech plugin %s is still listed in plugins.enabled; "
        ...
        for manifest in winners.values():
            ...
            # Relay lifecycle ownership now lives in the Nastech core. Loading
            ...
            if (
                ...
                loaded.error = (
                    "removed — Relay lifecycle is owned by Nastech core; configure "
                    f"{RELAY_PLUGINS_CONFIG_ENV} instead"
                )
                self._plugins[lookup_key] = loaded
                logger.warning(
                    "Refusing to load removed Nastech Relay plugin '%s'; %s",
                    lookup_key,
                    loaded.error,
                )
                continue
            ...
            # all ~20 of them added several seconds to every `nastech`
            # invocation — including plain `nastech chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Nastech ships
    ...
    def register_approval_transport(
        ...
        *,
        ...
        from nastech_cli.approval_transport import RegisteredApprovalTransport
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            ...
            present=present_fn,
            plugin_id=plugin_id,
            profile_home=str(get_nastech_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None:
            return None
        if registered.profile_home != str(get_nastech_home().resolve()):
            return None
        return registered
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 3. Project plugins (./.nastech/plugins/), only when explicitly opted
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for manifest in winners.values():
            ...
            if lookup_key not in enabled and manifest.name not in enabled:
                continue
            try:
                from nastech_cli.agent_plugins import _discover_mcp
        ...
        *,
        ...
        for child in sorted(path.iterdir()):
            ...
            if portable_file.exists() or portable_file.is_symlink():
                try:
                    from nastech_cli.agent_plugins import read_agent_plugin_manifest
    ...
    def _classify_entrypoint_kind(self, ep) -> str:
        ...
        module's import cost in every Nastech process (e.g. a pip
    ...
    def _scan_entry_points(self) -> List[PluginManifest]:
        ...
        declarations from the ``nastech_agent.plugin_capabilities`` group.
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``nastech plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`nastech plugins
    ...
    def _register_deferred_platform_tools(
        ...
        ``nastech tools`` checklist, and even an explicit ``platform_toolsets``
        ...
        try:
            ...
            # `nastech plugins list` under-reports what the process is actually
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        python_dependencies is a declaration seam ONLY: Nastech validates and
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Nastech does not install plugin dependencies "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # above cannot see them and `nastech plugins list` would
    ...
    def _load_portable_plugin(
        ...
        lookup_key = manifest.key or manifest.name
        try:
            from nastech_cli.agent_plugins import load_agent_plugin

            package = load_agent_plugin(
                Path(manifest.path),
                get_nastech_home() / "plugin-data" / manifest.skill_namespace,
            )
            ctx = PluginContext(manifest, self)
    ...
    def _load_directory_module(
        ...
        *,
        ...
        # from a different Nastech home (profile switch reusing a slug
        # like "nastech-lcm") or from an earlier force=True reload in the
            ...
            try:
                if use_timeout:
                    ...
                    thread = threading.Thread(
                        target=_runner,
                        name=f"nastech-hook-{callback_name}"[:40],
                        daemon=True,
                    )
                    thread.start()
...
# Keyed cache: resolved Nastech home -> PluginManager. Nastech supports
# multiple profiles via different NASTECH_HOME directories, and a single
# long-lived process (gateway multiplexer, test session, embedder) can
# switch between them via ``set_nastech_home_override()`` — which is a
# ContextVar and deliberately does NOT touch os.environ (see
# nastech_constants.set_nastech_home_override). A process-wide single-slot
...
def _plugin_home_key() -> Path:
    ...
    Plugins are discovered from ``get_nastech_home() / "plugins"`` and some
    plugins (notably context engines such as nastech-lcm) capture that home
    at registration time for profile-scoped storage. A long-lived process
    can temporarily switch Nastech home (env var *or* the context-local
    ``set_nastech_home_override()``) while serving another profile, so the
    plugin manager must be scoped to the active Nastech home instead of
    being one process-wide singleton.
    """
    try:
        return get_nastech_home().expanduser().resolve()
    except Exception:
        return get_nastech_home().expanduser()
...
def _clear_plugin_submodules(manager: Optional[PluginManager]) -> None:
    ...
    ``nastech_plugins.<slug>`` and registers that top-level module in
    ...
    *relative* import (``from . import foo``, ``from .sub import bar``)
    ...
    ``nastech_plugins.<slug>.<submodule>``. When we swap in a fresh manager
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Nastech profile/home.
    ...
    profile switch — via ``NASTECH_HOME`` or the context-local
    ``set_nastech_home_override()`` — gets its own manager with its own
    ...
    with _plugin_managers_lock:
        ...
        if manager is None:
            manager = PluginManager(scope_key=nastech_home_key(current_home))
...
def _plugin_toolset_keys_cache_path():
    ...
    return get_nastech_home() / "cache" / "plugin_toolset_keys.json"
...
def _get_pre_tool_call_directive_details(
    ...
    from nastech_cli.lifecycle import invoke_hook as invoke_lifecycle_hook
...
def get_plugin_subscriptions() -> Dict[str, List[Callable]]:
    ...
    (``<plugin_key>:<event>`` or ``nastech:<event>``) to subscriber callbacks in
