"""Nastech Plugin System — discovers, loads, and manages plugins.

Sources, later overriding earlier on key collision: bundled ``<repo>/plugins/<name>/`` (``memory/``
and ``context_engine/`` have their own discovery), user ``~/.nastech/plugins/<name>/``, project
``./.nastech/plugins/<name>/`` (opt-in via ``NASTECH_ENABLE_PROJECT_PLUGINS``), and pip packages in
the ``nastech_agent.plugins`` entry-point group. A directory plugin needs a ``plugin.yaml`` manifest
...
from nastech_constants import get_nastech_home, get_process_nastech_home, nastech_home_key
from registration_lifecycle import replacement_coordinator
from utils import env_var_enabled
from nastech_cli.config import load_config_readonly
from nastech_cli.middleware import VALID_MIDDLEWARE
from nastech_cli.plugin_capabilities import plugin_capability_granted
from nastech_cli.relay_plugin_cutover import RELAY_PLUGINS_CONFIG_ENV, legacy_relay_plugin_keys
# Sibling modules' names are re-exported here (origin) so plugins and tests keep one import path.
from nastech_cli.plugins_manifest import (  # noqa: F401 — re-exported
...
from nastech_cli.plugins_discovery import (  # noqa: F401 — re-exported
...
from nastech_cli.plugins_loader import (
...
from nastech_cli.plugins_dispatch import (  # noqa: F401 — re-exported
    DEFAULT_SYSTEM_PROMPT_SECTION_MAX_CHARS, NASTECH_EVENT_NAMESPACE, MAX_SYSTEM_PROMPT_SECTION_CHARS,
...
from nastech_cli.plugins_ledger import PluginLedgerMixin, PluginRegistration
from nastech_cli.plugins_state import (
...
def get_bundled_plugins_dir() -> Path:
    """Bundled ``plugins/`` dir: ``NASTECH_BUNDLED_PLUGINS`` (Nix wrapper / packaged installs, read-only
...
# ``NASTECH_PLUGINS_DEBUG=1`` tees verbose discovery logs to stderr in addition to agent.log. Read
# once at import; tests flip it mid-process via ``_install_plugin_debug_handler(force=True)``.
_PLUGINS_DEBUG = env_var_enabled("NASTECH_PLUGINS_DEBUG")
_DEBUG_HANDLER_INSTALLED = False


def _install_plugin_debug_handler(force: bool = False) -> None:
    """When NASTECH_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG (once per process)."""
    ...
    logger.debug("NASTECH_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled")
...
VALID_HOOKS: Set[str] = {
    ...
    "pre_transcription",
    # Kanban task observers (nastech_cli.kanban_db), fired AFTER the DB commit so a slow plugin never
    ...
    # | "idle", result: nastech_cli.kanban_db.DispatchResult (spawned, reclaimed, promoted,
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Active profile name (``"default"``, the ``~/.nastech/profiles/<name>`` id, or ``"custom"``),
        derived from ``NASTECH_HOME`` — not ``_cli_ref``, which is None outside the interactive CLI —
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Nastech core) are trusted by default — an override there is a
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (``nastech <name> ...``). *setup_fn* receives the argparse
    ...
    def register_dashboard_auth_provider(self, provider) -> Optional[PluginRegistration]:
        """Register a :class:`nastech_cli.dashboard_auth.DashboardAuthProvider` for the dashboard
        ...
        from nastech_cli.dashboard_auth.registry import register_global_provider, unregister_global_provider
        if self._wrong_type(provider, DashboardAuthProvider, "dashboard-auth provider"):
            return
        launch_scope = nastech_home_key(get_process_nastech_home())
    ...
    def register_auxiliary_task(
        ...
        from nastech_cli.main_provider_setup import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def emit(self, event: str, payload: Optional[dict] = None) -> int:
        ...
        the subscriber count scheduled. Any ``':'`` in the name (``nastech:x`` is reserved for core,
        ...
        if ":" in event:
            ...
            raise ValueError(f"Plugin '{plugin_key}' may not emit '{event}': emit only the bare event name; "
                             f"the namespace is forced to '{plugin_key}:' and the '{NASTECH_EVENT_NAMESPACE}:' "
    ...
    def register_skill(
        ...
        Not in ``~/.nastech/skills/`` nor ``<available_skills>`` — explicit loads only. Raises
...
_SCOPED_PROVIDER_REGISTRARS: Tuple[Tuple[str, str, str, str, str, str, Dict[str, Any]], ...] = (
    ...
    ("register_secret_source", "secret_source", "agent.secret_sources.registry",
     ...
     "Register a :class:`agent.secret_sources.base.SecretSource`, run by ``load_nastech_dotenv()`` "
     "(after ``~/.nastech/.env``, before credentials are read) when ``secrets.<name>`` is enabled. The "
...
class PluginManager(PluginLoaderMixin, PluginDispatchMixin, PluginLedgerMixin):
    ...
    def __init__(self, scope_key: Optional[str] = None) -> None:
        ...
        # is keyed per (nastech_home, plugin_id) and every inverse is identity-conditional — one
        ...
        # and contributed tool names (so `nastech plugins list` still attributes them).
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        with self._discovery_lock, _plugin_home_scope(self.home_path):
            ...
            try:
                ...
                # load_nastech_dotenv() ran at import, before plugin secret sources existed: re-pull.
                # Plugin secret sources register during discover; the initial load_nastech_dotenv() already
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        if stale_relay_keys:
            logger.warning("Removed Nastech plugin %s is still listed in plugins.enabled; "
    ...
    def _refresh_plugin_compat_report(self, manifests: List[PluginManifest]) -> None:
        """Refresh NASTECH_HOME/.plugin-compat-report.json from this discovery pass (nastech_cli.plugin_compat).
        ...
        """
        try:
            from nastech_cli.plugin_compat import compat_report
    ...
    def register_approval_transport(self, name: str, present_fn: Callable, *, plugin_id: str) -> None:
        ...
        self._approval_transports[clean] = RegisteredApprovalTransport(
            name=clean, present=present_fn, plugin_id=plugin_id, profile_home=str(get_nastech_home().resolve()),
        )
    ...
    def get_approval_transport(self, name: str):
        ...
        if registered is None or registered.profile_home != str(get_nastech_home().resolve()):
            return None
        return registered
    ...
    def has_enabled_portable_mcp(self, raw_config: Mapping[str, Any]) -> bool:
        ...
        for lookup_key, manifest in {manifest_key(m): m for m in self._collect_directory_manifests()}.items():
            ...
            try:  # lazy: this is a startup probe, keep agent_plugins unimported unless needed
                ...
                if _discover_mcp(Path(manifest.path), get_nastech_home() / "plugin-data"
...
# Resolved Nastech home -> PluginManager. A process can switch profiles via
# ``set_nastech_home_override()``; a single slot would leak one profile's plugin/context-engine state
...
def _plugin_home_key() -> Path:
    """Resolved active Nastech home — the key for per-profile plugin managers (plugins capture the
...
def get_plugin_manager() -> PluginManager:
    """Return the plugin manager for the active Nastech profile/home (cached per resolved home; a
...
def _reset_plugin_managers_for_tests() -> None:
    ...
    try:
        from nastech_cli.dashboard_auth.registry import clear_providers
        clear_providers()
    except Exception:
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    # async hook under a running loop reads the default NASTECH_HOME and get_secret raises.
    threading.Thread(target=contextvars.copy_context().run, args=(_runner,),
                     name="nastech-plugin-command-await", daemon=True).start()
...
def get_plugin_toolsets() -> List[tuple]:
    """Plugin toolsets as ``(key, label, description)`` tuples for the ``nastech tools`` TUI."""
    manager = get_plugin_manager()
...
_PLUGIN_COMPAT_LAZY = {
    'CAPABILITY_REGISTRY': ('nastech_cli.plugin_capabilities', 'CAPABILITY_REGISTRY'),
    'ENTRY_POINT_CAPABILITIES_GROUP': ('nastech_cli.plugins_discovery', 'ENTRY_POINT_CAPABILITIES_GROUP'),
    'LEGACY_RELAY_PLUGIN_KEYS': ('nastech_cli.relay_plugin_cutover', 'LEGACY_RELAY_PLUGIN_KEYS'),
    'MAX_SYSTEM_PROMPT_SECTIONS': ('nastech_cli.plugins_dispatch', 'MAX_SYSTEM_PROMPT_SECTIONS'),
    'OBSERVER_SCHEMA_VERSION': ('nastech_cli.middleware', 'OBSERVER_SCHEMA_VERSION'),
    'VALID_CAPABILITY_IDS': ('nastech_cli.plugin_capabilities', 'VALID_CAPABILITY_IDS'),
    'cfg_get': ('nastech_cli.config', 'cfg_get'),
    'fast_safe_load': ('utils', 'fast_safe_load'),
    'format_system_prompt_section': ('nastech_cli.plugins_dispatch', 'format_system_prompt_section'),
    'reset_nastech_home_override': ('nastech_constants', 'reset_nastech_home_override'),
    'set_nastech_home_override': ('nastech_constants', 'set_nastech_home_override'),
}
