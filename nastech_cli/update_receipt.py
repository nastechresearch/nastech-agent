...
1. **Update receipt** — a machine-readable JSON record of what one
   ``nastech update`` run discovered, did, skipped (and why), written to
   ``<NASTECH_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``nastech_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``nastech update`` is a single-threaded CLI
    ...
    def gateway_restart_result(
        ...
        *,
        ...
        if fresh_recovery is not None:
            ...
            # Serve/dashboard coverage (#92145). ``nastech serve`` hosts
...
def _receipt_dir() -> Path:
    ...
    return get_nastech_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``nastech update`` has many early-termination paths (Windows
