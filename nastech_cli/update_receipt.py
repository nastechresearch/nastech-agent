...
# ``nastech update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``nastech update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``nastech serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``nastech_constants`` (stdlib-only), never ``nastech_cli.config``: the receipt must be
    # writable from the refused/failed paths where config loading itself may be what broke
    # (#112465, #112558).
    from nastech_constants import get_nastech_home

    return get_nastech_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``nastech update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from nastech_cli.profiles import _get_default_nastech_home, _get_profiles_root, _PROFILE_ID_RE
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `nastech gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if stale_or_down:
        ...
        print("  Run `nastech gateway restart` (or `nastech -p <profile> gateway restart` for a named")
        print("  profile), then `nastech gateway status` to confirm.")
