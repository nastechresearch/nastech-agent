...
One read-only pass that answers, BEFORE any mutation: what Nastech runtimes
...
`detect_install_method`) — so `nastech update --plan` can run on a live
...
Deployment kinds (the concept most fleet-update bugs were missing):
    ...
    git      — source checkout; updatable in place via `nastech update`
...
Supervisors (how a runtime is restarted after code changes):
    ...
    desktop           — Desktop app supervises `nastech serve`; it respawns
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    if profile != "default":
        return f"nastech -p {profile} gateway restart"
...
def collect_runtime_inventory() -> UpdatePlan:
    ...
    # launched `nastech serve --host <ip>` for a remote Desktop, or a
    # long-lived `nastech dashboard`. Every serve/dashboard registers itself
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``nastech-serve*``/``nastech-dashboard*`` unit for *profile*?

    Serve/dashboard runtimes have their OWN unit vocabulary; the gateway's
    ``nastech-gateway*`` names never cover them (#100479). Exact names only —
    ``work`` must not claim ``nastech-serve-workbench`` — and a scope prefix
    (``user/nastech-serve``) is tolerated because the restart phase records
    ...
    if profile == "default":
        return name in {"nastech-serve", "nastech-dashboard"}
    return name in {f"nastech-serve-{profile}", f"nastech-dashboard-{profile}"}
...
def match_runtime_outcomes(
    ...
    *,
    ...
    (#100479): a ``nastech-serve*``/``nastech-dashboard*`` unit, a killed
    PID, or — when the caller passes ``stale_serve_pids`` (the
    ``(pid, create_time)``-verified survivor probe,
    :func:`nastech_cli.update_abort_recovery._surviving_pre_update_serve_runtimes`)
    ...
    gateway's outcome: ``relaunched_profiles`` and ``nastech-gateway*`` name a
    ...
    try:
        ...
        for runtime in plan.runtimes:
            ...
            if r.kind in _SERVE_KINDS:
                ...
                continue
            outcome = "unaccounted"
            # The bare "nastech-gateway" unit name is gateway-specific: a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        print("      relaunch `nastech serve` / `nastech dashboard` / the Desktop app")
    return True
