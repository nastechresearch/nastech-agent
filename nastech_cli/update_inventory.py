...
One read-only pass answering, BEFORE any mutation: which Nastech runtimes run on this machine, how
each is deployed, which ones this update touches, and how each restarts. Every collector is a
side-effect-free probe, so ``nastech update --plan`` is safe on a live fleet.
"""
...
@dataclass
class RuntimeRecord:
    """One running (or expected) Nastech runtime on this machine."""
...
class UpdatePlan:
    ...
    update_mechanism: str = "nastech update"
...
def _detect_supervisor_for_pid(pid: int, service_pids: set, windows_service_pids: set | None = None) -> str:
    ...
    with suppress(Exception):
        from nastech_cli.gateway import is_macos, supports_systemd_services
...
def describe_restart_mechanism(mechanism: str, profile: str) -> str:
    ...
    return _MECHANISM_DESCRIPTIONS.get(mechanism) or (
        f"nastech -p {profile} gateway restart" if profile != "default" else "nastech gateway restart"
    )
...
def _collect_install_shape(plan: UpdatePlan) -> None:
    with _probe("Install-method probe"):
        from nastech_cli.config import detect_install_method, get_managed_system, recommended_update_command_for_method
        ...
        with _probe("Image provenance probe"):
            # See #91277.
            from nastech_cli.image_provenance import read_image_provenance
...
def _supervisor_classifier() -> Callable[[int], str]:
    ...
    with _probe("Service-PID probe"):
        from nastech_cli.gateway import _get_service_pids
    ...
    with _probe("Windows SCM service-ownership probe"):
        from nastech_cli.gateway import find_windows_gateway_services
...
def _collect_gateway_runtimes(plan: UpdatePlan, profile_homes: list, seen: set[int]) -> None:
    ...
    with _probe("Gateway-state inventory"):
        ...
        from nastech_cli.update_receipt import _socket_identity
        ...
        for profile, home in profile_homes:
            ...
            if sock is not None:
                ...
                # touch, so `nastech update` exited partial (#109680).
    ...
    with _probe("PID-file gateway inventory"):
        from nastech_cli.gateway import find_profile_gateway_processes
...
def _collect_ledger_runtimes(plan: UpdatePlan, seen: set[int]) -> None:
    ...
    (a manual `nastech serve --host <ip>` for a remote Desktop, a long-lived `nastech dashboard`).
...
def print_update_plan(plan: UpdatePlan) -> None:
    ...
    if not plan.runtimes:
        print("  Running Nastech services: none detected — code swap only.")
        return
...
def _serve_unit_matches_profile(profile: str, unit: object) -> bool:
    """Does *unit* name a ``nastech-serve*``/``nastech-dashboard*`` unit for *profile*? (OWN vocabulary;
    the gateway's ``nastech-gateway*`` names never cover serve/dashboard runtimes.)

    Exact names only — ``work`` must not claim ``nastech-serve-workbench`` — and a scope prefix
    (``user/nastech-serve``) is tolerated because the restart phase records scope-qualified identities in
    some lists. See #100479.
    """
    name = str(unit).removesuffix(".service").rsplit("/", 1)[-1]
    suffix = "" if profile == "default" else f"-{profile}"
    return name in {f"nastech-serve{suffix}", f"nastech-dashboard{suffix}"}
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    Never substring-match: ``foo`` must not claim ``nastech-gateway-foobar.service``.
    Launchd labels are ``ai.nastech.gateway`` / ``ai.nastech.gateway-<profile>`` — they do
    not contain the substring ``nastech-gateway``, so a successful macOS kickstart must
    still credit the planned default gateway. A scope prefix (``user/nastech-gateway``,
    ``gui/501/ai.nastech.gateway``) is stripped the same way serve units are.
    """
    name = str(service).removesuffix(".service").rsplit("/", 1)[-1]
    if profile == "default":
        return name in {"nastech-gateway", "ai.nastech.gateway", "gateway", "gateway-default"}
    return name in {f"nastech-gateway-{profile}", f"ai.nastech.gateway-{profile}", f"gateway-{profile}"}
...
def match_runtime_outcomes(
    ...
    See #91277.
    They never borrow the gateway's outcome: ``relaunched_profiles`` and ``nastech-gateway*`` name a
...
def report_unaccounted_runtimes(outcomes: list[dict[str, Any]]) -> bool:
    ...
    if manual:
        ...
        for o in manual:
            print(f"    • {o['kind']} [{o['profile']}] pid {o['pid']}: relaunch `nastech serve` / `nastech dashboard`, or reconnect Desktop for an SSH backend")
    ...
    if any(o.get("kind") not in _SERVE_KINDS for o in missed):
        print("      nastech gateway restart                # active profile")
        print("      nastech -p <profile> gateway restart   # named profile")
    if any(o.get("kind") in _SERVE_KINDS for o in missed):
        ...
        if sys.platform == "linux":
            print("      systemctl --user restart nastech-serve.service   # unit-managed serve")
        print("      relaunch `nastech serve` / `nastech dashboard`")
    return True
