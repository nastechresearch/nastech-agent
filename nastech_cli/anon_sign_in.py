...
UPGRADE_START = "Sign in with a Nastech account to unlock more models and tools."
...
UPGRADE_UNAVAILABLE = "The free tier is not available right now; run `nastech auth add nastech` to sign in."
UPGRADE_REASON_COPY = {
...
UPGRADE_SERVICE_BUSY = ("Signing in couldn't finish because the service is busy. "
                        "Try again in {wait}. Your session is still here in the meantime.")
UPGRADE_SERVICE_UNREACHABLE = ("The service couldn't be reached to finish signing you in. "
                               "Check your internet connection and try again. Your session is still here.")

UPGRADE_NO_DEFAULT_TERMINAL = "No default model is set yet; run `nastech model` to pick one."
...
LOGIN_DM_ONLY = "Sign in from a direct message with Nastech."
LOGIN_BUSY_ELSEWHERE = "Another sign-in is already running on this Nastech. Try again in a few minutes."
LOGIN_NOT_ALLOWED = "Only an operator of this Nastech can sign it in."
...
# the in-chat form (never a raw exception, a URL or a ``nastech`` verb) and ``.copy_terminal`` the
...
class Failed(SignInState):
    ...
    def retryable(self) -> bool:
        from nastech_cli import anon_auth as _core
...
def run_sign_in(
    *,
    ...
    """Sign the free tier into a Nastech account, keeping its connectors. Yields :class:`SignInState`s.
    ...
    *client_factory* is the HTTP client seam, ``client_factory(timeout_seconds, verify)``.
    """
    from nastech_cli import anon_auth as _core
    from nastech_cli.auth import PROVIDER_REGISTRY, _resolve_verify
    from nastech_cli.auth_device_flow import _request_device_code
    from nastech_cli.auth_nastech import _nastech_http_client
    ...
    try:
        with open_scope():
            state = _core.current_nastech_state()
    ...
    try:
        ...
        open_client = client_factory or _nastech_http_client
    ...
    try:
        ...
        with open_scope():
            with guard() as may_persist:
                if may_persist:
                    _core.persist_nastech_credentials(account_state)
            if may_persist:
                settled = _core.settle_after_upgrade(account_state)
    except Exception as exc:
        # persist_nastech_credentials takes the auth-store lock, writes auth.json, takes the shared
