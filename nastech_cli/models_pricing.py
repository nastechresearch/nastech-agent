...
Nastech Portal sale chrome and org-policy filtering, and the Vercel AI Gateway / Novita / Fireworks /
DeepInfra pricing adapters. Split out of ``nastech_cli.models``; helpers still defined there are
looked up on ``nastech_cli.models`` at call time so ``patch("nastech_cli.models.<name>")`` mocks keep
intercepting.
"""
...
from nastech_cli.models_reasoning_caps import _seed_reasoning_caps
...
def compute_sale_discount(prompt: str, completion: str, original: Any) -> tuple[int, str, str] | None:
    """Sale chrome from gateway ``pricing.original`` (Nastech Portal only; callers gate on the provider
...
def _get_json(url: str, headers: dict[str, str], timeout: float, opener=None) -> Optional[dict]:
    ...
    from nastech_cli.models import _urlopen_model_catalog_request
...
def _pricing_entry(pricing: dict, prompt_key: str = "prompt", completion_key: str = "completion") -> dict[str, Any]:
    ...
    ``pricing`` block whose cache fields already use the nastech names."""
    entry: dict[str, Any] = {
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    another's read. *include_sale_original* (Nastech Portal only) copies the gateway's pre-discount
    ...
    for item in payload.get("data", []):
        ...
        if mid and isinstance(pricing, dict):
            ...
            # Sale chrome is Nastech Portal-only; never copy pricing.original for other catalogs.
...
def _resolve_nastech_pricing_credentials() -> tuple[str, str]:
    """``(api_key, base_url)`` for Nastech Portal pricing; base_url is the bare origin (no ``/v1``).
    Precedence mirrors runtime credential resolution: ``NASTECH_INFERENCE_BASE_URL`` (staging /
...
# How long a Nastech catalog stays trusted. Its contents depend on the org's policy, which an admin
...
def _fetch_nastech_pricing(api_key: str, base_url: str, *, force_refresh: bool) -> dict[str, dict[str, Any]]:
    """Shared by pricing and policy lookups so both read one cache entry."""
    return fetch_models_with_pricing(
        api_key=api_key,
        base_url=base_url,
        force_refresh=force_refresh,
        include_sale_original=True,  # Sale chrome (pricing.original) is Nastech Portal-only.
        cache_ttl_seconds=_NASTECH_CATALOG_TTL_SECONDS,
    )


def nastech_policy_allowed_ids(*, force_refresh: bool = False) -> Optional[set[str]]:
    """The Nastech model ids the caller's org may reach (keys of an authenticated ``GET /v1/models``,
    ...
    if not api_key or not base_url:
        return None
    return set(_fetch_nastech_pricing(api_key, base_url, force_refresh=force_refresh)) or None
...
def _remember_provider_cache_key(provider: str, cache_key: str) -> None:
    from nastech_cli.models import _pricing_profile_key
...
def _fetch_nastech_pricing_for_provider(*, force_refresh: bool = False) -> dict[str, dict[str, Any]]:
    api_key, base_url = _resolve_nastech_pricing_credentials()
    if not base_url:
        return {}
    _remember_provider_cache_key("nastech", base_url.rstrip("/"))
    return _fetch_nastech_pricing(api_key, base_url, force_refresh=force_refresh)
...
def get_cached_nastech_inference_base_url() -> str:
    """The profile's persisted Nastech endpoint (bare origin, no ``/v1``) without refreshing auth."""
    try:
        from nastech_cli.auth import (
            _load_auth_store, _load_provider_state, _optional_base_url, _validate_nastech_inference_url_from_network,
        )

        state = _load_provider_state(_load_auth_store(), "nastech") or {}
        url = _validate_nastech_inference_url_from_network(_optional_base_url(state.get("inference_base_url"))) or ""
        return url.rstrip("/").removesuffix("/v1")
    except Exception:
        return ""


# Static endpoint identity per provider; dynamic ones (deepinfra, nastech) are resolved in pricing_cache_scope.
...
def pricing_cache_scope(provider: str, *, current_provider: str = "", current_base_url: str = "") -> str:
    ...
    from nastech_cli.models import _deepinfra_catalog_url, _pricing_profile_key, normalize_provider
    ...
    if normalized == "nastech":
        ...
        return _pricing_provider_cache_keys.get((_pricing_profile_key(), normalized), _DEFAULT_NASTECH_INFERENCE_BASE)
    return ""
...
def _cached_only_pricing(normalized: str) -> dict[str, dict[str, str]]:
    ...
    from nastech_cli.models import _deepinfra_catalog_cache, _deepinfra_catalog_url, _pricing_profile_key
    if normalized == "deepinfra":
...
def _fetch_novita_pricing(timeout: float = 8.0, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json", "User-Agent": _NASTECH_USER_AGENT}
...
_PRICING_FETCHERS = {
    ...
    "nastech": _fetch_nastech_pricing_for_provider,
}
