...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Nastech' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Nastech shells out to git from many non-interactive contexts — MCP catalog
    ...
    * isolated git config — inherited ``GIT_CONFIG_*`` overrides, global/system
      ...
      hang or mutate Nastech's internal plumbing calls.
...
def _text_names_nastech(text: str) -> bool:
    """True when *text* names Nastech at a path-segment / token boundary.

    A bare ``"nastech" in text`` substring test would also match unrelated
    ...
    that *starts with* ``nastech`` (``nastech``, ``nastech.exe``, ``nastech_cli``,
    ``nastech-agent``, ``nastech-runtime``) or the hidden-dir form
    ``.nastech``/``.nastech-runtime``.
    """
    for token in re.split(r"[\\/\s=,;\"']+", text.lower()):
        if token.startswith("nastech") or token.startswith(".nastech"):
            return True
    return False
...
def pid_is_nastech(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a Nastech process. When
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # ``pid_is_nastech`` guard is for BARE pids from state files or process
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``nastech update`` on slow-WMI machines
