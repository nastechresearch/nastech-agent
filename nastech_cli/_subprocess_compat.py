...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_NASTECH_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Nastech's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Nastech shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_nastech(text: str) -> bool:
    r"""True when *text* names Nastech at a path-segment / token boundary.

    A bare ``"nastech" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\shermesa\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("nastech", ".nastech"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_nastech(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Nastech code."""
    try:
        import psutil
        ...
        return _text_names_nastech(f"{command} {executable}")
    except Exception:
        return False


def pid_is_nastech(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Nastech process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_nastech(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_nastech`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``nastech update`` on slow-WMI
