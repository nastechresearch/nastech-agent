...
cycle) and tests patch ``nastech_cli.config.load_config`` etc. at call time.
"""
...
def _ask(prompt: str, *, secret: bool = False, raw: bool = False, cancel_msg: str | None = "\nCancelled.",
    ...
    if secret:
        from nastech_cli.secret_prompt import masked_secret_prompt as fn
    else:
...
def _commit_model_config(cfg: dict) -> None:
    ...
    from nastech_cli.auth import deactivate_provider
...
def _login_retry_context(args) -> tuple[str, str]:
    ...
    among its positional args. Nastech keeps ``nastech portal``; every other OAuth provider is retried
    with ``nastech auth add <provider>`` and named by its own portal host (``nastech login`` no longer
    exists). Falls back to ``nastech model`` when no provider config is in play."""
    pconfig = next((a for a in args if hasattr(a, "id") and hasattr(a, "portal_base_url")), None)
    if pconfig is None:
        return "nastech model", "the sign-in service"
    ...
    if provider_id == "nastech":
        return "nastech portal", host
    return (f"nastech auth add {provider_id}" if provider_id else "nastech model"), host
