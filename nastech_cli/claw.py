...
from nastech_cli.setup import (Colors, color, print_header, print_info, print_success, print_error,
                              prompt_yes_no)
...
_SCRIPT_REL = Path("migration", "openclaw-migration", "scripts", "openclaw_to_nastech.py")
...
_OPENCLAW_SCRIPT_INSTALLED = get_nastech_home() / "skills" / _SCRIPT_REL
...
# `nastech claw migrate` flags/defaults. Secrets are never included implicitly: --migrate-secrets
...
def _print_banner(title: str) -> None:
    ...
    print()
    rule = "─" * 57
    for line in (f"┌{rule}┐", f"│          𓄃 Nastech — {title:<35s}│", f"└{rule}┘"):
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Warn if a Nastech gateway has connected platforms (token conflicts, e.g. Telegram 409)."""
    ...
    if connected and _warn_running(
        auto_yes, "Nastech gateway is running with active connections: " + ", ".join(connected), [],
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: nastech claw <command> [options]\n\nCommands:\n"
              "  migrate          Migrate settings from OpenClaw to Nastech\n"
              "  cleanup          Archive leftover OpenClaw directories after migration\n\n"
              "Run 'nastech claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Nastech migration: preflight, preview, confirm, back up, apply."""
    ...
    opts.nastech_home = get_nastech_home()
    print()
    print_header("Migration Settings")
    _info(f"Source:      {opts.source_dir}", f"Target:      {opts.nastech_home}",
    ...
    if _confirm(opts.yes, "Proceed with migration?", default=True, declined="Migration cancelled.",
                non_tty=("Non-interactive session — preview only.",
                         "To execute, re-run with: nastech claw migrate --yes")):
        _apply_migration(run_migrator, opts)
    # Source directory is left untouched — archiving is `nastech claw cleanup`'s job.
...
def _load_migrator(script_path: Path, opts: SimpleNamespace) -> Optional[Callable[[bool], dict]]:
    ...
    return lambda execute: mod.Migrator(
        source_root=opts.source_dir.resolve(), target_root=opts.nastech_home.resolve(),
...
def _apply_migration(run_migrator: Callable[[bool], dict], opts: SimpleNamespace) -> None:
    ...
    restorable with `nastech import`: one restore point before any mutation, pruned to the last 5."""
    backup_archive: Optional[Path] = None
    if not opts.no_backup:
        try:
            from nastech_cli.backup import create_pre_migration_backup
            from nastech_cli.sizefmt import format_bytes as _format_size
            backup_archive = create_pre_migration_backup(nastech_home=opts.nastech_home)
            if backup_archive:
                print()
    ...
    try:
        ...
        if backup_archive:
            _info(f"A pre-migration backup is available at: {backup_archive}",
                  f"Restore with: nastech import {backup_archive.name}")
        return
...
def _cmd_cleanup(args):
    ...
    if running and not _warn_running(
        ...
        "Proceed anyway?",
        declined="Aborted. Stop OpenClaw first, then re-run: nastech claw cleanup",
        ...
        if dry_run:
            ...
            print_info(f"Would archive: {source_dir} → {archive_path}")
                      ...
                      non_tty=(f"Non-interactive session — would archive: {source_dir}",
                               "To execute, re-run with: nastech claw cleanup --yes")):
            try:
    ...
    if dry_run:
        ...
        if any(i.get("kind") == "provider-keys" and i.get("status") == "skipped" for i in items):
            ...
            _info("", "To migrate API keys, re-run with:",
                  "  nastech claw migrate --migrate-secrets", "", "Or add your key manually:",
                  "  nastech config set OPENROUTER_API_KEY sk-or-v1-...")
