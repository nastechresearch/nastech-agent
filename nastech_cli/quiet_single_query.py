"""Quiet ``nastech chat -Q`` helpers: bind this session's key and resume nested notifies.

Bot Mode delivers a local DM as ``nastech -p <bot> chat -Q --query-file``. Interactive
...
``NASTECH_SESSION_KEY`` and never woke the recipient. Quiet also printed and exited
...
# ``nastech kanban dispatch`` process never reaped the worker, so ``os.waitpid`` cannot tell it how
...
# contract as NASTECH_TURN_AUTHOR): nothing the turn spawns inherits it, and a nested one-shot
# never writes over its host's report — the record also carries the writer's pid.
TURN_REPORT_FILE_ENV = "NASTECH_QUIET_TURN_REPORT_FILE"
