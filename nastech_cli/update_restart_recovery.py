...
- **gateway profiles**, relaunched through the existing per-profile
  ``nastech_cli.main -p <profile> gateway restart`` command; and
- **``nastech-serve*`` systemd units**, restarted directly through
  ``systemctl``.  ``nastech serve`` is not a gateway profile and has no
  ...
  inventory, so a manually launched or Desktop-owned ``nastech serve`` — which
  has no relaunch authority — can never enter this path.

Serve-unit identity is always ``<scope>/<unit>`` (``user/nastech-serve``,
``system/nastech-serve``).  The two managers can each own a unit of the same
...
_RECOVERY_ENV = "NASTECH_UPDATE_RESTART_RECOVERY"
_GATEWAY_MARKERS = ("_NASTECH_GATEWAY", "NASTECH_GATEWAY", "NASTECH_GATEWAY_MODE")
_PROFILE_RESTART_TIMEOUT = 90
_VERIFY_TIMEOUT = 15
_PROFILE_ID_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,63}$")
_SUPERVISOR_RE = re.compile(r"^[a-z0-9][a-z0-9_-]{0,31}$")
_UNIT_RE = re.compile(r"^nastech-serve(-[a-z0-9][a-z0-9_-]{0,63})?\.service$")
_SERVE_UNIT_PATTERN = "nastech-serve*"
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    return (
        f"nastech-gateway-{profile}.service",
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    any Nastech platform helper — importing the freshly pulled tree is exactly
    what aborted the phase that called us.

    Each scope is returned with its label because ``nastech-serve.service`` in
    the user manager and ``nastech-serve.service`` in the system manager are
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    for line in (getattr(result, "stdout", "") or "").splitlines():
        ...
        if not parts:
            continue
        # The glob is a systemd pattern, not a name gate: `nastech-serve*` also
        # matches the unrelated `nastech-server.service`. Require the exact
...
def _normalized_skips(
    ...
    Qualified entries (``{"scope": "user", "unit": "nastech-serve"}`` or the
    equivalent ``"user/nastech-serve"`` string) suppress exactly one process.

    A bare ``"nastech-serve"`` carries no scope and therefore cannot say WHICH
...
def restart_serve_units(
    *,
    ...
    """Restart every active ``nastech-serve*`` systemd unit from this process.

    ``nastech serve`` hosts ``tui_gateway.server`` and is restarted by the
    ...
    part-way — systemd lists ``nastech-gateway.service`` before
    ``nastech-serve.service``, so the gateway is typically already done — the
    ...
    or Desktop-owned ``nastech serve`` owns no unit and therefore cannot be
    touched here.

    Every identity in and out of this function is scope-qualified. The user
    manager and the system manager can each own a ``nastech-serve.service``,
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # shape carries the systemd scope, because `nastech-serve.service`
