...
def _launch_profile() -> str:
    """Profile this recovery process was launched as: ``<root>/profiles/<name>`` in ``NASTECH_HOME`` names it,
    anything else is the default profile. Mirrors ``nastech_cli.profiles.profile_root_for_env_home`` without
    importing it — this module stays stdlib-only at import time."""
    home = os.environ.get("NASTECH_HOME", "").strip()
    if home:
...
def _run_profile_restart(profile: str, *, run: Callable[..., Any]) -> bool:
    ...
    argv = [sys.executable, "-m", "nastech_cli.main", "-p", profile, "gateway", "restart"]
...
def _systemd_unit_candidates(profile: str) -> tuple[str, ...]:
    ...
    if profile == "default":
        return ("nastech-gateway.service", "gateway.service", "gateway-default.service")
    return (f"nastech-gateway-{profile}.service", f"gateway-{profile}.service")
...
def _systemctl_scopes() -> list[tuple[str, list[str]]]:
    ...
    ``systemctl`` comes from ``shutil.which`` so this module never imports a Nastech platform helper —
...
def _listed_serve_units(scope: list[str], *, run: Callable[..., Any]) -> list[str]:
    ...
    if result is None:
        return []
    # The glob is a systemd pattern, not a name gate (`nastech-serve*` also matches
    # `nastech-server.service`): require the exact base unit or the profile family.
    units: list[str] = []
...
def _normalized_skips(skip_units: Iterable[Any]) -> tuple[set[tuple[str, str]], set[str]]:
    """Split already-settled units into scope-qualified and legacy entries (a bare ``"nastech-serve"``
...
def restart_serve_units(
    *, skip_units: Iterable[Any] = (), run: Callable[..., Any] = subprocess.run, sleep: Callable[[float], Any] = time.sleep
    ...
    Desktop-owned ``nastech serve`` (no unit) structurally cannot be touched here.
    """
...
def _parse_payload(stream) -> tuple[list[str], dict[str, str], bool, list[str]]:
    ...
    if raw_serve is not None:
        ...
        for entry in raw_skip:
            ...
            # scope (`nastech-serve.service` can exist in BOTH managers; settling one says nothing about
