...
Mixin split out of ``cli.py``; bound onto ``NastechCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``NastechCLI.<name>(self, ...)``.
"""
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from nastech_cli.model_switch import format_model_for_display, resolve_display_context_length
    ...
    # Provider-aware context chain: Codex OAuth / Copilot / Nastech caps win over the raw
...
def _commit_model_switch(
    ...
    from cli import NastechCLI, _cprint
    old_model = cli.model
    ...
    if persist_global:
        from nastech_cli.model_switch import persist_model_selection
    ...
    if not one_turn:
        NastechCLI._persist_model_switch_to_session(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from nastech_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _restore_session_model(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        from nastech_cli.local_runtime.endpoint import LLAMACPP_ALIASES
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `nastech model` / gateway pickers); live catalog only when
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from nastech_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
