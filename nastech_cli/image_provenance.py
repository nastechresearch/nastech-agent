"""Image-authored deployment provenance for immutable Nastech runtimes.

The published image bakes ``/etc/nastech/image-provenance.json`` outside both
``$NASTECH_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/nastech/image-provenance.json")
