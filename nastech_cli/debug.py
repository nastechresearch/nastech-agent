...
# ``nastech debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``nastech debug delete`` manually
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``nastech dump`` is run internally.
    """
    buf = io.StringIO()
...
# Nastech-S3 envelope format id; the discord-support viewer keys off it.
...
def build_debug_share(
    ...
    Shared by ``nastech debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
...
def run_debug_share(args):
    ...
    if getattr(args, "nastech", False):
        return _run_debug_share_nastech(args, log_lines=log_lines, redact=redact)
...
def _run_debug_share_nastech(args, *, log_lines: int, redact: bool) -> None:
    """``nastech debug share --nastech``: gzip the same bundle into the Nastech envelope → Nastech-S3."""
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("nastech debug share --nastech: applied force-mode redaction before upload")
    print("Uploading to Nastech diagnostics storage...")
    try:
        res = share_to_nastech(build_nastech_bundle(bundle, redact=redact))
    except Exception as exc:
        print(f"\nNastech upload failed: {exc}\n"
              "\nThe Nastech diagnostics service may be unavailable or not yet provisioned.\n"
              "Run `nastech debug share --local` to print the report instead, "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: nastech debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'nastech debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
...
Options (share):
  ...
  --nastech       Upload to Nastech-internal storage (private, staff-only,
