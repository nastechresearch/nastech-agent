...
Currently supports:
    nastech debug share    Upload debug report (system info + logs) to a
                          ...
                          to disable.
                          Pass ``--nastech`` to upload instead to Nastech-internal
                          storage (AWS S3) via a signed URL minted by the
                          Nastech account service: the bundle is private
                          (viewable only by Nastech staff / allowlisted mods via
...
# desktop app talks to a remote/docker/SSH backend, `nastech debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Nastech Desktop on the machine running the app, not by this "
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and Nastech-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the Nastech-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_NASTECH_BUNDLE_FORMAT = "nastech-debug-share/1"
...
def collect_share_bundle(
    ...
    Nastech-S3 path (``--nastech``).  Centralising it guarantees the Nastech bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the Nastech path must never see raw
    logs.
...
def build_nastech_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the Nastech envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "nastech-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    # shared collector so the paste.rs and Nastech-S3 paths build identical,
...
def run_debug_share(args):
    ...
    nastech = getattr(args, "nastech", False)
    ...
    if nastech:
        _run_debug_share_nastech(args, log_lines=log_lines, redact=redact)
        return
...
_NASTECH_PRIVACY_NOTICE = """\
⚠️  --nastech: This uploads your debug bundle to Nastech-INTERNAL storage (AWS S3),
  ...
  • The bundle is viewable only by Nastech staff (and allowlisted Discord mods)
...
def _run_debug_share_nastech(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``nastech debug share --nastech``: upload the bundle to Nastech-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the Nastech envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from nastech_cli.diagnostics_upload import share_to_nastech

    print(_NASTECH_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "nastech debug share --nastech: applied force-mode redaction before upload"
        )
    blob = build_nastech_bundle(bundle, redact=redact)

    print("Uploading to Nastech diagnostics storage...")
    try:
        res = share_to_nastech(blob)
    except Exception as exc:
        print(
            f"\nNastech upload failed: {exc}\n"
            "\nThe Nastech diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `nastech debug share --local` to print the report instead, "
    ...
    print("\nDebug bundle uploaded to Nastech (private):")
    if view_url:
...
def run_debug(args):
    ...
    if subcmd == "share":
        ...
        print("  --nastech       Upload to Nastech-internal storage (private, staff-only,")
