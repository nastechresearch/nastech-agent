...
def _is_nastech_gateway_run_argv(command: Sequence[str]) -> bool:
    ...
    The wrapper is generic. Only historical/current Nastech gateway shapes
...
def _prepare_child_command(
    ...
    ``--external-supervisor`` on the inner ``gateway run`` so ``nastech update``
    ...
    if not _is_launchd_supervised(environ):
        return argv
    if not _is_nastech_gateway_run_argv(argv):
        return argv
