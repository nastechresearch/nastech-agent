...
``nastech update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/nastech/image-provenance.json``,
   written by the image build — see :mod:`nastech_cli.image_provenance`):
