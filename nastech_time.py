...
``NASTECH_TIMEZONE`` env var, then ``timezone`` in ``~/.nastech/config.yaml``, else server-local
...
# NASTECH_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``NASTECH_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("NASTECH_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without nastech_cli importable.
        try:
